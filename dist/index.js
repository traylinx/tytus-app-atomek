 ...
 */
const br = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), vr = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), Sn = (e) => {
  const t = vr(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, Qn = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), yr = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const xr = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Nr = te("check", xr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const zr = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], Dt = te("chevron-down", zr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const as = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function rs() {
  const e = wn();
...
async function ss() {
  ...
  if (!e) return da(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((r) => ca(r, r.name, "local-file")));
  return gn(n.filter(Boolean));
}
async function is() {
  ...
  return await la(t, t.name, n, 0, 320), { name: t.name, handle: t, files: gn(n) };
}
function wn() {
  const e = window;
...
async function la(e, t, n, r, s) {
  if (!(n.length >= s || r > 8))
    for await (const i of e.values()) {
      if (n.length >= s) break;
      if (i.kind === "directory") {
        if (as.has(i.name)) continue;
        await la(i, `${t}/${i.name}`, n, r + 1, s);
        continue;
      }
      const o = i, c = await ca(o, `${t}/${o.name}`, "local-folder");
      c && n.push(c);
    }
}
function da(e) {
...
const nt = "atomek:default", cs = 3e3, kn = "tytus.atomek.threadTitleOverrides", Mn = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, vt = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, ls = (e, t = cs) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Vn = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, ds = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Dn = (e) => ({
  id: e.id,
  title: e.title,
  kind: ds(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = Pn();
    n[e] = t, localStorage.setItem(kn, JSON.stringify(n));
  } catch {
  }
}, fs = (e) => {
  try {
    const t = Pn();
    delete t[e], localStorage.setItem(kn, JSON.stringify(t));
  } catch {
  }
}, Fn = (e) => {
  const t = Pn();
...
function ms({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const s = e.ai, [i, o] = N(null), [c, m] = N([]), [b, p] = N([]), [h, w] = N([]), [T, S] = N([]), [J, _] = N(Mn), [R, se] = N(!1), ce = Ge(!0), C = Ge(null), E = P(async () => {
    if (!s) {
      _(Mn);
      return;
    }
    ...
    if (s)
      try {
        const y = await s.listArtifacts({ threadId: x });
        if (!ce.current) return;
        w(y.map(Dn));
      } catch (y) {
        r(`AI artifacts unavailable: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [s, r]), M = P(async (x) => {
    if (s)
      try {
        const y = await s.listMessages(x);
        if (!ce.current) return;
        const V = c.find((q) => q.id === x) ?? null;
        V && o(V), p(y.map(vt).filter(Boolean)), S([]), await K(x);
      } catch (y) {
        r(`Load chat failed: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [s, K, r, c]), ne = P(async () => {
    if (s)
      try {
        const x = Fn(await s.listThreads({ workspaceKey: nt, status: "active" })), y = x[0] ?? await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
        if (!ce.current) return;
        m(x[0] ? x : [y]), o(y);
        const V = await s.listMessages(y.id);
        if (!ce.current) return;
        p(V.map(vt).filter(Boolean)), await K(y.id);
      } catch (x) {
        r(`AI chat unavailable: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [s, K, r]);
  le(() => (ce.current = !0, E(), ne(), () => {
    ce.current = !1;
  }), [ne, E]);
  const I = P(async () => {
    if (!s) return null;
    if (i) return i;
    const x = await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
    return ce.current && (o(x), m((y) => [x, ...y.filter((V) => V.id !== x.id)])), x;
  }, [s, i]), Me = P(async () => {
    if (!s) return;
    const x = await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
    o(x), m((y) => [x, ...y.filter((V) => V.id !== x.id)]), p([]), w([]), S([]), r("New AI chat created");
  }, [s, r]), A = P(async (x, y) => {
    if (!s) return;
    const V = y.trim();
    if (V)
      try {
        const q = s.updateThread, oe = c.find((he) => he.id === x) ?? i, O = typeof q == "function" ? await q({ threadId: x, title: V }) : { ...oe ?? await I(), id: x, title: V, updatedAt: Date.now() };
        typeof q != "function" && hs(x, V), m((he) => he.map((Te) => Te.id === O.id ? O : Te)), o((he) => he?.id === O.id ? O : he), r(typeof q == "function" ? `Renamed chat: ${O.title}` : `Renamed chat locally: ${O.title}`);
      } catch (q) {
        r(`Rename chat failed: ${q instanceof Error ? q.message : String(q)}`);
      }
  }, [s, I, r, i, c]), de = P(async (x) => {
    if (s)
      try {
        await s.deleteThread(x), fs(x);
        const y = c.filter((V) => V.id !== x);
        if (m(y), i?.id === x) {
          const V = y[0] ?? await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
          o(V), m((oe) => oe.some((O) => O.id === V.id) ? oe : [V, ...oe]);
          const q = await s.listMessages(V.id);
          p(q.map(vt).filter(Boolean)), await K(V.id);
        }
        r("Deleted AI chat");
      } catch (y) {
        r(`Delete chat failed: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [s, K, r, i?.id, c]), be = P(async (x) => {
    if (!s) return null;
    try {
      const y = await I();
      if (!y) return null;
      const V = await s.createArtifact({
        threadId: y.id,
        messageId: x.messageId ?? null,
        title: x.title?.trim() || Vn(x.body, "Atomek artifact"),
        kind: x.kind ?? "markdown",
        body: x.body
      }), q = Dn(V);
      return w((oe) => [q, ...oe.filter((O) => O.id !== q.id)]), r(`Saved AI artifact: ${q.title}`), q;
    } catch (y) {
      return r(`Save artifact failed: ${y instanceof Error ? y.message : String(y)}`), null;
    }
  }, [s, I, r]), xe = P(async (x) => {
    if (s)
      try {
        await s.deleteArtifact(x), w((y) => y.filter((V) => V.id !== x)), r("Deleted AI artifact");
      } catch (y) {
        r(`Delete artifact failed: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [s, r]), Q = P(async (x) => {
    if (!s) return null;
    try {
      const y = await s.writeMemory({
        title: x.title?.trim() || Vn(x.body, "Atomek memory"),
        body: x.body,
        metadata: {
          source: "atomek",
          messageId: x.messageId ?? null
      ...
      return S((V) => [y, ...V.filter((q) => q.id !== y.id)].slice(0, 5)), r(`Remembered: ${y.title}`), y;
    } catch (y) {
      return r(`Remember failed: ${y instanceof Error ? y.message : String(y)}`), null;
    }
  }, [s, r]), ve = P(async (x) => {
    if (!s) return [];
    const y = await s.searchMemory({ query: x, limit: 5 });
    return ce.current && S(y), y;
  }, [s]), Ve = P(async (x, y = {}) => {
    const V = x.trim();
    if (!V || !s) return null;
    se(!0);
    ...
    try {
      const O = await I();
      if (!O) return null;
      const he = await ve(V).catch(() => []), Te = us(he), Je = y.requestContext ?? t, Ae = Te ? [...Je, Te] : [...Je];
      let je = null;
      for await (const $ of s.sendMessage({
        threadId: O.id,
        body: V,
        ...
        if ($.type === "message_created") {
          const ae = vt($.message);
          if (!ae) continue;
          ae.role === "assistant" && (je = ae.id), p((G) => [...G.filter((re) => re.id !== ae.id), ae]);
        }
        if ($.type === "token" && (je = $.messageId, p((ae) => ae.some((G) => G.id === $.messageId) ? ae.map(
          (G) => G.id === $.messageId ? { ...G, body: $.body, status: "streaming" } : G
        ) : [...ae, {
          id: $.messageId,
          role: "assistant",
          body: $.body,
          status: "streaming",
          createdAt: Date.now()
        }])), $.type === "message_updated" || $.type === "done") {
          const ae = vt($.message);
          if (!ae) continue;
          ae.role === "assistant" && (q = ae), p((G) => G.map((re) => re.id === ae.id ? ae : re)), ae.gatewayLabel && r(`AI answered via ${ae.gatewayLabel}`);
        }
        if ($.type === "run_failed") {
          const ae = oe.signal.aborted;
          r(ae ? "AI response stopped" : `AI failed: ${$.error}`), je && p((G) => G.map((re) => re.id !== je ? re : ae ? { ...re, status: "complete", error: void 0, body: re.body || "Stopped by user." } : { ...re, status: "error", error: $.error, body: $.error }));
        }
      }
      const ue = Fn(await s.listThreads({ workspaceKey: nt, status: "active" }).catch(() => []));
      return ce.current && ue.length > 0 && m(ue), E(), q;
    } catch (O) {
      return O instanceof DOMException && O.name === "AbortError" ? (r("AI response stopped"), q) : (r(`AI failed: ${O instanceof Error ? O.message : String(O)}`), null);
    } finally {
      C.current === oe && (C.current = null), se(!1);
    }
  }, [s, n.gatewayPreference, n.model, I, ve, E, t, r]), k = P(() => {
    C.current?.abort(), r("Stopping AI response…");
  }, [r]);
  return {
    aiStatus: J,
    artifacts: h,
...
const ps = (e) => `tytus-workbench:///${encodeURI(e.path)}`, bs = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, vs = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: s }) => {
...
}, ys = "active-file", ws = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", gs = (e, t = "file") => `${t}:${e.id}`, ks = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let s = [];
  return t.scope === "active-selection" ? r?.selection ? s = [Ps(r, r.selection, !0)] : r && (s = [Ot(r, !0, "Active file")]) : t.scope === "active-file" ? r && (s = [Ot(r, !0, "Active file")]) : t.scope === "open-editors" ? s = e.openDocumentIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ot(i, !0, "Open editor")) : t.scope === "selected-files" && (s = t.selectedFileIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ot(i, !1, "Selected file"))), Ns(s).filter((i) => !n.has(i.id));
}, Ns = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, zs = 4e3, Xs = 8e3, Ts = 4e3, Ls = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, As = (e, t, n) => {
  const r = xs(e, n), s = [], i = new Map(t.map((o) => [o.id, o]));
  for (const o of r) {
    if (!o.fileId) continue;
    const c = i.get(o.fileId), m = e.byId.get(o.fileId);
    ...
    const b = [
      ...
      o.range ? `Range: ${In(o.range)}` : null
    ].filter(Boolean).join(`
`), p = o.kind === "selection" && o.range ? Os(c.content, o.range) : c.content, h = o.kind === "selection" ? zs : o.label.startsWith("Open editor") ? Ts : Xs;
    s.push({
...
`));
}, In = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, qn = 75e4, js = 2800, Hs = 240, Cs = 80, Ss = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Ue(t.content)}`).sort().join("")
), ln = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? qn) || e.content.length > (t.maxFileBytes ?? qn) ? "huge" : !t.includeDirty && e.dirty ? null : qs(e.path) ? "vendor" : Ys(e) ? "binary" : null;
}, Yn = (e, t = {}) => {
  const n = Date.now(), r = [], s = [], i = [];
  for (const o of e) {
    const c = o.size ?? o.content.length, m = ln(o, t);
    if (m) {
      s.push({ fileId: o.id, name: o.name, path: o.path, language: o.language, reason: m, size: c });
      continue;
    }
    const b = Ue(o.content), p = Ms(o, b, n, t);
    i.push(...p), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
...
}, Ms = (e, t = Ue(e.content), n = Date.now(), r = {}) => {
  const s = Math.max(500, r.maxChunkChars ?? js), i = Math.min(Math.max(0, r.chunkOverlapChars ?? Hs), Math.floor(s / 2)), o = Math.max(1, r.maxChunksPerFile ?? Cs), c = Fs(e.content), m = [];
  let b = 0, p = 0;
  for (; b < e.content.length && m.length < o; ) {
    const h = Ds(e.content, b, Math.min(e.content.length, b + s)), w = e.content.slice(b, h).trim();
    if (w.length > 0) {
      const T = Is(c, b, h);
      m.push({
        id: `${e.id}:${t}:${p}:${T.startLineNumber}-${T.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const r = Rn(e, t), s = Rn(e, Math.max(t, n - 1));
  return {
    startLineNumber: r + 1,
    startColumn: t - e[r] + 1,
    endLineNumber: s + 1,
  ...
  return 0;
}, qs = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => Ss.has(t)), Ys = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && Ws.has(t) || e.content.includes("\0")) return !0;
...
class Rs {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Yn(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Yn(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return Es(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const at = (e = [], t = {}) => new Rs(e, t), Es = (e, t, n = {}) => {
  ...
  for (const c of e.files) {
    ...
    if (ln(m, n)) {
      i.push({ fileId: c.fileId, path: m.path, status: "skipped-now", indexedHash: c.hash, currentHash: Ue(m.content) });
      continue;
    }
    const p = Ue(m.content);
    p !== c.hash ? i.push({ fileId: c.fileId, path: m.path, status: "changed", indexedHash: c.hash, currentHash: p }) : m.dirty !== c.dirty && i.push({ fileId: c.fileId, path: m.path, status: "dirty-state-changed", indexedHash: c.hash, currentHash: p });
  }
  for (const c of t)
    !s.has(c.id) && !ln(c, n) && i.push({ fileId: c.id, path: c.path, status: "new", currentHash: Ue(c.content) });
  const o = e.signature !== fa(t);
  return { stale: i.length > 0, signatureChanged: o, files: i };
}, Js = 8, Zs = 12e3, ma = (e, t, n = {}, r) => {
  const s = Us(t);
  if (s.length === 0) return [];
  const i = new Set(r?.files.map((p) => p.fileId) ?? []), o = e.chunks.filter((p) => n.includeDirty !== !1 || !p.dirty).map((p) => Gs(p, s)).filter((p) => p.score >= (n.minScore ?? 1)).sort((p, h) => h.score - p.score || p.chunk.path.localeCompare(h.chunk.path) || p.chunk.ordinal - h.chunk.ordinal), c = [], m = /* @__PURE__ */ new Set();
  let b = n.maxChars ?? Zs;
  for (const p of o) {
    if (c.length >= (n.limit ?? Js) || b <= 0) break;
    const h = `${p.chunk.fileId}:${p.chunk.ordinal}`;
    if (m.has(h)) continue;
    m.add(h);
    const w = p.chunk.text.length > b ? `${p.chunk.text.slice(0, Math.max(0, b - 28))}
[...context clipped...]` : p.chunk.text;
    c.push(Ks(p, w, i.has(p.chunk.fileId))), b -= w.length;
  }
  return c;
}, Ks = (e, t = e.chunk.text, n = !1) => {
...
`), Us = (e) => Array.from(new Set(
  ...
  const c = [];
  for (const m of t) {
    const b = en(n, m), p = en(r, m), h = en(s, m), w = i === m ? 1 : 0;
    b + p + h + w !== 0 && (c.push(m), o += Math.min(b, 12), o += p * 4, o += h * 6, o += w * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (o += 3));
  }
  ...
  const n = e.toLowerCase(), r = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, b) => m - b)[0] ?? 0, s = Math.max(0, r - 120), i = Math.min(e.length, r + 260), o = s > 0 ? "…" : "", c = i < e.length ? "…" : "";
  return `${o}${e.slice(s, i).replace(/\s+/g, " ").trim()}${c}`;
}, $s = (e, t = {}) => {
  const { autoRefresh: n = !1, ...r } = t, s = Ge(e), i = Ge(null);
  i.current || (i.current = at(e, r));
  const [o, c] = N(() => i.current?.getSnapshot() ?? at(e, r).getSnapshot());
  s.current = e;
  const m = P((w = s.current) => {
    const T = (i.current ?? at([], r)).refresh(w, r);
    return c(T), T;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), b = P((w = s.current) => {
    const T = (i.current ?? at([], r)).update(w, r);
    return c(T), T;
  }, [m]);
  le(() => {
    n && m(e);
  }, [n, e, m]);
  const p = (i.current ?? at([], r)).staleReport(e), h = P((w, T = {}) => {
    const S = i.current ?? at([], r);
    return ma(S.getSnapshot(), w, T, S.staleReport(s.current));
  }, []);
  return { snapshot: o, staleReport: p, isStale: p.stale, refresh: m, update: b, retrieve: h };
}, _s = "tytus.atomek.semanticVector:v1", pa = "__gateway_default__", ba = (e) => e?.trim() || pa, ei = (e) => {
  const t = ba(e);
  ...
  if (!ya()) return null;
  try {
    const r = va(e, t, n), s = window.localStorage.getItem(r);
    if (!s) return null;
  ...
  if (!ya()) return null;
  try {
    const t = va(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, yt = 8, ai = 12e3, ri = 180, si = async (e, t, n, r, s = {}, i, o) => {
  const c = ma(t, n, { ...s, limit: Math.max(s.limit ?? yt, 16) }, i), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: c.slice(0, s.limit ?? yt), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const b = ba(r.embeddingModel), p = await m.embedText({
      input: n,
    ...
    const w = new Map(c.map((C) => [C.chunkId, C])), T = hi(n), S = /* @__PURE__ */ new Map();
    t.chunks.filter((C) => s.includeDirty !== !1 || !C.dirty).slice(0, ri).forEach((C) => S.set(C.id, C)), c.forEach((C) => {
      const E = t.byChunkId.get(C.chunkId);
      E && S.set(E.id, E);
    });
    const J = Array.from(S.values()), _ = [];
    let R = 0;
    for (const C of J) {
      o?.aborted;
      const E = ti(e.appId, b, C) ?? await oi(m, e.appId, b, C, r, o);
      if (!E) continue;
      E.updatedAt > Date.now() - 1e3 && (R += 1);
      const K = di(h, E.vector);
      if (!Number.isFinite(K)) continue;
      const ne = w.get(C.id)?.score ?? fi(C, T), I = ui(ne) * 0.42 + Math.max(0, K) * 0.58;
      _.push({ chunk: C, keywordScore: ne, vectorScore: K, score: I, matchedTerms: T.filter((Me) => C.text.toLowerCase().includes(Me)) });
    }
    if (_.length === 0) return { hits: c.slice(0, s.limit ?? yt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: R };
    const se = new Set(i?.files.map((C) => C.fileId) ?? []), ce = _.sort((C, E) => E.score - C.score || E.keywordScore - C.keywordScore || C.chunk.path.localeCompare(E.chunk.path) || C.chunk.ordinal - E.chunk.ordinal).slice(0, Math.max(s.limit ?? yt, 1)).map((C) => ci(C, se.has(C.chunk.fileId)));
    return {
      hits: li(ce, s.maxChars ?? ai),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${ei(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: R
    };
  } catch (b) {
    return {
      hits: c.slice(0, s.limit ?? yt),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: mi(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, ui = (e) => Math.min(1, Math.max(0, e / 24)), hi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), fi = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, pi = /```([^\n`]*)\n([\s\S]*?)```/g, bi = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function xn(e) {
  return Array.from(e.matchAll(pi)).map((t) => ga(t[1] ?? "", ki(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function ga(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, s = [];
  for (const i of n) {
    const o = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? r[o[1].toLowerCase()] = xi(o[2]) : s.push(i.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: s, attrs: r, content: t };
}
function Le(e, t) {
  return (typeof e == "string" ? ga(e, "").flags : e.flags).includes(t.toLowerCase());
}
function ka(e) {
  return [...xn(e).filter((r) => Le(r, "diff") || Le(r, "patch")).map((r) => r.content), e].filter((r, s, i) => vi(r) && i.findIndex((o) => o === r) === s);
}
function vi(e) {
...
`), r = [];
  ...
  }), r.length === 0) return [{ raw: t, ...tn(t) }];
  const s = Array.from(new Set(r)).sort((o, c) => o - c), i = [];
  for (let o = 0; o < s.length; o += 1) {
    const c = s[o], b = s.find((h) => h > c && n[h].startsWith("diff --git ")) ?? n.length, p = n.slice(c, b).join(`
`);
    /^@@\s+-\d+/m.test(p) && i.push({ raw: p, ...tn(p) });
  }
  return i.length > 0 ? i : [{ raw: t, ...tn(t) }];
}
function tn(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), r = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), s = Ie(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = Ie(t?.[3] || t?.[4] || r?.[1] || r?.[2] || "") || null, o = i && i !== "/dev/null" ? i : s;
...
function dn(e) {
  return xn(e).filter((t) => wi(t)).map((t) => ({
...
`), s = [];
  ...
  for (let c = 0; c < r.length; c += 1) {
    const m = r[c].match(bi);
    if (!m) continue;
    o = !0;
    const b = Number(m[1]), p = Math.max(0, b - 1);
    if (p < i) return null;
    for (s.push(...n.slice(i, p)), i = p, c += 1; c < r.length; c += 1) {
      const h = r[c];
      if (h.startsWith("@@ ")) {
        c -= 1;
        break;
...
function Ni(e) {
  ...
  for (const p of ka(t))
    for (const h of yi(p)) {
      if (!h.path) {
        if (s) {
          const S = un(s.content, h.raw);
          S && S !== s.content && o.set(s.id, jt({
            file: s,
            proposedContent: S,
            sourceTitle: r,
      ...
      const w = En(n, h.path);
      if (!w) {
        c.push(`${h.path}: no opened file`);
        continue;
      }
      const T = un(w.file.content, h.raw);
      if (!T || T === w.file.content) {
        c.push(`${h.path}: patch did not match or produced no change`);
        continue;
      }
      o.set(w.file.id, jt({
        file: w.file,
  ...
  for (const p of dn(t)) {
    const h = p.path ? En(n, p.path) : s ? { file: s, normalizedPatchPath: Ie(s.path), confidence: "exact" } : null;
    if (!h) {
      c.push(`${p.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (p.content === h.file.content) {
      c.push(`${h.file.path}: replacement produced no change`);
      continue;
    }
    o.set(h.file.id, jt({
      file: h.file,
      ...
      versions: i,
      targetPath: p.path ?? h.file.path,
      match: h
    }));
  }
  const m = dn(t).some((p) => p.path);
  if (o.size === 0 && s && !m) {
    const p = zi(t, s);
    p && p.content !== s.content && o.set(s.id, jt({
      file: s,
      proposedContent: p.content,
      sourceTitle: r,
      extractionLabel: p.label,
...
`), s = Math.max(n.length, r.length);
  let i = 0, o = 0, c = 0;
  for (let m = 0; m < s; m += 1)
    n[m] !== r[m] && (n[m] === void 0 ? i += 1 : r[m] === void 0 ? o += 1 : c += 1);
...
function jt(e) {
  ...
  return {
    ...
    stats: Xi(e.file.content, e.proposedContent),
    base: {
...
const It = (e) => typeof e == "object" && e !== null, Oi = (e) => It(e.ai) ? e.ai : null, ji = (e) => {
  if (!It(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, Hi = (e) => {
  const t = ji(e);
  return !t || !It(e) ? null : { ...e, id: t };
}, Ce = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Ce) : It(e) ? Object.values(e).some(Ce) : !1, Ci = async (e, t) => {
  const n = Oi(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(Hi).filter((s) => !!s) : [];
}, Si = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Ce(e.capability) || Ce(e.capabilities) || Ce(e.modality) || Ce(e.modalities) || Ce(e.task) || Ce(e.tasks) || Ce(e.type) || Ce(e.kind), xa = (e) => typeof e == "object" && e !== null, Wi = (e) => xa(e.ai) ? e.ai : null, Na = (e) => typeof Wi(e)?.embedText == "function", Mi = async (e, t) => Na(e) ? (await Ci(e, t)).filter(Si) : [], Vi = (e) => xa(e.ai) ? Na(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", Di = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], Fi = [
...
function Ri(e) {
  const t = e.results.length > 0 ? e.results.map((r, s) => [
    ...
    `Status: ${r.status}`,
    "Output:",
    Qi(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function Ji(e) {
  ...
  for (const r of t) {
    const s = Zi(r.content);
    if (!s?.scripts) continue;
    const i = Xa(r.path), o = Ki(e, i, s);
    if (!o) continue;
    const c = Object.keys(s.scripts).filter((b) => typeof s.scripts?.[b] == "string"), m = Bi(c);
    for (const b of m) {
      const p = `${o} run ${b}`;
      n.push({
        id: Ta(`${i}:${p}`),
...
function Ki(e, t, n) {
  ...
  for (const [r, s] of Fi)
    if (e.some((i) => Gi(i.path) === r && Xa(i.path) === t)) return s;
  return null;
}
function Bi(e) {
  const t = Di.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, s) => r.localeCompare(s));
  return [...t, ...n];
}
function Ui(e) {
  const t = /* @__PURE__ */ new Set();
...
function no({ className: e }) {
  return /* @__PURE__ */ a("img", { src: to, alt: "ATOMEK", className: e, draggable: !1 });
}
const ao = mr(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), ro = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Aa = "tytus.workspace.recent", Oa = "tytus.workspace.layout", ja = "tytus.atomek.chatAiSettings", Ha = "tytus.atomek.currentMission", hn = "tytus.atomek.currentMissionChanged", Wt = {
...
function nn(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - so), r = n < 1180, s = r ? 200 : 240, i = r ? 300 : 340, o = r ? 420 : 560, c = Math.max(s, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, c), b = Math.floor(n * (r ? 0.34 : 0.36)), p = n - m - o, h = Math.max(i, Math.min(r ? 500 : 640, b, p));
  return { primaryMin: s, primaryMax: c, secondaryMin: i, secondaryMax: h };
}
function io(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let r = 0, s = 0, i;
...
function lo(e) {
  const t = e?.resources ?? [], n = (c) => c.status === "ready" || c.status === "available", r = t.filter((c) => c.kind === "pod-agent" && ct(c) === "openclaw"), s = t.filter((c) => c.kind === "pod-agent" && ct(c) === "hermes"), i = t.filter((c) => c.kind === "local-cli" && n(c)), o = t.filter((c) => c.kind === "shared-folder" && n(c));
  return [
...
function wt(e, t, n, r, s) {
  return {
    ...
    resourceLabel: r ? Pe(r) : s,
    status: r?.status ?? "needs-setup",
    trustTier: r?.trustTier ?? "not-available",
    sandbox: r?.sandbox ?? "none"
  };
}
function lt(e, t = "repo-repair") {
  const n = Pt.find((I) => I.id === t) ?? Pt[0], r = e?.resources ?? [], s = Be(r, (I) => I.kind === "pod-agent" && ct(I) === "openclaw"), i = Be(r, (I) => I.kind === "pod-agent" && ct(I) === "hermes"), o = Be(r, (I) => I.kind === "ail-route"), c = Be(r, (I) => I.kind === "shared-folder"), m = Fe(r, "local-cli", ["claude"]), b = Fe(r, "local-cli", ["opencode", "open code"]), p = Fe(r, "local-cli", ["codex"]), h = Fe(r, "local-cli", ["pi"]), w = Fe(r, "local-cli", ["kimi"]), T = Be(r, (I) => I.kind === "local-cli"), S = Fe(r, "app-skill", ["juli3ta", "music", "song"]), J = Fe(r, "app-skill", ["blender", "3d"]), _ = Fe(r, "app-skill", ["remotion", "video", "render"]), R = Be(r, (I) => I.kind === "app-skill"), se = t === "pod-local" ? s ?? i ?? o ?? m ?? T : t === "creative-production" ? i ?? o ?? m ?? T : t === "research-watch" ? s ?? o ?? i ?? m ?? T : m ?? b ?? o ?? T, ce = t === "creative-production" ? S ?? J ?? _ ?? R ?? b ?? m ?? T : b ?? m ?? p ?? T, C = t === "pod-local" ? p ?? h ?? w ?? s ?? m ?? T : t === "research-watch" ? h ?? w ?? p ?? s ?? T : p ?? h ?? s ?? w ?? T, E = t === "creative-production" ? S ?? J ?? _ ?? R : t === "research-watch" ? Fe(r, "app-skill", ["browser", "channel", "telegram"]) ?? R : R, K = [
    wt("planner", "Planner", "Break goal into tasks, risks, and required context.", se, "No planner agent ready"),
    wt("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", ce, "No local/app implementer ready"),
    wt("reviewer", "Reviewer", "Independent critique before approval or handoff.", C, "No reviewer agent ready"),
    wt("team-desk", "Team Desk", "Shared mission folder for transcripts, outputs, proposals, and handoff.", c, "Mission folder only until shared folder is bound")
  ];
  (t === "creative-production" || E) && K.push(wt("app-tool", "App Tool", "Drive installed local app skill through mission context.", E, "No configured app skill"));
  const M = K.filter((I) => I.status === "ready" || I.status === "available" || I.status === "degraded").length, ne = M >= K.length - 1 ? "ready" : M >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: ne, assignments: K };
}
function Qe(e, t, n) {
  if (n && Pt.some((s) => s.id === n)) return n;
  const r = e.toLowerCase();
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(r) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(r) ? "research-watch" : (t?.resources ?? []).some((s) => s.kind === "pod-agent" && Sa(s)) ? "pod-local" : "repo-repair";
}
function fo(e) {
  return Pt.map((t) => lt(e, t.id));
}
function St(e) {
...
function kt(e) {
  ...
  try {
    localStorage.setItem(Ha, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(hn, { detail: t }));
  } catch {
  }
}
function po() {
  try {
    ...
    return !t.missionId || !t.title ? null : {
      ...
      teamPresetId: Pt.some((n) => n.id === t.teamPresetId) ? t.teamPresetId : void 0
    };
  } catch {
    return null;
  }
...
function xt(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", s = lt(t, Qe(r, t, n)), i = (h) => s.assignments.find((w) => w.role === h), o = i("planner"), c = i("implementer"), m = i("reviewer"), b = i("team-desk"), p = i("app-tool");
  return [
    {
      id: "task-scope",
    ...
    },
    {
      ...
      resourceHint: c ? St(c) : "Local/app implementer",
      role: "implementer",
    ...
    } : null,
    {
      ...
      resourceHint: m ? `${St(m)} · ${b?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
...
function Mt(e, t, n, r, s, i) {
  const o = lt(t, Qe(s || e.goal, t, i ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
...
function Va(e, t, n, r) {
  const s = Qe(n || e.goal, t, r ?? e.teamPresetId), i = lt(t, s), o = xt(n || e.goal, t, s);
...
function vo({ host: e }) {
  const t = Ge(null), [n, r] = N(0), s = pe(() => $o(), []), [i, o] = N("computer"), [c, m] = N(s.primaryVisible), [b, p] = N(s.primaryWidth), [h, w] = N("chat"), [T, S] = N(s.secondaryVisible), [J, _] = N(s.secondaryWidth), [R, se] = N(!1), [ce, C] = N("problems"), [E, K] = N(s.markdownPreviewVisible), [M, ne] = N(!1), [I, Me] = N(null), [A, de] = N([]), [be, xe] = N([]), [Q, ve] = N(null), [Ve, k] = N(""), [x, y] = N({ lineNumber: 1, column: 1 }), [V, q] = N(null), [oe, O] = N({}), [he, Te] = N(ys), [Je, Ae] = N([]), [je, ue] = N([]), [$, ae] = N(null), [G, re] = N(""), [qt, dt] = N(!1), [qe, Yt] = N(""), [Nt, $e] = N([]), [fe, Ye] = N(null), [l, g] = N(null), [X, j] = N(!1), [L, Y] = N(() => _o()), [B, F] = N(null), [ye, Oe] = N(null), [U, W] = N(""), [ee, ge] = N(""), [me, Ne] = N(""), [De, ut] = N("failed"), [zt, Xt] = N(null), [_e, H] = N("Ready"), [ht, Ea] = N(() => Qo()), ze = be.map((d) => A.find((f) => f.id === d)).filter(Boolean), D = Q ? A.find((d) => d.id === Q) ?? null : null, Xn = pe(() => vs({ files: A, openEditorIds: be, activeFileId: Q, versions: oe, activeSelection: V }), [Q, V, oe, A, be]), Tn = pe(() => ({
    scope: he,
    removedAttachmentIds: Je,
    selectedFileIds: []
  }), [he, Je]), ft = pe(() => As(Xn, A, Tn), [Tn, Xn, A]), Re = $s(A, { autoRefresh: !0, includeDirty: !0 }), Ln = pe(() => je.map((d) => ({
    id: d.id,
    kind: "index-hit",
    label: d.label,
    path: d.path,
    ...
    snippet: d.snippet
  })), [je]), Ja = pe(() => [...ft.attachments, ...Ln], [ft.attachments, Ln]), Z = ms({ host: e, requestContext: ft.parts, chatSettings: L, setStatus: H }), Ze = pe(
    () => [...Z.artifacts, ...Nt].sort((d, f) => f.createdAt - d.createdAt),
    [Z.artifacts, Nt]
  ), Tt = X && !D, Rt = !D && !Tt && !M, ke = pe(() => A.filter((d) => d.dirty), [A]), Za = pe(() => {
    const d = Ve.trim().toLowerCase();
    return d ? A.filter((f) => f.path.toLowerCase().includes(d)) : A;
  }, [A, Ve]);
  le(() => {
    O((d) => {
      const f = {};
      for (const v of A) f[v.id] = d[v.id] ?? 1;
      return f;
    });
  }, [A]), le(() => {
    Ae([]), ue([]);
  ...
  const Ka = P((d) => {
    if (d.kind === "index-hit") {
      ue((f) => f.filter((v) => v.id !== d.id)), H(`Removed project context: ${d.label}`);
      return;
    }
    Ae((f) => f.includes(d.id) ? f : [...f, d.id]), H(`Removed chat context: ${d.label}`);
  }, []), Lt = P(() => {
    j(!0), ve(null), ne(!0), H("Atomek settings opened");
  }, []), An = P(() => {
    j(!1), ve((d) => d ?? be.at(-1) ?? null), be.length === 0 && ne(!1), H("Atomek settings closed");
  }, [be]), et = P((d) => {
    O((f) => ({ ...f, [d]: (f[d] ?? 1) + 1 }));
  }, []), Ba = P((d) => {
    d.preventDefault(), d.currentTarget.setPointerCapture?.(d.pointerId);
    const f = nn(n), v = d.clientX, z = J, ie = (bt) => {
      const $t = z + (v - bt.clientX);
      _(Ct($t, f.secondaryMin, f.secondaryMax));
    }, He = () => {
      window.removeEventListener("pointermove", ie), window.removeEventListener("pointerup", He);
    };
    window.addEventListener("pointermove", ie), window.addEventListener("pointerup", He);
  }, [J, n]), Ua = P((d) => {
    d.preventDefault(), d.currentTarget.setPointerCapture?.(d.pointerId);
    const f = nn(n), v = d.clientX, z = b, ie = (bt) => {
      const $t = z + (bt.clientX - v);
      p(Ct($t, f.primaryMin, f.primaryMax));
    }, He = () => {
      window.removeEventListener("pointermove", ie), window.removeEventListener("pointerup", He);
    };
    window.addEventListener("pointermove", ie), window.addEventListener("pointerup", He);
  }, [b, n]), At = P((d) => {
    const f = [d, ...ht.filter((v) => v.path !== d.path)].slice(0, 6);
    Ea(f), localStorage.setItem(Aa, JSON.stringify(f));
  }, [ht]), we = P((d, f) => {
    xe((v) => v.includes(d.id) ? v : [...v, d.id]), ve(d.id), ne(!1), Xt(f ?? null), y({ lineNumber: f ?? 1, column: 1 });
  }, []), Ga = P((d) => {
    if (!d.fileId) return;
    const f = A.find((v) => v.id === d.fileId);
    f && (we(f, d.range?.startLineNumber ?? 1), H(`Revealed context: ${d.label}`));
  }, [A, we]), mt = P(async () => {
    if (an(ke, "open new files"))
      try {
        const d = await ss();
        if (d.length === 0) return;
        de((f) => Bo(f, d)), d.forEach((f) => At({ name: f.name, path: f.path, at: Date.now() })), we(d[0]), H(`Opened ${d.length} local file${d.length === 1 ? "" : "s"}`);
      } catch (d) {
        d.name !== "AbortError" && H(`Open file failed: ${d.message}`);
      }
  }, [ke, we, At]), Et = P(async () => {
    if (an(ke, "open another folder"))
      try {
        const d = await is();
        Me(d), de(d.files), xe([]), ve(null), ne(!1), At({ name: d.name, path: d.name, at: Date.now() }), H(`${d.handle ? "Opened local folder" : "Opened browser fallback folder"} ${d.name} (${d.files.length} text files indexed)`);
      } catch (d) {
        d.name !== "AbortError" && H(`Open folder failed: ${d.message}`);
      }
  }, [ke, At]), Qa = P((d) => {
    if (!Q) return;
    let f = !1;
    de((v) => v.map((z) => z.id !== Q || z.content === d ? z : (f = !0, { ...z, content: d, dirty: !0 }))), f && et(Q);
  }, [Q, et]), Jt = P(async () => {
    if (D)
      try {
        const d = await _t(D);
        de((f) => f.map((v) => v.id === d.id ? d : v)), H(`Saved ${d.name}`);
      } catch (d) {
        H(`Save failed: ${d.message}`);
      }
  }, [D]), $a = P(async (d) => {
    const f = A.find((z) => z.id === d);
    if (!f) return null;
    const v = await _t(f);
    return de((z) => z.map((ie) => ie.id === v.id ? v : ie)), v;
  }, [A]), On = P(async () => {
    const d = A.filter((f) => f.dirty);
    if (d.length === 0) {
      H("No dirty files to save");
      return;
    }
    try {
      const f = await Promise.all(d.map((z) => _t(z))), v = new Map(f.map((z) => [z.id, z]));
      de((z) => z.map((ie) => v.get(ie.id) ?? ie)), F(null), H(`Saved ${f.length} dirty file${f.length === 1 ? "" : "s"}`);
    } catch (f) {
      H(`Save all failed: ${f.message}`);
    }
  }, [A]), Zt = P((d) => {
    const f = A.find((v) => v.id === d);
    if (f?.dirty && !window.confirm(`${f.name} has unsaved changes. Close without saving?`)) {
      H(`Close canceled — ${f.name} has unsaved changes`);
      return;
    }
    xe((v) => {
      const z = v.filter((ie) => ie !== d);
      return Q === d && (ve(z.at(-1) ?? null), q(null)), z;
    });
  }, [Q, A]), _a = P(() => {
    an(ke, "close all editors") && (xe([]), ve(null), Xt(null), ne(!1), H("Closed all editors"));
  }, [ke]), Kt = P(() => {
    const d = A.filter((v) => v.name.startsWith("Untitled")).length + 1, f = {
...
`,
    ...
    return ue(z), z.length === 0 ? (H(Re.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), f) : (v.mode !== "hybrid" && v.reason ? H(v.reason) : H(`${z.length} project context hit${z.length === 1 ? "" : "s"} · ${v.reason ?? "hybrid retrieval"}`), [
      ...f,
      ...ii(z)
    ]);
  }, [ft.parts, he, L, e, Re]), Bt = P(() => {
    if (!D && ze.length === 0) {
      H("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const f = [
      `Create a polished Markdown artifact from ${D?.path ?? `${ze.length} open editors`}.`,
    ...
    S(!0), w("chat"), H("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const v = await Ee(f), z = await Z.askAgent(f, { requestContext: v });
      !z || z.status === "error" || Z.createArtifact({
        messageId: z.id,
        title: `AI synthesis — ${D?.name ?? "open workspace"}`,
        kind: "markdown",
        body: z.body
      }).then(() => {
        S(!0), w("outputs"), se(!0), C("output");
      });
    })();
  }, [D, Z, Ee, ze.length]), Ut = P((d) => {
    const f = A.find((v) => v.path === d.path || v.name === d.name);
    if (f) {
      we(f), H(`Opened recent ${f.name}`);
      return;
    }
    H("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [A, we]), Xe = P((d) => {
    S(!0), w("chat"), re(""), (async () => {
      const f = await Ee(d);
      await Z.askAgent(d, { requestContext: f });
    })();
  }, [Z, Ee]), jn = P(async (d) => {
    if (!e.skills?.get) {
      H("Tytus skill registry is not available in this host build");
      return;
    }
    try {
      const f = await e.skills.get(d.id), v = f.body.length > 4500 ? `${f.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : f.body, z = [
...
`);
      re((ie) => [ie.trim(), z].filter(Boolean).join(`

`)), S(!0), w("chat"), H(`Attached skill ${f.title} to chat input`);
    } catch (f) {
      H(`Failed to attach skill: ${f instanceof Error ? f.message : String(f)}`);
    }
  }, [e.skills]), er = P((d) => {
    const f = Z.messages.findIndex((z) => z.id === d.id), v = Z.messages.slice(0, f < 0 ? void 0 : f).reverse().find((z) => z.role === "user");
    if (!v?.body.trim()) {
      H("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const z = await Ee(v.body);
      await Z.askAgent(v.body, { requestContext: z });
    })();
  }, [Z, Ee]), tr = P((d) => {
    Z.createArtifact({
      messageId: d.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    if (!D) {
      H("No active file to save as AI artifact");
      return;
    }
    Z.createArtifact({
      title: D.path,
    ...
    const f = sn(A, rn(d.title || d.kind)), v = {
    ...
    de((z) => [...z, v]), we(v), H(`Opened ${d.title} as editable file`);
  }, [A, we]), Qt = P(() => {
    const d = window.prompt("Check command/name (manual capture only; Atomek will not execute it)");
    if (d === null) return;
    const f = d.trim();
    if (!f) {
      H("Manual check capture canceled — command/name was empty");
      return;
    }
...
`), ie = {
    ...
    $e((He) => [ie, ...He]), se(!0), C("output"), H(`Captured manual check: ${f}`);
  }, []), tt = P((d, f) => {
    const v = Ni({
      body: f,
      files: A,
      ...
      edits: v.edits.map(Uo),
      skipped: v.skipped
    };
    return v.edits.length > 1 ? (g(z), Ye(null), H(`Previewing AI workspace patch for ${v.edits.length} files`), !0) : v.edits.length === 1 ? (Ye(z.edits[0]), g(null), H(`Previewing AI patch for ${z.edits[0].fileName}`), !0) : (H(v.skipped.length > 0 ? `No applicable edit found. ${v.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [D, oe, A]), Hn = P((d, f) => {
    const v = {
    ...
    $e((z) => [v, ...z]), S(!0), w("outputs"), se(!0), C("output"), H(`Saved local job output: ${d}`), (f.includes("```diff") || f.includes("--- a/")) && tt(d, f);
  }, [tt]), rr = P(() => {
    const d = G.trim();
    if (!d) return;
    re("");
    const f = Mo(d);
    (async () => {
      const v = await Ee(d), z = await Z.askAgent(d, { requestContext: v });
      if (!f || !z || z.status === "error") return;
      tt(z.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", z.body) || H("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [Z, Ee, G, tt]), sr = P(() => {
    $ && (ae(null), Xe(Vo($)));
  }, [Xe, $]), pt = P((d) => {
    ...
    Ne(v), W(""), ge(""), ut("failed"), se(!0), C("terminal"), H(f.commands.length > 0 ? `Manual check ready: copy ${f.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [A]), ir = P((d) => {
    d.trim() && (ot(d), Ne(d), H(`Manual check command copied: ${d}`));
  }, []), or = P(() => {
    Oe((d) => {
      if (!d) return d;
      const f = qi(d, U), v = f.commands.at(-1)?.command ?? "";
      return v && Ne(v), f;
    }), W("");
  }, [U]), cr = P(() => {
    if (!ye || !me.trim()) {
      H("Select or enter a manual check command before capturing output");
      return;
    }
    const d = me.trim();
    Oe((f) => f && Yi(f, d, De, ee)), ge(""), H(`Captured manual check result: ${d} (${De})`);
  }, [ee, me, ye, De]), lr = P(() => {
    ye && Xe(Ri(ye));
  }, [Xe, ye]), dr = P(() => {
    if (!fe) return;
    const d = A.find((f) => f.id === fe.fileId);
    if (!d) {
      H(`Cannot apply edit — ${fe.fileName} is no longer open`), Ye(null);
      return;
    }
    d.content !== fe.originalContent && !window.confirm(`${fe.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (de((f) => f.map((v) => v.id === fe.fileId ? { ...v, content: fe.proposedContent, dirty: !0 } : v)), et(fe.fileId), F(`AI edit applied to ${d.name}. Save All to persist it to disk.`), Ye(null), pt(`AI edit applied to ${d.name}`), H(`Applied AI edit to ${d.name} — unsaved; manual check ready`));
  }, [et, A, fe, pt]), ur = P(() => {
    if (!fe) return;
    const d = sn(A, `${rn(fe.fileName)}-ai-edit`), f = {
      id: `pending-edit-${fe.fileId}-${Date.now()}`,
      name: d,
      path: d,
      language: D?.language ?? "markdown",
      content: fe.proposedContent,
      dirty: !0,
      source: "generated"
    };
    de((v) => [...v, f]), we(f), Ye(null), H(`Opened proposed edit as ${d}`);
  }, [D?.language, A, we, fe]), hr = P(() => {
    if (!l) return;
    const d = new Map(l.edits.map((v) => [v.fileId, v]));
    A.some((v) => {
      const z = d.get(v.id);
      return z && v.content !== z.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (de((v) => v.map((z) => {
      const ie = d.get(z.id);
      return ie ? { ...z, content: ie.proposedContent, dirty: !0 } : z;
    })), d.forEach((v, z) => et(z)), F(`AI workspace patch applied to ${d.size} file${d.size === 1 ? "" : "s"}. Save All to persist changes.`), g(null), pt(`AI workspace patch applied to ${d.size} file${d.size === 1 ? "" : "s"}`), H(`Applied AI workspace patch to ${d.size} file${d.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [et, A, l, pt]), fr = P(() => {
    if (!l) return;
    const d = [...A], f = l.edits.map((v) => {
      const z = sn(d, `${rn(v.fileName)}-ai-edit`), ie = A.find((bt) => bt.id === v.fileId), He = {
        id: `workspace-patch-${v.fileId}-${Date.now()}-${z}`,
        name: z,
        path: z,
        language: ie?.language ?? "markdown",
    ...
    de((v) => [...v, ...f]), f[0] && we(f[0]), g(null), H(`Opened ${f.length} proposed edit file${f.length === 1 ? "" : "s"}`);
  }, [A, we, l]), Cn = P((d) => {
    if (d === "explain") {
      Xe("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (d === "improve") {
      Xe("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (d === "plan") {
      Xe("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (d === "edit") {
      Xe("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    Xe("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [Xe]);
  return le(() => {
    const d = (f) => {
      (f.metaKey || f.ctrlKey) && (f.key.toLowerCase() === "o" && (f.preventDefault(), mt()), f.key.toLowerCase() === "s" && (f.preventDefault(), Jt()), f.key.toLowerCase() === "w" && (f.preventDefault(), Q && Zt(Q)), f.key.toLowerCase() === "b" && (f.preventDefault(), m((z) => !z)), f.shiftKey && f.key.toLowerCase() === "f" && (f.preventDefault(), o("search"), m(!0)), (f.key.toLowerCase() === "k" || f.key.toLowerCase() === "p") && (f.preventDefault(), dt(!0)));
    };
    return window.addEventListener("keydown", d), () => window.removeEventListener("keydown", d);
  }, [Q, Zt, mt, Jt]), le(() => {
    const d = (f) => {
      ke.length !== 0 && (f.preventDefault(), f.returnValue = "");
    };
    ...
    const d = t.current;
    if (!d) return;
    const f = () => r(Math.round(d.getBoundingClientRect().width));
    f();
    ...
    if (!n) return;
    const d = nn(n);
    p((f) => Ct(f, d.primaryMin, d.primaryMax)), _((f) => Ct(f, d.secondaryMin, d.secondaryMax));
  }, [n]), le(() => {
    const d = { primaryVisible: c, primaryWidth: b, secondaryVisible: T, secondaryWidth: J, markdownPreviewVisible: E };
    localStorage.setItem(Oa, JSON.stringify(d));
  }, [E, c, b, T, J]), le(() => {
    localStorage.setItem(ja, JSON.stringify(L));
  }, [L]), /* @__PURE__ */ u(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${c ? "" : "no-primary"} ${T ? "" : "no-secondary"} ${R ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${b}px`, "--workbench-secondary-width": `${J}px` },
      children: [
        /* @__PURE__ */ a(yo, { active: i, setActive: (d) => {
          o(d), m(!0);
        }, openSettings: Lt, settingsActive: Tt }),
        c && /* @__PURE__ */ u("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            wo,
            {
              host: e,
              ...
              setStatus: H,
              hasFsAccess: rs(),
              attachSkillToChat: jn,
              saveLocalJobOutput: Hn,
              activeFile: D
            }
          ),
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: Ua, title: "Resize Explorer" })
        ] }),
          ...
          /* @__PURE__ */ u("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              xo,
              {
                ...
                closeSettings: An,
                secondaryVisible: T,
                toggleSecondary: () => S((d) => !d),
                canPreview: D?.language === "markdown",
                previewVisible: E,
                togglePreview: () => K((d) => !d)
              }
            ),
            /* @__PURE__ */ a(Po, { file: D, folder: I, showWelcome: Rt }),
            B && ke.length > 0 ? /* @__PURE__ */ u("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: B }),
              /* @__PURE__ */ a("button", { onClick: () => {
                On();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => F(null), title: "Dismiss", children: /* @__PURE__ */ a(We, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: D ? /* @__PURE__ */ u("div", { className: D.language === "markdown" && E ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(pr, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                ao,
                {
                  file: D,
                  revealLine: zt,
              ...
              D.language === "markdown" && E && /* @__PURE__ */ a(Oo, { content: D.content })
            ] }) : Tt ? /* @__PURE__ */ a(
              Do,
              {
                host: e,
            ...
            ) : Rt ? /* @__PURE__ */ a(Xo, { host: e, openFile: mt, openFolder: Et, newFile: Kt, recent: ht, reopenRecent: Ut, setStatus: H, openControlTower: () => {
              o("computer"), m(!0);
            }, openChat: () => {
              w("chat"), S(!0);
            } }) : /* @__PURE__ */ u("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(Ft, { size: 34 }),
            ...
            R && /* @__PURE__ */ a(
              Ho,
              {
                ...
                runAiSynthesis: Bt,
                captureManualCheck: Qt,
                openOutputAsFile: Gt,
                manualCheckSession: ye,
                manualCheckCommandInput: U,
                setManualCheckCommandInput: W,
                manualCheckOutputInput: ee,
                setManualCheckOutputInput: ge,
                manualCheckSelectedCommand: me,
                setManualCheckSelectedCommand: Ne,
                manualCheckStatus: De,
                setManualCheckStatus: ut,
                copyManualCheckCommand: ir,
                addManualCheckCommand: or,
                recordManualCheckResult: cr,
                askAgentFromManualChecks: lr,
                onClose: () => se(!1)
              }
            )
          ] })
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", d.body),
            ...
            contextScope: he,
            setContextScope: Te,
            contextAttachments: Ja,
            removeContextAttachment: Ka,
            revealContextAttachment: Ga,
            projectIndexSummary: `${Re.snapshot.files.length} files · ${Re.snapshot.chunks.length} chunks`,
            projectIndexStale: Re.isStale,
            refreshProjectIndex: () => {
              const d = Re.refresh(A);
              H(`Project index refreshed: ${d.files.length} files · ${d.chunks.length} chunks`);
            },
            onResizeStart: Ba,
            onClose: () => S(!1)
          }
        ...
        qt && /* @__PURE__ */ a(
          Ao,
          {
            ...
            commands: [
              ...
              ...ht.map((d) => ({ label: `File: Open Recent — ${d.name}`, detail: d.path, run: () => Ut(d) })),
              { label: "File: Save All", detail: `${ke.length} dirty file${ke.length === 1 ? "" : "s"}`, run: () => {
                On();
              }, disabled: ke.length === 0 },
              { label: "File: Close All Editors", detail: `${ze.length} open editor${ze.length === 1 ? "" : "s"}`, run: _a, disabled: ze.length === 0 },
              ...
              { label: "Help: Show Agent Team", detail: "Open the Atomek Resource Fabric landing page", run: () => {
                ve(null), ne(!1), o("computer"), m(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: c ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => m((d) => !d) },
              { label: "View: Toggle Chat Panel", detail: T ? "Hide right AI side bar" : "Show right AI side bar", run: () => S((d) => !d) },
              { label: "View: Toggle Bottom Panel", detail: R ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => se((d) => !d) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Lt },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => pt("Manual check requested from command palette") },
              { label: "View: Toggle Markdown Preview", detail: D?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => K((d) => !d), disabled: D?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: D || ze.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Bt, disabled: !D && ze.length === 0 },
              { label: "AI: Explain Active File", detail: D ? `Ask Cortex to explain ${D.path}` : "Open a file first", run: () => Xe("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !D },
              { label: "AI: Improve Active File", detail: D ? `Ask Cortex for concrete edits to ${D.path}` : "Open a file first", run: () => Xe("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !D },
              { label: "AI: Draft Editable Replacement", detail: D ? `Ask Cortex for a full-file replacement for ${D.path}` : "Open a file first", run: () => Cn("edit"), disabled: !D },
              { label: "AI: Plan Workspace Work", detail: ze.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => Xe("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: ze.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: D ? "Persist active file in host.ai artifacts" : "Open a file first", run: ar, disabled: !D },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Qt },
              { label: "AI: Open Latest Artifact as File", detail: Ze[0] ? `Create editable file from ${Ze[0].title}` : "No outputs yet", run: () => Ze[0] && Gt(Ze[0]), disabled: Ze.length === 0 }
            ],
            openWorkbenchFile: we,
            onClose: () => dt(!1)
          }
        ...
        /* @__PURE__ */ a(Ko, { status: _e, file: D ?? ro, cursor: x, fileCount: A.length, dirtyCount: ke.length })
      ]
    }
  );
}
function yo({ active: e, setActive: t, openSettings: n, settingsActive: r }) {
  return /* @__PURE__ */ u("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-activity-brand", title: "Atomek", children: /* @__PURE__ */ a(La, { size: 30, variant: "cream" }) }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(ea, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Br, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(na, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a($n, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(pn, { size: 25 }), label: "Agent Team", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Tr, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a($r, { size: 23 }), label: "Settings", active: r, onClick: n })
  ] });
}
function Ke({ icon: e, label: t, active: n, onClick: r }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: r, children: e });
}
function wo(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(To, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(Un, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(Un, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(Ra, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(go, { ...e });
}
function go(e) {
  const t = !e.folder, n = pe(() => ko(e.files, e.folder?.name), [e.files, e.folder?.name]), [r, s] = N(() => /* @__PURE__ */ new Set());
  le(() => {
    const o = Da(n);
    s((c) => {
      const m = new Set(c);
      return o.forEach((b) => m.add(b)), m;
    });
  }, [n]);
  const i = P((o) => {
    s((c) => {
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ u(Se, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((o) => /* @__PURE__ */ a(zn, { file: o, active: o.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(o), label: o.name, detail: o.path }, o.id)),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(Dt, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : Fa(n, e.activeFileId, e.openWorkbenchFile, r, i)
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((o) => /* @__PURE__ */ u("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(o), children: [
...
function zn({ file: e, active: t, onOpen: n, basePath: r, depth: s = 0, label: i, detail: o }) {
  const c = r && e.path.startsWith(`${r}/`) ? e.path.slice(r.length + 1) : e.path, m = s || Math.max(0, c.split("/").length - 1);
...
function ko(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), s = (o, c, m = n) => {
    const b = r.get(o);
    if (b) return b;
  ...
  e.forEach((o) => {
    ...
    m.slice(0, -1).forEach((h) => {
      p = p ? `${p}/${h}` : h, b = s(p, h, b).children;
    }), b.push({ name: m.at(-1) ?? o.name, path: c, file: o, children: [] });
  });
  const i = (o) => o.sort((c, m) => +!!c.file - +!!m.file || c.name.localeCompare(m.name)).map((c) => ({ ...c, children: i(c.children) }));
  return i(n);
}
function Da(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...Da(t.children)]);
}
function Fa(e, t, n, r, s, i = 0) {
  return e.map((o) => {
    if (o.file)
      return /* @__PURE__ */ a(zn, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: i, label: o.name }, o.file.id);
    ...
    return /* @__PURE__ */ u("div", { children: [
      /* @__PURE__ */ u("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => s(o.path), title: c ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        c ? /* @__PURE__ */ a(Dt, { size: 12 }) : /* @__PURE__ */ a(Dt, { className: "workbench-chevron-collapsed", size: 12 }),
        c ? /* @__PURE__ */ a(yn, { size: 14 }) : /* @__PURE__ */ a(ta, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: o.children.length })
      ] }),
      c ? Fa(o.children, t, n, r, s, i + 1) : null
    ] }, o.path);
  });
}
function Po({ file: e, folder: t, showWelcome: n }) {
...
function xo(e) {
  return /* @__PURE__ */ u("div", { className: "workbench-tabs", children: [
    ...
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(Yr, { size: 16 }) })
  ] });
}
function No(e) {
...
function zo(e) {
  ...
  return [...e?.resources ?? []].sort((n, r) => (t[n.kind] ?? 9) - (t[r.kind] ?? 9) || Pe(n).localeCompare(Pe(r))).slice(0, 8);
}
function Xo({
  host: e,
  openFile: t,
  ...
  const [b, p] = N("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [h, w] = N(null), [T, S] = N([]), [J, _] = N(!1), [R, se] = N(null), [ce, C] = N(null), [E, K] = N("pod-local"), M = P(async () => {
    _(!0), C(null);
    try {
      ...
      w(k ?? null), S(x), k && o(`Agent team loaded · ${k.resources.length} resources · ${k.warnings.length} warnings`);
    } catch (k) {
      const x = k instanceof Error ? k.message : String(k);
      C(x), o(`Agent team resource load failed: ${x}`);
    } finally {
      _(!1);
    }
  }, [e.missions, e.resources, o]), ne = P((k, x) => {
    try {
      e.windows.openOrFocus(k), o(`Opened ${x}`);
    } catch (y) {
      o(`Open ${x} failed: ${y instanceof Error ? y.message : String(y)}`);
    }
  }, [e.windows, o]);
  le(() => {
    M();
  ...
  const I = P(async () => {
    ...
    const k = b.trim() || "Coordinate a Tytus mission.", x = Qe(k, h, E);
    _(!0);
    try {
      const y = await e.missions.create({
        ...
        goal: y.goal,
        rootPath: y.rootPath,
        name: y.rootPath.split("/").pop() || y.missionId,
        source: "tray",
        teamPresetId: x
      }, q = {
      ...
      await e.missions.write({
        rootPath: y.rootPath,
        files: [
          { path: "MISSION.md", content: Mt(V, h, null, [], k, x) },
          { path: "MISSION.json", content: Va(V, h, k, x) },
          { path: "RESOURCES.md", content: Vt(h) },
          { path: "TASKS.md", content: Wa(xt(k, h, x)) },
          { path: "HANDOFF.md", content: Ma(V) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(q)}
` },
...
`) }
      ...
      }), se(y), kt(V), o(`Mission created: ${y.rootPath}`), c();
    } catch (y) {
      o(`Mission create failed: ${y instanceof Error ? y.message : String(y)}`);
    } finally {
      _(!1);
    }
  }, [b, h, e.missions, c, o, E]), Me = No(h), A = lo(h), de = uo(h), be = zo(h), xe = h?.warnings ?? [], Q = fo(h), ve = lt(h, Qe(b, h, E)), Ve = xt(b, h, ve.id);
  return /* @__PURE__ */ u("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ u("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ u("div", { className: "workbench-brand-lockup", "aria-label": "Atomek", children: [
        /* @__PURE__ */ a(La, { size: 74, variant: "acid" }),
        /* @__PURE__ */ u("div", { children: [
          /* @__PURE__ */ a(no, { className: "workbench-brand-wordmark" }),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-control-goal-row", children: [
        /* @__PURE__ */ a("textarea", { value: b, onChange: (k) => p(k.target.value), rows: 3, "aria-label": "Mission goal" }),
      ...
      /* @__PURE__ */ a("div", { className: "workbench-team-preset-strip", "aria-label": "Team presets", children: Q.map((k) => /* @__PURE__ */ u(
        "button",
        {
          className: `workbench-team-preset-card ${k.id === ve.id ? "active" : ""} ${k.readiness}`,
          onClick: () => K(k.id),
          children: [
            /* @__PURE__ */ u("div", { children: [
            ...
            /* @__PURE__ */ a("small", { children: k.assignments.map((x) => x.resourceLabel).slice(0, 3).join(" · ") })
          ]
        },
        k.id
      )) }),
      R ? /* @__PURE__ */ u("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: R.rootPath })
      ] }) : null,
      ce ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: ce }) : null
    ] }),
    /* @__PURE__ */ u("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ u("header", { children: [
          ...
          /* @__PURE__ */ u("button", { className: "workbench-button-subtle", onClick: () => {
            ...
            /* @__PURE__ */ a(gt, { size: 13 }),
            " ",
            J ? "Refreshing…" : "Refresh"
          ] })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: Me.map((k) => /* @__PURE__ */ u("div", { className: "workbench-control-metric", children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-agent-brand-grid", children: A.map((k) => /* @__PURE__ */ u("div", { className: `workbench-agent-brand-card ${k.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ u("div", { children: [
            /* @__PURE__ */ a("strong", { children: k.label }),
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: de.map((k, x) => /* @__PURE__ */ u("div", { className: `workbench-fabric-node ${k.status.replace(/\s+/g, "-")}`, children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-fabric-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Ht.sharedFiles, "Shared Files"), children: "Open shared files" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Ht.podInspector, "Pod Inspector"), children: "Open pods" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Ht.channels, "Channels"), children: "Open channels" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Ht.settings, "Agent Settings"), children: "Open agent settings" })
        ] })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        ...
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => p("Review this repo with local OpenCode/Claude, then ask Codex or pi for an independent review. Save all transcripts and proposed patches in the mission folder."), children: "Review + patch repo" }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => p("Ask OpenClaw for independent critique, use Hermes when allocated for deep planning/copy, then run local Claude/OpenCode/Codex for implementation. Keep shared context in the mission folder and approval-gate all edits."), children: "OpenClaw/Hermes + local agent" }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => p("Coordinate creative output: script, audio, visuals, render assets, and final handoff through app skills and shared folders."), children: "Creative production" })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ u("header", { children: [
          /* @__PURE__ */ a("strong", { children: "Resume" }),
          /* @__PURE__ */ a("span", { children: T.length ? `${T.length} missions` : "none yet" })
        ] }),
        T.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Create a mission once; it stays in Tytus Home/Missions and can be resumed here." }) : null,
        T.slice(0, 4).map((k) => /* @__PURE__ */ u("button", { className: "workbench-control-preset", onClick: () => {
          kt(k), c();
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-control-resource-list", children: [
          ...
          be.map((k) => /* @__PURE__ */ u("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: Pe(k) }),
              /* @__PURE__ */ a("span", { children: Nn(k) })
            ] }),
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: Ve.map((k, x) => /* @__PURE__ */ u("button", { className: `workbench-task-card ${k.status}`, onClick: () => p(k.prompt), children: [
          /* @__PURE__ */ a("span", { children: x + 1 }),
...
function To({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: s }) {
  const i = pe(() => Lo(e, t), [e, t]), o = pe(() => {
    const c = /* @__PURE__ */ new Map();
    return i.forEach((m) => c.set(m.file.id, [...c.get(m.file.id) ?? [], m])), Array.from(c.values()).slice(0, 50);
  }, [i]);
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? o.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : o.map((c) => {
        ...
        return /* @__PURE__ */ u("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(zn, { file: m, active: m.id === s, onOpen: () => r(m) }),
          c.slice(0, 5).map((b) => /* @__PURE__ */ u("button", { className: "workbench-search-hit", onClick: () => r(m, b.lineNumber), title: b.line, children: [
            /* @__PURE__ */ a("span", { className: "workbench-search-line", children: b.lineNumber }),
            /* @__PURE__ */ a("span", { children: b.line })
          ] }, `${m.id}-${b.lineNumber}-${b.line}`)),
...
function Ao(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((s) => ({
    label: s.path,
    detail: `Open ${ia(s.language)} file`,
...
function jo(e) {
  const t = e.session ? Ei(e.session) : "pending";
  return /* @__PURE__ */ u("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ u(Se, { children: [
...
function Ho(e) {
  return /* @__PURE__ */ u("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(Ya, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Co(e) {
  return /* @__PURE__ */ u("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ u("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(Jr, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(Or, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(We, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(So, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      Ra,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ a(Ya, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function Ia({ body: e }) {
  const t = pe(() => io(e), [e]), [n, r] = N(null), s = P((i, o) => {
    (async () => await ot(o) && (r(i), window.setTimeout(() => r((m) => m === i ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((i) => {
      ...
      return /* @__PURE__ */ u("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ u("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ u("button", { onClick: () => s(i.key, i.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ a(Nr, { size: 12 }) : /* @__PURE__ */ a(cn, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: ha(i.body) }
      },
      i.key
    );
  }) });
...
function So(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Ge(null), [r, s] = N(!0), [i, o] = N(!1), c = pe(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  le(() => {
    const h = n.current;
  ...
  const m = P(() => {
    const h = n.current;
    if (!h) return;
    const T = h.scrollHeight - h.scrollTop - h.clientHeight < 48;
    s(T), T && o(!1);
  }, []), b = P(() => {
    const h = n.current;
  ...
  return /* @__PURE__ */ u("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ u("option", { value: h.id, children: [
              h.title,
              " · ",
              Wo(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ u("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ u("div", { children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(Ia, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ u(Se, { children: [
          /* @__PURE__ */ a("br", {}),
        ...
        h.role === "assistant" && h.status !== "streaming" && h.status !== "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => p(h), title: "Copy this answer", children: [
            /* @__PURE__ */ a(cn, { size: 12 }),
            " Copy"
          ] }),
          ...
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(h), disabled: e.busy, title: "Regenerate this answer", children: [
            /* @__PURE__ */ a(gt, { size: 12 }),
            " Regenerate"
          ] })
        ] }) : null,
        h.role === "assistant" && h.status === "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => p(h), title: "Copy this error", children: [
            /* @__PURE__ */ a(cn, { size: 12 }),
            " Copy"
          ] }),
      ...
      i ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: b, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ u("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("em", { children: Zn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ u("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ u(Se, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ u("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ a(Wn, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((h) => {
            const w = typeof h.score == "number" ? h.score.toFixed(2) : null, T = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, S = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, J = [
              h.path,
            ...
            return /* @__PURE__ */ u("span", { className: "workbench-chat-chip", title: J, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ a(Wn, { size: 13 }) }),
              h.label,
        ...
        /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: Zn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(ra, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(Gr, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Zn(e, t, n) {
  const r = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : qa(e.gatewayPreference)];
...
function Do(e) {
  const [t, n] = N([]), [r, s] = N("Loading gateway models…"), [i, o] = N([]), [c, m] = N("Checking embedding capability…");
  le(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(S.map((J) => ({ id: J.id, gatewayLabel: J.gatewayLabel }))), s(S.length > 0 ? `${S.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (S) {
        if (w.signal.aborted) return;
        n([]), s(`Model discovery failed: ${S instanceof Error ? S.message : String(S)}`);
      }
    })(), () => w.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), le(() => {
    const w = new AbortController();
    return (async () => {
      const S = Vi(e.host);
      if (S) {
      ...
      try {
        const J = await Mi(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        o(J.map((_) => ({ id: _.id, gatewayLabel: _.gatewayLabel ?? _.source ?? "AIL" }))), m(J.length > 0 ? `${J.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (J) {
        if (w.signal.aborted) return;
        o([]), m(`Embedding model discovery failed: ${J instanceof Error ? J.message : String(J)}`);
      }
    })(), () => w.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ u("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          qa(e.chatSettings.gatewayPreference),
    ...
    /* @__PURE__ */ u("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(Wt), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function Ya({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: s, openOutputAsFile: i, previewEditFromOutput: o, canPreviewEdit: c = !1, compact: m = !1 }) {
  return /* @__PURE__ */ u("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((b) => /* @__PURE__ */ u("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ u("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void ot(b.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => i(b), children: "Open as file" }),
        o ? /* @__PURE__ */ a(
          "button",
          {
      ...
      /* @__PURE__ */ a(Ia, { body: b.body })
    ] }, b.id))
  ] });
}
function Fo({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ u("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ a("button", { title: "Close", onClick: r, children: /* @__PURE__ */ a(We, { size: 16 }) })
    ] }),
...
function Io({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ u("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: Go(i.proposedContent) })
      ] }, i.fileId)),
...
function Yo(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, qo(e.content), "```");
  else if (t.length > 0) {
...
function Ra({
  ...
  const [c, m] = N([]), [b, p] = N([]), [h, w] = N(null), [T, S] = N(!1), [J, _] = N(null), [R, se] = N("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [ce, C] = N(null), [E, K] = N([]), [M, ne] = N(() => po()), [I, Me] = N([]), [A, de] = N([]), [be, xe] = N([]), Q = Ge([]), [ve, Ve] = N("task-execute"), [k, x] = N("mission"), y = E.find((l) => l.status === "running" || l.status === "canceling") ?? E[0] ?? null, V = o === "dock", q = pe(() => lt(h, Qe(R || M?.goal || "", h, M?.teamPresetId)), [R, M?.goal, M?.teamPresetId, h]), oe = pe(() => xt(R || M?.goal || "", h, q.id), [R, M?.goal, q.id, h]), O = oe.find((l) => l.id === ve) ?? oe[1] ?? oe[0] ?? null, he = i.filter((l) => l.dirty).length, Te = s ? `${s.path} · ${s.language} · ${s.content.length.toLocaleString()} chars${s.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${he ? ` · ${he} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Je = pe(() => [
    {
  ...
  const Ae = P(async (l = M) => {
    if (!l?.rootPath || !e.missions?.listRuns)
      return xe([]), [];
    try {
      const g = await e.missions.listRuns(l.rootPath);
      return xe(g), Q.current = g, g;
    } catch (g) {
      return t(`Mission run history failed: ${g instanceof Error ? g.message : String(g)}`), [];
    }
  }, [e.missions, M, t]), je = P(async (l, g) => {
    if (!l) return;
    const X = [...g].sort((L, Y) => Jn(Y).localeCompare(Jn(L))).slice(0, 100), j = X.map((L) => JSON.stringify(L)).join(`
`) + (X.length ? `
` : "");
    l.rootPath && e.missions?.write ? await e.missions.write({ rootPath: l.rootPath, files: [{ path: "RUNS.jsonl", content: j }] }) : l.handle && await fn(l.handle, "RUNS.jsonl", j), xe(X), Q.current = X;
  }, [e.missions]), ue = P(async (l, g = M) => {
    if (!g) return;
    const X = [
      l,
      ...Q.current.filter((j) => j.id !== l.id)
    ];
    await je(g, X);
  }, [M, je]), $ = P(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      m([]), p([]), w(null), _("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    S(!0), _(null);
    try {
      const [l, g, X, j] = await Promise.all([
      ...
      m(l), p(g), w(X), Me(j), M?.rootPath && Ae(M), t(`Agent team loaded · ${l.length} tools · ${g.length} skills · ${j.length} missions${X ? ` · ${X.resources.length} resources` : ""}`);
    } catch (l) {
      _(l instanceof Error ? l.message : String(l));
    } finally {
      S(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, Ae, M, t]);
  le(() => {
    $();
  }, [$]), le(() => {
    const l = (g) => {
      const X = g.detail;
      X?.missionId && (ne(X), Ae(X)), $();
    };
    return window.addEventListener(hn, l), () => window.removeEventListener(hn, l);
  }, [$, Ae]);
  const ae = P((l) => {
    const g = bo(l);
    ne(g), kt(g), Ae(g), de([{
      ...
      data: { runCount: l.runCount ?? 0, taskCount: l.taskCount ?? 0 }
    }]), se(l.goal || `Continue mission ${l.title}. Review MISSION.md, TASKS.md, RESOURCES.md, and runs/ before acting.`), t(`Resumed mission: ${l.rootPath}`);
  }, [Ae, t]), G = P(async (l, g, X = []) => {
    const j = [
      ...A,
      ...X,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], L = Qe(g || l.goal, h, l.teamPresetId), Y = xt(g || l.goal, h, L), B = [
      { path: "MISSION.md", content: Mt(l, h, s, i, g, L) },
      { path: "MISSION.json", content: Va(l, h, g, L) },
      { path: "RESOURCES.md", content: Vt(h) },
      { path: "TASKS.md", content: Wa(Y) },
      { path: "HANDOFF.md", content: Ma(l) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: j.map((F) => JSON.stringify(F)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: Q.current.map((F) => JSON.stringify(F)).join(`
`) + (Q.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (l.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: l.rootPath, files: B });
    else if (l.handle) {
      await st(l.handle, "runs"), await st(l.handle, "outputs"), await st(l.handle, "proposals"), await st(l.handle, "approvals");
      for (const F of B) await co(l.handle, F.path, F.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    de(j), kt(l);
  }, [s, e.missions, A, i, h]), re = P(async (l, g = {}) => {
    if (M)
      return await G(M, l), M;
    const X = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, j = l.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let L = null;
    if (e.missions?.create) {
      const B = await e.missions.create({ title: X, goal: j });
      L = {
      ...
      L = {
        handle: B,
        name: B.name,
        missionId: `mission-${Date.now()}-${mo(B.name)}`,
        title: X,
        goal: j,
        source: "browser"
      };
    ...
    if (!L) return null;
    const Y = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${L.rootPath ?? L.name}` };
    return ne(L), kt(L), de([Y]), await G(L, j, [Y]), t(`Mission pack ready in ${L.rootPath ?? L.name}`), L;
  }, [e.missions, M, t, G]), qt = P(async (l) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const g = R.trim() || `Open ${l.label} from Atomek with current context.`, X = l.kind === "ai-cli" ? await re(g) : M;
      await e.local.openTerminal({
        toolId: l.id,
        command: l.command,
        cwd: X?.rootPath ?? Bn(s),
        prompt: X ? `Atomek mission pack ready at ${X.rootPath ?? X.name}. Read MISSION.md and RESOURCES.md. ${Te}` : `Opened from Atomek. ${Te}`
      }), t(l.kind === "ai-cli" ? `Opened ${l.label} in Tytus Terminal with mission context. Press Enter there to start it.` : "Opened Tytus Terminal");
    } catch (g) {
      t(`Terminal launch failed: ${g instanceof Error ? g.message : String(g)}`);
    }
  }, [s, Te, re, e.local, R, M, t]), dt = P(async () => {
    try {
      await re(R.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (l) {
      t(`Mission folder setup failed: ${l instanceof Error ? l.message : String(l)}`);
    }
  }, [re, R, t]), qe = P(async (l, g, X, j = M, L) => {
    if (!j) return;
    const Y = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), B = L ?? `runs/${Y}-${l.id}.md`, F = B.split("/").pop() || `${Y}-${l.id}.md`;
    if (j.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: j.rootPath, files: [{ path: B, content: g }] });
    else if (j.handle) {
      const ye = await st(j.handle, "runs");
      await fn(ye, F, g);
    }
    await G(j, R, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${l.label} exited ${X}; transcript saved to ${B}`,
      data: { toolId: l.id, exitCode: X, transcript: B }
    }]);
  }, [e.missions, R, M, G]), Yt = P(async (l) => {
    ...
    let X = null;
    try {
      X = await re(g), X && await G(X, g, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${l.label} background review started`,
        data: { toolId: l.id, taskId: O?.id ?? "manual", taskTitle: O?.title ?? "Manual run" }
      }]);
    } catch (Y) {
      t(`Mission pack failed before local job start: ${Y instanceof Error ? Y.message : String(Y)}`);
      return;
    }
    C(l.id);
    const j = `local-run-${Date.now()}-${l.id}`, L = rt();
    K((Y) => [{
      id: j,
      toolId: l.id,
      label: l.label,
      ...
      startedAt: Date.now(),
      taskId: O?.id ?? "manual",
      taskTitle: O?.title ?? "Manual run",
      lines: [`[Atomek] Starting ${l.label} local job for ${O?.title ?? "manual run"}…`]
    }, ...Y].slice(0, 6));
    try {
      const Y = await e.local.runJob({
        toolId: l.id,
        prompt: X ? [
...
`)
      }), B = Y.transcriptPath ?? (X?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${l.id}.md` : void 0), F = [
        `[Atomek] Started ${l.label} local job ${Y.id}`,
        O ? `[Atomek] Task: ${O.title} (${O.id})` : "[Atomek] Task: manual",
        ...
        taskTitle: O?.title ?? "Manual run",
        transcriptPath: B,
        summary: `Started ${l.label} for ${O?.title ?? "manual run"}`
      };
      ue(ye, X).catch((U) => {
        t(`Mission run index failed: ${U instanceof Error ? U.message : String(U)}`);
      }), K((U) => U.map((W) => W.id === j ? { ...W, jobId: Y.id, transcriptPath: B } : W));
      const Oe = (U) => {
        K((W) => W.map((ee) => ee.id === j ? U(ee) : ee));
      };
      e.local.streamJob(Y.id, {
        onLog: (U) => {
          F.push(U), Oe((W) => ({ ...W, lines: F.slice(-500) }));
        },
        onDone: (U) => {
          U && (F.push(U), Oe((W) => ({ ...W, lines: F.slice(-500) })));
        },
        onFail: (U) => {
          F.push(`[FAIL] ${U}`), Oe((W) => ({
            ...W,
          ...
          }), r(`${l.label} local job failed`, F.join(`
`)), qe(l, F.join(`
`), -1, X, B).catch((W) => {
...
`);
          Oe((ee) => ({
            ...
            finishedAt: rt(),
            exitCode: U,
            summary: `${l.label} exited ${U}`
          }, X).catch((ee) => {
            t(`Mission run index failed: ${ee instanceof Error ? ee.message : String(ee)}`);
          }), r(`${l.label} local job`, W), qe(l, W, U, X, B).catch((ee) => {
            t(`Mission transcript save failed: ${ee instanceof Error ? ee.message : String(ee)}`);
          }), C(null);
        },
        onError: () => t(`Local job stream issue for ${l.label}`)
      }), t(`Started ${l.label} mission run${O ? ` · ${O.title}` : ""}`);
    } catch (Y) {
      C(null), K((B) => B.map((F) => F.id === j ? {
        ...F,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...F.lines, `[Atomek] Failed to start: ${Y instanceof Error ? Y.message : String(Y)}`]
      } : F)), t(`Local job failed to start: ${Y instanceof Error ? Y.message : String(Y)}`);
    }
  }, [s, re, e.local, R, i, h, r, qe, O, t, ue, G]), Nt = P(async (l) => {
    const g = mn(l);
    if (!g) {
      t(`Cannot dispatch ${Pe(l)}: missing pod id`);
      return;
    }
    const X = R.trim();
    if (!X) {
    ...
    let j = null;
    const L = Pe(l), Y = `pod-run-${Date.now()}-${g}`, B = rt(), F = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${l.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      j = await re(X), j && await G(j, X, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${L} mission task started`,
        data: { resourceId: l.id, podId: g, taskId: O?.id ?? "manual", taskTitle: O?.title ?? "Manual pod run" }
      }]);
    } catch (W) {
    ...
    const ye = {
      ...
      startedAt: B,
      taskId: O?.id ?? "manual",
      taskTitle: O?.title ?? "Manual pod run",
      transcriptPath: F,
      summary: `Started ${L} for ${O?.title ?? "manual pod run"}`
    };
    K((W) => [{
      id: Y,
      toolId: l.id,
      ...
      taskTitle: O?.title ?? "Manual pod run",
      transcriptPath: F,
      lines: [`[Atomek] Dispatching ${L} via Tytus host bridge…`, `[Atomek] Pod: ${g}`, `[Atomek] Task: ${O?.title ?? "manual pod run"}`]
    }, ...W].slice(0, 6)), await ue(ye, j).catch((W) => {
      t(`Mission run index failed: ${W instanceof Error ? W.message : String(W)}`);
    });
    const Oe = [], U = (W) => {
      Oe.push(W), K((ee) => ee.map((ge) => ge.id === Y ? { ...ge, lines: [...ge.lines, W].slice(-500) } : ge));
    };
    try {
      const W = await e.daemon.callPodEndpoint(g, "/v1/models", { method: "GET" });
      if (!W.ok)
        throw new Error(`/v1/models ${W.status}: ${await W.text()}`);
      const ee = await W.json(), ge = Jo(ee);
      if (!ge)
        throw new Error("pod /v1/models returned no model id");
      U(`[Atomek] Selected pod model from live metadata: ${ge}`);
...
`)
            },
            {
              role: "user",
              content: [
                j ? `Mission folder: ${j.rootPath ?? j.name}` : "Mission folder: not available",
                j ? `Mission: ${j.title}` : "",
                j ? `Goal: ${j.goal}` : "",
                O ? `Task: ${O.title} (${O.id})` : "Task: manual",
                "",
                "Mission context:",
                j ? Mt(j, h, s, i, X) : "",
                h ? Vt(h) : "",
...
`)
      ...
      if (!me.ok)
        throw new Error(`/v1/chat/completions ${me.status}: ${Ne}`);
      let De = Ne;
      try {
        De = JSON.parse(Ne);
      } catch {
      }
      const ut = typeof De == "string" ? De : Zo(De);
      U(ut);
      const zt = [
        `# Pod job — ${L}`,
        "",
        ...
        `- Pod: ${g}`,
        `- Captured: ${(/* @__PURE__ */ new Date()).toISOString()}`,
        `- Task: ${O?.title ?? "Manual pod run"}`,
        "",
        ut,
        ""
      ].join(`
`);
      K((Xt) => Xt.map((_e) => _e.id === Y ? { ..._e, status: "complete", finishedAt: Date.now(), lines: [..._e.lines, "[Atomek] Pod task complete."].slice(-500) } : _e)), await ue({ ...ye, status: "complete", finishedAt: rt(), summary: `${L} completed`, transcriptPath: F }, j), r(`${L} pod job`, zt), await qe({ id: l.id, label: L, kind: "pod-agent", status: l.status }, zt, 0, j, F), t(`${L} completed mission task`);
    } catch (W) {
      const ee = W instanceof Error ? W.message : String(W);
      U(`[FAIL] ${ee}`), K((me) => me.map((Ne) => Ne.id === Y ? { ...Ne, status: "failed", finishedAt: Date.now(), lines: Ne.lines.slice(-500) } : Ne)), await ue({ ...ye, status: "failed", finishedAt: rt(), exitCode: -1, summary: ee, transcriptPath: F }, j).catch((me) => {
        t(`Mission run index failed: ${me instanceof Error ? me.message : String(me)}`);
      });
      const ge = [`# Pod job failed — ${L}`, "", `- Pod: ${g}`, `- Error: ${ee}`, "", "```text", Oe.join(`
`), "```", ""].join(`
`);
      r(`${L} pod job failed`, ge), await qe({ id: l.id, label: L, kind: "pod-agent", status: l.status }, ge, -1, j, F).catch((me) => {
        t(`Pod transcript save failed: ${me instanceof Error ? me.message : String(me)}`);
      }), t(`Pod task failed: ${ee}`);
    }
  }, [s, re, e.daemon, R, i, h, r, qe, O, t, ue, G]), $e = P(async (l) => {
    if (!l.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    K((g) => g.map((X) => X.id === l.id ? { ...X, status: "canceling", lines: [...X.lines, "[Atomek] Cancel requested…"] } : X)), ue({
      id: l.id,
      jobId: l.jobId,
      toolId: l.toolId,
      label: l.label,
    ...
    try {
      await e.local.cancelJob(l.jobId), t(`Cancel requested for ${l.label}`);
    } catch (g) {
      K((X) => X.map((j) => j.id === l.id ? { ...j, status: "running", lines: [...j.lines, `[Atomek] Cancel failed: ${g instanceof Error ? g.message : String(g)}`] } : j)), t(`Local job cancel failed: ${g instanceof Error ? g.message : String(g)}`);
    }
  }, [e.local, t, ue]), fe = P(async (l) => {
    const g = [
      `Use Tytus resource "${Pe(l)}" (${l.kind}) for the next mission step.`,
      `Capabilities: ${l.capabilities.join(", ") || "status only"}.`,
      `Sandbox: ${l.sandbox}. Trust: ${l.trustTier}.`,
      l.allowedRoots.length ? `Allowed roots: ${l.allowedRoots.join(", ")}` : "No direct roots exposed.",
...
`);
    se(g), M && await G(M, g, [{
      ...
      message: `Selected resource ${Pe(l)}`,
      data: { resourceId: l.id, kind: l.kind, status: l.status }
    }]), t(`Selected ${Pe(l)} for mission`);
  }, [M, t, G]), Ye = P((l) => {
    const g = l.setupAction, X = g?.commandPreview ? `${g.label}: ${g.commandPreview}` : g?.deepLink ? `${g.label}: ${g.deepLink}` : g?.label ?? `${Pe(l)} needs setup`;
    t(X), ot(g?.commandPreview ?? g?.deepLink ?? X);
  }, [t]);
  return /* @__PURE__ */ u("aside", { className: V ? "workbench-agent-dock" : "workbench-sidebar", children: [
    V ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "AGENT TEAM" }),
    /* @__PURE__ */ u("div", { className: V ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ u("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ a(pn, { size: 18 }),
        /* @__PURE__ */ u("div", { children: [
          /* @__PURE__ */ a("strong", { children: V ? "Mission Runs" : "OpenClaw + Hermes Team Board" }),
      ...
      /* @__PURE__ */ u("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ...
        /* @__PURE__ */ a(gt, { size: 14 }),
        " ",
        T ? "Refreshing…" : "Refresh capabilities"
      ] }),
      J && /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: J }),
      /* @__PURE__ */ u("div", { className: "workbench-team-switcher", role: "tablist", "aria-label": "Atomek agent team views", children: [
        /* @__PURE__ */ a("button", { className: k === "mission" ? "active" : "", onClick: () => x("mission"), children: "Mission" }),
        /* @__PURE__ */ a("button", { className: k === "runs" ? "active" : "", onClick: () => x("runs"), children: "Runs" }),
        /* @__PURE__ */ a("button", { className: k === "setup" ? "active" : "", onClick: () => x("setup"), children: "Setup" })
      ] }),
      k === "mission" ? /* @__PURE__ */ u(Se, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-context-card mission", children: [
          ...
          /* @__PURE__ */ a("span", { children: M ? `${M.rootPath ?? M.name} · ${M.source} · ${A.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ u("div", { className: `workbench-team-assignment-summary ${q.readiness}`, children: [
            /* @__PURE__ */ a("strong", { children: q.label }),
            /* @__PURE__ */ u("span", { children: [
              q.readiness,
              " · ",
              q.bestFor
          ...
          q.assignments.map((l) => /* @__PURE__ */ u("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: l.label }),
              /* @__PURE__ */ a("span", { children: l.resourceLabel })
            ] }),
            /* @__PURE__ */ a("em", { children: l.status })
          ] }, `${l.role}-${l.resourceId}`))
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M && void G(M, R), disabled: !M, children: "Rewrite context files" })
        ] }),
        I.length ? /* @__PURE__ */ u(Se, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
          /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: I.slice(0, V ? 3 : 5).map((l) => /* @__PURE__ */ u(
            "button",
            {
              className: `workbench-mission-row ${M?.missionId === l.missionId ? "active" : ""}`,
              onClick: () => ae(l),
              title: l.rootPath,
              children: [
                /* @__PURE__ */ a("strong", { children: l.title }),
                /* @__PURE__ */ u("span", { children: [
                  l.status ?? "active",
                  " · ",
                  l.taskCount ?? 0,
        ...
        h?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: h.warnings.slice(0, 3).map((l) => /* @__PURE__ */ u("span", { children: [
          l.code,
          ": ",
          l.message
        ] }, `${l.code}-${l.resourceId ?? l.message}`)) }) : null,
        ...
        /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: Je.map((l) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => se(l.prompt), children: l.label }, l.label)) }),
        /* @__PURE__ */ a(
          "textarea",
          {
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: oe.map((l, g) => /* @__PURE__ */ u(
          "button",
          {
            className: `workbench-task-card ${l.status} ${O?.id === l.id ? "active" : ""}`,
            onClick: () => {
              Ve(l.id), se(l.prompt);
            },
            ...
            children: [
              ...
              /* @__PURE__ */ a("strong", { children: l.title }),
              /* @__PURE__ */ a("em", { children: l.resourceHint }),
              /* @__PURE__ */ a("small", { children: l.assignedResourceLabel })
            ]
          },
          l.id
        )) })
      ] }) : null,
      k === "runs" ? /* @__PURE__ */ u(Se, { children: [
        y ? /* @__PURE__ */ u("div", { className: "workbench-agent-run", children: [
          /* @__PURE__ */ u("header", { children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: y.label }),
              /* @__PURE__ */ u("span", { children: [
                y.status,
                typeof y.exitCode == "number" ? ` · exit ${y.exitCode}` : "",
                y.taskTitle ? ` · ${y.taskTitle}` : ""
              ] }),
              y.transcriptPath ? /* @__PURE__ */ a("span", { children: y.transcriptPath }) : null
            ] }),
            /* @__PURE__ */ u("div", { className: "workbench-agent-run-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => r(`${y.label} local job`, y.lines.join(`
`)), disabled: y.lines.length === 0, children: "Save output" }),
              y.jobId && (y.status === "running" || y.status === "canceling") ? /* @__PURE__ */ u("button", { className: "workbench-button-subtle danger", onClick: () => {
                $e(y);
              }, disabled: y.status === "canceling", children: [
                /* @__PURE__ */ a(ra, { size: 12 }),
                " ",
                y.status === "canceling" ? "Canceling…" : "Cancel"
              ] }) : null
            ] })
          ] }),
          /* @__PURE__ */ a("pre", { className: "workbench-computer-job-log", children: y.lines.join(`
`) || "[waiting for output]" })
        ] }) : null,
        be.length ? /* @__PURE__ */ u(Se, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: be.slice(0, V ? 4 : 8).map((l) => /* @__PURE__ */ u("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: l.taskTitle || l.label }),
              /* @__PURE__ */ u("span", { children: [
                l.label,
                " · ",
                l.status,
              ...
              l.transcriptPath ? /* @__PURE__ */ a("small", { children: l.transcriptPath }) : null
            ] }),
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void ot(l.transcriptPath ?? l.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, l.id)) })
        ] }) : null
      ] }) : null,
      k === "setup" ? /* @__PURE__ */ u(Se, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list compact", children: [
          !h && !T ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
          h?.resources.slice(0, V ? 8 : 5).map((l) => /* @__PURE__ */ u("div", { className: "workbench-resource-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: Pe(l) }),
              /* @__PURE__ */ a("span", { children: Nn(l) })
            ] }),
            /* @__PURE__ */ u("div", { className: "workbench-resource-row-actions", children: [
              /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${l.status}`, children: l.status }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                fe(l);
              }, children: "Use" }),
              l.kind === "pod-agent" && mn(l) ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Nt(l);
              }, children: "Ask pod" }) : null,
              l.status === "needs-setup" || l.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => Ye(l), children: "Setup" }) : null
            ] })
          ] }, l.id))
        ] }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          c.length === 0 && !T ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
          c.map((l) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            ...
            l.description ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: l.description }) : null,
            /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                qt(l);
              }, disabled: l.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: Ro(l) }),
              l.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Yt(l);
              }, disabled: l.status !== "available" || ce !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: Eo(l, ce) }) : null
            ] })
          ] }, l.id))
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          b.length === 0 && !T ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
          b.map((l) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            /* @__PURE__ */ u("div", { className: "workbench-computer-card-head", children: [
              /* @__PURE__ */ u("div", { children: [
                /* @__PURE__ */ a("strong", { children: l.title }),
                /* @__PURE__ */ u("span", { children: [
            ...
            /* @__PURE__ */ a("p", { className: "workbench-muted", children: l.description }),
            l.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: l.triggers.slice(0, 4).map((g) => /* @__PURE__ */ a("span", { children: g }, g)) }) : null,
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              n(l);
            }, disabled: l.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
          ] }, l.id))
        ] })
      ] }) : null
    ] })
...
function Ko({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: s }) {
  return /* @__PURE__ */ u("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: ia(t.language) })
  ] });
}
function an(e, t) {
...
function _o() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : Wt.gatewayPreference,
...
function ec({ host: e }) {
  return /* @__PURE__ */ a(vo, { host: e });
}
function ac(e) {
  return function() {
    return /* @__PURE__ */ a(ec, { host: e.host });
  };
}
export {
  ac as default
