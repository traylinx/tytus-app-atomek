...
}, Yn = (...e) => e.filter((t, n, i) => !!t && t.trim() !== "" && i.indexOf(t) === n).join(" ").trim(), gi = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Ci = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], $i = te("check", Ci);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const xi = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], Ft = te("chevron-down", xi);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const no = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function ao() {
  const e = wn();
  return !!(e?.showOpenFilePicker && e.showDirectoryPicker);
}
async function io() {
  const e = wn()?.showOpenFilePicker;
  if (!e) return da(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((i) => la(i, i.name, "local-file")));
  return yn(n.filter(Boolean));
}
async function oo() {
  const e = wn()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await da(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await ca(t, t.name, n, 0, 320), { name: t.name, handle: t, files: yn(n) };
}
function wn() {
  const e = window;
...
async function ca(e, t, n, i, o) {
  if (!(n.length >= o || i > 8))
    for await (const r of e.values()) {
      if (n.length >= o) break;
      if (r.kind === "directory") {
        if (no.has(r.name)) continue;
        await ca(r, `${t}/${r.name}`, n, i + 1, o);
        continue;
      }
      const s = r, l = await la(s, `${t}/${s.name}`, "local-folder");
      l && n.push(l);
    }
}
function da(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const i = Array.from(n.files ?? []), o = await Promise.all(i.map(async (r) => {
        const s = r.webkitRelativePath || r.name;
        return sa(s, r.size) ? {
          id: ua(s),
          name: r.name,
          path: s,
      ...
      n.remove(), t(yn(o.filter(Boolean)));
    }, document.body.append(n), n.click();
  });
}
async function Zt(e) {
  if (!e.handle?.createWritable)
    return ro(e), { ...e, dirty: !1 };
  const t = await e.handle.createWritable();
  return await t.write(e.content), await t.close(), { ...e, dirty: !1 };
}
function ro(e) {
  const t = new Blob([e.content], { type: "text/plain;charset=utf-8" }), n = URL.createObjectURL(t), i = document.createElement("a");
  i.href = n, i.download = e.name, i.click(), URL.revokeObjectURL(n);
}
function ua(e) {
  let t = 0;
...
}, lo = (e, t = so) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Dn = (e, t) => {
  const n = e.split(`
`).map((i) => i.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, co = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Fn = (e) => ({
  id: e.id,
  title: e.title,
  kind: co(e.kind),
  body: e.body,
...
`)
  ...
  try {
    ...
    return !t || typeof t != "object" ? {} : Object.fromEntries(
      Object.entries(t).filter(([, n]) => typeof n == "string" && n.trim()).map(([n, i]) => [n, String(i)])
    );
  } catch {
    return {};
  }
  ...
  try {
    const n = Cn();
    n[e] = t, localStorage.setItem(vn, JSON.stringify(n));
  } catch {
  }
}, mo = (e) => {
  try {
    const t = Cn();
    delete t[e], localStorage.setItem(vn, JSON.stringify(t));
  } catch {
  }
}, zn = (e) => {
  const t = Cn();
...
function po({ host: e, requestContext: t, chatSettings: n, setStatus: i }) {
  const o = e.ai, [r, s] = x(null), [l, p] = x([]), [b, f] = x([]), [h, w] = x([]), [I, E] = x([]), [V, Z] = x(jn), [U, oe] = x(!1), le = Xe(!0), O = Xe(null), W = C(async () => {
    if (!o) {
      Z(jn);
      return;
    }
    ...
    if (o)
      try {
        const k = await o.listArtifacts({ threadId: $ });
        if (!le.current) return;
        w(k.map(Fn));
      } catch (k) {
        i(`AI artifacts unavailable: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [o, i]), j = C(async ($) => {
    if (o)
      try {
        const k = await o.listMessages($);
        if (!le.current) return;
        const D = l.find((H) => H.id === $) ?? null;
        D && s(D), f(k.map(gt).filter(Boolean)), E([]), await K($);
      } catch (k) {
        i(`Load chat failed: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [o, K, i, l]), ne = C(async () => {
    if (o)
      try {
        const $ = zn(await o.listThreads({ workspaceKey: nt, status: "active" })), k = $[0] ?? await o.createThread({ workspaceKey: nt, title: "Atomek chat" });
        if (!le.current) return;
        p($[0] ? $ : [k]), s(k);
        const D = await o.listMessages(k.id);
        if (!le.current) return;
        f(D.map(gt).filter(Boolean)), await K(k.id);
      } catch ($) {
        i(`AI chat unavailable: ${$ instanceof Error ? $.message : String($)}`);
      }
  }, [o, K, i]);
  ce(() => (le.current = !0, W(), ne(), () => {
    le.current = !1;
  }), [ne, W]);
  const _ = C(async () => {
    if (!o) return null;
    if (r) return r;
    const $ = await o.createThread({ workspaceKey: nt, title: "Atomek chat" });
    return le.current && (s($), p((k) => [$, ...k.filter((D) => D.id !== $.id)])), $;
  }, [o, r]), je = C(async () => {
    if (!o) return;
    const $ = await o.createThread({ workspaceKey: nt, title: "Atomek chat" });
    s($), p((k) => [$, ...k.filter((D) => D.id !== $.id)]), f([]), w([]), E([]), i("New AI chat created");
  }, [o, i]), M = C(async ($, k) => {
    if (!o) return;
    const D = k.trim();
    if (D)
      try {
        const H = o.updateThread, se = l.find((he) => he.id === $) ?? r, P = typeof H == "function" ? await H({ threadId: $, title: D }) : { ...se ?? await _(), id: $, title: D, updatedAt: Date.now() };
        typeof H != "function" && ho($, D), p((he) => he.map((Ie) => Ie.id === P.id ? P : Ie)), s((he) => he?.id === P.id ? P : he), i(typeof H == "function" ? `Renamed chat: ${P.title}` : `Renamed chat locally: ${P.title}`);
      } catch (H) {
        i(`Rename chat failed: ${H instanceof Error ? H.message : String(H)}`);
      }
  }, [o, _, i, r, l]), de = C(async ($) => {
    if (o)
      try {
        await o.deleteThread($), mo($);
        const k = l.filter((D) => D.id !== $);
        if (p(k), r?.id === $) {
          const D = k[0] ?? await o.createThread({ workspaceKey: nt, title: "Atomek chat" });
          s(D), p((se) => se.some((P) => P.id === D.id) ? se : [D, ...se]);
          const H = await o.listMessages(D.id);
          f(H.map(gt).filter(Boolean)), await K(D.id);
        }
        i("Deleted AI chat");
      } catch (k) {
        i(`Delete chat failed: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [o, K, i, r?.id, l]), be = C(async ($) => {
    if (!o) return null;
    try {
      const k = await _();
      if (!k) return null;
      const D = await o.createArtifact({
        threadId: k.id,
        messageId: $.messageId ?? null,
        title: $.title?.trim() || Dn($.body, "Atomek artifact"),
        kind: $.kind ?? "markdown",
        body: $.body
      }), H = Fn(D);
      return w((se) => [H, ...se.filter((P) => P.id !== H.id)]), i(`Saved AI artifact: ${H.title}`), H;
    } catch (k) {
      return i(`Save artifact failed: ${k instanceof Error ? k.message : String(k)}`), null;
    }
  }, [o, _, i]), $e = C(async ($) => {
    if (o)
      try {
        await o.deleteArtifact($), w((k) => k.filter((D) => D.id !== $)), i("Deleted AI artifact");
      } catch (k) {
        i(`Delete artifact failed: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [o, i]), Y = C(async ($) => {
    if (!o) return null;
    try {
      const k = await o.writeMemory({
        title: $.title?.trim() || Dn($.body, "Atomek memory"),
        body: $.body,
        metadata: {
          source: "atomek",
          messageId: $.messageId ?? null
      ...
      return E((D) => [k, ...D.filter((H) => H.id !== k.id)].slice(0, 5)), i(`Remembered: ${k.title}`), k;
    } catch (k) {
      return i(`Remember failed: ${k instanceof Error ? k.message : String(k)}`), null;
    }
  }, [o, i]), ge = C(async ($) => {
    if (!o) return [];
    const k = await o.searchMemory({ query: $, limit: 5 });
    return le.current && E(k), k;
  }, [o]), De = C(async ($, k = {}) => {
    const D = $.trim();
    if (!D || !o) return null;
    oe(!0);
    ...
    try {
      const P = await _();
      if (!P) return null;
      const he = await ge(D).catch(() => []), Ie = uo(he), Ve = k.requestContext ?? t, Me = Ie ? [...Ve, Ie] : [...Ve];
      let Te = null;
      for await (const Q of o.sendMessage({
        threadId: P.id,
        body: D,
        ...
        if (Q.type === "message_created") {
          const ae = gt(Q.message);
          if (!ae) continue;
          ae.role === "assistant" && (Te = ae.id), f((X) => [...X.filter((ie) => ie.id !== ae.id), ae]);
        }
        if (Q.type === "token" && (Te = Q.messageId, f((ae) => ae.some((X) => X.id === Q.messageId) ? ae.map(
          (X) => X.id === Q.messageId ? { ...X, body: Q.body, status: "streaming" } : X
        ) : [...ae, {
          id: Q.messageId,
          role: "assistant",
          body: Q.body,
          status: "streaming",
          createdAt: Date.now()
        }])), Q.type === "message_updated" || Q.type === "done") {
          const ae = gt(Q.message);
          if (!ae) continue;
          ae.role === "assistant" && (H = ae), f((X) => X.map((ie) => ie.id === ae.id ? ae : ie)), ae.gatewayLabel && i(`AI answered via ${ae.gatewayLabel}`);
        }
        if (Q.type === "run_failed") {
          const ae = se.signal.aborted;
          i(ae ? "AI response stopped" : `AI failed: ${Q.error}`), Te && f((X) => X.map((ie) => ie.id !== Te ? ie : ae ? { ...ie, status: "complete", error: void 0, body: ie.body || "Stopped by user." } : { ...ie, status: "error", error: Q.error, body: Q.error }));
        }
      }
      const ue = zn(await o.listThreads({ workspaceKey: nt, status: "active" }).catch(() => []));
      return le.current && ue.length > 0 && p(ue), W(), H;
    } catch (P) {
      return P instanceof DOMException && P.name === "AbortError" ? (i("AI response stopped"), H) : (i(`AI failed: ${P instanceof Error ? P.message : String(P)}`), null);
    } finally {
      O.current === se && (O.current = null), oe(!1);
    }
  }, [o, n.gatewayPreference, n.model, _, ge, W, t, i]), v = C(() => {
    O.current?.abort(), i("Stopping AI response…");
  }, [i]);
  return {
    aiStatus: V,
    artifacts: h,
...
const fo = (e) => `tytus-workbench:///${encodeURI(e.path)}`, bo = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, go = ({ files: e, openEditorIds: t, activeFileId: n, versions: i, activeSelection: o }) => {
  const r = new Set(t), s = e.map((l) => {
    const p = l.id === n;
    return {
      id: l.id,
      uri: fo(l),
      ...
      language: l.language,
      version: i[l.id] ?? 1,
      contentHash: bo(l.content),
      dirty: l.dirty,
      source: l.source,
      selection: p && o ? o : void 0,
      updatedAt: Date.now(),
      open: r.has(l.id),
      active: p
    };
  ...
  return {
    ...
    activeDocumentId: n,
    openDocumentIds: t.filter((l) => s.some((p) => p.id === l))
  };
}, ko = "active-file", wo = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", yo = (e, t = "file") => `${t}:${e.id}`, vo = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  const n = new Set(t.removedAttachmentIds), i = e.activeDocumentId ? e.byId.get(e.activeDocumentId) ?? null : null;
  let o = [];
  return t.scope === "active-selection" ? i?.selection ? o = [Co(i, i.selection, !0)] : i && (o = [Pt(i, !0, "Active file")]) : t.scope === "active-file" ? i && (o = [Pt(i, !0, "Active file")]) : t.scope === "open-editors" ? o = e.openDocumentIds.map((r) => e.byId.get(r)).filter(Boolean).map((r) => Pt(r, !0, "Open editor")) : t.scope === "selected-files" && (o = t.selectedFileIds.map((r) => e.byId.get(r)).filter(Boolean).map((r) => Pt(r, !1, "Selected file"))), xo(o).filter((r) => !n.has(r.id));
}, xo = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, Ao = 4e3, No = 8e3, Io = 4e3, So = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Mo = (e, t, n) => {
  const i = $o(e, n), o = [], r = new Map(t.map((s) => [s.id, s]));
  for (const s of i) {
    if (!s.fileId) continue;
    const l = r.get(s.fileId), p = e.byId.get(s.fileId);
    if (!l || !p || !s.includeBody) continue;
    const b = [
      `Path: ${l.path}`,
      `Language: ${l.language}`,
      `Version: ${p.version}`,
      ...
      s.range ? `Range: ${_n(s.range)}` : null
    ].filter(Boolean).join(`
`), f = s.kind === "selection" && s.range ? Po(l.content, s.range) : l.content, h = s.kind === "selection" ? Ao : s.label.startsWith("Open editor") ? Io : No;
    o.push({
...
`)).join(`
`)
  }), { parts: o, attachments: i };
}, Po = (e, t) => {
  const n = e.split(`
`), i = Math.max(1, t.startLineNumber), o = Math.max(i, t.endLineNumber), r = n.slice(i - 1, o);
  return r.length === 0 ? "" : r.length === 1 ? r[0].slice(Math.max(0, t.startColumn - 1), Math.max(0, t.endColumn - 1)) : (r[0] = r[0].slice(Math.max(0, t.startColumn - 1)), r[r.length - 1] = r[r.length - 1].slice(0, Math.max(0, t.endColumn - 1)), r.join(`
`));
}, _n = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, Hn = 75e4, To = 2800, Lo = 240, Oo = 80, Eo = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Je(t.content)}`).sort().join("")
), cn = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? Hn) || e.content.length > (t.maxFileBytes ?? Hn) ? "huge" : !t.includeDirty && e.dirty ? null : Ho(e.path) ? "vendor" : Bo(e) ? "binary" : null;
}, Bn = (e, t = {}) => {
  const n = Date.now(), i = [], o = [], r = [];
  for (const s of e) {
    const l = s.size ?? s.content.length, p = cn(s, t);
    if (p) {
      o.push({ fileId: s.id, name: s.name, path: s.path, language: s.language, reason: p, size: l });
      continue;
    }
    const b = Je(s.content), f = jo(s, b, n, t);
    r.push(...f), i.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
  ...
  return Do({
    ...
    skipped: o,
    chunks: r,
    byFileId: new Map(i.map((s) => [s.fileId, s])),
    byChunkId: new Map(r.map((s) => [s.id, s]))
  });
}, jo = (e, t = Je(e.content), n = Date.now(), i = {}) => {
  const o = Math.max(500, i.maxChunkChars ?? To), r = Math.min(Math.max(0, i.chunkOverlapChars ?? Lo), Math.floor(o / 2)), s = Math.max(1, i.maxChunksPerFile ?? Oo), l = zo(e.content), p = [];
  let b = 0, f = 0;
  for (; b < e.content.length && p.length < s; ) {
    const h = Fo(e.content, b, Math.min(e.content.length, b + o)), w = e.content.slice(b, h).trim();
    if (w.length > 0) {
      const I = _o(l, b, h);
      p.push({
        id: `${e.id}:${t}:${f}:${I.startLineNumber}-${I.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
  ...
  const i = Math.max(t + 1, n - 500), o = e.lastIndexOf(`
`, n);
  if (o >= i) return o + 1;
  const r = Math.max(e.lastIndexOf(". ", n), e.lastIndexOf("; ", n));
  if (r >= i) return r + 1;
...
` && t.push(n + 1);
  ...
  const i = Un(e, t), o = Un(e, Math.max(t, n - 1));
  return {
    startLineNumber: i + 1,
    startColumn: t - e[i] + 1,
    endLineNumber: o + 1,
  ...
  for (; n <= i; ) {
    ...
    if (e[o] <= t && (o === e.length - 1 || e[o + 1] > t)) return o;
    e[o] > t ? i = o - 1 : n = o + 1;
  }
  return 0;
}, Ho = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => Eo.has(t)), Bo = (e) => {
  ...
  return i / n.length > 0.08;
};
class Uo {
  snapshot;
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Bn(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Bn(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return Wo(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const at = (e = [], t = {}) => new Uo(e, t), Wo = (e, t, n = {}) => {
  const i = new Map(t.map((l) => [l.id, l])), o = e.byFileId, r = [];
  for (const l of e.files) {
    const p = i.get(l.fileId);
    if (!p) {
      r.push({ fileId: l.fileId, path: l.path, status: "deleted", indexedHash: l.hash });
      continue;
    }
    if (cn(p, n)) {
      r.push({ fileId: l.fileId, path: p.path, status: "skipped-now", indexedHash: l.hash, currentHash: Je(p.content) });
      continue;
    }
    const f = Je(p.content);
    f !== l.hash ? r.push({ fileId: l.fileId, path: p.path, status: "changed", indexedHash: l.hash, currentHash: f }) : p.dirty !== l.dirty && r.push({ fileId: l.fileId, path: p.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: f });
  }
  for (const l of t)
    !o.has(l.id) && !cn(l, n) && r.push({ fileId: l.id, path: l.path, status: "new", currentHash: Je(l.content) });
  const s = e.signature !== ma(t);
  return { stale: r.length > 0, signatureChanged: s, files: r };
}, Vo = 8, qo = 12e3, pa = (e, t, n = {}, i) => {
  const o = Jo(t);
  if (o.length === 0) return [];
  const r = new Set(i?.files.map((f) => f.fileId) ?? []), s = e.chunks.filter((f) => n.includeDirty !== !1 || !f.dirty).map((f) => Xo(f, o)).filter((f) => f.score >= (n.minScore ?? 1)).sort((f, h) => h.score - f.score || f.chunk.path.localeCompare(h.chunk.path) || f.chunk.ordinal - h.chunk.ordinal), l = [], p = /* @__PURE__ */ new Set();
  let b = n.maxChars ?? qo;
  for (const f of s) {
    if (l.length >= (n.limit ?? Vo) || b <= 0) break;
    const h = `${f.chunk.fileId}:${f.chunk.ordinal}`;
    if (p.has(h)) continue;
    p.add(h);
    const w = f.chunk.text.length > b ? `${f.chunk.text.slice(0, Math.max(0, b - 28))}
[...context clipped...]` : f.chunk.text;
    l.push(Ko(f, w, r.has(f.chunk.fileId))), b -= w.length;
  }
  return l;
}, Ko = (e, t = e.chunk.text, n = !1) => {
  const { chunk: i, score: o, matchedTerms: r } = e;
  return {
    id: `index-hit:${i.id}`,
    kind: "index-hit",
    label: `${i.path}:${i.range.startLineNumber}-${i.range.endLineNumber}`,
    fileId: i.fileId,
    chunkId: i.id,
    path: i.path,
    range: i.range,
...
`), Jo = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Xo = (e, t) => {
  const n = e.text.toLowerCase(), i = e.path.toLowerCase(), o = e.name.toLowerCase(), r = e.language.toLowerCase();
  let s = 0;
  const l = [];
  for (const p of t) {
    const b = en(n, p), f = en(i, p), h = en(o, p), w = r === p ? 1 : 0;
    b + f + h + w !== 0 && (l.push(p), s += Math.min(b, 12), s += f * 4, s += h * 6, s += w * 3, (n.includes(`function ${p}`) || n.includes(`const ${p}`) || n.includes(`class ${p}`) || n.includes(`type ${p}`)) && (s += 3));
  }
  return s += Math.max(0, 2 - e.ordinal * 0.05), { chunk: e, score: s, matchedTerms: l };
}, en = (e, t) => {
  let n = 0, i = e.indexOf(t);
  for (; i !== -1; )
    n += 1, i = e.indexOf(t, i + t.length);
  return n;
}, Yo = (e, t) => {
  const n = e.toLowerCase(), i = t.map((p) => n.indexOf(p)).filter((p) => p >= 0).sort((p, b) => p - b)[0] ?? 0, o = Math.max(0, i - 120), r = Math.min(e.length, i + 260), s = o > 0 ? "…" : "", l = r < e.length ? "…" : "";
  return `${s}${e.slice(o, r).replace(/\s+/g, " ").trim()}${l}`;
}, Qo = (e, t = {}) => {
  const { autoRefresh: n = !1, ...i } = t, o = Xe(e), r = Xe(null);
  r.current || (r.current = at(e, i));
  const [s, l] = x(() => r.current?.getSnapshot() ?? at(e, i).getSnapshot());
  o.current = e;
  const p = C((w = o.current) => {
    const I = (r.current ?? at([], i)).refresh(w, i);
    return l(I), I;
  }, [i.maxFileBytes, i.maxChunkChars, i.chunkOverlapChars, i.maxChunksPerFile, i.includeDirty]), b = C((w = o.current) => {
    const I = (r.current ?? at([], i)).update(w, i);
    return l(I), I;
  }, [p]);
  ce(() => {
    n && p(e);
  }, [n, e, p]);
  const f = (r.current ?? at([], i)).staleReport(e), h = C((w, I = {}) => {
    const E = r.current ?? at([], i);
    return pa(E.getSnapshot(), w, I, E.staleReport(o.current));
  }, []);
  return { snapshot: s, staleReport: f, isStale: f.stale, refresh: p, update: b, retrieve: h };
}, Zo = "tytus.atomek.semanticVector:v1", fa = "__gateway_default__", ba = (e) => e?.trim() || fa, er = (e) => {
  const t = ba(e);
  return t === fa ? "gateway default" : t;
}, ga = (e, t, n) => [
  Zo,
  ...
  if (!ka()) return null;
  try {
    const i = ga(e, t, n), o = window.localStorage.getItem(i);
    if (!o) return null;
    const r = JSON.parse(o);
    return r.chunkId !== n.id || r.contentHash !== n.hash || r.modelAlias !== t || !Array.isArray(r.vector) || r.vector.some((s) => typeof s != "number" || !Number.isFinite(s)) ? null : {
      key: i,
      appId: e,
      chunkId: n.id,
      contentHash: n.hash,
      ...
      gatewayLabel: typeof r.gatewayLabel == "string" ? r.gatewayLabel : "",
      dim: r.vector.length,
      vector: r.vector,
  ...
  if (!ka()) return null;
  try {
    const t = ga(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, kt = 8, ar = 12e3, ir = 180, or = async (e, t, n, i, o = {}, r, s) => {
  const l = pa(t, n, { ...o, limit: Math.max(o.limit ?? kt, 16) }, r), p = e.ai;
  if (typeof p?.embedText != "function")
    return { hits: l.slice(0, o.limit ?? kt), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const b = ba(i.embeddingModel), f = await p.embedText({
      input: n,
    ...
    const w = new Map(l.map((O) => [O.chunkId, O])), I = hr(n), E = /* @__PURE__ */ new Map();
    t.chunks.filter((O) => o.includeDirty !== !1 || !O.dirty).slice(0, ir).forEach((O) => E.set(O.id, O)), l.forEach((O) => {
      const W = t.byChunkId.get(O.chunkId);
      W && E.set(W.id, W);
    });
    const V = Array.from(E.values()), Z = [];
    let U = 0;
    for (const O of V) {
      s?.aborted;
      const W = tr(e.appId, b, O) ?? await sr(p, e.appId, b, O, i, s);
      if (!W) continue;
      W.updatedAt > Date.now() - 1e3 && (U += 1);
      const K = dr(h, W.vector);
      if (!Number.isFinite(K)) continue;
      const ne = w.get(O.id)?.score ?? mr(O, I), _ = ur(ne) * 0.42 + Math.max(0, K) * 0.58;
      Z.push({ chunk: O, keywordScore: ne, vectorScore: K, score: _, matchedTerms: I.filter((je) => O.text.toLowerCase().includes(je)) });
    }
    if (Z.length === 0) return { hits: l.slice(0, o.limit ?? kt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: U };
    const oe = new Set(r?.files.map((O) => O.fileId) ?? []), le = Z.sort((O, W) => W.score - O.score || W.keywordScore - O.keywordScore || O.chunk.path.localeCompare(W.chunk.path) || O.chunk.ordinal - W.chunk.ordinal).slice(0, Math.max(o.limit ?? kt, 1)).map((O) => lr(O, oe.has(O.chunk.fileId)));
    return {
      hits: cr(le, o.maxChars ?? ar),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${er(i.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: U
    };
  } catch (b) {
    return {
      hits: l.slice(0, o.limit ?? kt),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: pr(e.chunk.text, e.matchedTerms),
    score: e.score,
  ...
  for (const o of e) {
    if (n <= 0) break;
    const r = o.text.length > n ? `${o.text.slice(0, Math.max(0, n - 28))}
[...context clipped...]` : o.text;
    i.push({ ...o, text: r, charCount: r.length }), n -= r.length;
  }
  return i;
}, wa = (e) => {
  ...
  let i = 0, o = 0, r = 0;
  for (let s = 0; s < n; s += 1)
    i += e[s] * t[s], o += e[s] * e[s], r += t[s] * t[s];
  return o === 0 || r === 0 ? 0 : i / (Math.sqrt(o) * Math.sqrt(r));
}, ur = (e) => Math.min(1, Math.max(0, e / 24)), hr = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), mr = (e, t) => {
  const n = `${e.path}
${e.name}
${e.language}
${e.text}`.toLowerCase();
  return t.reduce((i, o) => i + (n.includes(o) ? 1 : 0), 0);
}, pr = (e, t) => {
  const n = e.toLowerCase(), i = t.map((s) => n.indexOf(s)).filter((s) => s >= 0).sort((s, l) => s - l)[0] ?? 0, o = Math.max(0, i - 120), r = Math.min(e.length, i + 260);
  return `${o > 0 ? "…" : ""}${e.slice(o, r).replace(/\s+/g, " ").trim()}${r < e.length ? "…" : ""}`;
}, fr = /```([^\n`]*)\n([\s\S]*?)```/g, br = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function $n(e) {
  return Array.from(e.matchAll(fr)).map((t) => ya(t[1] ?? "", vr(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function ya(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), i = {}, o = [];
  for (const r of n) {
    const s = r.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? i[s[1].toLowerCase()] = $r(s[2]) : o.push(r.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: o, attrs: i, content: t };
}
function Se(e, t) {
  return (typeof e == "string" ? ya(e, "").flags : e.flags).includes(t.toLowerCase());
}
function va(e) {
  return [...$n(e).filter((i) => Se(i, "diff") || Se(i, "patch")).map((i) => i.content), e].filter((i, o, r) => gr(i) && r.findIndex((s) => s === i) === o);
}
function gr(e) {
...
`), i = [];
  if (n.forEach((s, l) => {
    (s.startsWith("diff --git ") || s.startsWith("--- ")) && i.push(l);
  }), i.length === 0) return [{ raw: t, ...tn(t) }];
  const o = Array.from(new Set(i)).sort((s, l) => s - l), r = [];
  for (let s = 0; s < o.length; s += 1) {
    const l = o[s], b = o.find((h) => h > l && n[h].startsWith("diff --git ")) ?? n.length, f = n.slice(l, b).join(`
`);
    /^@@\s+-\d+/m.test(f) && r.push({ raw: f, ...tn(f) });
  }
  return r.length > 0 ? r : [{ raw: t, ...tn(t) }];
}
function tn(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), i = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), o = _e(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, r = _e(t?.[3] || t?.[4] || i?.[1] || i?.[2] || "") || null, s = r && r !== "/dev/null" ? r : o;
  return { path: s && s !== "/dev/null" ? s : null, oldPath: o, newPath: r };
}
function _e(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function dn(e) {
  return $n(e).filter((t) => wr(t)).map((t) => ({
    content: Cr(t.content),
...
function wr(e) {
  return Se(e, "atomek-replace") || Se(e, "atomek-full") || Se(e, "full-replacement") || Se(e, "full") || Se(e, "replace");
}
function yr(e) {
...
`).slice(0, 5);
  for (const i of n) {
    const o = i.match(/^\s*(?:\/\/|#|<!--)?\s*(?:atomek-)?(?:path|file)\s*:\s*([^\s>]+)\s*(?:-->)?\s*$/i);
    if (o) return _e(o[1]);
  }
  return null;
}
...
`), o = [];
  ...
  for (let l = 0; l < i.length; l += 1) {
    const p = i[l].match(br);
    if (!p) continue;
    s = !0;
    const b = Number(p[1]), f = Math.max(0, b - 1);
    if (f < r) return null;
    for (o.push(...n.slice(r, f)), r = f, l += 1; l < i.length; l += 1) {
      const h = i[l];
      if (h.startsWith("@@ ")) {
        l -= 1;
        break;
  ...
  return s ? (o.push(...n.slice(r)), o.join(`
`)) : null;
}
function vr(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function xr(e) {
  const { body: t, files: n, sourceTitle: i, activeFile: o = null, versions: r = {} } = e, s = /* @__PURE__ */ new Map(), l = [];
  for (const f of va(t))
    for (const h of kr(f)) {
      if (!h.path) {
        if (o) {
          const E = un(o.content, h.raw);
          E && E !== o.content && s.set(o.id, Tt({
            file: o,
            proposedContent: E,
            sourceTitle: i,
      ...
      const I = un(w.file.content, h.raw);
  ...
  for (const f of dn(t)) {
    const h = f.path ? Wn(n, f.path) : o ? { file: o, normalizedPatchPath: _e(o.path), confidence: "exact" } : null;
    if (!h) {
      l.push(`${f.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (f.content === h.file.content) {
    ...
    s.set(h.file.id, Tt({
      ...
      sourceTitle: i,
      extractionLabel: f.label,
      versions: r,
      targetPath: f.path ?? h.file.path,
      match: h
    }));
  }
  const p = dn(t).some((f) => f.path);
  if (s.size === 0 && o && !p) {
    const f = Ar(t, o);
    f && f.content !== o.content && s.set(o.id, Tt({
      file: o,
      proposedContent: f.content,
      sourceTitle: i,
      extractionLabel: f.label,
  ...
  return {
    ...
    stats: Ir(b.map((f) => f.stats)),
    kind: b.length === 0 ? "empty" : b.length === 1 ? "single-file" : "multi-file"
  };
}
function Ar(e, t) {
  for (const p of va(e)) {
    const b = un(t.content, p);
    if (b) return { content: b, label: "unified diff patch" };
  }
  const n = $n(e);
  if (n.length === 0) return null;
  const i = Mr(t), o = dn(e)[0];
  if (o) return { content: o.content, label: o.label };
  const r = n.find((p) => i.some((b) => Se(p, b)) && !Se(p, "diff") && !Se(p, "patch"));
  if (r) return { content: r.content, label: `matched ${r.lang || t.language} block` };
  const l = n.filter((p) => !Se(p, "diff") && !Se(p, "patch")).sort((p, b) => b.content.length - p.content.length)[0];
  return l ? { content: l.content, label: `largest fenced block (${l.lang || "plain"})` } : null;
}
function Wn(e, t) {
  const n = _e(t), i = n.split("/").at(-1) ?? n, o = e.find((l) => _e(l.path) === n);
  if (o) return { file: o, normalizedPatchPath: n, confidence: "exact" };
  const r = e.find((l) => _e(l.path).endsWith(`/${n}`));
  if (r) return { file: r, normalizedPatchPath: n, confidence: "suffix" };
  const s = e.find((l) => l.name === i || _e(l.path).endsWith(`/${i}`));
...
`), o = Math.max(n.length, i.length);
  let r = 0, s = 0, l = 0;
  for (let p = 0; p < o; p += 1)
    n[p] !== i[p] && (n[p] === void 0 ? r += 1 : i[p] === void 0 ? s += 1 : l += 1);
  return { added: r, removed: s, changed: l };
}
function Ir(e) {
  return e.reduce((t, n) => ({
...
function Tt(e) {
  ...
  return {
    ...
    stats: Nr(e.file.content, e.proposedContent),
    base: {
...
const _t = (e) => typeof e == "object" && e !== null, Pr = (e) => _t(e.ai) ? e.ai : null, Tr = (e) => {
  if (!_t(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, Lr = (e) => {
  const t = Tr(e);
  return !t || !_t(e) ? null : { ...e, id: t };
}, Oe = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Oe) : _t(e) ? Object.values(e).some(Oe) : !1, Or = async (e, t) => {
  const n = Pr(e);
  if (typeof n?.listModels != "function") return [];
  const i = await n.listModels(t);
  return Array.isArray(i) ? i.map(Lr).filter((o) => !!o) : [];
}, Er = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Oe(e.capability) || Oe(e.capabilities) || Oe(e.modality) || Oe(e.modalities) || Oe(e.task) || Oe(e.tasks) || Oe(e.type) || Oe(e.kind), $a = (e) => typeof e == "object" && e !== null, Rr = (e) => $a(e.ai) ? e.ai : null, xa = (e) => typeof Rr(e)?.embedText == "function", jr = async (e, t) => xa(e) ? (await Or(e, t)).filter(Er) : [], Dr = (e) => $a(e.ai) ? xa(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", Fr = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], zr = [
...
function Hr(e, t) {
  const n = Aa(t);
  return !n || e.commands.some((i) => i.command === n) ? e : {
    ...e,
    commands: [
      ...e.commands,
      {
...
function Ur(e) {
  const t = e.results.length > 0 ? e.results.map((i, o) => [
    `Check ${o + 1}: ${i.command}`,
    `Status: ${i.status}`,
    "Output:",
    Yr(i.output || "(no output pasted)", "text")
  ].join(`
`)).join(`

`) : "No manual check output was captured yet.", n = e.commands.length > 0 ? e.commands.map((i) => `- ${i.command}`).join(`
...
function Vr(e) {
  const t = e.filter((i) => i.name === "package.json" || i.path.endsWith("/package.json")), n = [];
  for (const i of t) {
    const o = qr(i.content);
    if (!o?.scripts) continue;
    const r = Na(i.path), s = Kr(e, r, o);
    if (!s) continue;
    const l = Object.keys(o.scripts).filter((b) => typeof o.scripts?.[b] == "string"), p = Gr(l);
    for (const b of p) {
      const f = `${s} run ${b}`;
      n.push({
        id: Ia(`${r}:${f}`),
...
function Kr(e, t, n) {
  if (typeof n.packageManager == "string") {
    const i = n.packageManager.split("@")[0]?.trim();
    if (i) return i;
  }
  for (const [i, o] of zr)
    if (e.some((r) => Xr(r.path) === i && Na(r.path) === t)) return o;
  return null;
}
function Gr(e) {
  const t = Fr.filter((i) => e.includes(i)), n = e.filter((i) => !t.includes(i)).filter((i) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(i)).sort((i, o) => i.localeCompare(o));
  return [...t, ...n];
}
function Jr(e) {
  const t = /* @__PURE__ */ new Set();
...
const Qr = mi(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Zr = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Sa = "tytus.workspace.recent", Ma = "tytus.workspace.layout", Pa = "tytus.atomek.chatAiSettings", Ta = "tytus.atomek.currentMission", hn = "tytus.atomek.currentMissionChanged", Rt = {
...
function nn(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - es), i = n < 1180, o = i ? 200 : 240, r = i ? 300 : 340, s = i ? 420 : 560, l = Math.max(o, Math.min(i ? 340 : 420, Math.floor(n * 0.28))), p = Math.min(300, l), b = Math.floor(n * (i ? 0.34 : 0.36)), f = n - p - s, h = Math.max(r, Math.min(i ? 500 : 640, b, f));
  return { primaryMin: o, primaryMax: l, secondaryMin: r, secondaryMax: h };
}
function ts(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let i = 0, o = 0, r;
  for (; (r = n.exec(e)) !== null; ) {
    if (r.index > i) {
      const l = e.slice(i, r.index);
      l.trim() && t.push({ type: "markdown", body: l, key: `md-${o}` });
    }
    t.push({
      type: "code",
      language: r[1]?.trim() || "text",
      body: r[2] ?? "",
      key: `code-${o}`
    }), i = r.index + r[0].length, o += 1;
  }
  const s = e.slice(i);
  return (s.trim() || t.length === 0) && t.push({ type: "markdown", body: s, key: `md-${o}` }), t;
}
async function st(e) {
  if (!e) return !1;
  try {
...
async function mn(e, t, n) {
  const i = e.getFileHandle;
  if (!i) throw new Error("Selected mission folder is read-only in this browser context");
  const o = await i.call(e, t, { create: !0 });
  if (!o.createWritable) throw new Error(`Cannot write ${t}; File System Access write handle unavailable`);
  const r = await o.createWritable();
...
async function as(e, t, n) {
  ...
  for (const r of i.slice(0, -1))
    o = await ot(o, r);
  await mn(o, i[i.length - 1], n);
}
function La(e) {
  const t = e.reduce((n, i) => (n[i.kind] = (n[i.kind] ?? 0) + 1, n), {});
  return Object.entries(t).map(([n, i]) => `${i} ${n}`).join(" · ") || "no resources";
}
function rt(e, t) {
...
function is(e) {
  const t = e?.resources ?? [], n = (l) => l.status === "ready" || l.status === "available", i = t.filter((l) => l.kind === "pod-agent" && lt(l) === "openclaw"), o = t.filter((l) => l.kind === "pod-agent" && lt(l) === "hermes"), r = t.filter((l) => l.kind === "local-cli" && n(l)), s = t.filter((l) => l.kind === "shared-folder" && n(l));
  return [
    { label: "OpenClaw", value: i.length, detail: "fast pod agents for critique, planning, channel/app workflows", status: i.some(n) ? "ready" : "not allocated" },
    { label: "Hermes", value: o.length, detail: "heavier pod agent family when allocated", status: o.some(n) ? "ready" : "available when installed" },
    { label: "Local agents", value: r.length, detail: "Claude, OpenCode, Codex, pi, Kimi through Tytus tray", status: r.length ? "ready" : "missing" },
    { label: "Shared folders", value: s.length, detail: "mission context and handoff fabric for the whole team", status: s.length ? "ready" : "needs setup" }
  ];
}
function os(e) {
  const t = e?.resources ?? [], n = (i) => t.some((o) => o.kind === i && (o.status === "ready" || o.status === "available"));
  return [
...
function Ge(e, t) {
  return e.find((n) => Oa(n) && t(n)) ?? null;
}
function ze(e, t, n) {
  return Ge(e, (i) => {
    if (i.kind !== t) return !1;
    const o = rs(i);
    return n.some((r) => {
      ...
      if (s.length <= 4) {
        const l = s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        return new RegExp(`(^|[^a-z0-9])${l}([^a-z0-9]|$)`).test(o);
      }
      return o.includes(s);
    });
  });
...
function wt(e, t, n, i, o) {
  return {
    ...
    label: t,
    purpose: n,
    resourceId: i?.id ?? `missing:${e}`,
    resourceLabel: i ? Ce(i) : o,
    status: i?.status ?? "needs-setup",
    trustTier: i?.trustTier ?? "not-available",
    sandbox: i?.sandbox ?? "none"
  };
}
function ct(e, t = "repo-repair") {
  const n = Ct.find((_) => _.id === t) ?? Ct[0], i = e?.resources ?? [], o = Ge(i, (_) => _.kind === "pod-agent" && lt(_) === "openclaw"), r = Ge(i, (_) => _.kind === "pod-agent" && lt(_) === "hermes"), s = Ge(i, (_) => _.kind === "ail-route"), l = Ge(i, (_) => _.kind === "shared-folder"), p = ze(i, "local-cli", ["claude"]), b = ze(i, "local-cli", ["opencode", "open code"]), f = ze(i, "local-cli", ["codex"]), h = ze(i, "local-cli", ["pi"]), w = ze(i, "local-cli", ["kimi"]), I = Ge(i, (_) => _.kind === "local-cli"), E = ze(i, "app-skill", ["juli3ta", "music", "song"]), V = ze(i, "app-skill", ["blender", "3d"]), Z = ze(i, "app-skill", ["remotion", "video", "render"]), U = Ge(i, (_) => _.kind === "app-skill"), oe = t === "pod-local" ? o ?? r ?? s ?? p ?? I : t === "creative-production" ? r ?? s ?? p ?? I : t === "research-watch" ? o ?? s ?? r ?? p ?? I : p ?? b ?? s ?? I, le = t === "creative-production" ? E ?? V ?? Z ?? U ?? b ?? p ?? I : b ?? p ?? f ?? I, O = t === "pod-local" ? f ?? h ?? w ?? o ?? p ?? I : t === "research-watch" ? h ?? w ?? f ?? o ?? I : f ?? h ?? o ?? w ?? I, W = t === "creative-production" ? E ?? V ?? Z ?? U : t === "research-watch" ? ze(i, "app-skill", ["browser", "channel", "telegram"]) ?? U : U, K = [
    wt("planner", "Planner", "Break goal into tasks, risks, and required context.", oe, "No planner agent ready"),
    wt("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", le, "No local/app implementer ready"),
    wt("reviewer", "Reviewer", "Independent critique before approval or handoff.", O, "No reviewer agent ready"),
    wt("team-desk", "Team Desk", "Shared mission folder for transcripts, outputs, proposals, and handoff.", l, "Mission folder only until shared folder is bound")
  ];
  (t === "creative-production" || W) && K.push(wt("app-tool", "App Tool", "Drive installed local app skill through mission context.", W, "No configured app skill"));
  const j = K.filter((_) => _.status === "ready" || _.status === "available" || _.status === "degraded").length, ne = j >= K.length - 1 ? "ready" : j >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: ne, assignments: K };
}
function Ye(e, t, n) {
  if (n && Ct.some((o) => o.id === n)) return n;
  const i = e.toLowerCase();
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(i) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(i) ? "research-watch" : (t?.resources ?? []).some((o) => o.kind === "pod-agent" && Oa(o)) ? "pod-local" : "repo-repair";
}
function ss(e) {
  return Ct.map((t) => ct(e, t.id));
}
function Et(e) {
...
function vt(e) {
  ...
  try {
    localStorage.setItem(Ta, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(hn, { detail: t }));
  } catch {
  }
}
function cs() {
...
function $t(e, t, n) {
  const i = e.trim() || "Coordinate a Tytus mission.", o = ct(t, Ye(i, t, n)), r = (h) => o.assignments.find((w) => w.role === h), s = r("planner"), l = r("implementer"), p = r("reviewer"), b = r("team-desk"), f = r("app-tool");
  return [
    {
      id: "task-scope",
      title: "Scope mission and context",
      prompt: `Planner role: turn this goal into an executable mission plan, list required files/assets, and define approval gates. Goal: ${i}`,
      resourceHint: s ? Et(s) : "Planner agent",
      role: "planner",
      assignedResourceLabel: s?.resourceLabel ?? "Planner agent",
      status: "ready",
    ...
    },
    {
      ...
      prompt: `Implementer role: use the mission folder, selected files, and shared/team desk context to execute the smallest safe step. Return transcript, artifact, or patch proposal only. Goal: ${i}`,
      resourceHint: l ? Et(l) : "Local/app implementer",
      role: "implementer",
    ...
    f ? {
      ...
      prompt: `App-tool role: use the relevant app skill only through configured Tytus/app instructions. Save source assets and outputs into the mission/team desk. Goal: ${i}`,
      resourceHint: Et(f),
      role: "app-tool",
      assignedResourceLabel: f.resourceLabel,
      status: f.status === "ready" || f.status === "available" ? "waiting" : "needs-approval",
      expectedOutputs: ["app output", "asset path", "usage notes"]
    } : null,
    {
      id: "task-handoff",
      title: "Review and hand off",
      prompt: `Reviewer role: independently review the outputs, list approval decisions, and prepare a handoff that another agent can continue from. Goal: ${i}`,
      resourceHint: p ? `${Et(p)} · ${b?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
...
function jt(e, t, n, i, o, r) {
  const s = ct(t, Ye(o || e.goal, t, r ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
    ...
    "## Team preset",
    "",
    `- Preset: ${s.label} (${s.readiness})`,
    `- Best for: ${s.bestFor}`,
    `- Summary: ${s.summary}`,
    "",
    ...s.assignments.map((l) => `- ${l.label}: ${l.resourceLabel} — ${l.purpose} [${l.status}, ${l.trustTier}, ${l.sandbox}]`),
    "",
    "## Current Atomek context",
    "",
    n ? `- Active file: \`${n.path}\` (${n.language}, ${n.content.length} chars${n.dirty ? ", dirty" : ""})` : "- Active file: none",
    `- Open editors: ${i.length}`,
    "",
    "## Current task",
    "",
    o || "(no task prompt set)",
    "",
    "## Resource graph",
    "",
    t ? `- ${La(t.resources)}` : "- not loaded",
...
function ja(e, t, n, i) {
  const o = Ye(n || e.goal, t, i ?? e.teamPresetId), r = ct(t, o), s = $t(n || e.goal, t, o);
  return JSON.stringify({
    ...
    storage: {
      ...
      teamDesk: r.assignments.find((l) => l.role === "team-desk")?.resourceLabel ?? "mission folder",
      paths: {
        runs: "runs/",
        outputs: "outputs/",
        proposals: "proposals/",
...
function us({ host: e }) {
  const t = Xe(null), [n, i] = x(0), o = fe(() => qs(), []), [r, s] = x("computer"), [l, p] = x(o.primaryVisible), [b, f] = x(o.primaryWidth), [h, w] = x("chat"), [I, E] = x(o.secondaryVisible), [V, Z] = x(o.secondaryWidth), [U, oe] = x(!1), [le, O] = x("problems"), [W, K] = x(o.markdownPreviewVisible), [j, ne] = x(!1), [_, je] = x(null), [M, de] = x([]), [be, $e] = x([]), [Y, ge] = x(null), [De, v] = x(""), [$, k] = x({ lineNumber: 1, column: 1 }), [D, H] = x(null), [se, P] = x({}), [he, Ie] = x(ko), [Ve, Me] = x([]), [Te, ue] = x([]), [Q, ae] = x(null), [X, ie] = x(""), [Ht, dt] = x(!1), [He, Bt] = x(""), [xt, Qe] = x([]), [me, Be] = x(null), [c, y] = x(null), [N, T] = x(!1), [S, B] = x(() => Ks()), [G, z] = x(null), [ke, Pe] = x(null), [J, R] = x(""), [ee, ye] = x(""), [pe, xe] = x(""), [Fe, ut] = x("failed"), [At, Nt] = x(null), [Ze, L] = x("Ready"), [ht, Ua] = x(() => Vs()), Ae = be.map((d) => M.find((m) => m.id === d)).filter(Boolean), F = Y ? M.find((d) => d.id === Y) ?? null : null, Nn = fe(() => go({ files: M, openEditorIds: be, activeFileId: Y, versions: se, activeSelection: D }), [Y, D, se, M, be]), In = fe(() => ({
    scope: he,
    removedAttachmentIds: Ve,
    selectedFileIds: []
  }), [he, Ve]), mt = fe(() => Mo(Nn, M, In), [In, Nn, M]), Ue = Qo(M, { autoRefresh: !0, includeDirty: !0 }), Sn = fe(() => Te.map((d) => ({
    id: d.id,
    kind: "index-hit",
    label: d.label,
    path: d.path,
    ...
    snippet: d.snippet
  })), [Te]), Wa = fe(() => [...mt.attachments, ...Sn], [mt.attachments, Sn]), q = po({ host: e, requestContext: mt.parts, chatSettings: S, setStatus: L }), qe = fe(
    () => [...q.artifacts, ...xt].sort((d, m) => m.createdAt - d.createdAt),
    [q.artifacts, xt]
  ), It = N && !F, Ut = !F && !It && !j, ve = fe(() => M.filter((d) => d.dirty), [M]), Va = fe(() => {
    const d = De.trim().toLowerCase();
    return d ? M.filter((m) => m.path.toLowerCase().includes(d)) : M;
  }, [M, De]);
  ce(() => {
    P((d) => {
      const m = {};
      for (const g of M) m[g.id] = d[g.id] ?? 1;
      return m;
    });
  }, [M]), ce(() => {
    Me([]), ue([]);
  ...
  const qa = C((d) => {
    if (d.kind === "index-hit") {
      ue((m) => m.filter((g) => g.id !== d.id)), L(`Removed project context: ${d.label}`);
      return;
    }
    Me((m) => m.includes(d.id) ? m : [...m, d.id]), L(`Removed chat context: ${d.label}`);
  }, []), St = C(() => {
    T(!0), ge(null), ne(!0), L("Atomek settings opened");
  }, []), Mn = C(() => {
    T(!1), ge((d) => d ?? be.at(-1) ?? null), be.length === 0 && ne(!1), L("Atomek settings closed");
  }, [be]), et = C((d) => {
    P((m) => ({ ...m, [d]: (m[d] ?? 1) + 1 }));
  }, []), Ka = C((d) => {
    d.preventDefault(), d.currentTarget.setPointerCapture?.(d.pointerId);
    const m = nn(n), g = d.clientX, A = V, re = (bt) => {
      const Qt = A + (g - bt.clientX);
      Z(Ot(Qt, m.secondaryMin, m.secondaryMax));
    }, Le = () => {
      window.removeEventListener("pointermove", re), window.removeEventListener("pointerup", Le);
    };
    window.addEventListener("pointermove", re), window.addEventListener("pointerup", Le);
  }, [V, n]), Ga = C((d) => {
    d.preventDefault(), d.currentTarget.setPointerCapture?.(d.pointerId);
    const m = nn(n), g = d.clientX, A = b, re = (bt) => {
      const Qt = A + (bt.clientX - g);
      f(Ot(Qt, m.primaryMin, m.primaryMax));
    }, Le = () => {
      window.removeEventListener("pointermove", re), window.removeEventListener("pointerup", Le);
    };
    window.addEventListener("pointermove", re), window.addEventListener("pointerup", Le);
  }, [b, n]), Mt = C((d) => {
    const m = [d, ...ht.filter((g) => g.path !== d.path)].slice(0, 6);
    Ua(m), localStorage.setItem(Sa, JSON.stringify(m));
  }, [ht]), we = C((d, m) => {
    $e((g) => g.includes(d.id) ? g : [...g, d.id]), ge(d.id), ne(!1), Nt(m ?? null), k({ lineNumber: m ?? 1, column: 1 });
  }, []), Ja = C((d) => {
    if (!d.fileId) return;
    const m = M.find((g) => g.id === d.fileId);
    m && (we(m, d.range?.startLineNumber ?? 1), L(`Revealed context: ${d.label}`));
  }, [M, we]), pt = C(async () => {
    if (an(ve, "open new files"))
      try {
        const d = await io();
        if (d.length === 0) return;
        de((m) => Bs(m, d)), d.forEach((m) => Mt({ name: m.name, path: m.path, at: Date.now() })), we(d[0]), L(`Opened ${d.length} local file${d.length === 1 ? "" : "s"}`);
      } catch (d) {
        d.name !== "AbortError" && L(`Open file failed: ${d.message}`);
      }
  }, [ve, we, Mt]), Wt = C(async () => {
    if (an(ve, "open another folder"))
      try {
        const d = await oo();
        je(d), de(d.files), $e([]), ge(null), ne(!1), Mt({ name: d.name, path: d.name, at: Date.now() }), L(`${d.handle ? "Opened local folder" : "Opened browser fallback folder"} ${d.name} (${d.files.length} text files indexed)`);
      } catch (d) {
        d.name !== "AbortError" && L(`Open folder failed: ${d.message}`);
      }
  }, [ve, Mt]), Xa = C((d) => {
    if (!Y) return;
    let m = !1;
    de((g) => g.map((A) => A.id !== Y || A.content === d ? A : (m = !0, { ...A, content: d, dirty: !0 }))), m && et(Y);
  }, [Y, et]), Vt = C(async () => {
    if (F)
      try {
        const d = await Zt(F);
        de((m) => m.map((g) => g.id === d.id ? d : g)), L(`Saved ${d.name}`);
      } catch (d) {
        L(`Save failed: ${d.message}`);
      }
  }, [F]), Ya = C(async (d) => {
    const m = M.find((A) => A.id === d);
    if (!m) return null;
    const g = await Zt(m);
    return de((A) => A.map((re) => re.id === g.id ? g : re)), g;
  }, [M]), Pn = C(async () => {
    const d = M.filter((m) => m.dirty);
    if (d.length === 0) {
      L("No dirty files to save");
      return;
    }
    try {
      const m = await Promise.all(d.map((A) => Zt(A))), g = new Map(m.map((A) => [A.id, A]));
      de((A) => A.map((re) => g.get(re.id) ?? re)), z(null), L(`Saved ${m.length} dirty file${m.length === 1 ? "" : "s"}`);
    } catch (m) {
      L(`Save all failed: ${m.message}`);
    }
  }, [M]), qt = C((d) => {
    const m = M.find((g) => g.id === d);
    if (m?.dirty && !window.confirm(`${m.name} has unsaved changes. Close without saving?`)) {
      L(`Close canceled — ${m.name} has unsaved changes`);
      return;
    }
    $e((g) => {
      const A = g.filter((re) => re !== d);
      return Y === d && (ge(A.at(-1) ?? null), H(null)), A;
    });
  }, [Y, M]), Qa = C(() => {
    an(ve, "close all editors") && ($e([]), ge(null), Nt(null), ne(!1), L("Closed all editors"));
  }, [ve]), Kt = C(() => {
    const d = M.filter((g) => g.name.startsWith("Untitled")).length + 1, m = {
...
`,
    ...
    return ue(A), A.length === 0 ? (L(Ue.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), m) : (g.mode !== "hybrid" && g.reason ? L(g.reason) : L(`${A.length} project context hit${A.length === 1 ? "" : "s"} · ${g.reason ?? "hybrid retrieval"}`), [
      ...m,
      ...rr(A)
    ]);
  }, [mt.parts, he, S, e, Ue]), Gt = C(() => {
    if (!F && Ae.length === 0) {
      L("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const m = [
      `Create a polished Markdown artifact from ${F?.path ?? `${Ae.length} open editors`}.`,
    ...
    E(!0), w("chat"), L("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const g = await We(m), A = await q.askAgent(m, { requestContext: g });
      !A || A.status === "error" || q.createArtifact({
        messageId: A.id,
        title: `AI synthesis — ${F?.name ?? "open workspace"}`,
        kind: "markdown",
        body: A.body
      }).then(() => {
        E(!0), w("outputs"), oe(!0), O("output");
      });
    })();
  }, [F, q, We, Ae.length]), Jt = C((d) => {
    const m = M.find((g) => g.path === d.path || g.name === d.name);
    if (m) {
      we(m), L(`Opened recent ${m.name}`);
      return;
    }
    L("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [M, we]), Ne = C((d) => {
    E(!0), w("chat"), ie(""), (async () => {
      const m = await We(d);
      await q.askAgent(d, { requestContext: m });
    })();
  }, [q, We]), Tn = C(async (d) => {
    if (!e.skills?.get) {
      L("Tytus skill registry is not available in this host build");
      return;
    }
    try {
      const m = await e.skills.get(d.id), g = m.body.length > 4500 ? `${m.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : m.body, A = [
        `Use Tytus skill "${m.title}" (${m.id}).`,
        `Driver: ${m.driver}. Source: ${m.source}. Status: ${m.status}.`,
...
`);
      ie((re) => [re.trim(), A].filter(Boolean).join(`

`)), E(!0), w("chat"), L(`Attached skill ${m.title} to chat input`);
    } catch (m) {
      L(`Failed to attach skill: ${m instanceof Error ? m.message : String(m)}`);
    }
  }, [e.skills]), Za = C((d) => {
    const m = q.messages.findIndex((A) => A.id === d.id), g = q.messages.slice(0, m < 0 ? void 0 : m).reverse().find((A) => A.role === "user");
    if (!g?.body.trim()) {
      L("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const A = await We(g.body);
      await q.askAgent(g.body, { requestContext: A });
    })();
  }, [q, We]), ei = C((d) => {
    q.createArtifact({
      messageId: d.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    if (!F) {
      L("No active file to save as AI artifact");
      return;
    }
    q.createArtifact({
      title: F.path,
      kind: F.language === "markdown" ? "markdown" : "report",
      body: F.content
    }).then(() => {
      E(!0), w("outputs"), oe(!0), O("output");
    });
  }, [F, q]), Xt = C((d) => {
    const m = rn(M, on(d.title || d.kind)), g = {
      id: `artifact-file-${d.id}-${Date.now()}`,
      name: m,
      path: m,
      language: "markdown",
      content: d.body,
    ...
    de((A) => [...A, g]), we(g), L(`Opened ${d.title} as editable file`);
  }, [M, we]), Yt = C(() => {
    const d = window.prompt("Check command/name (manual capture only; Atomek will not execute it)");
    if (d === null) return;
    const m = d.trim();
    if (!m) {
      L("Manual check capture canceled — command/name was empty");
      return;
    }
    const g = window.prompt("Paste check output/result");
    if (g === null) return;
    const A = [
      "# Manual check result",
      "",
      `- Captured: ${(/* @__PURE__ */ new Date()).toISOString()}`,
      `- Command/name: \`${m.replace(/`/g, "\\`")}\``,
...
`), re = {
      id: `manual-check-${Date.now()}`,
      title: `Manual check — ${m.slice(0, 80)}`,
      kind: "report",
      body: A,
      createdAt: Date.now(),
      source: "local"
    };
    Qe((Le) => [re, ...Le]), oe(!0), O("output"), L(`Captured manual check: ${m}`);
  }, []), tt = C((d, m) => {
    const g = xr({
      body: m,
      files: M,
      ...
      edits: g.edits.map(Us),
      skipped: g.skipped
    };
    return g.edits.length > 1 ? (y(A), Be(null), L(`Previewing AI workspace patch for ${g.edits.length} files`), !0) : g.edits.length === 1 ? (Be(A.edits[0]), y(null), L(`Previewing AI patch for ${A.edits[0].fileName}`), !0) : (L(g.skipped.length > 0 ? `No applicable edit found. ${g.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [F, se, M]), Ln = C((d, m) => {
    const g = {
    ...
    Qe((A) => [g, ...A]), E(!0), w("outputs"), oe(!0), O("output"), L(`Saved local job output: ${d}`), (m.includes("```diff") || m.includes("--- a/")) && tt(d, m);
  }, [tt]), ai = C(() => {
    const d = X.trim();
    if (!d) return;
    ie("");
    const m = Ps(d);
    (async () => {
      const g = await We(d), A = await q.askAgent(d, { requestContext: g });
      if (!m || !A || A.status === "error") return;
      tt(A.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", A.body) || L("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [q, We, X, tt]), ii = C(() => {
    Q && (ae(null), Ne(Ts(Q)));
  }, [Ne, Q]), ft = C((d) => {
    const m = _r(M, d);
    Pe(m);
    const g = m.commands[0]?.command ?? "";
    xe(g), R(""), ye(""), ut("failed"), oe(!0), O("terminal"), L(m.commands.length > 0 ? `Manual check ready: copy ${m.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [M]), oi = C((d) => {
    d.trim() && (st(d), xe(d), L(`Manual check command copied: ${d}`));
  }, []), ri = C(() => {
    Pe((d) => {
      if (!d) return d;
      const m = Hr(d, J), g = m.commands.at(-1)?.command ?? "";
      return g && xe(g), m;
    }), R("");
  }, [J]), si = C(() => {
    if (!ke || !pe.trim()) {
      L("Select or enter a manual check command before capturing output");
      return;
    }
    const d = pe.trim();
    Pe((m) => m && Br(m, d, Fe, ee)), ye(""), L(`Captured manual check result: ${d} (${Fe})`);
  }, [ee, pe, ke, Fe]), li = C(() => {
    ke && Ne(Ur(ke));
  }, [Ne, ke]), ci = C(() => {
    if (!me) return;
    const d = M.find((m) => m.id === me.fileId);
    if (!d) {
      L(`Cannot apply edit — ${me.fileName} is no longer open`), Be(null);
      return;
    }
    d.content !== me.originalContent && !window.confirm(`${me.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (de((m) => m.map((g) => g.id === me.fileId ? { ...g, content: me.proposedContent, dirty: !0 } : g)), et(me.fileId), z(`AI edit applied to ${d.name}. Save All to persist it to disk.`), Be(null), ft(`AI edit applied to ${d.name}`), L(`Applied AI edit to ${d.name} — unsaved; manual check ready`));
  }, [et, M, me, ft]), di = C(() => {
    if (!me) return;
    const d = rn(M, `${on(me.fileName)}-ai-edit`), m = {
      id: `pending-edit-${me.fileId}-${Date.now()}`,
      name: d,
      path: d,
      language: F?.language ?? "markdown",
      content: me.proposedContent,
      dirty: !0,
      source: "generated"
    };
    de((g) => [...g, m]), we(m), Be(null), L(`Opened proposed edit as ${d}`);
  }, [F?.language, M, we, me]), ui = C(() => {
    if (!c) return;
    const d = new Map(c.edits.map((g) => [g.fileId, g]));
    M.some((g) => {
      const A = d.get(g.id);
      return A && g.content !== A.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (de((g) => g.map((A) => {
      const re = d.get(A.id);
      return re ? { ...A, content: re.proposedContent, dirty: !0 } : A;
    })), d.forEach((g, A) => et(A)), z(`AI workspace patch applied to ${d.size} file${d.size === 1 ? "" : "s"}. Save All to persist changes.`), y(null), ft(`AI workspace patch applied to ${d.size} file${d.size === 1 ? "" : "s"}`), L(`Applied AI workspace patch to ${d.size} file${d.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [et, M, c, ft]), hi = C(() => {
    if (!c) return;
    const d = [...M], m = c.edits.map((g) => {
      const A = rn(d, `${on(g.fileName)}-ai-edit`), re = M.find((bt) => bt.id === g.fileId), Le = {
        id: `workspace-patch-${g.fileId}-${Date.now()}-${A}`,
        name: A,
        path: A,
        language: re?.language ?? "markdown",
    ...
    de((g) => [...g, ...m]), m[0] && we(m[0]), y(null), L(`Opened ${m.length} proposed edit file${m.length === 1 ? "" : "s"}`);
  }, [M, we, c]), On = C((d) => {
    if (d === "explain") {
      Ne("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (d === "improve") {
      Ne("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (d === "plan") {
      Ne("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (d === "edit") {
      Ne("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    Ne("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [Ne]);
  return ce(() => {
    const d = (m) => {
      (m.metaKey || m.ctrlKey) && (m.key.toLowerCase() === "o" && (m.preventDefault(), pt()), m.key.toLowerCase() === "s" && (m.preventDefault(), Vt()), m.key.toLowerCase() === "w" && (m.preventDefault(), Y && qt(Y)), m.key.toLowerCase() === "b" && (m.preventDefault(), p((A) => !A)), m.shiftKey && m.key.toLowerCase() === "f" && (m.preventDefault(), s("search"), p(!0)), (m.key.toLowerCase() === "k" || m.key.toLowerCase() === "p") && (m.preventDefault(), dt(!0)));
    };
    return window.addEventListener("keydown", d), () => window.removeEventListener("keydown", d);
  }, [Y, qt, pt, Vt]), ce(() => {
    const d = (m) => {
      ve.length !== 0 && (m.preventDefault(), m.returnValue = "");
    };
    ...
    const d = t.current;
    if (!d) return;
    const m = () => i(Math.round(d.getBoundingClientRect().width));
    m();
    ...
    if (!n) return;
    const d = nn(n);
    f((m) => Ot(m, d.primaryMin, d.primaryMax)), Z((m) => Ot(m, d.secondaryMin, d.secondaryMax));
  }, [n]), ce(() => {
    const d = { primaryVisible: l, primaryWidth: b, secondaryVisible: I, secondaryWidth: V, markdownPreviewVisible: W };
    localStorage.setItem(Ma, JSON.stringify(d));
  }, [W, l, b, I, V]), ce(() => {
    ...
    {
      ref: t,
      className: `workbench-workbench ${l ? "" : "no-primary"} ${I ? "" : "no-secondary"} ${U ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${b}px`, "--workbench-secondary-width": `${V}px` },
      children: [
        /* @__PURE__ */ a(hs, { active: r, setActive: (d) => {
          s(d), p(!0);
        }, openSettings: St, settingsActive: It }),
        l && /* @__PURE__ */ u("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            ms,
            {
              host: e,
              ...
              setStatus: L,
              hasFsAccess: ao(),
              attachSkillToChat: Tn,
              saveLocalJobOutput: Ln,
              activeFile: F
            }
          ),
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: Ga, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ u("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => dt(!0), children: "Workspace" }),
          /* @__PURE__ */ u("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              gs,
              {
                ...
                closeSettings: Mn,
                secondaryVisible: I,
                toggleSecondary: () => E((d) => !d),
                canPreview: F?.language === "markdown",
                previewVisible: W,
                togglePreview: () => K((d) => !d)
              }
            ),
            /* @__PURE__ */ a(bs, { file: F, folder: _, showWelcome: Ut }),
            G && ve.length > 0 ? /* @__PURE__ */ u("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: G }),
              /* @__PURE__ */ a("button", { onClick: () => {
                Pn();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => z(null), title: "Dismiss", children: /* @__PURE__ */ a(Re, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: F ? /* @__PURE__ */ u("div", { className: F.language === "markdown" && W ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(pi, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                Qr,
                {
                  file: F,
                  revealLine: At,
              ...
              F.language === "markdown" && W && /* @__PURE__ */ a(xs, { content: F.content })
            ] }) : It ? /* @__PURE__ */ a(
              Ls,
              {
                host: e,
            ...
            ) : Ut ? /* @__PURE__ */ a(ys, { host: e, openFile: pt, openFolder: Wt, newFile: Kt, recent: ht, reopenRecent: Jt, setStatus: L, openControlTower: () => {
              s("computer"), p(!0);
            }, openChat: () => {
              w("chat"), E(!0);
            } }) : /* @__PURE__ */ u("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(zt, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(!1), children: "Show Agent Team" })
            ] }) }),
            U && /* @__PURE__ */ a(
              Ns,
              {
                ...
                runAiSynthesis: Gt,
                captureManualCheck: Yt,
                openOutputAsFile: Xt,
                manualCheckSession: ke,
                manualCheckCommandInput: J,
                setManualCheckCommandInput: R,
                manualCheckOutputInput: ee,
                setManualCheckOutputInput: ye,
                manualCheckSelectedCommand: pe,
                setManualCheckSelectedCommand: xe,
                manualCheckStatus: Fe,
                setManualCheckStatus: ut,
                copyManualCheckCommand: oi,
                addManualCheckCommand: ri,
                recordManualCheckResult: si,
                askAgentFromManualChecks: li,
                onClose: () => oe(!1)
              }
            )
          ] })
        ...
        I && /* @__PURE__ */ a(
          Is,
          {
            ...
            saveMessageAsArtifact: ei,
            rememberMessage: ti,
            previewEditFromMessage: (d) => tt(d.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", d.body),
            runQuickPrompt: On,
            pendingPatchPrompt: Q,
            generatePatchPrompt: ii,
            ...
            captureManualCheck: Yt,
            openOutputAsFile: Xt,
            previewEditFromOutput: (d) => tt(d.title, d.body),
            ...
            contextScope: he,
            setContextScope: Ie,
            contextAttachments: Wa,
            removeContextAttachment: qa,
            revealContextAttachment: Ja,
            projectIndexSummary: `${Ue.snapshot.files.length} files · ${Ue.snapshot.chunks.length} chunks`,
            projectIndexStale: Ue.isStale,
            refreshProjectIndex: () => {
              const d = Ue.refresh(M);
              L(`Project index refreshed: ${d.files.length} files · ${d.chunks.length} chunks`);
            },
            onResizeStart: Ka,
            onClose: () => E(!1)
          }
        ...
        Ht && /* @__PURE__ */ a(
          $s,
          {
            ...
            activeFile: F,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: Kt },
              ...
              ...ht.map((d) => ({ label: `File: Open Recent — ${d.name}`, detail: d.path, run: () => Jt(d) })),
              { label: "File: Save All", detail: `${ve.length} dirty file${ve.length === 1 ? "" : "s"}`, run: () => {
                Pn();
              }, disabled: ve.length === 0 },
              { label: "File: Close All Editors", detail: `${Ae.length} open editor${Ae.length === 1 ? "" : "s"}`, run: Qa, disabled: Ae.length === 0 },
              ...
              { label: "Help: Show Agent Team", detail: "Open the Atomek Resource Fabric landing page", run: () => {
                ge(null), ne(!1), s("computer"), p(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: l ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => p((d) => !d) },
              { label: "View: Toggle Chat Panel", detail: I ? "Hide right AI side bar" : "Show right AI side bar", run: () => E((d) => !d) },
              { label: "View: Toggle Bottom Panel", detail: U ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => oe((d) => !d) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: St },
              ...
              { label: "View: Toggle Markdown Preview", detail: F?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => K((d) => !d), disabled: F?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: F || Ae.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Gt, disabled: !F && Ae.length === 0 },
              { label: "AI: Explain Active File", detail: F ? `Ask Cortex to explain ${F.path}` : "Open a file first", run: () => Ne("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !F },
              { label: "AI: Improve Active File", detail: F ? `Ask Cortex for concrete edits to ${F.path}` : "Open a file first", run: () => Ne("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !F },
              { label: "AI: Draft Editable Replacement", detail: F ? `Ask Cortex for a full-file replacement for ${F.path}` : "Open a file first", run: () => On("edit"), disabled: !F },
              { label: "AI: Plan Workspace Work", detail: Ae.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => Ne("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: Ae.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: F ? "Persist active file in host.ai artifacts" : "Open a file first", run: ni, disabled: !F },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Yt },
              { label: "AI: Open Latest Artifact as File", detail: qe[0] ? `Create editable file from ${qe[0].title}` : "No outputs yet", run: () => qe[0] && Xt(qe[0]), disabled: qe.length === 0 }
            ],
            openWorkbenchFile: we,
            onClose: () => dt(!1)
          }
        ...
        /* @__PURE__ */ a(Hs, { status: Ze, file: F ?? Zr, cursor: $, fileCount: M.length, dirtyCount: ve.length })
      ]
    }
  );
}
function hs({ active: e, setActive: t, openSettings: n, settingsActive: i }) {
  return /* @__PURE__ */ u("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(ea, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Ki, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(na, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Qn, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(fn, { size: 25 }), label: "Agent Team", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Ni, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Yi, { size: 23 }), label: "Settings", active: i, onClick: n })
  ] });
}
function Ke({ icon: e, label: t, active: n, onClick: i }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: i, children: e });
}
function ms(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(vs, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(Jn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(Jn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(Ba, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(ps, { ...e });
}
function ps(e) {
  const t = !e.folder, n = fe(() => fs(e.files, e.folder?.name), [e.files, e.folder?.name]), [i, o] = x(() => /* @__PURE__ */ new Set());
  ce(() => {
    const s = Da(n);
    o((l) => {
      const p = new Set(l);
      return s.forEach((b) => p.add(b)), p;
    });
  }, [n]);
  const r = C((s) => {
    o((l) => {
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ u(Ee, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((s) => /* @__PURE__ */ a(An, { file: s, active: s.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(s), label: s.name, detail: s.path }, s.id)),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(Ft, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : Fa(n, e.activeFileId, e.openWorkbenchFile, i, r)
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((s) => /* @__PURE__ */ u("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(s), children: [
...
function An({ file: e, active: t, onOpen: n, basePath: i, depth: o = 0, label: r, detail: s }) {
  const l = i && e.path.startsWith(`${i}/`) ? e.path.slice(i.length + 1) : e.path, p = o || Math.max(0, l.split("/").length - 1);
  return /* @__PURE__ */ u("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": p }, onClick: n, title: e.path, children: [
    /* @__PURE__ */ a(Zn, { size: 14 }),
    /* @__PURE__ */ u("span", { className: "workbench-row-text", children: [
      /* @__PURE__ */ a("span", { className: "workbench-row-name", children: r ?? l }),
...
function fs(e, t) {
  const n = [], i = /* @__PURE__ */ new Map(), o = (s, l, p = n) => {
    const b = i.get(s);
    if (b) return b;
    const f = { name: l, path: s, children: [] };
    return i.set(s, f), p.push(f), f;
  };
  e.forEach((s) => {
    const l = t && s.path.startsWith(`${t}/`) ? s.path.slice(t.length + 1) : s.path, p = l.split("/").filter(Boolean);
    let b = n, f = "";
    p.slice(0, -1).forEach((h) => {
      f = f ? `${f}/${h}` : h, b = o(f, h, b).children;
    }), b.push({ name: p.at(-1) ?? s.name, path: l, file: s, children: [] });
  });
  const r = (s) => s.sort((l, p) => +!!l.file - +!!p.file || l.name.localeCompare(p.name)).map((l) => ({ ...l, children: r(l.children) }));
  return r(n);
}
function Da(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...Da(t.children)]);
}
function Fa(e, t, n, i, o, r = 0) {
  return e.map((s) => {
    if (s.file)
      return /* @__PURE__ */ a(An, { file: s.file, active: s.file.id === t, onOpen: () => n(s.file), depth: r, label: s.name }, s.file.id);
    const l = i.has(s.path);
    return /* @__PURE__ */ u("div", { children: [
      /* @__PURE__ */ u("button", { className: "workbench-folder-row", style: { "--workbench-depth": r }, onClick: () => o(s.path), title: l ? `Collapse ${s.name}` : `Expand ${s.name}`, children: [
        l ? /* @__PURE__ */ a(Ft, { size: 12 }) : /* @__PURE__ */ a(Ft, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ a(kn, { size: 14 }) : /* @__PURE__ */ a(ta, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: s.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: s.children.length })
      ] }),
      l ? Fa(s.children, t, n, i, o, r + 1) : null
    ] }, s.path);
  });
}
function bs({ file: e, folder: t, showWelcome: n }) {
  const i = n ? ["Agent Team"] : e?.path.split("/").filter(Boolean) ?? [], o = t && i[0] === t.name ? i.slice(1) : i;
  return /* @__PURE__ */ a("div", { className: "workbench-breadcrumb", children: o.length === 0 ? /* @__PURE__ */ a("span", { children: "Workspace" }) : o.map((r, s) => /* @__PURE__ */ u("span", { className: "workbench-breadcrumb-part", children: [
...
function gs(e) {
  return /* @__PURE__ */ u("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ u("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ a(Re, { size: 13 }) })
    ] }),
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ a(bn, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(Hi, { size: 16 }) })
  ] });
}
function ks(e) {
  const t = e?.resources ?? [], n = (o) => t.filter((r) => r.kind === o).length, i = t.filter((o) => o.status === "ready" || o.status === "available").length;
  return [
    ...
    { label: "Ready resources", value: i, detail: "usable now" }
  ];
}
function ws(e) {
  const t = {
  ...
  return [...e?.resources ?? []].sort((n, i) => (t[n.kind] ?? 9) - (t[i.kind] ?? 9) || Ce(n).localeCompare(Ce(i))).slice(0, 8);
}
function ys({
  host: e,
  openFile: t,
  ...
  const [b, f] = x("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [h, w] = x(null), [I, E] = x([]), [V, Z] = x(!1), [U, oe] = x(null), [le, O] = x(null), [W, K] = x("pod-local"), j = C(async () => {
    Z(!0), O(null);
    try {
      ...
      w(v ?? null), E($), v && s(`Agent team loaded · ${v.resources.length} resources · ${v.warnings.length} warnings`);
    } catch (v) {
      const $ = v instanceof Error ? v.message : String(v);
      O($), s(`Agent team resource load failed: ${$}`);
    } finally {
      Z(!1);
    }
  }, [e.missions, e.resources, s]), ne = C((v, $) => {
    try {
      e.windows.openOrFocus(v), s(`Opened ${$}`);
    } catch (k) {
      s(`Open ${$} failed: ${k instanceof Error ? k.message : String(k)}`);
    }
  }, [e.windows, s]);
  ce(() => {
    j();
  ...
  const _ = C(async () => {
    if (!e.missions?.create || !e.missions?.write) {
      s("Mission API unavailable in this Tytus host build"), l();
      return;
    }
    const v = b.trim() || "Coordinate a Tytus mission.", $ = Ye(v, h, W);
    Z(!0);
    try {
      const k = await e.missions.create({
        ...
        goal: k.goal,
        rootPath: k.rootPath,
        name: k.rootPath.split("/").pop() || k.missionId,
        source: "tray",
        teamPresetId: $
      }, H = {
      ...
      await e.missions.write({
        rootPath: k.rootPath,
        files: [
          { path: "MISSION.md", content: jt(D, h, null, [], v, $) },
          { path: "MISSION.json", content: ja(D, h, v, $) },
          { path: "RESOURCES.md", content: Dt(h) },
          { path: "TASKS.md", content: Ea($t(v, h, $)) },
          { path: "HANDOFF.md", content: Ra(D) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(H)}
` },
...
`) }
      ...
      }), oe(k), vt(D), s(`Mission created: ${k.rootPath}`), l();
    } catch (k) {
      s(`Mission create failed: ${k instanceof Error ? k.message : String(k)}`);
    } finally {
      Z(!1);
    }
  }, [b, h, e.missions, l, s, W]), je = ks(h), M = is(h), de = os(h), be = ws(h), $e = h?.warnings ?? [], Y = ss(h), ge = ct(h, Ye(b, h, W)), De = $t(b, h, ge.id);
  return /* @__PURE__ */ u("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ u("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ a("div", { className: "workbench-control-kicker", children: "Atomek Agent Team" }),
      /* @__PURE__ */ a("h1", { children: "OpenClaw, Hermes, local agents, one mission." }),
      /* @__PURE__ */ a("p", { children: "Atomek is the Tytus resource fabric cockpit: OpenClaw/Hermes pods, local AI CLIs, shared folders, local apps, chat, files, outputs, and approvals around one durable mission folder." }),
      /* @__PURE__ */ u("div", { className: "workbench-control-goal-row", children: [
        /* @__PURE__ */ a("textarea", { value: b, onChange: (v) => f(v.target.value), rows: 3, "aria-label": "Mission goal" }),
        /* @__PURE__ */ u("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            _();
          }, disabled: V, children: "Start mission" }),
      ...
      /* @__PURE__ */ a("div", { className: "workbench-team-preset-strip", "aria-label": "Team presets", children: Y.map((v) => /* @__PURE__ */ u(
        "button",
        {
          className: `workbench-team-preset-card ${v.id === ge.id ? "active" : ""} ${v.readiness}`,
          onClick: () => K(v.id),
          children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: v.label }),
              /* @__PURE__ */ a("span", { children: v.readiness })
            ] }),
            /* @__PURE__ */ a("p", { children: v.summary }),
            /* @__PURE__ */ a("small", { children: v.assignments.map(($) => $.resourceLabel).slice(0, 3).join(" · ") })
          ]
        },
        v.id
      )) }),
      U ? /* @__PURE__ */ u("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: U.rootPath })
      ] }) : null,
      le ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: le }) : null
    ] }),
    /* @__PURE__ */ u("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: je.map((v) => /* @__PURE__ */ u("div", { className: "workbench-control-metric", children: [
          /* @__PURE__ */ a("strong", { children: v.value }),
          /* @__PURE__ */ a("span", { children: v.label }),
          /* @__PURE__ */ a("em", { children: v.detail })
        ] }, v.label)) })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ u("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-agent-brand-grid", children: M.map((v) => /* @__PURE__ */ u("div", { className: `workbench-agent-brand-card ${v.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ u("div", { children: [
            /* @__PURE__ */ a("strong", { children: v.label }),
            /* @__PURE__ */ a("span", { children: v.status })
          ] }),
          /* @__PURE__ */ a("b", { children: v.value }),
          /* @__PURE__ */ a("p", { children: v.detail })
        ] }, v.label)) })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ u("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: de.map((v, $) => /* @__PURE__ */ u("div", { className: `workbench-fabric-node ${v.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ a("span", { children: $ + 1 }),
          /* @__PURE__ */ a("strong", { children: v.label }),
          /* @__PURE__ */ a("em", { children: v.status }),
          /* @__PURE__ */ a("p", { children: v.detail })
        ] }, v.label)) }),
        /* @__PURE__ */ u("div", { className: "workbench-fabric-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Lt.sharedFiles, "Shared Files"), children: "Open shared files" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Lt.podInspector, "Pod Inspector"), children: "Open pods" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Lt.channels, "Channels"), children: "Open channels" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ne(Lt.settings, "Agent Settings"), children: "Open agent settings" })
        ] })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        ...
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => f("Review this repo with local OpenCode/Claude, then ask Codex or pi for an independent review. Save all transcripts and proposed patches in the mission folder."), children: "Review + patch repo" }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => f("Ask OpenClaw for independent critique, use Hermes when allocated for deep planning/copy, then run local Claude/OpenCode/Codex for implementation. Keep shared context in the mission folder and approval-gate all edits."), children: "OpenClaw/Hermes + local agent" }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => f("Coordinate creative output: script, audio, visuals, render assets, and final handoff through app skills and shared folders."), children: "Creative production" })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ u("header", { children: [
        ...
        I.slice(0, 4).map((v) => /* @__PURE__ */ u("button", { className: "workbench-control-preset", onClick: () => {
          vt(v), l();
        }, title: v.rootPath, children: [
          v.title,
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        ...
        /* @__PURE__ */ u("button", { className: "workbench-start-link", onClick: t, children: [
          /* @__PURE__ */ a(ea, { size: 18 }),
          "Open file"
        ] }),
        /* @__PURE__ */ u("button", { className: "workbench-start-link", onClick: i, children: [
          /* @__PURE__ */ a(gn, { size: 18 }),
          "New mission note"
        ] }),
        o.length ? /* @__PURE__ */ u("div", { className: "workbench-control-recent", children: [
          /* @__PURE__ */ a("span", { children: "Recent" }),
          o.slice(0, 3).map((v) => /* @__PURE__ */ a("button", { onClick: () => r(v), children: v.name }, `${v.path}-${v.at}`))
        ] }) : null
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ u("header", { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-control-resource-list", children: [
          be.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          be.map((v) => /* @__PURE__ */ u("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: Ce(v) }),
              /* @__PURE__ */ a("span", { children: xn(v) })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${v.status}`, children: v.status })
          ] }, v.id))
        ] }),
        $e.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: $e.slice(0, 2).map((v) => /* @__PURE__ */ u("span", { children: [
          v.code,
          ": ",
          v.message
        ] }, `${v.code}-${v.resourceId ?? v.message}`)) }) : null
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ a("header", { children: /* @__PURE__ */ a("strong", { children: "Recommended task graph" }) }),
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: De.map((v, $) => /* @__PURE__ */ u("button", { className: `workbench-task-card ${v.status}`, onClick: () => f(v.prompt), children: [
          /* @__PURE__ */ a("span", { children: $ + 1 }),
          /* @__PURE__ */ a("strong", { children: v.title }),
          /* @__PURE__ */ a("em", { children: v.resourceHint }),
          /* @__PURE__ */ a("small", { children: v.assignedResourceLabel })
        ] }, v.id)) })
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
...
function vs({ files: e, query: t, setQuery: n, openWorkbenchFile: i, activeFileId: o }) {
  const r = fe(() => Cs(e, t), [e, t]), s = fe(() => {
    const l = /* @__PURE__ */ new Map();
    return r.forEach((p) => l.set(p.file.id, [...l.get(p.file.id) ?? [], p])), Array.from(l.values()).slice(0, 50);
  }, [r]);
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      ...
      /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
        /* @__PURE__ */ a(zt, { size: 12 }),
        " Results"
      ] }),
      t.trim() ? s.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : s.map((l) => {
        const p = l[0].file;
        return /* @__PURE__ */ u("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(An, { file: p, active: p.id === o, onOpen: () => i(p) }),
          l.slice(0, 5).map((b) => /* @__PURE__ */ u("button", { className: "workbench-search-hit", onClick: () => i(p, b.lineNumber), title: b.line, children: [
...
function Cs(e, t) {
  ...
  const i = [];
  return e.forEach((o) => {
    o.path.toLowerCase().includes(n) && i.push({ file: o, lineNumber: 1, line: o.path }), o.content.split(`
`).some((r, s) => r.toLowerCase().includes(n) ? (i.push({ file: o, lineNumber: s + 1, line: r.trim() || "(blank line)" }), i.length >= 200) : !1);
  }), i.slice(0, 200);
}
function $s(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${ra(o.language)} file`,
    run: () => e.openWorkbenchFile(o)
  })), i = [...e.commands, ...n].filter((o) => !t || o.label.toLowerCase().includes(t) || o.detail.toLowerCase().includes(t)).slice(0, 12);
  return ce(() => {
    const o = (r) => {
      r.key === "Escape" && e.onClose();
    };
    ...
    /* @__PURE__ */ a(
      "input",
      {
        ...
        autoFocus: !0,
        value: e.query,
        onChange: (o) => e.setQuery(o.target.value),
        placeholder: "Type a command or file name...",
        onKeyDown: (o) => {
          o.key === "Enter" && i[0] && !i[0].disabled && (i[0].run(), e.onClose());
        }
      }
    ),
    /* @__PURE__ */ a("div", { className: "workbench-command-list", children: i.map((o) => /* @__PURE__ */ u(
      "button",
      {
        disabled: o.disabled,
        ...
        children: [
          ...
          /* @__PURE__ */ a("small", { children: o.detail })
        ]
      },
      `${o.label}-${o.detail}`
    )) })
...
function As(e) {
  const t = e.session ? Wr(e.session) : "pending";
  return /* @__PURE__ */ u("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ u(Ee, { children: [
      ...
      e.session.results.length > 0 ? /* @__PURE__ */ u("div", { className: "workbench-check-results", children: [
        ...
        e.session.results.map((n, i) => /* @__PURE__ */ u("article", { children: [
...
function Ns(e) {
  return /* @__PURE__ */ u("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ u("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ a("span", {}),
      /* @__PURE__ */ a("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ a(Re, { size: 14 }) })
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(Ha, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Is(e) {
  return /* @__PURE__ */ u("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ u("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(Wi, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(Mi, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(Re, { size: 15 }) })
      ] })
    ] }),
    ...
    ) : /* @__PURE__ */ a(Ha, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function za({ body: e }) {
  const t = fe(() => ts(e), [e]), [n, i] = x(null), o = C((r, s) => {
    (async () => await st(s) && (i(r), window.setTimeout(() => i((p) => p === r ? null : p), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((r) => {
    if (r.type === "code") {
      const s = n === r.key;
      return /* @__PURE__ */ u("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ u("div", { className: "workbench-rich-code-head", children: [
          /* @__PURE__ */ a("span", { children: r.language }),
          /* @__PURE__ */ u("button", { onClick: () => o(r.key, r.body), title: "Copy code block", children: [
            s ? /* @__PURE__ */ a($i, { size: 12 }) : /* @__PURE__ */ a(ln, { size: 12 }),
            s ? "Copied" : "Copy"
          ] })
        ] }),
        /* @__PURE__ */ a("pre", { children: /* @__PURE__ */ a("code", { children: r.body }) })
      ] }, r.key);
    }
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: ha(r.body) }
      },
      r.key
    );
  }) });
...
function Ss(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Xe(null), [i, o] = x(!0), [r, s] = x(!1), l = fe(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  ce(() => {
    const h = n.current;
    h && (i ? (h.scrollTop = h.scrollHeight, s(!1)) : s(!0));
  }, [i, l]);
  const p = C(() => {
    const h = n.current;
    if (!h) return;
    ...
    const h = n.current;
    h && (h.scrollTop = h.scrollHeight, o(!0), s(!1));
  }, []), f = C((h) => {
    st(h.body);
  }, []);
  return /* @__PURE__ */ u("div", { className: "workbench-chat-wrap", children: [
      ...
      /* @__PURE__ */ u(
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ u("option", { value: h.id, children: [
              h.title,
              " · ",
              Ms(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ u("div", { ref: n, className: "workbench-chat-transcript", onScroll: p, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ u("div", { children: [
      ...
      ] }) }) : e.chatMessages.map((h) => /* @__PURE__ */ u("div", { className: `workbench-chat-message ${h.role}`, children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(za, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ u(Ee, { children: [
          /* @__PURE__ */ a("br", {}),
          /* @__PURE__ */ a("small", { children: h.gatewayLabel })
        ] }) : null,
        h.role === "assistant" && h.status !== "streaming" && h.status !== "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => f(h), title: "Copy this answer", children: [
            /* @__PURE__ */ a(ln, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => e.saveMessageAsArtifact(h), title: "Save this answer as an output artifact", children: [
            /* @__PURE__ */ a(gn, { size: 12 }),
            " Save"
          ] }),
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => e.rememberMessage(h), title: "Store this answer in Atomek memory", children: [
            /* @__PURE__ */ a(na, { size: 12 }),
            " Remember"
          ] }),
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => e.previewEditFromMessage(h), disabled: e.workspaceFileCount === 0, title: "Preview an editable patch from this answer", children: [
            /* @__PURE__ */ a(bn, { size: 12 }),
            " Preview"
          ] }),
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(h), disabled: e.busy, title: "Regenerate this answer", children: [
            /* @__PURE__ */ a(yt, { size: 12 }),
            " Regenerate"
          ] })
        ] }) : null,
        h.role === "assistant" && h.status === "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => f(h), title: "Copy this error", children: [
            /* @__PURE__ */ a(ln, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(h), disabled: e.busy, children: [
            /* @__PURE__ */ a(yt, { size: 12 }),
            " Retry"
          ] })
        ] }) : null
      ] }, h.id)),
      r ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: b, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ u("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: wo(e.contextScope) }),
        /* @__PURE__ */ a("em", { children: qn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ u("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ u(Ee, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ u("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ a(Rn, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((h) => {
            const w = typeof h.score == "number" ? h.score.toFixed(2) : null, I = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, E = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, V = [
              h.path,
            ...
            return /* @__PURE__ */ u("span", { className: "workbench-chat-chip", title: V, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ a(Rn, { size: 13 }) }),
              h.label,
              w ? /* @__PURE__ */ a("small", { children: w }) : null,
              h.snippet ? /* @__PURE__ */ u("small", { children: [
                h.snippet.slice(0, 60),
                h.snippet.length > 60 ? "…" : ""
              ] }) : null,
              h.dirty ? /* @__PURE__ */ a("small", { children: "dirty" }) : null,
              h.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(h), title: "Remove context", children: /* @__PURE__ */ a(Re, { size: 11 }) }) : null
            ] }, h.id);
          }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: qn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(ia, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(Ji, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function qn(e, t, n) {
  const i = e.model.trim(), r = [e.gatewayPreference === "auto" ? t : _a(e.gatewayPreference)];
  return i && r.push(i), n > 0 && r.push(`${n} memories`), r.join(" · ");
}
function Ms(e) {
...
function Ls(e) {
  const [t, n] = x([]), [i, o] = x("Loading gateway models…"), [r, s] = x([]), [l, p] = x("Checking embedding capability…");
  ce(() => {
    const w = new AbortController();
    return (async () => {
      if (!e.host.ai?.listModels) {
        n([]), o("This Tytus build does not expose model discovery yet.");
        return;
      }
      o("Loading gateway models…");
      try {
        const E = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        n(E.map((V) => ({ id: V.id, gatewayLabel: V.gatewayLabel }))), o(E.length > 0 ? `${E.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (E) {
        if (w.signal.aborted) return;
        n([]), o(`Model discovery failed: ${E instanceof Error ? E.message : String(E)}`);
      }
    })(), () => w.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), ce(() => {
    const w = new AbortController();
    return (async () => {
      const E = Dr(e.host);
      if (E) {
      ...
      try {
        const V = await jr(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        s(V.map((Z) => ({ id: Z.id, gatewayLabel: Z.gatewayLabel ?? Z.source ?? "AIL" }))), p(V.length > 0 ? `${V.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (V) {
        if (w.signal.aborted) return;
        s([]), p(`Embedding model discovery failed: ${V instanceof Error ? V.message : String(V)}`);
      }
    })(), () => w.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ u("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ u("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ a("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ a(Re, { size: 15 }) })
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-settings-note", children: i })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-embedding-models", children: r.map((w) => /* @__PURE__ */ a("option", { value: w.id, children: w.gatewayLabel }, `${w.gatewayLabel}:${w.id}`)) })
        ] }),
    ...
    /* @__PURE__ */ u("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(Rt), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function Ha({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: i, captureManualCheck: o, openOutputAsFile: r, previewEditFromOutput: s, canPreviewEdit: l = !1, compact: p = !1 }) {
  return /* @__PURE__ */ u("div", { className: `workbench-panel-list ${p ? "compact" : ""}`, children: [
    /* @__PURE__ */ u("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      ...
      /* @__PURE__ */ u("button", { className: "workbench-button-subtle", onClick: o, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((b) => /* @__PURE__ */ u("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ u("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void st(b.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => r(b), children: "Open as file" }),
        s ? /* @__PURE__ */ a(
          "button",
          {
            className: Kn(b.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => s(b),
            disabled: !l,
            children: Kn(b.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        b.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(b.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ a(za, { body: b.body })
    ] }, b.id))
  ] });
}
function Os({ edit: e, onApply: t, onOpenAsFile: n, onClose: i }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ u("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ a("button", { title: "Close", onClick: i, children: /* @__PURE__ */ a(Re, { size: 16 }) })
    ] }),
    ...
    /* @__PURE__ */ u("footer", { className: "workbench-edit-review-actions", children: [
      /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: i, children: "Cancel" }),
...
function Es({ patch: e, onApply: t, onOpenAsFiles: n, onClose: i }) {
  const o = e.edits.reduce((r, s) => ({
    added: r.added + s.stats.added,
    removed: r.removed + s.stats.removed,
    changed: r.changed + s.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((r) => /* @__PURE__ */ u("article", { className: "workbench-workspace-patch-card", children: [
        /* @__PURE__ */ u("header", { children: [
          /* @__PURE__ */ a("strong", { children: r.fileName }),
          /* @__PURE__ */ u("span", { children: [
            r.extractionLabel,
            " · +",
            r.stats.added,
        ...
        /* @__PURE__ */ a("pre", { children: Ws(r.proposedContent) })
      ] }, r.fileId)),
...
function js(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, Rs(e.content), "```");
  else if (t.length > 0) {
    n.push("", `Open editors (${t.length}):`);
    for (const i of t.slice(0, 8)) n.push(`- ${i.path} (${i.language}, ${i.content.length} chars${i.dirty ? ", dirty" : ""})`);
  } else
...
function zs(e) {
  const n = e.data?.find((i) => typeof i.id == "string" && i.id.trim());
  return typeof n?.id == "string" ? n.id : null;
}
function _s(e) {
  const n = e.choices?.[0], i = n?.message?.content ?? n?.text ?? n?.delta?.content;
  return typeof i == "string" ? i : Array.isArray(i) ? i.map((o) => typeof o == "string" ? o : o && typeof o == "object" && "text" in o && typeof o.text == "string" ? o.text : "").join("") : JSON.stringify(e, null, 2);
}
function Ba({
  host: e,
  setStatus: t,
  attachSkillToChat: n,
  saveLocalJobOutput: i,
  activeFile: o,
  openEditors: r,
  variant: s = "sidebar"
}) {
  const [l, p] = x([]), [b, f] = x([]), [h, w] = x(null), [I, E] = x(!1), [V, Z] = x(null), [U, oe] = x("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [le, O] = x(null), [W, K] = x([]), [j, ne] = x(() => cs()), [_, je] = x([]), [M, de] = x([]), [be, $e] = x([]), Y = Xe([]), [ge, De] = x("task-execute"), [v, $] = x("mission"), k = W.find((c) => c.status === "running" || c.status === "canceling") ?? W[0] ?? null, D = s === "dock", H = fe(() => ct(h, Ye(U || j?.goal || "", h, j?.teamPresetId)), [U, j?.goal, j?.teamPresetId, h]), se = fe(() => $t(U || j?.goal || "", h, H.id), [U, j?.goal, H.id, h]), P = se.find((c) => c.id === ge) ?? se[1] ?? se[0] ?? null, he = r.filter((c) => c.dirty).length, Ie = o ? `${o.path} · ${o.language} · ${o.content.length.toLocaleString()} chars${o.dirty ? " · dirty" : ""}` : r.length > 0 ? `${r.length} open editor${r.length === 1 ? "" : "s"}${he ? ` · ${he} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Ve = fe(() => [
    {
  ...
  const Me = C(async (c = j) => {
    if (!c?.rootPath || !e.missions?.listRuns)
      return $e([]), [];
    try {
      const y = await e.missions.listRuns(c.rootPath);
      return $e(y), Y.current = y, y;
    } catch (y) {
      return t(`Mission run history failed: ${y instanceof Error ? y.message : String(y)}`), [];
    }
  }, [e.missions, j, t]), Te = C(async (c, y) => {
    if (!c) return;
    const N = [...y].sort((S, B) => Vn(B).localeCompare(Vn(S))).slice(0, 100), T = N.map((S) => JSON.stringify(S)).join(`
`) + (N.length ? `
` : "");
    c.rootPath && e.missions?.write ? await e.missions.write({ rootPath: c.rootPath, files: [{ path: "RUNS.jsonl", content: T }] }) : c.handle && await mn(c.handle, "RUNS.jsonl", T), $e(N), Y.current = N;
  }, [e.missions]), ue = C(async (c, y = j) => {
    if (!y) return;
    const N = [
      c,
      ...Y.current.filter((T) => T.id !== c.id)
    ];
    await Te(y, N);
  }, [j, Te]), Q = C(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      p([]), f([]), w(null), Z("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    E(!0), Z(null);
    try {
      const [c, y, N, T] = await Promise.all([
      ...
      p(c), f(y), w(N), je(T), j?.rootPath && Me(j), t(`Agent team loaded · ${c.length} tools · ${y.length} skills · ${T.length} missions${N ? ` · ${N.resources.length} resources` : ""}`);
    } catch (c) {
      Z(c instanceof Error ? c.message : String(c));
    } finally {
      E(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, Me, j, t]);
  ce(() => {
    Q();
  }, [Q]), ce(() => {
    const c = (y) => {
      const N = y.detail;
      N?.missionId && (ne(N), Me(N)), Q();
    };
    return window.addEventListener(hn, c), () => window.removeEventListener(hn, c);
  }, [Q, Me]);
  const ae = C((c) => {
    const y = ds(c);
    ne(y), vt(y), Me(y), de([{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "mission.resume",
      message: `Mission resumed from Atomek agent team: ${y.rootPath ?? y.name}`,
      data: { runCount: c.runCount ?? 0, taskCount: c.taskCount ?? 0 }
    }]), oe(c.goal || `Continue mission ${c.title}. Review MISSION.md, TASKS.md, RESOURCES.md, and runs/ before acting.`), t(`Resumed mission: ${c.rootPath}`);
  }, [Me, t]), X = C(async (c, y, N = []) => {
    const T = [
      ...M,
      ...N,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], S = Ye(y || c.goal, h, c.teamPresetId), B = $t(y || c.goal, h, S), G = [
      { path: "MISSION.md", content: jt(c, h, o, r, y, S) },
      { path: "MISSION.json", content: ja(c, h, y, S) },
      { path: "RESOURCES.md", content: Dt(h) },
      { path: "TASKS.md", content: Ea(B) },
      { path: "HANDOFF.md", content: Ra(c) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: T.map((z) => JSON.stringify(z)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: Y.current.map((z) => JSON.stringify(z)).join(`
`) + (Y.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (c.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: c.rootPath, files: G });
    else if (c.handle) {
      await ot(c.handle, "runs"), await ot(c.handle, "outputs"), await ot(c.handle, "proposals"), await ot(c.handle, "approvals");
      for (const z of G) await as(c.handle, z.path, z.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    de(T), vt(c);
  }, [o, e.missions, M, r, h]), ie = C(async (c, y = {}) => {
    if (j)
      return await X(j, c), j;
    const N = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, T = c.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let S = null;
    if (e.missions?.create) {
      const G = await e.missions.create({ title: N, goal: T });
      S = {
        missionId: G.missionId,
        title: G.title,
        goal: G.goal,
        rootPath: G.rootPath,
        name: G.rootPath.split("/").pop() || G.missionId,
        source: "tray"
      };
    } else if (y.allowBrowserPicker) {
      const G = await ns();
      if (!G)
      ...
      S = {
        handle: G,
        name: G.name,
        missionId: `mission-${Date.now()}-${ls(G.name)}`,
        title: N,
        goal: T,
        source: "browser"
      };
    ...
    if (!S) return null;
    const B = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${S.rootPath ?? S.name}` };
    return ne(S), vt(S), de([B]), await X(S, T, [B]), t(`Mission pack ready in ${S.rootPath ?? S.name}`), S;
  }, [e.missions, j, t, X]), Ht = C(async (c) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const y = U.trim() || `Open ${c.label} from Atomek with current context.`, N = c.kind === "ai-cli" ? await ie(y) : j;
      await e.local.openTerminal({
        toolId: c.id,
        command: c.command,
        cwd: N?.rootPath ?? Gn(o),
        prompt: N ? `Atomek mission pack ready at ${N.rootPath ?? N.name}. Read MISSION.md and RESOURCES.md. ${Ie}` : `Opened from Atomek. ${Ie}`
      }), t(c.kind === "ai-cli" ? `Opened ${c.label} in Tytus Terminal with mission context. Press Enter there to start it.` : "Opened Tytus Terminal");
    } catch (y) {
      t(`Terminal launch failed: ${y instanceof Error ? y.message : String(y)}`);
    }
  }, [o, Ie, ie, e.local, U, j, t]), dt = C(async () => {
    try {
      await ie(U.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (c) {
      t(`Mission folder setup failed: ${c instanceof Error ? c.message : String(c)}`);
    }
  }, [ie, U, t]), He = C(async (c, y, N, T = j, S) => {
    if (!T) return;
    const B = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), G = S ?? `runs/${B}-${c.id}.md`, z = G.split("/").pop() || `${B}-${c.id}.md`;
    if (T.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: T.rootPath, files: [{ path: G, content: y }] });
    else if (T.handle) {
      const ke = await ot(T.handle, "runs");
      await mn(ke, z, y);
    }
    await X(T, U, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${c.label} exited ${N}; transcript saved to ${G}`,
      data: { toolId: c.id, exitCode: N, transcript: G }
    }]);
  }, [e.missions, U, j, X]), Bt = C(async (c) => {
    ...
    let N = null;
    try {
      N = await ie(y), N && await X(N, y, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${c.label} background review started`,
        data: { toolId: c.id, taskId: P?.id ?? "manual", taskTitle: P?.title ?? "Manual run" }
      }]);
    } catch (B) {
      t(`Mission pack failed before local job start: ${B instanceof Error ? B.message : String(B)}`);
      return;
    }
    O(c.id);
    const T = `local-run-${Date.now()}-${c.id}`, S = it();
    K((B) => [{
      id: T,
      toolId: c.id,
      label: c.label,
      ...
      startedAt: Date.now(),
      taskId: P?.id ?? "manual",
      taskTitle: P?.title ?? "Manual run",
      lines: [`[Atomek] Starting ${c.label} local job for ${P?.title ?? "manual run"}…`]
    }, ...B].slice(0, 6));
    try {
...
`)
      }), G = B.transcriptPath ?? (N?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${c.id}.md` : void 0), z = [
        `[Atomek] Started ${c.label} local job ${B.id}`,
        P ? `[Atomek] Task: ${P.title} (${P.id})` : "[Atomek] Task: manual",
        ...
        taskTitle: P?.title ?? "Manual run",
        transcriptPath: G,
        summary: `Started ${c.label} for ${P?.title ?? "manual run"}`
      };
      ue(ke, N).catch((J) => {
        t(`Mission run index failed: ${J instanceof Error ? J.message : String(J)}`);
      }), K((J) => J.map((R) => R.id === T ? { ...R, jobId: B.id, transcriptPath: G } : R));
      const Pe = (J) => {
        K((R) => R.map((ee) => ee.id === T ? J(ee) : ee));
      };
      e.local.streamJob(B.id, {
        onLog: (J) => {
          z.push(J), Pe((R) => ({ ...R, lines: z.slice(-500) }));
        },
        onDone: (J) => {
          J && (z.push(J), Pe((R) => ({ ...R, lines: z.slice(-500) })));
        },
        onFail: (J) => {
          z.push(`[FAIL] ${J}`), Pe((R) => ({
            ...R,
          ...
          }), i(`${c.label} local job failed`, z.join(`
`)), He(c, z.join(`
`), -1, N, G).catch((R) => {
...
`);
          Pe((ee) => ({
            ...
            finishedAt: it(),
            exitCode: J,
            summary: `${c.label} exited ${J}`
          }, N).catch((ee) => {
            t(`Mission run index failed: ${ee instanceof Error ? ee.message : String(ee)}`);
          }), i(`${c.label} local job`, R), He(c, R, J, N, G).catch((ee) => {
            t(`Mission transcript save failed: ${ee instanceof Error ? ee.message : String(ee)}`);
          }), O(null);
        },
        onError: () => t(`Local job stream issue for ${c.label}`)
      }), t(`Started ${c.label} mission run${P ? ` · ${P.title}` : ""}`);
    } catch (B) {
      O(null), K((G) => G.map((z) => z.id === T ? {
        ...z,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...z.lines, `[Atomek] Failed to start: ${B instanceof Error ? B.message : String(B)}`]
      } : z)), t(`Local job failed to start: ${B instanceof Error ? B.message : String(B)}`);
    }
  }, [o, ie, e.local, U, r, h, i, He, P, t, ue, X]), xt = C(async (c) => {
    const y = pn(c);
    if (!y) {
      t(`Cannot dispatch ${Ce(c)}: missing pod id`);
      return;
    }
    const N = U.trim();
    if (!N) {
    ...
    let T = null;
    const S = Ce(c), B = `pod-run-${Date.now()}-${y}`, G = it(), z = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${c.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      T = await ie(N), T && await X(T, N, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${S} mission task started`,
        data: { resourceId: c.id, podId: y, taskId: P?.id ?? "manual", taskTitle: P?.title ?? "Manual pod run" }
      }]);
    } catch (R) {
    ...
    const ke = {
      ...
      startedAt: G,
      taskId: P?.id ?? "manual",
      taskTitle: P?.title ?? "Manual pod run",
      transcriptPath: z,
      summary: `Started ${S} for ${P?.title ?? "manual pod run"}`
    };
    K((R) => [{
      id: B,
      toolId: c.id,
      ...
      taskTitle: P?.title ?? "Manual pod run",
      transcriptPath: z,
      lines: [`[Atomek] Dispatching ${S} via Tytus host bridge…`, `[Atomek] Pod: ${y}`, `[Atomek] Task: ${P?.title ?? "manual pod run"}`]
    }, ...R].slice(0, 6)), await ue(ke, T).catch((R) => {
      t(`Mission run index failed: ${R instanceof Error ? R.message : String(R)}`);
    });
    const Pe = [], J = (R) => {
      Pe.push(R), K((ee) => ee.map((ye) => ye.id === B ? { ...ye, lines: [...ye.lines, R].slice(-500) } : ye));
    };
    try {
      const R = await e.daemon.callPodEndpoint(y, "/v1/models", { method: "GET" });
      if (!R.ok)
        throw new Error(`/v1/models ${R.status}: ${await R.text()}`);
      const ee = await R.json(), ye = zs(ee);
      if (!ye)
        throw new Error("pod /v1/models returned no model id");
      J(`[Atomek] Selected pod model from live metadata: ${ye}`);
      const pe = await e.daemon.callPodEndpoint(y, "/v1/chat/completions", {
        method: "POST",
...
`)
            },
            {
              role: "user",
              content: [
                T ? `Mission folder: ${T.rootPath ?? T.name}` : "Mission folder: not available",
                T ? `Mission: ${T.title}` : "",
                T ? `Goal: ${T.goal}` : "",
                P ? `Task: ${P.title} (${P.id})` : "Task: manual",
                "",
                "Mission context:",
                T ? jt(T, h, o, r, N) : "",
                h ? Dt(h) : "",
...
`)
      ...
      if (!pe.ok)
        throw new Error(`/v1/chat/completions ${pe.status}: ${xe}`);
      let Fe = xe;
      try {
        Fe = JSON.parse(xe);
      } catch {
      }
      const ut = typeof Fe == "string" ? Fe : _s(Fe);
      J(ut);
      const At = [
        `# Pod job — ${S}`,
        "",
        ...
        `- Pod: ${y}`,
        `- Captured: ${(/* @__PURE__ */ new Date()).toISOString()}`,
        `- Task: ${P?.title ?? "Manual pod run"}`,
        "",
        ut,
        ""
      ].join(`
`);
      K((Nt) => Nt.map((Ze) => Ze.id === B ? { ...Ze, status: "complete", finishedAt: Date.now(), lines: [...Ze.lines, "[Atomek] Pod task complete."].slice(-500) } : Ze)), await ue({ ...ke, status: "complete", finishedAt: it(), summary: `${S} completed`, transcriptPath: z }, T), i(`${S} pod job`, At), await He({ id: c.id, label: S, kind: "pod-agent", status: c.status }, At, 0, T, z), t(`${S} completed mission task`);
    } catch (R) {
      const ee = R instanceof Error ? R.message : String(R);
      J(`[FAIL] ${ee}`), K((pe) => pe.map((xe) => xe.id === B ? { ...xe, status: "failed", finishedAt: Date.now(), lines: xe.lines.slice(-500) } : xe)), await ue({ ...ke, status: "failed", finishedAt: it(), exitCode: -1, summary: ee, transcriptPath: z }, T).catch((pe) => {
        t(`Mission run index failed: ${pe instanceof Error ? pe.message : String(pe)}`);
      });
      const ye = [`# Pod job failed — ${S}`, "", `- Pod: ${y}`, `- Error: ${ee}`, "", "```text", Pe.join(`
`), "```", ""].join(`
`);
      i(`${S} pod job failed`, ye), await He({ id: c.id, label: S, kind: "pod-agent", status: c.status }, ye, -1, T, z).catch((pe) => {
        t(`Pod transcript save failed: ${pe instanceof Error ? pe.message : String(pe)}`);
      }), t(`Pod task failed: ${ee}`);
    }
  }, [o, ie, e.daemon, U, r, h, i, He, P, t, ue, X]), Qe = C(async (c) => {
    if (!c.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    K((y) => y.map((N) => N.id === c.id ? { ...N, status: "canceling", lines: [...N.lines, "[Atomek] Cancel requested…"] } : N)), ue({
      id: c.id,
      jobId: c.jobId,
      toolId: c.toolId,
      label: c.label,
      ...
      t(`Mission run index failed: ${y instanceof Error ? y.message : String(y)}`);
    });
    try {
      await e.local.cancelJob(c.jobId), t(`Cancel requested for ${c.label}`);
    } catch (y) {
      K((N) => N.map((T) => T.id === c.id ? { ...T, status: "running", lines: [...T.lines, `[Atomek] Cancel failed: ${y instanceof Error ? y.message : String(y)}`] } : T)), t(`Local job cancel failed: ${y instanceof Error ? y.message : String(y)}`);
    }
  }, [e.local, t, ue]), me = C(async (c) => {
    const y = [
      `Use Tytus resource "${Ce(c)}" (${c.kind}) for the next mission step.`,
      `Capabilities: ${c.capabilities.join(", ") || "status only"}.`,
      `Sandbox: ${c.sandbox}. Trust: ${c.trustTier}.`,
      c.allowedRoots.length ? `Allowed roots: ${c.allowedRoots.join(", ")}` : "No direct roots exposed.",
...
`);
    oe(y), j && await X(j, y, [{
      ...
      message: `Selected resource ${Ce(c)}`,
      data: { resourceId: c.id, kind: c.kind, status: c.status }
    }]), t(`Selected ${Ce(c)} for mission`);
  }, [j, t, X]), Be = C((c) => {
    const y = c.setupAction, N = y?.commandPreview ? `${y.label}: ${y.commandPreview}` : y?.deepLink ? `${y.label}: ${y.deepLink}` : y?.label ?? `${Ce(c)} needs setup`;
    t(N), st(y?.commandPreview ?? y?.deepLink ?? N);
  }, [t]);
  return /* @__PURE__ */ u("aside", { className: D ? "workbench-agent-dock" : "workbench-sidebar", children: [
    D ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "AGENT TEAM" }),
    /* @__PURE__ */ u("div", { className: D ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ u("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ a(fn, { size: 18 }),
        /* @__PURE__ */ u("div", { children: [
          /* @__PURE__ */ a("strong", { children: D ? "Mission Runs" : "OpenClaw + Hermes Team Board" }),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-team-switcher", role: "tablist", "aria-label": "Atomek agent team views", children: [
        /* @__PURE__ */ a("button", { className: v === "mission" ? "active" : "", onClick: () => $("mission"), children: "Mission" }),
        /* @__PURE__ */ a("button", { className: v === "runs" ? "active" : "", onClick: () => $("runs"), children: "Runs" }),
        /* @__PURE__ */ a("button", { className: v === "setup" ? "active" : "", onClick: () => $("setup"), children: "Setup" })
      ] }),
      v === "mission" ? /* @__PURE__ */ u(Ee, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-context-card mission", children: [
          /* @__PURE__ */ a("strong", { children: j ? j.title : "No mission folder selected" }),
          /* @__PURE__ */ a("span", { children: j ? `${j.rootPath ?? j.name} · ${j.source} · ${M.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
          h ? /* @__PURE__ */ u("span", { children: [
            La(h.resources),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ u("div", { className: `workbench-team-assignment-summary ${H.readiness}`, children: [
            /* @__PURE__ */ a("strong", { children: H.label }),
            /* @__PURE__ */ u("span", { children: [
              H.readiness,
              " · ",
              H.bestFor
          ...
          H.assignments.map((c) => /* @__PURE__ */ u("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: c.label }),
              /* @__PURE__ */ a("span", { children: c.resourceLabel })
            ] }),
            /* @__PURE__ */ a("em", { children: c.status })
          ] }, `${c.role}-${c.resourceId}`))
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
            dt();
          }, children: j ? "Refresh mission pack" : "Start mission pack" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => j && void X(j, U), disabled: !j, children: "Rewrite context files" })
        ] }),
        _.length ? /* @__PURE__ */ u(Ee, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
          /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: _.slice(0, D ? 3 : 5).map((c) => /* @__PURE__ */ u(
            "button",
            {
              className: `workbench-mission-row ${j?.missionId === c.missionId ? "active" : ""}`,
              onClick: () => ae(c),
              title: c.rootPath,
              children: [
                /* @__PURE__ */ a("strong", { children: c.title }),
                /* @__PURE__ */ u("span", { children: [
                  c.status ?? "active",
                  " · ",
                  c.taskCount ?? 0,
        ...
        h?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: h.warnings.slice(0, 3).map((c) => /* @__PURE__ */ u("span", { children: [
          c.code,
          ": ",
          c.message
        ] }, `${c.code}-${c.resourceId ?? c.message}`)) }) : null,
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-context-card", children: [
          ...
          /* @__PURE__ */ a("span", { children: o ? "Local agents receive clipped active-file content and must return previewable edits." : "Open a file to give local agents useful context." })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "TASK FOR LOCAL AGENT" }),
        /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: Ve.map((c) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => oe(c.prompt), children: c.label }, c.label)) }),
        /* @__PURE__ */ a(
          "textarea",
          {
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: se.map((c, y) => /* @__PURE__ */ u(
          "button",
          {
            className: `workbench-task-card ${c.status} ${P?.id === c.id ? "active" : ""}`,
            onClick: () => {
              De(c.id), oe(c.prompt);
            },
            title: "Load this task prompt",
            children: [
              /* @__PURE__ */ a("span", { children: y + 1 }),
              /* @__PURE__ */ a("strong", { children: c.title }),
              /* @__PURE__ */ a("em", { children: c.resourceHint }),
              /* @__PURE__ */ a("small", { children: c.assignedResourceLabel })
            ]
          },
          c.id
        )) })
      ] }) : null,
      v === "runs" ? /* @__PURE__ */ u(Ee, { children: [
        k ? /* @__PURE__ */ u("div", { className: "workbench-agent-run", children: [
          /* @__PURE__ */ u("header", { children: [
            /* @__PURE__ */ u("div", { children: [
            ...
            /* @__PURE__ */ u("div", { className: "workbench-agent-run-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => i(`${k.label} local job`, k.lines.join(`
`)), disabled: k.lines.length === 0, children: "Save output" }),
              k.jobId && (k.status === "running" || k.status === "canceling") ? /* @__PURE__ */ u("button", { className: "workbench-button-subtle danger", onClick: () => {
                Qe(k);
              }, disabled: k.status === "canceling", children: [
                /* @__PURE__ */ a(ia, { size: 12 }),
                " ",
...
`) || "[waiting for output]" })
        ...
        be.length ? /* @__PURE__ */ u(Ee, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: be.slice(0, D ? 4 : 8).map((c) => /* @__PURE__ */ u("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: c.taskTitle || c.label }),
              /* @__PURE__ */ u("span", { children: [
                c.label,
                " · ",
                c.status,
              ...
              c.transcriptPath ? /* @__PURE__ */ a("small", { children: c.transcriptPath }) : null
            ] }),
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void st(c.transcriptPath ?? c.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, c.id)) })
        ] }) : null
      ] }) : null,
      v === "setup" ? /* @__PURE__ */ u(Ee, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list compact", children: [
          ...
          h?.resources.slice(0, D ? 8 : 5).map((c) => /* @__PURE__ */ u("div", { className: "workbench-resource-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: Ce(c) }),
              /* @__PURE__ */ a("span", { children: xn(c) })
            ] }),
            /* @__PURE__ */ u("div", { className: "workbench-resource-row-actions", children: [
              /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${c.status}`, children: c.status }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                me(c);
              }, children: "Use" }),
              c.kind === "pod-agent" && pn(c) ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                xt(c);
              }, children: "Ask pod" }) : null,
              c.status === "needs-setup" || c.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => Be(c), children: "Setup" }) : null
            ] })
          ] }, c.id))
        ] }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          l.length === 0 && !I ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
          l.map((c) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            ...
            c.description ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: c.description }) : null,
            /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                Ht(c);
              }, disabled: c.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: Ds(c) }),
              c.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Bt(c);
              }, disabled: c.status !== "available" || le !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: Fs(c, le) }) : null
            ] })
          ] }, c.id))
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          b.length === 0 && !I ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
          b.map((c) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            /* @__PURE__ */ u("div", { className: "workbench-computer-card-head", children: [
              /* @__PURE__ */ u("div", { children: [
                /* @__PURE__ */ a("strong", { children: c.title }),
                /* @__PURE__ */ u("span", { children: [
            ...
            /* @__PURE__ */ a("p", { className: "workbench-muted", children: c.description }),
            c.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: c.triggers.slice(0, 4).map((y) => /* @__PURE__ */ a("span", { children: y }, y)) }) : null,
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              n(c);
            }, disabled: c.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
          ] }, c.id))
        ] })
      ] }) : null
    ] })
...
function Hs({ status: e, file: t, cursor: n, fileCount: i, dirtyCount: o }) {
  return /* @__PURE__ */ u("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: ra(t.language) })
  ] });
}
function an(e, t) {
...
function Ks() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : Rt.gatewayPreference,
...
function Gs({ host: e }) {
  return /* @__PURE__ */ a(us, { host: e });
}
function Ys(e) {
  return function() {
    return /* @__PURE__ */ a(Gs, { host: e.host });
  };
}
export {
  Ys as default
