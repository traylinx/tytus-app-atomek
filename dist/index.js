...
import { forwardRef as qa, createElement as Wn, useState as T, useRef as Xe, useCallback as N, useEffect as oe, createContext as xs, useMemo as de, useContext as Ps, lazy as Ns, Suspense as Ts } from "react";
...
}, Ya = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), Xs = (e) => {
  for (const t in e)
...
const Os = qa(
  ({
    ...
    "svg",
    {
      ...
      className: Ya("lucide", i),
      ...!s && !Xs(c) && { "aria-hidden": "true" },
      ...c
    },
    [
      ...o.map(([b, p]) => Wn(b, p)),
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Ws = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Vs = le("check", Ws);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Is = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], jt = le("chevron-down", Is);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const mi = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function fi() {
  const e = Qn();
...
async function pi() {
  ...
  if (!e) return ar(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((r) => $n(r, r.name, "local-file")));
  return Wt(n.filter(Boolean));
}
async function bi() {
  const e = Qn()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await ar(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await _n(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Wt(n) };
}
async function vi(e) {
  const t = await Promise.all(e.map((n) => $n(n, n.name, "local-file")));
  return Wt(t.filter(Boolean));
}
async function xa(e) {
  const t = [];
  return await _n(e, e.name, t, 0, 320), { name: e.name, handle: e, files: Wt(t) };
}
...
async function _n(e, t, n, r, i) {
  if (!(n.length >= i || r > 8))
    for await (const s of e.values()) {
      if (n.length >= i) break;
      if (s.kind === "directory") {
        if (mi.has(s.name)) continue;
        await _n(s, `${t}/${s.name}`, n, r + 1, i);
        continue;
      }
      const o = s, c = await $n(o, `${t}/${o.name}`, "local-folder");
      c && n.push(c);
    }
}
function ar(e) {
...
const Ze = {
  ...
  available: !0
}, gi = "atomek:selected-chat-target", un = {
  openclaw: "OpenClaw",
  hermes: "Hermes"
...
function wi(e) {
  try {
    return localStorage.getItem(ir(e))?.trim() || Ze.id;
  } catch {
    return Ze.id;
  }
}
function Na(e, t) {
  try {
    localStorage.setItem(ir(e), t || Ze.id);
  } catch {
  }
}
function wt(e, ...t) {
...
function Pi(e) {
  return e.kind !== "pod-agent" ? null : or([
    e.label,
    e.id,
    wt(e.metadata, "displayName", "display_name", "name", "podName", "pod_name"),
    wt(e.metadata, "agentFamily", "agent_family", "agentType", "agent_type", "internalAgentType", "brand")
  ].join(" "));
}
function Ni(e) {
...
function zi(e, t) {
  const n = wt(e.metadata, "displayName", "display_name", "customName", "custom_name", "podDisplayName", "pod_display_name", "name");
  if (n) {
    const r = ea(n).replace(/\bNemoClaw\b/gi, "OpenClaw").replace(/\s+/g, " ").trim();
    if (Ti(r)) return r;
  }
  return un[t];
}
...
function Li(e) {
  ...
  return e.map((s) => {
    ...
    const o = s.label, c = un[s.agentFamily];
    i[s.agentFamily] += 1;
...
function Xi(e) {
  const t = e.daemon?.state, n = t?.included ?? [], r = t?.agents ?? [], i = new Map(r.map((o) => [o.id, o])), s = n.flatMap((o) => {
    const c = o.id ?? "", b = { ...(o.agentId ? i.get(o.agentId) : void 0)?.meta ?? {}, ...o.meta ?? {} }, p = or([c, o.kind, wt(b, "displayName", "display_name", "agentFamily", "agentType", "brand")].join(" "));
    return !c || !p ? [] : [{
      id: `pod-agent.${c}`,
      kind: "pod-agent",
      label: un[p],
  ...
  return cr({ generatedAt: (/* @__PURE__ */ new Date()).toISOString(), resources: s });
}
async function Ci(e, t) {
  ...
  const r = cr(n), i = r.length > 0 ? [] : Xi(e), s = /* @__PURE__ */ new Map();
  for (const o of [Ze, ...r, ...i]) s.set(o.id, o);
  return [...s.values()];
}
const ta = "atomek", De = "atomek:default", Oi = 3e3, na = "tytus.atomek.threadTitleOverrides", lr = "tytus.atomek.selectedThreadId", Ta = {
  available: !1,
  source: "none",
...
}, Si = (e, t = Oi) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, An = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, ji = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", za = (e) => ({
  id: e.id,
  title: e.title,
  kind: ji(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = aa();
    n[e] = t, localStorage.setItem(na, JSON.stringify(n));
  } catch {
  }
}, Mi = (e) => {
  try {
    const t = aa();
    delete t[e], localStorage.setItem(na, JSON.stringify(t));
  } catch {
  }
}, La = (e) => {
  const t = aa();
  ...
  try {
    return localStorage.getItem(lr)?.trim() || null;
  } catch {
    return null;
  }
}, it = (e) => {
...
}, on = (e) => `${De}:agent-session:${e}`, cn = (e) => `${De}:agent-transcript:${e}`, ra = (e) => `${De}:${ta}:agent-session:${e}`, ln = (e) => `${De}:${ta}:agent-transcript:${e}`, dr = 100, Vi = (e, t) => {
  try {
    const n = localStorage.getItem(ra(e))?.trim();
    if (n) return n;
    const r = localStorage.getItem(on(e))?.trim();
    return r || t && t !== e && localStorage.getItem(on(t))?.trim() || null;
  } catch {
    return null;
  }
}, Ii = (e, t) => {
  try {
    localStorage.setItem(ra(e), t);
  } catch {
  }
}, Et = (e, t) => {
  try {
    const n = localStorage.getItem(ln(e)), r = n ? JSON.parse(n) : [], i = /* @__PURE__ */ new Map();
    for (const s of [...r, ...t]) i.set(s.id, s);
    localStorage.setItem(ln(e), JSON.stringify([...i.values()].slice(-dr)));
  } catch {
  }
}, nn = (e) => {
  if (!e) return [];
  ...
  try {
    const n = nn(localStorage.getItem(ln(e)));
    if (n.length > 0) return n;
    const r = nn(localStorage.getItem(cn(e)));
    return r.length > 0 || !t || t === e ? r : nn(localStorage.getItem(cn(t)));
  } catch {
    return [];
  }
};
function Di() {
  const e = `${De}:${ta}:agent-transcript:`, t = [];
  try {
    for (let n = 0; n < localStorage.length; n += 1) {
      const r = localStorage.key(n);
      if (!r || !r.startsWith(e)) continue;
      const i = nn(localStorage.getItem(r));
...
const ur = (e, t) => {
  try {
    localStorage.removeItem(ln(e)), localStorage.removeItem(ra(e)), localStorage.removeItem(cn(e)), localStorage.removeItem(on(e)), t && t !== e && (localStorage.removeItem(cn(t)), localStorage.removeItem(on(t)));
  } catch {
  }
}, Ei = (e) => {
  const t = e.daemon;
...
function Ji({ host: e, requestContext: t, chatSettings: n, selectedTarget: r = Ze, setStatus: i }) {
  const s = e.ai, [o, c] = T(null), [m, b] = T([]), [p, v] = T([]), [g, S] = T([]), [H, E] = T([]), [J, F] = T(Ta), [ae, he] = T(!1), M = Xe(!0), K = Xe(null), U = N(async () => {
    if (!s) {
      F(Ta);
      return;
    }
    ...
    if (s)
      try {
        const X = await s.listArtifacts({ threadId: x });
        if (!M.current) return;
        S(X.map(za));
      } catch (X) {
        i(`AI artifacts unavailable: ${X instanceof Error ? X.message : String(X)}`);
      }
  }, [s, i]), z = N(async (x) => {
    if (s)
      try {
        const X = await s.listMessages(x);
        if (!M.current) return;
        const R = m.find((C) => C.id === x) ?? null;
        R && c(R), it(x), v(X.map(mt).filter(Boolean)), E([]), await W(x);
      } catch (X) {
        i(`Load chat failed: ${X instanceof Error ? X.message : String(X)}`);
      }
  }, [s, W, i, m]), I = N(async () => {
    if (s)
      try {
        const x = La(await s.listThreads({ workspaceKey: De, status: "active" })), X = Wi(), R = x.find((re) => re.id === X) ?? x[0] ?? await s.createThread({ workspaceKey: De, title: "Atomek chat" });
        if (!M.current) return;
        b(x[0] ? x : [R]), c(R), it(R.id);
        const C = await s.listMessages(R.id);
        if (!M.current) return;
        v(C.map(mt).filter(Boolean)), await W(R.id);
      } catch (x) {
  ...
  const ye = r.kind === "pod-agent" ? r.podId : null;
  oe(() => {
    if (r.kind === "pod-agent") {
      v(Fi(r.id, r.podId)), E([]);
      return;
    ...
    return (async () => {
      try {
        const X = await s.listMessages(o.id);
        !x && M.current && v(X.map(mt).filter(Boolean));
      } catch {
      }
    })(), () => {
      x = !0;
  ...
  const ee = N(async () => {
    if (!s) return null;
    if (o) return o;
    const x = await s.createThread({ workspaceKey: De, title: "Atomek chat" });
    return M.current && (c(x), it(x.id), b((X) => [x, ...X.filter((R) => R.id !== x.id)])), x;
  }, [s, o]), me = N(async () => {
    if (r.kind === "pod-agent") {
      ur(r.id, r.podId), v([]), E([]), i(`Cleared ${r.label} chat`);
      return;
    }
    if (!s) return;
    const x = await s.createThread({ workspaceKey: De, title: "Atomek chat" });
    c(x), it(x.id), b((X) => [x, ...X.filter((R) => R.id !== x.id)]), v([]), S([]), E([]), i("New AI chat created");
  }, [s, r, i]), Pe = N(async (x, X) => {
    if (!s) return;
    const R = X.trim();
    if (R)
      try {
        const C = s.updateThread, re = m.find((ve) => ve.id === x) ?? o, ue = typeof C == "function" ? await C({ threadId: x, title: R }) : { ...re ?? await ee(), id: x, title: R, updatedAt: Date.now() };
        typeof C != "function" && Aa(x, R), b((ve) => ve.map((be) => be.id === ue.id ? ue : be)), c((ve) => ve?.id === ue.id ? ue : ve), i(typeof C == "function" ? `Renamed chat: ${ue.title}` : `Renamed chat locally: ${ue.title}`);
      } catch (C) {
        i(`Rename chat failed: ${C instanceof Error ? C.message : String(C)}`);
      }
  }, [s, ee, i, o, m]), Ce = N(async (x) => {
    if (s)
      try {
        await s.deleteThread(x), Mi(x);
        const X = m.filter((R) => R.id !== x);
        if (b(X), o?.id === x) {
          const R = X[0] ?? await s.createThread({ workspaceKey: De, title: "Atomek chat" });
          c(R), it(R.id), b((re) => re.some((ue) => ue.id === R.id) ? re : [R, ...re]);
          const C = await s.listMessages(R.id);
          v(C.map(mt).filter(Boolean)), await W(R.id);
        }
        i("Deleted AI chat");
      } catch (X) {
        i(`Delete chat failed: ${X instanceof Error ? X.message : String(X)}`);
      }
  }, [s, W, i, o?.id, m]), ke = N(async (x) => {
    if (!s) return null;
    try {
      const X = await ee();
      if (!X) return null;
      ...
      const R = await s.createArtifact({
        ...
        title: x.title?.trim() || An(x.body, "Atomek artifact"),
        kind: x.kind ?? "markdown",
        body: x.body
      }), C = za(R);
      return S((re) => [C, ...re.filter((ue) => ue.id !== C.id)]), i(`Saved AI artifact: ${C.title}`), C;
    } catch (X) {
      return i(`Save artifact failed: ${X instanceof Error ? X.message : String(X)}`), null;
    }
  }, [s, ee, i]), f = N(async (x) => {
    if (s)
      try {
        await s.deleteArtifact(x), S((X) => X.filter((R) => R.id !== x)), i("Deleted AI artifact");
      } catch (X) {
        i(`Delete artifact failed: ${X instanceof Error ? X.message : String(X)}`);
      }
  }, [s, i]), q = N(async (x) => {
    if (!s) return null;
    try {
      const X = await s.writeMemory({
        title: x.title?.trim() || An(x.body, "Atomek memory"),
        body: x.body,
        metadata: {
          source: "atomek",
          messageId: x.messageId ?? null
      ...
      return E((R) => [X, ...R.filter((C) => C.id !== X.id)].slice(0, 5)), i(`Remembered: ${X.title}`), X;
    } catch (X) {
      return i(`Remember failed: ${X instanceof Error ? X.message : String(X)}`), null;
    }
  }, [s, i]), G = N(async (x) => {
    if (!s) return [];
    const X = await s.searchMemory({ query: x, limit: 5 });
    return M.current && E(X), X;
  }, [s]), k = N(async (x, X = {}) => {
    const R = x.trim();
    if (!R) return null;
    const C = r;
    if (C.kind === "atomek-ai" && !s) return null;
    he(!0);
    let re = null;
    const ue = new AbortController();
    K.current = ue;
    try {
      const ve = X.requestContext ?? t;
      if (C.kind === "pod-agent") {
        const te = Ei(e), B = Date.now(), ce = {
          id: `agent-user-${B}`,
          role: "user",
          body: R,
          status: "complete",
          createdAt: B
        }, se = `agent-assistant-${B}`, Te = {
          id: se,
          role: "assistant",
          body: "",
          status: "streaming",
        ...
        if (v((d) => [...d, ce, Te]), Et(C.id, [ce, Te]), !C.available) {
          const d = Pa(C.description), w = { ...Te, body: d.message, status: "error", error: d.message };
          return v((A) => A.map((L) => L.id === se ? w : L)), Et(C.id, [w]), i(d.message), w;
        }
        if (!te) {
          const d = "Agent chat bridge unavailable in this Tytus host build. Update TytusOS host API/runtime to enable OpenClaw and Hermes chat.", w = { ...Te, body: d, status: "error", error: d };
          return v((A) => A.map((L) => L.id === se ? w : L)), Et(C.id, [w]), i("Agent chat bridge unavailable"), w;
        }
        let je = "";
        const Be = (d, w) => (v((A) => A.map((L) => L.id === se ? d : L)), Et(C.id, [ce, d]), i(w), d);
        try {
          for await (const d of te.chatAgent({
            podId: C.podId,
            routeId: C.routeId ?? null,
            sessionId: Vi(C.id, C.podId),
            message: Yi(R, ve),
            mode: "operator",
            target: "agent",
            ...
            if (d.type === "profile") {
              ...
              v(
                (A) => A.map(
                  (L) => L.id === se ? { ...L, gatewayLabel: w } : L
                )
              );
            }
            if (d.type === "session" && Ii(C.id, d.sessionId), d.type === "token" && (je = ea(`${je}${d.text}`), v((w) => w.map((A) => A.id === se ? { ...A, body: je, status: "streaming" } : A))), d.type === "error") {
              const w = Pa(d.message), A = { ...Te, body: w.message, status: "error", error: w.message };
              return Be(A, w.message);
            }
            if (d.type === "done")
              return re = { ...Te, body: je || "Agent finished without visible output.", status: "complete" }, Be(re, `${C.label} answered`);
          }
        } catch (d) {
          if (ue.signal.aborted || d instanceof DOMException && d.name === "AbortError")
            return re = { ...Te, body: je || "Stopped by user.", status: "complete" }, Be(re, `${C.label} response stopped`);
          throw d;
        }
        return ue.signal.aborted ? (re = { ...Te, body: je || "Stopped by user.", status: "complete" }, Be(re, `${C.label} response stopped`)) : re || (re = { ...Te, body: je || "Agent finished without visible output.", status: "complete" }, Be(re, `${C.label} answered`));
      }
      const be = await ee();
      if (!be || !s) return null;
      it(be.id);
      const Ke = (be.title ?? "").trim();
      if ((!Ke || Ke === "Atomek chat") && R.trim()) {
        const te = An(R, "Atomek chat");
        te && te !== Ke && (async () => {
          try {
            const B = s.updateThread, ce = typeof B == "function" ? await B({ threadId: be.id, title: te }) : { ...be, title: te, updatedAt: Date.now() };
            if (typeof B != "function" && Aa(be.id, te), !M.current) return;
            c((se) => se?.id === ce.id ? ce : se), b((se) => se.map((Te) => Te.id === ce.id ? ce : Te));
          } catch {
          }
        })();
      }
      const Ue = await G(R).catch(() => []), at = Hi(Ue), xe = at ? [...ve, at] : [...ve];
      let Ne = null;
      for await (const te of s.sendMessage({
        ...
        if (te.type === "message_created") {
          ...
          if (!B) continue;
          B.role === "assistant" && (Ne = B.id), v((ce) => [...ce.filter((se) => se.id !== B.id), B]);
        }
        if (te.type === "token" && (Ne = te.messageId, v((B) => B.some((ce) => ce.id === te.messageId) ? B.map(
          ...
          const B = mt(te.message);
          if (!B) continue;
          B.role === "assistant" && (re = B), v((ce) => ce.map((se) => se.id === B.id ? B : se)), B.gatewayLabel && i(`AI answered via ${B.gatewayLabel}`);
        }
        if (te.type === "run_failed") {
          const B = ue.signal.aborted;
          i(B ? "AI response stopped" : `AI failed: ${te.error}`), Ne && v((ce) => ce.map((se) => se.id !== Ne ? se : B ? { ...se, status: "complete", error: void 0, body: se.body || "Stopped by user." } : { ...se, status: "error", error: te.error, body: te.error }));
        }
      }
      const qe = La(await s.listThreads({ workspaceKey: De, status: "active" }).catch(() => []));
      return M.current && qe.length > 0 && b(qe), U(), re;
    } catch (ve) {
      ...
      K.current === ue && (K.current = null), he(!1);
    }
  }, [s, n.gatewayPreference, n.model, ee, e, G, U, t, r, i]), O = N(() => {
    K.current?.abort(), i("Stopping AI response…");
  }, [i]);
  return {
    aiStatus: J,
    artifacts: g,
...
const Zi = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Ki = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Ui = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: i }) => {
...
}, Bi = "active-file", Gi = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Qi = (e, t = "file") => `${t}:${e.id}`, $i = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
...
`), p = o.kind === "selection" && o.range ? oo(c.content, o.range) : c.content, v = o.kind === "selection" ? no : o.label.startsWith("Open editor") ? ro : ao;
    i.push({
...
`));
}, Xa = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, Ca = 75e4, co = 2800, lo = 240, uo = 80, ho = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? Ca) || e.content.length > (t.maxFileBytes ?? Ca) ? "huge" : !t.includeDirty && e.dirty ? null : go(e.path) ? "vendor" : wo(e) ? "binary" : null;
}, Oa = (e, t = {}) => {
  const n = Date.now(), r = [], i = [], s = [];
  for (const o of e) {
    const c = o.size ?? o.content.length, m = Fn(o, t);
    if (m) {
      i.push({ fileId: o.id, name: o.name, path: o.path, language: o.language, reason: m, size: c });
      continue;
    }
    const b = ct(o.content), p = fo(o, b, n, t);
    s.push(...p), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
...
}, fo = (e, t = ct(e.content), n = Date.now(), r = {}) => {
  const i = Math.max(500, r.maxChunkChars ?? co), s = Math.min(Math.max(0, r.chunkOverlapChars ?? lo), Math.floor(i / 2)), o = Math.max(1, r.maxChunksPerFile ?? uo), c = vo(e.content), m = [];
  let b = 0, p = 0;
  for (; b < e.content.length && m.length < o; ) {
    const v = bo(e.content, b, Math.min(e.content.length, b + i)), g = e.content.slice(b, v).trim();
    if (g.length > 0) {
      const S = yo(c, b, v);
      m.push({
...
` && t.push(n + 1);
  ...
  return 0;
}, go = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => ho.has(t)), wo = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && mo.has(t) || e.content.includes("\0")) return !0;
...
class ko {
  ...
  staleReport(t) {
    return xo(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const ft = (e = [], t = {}) => new ko(e, t), xo = (e, t, n = {}) => {
  ...
  for (const c of t)
    !i.has(c.id) && !Fn(c, n) && s.push({ fileId: c.id, path: c.path, status: "new", currentHash: ct(c.content) });
  const o = e.signature !== hr(t);
  return { stale: s.length > 0, signatureChanged: o, files: s };
}, Po = 8, No = 12e3, mr = (e, t, n = {}, r) => {
  const i = Ao(t);
  if (i.length === 0) return [];
  const s = new Set(r?.files.map((p) => p.fileId) ?? []), o = e.chunks.filter((p) => n.includeDirty !== !1 || !p.dirty).map((p) => Lo(p, i)).filter((p) => p.score >= (n.minScore ?? 1)).sort((p, v) => v.score - p.score || p.chunk.path.localeCompare(v.chunk.path) || p.chunk.ordinal - v.chunk.ordinal), c = [], m = /* @__PURE__ */ new Set();
  let b = n.maxChars ?? No;
  for (const p of o) {
    if (c.length >= (n.limit ?? Po) || b <= 0) break;
...
[...context clipped...]` : p.chunk.text;
    c.push(To(p, g, s.has(p.chunk.fileId))), b -= g.length;
  }
  return c;
}, To = (e, t = e.chunk.text, n = !1) => {
...
`), Ao = (e) => Array.from(new Set(
  ...
  const c = [];
  for (const m of t) {
    const b = Ln(n, m), p = Ln(r, m), v = Ln(i, m), g = s === m ? 1 : 0;
  ...
  const { autoRefresh: n = !1, ...r } = t, i = Xe(e), s = Xe(null);
  s.current || (s.current = ft(e, r));
  const [o, c] = T(() => s.current?.getSnapshot() ?? ft(e, r).getSnapshot());
  i.current = e;
  const m = N((g = i.current) => {
    const S = (s.current ?? ft([], r)).refresh(g, r);
    return c(S), S;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), b = N((g = i.current) => {
    const S = (s.current ?? ft([], r)).update(g, r);
    return c(S), S;
  }, [m]);
  oe(() => {
    n && m(e);
  }, [n, e, m]);
  const p = (s.current ?? ft([], r)).staleReport(e), v = N((g, S = {}) => {
    const H = s.current ?? ft([], r);
    return mr(H.getSnapshot(), g, S, H.staleReport(i.current));
  }, []);
  return { snapshot: o, staleReport: p, isStale: p.stale, refresh: m, update: b, retrieve: v };
}, Oo = "tytus.atomek.semanticVector:v1", fr = "__gateway_default__", pr = (e) => e?.trim() || fr, So = (e) => {
  const t = pr(e);
  return t === fr ? "gateway default" : t;
}, br = (e, t, n) => [
  Oo,
  ...
  if (!vr()) return null;
  try {
    const r = br(e, t, n), i = window.localStorage.getItem(r);
    if (!i) return null;
  ...
  if (!vr()) return null;
  try {
    const t = br(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, Ct = 8, Mo = 12e3, Wo = 180, Vo = async (e, t, n, r, i = {}, s, o) => {
  const c = mr(t, n, { ...i, limit: Math.max(i.limit ?? Ct, 16) }, s), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: c.slice(0, i.limit ?? Ct), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const b = pr(r.embeddingModel), p = await m.embedText({
      input: n,
    ...
    const g = new Map(c.map((M) => [M.chunkId, M])), S = Yo(n), H = /* @__PURE__ */ new Map();
    t.chunks.filter((M) => i.includeDirty !== !1 || !M.dirty).slice(0, Wo).forEach((M) => H.set(M.id, M)), c.forEach((M) => {
      const K = t.byChunkId.get(M.chunkId);
      K && H.set(K.id, K);
    });
    const E = Array.from(H.values()), J = [];
    let F = 0;
    for (const M of E) {
      o?.aborted;
      const K = jo(e.appId, b, M) ?? await Fo(m, e.appId, b, M, r, o);
      if (!K) continue;
      K.updatedAt > Date.now() - 1e3 && (F += 1);
      const U = Eo(v, K.vector);
      if (!Number.isFinite(U)) continue;
      const z = g.get(M.id)?.score ?? Jo(M, S), I = qo(z) * 0.42 + Math.max(0, U) * 0.58;
      J.push({ chunk: M, keywordScore: z, vectorScore: U, score: I, matchedTerms: S.filter((ye) => M.text.toLowerCase().includes(ye)) });
    }
    if (J.length === 0) return { hits: c.slice(0, i.limit ?? Ct), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: F };
    const ae = new Set(s?.files.map((M) => M.fileId) ?? []), he = J.sort((M, K) => K.score - M.score || K.keywordScore - M.keywordScore || M.chunk.path.localeCompare(K.chunk.path) || M.chunk.ordinal - K.chunk.ordinal).slice(0, Math.max(i.limit ?? Ct, 1)).map((M) => Do(M, ae.has(M.chunk.fileId)));
    return {
      hits: Ro(he, i.maxChars ?? Mo),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${So(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: F
    };
  } catch (b) {
    return {
      hits: c.slice(0, i.limit ?? Ct),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: Zo(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, qo = (e) => Math.min(1, Math.max(0, e / 24)), Yo = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Jo = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, Ko = /```([^\n`]*)\n([\s\S]*?)```/g, Uo = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function sa(e) {
  return Array.from(e.matchAll(Ko)).map((t) => gr(t[1] ?? "", _o(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function gr(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, i = [];
  for (const s of n) {
    const o = s.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? r[o[1].toLowerCase()] = tc(o[2]) : i.push(s.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: i, attrs: r, content: t };
}
function Ie(e, t) {
  return (typeof e == "string" ? gr(e, "").flags : e.flags).includes(t.toLowerCase());
}
function wr(e) {
  return [...sa(e).filter((r) => Ie(r, "diff") || Ie(r, "patch")).map((r) => r.content), e].filter((r, i, s) => Bo(r) && s.findIndex((o) => o === r) === i);
}
function Bo(e) {
...
`);
    /^@@\s+-\d+/m.test(p) && s.push({ raw: p, ...Xn(p) });
  }
  return s.length > 0 ? s : [{ raw: t, ...Xn(t) }];
}
function Xn(e) {
...
function Dn(e) {
  return sa(e).filter((t) => Qo(t)).map((t) => ({
    content: ec(t.content),
...
function nc(e) {
  ...
  for (const p of wr(t))
    for (const v of Go(p)) {
      if (!v.path) {
        if (i) {
          const H = Rn(i.content, v.raw);
          H && H !== i.content && o.set(i.id, Yt({
            file: i,
            proposedContent: H,
            sourceTitle: r,
      ...
      const S = Rn(g.file.content, v.raw);
  ...
  for (const p of Dn(t)) {
    const v = p.path ? ja(n, p.path) : i ? { file: i, normalizedPatchPath: $e(i.path), confidence: "exact" } : null;
    if (!v) {
  ...
  if (o.size === 0 && i && !m) {
    const p = ac(t, i);
    p && p.content !== i.content && o.set(i.id, Yt({
      file: i,
      proposedContent: p.content,
      sourceTitle: r,
      extractionLabel: p.label,
...
function Yt(e) {
  ...
  return {
    ...
    stats: rc(e.file.content, e.proposedContent),
    base: {
...
const hn = (e) => typeof e == "object" && e !== null, cc = (e) => hn(e.ai) ? e.ai : null, lc = (e) => {
  if (!hn(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, dc = (e) => {
  const t = lc(e);
  return !t || !hn(e) ? null : { ...e, id: t };
}, Je = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Je) : hn(e) ? Object.values(e).some(Je) : !1, uc = async (e, t) => {
  const n = cc(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(dc).filter((i) => !!i) : [];
}, hc = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Je(e.capability) || Je(e.capabilities) || Je(e.modality) || Je(e.modalities) || Je(e.task) || Je(e.tasks) || Je(e.type) || Je(e.kind), xr = (e) => typeof e == "object" && e !== null, mc = (e) => xr(e.ai) ? e.ai : null, Pr = (e) => typeof mc(e)?.embedText == "function", fc = async (e, t) => Pr(e) ? (await uc(e, t)).filter(hc) : [], pc = (e) => xr(e.ai) ? Pr(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", bc = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], vc = [
...
function kc(e) {
  const t = e.results.length > 0 ? e.results.map((r, i) => [
    ...
    `Status: ${r.status}`,
    "Output:",
    Xc(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function Pc(e) {
  ...
  for (const r of t) {
    const i = Nc(r.content);
    if (!i?.scripts) continue;
    const s = Tr(r.path), o = Tc(e, s, i);
    if (!o) continue;
    const c = Object.keys(i.scripts).filter((b) => typeof i.scripts?.[b] == "string"), m = zc(c);
    for (const b of m) {
      const p = `${o} run ${b}`;
      n.push({
        id: zr(`${s}:${p}`),
...
function Tc(e, t, n) {
  ...
  for (const [r, i] of vc)
    if (e.some((s) => Lc(s.path) === r && Tr(s.path) === t)) return i;
  return null;
}
function zc(e) {
  const t = bc.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, i) => r.localeCompare(i));
  return [...t, ...n];
}
function Ac(e) {
  const t = /* @__PURE__ */ new Set();
...
function Hc({ className: e }) {
  return /* @__PURE__ */ a("img", { src: jc, alt: "ATOMEK", className: e, draggable: !1 });
}
const Lr = {
...
}, Ha = { en: Lr, es: Mc }, Ma = (e) => (e || "en").toLowerCase().split("-")[0] || "en", Wc = (e, t) => t ? e.replace(/\{(\w+)\}/g, (n, r) => String(t[r] ?? `{${r}}`)) : e, Xr = xs({ locale: "en", t: (e) => Lr[e] ?? e });
function Vc({ host: e, children: t }) {
  ...
  const i = de(() => ({
    ...
    t: (s, o) => {
      ...
      return c && c !== s ? c : Wc(Ha[n]?.[s] ?? Ha.en[s] ?? s, o);
    }
  }), [e, n]);
  return /* @__PURE__ */ a(Xr.Provider, { value: i, children: t });
}
function Ee() {
  return Ps(Xr).t;
}
const Ic = `# Tytus Resource Fabric
...
], Yc = "tytus-atomek-handles", kt = "handles", Jc = 1, Cr = () => new Promise((e, t) => {
  const n = indexedDB.open(Yc, Jc);
  n.onupgradeneeded = () => {
    const r = n.result;
...
async function Wa(e, t) {
  ...
  await new Promise((r, i) => {
    const s = n.transaction(kt, "readwrite");
    s.objectStore(kt).put(t, e), s.oncomplete = () => r(), s.onerror = () => i(s.error ?? new Error("Failed to persist file handle"));
  }), n.close();
}
async function Cn(e) {
  const t = await Cr(), n = await new Promise((r, i) => {
...
const Zc = Ns(() => import("./WorkbenchMonacoEditor-DeMocMpz.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Kc = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Or = "tytus.workspace.recent", Sr = "tytus.workspace.layout", jr = "tytus.atomek.session.v2", Hr = "tytus.atomek.chatAiSettings", On = "atomek:default", Mr = "tytus.atomek.currentMission", En = "tytus.atomek.currentMissionChanged", Va = "0.4.32", an = {
...
function Sn(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - Uc), r = n < 1180, i = r ? 200 : 240, s = r ? 280 : 300, o = r ? 360 : 420, c = Math.max(i, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, c), b = Math.floor(n * (r ? 0.5 : 0.62)), p = n - m - o, v = Math.max(s, Math.min(r ? 1e3 : 1400, b, p));
...
function $c(e) {
  const t = e?.resources ?? [], n = (c) => c.status === "ready" || c.status === "available", r = t.filter((c) => c.kind === "pod-agent" && Pt(c) === "openclaw"), i = t.filter((c) => c.kind === "pod-agent" && Pt(c) === "hermes"), s = t.filter((c) => c.kind === "local-cli" && n(c)), o = t.filter((c) => c.kind === "shared-folder" && n(c));
  return [
...
function Nt(e, t = "repo-repair") {
  const n = Ht.find((I) => I.id === t) ?? Ht[0], r = e?.resources ?? [], i = ot(r, (I) => I.kind === "pod-agent" && Pt(I) === "openclaw"), s = ot(r, (I) => I.kind === "pod-agent" && Pt(I) === "hermes"), o = ot(r, (I) => I.kind === "ail-route"), c = ot(r, (I) => I.kind === "shared-folder"), m = Qe(r, "local-cli", ["claude"]), b = Qe(r, "local-cli", ["opencode", "open code"]), p = Qe(r, "local-cli", ["codex"]), v = Qe(r, "local-cli", ["pi"]), g = Qe(r, "local-cli", ["kimi"]), S = ot(r, (I) => I.kind === "local-cli"), H = Qe(r, "app-skill", ["juli3ta", "music", "song"]), E = Qe(r, "app-skill", ["blender", "3d"]), J = Qe(r, "app-skill", ["remotion", "video", "render"]), F = ot(r, (I) => I.kind === "app-skill"), ae = t === "pod-local" ? i ?? s ?? o ?? m ?? S : t === "creative-production" ? s ?? o ?? m ?? S : t === "research-watch" ? i ?? o ?? s ?? m ?? S : m ?? b ?? o ?? S, he = t === "creative-production" ? H ?? E ?? J ?? F ?? b ?? m ?? S : b ?? m ?? p ?? S, M = t === "pod-local" ? p ?? v ?? g ?? i ?? m ?? S : t === "research-watch" ? v ?? g ?? p ?? i ?? S : p ?? v ?? i ?? g ?? S, K = t === "creative-production" ? H ?? E ?? J ?? F : t === "research-watch" ? Qe(r, "app-skill", ["browser", "channel", "telegram"]) ?? F : F, U = [
    Ot("planner", "Planner", "Break goal into tasks, risks, and required context.", ae, "No planner agent ready"),
    Ot("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", he, "No local/app implementer ready"),
    Ot("reviewer", "Reviewer", "Independent critique before approval or handoff.", M, "No reviewer agent ready"),
  ...
  (t === "creative-production" || K) && U.push(Ot("app-tool", "App Tool", "Drive installed local app skill through mission context.", K, "No configured app skill"));
  const W = U.filter((I) => I.status === "ready" || I.status === "available" || I.status === "degraded").length, z = W >= U.length - 1 ? "ready" : W >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: z, assignments: U };
}
function lt(e, t, n) {
  if (n && Ht.some((i) => i.id === n)) return n;
  const r = e.toLowerCase();
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(r) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(r) ? "research-watch" : (t?.resources ?? []).some((i) => i.kind === "pod-agent" && Vr(i)) ? "pod-local" : "repo-repair";
}
function tl(e) {
  return Ht.map((t) => Nt(e, t.id));
}
function Kt(e) {
...
function St(e) {
  ...
  try {
    localStorage.setItem(Mr, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(En, { detail: t }));
  } catch {
  }
}
function al() {
...
function Mt(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", i = Nt(t, lt(r, t, n)), s = (v) => i.assignments.find((g) => g.role === v), o = s("planner"), c = s("implementer"), m = s("reviewer"), b = s("team-desk"), p = s("app-tool");
  return [
    {
      id: "task-scope",
    ...
    } : null,
    {
      ...
      resourceHint: m ? `${Kt(m)} · ${b?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
...
function rn(e, t, n, r, i, s) {
  const o = Nt(t, lt(i || e.goal, t, s ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
...
function Dr(e, t, n, r) {
  const i = lt(n || e.goal, t, r ?? e.teamPresetId), s = Nt(t, i), o = Mt(n || e.goal, t, i);
...
function sl({ host: e }) {
  const t = Ee(), n = Xe(null), [r, i] = T(0), s = de(() => Jl(), []), o = de(() => Dl(), []), c = de(() => Fl(Il(), o.recent), [o.recent]), [m, b] = T(o.activity ?? "computer"), [p, v] = T(s.primaryVisible), [g, S] = T(s.primaryWidth), [H, E] = T(o.secondaryTab ?? "chat"), [J, F] = T(s.secondaryVisible), [ae, he] = T(s.secondaryWidth), [M, K] = T(!!o.bottomPanelVisible), [U, W] = T(o.bottomPanelTab ?? "problems"), [z, I] = T(s.markdownPreviewVisible), [ye, ee] = T(!!o.welcomeClosed), [me, Pe] = T(o.folder ? { name: o.folder.name, files: [] } : null), [Ce, ke] = T(o.folder?.handleKey ?? null), [f, q] = T(() => Rl(o.files)), [G, k] = T(() => (o.openEditorIds ?? []).filter((l) => (o.files ?? []).some((h) => h.id === l))), [O, x] = T(() => o.activeFileId && (o.files ?? []).some((l) => l.id === o.activeFileId) ? o.activeFileId : null), [X, R] = T(o.query ?? ""), [C, re] = T({ lineNumber: 1, column: 1 }), [ue, ve] = T(null), [be, Ke] = T({}), [we, Ue] = T(Bi), [at, xe] = T([]), [Ne, qe] = T([]), [te, B] = T(null), [ce, se] = T(o.chatInput ?? ""), [Te, je] = T(!1), [Be, d] = T(""), [w, A] = T([]), [L, V] = T(null), [Z, $] = T(null), [_, We] = T(!1), [Ae, ie] = T(() => Zl()), [D, fe] = T([Ze]), [Oe, ze] = T(() => wi(On)), [Fe, Ye] = T(null), [Ge, dt] = T(null), [Tt, _e] = T(""), [mn, fn] = T(""), [Vt, It] = T(""), [Ft, ca] = T("failed"), [Ur, la] = T(null), [Br, j] = T("Ready"), [zt, Gr] = T(() => c), He = G.map((l) => f.find((h) => h.id === l)).filter(Boolean), Y = O ? f.find((l) => l.id === O) ?? null : null, da = de(() => Ui({ files: f, openEditorIds: G, activeFileId: O, versions: be, activeSelection: ue }), [O, ue, be, f, G]), ua = de(() => ({
    scope: we,
    removedAttachmentIds: at,
    selectedFileIds: []
  }), [we, at]), At = de(() => io(da, f, ua), [ua, da, f]), et = Co(f, { autoRefresh: !0, includeDirty: !0 }), ha = de(() => Ne.map((l) => ({
    id: l.id,
    kind: "index-hit",
    label: l.label,
    path: l.path,
    ...
    snippet: l.snippet
  })), [Ne]), Qr = de(() => [...At.attachments, ...ha], [At.attachments, ha]), ma = de(() => D.find((l) => l.id === Oe) ?? Ze, [D, Oe]), ne = Ji({ host: e, requestContext: At.parts, chatSettings: Ae, selectedTarget: ma, setStatus: j }), rt = de(
    () => [...ne.artifacts, ...w].sort((l, h) => h.createdAt - l.createdAt),
    [ne.artifacts, w]
  ), Dt = _ && !Y, pn = !Y && !Dt && !ye, Le = de(() => f.filter((l) => l.dirty), [f]), $r = de(() => {
  ...
  const _r = N((l) => {
    const h = D.find((y) => y.id === l) ?? Ze;
    ze(h.id), Na(On, h.id), j(`Chat target: ${h.label}`);
  }, [D, j]), es = N((l) => {
    if (l.kind === "index-hit") {
      qe((h) => h.filter((y) => y.id !== l.id)), j(`Removed project context: ${l.label}`);
      return;
    }
    xe((h) => h.includes(l.id) ? h : [...h, l.id]), j(`Removed chat context: ${l.label}`);
  }, []), Rt = N(() => {
    We(!0), x(null), ee(!0), j("Atomek settings opened");
  }, []), fa = N(() => {
    We(!1), x((l) => l ?? G.at(-1) ?? null), G.length === 0 && ee(!1), j("Atomek settings closed");
  }, [G]), ut = N((l) => {
    Ke((h) => ({ ...h, [l]: (h[l] ?? 1) + 1 }));
  }, []), ts = N((l) => {
    l.preventDefault(), l.currentTarget.setPointerCapture?.(l.pointerId);
    const h = Sn(r), y = l.clientX, P = ae, Q = (st) => {
      const Nn = P + (y - st.clientX);
      he(Zt(Nn, h.secondaryMin, h.secondaryMax));
    }, pe = () => {
    ...
    l.preventDefault(), l.currentTarget.setPointerCapture?.(l.pointerId);
    const h = Sn(r), y = l.clientX, P = g, Q = (st) => {
      const Nn = P + (st.clientX - y);
      S(Zt(Nn, h.primaryMin, h.primaryMax));
    }, pe = () => {
      window.removeEventListener("pointermove", Q), window.removeEventListener("pointerup", pe);
    };
    window.addEventListener("pointermove", Q), window.addEventListener("pointerup", pe);
    ...
    Gr((h) => {
      ...
      return localStorage.setItem(Or, JSON.stringify(y)), y;
    });
  }, []), ge = N((l, h) => {
    k((y) => y.includes(l.id) ? y : [...y, l.id]), x(l.id), ee(!1), la(h ?? null), re({ lineNumber: h ?? 1, column: 1 });
  }, []), as = N((l) => {
    const h = {
      id: `atomek-doc:${l.id}`,
      name: l.fileName,
    ...
    q((y) => Ut(y, [h])), ge(h), E("chat"), F(!0), I(!0), j(`Opened Atomek guide: ${l.title}`);
  }, [ge]), rs = N((l) => {
    if (!l.fileId) return;
    const h = f.find((y) => y.id === l.fileId);
    h && (ge(h, l.range?.startLineNumber ?? 1), j(`Revealed context: ${l.label}`));
  }, [f, ge]), Lt = N(async () => {
    if (jn(Le, "open new files"))
      try {
        const l = await pi();
        if (l.length === 0) return;
        q((h) => Ut(h, l)), await Promise.all(l.map(async (h) => {
          const y = h.handle ? `file:${h.path}` : void 0;
          h.handle && y && await Wa(y, h.handle), tt({ name: h.name, path: h.path, kind: "file", handleKey: y, at: Date.now() });
        })), ge(l[0]), j(`Opened ${l.length} local file${l.length === 1 ? "" : "s"}`);
      } catch (l) {
        l.name !== "AbortError" && j(`Open file failed: ${l.message}`);
      }
  }, [Le, ge, tt]), bn = N(async () => {
    if (jn(Le, "open another folder"))
      try {
        const l = await bi(), h = l.handle ? `folder:${l.name}:${Date.now()}` : null;
        l.handle && h && await Wa(h, l.handle), ke(h), Pe(l), q(l.files), k([]), x(null), ee(!1), tt({ name: l.name, path: l.name, kind: "folder", handleKey: h ?? void 0, at: Date.now() }), j(`${l.handle ? "Opened local folder" : "Opened browser fallback folder"} ${l.name} (${l.files.length} text files indexed)`);
      } catch (l) {
    ...
    if (!O) return;
    let h = !1;
    q((y) => y.map((P) => P.id !== O || P.content === l ? P : (h = !0, { ...P, content: l, dirty: !0 }))), h && ut(O);
  }, [O, ut]), vn = N(async () => {
    if (Y)
      try {
        const l = await zn(Y);
    ...
    try {
      const h = await Promise.all(l.map((P) => zn(P))), y = new Map(h.map((P) => [P.id, P]));
    ...
    jn(Le, "close all editors") && (k([]), x(null), la(null), ee(!1), j("Closed all editors"));
  }, [Le]), gn = N(() => {
...
`,
    ...
    return qe(P), P.length === 0 ? (j(et.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), h) : (y.mode !== "hybrid" && y.reason ? j(y.reason) : j(`${P.length} project context hit${P.length === 1 ? "" : "s"} · ${y.reason ?? "hybrid retrieval"}`), [
      ...h,
      ...Io(P)
    ]);
  }, [At.parts, we, Ae, e, et]), wn = N(() => {
    if (!Y && He.length === 0) {
      j("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const h = [
      `Create a polished Markdown artifact from ${Y?.path ?? `${He.length} open editors`}.`,
    ...
    F(!0), E("chat"), j("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const y = await nt(h), P = await ne.askAgent(h, { requestContext: y });
      !P || P.status === "error" || ne.createArtifact({
        ...
        F(!0), E("outputs"), K(!0), W("output");
      });
    })();
  }, [Y, ne, nt, He.length]), kn = N(async (l) => {
    if (l.kind === "folder") {
      const y = f.filter((P) => P.path === l.path || P.path.startsWith(`${l.path}/`));
      if (me?.name === l.name || y.length > 0) {
        y.length > 0 && (Pe((P) => P?.name === l.name ? P : { name: l.name, files: y }), q((P) => Ut(P, y))), b("explorer"), v(!0), x(null), ee(!1), tt({ ...l, at: Date.now() }), j(`Opened recent folder ${l.name}`);
        return;
      }
    }
    const h = f.find((y) => y.path === l.path || y.name === l.name);
    if (h) {
      ge(h), j(`Opened recent ${h.name}`);
      return;
    }
    if (!l.handleKey) {
    ...
    try {
      if (l.kind === "folder") {
        const Q = await Cn(l.handleKey);
        if (!Q || !await Tn(Q, "readwrite")) {
          j("Browser permission expired for this folder. Click Open Folder and pick it once to refresh the recent handle.");
          return;
        }
        const pe = await xa(Q);
        ke(l.handleKey), Pe(pe), q(pe.files), k([]), x(null), ee(!1), tt({ ...l, at: Date.now() }), j(`Reopened ${pe.name} (${pe.files.length} text files indexed)`);
        return;
      }
      const y = await Cn(l.handleKey);
      if (!y || !await Tn(y, "readwrite")) {
      ...
      q((Q) => Ut(Q, P)), ge(P[0]), tt({ ...l, at: Date.now() }), j(`Reopened ${P[0].name}`);
    } catch (y) {
      j(`Open recent failed: ${y instanceof Error ? y.message : String(y)}`);
    }
  }, [f, me?.name, ge, tt]), Me = N((l) => {
    F(!0), E("chat"), se(""), (async () => {
      const h = await nt(l);
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const h = Mn(f, Hn(l.title || l.kind)), y = {
    ...
    q((P) => [...P, y]), ge(y), j(`Opened ${l.title} as editable file`);
  }, [f, ge]), Pn = N(() => {
...
`), Q = {
    ...
    A((pe) => [Q, ...pe]), K(!0), W("output"), j(`Captured manual check: ${h}`);
  }, []), ht = N((l, h) => {
    const y = nc({
      body: h,
      files: f,
    ...
    return y.edits.length > 1 ? ($(P), V(null), j(`Previewing AI workspace patch for ${y.edits.length} files`), !0) : y.edits.length === 1 ? (V(P.edits[0]), $(null), j(`Previewing AI patch for ${P.edits[0].fileName}`), !0) : (j(y.skipped.length > 0 ? `No applicable edit found. ${y.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [Y, be, f]), va = N((l, h) => {
    const y = {
    ...
    A((P) => [y, ...P]), F(!0), E("outputs"), K(!0), W("output"), j(`Saved local job output: ${l}`), (h.includes("```diff") || h.includes("--- a/")) && ht(l, h);
  }, [ht]), hs = N(() => {
    const l = ce.trim();
    if (!l) return;
    se("");
    const h = Pl(l);
    (async () => {
      const y = await nt(l), P = await ne.askAgent(l, { requestContext: y });
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", P.body) || j("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    It(y), _e(""), fn(""), ca("failed"), K(!0), W("terminal"), j(h.commands.length > 0 ? `Manual check ready: copy ${h.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [f]), fs = N((l) => {
    l.trim() && (xt(l), It(l), j(`Manual check command copied: ${l}`));
  }, []), ps = N(() => {
    dt((l) => {
      if (!l) return l;
      const h = gc(l, Tt), y = h.commands.at(-1)?.command ?? "";
      return y && It(y), h;
    }), _e("");
  }, [Tt]), bs = N(() => {
    if (!Ge || !Vt.trim()) {
    ...
    const l = Vt.trim();
    dt((h) => h && wc(h, l, Ft, mn)), fn(""), j(`Captured manual check result: ${l} (${Ft})`);
  }, [mn, Vt, Ge, Ft]), vs = N(() => {
    Ge && Me(kc(Ge));
  }, [Me, Ge]), ys = N(() => {
    if (!L) return;
    const l = f.find((h) => h.id === L.fileId);
    if (!l) {
      j(`Cannot apply edit — ${L.fileName} is no longer open`), V(null);
      return;
    }
    l.content !== L.originalContent && !window.confirm(`${L.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (q((h) => h.map((y) => y.id === L.fileId ? { ...y, content: L.proposedContent, dirty: !0 } : y)), ut(L.fileId), Ye(`AI edit applied to ${l.name}. Save All to persist it to disk.`), V(null), Xt(`AI edit applied to ${l.name}`), j(`Applied AI edit to ${l.name} — unsaved; manual check ready`));
  }, [ut, f, L, Xt]), gs = N(() => {
    if (!L) return;
    const l = Mn(f, `${Hn(L.fileName)}-ai-edit`), h = {
      id: `pending-edit-${L.fileId}-${Date.now()}`,
      name: l,
      path: l,
      language: Y?.language ?? "markdown",
      content: L.proposedContent,
      dirty: !0,
      source: "generated"
    };
    q((y) => [...y, h]), ge(h), V(null), j(`Opened proposed edit as ${l}`);
  }, [Y?.language, f, ge, L]), ws = N(() => {
    if (!Z) return;
    const l = new Map(Z.edits.map((y) => [y.fileId, y]));
    f.some((y) => {
      const P = l.get(y.id);
    ...
    })), l.forEach((y, P) => ut(P)), Ye(`AI workspace patch applied to ${l.size} file${l.size === 1 ? "" : "s"}. Save All to persist changes.`), $(null), Xt(`AI workspace patch applied to ${l.size} file${l.size === 1 ? "" : "s"}`), j(`Applied AI workspace patch to ${l.size} file${l.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [ut, f, Z, Xt]), ks = N(() => {
    if (!Z) return;
    const l = [...f], h = Z.edits.map((y) => {
      const P = Mn(l, `${Hn(y.fileName)}-ai-edit`), Q = f.find((st) => st.id === y.fileId), pe = {
    ...
    q((y) => [...y, ...h]), h[0] && ge(h[0]), $(null), j(`Opened ${h.length} proposed edit file${h.length === 1 ? "" : "s"}`);
  }, [f, ge, Z]), ya = N((l) => {
    if (l === "explain") {
  ...
  return oe(() => {
    const l = (h) => {
      (h.metaKey || h.ctrlKey) && (h.key.toLowerCase() === "o" && (h.preventDefault(), Lt()), h.key.toLowerCase() === "s" && (h.preventDefault(), vn()), h.key.toLowerCase() === "w" && (h.preventDefault(), O && yn(O)), h.key.toLowerCase() === "b" && (h.preventDefault(), v((P) => !P)), h.shiftKey && h.key.toLowerCase() === "f" && (h.preventDefault(), b("search"), v(!0)), (h.key.toLowerCase() === "k" || h.key.toLowerCase() === "p") && (h.preventDefault(), je(!0)));
    };
    return window.addEventListener("keydown", l), () => window.removeEventListener("keydown", l);
  }, [O, yn, Lt, vn]), oe(() => {
    const l = (h) => {
      Le.length !== 0 && (h.preventDefault(), h.returnValue = "");
    };
    ...
    if (!r) return;
    const l = Sn(r);
    S((h) => Zt(h, l.primaryMin, l.primaryMax)), he((h) => Zt(h, l.secondaryMin, l.secondaryMax));
  }, [r]), oe(() => {
    const l = { primaryVisible: p, primaryWidth: g, secondaryVisible: J, secondaryWidth: ae, markdownPreviewVisible: z };
    localStorage.setItem(Sr, JSON.stringify(l));
  }, [z, p, g, J, ae]), oe(() => {
    if (!o.folder?.handleKey) return;
    let l = !1;
    return (async () => {
      try {
        const h = await Cn(o.folder?.handleKey ?? "");
        if (!h || !await Tn(h, "readwrite") || l) return;
        const y = await xa(h);
        if (l) return;
        ke(o.folder?.handleKey ?? null), Pe(y), q((P) => Yl(P, y.files)), k((P) => P.filter((Q) => y.files.some((pe) => pe.id === Q) || f.some((pe) => pe.id === Q))), j(`Restored ${y.name} (${y.files.length} text files indexed)`);
      } catch (h) {
    ...
    ql({
      activity: m,
      folder: me ? { name: me.name, handleKey: Ce } : null,
      files: El(f),
      openEditorIds: G,
      activeFileId: O,
      query: X,
  ...
  }, [m, O, U, M, ce, Ce, f, me, G, X, zt, H, ye]), oe(() => {
    localStorage.setItem(Hr, JSON.stringify(Ae));
  }, [Ae]), /* @__PURE__ */ u(
    "div",
    {
      ref: n,
      className: `workbench-workbench ${p ? "" : "no-primary"} ${J ? "" : "no-secondary"} ${M ? "has-bottom-panel" : ""}`,
      ...
      children: [
        ...
        p && /* @__PURE__ */ u("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            ol,
            {
              ...
              openFile: Lt,
              openFolder: bn,
              openWorkbenchFile: ge,
              newFile: gn,
              recent: zt,
              reopenRecent: kn,
              setStatus: j,
              hasFsAccess: fi(),
              attachSkillToChat: ba,
              saveLocalJobOutput: va,
              activeFile: Y
            }
          ),
        ...
        /* @__PURE__ */ u("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => je(!0), children: t("app.workspace") }),
          /* @__PURE__ */ u("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ a(dl, { file: Y, folder: me, showWelcome: pn }),
            Fe && Le.length > 0 ? /* @__PURE__ */ u("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: Fe }),
              /* @__PURE__ */ a("button", { onClick: () => {
                pa();
              }, children: t("shell.saveAll") }),
              /* @__PURE__ */ a("button", { onClick: () => Ye(null), title: t("shell.dismiss"), children: /* @__PURE__ */ a(Re, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: Y ? /* @__PURE__ */ u("div", { className: Y.language === "markdown" && z ? "workbench-editor-split" : "workbench-editor-single", children: [
              ...
              Y.language === "markdown" && z && /* @__PURE__ */ a(yl, { content: Y.content })
            ] }) : Dt ? /* @__PURE__ */ a(
              zl,
              {
                host: e,
            ...
            ) : pn ? /* @__PURE__ */ a(fl, { host: e, openFile: Lt, openFolder: bn, newFile: gn, recent: zt, reopenRecent: kn, setStatus: j, openControlTower: () => {
              b("computer"), v(!0);
            }, openChat: () => {
              E("chat"), F(!0);
            }, openEmbeddedDoc: as }) : /* @__PURE__ */ u("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(dn, { size: 34 }),
            ...
            M && /* @__PURE__ */ a(
              wl,
              {
                ...
                runAiSynthesis: wn,
                captureManualCheck: Pn,
                openOutputAsFile: xn,
                manualCheckSession: Ge,
                manualCheckCommandInput: Tt,
                setManualCheckCommandInput: _e,
                manualCheckOutputInput: mn,
                setManualCheckOutputInput: fn,
                manualCheckSelectedCommand: Vt,
                setManualCheckSelectedCommand: It,
                manualCheckStatus: Ft,
                setManualCheckStatus: ca,
        ...
        J && /* @__PURE__ */ a(
          kl,
          {
            ...
            rememberMessage: ds,
            previewEditFromMessage: (l) => ht(l.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", l.body),
            ...
            captureManualCheck: Pn,
            openOutputAsFile: xn,
            previewEditFromOutput: (l) => ht(l.title, l.body),
        ...
        Te && /* @__PURE__ */ a(
          vl,
          {
            ...
            activeFile: Y,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: gn },
              ...
              { label: "File: Save All", detail: `${Le.length} dirty file${Le.length === 1 ? "" : "s"}`, run: () => {
                pa();
              }, disabled: Le.length === 0 },
              { label: "File: Close All Editors", detail: `${He.length} open editor${He.length === 1 ? "" : "s"}`, run: os, disabled: He.length === 0 },
              ...
              { label: "View: Toggle Bottom Panel", detail: M ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => K((l) => !l) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Rt },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => Xt("Manual check requested from command palette") },
              ...
              { label: "Atomek: Create AI Synthesis", detail: Y || He.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: wn, disabled: !Y && He.length === 0 },
              { label: "AI: Explain Active File", detail: Y ? `Ask Cortex to explain ${Y.path}` : "Open a file first", run: () => Me("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !Y },
              { label: "AI: Improve Active File", detail: Y ? `Ask Cortex for concrete edits to ${Y.path}` : "Open a file first", run: () => Me("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !Y },
              { label: "AI: Draft Editable Replacement", detail: Y ? `Ask Cortex for a full-file replacement for ${Y.path}` : "Open a file first", run: () => ya("edit"), disabled: !Y },
              { label: "AI: Plan Workspace Work", detail: He.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => Me("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: He.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: Y ? "Persist active file in host.ai artifacts" : "Open a file first", run: us, disabled: !Y },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Pn },
              { label: "AI: Open Latest Artifact as File", detail: rt[0] ? `Create editable file from ${rt[0].title}` : "No outputs yet", run: () => rt[0] && xn(rt[0]), disabled: rt.length === 0 }
            ],
            openWorkbenchFile: ge,
            onClose: () => je(!1)
          }
        ...
        /* @__PURE__ */ a(Ml, { status: Br, file: Y ?? Kc, cursor: C, fileCount: f.length, dirtyCount: Le.length })
      ]
    }
  );
}
function il({ active: e, setActive: t, togglePrimary: n, openSettings: r, settingsActive: i }) {
  const s = Ee();
  return /* @__PURE__ */ u("aside", { className: "workbench-activity-bar", "aria-label": s("activity.aria"), children: [
    /* @__PURE__ */ a("button", { className: "workbench-activity-brand", title: s("activity.toggleSidebar"), "aria-label": s("activity.toggleSidebar"), onClick: n, children: /* @__PURE__ */ a(Ar, { size: 30, variant: "cream" }) }),
    /* @__PURE__ */ a(bt, { icon: /* @__PURE__ */ a(Ua, { size: 25 }), label: s("activity.explorer"), active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(bt, { icon: /* @__PURE__ */ a(Qa, { size: 25 }), label: s("activity.search"), active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(bt, { icon: /* @__PURE__ */ a(Ga, { size: 25 }), label: s("activity.sourceControl"), active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(bt, { icon: /* @__PURE__ */ a(Ja, { size: 25 }), label: s("activity.runDebug"), active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(bt, { icon: /* @__PURE__ */ a(Zn, { size: 25 }), label: s("app.agentTeam"), active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(bt, { icon: /* @__PURE__ */ a(oi, { size: 23 }), label: s("app.settings"), active: i, onClick: r })
  ] });
}
function bt({ icon: e, label: t, active: n, onClick: r }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: r, children: e });
}
function ol(e) {
  const t = Ee();
  return e.activity === "search" ? /* @__PURE__ */ a(pl, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(Ea, { title: t("activity.sourceControl").toUpperCase(), body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(Ea, { title: t("activity.runDebug").toUpperCase(), body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(Zr, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(cl, { ...e });
}
function cl(e) {
  const t = Ee(), n = !e.folder, r = de(() => ll(e.files, e.folder?.name), [e.files, e.folder?.name]), [i, s] = T(() => /* @__PURE__ */ new Set());
  oe(() => {
    const c = Rr(r);
    s((m) => {
      const b = new Set(m);
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      n ? /* @__PURE__ */ u(Ve, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: t("explorer.noOpenEditors") }) : e.openEditors.map((c) => /* @__PURE__ */ a(oa, { file: c, active: c.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(c), label: c.name, detail: c.path }, c.id)),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(jt, { size: 12 }),
          " ",
          e.folder?.name ?? t("app.workspace")
        ] }),
        r.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: t("explorer.noReadableFiles") }) : Er(r, e.activeFileId, e.openWorkbenchFile, i, o)
      ] }),
      ...
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: t("explorer.noRecentFolders") }) : e.recent.map((c) => /* @__PURE__ */ u("button", { className: "workbench-tree-row", onClick: () => {
        ...
        c.kind === "file" ? /* @__PURE__ */ a(Un, { size: 14 }) : /* @__PURE__ */ a(Ba, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: c.name })
      ] }, `${c.path}-${c.at}`))
    ] })
  ] });
}
function oa({ file: e, active: t, onOpen: n, basePath: r, depth: i = 0, label: s, detail: o }) {
...
function Rr(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...Rr(t.children)]);
}
function Er(e, t, n, r, i, s = 0) {
  return e.map((o) => {
    if (o.file)
      return /* @__PURE__ */ a(oa, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: s, label: o.name }, o.file.id);
    ...
    return /* @__PURE__ */ u("div", { children: [
      /* @__PURE__ */ u("button", { className: "workbench-folder-row", style: { "--workbench-depth": s }, onClick: () => i(o.path), title: c ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        c ? /* @__PURE__ */ a(jt, { size: 12 }) : /* @__PURE__ */ a(jt, { className: "workbench-chevron-collapsed", size: 12 }),
        c ? /* @__PURE__ */ a(Gn, { size: 14 }) : /* @__PURE__ */ a(Ba, { size: 14 }),
      ...
      c ? Er(o.children, t, n, r, i, s + 1) : null
    ] }, o.path);
  });
}
function dl({ file: e, folder: t, showWelcome: n }) {
  const r = Ee(), i = n ? [r("app.agentTeam")] : e?.path.split("/").filter(Boolean) ?? [], s = t && i[0] === t.name ? i.slice(1) : i;
...
function ul(e) {
  ...
  return /* @__PURE__ */ u("div", { className: "workbench-tabs", children: [
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: t("tabs.toggleMarkdownPreview"), onClick: e.togglePreview, children: /* @__PURE__ */ a(Kn, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: t("tabs.toggleChat"), onClick: e.toggleSecondary, children: /* @__PURE__ */ a(ti, { size: 16 }) })
  ] });
}
function hl(e) {
...
function fl({
  ...
  const [p, v] = T("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [g, S] = T(null), [H, E] = T([]), [J, F] = T(!1), [ae, he] = T(null), [M, K] = T(null), [U, W] = T("pod-local"), z = N(async () => {
    F(!0), K(null);
    try {
      ...
      K(O), o(`Agent team resource load failed: ${O}`);
    } finally {
      F(!1);
    }
  ...
  const ye = N(async () => {
    ...
    const k = p.trim() || "Coordinate a Tytus mission.", O = lt(k, g, U);
    F(!0);
    try {
      ...
      await e.missions.write({
        rootPath: x.rootPath,
        files: [
          { path: "MISSION.md", content: rn(X, g, null, [], k, O) },
          { path: "MISSION.json", content: Dr(X, g, k, O) },
          { path: "RESOURCES.md", content: sn(g) },
          { path: "TASKS.md", content: Ir(Mt(k, g, O)) },
          { path: "HANDOFF.md", content: Fr(X) },
...
`) }
      ...
      }), he(x), St(X), o(`Mission created: ${x.rootPath}`), c();
    } catch (x) {
      o(`Mission create failed: ${x instanceof Error ? x.message : String(x)}`);
    } finally {
      F(!1);
    }
  }, [p, g, e.missions, c, o, U]), ee = hl(g), me = $c(g), Pe = _c(g), Ce = ml(g), ke = g?.warnings ?? [], f = tl(g), q = Nt(g, lt(p, g, U)), G = Mt(p, g, q.id);
  return /* @__PURE__ */ u("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ u("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ u("div", { className: "workbench-brand-lockup", "aria-label": "Atomek", children: [
        /* @__PURE__ */ a(Ar, { size: 74, variant: "acid" }),
      ...
      M ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: M }) : null
    ] }),
    /* @__PURE__ */ u("section", { className: "workbench-control-grid", children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-agent-brand-grid", children: me.map((k) => /* @__PURE__ */ u("div", { className: `workbench-agent-brand-card ${k.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ u("div", { children: [
            /* @__PURE__ */ a("strong", { children: k.label }),
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-doc-grid", children: qc.map((k) => /* @__PURE__ */ u("button", { className: "workbench-doc-card", onClick: () => b(k), title: k.summary, children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: Pe.map((k, O) => /* @__PURE__ */ u("div", { className: `workbench-fabric-node ${k.status.replace(/\s+/g, "-")}`, children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-fabric-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => I(Jt.sharedFiles, "Shared Files"), children: "Open shared files" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => I(Jt.podInspector, "Pod Inspector"), children: "Open pods" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => I(Jt.channels, "Channels"), children: "Open channels" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => I(Jt.settings, "Agent Settings"), children: "Open agent settings" })
        ] })
      ] }),
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        ...
        H.slice(0, 4).map((k) => /* @__PURE__ */ u("button", { className: "workbench-control-preset", onClick: () => {
          St(k), c();
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-control-resource-list", children: [
          Ce.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          Ce.map((k) => /* @__PURE__ */ u("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: Se(k) }),
              /* @__PURE__ */ a("span", { children: ia(k) })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${k.status}`, children: k.status })
          ] }, k.id))
        ] }),
        ke.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: ke.slice(0, 2).map((k) => /* @__PURE__ */ u("span", { children: [
          k.code,
          ": ",
          k.message
...
function pl({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: i }) {
  const s = Ee(), o = de(() => bl(e, t), [e, t]), c = de(() => {
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? c.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: s("search.noMatches") }) : c.map((m) => {
        ...
        return /* @__PURE__ */ u("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(oa, { file: b, active: b.id === i, onOpen: () => r(b) }),
...
function vl(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${tr(i.language)} file`,
...
function gl(e) {
  const t = e.session ? xc(e.session) : "pending";
...
function wl(e) {
  return /* @__PURE__ */ u("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(Jr, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function kl(e) {
  const t = Ee(), [n, r] = T(!1), [i, s] = T(""), [o, c] = T([]), m = Xe(null), b = Xe(null), p = Xe(null), [v, g] = T({ top: 0, right: 0 }), S = de(
    () => e.chatThreads.map((z) => ({
      kind: "atomek-thread",
      id: `thread:${z.id}`,
      title: z.title,
    ...
    const I = e.chatTargets.find((ye) => ye.id === z.targetId);
    return I ? {
      kind: "pod-agent",
      id: `agent:${z.targetId}`,
      title: I.label,
  ...
  }).filter((z) => z !== null), [o, e.chatTargets]), E = de(() => [...S, ...H].sort((z, I) => I.lastActivityAt - z.lastActivityAt), [S, H]), J = de(() => e.selectedChatTarget.kind === "atomek-ai" && e.chatThread ? E.find((z) => z.kind === "atomek-thread" && z.thread.id === e.chatThread?.id) ?? null : e.selectedChatTarget.kind !== "atomek-ai" ? E.find((z) => z.kind === "pod-agent" && z.targetId === e.selectedChatTarget.id) ?? null : null, [E, e.selectedChatTarget, e.chatThread]), F = N((z, I) => I ? `${z.title} ${z.kind === "pod-agent" ? z.subtitle : ""}`.toLowerCase().includes(I) : !0, []), ae = i.trim().toLowerCase(), he = J ? F(J, ae) : !1, M = de(() => E.filter((z) => z.id !== J?.id).filter((z) => F(z, ae)), [E, J, ae, F]), K = N(() => {
    c(Di());
  }, []), U = N((z) => {
    ...
    window.confirm(`Delete conversation "${z.title}"?`) && (z.kind === "atomek-thread" ? e.deleteThread(z.thread.id) : (Ri(z.targetId), K()));
  }, [e, K]);
  return oe(() => {
    if (!n) {
      s("");
    ...
    const z = () => {
      ...
      g({
        top: me.bottom + 6,
        right: Math.max(8, window.innerWidth - me.right)
      });
    };
    z(), window.setTimeout(() => p.current?.focus(), 0);
    const I = (ee) => {
      const me = ee.target;
      me && (b.current?.contains(me) || m.current?.contains(me) || r(!1));
    }, ye = (ee) => {
      ee.key === "Escape" && r(!1);
    };
    return window.addEventListener("resize", z), window.addEventListener("scroll", z, !0), window.addEventListener("mousedown", I), window.addEventListener("keydown", ye), () => {
      window.removeEventListener("resize", z), window.removeEventListener("scroll", z, !0), window.removeEventListener("mousedown", I), window.removeEventListener("keydown", ye);
    };
    ...
    /* @__PURE__ */ u("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ u("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: t("secondary.newChat"), onClick: e.newChat, children: /* @__PURE__ */ a(In, { size: 15 }) }),
        /* @__PURE__ */ a(
          "button",
          {
            ref: m,
            ...
            children: /* @__PURE__ */ a(Za, { size: 15 })
          }
        ),
        /* @__PURE__ */ a("button", { title: t("secondary.chatSettings"), onClick: e.openSettings, children: /* @__PURE__ */ a(Ka, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: t("secondary.closeChat"), onClick: e.onClose, children: /* @__PURE__ */ a(Re, { size: 15 }) })
      ] })
    ] }),
        ...
        "div",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ u("div", { className: "workbench-history-portal-list", children: [
              ...
              M.length > 0 ? /* @__PURE__ */ u(Ve, { children: [
                ...
                M.map((z) => /* @__PURE__ */ u(
                  "div",
                  {
                    ...
                    children: [
                      ...
                      /* @__PURE__ */ a("small", { children: Jn(z.lastActivityAt) }),
                      /* @__PURE__ */ a(
                        "button",
                        {
                          className: "workbench-history-portal-delete",
                          onClick: (I) => {
                            I.stopPropagation(), W(z);
                          },
                          title: t("history.deleteConversation"),
                          "aria-label": t("history.deleteConversation"),
                          children: /* @__PURE__ */ a(ka, { size: 13 })
              ...
              !he && M.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-history-portal-empty", children: i.trim() ? t("history.noMatches") : t("history.empty") }) : null
            ] }),
    ...
    e.tab === "chat" ? /* @__PURE__ */ a(xl, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      Zr,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ a(Jr, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function qr({ body: e }) {
  const t = de(() => Bc(e), [e]), [n, r] = T(null), i = N((s, o) => {
    (async () => await xt(o) && (r(s), window.setTimeout(() => r((m) => m === s ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((s) => {
      ...
      return /* @__PURE__ */ u("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ u("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ u("button", { onClick: () => i(s.key, s.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ a(Vs, { size: 12 }) : /* @__PURE__ */ a(Vn, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: sr(s.body) }
      },
      s.key
    );
  }) });
...
function xl(e) {
  const t = Ee(), n = e.selectedChatTarget.available, r = e.selectedChatTarget.kind === "atomek-ai", i = e.chatInput.trim().length > 0 && !e.busy && n, s = Xe(null), [o, c] = T(!0), [m, b] = T(!1), p = Xe(null), v = Xe(null), g = Xe(null), S = Xe(null), [H, E] = T(!1), [J, F] = T(0), ae = typeof window < "u" && !!(window.SpeechRecognition || window.webkitSpeechRecognition), he = N(() => {
    const f = g.current;
    try {
      f?.stop?.();
    } catch {
  ...
  }, U = () => p.current?.removeAttribute("open"), W = () => v.current?.removeAttribute("open"), z = (f) => {
    e.setContextScope(f), f === "indexed-project" && e.refreshProjectIndex(), U();
  }, I = (f) => {
    e.selectChatTarget(f), W();
  }, ye = de(() => e.chatMessages.map((f) => `${f.id}:${f.status ?? ""}:${f.body.length}`).join("|"), [e.chatMessages]);
  oe(() => {
    const f = s.current;
  ...
  const ee = N(() => {
    ...
    xt(f.body);
  }, []), Ce = Xe(null), ke = () => Ce.current?.removeAttribute("open");
  return /* @__PURE__ */ u("div", { className: "workbench-chat-wrap", children: [
      ...
      /* @__PURE__ */ u("span", { className: "workbench-chat-thread-actions", children: [
        /* @__PURE__ */ a(
          "button",
          {
            ...
            children: /* @__PURE__ */ a(In, { size: 15 })
          }
        ),
        r && e.chatThreads.length > 0 ? /* @__PURE__ */ u("details", { ref: Ce, className: "workbench-chat-iconmenu", children: [
          /* @__PURE__ */ a("summary", { className: "workbench-chat-iconbtn", title: t("chat.history"), "aria-label": t("chat.history"), children: /* @__PURE__ */ a(Za, { size: 15 }) }),
          /* @__PURE__ */ u("div", { className: "workbench-chat-history-pop", role: "menu", children: [
            ...
            e.chatThreads.map((f) => /* @__PURE__ */ u(
              "button",
              {
                ...
                onClick: () => {
                  e.selectThread(f.id), ke();
                },
                title: f.title,
                children: [
                  /* @__PURE__ */ a("span", { className: "workbench-chat-history-title", children: f.title }),
                  /* @__PURE__ */ a("small", { children: Jn(f.lastMessageAt ?? f.updatedAt) })
                ]
              },
              f.id
            ))
        ...
        r ? /* @__PURE__ */ u("details", { className: "workbench-chat-iconmenu", children: [
          /* @__PURE__ */ a("summary", { className: "workbench-chat-iconbtn", title: t("chat.actions"), "aria-label": t("chat.actions"), children: /* @__PURE__ */ a(Ka, { size: 15 }) }),
    ...
    /* @__PURE__ */ u("div", { ref: s, className: "workbench-chat-transcript", onScroll: ee, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ u("div", { children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(qr, { body: f.body }),
        f.gatewayLabel ? /* @__PURE__ */ u(Ve, { children: [
          /* @__PURE__ */ a("br", {}),
        ...
        f.role === "assistant" && f.status !== "streaming" && f.status !== "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action", onClick: () => Pe(f), title: t("chat.copyAnswer"), "aria-label": t("chat.copyAnswer"), children: /* @__PURE__ */ a(Vn, { size: 14 }) }),
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action", onClick: () => e.saveMessageAsArtifact(f), title: t("chat.saveArtifact"), "aria-label": t("chat.saveArtifact"), children: /* @__PURE__ */ a(Bn, { size: 14 }) }),
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action", onClick: () => e.rememberMessage(f), title: "Store in Atomek memory", "aria-label": t("chat.remember"), children: /* @__PURE__ */ a(Ga, { size: 14 }) }),
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action", onClick: () => e.previewEditFromMessage(f), disabled: e.workspaceFileCount === 0, title: t("chat.previewPatch"), "aria-label": t("chat.previewPatch"), children: /* @__PURE__ */ a(Kn, { size: 14 }) }),
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(f), disabled: e.busy, title: t("chat.regenerate"), "aria-label": t("chat.regenerate"), children: /* @__PURE__ */ a(gt, { size: 14 }) })
        ] }) : null,
        f.role === "assistant" && f.status === "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action", onClick: () => Pe(f), title: t("chat.copyError"), "aria-label": t("chat.copyError"), children: /* @__PURE__ */ a(Vn, { size: 14 }) }),
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(f), disabled: e.busy, title: t("chat.retry"), "aria-label": t("chat.retry"), children: /* @__PURE__ */ a(gt, { size: 14 }) })
        ] }) : null
      ] }, f.id)),
      m ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: me, children: t("chat.jumpLatest") }) : null
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ u("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: Gi(e.contextScope) }),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ u("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ u("span", { className: "workbench-chat-chip muted", children: [
            ...
            return /* @__PURE__ */ u("span", { className: "workbench-chat-chip", title: O, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(f), disabled: !f.fileId, title: t("chat.revealContext"), children: /* @__PURE__ */ a(wa, { size: 13 }) }),
              f.label,
              ...
              f.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(f), title: t("chat.removeContext"), children: /* @__PURE__ */ a(Re, { size: 11 }) }) : null
            ] }, f.id);
          }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar atomek-input", children: [
          /* @__PURE__ */ u("details", { ref: p, className: "workbench-chat-attach", children: [
            /* @__PURE__ */ a("summary", { title: t("chat.addContext"), "aria-label": t("chat.addContext"), children: /* @__PURE__ */ a(In, { size: 16 }) }),
          ...
          H ? /* @__PURE__ */ u("div", { className: "workbench-chat-recording", children: [
            /* @__PURE__ */ a("button", { className: "workbench-chat-recording-cancel", onClick: he, title: t("chat.cancelRecording"), "aria-label": t("chat.cancelRecording"), children: /* @__PURE__ */ a(Re, { size: 14 }) }),
            ...
            /* @__PURE__ */ a("span", { className: "workbench-chat-recording-time", children: K(J) })
          ...
          /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar-right", children: [
            H ? null : /* @__PURE__ */ a(
              "button",
              {
                ...
                children: /* @__PURE__ */ a(_s, { size: 16 })
              }
            ),
            ...
            e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: t("chat.stop"), "aria-label": t("chat.stop"), children: /* @__PURE__ */ a(_a, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${i ? "ready" : ""}`, onClick: e.askAgent, title: t("chat.send"), disabled: !i, "aria-label": t("chat.sendMessage"), children: /* @__PURE__ */ a(js, { size: 18 }) })
          ] })
        ] })
      ] })
    ] })
...
function Fa(e, t, n) {
  e.model;
  const i = [e.gatewayPreference === "auto" ? t : Yr(e.gatewayPreference)];
...
function Tl({ host: e, appName: t, currentVersion: n }) {
  const r = e.apps, [i, s] = T(null), [o, c] = T(!1), [m, b] = T(null), p = N(async () => {
    ...
    c(!0), b(null);
    try {
      const H = await r.checkUpdate();
      s(H), H.error && b(H.error);
    } catch (H) {
  ...
  const v = N(async () => {
    ...
    c(!0), b(null);
    try {
      const H = await r.updateSelf();
      s(H), b(H.error ?? `${t} updated. Close and reopen the app to load the new bundle.`);
    } catch (H) {
      b(H instanceof Error ? H.message : String(H));
    } finally {
      c(!1);
...
function zl(e) {
  ...
  oe(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(H.map((E) => ({ id: E.id, gatewayLabel: E.gatewayLabel }))), i(H.length > 0 ? `${H.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (H) {
        if (g.signal.aborted) return;
        n([]), i(`Model discovery failed: ${H instanceof Error ? H.message : String(H)}`);
      }
    })(), () => g.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ u("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ a(Tl, { host: e.host, appName: "Atomek", currentVersion: Va }),
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          Yr(e.chatSettings.gatewayPreference),
    ...
    /* @__PURE__ */ u("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(an), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function Jr({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: i, openOutputAsFile: s, previewEditFromOutput: o, canPreviewEdit: c = !1, compact: m = !1 }) {
  const b = Ee();
  return /* @__PURE__ */ u("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((p) => /* @__PURE__ */ u("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ u("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void xt(p.body), children: "Copy" }),
      ...
      /* @__PURE__ */ a(qr, { body: p.body })
    ] }, p.id))
  ] });
}
function Al({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ u("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ a("button", { title: "Close", onClick: r, children: /* @__PURE__ */ a(Re, { size: 16 }) })
    ] }),
...
function Ll({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
...
function Cl(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, Xl(e.content), "```");
  else if (t.length > 0) {
...
function Hl(e) {
  ...
  return typeof r == "string" ? r : Array.isArray(r) ? r.map((i) => typeof i == "string" ? i : i && typeof i == "object" && "text" in i && typeof i.text == "string" ? i.text : "").join("") : JSON.stringify(e, null, 2);
}
function Zr({
  host: e,
  setStatus: t,
  ...
  const [c, m] = T([]), [b, p] = T([]), [v, g] = T(null), [S, H] = T(!1), [E, J] = T(null), [F, ae] = T("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [he, M] = T(null), [K, U] = T([]), [W, z] = T(() => al()), [I, ye] = T([]), [ee, me] = T([]), [Pe, Ce] = T([]), ke = Xe([]), [f, q] = T("task-execute"), [G, k] = T("mission"), O = K.find((d) => d.status === "running" || d.status === "canceling") ?? K[0] ?? null, x = o === "dock", X = de(() => Nt(v, lt(F || W?.goal || "", v, W?.teamPresetId)), [F, W?.goal, W?.teamPresetId, v]), R = de(() => Mt(F || W?.goal || "", v, X.id), [F, W?.goal, X.id, v]), C = R.find((d) => d.id === f) ?? R[1] ?? R[0] ?? null, re = s.filter((d) => d.dirty).length, ue = i ? `${i.path} · ${i.language} · ${i.content.length.toLocaleString()} chars${i.dirty ? " · dirty" : ""}` : s.length > 0 ? `${s.length} open editor${s.length === 1 ? "" : "s"}${re ? ` · ${re} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", ve = de(() => [
    {
  ...
  const be = N(async (d = W) => {
    if (!d?.rootPath || !e.missions?.listRuns)
      return Ce([]), [];
    try {
      const w = await e.missions.listRuns(d.rootPath);
      return Ce(w), ke.current = w, w;
    } catch (w) {
      return t(`Mission run history failed: ${w instanceof Error ? w.message : String(w)}`), [];
    }
  }, [e.missions, W, t]), Ke = N(async (d, w) => {
    if (!d) return;
    const A = [...w].sort((V, Z) => Ia(Z).localeCompare(Ia(V))).slice(0, 100), L = A.map((V) => JSON.stringify(V)).join(`
`) + (A.length ? `
` : "");
    d.rootPath && e.missions?.write ? await e.missions.write({ rootPath: d.rootPath, files: [{ path: "RUNS.jsonl", content: L }] }) : d.handle && await qn(d.handle, "RUNS.jsonl", L), Ce(A), ke.current = A;
  }, [e.missions]), we = N(async (d, w = W) => {
    if (!w) return;
    const A = [
      d,
      ...ke.current.filter((L) => L.id !== d.id)
    ];
    await Ke(w, A);
  }, [W, Ke]), Ue = N(async () => {
    ...
    H(!0), J(null);
    try {
      const [d, w, A, L] = await Promise.all([
        e.local?.listTools?.().catch((V) => (t(`Local tool discovery failed: ${V instanceof Error ? V.message : String(V)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((V) => (t(`Skill registry discovery failed: ${V instanceof Error ? V.message : String(V)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((V) => (t(`Resource graph discovery failed: ${V instanceof Error ? V.message : String(V)}`), null)) ?? Promise.resolve(null),
        e.missions?.list?.().catch((V) => (t(`Mission list failed: ${V instanceof Error ? V.message : String(V)}`), [])) ?? Promise.resolve([])
      ]);
      m(d), p(w), g(A), ye(L), W?.rootPath && be(W), t(`Agent team loaded · ${d.length} tools · ${w.length} skills · ${L.length} missions${A ? ` · ${A.resources.length} resources` : ""}`);
    } catch (d) {
      J(d instanceof Error ? d.message : String(d));
    } finally {
      H(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, be, W, t]);
  oe(() => {
    Ue();
  }, [Ue]), oe(() => {
    const d = (w) => {
      const A = w.detail;
      A?.missionId && (z(A), be(A)), Ue();
    };
  ...
  }, [be, t]), xe = N(async (d, w, A = []) => {
    const L = [
      ...ee,
      ...A,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], V = lt(w || d.goal, v, d.teamPresetId), Z = Mt(w || d.goal, v, V), $ = [
      { path: "MISSION.md", content: rn(d, v, i, s, w, V) },
      { path: "MISSION.json", content: Dr(d, v, w, V) },
      { path: "RESOURCES.md", content: sn(v) },
      { path: "TASKS.md", content: Ir(Z) },
      { path: "HANDOFF.md", content: Fr(d) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: L.map((_) => JSON.stringify(_)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: ke.current.map((_) => JSON.stringify(_)).join(`
`) + (ke.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (d.rootPath && e.missions?.write)
      ...
      await vt(d.handle, "runs"), await vt(d.handle, "outputs"), await vt(d.handle, "proposals"), await vt(d.handle, "approvals");
      for (const _ of $) await Qc(d.handle, _.path, _.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    me(L), St(d);
  }, [i, e.missions, ee, s, v]), Ne = N(async (d, w = {}) => {
    if (W)
      return await xe(W, d), W;
    const A = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, L = d.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let V = null;
    if (e.missions?.create) {
      const $ = await e.missions.create({ title: A, goal: L });
      V = {
        missionId: $.missionId,
        title: $.title,
        goal: $.goal,
      ...
      V = {
        handle: $,
        name: $.name,
        missionId: `mission-${Date.now()}-${nl($.name)}`,
        title: A,
        goal: L,
        source: "browser"
      };
    ...
    if (!V) return null;
    const Z = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${V.rootPath ?? V.name}` };
    return z(V), St(V), me([Z]), await xe(V, L, [Z]), t(`Mission pack ready in ${V.rootPath ?? V.name}`), V;
  }, [e.missions, W, t, xe]), qe = N(async (d) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const w = F.trim() || `Open ${d.label} from Atomek with current context.`, A = d.kind === "ai-cli" ? await Ne(w) : W;
      await e.local.openTerminal({
        toolId: d.id,
        command: d.command,
        cwd: A?.rootPath ?? Ra(i),
        prompt: A ? `Atomek mission pack ready at ${A.rootPath ?? A.name}. Read MISSION.md and RESOURCES.md. ${ue}` : `Opened from Atomek. ${ue}`
  ...
  }, [i, ue, Ne, e.local, F, W, t]), te = N(async () => {
    try {
      await Ne(F.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (d) {
      t(`Mission folder setup failed: ${d instanceof Error ? d.message : String(d)}`);
    }
  }, [Ne, F, t]), B = N(async (d, w, A, L = W, V) => {
    if (!L) return;
    const Z = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), $ = V ?? `runs/${Z}-${d.id}.md`, _ = $.split("/").pop() || `${Z}-${d.id}.md`;
    if (L.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: L.rootPath, files: [{ path: $, content: w }] });
    else if (L.handle) {
      const We = await vt(L.handle, "runs");
      await qn(We, _, w);
    }
    await xe(L, F, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${d.label} exited ${A}; transcript saved to ${$}`,
      data: { toolId: d.id, exitCode: A, transcript: $ }
    }]);
  }, [e.missions, F, W, xe]), ce = N(async (d) => {
    ...
    let A = null;
    try {
      A = await Ne(w), A && await xe(A, w, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${d.label} background review started`,
        data: { toolId: d.id, taskId: C?.id ?? "manual", taskTitle: C?.title ?? "Manual run" }
      }]);
    } catch (Z) {
      t(`Mission pack failed before local job start: ${Z instanceof Error ? Z.message : String(Z)}`);
      return;
    }
    M(d.id);
    const L = `local-run-${Date.now()}-${d.id}`, V = pt();
    U((Z) => [{
      id: L,
      toolId: d.id,
      label: d.label,
      ...
      startedAt: Date.now(),
      taskId: C?.id ?? "manual",
      taskTitle: C?.title ?? "Manual run",
      lines: [`[Atomek] Starting ${d.label} local job for ${C?.title ?? "manual run"}…`]
    }, ...Z].slice(0, 6));
    try {
      const Z = await e.local.runJob({
        toolId: d.id,
        prompt: A ? [
...
`)
      }), $ = Z.transcriptPath ?? (A?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${d.id}.md` : void 0), _ = [
        `[Atomek] Started ${d.label} local job ${Z.id}`,
        C ? `[Atomek] Task: ${C.title} (${C.id})` : "[Atomek] Task: manual",
        ...
        taskTitle: C?.title ?? "Manual run",
        transcriptPath: $,
        summary: `Started ${d.label} for ${C?.title ?? "manual run"}`
      };
      we(We, A).catch((ie) => {
        t(`Mission run index failed: ${ie instanceof Error ? ie.message : String(ie)}`);
      }), U((ie) => ie.map((D) => D.id === L ? { ...D, jobId: Z.id, transcriptPath: $ } : D));
      const Ae = (ie) => {
        U((D) => D.map((fe) => fe.id === L ? ie(fe) : fe));
      };
      e.local.streamJob(Z.id, {
        onLog: (ie) => {
          _.push(ie), Ae((D) => ({ ...D, lines: _.slice(-500) }));
        },
        onDone: (ie) => {
          ie && (_.push(ie), Ae((D) => ({ ...D, lines: _.slice(-500) })));
        },
        onFail: (ie) => {
          _.push(`[FAIL] ${ie}`), Ae((D) => ({
            ...D,
            ...
            t(`Mission run index failed: ${D instanceof Error ? D.message : String(D)}`);
          }), r(`${d.label} local job failed`, _.join(`
`)), B(d, _.join(`
`), -1, A, $).catch((D) => {
            t(`Mission transcript save failed: ${D instanceof Error ? D.message : String(D)}`);
          }), M(null);
        },
        onExit: (ie) => {
          const D = [
...
`);
          Ae((fe) => ({
            ...
            finishedAt: pt(),
            exitCode: ie,
            summary: `${d.label} exited ${ie}`
          }, A).catch((fe) => {
            t(`Mission run index failed: ${fe instanceof Error ? fe.message : String(fe)}`);
          }), r(`${d.label} local job`, D), B(d, D, ie, A, $).catch((fe) => {
            t(`Mission transcript save failed: ${fe instanceof Error ? fe.message : String(fe)}`);
          }), M(null);
        },
        onError: () => t(`Local job stream issue for ${d.label}`)
      }), t(`Started ${d.label} mission run${C ? ` · ${C.title}` : ""}`);
    } catch (Z) {
      M(null), U(($) => $.map((_) => _.id === L ? {
        ..._,
        status: "failed",
        finishedAt: Date.now(),
        lines: [..._.lines, `[Atomek] Failed to start: ${Z instanceof Error ? Z.message : String(Z)}`]
      } : _)), t(`Local job failed to start: ${Z instanceof Error ? Z.message : String(Z)}`);
    }
  }, [i, Ne, e.local, F, s, v, r, B, C, t, we, xe]), se = N(async (d) => {
    const w = Yn(d);
    if (!w) {
    ...
    let L = null;
    const V = Se(d), Z = `pod-run-${Date.now()}-${w}`, $ = pt(), _ = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${d.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      L = await Ne(A), L && await xe(L, A, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${V} mission task started`,
        data: { resourceId: d.id, podId: w, taskId: C?.id ?? "manual", taskTitle: C?.title ?? "Manual pod run" }
      }]);
    } catch (D) {
      t(`Mission pack failed before pod task: ${D instanceof Error ? D.message : String(D)}`);
      return;
    }
    const We = {
      id: Z,
      ...
      startedAt: $,
      taskId: C?.id ?? "manual",
      taskTitle: C?.title ?? "Manual pod run",
      transcriptPath: _,
      summary: `Started ${V} for ${C?.title ?? "manual pod run"}`
    };
    U((D) => [{
      id: Z,
      toolId: d.id,
      ...
      taskTitle: C?.title ?? "Manual pod run",
      transcriptPath: _,
      lines: [`[Atomek] Dispatching ${V} via Tytus host bridge…`, `[Atomek] Pod: ${w}`, `[Atomek] Task: ${C?.title ?? "manual pod run"}`]
    }, ...D].slice(0, 6)), await we(We, L).catch((D) => {
      t(`Mission run index failed: ${D instanceof Error ? D.message : String(D)}`);
    });
    const Ae = [], ie = (D) => {
      Ae.push(D), U((fe) => fe.map((Oe) => Oe.id === Z ? { ...Oe, lines: [...Oe.lines, D].slice(-500) } : Oe));
    };
    try {
      const D = await e.daemon.callPodEndpoint(w, "/v1/models", { method: "GET" });
      if (!D.ok)
        throw new Error(`/v1/models ${D.status}: ${await D.text()}`);
      const fe = await D.json(), Oe = jl(fe);
      if (!Oe)
        throw new Error("pod /v1/models returned no model id");
      ie(`[Atomek] Selected pod model from live metadata: ${Oe}`);
      const ze = await e.daemon.callPodEndpoint(w, "/v1/chat/completions", {
        method: "POST",
...
`)
            },
            {
              role: "user",
              content: [
                L ? `Mission folder: ${L.rootPath ?? L.name}` : "Mission folder: not available",
                L ? `Mission: ${L.title}` : "",
                L ? `Goal: ${L.goal}` : "",
                C ? `Task: ${C.title} (${C.id})` : "Task: manual",
                "",
                "Mission context:",
                L ? rn(L, v, i, s, A) : "",
                v ? sn(v) : "",
...
`)
      ...
      if (!ze.ok)
        throw new Error(`/v1/chat/completions ${ze.status}: ${Fe}`);
      let Ye = Fe;
      try {
        Ye = JSON.parse(Fe);
      } catch {
      }
      const Ge = typeof Ye == "string" ? Ye : Hl(Ye);
      ie(Ge);
      const dt = [
        `# Pod job — ${V}`,
        "",
        ...
        `- Pod: ${w}`,
        `- Captured: ${(/* @__PURE__ */ new Date()).toISOString()}`,
        `- Task: ${C?.title ?? "Manual pod run"}`,
        "",
        Ge,
        ""
      ].join(`
`);
      U((Tt) => Tt.map((_e) => _e.id === Z ? { ..._e, status: "complete", finishedAt: Date.now(), lines: [..._e.lines, "[Atomek] Pod task complete."].slice(-500) } : _e)), await we({ ...We, status: "complete", finishedAt: pt(), summary: `${V} completed`, transcriptPath: _ }, L), r(`${V} pod job`, dt), await B({ id: d.id, label: V, kind: "pod-agent", status: d.status }, dt, 0, L, _), t(`${V} completed mission task`);
    } catch (D) {
      const fe = D instanceof Error ? D.message : String(D);
      ie(`[FAIL] ${fe}`), U((ze) => ze.map((Fe) => Fe.id === Z ? { ...Fe, status: "failed", finishedAt: Date.now(), lines: Fe.lines.slice(-500) } : Fe)), await we({ ...We, status: "failed", finishedAt: pt(), exitCode: -1, summary: fe, transcriptPath: _ }, L).catch((ze) => {
        t(`Mission run index failed: ${ze instanceof Error ? ze.message : String(ze)}`);
      });
      const Oe = [`# Pod job failed — ${V}`, "", `- Pod: ${w}`, `- Error: ${fe}`, "", "```text", Ae.join(`
`), "```", ""].join(`
`);
      r(`${V} pod job failed`, Oe), await B({ id: d.id, label: V, kind: "pod-agent", status: d.status }, Oe, -1, L, _).catch((ze) => {
        t(`Pod transcript save failed: ${ze instanceof Error ? ze.message : String(ze)}`);
      }), t(`Pod task failed: ${fe}`);
    }
  }, [i, Ne, e.daemon, F, s, v, r, B, C, t, we, xe]), Te = N(async (d) => {
    if (!d.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    U((w) => w.map((A) => A.id === d.id ? { ...A, status: "canceling", lines: [...A.lines, "[Atomek] Cancel requested…"] } : A)), we({
      id: d.id,
      jobId: d.jobId,
      toolId: d.toolId,
      label: d.label,
    ...
    try {
      ...
      U((A) => A.map((L) => L.id === d.id ? { ...L, status: "running", lines: [...L.lines, `[Atomek] Cancel failed: ${w instanceof Error ? w.message : String(w)}`] } : L)), t(`Local job cancel failed: ${w instanceof Error ? w.message : String(w)}`);
    }
  }, [e.local, t, we]), je = N(async (d) => {
    const w = [
...
`);
    ...
    const w = d.setupAction, A = w?.commandPreview ? `${w.label}: ${w.commandPreview}` : w?.deepLink ? `${w.label}: ${w.deepLink}` : w?.label ?? `${Se(d)} needs setup`;
    t(A), xt(w?.commandPreview ?? w?.deepLink ?? A);
  }, [t]);
  return /* @__PURE__ */ u("aside", { className: x ? "workbench-agent-dock" : "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: x ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      ...
      G === "mission" ? /* @__PURE__ */ u(Ve, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ u("div", { className: `workbench-team-assignment-summary ${X.readiness}`, children: [
            /* @__PURE__ */ a("strong", { children: X.label }),
            /* @__PURE__ */ u("span", { children: [
              X.readiness,
              " · ",
              X.bestFor
          ...
          X.assignments.map((d) => /* @__PURE__ */ u("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ u("div", { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => W && void xe(W, F), disabled: !W, children: "Rewrite context files" })
        ] }),
        I.length ? /* @__PURE__ */ u(Ve, { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: R.map((d, w) => /* @__PURE__ */ u(
          "button",
          {
            className: `workbench-task-card ${d.status} ${C?.id === d.id ? "active" : ""}`,
            onClick: () => {
              q(d.id), ae(d.prompt);
            },
...
`) || "[waiting for output]" })
        ...
        Pe.length ? /* @__PURE__ */ u(Ve, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: Pe.slice(0, x ? 4 : 8).map((d) => /* @__PURE__ */ u("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ u("div", { children: [
            ...
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void xt(d.transcriptPath ?? d.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, d.id)) })
        ] }) : null
      ] }) : null,
      G === "setup" ? /* @__PURE__ */ u(Ve, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list compact", children: [
          ...
          v?.resources.slice(0, x ? 8 : 5).map((d) => /* @__PURE__ */ u("div", { className: "workbench-resource-row", children: [
            ...
            /* @__PURE__ */ u("div", { className: "workbench-resource-row-actions", children: [
              ...
              d.kind === "pod-agent" && Yn(d) ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                se(d);
              }, children: "Ask pod" }) : null,
              d.status === "needs-setup" || d.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => Be(d), children: "Setup" }) : null
            ] })
          ] }, d.id))
        ] }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          ...
          c.map((d) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            ...
            /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                qe(d);
              }, disabled: d.status !== "available", title: "Launch this tool in a fresh Tytus Terminal with mission context and execute it immediately.", children: Ol(d) }),
              d.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                ce(d);
              }, disabled: d.status !== "available" || he !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: Sl(d, he) }) : null
            ] })
          ] }, d.id))
        ] }),
...
function Ml({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: i }) {
  const s = Ee(), o = e === "Ready" ? s("app.ready") : e;
  return /* @__PURE__ */ u("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: tr(t.language) })
  ] });
}
function jn(e, t) {
...
function Zl() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : an.gatewayPreference,
...
function Kl({ host: e }) {
  return /* @__PURE__ */ a(Vc, { host: e, children: /* @__PURE__ */ a(sl, { host: e }) });
}
function Ql(e) {
  return function() {
