...
import { jsx as a, jsxs as u, Fragment as We } from "react/jsx-runtime";
import { forwardRef as Qn, createElement as on, useState as x, useRef as Ge, useCallback as P, useEffect as ce, lazy as br, useMemo as pe, Suspense as vr } from "react";
/**
 ...
 */
const yr = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), wr = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), Sn = (e) => {
  const t = wr(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, $n = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), gr = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const zr = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Xr = _("check", zr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Tr = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], Dt = _("chevron-down", Tr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const ss = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function is() {
  const e = wn();
...
async function cs() {
  ...
  if (!e)
    return { name: "Browser fallback folder", files: await ua(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await da(t, t.name, n, 0, 320), { name: t.name, handle: t, files: gn(n) };
}
function wn() {
  const e = window;
...
async function da(e, t, n, r, s) {
  if (!(n.length >= s || r > 8))
    for await (const i of e.values()) {
      if (n.length >= s) break;
      if (i.kind === "directory") {
        if (ss.has(i.name)) continue;
        await da(i, `${t}/${i.name}`, n, r + 1, s);
        continue;
      }
      const o = i, c = await la(o, `${t}/${o.name}`, "local-folder");
      c && n.push(c);
    }
}
function ua(e) {
...
`).map((c) => `<tr>${c.split("|").filter(Boolean).map(
      (v, b) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${b % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${v.trim()}</td>`
    ).join("")}</tr>`).join("");
...
const nt = "atomek:default", ds = 3e3, kn = "tytus.atomek.threadTitleOverrides", Mn = {
  available: !1,
  source: "none",
...
}, us = (e, t = ds) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Vn = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, hs = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Dn = (e) => ({
  id: e.id,
  title: e.title,
  kind: hs(e.kind),
  body: e.body,
...
function bs({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const s = e.ai, [i, o] = x(null), [c, m] = x([]), [v, b] = x([]), [h, y] = x([]), [L, H] = x([]), [U, ee] = x(Mn), [q, te] = x(!1), le = Ge(!0), S = Ge(null), E = P(async () => {
    if (!s) {
      ee(Mn);
      return;
    }
    ...
    if (s)
      try {
        const g = await s.listArtifacts({ threadId: p });
    ...
    if (s)
      try {
        const g = await s.listMessages(p);
        if (!le.current) return;
        const T = c.find((F) => F.id === p) ?? null;
        T && o(T), b(g.map(vt).filter(Boolean)), H([]), await Y(p);
      } catch (g) {
    ...
    if (s)
      try {
        const p = Fn(await s.listThreads({ workspaceKey: nt, status: "active" })), g = p[0] ?? await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
        if (!le.current) return;
        m(p[0] ? p : [g]), o(g);
        const T = await s.listMessages(g.id);
        if (!le.current) return;
        b(T.map(vt).filter(Boolean)), await Y(g.id);
      } catch (p) {
        r(`AI chat unavailable: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [s, Y, r]);
  ce(() => (le.current = !0, E(), ne(), () => {
    le.current = !1;
  }), [ne, E]);
  const D = P(async () => {
    if (!s) return null;
    if (i) return i;
    const p = await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
    return le.current && (o(p), m((g) => [p, ...g.filter((T) => T.id !== p.id)])), p;
  }, [s, i]), Ve = P(async () => {
    if (!s) return;
    const p = await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
    o(p), m((g) => [p, ...g.filter((T) => T.id !== p.id)]), b([]), y([]), H([]), r("New AI chat created");
  }, [s, r]), A = P(async (p, g) => {
    if (!s) return;
    const T = g.trim();
    if (T)
      try {
        const F = s.updateThread, ae = c.find((ue) => ue.id === p) ?? i, O = typeof F == "function" ? await F({ threadId: p, title: T }) : { ...ae ?? await D(), id: p, title: T, updatedAt: Date.now() };
        typeof F != "function" && ms(p, T), m((ue) => ue.map((Te) => Te.id === O.id ? O : Te)), o((ue) => ue?.id === O.id ? O : ue), r(typeof F == "function" ? `Renamed chat: ${O.title}` : `Renamed chat locally: ${O.title}`);
      } catch (F) {
        r(`Rename chat failed: ${F instanceof Error ? F.message : String(F)}`);
      }
  }, [s, D, r, i, c]), oe = P(async (p) => {
    if (s)
      try {
        await s.deleteThread(p), ps(p);
        const g = c.filter((T) => T.id !== p);
        if (m(g), i?.id === p) {
          const T = g[0] ?? await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
          o(T), m((ae) => ae.some((O) => O.id === T.id) ? ae : [T, ...ae]);
          const F = await s.listMessages(T.id);
          b(F.map(vt).filter(Boolean)), await Y(T.id);
        }
        r("Deleted AI chat");
      } catch (g) {
        r(`Delete chat failed: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [s, Y, r, i?.id, c]), be = P(async (p) => {
    if (!s) return null;
    try {
      const g = await D();
      if (!g) return null;
      const T = await s.createArtifact({
        ...
        title: p.title?.trim() || Vn(p.body, "Atomek artifact"),
        kind: p.kind ?? "markdown",
        body: p.body
      }), F = Dn(T);
      return y((ae) => [F, ...ae.filter((O) => O.id !== F.id)]), r(`Saved AI artifact: ${F.title}`), F;
    } catch (g) {
    ...
    if (s)
      try {
        await s.deleteArtifact(p), y((g) => g.filter((T) => T.id !== p)), r("Deleted AI artifact");
      } catch (g) {
    ...
    try {
      const g = await s.writeMemory({
        title: p.title?.trim() || Vn(p.body, "Atomek memory"),
        body: p.body,
        metadata: {
          source: "atomek",
          messageId: p.messageId ?? null
      ...
      return H((T) => [g, ...T.filter((F) => F.id !== g.id)].slice(0, 5)), r(`Remembered: ${g.title}`), g;
    } catch (g) {
      return r(`Remember failed: ${g instanceof Error ? g.message : String(g)}`), null;
    }
  }, [s, r]), ve = P(async (p) => {
    if (!s) return [];
    const g = await s.searchMemory({ query: p, limit: 5 });
    return le.current && H(g), g;
  }, [s]), Ce = P(async (p, g = {}) => {
    const T = p.trim();
    if (!T || !s) return null;
    te(!0);
    ...
    try {
      const O = await D();
      if (!O) return null;
      const ue = await ve(T).catch(() => []), Te = fs(ue), Je = g.requestContext ?? t, Ae = Te ? [...Je, Te] : [...Je];
      let He = null;
      for await (const Q of s.sendMessage({
        threadId: O.id,
        body: T,
        ...
        if (Q.type === "message_created") {
          const re = vt(Q.message);
          if (!re) continue;
          re.role === "assistant" && (He = re.id), b((G) => [...G.filter((se) => se.id !== re.id), re]);
        }
        if (Q.type === "token" && (He = Q.messageId, b((re) => re.some((G) => G.id === Q.messageId) ? re.map(
          (G) => G.id === Q.messageId ? { ...G, body: Q.body, status: "streaming" } : G
        ) : [...re, {
          id: Q.messageId,
          role: "assistant",
          body: Q.body,
          ...
          const re = vt(Q.message);
          if (!re) continue;
          re.role === "assistant" && (F = re), b((G) => G.map((se) => se.id === re.id ? re : se)), re.gatewayLabel && r(`AI answered via ${re.gatewayLabel}`);
        }
        if (Q.type === "run_failed") {
          const re = ae.signal.aborted;
          r(re ? "AI response stopped" : `AI failed: ${Q.error}`), He && b((G) => G.map((se) => se.id !== He ? se : re ? { ...se, status: "complete", error: void 0, body: se.body || "Stopped by user." } : { ...se, status: "error", error: Q.error, body: Q.error }));
        }
      }
      const de = Fn(await s.listThreads({ workspaceKey: nt, status: "active" }).catch(() => []));
      return le.current && de.length > 0 && m(de), E(), F;
    } catch (O) {
      return O instanceof DOMException && O.name === "AbortError" ? (r("AI response stopped"), F) : (r(`AI failed: ${O instanceof Error ? O.message : String(O)}`), null);
    } finally {
      S.current === ae && (S.current = null), te(!1);
    }
  }, [s, n.gatewayPreference, n.model, D, ve, E, t, r]), Xe = P(() => {
    S.current?.abort(), r("Stopping AI response…");
  }, [r]);
  return {
    aiStatus: U,
    artifacts: h,
...
const vs = (e) => `tytus-workbench:///${encodeURI(e.path)}`, ys = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, ws = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: s }) => {
...
}, gs = "active-file", ks = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Ps = (e, t = "file") => `${t}:${e.id}`, xs = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
...
`), b = o.kind === "selection" && o.range ? Hs(c.content, o.range) : c.content, h = o.kind === "selection" ? Ts : o.label.startsWith("Open editor") ? As : Ls;
    s.push({
...
`));
}, In = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, qn = 75e4, js = 2800, Ss = 240, Ws = 80, Ms = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Be(t.content)}`).sort().join("")
), ln = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? qn) || e.content.length > (t.maxFileBytes ?? qn) ? "huge" : !t.includeDirty && e.dirty ? null : Ys(e.path) ? "vendor" : Es(e) ? "binary" : null;
}, Rn = (e, t = {}) => {
  ...
  for (const o of e) {
    ...
    const v = Be(o.content), b = Ds(o, v, n, t);
    i.push(...b), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
...
}, Ds = (e, t = Be(e.content), n = Date.now(), r = {}) => {
  const s = Math.max(500, r.maxChunkChars ?? js), i = Math.min(Math.max(0, r.chunkOverlapChars ?? Ss), Math.floor(s / 2)), o = Math.max(1, r.maxChunksPerFile ?? Ws), c = qs(e.content), m = [];
  let v = 0, b = 0;
  for (; v < e.content.length && m.length < o; ) {
    const h = Is(e.content, v, Math.min(e.content.length, v + s)), y = e.content.slice(v, h).trim();
    if (y.length > 0) {
      const L = Rs(c, v, h);
      m.push({
        id: `${e.id}:${t}:${b}:${L.startLineNumber}-${L.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const r = Yn(e, t), s = Yn(e, Math.max(t, n - 1));
  return {
    startLineNumber: r + 1,
    startColumn: t - e[r] + 1,
    endLineNumber: s + 1,
  ...
  return 0;
}, Ys = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => Ms.has(t)), Es = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && Vs.has(t) || e.content.includes("\0")) return !0;
...
class Js {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Rn(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Rn(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return Zs(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const at = (e = [], t = {}) => new Js(e, t), Zs = (e, t, n = {}) => {
  ...
  for (const c of e.files) {
    ...
    if (ln(m, n)) {
      i.push({ fileId: c.fileId, path: m.path, status: "skipped-now", indexedHash: c.hash, currentHash: Be(m.content) });
      continue;
    }
    const b = Be(m.content);
    b !== c.hash ? i.push({ fileId: c.fileId, path: m.path, status: "changed", indexedHash: c.hash, currentHash: b }) : m.dirty !== c.dirty && i.push({ fileId: c.fileId, path: m.path, status: "dirty-state-changed", indexedHash: c.hash, currentHash: b });
  }
  for (const c of t)
    !s.has(c.id) && !ln(c, n) && i.push({ fileId: c.id, path: c.path, status: "new", currentHash: Be(c.content) });
  const o = e.signature !== ma(t);
  return { stale: i.length > 0, signatureChanged: o, files: i };
}, Ks = 8, Us = 12e3, pa = (e, t, n = {}, r) => {
  const s = Qs(t);
  if (s.length === 0) return [];
  const i = new Set(r?.files.map((b) => b.fileId) ?? []), o = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => $s(b, s)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, h) => h.score - b.score || b.chunk.path.localeCompare(h.chunk.path) || b.chunk.ordinal - h.chunk.ordinal), c = [], m = /* @__PURE__ */ new Set();
  let v = n.maxChars ?? Us;
  for (const b of o) {
    if (c.length >= (n.limit ?? Ks) || v <= 0) break;
    const h = `${b.chunk.fileId}:${b.chunk.ordinal}`;
    if (m.has(h)) continue;
    m.add(h);
    const y = b.chunk.text.length > v ? `${b.chunk.text.slice(0, Math.max(0, v - 28))}
[...context clipped...]` : b.chunk.text;
    c.push(Bs(b, y, i.has(b.chunk.fileId))), v -= y.length;
  }
  return c;
}, Bs = (e, t = e.chunk.text, n = !1) => {
...
`), Qs = (e) => Array.from(new Set(
  ...
  const c = [];
  for (const m of t) {
    const v = en(n, m), b = en(r, m), h = en(s, m), y = i === m ? 1 : 0;
    v + b + h + y !== 0 && (c.push(m), o += Math.min(v, 12), o += b * 4, o += h * 6, o += y * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (o += 3));
  }
  ...
  const n = e.toLowerCase(), r = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, v) => m - v)[0] ?? 0, s = Math.max(0, r - 120), i = Math.min(e.length, r + 260), o = s > 0 ? "…" : "", c = i < e.length ? "…" : "";
  return `${o}${e.slice(s, i).replace(/\s+/g, " ").trim()}${c}`;
}, ei = (e, t = {}) => {
  const { autoRefresh: n = !1, ...r } = t, s = Ge(e), i = Ge(null);
  i.current || (i.current = at(e, r));
  const [o, c] = x(() => i.current?.getSnapshot() ?? at(e, r).getSnapshot());
  s.current = e;
  const m = P((y = s.current) => {
    const L = (i.current ?? at([], r)).refresh(y, r);
    return c(L), L;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), v = P((y = s.current) => {
    const L = (i.current ?? at([], r)).update(y, r);
    return c(L), L;
  }, [m]);
  ce(() => {
    n && m(e);
  }, [n, e, m]);
  const b = (i.current ?? at([], r)).staleReport(e), h = P((y, L = {}) => {
    const H = i.current ?? at([], r);
    return pa(H.getSnapshot(), y, L, H.staleReport(s.current));
  }, []);
  return { snapshot: o, staleReport: b, isStale: b.stale, refresh: m, update: v, retrieve: h };
}, ti = "tytus.atomek.semanticVector:v1", ba = "__gateway_default__", va = (e) => e?.trim() || ba, ni = (e) => {
  const t = va(e);
  return t === ba ? "gateway default" : t;
}, ya = (e, t, n) => [
  ti,
  ...
  if (!wa()) return null;
  try {
    const r = ya(e, t, n), s = window.localStorage.getItem(r);
    if (!s) return null;
  ...
  if (!wa()) return null;
  try {
    const t = ya(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, yt = 8, si = 12e3, ii = 180, oi = async (e, t, n, r, s = {}, i, o) => {
  const c = pa(t, n, { ...s, limit: Math.max(s.limit ?? yt, 16) }, i), m = e.ai;
  ...
  try {
    const v = va(r.embeddingModel), b = await m.embedText({
      input: n,
    ...
    const y = new Map(c.map((S) => [S.chunkId, S])), L = mi(n), H = /* @__PURE__ */ new Map();
    t.chunks.filter((S) => s.includeDirty !== !1 || !S.dirty).slice(0, ii).forEach((S) => H.set(S.id, S)), c.forEach((S) => {
      const E = t.byChunkId.get(S.chunkId);
      E && H.set(E.id, E);
    });
    const U = Array.from(H.values()), ee = [];
    let q = 0;
    for (const S of U) {
      o?.aborted;
      const E = ai(e.appId, v, S) ?? await li(m, e.appId, v, S, r, o);
      if (!E) continue;
      E.updatedAt > Date.now() - 1e3 && (q += 1);
      const Y = hi(h, E.vector);
      if (!Number.isFinite(Y)) continue;
      const ne = y.get(S.id)?.score ?? pi(S, L), D = fi(ne) * 0.42 + Math.max(0, Y) * 0.58;
      ee.push({ chunk: S, keywordScore: ne, vectorScore: Y, score: D, matchedTerms: L.filter((Ve) => S.text.toLowerCase().includes(Ve)) });
    }
    if (ee.length === 0) return { hits: c.slice(0, s.limit ?? yt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: q };
    const te = new Set(i?.files.map((S) => S.fileId) ?? []), le = ee.sort((S, E) => E.score - S.score || E.keywordScore - S.keywordScore || S.chunk.path.localeCompare(E.chunk.path) || S.chunk.ordinal - E.chunk.ordinal).slice(0, Math.max(s.limit ?? yt, 1)).map((S) => di(S, te.has(S.chunk.fileId)));
    return {
      hits: ui(le, s.maxChars ?? si),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${ni(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: q
    };
  } catch (v) {
    return {
      ...
      reason: `Semantic retrieval failed (${v instanceof Error ? v.message : String(v)}) — using keyword retrieval.`,
      embeddedChunks: 0
    };
  }
}, ci = (e) => e.map((t) => ({
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: bi(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, fi = (e) => Math.min(1, Math.max(0, e / 24)), mi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), pi = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, vi = /```([^\n`]*)\n([\s\S]*?)```/g, yi = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function xn(e) {
  return Array.from(e.matchAll(vi)).map((t) => ka(t[1] ?? "", xi(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function ka(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, s = [];
  for (const i of n) {
    const o = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? r[o[1].toLowerCase()] = zi(o[2]) : s.push(i.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: s, attrs: r, content: t };
}
function Le(e, t) {
  return (typeof e == "string" ? ka(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Pa(e) {
  return [...xn(e).filter((r) => Le(r, "diff") || Le(r, "patch")).map((r) => r.content), e].filter((r, s, i) => wi(r) && i.findIndex((o) => o === r) === s);
}
function wi(e) {
...
`), r = [];
  ...
  for (let o = 0; o < s.length; o += 1) {
    const c = s[o], v = s.find((h) => h > c && n[h].startsWith("diff --git ")) ?? n.length, b = n.slice(c, v).join(`
`);
    /^@@\s+-\d+/m.test(b) && i.push({ raw: b, ...tn(b) });
  }
...
function dn(e) {
  return xn(e).filter((t) => ki(t)).map((t) => ({
    content: Ni(t.content),
...
`), s = [];
  ...
  for (let c = 0; c < r.length; c += 1) {
    const m = r[c].match(yi);
    if (!m) continue;
    o = !0;
    const v = Number(m[1]), b = Math.max(0, v - 1);
    if (b < i) return null;
    for (s.push(...n.slice(i, b)), i = b, c += 1; c < r.length; c += 1) {
      const h = r[c];
      if (h.startsWith("@@ ")) {
        c -= 1;
        break;
      ...
      const y = h[0], L = h.slice(1);
      if (y === " ") {
        if (n[i] !== L) return null;
        s.push(L), i += 1;
        continue;
      }
      if (y === "-") {
...
function Xi(e) {
  ...
  for (const b of Pa(t))
    for (const h of gi(b)) {
      if (!h.path) {
        if (s) {
          const H = un(s.content, h.raw);
          H && H !== s.content && o.set(s.id, Ct({
            file: s,
            proposedContent: H,
            sourceTitle: r,
      ...
      const L = un(y.file.content, h.raw);
      if (!L || L === y.file.content) {
  ...
  for (const b of dn(t)) {
    const h = b.path ? En(n, b.path) : s ? { file: s, normalizedPatchPath: Ie(s.path), confidence: "exact" } : null;
    if (!h) {
      c.push(`${b.path ?? "replacement block"}: no opened file`);
      continue;
    }
  ...
  const m = dn(t).some((b) => b.path);
  if (o.size === 0 && s && !m) {
    const b = Ti(t, s);
    b && b.content !== s.content && o.set(s.id, Ct({
      file: s,
      proposedContent: b.content,
      sourceTitle: r,
      extractionLabel: b.label,
  ...
  const v = Array.from(o.values());
  return {
    sourceTitle: r,
    edits: v,
    skipped: Array.from(new Set(c)),
    stats: Ai(v.map((b) => b.stats)),
    kind: v.length === 0 ? "empty" : v.length === 1 ? "single-file" : "multi-file"
  };
}
function Ti(e, t) {
  for (const m of Pa(e)) {
    const v = un(t.content, m);
    if (v) return { content: v, label: "unified diff patch" };
  }
  const n = xn(e);
  if (n.length === 0) return null;
  const r = Ci(t), s = dn(e)[0];
  if (s) return { content: s.content, label: s.label };
  const i = n.find((m) => r.some((v) => Le(m, v)) && !Le(m, "diff") && !Le(m, "patch"));
  if (i) return { content: i.content, label: `matched ${i.lang || t.language} block` };
  const c = n.filter((m) => !Le(m, "diff") && !Le(m, "patch")).sort((m, v) => v.content.length - m.content.length)[0];
...
function Ct(e) {
  ...
  return {
    ...
    stats: Li(e.file.content, e.proposedContent),
    base: {
...
const It = (e) => typeof e == "object" && e !== null, Hi = (e) => It(e.ai) ? e.ai : null, ji = (e) => {
  if (!It(e)) return null;
  const t = e.id;
...
}, Se = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Se) : It(e) ? Object.values(e).some(Se) : !1, Wi = async (e, t) => {
  const n = Hi(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(Si).filter((s) => !!s) : [];
}, Mi = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Se(e.capability) || Se(e.capabilities) || Se(e.modality) || Se(e.modalities) || Se(e.task) || Se(e.tasks) || Se(e.type) || Se(e.kind), Na = (e) => typeof e == "object" && e !== null, Vi = (e) => Na(e.ai) ? e.ai : null, za = (e) => typeof Vi(e)?.embedText == "function", Di = async (e, t) => za(e) ? (await Wi(e, t)).filter(Mi) : [], Fi = (e) => Na(e.ai) ? za(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", Ii = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], qi = [
...
function Ji(e) {
  const t = e.results.length > 0 ? e.results.map((r, s) => [
    ...
    `Status: ${r.status}`,
    "Output:",
    _i(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function Ki(e) {
  ...
  for (const r of t) {
    const s = Ui(r.content);
    if (!s?.scripts) continue;
    const i = Ta(r.path), o = Bi(e, i, s);
    if (!o) continue;
    const c = Object.keys(s.scripts).filter((v) => typeof s.scripts?.[v] == "string"), m = Gi(c);
    for (const v of m) {
      const b = `${o} run ${v}`;
      n.push({
        id: La(`${i}:${b}`),
...
function Bi(e, t, n) {
  ...
  for (const [r, s] of qi)
    if (e.some((i) => $i(i.path) === r && Ta(i.path) === t)) return s;
  return null;
}
function Gi(e) {
  const t = Ii.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, s) => r.localeCompare(s));
  return [...t, ...n];
}
function Qi(e) {
  const t = /* @__PURE__ */ new Set();
...
function ro({ className: e }) {
  return /* @__PURE__ */ a("img", { src: ao, alt: "ATOMEK", className: e, draggable: !1 });
}
const so = `# Tytus Resource Fabric
...
`, uo = [
...
], ho = br(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), fo = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Oa = "tytus.workspace.recent", Ca = "tytus.workspace.layout", Ha = "tytus.atomek.chatAiSettings", ja = "tytus.atomek.currentMission", hn = "tytus.atomek.currentMissionChanged", Wt = {
...
function nn(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - mo), r = n < 1180, s = r ? 200 : 240, i = r ? 300 : 340, o = r ? 420 : 560, c = Math.max(s, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, c), v = Math.floor(n * (r ? 0.34 : 0.36)), b = n - m - o, h = Math.max(i, Math.min(r ? 500 : 640, v, b));
...
function lt(e, t = "repo-repair") {
  const n = Pt.find((D) => D.id === t) ?? Pt[0], r = e?.resources ?? [], s = Ue(r, (D) => D.kind === "pod-agent" && ct(D) === "openclaw"), i = Ue(r, (D) => D.kind === "pod-agent" && ct(D) === "hermes"), o = Ue(r, (D) => D.kind === "ail-route"), c = Ue(r, (D) => D.kind === "shared-folder"), m = Fe(r, "local-cli", ["claude"]), v = Fe(r, "local-cli", ["opencode", "open code"]), b = Fe(r, "local-cli", ["codex"]), h = Fe(r, "local-cli", ["pi"]), y = Fe(r, "local-cli", ["kimi"]), L = Ue(r, (D) => D.kind === "local-cli"), H = Fe(r, "app-skill", ["juli3ta", "music", "song"]), U = Fe(r, "app-skill", ["blender", "3d"]), ee = Fe(r, "app-skill", ["remotion", "video", "render"]), q = Ue(r, (D) => D.kind === "app-skill"), te = t === "pod-local" ? s ?? i ?? o ?? m ?? L : t === "creative-production" ? i ?? o ?? m ?? L : t === "research-watch" ? s ?? o ?? i ?? m ?? L : m ?? v ?? o ?? L, le = t === "creative-production" ? H ?? U ?? ee ?? q ?? v ?? m ?? L : v ?? m ?? b ?? L, S = t === "pod-local" ? b ?? h ?? y ?? s ?? m ?? L : t === "research-watch" ? h ?? y ?? b ?? s ?? L : b ?? h ?? s ?? y ?? L, E = t === "creative-production" ? H ?? U ?? ee ?? q : t === "research-watch" ? Fe(r, "app-skill", ["browser", "channel", "telegram"]) ?? q : q, Y = [
    wt("planner", "Planner", "Break goal into tasks, risks, and required context.", te, "No planner agent ready"),
    ...
    wt("reviewer", "Reviewer", "Independent critique before approval or handoff.", S, "No reviewer agent ready"),
  ...
  (t === "creative-production" || E) && Y.push(wt("app-tool", "App Tool", "Drive installed local app skill through mission context.", E, "No configured app skill"));
  const M = Y.filter((D) => D.status === "ready" || D.status === "available" || D.status === "degraded").length, ne = M >= Y.length - 1 ? "ready" : M >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: ne, assignments: Y };
}
function Qe(e, t, n) {
  if (n && Pt.some((s) => s.id === n)) return n;
  const r = e.toLowerCase();
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(r) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(r) ? "research-watch" : (t?.resources ?? []).some((s) => s.kind === "pod-agent" && Wa(s)) ? "pod-local" : "repo-repair";
}
function ko(e) {
...
function kt(e) {
  ...
  try {
    localStorage.setItem(ja, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(hn, { detail: t }));
  } catch {
  }
}
function xo() {
...
function xt(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", s = lt(t, Qe(r, t, n)), i = (h) => s.assignments.find((y) => y.role === h), o = i("planner"), c = i("implementer"), m = i("reviewer"), v = i("team-desk"), b = i("app-tool");
  return [
    {
      id: "task-scope",
    ...
    } : null,
    {
      ...
      resourceHint: m ? `${St(m)} · ${v?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
...
function zo({ host: e }) {
  const t = Ge(null), [n, r] = x(0), s = pe(() => sc(), []), [i, o] = x("computer"), [c, m] = x(s.primaryVisible), [v, b] = x(s.primaryWidth), [h, y] = x("chat"), [L, H] = x(s.secondaryVisible), [U, ee] = x(s.secondaryWidth), [q, te] = x(!1), [le, S] = x("problems"), [E, Y] = x(s.markdownPreviewVisible), [M, ne] = x(!1), [D, Ve] = x(null), [A, oe] = x([]), [be, Pe] = x([]), [B, ve] = x(null), [Ce, Xe] = x(""), [p, g] = x({ lineNumber: 1, column: 1 }), [T, F] = x(null), [ae, O] = x({}), [ue, Te] = x(gs), [Je, Ae] = x([]), [He, de] = x([]), [Q, re] = x(null), [G, se] = x(""), [qt, dt] = x(!1), [qe, Rt] = x(""), [Nt, $e] = x([]), [he, Re] = x(null), [d, k] = x(null), [z, C] = x(!1), [X, R] = x(() => ic()), [Z, I] = x(null), [ye, Oe] = x(null), [K, W] = x(""), [$, we] = x(""), [fe, xe] = x(""), [De, ut] = x("failed"), [zt, Xt] = x(null), [_e, j] = x("Ready"), [ht, Ja] = x(() => rc()), Ne = be.map((l) => A.find((f) => f.id === l)).filter(Boolean), V = B ? A.find((l) => l.id === B) ?? null : null, Xn = pe(() => ws({ files: A, openEditorIds: be, activeFileId: B, versions: ae, activeSelection: T }), [B, T, ae, A, be]), Tn = pe(() => ({
    scope: ue,
    removedAttachmentIds: Je,
    selectedFileIds: []
  }), [ue, Je]), ft = pe(() => Cs(Xn, A, Tn), [Tn, Xn, A]), Ye = ei(A, { autoRefresh: !0, includeDirty: !0 }), Ln = pe(() => He.map((l) => ({
    id: l.id,
    kind: "index-hit",
    label: l.label,
    path: l.path,
    ...
    snippet: l.snippet
  })), [He]), Za = pe(() => [...ft.attachments, ...Ln], [ft.attachments, Ln]), J = bs({ host: e, requestContext: ft.parts, chatSettings: X, setStatus: j }), Ze = pe(
    () => [...J.artifacts, ...Nt].sort((l, f) => f.createdAt - l.createdAt),
    [J.artifacts, Nt]
  ), Tt = z && !V, Yt = !V && !Tt && !M, ge = pe(() => A.filter((l) => l.dirty), [A]), Ka = pe(() => {
    const l = Ce.trim().toLowerCase();
    return l ? A.filter((f) => f.path.toLowerCase().includes(l)) : A;
  }, [A, Ce]);
  ce(() => {
    O((l) => {
      const f = {};
      for (const w of A) f[w.id] = l[w.id] ?? 1;
      return f;
    });
  }, [A]), ce(() => {
    Ae([]), de([]);
  ...
  const Ua = P((l) => {
    if (l.kind === "index-hit") {
      de((f) => f.filter((w) => w.id !== l.id)), j(`Removed project context: ${l.label}`);
      return;
    }
    Ae((f) => f.includes(l.id) ? f : [...f, l.id]), j(`Removed chat context: ${l.label}`);
  }, []), Lt = P(() => {
    C(!0), ve(null), ne(!0), j("Atomek settings opened");
  }, []), An = P(() => {
    C(!1), ve((l) => l ?? be.at(-1) ?? null), be.length === 0 && ne(!1), j("Atomek settings closed");
  }, [be]), et = P((l) => {
    O((f) => ({ ...f, [l]: (f[l] ?? 1) + 1 }));
  }, []), Ba = P((l) => {
    l.preventDefault(), l.currentTarget.setPointerCapture?.(l.pointerId);
    const f = nn(n), w = l.clientX, N = U, ie = (bt) => {
      const $t = N + (w - bt.clientX);
      ee(jt($t, f.secondaryMin, f.secondaryMax));
    }, je = () => {
      window.removeEventListener("pointermove", ie), window.removeEventListener("pointerup", je);
    };
    window.addEventListener("pointermove", ie), window.addEventListener("pointerup", je);
  }, [U, n]), Ga = P((l) => {
    l.preventDefault(), l.currentTarget.setPointerCapture?.(l.pointerId);
    const f = nn(n), w = l.clientX, N = v, ie = (bt) => {
      const $t = N + (bt.clientX - w);
      b(jt($t, f.primaryMin, f.primaryMax));
    }, je = () => {
      window.removeEventListener("pointermove", ie), window.removeEventListener("pointerup", je);
    };
    window.addEventListener("pointermove", ie), window.addEventListener("pointerup", je);
  }, [v, n]), At = P((l) => {
    const f = [l, ...ht.filter((w) => w.path !== l.path)].slice(0, 6);
    Ja(f), localStorage.setItem(Oa, JSON.stringify(f));
  }, [ht]), me = P((l, f) => {
    Pe((w) => w.includes(l.id) ? w : [...w, l.id]), ve(l.id), ne(!1), Xt(f ?? null), g({ lineNumber: f ?? 1, column: 1 });
  }, []), Qa = P((l) => {
    const f = {
      id: `atomek-doc:${l.id}`,
      name: l.fileName,
    ...
    oe((w) => Gn(w, [f])), me(f), y("chat"), H(!0), Y(!0), j(`Opened Atomek guide: ${l.title}`);
  }, [me]), $a = P((l) => {
    if (!l.fileId) return;
    const f = A.find((w) => w.id === l.fileId);
    f && (me(f, l.range?.startLineNumber ?? 1), j(`Revealed context: ${l.label}`));
  }, [A, me]), mt = P(async () => {
    if (an(ge, "open new files"))
      try {
        const l = await os();
        if (l.length === 0) return;
        oe((f) => Gn(f, l)), l.forEach((f) => At({ name: f.name, path: f.path, at: Date.now() })), me(l[0]), j(`Opened ${l.length} local file${l.length === 1 ? "" : "s"}`);
      } catch (l) {
        l.name !== "AbortError" && j(`Open file failed: ${l.message}`);
      }
  }, [ge, me, At]), Et = P(async () => {
    if (an(ge, "open another folder"))
      try {
        const l = await cs();
        Ve(l), oe(l.files), Pe([]), ve(null), ne(!1), At({ name: l.name, path: l.name, at: Date.now() }), j(`${l.handle ? "Opened local folder" : "Opened browser fallback folder"} ${l.name} (${l.files.length} text files indexed)`);
      } catch (l) {
    ...
    if (!B) return;
    let f = !1;
    oe((w) => w.map((N) => N.id !== B || N.content === l ? N : (f = !0, { ...N, content: l, dirty: !0 }))), f && et(B);
  }, [B, et]), Jt = P(async () => {
    if (V)
      try {
        const l = await _t(V);
        oe((f) => f.map((w) => w.id === l.id ? l : w)), j(`Saved ${l.name}`);
      } catch (l) {
        j(`Save failed: ${l.message}`);
      }
  }, [V]), er = P(async (l) => {
    const f = A.find((N) => N.id === l);
    if (!f) return null;
    const w = await _t(f);
    return oe((N) => N.map((ie) => ie.id === w.id ? w : ie)), w;
  }, [A]), On = P(async () => {
    const l = A.filter((f) => f.dirty);
    if (l.length === 0) {
      j("No dirty files to save");
      return;
    }
    try {
      const f = await Promise.all(l.map((N) => _t(N))), w = new Map(f.map((N) => [N.id, N]));
      oe((N) => N.map((ie) => w.get(ie.id) ?? ie)), I(null), j(`Saved ${f.length} dirty file${f.length === 1 ? "" : "s"}`);
    } catch (f) {
      j(`Save all failed: ${f.message}`);
    }
  }, [A]), Zt = P((l) => {
    const f = A.find((w) => w.id === l);
    if (f?.dirty && !window.confirm(`${f.name} has unsaved changes. Close without saving?`)) {
      j(`Close canceled — ${f.name} has unsaved changes`);
      return;
    }
    Pe((w) => {
      const N = w.filter((ie) => ie !== l);
      return B === l && (ve(N.at(-1) ?? null), F(null)), N;
    });
  }, [B, A]), tr = P(() => {
    an(ge, "close all editors") && (Pe([]), ve(null), Xt(null), ne(!1), j("Closed all editors"));
  }, [ge]), Kt = P(() => {
    const l = A.filter((w) => w.name.startsWith("Untitled")).length + 1, f = {
...
`,
    ...
    return de(N), N.length === 0 ? (j(Ye.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), f) : (w.mode !== "hybrid" && w.reason ? j(w.reason) : j(`${N.length} project context hit${N.length === 1 ? "" : "s"} · ${w.reason ?? "hybrid retrieval"}`), [
      ...f,
      ...ci(N)
    ]);
  }, [ft.parts, ue, X, e, Ye]), Ut = P(() => {
    if (!V && Ne.length === 0) {
      j("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const f = [
      `Create a polished Markdown artifact from ${V?.path ?? `${Ne.length} open editors`}.`,
    ...
    H(!0), y("chat"), j("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const w = await Ee(f), N = await J.askAgent(f, { requestContext: w });
      !N || N.status === "error" || J.createArtifact({
        messageId: N.id,
        title: `AI synthesis — ${V?.name ?? "open workspace"}`,
        kind: "markdown",
        body: N.body
      }).then(() => {
        H(!0), y("outputs"), te(!0), S("output");
      });
    })();
  }, [V, J, Ee, Ne.length]), Bt = P((l) => {
    const f = A.find((w) => w.path === l.path || w.name === l.name);
    if (f) {
      me(f), j(`Opened recent ${f.name}`);
      return;
    }
    j("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [A, me]), ze = P((l) => {
    H(!0), y("chat"), se(""), (async () => {
      const f = await Ee(l);
      await J.askAgent(l, { requestContext: f });
    })();
  }, [J, Ee]), Cn = P(async (l) => {
    if (!e.skills?.get) {
    ...
    try {
      const f = await e.skills.get(l.id), w = f.body.length > 4500 ? `${f.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : f.body, N = [
...
`);
      se((ie) => [ie.trim(), N].filter(Boolean).join(`

`)), H(!0), y("chat"), j(`Attached skill ${f.title} to chat input`);
    } catch (f) {
      j(`Failed to attach skill: ${f instanceof Error ? f.message : String(f)}`);
    }
  }, [e.skills]), nr = P((l) => {
    const f = J.messages.findIndex((N) => N.id === l.id), w = J.messages.slice(0, f < 0 ? void 0 : f).reverse().find((N) => N.role === "user");
    if (!w?.body.trim()) {
      j("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const N = await Ee(w.body);
      await J.askAgent(w.body, { requestContext: N });
    })();
  }, [J, Ee]), ar = P((l) => {
    J.createArtifact({
      messageId: l.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const f = sn(A, rn(l.title || l.kind)), w = {
    ...
    oe((N) => [...N, w]), me(w), j(`Opened ${l.title} as editable file`);
  }, [A, me]), Qt = P(() => {
    ...
    const w = window.prompt("Paste check output/result");
    if (w === null) return;
    const N = [
      "# Manual check result",
      "",
...
`), ie = {
    ...
    $e((je) => [ie, ...je]), te(!0), S("output"), j(`Captured manual check: ${f}`);
  }, []), tt = P((l, f) => {
    const w = Xi({
      body: f,
      files: A,
      ...
      edits: w.edits.map(nc),
      skipped: w.skipped
    };
    return w.edits.length > 1 ? (k(N), Re(null), j(`Previewing AI workspace patch for ${w.edits.length} files`), !0) : w.edits.length === 1 ? (Re(N.edits[0]), k(null), j(`Previewing AI patch for ${N.edits[0].fileName}`), !0) : (j(w.skipped.length > 0 ? `No applicable edit found. ${w.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [V, ae, A]), Hn = P((l, f) => {
    const w = {
    ...
    $e((N) => [w, ...N]), H(!0), y("outputs"), te(!0), S("output"), j(`Saved local job output: ${l}`), (f.includes("```diff") || f.includes("--- a/")) && tt(l, f);
  }, [tt]), ir = P(() => {
    const l = G.trim();
    if (!l) return;
    se("");
    const f = Eo(l);
    (async () => {
      const w = await Ee(l), N = await J.askAgent(l, { requestContext: w });
      if (!f || !N || N.status === "error") return;
      tt(N.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", N.body) || j("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [J, Ee, G, tt]), or = P(() => {
    Q && (re(null), ze(Jo(Q)));
  }, [ze, Q]), pt = P((l) => {
    const f = Ri(A, l);
    Oe(f);
    const w = f.commands[0]?.command ?? "";
    xe(w), W(""), we(""), ut("failed"), te(!0), S("terminal"), j(f.commands.length > 0 ? `Manual check ready: copy ${f.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [A]), cr = P((l) => {
    l.trim() && (ot(l), xe(l), j(`Manual check command copied: ${l}`));
  }, []), lr = P(() => {
    Oe((l) => {
      if (!l) return l;
      const f = Yi(l, K), w = f.commands.at(-1)?.command ?? "";
      return w && xe(w), f;
    }), W("");
  }, [K]), dr = P(() => {
    if (!ye || !fe.trim()) {
    ...
    const l = fe.trim();
    Oe((f) => f && Ei(f, l, De, $)), we(""), j(`Captured manual check result: ${l} (${De})`);
  }, [$, fe, ye, De]), ur = P(() => {
    ye && ze(Ji(ye));
  }, [ze, ye]), hr = P(() => {
    if (!he) return;
    const l = A.find((f) => f.id === he.fileId);
    if (!l) {
      j(`Cannot apply edit — ${he.fileName} is no longer open`), Re(null);
      return;
    }
    l.content !== he.originalContent && !window.confirm(`${he.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (oe((f) => f.map((w) => w.id === he.fileId ? { ...w, content: he.proposedContent, dirty: !0 } : w)), et(he.fileId), I(`AI edit applied to ${l.name}. Save All to persist it to disk.`), Re(null), pt(`AI edit applied to ${l.name}`), j(`Applied AI edit to ${l.name} — unsaved; manual check ready`));
  }, [et, A, he, pt]), fr = P(() => {
    if (!he) return;
    const l = sn(A, `${rn(he.fileName)}-ai-edit`), f = {
      id: `pending-edit-${he.fileId}-${Date.now()}`,
      name: l,
      path: l,
      language: V?.language ?? "markdown",
      content: he.proposedContent,
      dirty: !0,
      source: "generated"
    };
    oe((w) => [...w, f]), me(f), Re(null), j(`Opened proposed edit as ${l}`);
  }, [V?.language, A, me, he]), mr = P(() => {
    if (!d) return;
    const l = new Map(d.edits.map((w) => [w.fileId, w]));
    A.some((w) => {
      const N = l.get(w.id);
      return N && w.content !== N.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (oe((w) => w.map((N) => {
      const ie = l.get(N.id);
      return ie ? { ...N, content: ie.proposedContent, dirty: !0 } : N;
    })), l.forEach((w, N) => et(N)), I(`AI workspace patch applied to ${l.size} file${l.size === 1 ? "" : "s"}. Save All to persist changes.`), k(null), pt(`AI workspace patch applied to ${l.size} file${l.size === 1 ? "" : "s"}`), j(`Applied AI workspace patch to ${l.size} file${l.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [et, A, d, pt]), pr = P(() => {
    if (!d) return;
    const l = [...A], f = d.edits.map((w) => {
      const N = sn(l, `${rn(w.fileName)}-ai-edit`), ie = A.find((bt) => bt.id === w.fileId), je = {
        id: `workspace-patch-${w.fileId}-${Date.now()}-${N}`,
        name: N,
        path: N,
        language: ie?.language ?? "markdown",
        content: w.proposedContent,
        dirty: !0,
        source: "generated"
      };
      return l.push(je), je;
    });
    oe((w) => [...w, ...f]), f[0] && me(f[0]), k(null), j(`Opened ${f.length} proposed edit file${f.length === 1 ? "" : "s"}`);
  }, [A, me, d]), jn = P((l) => {
    if (l === "explain") {
  ...
  return ce(() => {
    const l = (f) => {
      (f.metaKey || f.ctrlKey) && (f.key.toLowerCase() === "o" && (f.preventDefault(), mt()), f.key.toLowerCase() === "s" && (f.preventDefault(), Jt()), f.key.toLowerCase() === "w" && (f.preventDefault(), B && Zt(B)), f.key.toLowerCase() === "b" && (f.preventDefault(), m((N) => !N)), f.shiftKey && f.key.toLowerCase() === "f" && (f.preventDefault(), o("search"), m(!0)), (f.key.toLowerCase() === "k" || f.key.toLowerCase() === "p") && (f.preventDefault(), dt(!0)));
    };
    ...
    if (!n) return;
    const l = nn(n);
    b((f) => jt(f, l.primaryMin, l.primaryMax)), ee((f) => jt(f, l.secondaryMin, l.secondaryMax));
  }, [n]), ce(() => {
    const l = { primaryVisible: c, primaryWidth: v, secondaryVisible: L, secondaryWidth: U, markdownPreviewVisible: E };
    localStorage.setItem(Ca, JSON.stringify(l));
  }, [E, c, v, L, U]), ce(() => {
    localStorage.setItem(Ha, JSON.stringify(X));
  }, [X]), /* @__PURE__ */ u(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${c ? "" : "no-primary"} ${L ? "" : "no-secondary"} ${q ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${v}px`, "--workbench-secondary-width": `${U}px` },
      children: [
        /* @__PURE__ */ a(Xo, { active: i, setActive: (l) => {
          o(l), m(!0);
        ...
        c && /* @__PURE__ */ u("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            To,
            {
              ...
              openFile: mt,
              openFolder: Et,
              openWorkbenchFile: me,
              newFile: Kt,
              recent: ht,
              reopenRecent: Bt,
              setStatus: j,
              hasFsAccess: is(),
              attachSkillToChat: Cn,
          ...
          /* @__PURE__ */ u("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              Co,
              {
                ...
                closeSettings: An,
                secondaryVisible: L,
                toggleSecondary: () => H((l) => !l),
                canPreview: V?.language === "markdown",
                previewVisible: E,
                togglePreview: () => Y((l) => !l)
              }
            ),
            /* @__PURE__ */ a(Oo, { file: V, folder: D, showWelcome: Yt }),
            Z && ge.length > 0 ? /* @__PURE__ */ u("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: Z }),
              /* @__PURE__ */ a("button", { onClick: () => {
                On();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => I(null), title: "Dismiss", children: /* @__PURE__ */ a(Me, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: V ? /* @__PURE__ */ u("div", { className: V.language === "markdown" && E ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(vr, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                ho,
                {
                  file: V,
                  revealLine: zt,
              ...
              V.language === "markdown" && E && /* @__PURE__ */ a(Do, { content: V.content })
            ] }) : Tt ? /* @__PURE__ */ a(
              Zo,
              {
                host: e,
            ...
            ) : Yt ? /* @__PURE__ */ a(So, { host: e, openFile: mt, openFolder: Et, newFile: Kt, recent: ht, reopenRecent: Bt, setStatus: j, openControlTower: () => {
              o("computer"), m(!0);
            }, openChat: () => {
              y("chat"), H(!0);
            }, openEmbeddedDoc: Qa }) : /* @__PURE__ */ u("div", { className: "workbench-no-editor", children: [
            ...
            q && /* @__PURE__ */ a(
              Io,
              {
                ...
                manualCheckCommandInput: K,
                setManualCheckCommandInput: W,
                manualCheckOutputInput: $,
                setManualCheckOutputInput: we,
                manualCheckSelectedCommand: fe,
                setManualCheckSelectedCommand: xe,
                manualCheckStatus: De,
                setManualCheckStatus: ut,
                copyManualCheckCommand: cr,
                addManualCheckCommand: lr,
                recordManualCheckResult: dr,
                askAgentFromManualChecks: ur,
                onClose: () => te(!1)
              }
            )
          ] })
        ...
        L && /* @__PURE__ */ a(
          qo,
          {
            ...
            saveMessageAsArtifact: ar,
            rememberMessage: rr,
            previewEditFromMessage: (l) => tt(l.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", l.body),
            runQuickPrompt: jn,
            pendingPatchPrompt: Q,
            generatePatchPrompt: or,
            ...
            captureManualCheck: Qt,
            openOutputAsFile: Gt,
            previewEditFromOutput: (l) => tt(l.title, l.body),
            ...
            contextScope: ue,
            setContextScope: Te,
            contextAttachments: Za,
            removeContextAttachment: Ua,
            revealContextAttachment: $a,
            projectIndexSummary: `${Ye.snapshot.files.length} files · ${Ye.snapshot.chunks.length} chunks`,
            projectIndexStale: Ye.isStale,
            refreshProjectIndex: () => {
              const l = Ye.refresh(A);
        ...
        qt && /* @__PURE__ */ a(
          Vo,
          {
            ...
            commands: [
              ...
              ...ht.map((l) => ({ label: `File: Open Recent — ${l.name}`, detail: l.path, run: () => Bt(l) })),
              { label: "File: Save All", detail: `${ge.length} dirty file${ge.length === 1 ? "" : "s"}`, run: () => {
                On();
              }, disabled: ge.length === 0 },
              { label: "File: Close All Editors", detail: `${Ne.length} open editor${Ne.length === 1 ? "" : "s"}`, run: tr, disabled: Ne.length === 0 },
              ...
              { label: "View: Toggle Primary Side Bar", detail: c ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => m((l) => !l) },
              { label: "View: Toggle Chat Panel", detail: L ? "Hide right AI side bar" : "Show right AI side bar", run: () => H((l) => !l) },
              { label: "View: Toggle Bottom Panel", detail: q ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => te((l) => !l) },
              ...
              { label: "View: Toggle Markdown Preview", detail: V?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => Y((l) => !l), disabled: V?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: V || Ne.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Ut, disabled: !V && Ne.length === 0 },
              { label: "AI: Explain Active File", detail: V ? `Ask Cortex to explain ${V.path}` : "Open a file first", run: () => ze("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !V },
              { label: "AI: Improve Active File", detail: V ? `Ask Cortex for concrete edits to ${V.path}` : "Open a file first", run: () => ze("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !V },
              { label: "AI: Draft Editable Replacement", detail: V ? `Ask Cortex for a full-file replacement for ${V.path}` : "Open a file first", run: () => jn("edit"), disabled: !V },
              ...
              { label: "AI: Save Active File as Artifact", detail: V ? "Persist active file in host.ai artifacts" : "Open a file first", run: sr, disabled: !V },
        ...
        /* @__PURE__ */ a(tc, { status: _e, file: V ?? fo, cursor: p, fileCount: A.length, dirtyCount: ge.length })
      ]
    }
  );
}
function Xo({ active: e, setActive: t, openSettings: n, settingsActive: r }) {
  return /* @__PURE__ */ u("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-activity-brand", title: "Atomek", children: /* @__PURE__ */ a(Aa, { size: 30, variant: "cream" }) }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(ta, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Gr, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(aa, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(_n, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    ...
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(Ar, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(Ke, { icon: /* @__PURE__ */ a(es, { size: 23 }), label: "Settings", active: r, onClick: n })
  ] });
}
...
function To(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(Wo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(Bn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(Bn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(Ea, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(Lo, { ...e });
}
function Lo(e) {
  const t = !e.folder, n = pe(() => Ao(e.files, e.folder?.name), [e.files, e.folder?.name]), [r, s] = x(() => /* @__PURE__ */ new Set());
  ce(() => {
    const o = Fa(n);
    s((c) => {
      const m = new Set(c);
      return o.forEach((v) => m.add(v)), m;
    });
  }, [n]);
  const i = P((o) => {
    s((c) => {
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ u(We, { children: [
        ...
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : Ia(n, e.activeFileId, e.openWorkbenchFile, r, i)
      ] }),
...
function Ao(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), s = (o, c, m = n) => {
    const v = r.get(o);
    if (v) return v;
    const b = { name: c, path: o, children: [] };
  ...
  e.forEach((o) => {
    ...
    m.slice(0, -1).forEach((h) => {
      b = b ? `${b}/${h}` : h, v = s(b, h, v).children;
    }), v.push({ name: m.at(-1) ?? o.name, path: c, file: o, children: [] });
  });
  const i = (o) => o.sort((c, m) => +!!c.file - +!!m.file || c.name.localeCompare(m.name)).map((c) => ({ ...c, children: i(c.children) }));
  return i(n);
}
function Fa(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...Fa(t.children)]);
}
function Ia(e, t, n, r, s, i = 0) {
  return e.map((o) => {
    if (o.file)
    ...
    return /* @__PURE__ */ u("div", { children: [
      /* @__PURE__ */ u("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => s(o.path), title: c ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        ...
        c ? /* @__PURE__ */ a(yn, { size: 14 }) : /* @__PURE__ */ a(na, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: o.children.length })
      ] }),
      c ? Ia(o.children, t, n, r, s, i + 1) : null
    ] }, o.path);
  });
}
function Oo({ file: e, folder: t, showWelcome: n }) {
...
function Co(e) {
  return /* @__PURE__ */ u("div", { className: "workbench-tabs", children: [
    ...
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(Er, { size: 16 }) })
  ] });
}
function Ho(e) {
...
function So({
  ...
  const [b, h] = x("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [y, L] = x(null), [H, U] = x([]), [ee, q] = x(!1), [te, le] = x(null), [S, E] = x(null), [Y, M] = x("pod-local"), ne = P(async () => {
    q(!0), E(null);
    try {
      ...
      L(p ?? null), U(g), p && o(`Agent team loaded · ${p.resources.length} resources · ${p.warnings.length} warnings`);
    } catch (p) {
      const g = p instanceof Error ? p.message : String(p);
      E(g), o(`Agent team resource load failed: ${g}`);
    } finally {
      q(!1);
    }
  }, [e.missions, e.resources, o]), D = P((p, g) => {
    try {
      e.windows.openOrFocus(p), o(`Opened ${g}`);
    } catch (T) {
  ...
  const Ve = P(async () => {
    ...
    const p = b.trim() || "Coordinate a Tytus mission.", g = Qe(p, y, Y);
    q(!0);
    try {
      const T = await e.missions.create({
        ...
        data: { resourceCount: y?.resources.length ?? 0 }
      };
      await e.missions.write({
        rootPath: T.rootPath,
        files: [
          { path: "MISSION.md", content: Mt(F, y, null, [], p, g) },
          { path: "MISSION.json", content: Da(F, y, p, g) },
          { path: "RESOURCES.md", content: Vt(y) },
          { path: "TASKS.md", content: Ma(xt(p, y, g)) },
          { path: "HANDOFF.md", content: Va(F) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(ae)}
` },
...
`) }
      ...
      }), le(T), kt(F), o(`Mission created: ${T.rootPath}`), c();
    } catch (T) {
      o(`Mission create failed: ${T instanceof Error ? T.message : String(T)}`);
    } finally {
      q(!1);
    }
  }, [b, y, e.missions, c, o, Y]), A = Ho(y), oe = yo(y), be = wo(y), Pe = jo(y), B = y?.warnings ?? [], ve = ko(y), Ce = lt(y, Qe(b, y, Y)), Xe = xt(b, y, Ce.id);
  return /* @__PURE__ */ u("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ u("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ u("div", { className: "workbench-brand-lockup", "aria-label": "Atomek", children: [
        /* @__PURE__ */ a(Aa, { size: 74, variant: "acid" }),
        /* @__PURE__ */ u("div", { children: [
          /* @__PURE__ */ a(ro, { className: "workbench-brand-wordmark" }),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-control-goal-row", children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            Ve();
          }, disabled: ee, children: "Start mission" }),
      ...
      /* @__PURE__ */ a("div", { className: "workbench-team-preset-strip", "aria-label": "Team presets", children: ve.map((p) => /* @__PURE__ */ u(
        "button",
        {
          className: `workbench-team-preset-card ${p.id === Ce.id ? "active" : ""} ${p.readiness}`,
          onClick: () => M(p.id),
          children: [
            /* @__PURE__ */ u("div", { children: [
            ...
            /* @__PURE__ */ a("small", { children: p.assignments.map((g) => g.resourceLabel).slice(0, 3).join(" · ") })
          ]
        },
        p.id
      )) }),
      te ? /* @__PURE__ */ u("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: te.rootPath })
      ] }) : null,
    ...
    /* @__PURE__ */ u("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ u("header", { children: [
          ...
          /* @__PURE__ */ u("button", { className: "workbench-button-subtle", onClick: () => {
            ne();
          }, disabled: ee, children: [
            /* @__PURE__ */ a(gt, { size: 13 }),
            " ",
            ee ? "Refreshing…" : "Refresh"
          ] })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: A.map((p) => /* @__PURE__ */ u("div", { className: "workbench-control-metric", children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-agent-brand-grid", children: oe.map((p) => /* @__PURE__ */ u("div", { className: `workbench-agent-brand-card ${p.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ u("div", { children: [
            /* @__PURE__ */ a("strong", { children: p.label }),
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-doc-grid", children: uo.map((p) => /* @__PURE__ */ u("button", { className: "workbench-doc-card", onClick: () => v(p), title: p.summary, children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: be.map((p, g) => /* @__PURE__ */ u("div", { className: `workbench-fabric-node ${p.status.replace(/\s+/g, "-")}`, children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ u("header", { children: [
          ...
          /* @__PURE__ */ a("span", { children: H.length ? `${H.length} missions` : "none yet" })
        ] }),
        H.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Create a mission once; it stays in Tytus Home/Missions and can be resumed here." }) : null,
        H.slice(0, 4).map((p) => /* @__PURE__ */ u("button", { className: "workbench-control-preset", onClick: () => {
          kt(p), c();
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card", children: [
        ...
        s.length ? /* @__PURE__ */ u("div", { className: "workbench-control-recent", children: [
          ...
          s.slice(0, 3).map((p) => /* @__PURE__ */ a("button", { onClick: () => i(p), children: p.name }, `${p.path}-${p.at}`))
        ] }) : null
      ] }),
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ u("header", { children: [
          /* @__PURE__ */ a("strong", { children: "Resource graph" }),
          /* @__PURE__ */ a("span", { children: y ? `${y.resources.length} discovered` : "not loaded" })
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-control-resource-list", children: [
          Pe.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          Pe.map((p) => /* @__PURE__ */ u("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: ke(p) }),
              /* @__PURE__ */ a("span", { children: Nn(p) })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${p.status}`, children: p.status })
          ] }, p.id))
        ] }),
        B.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: B.slice(0, 2).map((p) => /* @__PURE__ */ u("span", { children: [
          p.code,
          ": ",
          p.message
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: Xe.map((p, g) => /* @__PURE__ */ u("button", { className: `workbench-task-card ${p.status}`, onClick: () => h(p.prompt), children: [
          /* @__PURE__ */ a("span", { children: g + 1 }),
          /* @__PURE__ */ a("strong", { children: p.title }),
...
function Wo({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: s }) {
  const i = pe(() => Mo(e, t), [e, t]), o = pe(() => {
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? o.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : o.map((c) => {
        ...
        return /* @__PURE__ */ u("div", { className: "workbench-search-group", children: [
          ...
          c.slice(0, 5).map((v) => /* @__PURE__ */ u("button", { className: "workbench-search-hit", onClick: () => r(m, v.lineNumber), title: v.line, children: [
            /* @__PURE__ */ a("span", { className: "workbench-search-line", children: v.lineNumber }),
            /* @__PURE__ */ a("span", { children: v.line })
          ] }, `${m.id}-${v.lineNumber}-${v.line}`)),
...
function Vo(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((s) => ({
    label: s.path,
    detail: `Open ${oa(s.language)} file`,
...
function Fo(e) {
  const t = e.session ? Zi(e.session) : "pending";
  return /* @__PURE__ */ u("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ u(We, { children: [
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ u("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(Kr, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(Hr, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(Me, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(Ro, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      Ea,
      {
        host: e.host,
...
function qa({ body: e }) {
  const t = pe(() => po(e), [e]), [n, r] = x(null), s = P((i, o) => {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((i) => {
      ...
      return /* @__PURE__ */ u("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ u("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ u("button", { onClick: () => s(i.key, i.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ a(Xr, { size: 12 }) : /* @__PURE__ */ a(cn, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: fa(i.body) }
      },
      i.key
    );
  }) });
...
function Ro(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Ge(null), [r, s] = x(!0), [i, o] = x(!1), c = pe(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  ce(() => {
    const h = n.current;
  ...
  const m = P(() => {
    const h = n.current;
    if (!h) return;
    const L = h.scrollHeight - h.scrollTop - h.clientHeight < 48;
    s(L), L && o(!1);
  }, []), v = P(() => {
    const h = n.current;
  ...
  return /* @__PURE__ */ u("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ u("option", { value: h.id, children: [
              h.title,
              " · ",
              Yo(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ u("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ u("div", { children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(qa, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ u(We, { children: [
          /* @__PURE__ */ a("br", {}),
        ...
        h.role === "assistant" && h.status !== "streaming" && h.status !== "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => b(h), title: "Copy this answer", children: [
        ...
        h.role === "assistant" && h.status === "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-chat-message-action", onClick: () => b(h), title: "Copy this error", children: [
            /* @__PURE__ */ a(cn, { size: 12 }),
            " Copy"
          ] }),
      ...
      i ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: v, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ u("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: ks(e.contextScope) }),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ u("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ u(We, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ u("span", { className: "workbench-chat-chip muted", children: [
            ...
            const y = typeof h.score == "number" ? h.score.toFixed(2) : null, L = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, H = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, U = [
              h.path,
            ...
            return /* @__PURE__ */ u("span", { className: "workbench-chat-chip", title: U, children: [
              ...
              h.label,
              y ? /* @__PURE__ */ a("small", { children: y }) : null,
        ...
        /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar compact", children: [
          ...
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(sa, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a($r, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Zn(e, t, n) {
  const r = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : Ra(e.gatewayPreference)];
...
function Zo(e) {
  const [t, n] = x([]), [r, s] = x("Loading gateway models…"), [i, o] = x([]), [c, m] = x("Checking embedding capability…");
  ce(() => {
    ...
    return (async () => {
      ...
      try {
        const H = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: y.signal
        });
        if (y.signal.aborted) return;
        n(H.map((U) => ({ id: U.id, gatewayLabel: U.gatewayLabel }))), s(H.length > 0 ? `${H.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (H) {
        if (y.signal.aborted) return;
        n([]), s(`Model discovery failed: ${H instanceof Error ? H.message : String(H)}`);
      }
    })(), () => y.abort();
    ...
    return (async () => {
      ...
      try {
        const U = await Di(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: y.signal
        });
        if (y.signal.aborted) return;
        o(U.map((ee) => ({ id: ee.id, gatewayLabel: ee.gatewayLabel ?? ee.source ?? "AIL" }))), m(U.length > 0 ? `${U.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (U) {
        if (y.signal.aborted) return;
        o([]), m(`Embedding model discovery failed: ${U instanceof Error ? U.message : String(U)}`);
      }
    })(), () => y.abort();
  }, [e.chatSettings.gatewayPreference, e.host]);
  const v = (y) => {
    e.onChange({ ...e.chatSettings, gatewayPreference: y });
  }, b = (y) => {
    e.onChange({ ...e.chatSettings, model: y });
  }, h = (y) => {
    e.onChange({ ...e.chatSettings, embeddingModel: y });
  };
    ...
    /* @__PURE__ */ u("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-chat-models", children: t.map((y) => /* @__PURE__ */ a("option", { value: y.id, children: y.gatewayLabel }, `${y.gatewayLabel}:${y.id}`)) })
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          Ra(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-embedding-models", children: i.map((y) => /* @__PURE__ */ a("option", { value: y.id, children: y.gatewayLabel }, `${y.gatewayLabel}:${y.id}`)) })
        ] }),
...
function Ya({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: s, openOutputAsFile: i, previewEditFromOutput: o, canPreviewEdit: c = !1, compact: m = !1 }) {
  return /* @__PURE__ */ u("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((v) => /* @__PURE__ */ u("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ u("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ a("strong", { children: v.title }),
        /* @__PURE__ */ a("span", { children: v.source === "ai" ? `AI · ${v.kind}` : v.kind }),
        /* @__PURE__ */ a("button", { onClick: () => void ot(v.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => i(v), children: "Open as file" }),
        o ? /* @__PURE__ */ a(
          "button",
          {
            className: Kn(v.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(v),
            disabled: !c,
            children: Kn(v.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        v.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(v.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ a(qa, { body: v.body })
    ] }, v.id))
  ] });
}
function Ko({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
...
function Uo({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ u("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: ac(i.proposedContent) })
      ] }, i.fileId)),
...
function Go(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, Bo(e.content), "```");
  else if (t.length > 0) {
...
function Ea({
  ...
  const [c, m] = x([]), [v, b] = x([]), [h, y] = x(null), [L, H] = x(!1), [U, ee] = x(null), [q, te] = x("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [le, S] = x(null), [E, Y] = x([]), [M, ne] = x(() => xo()), [D, Ve] = x([]), [A, oe] = x([]), [be, Pe] = x([]), B = Ge([]), [ve, Ce] = x("task-execute"), [Xe, p] = x("mission"), g = E.find((d) => d.status === "running" || d.status === "canceling") ?? E[0] ?? null, T = o === "dock", F = pe(() => lt(h, Qe(q || M?.goal || "", h, M?.teamPresetId)), [q, M?.goal, M?.teamPresetId, h]), ae = pe(() => xt(q || M?.goal || "", h, F.id), [q, M?.goal, F.id, h]), O = ae.find((d) => d.id === ve) ?? ae[1] ?? ae[0] ?? null, ue = i.filter((d) => d.dirty).length, Te = s ? `${s.path} · ${s.language} · ${s.content.length.toLocaleString()} chars${s.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${ue ? ` · ${ue} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Je = pe(() => [
    {
  ...
  const Ae = P(async (d = M) => {
    if (!d?.rootPath || !e.missions?.listRuns)
      return Pe([]), [];
    try {
      const k = await e.missions.listRuns(d.rootPath);
      return Pe(k), B.current = k, k;
    } catch (k) {
      return t(`Mission run history failed: ${k instanceof Error ? k.message : String(k)}`), [];
    }
  }, [e.missions, M, t]), He = P(async (d, k) => {
    if (!d) return;
    const z = [...k].sort((X, R) => Jn(R).localeCompare(Jn(X))).slice(0, 100), C = z.map((X) => JSON.stringify(X)).join(`
`) + (z.length ? `
` : "");
    d.rootPath && e.missions?.write ? await e.missions.write({ rootPath: d.rootPath, files: [{ path: "RUNS.jsonl", content: C }] }) : d.handle && await fn(d.handle, "RUNS.jsonl", C), Pe(z), B.current = z;
  }, [e.missions]), de = P(async (d, k = M) => {
    if (!k) return;
    const z = [
      d,
      ...B.current.filter((C) => C.id !== d.id)
    ];
    await He(k, z);
  }, [M, He]), Q = P(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      m([]), b([]), y(null), ee("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    H(!0), ee(null);
    try {
      const [d, k, z, C] = await Promise.all([
      ...
      m(d), b(k), y(z), Ve(C), M?.rootPath && Ae(M), t(`Agent team loaded · ${d.length} tools · ${k.length} skills · ${C.length} missions${z ? ` · ${z.resources.length} resources` : ""}`);
    } catch (d) {
      ee(d instanceof Error ? d.message : String(d));
    } finally {
      H(!1);
    }
  ...
  ce(() => {
    ...
    const d = (k) => {
      const z = k.detail;
      z?.missionId && (ne(z), Ae(z)), Q();
    };
    return window.addEventListener(hn, d), () => window.removeEventListener(hn, d);
  }, [Q, Ae]);
  const re = P((d) => {
    const k = No(d);
    ne(k), kt(k), Ae(k), oe([{
    ...
    }]), te(d.goal || `Continue mission ${d.title}. Review MISSION.md, TASKS.md, RESOURCES.md, and runs/ before acting.`), t(`Resumed mission: ${d.rootPath}`);
  }, [Ae, t]), G = P(async (d, k, z = []) => {
    const C = [
      ...A,
      ...z,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], X = Qe(k || d.goal, h, d.teamPresetId), R = xt(k || d.goal, h, X), Z = [
      { path: "MISSION.md", content: Mt(d, h, s, i, k, X) },
      { path: "MISSION.json", content: Da(d, h, k, X) },
      { path: "RESOURCES.md", content: Vt(h) },
      { path: "TASKS.md", content: Ma(R) },
      { path: "HANDOFF.md", content: Va(d) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: C.map((I) => JSON.stringify(I)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: B.current.map((I) => JSON.stringify(I)).join(`
`) + (B.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (d.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: d.rootPath, files: Z });
    else if (d.handle) {
      await st(d.handle, "runs"), await st(d.handle, "outputs"), await st(d.handle, "proposals"), await st(d.handle, "approvals");
      for (const I of Z) await vo(d.handle, I.path, I.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    oe(C), kt(d);
  }, [s, e.missions, A, i, h]), se = P(async (d, k = {}) => {
    if (M)
      return await G(M, d), M;
    const z = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, C = d.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let X = null;
    if (e.missions?.create) {
      const Z = await e.missions.create({ title: z, goal: C });
      X = {
        missionId: Z.missionId,
        title: Z.title,
        goal: Z.goal,
        rootPath: Z.rootPath,
        name: Z.rootPath.split("/").pop() || Z.missionId,
        source: "tray"
      };
      ...
      X = {
        handle: Z,
        name: Z.name,
        missionId: `mission-${Date.now()}-${Po(Z.name)}`,
        title: z,
        goal: C,
        source: "browser"
      };
    ...
    if (!X) return null;
    const R = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${X.rootPath ?? X.name}` };
    return ne(X), kt(X), oe([R]), await G(X, C, [R]), t(`Mission pack ready in ${X.rootPath ?? X.name}`), X;
  }, [e.missions, M, t, G]), qt = P(async (d) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const k = q.trim() || `Open ${d.label} from Atomek with current context.`, z = d.kind === "ai-cli" ? await se(k) : M;
      await e.local.openTerminal({
        toolId: d.id,
        command: d.command,
        cwd: z?.rootPath ?? Un(s),
        prompt: z ? `Atomek mission pack ready at ${z.rootPath ?? z.name}. Read MISSION.md and RESOURCES.md. ${Te}` : `Opened from Atomek. ${Te}`
      }), t(d.kind === "ai-cli" ? `Opened ${d.label} in Tytus Terminal with mission context. Press Enter there to start it.` : "Opened Tytus Terminal");
    } catch (k) {
      t(`Terminal launch failed: ${k instanceof Error ? k.message : String(k)}`);
    }
  }, [s, Te, se, e.local, q, M, t]), dt = P(async () => {
    try {
      await se(q.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (d) {
      t(`Mission folder setup failed: ${d instanceof Error ? d.message : String(d)}`);
    }
  }, [se, q, t]), qe = P(async (d, k, z, C = M, X) => {
    if (!C) return;
    const R = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), Z = X ?? `runs/${R}-${d.id}.md`, I = Z.split("/").pop() || `${R}-${d.id}.md`;
    if (C.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: C.rootPath, files: [{ path: Z, content: k }] });
    else if (C.handle) {
      const ye = await st(C.handle, "runs");
      await fn(ye, I, k);
    }
    await G(C, q, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${d.label} exited ${z}; transcript saved to ${Z}`,
      data: { toolId: d.id, exitCode: z, transcript: Z }
    }]);
  }, [e.missions, q, M, G]), Rt = P(async (d) => {
    ...
    let z = null;
    try {
      z = await se(k), z && await G(z, k, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${d.label} background review started`,
        data: { toolId: d.id, taskId: O?.id ?? "manual", taskTitle: O?.title ?? "Manual run" }
      }]);
    } catch (R) {
      t(`Mission pack failed before local job start: ${R instanceof Error ? R.message : String(R)}`);
      return;
    }
    S(d.id);
    const C = `local-run-${Date.now()}-${d.id}`, X = rt();
    Y((R) => [{
      id: C,
      toolId: d.id,
      label: d.label,
      ...
      lines: [`[Atomek] Starting ${d.label} local job for ${O?.title ?? "manual run"}…`]
    }, ...R].slice(0, 6));
    try {
      const R = await e.local.runJob({
        toolId: d.id,
        prompt: z ? [
...
`)
      }), Z = R.transcriptPath ?? (z?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${d.id}.md` : void 0), I = [
        `[Atomek] Started ${d.label} local job ${R.id}`,
        ...
        taskTitle: O?.title ?? "Manual run",
        transcriptPath: Z,
        summary: `Started ${d.label} for ${O?.title ?? "manual run"}`
      };
      de(ye, z).catch((K) => {
        t(`Mission run index failed: ${K instanceof Error ? K.message : String(K)}`);
      }), Y((K) => K.map((W) => W.id === C ? { ...W, jobId: R.id, transcriptPath: Z } : W));
      const Oe = (K) => {
        Y((W) => W.map(($) => $.id === C ? K($) : $));
      };
      e.local.streamJob(R.id, {
        onLog: (K) => {
          I.push(K), Oe((W) => ({ ...W, lines: I.slice(-500) }));
        },
        onDone: (K) => {
          K && (I.push(K), Oe((W) => ({ ...W, lines: I.slice(-500) })));
        },
        onFail: (K) => {
          I.push(`[FAIL] ${K}`), Oe((W) => ({
            ...W,
          ...
          }), r(`${d.label} local job failed`, I.join(`
`)), qe(d, I.join(`
`), -1, z, Z).catch((W) => {
...
`);
          Oe(($) => ({
            ...
            finishedAt: rt(),
            exitCode: K,
            summary: `${d.label} exited ${K}`
          }, z).catch(($) => {
            t(`Mission run index failed: ${$ instanceof Error ? $.message : String($)}`);
          }), r(`${d.label} local job`, W), qe(d, W, K, z, Z).catch(($) => {
            t(`Mission transcript save failed: ${$ instanceof Error ? $.message : String($)}`);
          }), S(null);
        },
        onError: () => t(`Local job stream issue for ${d.label}`)
      }), t(`Started ${d.label} mission run${O ? ` · ${O.title}` : ""}`);
    } catch (R) {
      S(null), Y((Z) => Z.map((I) => I.id === C ? {
        ...I,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...I.lines, `[Atomek] Failed to start: ${R instanceof Error ? R.message : String(R)}`]
      } : I)), t(`Local job failed to start: ${R instanceof Error ? R.message : String(R)}`);
    }
  }, [s, se, e.local, q, i, h, r, qe, O, t, de, G]), Nt = P(async (d) => {
    const k = mn(d);
    if (!k) {
      t(`Cannot dispatch ${ke(d)}: missing pod id`);
      return;
    }
    const z = q.trim();
    if (!z) {
    ...
    let C = null;
    const X = ke(d), R = `pod-run-${Date.now()}-${k}`, Z = rt(), I = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${d.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      C = await se(z), C && await G(C, z, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${X} mission task started`,
        data: { resourceId: d.id, podId: k, taskId: O?.id ?? "manual", taskTitle: O?.title ?? "Manual pod run" }
      }]);
    } catch (W) {
    ...
    const ye = {
      ...
      transcriptPath: I,
      summary: `Started ${X} for ${O?.title ?? "manual pod run"}`
    };
    Y((W) => [{
      id: R,
      toolId: d.id,
      ...
      taskTitle: O?.title ?? "Manual pod run",
      transcriptPath: I,
      lines: [`[Atomek] Dispatching ${X} via Tytus host bridge…`, `[Atomek] Pod: ${k}`, `[Atomek] Task: ${O?.title ?? "manual pod run"}`]
    }, ...W].slice(0, 6)), await de(ye, C).catch((W) => {
      t(`Mission run index failed: ${W instanceof Error ? W.message : String(W)}`);
    });
    const Oe = [], K = (W) => {
      Oe.push(W), Y(($) => $.map((we) => we.id === R ? { ...we, lines: [...we.lines, W].slice(-500) } : we));
    };
    try {
      const W = await e.daemon.callPodEndpoint(k, "/v1/models", { method: "GET" });
      if (!W.ok)
        throw new Error(`/v1/models ${W.status}: ${await W.text()}`);
      const $ = await W.json(), we = _o($);
      if (!we)
        throw new Error("pod /v1/models returned no model id");
      K(`[Atomek] Selected pod model from live metadata: ${we}`);
      const fe = await e.daemon.callPodEndpoint(k, "/v1/chat/completions", {
        method: "POST",
...
`)
            },
            {
              role: "user",
              content: [
                C ? `Mission folder: ${C.rootPath ?? C.name}` : "Mission folder: not available",
                C ? `Mission: ${C.title}` : "",
                C ? `Goal: ${C.goal}` : "",
...
`)
      ...
      if (!fe.ok)
        throw new Error(`/v1/chat/completions ${fe.status}: ${xe}`);
      let De = xe;
      try {
        De = JSON.parse(xe);
      } catch {
      }
      const ut = typeof De == "string" ? De : ec(De);
      K(ut);
      const zt = [
        `# Pod job — ${X}`,
        "",
...
`);
      Y((Xt) => Xt.map((_e) => _e.id === R ? { ..._e, status: "complete", finishedAt: Date.now(), lines: [..._e.lines, "[Atomek] Pod task complete."].slice(-500) } : _e)), await de({ ...ye, status: "complete", finishedAt: rt(), summary: `${X} completed`, transcriptPath: I }, C), r(`${X} pod job`, zt), await qe({ id: d.id, label: X, kind: "pod-agent", status: d.status }, zt, 0, C, I), t(`${X} completed mission task`);
    } catch (W) {
      const $ = W instanceof Error ? W.message : String(W);
      K(`[FAIL] ${$}`), Y((fe) => fe.map((xe) => xe.id === R ? { ...xe, status: "failed", finishedAt: Date.now(), lines: xe.lines.slice(-500) } : xe)), await de({ ...ye, status: "failed", finishedAt: rt(), exitCode: -1, summary: $, transcriptPath: I }, C).catch((fe) => {
        t(`Mission run index failed: ${fe instanceof Error ? fe.message : String(fe)}`);
      });
      const we = [`# Pod job failed — ${X}`, "", `- Pod: ${k}`, `- Error: ${$}`, "", "```text", Oe.join(`
`), "```", ""].join(`
`);
      r(`${X} pod job failed`, we), await qe({ id: d.id, label: X, kind: "pod-agent", status: d.status }, we, -1, C, I).catch((fe) => {
        t(`Pod transcript save failed: ${fe instanceof Error ? fe.message : String(fe)}`);
      }), t(`Pod task failed: ${$}`);
    }
  }, [s, se, e.daemon, q, i, h, r, qe, O, t, de, G]), $e = P(async (d) => {
    if (!d.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    Y((k) => k.map((z) => z.id === d.id ? { ...z, status: "canceling", lines: [...z.lines, "[Atomek] Cancel requested…"] } : z)), de({
      id: d.id,
      jobId: d.jobId,
      toolId: d.toolId,
      label: d.label,
    ...
    try {
      ...
      Y((z) => z.map((C) => C.id === d.id ? { ...C, status: "running", lines: [...C.lines, `[Atomek] Cancel failed: ${k instanceof Error ? k.message : String(k)}`] } : C)), t(`Local job cancel failed: ${k instanceof Error ? k.message : String(k)}`);
    }
  }, [e.local, t, de]), he = P(async (d) => {
    const k = [
      `Use Tytus resource "${ke(d)}" (${d.kind}) for the next mission step.`,
...
`);
    te(k), M && await G(M, k, [{
      ...
      message: `Selected resource ${ke(d)}`,
      data: { resourceId: d.id, kind: d.kind, status: d.status }
    }]), t(`Selected ${ke(d)} for mission`);
  }, [M, t, G]), Re = P((d) => {
    const k = d.setupAction, z = k?.commandPreview ? `${k.label}: ${k.commandPreview}` : k?.deepLink ? `${k.label}: ${k.deepLink}` : k?.label ?? `${ke(d)} needs setup`;
    t(z), ot(k?.commandPreview ?? k?.deepLink ?? z);
  }, [t]);
  return /* @__PURE__ */ u("aside", { className: T ? "workbench-agent-dock" : "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: T ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      ...
      /* @__PURE__ */ u("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ...
        " ",
        L ? "Refreshing…" : "Refresh capabilities"
      ] }),
      U && /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: U }),
      /* @__PURE__ */ u("div", { className: "workbench-team-switcher", role: "tablist", "aria-label": "Atomek agent team views", children: [
        /* @__PURE__ */ a("button", { className: Xe === "mission" ? "active" : "", onClick: () => p("mission"), children: "Mission" }),
        /* @__PURE__ */ a("button", { className: Xe === "runs" ? "active" : "", onClick: () => p("runs"), children: "Runs" }),
        /* @__PURE__ */ a("button", { className: Xe === "setup" ? "active" : "", onClick: () => p("setup"), children: "Setup" })
      ] }),
      Xe === "mission" ? /* @__PURE__ */ u(We, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ u("div", { className: `workbench-team-assignment-summary ${F.readiness}`, children: [
          ...
          F.assignments.map((d) => /* @__PURE__ */ u("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ u("div", { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M && void G(M, q), disabled: !M, children: "Rewrite context files" })
        ] }),
        D.length ? /* @__PURE__ */ u(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
          /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: D.slice(0, T ? 3 : 5).map((d) => /* @__PURE__ */ u(
            "button",
            {
        ...
        h?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: h.warnings.slice(0, 3).map((d) => /* @__PURE__ */ u("span", { children: [
          d.code,
          ": ",
          d.message
        ...
        /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: Je.map((d) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => te(d.prompt), children: d.label }, d.label)) }),
        /* @__PURE__ */ a(
          "textarea",
          {
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: ae.map((d, k) => /* @__PURE__ */ u(
          "button",
          {
            className: `workbench-task-card ${d.status} ${O?.id === d.id ? "active" : ""}`,
            onClick: () => {
              Ce(d.id), te(d.prompt);
            },
      ...
      Xe === "runs" ? /* @__PURE__ */ u(We, { children: [
        g ? /* @__PURE__ */ u("div", { className: "workbench-agent-run", children: [
          /* @__PURE__ */ u("header", { children: [
            /* @__PURE__ */ u("div", { children: [
...
`)), disabled: g.lines.length === 0, children: "Save output" }),
              g.jobId && (g.status === "running" || g.status === "canceling") ? /* @__PURE__ */ u("button", { className: "workbench-button-subtle danger", onClick: () => {
                $e(g);
              }, disabled: g.status === "canceling", children: [
                /* @__PURE__ */ a(sa, { size: 12 }),
                " ",
...
`) || "[waiting for output]" })
        ...
        be.length ? /* @__PURE__ */ u(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: be.slice(0, T ? 4 : 8).map((d) => /* @__PURE__ */ u("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ u("div", { children: [
            ...
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void ot(d.transcriptPath ?? d.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, d.id)) })
        ] }) : null
      ] }) : null,
      Xe === "setup" ? /* @__PURE__ */ u(We, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list compact", children: [
          !h && !L ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
          h?.resources.slice(0, T ? 8 : 5).map((d) => /* @__PURE__ */ u("div", { className: "workbench-resource-row", children: [
            /* @__PURE__ */ u("div", { children: [
              /* @__PURE__ */ a("strong", { children: ke(d) }),
              /* @__PURE__ */ a("span", { children: Nn(d) })
            ] }),
            /* @__PURE__ */ u("div", { className: "workbench-resource-row-actions", children: [
              ...
              d.kind === "pod-agent" && mn(d) ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Nt(d);
              }, children: "Ask pod" }) : null,
              d.status === "needs-setup" || d.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => Re(d), children: "Setup" }) : null
            ] })
          ] }, d.id))
        ] }),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          c.length === 0 && !L ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
          c.map((d) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            ...
            /* @__PURE__ */ u("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                qt(d);
              }, disabled: d.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: Qo(d) }),
              d.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Rt(d);
              }, disabled: d.status !== "available" || le !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: $o(d, le) }) : null
            ] })
          ] }, d.id))
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          v.length === 0 && !L ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
          v.map((d) => /* @__PURE__ */ u("div", { className: "workbench-computer-card", children: [
            /* @__PURE__ */ u("div", { className: "workbench-computer-card-head", children: [
              /* @__PURE__ */ u("div", { children: [
                /* @__PURE__ */ a("strong", { children: d.title }),
                /* @__PURE__ */ u("span", { children: [
...
function tc({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: s }) {
  return /* @__PURE__ */ u("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: oa(t.language) })
  ] });
}
function an(e, t) {
...
function oc({ host: e }) {
  return /* @__PURE__ */ a(zo, { host: e });
}
function dc(e) {
  return function() {
    return /* @__PURE__ */ a(oc, { host: e.host });
  };
}
export {
  dc as default
