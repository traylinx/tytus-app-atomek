...
import { forwardRef as st, createElement as He, useState as A, useRef as tt, useMemo as J, useCallback as v, useEffect as te, lazy as Gt, Suspense as Yt } from "react";
/**
...
}, dt = (...e) => e.filter((t, a, i) => !!t && t.trim() !== "" && i.indexOf(t) === a).join(" ").trim(), en = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const sn = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], $e = P("chevron-down", sn);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const qn = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Un() {
  const e = Ke();
...
async function Kn() {
  ...
  if (!e) return Nt(!1);
  const t = await e({ multiple: !0 }), a = await Promise.all(t.map((i) => yt(i, i.name, "local-file")));
  return Xe(a.filter(Boolean));
}
async function Xn() {
  const e = Ke()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await Nt(!0) };
  const t = await e({ mode: "readwrite" }), a = [];
  return await vt(t, t.name, a, 0, 320), { name: t.name, handle: t, files: Xe(a) };
}
function Ke() {
  const e = window;
...
async function vt(e, t, a, i, o) {
  if (!(a.length >= o || i > 8))
    for await (const l of e.values()) {
      if (a.length >= o) break;
      if (l.kind === "directory") {
        if (qn.has(l.name)) continue;
        await vt(l, `${t}/${l.name}`, a, i + 1, o);
        continue;
      }
      const h = l, s = await yt(h, `${t}/${h.name}`, "local-folder");
      s && a.push(s);
    }
}
function Nt(e) {
...
const Yn = 8e3, Jn = 5, at = (e, t = Yn) => e.length <= t ? e : `${e.slice(0, t)}
...
[...clipped ${e.length - t} chars...]`, Zn = (e, t) => {
  ...
  for (const i of t.slice(0, Jn))
    e && i.id === e.id || a.push({
      kind: "file",
      title: `Open editor: ${i.path}`,
      text: at(i.content, 4e3)
    });
...
}, ta = (e, t = ea) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, it = (e, t) => {
  const a = e.split(`
`).map((i) => i.replace(/^#+\s*/, "").trim()).find(Boolean);
  return a ? a.length > 80 ? `${a.slice(0, 77)}...` : a : t;
}, na = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", ot = (e) => ({
  id: e.id,
  title: e.title,
  kind: na(e.kind),
  body: e.body,
...
function ra({ host: e, activeFile: t, openEditors: a, chatSettings: i, setStatus: o }) {
  const l = e.ai, [h, s] = A(null), [f, m] = A([]), [g, N] = A([]), [R, D] = A([]), [he, K] = A([]), [Se, Z] = A(rt), [ae, ue] = A(!1), B = tt(!0), j = tt(null), le = J(() => Zn(t, a), [t, a]), ce = v(async () => {
    if (!l) {
      Z(rt);
      return;
    }
    ...
    if (l)
      try {
        ...
        if (!B.current) return;
        const x = f.find((S) => S.id === b) ?? null;
        x && s(x), N(p.map(ke).filter(Boolean)), K([]), await y(b);
      } catch (p) {
    ...
    if (l)
      try {
        const b = await l.listThreads({ workspaceKey: de, status: "active" }), p = b[0] ?? await l.createThread({ workspaceKey: de, title: "Atomek chat" });
        if (!B.current) return;
        m(b[0] ? b : [p]), s(p);
        const x = await l.listMessages(p.id);
        if (!B.current) return;
        N(x.map(ke).filter(Boolean)), await y(p.id);
      } catch (b) {
        o(`AI chat unavailable: ${b instanceof Error ? b.message : String(b)}`);
      }
  }, [l, y, o]);
  te(() => (B.current = !0, ce(), ve(), () => {
    B.current = !1;
  }), [ve, ce]);
  const G = v(async () => {
    if (!l) return null;
    ...
    return B.current && (s(b), m((p) => [b, ...p.filter((x) => x.id !== b.id)])), b;
  }, [l, h]), T = v(async () => {
    if (!l) return;
    const b = await l.createThread({ workspaceKey: de, title: "Atomek chat" });
    s(b), m((p) => [b, ...p.filter((x) => x.id !== b.id)]), N([]), D([]), K([]), o("New AI chat created");
  }, [l, o]), ee = v(async (b, p) => {
    if (!l) return;
    const x = p.trim();
    if (x)
      try {
        const S = await l.updateThread({ threadId: b, title: x });
        m((I) => I.map((M) => M.id === S.id ? S : M)), s((I) => I?.id === S.id ? S : I), o(`Renamed chat: ${S.title}`);
      } catch (S) {
        o(`Rename chat failed: ${S instanceof Error ? S.message : String(S)}`);
      }
  }, [l, o]), me = v(async (b) => {
    if (l)
      try {
        await l.deleteThread(b);
        const p = f.filter((x) => x.id !== b);
        if (m(p), h?.id === b) {
          const x = p[0] ?? await l.createThread({ workspaceKey: de, title: "Atomek chat" });
          s(x), m((I) => I.some((M) => M.id === x.id) ? I : [x, ...I]);
          const S = await l.listMessages(x.id);
          N(S.map(ke).filter(Boolean)), await y(x.id);
        }
        o("Deleted AI chat");
      } catch (p) {
        o(`Delete chat failed: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [l, y, o, h?.id, f]), Pe = v(async (b) => {
    if (!l) return null;
    try {
      const p = await G();
      if (!p) return null;
      const x = await l.createArtifact({
        threadId: p.id,
        messageId: b.messageId ?? null,
        title: b.title?.trim() || it(b.body, "Atomek artifact"),
        kind: b.kind ?? "markdown",
        body: b.body
      }), S = ot(x);
      return D((I) => [S, ...I.filter((M) => M.id !== S.id)]), o(`Saved AI artifact: ${S.title}`), S;
    } catch (p) {
    ...
    if (l)
      try {
        await l.deleteArtifact(b), D((p) => p.filter((x) => x.id !== b)), o("Deleted AI artifact");
      } catch (p) {
    ...
    try {
      const p = await l.writeMemory({
        title: b.title?.trim() || it(b.body, "Atomek memory"),
        body: b.body,
        metadata: {
          source: "atomek",
          messageId: b.messageId ?? null
      ...
      return K((x) => [p, ...x.filter((S) => S.id !== p.id)].slice(0, 5)), o(`Remembered: ${p.title}`), p;
    } catch (p) {
    ...
    try {
      const I = await G();
      if (!I) return null;
      const M = await re(p).catch(() => []), X = aa(M), Q = X ? [...le, X] : le;
      let V = null;
      for await (const z of l.sendMessage({
        threadId: I.id,
        body: p,
        ...
        if (z.type === "message_created") {
          const F = ke(z.message);
          if (!F) continue;
          F.role === "assistant" && (V = F.id), N((H) => [...H.filter((q) => q.id !== F.id), F]);
        }
        if (z.type === "token" && (V = z.messageId, N((F) => F.map(
          (H) => H.id === z.messageId ? { ...H, body: z.body, status: "streaming" } : H
        ))), z.type === "message_updated" || z.type === "done") {
          const F = ke(z.message);
          if (!F) continue;
          F.role === "assistant" && (x = F), N((H) => H.map((q) => q.id === F.id ? F : q)), F.gatewayLabel && o(`AI answered via ${F.gatewayLabel}`);
        }
        if (z.type === "run_failed") {
          const F = S.signal.aborted;
          o(F ? "AI response stopped" : `AI failed: ${z.error}`), V && N((H) => H.map(
            (q) => q.id === V ? { ...q, status: "error", error: z.error, body: F ? "Stopped by user." : z.error } : q
          ));
        }
      }
      const Ae = await l.listThreads({ workspaceKey: de, status: "active" }).catch(() => []);
      return B.current && Ae.length > 0 && m(Ae), ce(), x;
    } catch (I) {
      return I instanceof DOMException && I.name === "AbortError" ? (o("AI response stopped"), x) : (o(`AI failed: ${I instanceof Error ? I.message : String(I)}`), null);
    } finally {
      j.current === S && (j.current = null), ue(!1);
    }
  }, [l, i.gatewayPreference, i.model, le, G, re, ce, o]), Fe = v(() => {
    j.current?.abort(), o("Stopping AI response…");
  }, [o]);
  return {
    aiStatus: Se,
    artifacts: R,
...
const ia = Gt(() => import("./WorkbenchMonacoEditor-BaNVDl1J.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), oa = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Ct = "tytus.workspace.recent", xt = "tytus.workspace.layout", $t = "tytus.atomek.chatAiSettings", xe = {
  gatewayPreference: "auto",
  model: ""
};
function la({ host: e }) {
  const t = J(() => Wa(), []), [a, i] = A("explorer"), [o, l] = A(t.primaryVisible), [h, s] = A(t.primaryWidth), [f, m] = A("chat"), [g, N] = A(t.secondaryVisible), [R, D] = A(t.secondaryWidth), [he, K] = A(!1), [Se, Z] = A("problems"), [ae, ue] = A(t.markdownPreviewVisible), [B, j] = A(!1), [le, ce] = A(null), [y, L] = A([]), [ve, G] = A([]), [T, ee] = A(null), [me, Pe] = A(""), [Me, Ne] = A({ lineNumber: 1, column: 1 }), [re, fe] = A(""), [Fe, b] = A(!1), [p, x] = A(""), [S, I] = A([]), [M, X] = A(null), [Q, V] = A(null), [Ae, z] = A(!1), [F, H] = A(() => Ra()), [q, pe] = A(null), [Et, Ye] = A(null), [zt, $] = A("Ready"), [be, Ot] = A(() => _a()), W = ve.map((r) => y.find((d) => d.id === r)).filter(Boolean), w = T ? y.find((r) => r.id === T) ?? null : null, C = ra({ host: e, activeFile: w, openEditors: W, chatSettings: F, setStatus: $ }), ie = J(
    () => [...C.artifacts, ...S].sort((r, d) => d.createdAt - r.createdAt),
    [C.artifacts, S]
  ), Ie = !w && !B, O = J(() => y.filter((r) => r.dirty), [y]), Lt = J(() => {
    const r = me.trim().toLowerCase();
    return r ? y.filter((d) => d.path.toLowerCase().includes(r)) : y;
  }, [y, me]), Tt = v((r) => {
    r.preventDefault();
    const d = r.clientX, u = R, k = (se) => {
      const ge = u + (d - se.clientX);
      D(Math.max(380, Math.min(760, ge)));
    }, E = () => {
      window.removeEventListener("pointermove", k), window.removeEventListener("pointerup", E);
    };
    window.addEventListener("pointermove", k), window.addEventListener("pointerup", E);
  }, [R]), _t = v((r) => {
    r.preventDefault();
    const d = r.clientX, u = h, k = (se) => {
    ...
    Ot(d), localStorage.setItem(Ct, JSON.stringify(d));
  }, [be]), _ = v((r, d) => {
    G((u) => u.includes(r.id) ? u : [...u, r.id]), ee(r.id), j(!1), Ye(d ?? null), Ne({ lineNumber: d ?? 1, column: 1 });
  }, []), we = v(async () => {
    if (De(O, "open new files"))
      try {
        const r = await Kn();
        if (r.length === 0) return;
        L((d) => Ma(d, r)), r.forEach((d) => Ce({ name: d.name, path: d.path, at: Date.now() })), _(r[0]), $(`Opened ${r.length} local file${r.length === 1 ? "" : "s"}`);
      } catch (r) {
        r.name !== "AbortError" && $(`Open file failed: ${r.message}`);
      }
  }, [O, _, Ce]), Ee = v(async () => {
    if (De(O, "open another folder"))
      try {
        const r = await Xn();
        ce(r), L(r.files), G([]), ee(null), j(!1), Ce({ name: r.name, path: r.name, at: Date.now() }), $(`${r.handle ? "Opened local folder" : "Opened browser fallback folder"} ${r.name} (${r.files.length} text files indexed)`);
      } catch (r) {
        r.name !== "AbortError" && $(`Open folder failed: ${r.message}`);
      }
  }, [O, Ce]), Wt = v((r) => {
    T && L((d) => d.map((u) => u.id === T ? { ...u, content: r, dirty: r !== u.content || u.dirty } : u));
  }, [T]), ze = v(async () => {
    if (w)
      try {
        const r = await Re(w);
    ...
    const d = y.find((k) => k.id === r);
    if (!d) return null;
    const u = await Re(d);
    return L((k) => k.map((E) => E.id === u.id ? u : E)), u;
  }, [y]), Je = v(async () => {
    const r = y.filter((d) => d.dirty);
    if (r.length === 0) {
      $("No dirty files to save");
      return;
    }
    try {
      const d = await Promise.all(r.map((k) => Re(k))), u = new Map(d.map((k) => [k.id, k]));
      L((k) => k.map((E) => u.get(E.id) ?? E)), pe(null), $(`Saved ${d.length} dirty file${d.length === 1 ? "" : "s"}`);
    } catch (d) {
      $(`Save all failed: ${d.message}`);
    }
  }, [y]), Oe = v((r) => {
    const d = y.find((u) => u.id === r);
    if (d?.dirty && !window.confirm(`${d.name} has unsaved changes. Close without saving?`)) {
      $(`Close canceled — ${d.name} has unsaved changes`);
      return;
    }
    G((u) => {
      const k = u.filter((E) => E !== r);
      return T === r && ee(k.at(-1) ?? null), k;
    });
  }, [T, y]), Dt = v(() => {
    De(O, "close all editors") && (G([]), ee(null), Ye(null), j(!1), $("Closed all editors"));
  }, [O]), Le = v(() => {
    const r = y.filter((u) => u.name.startsWith("Untitled")).length + 1, d = {
...
`,
    ...
    const d = [
      `Create a polished Markdown artifact from ${w?.path ?? `${W.length} open editors`}.`,
    ...
    N(!0), m("chat"), $("Asking Atomek to synthesize an AI artifact…"), C.askAgent(d).then((u) => {
      !u || u.status === "error" || C.createArtifact({
        messageId: u.id,
        title: `AI synthesis — ${w?.name ?? "open workspace"}`,
        kind: "markdown",
        body: u.body
      }).then(() => {
        N(!0), m("outputs"), K(!0), Z("output");
      });
    });
  }, [w, C, W.length]), _e = v((r) => {
    const d = y.find((u) => u.path === r.path || u.name === r.name);
    if (d) {
      _(d), $(`Opened recent ${d.name}`);
      return;
    }
    ...
    N(!0), m("chat"), fe(""), C.askAgent(r);
  }, [C]), jt = v((r) => {
    const d = C.messages.findIndex((k) => k.id === r.id), u = C.messages.slice(0, d < 0 ? void 0 : d).reverse().find((k) => k.role === "user");
    if (!u?.body.trim()) {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const d = je(y, Be(r.title || r.kind)), u = {
    ...
    L((k) => [...k, u]), _(u), $(`Opened ${r.title} as editable file`);
  }, [y, _]), Ze = v((r, d) => {
    const u = Fa(d, y, r);
    if (u.edits.length > 1) {
      V(u), X(null), $(`Previewing AI workspace patch for ${u.edits.length} files`);
      return;
    }
    if (u.edits.length === 1) {
      X(u.edits[0]), V(null), $(`Previewing AI patch for ${u.edits[0].fileName}`);
      return;
    }
    if (!w) {
    ...
    V(null), X({
      ...
      proposedContent: k.content,
      sourceTitle: r,
      extractionLabel: k.label,
      stats: It(w.content, k.content)
    }), $(`Previewing AI edit for ${w.name}`);
  }, [w, y]), Ut = v(() => {
    if (!M) return;
    const r = y.find((d) => d.id === M.fileId);
    if (!r) {
      $(`Cannot apply edit — ${M.fileName} is no longer open`), X(null);
      return;
    }
    r.content !== M.originalContent && !window.confirm(`${M.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (L((d) => d.map((u) => u.id === M.fileId ? { ...u, content: M.proposedContent, dirty: !0 } : u)), pe(`AI edit applied to ${r.name}. Save All to persist it to disk.`), X(null), $(`Applied AI edit to ${r.name} — unsaved`));
  }, [y, M]), Kt = v(() => {
    if (!M) return;
    const r = je(y, `${Be(M.fileName)}-ai-edit`), d = {
      id: `pending-edit-${M.fileId}-${Date.now()}`,
      name: r,
      path: r,
    ...
    L((u) => [...u, d]), _(d), X(null), $(`Opened proposed edit as ${r}`);
  }, [w?.language, y, _, M]), Xt = v(() => {
    if (!Q) return;
    const r = new Map(Q.edits.map((u) => [u.fileId, u]));
    y.some((u) => {
      const k = r.get(u.id);
      return k && u.content !== k.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (L((u) => u.map((k) => {
      const E = r.get(k.id);
      return E ? { ...k, content: E.proposedContent, dirty: !0 } : k;
    })), pe(`AI workspace patch applied to ${r.size} file${r.size === 1 ? "" : "s"}. Save All to persist changes.`), V(null), $(`Applied AI workspace patch to ${r.size} file${r.size === 1 ? "" : "s"} — unsaved`));
  }, [y, Q]), Qt = v(() => {
    if (!Q) return;
    const r = [...y], d = Q.edits.map((u) => {
      const k = je(r, `${Be(u.fileName)}-ai-edit`), E = y.find((ge) => ge.id === u.fileId), se = {
        id: `workspace-patch-${u.fileId}-${Date.now()}-${k}`,
        name: k,
        path: k,
    ...
    L((u) => [...u, ...d]), d[0] && _(d[0]), V(null), $(`Opened ${d.length} proposed edit file${d.length === 1 ? "" : "s"}`);
  }, [y, _, Q]), et = v((r) => {
    if (r === "explain") {
      Y("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (r === "improve") {
      Y("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (r === "plan") {
      Y("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (r === "edit") {
      Y("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    Y("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [Y]);
  return te(() => {
    const r = (d) => {
      (d.metaKey || d.ctrlKey) && (d.key.toLowerCase() === "o" && (d.preventDefault(), we()), d.key.toLowerCase() === "s" && (d.preventDefault(), ze()), d.key.toLowerCase() === "w" && (d.preventDefault(), T && Oe(T)), d.key.toLowerCase() === "b" && (d.preventDefault(), l((k) => !k)), d.shiftKey && d.key.toLowerCase() === "f" && (d.preventDefault(), i("search"), l(!0)), (d.key.toLowerCase() === "k" || d.key.toLowerCase() === "p") && (d.preventDefault(), b(!0)));
    };
    return window.addEventListener("keydown", r), () => window.removeEventListener("keydown", r);
  }, [T, Oe, we, ze]), te(() => {
    const r = (d) => {
      O.length !== 0 && (d.preventDefault(), d.returnValue = "");
    };
    return window.addEventListener("beforeunload", r), () => window.removeEventListener("beforeunload", r);
  }, [O.length]), te(() => {
    O.length === 0 && pe(null);
  }, [O.length]), te(() => {
    const r = { primaryVisible: o, primaryWidth: h, secondaryVisible: g, secondaryWidth: R, markdownPreviewVisible: ae };
    localStorage.setItem(xt, JSON.stringify(r));
  }, [ae, o, h, g, R]), te(() => {
    localStorage.setItem($t, JSON.stringify(F));
  }, [F]), /* @__PURE__ */ c(
    "div",
    {
      className: `workbench-workbench ${o ? "" : "no-primary"} ${g ? "" : "no-secondary"} ${he ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${h}px`, "--workbench-secondary-width": `${R}px` },
      children: [
        /* @__PURE__ */ n(ca, { active: a, setActive: (r) => {
          i(r), l(!0);
        }, openSettings: () => z(!0) }),
        o && /* @__PURE__ */ c("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ n("div", { className: "workbench-primary-resizer", onPointerDown: _t, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ c("main", { className: "workbench-editor-area", children: [
          ...
          /* @__PURE__ */ c("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ n(ua, { file: w, folder: le, showWelcome: Ie }),
            q && O.length > 0 ? /* @__PURE__ */ c("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ n("span", { children: q }),
            ...
            /* @__PURE__ */ n("div", { className: "workbench-editor-content", children: w ? /* @__PURE__ */ c("div", { className: w.language === "markdown" && ae ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ n("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ n(Yt, { fallback: /* @__PURE__ */ n("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ n(
                ia,
                {
                  file: w,
                  revealLine: Et,
              ...
              w.language === "markdown" && ae && /* @__PURE__ */ n(ga, { content: w.content })
            ] }) : Ie ? /* @__PURE__ */ n(fa, { openFile: we, openFolder: Ee, newFile: Le, recent: be, reopenRecent: _e, setStatus: $ }) : /* @__PURE__ */ c("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ n(qe, { size: 34 }),
              /* @__PURE__ */ n("p", { children: "No editor open" }),
              /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: () => j(!1), children: "Show Welcome" })
            ] }) }),
            he && /* @__PURE__ */ n(
              ka,
              {
        ...
        g && /* @__PURE__ */ n(
          ya,
          {
            ...
            chatThread: C.thread,
            chatThreads: C.threads,
            askAgent: Bt,
            stopChat: C.stopChat,
            regenerateMessage: jt,
            newChat: () => {
              C.newChat();
            },
            selectThread: (r) => {
            ...
            saveMessageAsArtifact: Vt,
            rememberMessage: Ht,
            previewEditFromMessage: (r) => Ze(r.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", r.body),
            runQuickPrompt: et,
            workspaceFileCount: y.length,
            aiStatus: C.aiStatus,
            chatSettings: F,
            openSettings: () => z(!0),
            busy: C.busy,
            memoryHitCount: C.memoryHits.length,
            outputs: ie,
            runAiSynthesis: Te,
            openOutputAsFile: We,
            previewEditFromOutput: (r) => Ze(r.title, r.body),
            canPreviewEdit: y.length > 0,
            clearOutputs: () => I([]),
            deleteArtifact: (r) => {
              C.deleteArtifact(r);
            },
        ...
        Fe && /* @__PURE__ */ n(
          wa,
          {
            ...
            activeFile: w,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: Le },
              ...
              { label: "File: Save All", detail: `${O.length} dirty file${O.length === 1 ? "" : "s"}`, run: () => {
                Je();
              }, disabled: O.length === 0 },
              { label: "File: Close All Editors", detail: `${W.length} open editor${W.length === 1 ? "" : "s"}`, run: Dt, disabled: W.length === 0 },
              ...
              { label: "View: Toggle Bottom Panel", detail: he ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => K((r) => !r) },
              ...
              { label: "Atomek: Create AI Synthesis", detail: w || W.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Te, disabled: !w && W.length === 0 },
              { label: "AI: Explain Active File", detail: w ? `Ask Cortex to explain ${w.path}` : "Open a file first", run: () => Y("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !w },
              { label: "AI: Improve Active File", detail: w ? `Ask Cortex for concrete edits to ${w.path}` : "Open a file first", run: () => Y("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !w },
              { label: "AI: Draft Editable Replacement", detail: w ? `Ask Cortex for a full-file replacement for ${w.path}` : "Open a file first", run: () => et("edit"), disabled: !w },
              { label: "AI: Plan Workspace Work", detail: W.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => Y("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: W.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: w ? "Persist active file in host.ai artifacts" : "Open a file first", run: qt, disabled: !w },
              { label: "AI: Open Latest Artifact as File", detail: ie[0] ? `Create editable file from ${ie[0].title}` : "No outputs yet", run: () => ie[0] && We(ie[0]), disabled: ie.length === 0 }
            ],
            openWorkbenchFile: _,
            onClose: () => b(!1)
          }
        ...
        /* @__PURE__ */ n(Pa, { status: zt, file: w ?? oa, cursor: Me, fileCount: y.length, dirtyCount: O.length })
      ]
    }
  );
}
function ca({ active: e, setActive: t, openSettings: a }) {
  return /* @__PURE__ */ c("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(pt, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(On, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(An, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(cn, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(ht, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ n("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(hn, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(Wn, { size: 23 }), label: "Manage", active: !1, onClick: a })
  ] });
}
...
function sa(e) {
  return e.activity === "search" ? /* @__PURE__ */ n(pa, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ n(ct, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ n(ct, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ n(Sa, {}) : /* @__PURE__ */ n(da, { ...e });
}
function da(e) {
  const t = !e.folder, a = J(() => ha(e.files, e.folder?.name), [e.files, e.folder?.name]);
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ c(Ve, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((i) => /* @__PURE__ */ n(Qe, { file: i, active: i.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(i) }, i.id)),
        /* @__PURE__ */ c("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ n($e, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        a.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No readable text files found." }) : St(a, e.activeFileId, e.openWorkbenchFile)
      ] }),
      ...
      e.recent.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((i) => /* @__PURE__ */ c("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(i), children: [
        /* @__PURE__ */ n(vn, { size: 14 }),
        /* @__PURE__ */ n("span", { className: "workbench-row-name", children: i.name })
      ] }, `${i.path}-${i.at}`))
    ] })
  ] });
}
function Qe({ file: e, active: t, onOpen: a, basePath: i, depth: o = 0, label: l }) {
...
function St(e, t, a, i = 0) {
  return e.map((o) => o.file ? /* @__PURE__ */ n(Qe, { file: o.file, active: o.file.id === t, onOpen: () => a(o.file), depth: i, label: o.name }, o.file.id) : /* @__PURE__ */ c("div", { children: [
    ...
    St(o.children, t, a, i + 1)
  ] }, o.path));
}
function ua({ file: e, folder: t, showWelcome: a }) {
...
function ma(e) {
  return /* @__PURE__ */ c("div", { className: "workbench-tabs", children: [
    ...
    e.canPreview && /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ n(ut, { size: 16 }) }),
    /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ n(Sn, { size: 16 }) })
  ] });
}
function fa({ openFile: e, openFolder: t, newFile: a, recent: i, reopenRecent: o, setStatus: l }) {
...
function pa({ files: e, query: t, setQuery: a, openWorkbenchFile: i, activeFileId: o }) {
  const l = J(() => ba(e, t), [e, t]), h = J(() => {
    ...
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? h.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No matches." }) : h.map((s) => {
        ...
        return /* @__PURE__ */ c("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ n(Qe, { file: f, active: f.id === o, onOpen: () => i(f) }),
...
function wa(e) {
  const t = e.query.trim().toLowerCase(), a = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${gt(o.language)} file`,
...
function ka(e) {
  return /* @__PURE__ */ c("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ n(Pt, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function ya(e) {
  return /* @__PURE__ */ c("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ c("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ n("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ n(bt, { size: 15 }) }),
        /* @__PURE__ */ n("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ n(mn, { size: 16 }) }),
        /* @__PURE__ */ n("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ n(ne, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ n(va, { ...e }) : /* @__PURE__ */ n(Pt, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function va(e) {
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((t) => /* @__PURE__ */ c("option", { value: t.id, children: [
              t.title,
              " · ",
              Aa(t.lastMessageAt ?? t.updatedAt)
            ] }, t.id))
          ]
        }
      ),
      ...
      /* @__PURE__ */ c("div", { className: "workbench-chat-box", children: [
        ...
        /* @__PURE__ */ c("div", { className: "workbench-chat-toolbar", children: [
          /* @__PURE__ */ c("button", { className: "workbench-chat-mode", onClick: e.openSettings, title: "Chat AI settings", children: [
            Ge(e.chatSettings.gatewayPreference),
            " ",
            /* @__PURE__ */ n($e, { size: 12 })
          ] }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-mode", onClick: () => e.runQuickPrompt("plan"), disabled: e.busy, children: "Plan" }),
          /* @__PURE__ */ n("span", {}),
          e.busy ? /* @__PURE__ */ n("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ n(jn, { size: 14 }) }) : /* @__PURE__ */ n("button", { className: "workbench-chat-send", onClick: e.askAgent, title: "Send", children: /* @__PURE__ */ n(Tn, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Na(e, t, a) {
  const i = e.model.trim(), l = [e.gatewayPreference === "auto" ? t : Ge(e.gatewayPreference)];
...
function Pt({ outputs: e, clearOutputs: t, deleteArtifact: a, runAiSynthesis: i, openOutputAsFile: o, previewEditFromOutput: l, canPreviewEdit: h = !1, compact: s = !1 }) {
    ...
    e.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ c("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ c("div", { className: "workbench-output-head", children: [
        ...
        l ? /* @__PURE__ */ n(
          "button",
          {
            className: lt(f.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => l(f),
            disabled: !h,
            children: lt(f.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
...
function xa({ edit: e, onApply: t, onOpenAsFile: a, onClose: i }) {
...
function $a({ patch: e, onApply: t, onOpenAsFiles: a, onClose: i }) {
  ...
  return /* @__PURE__ */ n("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ c("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((l) => /* @__PURE__ */ c("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ n("pre", { children: Oa(l.proposedContent) })
      ] }, l.fileId)),
...
function Pa({ status: e, file: t, cursor: a, fileCount: i, dirtyCount: o }) {
  return /* @__PURE__ */ c("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ n("span", { children: gt(t.language) })
  ] });
}
function De(e, t) {
...
function Fa(e, t, a) {
  const o = [
    ...Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((s) => ({
      ...
      content: Ft(s[2])
    })).filter((s) => s.content.trim().length > 0).filter((s) => U(s.lang, "diff") || U(s.lang, "patch")).map((s) => s.content),
    e
...
function Ea(e) {
  ...
  if (t) return ye(t[3] || t[4] || t[1] || t[2] || "");
  const a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m);
  if (a) return ye(a[1] || a[2] || "");
  const i = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m);
  return i ? ye(i[1] || i[2] || "") : null;
}
function ye(e) {
  return e.replace(/^["']|["']$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function za(e, t) {
  const a = ye(t), i = a.split("/").at(-1) ?? a;
  return e.find((o) => {
    const l = ye(o.path);
...
function La(e, t) {
  const a = Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((m) => ({
    ...
    ...a.filter((m) => U(m.lang, "diff") || U(m.lang, "patch")).map((m) => m.content),
    e
  ];
  for (const m of i) {
    const g = Mt(t.content, m);
    if (g) return { content: g, label: "unified diff patch" };
  }
  if (a.length === 0) return null;
  const o = Ta(t), l = a.find((m) => U(m.lang, "atomek-replace") || U(m.lang, "replace"));
  if (l) return { content: l.content, label: `replacement block (${l.lang || "plain"})` };
  const h = a.find((m) => o.some((g) => U(m.lang, g)) && !U(m.lang, "diff") && !U(m.lang, "patch"));
  if (h) return { content: h.content, label: `matched ${h.lang || t.language} block` };
  const f = a.filter((m) => !U(m.lang, "diff") && !U(m.lang, "patch")).sort((m, g) => g.content.length - m.content.length)[0];
...
function Da({ host: e }) {
  return /* @__PURE__ */ n(la, { host: e });
}
function Va(e) {
  return function() {
    return /* @__PURE__ */ n(Da, { host: e.host });
  };
}
export {
  Va as default
