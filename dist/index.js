 ...
 */
const Zn = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), ea = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), Et = (e) => {
  const t = ea(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, Xt = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), ta = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const la = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], ut = E("chevron-down", la);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const Va = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Ua() {
  const e = gt();
...
async function qa() {
  ...
  if (!e) return on(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((a) => an(a, a.name, "local-file")));
  return yt(n.filter(Boolean));
}
async function Ka() {
  const e = gt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await on(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await rn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: yt(n) };
}
function gt() {
  const e = window;
...
async function rn(e, t, n, a, r) {
  if (!(n.length >= r || a > 8))
    for await (const o of e.values()) {
      if (n.length >= r) break;
      if (o.kind === "directory") {
        if (Va.has(o.name)) continue;
        await rn(o, `${t}/${o.name}`, n, a + 1, r);
        continue;
      }
      const s = o, l = await an(s, `${t}/${s.name}`, "local-folder");
      l && n.push(l);
    }
}
function on(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const a = Array.from(n.files ?? []), r = await Promise.all(a.map(async (o) => {
...
`).map(
    ...
    const o = a.split("|").filter(Boolean).map((l) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${l.trim()}</th>`).join(""), s = r.trim().split(`
`).map((l) => `<tr>${l.split("|").filter(Boolean).map(
      (d, f) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${f % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${d.trim()}</td>`
    ).join("")}</tr>`).join("");
...
const Ne = "atomek:default", Ya = 3e3, wt = "tytus.atomek.threadTitleOverrides", Ot = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, Oe = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, Ga = (e, t = Ya) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, zt = (e, t) => {
  const n = e.split(`
`).map((a) => a.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Qa = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", _t = (e) => ({
  id: e.id,
  title: e.title,
  kind: Qa(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = kt();
    n[e] = t, localStorage.setItem(wt, JSON.stringify(n));
  } catch {
  }
}, tr = (e) => {
  try {
    const t = kt();
    delete t[e], localStorage.setItem(wt, JSON.stringify(t));
  } catch {
  }
}, Rt = (e) => {
  const t = kt();
...
function nr({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const r = e.ai, [o, s] = C(null), [l, h] = C([]), [d, f] = C([]), [g, b] = C([]), [M, P] = C([]), [_, J] = C(Ot), [ne, ae] = C(!1), U = _e(!0), S = _e(null), F = x(async () => {
    if (!r) {
      J(Ot);
      return;
    }
    try {
      ...
      J({
        ...
        source: "none",
        label: "Tytus AI unavailable",
        reason: k instanceof Error ? k.message : String(k)
      });
    }
  }, [r]), Y = x(async (k) => {
    if (r)
      try {
        const w = await r.listArtifacts({ threadId: k });
        if (!U.current) return;
        b(w.map(_t));
      } catch (w) {
        a(`AI artifacts unavailable: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [r, a]), Re = x(async (k) => {
    if (r)
      try {
        const w = await r.listMessages(k);
        if (!U.current) return;
        const $ = l.find((L) => L.id === k) ?? null;
        $ && s($), f(w.map(Oe).filter(Boolean)), P([]), await Y(k);
      } catch (w) {
        a(`Load chat failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [r, Y, a, l]), v = x(async () => {
    if (r)
      try {
        const k = Rt(await r.listThreads({ workspaceKey: Ne, status: "active" })), w = k[0] ?? await r.createThread({ workspaceKey: Ne, title: "Atomek chat" });
        if (!U.current) return;
        h(k[0] ? k : [w]), s(w);
        const $ = await r.listMessages(w.id);
        if (!U.current) return;
        f($.map(Oe).filter(Boolean)), await Y(w.id);
      } catch (k) {
        a(`AI chat unavailable: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [r, Y, a]);
  X(() => (U.current = !0, F(), v(), () => {
    U.current = !1;
  ...
  const z = x(async () => {
    if (!r) return null;
    if (o) return o;
    const k = await r.createThread({ workspaceKey: Ne, title: "Atomek chat" });
    return U.current && (s(k), h((w) => [k, ...w.filter(($) => $.id !== k.id)])), k;
  }, [r, o]), se = x(async () => {
    if (!r) return;
    const k = await r.createThread({ workspaceKey: Ne, title: "Atomek chat" });
    s(k), h((w) => [k, ...w.filter(($) => $.id !== k.id)]), f([]), b([]), P([]), a("New AI chat created");
  }, [r, a]), ye = x(async (k, w) => {
    if (!r) return;
    const $ = w.trim();
    if ($)
      try {
        const L = r.updateThread, q = l.find((ee) => ee.id === k) ?? o, T = typeof L == "function" ? await L({ threadId: k, title: $ }) : { ...q ?? await z(), id: k, title: $, updatedAt: Date.now() };
        typeof L != "function" && er(k, $), h((ee) => ee.map((le) => le.id === T.id ? T : le)), s((ee) => ee?.id === T.id ? T : ee), a(typeof L == "function" ? `Renamed chat: ${T.title}` : `Renamed chat locally: ${T.title}`);
      } catch (L) {
        a(`Rename chat failed: ${L instanceof Error ? L.message : String(L)}`);
      }
  }, [r, z, a, o, l]), j = x(async (k) => {
    if (r)
      try {
        await r.deleteThread(k), tr(k);
        const w = l.filter(($) => $.id !== k);
        if (h(w), o?.id === k) {
          const $ = w[0] ?? await r.createThread({ workspaceKey: Ne, title: "Atomek chat" });
          s($), h((q) => q.some((T) => T.id === $.id) ? q : [$, ...q]);
          const L = await r.listMessages($.id);
          f(L.map(Oe).filter(Boolean)), await Y($.id);
        }
        a("Deleted AI chat");
      } catch (w) {
        a(`Delete chat failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [r, Y, a, o?.id, l]), ce = x(async (k) => {
    if (!r) return null;
    try {
      const w = await z();
      if (!w) return null;
      const $ = await r.createArtifact({
        threadId: w.id,
        messageId: k.messageId ?? null,
        title: k.title?.trim() || zt(k.body, "Atomek artifact"),
        kind: k.kind ?? "markdown",
        body: k.body
      }), L = _t($);
      return b((q) => [L, ...q.filter((T) => T.id !== L.id)]), a(`Saved AI artifact: ${L.title}`), L;
    } catch (w) {
      return a(`Save artifact failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [r, z, a]), Se = x(async (k) => {
    if (r)
      try {
        await r.deleteArtifact(k), b((w) => w.filter(($) => $.id !== k)), a("Deleted AI artifact");
      } catch (w) {
        a(`Delete artifact failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [r, a]), Je = x(async (k) => {
    if (!r) return null;
    try {
      const w = await r.writeMemory({
        title: k.title?.trim() || zt(k.body, "Atomek memory"),
        body: k.body,
        metadata: {
          source: "atomek",
          messageId: k.messageId ?? null
      ...
      return P(($) => [w, ...$.filter((L) => L.id !== w.id)].slice(0, 5)), a(`Remembered: ${w.title}`), w;
    } catch (w) {
      return a(`Remember failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [r, a]), Ie = x(async (k) => {
    if (!r) return [];
    const w = await r.searchMemory({ query: k, limit: 5 });
    return U.current && P(w), w;
  }, [r]), De = x(async (k, w = {}) => {
    const $ = k.trim();
    if (!$ || !r) return null;
    ae(!0);
    ...
    try {
      const T = await z();
      if (!T) return null;
      const ee = await Ie($).catch(() => []), le = Za(ee), de = w.requestContext ?? t, we = le ? [...de, le] : [...de];
      let ke = null;
      for await (const R of r.sendMessage({
        threadId: T.id,
        body: $,
        ...
        if (R.type === "message_created") {
          const O = Oe(R.message);
          if (!O) continue;
          O.role === "assistant" && (ke = O.id), f((W) => [...W.filter((G) => G.id !== O.id), O]);
        }
        if (R.type === "token" && (ke = R.messageId, f((O) => O.some((W) => W.id === R.messageId) ? O.map(
          (W) => W.id === R.messageId ? { ...W, body: R.body, status: "streaming" } : W
        ) : [...O, {
          id: R.messageId,
          role: "assistant",
          body: R.body,
          ...
          const O = Oe(R.message);
          if (!O) continue;
          O.role === "assistant" && (L = O), f((W) => W.map((G) => G.id === O.id ? O : G)), O.gatewayLabel && a(`AI answered via ${O.gatewayLabel}`);
        }
        if (R.type === "run_failed") {
          const O = q.signal.aborted;
          a(O ? "AI response stopped" : `AI failed: ${R.error}`), ke && f((W) => W.map((G) => G.id !== ke ? G : O ? { ...G, status: "complete", error: void 0, body: G.body || "Stopped by user." } : { ...G, status: "error", error: R.error, body: R.error }));
        }
      }
      const ve = Rt(await r.listThreads({ workspaceKey: Ne, status: "active" }).catch(() => []));
      return U.current && ve.length > 0 && h(ve), F(), L;
    } catch (T) {
      return T instanceof DOMException && T.name === "AbortError" ? (a("AI response stopped"), L) : (a(`AI failed: ${T instanceof Error ? T.message : String(T)}`), null);
    } finally {
      S.current === q && (S.current = null), ae(!1);
    }
  }, [r, n.gatewayPreference, n.model, z, Ie, F, t, a]), je = x(() => {
    S.current?.abort(), a("Stopping AI response…");
  }, [a]);
  return {
    aiStatus: _,
    artifacts: g,
...
const ar = (e) => `tytus-workbench:///${encodeURI(e.path)}`, rr = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, ir = ({ files: e, openEditorIds: t, activeFileId: n, versions: a, activeSelection: r }) => {
  const o = new Set(t), s = e.map((l) => {
    ...
    return {
      ...
      language: l.language,
      version: a[l.id] ?? 1,
      contentHash: rr(l.content),
      dirty: l.dirty,
      source: l.source,
      selection: h && r ? r : void 0,
      updatedAt: Date.now(),
      open: o.has(l.id),
      active: h
    };
  ...
  return {
    ...
    activeDocumentId: n,
    openDocumentIds: t.filter((l) => s.some((h) => h.id === l))
  };
}, or = "active-file", sr = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", cr = (e, t = "file") => `${t}:${e.id}`, lr = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let r = [];
  return t.scope === "active-selection" ? a?.selection ? r = [dr(a, a.selection, !0)] : a && (r = [We(a, !0, "Active file")]) : t.scope === "active-file" ? a && (r = [We(a, !0, "Active file")]) : t.scope === "open-editors" ? r = e.openDocumentIds.map((o) => e.byId.get(o)).filter(Boolean).map((o) => We(o, !0, "Open editor")) : t.scope === "selected-files" && (r = t.selectedFileIds.map((o) => e.byId.get(o)).filter(Boolean).map((o) => We(o, !1, "Selected file"))), ur(r).filter((o) => !n.has(o.id));
}, ur = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, mr = 4e3, fr = 8e3, pr = 4e3, br = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, gr = (e, t, n) => {
  const a = hr(e, n), r = [], o = new Map(t.map((s) => [s.id, s]));
  for (const s of a) {
    if (!s.fileId) continue;
    const l = o.get(s.fileId), h = e.byId.get(s.fileId);
    if (!l || !h || !s.includeBody) continue;
    const d = [
      `Path: ${l.path}`,
      `Language: ${l.language}`,
      `Version: ${h.version}`,
      ...
      s.range ? `Range: ${Dt(s.range)}` : null
    ].filter(Boolean).join(`
`), f = s.kind === "selection" && s.range ? yr(l.content, s.range) : l.content, g = s.kind === "selection" ? mr : s.label.startsWith("Open editor") ? pr : fr;
    r.push({
...
`)).join(`
`)
  }), { parts: r, attachments: a };
}, yr = (e, t) => {
  const n = e.split(`
`), a = Math.max(1, t.startLineNumber), r = Math.max(a, t.endLineNumber), o = n.slice(a - 1, r);
...
`));
}, Dt = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, jt = 75e4, wr = 2800, kr = 240, vr = 80, xr = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? jt) || e.content.length > (t.maxFileBytes ?? jt) ? "huge" : !t.includeDirty && e.dirty ? null : Mr(e.path) ? "vendor" : Pr(e) ? "binary" : null;
}, Ht = (e, t = {}) => {
  const n = Date.now(), a = [], r = [], o = [];
  for (const s of e) {
    const l = s.size ?? s.content.length, h = mt(s, t);
    if (h) {
      r.push({ fileId: s.id, name: s.name, path: s.path, language: s.language, reason: h, size: l });
      continue;
    }
    const d = ge(s.content), f = Ar(s, d, n, t);
    o.push(...f), a.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
...
}, Ar = (e, t = ge(e.content), n = Date.now(), a = {}) => {
  const r = Math.max(500, a.maxChunkChars ?? wr), o = Math.min(Math.max(0, a.chunkOverlapChars ?? kr), Math.floor(r / 2)), s = Math.max(1, a.maxChunksPerFile ?? vr), l = Sr(e.content), h = [];
  let d = 0, f = 0;
  for (; d < e.content.length && h.length < s; ) {
    const g = $r(e.content, d, Math.min(e.content.length, d + r)), b = e.content.slice(d, g).trim();
    if (b.length > 0) {
      const M = Ir(l, d, g);
      h.push({
        id: `${e.id}:${t}:${f}:${M.startLineNumber}-${M.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
  ...
  const a = Math.max(t + 1, n - 500), r = e.lastIndexOf(`
`, n);
  if (r >= a) return r + 1;
...
` && t.push(n + 1);
  ...
  const a = Bt(e, t), r = Bt(e, Math.max(t, n - 1));
  return {
    startLineNumber: a + 1,
    startColumn: t - e[a] + 1,
    endLineNumber: r + 1,
  ...
  for (; n <= a; ) {
    const r = Math.floor((n + a) / 2);
    if (e[r] <= t && (r === e.length - 1 || e[r + 1] > t)) return r;
    e[r] > t ? a = r - 1 : n = r + 1;
  }
  return 0;
}, Mr = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => xr.has(t)), Pr = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && Cr.has(t) || e.content.includes("\0")) return !0;
...
class Lr {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Ht(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Ht(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return Er(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const $e = (e = [], t = {}) => new Lr(e, t), Er = (e, t, n = {}) => {
  const a = new Map(t.map((l) => [l.id, l])), r = e.byFileId, o = [];
  for (const l of e.files) {
    const h = a.get(l.fileId);
    if (!h) {
      o.push({ fileId: l.fileId, path: l.path, status: "deleted", indexedHash: l.hash });
      continue;
    }
    if (mt(h, n)) {
      o.push({ fileId: l.fileId, path: h.path, status: "skipped-now", indexedHash: l.hash, currentHash: ge(h.content) });
      continue;
    }
    const f = ge(h.content);
    f !== l.hash ? o.push({ fileId: l.fileId, path: h.path, status: "changed", indexedHash: l.hash, currentHash: f }) : h.dirty !== l.dirty && o.push({ fileId: l.fileId, path: h.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: f });
  }
  for (const l of t)
    !r.has(l.id) && !mt(l, n) && o.push({ fileId: l.id, path: l.path, status: "new", currentHash: ge(l.content) });
  const s = e.signature !== cn(t);
  return { stale: o.length > 0, signatureChanged: s, files: o };
}, Fr = 8, Tr = 12e3, ln = (e, t, n = {}, a) => {
  const r = _r(t);
  if (r.length === 0) return [];
  const o = new Set(a?.files.map((f) => f.fileId) ?? []), s = e.chunks.filter((f) => n.includeDirty !== !1 || !f.dirty).map((f) => Rr(f, r)).filter((f) => f.score >= (n.minScore ?? 1)).sort((f, g) => g.score - f.score || f.chunk.path.localeCompare(g.chunk.path) || f.chunk.ordinal - g.chunk.ordinal), l = [], h = /* @__PURE__ */ new Set();
  let d = n.maxChars ?? Tr;
  for (const f of s) {
    if (l.length >= (n.limit ?? Fr) || d <= 0) break;
    const g = `${f.chunk.fileId}:${f.chunk.ordinal}`;
    if (h.has(g)) continue;
    h.add(g);
    const b = f.chunk.text.length > d ? `${f.chunk.text.slice(0, Math.max(0, d - 28))}
[...context clipped...]` : f.chunk.text;
    l.push(Or(f, b, o.has(f.chunk.fileId))), d -= b.length;
  }
  return l;
}, Or = (e, t = e.chunk.text, n = !1) => {
  const { chunk: a, score: r, matchedTerms: o } = e;
  return {
    id: `index-hit:${a.id}`,
    kind: "index-hit",
...
`), _r = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Rr = (e, t) => {
  const n = e.text.toLowerCase(), a = e.path.toLowerCase(), r = e.name.toLowerCase(), o = e.language.toLowerCase();
  let s = 0;
  const l = [];
  for (const h of t) {
    const d = ot(n, h), f = ot(a, h), g = ot(r, h), b = o === h ? 1 : 0;
    d + f + g + b !== 0 && (l.push(h), s += Math.min(d, 12), s += f * 4, s += g * 6, s += b * 3, (n.includes(`function ${h}`) || n.includes(`const ${h}`) || n.includes(`class ${h}`) || n.includes(`type ${h}`)) && (s += 3));
  }
  ...
  const n = e.toLowerCase(), a = t.map((h) => n.indexOf(h)).filter((h) => h >= 0).sort((h, d) => h - d)[0] ?? 0, r = Math.max(0, a - 120), o = Math.min(e.length, a + 260), s = r > 0 ? "…" : "", l = o < e.length ? "…" : "";
  return `${s}${e.slice(r, o).replace(/\s+/g, " ").trim()}${l}`;
}, jr = (e, t = {}) => {
  const { autoRefresh: n = !1, ...a } = t, r = _e(e), o = _e(null);
  o.current || (o.current = $e(e, a));
  const [s, l] = C(() => o.current?.getSnapshot() ?? $e(e, a).getSnapshot());
  r.current = e;
  const h = x((b = r.current) => {
    const M = (o.current ?? $e([], a)).refresh(b, a);
    return l(M), M;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), d = x((b = r.current) => {
    const M = (o.current ?? $e([], a)).update(b, a);
    return l(M), M;
  }, [h]);
  X(() => {
    n && h(e);
  }, [n, e, h]);
  const f = (o.current ?? $e([], a)).staleReport(e), g = x((b, M = {}) => {
    const P = o.current ?? $e([], a);
    return ln(P.getSnapshot(), b, M, P.staleReport(r.current));
  }, []);
  return { snapshot: s, staleReport: f, isStale: f.stale, refresh: h, update: d, retrieve: g };
}, Hr = "tytus.atomek.semanticVector:v1", dn = "__gateway_default__", hn = (e) => e?.trim() || dn, Br = (e) => {
  const t = hn(e);
  return t === dn ? "gateway default" : t;
}, un = (e, t, n) => [
  Hr,
  ...
  if (!mn()) return null;
  try {
    const a = un(e, t, n), r = window.localStorage.getItem(a);
    if (!r) return null;
  ...
  if (!mn()) return null;
  try {
    const t = un(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, ze = 8, Ur = 12e3, qr = 180, Kr = async (e, t, n, a, r = {}, o, s) => {
  const l = ln(t, n, { ...r, limit: Math.max(r.limit ?? ze, 16) }, o), h = e.ai;
  if (typeof h?.embedText != "function")
    return { hits: l.slice(0, r.limit ?? ze), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const d = hn(a.embeddingModel), f = await h.embedText({
      input: n,
      gatewayPreference: a.gatewayPreference,
      model: a.embeddingModel.trim() || void 0,
      signal: s
    }), g = fn(f.embedding);
    if (!g) throw new Error("host.ai.embedText returned no embedding vector");
    const b = new Map(l.map((S) => [S.chunkId, S])), M = ei(n), P = /* @__PURE__ */ new Map();
    t.chunks.filter((S) => r.includeDirty !== !1 || !S.dirty).slice(0, qr).forEach((S) => P.set(S.id, S)), l.forEach((S) => {
      const F = t.byChunkId.get(S.chunkId);
      F && P.set(F.id, F);
    });
    const _ = Array.from(P.values()), J = [];
    let ne = 0;
    for (const S of _) {
      s?.aborted;
      const F = Wr(e.appId, d, S) ?? await Jr(h, e.appId, d, S, a, s);
      if (!F) continue;
      F.updatedAt > Date.now() - 1e3 && (ne += 1);
      const Y = Qr(g, F.vector);
      if (!Number.isFinite(Y)) continue;
      const v = b.get(S.id)?.score ?? ti(S, M), z = Zr(v) * 0.42 + Math.max(0, Y) * 0.58;
      J.push({ chunk: S, keywordScore: v, vectorScore: Y, score: z, matchedTerms: M.filter((se) => S.text.toLowerCase().includes(se)) });
    }
    if (J.length === 0) return { hits: l.slice(0, r.limit ?? ze), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: ne };
    const ae = new Set(o?.files.map((S) => S.fileId) ?? []), U = J.sort((S, F) => F.score - S.score || F.keywordScore - S.keywordScore || S.chunk.path.localeCompare(F.chunk.path) || S.chunk.ordinal - F.chunk.ordinal).slice(0, Math.max(r.limit ?? ze, 1)).map((S) => Yr(S, ae.has(S.chunk.fileId)));
    return {
      hits: Gr(U, r.maxChars ?? Ur),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${Br(a.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: ne
    };
  } catch (d) {
    return {
      hits: l.slice(0, r.limit ?? ze),
      mode: "vector-fallback",
      reason: `Semantic retrieval failed (${d instanceof Error ? d.message : String(d)}) — using keyword retrieval.`,
      embeddedChunks: 0
    };
  }
}, Xr = (e) => e.map((t) => ({
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: ni(e.chunk.text, e.matchedTerms),
    score: e.score,
  ...
  for (const r of e) {
    if (n <= 0) break;
    const o = r.text.length > n ? `${r.text.slice(0, Math.max(0, n - 28))}
[...context clipped...]` : r.text;
    a.push({ ...r, text: o, charCount: o.length }), n -= o.length;
  }
  return a;
}, fn = (e) => {
  ...
  let a = 0, r = 0, o = 0;
  for (let s = 0; s < n; s += 1)
    a += e[s] * t[s], r += e[s] * e[s], o += t[s] * t[s];
  return r === 0 || o === 0 ? 0 : a / (Math.sqrt(r) * Math.sqrt(o));
}, Zr = (e) => Math.min(1, Math.max(0, e / 24)), ei = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), ti = (e, t) => {
  const n = `${e.path}
${e.name}
${e.language}
${e.text}`.toLowerCase();
  return t.reduce((a, r) => a + (n.includes(r) ? 1 : 0), 0);
}, ni = (e, t) => {
  const n = e.toLowerCase(), a = t.map((s) => n.indexOf(s)).filter((s) => s >= 0).sort((s, l) => s - l)[0] ?? 0, r = Math.max(0, a - 120), o = Math.min(e.length, a + 260);
  return `${r > 0 ? "…" : ""}${e.slice(r, o).replace(/\s+/g, " ").trim()}${o < e.length ? "…" : ""}`;
}, ai = /```([^\n`]*)\n([\s\S]*?)```/g, ri = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function vt(e) {
  return Array.from(e.matchAll(ai)).map((t) => pn(t[1] ?? "", li(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function pn(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), a = {}, r = [];
  for (const o of n) {
    const s = o.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? a[s[1].toLowerCase()] = hi(s[2]) : r.push(o.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: r, attrs: a, content: t };
}
function Z(e, t) {
  return (typeof e == "string" ? pn(e, "").flags : e.flags).includes(t.toLowerCase());
}
function bn(e) {
  return [...vt(e).filter((a) => Z(a, "diff") || Z(a, "patch")).map((a) => a.content), e].filter((a, r, o) => ii(a) && o.findIndex((s) => s === a) === r);
}
function ii(e) {
...
`), a = [];
  ...
  }), a.length === 0) return [{ raw: t, ...st(t) }];
  const r = Array.from(new Set(a)).sort((s, l) => s - l), o = [];
  for (let s = 0; s < r.length; s += 1) {
    const l = r[s], d = r.find((g) => g > l && n[g].startsWith("diff --git ")) ?? n.length, f = n.slice(l, d).join(`
`);
    /^@@\s+-\d+/m.test(f) && o.push({ raw: f, ...st(f) });
  }
  return o.length > 0 ? o : [{ raw: t, ...st(t) }];
}
function st(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), r = ie(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, o = ie(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, s = o && o !== "/dev/null" ? o : r;
  return { path: s && s !== "/dev/null" ? s : null, oldPath: r, newPath: o };
}
function ie(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function ft(e) {
  return vt(e).filter((t) => si(t)).map((t) => ({
...
function si(e) {
  return Z(e, "atomek-replace") || Z(e, "atomek-full") || Z(e, "full-replacement") || Z(e, "full") || Z(e, "replace");
}
function ci(e) {
...
`).slice(0, 5);
  for (const a of n) {
    const r = a.match(/^\s*(?:\/\/|#|<!--)?\s*(?:atomek-)?(?:path|file)\s*:\s*([^\s>]+)\s*(?:-->)?\s*$/i);
    if (r) return ie(r[1]);
  }
  return null;
}
...
`), r = [];
  ...
  for (let l = 0; l < a.length; l += 1) {
    const h = a[l].match(ri);
    if (!h) continue;
    s = !0;
    const d = Number(h[1]), f = Math.max(0, d - 1);
    if (f < o) return null;
    for (r.push(...n.slice(o, f)), o = f, l += 1; l < a.length; l += 1) {
      const g = a[l];
      if (g.startsWith("@@ ")) {
        l -= 1;
        break;
      }
      if (g.startsWith("diff --git ") || g.startsWith("--- ") || g.startsWith("+++ ") || g.startsWith("\\ No newline at end of file")) continue;
      const b = g[0], M = g.slice(1);
      if (b === " ") {
  ...
  return s ? (r.push(...n.slice(o)), r.join(`
`)) : null;
}
function li(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function ui(e) {
  const { body: t, files: n, sourceTitle: a, activeFile: r = null, versions: o = {} } = e, s = /* @__PURE__ */ new Map(), l = [];
  for (const f of bn(t))
    for (const g of oi(f)) {
      if (!g.path) {
        if (r) {
          const P = pt(r.content, g.raw);
          P && P !== r.content && s.set(r.id, Ve({
            file: r,
            proposedContent: P,
            sourceTitle: a,
      ...
      const b = Wt(n, g.path);
      if (!b) {
        l.push(`${g.path}: no opened file`);
        continue;
      }
      const M = pt(b.file.content, g.raw);
      if (!M || M === b.file.content) {
        l.push(`${g.path}: patch did not match or produced no change`);
        continue;
      }
      s.set(b.file.id, Ve({
        file: b.file,
        proposedContent: M,
        sourceTitle: a,
        extractionLabel: `workspace diff (${g.path})`,
        versions: o,
        targetPath: g.path,
        match: b
      }));
  ...
  for (const f of ft(t)) {
    const g = f.path ? Wt(n, f.path) : r ? { file: r, normalizedPatchPath: ie(r.path), confidence: "exact" } : null;
    if (!g) {
      l.push(`${f.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (f.content === g.file.content) {
      l.push(`${g.file.path}: replacement produced no change`);
      continue;
    }
    s.set(g.file.id, Ve({
      file: g.file,
      ...
      versions: o,
      targetPath: f.path ?? g.file.path,
      match: g
    }));
  }
  const h = ft(t).some((f) => f.path);
  if (s.size === 0 && r && !h) {
    const f = mi(t, r);
    f && f.content !== r.content && s.set(r.id, Ve({
      file: r,
      proposedContent: f.content,
      sourceTitle: a,
      extractionLabel: f.label,
  ...
  return {
    ...
    stats: pi(d.map((f) => f.stats)),
    kind: d.length === 0 ? "empty" : d.length === 1 ? "single-file" : "multi-file"
  };
}
function mi(e, t) {
  for (const h of bn(e)) {
    const d = pt(t.content, h);
    if (d) return { content: d, label: "unified diff patch" };
  }
  const n = vt(e);
  if (n.length === 0) return null;
  const a = gi(t), r = ft(e)[0];
  if (r) return { content: r.content, label: r.label };
  const o = n.find((h) => a.some((d) => Z(h, d)) && !Z(h, "diff") && !Z(h, "patch"));
  if (o) return { content: o.content, label: `matched ${o.lang || t.language} block` };
  const l = n.filter((h) => !Z(h, "diff") && !Z(h, "patch")).sort((h, d) => d.content.length - h.content.length)[0];
  return l ? { content: l.content, label: `largest fenced block (${l.lang || "plain"})` } : null;
}
function Wt(e, t) {
  const n = ie(t), a = n.split("/").at(-1) ?? n, r = e.find((l) => ie(l.path) === n);
  if (r) return { file: r, normalizedPatchPath: n, confidence: "exact" };
  const o = e.find((l) => ie(l.path).endsWith(`/${n}`));
  if (o) return { file: o, normalizedPatchPath: n, confidence: "suffix" };
  const s = e.find((l) => l.name === a || ie(l.path).endsWith(`/${a}`));
  return s ? { file: s, normalizedPatchPath: n, confidence: "basename" } : null;
}
function fi(e, t) {
  const n = e.split(`
`), a = t.split(`
`), r = Math.max(n.length, a.length);
  let o = 0, s = 0, l = 0;
  for (let h = 0; h < r; h += 1)
    n[h] !== a[h] && (n[h] === void 0 ? o += 1 : a[h] === void 0 ? s += 1 : l += 1);
...
const Xe = (e) => typeof e == "object" && e !== null, yi = (e) => Xe(e.ai) ? e.ai : null, wi = (e) => {
  if (!Xe(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, ki = (e) => {
  const t = wi(e);
  return !t || !Xe(e) ? null : { ...e, id: t };
}, re = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(re) : Xe(e) ? Object.values(e).some(re) : !1, vi = async (e, t) => {
  const n = yi(e);
  if (typeof n?.listModels != "function") return [];
  const a = await n.listModels(t);
  return Array.isArray(a) ? a.map(ki).filter((r) => !!r) : [];
}, xi = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : re(e.capability) || re(e.capabilities) || re(e.modality) || re(e.modalities) || re(e.task) || re(e.tasks) || re(e.type) || re(e.kind), yn = (e) => typeof e == "object" && e !== null, Ci = (e) => yn(e.ai) ? e.ai : null, wn = (e) => typeof Ci(e)?.embedText == "function", Ai = async (e, t) => wn(e) ? (await vi(e, t)).filter(xi) : [], Ni = (e) => yn(e.ai) ? wn(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", $i = Gn(() => import("./WorkbenchMonacoEditor-DbnEHW95.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Si = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, kn = "tytus.workspace.recent", vn = "tytus.workspace.layout", xn = "tytus.atomek.chatAiSettings", Ue = {
  gatewayPreference: "auto",
  model: "",
  embeddingModel: ""
};
function Ii({ host: e }) {
  const t = K(() => to(), []), [n, a] = C("explorer"), [r, o] = C(t.primaryVisible), [s, l] = C(t.primaryWidth), [h, d] = C("chat"), [f, g] = C(t.secondaryVisible), [b, M] = C(t.secondaryWidth), [P, _] = C(!1), [J, ne] = C("problems"), [ae, U] = C(t.markdownPreviewVisible), [S, F] = C(!1), [Y, Re] = C(null), [v, z] = C([]), [se, ye] = C([]), [j, ce] = C(null), [Se, Je] = C(""), [Ie, De] = C({ lineNumber: 1, column: 1 }), [je, k] = C(null), [w, $] = C({}), [L, q] = C(or), [T, ee] = C([]), [le, de] = C([]), [we, ke] = C(null), [ve, R] = C(""), [O, W] = C(!1), [G, $n] = C(""), [Ct, At] = C([]), [H, xe] = C(null), [he, Me] = C(null), [Sn, Ye] = C(!1), [fe, In] = C(() => no()), [Nt, Pe] = C(null), [Mn, $t] = C(null), [Pn, N] = C("Ready"), [Le, Ln] = C(() => eo()), Q = se.map((c) => v.find((m) => m.id === c)).filter(Boolean), A = j ? v.find((c) => c.id === j) ?? null : null, St = K(() => ir({ files: v, openEditorIds: se, activeFileId: j, versions: w, activeSelection: je }), [j, je, w, v, se]), It = K(() => ({
    scope: L,
    removedAttachmentIds: T,
    selectedFileIds: []
  }), [L, T]), Ee = K(() => gr(St, v, It), [It, St, v]), ue = jr(v, { autoRefresh: !0, includeDirty: !0 }), Mt = K(() => le.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [le]), En = K(() => [...Ee.attachments, ...Mt], [Ee.attachments, Mt]), I = nr({ host: e, requestContext: Ee.parts, chatSettings: fe, setStatus: N }), pe = K(
    () => [...I.artifacts, ...Ct].sort((c, m) => m.createdAt - c.createdAt),
    [I.artifacts, Ct]
  ), Ge = !A && !S, V = K(() => v.filter((c) => c.dirty), [v]), Fn = K(() => {
    const c = Se.trim().toLowerCase();
    return c ? v.filter((m) => m.path.toLowerCase().includes(c)) : v;
  }, [v, Se]);
  X(() => {
    $((c) => {
      const m = {};
      for (const p of v) m[p.id] = c[p.id] ?? 1;
      return m;
    });
  }, [v]), X(() => {
    ee([]), de([]);
  ...
  const Tn = x((c) => {
    if (c.kind === "index-hit") {
      de((m) => m.filter((p) => p.id !== c.id)), N(`Removed project context: ${c.label}`);
      return;
    }
    ee((m) => m.includes(c.id) ? m : [...m, c.id]), N(`Removed chat context: ${c.label}`);
  }, []), Ce = x((c) => {
    $((m) => ({ ...m, [c]: (m[c] ?? 1) + 1 }));
  }, []), On = x((c) => {
    c.preventDefault();
    const m = c.clientX, p = b, y = (Ae) => {
      const Te = p + (m - Ae.clientX);
      M(Math.max(380, Math.min(760, Te)));
    }, D = () => {
      window.removeEventListener("pointermove", y), window.removeEventListener("pointerup", D);
    };
    window.addEventListener("pointermove", y), window.addEventListener("pointerup", D);
  }, [b]), zn = x((c) => {
    c.preventDefault();
    const m = c.clientX, p = s, y = (Ae) => {
      const Te = p + (Ae.clientX - m);
      l(Math.max(240, Math.min(460, Te)));
    }, D = () => {
      window.removeEventListener("pointermove", y), window.removeEventListener("pointerup", D);
    };
    window.addEventListener("pointermove", y), window.addEventListener("pointerup", D);
  }, [s]), He = x((c) => {
    const m = [c, ...Le.filter((p) => p.path !== c.path)].slice(0, 6);
    Ln(m), localStorage.setItem(kn, JSON.stringify(m));
  }, [Le]), B = x((c, m) => {
    ye((p) => p.includes(c.id) ? p : [...p, c.id]), ce(c.id), F(!1), $t(m ?? null), De({ lineNumber: m ?? 1, column: 1 });
  }, []), _n = x((c) => {
    if (!c.fileId) return;
    const m = v.find((p) => p.id === c.fileId);
    m && (B(m, c.range?.startLineNumber ?? 1), N(`Revealed context: ${c.label}`));
  }, [v, B]), Fe = x(async () => {
    if (ct(V, "open new files"))
      try {
        const c = await qa();
        if (c.length === 0) return;
        z((m) => Gi(m, c)), c.forEach((m) => He({ name: m.name, path: m.path, at: Date.now() })), B(c[0]), N(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && N(`Open file failed: ${c.message}`);
      }
  }, [V, B, He]), Qe = x(async () => {
    if (ct(V, "open another folder"))
      try {
        const c = await Ka();
        Re(c), z(c.files), ye([]), ce(null), F(!1), He({ name: c.name, path: c.name, at: Date.now() }), N(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!j) return;
    let m = !1;
    z((p) => p.map((y) => y.id !== j || y.content === c ? y : (m = !0, { ...y, content: c, dirty: !0 }))), m && Ce(j);
  }, [j, Ce]), Ze = x(async () => {
    if (A)
      try {
        const c = await it(A);
        z((m) => m.map((p) => p.id === c.id ? c : p)), N(`Saved ${c.name}`);
      } catch (c) {
        N(`Save failed: ${c.message}`);
      }
  }, [A]), Dn = x(async (c) => {
    const m = v.find((y) => y.id === c);
    if (!m) return null;
    const p = await it(m);
    return z((y) => y.map((D) => D.id === p.id ? p : D)), p;
  }, [v]), Pt = x(async () => {
    const c = v.filter((m) => m.dirty);
    if (c.length === 0) {
      N("No dirty files to save");
      return;
    }
    try {
      const m = await Promise.all(c.map((y) => it(y))), p = new Map(m.map((y) => [y.id, y]));
      z((y) => y.map((D) => p.get(D.id) ?? D)), Pe(null), N(`Saved ${m.length} dirty file${m.length === 1 ? "" : "s"}`);
    } catch (m) {
      N(`Save all failed: ${m.message}`);
    }
  }, [v]), et = x((c) => {
    const m = v.find((p) => p.id === c);
    if (m?.dirty && !window.confirm(`${m.name} has unsaved changes. Close without saving?`)) {
      N(`Close canceled — ${m.name} has unsaved changes`);
      return;
    }
    ye((p) => {
      const y = p.filter((D) => D !== c);
      return j === c && (ce(y.at(-1) ?? null), k(null)), y;
    });
  }, [j, v]), jn = x(() => {
    ct(V, "close all editors") && (ye([]), ce(null), $t(null), F(!1), N("Closed all editors"));
  }, [V]), tt = x(() => {
    const c = v.filter((p) => p.name.startsWith("Untitled")).length + 1, m = {
...
`,
    ...
    return de(y), y.length === 0 ? (N(ue.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), m) : (p.mode !== "hybrid" && p.reason ? N(p.reason) : N(`${y.length} project context hit${y.length === 1 ? "" : "s"} · ${p.reason ?? "hybrid retrieval"}`), [
      ...m,
      ...Xr(y)
    ]);
  }, [Ee.parts, L, fe, e, ue]), nt = x(() => {
    if (!A && Q.length === 0) {
      N("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const m = [
      `Create a polished Markdown artifact from ${A?.path ?? `${Q.length} open editors`}.`,
    ...
    g(!0), d("chat"), N("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const p = await me(m), y = await I.askAgent(m, { requestContext: p });
      !y || y.status === "error" || I.createArtifact({
        messageId: y.id,
        title: `AI synthesis — ${A?.name ?? "open workspace"}`,
        kind: "markdown",
        body: y.body
      }).then(() => {
        g(!0), d("outputs"), _(!0), ne("output");
      });
    })();
  }, [A, I, me, Q.length]), at = x((c) => {
    const m = v.find((p) => p.path === c.path || p.name === c.name);
    if (m) {
      B(m), N(`Opened recent ${m.name}`);
      return;
    }
    ...
    g(!0), d("chat"), R(""), (async () => {
      const m = await me(c);
      await I.askAgent(c, { requestContext: m });
    })();
  }, [I, me]), Hn = x((c) => {
    const m = I.messages.findIndex((y) => y.id === c.id), p = I.messages.slice(0, m < 0 ? void 0 : m).reverse().find((y) => y.role === "user");
    if (!p?.body.trim()) {
      N("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const y = await me(p.body);
      await I.askAgent(p.body, { requestContext: y });
    })();
  }, [I, me]), Bn = x((c) => {
    I.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    I.createArtifact({
      title: A.path,
      kind: A.language === "markdown" ? "markdown" : "report",
      body: A.content
    }).then(() => {
      g(!0), d("outputs"), _(!0), ne("output");
    });
  }, [A, I]), rt = x((c) => {
    const m = dt(v, lt(c.title || c.kind)), p = {
    ...
    z((y) => [...y, p]), B(p), N(`Opened ${c.title} as editable file`);
  }, [v, B]), Be = x((c, m) => {
    const p = ui({
      body: m,
      files: v,
      ...
      edits: p.edits.map(Qi),
      skipped: p.skipped
    };
    return p.edits.length > 1 ? (Me(y), xe(null), N(`Previewing AI workspace patch for ${p.edits.length} files`), !0) : p.edits.length === 1 ? (xe(y.edits[0]), Me(null), N(`Previewing AI patch for ${y.edits[0].fileName}`), !0) : (N(p.skipped.length > 0 ? `No applicable edit found. ${p.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [A, w, v]), Un = x(() => {
    const c = ve.trim();
    if (!c) return;
    R("");
    const m = Vi(c);
    (async () => {
      const p = await me(c), y = await I.askAgent(c, { requestContext: p });
      if (!m || !y || y.status === "error") return;
      Be(y.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", y.body) || N("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    if (!H) return;
    const c = v.find((m) => m.id === H.fileId);
    if (!c) {
      N(`Cannot apply edit — ${H.fileName} is no longer open`), xe(null);
      return;
    }
    c.content !== H.originalContent && !window.confirm(`${H.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (z((m) => m.map((p) => p.id === H.fileId ? { ...p, content: H.proposedContent, dirty: !0 } : p)), Ce(H.fileId), Pe(`AI edit applied to ${c.name}. Save All to persist it to disk.`), xe(null), N(`Applied AI edit to ${c.name} — unsaved`));
  }, [Ce, v, H]), Xn = x(() => {
    if (!H) return;
    const c = dt(v, `${lt(H.fileName)}-ai-edit`), m = {
      id: `pending-edit-${H.fileId}-${Date.now()}`,
      name: c,
      path: c,
      language: A?.language ?? "markdown",
      content: H.proposedContent,
      dirty: !0,
      source: "generated"
    };
    z((p) => [...p, m]), B(m), xe(null), N(`Opened proposed edit as ${c}`);
  }, [A?.language, v, B, H]), Jn = x(() => {
    if (!he) return;
    const c = new Map(he.edits.map((p) => [p.fileId, p]));
    v.some((p) => {
      const y = c.get(p.id);
      return y && p.content !== y.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (z((p) => p.map((y) => {
      const D = c.get(y.id);
      return D ? { ...y, content: D.proposedContent, dirty: !0 } : y;
    })), c.forEach((p, y) => Ce(y)), Pe(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), Me(null), N(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved`));
  }, [Ce, v, he]), Yn = x(() => {
    if (!he) return;
    const c = [...v], m = he.edits.map((p) => {
      const y = dt(c, `${lt(p.fileName)}-ai-edit`), D = v.find((Te) => Te.id === p.fileId), Ae = {
        id: `workspace-patch-${p.fileId}-${Date.now()}-${y}`,
        name: y,
        path: y,
    ...
    z((p) => [...p, ...m]), m[0] && B(m[0]), Me(null), N(`Opened ${m.length} proposed edit file${m.length === 1 ? "" : "s"}`);
  }, [v, B, he]), Lt = x((c) => {
    if (c === "explain") {
  ...
  return X(() => {
    const c = (m) => {
      (m.metaKey || m.ctrlKey) && (m.key.toLowerCase() === "o" && (m.preventDefault(), Fe()), m.key.toLowerCase() === "s" && (m.preventDefault(), Ze()), m.key.toLowerCase() === "w" && (m.preventDefault(), j && et(j)), m.key.toLowerCase() === "b" && (m.preventDefault(), o((y) => !y)), m.shiftKey && m.key.toLowerCase() === "f" && (m.preventDefault(), a("search"), o(!0)), (m.key.toLowerCase() === "k" || m.key.toLowerCase() === "p") && (m.preventDefault(), W(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [j, et, Fe, Ze]), X(() => {
    const c = (m) => {
      V.length !== 0 && (m.preventDefault(), m.returnValue = "");
    };
    return window.addEventListener("beforeunload", c), () => window.removeEventListener("beforeunload", c);
  }, [V.length]), X(() => {
    V.length === 0 && Pe(null);
  }, [V.length]), X(() => {
    const c = { primaryVisible: r, primaryWidth: s, secondaryVisible: f, secondaryWidth: b, markdownPreviewVisible: ae };
    localStorage.setItem(vn, JSON.stringify(c));
  }, [ae, r, s, f, b]), X(() => {
    localStorage.setItem(xn, JSON.stringify(fe));
  }, [fe]), /* @__PURE__ */ u(
    "div",
    {
      className: `workbench-workbench ${r ? "" : "no-primary"} ${f ? "" : "no-secondary"} ${P ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${s}px`, "--workbench-secondary-width": `${b}px` },
      children: [
        /* @__PURE__ */ i(Mi, { active: n, setActive: (c) => {
          a(c), o(!0);
        }, openSettings: () => Ye(!0) }),
        r && /* @__PURE__ */ u("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ i(
            Pi,
            {
              activity: n,
          ...
          /* @__PURE__ */ i("div", { className: "workbench-primary-resizer", onPointerDown: zn, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ u("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ i("button", { className: "workbench-command-center", onClick: () => W(!0), children: "Workspace" }),
          /* @__PURE__ */ u("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ i(
              Ti,
              {
                ...
                closeWelcome: () => F(!0),
                secondaryVisible: f,
                toggleSecondary: () => g((c) => !c),
                canPreview: A?.language === "markdown",
                previewVisible: ae,
                togglePreview: () => U((c) => !c)
              }
            ),
            /* @__PURE__ */ i(Fi, { file: A, folder: Y, showWelcome: Ge }),
            Nt && V.length > 0 ? /* @__PURE__ */ u("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ i("span", { children: Nt }),
              /* @__PURE__ */ i("button", { onClick: () => {
                Pt();
              }, children: "Save all" }),
              /* @__PURE__ */ i("button", { onClick: () => Pe(null), title: "Dismiss", children: /* @__PURE__ */ i(oe, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ i("div", { className: "workbench-editor-content", children: A ? /* @__PURE__ */ u("div", { className: A.language === "markdown" && ae ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ i("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ i(Qn, { fallback: /* @__PURE__ */ i("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ i(
                $i,
                {
                  file: A,
                  revealLine: Mn,
              ...
              A.language === "markdown" && ae && /* @__PURE__ */ i(Di, { content: A.content })
            ] }) : Ge ? /* @__PURE__ */ i(Oi, { openFile: Fe, openFolder: Qe, newFile: tt, recent: Le, reopenRecent: at, setStatus: N }) : /* @__PURE__ */ u("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ i(Ke, { size: 34 }),
              /* @__PURE__ */ i("p", { children: "No editor open" }),
              /* @__PURE__ */ i("button", { className: "workbench-button-subtle", onClick: () => F(!1), children: "Show Welcome" })
            ] }) }),
            P && /* @__PURE__ */ i(
              ji,
              {
        ...
        f && /* @__PURE__ */ i(
          Hi,
          {
            ...
            chatThread: I.thread,
            chatThreads: I.threads,
            askAgent: Un,
            stopChat: I.stopChat,
            regenerateMessage: Hn,
            newChat: () => {
              I.newChat();
            },
            selectThread: (c) => {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
        ...
        O && /* @__PURE__ */ i(
          Ri,
          {
            ...
            activeFile: A,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: tt },
              ...
              ...Le.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => at(c) })),
              { label: "File: Save All", detail: `${V.length} dirty file${V.length === 1 ? "" : "s"}`, run: () => {
                Pt();
              }, disabled: V.length === 0 },
              { label: "File: Close All Editors", detail: `${Q.length} open editor${Q.length === 1 ? "" : "s"}`, run: jn, disabled: Q.length === 0 },
              ...
              { label: "View: Toggle Primary Side Bar", detail: r ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => o((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: f ? "Hide right AI side bar" : "Show right AI side bar", run: () => g((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: P ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => _((c) => !c) },
              { label: "View: Toggle Markdown Preview", detail: A?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => U((c) => !c), disabled: A?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: A || Q.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: nt, disabled: !A && Q.length === 0 },
              { label: "AI: Explain Active File", detail: A ? `Ask Cortex to explain ${A.path}` : "Open a file first", run: () => te("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !A },
              { label: "AI: Improve Active File", detail: A ? `Ask Cortex for concrete edits to ${A.path}` : "Open a file first", run: () => te("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !A },
              { label: "AI: Draft Editable Replacement", detail: A ? `Ask Cortex for a full-file replacement for ${A.path}` : "Open a file first", run: () => Lt("edit"), disabled: !A },
              { label: "AI: Plan Workspace Work", detail: Q.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => te("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: Q.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: A ? "Persist active file in host.ai artifacts" : "Open a file first", run: Vn, disabled: !A },
              { label: "AI: Open Latest Artifact as File", detail: pe[0] ? `Create editable file from ${pe[0].title}` : "No outputs yet", run: () => pe[0] && rt(pe[0]), disabled: pe.length === 0 }
            ],
            openWorkbenchFile: B,
            onClose: () => W(!1)
          }
        ...
        /* @__PURE__ */ i(Yi, { status: Pn, file: A ?? Si, cursor: Ie, fileCount: v.length, dirtyCount: V.length })
      ]
    }
  );
}
function Mi({ active: e, setActive: t, openSettings: n }) {
  return /* @__PURE__ */ u("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(Zt, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(Fa, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(Ca, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(ca, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(Jt, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ i("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(ha, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ i(be, { icon: /* @__PURE__ */ i(_a, { size: 23 }), label: "Manage", active: !1, onClick: n })
  ] });
}
function be({ icon: e, label: t, active: n, onClick: a }) {
  return /* @__PURE__ */ i("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: a, children: e });
}
function Pi(e) {
  return e.activity === "search" ? /* @__PURE__ */ i(zi, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ i(qt, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ i(qt, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ i(Ji, {}) : /* @__PURE__ */ i(Li, { ...e });
}
function Li(e) {
  const t = !e.folder, n = K(() => Ei(e.files, e.folder?.name), [e.files, e.folder?.name]);
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ i("div", { className: "workbench-sidebar-title", children: "EXPLORER" }),
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ u(qe, { children: [
        /* @__PURE__ */ i("p", { style: { fontWeight: 600, margin: "10px 0" }, children: "NO FOLDER OPENED" }),
        /* @__PURE__ */ i("p", { className: "workbench-muted", children: "You have not yet opened a folder." }),
        /* @__PURE__ */ i("button", { className: "workbench-button-blue", onClick: e.openFolder, children: "Open Folder" }),
        /* @__PURE__ */ i("button", { className: "workbench-button-blue", onClick: e.openFile, children: "Open File" }),
        /* @__PURE__ */ i("button", { className: "workbench-button-blue", onClick: () => e.recent[0] ? e.reopenRecent(e.recent[0]) : e.setStatus("No recent local workspace yet."), children: "Open Recent" }),
        /* @__PURE__ */ i("p", { className: "workbench-muted", children: e.hasFsAccess ? "Local files use browser-native File System Access API." : "Browser fallback may show a file chooser label." })
        ...
        /* @__PURE__ */ u("div", { className: "workbench-sidebar-actions", children: [
          /* @__PURE__ */ u("button", { className: "workbench-button-subtle", onClick: e.openFile, children: [
            /* @__PURE__ */ i(Qt, { size: 14 }),
            "Open File"
          ] }),
          /* @__PURE__ */ u("button", { className: "workbench-button-subtle", onClick: e.openFolder, children: [
            /* @__PURE__ */ i(bt, { size: 14 }),
            "Open Folder"
          ] })
        ] }),
        /* @__PURE__ */ i("input", { className: "workbench-input", value: e.query, onChange: (a) => e.setQuery(a.target.value), placeholder: "Search files" }),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ i(ut, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((a) => /* @__PURE__ */ i(xt, { file: a, active: a.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(a) }, a.id)),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ i(ut, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No readable text files found." }) : Cn(n, e.activeFileId, e.openWorkbenchFile)
      ] }),
      /* @__PURE__ */ i("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((a) => /* @__PURE__ */ u("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(a), children: [
        /* @__PURE__ */ i(va, { size: 14 }),
        /* @__PURE__ */ i("span", { className: "workbench-row-name", children: a.name })
      ] }, `${a.path}-${a.at}`))
    ] })
  ] });
}
function xt({ file: e, active: t, onOpen: n, basePath: a, depth: r = 0, label: o }) {
  const s = a && e.path.startsWith(`${a}/`) ? e.path.slice(a.length + 1) : e.path, l = r || Math.max(0, s.split("/").length - 1);
  return /* @__PURE__ */ u("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": l }, onClick: n, title: e.path, children: [
    /* @__PURE__ */ i(Gt, { size: 14 }),
    /* @__PURE__ */ i("span", { className: "workbench-row-name", children: o ?? s }),
    e.dirty && /* @__PURE__ */ i("span", { className: "workbench-row-meta", children: "●" })
  ] });
}
function Ei(e, t) {
  const n = [], a = /* @__PURE__ */ new Map(), r = (s, l, h = n) => {
    const d = a.get(s);
    if (d) return d;
    const f = { name: l, path: s, children: [] };
    return a.set(s, f), h.push(f), f;
  };
  e.forEach((s) => {
    const l = t && s.path.startsWith(`${t}/`) ? s.path.slice(t.length + 1) : s.path, h = l.split("/").filter(Boolean);
    let d = n, f = "";
    h.slice(0, -1).forEach((g) => {
      f = f ? `${f}/${g}` : g, d = r(f, g, d).children;
    }), d.push({ name: h.at(-1) ?? s.name, path: l, file: s, children: [] });
  });
  const o = (s) => s.sort((l, h) => +!!l.file - +!!h.file || l.name.localeCompare(h.name)).map((l) => ({ ...l, children: o(l.children) }));
  return o(n);
}
function Cn(e, t, n, a = 0) {
  return e.map((r) => r.file ? /* @__PURE__ */ i(xt, { file: r.file, active: r.file.id === t, onOpen: () => n(r.file), depth: a, label: r.name }, r.file.id) : /* @__PURE__ */ u("div", { children: [
    /* @__PURE__ */ u("div", { className: "workbench-folder-row", style: { "--workbench-depth": a }, children: [
      /* @__PURE__ */ i(ut, { size: 12 }),
      /* @__PURE__ */ i(bt, { size: 14 }),
      /* @__PURE__ */ i("span", { className: "workbench-row-name", children: r.name })
    ] }),
    Cn(r.children, t, n, a + 1)
  ] }, r.path));
}
function Fi({ file: e, folder: t, showWelcome: n }) {
  const a = n ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], r = t && a[0] === t.name ? a.slice(1) : a;
  return /* @__PURE__ */ i("div", { className: "workbench-breadcrumb", children: r.length === 0 ? /* @__PURE__ */ i("span", { children: "Workspace" }) : r.map((o, s) => /* @__PURE__ */ u("span", { className: "workbench-breadcrumb-part", children: [
    s > 0 && /* @__PURE__ */ i("span", { className: "workbench-breadcrumb-sep", children: "›" }),
    o
  ] }, `${o}-${s}`)) });
}
function Ti(e) {
  return /* @__PURE__ */ u("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ u("button", { className: "workbench-tab active", children: [
      ...
      /* @__PURE__ */ i("span", { className: "workbench-tab-name", children: "Welcome" }),
      /* @__PURE__ */ i("span", { className: "workbench-tab-close", role: "button", tabIndex: 0, onClick: (t) => {
        t.stopPropagation(), e.closeWelcome();
      }, children: /* @__PURE__ */ i(oe, { size: 13 }) })
    ] }),
    e.openEditors.map((t) => /* @__PURE__ */ u("button", { className: `workbench-tab ${t.id === e.activeFileId ? "active" : ""}`, onClick: () => e.setActiveFileId(t.id), title: t.path, children: [
      /* @__PURE__ */ i(Gt, { size: 15 }),
      /* @__PURE__ */ u("span", { className: "workbench-tab-name", children: [
        t.dirty && /* @__PURE__ */ i("span", { className: "workbench-dirty-dot", children: "●" }),
        t.name
      ] }),
      t.dirty && /* @__PURE__ */ i("span", { className: "workbench-tab-save", role: "button", tabIndex: 0, title: "Save", onClick: (n) => {
        n.stopPropagation(), e.saveFile(t.id);
      }, children: "Save" }),
      /* @__PURE__ */ i("span", { className: "workbench-tab-close", role: "button", tabIndex: 0, onClick: (n) => {
        n.stopPropagation(), e.closeEditor(t.id);
      }, children: /* @__PURE__ */ i(oe, { size: 13 }) })
    ] }, t.id)),
    /* @__PURE__ */ i("div", { style: { flex: 1 } }),
    e.canPreview && /* @__PURE__ */ i("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ i(Yt, { size: 16 }) }),
    /* @__PURE__ */ i("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ i(Sa, { size: 16 }) })
  ] });
}
function Oi({ openFile: e, openFolder: t, newFile: n, recent: a, reopenRecent: r, setStatus: o }) {
  return /* @__PURE__ */ u("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ u("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ u("section", { children: [
        /* @__PURE__ */ i("h1", { children: "Atomek" }),
        /* @__PURE__ */ i("div", { className: "workbench-welcome-subtitle", children: "Shape files into working artifacts" }),
        /* @__PURE__ */ i("h2", { children: "Start" }),
        /* @__PURE__ */ u("button", { className: "workbench-start-link", onClick: n, children: [
          /* @__PURE__ */ i(Qt, { size: 18 }),
          "New File..."
        ] }),
        /* @__PURE__ */ u("button", { className: "workbench-start-link", onClick: e, children: [
          /* @__PURE__ */ i(Zt, { size: 18 }),
          "Open File..."
        ] }),
        /* @__PURE__ */ u("button", { className: "workbench-start-link", onClick: t, children: [
          /* @__PURE__ */ i(bt, { size: 18 }),
          "Open Folder..."
        ] }),
        /* @__PURE__ */ i("h2", { children: "Recent" }),
        a.length === 0 ? /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : a.map((s) => /* @__PURE__ */ u("button", { className: "workbench-start-link", onClick: () => r(s), children: [
          s.name,
          /* @__PURE__ */ i("span", { className: "workbench-muted", children: "~" })
        ] }, `${s.path}-${s.at}`))
      ] }),
      /* @__PURE__ */ u("section", { children: [
        /* @__PURE__ */ i("h2", { children: "Walkthroughs" }),
        /* @__PURE__ */ u("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ i("strong", { children: "Get Started with Atomek" }),
          /* @__PURE__ */ i("span", { className: "workbench-muted", children: "Open local files, edit with Monaco, then wire Tytus agents and Cortex next." })
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ i("strong", { children: "Browse & Edit Local Workspaces" }),
          /* @__PURE__ */ i("span", { className: "workbench-muted", children: "Uses browser-native File System Access API on supported Chromium builds." })
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ i("strong", { children: "Learn the Fundamentals" }),
          /* @__PURE__ */ i("span", { className: "workbench-muted", children: "Explorer, tabs, editor, status bar, chat surface." })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ u("label", { className: "workbench-welcome-checkbox", children: [
      /* @__PURE__ */ i("input", { type: "checkbox", defaultChecked: !0 }),
      " Show welcome page on startup"
    ] })
  ] });
}
function zi({ files: e, query: t, setQuery: n, openWorkbenchFile: a, activeFileId: r }) {
  const o = K(() => _i(e, t), [e, t]), s = K(() => {
    const l = /* @__PURE__ */ new Map();
    return o.forEach((h) => l.set(h.file.id, [...l.get(h.file.id) ?? [], h])), Array.from(l.values()).slice(0, 50);
  }, [o]);
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ i("div", { className: "workbench-sidebar-title", children: "SEARCH" }),
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ i("input", { className: "workbench-input", value: t, onChange: (l) => n(l.target.value), placeholder: "Search files and text", autoFocus: !0 }),
      /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
        /* @__PURE__ */ i(Ke, { size: 12 }),
        " Results"
      ] }),
      t.trim() ? s.length === 0 ? /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No matches." }) : s.map((l) => {
        const h = l[0].file;
        return /* @__PURE__ */ u("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ i(xt, { file: h, active: h.id === r, onOpen: () => a(h) }),
          l.slice(0, 5).map((d) => /* @__PURE__ */ u("button", { className: "workbench-search-hit", onClick: () => a(h, d.lineNumber), title: d.line, children: [
            /* @__PURE__ */ i("span", { className: "workbench-search-line", children: d.lineNumber }),
            /* @__PURE__ */ i("span", { children: d.line })
          ] }, `${h.id}-${d.lineNumber}-${d.line}`)),
      ...
      }) : /* @__PURE__ */ i("p", { className: "workbench-muted", children: "Type to search filenames and text in the opened workspace." })
    ] })
  ] });
}
function _i(e, t) {
  ...
  const a = [];
  return e.forEach((r) => {
    r.path.toLowerCase().includes(n) && a.push({ file: r, lineNumber: 1, line: r.path }), r.content.split(`
`).some((o, s) => o.toLowerCase().includes(n) ? (a.push({ file: r, lineNumber: s + 1, line: o.trim() || "(blank line)" }), a.length >= 200) : !1);
  }), a.slice(0, 200);
}
function Ri(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((r) => ({
    label: r.path,
    detail: `Open ${tn(r.language)} file`,
    run: () => e.openWorkbenchFile(r)
  })), a = [...e.commands, ...n].filter((r) => !t || r.label.toLowerCase().includes(t) || r.detail.toLowerCase().includes(t)).slice(0, 12);
  return X(() => {
    const r = (o) => {
      o.key === "Escape" && e.onClose();
    };
    return window.addEventListener("keydown", r), () => window.removeEventListener("keydown", r);
  }, [e.onClose]), /* @__PURE__ */ i("div", { className: "workbench-command-overlay", role: "dialog", "aria-label": "Command Palette", children: /* @__PURE__ */ u("div", { className: "workbench-command-palette", children: [
    /* @__PURE__ */ i(
      "input",
      {
        ...
        onKeyDown: (r) => {
          r.key === "Enter" && a[0] && !a[0].disabled && (a[0].run(), e.onClose());
        }
      }
    ),
    /* @__PURE__ */ i("div", { className: "workbench-command-list", children: a.map((r) => /* @__PURE__ */ u(
      "button",
      {
        disabled: r.disabled,
        ...
        children: [
          ...
          /* @__PURE__ */ i("small", { children: r.detail })
        ]
      },
      `${r.label}-${r.detail}`
    )) })
...
function Di({ content: e }) {
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-markdown-preview", children: [
    /* @__PURE__ */ u("div", { className: "workbench-preview-title", children: [
      /* @__PURE__ */ i(Yt, { size: 13 }),
      " Preview"
    ] }),
    /* @__PURE__ */ i("div", { className: "workbench-preview-body", dangerouslySetInnerHTML: { __html: t } })
  ] });
}
function ji(e) {
  return /* @__PURE__ */ u("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ u("div", { className: "workbench-bottom-tabs", children: [
      /* @__PURE__ */ i("button", { className: e.tab === "problems" ? "active" : "", onClick: () => e.setTab("problems"), children: "PROBLEMS" }),
      /* @__PURE__ */ i("button", { className: e.tab === "output" ? "active" : "", onClick: () => e.setTab("output"), children: "OUTPUT" }),
      /* @__PURE__ */ i("button", { className: e.tab === "terminal" ? "active" : "", onClick: () => e.setTab("terminal"), children: "TERMINAL" }),
      /* @__PURE__ */ i("span", {}),
      /* @__PURE__ */ i("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ i(oe, { size: 14 }) })
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-bottom-body", children: [
      e.tab === "problems" && /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No problems detected in open files. Diagnostics wire in after the base shell is approved." }),
      e.tab === "terminal" && /* @__PURE__ */ u("div", { children: [
        /* @__PURE__ */ i("p", { className: "workbench-muted", children: "Terminal is parked. Native command execution belongs to the pod/daemon integration sprint." }),
        /* @__PURE__ */ i("pre", { className: "workbench-terminal-placeholder", children: "$ atomek --local-first" })
      ] }),
      e.tab === "output" && /* @__PURE__ */ i(Nn, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Hi(e) {
  return /* @__PURE__ */ u("aside", { className: "workbench-secondary", children: [
    /* @__PURE__ */ i("div", { className: "workbench-secondary-resizer", onPointerDown: e.onResizeStart, title: "Resize Chat" }),
    /* @__PURE__ */ u("div", { className: "workbench-secondary-tabs", children: [
      /* @__PURE__ */ u("div", { className: "workbench-secondary-tab-group", children: [
        /* @__PURE__ */ i("button", { className: `workbench-secondary-tab ${e.tab === "chat" ? "active" : ""}`, onClick: () => e.setTab("chat"), children: "CHAT" }),
        /* @__PURE__ */ i("button", { className: `workbench-secondary-tab ${e.tab === "outputs" ? "active" : ""}`, onClick: () => e.setTab("outputs"), children: "OUTPUTS" })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ i("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ i(Pa, { size: 15 }) }),
        /* @__PURE__ */ i("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ i(ma, { size: 16 }) }),
        /* @__PURE__ */ i("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ i(oe, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ i(Bi, { ...e }) : /* @__PURE__ */ i(Nn, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function Bi(e) {
  const t = _e(null), [n, a] = C(!0), [r, o] = C(!1), s = K(() => e.chatMessages.map((d) => `${d.id}:${d.status ?? ""}:${d.body.length}`).join("|"), [e.chatMessages]);
  X(() => {
    const d = t.current;
    d && (n ? (d.scrollTop = d.scrollHeight, o(!1)) : o(!0));
  }, [n, s]);
  const l = x(() => {
    const d = t.current;
    if (!d) return;
    const g = d.scrollHeight - d.scrollTop - d.clientHeight < 48;
    a(g), g && o(!1);
  }, []), h = x(() => {
    const d = t.current;
    d && (d.scrollTop = d.scrollHeight, a(!0), o(!1));
  }, []);
  return /* @__PURE__ */ u("div", { className: "workbench-chat-wrap", children: [
    /* @__PURE__ */ u("div", { className: "workbench-chat-threadbar", children: [
      /* @__PURE__ */ u(
        "select",
        {
          ...
          onChange: (d) => e.selectThread(d.target.value),
          disabled: e.busy || e.chatThreads.length === 0,
          title: "Select chat thread",
          children: [
            e.chatThreads.length === 0 ? /* @__PURE__ */ i("option", { value: "", children: "No chats" }) : null,
            e.chatThreads.map((d) => /* @__PURE__ */ u("option", { value: d.id, children: [
              d.title,
              " · ",
              Wi(d.lastMessageAt ?? d.updatedAt)
            ] }, d.id))
          ]
        }
      ),
      /* @__PURE__ */ i(
        "button",
        {
          onClick: () => {
            ...
            const d = window.prompt("Rename chat", e.chatThread.title);
    ...
    /* @__PURE__ */ u("div", { ref: t, className: "workbench-chat-transcript", onScroll: l, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ i("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ u("div", { children: [
        /* @__PURE__ */ i(Na, { size: 48 }),
        /* @__PURE__ */ i("h3", { children: "Build with Agent" }),
        /* @__PURE__ */ i("p", { children: "Ask about open files, request a plan, or draft an artifact." }),
        /* @__PURE__ */ i("p", { className: "workbench-chat-empty-link", children: e.aiStatus.available ? e.aiStatus.label : e.aiStatus.reason ?? e.aiStatus.label })
      ] }) }) : e.chatMessages.map((d) => /* @__PURE__ */ u("div", { className: `workbench-chat-message ${d.role}`, children: [
        /* @__PURE__ */ i("strong", { children: d.role === "user" ? "You" : "Atomek" }),
        d.status === "streaming" ? /* @__PURE__ */ i("em", { children: " streaming" }) : null,
        d.status === "error" ? /* @__PURE__ */ i("em", { children: " error" }) : null,
        /* @__PURE__ */ i("br", {}),
        d.body,
        d.gatewayLabel ? /* @__PURE__ */ u(qe, { children: [
          /* @__PURE__ */ i("br", {}),
          /* @__PURE__ */ i("small", { children: d.gatewayLabel })
        ] }) : null,
        d.role === "assistant" && d.status !== "streaming" && d.status !== "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ i("button", { onClick: () => e.saveMessageAsArtifact(d), children: "Save artifact" }),
          /* @__PURE__ */ i("button", { onClick: () => e.rememberMessage(d), children: "Remember" }),
          /* @__PURE__ */ i("button", { onClick: () => e.previewEditFromMessage(d), disabled: e.workspaceFileCount === 0, children: "Preview edit" }),
          /* @__PURE__ */ u("button", { onClick: () => e.regenerateMessage(d), disabled: e.busy, children: [
            /* @__PURE__ */ i(Tt, { size: 11 }),
            " Regenerate"
          ] })
        ] }) : null,
        d.role === "assistant" && d.status === "error" ? /* @__PURE__ */ i("div", { className: "workbench-chat-message-actions", children: /* @__PURE__ */ i("button", { onClick: () => e.regenerateMessage(d), disabled: e.busy, children: "Retry" }) }) : null
      ] }, d.id)),
      r ? /* @__PURE__ */ i("button", { className: "workbench-chat-jump", onClick: h, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ u("div", { className: "workbench-chat-tip", children: [
        /* @__PURE__ */ i("span", { children: "Context" }),
        /* @__PURE__ */ i("strong", { children: sr(e.contextScope) }),
        /* @__PURE__ */ i("em", { children: Vt(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ u("div", { className: "workbench-chat-attachments", children: [
          /* @__PURE__ */ u(
            "select",
            {
              ...
              onChange: (d) => e.setContextScope(d.target.value),
              disabled: e.busy,
              title: "Context scope for next message",
              children: [
                /* @__PURE__ */ i("option", { value: "none", children: "No context" }),
                /* @__PURE__ */ i("option", { value: "active-selection", children: "Selection" }),
                /* @__PURE__ */ i("option", { value: "active-file", children: "Active file" }),
                /* @__PURE__ */ i("option", { value: "open-editors", children: "Open editors" }),
                /* @__PURE__ */ i("option", { value: "indexed-project", children: "Indexed project" })
              ]
            }
          ),
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ u("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ i(Ft, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((d) => {
            const f = typeof d.score == "number" ? d.score.toFixed(2) : null, g = typeof d.vectorScore == "number" ? d.vectorScore.toFixed(2) : null, b = typeof d.keywordScore == "number" ? d.keywordScore.toFixed(1) : null, M = [
              d.path,
              d.range ? `${d.range.startLineNumber}:${d.range.startColumn}-${d.range.endLineNumber}:${d.range.endColumn}` : null,
              f ? `score ${f}` : null,
              g ? `vector ${g}` : null,
              b ? `keyword ${b}` : null,
              d.snippet,
            ...
            return /* @__PURE__ */ u("span", { className: "workbench-chat-chip", title: M, children: [
              /* @__PURE__ */ i("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(d), disabled: !d.fileId, title: "Reveal context", children: /* @__PURE__ */ i(Ft, { size: 13 }) }),
              d.label,
              f ? /* @__PURE__ */ i("small", { children: f }) : null,
              d.snippet ? /* @__PURE__ */ u("small", { children: [
                d.snippet.slice(0, 60),
                d.snippet.length > 60 ? "…" : ""
              ] }) : null,
              d.dirty ? /* @__PURE__ */ i("small", { children: "dirty" }) : null,
              d.removable ? /* @__PURE__ */ i("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(d), title: "Remove context", children: /* @__PURE__ */ i(oe, { size: 11 }) }) : null
            ] }, d.id);
          }),
          /* @__PURE__ */ i("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("explain"), disabled: !e.activeFile || e.busy, children: "Explain" }),
          /* @__PURE__ */ i("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("improve"), disabled: !e.activeFile || e.busy, children: "Improve" }),
          /* @__PURE__ */ i("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("edit"), disabled: !e.activeFile || e.busy, children: "Edit" }),
          /* @__PURE__ */ i("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("draft"), disabled: e.busy, children: "Draft" })
        ] }),
        e.pendingPatchPrompt ? /* @__PURE__ */ i("button", { className: "workbench-chat-generate-patch", onClick: e.generatePatchPrompt, disabled: e.busy, children: "Generate patch for last edit request" }) : null,
        /* @__PURE__ */ i(
          "textarea",
          {
            className: "workbench-chat-textarea",
            value: e.chatInput,
            onChange: (d) => e.setChatInput(d.target.value),
            onKeyDown: (d) => {
              d.key === "Enter" && !d.shiftKey && (d.preventDefault(), e.busy || e.askAgent());
            },
        ...
        /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ i("span", { className: "workbench-chat-route-summary", children: Vt(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ i("span", {}),
          e.busy ? /* @__PURE__ */ i("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ i(Ha, { size: 14 }) }) : /* @__PURE__ */ i("button", { className: "workbench-chat-send", onClick: e.askAgent, title: "Send", children: /* @__PURE__ */ i(Oa, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Vt(e, t, n) {
  const a = e.model.trim(), o = [e.gatewayPreference === "auto" ? t : An(e.gatewayPreference)];
...
function qi(e) {
  const [t, n] = C([]), [a, r] = C("Loading gateway models…"), [o, s] = C([]), [l, h] = C("Checking embedding capability…");
  X(() => {
    ...
    return (async () => {
      ...
      try {
        const P = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: b.signal
        });
        if (b.signal.aborted) return;
        n(P.map((_) => ({ id: _.id, gatewayLabel: _.gatewayLabel }))), r(P.length > 0 ? `${P.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (P) {
        if (b.signal.aborted) return;
        n([]), r(`Model discovery failed: ${P instanceof Error ? P.message : String(P)}`);
      }
    })(), () => b.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), X(() => {
    const b = new AbortController();
    return (async () => {
      const P = Ni(e.host);
      if (P) {
      ...
      try {
        const _ = await Ai(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: b.signal
        });
        if (b.signal.aborted) return;
        s(_.map((J) => ({ id: J.id, gatewayLabel: J.gatewayLabel ?? J.source ?? "AIL" }))), h(_.length > 0 ? `${_.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (_) {
        if (b.signal.aborted) return;
        s([]), h(`Embedding model discovery failed: ${_ instanceof Error ? _.message : String(_)}`);
      }
    })(), () => b.abort();
  }, [e.chatSettings.gatewayPreference, e.host]);
  const d = (b) => {
    e.onChange({ ...e.chatSettings, gatewayPreference: b });
  }, f = (b) => {
    e.onChange({ ...e.chatSettings, model: b });
  }, g = (b) => {
    e.onChange({ ...e.chatSettings, embeddingModel: b });
  };
  return /* @__PURE__ */ i("div", { className: "workbench-settings-backdrop", onClick: e.onClose, children: /* @__PURE__ */ u("section", { className: "workbench-settings-dialog", onClick: (b) => b.stopPropagation(), role: "dialog", "aria-modal": "true", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ u("header", { className: "workbench-settings-header", children: [
      /* @__PURE__ */ i(Da, { size: 15 }),
      /* @__PURE__ */ i("strong", { children: "Atomek Settings" }),
      /* @__PURE__ */ i("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ i(oe, { size: 15 }) })
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        /* @__PURE__ */ i("h3", { children: "Chat AI routing" }),
        /* @__PURE__ */ i("p", { children: "Choose which AIL gateway Atomek uses. Model names are not hardcoded here: enter an AIL alias/model from your global gateway config, or leave it empty for the gateway default." }),
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ u(
            "select",
            {
              ...
              children: [
                /* @__PURE__ */ i("option", { value: "auto", children: "Auto failover" }),
                /* @__PURE__ */ i("option", { value: "remote", children: "Remote Tytus AIL only" }),
                /* @__PURE__ */ i("option", { value: "local", children: "Local AIL only" })
              ]
            }
          )
        ] }),
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ i("datalist", { id: "atomek-chat-models", children: t.map((b) => /* @__PURE__ */ i("option", { value: b.id, children: b.gatewayLabel }, `${b.gatewayLabel}:${b.id}`)) })
        ] }),
        /* @__PURE__ */ u("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          An(e.chatSettings.gatewayPreference),
          e.chatSettings.model.trim() ? ` · ${e.chatSettings.model.trim()}` : " · gateway default"
        ] }),
        /* @__PURE__ */ i("div", { className: "workbench-settings-note", children: a })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        /* @__PURE__ */ i("h3", { children: "Project context / embeddings" }),
        /* @__PURE__ */ i("p", { children: "Atomek keeps retrieval model selection dynamic. Leave empty for AIL global defaults, or pin an AIL embedding alias exposed by your gateway." }),
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ i("datalist", { id: "atomek-embedding-models", children: o.map((b) => /* @__PURE__ */ i("option", { value: b.id, children: b.gatewayLabel }, `${b.gatewayLabel}:${b.id}`)) })
        ] }),
        /* @__PURE__ */ i("div", { className: "workbench-settings-note", children: e.chatSettings.embeddingModel.trim() ? `Embedding alias: ${e.chatSettings.embeddingModel.trim()}` : "Embedding alias: gateway default" }),
        /* @__PURE__ */ i("div", { className: "workbench-settings-note", children: l })
      ] })
    ] }),
    /* @__PURE__ */ u("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ i("button", { onClick: () => e.onChange(Ue), children: "Reset" }),
      /* @__PURE__ */ i("button", { onClick: e.onClose, children: "Done" })
    ] })
  ] }) });
}
function Nn({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, openOutputAsFile: r, previewEditFromOutput: o, canPreviewEdit: s = !1, compact: l = !1 }) {
  return /* @__PURE__ */ u("div", { className: `workbench-panel-list ${l ? "compact" : ""}`, children: [
    /* @__PURE__ */ u("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ u("button", { className: "workbench-button-subtle", onClick: a, children: [
        /* @__PURE__ */ i(oa, { size: 14 }),
        "AI synthesis"
      ] }),
      /* @__PURE__ */ i("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ i("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((h) => /* @__PURE__ */ u("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ u("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ i("strong", { children: h.title }),
        /* @__PURE__ */ i("span", { children: h.source === "ai" ? `AI · ${h.kind}` : h.kind }),
        /* @__PURE__ */ i("button", { onClick: () => r(h), children: "Open as file" }),
        o ? /* @__PURE__ */ i(
          "button",
          {
            className: Ut(h.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(h),
            disabled: !s,
            children: Ut(h.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        h.source === "ai" ? /* @__PURE__ */ i("button", { onClick: () => n(h.id), children: "Delete" }) : null
      ] }),
      h.body
    ] }, h.id))
  ] });
}
function Ki({ edit: e, onApply: t, onOpenAsFile: n, onClose: a }) {
  return /* @__PURE__ */ i("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ u("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ u("div", { children: [
        /* @__PURE__ */ i("strong", { children: "Review AI edit" }),
        /* @__PURE__ */ i("span", { children: e.fileName })
      ] }),
      /* @__PURE__ */ i("button", { title: "Close", onClick: a, children: /* @__PURE__ */ i(oe, { size: 16 }) })
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-edit-review-meta", children: [
      ...
      /* @__PURE__ */ i("span", { children: e.extractionLabel }),
      /* @__PURE__ */ u("span", { children: [
        "+",
        e.stats.added,
        " / -",
    ...
    /* @__PURE__ */ u("div", { className: "workbench-edit-review-grid", children: [
      /* @__PURE__ */ u("div", { className: "workbench-edit-review-pane", children: [
        /* @__PURE__ */ i("h4", { children: "Current" }),
        /* @__PURE__ */ i("pre", { children: e.originalContent })
      ] }),
      /* @__PURE__ */ u("div", { className: "workbench-edit-review-pane proposed", children: [
        /* @__PURE__ */ i("h4", { children: "Proposed" }),
        /* @__PURE__ */ i("pre", { children: e.proposedContent })
      ] })
    ] }),
    /* @__PURE__ */ u("footer", { className: "workbench-edit-review-actions", children: [
      /* @__PURE__ */ i("button", { className: "workbench-button-subtle", onClick: a, children: "Cancel" }),
      /* @__PURE__ */ i("button", { className: "workbench-button-subtle", onClick: n, children: "Open proposed as file" }),
      /* @__PURE__ */ i("button", { className: "workbench-button-primary", onClick: t, children: "Apply to active file" })
    ] })
  ] }) });
}
function Xi({ patch: e, onApply: t, onOpenAsFiles: n, onClose: a }) {
  const r = e.edits.reduce((o, s) => ({
    added: o.added + s.stats.added,
    removed: o.removed + s.stats.removed,
    changed: o.changed + s.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ i("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ u("section", { className: "workbench-edit-review workspace", children: [
    /* @__PURE__ */ u("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ u("div", { children: [
        /* @__PURE__ */ i("strong", { children: "Review AI workspace patch" }),
        /* @__PURE__ */ i("span", { children: e.sourceTitle })
      ] }),
      /* @__PURE__ */ i("button", { title: "Close", onClick: a, children: /* @__PURE__ */ i(oe, { size: 16 }) })
    ] }),
    /* @__PURE__ */ u("div", { className: "workbench-edit-review-meta", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((o) => /* @__PURE__ */ u("article", { className: "workbench-workspace-patch-card", children: [
        /* @__PURE__ */ u("header", { children: [
          /* @__PURE__ */ i("strong", { children: o.fileName }),
          /* @__PURE__ */ u("span", { children: [
            o.extractionLabel,
            " · +",
            o.stats.added,
        ...
        /* @__PURE__ */ i("pre", { children: Zi(o.proposedContent) })
      ] }, o.fileId)),
      e.skipped.length > 0 ? /* @__PURE__ */ u("article", { className: "workbench-workspace-patch-card skipped", children: [
        /* @__PURE__ */ u("header", { children: [
          /* @__PURE__ */ i("strong", { children: "Skipped" }),
          /* @__PURE__ */ i("span", { children: "Paths not open or hunks did not match" })
        ] }),
        /* @__PURE__ */ i("pre", { children: e.skipped.join(`
`) })
      ] }) : null
    ] }),
    /* @__PURE__ */ u("footer", { className: "workbench-edit-review-actions", children: [
      /* @__PURE__ */ i("button", { className: "workbench-button-subtle", onClick: a, children: "Cancel" }),
      /* @__PURE__ */ i("button", { className: "workbench-button-subtle", onClick: n, children: "Open proposals as files" }),
      /* @__PURE__ */ i("button", { className: "workbench-button-primary", onClick: t, children: "Apply workspace patch" })
    ] })
  ] }) });
}
function Ji() {
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ i("div", { className: "workbench-sidebar-title", children: "EXTENSIONS" }),
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ u("div", { className: "workbench-extension-card", children: [
        /* @__PURE__ */ i("strong", { children: "TYTUS EXTENSIONS" }),
        /* @__PURE__ */ i("p", { className: "workbench-muted", children: "Connector surface kept intentionally. Chat, agents, pods, Hyperframes, Blender, and media tools wire in from here." })
      ] }),
      [
      ...
      ].map((t) => /* @__PURE__ */ u("div", { className: "workbench-extension-row", children: [
        /* @__PURE__ */ i(Jt, { size: 16 }),
        /* @__PURE__ */ u("div", { children: [
          /* @__PURE__ */ i("strong", { children: t.name }),
          /* @__PURE__ */ i("span", { children: t.detail })
        ] }),
        /* @__PURE__ */ i("em", { children: "coming soon" })
      ] }, t.name))
    ] })
  ] });
}
function qt({ title: e, body: t }) {
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ i("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ i("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function Yi({ status: e, file: t, cursor: n, fileCount: a, dirtyCount: r }) {
  return /* @__PURE__ */ u("footer", { className: "workbench-statusbar", children: [
    /* @__PURE__ */ i("span", { children: "main" }),
    /* @__PURE__ */ u("span", { children: [
      a,
      " files"
    ] }),
    r > 0 && /* @__PURE__ */ u("span", { children: [
      r,
      " unsaved"
    ] }),
    /* @__PURE__ */ i("span", { className: "workbench-status-spacer" }),
    ...
    /* @__PURE__ */ i("span", { children: "Spaces: 2" }),
    /* @__PURE__ */ i("span", { children: "UTF-8" }),
    /* @__PURE__ */ i("span", { children: "LF" }),
    /* @__PURE__ */ i("span", { children: tn(t.language) })
  ] });
}
function ct(e, t) {
...
function no() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : Ue.gatewayPreference,
...
function ao({ host: e }) {
  return /* @__PURE__ */ i(Ii, { host: e });
}
function oo(e) {
  return function() {
    return /* @__PURE__ */ i(ao, { host: e.host });
  };
}
export {
  oo as default
