...
import { forwardRef as wn, createElement as Lt, useState as C, useRef as Le, useCallback as y, useEffect as H, lazy as Ra, useMemo as Y, Suspense as _a } from "react";
/**
 ...
 */
const ja = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Da = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), rn = (e) => {
  const t = Da(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, vn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), Ba = (e) => {
  for (const t in e)
...
const Wa = wn(
  ({
    ...
    "svg",
    {
      ...
      className: vn("lucide", o),
      ...!i && !Ba(l) && { "aria-hidden": "true" },
      ...l
    },
    [
      ...c.map(([f, p]) => Lt(f, p)),
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const qa = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Ka = F("check", qa);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Ja = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], at = F("chevron-down", Ja);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
async function Mr() {
  ...
  if (!e) return En(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((a) => Ln(a, a.name, "local-file")));
  return Bt(n.filter(Boolean));
}
async function Pr() {
  const e = Dt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await En(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await Tn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Bt(n) };
}
function Dt() {
  const e = window;
...
async function Tn(e, t, n, a, o) {
  if (!(n.length >= o || a > 8))
    for await (const i of e.values()) {
      if (n.length >= o) break;
      if (i.kind === "directory") {
        if (Sr.has(i.name)) continue;
        await Tn(i, `${t}/${i.name}`, n, a + 1, o);
        continue;
      }
      const c = i, l = await Ln(c, `${t}/${c.name}`, "local-folder");
      l && n.push(l);
    }
}
function En(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const a = Array.from(n.files ?? []), o = await Promise.all(a.map(async (i) => {
        const c = i.webkitRelativePath || i.name;
        return Pn(c, i.size) ? {
          id: On(c),
          name: i.name,
          path: c,
...
`).map(
    ...
    const i = a.split("|").filter(Boolean).map((l) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${l.trim()}</th>`).join(""), c = o.trim().split(`
`).map((l) => `<tr>${l.split("|").filter(Boolean).map(
    ...
    return `<table style="border-collapse:collapse;margin:16px 0;width:100%;border:1px solid var(--border-default)"><thead><tr>${i}</tr></thead><tbody>${c}</tbody></table>`;
  }), t = t.replace(/^(?!<[a-z])(.+)$/gim, '<p style="line-height:1.6;margin:12px 0;color:var(--text-primary)">$1</p>'), t = t.replace(/\n+/g, `
`), t;
}
const Me = "atomek:default", Tr = 3e3, Ht = "tytus.atomek.threadTitleOverrides", cn = {
  available: !1,
  source: "none",
...
}, Or = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", ln = (e) => ({
  id: e.id,
  title: e.title,
  kind: Or(e.kind),
  body: e.body,
...
function _r({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const o = e.ai, [i, c] = C(null), [l, m] = C([]), [f, p] = C([]), [u, g] = C([]), [S, I] = C([]), [z, W] = C(cn), [ae, q] = C(!1), V = Le(!0), M = Le(null), E = y(async () => {
    if (!o) {
      W(cn);
      return;
    }
    ...
    if (o)
      try {
        const x = await o.listArtifacts({ threadId: A });
        if (!V.current) return;
        g(x.map(ln));
      } catch (x) {
        a(`AI artifacts unavailable: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [o, a]), pe = y(async (A) => {
    if (o)
      try {
        ...
        P && c(P), p(x.map(Be).filter(Boolean)), I([]), await Q(A);
      } catch (x) {
    ...
    if (o)
      try {
        const A = dn(await o.listThreads({ workspaceKey: Me, status: "active" })), x = A[0] ?? await o.createThread({ workspaceKey: Me, title: "Atomek chat" });
        if (!V.current) return;
        m(A[0] ? A : [x]), c(x);
  ...
  const re = y(async () => {
    if (!o) return null;
    if (i) return i;
    const A = await o.createThread({ workspaceKey: Me, title: "Atomek chat" });
    return V.current && (c(A), m((x) => [A, ...x.filter((P) => P.id !== A.id)])), A;
  }, [o, i]), w = y(async () => {
    if (!o) return;
    const A = await o.createThread({ workspaceKey: Me, title: "Atomek chat" });
    c(A), m((x) => [A, ...x.filter((P) => P.id !== A.id)]), p([]), g([]), I([]), a("New AI chat created");
  }, [o, a]), v = y(async (A, x) => {
    if (!o) return;
    const P = x.trim();
    if (P)
      try {
        const O = o.updateThread, U = l.find((Z) => Z.id === A) ?? i, _ = typeof O == "function" ? await O({ threadId: A, title: P }) : { ...U ?? await re(), id: A, title: P, updatedAt: Date.now() };
        typeof O != "function" && zr(A, P), m((Z) => Z.map((ye) => ye.id === _.id ? _ : ye)), c((Z) => Z?.id === _.id ? _ : Z), a(typeof O == "function" ? `Renamed chat: ${_.title}` : `Renamed chat locally: ${_.title}`);
      } catch (O) {
    ...
    if (o)
      try {
        await o.deleteThread(A), Rr(A);
        const x = l.filter((P) => P.id !== A);
        if (m(x), i?.id === A) {
          const P = x[0] ?? await o.createThread({ workspaceKey: Me, title: "Atomek chat" });
          c(P), m((U) => U.some((_) => _.id === P.id) ? U : [P, ...U]);
          const O = await o.listMessages(P.id);
          p(O.map(Be).filter(Boolean)), await Q(P.id);
        }
        a("Deleted AI chat");
      } catch (x) {
        a(`Delete chat failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [o, Q, a, i?.id, l]), J = y(async (A) => {
    if (!o) return null;
    try {
      const x = await re();
      if (!x) return null;
      const P = await o.createArtifact({
        threadId: x.id,
      ...
      return g((U) => [O, ...U.filter((_) => _.id !== O.id)]), a(`Saved AI artifact: ${O.title}`), O;
    } catch (x) {
    ...
    if (o)
      try {
        await o.deleteArtifact(A), g((x) => x.filter((P) => P.id !== A)), a("Deleted AI artifact");
      } catch (x) {
    ...
    try {
      const x = await o.writeMemory({
        title: A.title?.trim() || sn(A.body, "Atomek memory"),
        body: A.body,
        metadata: {
          source: "atomek",
          messageId: A.messageId ?? null
    ...
    if (!o) return [];
    const x = await o.searchMemory({ query: A, limit: 5 });
    return V.current && I(x), x;
  }, [o]), Te = y(async (A, x = {}) => {
    const P = A.trim();
    if (!P || !o) return null;
    q(!0);
    ...
    try {
      const _ = await re();
      if (!_) return null;
      const Z = await ce(P).catch(() => []), ye = Fr(Z), Ee = x.requestContext ?? t, Ve = ye ? [...Ee, ye] : [...Ee];
      let we = null;
      ...
      return V.current && fe.length > 0 && m(fe), E(), O;
    } catch (_) {
      ...
      M.current === U && (M.current = null), q(!1);
    }
  }, [o, n.gatewayPreference, n.model, re, ce, E, t, a]), ct = y(() => {
...
const jr = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Dr = (e) => {
  ...
  return {
    ...
    activeDocumentId: n,
    openDocumentIds: t.filter((l) => c.some((m) => m.id === l))
  };
...
[...clipped ${e.length - t} chars...]`, Zr = (e, t, n) => {
  const a = Kr(e, n), o = [], i = new Map(t.map((c) => [c.id, c]));
  for (const c of a) {
    if (!c.fileId) continue;
    const l = i.get(c.fileId), m = e.byId.get(c.fileId);
    if (!l || !m || !c.includeBody) continue;
    const f = [
      `Path: ${l.path}`,
      `Language: ${l.language}`,
      `Version: ${m.version}`,
      ...
      c.range ? `Range: ${hn(c.range)}` : null
    ].filter(Boolean).join(`
`), p = c.kind === "selection" && c.range ? eo(l.content, c.range) : l.content, u = c.kind === "selection" ? Xr : c.label.startsWith("Open editor") ? Yr : Gr;
    o.push({
...
`));
}, hn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, un = 75e4, to = 2800, no = 240, ao = 80, ro = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? un) || e.content.length > (t.maxFileBytes ?? un) ? "huge" : !t.includeDirty && e.dirty ? null : uo(e.path) ? "vendor" : mo(e) ? "binary" : null;
}, mn = (e, t = {}) => {
  const n = Date.now(), a = [], o = [], i = [];
  for (const c of e) {
    const l = c.size ?? c.content.length, m = Et(c, t);
    if (m) {
      o.push({ fileId: c.id, name: c.name, path: c.path, language: c.language, reason: m, size: l });
      continue;
    }
    const f = Ae(c.content), p = io(c, f, n, t);
    i.push(...p), a.push({
      fileId: c.id,
      name: c.name,
      path: c.path,
  ...
  return co({
    ...
    skipped: o,
    chunks: i,
    byFileId: new Map(a.map((c) => [c.fileId, c])),
    byChunkId: new Map(i.map((c) => [c.id, c]))
  });
}, io = (e, t = Ae(e.content), n = Date.now(), a = {}) => {
  const o = Math.max(500, a.maxChunkChars ?? to), i = Math.min(Math.max(0, a.chunkOverlapChars ?? no), Math.floor(o / 2)), c = Math.max(1, a.maxChunksPerFile ?? ao), l = lo(e.content), m = [];
  let f = 0, p = 0;
  for (; f < e.content.length && m.length < c; ) {
    const u = so(e.content, f, Math.min(e.content.length, f + o)), g = e.content.slice(f, u).trim();
    if (g.length > 0) {
      const S = ho(l, f, u);
      m.push({
...
` && t.push(n + 1);
  ...
  return 0;
}, uo = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => ro.has(t)), mo = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && oo.has(t) || e.content.includes("\0")) return !0;
...
class po {
  ...
  staleReport(t) {
    return fo(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const Pe = (e = [], t = {}) => new po(e, t), fo = (e, t, n = {}) => {
  ...
  for (const l of e.files) {
    ...
    if (Et(m, n)) {
      i.push({ fileId: l.fileId, path: m.path, status: "skipped-now", indexedHash: l.hash, currentHash: Ae(m.content) });
      continue;
    }
    const p = Ae(m.content);
    p !== l.hash ? i.push({ fileId: l.fileId, path: m.path, status: "changed", indexedHash: l.hash, currentHash: p }) : m.dirty !== l.dirty && i.push({ fileId: l.fileId, path: m.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: p });
  }
  for (const l of t)
    !o.has(l.id) && !Et(l, n) && i.push({ fileId: l.id, path: l.path, status: "new", currentHash: Ae(l.content) });
  const c = e.signature !== zn(t);
  return { stale: i.length > 0, signatureChanged: c, files: i };
}, bo = 8, go = 12e3, Rn = (e, t, n = {}, a) => {
  const o = wo(t);
  if (o.length === 0) return [];
  const i = new Set(a?.files.map((p) => p.fileId) ?? []), c = e.chunks.filter((p) => n.includeDirty !== !1 || !p.dirty).map((p) => vo(p, o)).filter((p) => p.score >= (n.minScore ?? 1)).sort((p, u) => u.score - p.score || p.chunk.path.localeCompare(u.chunk.path) || p.chunk.ordinal - u.chunk.ordinal), l = [], m = /* @__PURE__ */ new Set();
  let f = n.maxChars ?? go;
  for (const p of c) {
    if (l.length >= (n.limit ?? bo) || f <= 0) break;
...
[...context clipped...]` : p.chunk.text;
    l.push(ko(p, g, i.has(p.chunk.fileId))), f -= g.length;
  }
  return l;
...
`), wo = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const f = $t(n, m), p = $t(a, m), u = $t(o, m), g = i === m ? 1 : 0;
    f + p + u + g !== 0 && (l.push(m), c += Math.min(f, 12), c += p * 4, c += u * 6, c += g * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (c += 3));
  }
  ...
  const n = e.toLowerCase(), a = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, f) => m - f)[0] ?? 0, o = Math.max(0, a - 120), i = Math.min(e.length, a + 260), c = o > 0 ? "…" : "", l = i < e.length ? "…" : "";
  return `${c}${e.slice(o, i).replace(/\s+/g, " ").trim()}${l}`;
}, xo = (e, t = {}) => {
  const { autoRefresh: n = !1, ...a } = t, o = Le(e), i = Le(null);
  i.current || (i.current = Pe(e, a));
  const [c, l] = C(() => i.current?.getSnapshot() ?? Pe(e, a).getSnapshot());
  o.current = e;
  const m = y((g = o.current) => {
    const S = (i.current ?? Pe([], a)).refresh(g, a);
    return l(S), S;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), f = y((g = o.current) => {
    const S = (i.current ?? Pe([], a)).update(g, a);
    return l(S), S;
  }, [m]);
  H(() => {
    n && m(e);
  }, [n, e, m]);
  const p = (i.current ?? Pe([], a)).staleReport(e), u = y((g, S = {}) => {
    const I = i.current ?? Pe([], a);
    return Rn(I.getSnapshot(), g, S, I.staleReport(o.current));
  }, []);
  return { snapshot: c, staleReport: p, isStale: p.stale, refresh: m, update: f, retrieve: u };
}, Ao = "tytus.atomek.semanticVector:v1", _n = "__gateway_default__", jn = (e) => e?.trim() || _n, $o = (e) => {
  const t = jn(e);
  return t === _n ? "gateway default" : t;
}, Dn = (e, t, n) => [
  Ao,
  ...
  if (!Bn()) return null;
  try {
    const a = Dn(e, t, n), o = window.localStorage.getItem(a);
    if (!o) return null;
    const i = JSON.parse(o);
    return i.chunkId !== n.id || i.contentHash !== n.hash || i.modelAlias !== t || !Array.isArray(i.vector) || i.vector.some((c) => typeof c != "number" || !Number.isFinite(c)) ? null : {
      key: a,
      appId: e,
      chunkId: n.id,
      contentHash: n.hash,
  ...
  if (!Bn()) return null;
  try {
    const t = Dn(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, He = 8, Io = 12e3, Mo = 180, Po = async (e, t, n, a, o = {}, i, c) => {
  const l = Rn(t, n, { ...o, limit: Math.max(o.limit ?? He, 16) }, i), m = e.ai;
  ...
  try {
    const f = jn(a.embeddingModel), p = await m.embedText({
      input: n,
    ...
    const g = new Map(l.map((M) => [M.chunkId, M])), S = Ro(n), I = /* @__PURE__ */ new Map();
    t.chunks.filter((M) => o.includeDirty !== !1 || !M.dirty).slice(0, Mo).forEach((M) => I.set(M.id, M)), l.forEach((M) => {
      const E = t.byChunkId.get(M.chunkId);
      E && I.set(E.id, E);
    });
    const z = Array.from(I.values()), W = [];
    let ae = 0;
    for (const M of z) {
      c?.aborted;
      const E = No(e.appId, f, M) ?? await To(m, e.appId, f, M, a, c);
      if (!E) continue;
      E.updatedAt > Date.now() - 1e3 && (ae += 1);
      const Q = Fo(u, E.vector);
      if (!Number.isFinite(Q)) continue;
      const K = g.get(M.id)?.score ?? _o(M, S), re = zo(K) * 0.42 + Math.max(0, Q) * 0.58;
      W.push({ chunk: M, keywordScore: K, vectorScore: Q, score: re, matchedTerms: S.filter((w) => M.text.toLowerCase().includes(w)) });
    }
    if (W.length === 0) return { hits: l.slice(0, o.limit ?? He), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: ae };
    const q = new Set(i?.files.map((M) => M.fileId) ?? []), V = W.sort((M, E) => E.score - M.score || E.keywordScore - M.keywordScore || M.chunk.path.localeCompare(E.chunk.path) || M.chunk.ordinal - E.chunk.ordinal).slice(0, Math.max(o.limit ?? He, 1)).map((M) => Eo(M, q.has(M.chunk.fileId)));
    return {
      hits: Oo(V, o.maxChars ?? Io),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${$o(a.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: ae
    };
  } catch (f) {
    return {
  ...
  text: yo(t)
})), To = async (e, t, n, a, o, i) => {
  if (typeof e.embedText != "function") return null;
  const c = await e.embedText({
    input: a.text,
...
[...context clipped...]` : o.text;
  ...
  let a = 0, o = 0, i = 0;
  for (let c = 0; c < n; c += 1)
    a += e[c] * t[c], o += e[c] * e[c], i += t[c] * t[c];
  return o === 0 || i === 0 ? 0 : a / (Math.sqrt(o) * Math.sqrt(i));
}, zo = (e) => Math.min(1, Math.max(0, e / 24)), Ro = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), _o = (e, t) => {
  const n = `${e.path}
${e.name}
...
${e.text}`.toLowerCase();
  ...
  const n = e.toLowerCase(), a = t.map((c) => n.indexOf(c)).filter((c) => c >= 0).sort((c, l) => c - l)[0] ?? 0, o = Math.max(0, a - 120), i = Math.min(e.length, a + 260);
  return `${o > 0 ? "…" : ""}${e.slice(o, i).replace(/\s+/g, " ").trim()}${i < e.length ? "…" : ""}`;
}, Do = /```([^\n`]*)\n([\s\S]*?)```/g, Bo = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Vt(e) {
  return Array.from(e.matchAll(Do)).map((t) => Wn(t[1] ?? "", qo(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Wn(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), a = {}, o = [];
  for (const i of n) {
    const c = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    c ? a[c[1].toLowerCase()] = Jo(c[2]) : o.push(i.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: o, attrs: a, content: t };
}
function le(e, t) {
  return (typeof e == "string" ? Wn(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Vn(e) {
  return [...Vt(e).filter((a) => le(a, "diff") || le(a, "patch")).map((a) => a.content), e].filter((a, o, i) => Ho(a) && i.findIndex((c) => c === a) === o);
}
function Ho(e) {
...
`), a = [];
  if (n.forEach((c, l) => {
    (c.startsWith("diff --git ") || c.startsWith("--- ")) && a.push(l);
  }), a.length === 0) return [{ raw: t, ...Nt(t) }];
  const o = Array.from(new Set(a)).sort((c, l) => c - l), i = [];
  for (let c = 0; c < o.length; c += 1) {
    const l = o[c], f = o.find((u) => u > l && n[u].startsWith("diff --git ")) ?? n.length, p = n.slice(l, f).join(`
`);
    /^@@\s+-\d+/m.test(p) && i.push({ raw: p, ...Nt(p) });
  }
  return i.length > 0 ? i : [{ raw: t, ...Nt(t) }];
}
function Nt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), o = me(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = me(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, c = i && i !== "/dev/null" ? i : o;
  return { path: c && c !== "/dev/null" ? c : null, oldPath: o, newPath: i };
}
function me(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function Ot(e) {
  return Vt(e).filter((t) => Vo(t)).map((t) => ({
    content: Ko(t.content),
...
`), o = [];
  ...
  for (let l = 0; l < a.length; l += 1) {
    ...
    if (p < i) return null;
    for (o.push(...n.slice(i, p)), i = p, l += 1; l < a.length; l += 1) {
      const u = a[l];
      if (u.startsWith("@@ ")) {
        l -= 1;
        break;
  ...
  return c ? (o.push(...n.slice(i)), o.join(`
`)) : null;
}
function qo(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function Xo(e) {
  const { body: t, files: n, sourceTitle: a, activeFile: o = null, versions: i = {} } = e, c = /* @__PURE__ */ new Map(), l = [];
  for (const p of Vn(t))
    for (const u of Wo(p)) {
      if (!u.path) {
        if (o) {
          const I = Ft(o.content, u.raw);
          I && I !== o.content && c.set(o.id, Ze({
            file: o,
            proposedContent: I,
            sourceTitle: a,
  ...
  for (const p of Ot(t)) {
    const u = p.path ? fn(n, p.path) : o ? { file: o, normalizedPatchPath: me(o.path), confidence: "exact" } : null;
    if (!u) {
  ...
  if (c.size === 0 && o && !m) {
    const p = Go(t, o);
    p && p.content !== o.content && c.set(o.id, Ze({
      file: o,
      proposedContent: p.content,
      sourceTitle: a,
      extractionLabel: p.label,
...
function Go(e, t) {
  ...
  const l = n.filter((m) => !le(m, "diff") && !le(m, "patch")).sort((m, f) => f.content.length - m.content.length)[0];
  return l ? { content: l.content, label: `largest fenced block (${l.lang || "plain"})` } : null;
}
function fn(e, t) {
  const n = me(t), a = n.split("/").at(-1) ?? n, o = e.find((l) => me(l.path) === n);
  if (o) return { file: o, normalizedPatchPath: n, confidence: "exact" };
  const i = e.find((l) => me(l.path).endsWith(`/${n}`));
  if (i) return { file: i, normalizedPatchPath: n, confidence: "suffix" };
  const c = e.find((l) => l.name === a || me(l.path).endsWith(`/${a}`));
...
`), o = Math.max(n.length, a.length);
  let i = 0, c = 0, l = 0;
  for (let m = 0; m < o; m += 1)
    n[m] !== a[m] && (n[m] === void 0 ? i += 1 : a[m] === void 0 ? c += 1 : l += 1);
  return { added: i, removed: c, changed: l };
}
function Qo(e) {
  return e.reduce((t, n) => ({
...
function Ze(e) {
  ...
  return {
    ...
    stats: Yo(e.file.content, e.proposedContent),
    base: {
...
const ot = (e) => typeof e == "object" && e !== null, ti = (e) => ot(e.ai) ? e.ai : null, ni = (e) => {
  if (!ot(e)) return null;
  const t = e.id;
...
}, he = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(he) : ot(e) ? Object.values(e).some(he) : !1, ri = async (e, t) => {
  const n = ti(e);
  if (typeof n?.listModels != "function") return [];
  const a = await n.listModels(t);
  return Array.isArray(a) ? a.map(ai).filter((o) => !!o) : [];
}, oi = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : he(e.capability) || he(e.capabilities) || he(e.modality) || he(e.modalities) || he(e.task) || he(e.tasks) || he(e.type) || he(e.kind), qn = (e) => typeof e == "object" && e !== null, ii = (e) => qn(e.ai) ? e.ai : null, Kn = (e) => typeof ii(e)?.embedText == "function", ci = async (e, t) => Kn(e) ? (await ri(e, t)).filter(oi) : [], si = (e) => qn(e.ai) ? Kn(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", li = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], di = [
...
function pi(e) {
  const t = e.results.length > 0 ? e.results.map((a, o) => [
    ...
    `Status: ${a.status}`,
    "Output:",
    Ci(a.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function bi(e) {
  ...
  for (const a of t) {
    const o = gi(a.content);
    if (!o?.scripts) continue;
    const i = Xn(a.path), c = ki(e, i, o);
    if (!c) continue;
    const l = Object.keys(o.scripts).filter((f) => typeof o.scripts?.[f] == "string"), m = yi(l);
    for (const f of m) {
      const p = `${c} run ${f}`;
      n.push({
        id: Gn(`${i}:${p}`),
...
function ki(e, t, n) {
  ...
  for (const [a, o] of di)
    if (e.some((i) => vi(i.path) === a && Xn(i.path) === t)) return o;
  return null;
}
function yi(e) {
  const t = li.filter((a) => e.includes(a)), n = e.filter((a) => !t.includes(a)).filter((a) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(a)).sort((a, o) => a.localeCompare(o));
  return [...t, ...n];
}
function wi(e) {
  const t = /* @__PURE__ */ new Set();
...
const xi = Ra(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Ai = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Yn = "tytus.workspace.recent", Qn = "tytus.workspace.layout", Zn = "tytus.atomek.chatAiSettings", nt = {
...
function St(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - $i), a = n < 1180, o = a ? 200 : 240, i = a ? 300 : 340, c = a ? 420 : 560, l = Math.max(o, Math.min(a ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), f = Math.floor(n * (a ? 0.34 : 0.36)), p = n - m - c, u = Math.max(i, Math.min(a ? 500 : 640, f, p));
...
function Ni(e) {
  ...
  const c = e.slice(a);
  return (c.trim() || t.length === 0) && t.push({ type: "markdown", body: c, key: `md-${o}` }), t;
}
async function it(e) {
  if (!e) return !1;
  try {
...
function Si({ host: e }) {
  const t = Le(null), [n, a] = C(0), o = Y(() => oc(), []), [i, c] = C("explorer"), [l, m] = C(o.primaryVisible), [f, p] = C(o.primaryWidth), [u, g] = C("chat"), [S, I] = C(o.secondaryVisible), [z, W] = C(o.secondaryWidth), [ae, q] = C(!1), [V, M] = C("problems"), [E, Q] = C(o.markdownPreviewVisible), [pe, K] = C(!1), [re, w] = C(null), [v, T] = C([]), [J, ie] = C([]), [B, ce] = C(null), [Te, ct] = C(""), [A, x] = C({ lineNumber: 1, column: 1 }), [P, O] = C(null), [U, _] = C({}), [Z, ye] = C(Hr), [Ee, Ve] = C([]), [we, fe] = C([]), [D, j] = C(null), [ee, te] = C(""), [oa, st] = C(!1), [ia, ca] = C(""), [qt, Ue] = C([]), [X, $e] = C(null), [be, Oe] = C(null), [Kt, Jt] = C(!1), [ve, sa] = C(() => ic()), [Xt, Fe] = C(null), [Ne, lt] = C(null), [dt, ht] = C(""), [ut, mt] = C(""), [qe, Ke] = C(""), [Je, Gt] = C("failed"), [la, Yt] = C(null), [da, $] = C("Ready"), [ze, ha] = C(() => rc()), se = J.map((s) => v.find((d) => d.id === s)).filter(Boolean), N = B ? v.find((s) => s.id === B) ?? null : null, Qt = Y(() => Br({ files: v, openEditorIds: J, activeFileId: B, versions: U, activeSelection: P }), [B, P, U, v, J]), Zt = Y(() => ({
    scope: Z,
    removedAttachmentIds: Ee,
    selectedFileIds: []
  }), [Z, Ee]), Re = Y(() => Zr(Qt, v, Zt), [Zt, Qt, v]), ge = xo(v, { autoRefresh: !0, includeDirty: !0 }), en = Y(() => we.map((s) => ({
    id: s.id,
    kind: "index-hit",
    label: s.label,
    path: s.path,
    ...
    snippet: s.snippet
  })), [we]), ua = Y(() => [...Re.attachments, ...en], [Re.attachments, en]), L = _r({ host: e, requestContext: Re.parts, chatSettings: ve, setStatus: $ }), Ce = Y(
    () => [...L.artifacts, ...qt].sort((s, d) => d.createdAt - s.createdAt),
    [L.artifacts, qt]
  ), Xe = Kt && !N, pt = !N && !Xe && !pe, ne = Y(() => v.filter((s) => s.dirty), [v]), ma = Y(() => {
    const s = Te.trim().toLowerCase();
    return s ? v.filter((d) => d.path.toLowerCase().includes(s)) : v;
  }, [v, Te]);
  H(() => {
    _((s) => {
      const d = {};
      for (const b of v) d[b.id] = s[b.id] ?? 1;
      return d;
    });
  }, [v]), H(() => {
    Ve([]), fe([]);
  ...
  const pa = y((s) => {
    if (s.kind === "index-hit") {
      fe((d) => d.filter((b) => b.id !== s.id)), $(`Removed project context: ${s.label}`);
      return;
    }
    Ve((d) => d.includes(s.id) ? d : [...d, s.id]), $(`Removed chat context: ${s.label}`);
  }, []), Ge = y(() => {
    Jt(!0), ce(null), K(!0), $("Atomek settings opened");
  }, []), tn = y(() => {
    Jt(!1), ce((s) => s ?? J.at(-1) ?? null), J.length === 0 && K(!1), $("Atomek settings closed");
  }, [J]), Se = y((s) => {
    _((d) => ({ ...d, [s]: (d[s] ?? 1) + 1 }));
  }, []), fa = y((s) => {
    s.preventDefault(), s.currentTarget.setPointerCapture?.(s.pointerId);
    const d = St(n), b = s.clientX, k = z, R = (De) => {
      const xt = k + (b - De.clientX);
      W(et(xt, d.secondaryMin, d.secondaryMax));
    }, de = () => {
      window.removeEventListener("pointermove", R), window.removeEventListener("pointerup", de);
    };
    window.addEventListener("pointermove", R), window.addEventListener("pointerup", de);
  }, [z, n]), ba = y((s) => {
    s.preventDefault(), s.currentTarget.setPointerCapture?.(s.pointerId);
    const d = St(n), b = s.clientX, k = f, R = (De) => {
      const xt = k + (De.clientX - b);
      p(et(xt, d.primaryMin, d.primaryMax));
    }, de = () => {
      window.removeEventListener("pointermove", R), window.removeEventListener("pointerup", de);
    };
    window.addEventListener("pointermove", R), window.addEventListener("pointerup", de);
  }, [f, n]), Ye = y((s) => {
    const d = [s, ...ze.filter((b) => b.path !== s.path)].slice(0, 6);
    ha(d), localStorage.setItem(Yn, JSON.stringify(d));
  }, [ze]), G = y((s, d) => {
    ie((b) => b.includes(s.id) ? b : [...b, s.id]), ce(s.id), K(!1), Yt(d ?? null), x({ lineNumber: d ?? 1, column: 1 });
  }, []), ga = y((s) => {
    if (!s.fileId) return;
    const d = v.find((b) => b.id === s.fileId);
    d && (G(d, s.range?.startLineNumber ?? 1), $(`Revealed context: ${s.label}`));
  }, [v, G]), _e = y(async () => {
    if (It(ne, "open new files"))
      try {
        const s = await Mr();
        if (s.length === 0) return;
        T((d) => tc(d, s)), s.forEach((d) => Ye({ name: d.name, path: d.path, at: Date.now() })), G(s[0]), $(`Opened ${s.length} local file${s.length === 1 ? "" : "s"}`);
      } catch (s) {
        s.name !== "AbortError" && $(`Open file failed: ${s.message}`);
      }
  }, [ne, G, Ye]), ft = y(async () => {
    if (It(ne, "open another folder"))
      try {
        const s = await Pr();
        w(s), T(s.files), ie([]), ce(null), K(!1), Ye({ name: s.name, path: s.name, at: Date.now() }), $(`${s.handle ? "Opened local folder" : "Opened browser fallback folder"} ${s.name} (${s.files.length} text files indexed)`);
      } catch (s) {
    ...
    if (!B) return;
    let d = !1;
    T((b) => b.map((k) => k.id !== B || k.content === s ? k : (d = !0, { ...k, content: s, dirty: !0 }))), d && Se(B);
  }, [B, Se]), bt = y(async () => {
    if (N)
      try {
        const s = await At(N);
        T((d) => d.map((b) => b.id === s.id ? s : b)), $(`Saved ${s.name}`);
      } catch (s) {
        $(`Save failed: ${s.message}`);
      }
  }, [N]), ya = y(async (s) => {
    const d = v.find((k) => k.id === s);
    if (!d) return null;
    const b = await At(d);
    return T((k) => k.map((R) => R.id === b.id ? b : R)), b;
  }, [v]), nn = y(async () => {
    const s = v.filter((d) => d.dirty);
    if (s.length === 0) {
      $("No dirty files to save");
      return;
    }
    try {
      const d = await Promise.all(s.map((k) => At(k))), b = new Map(d.map((k) => [k.id, k]));
      T((k) => k.map((R) => b.get(R.id) ?? R)), Fe(null), $(`Saved ${d.length} dirty file${d.length === 1 ? "" : "s"}`);
    } catch (d) {
      $(`Save all failed: ${d.message}`);
    }
  }, [v]), gt = y((s) => {
    const d = v.find((b) => b.id === s);
    if (d?.dirty && !window.confirm(`${d.name} has unsaved changes. Close without saving?`)) {
      $(`Close canceled — ${d.name} has unsaved changes`);
      return;
    }
    ie((b) => {
      const k = b.filter((R) => R !== s);
      return B === s && (ce(k.at(-1) ?? null), O(null)), k;
    });
  }, [B, v]), wa = y(() => {
    It(ne, "close all editors") && (ie([]), ce(null), Yt(null), K(!1), $("Closed all editors"));
  }, [ne]), kt = y(() => {
    const s = v.filter((b) => b.name.startsWith("Untitled")).length + 1, d = {
...
`,
  ...
  }, [Re.parts, Z, ve, e, ge]), yt = y(() => {
    if (!N && se.length === 0) {
      $("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const d = [
      `Create a polished Markdown artifact from ${N?.path ?? `${se.length} open editors`}.`,
    ...
    I(!0), g("chat"), $("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const b = await ke(d), k = await L.askAgent(d, { requestContext: b });
      !k || k.status === "error" || L.createArtifact({
        messageId: k.id,
        title: `AI synthesis — ${N?.name ?? "open workspace"}`,
        kind: "markdown",
        body: k.body
      }).then(() => {
        I(!0), g("outputs"), q(!0), M("output");
      });
    })();
  }, [N, L, ke, se.length]), wt = y((s) => {
    const d = v.find((b) => b.path === s.path || b.name === s.name);
    if (d) {
      G(d), $(`Opened recent ${d.name}`);
      return;
    }
    ...
    I(!0), g("chat"), te(""), (async () => {
      const d = await ke(s);
      await L.askAgent(s, { requestContext: d });
    })();
  }, [L, ke]), va = y(async (s) => {
    if (!e.skills?.get) {
    ...
    try {
      const d = await e.skills.get(s.id), b = d.body.length > 4500 ? `${d.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : d.body, k = [
...
`);
      te((R) => [R.trim(), k].filter(Boolean).join(`

`)), I(!0), g("chat"), $(`Attached skill ${d.title} to chat input`);
    } catch (d) {
      $(`Failed to attach skill: ${d instanceof Error ? d.message : String(d)}`);
    }
  }, [e.skills]), Ca = y((s) => {
    const d = L.messages.findIndex((k) => k.id === s.id), b = L.messages.slice(0, d < 0 ? void 0 : d).reverse().find((k) => k.role === "user");
    if (!b?.body.trim()) {
      $("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const k = await ke(b.body);
      await L.askAgent(b.body, { requestContext: k });
    })();
  }, [L, ke]), xa = y((s) => {
    L.createArtifact({
      messageId: s.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const d = Pt(v, Mt(s.title || s.kind)), b = {
    ...
    T((k) => [...k, b]), G(b), $(`Opened ${s.title} as editable file`);
  }, [v, G]), Ct = y(() => {
...
`), R = {
    ...
    Ue((k) => [b, ...k]), I(!0), g("outputs"), q(!0), M("output"), $(`Saved local job output: ${s}`), (d.includes("```diff") || d.includes("--- a/")) && Ie(s, d);
  }, [Ie]), Sa = y(() => {
    const s = ee.trim();
    if (!s) return;
    te("");
    const d = Vi(s);
    (async () => {
      const b = await ke(s), k = await L.askAgent(s, { requestContext: b });
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", k.body) || $("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    Ke(b), ht(""), mt(""), Gt("failed"), q(!0), M("terminal"), $(d.commands.length > 0 ? `Manual check ready: copy ${d.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [v]), Ma = y((s) => {
    s.trim() && (it(s), Ke(s), $(`Manual check command copied: ${s}`));
  }, []), Pa = y(() => {
    lt((s) => {
      if (!s) return s;
      const d = ui(s, dt), b = d.commands.at(-1)?.command ?? "";
      return b && Ke(b), d;
    }), ht("");
  }, [dt]), La = y(() => {
    if (!Ne || !qe.trim()) {
    ...
    const s = qe.trim();
    lt((d) => d && mi(d, s, Je, ut)), mt(""), $(`Captured manual check result: ${s} (${Je})`);
  }, [ut, qe, Ne, Je]), Ta = y(() => {
    Ne && oe(pi(Ne));
  }, [oe, Ne]), Ea = y(() => {
    if (!X) return;
    const s = v.find((d) => d.id === X.fileId);
    if (!s) {
      $(`Cannot apply edit — ${X.fileName} is no longer open`), $e(null);
      return;
    }
    s.content !== X.originalContent && !window.confirm(`${X.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (T((d) => d.map((b) => b.id === X.fileId ? { ...b, content: X.proposedContent, dirty: !0 } : b)), Se(X.fileId), Fe(`AI edit applied to ${s.name}. Save All to persist it to disk.`), $e(null), je(`AI edit applied to ${s.name}`), $(`Applied AI edit to ${s.name} — unsaved; manual check ready`));
  }, [Se, v, X, je]), Oa = y(() => {
    if (!X) return;
    const s = Pt(v, `${Mt(X.fileName)}-ai-edit`), d = {
    ...
    T((b) => [...b, d]), G(d), $e(null), $(`Opened proposed edit as ${s}`);
  }, [N?.language, v, G, X]), Fa = y(() => {
    if (!be) return;
    const s = new Map(be.edits.map((b) => [b.fileId, b]));
    v.some((b) => {
      const k = s.get(b.id);
      return k && b.content !== k.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (T((b) => b.map((k) => {
      const R = s.get(k.id);
      return R ? { ...k, content: R.proposedContent, dirty: !0 } : k;
    })), s.forEach((b, k) => Se(k)), Fe(`AI workspace patch applied to ${s.size} file${s.size === 1 ? "" : "s"}. Save All to persist changes.`), Oe(null), je(`AI workspace patch applied to ${s.size} file${s.size === 1 ? "" : "s"}`), $(`Applied AI workspace patch to ${s.size} file${s.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Se, v, be, je]), za = y(() => {
    if (!be) return;
    const s = [...v], d = be.edits.map((b) => {
      const k = Pt(s, `${Mt(b.fileName)}-ai-edit`), R = v.find((De) => De.id === b.fileId), de = {
    ...
    T((b) => [...b, ...d]), d[0] && G(d[0]), Oe(null), $(`Opened ${d.length} proposed edit file${d.length === 1 ? "" : "s"}`);
  }, [v, G, be]), an = y((s) => {
    if (s === "explain") {
      oe("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (s === "improve") {
      oe("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (s === "plan") {
      oe("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (s === "edit") {
      oe("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    oe("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [oe]);
  return H(() => {
    const s = (d) => {
      (d.metaKey || d.ctrlKey) && (d.key.toLowerCase() === "o" && (d.preventDefault(), _e()), d.key.toLowerCase() === "s" && (d.preventDefault(), bt()), d.key.toLowerCase() === "w" && (d.preventDefault(), B && gt(B)), d.key.toLowerCase() === "b" && (d.preventDefault(), m((k) => !k)), d.shiftKey && d.key.toLowerCase() === "f" && (d.preventDefault(), c("search"), m(!0)), (d.key.toLowerCase() === "k" || d.key.toLowerCase() === "p") && (d.preventDefault(), st(!0)));
    };
    ...
    const s = t.current;
    if (!s) return;
    const d = () => a(Math.round(s.getBoundingClientRect().width));
    d();
    ...
    if (!n) return;
    const s = St(n);
    p((d) => et(d, s.primaryMin, s.primaryMax)), W((d) => et(d, s.secondaryMin, s.secondaryMax));
  }, [n]), H(() => {
    const s = { primaryVisible: l, primaryWidth: f, secondaryVisible: S, secondaryWidth: z, markdownPreviewVisible: E };
    localStorage.setItem(Qn, JSON.stringify(s));
  }, [E, l, f, S, z]), H(() => {
    localStorage.setItem(Zn, JSON.stringify(ve));
  }, [ve]), /* @__PURE__ */ h(
    "div",
    {
      ref: t,
      ...
      children: [
        /* @__PURE__ */ r(Ii, { active: i, setActive: (s) => {
          c(s), m(!0);
        ...
        l && /* @__PURE__ */ h("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ r(
            Mi,
            {
              ...
              saveLocalJobOutput: Na,
              activeFile: N
            }
          ),
          /* @__PURE__ */ r("div", { className: "workbench-primary-resizer", onPointerDown: ba, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ h("main", { className: "workbench-editor-area", children: [
          ...
          /* @__PURE__ */ h("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ r(Ti, { file: N, folder: re, showWelcome: pt }),
            Xt && ne.length > 0 ? /* @__PURE__ */ h("div", { className: "workbench-ai-dirty-banner", children: [
              ...
              /* @__PURE__ */ r("button", { onClick: () => Fe(null), title: "Dismiss", children: /* @__PURE__ */ r(ue, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: N ? /* @__PURE__ */ h("div", { className: N.language === "markdown" && E ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ r("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ r(_a, { fallback: /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ r(
                xi,
                {
                  file: N,
                  revealLine: la,
              ...
              N.language === "markdown" && E && /* @__PURE__ */ r(_i, { content: N.content })
            ] }) : Xe ? /* @__PURE__ */ r(
              qi,
              {
                host: e,
            ...
            ) : pt ? /* @__PURE__ */ r(Oi, { openFile: _e, openFolder: ft, newFile: kt, recent: ze, reopenRecent: wt, setStatus: $ }) : /* @__PURE__ */ h("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ r(rt, { size: 34 }),
            ...
            ae && /* @__PURE__ */ r(
              Di,
              {
                ...
                copyManualCheckCommand: Ma,
                addManualCheckCommand: Pa,
                recordManualCheckResult: La,
                askAgentFromManualChecks: Ta,
                onClose: () => q(!1)
              }
            )
          ] })
        ...
        S && /* @__PURE__ */ r(
          Bi,
          {
            ...
            saveMessageAsArtifact: xa,
            rememberMessage: Aa,
            previewEditFromMessage: (s) => Ie(s.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", s.body),
            runQuickPrompt: an,
            pendingPatchPrompt: D,
            generatePatchPrompt: Ia,
            ...
            captureManualCheck: Ct,
            openOutputAsFile: vt,
            previewEditFromOutput: (s) => Ie(s.title, s.body),
            ...
            contextScope: Z,
            setContextScope: ye,
            contextAttachments: ua,
            removeContextAttachment: pa,
            revealContextAttachment: ga,
            projectIndexSummary: `${ge.snapshot.files.length} files · ${ge.snapshot.chunks.length} chunks`,
            projectIndexStale: ge.isStale,
            refreshProjectIndex: () => {
              const s = ge.refresh(v);
        ...
        oa && /* @__PURE__ */ r(
          Ri,
          {
            ...
            commands: [
              ...
              ...ze.map((s) => ({ label: `File: Open Recent — ${s.name}`, detail: s.path, run: () => wt(s) })),
              { label: "File: Save All", detail: `${ne.length} dirty file${ne.length === 1 ? "" : "s"}`, run: () => {
                nn();
              }, disabled: ne.length === 0 },
              { label: "File: Close All Editors", detail: `${se.length} open editor${se.length === 1 ? "" : "s"}`, run: wa, disabled: se.length === 0 },
              ...
              { label: "View: Toggle Bottom Panel", detail: ae ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => q((s) => !s) },
              ...
              { label: "View: Toggle Markdown Preview", detail: N?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => Q((s) => !s), disabled: N?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: N || se.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: yt, disabled: !N && se.length === 0 },
              { label: "AI: Explain Active File", detail: N ? `Ask Cortex to explain ${N.path}` : "Open a file first", run: () => oe("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !N },
              { label: "AI: Improve Active File", detail: N ? `Ask Cortex for concrete edits to ${N.path}` : "Open a file first", run: () => oe("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !N },
              { label: "AI: Draft Editable Replacement", detail: N ? `Ask Cortex for a full-file replacement for ${N.path}` : "Open a file first", run: () => an("edit"), disabled: !N },
              ...
              { label: "AI: Save Active File as Artifact", detail: N ? "Persist active file in host.ai artifacts" : "Open a file first", run: $a, disabled: !N },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Ct },
              { label: "AI: Open Latest Artifact as File", detail: Ce[0] ? `Create editable file from ${Ce[0].title}` : "No outputs yet", run: () => Ce[0] && vt(Ce[0]), disabled: Ce.length === 0 }
            ],
            openWorkbenchFile: G,
            onClose: () => st(!1)
          }
        ...
        /* @__PURE__ */ r(ec, { status: da, file: N ?? Ai, cursor: A, fileCount: v.length, dirtyCount: ne.length })
      ]
    }
  );
}
function Ii({ active: e, setActive: t, openSettings: n, settingsActive: a }) {
  return /* @__PURE__ */ h("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    ...
    /* @__PURE__ */ r(xe, { icon: /* @__PURE__ */ r(gr, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ r(xe, { icon: /* @__PURE__ */ r(Nn, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    ...
    /* @__PURE__ */ r(xe, { icon: /* @__PURE__ */ r(Ga, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ r(xe, { icon: /* @__PURE__ */ r(vr, { size: 23 }), label: "Settings", active: a, onClick: n })
  ] });
}
...
function Mi(e) {
  return e.activity === "search" ? /* @__PURE__ */ r(Fi, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ r(yn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ r(yn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ r(Zi, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ r(Pi, { ...e });
}
function Pi(e) {
  const t = !e.folder, n = Y(() => Li(e.files, e.folder?.name), [e.files, e.folder?.name]), [a, o] = C(() => /* @__PURE__ */ new Set());
  H(() => {
    const c = ea(n);
    o((l) => {
      const m = new Set(l);
      return c.forEach((f) => m.add(f)), m;
    });
  }, [n]);
  const i = y((c) => {
    o((l) => {
  ...
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ h(We, { children: [
        ...
        /* @__PURE__ */ r("input", { className: "workbench-input", value: e.query, onChange: (c) => e.setQuery(c.target.value), placeholder: "Search files" }),
        /* @__PURE__ */ h("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(at, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((c) => /* @__PURE__ */ r(Ut, { file: c, active: c.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(c) }, c.id)),
        /* @__PURE__ */ h("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(at, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No readable text files found." }) : ta(n, e.activeFileId, e.openWorkbenchFile, a, i)
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((c) => /* @__PURE__ */ h("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(c), children: [
        /* @__PURE__ */ r($n, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: c.name })
      ] }, `${c.path}-${c.at}`))
    ] })
  ] });
}
function Ut({ file: e, active: t, onOpen: n, basePath: a, depth: o = 0, label: i }) {
  const c = a && e.path.startsWith(`${a}/`) ? e.path.slice(a.length + 1) : e.path, l = o || Math.max(0, c.split("/").length - 1);
  return /* @__PURE__ */ h("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": l }, onClick: n, title: e.path, children: [
    /* @__PURE__ */ r(xn, { size: 14 }),
    /* @__PURE__ */ r("span", { className: "workbench-row-name", children: i ?? c }),
    e.dirty && /* @__PURE__ */ r("span", { className: "workbench-row-meta", children: "●" })
  ] });
}
function Li(e, t) {
  const n = [], a = /* @__PURE__ */ new Map(), o = (c, l, m = n) => {
    const f = a.get(c);
    if (f) return f;
    const p = { name: l, path: c, children: [] };
    return a.set(c, p), m.push(p), p;
  };
  e.forEach((c) => {
    const l = t && c.path.startsWith(`${t}/`) ? c.path.slice(t.length + 1) : c.path, m = l.split("/").filter(Boolean);
    let f = n, p = "";
    m.slice(0, -1).forEach((u) => {
      p = p ? `${p}/${u}` : u, f = o(p, u, f).children;
    }), f.push({ name: m.at(-1) ?? c.name, path: l, file: c, children: [] });
  });
  const i = (c) => c.sort((l, m) => +!!l.file - +!!m.file || l.name.localeCompare(m.name)).map((l) => ({ ...l, children: i(l.children) }));
  return i(n);
}
function ea(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...ea(t.children)]);
}
function ta(e, t, n, a, o, i = 0) {
  return e.map((c) => {
    if (c.file)
      return /* @__PURE__ */ r(Ut, { file: c.file, active: c.file.id === t, onOpen: () => n(c.file), depth: i, label: c.name }, c.file.id);
    const l = a.has(c.path);
    return /* @__PURE__ */ h("div", { children: [
      /* @__PURE__ */ h("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => o(c.path), title: l ? `Collapse ${c.name}` : `Expand ${c.name}`, children: [
        l ? /* @__PURE__ */ r(at, { size: 12 }) : /* @__PURE__ */ r(at, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ r(jt, { size: 14 }) : /* @__PURE__ */ r($n, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: c.name }),
        /* @__PURE__ */ r("span", { className: "workbench-row-meta", children: c.children.length })
      ] }),
      l ? ta(c.children, t, n, a, o, i + 1) : null
    ] }, c.path);
  });
}
function Ti({ file: e, folder: t, showWelcome: n }) {
  const a = n ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], o = t && a[0] === t.name ? a.slice(1) : a;
  return /* @__PURE__ */ r("div", { className: "workbench-breadcrumb", children: o.length === 0 ? /* @__PURE__ */ r("span", { children: "Workspace" }) : o.map((i, c) => /* @__PURE__ */ h("span", { className: "workbench-breadcrumb-part", children: [
    c > 0 && /* @__PURE__ */ r("span", { className: "workbench-breadcrumb-sep", children: "›" }),
    i
  ] }, `${i}-${c}`)) });
}
function Ei(e) {
...
function Oi({ openFile: e, openFolder: t, newFile: n, recent: a, reopenRecent: o, setStatus: i }) {
  return /* @__PURE__ */ h("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ h("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ h("section", { children: [
        ...
        a.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : a.map((c) => /* @__PURE__ */ h("button", { className: "workbench-start-link", onClick: () => o(c), children: [
          c.name,
...
function Fi({ files: e, query: t, setQuery: n, openWorkbenchFile: a, activeFileId: o }) {
  const i = Y(() => zi(e, t), [e, t]), c = Y(() => {
  ...
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? c.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No matches." }) : c.map((l) => {
        const m = l[0].file;
        return /* @__PURE__ */ h("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ r(Ut, { file: m, active: m.id === o, onOpen: () => a(m) }),
...
`).some((i, c) => i.toLowerCase().includes(n) ? (a.push({ file: o, lineNumber: c + 1, line: i.trim() || "(blank line)" }), a.length >= 200) : !1);
  }), a.slice(0, 200);
}
function Ri(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${Mn(o.language)} file`,
...
function ji(e) {
  const t = e.session ? fi(e.session) : "pending";
...
function Di(e) {
  return /* @__PURE__ */ h("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ h("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ r("span", {}),
      /* @__PURE__ */ r("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ r(ue, { size: 14 }) })
    ] }),
    /* @__PURE__ */ h("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ r(ra, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Bi(e) {
  return /* @__PURE__ */ h("aside", { className: "workbench-secondary", children: [
    ...
    e.tab === "chat" ? /* @__PURE__ */ r(Hi, { ...e }) : /* @__PURE__ */ r(ra, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function na({ body: e }) {
  const t = Y(() => Ni(e), [e]), [n, a] = C(null), o = y((i, c) => {
    (async () => await it(c) && (a(i), window.setTimeout(() => a((m) => m === i ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ r("div", { className: "workbench-rich-body", children: t.map((i) => {
      ...
      return /* @__PURE__ */ h("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ h("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ h("button", { onClick: () => o(i.key, i.body), title: "Copy code block", children: [
            c ? /* @__PURE__ */ r(Ka, { size: 12 }) : /* @__PURE__ */ r(Tt, { size: 12 }),
            c ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ r(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Fn(i.body) }
      },
      i.key
    );
  }) });
...
function Hi(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Le(null), [a, o] = C(!0), [i, c] = C(!1), l = Y(() => e.chatMessages.map((u) => `${u.id}:${u.status ?? ""}:${u.body.length}`).join("|"), [e.chatMessages]);
  H(() => {
    const u = n.current;
    u && (a ? (u.scrollTop = u.scrollHeight, c(!1)) : c(!0));
  }, [a, l]);
  const m = y(() => {
    const u = n.current;
    if (!u) return;
    ...
    const u = n.current;
    u && (u.scrollTop = u.scrollHeight, o(!0), c(!1));
  }, []), p = y((u) => {
    it(u.body);
  }, []);
  return /* @__PURE__ */ h("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((u) => /* @__PURE__ */ h("option", { value: u.id, children: [
              u.title,
              " · ",
              Wi(u.lastMessageAt ?? u.updatedAt)
            ] }, u.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ h("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      ...
      i ? /* @__PURE__ */ r("button", { className: "workbench-chat-jump", onClick: f, children: "Jump to latest" }) : null
    ] }),
      ...
      /* @__PURE__ */ h("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ h("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ h("span", { className: "workbench-chat-chip muted", children: [
            ...
            const g = typeof u.score == "number" ? u.score.toFixed(2) : null, S = typeof u.vectorScore == "number" ? u.vectorScore.toFixed(2) : null, I = typeof u.keywordScore == "number" ? u.keywordScore.toFixed(1) : null, z = [
              u.path,
            ...
            return /* @__PURE__ */ h("span", { className: "workbench-chat-chip", title: z, children: [
              ...
              u.removable ? /* @__PURE__ */ r("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(u), title: "Remove context", children: /* @__PURE__ */ r(ue, { size: 11 }) }) : null
            ] }, u.id);
          }),
...
function bn(e, t, n) {
  const a = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : aa(e.gatewayPreference)];
...
function qi(e) {
  const [t, n] = C([]), [a, o] = C("Loading gateway models…"), [i, c] = C([]), [l, m] = C("Checking embedding capability…");
  H(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(I.map((z) => ({ id: z.id, gatewayLabel: z.gatewayLabel }))), o(I.length > 0 ? `${I.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (I) {
        if (g.signal.aborted) return;
        n([]), o(`Model discovery failed: ${I instanceof Error ? I.message : String(I)}`);
      }
    })(), () => g.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), H(() => {
    const g = new AbortController();
    return (async () => {
      const I = si(e.host);
      if (I) {
      ...
      try {
        const z = await ci(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: g.signal
        });
        if (g.signal.aborted) return;
        c(z.map((W) => ({ id: W.id, gatewayLabel: W.gatewayLabel ?? W.source ?? "AIL" }))), m(z.length > 0 ? `${z.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (z) {
        if (g.signal.aborted) return;
        c([]), m(`Embedding model discovery failed: ${z instanceof Error ? z.message : String(z)}`);
      }
    })(), () => g.abort();
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ h("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ h("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ r("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ r(ue, { size: 15 }) })
    ] }),
    /* @__PURE__ */ h("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ h("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ h("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          aa(e.chatSettings.gatewayPreference),
...
function ra({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: o, openOutputAsFile: i, previewEditFromOutput: c, canPreviewEdit: l = !1, compact: m = !1 }) {
  return /* @__PURE__ */ h("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ h("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ h("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ r("button", { onClick: () => i(f), children: "Open as file" }),
        c ? /* @__PURE__ */ r(
          "button",
          {
...
function Ki({ edit: e, onApply: t, onOpenAsFile: n, onClose: a }) {
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ h("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ h("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ r("button", { title: "Close", onClick: a, children: /* @__PURE__ */ r(ue, { size: 16 }) })
    ] }),
...
function Ji({ patch: e, onApply: t, onOpenAsFiles: n, onClose: a }) {
  const o = e.edits.reduce((i, c) => ({
    added: i.added + c.stats.added,
    removed: i.removed + c.stats.removed,
    changed: i.changed + c.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ h("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ h("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ r("pre", { children: ac(i.proposedContent) })
      ] }, i.fileId)),
...
function Gi(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, Xi(e.content), "```");
  else if (t.length > 0) {
...
function Zi({ host: e, setStatus: t, attachSkillToChat: n, saveLocalJobOutput: a, activeFile: o, openEditors: i }) {
  const [c, l] = C([]), [m, f] = C([]), [p, u] = C(!1), [g, S] = C(null), [I, z] = C("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [W, ae] = C(null), [q, V] = C(""), M = i.filter((w) => w.dirty).length, E = o ? `${o.path} · ${o.language} · ${o.content.length.toLocaleString()} chars${o.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${M ? ` · ${M} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Q = Y(() => [
    {
    ...
    if (!e.local?.listTools && !e.skills?.list) {
      l([]), f([]), S("This Tytus host build does not expose local tools or skill registry yet.");
      return;
    }
    u(!0), S(null);
    try {
      ...
      l(w), f(v), t(`Computer loaded · ${w.length} tools · ${v.length} skills`);
    } catch (w) {
  ...
  const K = y(async (w) => {
    ...
    try {
      await e.local.openTerminal({
        ...
        command: w.command,
        cwd: kn(o),
        prompt: `Opened from Atomek. ${E}`
      }), t(`Opened ${w.label} interactively in Tytus Terminal`);
    } catch (v) {
      t(`Terminal launch failed: ${v instanceof Error ? v.message : String(v)}`);
    }
  }, [o, E, e.local, t]), re = y(async (w) => {
...
`).slice(-12e3));
        ...
        onFail: (ie) => {
          J.push(`[FAIL] ${ie}`), V(J.join(`
`).slice(-12e3)), a(`${w.label} local job failed`, J.join(`
`)), ae(null);
        },
        onExit: (ie) => {
          const B = [
...
`);
      ...
      ae(null), t(`Local job failed to start: ${T instanceof Error ? T.message : String(T)}`);
    }
  }, [o, e.local, I, i, a, t]);
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ h("div", { className: "workbench-computer-hero", children: [
        ...
        /* @__PURE__ */ h("div", { children: [
          /* @__PURE__ */ r("strong", { children: "Computer cockpit" }),
          /* @__PURE__ */ r("p", { className: "workbench-muted", children: "Use installed local agents and app skills against the active Atomek file/context. Tytus launches only allowlisted tools through the same-origin bridge. No raw browser shell. No hardcoded models." })
        ] })
      ] }),
      /* @__PURE__ */ h("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ...
        " ",
        p ? "Refreshing…" : "Refresh capabilities"
      ] }),
      g && /* @__PURE__ */ r("div", { className: "workbench-inline-error", children: g }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "ACTIVE CONTEXT" }),
      /* @__PURE__ */ h("div", { className: "workbench-computer-context-card", children: [
        /* @__PURE__ */ r("strong", { children: E }),
        /* @__PURE__ */ r("span", { children: o ? "Local agents receive clipped active-file content and must return previewable edits." : "Open a file to give local agents useful context." })
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "TASK FOR LOCAL AGENT" }),
      /* @__PURE__ */ r("div", { className: "workbench-computer-presets", children: Q.map((w) => /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => z(w.prompt), children: w.label }, w.label)) }),
      /* @__PURE__ */ r(
        "textarea",
        {
      ...
      q ? /* @__PURE__ */ r("pre", { className: "workbench-computer-job-log", children: q }) : null,
      ...
      /* @__PURE__ */ h("div", { className: "workbench-computer-list", children: [
        c.length === 0 && !p ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        c.map((w) => /* @__PURE__ */ h("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ h("div", { className: "workbench-computer-card-head", children: [
            ...
            /* @__PURE__ */ r("span", { className: `workbench-computer-pill ${w.status}`, children: w.status })
          ] }),
          w.description ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: w.description }) : null,
          /* @__PURE__ */ h("div", { className: "workbench-computer-actions", children: [
            /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
              K(w);
            }, disabled: w.status !== "available", title: "Launch this tool in the real Tytus terminal with current Atomek context prefilled.", children: Yi(w) }),
            w.kind === "ai-cli" ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
              re(w);
            }, disabled: w.status !== "available" || W !== null, title: "Run this local agent headlessly and stream output back into Atomek Outputs. It cannot write files directly.", children: Qi(w, W) }) : null
          ] })
        ] }, w.id))
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
      /* @__PURE__ */ h("div", { className: "workbench-computer-list", children: [
        m.length === 0 && !p ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
        m.map((w) => /* @__PURE__ */ h("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ h("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ h("div", { children: [
              /* @__PURE__ */ r("strong", { children: w.title }),
              /* @__PURE__ */ h("span", { children: [
                w.driver,
                " · ",
                w.source,
          ...
          /* @__PURE__ */ r("p", { className: "workbench-muted", children: w.description }),
          w.triggers?.length ? /* @__PURE__ */ r("div", { className: "workbench-computer-triggers", children: w.triggers.slice(0, 4).map((v) => /* @__PURE__ */ r("span", { children: v }, v)) }) : null,
          /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
            n(w);
          }, disabled: w.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
        ] }, w.id))
      ] })
    ] })
  ] });
}
function yn({ title: e, body: t }) {
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ r("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function ec({ status: e, file: t, cursor: n, fileCount: a, dirtyCount: o }) {
  return /* @__PURE__ */ h("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ r("span", { children: Mn(t.language) })
  ] });
}
function It(e, t) {
