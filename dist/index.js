...
import { forwardRef as Ga, createElement as En, useState as P, useRef as Re, useCallback as w, useEffect as ve, createContext as xs, useMemo as ue, useContext as Ps, lazy as Ns, Suspense as Ts } from "react";
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Fs = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], qt = ye("chevron-down", Fs);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
`).map((l) => `<tr>${l.split("|").filter(Boolean).map(
      (v, p) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${p % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${v.trim()}</td>`
    ).join("")}</tr>`).join("");
...
function Ni(e) {
  const t = e?.resources ?? [], n = (l) => l.status === "ready" || l.status === "available", r = t.filter((l) => l.kind === "pod-agent" && Ct(l) === "openclaw"), i = t.filter((l) => l.kind === "pod-agent" && Ct(l) === "hermes"), s = t.filter((l) => l.kind === "local-cli" && n(l)), o = t.filter((l) => l.kind === "shared-folder" && n(l));
  return [
...
function jt(e, t = "repo-repair") {
  const n = Yt.find((x) => x.id === t) ?? Yt[0], r = e?.resources ?? [], i = gt(r, (x) => x.kind === "pod-agent" && Ct(x) === "openclaw"), s = gt(r, (x) => x.kind === "pod-agent" && Ct(x) === "hermes"), o = gt(r, (x) => x.kind === "ail-route"), l = gt(r, (x) => x.kind === "shared-folder"), m = ct(r, "local-cli", ["claude"]), v = ct(r, "local-cli", ["opencode", "open code"]), p = ct(r, "local-cli", ["codex"]), y = ct(r, "local-cli", ["pi"]), T = ct(r, "local-cli", ["kimi"]), z = gt(r, (x) => x.kind === "local-cli"), V = ct(r, "app-skill", ["juli3ta", "music", "song"]), R = ct(r, "app-skill", ["blender", "3d"]), Y = ct(r, "app-skill", ["remotion", "video", "render"]), B = gt(r, (x) => x.kind === "app-skill"), we = t === "pod-local" ? i ?? s ?? o ?? m ?? z : t === "creative-production" ? s ?? o ?? m ?? z : t === "research-watch" ? i ?? o ?? s ?? m ?? z : m ?? v ?? o ?? z, G = t === "creative-production" ? V ?? R ?? Y ?? B ?? v ?? m ?? z : v ?? m ?? p ?? z, M = t === "pod-local" ? p ?? y ?? T ?? i ?? m ?? z : t === "research-watch" ? y ?? T ?? p ?? i ?? z : p ?? y ?? i ?? T ?? z, I = t === "creative-production" ? V ?? R ?? Y ?? B : t === "research-watch" ? ct(r, "app-skill", ["browser", "channel", "telegram"]) ?? B : B, he = [
    Ft("planner", "Planner", "Break goal into tasks, risks, and required context.", we, "No planner agent ready"),
    Ft("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", G, "No local/app implementer ready"),
    Ft("reviewer", "Reviewer", "Independent critique before approval or handoff.", M, "No reviewer agent ready"),
    Ft("team-desk", "Team Desk", "Shared mission folder for transcripts, outputs, proposals, and handoff.", l, "Mission folder only until shared folder is bound")
  ];
  (t === "creative-production" || I) && he.push(Ft("app-tool", "App Tool", "Drive installed local app skill through mission context.", I, "No configured app skill"));
  const se = he.filter((x) => x.status === "ready" || x.status === "available" || x.status === "degraded").length, N = se >= he.length - 1 ? "ready" : se >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: N, assignments: he };
}
function kt(e, t, n) {
...
function Ai(e) {
  return Yt.map((t) => jt(e, t.id));
}
function Ut(e) {
  return `${e.label}: ${e.resourceLabel} (${e.status})`;
}
function bn(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", i = jt(t, kt(r, t, n)), s = (y) => i.assignments.find((T) => T.role === y), o = s("planner"), l = s("implementer"), m = s("reviewer"), v = s("team-desk"), p = s("app-tool");
  return [
    {
      id: "task-scope",
    ...
    } : null,
    {
      ...
      resourceHint: m ? `${Ut(m)} · ${v?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
...
function Kn(e, t, n, r, i, s) {
  const o = jt(t, kt(i || e.goal, t, s ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
...
function Ci(e, t, n, r) {
  const i = kt(n || e.goal, t, r ?? e.teamPresetId), s = jt(t, i), o = bn(n || e.goal, t, i);
...
function qi(e) {
  return e.kind !== "pod-agent" ? null : pr([
    e.label,
    e.id,
    Ot(e.metadata, "displayName", "display_name", "name", "podName", "pod_name"),
    Ot(e.metadata, "agentFamily", "agent_family", "agentType", "agent_type", "internalAgentType", "brand")
  ].join(" "));
}
function Yi(e) {
...
function Zi(e, t) {
  const n = Ot(e.metadata, "displayName", "display_name", "customName", "custom_name", "podDisplayName", "pod_display_name", "name");
  if (n) {
...
function Bi(e) {
  ...
  return e.map((s) => {
    ...
    r[m] = (r[m] ?? 0) + 1;
    const v = t.filter((T) => T.label.toLowerCase() === m).length > 1, p = o === l && n[s.agentFamily] > 1;
    if (!v && !p) return s;
    const y = p ? i[s.agentFamily] : r[m];
    return { ...s, label: `${o} ${y}` };
  });
}
function Ui(e) {
  const t = e.daemon?.state, n = t?.included ?? [], r = t?.agents ?? [], i = new Map(r.map((o) => [o.id, o])), s = n.flatMap((o) => {
    const l = o.id ?? "", v = { ...(o.agentId ? i.get(o.agentId) : void 0)?.meta ?? {}, ...o.meta ?? {} }, p = pr([l, o.kind, Ot(v, "displayName", "display_name", "agentFamily", "agentType", "brand")].join(" "));
    return !l || !p ? [] : [{
      id: `pod-agent.${l}`,
      kind: "pod-agent",
      label: vn[p],
...
function ho({ host: e, requestContext: t, chatSettings: n, selectedTarget: r = st, setStatus: i }) {
  const s = e.ai, [o, l] = P(null), [m, v] = P([]), [p, y] = P([]), [T, z] = P([]), [V, R] = P([]), [Y, B] = P(ja), [we, G] = P(!1), M = Re(!0), I = Re(null), he = w(async () => {
    if (!s) {
      B(ja);
      return;
    }
  ...
  const ne = w(async () => {
    ...
    return M.current && (l(L), yt(L.id), v((O) => [L, ...O.filter((q) => q.id !== L.id)])), L;
  }, [s, o]), ze = w(async () => {
    if (r.kind === "pod-agent") {
    ...
    if (!s) return;
    const L = await s.createThread({ workspaceKey: nt, title: "Atomek chat" });
    l(L), yt(L.id), v((O) => [L, ...O.filter((q) => q.id !== L.id)]), y([]), z([]), R([]), i("New AI chat created");
    ...
    if (q)
      try {
        ...
        typeof S != "function" && Wa(L, q), v((ge) => ge.map((pe) => pe.id === ce.id ? ce : pe)), l((ge) => ge?.id === ce.id ? ce : ge), i(typeof S == "function" ? `Renamed chat: ${ce.title}` : `Renamed chat locally: ${ce.title}`);
      } catch (S) {
    ...
    if (s)
      try {
        ...
        if (v(O), o?.id === L) {
          ...
          l(q), yt(q.id), v(($) => $.some((ce) => ce.id === q.id) ? $ : [q, ...$]);
          const S = await s.listMessages(q.id);
          y(S.map(zt).filter(Boolean)), await se(q.id);
        }
        i("Deleted AI chat");
    ...
    try {
      ...
      if (S.kind === "pod-agent") {
        ...
        if (y((Z) => [...Z, re, Pe]), Gt(S.id, [re, Pe]), !S.available) {
          const Z = Gn(S.description), be = { ...Pe, body: Z.message, status: "error", error: Z.message };
          return y((Ae) => Ae.map((le) => le.id === me ? be : le)), Gt(S.id, [be]), i(Z.message), be;
        }
        if (!_) {
          const Z = "Agent chat bridge unavailable in this Tytus host build. Update TytusOS host API/runtime to enable OpenClaw and Hermes chat.", be = { ...Pe, body: Z, status: "error", error: Z };
          return y((Ae) => Ae.map((le) => le.id === me ? be : le)), Gt(S.id, [be]), i("Agent chat bridge unavailable"), be;
        }
        let He = "";
        const it = (Z, be) => (y((Ae) => Ae.map((le) => le.id === me ? Z : le)), Gt(S.id, [re, Z]), i(be), Z);
        try {
          for await (const Z of _.chatAgent({
            ...
            if (Z.type === "profile") {
              const be = Z.profile === "local" ? "Local Cortex" : "Cloud Cortex";
              y(
                (Ae) => Ae.map(
                  (le) => le.id === me ? { ...le, gatewayLabel: be } : le
                )
              );
            }
            if (Z.type === "session" && ro(S.id, Z.sessionId), Z.type === "token" && (He = yn(`${He}${Z.text}`), y((be) => be.map((Ae) => Ae.id === me ? { ...Ae, body: He, status: "streaming" } : Ae))), Z.type === "error") {
              const be = Gn(Z.message), Ae = { ...Pe, body: be.message, status: "error", error: be.message };
              return it(Ae, be.message);
            }
            if (Z.type === "done")
      ...
      if (!pe || !s) return null;
      yt(pe.id);
      const g = (pe.title ?? "").trim();
      if ((!g || g === "Atomek chat") && q.trim()) {
        const _ = Hn(q, "Atomek chat");
        _ && _ !== g && (async () => {
          try {
            const ae = s.updateThread, re = typeof ae == "function" ? await ae({ threadId: pe.id, title: _ }) : { ...pe, title: _, updatedAt: Date.now() };
            if (typeof ae != "function" && Wa(pe.id, _), !M.current) return;
            l((me) => me?.id === re.id ? re : me), v((me) => me.map((Pe) => Pe.id === re.id ? re : Pe));
          } catch {
          }
        })();
      }
      ...
      return M.current && Ee.length > 0 && v(Ee), he(), $;
    } catch (ge) {
...
`), p = o.kind === "selection" && o.range ? Lo(l.content, o.range) : l.content, y = o.kind === "selection" ? Po : o.label.startsWith("Open editor") ? To : No;
    i.push({
...
`));
  ...
  for (const o of e) {
    ...
    const v = wt(o.content), p = jo(o, v, n, t);
    s.push(...p), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
  ...
  for (; v < e.content.length && m.length < o; ) {
    const y = Wo(e.content, v, Math.min(e.content.length, v + i)), T = e.content.slice(v, y).trim();
    if (T.length > 0) {
      const z = Io(l, v, y);
      m.push({
        id: `${e.id}:${t}:${p}:${z.startLineNumber}-${z.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
const At = (e = [], t = {}) => new Do(e, t), Eo = (e, t, n = {}) => {
  ...
  for (const l of e.files) {
    ...
    const p = wt(m.content);
    p !== l.hash ? s.push({ fileId: l.fileId, path: m.path, status: "changed", indexedHash: l.hash, currentHash: p }) : m.dirty !== l.dirty && s.push({ fileId: l.fileId, path: m.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: p });
  }
  for (const l of t)
  ...
  const s = new Set(r?.files.map((p) => p.fileId) ?? []), o = e.chunks.filter((p) => n.includeDirty !== !1 || !p.dirty).map((p) => Bo(p, i)).filter((p) => p.score >= (n.minScore ?? 1)).sort((p, y) => y.score - p.score || p.chunk.path.localeCompare(y.chunk.path) || p.chunk.ordinal - y.chunk.ordinal), l = [], m = /* @__PURE__ */ new Set();
  let v = n.maxChars ?? Yo;
  for (const p of o) {
    if (l.length >= (n.limit ?? qo) || v <= 0) break;
    const y = `${p.chunk.fileId}:${p.chunk.ordinal}`;
    if (m.has(y)) continue;
    m.add(y);
    const T = p.chunk.text.length > v ? `${p.chunk.text.slice(0, Math.max(0, v - 28))}
[...context clipped...]` : p.chunk.text;
    l.push(Jo(p, T, s.has(p.chunk.fileId))), v -= T.length;
  }
  return l;
...
`), Ko = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const v = jn(n, m), p = jn(r, m), y = jn(i, m), T = s === m ? 1 : 0;
    v + p + y + T !== 0 && (l.push(m), o += Math.min(v, 12), o += p * 4, o += y * 6, o += T * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (o += 3));
  }
  ...
  const n = e.toLowerCase(), r = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, v) => m - v)[0] ?? 0, i = Math.max(0, r - 120), s = Math.min(e.length, r + 260), o = i > 0 ? "…" : "", l = s < e.length ? "…" : "";
  ...
  i.current = e;
  const m = w((T = i.current) => {
    const z = (s.current ?? At([], r)).refresh(T, r);
    return l(z), z;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), v = w((T = i.current) => {
    const z = (s.current ?? At([], r)).update(T, r);
    return l(z), z;
  }, [m]);
  ve(() => {
    n && m(e);
  }, [n, e, m]);
  const p = (s.current ?? At([], r)).staleReport(e), y = w((T, z = {}) => {
    const V = s.current ?? At([], r);
    return kr(V.getSnapshot(), T, z, V.staleReport(i.current));
  }, []);
  return { snapshot: o, staleReport: p, isStale: p.stale, refresh: m, update: v, retrieve: y };
...
}, Rt = 8, tc = 12e3, nc = 180, ac = async (e, t, n, r, i = {}, s, o) => {
  const l = kr(t, n, { ...i, limit: Math.max(i.limit ?? Rt, 16) }, s), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: l.slice(0, i.limit ?? Rt), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const v = Pr(r.embeddingModel), p = await m.embedText({
      input: n,
    ...
    const T = new Map(l.map((M) => [M.chunkId, M])), z = dc(n), V = /* @__PURE__ */ new Map();
    ...
    for (const M of R) {
      o?.aborted;
      const I = _o(e.appId, v, M) ?? await sc(m, e.appId, v, M, r, o);
      if (!I) continue;
      I.updatedAt > Date.now() - 1e3 && (B += 1);
      const he = cc(y, I.vector);
      if (!Number.isFinite(he)) continue;
      const N = T.get(M.id)?.score ?? uc(M, z), x = lc(N) * 0.42 + Math.max(0, he) * 0.58;
      Y.push({ chunk: M, keywordScore: N, vectorScore: he, score: x, matchedTerms: z.filter((Te) => M.text.toLowerCase().includes(Te)) });
    }
    if (Y.length === 0) return { hits: l.slice(0, i.limit ?? Rt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: B };
    const we = new Set(s?.files.map((M) => M.fileId) ?? []), G = Y.sort((M, I) => I.score - M.score || I.keywordScore - M.keywordScore || M.chunk.path.localeCompare(I.chunk.path) || M.chunk.ordinal - I.chunk.ordinal).slice(0, Math.max(i.limit ?? Rt, 1)).map((M) => ic(M, we.has(M.chunk.fileId)));
    return {
...
`), r = [];
  ...
  for (let o = 0; o < i.length; o += 1) {
    const l = i[o], v = i.find((y) => y > l && n[y].startsWith("diff --git ")) ?? n.length, p = n.slice(l, v).join(`
`);
    /^@@\s+-\d+/m.test(p) && s.push({ raw: p, ...Mn(p) });
  }
...
`), i = [];
  ...
  for (let l = 0; l < r.length; l += 1) {
    ...
    if (!m) continue;
    o = !0;
    const v = Number(m[1]), p = Math.max(0, v - 1);
    if (p < s) return null;
    for (i.push(...n.slice(s, p)), s = p, l += 1; l < r.length; l += 1) {
      const y = r[l];
      if (y.startsWith("@@ ")) {
        l -= 1;
        break;
...
function xc(e) {
  ...
  for (const p of Lr(t))
    for (const y of bc(p)) {
      ...
      const z = _n(T.file.content, y.raw);
      if (!z || z === T.file.content) {
  ...
  for (const p of $n(t)) {
    const y = p.path ? Ea(n, p.path) : i ? { file: i, normalizedPatchPath: lt(i.path), confidence: "exact" } : null;
    if (!y) {
      l.push(`${p.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (p.content === y.file.content) {
    ...
    o.set(y.file.id, $t({
      ...
      versions: s,
      targetPath: p.path ?? y.file.path,
      match: y
    }));
  }
  const m = $n(t).some((p) => p.path);
  if (o.size === 0 && i && !m) {
    const p = Pc(t, i);
    p && p.content !== i.content && o.set(i.id, $t({
      file: i,
      proposedContent: p.content,
      sourceTitle: r,
      extractionLabel: p.label,
...
function Pc(e, t) {
  ...
  const s = n.find((m) => r.some((v) => $e(m, v)) && !$e(m, "diff") && !$e(m, "patch"));
  if (s) return { content: s.content, label: `matched ${s.lang || t.language} block` };
  const l = n.filter((m) => !$e(m, "diff") && !$e(m, "patch")).sort((m, v) => v.content.length - m.content.length)[0];
...
function qc(e) {
  ...
  for (const r of t) {
    ...
    if (!o) continue;
    const l = Object.keys(i.scripts).filter((v) => typeof i.scripts?.[v] == "string"), m = Zc(l);
    for (const v of m) {
      const p = `${o} run ${v}`;
      n.push({
        id: jr(`${s}:${p}`),
...
function al({ host: e, children: t }) {
  ...
  ve(() => e.i18n?.onLocaleChange((s) => r(Ya(s))), [e]);
  const i = ue(() => ({
    locale: n,
    t: (s, o) => {
...
], dl = "tytus-atomek-handles", St = "handles", ul = 1, Ir = () => new Promise((e, t) => {
  const n = indexedDB.open(dl, ul);
  n.onupgradeneeded = () => {
    const r = n.result;
    r.objectStoreNames.contains(St) || r.createObjectStore(St);
  }, n.onsuccess = () => e(n.result), n.onerror = () => t(n.error ?? new Error("Failed to open Atomek handle store"));
});
async function Ja(e, t) {
  const n = await Ir();
  await new Promise((r, i) => {
    const s = n.transaction(St, "readwrite");
    s.objectStore(St).put(t, e), s.oncomplete = () => r(), s.onerror = () => i(s.error ?? new Error("Failed to persist file handle"));
  }), n.close();
}
async function Wn(e) {
  const t = await Ir(), n = await new Promise((r, i) => {
    const o = t.transaction(St, "readonly").objectStore(St).get(e);
...
function In(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - fl), r = n < 1180, i = r ? 200 : 240, s = r ? 280 : 300, o = r ? 360 : 420, l = Math.max(i, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), v = Math.floor(n * (r ? 0.5 : 0.62)), p = n - m - o, y = Math.max(s, Math.min(r ? 1e3 : 1400, v, p));
...
function gl({ host: e }) {
  const t = _e(), n = Re(null), [r, i] = P(0), s = ue(() => ad(), []), o = ue(() => $l(), []), l = ue(() => Ql(Gl(), o.recent), [o.recent]), [m, v] = P(() => pl(o.activity) || "explorer"), [p, y] = P(s.primaryVisible), [T, z] = P(s.primaryWidth), [V, R] = P(o.secondaryTab ?? "chat"), [Y, B] = P(s.secondaryVisible), [we, G] = P(s.secondaryWidth), [M, I] = P(!!o.bottomPanelVisible), [he, se] = P(() => bl(o.bottomPanelTab)), [N, x] = P(s.markdownPreviewVisible), [Te, ne] = P(!!o.welcomeClosed), [ze, Me] = P(o.folder ? { name: o.folder.name, files: [] } : null), [De, We] = P(o.folder?.handleKey ?? null), [f, D] = P(() => _l(o.files)), [Q, ke] = P(() => (o.openEditorIds ?? []).filter((c) => (o.files ?? []).some((h) => h.id === c))), [ie, L] = P(() => o.activeFileId && (o.files ?? []).some((c) => c.id === o.activeFileId) ? o.activeFileId : null), [O, q] = P(o.query ?? ""), [S, $] = P({ lineNumber: 1, column: 1 }), [ce, ge] = P(null), [pe, g] = P({}), [X, Xe] = P(bo), [E, Ue] = P([]), [et, Ee] = P([]), [_, ae] = P(null), [re, me] = P(o.chatInput ?? ""), [Pe, He] = P(!1), [it, Z] = P(""), [be, Ae] = P([]), [le, Je] = P(null), [Ge, ot] = P(null), [Zt, pt] = P(!1), [tt, wn] = P(() => rd()), [d, A] = P([st]), [H, C] = P(() => Ri(Vn)), [F, ee] = P(null), [W, Le] = P(null), [xe, de] = P(""), [qe, fe] = P(""), [te, U] = P(""), [Ce, Ze] = P("failed"), [Ye, Oe] = P(null), [dt, j] = P("Ready"), [Mt, Gr] = P(() => l), Ke = Q.map((c) => f.find((h) => h.id === c)).filter(Boolean), J = ie ? f.find((c) => c.id === ie) ?? null : null, ga = ue(() => po({ files: f, openEditorIds: Q, activeFileId: ie, versions: pe, activeSelection: ce }), [ie, ce, pe, f, Q]), wa = ue(() => ({
    scope: X,
    removedAttachmentIds: E,
    selectedFileIds: []
  }), [X, E]), Wt = ue(() => Ao(ga, f, wa), [wa, ga, f]), ut = Go(f, { autoRefresh: !0, includeDirty: !0 }), ka = ue(() => et.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [et]), Qr = ue(() => [...Wt.attachments, ...ka], [Wt.attachments, ka]), xa = ue(() => d.find((c) => c.id === H) ?? st, [d, H]), oe = ho({ host: e, requestContext: Wt.parts, chatSettings: tt, selectedTarget: xa, setStatus: j }), bt = ue(
    () => [...oe.artifacts, ...be].sort((c, h) => h.createdAt - c.createdAt),
    [oe.artifacts, be]
  ...
  ve(() => {
    g((c) => {
      const h = {};
      for (const b of f) h[b.id] = c[b.id] ?? 1;
      return h;
    });
  }, [f]), ve(() => {
    Ue([]), Ee([]);
  ...
  const _r = w((c) => {
    const h = d.find((b) => b.id === c) ?? st;
    C(h.id), Ha(Vn, h.id), j(`Chat target: ${h.label}`);
  }, [d, j]), es = w((c) => {
    if (c.kind === "index-hit") {
      Ee((h) => h.filter((b) => b.id !== c.id)), j(`Removed project context: ${c.label}`);
      return;
    }
    ...
    const h = In(r), b = c.clientX, k = we, K = (je) => {
      const vt = k + (b - je.clientX);
    ...
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const h = In(r), b = c.clientX, k = T, K = (je) => {
      const vt = k + (je.clientX - b);
    ...
    Gr((h) => {
      const b = [c, ...h.filter((k) => k.path !== c.path)].slice(0, 10);
      return localStorage.setItem(Fr, JSON.stringify(b)), b;
    });
  }, []), Se = w((c, h) => {
    ke((b) => b.includes(c.id) ? b : [...b, c.id]), L(c.id), ne(!1), Oe(h ?? null), $({ lineNumber: h ?? 1, column: 1 });
  }, []), xn = w((c, h, b = {}) => {
    const k = b.primaryOnly === !1 ? h : ji(h), K = Hi(c, k);
    ...
    if (je.length > 0 && (D((Ie) => Dt(Ie, je)), b.openTabs !== !1 && (ke((Ie) => Array.from(/* @__PURE__ */ new Set([...Ie, ...je.map((On) => On.id)]))), b.activate !== !1))) {
      const Ie = je.find((On) => On.mission?.relPath === b.activatePath) ?? je[0];
      L(Ie.id), ne(!1), x(!0), Oe(null), $({ lineNumber: 1, column: 1 });
    }
    if (!b.quiet) {
      const Ie = b.openTabs === !1 ? "Updated" : "Opened";
    ...
    D((b) => Dt(b, [h])), Se(h), R("chat"), B(!0), x(!0), j(`Opened Atomek guide: ${c.title}`);
  }, [Se]), rs = w((c) => {
    if (!c.fileId) return;
    const h = f.find((b) => b.id === c.fileId);
    ...
    if (Fn(Ve, "open new files"))
      try {
        ...
        D((h) => Dt(h, c)), await Promise.all(c.map(async (h) => {
          const b = h.handle ? `file:${h.path}` : void 0;
          h.handle && b && await Ja(b, h.handle), ht({ name: h.name, path: h.path, kind: "file", handleKey: b, at: Date.now() });
    ...
    if (!ie) return;
    let h = !1;
    D((b) => b.map((k) => k.id !== ie || k.content === c ? k : (h = !0, { ...k, content: c, dirty: !0 }))), h && xt(ie);
  }, [ie, xt]), Pt = w(async (c) => {
    if (c.source !== "mission") return wi(c);
    const h = c.mission?.relPath, b = c.mission?.rootPath;
    if (!h)
      throw new Error("Mission file save requires mission metadata. Recreate or refresh the mission pack.");
    if (vl(h), b && e.missions?.write)
      return await e.missions.write({ rootPath: b, files: [{ path: h, content: c.content }] }), { ...c, dirty: !1 };
    if (c.mission?.handle)
    ...
    if (J)
      try {
        const c = await Pt(J);
        D((h) => h.map((b) => b.id === c.id ? c : b)), j(`Saved ${c.name}`);
      } catch (c) {
    ...
    if (!h) return null;
    const b = await Pt(h);
    return D((k) => k.map((K) => K.id === b.id ? b : K)), b;
  }, [f, Pt]), Na = w(async () => {
...
`,
    ...
    return Ee(k), k.length === 0 ? (j(ut.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), h) : (b.mode !== "hybrid" && b.reason ? j(b.reason) : j(`${k.length} project context hit${k.length === 1 ? "" : "s"} · ${b.reason ?? "hybrid retrieval"}`), [
      ...h,
      ...rc(k)
    ]);
  }, [Wt.parts, X, tt, e, ut]), An = w(() => {
    if (!J && Ke.length === 0) {
    ...
    B(!0), R("chat"), j("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const b = await mt(h), k = await oe.askAgent(h, { requestContext: b });
    ...
    if (c.kind === "folder") {
      const b = f.filter((k) => k.path === c.path || k.path.startsWith(`${c.path}/`));
      if (ze?.name === c.name || b.length > 0) {
        b.length > 0 && (Me((k) => k?.name === c.name ? k : { name: c.name, files: b }), D((k) => Dt(k, b))), v("explorer"), y(!0), L(null), ne(!1), ht({ ...c, at: Date.now() }), j(`Opened recent folder ${c.name}`);
        return;
      }
    }
    const h = f.find((b) => b.path === c.path || b.name === c.name);
    if (h) {
    ...
    try {
      ...
      const b = await Wn(c.handleKey);
      if (!b || !await Sn(b, "readwrite")) {
      ...
      D((K) => Dt(K, k)), Se(k[0]), ht({ ...c, at: Date.now() }), j(`Reopened ${k[0].name}`);
    } catch (b) {
      j(`Open recent failed: ${b instanceof Error ? b.message : String(b)}`);
    }
    ...
    try {
      const h = await e.skills.get(c.id), b = h.body.length > 4500 ? `${h.body.slice(0, 4500)}
...
`)), B(!0), R("chat"), j(`Attached skill ${h.title} to chat input`);
    ...
    const h = oe.messages.findIndex((k) => k.id === c.id), b = oe.messages.slice(0, h < 0 ? void 0 : h).reverse().find((k) => k.role === "user");
    if (!b?.body.trim()) {
      j("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const k = await mt(b.body);
      await oe.askAgent(b.body, { requestContext: k });
    })();
  }, [oe, mt]), ls = w((c) => {
    oe.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const h = Dn(f, Rn(c.title || c.kind)), b = {
    ...
    D((k) => [...k, b]), Se(b), j(`Opened ${c.title} as editable file`);
  }, [f, Se]), Cn = w(() => {
...
`), K = {
    ...
    const b = xc({
      ...
      edits: b.edits.map(Bl),
      skipped: b.skipped
    };
    return b.edits.length > 1 ? (ot(k), Je(null), j(`Previewing AI workspace patch for ${b.edits.length} files`), !0) : b.edits.length === 1 ? (Je(k.edits[0]), ot(null), j(`Previewing AI patch for ${k.edits[0].fileName}`), !0) : (j(b.skipped.length > 0 ? `No applicable edit found. ${b.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [J, pe, f]), za = w((c, h) => {
    const b = {
    ...
    Ae((k) => [b, ...k]), B(!0), R("outputs"), I(!0), se("output"), j(`Saved local job output: ${c}`), (h.includes("```diff") || h.includes("--- a/")) && Nt(c, h);
  }, [Nt]), hs = w(() => {
    const c = re.trim();
    if (!c) return;
    me("");
    const h = Vl(c);
    (async () => {
      const b = await mt(c), k = await oe.askAgent(c, { requestContext: b });
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", k.body) || j("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    U(b), de(""), fe(""), Ze("failed"), I(!0), se("terminal"), j(h.commands.length > 0 ? `Manual check ready: copy ${h.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [f]), fs = w((c) => {
    c.trim() && (Ht(c), U(c), j(`Manual check command copied: ${c}`));
  }, []), ps = w(() => {
    Le((c) => {
      if (!c) return c;
      const h = Fc(c, xe), b = h.commands.at(-1)?.command ?? "";
      return b && U(b), h;
    }), de("");
  }, [xe]), bs = w(() => {
    if (!W || !te.trim()) {
    ...
    c.content !== le.originalContent && !window.confirm(`${le.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (D((h) => h.map((b) => b.id === le.fileId ? { ...b, content: le.proposedContent, dirty: !0 } : b)), xt(le.fileId), ee(`AI edit applied to ${c.name}. Save All to persist it to disk.`), Je(null), It(`AI edit applied to ${c.name}`), j(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [xt, f, le, It]), gs = w(() => {
    if (!le) return;
    ...
    D((b) => [...b, h]), Se(h), Je(null), j(`Opened proposed edit as ${c}`);
  }, [J?.language, f, Se, le]), ws = w(() => {
    if (!Ge) return;
    const c = new Map(Ge.edits.map((b) => [b.fileId, b]));
    f.some((b) => {
      const k = c.get(b.id);
      return k && b.content !== k.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (D((b) => b.map((k) => {
      const K = c.get(k.id);
      return K ? { ...k, content: K.proposedContent, dirty: !0 } : k;
    })), c.forEach((b, k) => xt(k)), ee(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), ot(null), It(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), j(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [xt, f, Ge, It]), ks = w(() => {
    if (!Ge) return;
    const c = [...f], h = Ge.edits.map((b) => {
      const k = Dn(c, `${Rn(b.fileName)}-ai-edit`), K = f.find((je) => je.id === b.fileId), Ne = {
    ...
    D((b) => [...b, ...h]), h[0] && Se(h[0]), ot(null), j(`Opened ${h.length} proposed edit file${h.length === 1 ? "" : "s"}`);
  }, [f, Se, Ge]), Aa = w((c) => {
    if (c === "explain") {
  ...
  return ve(() => {
    const c = (h) => {
      (h.metaKey || h.ctrlKey) && (h.key.toLowerCase() === "o" && (h.preventDefault(), Vt()), h.key.toLowerCase() === "s" && (h.preventDefault(), Nn()), h.key.toLowerCase() === "w" && (h.preventDefault(), ie && Tn(ie)), h.key.toLowerCase() === "b" && (h.preventDefault(), y((k) => !k)), h.shiftKey && h.key.toLowerCase() === "f" && (h.preventDefault(), v("search"), y(!0)), (h.key.toLowerCase() === "k" || h.key.toLowerCase() === "p") && (h.preventDefault(), He(!0)));
    };
    ...
    const c = { primaryVisible: p, primaryWidth: T, secondaryVisible: Y, secondaryWidth: we, markdownPreviewVisible: N };
    ...
    return (async () => {
      try {
        ...
        const b = await Oa(h);
        if (c) return;
        We(o.folder?.handleKey ?? null), Me(b), D((k) => nd(k, b.files)), ke((k) => k.filter((K) => b.files.some((Ne) => Ne.id === K) || f.some((Ne) => Ne.id === K))), j(`Restored ${b.name} (${b.files.length} text files indexed)`);
      } catch (h) {
  ...
  }, [m, ie, he, M, re, De, f, ze, Q, O, Mt, V, Te]), ve(() => {
    localStorage.setItem(Er, JSON.stringify(tt));
  }, [tt]), /* @__PURE__ */ u(
    "div",
    {
      ref: n,
      className: `workbench-workbench ${p ? "" : "no-primary"} ${Y ? "" : "no-secondary"} ${M ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${T}px`, "--workbench-secondary-width": `${we}px` },
      children: [
        /* @__PURE__ */ a(
          wl,
          {
        ...
        /* @__PURE__ */ u("main", { className: "workbench-editor-area", children: [
          ...
          /* @__PURE__ */ u("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: J ? /* @__PURE__ */ u("div", { className: J.language === "markdown" && N ? "workbench-editor-split" : "workbench-editor-single", children: [
              ...
              J.language === "markdown" && N && /* @__PURE__ */ a(Sl, { content: J.content })
            ] }) : Kt ? /* @__PURE__ */ a(
              Rl,
              {
                host: e,
            ...
            ) : kn ? /* @__PURE__ */ a(Ll, { host: e, openFile: Vt, openFolder: Pn, newFile: zn, recent: Mt, reopenRecent: Ln, setStatus: j, openControlTower: () => {
              v("computer"), y(!0);
            }, openChat: () => {
              R("chat"), B(!0);
        ...
        Pe && /* @__PURE__ */ a(
          Ol,
          {
            ...
            commands: [
              ...
              ...Mt.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => {
                Ln(c);
              } })),
              ...
              { label: "Help: Show Agent Team", detail: "Open the Atomek Resource Fabric landing page", run: () => {
                L(null), ne(!1), v("computer"), y(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: p ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => y((c) => !c) },
              ...
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => It("Manual check requested from command palette") },
...
function xl(e) {
  const t = _e(), n = !e.folder, r = ue(() => Pl(e.files, e.folder?.name), [e.files, e.folder?.name]), [i, s] = P(() => /* @__PURE__ */ new Set());
  ve(() => {
    const l = qr(r);
    s((m) => {
      const v = new Set(m);
      return l.forEach((p) => v.add(p)), v;
    });
  }, [r]);
  const o = w((l) => {
    s((m) => {
  ...
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      n ? /* @__PURE__ */ u(Qe, { children: [
        ...
        /* @__PURE__ */ a("input", { className: "workbench-input", value: e.query, onChange: (l) => e.setQuery(l.target.value), placeholder: t("explorer.searchFiles") }),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(qt, { size: 12 }),
          " ",
          t("explorer.openEditors")
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: t("explorer.noOpenEditors") }) : e.openEditors.map((l) => /* @__PURE__ */ a(ya, { file: l, active: l.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(l), label: l.name, detail: l.path }, l.id)),
        /* @__PURE__ */ u("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(qt, { size: 12 }),
          " ",
          e.folder?.name ?? t("app.workspace")
        ] }),
        r.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: t("explorer.noReadableFiles") }) : Yr(r, e.activeFileId, e.openWorkbenchFile, i, o)
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: t("explorer.recent") }),
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: t("explorer.noRecentFolders") }) : e.recent.map((l) => /* @__PURE__ */ u("button", { className: "workbench-tree-row", onClick: () => {
        e.reopenRecent(l);
      }, title: l.path, children: [
        l.kind === "file" ? /* @__PURE__ */ a(aa, { size: 14 }) : /* @__PURE__ */ a(er, { size: 14 }),
...
function Pl(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), i = (o, l, m = n) => {
    const v = r.get(o);
    if (v) return v;
    const p = { name: l, path: o, children: [] };
  ...
  e.forEach((o) => {
    ...
    m.slice(0, -1).forEach((y) => {
      p = p ? `${p}/${y}` : y, v = i(p, y, v).children;
    }), v.push({ name: m.at(-1) ?? o.name, path: l, file: o, children: [] });
  });
...
function Yr(e, t, n, r, i, s = 0) {
  return e.map((o) => {
    if (o.file)
      return /* @__PURE__ */ a(ya, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: s, label: o.name }, o.file.id);
    const l = r.has(o.path);
    return /* @__PURE__ */ u("div", { children: [
      /* @__PURE__ */ u("button", { className: "workbench-folder-row", style: { "--workbench-depth": s }, onClick: () => i(o.path), title: l ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        l ? /* @__PURE__ */ a(qt, { size: 12 }) : /* @__PURE__ */ a(qt, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ a(ia, { size: 14 }) : /* @__PURE__ */ a(er, { size: 14 }),
      ...
      l ? Yr(o.children, t, n, r, i, s + 1) : null
    ] }, o.path);
  });
}
...
function Ll({
  ...
  }), [y]), [z, V] = P(() => y("mission.default.podLocal.name")), [R, Y] = P(() => y("mission.default.podLocal.goal")), [B, we] = P(!1), [G, M] = P(!1), [I, he] = P(null), [se, N] = P([]), [x, Te] = P(!1), [ne, ze] = P(null), [Me, De] = P(null), [We, f] = P("pod-local"), D = w(async () => {
    Te(!0), De(null);
    try {
      ...
      he(g ?? null), N(X), g && o(`Agent team loaded · ${g.resources.length} resources · ${g.warnings.length} warnings`);
    } catch (g) {
...
`]
      ...
      await e.missions.write({
        rootPath: E.rootPath,
        files: Ee
      }), ze(E), Et(Ue), p(Ue, Ee), o(`Mission created: ${E.rootPath}`), l();
    } catch (E) {
      o(`Mission create failed: ${E instanceof Error ? E.message : String(E)}`);
    } finally {
      Te(!1);
    }
  }, [R, I, e.missions, z, l, p, o, We]), ie = w((g) => {
    f(g);
    const X = T[g];
    B || V(X.name), G || Y(X.goal);
  }, [G, B, T]), L = zl(I), O = Ni(I), q = Ti(I), S = Al(I), $ = I?.warnings ?? [], ce = Ai(I), ge = jt(I, kt(R, I, We)), pe = bn(R, I, ge.id);
  return /* @__PURE__ */ u("div", { className: "workbench-welcome workbench-control-home", children: [
    ...
    /* @__PURE__ */ u("section", { className: "workbench-control-grid", children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-doc-grid", children: ll.map((g) => /* @__PURE__ */ u("button", { className: "workbench-doc-card", onClick: () => v(g), title: g.summary, children: [
      ...
      /* @__PURE__ */ u("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: pe.map((g, X) => /* @__PURE__ */ u("button", { className: `workbench-task-card ${g.status}`, onClick: () => Y(g.prompt), children: [
          /* @__PURE__ */ a("span", { children: X + 1 }),
          /* @__PURE__ */ a("strong", { children: g.title }),
...
function Xl({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: i }) {
  const s = _e(), o = ue(() => Cl(e, t), [e, t]), l = ue(() => {
    ...
    return o.forEach((v) => m.set(v.file.id, [...m.get(v.file.id) ?? [], v])), Array.from(m.values()).slice(0, 50);
  }, [o]);
  return /* @__PURE__ */ u("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ u("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? l.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: s("search.noMatches") }) : l.map((m) => {
        ...
        return /* @__PURE__ */ u("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(ya, { file: v, active: v.id === i, onOpen: () => r(v) }),
          m.slice(0, 5).map((p) => /* @__PURE__ */ u("button", { className: "workbench-search-hit", onClick: () => r(v, p.lineNumber), title: p.line, children: [
            /* @__PURE__ */ a("span", { className: "workbench-search-line", children: p.lineNumber }),
            /* @__PURE__ */ a("span", { children: p.line })
          ] }, `${v.id}-${p.lineNumber}-${p.line}`)),
...
function Ml(e) {
  const t = _e(), [n, r] = P(!1), [i, s] = P(""), [o, l] = P([]), m = Re(null), v = Re(null), p = Re(null), [y, T] = P({ top: 0, right: 0 }), z = ue(
    () => e.chatThreads.map((N) => ({
      kind: "atomek-thread",
      id: `thread:${N.id}`,
      title: N.title,
      lastActivityAt: N.lastMessageAt ?? N.updatedAt ?? 0,
      thread: N
    })),
    [e.chatThreads]
  ), V = ue(() => o.map((N) => {
    const x = e.chatTargets.find((Te) => Te.id === N.targetId);
    return x ? {
      kind: "pod-agent",
      id: `agent:${N.targetId}`,
      title: x.label,
      subtitle: N.preview || x.description || `${N.messageCount} messages`,
      lastActivityAt: N.lastActivityAt,
      targetId: N.targetId
    } : null;
  }).filter((N) => N !== null), [o, e.chatTargets]), R = ue(() => [...z, ...V].sort((N, x) => x.lastActivityAt - N.lastActivityAt), [z, V]), Y = ue(() => e.selectedChatTarget.kind === "atomek-ai" && e.chatThread ? R.find((N) => N.kind === "atomek-thread" && N.thread.id === e.chatThread?.id) ?? null : e.selectedChatTarget.kind !== "atomek-ai" ? R.find((N) => N.kind === "pod-agent" && N.targetId === e.selectedChatTarget.id) ?? null : null, [R, e.selectedChatTarget, e.chatThread]), B = w((N, x) => x ? `${N.title} ${N.kind === "pod-agent" ? N.subtitle : ""}`.toLowerCase().includes(x) : !0, []), we = i.trim().toLowerCase(), G = Y ? B(Y, we) : !1, M = ue(() => R.filter((N) => N.id !== Y?.id).filter((N) => B(N, we)), [R, Y, we, B]), I = w(() => {
    l(io());
  }, []), he = w((N) => {
    r(!1), N.kind === "atomek-thread" ? (e.selectedChatTarget.kind !== "atomek-ai" && e.selectChatTarget(st.id), e.selectThread(N.thread.id)) : e.selectChatTarget(N.targetId), e.tab !== "chat" && e.setTab("chat");
  }, [e]), se = w((N) => {
    window.confirm(`Delete conversation "${N.title}"?`) && (N.kind === "atomek-thread" ? e.deleteThread(N.thread.id) : (oo(N.targetId), I()));
  }, [e, I]);
  return ve(() => {
    if (!n) {
      s("");
    ...
    N(), window.setTimeout(() => p.current?.focus(), 0);
    const x = (ne) => {
      const ze = ne.target;
      ze && (v.current?.contains(ze) || m.current?.contains(ze) || r(!1));
    }, Te = (ne) => {
      ne.key === "Escape" && r(!1);
    };
    return window.addEventListener("resize", N), window.addEventListener("scroll", N, !0), window.addEventListener("mousedown", x), window.addEventListener("keydown", Te), () => {
      window.removeEventListener("resize", N), window.removeEventListener("scroll", N, !0), window.removeEventListener("mousedown", x), window.removeEventListener("keydown", Te);
    };
              ...
              M.length > 0 ? /* @__PURE__ */ u(Qe, { children: [
                ...
                M.map((N) => /* @__PURE__ */ u(
                  "div",
                  {
                    ...
                    children: [
                      /* @__PURE__ */ u("span", { className: "workbench-history-portal-title", children: [
                        N.kind === "pod-agent" ? /* @__PURE__ */ a("span", { className: "workbench-history-portal-badge", children: t("history.agent") }) : null,
                        N.title
                      ] }),
                      /* @__PURE__ */ a("small", { children: ea(N.lastActivityAt) }),
                      /* @__PURE__ */ a(
                        "button",
                        {
                          className: "workbench-history-portal-delete",
                          onClick: (x) => {
                            x.stopPropagation(), se(N);
                          },
                          title: t("history.deleteConversation"),
                          "aria-label": t("history.deleteConversation"),
                          children: /* @__PURE__ */ a(Ca, { size: 13 })
...
function Jr({ body: e }) {
  const t = ue(() => yl(e), [e]), [n, r] = P(null), i = w((s, o) => {
    (async () => await Ht(o) && (r(s), window.setTimeout(() => r((m) => m === s ? null : m), 1200)))();
  }, []);
...
function Wl(e) {
  const t = _e(), n = e.selectedChatTarget.available, r = e.selectedChatTarget.kind === "atomek-ai", i = e.chatInput.trim().length > 0 && !e.busy && n, s = Re(null), [o, l] = P(!0), [m, v] = P(!1), p = Re(null), y = Re(null), T = Re(null), z = Re(null), [V, R] = P(!1), [Y, B] = P(0), we = typeof window < "u" && !!(window.SpeechRecognition || window.webkitSpeechRecognition), G = w(() => {
    const f = T.current;
    try {
      f?.stop?.();
    } catch {
    }
    T.current = null, z.current !== null && (window.clearInterval(z.current), z.current = null), R(!1), B(0);
  }, []), M = w(() => {
    if (!we || V) return;
  ...
  }, he = () => p.current?.removeAttribute("open"), se = () => y.current?.removeAttribute("open"), N = (f) => {
  ...
  ve(() => {
    const f = s.current;
    f && (o ? (f.scrollTop = f.scrollHeight, v(!1)) : v(!0));
  }, [o, Te]);
  const ne = w(() => {
    const f = s.current;
    if (!f) return;
    ...
    const f = s.current;
    f && (f.scrollTop = f.scrollHeight, l(!0), v(!1));
  }, []), Me = w((f) => {
    Ht(f.body);
    ...
    /* @__PURE__ */ u("div", { ref: s, className: "workbench-chat-transcript", onScroll: ne, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ u("div", { children: [
        ...
        f.role === "assistant" && f.status !== "streaming" && f.status !== "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(f), disabled: e.busy, title: t("chat.regenerate"), "aria-label": t("chat.regenerate"), children: /* @__PURE__ */ a(Xt, { size: 14 }) })
        ] }) : null,
        f.role === "assistant" && f.status === "error" ? /* @__PURE__ */ u("div", { className: "workbench-chat-message-actions", children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(f), disabled: e.busy, title: t("chat.retry"), "aria-label": t("chat.retry"), children: /* @__PURE__ */ a(Xt, { size: 14 }) })
        ] }) : null
      ] }, f.id)),
        ...
        /* @__PURE__ */ u("div", { className: "workbench-chat-toolbar atomek-input", children: [
          /* @__PURE__ */ u("details", { ref: p, className: "workbench-chat-attach", children: [
            ...
            /* @__PURE__ */ u("div", { className: "workbench-chat-attach-menu", role: "menu", children: [
              /* @__PURE__ */ a("button", { onClick: () => N("active-file"), disabled: !e.activeFile, title: t("chat.useActiveFile"), children: t("chat.activeFile") }),
              /* @__PURE__ */ a("button", { onClick: () => N("active-selection"), disabled: !e.activeFile, title: t("chat.useSelection"), children: t("chat.selection") }),
              /* @__PURE__ */ a("button", { onClick: () => N("open-editors"), title: t("chat.useOpenEditors"), children: t("chat.openEditors") }),
              /* @__PURE__ */ a("button", { onClick: () => N("indexed-project"), title: t("chat.useIndexedProject"), children: t("chat.indexedProject") }),
              /* @__PURE__ */ a("button", { onClick: () => N("none"), title: t("chat.noContext"), children: t("chat.noContext") })
            ] })
          ] }),
...
function Fl({ host: e, appName: t, currentVersion: n }) {
  const r = e.apps, [i, s] = P(null), [o, l] = P(!1), [m, v] = P(null), p = w(async () => {
    if (!r?.checkUpdate) {
      v("Update checks need a newer Tytus OS build.");
      return;
    }
    l(!0), v(null);
    try {
      const V = await r.checkUpdate();
      s(V), V.error && v(V.error);
    } catch (V) {
      v(V instanceof Error ? V.message : String(V));
    } finally {
      l(!1);
    }
  }, [r]);
  ...
  const y = w(async () => {
    if (!r?.updateSelf) {
      v("Update needs a newer Tytus OS build.");
      return;
    }
    l(!0), v(null);
    try {
      const V = await r.updateSelf();
      s(V), v(V.error ?? `${t} updated. Close and reopen the app to load the new bundle.`);
    } catch (V) {
      v(V instanceof Error ? V.message : String(V));
    } finally {
      l(!1);
    }
  }, [r, t]), T = i?.latestVersion ?? n, z = !!i?.updateAvailable;
...
function Rl(e) {
  ...
  }, [e.chatSettings.gatewayPreference, e.host.ai]), ve(() => {
    const T = new AbortController();
    return (async () => {
      const V = Mc(e.host);
      if (V) {
  ...
  const v = (T) => {
    e.onChange({ ...e.chatSettings, gatewayPreference: T });
  }, p = (T) => {
    e.onChange({ ...e.chatSettings, model: T });
  }, y = (T) => {
    e.onChange({ ...e.chatSettings, embeddingModel: T });
  };
    ...
    /* @__PURE__ */ u("div", { className: "workbench-settings-body", children: [
      ...
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-chat-models", children: t.map((T) => /* @__PURE__ */ a("option", { value: T.id, children: T.gatewayLabel }, `${T.gatewayLabel}:${T.id}`)) })
        ] }),
      ...
      /* @__PURE__ */ u("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ u("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-embedding-models", children: s.map((T) => /* @__PURE__ */ a("option", { value: T.id, children: T.gatewayLabel }, `${T.gatewayLabel}:${T.id}`)) })
        ] }),
  ...
  return /* @__PURE__ */ u("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((p) => /* @__PURE__ */ u("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ u("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void Ht(p.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => s(p), children: "Open as file" }),
        o ? /* @__PURE__ */ a(
          "button",
          {
            className: Ba(p.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(p),
            disabled: !l,
            children: Ba(p.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        p.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(p.id), children: v("chat.delete") }) : null
      ] }),
      /* @__PURE__ */ a(Jr, { body: p.body })
    ] }, p.id))
  ] });
}
...
function Br({
  ...
  const m = _e(), [v, p] = P([]), [y, T] = P([]), [z, V] = P(null), [R, Y] = P(!1), [B, we] = P(null), [G, M] = P("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [I, he] = P(null), [se, N] = P([]), [x, Te] = P(() => Vi()), [ne, ze] = P([]), [Me, De] = P([]), [We, f] = P([]), [D, Q] = P([]), ke = Re([]), [ie, L] = P("task-execute"), [O, q] = P("mission"), S = se.find((d) => d.status === "running" || d.status === "canceling") ?? se[0] ?? null, $ = se.some((d) => d.status === "running" || d.status === "canceling"), ce = l === "dock", ge = x?.goal || G || "", pe = ue(() => jt(z, kt(ge, z, x?.teamPresetId)), [x?.teamPresetId, ge, z]), g = ue(() => bn(ge, z, pe.id), [ge, pe.id, z]), X = g.find((d) => d.id === ie) ?? g[1] ?? g[0] ?? null, Xe = ue(() => X ? pe.assignments.find((d) => d.role === X.role) ?? pe.assignments.find((d) => d.role === "implementer") ?? pe.assignments[0] ?? null : null, [pe.assignments, X]), E = ue(() => !Xe || !z ? null : z.resources.find((d) => d.id === Xe.resourceId) ?? null, [z, Xe]), Ue = ue(() => new Map(D.map((d) => [d.path, d])), [D]), et = w((d) => {
    if (!x) {
...
` : "");
    ...
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      p([]), T([]), V(null), we("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    Y(!0), we(null);
    try {
      ...
      p(d), T(A), V(H), ze(C), x?.rootPath && re(x), t(`Agent team loaded · ${d.length} tools · ${A.length} skills · ${C.length} missions${H ? ` · ${H.resources.length} resources` : ""}`);
    } catch (d) {
...
` : "") } : de), Le = new Set(o.filter((de) => de.source === "mission" && de.dirty && de.mission?.missionId === d.missionId).map((de) => de.mission?.relPath).filter(Boolean)), xe = W.filter((de) => !Le.has(de.path));
    ...
    De(F), Q(W), i(d, W, C.openFiles ? { activate: C.activate } : { openTabs: !1, activate: !1, quiet: !0 }), Et(d);
  }, [s, e.missions, Me, o, i, z]), be = w(async (d, A = {}) => {
    if (x)
    ...
    return Te(F), Et(F), De([ee]), await Z(F, C, [ee], { openFiles: A.openFiles ?? !0, activate: A.activate ?? !0 }), t(`Mission pack ready in ${F.rootPath ?? F.name}`), F;
    ...
    try {
      const H = (A ?? G).trim() || `Open ${d.label} from Atomek with current context.`, C = d.kind === "ai-cli" ? await be(H) : x;
  ...
  }, [s, _, be, e.local, G, x, t]), le = w(async () => {
    try {
      await be(G.trim(), { allowBrowserPicker: !0, openFiles: !0, activate: !0 }) || t("Mission folder setup skipped.");
    } catch (d) {
      t(`Mission folder setup failed: ${d instanceof Error ? d.message : String(d)}`);
    }
  }, [be, G, t]), Je = w(async (d, A, H, C = x, F) => {
    if (!C) return;
...
`)
      ...
      }), N((fe) => fe.map((te) => te.id === F ? { ...te, jobId: W.id, transcriptPath: Le } : te));
      const qe = (fe) => {
        N((te) => te.map((U) => U.id === F ? fe(U) : U));
      };
...
`);
      ...
      he(null), N((Le) => Le.map((xe) => xe.id === F ? {
        ...xe,
        status: "failed",
        finishedAt: Date.now(),
  ...
  }, [s, be, e.local, G, o, z, r, Je, X, t, Pe, Z]), ot = w(async (d, A) => {
    const H = Zn(d), C = Pi(d);
    if (!H) {
    ...
    const fe = [], te = (U) => {
      fe.push(U), N((Ce) => Ce.map((Ze) => Ze.id === Le ? { ...Ze, lines: [...Ze.lines, U].slice(-500) } : Ze));
    };
    try {
...
`);
      N((Oe) => Oe.map((dt) => dt.id === Le ? { ...dt, status: "complete", finishedAt: Date.now(), lines: [...dt.lines, "[Atomek] Pod task complete."].slice(-500) } : dt)), await Pe({ ...qe, status: "complete", finishedAt: Tt(), summary: `${W} completed`, transcriptPath: de }, ee), r(`${W} pod job`, Ye), await Je({ id: d.id, label: W, kind: "pod-agent", status: d.status }, Ye, 0, ee, de), t(`${W} completed mission task`);
    } catch (U) {
      const Ce = U instanceof Error ? U.message : String(U);
      te(`[FAIL] ${Ce}`), N((Ye) => Ye.map((Oe) => Oe.id === Le ? { ...Oe, status: "failed", finishedAt: Date.now(), lines: Oe.lines.slice(-500) } : Oe)), await Pe({ ...qe, status: "failed", finishedAt: Tt(), exitCode: -1, summary: Ce, transcriptPath: de }, ee).catch((Ye) => {
...
`);
  ...
  }, [s, be, e.daemon, G, o, z, r, Je, X, t, Pe, Z]), Zt = w(async (d) => {
    if (!d.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    N((A) => A.map((H) => H.id === d.id ? { ...H, status: "canceling", lines: [...H.lines, "[Atomek] Cancel requested…"] } : H)), Pe({
      id: d.id,
      jobId: d.jobId,
      toolId: d.toolId,
      label: d.label,
    ...
    try {
      ...
      N((H) => H.map((C) => C.id === d.id ? { ...C, status: "running", lines: [...C.lines, `[Atomek] Cancel failed: ${A instanceof Error ? A.message : String(A)}`] } : C)), t(`Local job cancel failed: ${A instanceof Error ? A.message : String(A)}`);
    }
...
`);
    ...
    t(H), Ht(A?.commandPreview ?? A?.deepLink ?? H);
  }, [t]), wn = w(async () => {
    try {
      if ($) {
      ...
      if (E.kind === "local-cli") {
        const H = (typeof E.metadata?.id == "string" ? E.metadata.id : "") || E.id.replace(/^local-cli\./, ""), C = v.find((F) => F.id === H || F.command === H || F.label.toLowerCase() === E.label.toLowerCase());
        if (!C) {
  ...
  }, [n, $, x, Ae, Ge, ot, Xe, E, X, t, tt, y, v, pt, Z]);
    ...
    /* @__PURE__ */ u("div", { className: ce ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      ...
      O === "mission" ? /* @__PURE__ */ u(Qe, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ u("div", { className: `workbench-team-assignment-summary ${pe.readiness}`, children: [
            /* @__PURE__ */ a("strong", { children: pe.label }),
            /* @__PURE__ */ u("span", { children: [
              pe.readiness,
              " · ",
              pe.bestFor
          ...
          pe.assignments.map((d) => /* @__PURE__ */ u("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ u("div", { children: [
...
`) || "[waiting for output]" })
        ...
        We.length ? /* @__PURE__ */ u(Qe, { children: [
          ...
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: We.slice(0, ce ? 4 : 8).map((d) => /* @__PURE__ */ u("div", { className: "workbench-run-history-row", children: [
            ...
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void Ht(d.transcriptPath ?? d.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, d.id)) })
        ] }) : null
      ] }) : null,
      O === "setup" ? /* @__PURE__ */ u(Qe, { children: [
        ...
        /* @__PURE__ */ u("div", { className: "workbench-computer-list", children: [
          v.length === 0 && !R ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
