 ...
 */
const Xt = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Qt = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, a, i) => i ? i.toUpperCase() : a.toLowerCase()
), Ge = (e) => {
  const t = Qt(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, at = (...e) => e.filter((t, a, i) => !!t && t.trim() !== "" && i.indexOf(t) === a).join(" ").trim(), Gt = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const rn = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], ve = x("chevron-down", rn);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const Wn = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Tn() {
  const e = Ve();
  return !!(e?.showOpenFilePicker && e.showDirectoryPicker);
}
async function Rn() {
  const e = Ve()?.showOpenFilePicker;
  if (!e) return ft(!1);
  const t = await e({ multiple: !0 }), a = await Promise.all(t.map((i) => mt(i, i.name, "local-file")));
  return He(a.filter(Boolean));
}
async function Dn() {
  const e = Ve()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await ft(!0) };
  const t = await e({ mode: "readwrite" }), a = [];
  return await pt(t, t.name, a, 0, 320), { name: t.name, handle: t, files: He(a) };
}
function Ve() {
  const e = window;
...
async function pt(e, t, a, i, o) {
  if (!(a.length >= o || i > 8))
    for await (const l of e.values()) {
      if (a.length >= o) break;
      if (l.kind === "directory") {
        if (Wn.has(l.name)) continue;
        await pt(l, `${t}/${l.name}`, a, i + 1, o);
        continue;
      }
      const h = l, s = await mt(h, `${t}/${h.name}`, "local-folder");
      s && a.push(s);
    }
}
function ft(e) {
  return new Promise((t) => {
    ...
    a.type = "file", a.multiple = !0, a.style.display = "none", e && a.setAttribute("webkitdirectory", ""), a.onchange = async () => {
      ...
      a.remove(), t(He(o.filter(Boolean)));
    }, document.body.append(a), a.click();
  });
}
async function Oe(e) {
  if (!e.handle?.createWritable)
    return Bn(e), { ...e, dirty: !1 };
...
`).map((s) => `<tr>${s.split("|").filter(Boolean).map(
      (u, f) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${f % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${u.trim()}</td>`
    ).join("")}</tr>`).join("");
...
const Vn = 8e3, Hn = 5, Ye = (e, t = Vn) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, qn = (e, t) => {
  const a = [];
  e && a.push({
    kind: "file",
  ...
  for (const i of t.slice(0, Hn))
    e && i.id === e.id || a.push({
      kind: "file",
      title: `Open editor: ${i.path}`,
      text: Ye(i.content, 4e3)
    });
...
}, Kn = (e, t = Un) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Ze = (e, t) => {
  const a = e.split(`
`).map((i) => i.replace(/^#+\s*/, "").trim()).find(Boolean);
  return a ? a.length > 80 ? `${a.slice(0, 77)}...` : a : t;
}, Xn = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", et = (e) => ({
  id: e.id,
  title: e.title,
  kind: Xn(e.kind),
  body: e.body,
...
function Gn({ host: e, activeFile: t, openEditors: a, chatSettings: i, setStatus: o }) {
  const l = e.ai, [h, s] = N(null), [p, u] = N([]), [f, C] = N([]), [W, B] = N([]), [he, j] = N(Je), [Ne, G] = N(!1), z = qt(!0), re = q(() => qn(t, a), [t, a]), ie = v(async () => {
    if (!l) {
      j(Je);
      return;
    }
    try {
      ...
      j({
        ...
        source: "none",
        label: "Tytus AI unavailable",
        reason: y instanceof Error ? y.message : String(y)
      });
    }
  }, [l]), V = v(async (y) => {
    if (l)
      try {
        const g = await l.listArtifacts({ threadId: y });
        if (!z.current) return;
        C(g.map(et));
      } catch (g) {
        o(`AI artifacts unavailable: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [l, o]), ue = v(async () => {
    if (l)
      try {
        const g = (await l.listThreads({ workspaceKey: ke, status: "active" }))[0] ?? await l.createThread({ workspaceKey: ke, title: "Atomek chat" });
        if (!z.current) return;
        s(g);
        const E = await l.listMessages(g.id);
        if (!z.current) return;
        u(E.map(ze).filter(Boolean)), await V(g.id);
      } catch (y) {
        o(`AI chat unavailable: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [l, V, o]);
  ne(() => (z.current = !0, ie(), ue(), () => {
    z.current = !1;
  }), [ue, ie]);
  const oe = v(async () => {
    if (!l) return null;
    if (h) return h;
    const y = await l.createThread({ workspaceKey: ke, title: "Atomek chat" });
    return z.current && s(y), y;
  }, [l, h]), k = v(async () => {
    if (!l) return;
    const y = await l.createThread({ workspaceKey: ke, title: "Atomek chat" });
    s(y), u([]), C([]), B([]), o("New AI chat created");
  }, [l, o]), L = v(async (y) => {
    if (!l) return null;
    try {
      const g = await oe();
      if (!g) return null;
      const E = await l.createArtifact({
        threadId: g.id,
        messageId: y.messageId ?? null,
        title: y.title?.trim() || Ze(y.body, "Atomek artifact"),
        kind: y.kind ?? "markdown",
        body: y.body
      }), R = et(E);
      return C((K) => [R, ...K.filter((Y) => Y.id !== R.id)]), o(`Saved AI artifact: ${R.title}`), R;
    } catch (g) {
      return o(`Save artifact failed: ${g instanceof Error ? g.message : String(g)}`), null;
    }
  }, [l, oe, o]), Ce = v(async (y) => {
    if (l)
      try {
        await l.deleteArtifact(y), C((g) => g.filter((E) => E.id !== y)), o("Deleted AI artifact");
      } catch (g) {
        o(`Delete artifact failed: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [l, o]), le = v(async (y) => {
    if (!l) return null;
    try {
      const g = await l.writeMemory({
        title: y.title?.trim() || Ze(y.body, "Atomek memory"),
        body: y.body,
        metadata: {
          source: "atomek",
          messageId: y.messageId ?? null
      ...
      return B((E) => [g, ...E.filter((R) => R.id !== g.id)].slice(0, 5)), o(`Remembered: ${g.title}`), g;
    } catch (g) {
      return o(`Remember failed: ${g instanceof Error ? g.message : String(g)}`), null;
    }
  }, [l, o]), M = v(async (y) => {
    if (!l) return [];
    const g = await l.searchMemory({ query: y, limit: 5 });
    return z.current && B(g), g;
  }, [l]), U = v(async (y) => {
    const g = y.trim();
    if (!(!g || !l)) {
      G(!0);
      try {
        const E = await oe();
        if (!E) return;
        const R = await M(g).catch(() => []), K = Qn(R), Y = K ? [...re, K] : re;
        let ce = null;
        for await (const P of l.sendMessage({
          threadId: E.id,
          body: g,
          ...
          if (P.type === "message_created") {
            const F = ze(P.message);
            if (!F) continue;
            F.role === "assistant" && (ce = F.id), u((_) => [..._.filter((J) => J.id !== F.id), F]);
          }
          if (P.type === "token" && (ce = P.messageId, u((F) => F.map(
            (_) => _.id === P.messageId ? { ..._, body: P.body, status: "streaming" } : _
          ))), P.type === "message_updated" || P.type === "done") {
            const F = ze(P.message);
            if (!F) continue;
            u((_) => _.map((J) => J.id === F.id ? F : J)), F.gatewayLabel && o(`AI answered via ${F.gatewayLabel}`);
          }
          P.type === "run_failed" && (o(`AI failed: ${P.error}`), ce && u((F) => F.map(
            (_) => _.id === ce ? { ..._, status: "error", error: P.error, body: P.error } : _
          )));
        }
        ie();
      } catch (E) {
        o(`AI failed: ${E instanceof Error ? E.message : String(E)}`);
      } finally {
        G(!1);
      }
    }
  }, [l, i.gatewayPreference, i.model, re, oe, M, ie, o]);
  return {
    aiStatus: he,
    artifacts: f,
    busy: Ne,
...
const Yn = Ut(() => import("./WorkbenchMonacoEditor-BaNVDl1J.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Jn = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, wt = "tytus.workspace.recent", gt = "tytus.workspace.layout", kt = "tytus.atomek.chatAiSettings", ye = {
  gatewayPreference: "auto",
  model: ""
};
function Zn({ host: e }) {
  const t = q(() => Fa(), []), [a, i] = N("explorer"), [o, l] = N(t.primaryVisible), [h, s] = N(t.primaryWidth), [p, u] = N("chat"), [f, C] = N(t.secondaryVisible), [W, B] = N(t.secondaryWidth), [he, j] = N(!1), [Ne, G] = N("problems"), [z, re] = N(t.markdownPreviewVisible), [ie, V] = N(!1), [ue, oe] = N(null), [k, L] = N([]), [Ce, le] = N([]), [M, U] = N(null), [y, g] = N(""), [E, R] = N({ lineNumber: 1, column: 1 }), [K, Y] = N(""), [ce, P] = N(!1), [F, _] = N(""), [J, Ae] = N([]), [I, Z] = N(null), [X, se] = N(null), [xt, xe] = N(!1), [me, $t] = N(() => Ma()), [St, Ke] = N(null), [Pt, A] = N("Ready"), [pe, Ft] = N(() => Pa()), Q = Ce.map((r) => k.find((d) => d.id === r)).filter(Boolean), b = M ? k.find((r) => r.id === M) ?? null : null, $ = Gn({ host: e, activeFile: b, openEditors: Q, chatSettings: me, setStatus: A }), ee = q(
    () => [...$.artifacts, ...J].sort((r, d) => d.createdAt - r.createdAt),
    [$.artifacts, J]
  ), $e = !b && !ie, T = q(() => k.filter((r) => r.dirty), [k]), Mt = q(() => {
    const r = y.trim().toLowerCase();
    return r ? k.filter((d) => d.path.toLowerCase().includes(r)) : k;
  }, [k, y]), Et = v((r) => {
    r.preventDefault();
    const d = r.clientX, m = W, w = (de) => {
      const be = m + (d - de.clientX);
      B(Math.max(380, Math.min(760, be)));
    }, S = () => {
      window.removeEventListener("pointermove", w), window.removeEventListener("pointerup", S);
    };
    window.addEventListener("pointermove", w), window.addEventListener("pointerup", S);
  }, [W]), It = v((r) => {
    r.preventDefault();
    const d = r.clientX, m = h, w = (de) => {
      const be = m + (de.clientX - d);
      s(Math.max(240, Math.min(460, be)));
    }, S = () => {
      window.removeEventListener("pointermove", w), window.removeEventListener("pointerup", S);
    };
    window.addEventListener("pointermove", w), window.addEventListener("pointerup", S);
  }, [h]), ge = v((r) => {
    const d = [r, ...pe.filter((m) => m.path !== r.path)].slice(0, 6);
    Ft(d), localStorage.setItem(wt, JSON.stringify(d));
  }, [pe]), O = v((r, d) => {
    le((m) => m.includes(r.id) ? m : [...m, r.id]), U(r.id), V(!1), Ke(d ?? null), R({ lineNumber: d ?? 1, column: 1 });
  }, []), fe = v(async () => {
    if (_e(T, "open new files"))
      try {
        const r = await Rn();
        if (r.length === 0) return;
        L((d) => ya(d, r)), r.forEach((d) => ge({ name: d.name, path: d.path, at: Date.now() })), O(r[0]), A(`Opened ${r.length} local file${r.length === 1 ? "" : "s"}`);
      } catch (r) {
        r.name !== "AbortError" && A(`Open file failed: ${r.message}`);
      }
  }, [T, O, ge]), Se = v(async () => {
    if (_e(T, "open another folder"))
      try {
        const r = await Dn();
        oe(r), L(r.files), le([]), U(null), V(!1), ge({ name: r.name, path: r.name, at: Date.now() }), A(`${r.handle ? "Opened local folder" : "Opened browser fallback folder"} ${r.name} (${r.files.length} text files indexed)`);
      } catch (r) {
        r.name !== "AbortError" && A(`Open folder failed: ${r.message}`);
      }
  }, [T, ge]), Lt = v((r) => {
    M && L((d) => d.map((m) => m.id === M ? { ...m, content: r, dirty: r !== m.content || m.dirty } : m));
  }, [M]), Pe = v(async () => {
    if (b)
      try {
        const r = await Oe(b);
        L((d) => d.map((m) => m.id === r.id ? r : m)), A(`Saved ${r.name}`);
      } catch (r) {
        A(`Save failed: ${r.message}`);
      }
  }, [b]), Ot = v(async (r) => {
    const d = k.find((w) => w.id === r);
    if (!d) return null;
    const m = await Oe(d);
    return L((w) => w.map((S) => S.id === m.id ? m : S)), m;
  }, [k]), zt = v(async () => {
    ...
    try {
      const d = await Promise.all(r.map((w) => Oe(w))), m = new Map(d.map((w) => [w.id, w]));
      L((w) => w.map((S) => m.get(S.id) ?? S)), A(`Saved ${d.length} dirty file${d.length === 1 ? "" : "s"}`);
    } catch (d) {
      A(`Save all failed: ${d.message}`);
    }
  }, [k]), Fe = v((r) => {
    const d = k.find((m) => m.id === r);
    if (d?.dirty && !window.confirm(`${d.name} has unsaved changes. Close without saving?`)) {
      A(`Close canceled — ${d.name} has unsaved changes`);
      return;
    }
    le((m) => {
      const w = m.filter((S) => S !== r);
      return M === r && U(w.at(-1) ?? null), w;
    });
  }, [M, k]), _t = v(() => {
    _e(T, "close all editors") && (le([]), U(null), Ke(null), V(!1), A("Closed all editors"));
  }, [T]), Me = v(() => {
    const r = k.filter((m) => m.name.startsWith("Untitled")).length + 1, d = {
...
`,
    ...
    const r = b ?? k[0];
    if (!r) {
      A("No open file to synthesize");
      return;
    }
    const d = r.content.split(`
`).map((w) => w.trim()).filter(Boolean).slice(0, 6), m = {
      id: `output-${Date.now()}`,
      title: `Local draft from ${r.name}`,
      kind: "local-draft",
      body: ["# Local synthesis", "", ...d.map((w) => `- ${w.replace(/^#+\s*/, "")}`), "", "> Deterministic local draft. Real agents wire in next sprint."].join(`
`),
      createdAt: Date.now()
    };
    Ae((w) => [m, ...w]), j(!0), G("output"), u("outputs"), C(!0), A("Local synthesis created");
  }, [b, k]), Ie = v((r) => {
    const d = k.find((m) => m.path === r.path || m.name === r.name);
    if (d) {
      O(d), A(`Opened recent ${d.name}`);
      return;
    }
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    $.createArtifact({
      title: b.path,
      kind: b.language === "markdown" ? "markdown" : "report",
      body: b.content
    }).then(() => {
      C(!0), u("outputs"), j(!0), G("output");
    });
  }, [b, $]), Le = v((r) => {
    const d = Te(k, We(r.title || r.kind)), m = {
    ...
    L((w) => [...w, m]), O(m), A(`Opened ${r.title} as editable file`);
  }, [k, O]), Xe = v((r, d) => {
    const m = va(d, k, r);
    if (m.edits.length > 1) {
      se(m), Z(null), A(`Previewing AI workspace patch for ${m.edits.length} files`);
      return;
    }
    if (m.edits.length === 1) {
      Z(m.edits[0]), se(null), A(`Previewing AI patch for ${m.edits[0].fileName}`);
      return;
    }
    if (!b) {
      A("Open a file before previewing an AI edit, or ask Atomek for a git-style diff against opened files.");
      return;
    }
    const w = $a(d, b);
    if (!w) {
      A("No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again.");
      return;
    }
    se(null), Z({
      fileId: b.id,
      ...
      proposedContent: w.content,
      sourceTitle: r,
      extractionLabel: w.label,
      stats: At(b.content, w.content)
    }), A(`Previewing AI edit for ${b.name}`);
  }, [b, k]), Bt = v(() => {
    if (!I) return;
    const r = k.find((d) => d.id === I.fileId);
    if (!r) {
      A(`Cannot apply edit — ${I.fileName} is no longer open`), Z(null);
      return;
    }
    r.content !== I.originalContent && !window.confirm(`${I.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (L((d) => d.map((m) => m.id === I.fileId ? { ...m, content: I.proposedContent, dirty: !0 } : m)), Z(null), A(`Applied AI edit to ${r.name}`));
  }, [k, I]), jt = v(() => {
    if (!I) return;
    const r = Te(k, `${We(I.fileName)}-ai-edit`), d = {
      id: `pending-edit-${I.fileId}-${Date.now()}`,
      name: r,
      path: r,
      language: b?.language ?? "markdown",
      content: I.proposedContent,
      dirty: !0,
      source: "generated"
    };
    L((m) => [...m, d]), O(d), Z(null), A(`Opened proposed edit as ${r}`);
  }, [b?.language, k, O, I]), Vt = v(() => {
    if (!X) return;
    const r = new Map(X.edits.map((m) => [m.fileId, m]));
    k.some((m) => {
      const w = r.get(m.id);
      return w && m.content !== w.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (L((m) => m.map((w) => {
      const S = r.get(w.id);
      return S ? { ...w, content: S.proposedContent, dirty: !0 } : w;
    })), se(null), A(`Applied AI workspace patch to ${r.size} file${r.size === 1 ? "" : "s"}`));
  }, [k, X]), Ht = v(() => {
    if (!X) return;
    const r = [...k], d = X.edits.map((m) => {
      const w = Te(r, `${We(m.fileName)}-ai-edit`), S = k.find((be) => be.id === m.fileId), de = {
        id: `workspace-patch-${m.fileId}-${Date.now()}-${w}`,
        name: w,
        path: w,
    ...
    L((m) => [...m, ...d]), d[0] && O(d[0]), se(null), A(`Opened ${d.length} proposed edit file${d.length === 1 ? "" : "s"}`);
  }, [k, O, X]), Qe = v((r) => {
    if (r === "explain") {
      H("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (r === "improve") {
      H("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (r === "plan") {
      H("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (r === "edit") {
      H("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    H("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [H]);
  return ne(() => {
    const r = (d) => {
      (d.metaKey || d.ctrlKey) && (d.key.toLowerCase() === "o" && (d.preventDefault(), fe()), d.key.toLowerCase() === "s" && (d.preventDefault(), Pe()), d.key.toLowerCase() === "w" && (d.preventDefault(), M && Fe(M)), d.key.toLowerCase() === "b" && (d.preventDefault(), l((w) => !w)), d.shiftKey && d.key.toLowerCase() === "f" && (d.preventDefault(), i("search"), l(!0)), (d.key.toLowerCase() === "k" || d.key.toLowerCase() === "p") && (d.preventDefault(), P(!0)));
    };
    return window.addEventListener("keydown", r), () => window.removeEventListener("keydown", r);
  }, [M, Fe, fe, Pe]), ne(() => {
    const r = (d) => {
      T.length !== 0 && (d.preventDefault(), d.returnValue = "");
    };
    return window.addEventListener("beforeunload", r), () => window.removeEventListener("beforeunload", r);
  }, [T.length]), ne(() => {
    const r = { primaryVisible: o, primaryWidth: h, secondaryVisible: f, secondaryWidth: W, markdownPreviewVisible: z };
    localStorage.setItem(gt, JSON.stringify(r));
  }, [z, o, h, f, W]), ne(() => {
    localStorage.setItem(kt, JSON.stringify(me));
  }, [me]), /* @__PURE__ */ c(
    "div",
    {
      className: `workbench-workbench ${o ? "" : "no-primary"} ${f ? "" : "no-secondary"} ${he ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${h}px`, "--workbench-secondary-width": `${W}px` },
      children: [
        /* @__PURE__ */ n(ea, { active: a, setActive: (r) => {
          i(r), l(!0);
        }, openSettings: () => xe(!0) }),
        o && /* @__PURE__ */ c("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ n("div", { className: "workbench-primary-resizer", onPointerDown: It, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ c("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ n("button", { className: "workbench-command-center", onClick: () => P(!0), children: "Workspace" }),
          /* @__PURE__ */ c("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ n(
              ia,
              {
                ...
                closeWelcome: () => V(!0),
                secondaryVisible: f,
                toggleSecondary: () => C((r) => !r),
                canPreview: b?.language === "markdown",
                previewVisible: z,
                togglePreview: () => re((r) => !r)
              }
            ),
            /* @__PURE__ */ n(ra, { file: b, folder: ue, showWelcome: $e }),
            /* @__PURE__ */ n("div", { className: "workbench-editor-content", children: b ? /* @__PURE__ */ c("div", { className: b.language === "markdown" && z ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ n("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ n(Kt, { fallback: /* @__PURE__ */ n("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ n(
                Yn,
                {
                  file: b,
                  revealLine: St,
              ...
              b.language === "markdown" && z && /* @__PURE__ */ n(da, { content: b.content })
            ] }) : $e ? /* @__PURE__ */ n(oa, { openFile: fe, openFolder: Se, newFile: Me, recent: pe, reopenRecent: Ie, setStatus: A }) : /* @__PURE__ */ c("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ n(Be, { size: 34 }),
              /* @__PURE__ */ n("p", { children: "No editor open" }),
              /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: () => V(!1), children: "Show Welcome" })
            ] }) }),
            he && /* @__PURE__ */ n(
              ha,
              {
        ...
        f && /* @__PURE__ */ n(
          ua,
          {
            ...
            saveMessageAsArtifact: Tt,
            rememberMessage: Rt,
            previewEditFromMessage: (r) => Xe(r.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", r.body),
            ...
            runLocalSynthesis: Ee,
            openOutputAsFile: Le,
            previewEditFromOutput: (r) => Xe(r.title, r.body),
        ...
        ce && /* @__PURE__ */ n(
          sa,
          {
            ...
            activeFile: b,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: Me },
              ...
              ...pe.map((r) => ({ label: `File: Open Recent — ${r.name}`, detail: r.path, run: () => Ie(r) })),
              { label: "File: Save All", detail: `${T.length} dirty file${T.length === 1 ? "" : "s"}`, run: () => {
                zt();
              }, disabled: T.length === 0 },
              { label: "File: Close All Editors", detail: `${Q.length} open editor${Q.length === 1 ? "" : "s"}`, run: _t, disabled: Q.length === 0 },
              ...
              { label: "View: Toggle Chat Panel", detail: f ? "Hide right AI side bar" : "Show right AI side bar", run: () => C((r) => !r) },
              ...
              { label: "View: Toggle Markdown Preview", detail: b?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => re((r) => !r), disabled: b?.language !== "markdown" },
              { label: "Atomek: Create Local Draft", detail: "Deterministic local synthesis from the active file", run: Ee },
              { label: "AI: Explain Active File", detail: b ? `Ask Cortex to explain ${b.path}` : "Open a file first", run: () => H("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !b },
              { label: "AI: Improve Active File", detail: b ? `Ask Cortex for concrete edits to ${b.path}` : "Open a file first", run: () => H("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !b },
              { label: "AI: Draft Editable Replacement", detail: b ? `Ask Cortex for a full-file replacement for ${b.path}` : "Open a file first", run: () => Qe("edit"), disabled: !b },
              { label: "AI: Plan Workspace Work", detail: Q.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => H("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: Q.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: b ? "Persist active file in host.ai artifacts" : "Open a file first", run: Dt, disabled: !b },
              { label: "AI: Open Latest Artifact as File", detail: ee[0] ? `Create editable file from ${ee[0].title}` : "No outputs yet", run: () => ee[0] && Le(ee[0]), disabled: ee.length === 0 }
            ],
            openWorkbenchFile: O,
            onClose: () => P(!1)
          }
        ...
        /* @__PURE__ */ n(ka, { status: Pt, file: b ?? Jn, cursor: E, fileCount: k.length, dirtyCount: T.length })
      ]
    }
  );
}
function ea({ active: e, setActive: t, openSettings: a }) {
  return /* @__PURE__ */ c("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(ct, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(Pn, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(kn, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(an, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(rt, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ n("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(ln, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ n(te, { icon: /* @__PURE__ */ n(In, { size: 23 }), label: "Manage", active: !1, onClick: a })
  ] });
}
function te({ icon: e, label: t, active: a, onClick: i }) {
  return /* @__PURE__ */ n("button", { className: `workbench-activity-button ${a ? "active" : ""}`, title: t, "aria-label": t, onClick: i, children: e });
}
function ta(e) {
  return e.activity === "search" ? /* @__PURE__ */ n(la, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ n(tt, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ n(tt, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ n(ga, {}) : /* @__PURE__ */ n(na, { ...e });
}
function na(e) {
  const t = !e.folder, a = q(() => aa(e.files, e.folder?.name), [e.files, e.folder?.name]);
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ c(Re, { children: [
        ...
        a.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No readable text files found." }) : yt(a, e.activeFileId, e.openWorkbenchFile)
      ] }),
...
function qe({ file: e, active: t, onOpen: a, basePath: i, depth: o = 0, label: l }) {
  ...
  return /* @__PURE__ */ c("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": s }, onClick: a, title: e.path, children: [
    /* @__PURE__ */ n(ot, { size: 14 }),
    /* @__PURE__ */ n("span", { className: "workbench-row-name", children: l ?? h }),
    e.dirty && /* @__PURE__ */ n("span", { className: "workbench-row-meta", children: "●" })
  ] });
}
function aa(e, t) {
  const a = [], i = /* @__PURE__ */ new Map(), o = (h, s, p = a) => {
    const u = i.get(h);
    if (u) return u;
    const f = { name: s, path: h, children: [] };
    return i.set(h, f), p.push(f), f;
  };
  e.forEach((h) => {
    const s = t && h.path.startsWith(`${t}/`) ? h.path.slice(t.length + 1) : h.path, p = s.split("/").filter(Boolean);
    let u = a, f = "";
    p.slice(0, -1).forEach((C) => {
      f = f ? `${f}/${C}` : C, u = o(f, C, u).children;
    }), u.push({ name: p.at(-1) ?? h.name, path: s, file: h, children: [] });
  });
  const l = (h) => h.sort((s, p) => +!!s.file - +!!p.file || s.name.localeCompare(p.name)).map((s) => ({ ...s, children: l(s.children) }));
  return l(a);
}
function yt(e, t, a, i = 0) {
  return e.map((o) => o.file ? /* @__PURE__ */ n(qe, { file: o.file, active: o.file.id === t, onOpen: () => a(o.file), depth: i, label: o.name }, o.file.id) : /* @__PURE__ */ c("div", { children: [
    ...
    yt(o.children, t, a, i + 1)
  ] }, o.path));
}
function ra({ file: e, folder: t, showWelcome: a }) {
...
function ia(e) {
  return /* @__PURE__ */ c("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ c("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ n(ae, { size: 13 }) })
    ] }),
    ...
    e.canPreview && /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ n(it, { size: 16 }) }),
    /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ n(Cn, { size: 16 }) })
  ] });
}
function oa({ openFile: e, openFolder: t, newFile: a, recent: i, reopenRecent: o, setStatus: l }) {
...
function la({ files: e, query: t, setQuery: a, openWorkbenchFile: i, activeFileId: o }) {
  const l = q(() => ca(e, t), [e, t]), h = q(() => {
    const s = /* @__PURE__ */ new Map();
    return l.forEach((p) => s.set(p.file.id, [...s.get(p.file.id) ?? [], p])), Array.from(s.values()).slice(0, 50);
  }, [l]);
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? h.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No matches." }) : h.map((s) => {
        ...
        return /* @__PURE__ */ c("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ n(qe, { file: p, active: p.id === o, onOpen: () => i(p) }),
          s.slice(0, 5).map((u) => /* @__PURE__ */ c("button", { className: "workbench-search-hit", onClick: () => i(p, u.lineNumber), title: u.line, children: [
            /* @__PURE__ */ n("span", { className: "workbench-search-line", children: u.lineNumber }),
            /* @__PURE__ */ n("span", { children: u.line })
          ] }, `${p.id}-${u.lineNumber}-${u.line}`)),
...
function sa(e) {
  const t = e.query.trim().toLowerCase(), a = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${ht(o.language)} file`,
...
function ha(e) {
  return /* @__PURE__ */ c("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ c("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ n("span", {}),
      /* @__PURE__ */ n("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ n(ae, { size: 14 }) })
    ] }),
    /* @__PURE__ */ c("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ n(vt, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runLocalSynthesis: e.runLocalSynthesis, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function ua(e) {
  return /* @__PURE__ */ c("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ c("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ n("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ n(st, { size: 15 }) }),
        /* @__PURE__ */ n("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ n(sn, { size: 16 }) }),
        /* @__PURE__ */ n("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ n(ae, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ n(ma, { ...e }) : /* @__PURE__ */ n(vt, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runLocalSynthesis: e.runLocalSynthesis, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function ma(e) {
  return /* @__PURE__ */ c("div", { className: "workbench-chat-wrap", children: [
    /* @__PURE__ */ n("div", { className: "workbench-chat-transcript", children: e.chatMessages.length === 0 ? /* @__PURE__ */ n("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ c("div", { children: [
      ...
      /* @__PURE__ */ n("br", {}),
      t.body,
      t.gatewayLabel ? /* @__PURE__ */ c(Re, { children: [
        /* @__PURE__ */ n("br", {}),
      ...
      /* @__PURE__ */ c("div", { className: "workbench-chat-box", children: [
        ...
        /* @__PURE__ */ c("div", { className: "workbench-chat-toolbar", children: [
          /* @__PURE__ */ c("button", { className: "workbench-chat-mode", onClick: e.openSettings, title: "Chat AI settings", children: [
            Ue(e.chatSettings.gatewayPreference),
            " ",
            /* @__PURE__ */ n(ve, { size: 12 })
          ] }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-mode", onClick: () => e.runQuickPrompt("plan"), disabled: e.busy, children: "Plan" }),
          /* @__PURE__ */ n("span", {}),
          /* @__PURE__ */ n("button", { className: "workbench-chat-send", onClick: e.askAgent, disabled: e.busy, title: "Send", children: /* @__PURE__ */ n(Mn, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function pa(e, t, a) {
  const i = e.model.trim(), l = [e.gatewayPreference === "auto" ? t : Ue(e.gatewayPreference)];
  return i && l.push(i), a > 0 && l.push(`${a} memories`), l.join(" · ");
}
function fa(e) {
  const [t, a] = N([]), [i, o] = N("Loading gateway models…");
  ne(() => {
    ...
    return (async () => {
      ...
      try {
        const u = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: s.signal
        });
        if (s.signal.aborted) return;
        a(u.map((f) => ({ id: f.id, gatewayLabel: f.gatewayLabel }))), o(u.length > 0 ? `${u.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (u) {
        if (s.signal.aborted) return;
        a([]), o(`Model discovery failed: ${u instanceof Error ? u.message : String(u)}`);
      }
    })(), () => s.abort();
  ...
  return /* @__PURE__ */ n("div", { className: "workbench-settings-backdrop", onClick: e.onClose, children: /* @__PURE__ */ c("section", { className: "workbench-settings-dialog", onClick: (s) => s.stopPropagation(), role: "dialog", "aria-modal": "true", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ c("header", { className: "workbench-settings-header", children: [
      /* @__PURE__ */ n(On, { size: 15 }),
      /* @__PURE__ */ n("strong", { children: "Atomek Settings" }),
      /* @__PURE__ */ n("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ n(ae, { size: 15 }) })
    ] }),
    ...
    /* @__PURE__ */ c("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ n("button", { onClick: () => e.onChange(ye), children: "Reset" }),
      /* @__PURE__ */ n("button", { onClick: e.onClose, children: "Done" })
    ] })
  ] }) });
}
function vt({ outputs: e, clearOutputs: t, deleteArtifact: a, runLocalSynthesis: i, openOutputAsFile: o, previewEditFromOutput: l, canPreviewEdit: h = !1, compact: s = !1 }) {
  return /* @__PURE__ */ c("div", { className: `workbench-panel-list ${s ? "compact" : ""}`, children: [
    /* @__PURE__ */ c("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ c("button", { className: "workbench-button-subtle", onClick: i, children: [
        /* @__PURE__ */ n(tn, { size: 14 }),
        "Local draft"
      ] }),
      /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create a local draft." }) : e.map((p) => /* @__PURE__ */ c("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ c("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ n("strong", { children: p.title }),
        /* @__PURE__ */ n("span", { children: p.source === "ai" ? `AI · ${p.kind}` : p.kind }),
        /* @__PURE__ */ n("button", { onClick: () => o(p), children: "Open as file" }),
        l ? /* @__PURE__ */ n("button", { onClick: () => l(p), disabled: !h, children: "Preview edit" }) : null,
        p.source === "ai" ? /* @__PURE__ */ n("button", { onClick: () => a(p.id), children: "Delete" }) : null
      ] }),
      p.body
    ] }, p.id))
  ] });
}
function ba({ edit: e, onApply: t, onOpenAsFile: a, onClose: i }) {
  return /* @__PURE__ */ n("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ c("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ c("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ n("button", { title: "Close", onClick: i, children: /* @__PURE__ */ n(ae, { size: 16 }) })
    ] }),
...
function wa({ patch: e, onApply: t, onOpenAsFiles: a, onClose: i }) {
  ...
  return /* @__PURE__ */ n("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ c("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((l) => /* @__PURE__ */ c("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ n("pre", { children: xa(l.proposedContent) })
      ] }, l.fileId)),
...
function ka({ status: e, file: t, cursor: a, fileCount: i, dirtyCount: o }) {
  return /* @__PURE__ */ c("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ n("span", { children: ht(t.language) })
  ] });
}
function _e(e, t) {
...
function va(e, t, a) {
  const o = [
    ...Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((s) => ({
      ...
      content: Ct(s[2])
    })).filter((s) => s.content.trim().length > 0).filter((s) => D(s.lang, "diff") || D(s.lang, "patch")).map((s) => s.content),
    e
  ...
  for (const s of o)
    for (const p of Na(s)) {
      ...
      const f = Aa(t, u);
      if (!f) {
        h.push(`${u}: no opened file`);
        continue;
      }
      const C = Nt(f.content, p);
      if (!C || C === f.content) {
        h.push(`${u}: patch did not match or produced no change`);
        continue;
      }
      l.set(f.id, {
        fileId: f.id,
        ...
        proposedContent: C,
        sourceTitle: a,
        extractionLabel: `workspace diff (${u})`,
        stats: At(f.content, C)
      });
    }
...
`), i = [];
  ...
  for (let h = 0; h < o.length; h += 1) {
    const s = o[h], u = o.find((C) => C > s && a[C].startsWith("diff --git ")) ?? a.length, f = a.slice(s, u).join(`
`);
    /^@@\s+-\d+/m.test(f) && l.push(f);
  }
  return l.length > 0 ? l : [t];
}
function Ca(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m);
  if (t) return we(t[3] || t[4] || t[1] || t[2] || "");
  const a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m);
  if (a) return we(a[1] || a[2] || "");
  const i = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m);
  return i ? we(i[1] || i[2] || "") : null;
}
function we(e) {
  return e.replace(/^["']|["']$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function Aa(e, t) {
  const a = we(t), i = a.split("/").at(-1) ?? a;
  return e.find((o) => {
    const l = we(o.path);
...
function $a(e, t) {
  const a = Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((u) => ({
    lang: u[1].trim().toLowerCase(),
    content: Ct(u[2])
  })).filter((u) => u.content.trim().length > 0), i = [
    ...a.filter((u) => D(u.lang, "diff") || D(u.lang, "patch")).map((u) => u.content),
    e
  ];
  for (const u of i) {
    const f = Nt(t.content, u);
    if (f) return { content: f, label: "unified diff patch" };
  }
  if (a.length === 0) return null;
  const o = Sa(t), l = a.find((u) => D(u.lang, "atomek-replace") || D(u.lang, "replace"));
  if (l) return { content: l.content, label: `replacement block (${l.lang || "plain"})` };
  const h = a.find((u) => o.some((f) => D(u.lang, f)) && !D(u.lang, "diff") && !D(u.lang, "patch"));
  if (h) return { content: h.content, label: `matched ${h.lang || t.language} block` };
  const p = a.filter((u) => !D(u.lang, "diff") && !D(u.lang, "patch")).sort((u, f) => f.content.length - u.content.length)[0];
  return p ? { content: p.content, label: `largest fenced block (${p.lang || "plain"})` } : null;
}
function Nt(e, t) {
...
`), o = [];
  ...
  for (let s = 0; s < i.length; s += 1) {
    const p = i[s].match(/^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/);
    if (!p) continue;
    h = !0;
    const u = Number(p[1]), f = Math.max(0, u - 1);
    if (f < l) return null;
    for (o.push(...a.slice(l, f)), l = f, s += 1; s < i.length; s += 1) {
      const C = i[s];
      if (C.startsWith("@@ ")) {
        s -= 1;
        break;
      }
      if (C.startsWith("diff --git ") || C.startsWith("--- ") || C.startsWith("+++ ") || C.startsWith("\\ No newline at end of file")) continue;
      const W = C[0], B = C.slice(1);
      if (W === " ") {
        if (a[l] !== B) return null;
        o.push(B), l += 1;
        continue;
      }
      if (W === "-") {
...
`), o = Math.max(a.length, i.length);
  let l = 0, h = 0, s = 0;
  for (let p = 0; p < o; p += 1)
    a[p] !== i[p] && (a[p] === void 0 ? l += 1 : i[p] === void 0 ? h += 1 : s += 1);
...
function Ma() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : ye.gatewayPreference,
...
function Ea({ host: e }) {
  return /* @__PURE__ */ n(Zn, { host: e });
}
function Oa(e) {
  return function() {
    return /* @__PURE__ */ n(Ea, { host: e.host });
  };
}
export {
  Oa as default
