...
import { forwardRef as ca, createElement as fn, useState as z, useRef as Qe, useCallback as N, useEffect as ie, lazy as Or, useMemo as oe, Suspense as Cr } from "react";
/**
 ...
 */
const Hr = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Sr = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), Yn = (e) => {
  const t = Sr(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, la = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), jr = (e) => {
  for (const t in e)
...
const Mr = ca(
  ({
    ...
    "svg",
    {
      ...
      className: la("lucide", s),
      ...!i && !jr(l) && { "aria-hidden": "true" },
      ...l
    },
    [
      ...o.map(([y, b]) => fn(y, b)),
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Fr = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Ir = ae("check", Fr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const qr = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], It = ae("chevron-down", qr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const vs = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function ys() {
  const e = Tn();
...
async function gs() {
  ...
  if (!e)
    return { name: "Browser fallback folder", files: await wa(!0) };
...
const st = "atomek:default", xs = 3e3, An = "tytus.atomek.threadTitleOverrides", Pa = "tytus.atomek.selectedThreadId", Kn = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, kt = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, Ns = (e, t = xs) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Bn = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, zs = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Un = (e) => ({
  id: e.id,
  title: e.title,
  kind: zs(e.kind),
  body: e.body,
...
`)
  ...
  try {
    return localStorage.getItem(Pa)?.trim() || null;
  } catch {
    return null;
  }
}, Be = (e) => {
  try {
    localStorage.setItem(Pa, e);
  } catch {
  }
};
function Os({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const s = e.ai, [i, o] = z(null), [l, p] = z([]), [y, b] = z([]), [h, g] = z([]), [C, S] = z([]), [K, E] = z(Kn), [q, ue] = z(!1), re = Qe(!0), j = Qe(null), U = N(async () => {
    if (!s) {
      E(Kn);
      return;
    }
    ...
    if (s)
      try {
        const w = await s.listArtifacts({ threadId: m });
        if (!re.current) return;
        g(w.map(Un));
      } catch (w) {
        r(`AI artifacts unavailable: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [s, r]), M = N(async (m) => {
    if (s)
      try {
        const w = await s.listMessages(m);
        if (!re.current) return;
        const T = l.find((F) => F.id === m) ?? null;
        T && o(T), Be(m), b(w.map(kt).filter(Boolean)), S([]), await R(m);
      } catch (w) {
        r(`Load chat failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [s, R, r, l]), ce = N(async () => {
    if (s)
      try {
        const m = Gn(await s.listThreads({ workspaceKey: st, status: "active" })), w = As(), T = m.find((se) => se.id === w) ?? m[0] ?? await s.createThread({ workspaceKey: st, title: "Atomek chat" });
        if (!re.current) return;
        p(m[0] ? m : [T]), o(T), Be(T.id);
        const F = await s.listMessages(T.id);
        if (!re.current) return;
        b(F.map(kt).filter(Boolean)), await R(T.id);
      } catch (m) {
        r(`AI chat unavailable: ${m instanceof Error ? m.message : String(m)}`);
      }
  }, [s, R, r]);
  ie(() => (re.current = !0, U(), ce(), () => {
    re.current = !1;
  }), [ce, U]);
  const D = N(async () => {
    if (!s) return null;
    ...
    return re.current && (o(m), Be(m.id), p((w) => [m, ...w.filter((T) => T.id !== m.id)])), m;
  }, [s, i]), he = N(async () => {
    if (!s) return;
    const m = await s.createThread({ workspaceKey: st, title: "Atomek chat" });
    o(m), Be(m.id), p((w) => [m, ...w.filter((T) => T.id !== m.id)]), b([]), g([]), S([]), r("New AI chat created");
  }, [s, r]), ve = N(async (m, w) => {
    if (!s) return;
    const T = w.trim();
    if (T)
      try {
        const F = s.updateThread, se = l.find((ke) => ke.id === m) ?? i, H = typeof F == "function" ? await F({ threadId: m, title: T }) : { ...se ?? await D(), id: m, title: T, updatedAt: Date.now() };
        typeof F != "function" && Xs(m, T), p((ke) => ke.map((Ne) => Ne.id === H.id ? H : Ne)), o((ke) => ke?.id === H.id ? H : ke), r(typeof F == "function" ? `Renamed chat: ${H.title}` : `Renamed chat locally: ${H.title}`);
      } catch (F) {
        r(`Rename chat failed: ${F instanceof Error ? F.message : String(F)}`);
      }
  }, [s, D, r, i, l]), Xe = N(async (m) => {
    if (s)
      try {
        await s.deleteThread(m), Ls(m);
        const w = l.filter((T) => T.id !== m);
        if (p(w), i?.id === m) {
          const T = w[0] ?? await s.createThread({ workspaceKey: st, title: "Atomek chat" });
          o(T), Be(T.id), p((se) => se.some((H) => H.id === T.id) ? se : [T, ...se]);
          const F = await s.listMessages(T.id);
          b(F.map(kt).filter(Boolean)), await R(T.id);
        }
        r("Deleted AI chat");
      } catch (w) {
        r(`Delete chat failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [s, R, r, i?.id, l]), He = N(async (m) => {
    if (!s) return null;
    try {
      const w = await D();
      if (!w) return null;
      ...
      const T = await s.createArtifact({
        ...
        title: m.title?.trim() || Bn(m.body, "Atomek artifact"),
        kind: m.kind ?? "markdown",
        body: m.body
      }), F = Un(T);
      return g((se) => [F, ...se.filter((H) => H.id !== F.id)]), r(`Saved AI artifact: ${F.title}`), F;
    } catch (w) {
      return r(`Save artifact failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [s, D, r]), Le = N(async (m) => {
    if (s)
      try {
        await s.deleteArtifact(m), g((w) => w.filter((T) => T.id !== m)), r("Deleted AI artifact");
      } catch (w) {
        r(`Delete artifact failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [s, r]), X = N(async (m) => {
    if (!s) return null;
    try {
      const w = await s.writeMemory({
        title: m.title?.trim() || Bn(m.body, "Atomek memory"),
        body: m.body,
        metadata: {
          source: "atomek",
          messageId: m.messageId ?? null
      ...
      return S((T) => [w, ...T.filter((F) => F.id !== w.id)].slice(0, 5)), r(`Remembered: ${w.title}`), w;
    } catch (w) {
      return r(`Remember failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [s, r]), _ = N(async (m) => {
    if (!s) return [];
    const w = await s.searchMemory({ query: m, limit: 5 });
    return re.current && S(w), w;
  }, [s]), be = N(async (m, w = {}) => {
    const T = m.trim();
    if (!T || !s) return null;
    ue(!0);
    ...
    try {
      ...
      if (!H) return null;
      Be(H.id);
      const ke = await _(T).catch(() => []), Ne = Ts(ke), Ve = w.requestContext ?? t, Se = Ne ? [...Ve, Ne] : [...Ve];
      let Pe = null;
      ...
      return re.current && ye.length > 0 && p(ye), U(), F;
    } catch (H) {
      ...
      j.current === se && (j.current = null), ue(!1);
    }
  }, [s, n.gatewayPreference, n.model, D, _, U, t, r]), fe = N(() => {
...
const Cs = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Hs = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Ss = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: s }) => {
...
}, js = "active-file", Ws = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Ms = (e, t = "file") => `${t}:${e.id}`, Vs = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let s = [];
  return t.scope === "active-selection" ? r?.selection ? s = [Ds(r, r.selection, !0)] : r && (s = [Ct(r, !0, "Active file")]) : t.scope === "active-file" ? r && (s = [Ct(r, !0, "Active file")]) : t.scope === "open-editors" ? s = e.openDocumentIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ct(i, !0, "Open editor")) : t.scope === "selected-files" && (s = t.selectedFileIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ct(i, !1, "Selected file"))), Is(s).filter((i) => !n.has(i.id));
}, Is = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, qs = 4e3, Rs = 8e3, Es = 4e3, Ys = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Js = (e, t, n) => {
  const r = Fs(e, n), s = [], i = new Map(t.map((o) => [o.id, o]));
  for (const o of r) {
...
`), b = o.kind === "selection" && o.range ? Zs(l.content, o.range) : l.content, h = o.kind === "selection" ? qs : o.label.startsWith("Open editor") ? Es : Rs;
    s.push({
...
`));
}, Qn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, $n = 75e4, Ks = 2800, Bs = 240, Us = 80, Gs = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? $n) || e.content.length > (t.maxFileBytes ?? $n) ? "huge" : !t.includeDirty && e.dirty ? null : ai(e.path) ? "vendor" : ri(e) ? "binary" : null;
}, _n = (e, t = {}) => {
  ...
  for (const o of e) {
    ...
    const y = Ge(o.content), b = $s(o, y, n, t);
    i.push(...b), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
...
}, $s = (e, t = Ge(e.content), n = Date.now(), r = {}) => {
  const s = Math.max(500, r.maxChunkChars ?? Ks), i = Math.min(Math.max(0, r.chunkOverlapChars ?? Bs), Math.floor(s / 2)), o = Math.max(1, r.maxChunksPerFile ?? Us), l = ti(e.content), p = [];
  let y = 0, b = 0;
  for (; y < e.content.length && p.length < o; ) {
    const h = ei(e.content, y, Math.min(e.content.length, y + s)), g = e.content.slice(y, h).trim();
    if (g.length > 0) {
      const C = ni(l, y, h);
      p.push({
...
` && t.push(n + 1);
  ...
  return 0;
}, ai = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => Gs.has(t)), ri = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && Qs.has(t) || e.content.includes("\0")) return !0;
...
class si {
  ...
  staleReport(t) {
    return ii(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const it = (e = [], t = {}) => new si(e, t), ii = (e, t, n = {}) => {
  ...
  const o = e.signature !== xa(t);
  return { stale: i.length > 0, signatureChanged: o, files: i };
}, oi = 8, ci = 12e3, Na = (e, t, n = {}, r) => {
  const s = ui(t);
  if (s.length === 0) return [];
  const i = new Set(r?.files.map((b) => b.fileId) ?? []), o = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => hi(b, s)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, h) => h.score - b.score || b.chunk.path.localeCompare(h.chunk.path) || b.chunk.ordinal - h.chunk.ordinal), l = [], p = /* @__PURE__ */ new Set();
  let y = n.maxChars ?? ci;
  for (const b of o) {
    if (l.length >= (n.limit ?? oi) || y <= 0) break;
    const h = `${b.chunk.fileId}:${b.chunk.ordinal}`;
    if (p.has(h)) continue;
    p.add(h);
    const g = b.chunk.text.length > y ? `${b.chunk.text.slice(0, Math.max(0, y - 28))}
[...context clipped...]` : b.chunk.text;
    l.push(li(b, g, i.has(b.chunk.fileId))), y -= g.length;
  }
  return l;
}, li = (e, t = e.chunk.text, n = !1) => {
...
`), ui = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const p of t) {
    const y = sn(n, p), b = sn(r, p), h = sn(s, p), g = i === p ? 1 : 0;
    y + b + h + g !== 0 && (l.push(p), o += Math.min(y, 12), o += b * 4, o += h * 6, o += g * 3, (n.includes(`function ${p}`) || n.includes(`const ${p}`) || n.includes(`class ${p}`) || n.includes(`type ${p}`)) && (o += 3));
  }
  ...
  s.current = e;
  const p = N((g = s.current) => {
    const C = (i.current ?? it([], r)).refresh(g, r);
    return l(C), C;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), y = N((g = s.current) => {
    const C = (i.current ?? it([], r)).update(g, r);
    return l(C), C;
  }, [p]);
  ie(() => {
    n && p(e);
  }, [n, e, p]);
  const b = (i.current ?? it([], r)).staleReport(e), h = N((g, C = {}) => {
    const S = i.current ?? it([], r);
    return Na(S.getSnapshot(), g, C, S.staleReport(s.current));
  }, []);
  return { snapshot: o, staleReport: b, isStale: b.stale, refresh: p, update: y, retrieve: h };
}, pi = "tytus.atomek.semanticVector:v1", za = "__gateway_default__", Ta = (e) => e?.trim() || za, bi = (e) => {
  const t = Ta(e);
  return t === za ? "gateway default" : t;
}, Xa = (e, t, n) => [
  pi,
  ...
  if (!La()) return null;
  try {
    const r = Xa(e, t, n), s = window.localStorage.getItem(r);
    if (!s) return null;
  ...
  if (!La()) return null;
  try {
    const t = Xa(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, Pt = 8, wi = 12e3, gi = 180, ki = async (e, t, n, r, s = {}, i, o) => {
  const l = Na(t, n, { ...s, limit: Math.max(s.limit ?? Pt, 16) }, i), p = e.ai;
  if (typeof p?.embedText != "function")
    return { hits: l.slice(0, s.limit ?? Pt), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const y = Ta(r.embeddingModel), b = await p.embedText({
      input: n,
    ...
    const g = new Map(l.map((j) => [j.chunkId, j])), C = Li(n), S = /* @__PURE__ */ new Map();
    t.chunks.filter((j) => s.includeDirty !== !1 || !j.dirty).slice(0, gi).forEach((j) => S.set(j.id, j)), l.forEach((j) => {
      const U = t.byChunkId.get(j.chunkId);
      U && S.set(U.id, U);
    });
    const K = Array.from(S.values()), E = [];
    let q = 0;
    for (const j of K) {
      o?.aborted;
      const U = vi(e.appId, y, j) ?? await xi(p, e.appId, y, j, r, o);
      if (!U) continue;
      U.updatedAt > Date.now() - 1e3 && (q += 1);
      const R = Ti(h, U.vector);
      if (!Number.isFinite(R)) continue;
      const ce = g.get(j.id)?.score ?? Ai(j, C), D = Xi(ce) * 0.42 + Math.max(0, R) * 0.58;
      E.push({ chunk: j, keywordScore: ce, vectorScore: R, score: D, matchedTerms: C.filter((he) => j.text.toLowerCase().includes(he)) });
    }
    if (E.length === 0) return { hits: l.slice(0, s.limit ?? Pt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: q };
    const ue = new Set(i?.files.map((j) => j.fileId) ?? []), re = E.sort((j, U) => U.score - j.score || U.keywordScore - j.keywordScore || j.chunk.path.localeCompare(U.chunk.path) || j.chunk.ordinal - U.chunk.ordinal).slice(0, Math.max(s.limit ?? Pt, 1)).map((j) => Ni(j, ue.has(j.chunk.fileId)));
    return {
      hits: zi(re, s.maxChars ?? wi),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${bi(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: q
    };
  } catch (y) {
    return {
      hits: l.slice(0, s.limit ?? Pt),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: Oi(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, Xi = (e) => Math.min(1, Math.max(0, e / 24)), Li = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Ai = (e, t) => {
  const n = `${e.path}
${e.name}
...
function Cn(e) {
  return Array.from(e.matchAll(Ci)).map((t) => Oa(t[1] ?? "", Vi(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Oa(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, s = [];
  for (const i of n) {
    const o = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? r[o[1].toLowerCase()] = Fi(o[2]) : s.push(i.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: s, attrs: r, content: t };
}
function Ce(e, t) {
  return (typeof e == "string" ? Oa(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Ca(e) {
  return [...Cn(e).filter((r) => Ce(r, "diff") || Ce(r, "patch")).map((r) => r.content), e].filter((r, s, i) => Si(r) && i.findIndex((o) => o === r) === s);
}
function Si(e) {
...
function bn(e) {
  return Cn(e).filter((t) => Wi(t)).map((t) => ({
    content: Di(t.content),
...
function Ii(e) {
  ...
  for (const b of Ca(t))
    for (const h of ji(b)) {
      if (!h.path) {
        if (s) {
          const S = vn(s.content, h.raw);
          S && S !== s.content && o.set(s.id, Ht({
            file: s,
            proposedContent: S,
            sourceTitle: r,
      ...
      const C = vn(g.file.content, h.raw);
      if (!C || C === g.file.content) {
  ...
  if (o.size === 0 && s && !p) {
    const b = qi(t, s);
    b && b.content !== s.content && o.set(s.id, Ht({
      file: s,
      proposedContent: b.content,
      sourceTitle: r,
      extractionLabel: b.label,
...
function Ht(e) {
  ...
  return {
    ...
    stats: Ri(e.file.content, e.proposedContent),
    base: {
...
const Rt = (e) => typeof e == "object" && e !== null, Zi = (e) => Rt(e.ai) ? e.ai : null, Ki = (e) => {
  if (!Rt(e)) return null;
  const t = e.id;
...
}, je = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(je) : Rt(e) ? Object.values(e).some(je) : !1, Ui = async (e, t) => {
  const n = Zi(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(Bi).filter((s) => !!s) : [];
}, Gi = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : je(e.capability) || je(e.capabilities) || je(e.modality) || je(e.modalities) || je(e.task) || je(e.tasks) || je(e.type) || je(e.kind), Sa = (e) => typeof e == "object" && e !== null, Qi = (e) => Sa(e.ai) ? e.ai : null, ja = (e) => typeof Qi(e)?.embedText == "function", $i = async (e, t) => ja(e) ? (await Ui(e, t)).filter(Gi) : [], _i = (e) => Sa(e.ai) ? ja(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", eo = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], to = [
...
function so(e) {
  const t = e.results.length > 0 ? e.results.map((r, s) => [
    ...
    `Status: ${r.status}`,
    "Output:",
    mo(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function oo(e) {
  ...
  for (const r of t) {
    const s = co(r.content);
    if (!s?.scripts) continue;
    const i = Ma(r.path), o = lo(e, i, s);
    if (!o) continue;
    const l = Object.keys(s.scripts).filter((y) => typeof s.scripts?.[y] == "string"), p = uo(l);
    for (const y of p) {
      const b = `${o} run ${y}`;
      n.push({
        id: Va(`${i}:${b}`),
...
function lo(e, t, n) {
  ...
  for (const [r, s] of to)
    if (e.some((i) => fo(i.path) === r && Ma(i.path) === t)) return s;
  return null;
}
function uo(e) {
  const t = eo.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, s) => r.localeCompare(s));
  return [...t, ...n];
}
function ho(e) {
  const t = /* @__PURE__ */ new Set();
...
function wo({ className: e }) {
  return /* @__PURE__ */ a("img", { src: yo, alt: "ATOMEK", className: e, draggable: !1 });
}
const go = `# Tytus Resource Fabric
...
], To = "tytus-atomek-handles", ut = "handles", Xo = 1, Fa = () => new Promise((e, t) => {
  const n = indexedDB.open(To, Xo);
  n.onupgradeneeded = () => {
    const r = n.result;
    r.objectStoreNames.contains(ut) || r.createObjectStore(ut);
  }, n.onsuccess = () => e(n.result), n.onerror = () => t(n.error ?? new Error("Failed to open Atomek handle store"));
});
async function na(e, t) {
  const n = await Fa();
  await new Promise((r, s) => {
    const i = n.transaction(ut, "readwrite");
    i.objectStore(ut).put(t, e), i.oncomplete = () => r(), i.onerror = () => s(i.error ?? new Error("Failed to persist file handle"));
  }), n.close();
}
async function cn(e) {
  const t = await Fa(), n = await new Promise((r, s) => {
    const o = t.transaction(ut, "readonly").objectStore(ut).get(e);
    o.onsuccess = () => r(o.result ?? null), o.onerror = () => s(o.error ?? new Error("Failed to read file handle"));
  });
  return t.close(), n;
}
const Lo = Or(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Ao = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Ia = "tytus.workspace.recent", qa = "tytus.workspace.layout", Ra = "tytus.atomek.session.v2", Ea = "tytus.atomek.chatAiSettings", Ya = "tytus.atomek.currentMission", yn = "tytus.atomek.currentMissionChanged", Vt = {
...
function ln(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - Oo), r = n < 1180, s = r ? 200 : 240, i = r ? 300 : 340, o = r ? 420 : 560, l = Math.max(s, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), p = Math.min(300, l), y = Math.floor(n * (r ? 0.34 : 0.36)), b = n - p - o, h = Math.max(i, Math.min(r ? 500 : 640, y, b));
...
function jo(e) {
  const t = e?.resources ?? [], n = (l) => l.status === "ready" || l.status === "available", r = t.filter((l) => l.kind === "pod-agent" && ft(l) === "openclaw"), s = t.filter((l) => l.kind === "pod-agent" && ft(l) === "hermes"), i = t.filter((l) => l.kind === "local-cli" && n(l)), o = t.filter((l) => l.kind === "shared-folder" && n(l));
  return [
...
function mt(e, t = "repo-repair") {
  const n = Tt.find((D) => D.id === t) ?? Tt[0], r = e?.resources ?? [], s = Ue(r, (D) => D.kind === "pod-agent" && ft(D) === "openclaw"), i = Ue(r, (D) => D.kind === "pod-agent" && ft(D) === "hermes"), o = Ue(r, (D) => D.kind === "ail-route"), l = Ue(r, (D) => D.kind === "shared-folder"), p = Ie(r, "local-cli", ["claude"]), y = Ie(r, "local-cli", ["opencode", "open code"]), b = Ie(r, "local-cli", ["codex"]), h = Ie(r, "local-cli", ["pi"]), g = Ie(r, "local-cli", ["kimi"]), C = Ue(r, (D) => D.kind === "local-cli"), S = Ie(r, "app-skill", ["juli3ta", "music", "song"]), K = Ie(r, "app-skill", ["blender", "3d"]), E = Ie(r, "app-skill", ["remotion", "video", "render"]), q = Ue(r, (D) => D.kind === "app-skill"), ue = t === "pod-local" ? s ?? i ?? o ?? p ?? C : t === "creative-production" ? i ?? o ?? p ?? C : t === "research-watch" ? s ?? o ?? i ?? p ?? C : p ?? y ?? o ?? C, re = t === "creative-production" ? S ?? K ?? E ?? q ?? y ?? p ?? C : y ?? p ?? b ?? C, j = t === "pod-local" ? b ?? h ?? g ?? s ?? p ?? C : t === "research-watch" ? h ?? g ?? b ?? s ?? C : b ?? h ?? s ?? g ?? C, U = t === "creative-production" ? S ?? K ?? E ?? q : t === "research-watch" ? Ie(r, "app-skill", ["browser", "channel", "telegram"]) ?? q : q, R = [
    xt("planner", "Planner", "Break goal into tasks, risks, and required context.", ue, "No planner agent ready"),
    xt("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", re, "No local/app implementer ready"),
    xt("reviewer", "Reviewer", "Independent critique before approval or handoff.", j, "No reviewer agent ready"),
    xt("team-desk", "Team Desk", "Shared mission folder for transcripts, outputs, proposals, and handoff.", l, "Mission folder only until shared folder is bound")
  ];
  (t === "creative-production" || U) && R.push(xt("app-tool", "App Tool", "Drive installed local app skill through mission context.", U, "No configured app skill"));
...
function $e(e, t, n) {
  ...
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(r) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(r) ? "research-watch" : (t?.resources ?? []).some((s) => s.kind === "pod-agent" && Za(s)) ? "pod-local" : "repo-repair";
}
function Vo(e) {
  return Tt.map((t) => mt(e, t.id));
}
function Wt(e) {
...
function zt(e) {
  ...
  try {
    localStorage.setItem(Ya, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(yn, { detail: t }));
  } catch {
  }
}
function Fo() {
...
function Xt(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", s = mt(t, $e(r, t, n)), i = (h) => s.assignments.find((g) => g.role === h), o = i("planner"), l = i("implementer"), p = i("reviewer"), y = i("team-desk"), b = i("app-tool");
  return [
    {
      id: "task-scope",
...
function Dt(e, t, n, r, s, i) {
  const o = mt(t, $e(s || e.goal, t, i ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
...
function Ua(e, t, n, r) {
  const s = $e(n || e.goal, t, r ?? e.teamPresetId), i = mt(t, s), o = Xt(n || e.goal, t, s);
...
function qo({ host: e }) {
  const t = Qe(null), [n, r] = z(0), s = oe(() => Xc(), []), i = oe(() => Pc(), []), o = oe(() => kc(gc(), i.recent), [i.recent]), [l, p] = z(i.activity ?? "computer"), [y, b] = z(s.primaryVisible), [h, g] = z(s.primaryWidth), [C, S] = z(i.secondaryTab ?? "chat"), [K, E] = z(s.secondaryVisible), [q, ue] = z(s.secondaryWidth), [re, j] = z(!!i.bottomPanelVisible), [U, R] = z(i.bottomPanelTab ?? "problems"), [M, ce] = z(s.markdownPreviewVisible), [D, he] = z(!!i.welcomeClosed), [ve, Xe] = z(i.folder ? { name: i.folder.name, files: [] } : null), [He, Le] = z(i.folder?.handleKey ?? null), [X, _] = z(() => xc(i.files)), [be, fe] = z(() => (i.openEditorIds ?? []).filter((c) => (i.files ?? []).some((f) => f.id === c))), [m, w] = z(() => i.activeFileId && (i.files ?? []).some((c) => c.id === i.activeFileId) ? i.activeFileId : null), [T, F] = z(i.query ?? ""), [se, H] = z({ lineNumber: 1, column: 1 }), [ke, Ne] = z(null), [Ve, Se] = z({}), [Pe, ye] = z(js), [ee, te] = z([]), [$, ne] = z([]), [_e, Et] = z(null), [Ae, et] = z(i.chatInput ?? ""), [Yt, pt] = z(!1), [Jt, Zt] = z(""), [u, k] = z([]), [x, O] = z(null), [L, I] = z(null), [B, Y] = z(!1), [me, De] = z(() => Lc()), [G, W] = z(null), [Q, we] = z(null), [pe, Oe] = z(""), [Re, tt] = z(""), [Ze, nt] = z(""), [Fe, jn] = z("failed"), [ar, Wn] = z(null), [rr, A] = z("Ready"), [bt, sr] = z(() => o), ze = be.map((c) => X.find((f) => f.id === c)).filter(Boolean), V = m ? X.find((c) => c.id === m) ?? null : null, Mn = oe(() => Ss({ files: X, openEditorIds: be, activeFileId: m, versions: Ve, activeSelection: ke }), [m, ke, Ve, X, be]), Vn = oe(() => ({
    scope: Pe,
    removedAttachmentIds: ee,
    selectedFileIds: []
  }), [Pe, ee]), vt = oe(() => Js(Mn, X, Vn), [Vn, Mn, X]), Ee = mi(X, { autoRefresh: !0, includeDirty: !0 }), Dn = oe(() => $.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [$]), ir = oe(() => [...vt.attachments, ...Dn], [vt.attachments, Dn]), J = Os({ host: e, requestContext: vt.parts, chatSettings: me, setStatus: A }), Ke = oe(
    () => [...J.artifacts, ...u].sort((c, f) => f.createdAt - c.createdAt),
    [J.artifacts, u]
  ), At = B && !V, Kt = !V && !At && !D, ge = oe(() => X.filter((c) => c.dirty), [X]), or = oe(() => {
    const c = T.trim().toLowerCase();
    return c ? X.filter((f) => f.path.toLowerCase().includes(c)) : X;
  }, [X, T]);
  ie(() => {
    Se((c) => {
      const f = {};
      for (const v of X) f[v.id] = c[v.id] ?? 1;
      return f;
    });
  }, [X]), ie(() => {
    te([]), ne([]);
  ...
  const cr = N((c) => {
    if (c.kind === "index-hit") {
      ne((f) => f.filter((v) => v.id !== c.id)), A(`Removed project context: ${c.label}`);
      return;
    }
    te((f) => f.includes(c.id) ? f : [...f, c.id]), A(`Removed chat context: ${c.label}`);
  }, []), Ot = N(() => {
    Y(!0), w(null), he(!0), A("Atomek settings opened");
  }, []), Fn = N(() => {
    Y(!1), w((c) => c ?? be.at(-1) ?? null), be.length === 0 && he(!1), A("Atomek settings closed");
  }, [be]), at = N((c) => {
    Se((f) => ({ ...f, [c]: (f[c] ?? 1) + 1 }));
  }, []), lr = N((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const f = ln(n), v = c.clientX, P = q, Z = (gt) => {
      const nn = P + (v - gt.clientX);
    ...
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const f = ln(n), v = c.clientX, P = h, Z = (gt) => {
      const nn = P + (gt.clientX - v);
      g(jt(nn, f.primaryMin, f.primaryMax));
    }, de = () => {
      window.removeEventListener("pointermove", Z), window.removeEventListener("pointerup", de);
    };
    window.addEventListener("pointermove", Z), window.addEventListener("pointerup", de);
    ...
    sr((f) => {
      ...
      return localStorage.setItem(Ia, JSON.stringify(v)), v;
    });
  }, []), le = N((c, f) => {
    fe((v) => v.includes(c.id) ? v : [...v, c.id]), w(c.id), he(!1), Wn(f ?? null), H({ lineNumber: f ?? 1, column: 1 });
  }, []), ur = N((c) => {
    const f = {
      id: `atomek-doc:${c.id}`,
      name: c.fileName,
    ...
    _((v) => Mt(v, [f])), le(f), S("chat"), E(!0), ce(!0), A(`Opened Atomek guide: ${c.title}`);
  }, [le]), hr = N((c) => {
    if (!c.fileId) return;
    const f = X.find((v) => v.id === c.fileId);
    f && (le(f, c.range?.startLineNumber ?? 1), A(`Revealed context: ${c.label}`));
  }, [X, le]), yt = N(async () => {
    if (dn(ge, "open new files"))
      try {
        ...
        })), le(c[0]), A(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && A(`Open file failed: ${c.message}`);
      }
  }, [ge, le, Ye]), Bt = N(async () => {
    if (dn(ge, "open another folder"))
      try {
        const c = await gs(), f = c.handle ? `folder:${c.name}:${Date.now()}` : null;
        c.handle && f && await na(f, c.handle), Le(f), Xe(c), _(c.files), fe([]), w(null), he(!1), Ye({ name: c.name, path: c.name, kind: "folder", handleKey: f ?? void 0, at: Date.now() }), A(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
        c.name !== "AbortError" && A(`Open folder failed: ${c.message}`);
      }
  }, [ge, Ye]), fr = N((c) => {
    if (!m) return;
    let f = !1;
    ...
    if (V)
      try {
        const c = await rn(V);
        _((f) => f.map((v) => v.id === c.id ? c : v)), A(`Saved ${c.name}`);
      } catch (c) {
        A(`Save failed: ${c.message}`);
      }
  }, [V]), mr = N(async (c) => {
    const f = X.find((P) => P.id === c);
    if (!f) return null;
    const v = await rn(f);
    return _((P) => P.map((Z) => Z.id === v.id ? v : Z)), v;
  }, [X]), In = N(async () => {
    const c = X.filter((f) => f.dirty);
    if (c.length === 0) {
      A("No dirty files to save");
      return;
    }
    try {
      const f = await Promise.all(c.map((P) => rn(P))), v = new Map(f.map((P) => [P.id, P]));
      _((P) => P.map((Z) => v.get(Z.id) ?? Z)), W(null), A(`Saved ${f.length} dirty file${f.length === 1 ? "" : "s"}`);
    } catch (f) {
      A(`Save all failed: ${f.message}`);
    }
  }, [X]), Gt = N((c) => {
    const f = X.find((v) => v.id === c);
    if (f?.dirty && !window.confirm(`${f.name} has unsaved changes. Close without saving?`)) {
      A(`Close canceled — ${f.name} has unsaved changes`);
      return;
    }
    fe((v) => {
      const P = v.filter((Z) => Z !== c);
      return m === c && (w(P.at(-1) ?? null), Ne(null)), P;
    });
  }, [m, X]), pr = N(() => {
    dn(ge, "close all editors") && (fe([]), w(null), Wn(null), he(!1), A("Closed all editors"));
  }, [ge]), Qt = N(() => {
    const c = X.filter((v) => v.name.startsWith("Untitled")).length + 1, f = {
...
`,
    ...
    return ne(P), P.length === 0 ? (A(Ee.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), f) : (v.mode !== "hybrid" && v.reason ? A(v.reason) : A(`${P.length} project context hit${P.length === 1 ? "" : "s"} · ${v.reason ?? "hybrid retrieval"}`), [
      ...f,
      ...Pi(P)
    ]);
  }, [vt.parts, Pe, me, e, Ee]), $t = N(() => {
    if (!V && ze.length === 0) {
      A("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const f = [
    ...
    E(!0), S("chat"), A("Asking Atomek to synthesize an AI artifact…"), (async () => {
      ...
      !P || P.status === "error" || J.createArtifact({
        ...
        E(!0), S("outputs"), j(!0), R("output");
      });
    })();
  }, [V, J, Je, ze.length]), _t = N(async (c) => {
    if (c.kind === "folder") {
      const v = X.filter((P) => P.path === c.path || P.path.startsWith(`${c.path}/`));
      if (ve?.name === c.name || v.length > 0) {
        v.length > 0 && (Xe((P) => P?.name === c.name ? P : { name: c.name, files: v }), _((P) => Mt(P, v))), p("explorer"), b(!0), w(null), he(!1), Ye({ ...c, at: Date.now() }), A(`Opened recent folder ${c.name}`);
        return;
      }
    }
    const f = X.find((v) => v.path === c.path || v.name === c.name);
    if (f) {
      le(f), A(`Opened recent ${f.name}`);
      return;
    }
    if (!c.handleKey) {
      A("Recent item has no stored browser permission. Use Open File or Open Folder once, then Atomek can restore it.");
      return;
    }
    try {
      if (c.kind === "folder") {
        ...
        if (!Z || !await an(Z, "readwrite")) {
          A("Browser permission expired for this folder. Click Open Folder and pick it once to refresh the recent handle.");
          return;
        }
        const de = await Zn(Z);
        Le(c.handleKey), Xe(de), _(de.files), fe([]), w(null), he(!1), Ye({ ...c, at: Date.now() }), A(`Reopened ${de.name} (${de.files.length} text files indexed)`);
        return;
      }
      const v = await cn(c.handleKey);
      if (!v || !await an(v, "readwrite")) {
        A("Browser permission expired for this file. Click Open File and pick it once to refresh the recent handle.");
        return;
      }
      const P = await ks([v]);
      if (!P[0]) {
        A(`Recent file is not readable text: ${c.name}`);
        return;
      }
      _((Z) => Mt(Z, P)), le(P[0]), Ye({ ...c, at: Date.now() }), A(`Reopened ${P[0].name}`);
    } catch (v) {
      A(`Open recent failed: ${v instanceof Error ? v.message : String(v)}`);
    }
  }, [X, ve?.name, le, Ye]), Te = N((c) => {
    E(!0), S("chat"), et(""), (async () => {
      const f = await Je(c);
    ...
    if (!e.skills?.get) {
      A("Tytus skill registry is not available in this host build");
      return;
    }
    try {
...
`)), E(!0), S("chat"), A(`Attached skill ${f.title} to chat input`);
    } catch (f) {
      A(`Failed to attach skill: ${f instanceof Error ? f.message : String(f)}`);
    }
  }, [e.skills]), br = N((c) => {
    const f = J.messages.findIndex((P) => P.id === c.id), v = J.messages.slice(0, f < 0 ? void 0 : f).reverse().find((P) => P.role === "user");
    if (!v?.body.trim()) {
      A("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    if (!V) {
      A("No active file to save as AI artifact");
      return;
    }
    J.createArtifact({
      title: V.path,
    ...
    const f = hn(X, un(c.title || c.kind)), v = {
    ...
    _((P) => [...P, v]), le(v), A(`Opened ${c.title} as editable file`);
  }, [X, le]), tn = N(() => {
    const c = window.prompt("Check command/name (manual capture only; Atomek will not execute it)");
    if (c === null) return;
    const f = c.trim();
    if (!f) {
      A("Manual check capture canceled — command/name was empty");
      return;
    }
...
`), Z = {
    ...
    k((de) => [Z, ...de]), j(!0), R("output"), A(`Captured manual check: ${f}`);
  }, []), rt = N((c, f) => {
    const v = Ii({
      body: f,
      files: X,
      ...
      edits: v.edits.map(yc),
      skipped: v.skipped
    };
    return v.edits.length > 1 ? (I(P), O(null), A(`Previewing AI workspace patch for ${v.edits.length} files`), !0) : v.edits.length === 1 ? (O(P.edits[0]), I(null), A(`Previewing AI patch for ${P.edits[0].fileName}`), !0) : (A(v.skipped.length > 0 ? `No applicable edit found. ${v.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [V, Ve, X]), Rn = N((c, f) => {
    const v = {
    ...
    k((P) => [v, ...P]), E(!0), S("outputs"), j(!0), R("output"), A(`Saved local job output: ${c}`), (f.includes("```diff") || f.includes("--- a/")) && rt(c, f);
  }, [rt]), gr = N(() => {
    const c = Ae.trim();
    if (!c) return;
    et("");
    ...
    (async () => {
      ...
      rt(P.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", P.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [J, Je, Ae, rt]), kr = N(() => {
    _e && (Et(null), Te(oc(_e)));
  }, [Te, _e]), wt = N((c) => {
    ...
    nt(v), Oe(""), tt(""), jn("failed"), j(!0), R("terminal"), A(f.commands.length > 0 ? `Manual check ready: copy ${f.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [X]), Pr = N((c) => {
    c.trim() && (ht(c), nt(c), A(`Manual check command copied: ${c}`));
  }, []), xr = N(() => {
    we((c) => {
      if (!c) return c;
      const f = ao(c, pe), v = f.commands.at(-1)?.command ?? "";
      return v && nt(v), f;
    }), Oe("");
  }, [pe]), Nr = N(() => {
    if (!Q || !Ze.trim()) {
      A("Select or enter a manual check command before capturing output");
      return;
    }
    const c = Ze.trim();
    we((f) => f && ro(f, c, Fe, Re)), tt(""), A(`Captured manual check result: ${c} (${Fe})`);
  }, [Re, Ze, Q, Fe]), zr = N(() => {
    Q && Te(so(Q));
  }, [Te, Q]), Tr = N(() => {
    if (!x) return;
    const c = X.find((f) => f.id === x.fileId);
    if (!c) {
      A(`Cannot apply edit — ${x.fileName} is no longer open`), O(null);
      return;
    }
    c.content !== x.originalContent && !window.confirm(`${x.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (_((f) => f.map((v) => v.id === x.fileId ? { ...v, content: x.proposedContent, dirty: !0 } : v)), at(x.fileId), W(`AI edit applied to ${c.name}. Save All to persist it to disk.`), O(null), wt(`AI edit applied to ${c.name}`), A(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [at, X, x, wt]), Xr = N(() => {
    if (!x) return;
    const c = hn(X, `${un(x.fileName)}-ai-edit`), f = {
    ...
    _((v) => [...v, f]), le(f), O(null), A(`Opened proposed edit as ${c}`);
  }, [V?.language, X, le, x]), Lr = N(() => {
    if (!L) return;
    const c = new Map(L.edits.map((v) => [v.fileId, v]));
    X.some((v) => {
      const P = c.get(v.id);
    ...
    })), c.forEach((v, P) => at(P)), W(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), I(null), wt(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), A(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [at, X, L, wt]), Ar = N(() => {
    if (!L) return;
    const c = [...X], f = L.edits.map((v) => {
      const P = hn(c, `${un(v.fileName)}-ai-edit`), Z = X.find((gt) => gt.id === v.fileId), de = {
    ...
    _((v) => [...v, ...f]), f[0] && le(f[0]), I(null), A(`Opened ${f.length} proposed edit file${f.length === 1 ? "" : "s"}`);
  }, [X, le, L]), En = N((c) => {
    if (c === "explain") {
  ...
  return ie(() => {
    const c = (f) => {
      (f.metaKey || f.ctrlKey) && (f.key.toLowerCase() === "o" && (f.preventDefault(), yt()), f.key.toLowerCase() === "s" && (f.preventDefault(), Ut()), f.key.toLowerCase() === "w" && (f.preventDefault(), m && Gt(m)), f.key.toLowerCase() === "b" && (f.preventDefault(), b((P) => !P)), f.shiftKey && f.key.toLowerCase() === "f" && (f.preventDefault(), p("search"), b(!0)), (f.key.toLowerCase() === "k" || f.key.toLowerCase() === "p") && (f.preventDefault(), pt(!0)));
    };
    ...
    if (!n) return;
    const c = ln(n);
    g((f) => jt(f, c.primaryMin, c.primaryMax)), ue((f) => jt(f, c.secondaryMin, c.secondaryMax));
  }, [n]), ie(() => {
    const c = { primaryVisible: y, primaryWidth: h, secondaryVisible: K, secondaryWidth: q, markdownPreviewVisible: M };
    localStorage.setItem(qa, JSON.stringify(c));
  }, [M, y, h, K, q]), ie(() => {
    ...
    return (async () => {
      try {
        ...
        const v = await Zn(f);
        if (c) return;
        Le(i.folder?.handleKey ?? null), Xe(v), _((P) => Tc(P, v.files)), fe((P) => P.filter((Z) => v.files.some((de) => de.id === Z) || X.some((de) => de.id === Z))), A(`Restored ${v.name} (${v.files.length} text files indexed)`);
      } catch (f) {
        c || A(`Workspace restore failed: ${f instanceof Error ? f.message : String(f)}`);
      }
    })(), () => {
      c = !0;
    };
    ...
    zc({
      activity: l,
      folder: ve ? { name: ve.name, handleKey: He } : null,
      files: Nc(X),
      openEditorIds: be,
      activeFileId: m,
      query: T,
  ...
  }, [l, m, U, re, Ae, He, X, ve, be, T, bt, C, D]), ie(() => {
    localStorage.setItem(Ea, JSON.stringify(me));
  }, [me]), /* @__PURE__ */ d(
    "div",
    {
      ref: t,
      ...
      children: [
        ...
        y && /* @__PURE__ */ d("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: dr, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => pt(!0), children: "Workspace" }),
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ a(Zo, { file: V, folder: ve, showWelcome: Kt }),
            ...
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: V ? /* @__PURE__ */ d("div", { className: V.language === "markdown" && M ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(Cr, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                Lo,
                {
                  file: V,
                  revealLine: ar,
              ...
              V.language === "markdown" && M && /* @__PURE__ */ a(ec, { content: V.content })
            ] }) : At ? /* @__PURE__ */ a(
              cc,
              {
                host: e,
            ...
            ) : Kt ? /* @__PURE__ */ a(Go, { host: e, openFile: yt, openFolder: Bt, newFile: Qt, recent: bt, reopenRecent: _t, setStatus: A, openControlTower: () => {
              p("computer"), b(!0);
            }, openChat: () => {
              S("chat"), E(!0);
            }, openEmbeddedDoc: ur }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
            ...
            re && /* @__PURE__ */ a(
              nc,
              {
                ...
                copyManualCheckCommand: Pr,
                addManualCheckCommand: xr,
                recordManualCheckResult: Nr,
                askAgentFromManualChecks: zr,
                onClose: () => j(!1)
              }
            )
          ] })
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            ...
            contextScope: Pe,
            setContextScope: ye,
            contextAttachments: ir,
            removeContextAttachment: cr,
            revealContextAttachment: hr,
            projectIndexSummary: `${Ee.snapshot.files.length} files · ${Ee.snapshot.chunks.length} chunks`,
            projectIndexStale: Ee.isStale,
            refreshProjectIndex: () => {
              const c = Ee.refresh(X);
              A(`Project index refreshed: ${c.files.length} files · ${c.chunks.length} chunks`);
            },
            onResizeStart: lr,
            onClose: () => E(!1)
          }
        ...
        Yt && /* @__PURE__ */ a(
          _o,
          {
            ...
            commands: [
              ...
              ...bt.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => {
                _t(c);
              } })),
              { label: "File: Save All", detail: `${ge.length} dirty file${ge.length === 1 ? "" : "s"}`, run: () => {
                In();
              }, disabled: ge.length === 0 },
              { label: "File: Close All Editors", detail: `${ze.length} open editor${ze.length === 1 ? "" : "s"}`, run: pr, disabled: ze.length === 0 },
              ...
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => wt("Manual check requested from command palette") },
              ...
              { label: "AI: Save Active File as Artifact", detail: V ? "Persist active file in host.ai artifacts" : "Open a file first", run: wr, disabled: !V },
        ...
        /* @__PURE__ */ a(vc, { status: rr, file: V ?? Ao, cursor: se, fileCount: X.length, dirtyCount: ge.length })
      ]
    }
  );
}
function Ro({ active: e, setActive: t, togglePrimary: n, openSettings: r, settingsActive: s }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a("button", { className: "workbench-activity-brand", title: "Toggle side bar", "aria-label": "Toggle side bar", onClick: n, children: /* @__PURE__ */ a(Da, { size: 30, variant: "cream" }) }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(ua, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(cs, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(fa, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(da, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    ...
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(hs, { size: 23 }), label: "Settings", active: s, onClick: r })
  ] });
}
...
function Eo(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(Qo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(oa, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(oa, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(tr, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(Yo, { ...e });
}
function Yo(e) {
  const t = !e.folder, n = oe(() => Jo(e.files, e.folder?.name), [e.files, e.folder?.name]), [r, s] = z(() => /* @__PURE__ */ new Set());
  ie(() => {
    const o = Ga(n);
    s((l) => {
      const p = new Set(l);
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(We, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((o) => /* @__PURE__ */ a(Sn, { file: o, active: o.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(o), label: o.name, detail: o.path }, o.id)),
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(It, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : Qa(n, e.activeFileId, e.openWorkbenchFile, r, i)
      ] }),
      ...
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((o) => /* @__PURE__ */ d("button", { className: "workbench-tree-row", onClick: () => {
        ...
        o.kind === "file" ? /* @__PURE__ */ a(xn, { size: 14 }) : /* @__PURE__ */ a(ha, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name })
      ] }, `${o.path}-${o.at}`))
    ] })
  ] });
}
function Sn({ file: e, active: t, onOpen: n, basePath: r, depth: s = 0, label: i, detail: o }) {
...
function Qa(e, t, n, r, s, i = 0) {
  return e.map((o) => {
    if (o.file)
      return /* @__PURE__ */ a(Sn, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: i, label: o.name }, o.file.id);
    ...
    return /* @__PURE__ */ d("div", { children: [
      /* @__PURE__ */ d("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => s(o.path), title: l ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        ...
        l ? /* @__PURE__ */ a(zn, { size: 14 }) : /* @__PURE__ */ a(ha, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: o.children.length })
      ] }),
      l ? Qa(o.children, t, n, r, s, i + 1) : null
    ] }, o.path);
  });
}
function Zo({ file: e, folder: t, showWelcome: n }) {
...
function Ko(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    ...
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(ns, { size: 16 }) })
  ] });
}
function Bo(e) {
...
function Go({
  ...
  const [b, h] = z("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [g, C] = z(null), [S, K] = z([]), [E, q] = z(!1), [ue, re] = z(null), [j, U] = z(null), [R, M] = z("pod-local"), ce = N(async () => {
    q(!0), U(null);
    try {
      ...
      C(m ?? null), K(w), m && o(`Agent team loaded · ${m.resources.length} resources · ${m.warnings.length} warnings`);
    } catch (m) {
      const w = m instanceof Error ? m.message : String(m);
      U(w), o(`Agent team resource load failed: ${w}`);
    } finally {
      q(!1);
    }
  }, [e.missions, e.resources, o]), D = N((m, w) => {
    try {
      e.windows.openOrFocus(m), o(`Opened ${w}`);
    } catch (T) {
  ...
  const he = N(async () => {
    ...
    const m = b.trim() || "Coordinate a Tytus mission.", w = $e(m, g, R);
    q(!0);
    try {
      ...
      await e.missions.write({
        rootPath: T.rootPath,
        files: [
          { path: "MISSION.md", content: Dt(F, g, null, [], m, w) },
          { path: "MISSION.json", content: Ua(F, g, m, w) },
          { path: "RESOURCES.md", content: Ft(g) },
          { path: "TASKS.md", content: Ka(Xt(m, g, w)) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(se)}
` },
...
`) }
  ...
  }, [b, g, e.missions, l, o, R]), ve = Bo(g), Xe = jo(g), He = Wo(g), Le = Uo(g), X = g?.warnings ?? [], _ = Vo(g), be = mt(g, $e(b, g, R)), fe = Xt(b, g, be.id);
  return /* @__PURE__ */ d("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ d("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ d("div", { className: "workbench-brand-lockup", "aria-label": "Atomek", children: [
        /* @__PURE__ */ a(Da, { size: 74, variant: "acid" }),
        /* @__PURE__ */ d("div", { children: [
          /* @__PURE__ */ a(wo, { className: "workbench-brand-wordmark" }),
      ...
      /* @__PURE__ */ a("div", { className: "workbench-team-preset-strip", "aria-label": "Team presets", children: _.map((m) => /* @__PURE__ */ d(
        "button",
        {
          ...
          children: [
            ...
            /* @__PURE__ */ a("small", { children: m.assignments.map((w) => w.resourceLabel).slice(0, 3).join(" · ") })
          ]
        },
        m.id
      )) }),
    ...
    /* @__PURE__ */ d("section", { className: "workbench-control-grid", children: [
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-doc-grid", children: zo.map((m) => /* @__PURE__ */ d("button", { className: "workbench-doc-card", onClick: () => y(m), title: m.summary, children: [
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: He.map((m, w) => /* @__PURE__ */ d("div", { className: `workbench-fabric-node ${m.status.replace(/\s+/g, "-")}`, children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-fabric-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(St.sharedFiles, "Shared Files"), children: "Open shared files" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(St.podInspector, "Pod Inspector"), children: "Open pods" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(St.channels, "Channels"), children: "Open channels" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(St.settings, "Agent Settings"), children: "Open agent settings" })
        ] })
      ] }),
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ d("header", { children: [
          ...
          /* @__PURE__ */ a("span", { children: S.length ? `${S.length} missions` : "none yet" })
        ] }),
        S.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Create a mission once; it stays in Tytus Home/Missions and can be resumed here." }) : null,
        S.slice(0, 4).map((m) => /* @__PURE__ */ d("button", { className: "workbench-control-preset", onClick: () => {
          zt(m), l();
        ...
        /* @__PURE__ */ d("div", { className: "workbench-control-resource-list", children: [
          ...
          Le.map((m) => /* @__PURE__ */ d("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ d("div", { children: [
              ...
              /* @__PURE__ */ a("span", { children: Hn(m) })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${m.status}`, children: m.status })
          ] }, m.id))
        ] }),
        X.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: X.slice(0, 2).map((m) => /* @__PURE__ */ d("span", { children: [
          m.code,
          ": ",
          m.message
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: fe.map((m, w) => /* @__PURE__ */ d("button", { className: `workbench-task-card ${m.status}`, onClick: () => h(m.prompt), children: [
          /* @__PURE__ */ a("span", { children: w + 1 }),
          /* @__PURE__ */ a("strong", { children: m.title }),
...
function Qo({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: s }) {
  const i = oe(() => $o(e, t), [e, t]), o = oe(() => {
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? o.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : o.map((l) => {
        ...
        return /* @__PURE__ */ d("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(Sn, { file: p, active: p.id === s, onOpen: () => r(p) }),
...
function _o(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((s) => ({
    label: s.path,
    detail: `Open ${va(s.language)} file`,
...
function tc(e) {
  const t = e.session ? io(e.session) : "pending";
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(ss, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(Yr, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(Me, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(rc, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      tr,
      {
        host: e.host,
...
function $a({ body: e }) {
  const t = oe(() => Co(e), [e]), [n, r] = z(null), s = N((i, o) => {
    (async () => await ht(o) && (r(i), window.setTimeout(() => r((p) => p === i ? null : p), 1200)))();
  }, []);
      ...
      return /* @__PURE__ */ d("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ d("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ d("button", { onClick: () => s(i.key, i.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ a(Ir, { size: 12 }) : /* @__PURE__ */ a(mn, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
  ...
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ d("option", { value: h.id, children: [
              h.title,
              " · ",
              sc(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ d("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ d("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: Ws(e.contextScope) }),
        /* @__PURE__ */ a("em", { children: ra(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            ...
            const g = typeof h.score == "number" ? h.score.toFixed(2) : null, C = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, S = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, K = [
              h.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: K, children: [
              ...
              h.label,
              g ? /* @__PURE__ */ a("small", { children: g }) : null,
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: ra(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(pa, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(ds, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function ra(e, t, n) {
  const r = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : _a(e.gatewayPreference)];
...
function cc(e) {
  ...
  ie(() => {
    ...
    return (async () => {
      ...
      try {
        const S = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: g.signal
        });
        if (g.signal.aborted) return;
        n(S.map((K) => ({ id: K.id, gatewayLabel: K.gatewayLabel }))), s(S.length > 0 ? `${S.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (S) {
        if (g.signal.aborted) return;
        n([]), s(`Model discovery failed: ${S instanceof Error ? S.message : String(S)}`);
      }
    })(), () => g.abort();
    ...
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          _a(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-embedding-models", children: i.map((g) => /* @__PURE__ */ a("option", { value: g.id, children: g.gatewayLabel }, `${g.gatewayLabel}:${g.id}`)) })
        ] }),
...
function er({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: s, openOutputAsFile: i, previewEditFromOutput: o, canPreviewEdit: l = !1, compact: p = !1 }) {
  return /* @__PURE__ */ d("div", { className: `workbench-panel-list ${p ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((y) => /* @__PURE__ */ d("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ d("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void ht(y.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => i(y), children: "Open as file" }),
        o ? /* @__PURE__ */ a(
          "button",
          {
            className: sa(y.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(y),
            disabled: !l,
            children: sa(y.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        y.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(y.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ a($a, { body: y.body })
    ] }, y.id))
  ] });
}
function lc({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
...
function dc({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: wc(i.proposedContent) })
      ] }, i.fileId)),
...
function hc(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, uc(e.content), "```");
  else if (t.length > 0) {
...
function tr({
  ...
  const [l, p] = z([]), [y, b] = z([]), [h, g] = z(null), [C, S] = z(!1), [K, E] = z(null), [q, ue] = z("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [re, j] = z(null), [U, R] = z([]), [M, ce] = z(() => Fo()), [D, he] = z([]), [ve, Xe] = z([]), [He, Le] = z([]), X = Qe([]), [_, be] = z("task-execute"), [fe, m] = z("mission"), w = U.find((u) => u.status === "running" || u.status === "canceling") ?? U[0] ?? null, T = o === "dock", F = oe(() => mt(h, $e(q || M?.goal || "", h, M?.teamPresetId)), [q, M?.goal, M?.teamPresetId, h]), se = oe(() => Xt(q || M?.goal || "", h, F.id), [q, M?.goal, F.id, h]), H = se.find((u) => u.id === _) ?? se[1] ?? se[0] ?? null, ke = i.filter((u) => u.dirty).length, Ne = s ? `${s.path} · ${s.language} · ${s.content.length.toLocaleString()} chars${s.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${ke ? ` · ${ke} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Ve = oe(() => [
    {
  ...
  const Se = N(async (u = M) => {
    if (!u?.rootPath || !e.missions?.listRuns)
      return Le([]), [];
    try {
      const k = await e.missions.listRuns(u.rootPath);
      return Le(k), X.current = k, k;
    } catch (k) {
      return t(`Mission run history failed: ${k instanceof Error ? k.message : String(k)}`), [];
    }
  }, [e.missions, M, t]), Pe = N(async (u, k) => {
    if (!u) return;
    const x = [...k].sort((L, I) => aa(I).localeCompare(aa(L))).slice(0, 100), O = x.map((L) => JSON.stringify(L)).join(`
`) + (x.length ? `
` : "");
    u.rootPath && e.missions?.write ? await e.missions.write({ rootPath: u.rootPath, files: [{ path: "RUNS.jsonl", content: O }] }) : u.handle && await wn(u.handle, "RUNS.jsonl", O), Le(x), X.current = x;
  }, [e.missions]), ye = N(async (u, k = M) => {
    if (!k) return;
    const x = [
      u,
      ...X.current.filter((O) => O.id !== u.id)
    ];
    await Pe(k, x);
  }, [M, Pe]), ee = N(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      p([]), b([]), g(null), E("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    S(!0), E(null);
    try {
      const [u, k, x, O] = await Promise.all([
        e.local?.listTools?.().catch((L) => (t(`Local tool discovery failed: ${L instanceof Error ? L.message : String(L)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((L) => (t(`Skill registry discovery failed: ${L instanceof Error ? L.message : String(L)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((L) => (t(`Resource graph discovery failed: ${L instanceof Error ? L.message : String(L)}`), null)) ?? Promise.resolve(null),
        e.missions?.list?.().catch((L) => (t(`Mission list failed: ${L instanceof Error ? L.message : String(L)}`), [])) ?? Promise.resolve([])
      ]);
      p(u), b(k), g(x), he(O), M?.rootPath && Se(M), t(`Agent team loaded · ${u.length} tools · ${k.length} skills · ${O.length} missions${x ? ` · ${x.resources.length} resources` : ""}`);
    } catch (u) {
      E(u instanceof Error ? u.message : String(u));
    } finally {
      S(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, Se, M, t]);
  ie(() => {
    ee();
  }, [ee]), ie(() => {
    const u = (k) => {
      const x = k.detail;
      x?.missionId && (ce(x), Se(x)), ee();
    };
  ...
  const te = N((u) => {
    ...
    ], L = $e(k || u.goal, h, u.teamPresetId), I = Xt(k || u.goal, h, L), B = [
      { path: "MISSION.md", content: Dt(u, h, s, i, k, L) },
      { path: "MISSION.json", content: Ua(u, h, k, L) },
      { path: "RESOURCES.md", content: Ft(h) },
      { path: "TASKS.md", content: Ka(I) },
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: O.map((Y) => JSON.stringify(Y)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: X.current.map((Y) => JSON.stringify(Y)).join(`
`) + (X.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (u.rootPath && e.missions?.write)
      ...
      for (const Y of B) await So(u.handle, Y.path, Y.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    Xe(O), zt(u);
  }, [s, e.missions, ve, i, h]), ne = N(async (u, k = {}) => {
    if (M)
      return await $(M, u), M;
    const x = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, O = u.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let L = null;
    if (e.missions?.create) {
      const B = await e.missions.create({ title: x, goal: O });
      L = {
      ...
      L = {
        handle: B,
        name: B.name,
        missionId: `mission-${Date.now()}-${Do(B.name)}`,
        title: x,
        goal: O,
        source: "browser"
      };
    ...
    if (!L) return null;
    const I = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${L.rootPath ?? L.name}` };
    return ce(L), zt(L), Xe([I]), await $(L, O, [I]), t(`Mission pack ready in ${L.rootPath ?? L.name}`), L;
  }, [e.missions, M, t, $]), _e = N(async (u) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const k = q.trim() || `Open ${u.label} from Atomek with current context.`, x = u.kind === "ai-cli" ? await ne(k) : M;
      await e.local.openTerminal({
        toolId: u.id,
        command: u.command,
        cwd: x?.rootPath ?? ia(s),
        prompt: x ? `Atomek mission pack ready at ${x.rootPath ?? x.name}. Read MISSION.md and RESOURCES.md. ${Ne}` : `Opened from Atomek. ${Ne}`
  ...
  }, [s, Ne, ne, e.local, q, M, t]), Et = N(async () => {
    try {
  ...
  }, [ne, q, t]), Ae = N(async (u, k, x, O = M, L) => {
    if (!O) return;
    const I = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), B = L ?? `runs/${I}-${u.id}.md`, Y = B.split("/").pop() || `${I}-${u.id}.md`;
    if (O.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: O.rootPath, files: [{ path: B, content: k }] });
    else if (O.handle) {
      const me = await lt(O.handle, "runs");
      await wn(me, Y, k);
    }
    await $(O, q, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${u.label} exited ${x}; transcript saved to ${B}`,
      data: { toolId: u.id, exitCode: x, transcript: B }
    }]);
  }, [e.missions, q, M, $]), et = N(async (u) => {
    ...
    j(u.id);
    const O = `local-run-${Date.now()}-${u.id}`, L = ot();
    R((I) => [{
      id: O,
      toolId: u.id,
      label: u.label,
...
`)
      }), B = I.transcriptPath ?? (x?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id}.md` : void 0), Y = [
      ...
      }), R((G) => G.map((W) => W.id === O ? { ...W, jobId: I.id, transcriptPath: B } : W));
      const De = (G) => {
        R((W) => W.map((Q) => Q.id === O ? G(Q) : Q));
      };
...
`);
          ...
          }), r(`${u.label} local job`, W), Ae(u, W, G, x, B).catch((Q) => {
  ...
  }, [s, ne, e.local, q, i, h, r, Ae, H, t, ye, $]), Yt = N(async (u) => {
    const k = gn(u);
    if (!k) {
    ...
    let O = null;
    const L = xe(u), I = `pod-run-${Date.now()}-${k}`, B = ot(), Y = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      O = await ne(x), O && await $(O, x, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${L} mission task started`,
    ...
    const me = {
      ...
      transcriptPath: Y,
      summary: `Started ${L} for ${H?.title ?? "manual pod run"}`
    };
    R((W) => [{
      id: I,
      toolId: u.id,
      ...
      taskTitle: H?.title ?? "Manual pod run",
      transcriptPath: Y,
      lines: [`[Atomek] Dispatching ${L} via Tytus host bridge…`, `[Atomek] Pod: ${k}`, `[Atomek] Task: ${H?.title ?? "manual pod run"}`]
    }, ...W].slice(0, 6)), await ye(me, O).catch((W) => {
      t(`Mission run index failed: ${W instanceof Error ? W.message : String(W)}`);
    });
    const De = [], G = (W) => {
...
`)
      ...
      const tt = typeof Re == "string" ? Re : bc(Re);
      G(tt);
      const Ze = [
        `# Pod job — ${L}`,
        "",
...
`);
      R((nt) => nt.map((Fe) => Fe.id === I ? { ...Fe, status: "complete", finishedAt: Date.now(), lines: [...Fe.lines, "[Atomek] Pod task complete."].slice(-500) } : Fe)), await ye({ ...me, status: "complete", finishedAt: ot(), summary: `${L} completed`, transcriptPath: Y }, O), r(`${L} pod job`, Ze), await Ae({ id: u.id, label: L, kind: "pod-agent", status: u.status }, Ze, 0, O, Y), t(`${L} completed mission task`);
    } catch (W) {
      const Q = W instanceof Error ? W.message : String(W);
      G(`[FAIL] ${Q}`), R((pe) => pe.map((Oe) => Oe.id === I ? { ...Oe, status: "failed", finishedAt: Date.now(), lines: Oe.lines.slice(-500) } : Oe)), await ye({ ...me, status: "failed", finishedAt: ot(), exitCode: -1, summary: Q, transcriptPath: Y }, O).catch((pe) => {
        t(`Mission run index failed: ${pe instanceof Error ? pe.message : String(pe)}`);
      });
      const we = [`# Pod job failed — ${L}`, "", `- Pod: ${k}`, `- Error: ${Q}`, "", "```text", De.join(`
`), "```", ""].join(`
`);
      r(`${L} pod job failed`, we), await Ae({ id: u.id, label: L, kind: "pod-agent", status: u.status }, we, -1, O, Y).catch((pe) => {
        t(`Pod transcript save failed: ${pe instanceof Error ? pe.message : String(pe)}`);
      }), t(`Pod task failed: ${Q}`);
    }
  }, [s, ne, e.daemon, q, i, h, r, Ae, H, t, ye, $]), pt = N(async (u) => {
    if (!u.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    R((k) => k.map((x) => x.id === u.id ? { ...x, status: "canceling", lines: [...x.lines, "[Atomek] Cancel requested…"] } : x)), ye({
      id: u.id,
      jobId: u.jobId,
      toolId: u.toolId,
      label: u.label,
    ...
    try {
      ...
      R((x) => x.map((O) => O.id === u.id ? { ...O, status: "running", lines: [...O.lines, `[Atomek] Cancel failed: ${k instanceof Error ? k.message : String(k)}`] } : O)), t(`Local job cancel failed: ${k instanceof Error ? k.message : String(k)}`);
    }
  }, [e.local, t, ye]), Jt = N(async (u) => {
    const k = [
...
`);
    ...
    const k = u.setupAction, x = k?.commandPreview ? `${k.label}: ${k.commandPreview}` : k?.deepLink ? `${k.label}: ${k.deepLink}` : k?.label ?? `${xe(u)} needs setup`;
    t(x), ht(k?.commandPreview ?? k?.deepLink ?? x);
  }, [t]);
  return /* @__PURE__ */ d("aside", { className: T ? "workbench-agent-dock" : "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: T ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      ...
      fe === "mission" ? /* @__PURE__ */ d(We, { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: se.map((u, k) => /* @__PURE__ */ d(
          "button",
          {
      ...
      fe === "runs" ? /* @__PURE__ */ d(We, { children: [
        w ? /* @__PURE__ */ d("div", { className: "workbench-agent-run", children: [
          /* @__PURE__ */ d("header", { children: [
            /* @__PURE__ */ d("div", { children: [
...
`)), disabled: w.lines.length === 0, children: "Save output" }),
              w.jobId && (w.status === "running" || w.status === "canceling") ? /* @__PURE__ */ d("button", { className: "workbench-button-subtle danger", onClick: () => {
                pt(w);
              }, disabled: w.status === "canceling", children: [
                /* @__PURE__ */ a(pa, { size: 12 }),
                " ",
...
`) || "[waiting for output]" })
        ...
        He.length ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: He.slice(0, T ? 4 : 8).map((u) => /* @__PURE__ */ d("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ d("div", { children: [
            ...
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void ht(u.transcriptPath ?? u.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, u.id)) })
        ] }) : null
      ] }) : null,
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
          ...
          l.map((u) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
            ...
            /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                _e(u);
              }, disabled: u.status !== "available", title: "Launch this tool in a fresh Tytus Terminal with mission context and execute it immediately.", children: fc(u) }),
              u.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                et(u);
              }, disabled: u.status !== "available" || re !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: mc(u, re) }) : null
            ] })
          ] }, u.id))
        ] }),
...
function vc({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: s }) {
  return /* @__PURE__ */ d("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: va(t.language) })
  ] });
}
function dn(e, t) {
...
function Ac({ host: e }) {
  return /* @__PURE__ */ a(qo, { host: e });
}
function Hc(e) {
  return function() {
    return /* @__PURE__ */ a(Ac, { host: e.host });
  };
}
export {
  Hc as default
