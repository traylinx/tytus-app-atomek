 ...
 */
const Lr = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Ar = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), Rn = (e) => {
  const t = Ar(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, oa = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), Or = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Wr = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Mr = ne("check", Wr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Vr = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], Vt = ne("chevron-down", Vr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const ps = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function bs() {
  const e = zn();
...
async function vs() {
  ...
  return zt(n.filter(Boolean));
}
async function ys() {
  const e = zn()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await va(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await Tn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: zt(n) };
}
async function ws(e) {
  const t = await Promise.all(e.map((n) => Xn(n, n.name, "local-file")));
  return zt(t.filter(Boolean));
}
async function Yn(e) {
  const t = [];
  return await Tn(e, e.name, t, 0, 320), { name: e.name, handle: e, files: zt(t) };
}
async function tn(e, t = "readwrite") {
  try {
...
async function Tn(e, t, n, r, s) {
  if (!(n.length >= s || r > 8))
    for await (const i of e.values()) {
      if (n.length >= s) break;
      if (i.kind === "directory") {
        if (ps.has(i.name)) continue;
        await Tn(i, `${t}/${i.name}`, n, r + 1, s);
        continue;
      }
...
const rt = "atomek:default", ks = 3e3, Ln = "tytus.atomek.threadTitleOverrides", Jn = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, yt = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, Ps = (e, t = ks) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Zn = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, xs = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Kn = (e) => ({
  id: e.id,
  title: e.title,
  kind: xs(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = An();
    n[e] = t, localStorage.setItem(Ln, JSON.stringify(n));
  } catch {
  }
}, Xs = (e) => {
  try {
    const t = An();
    delete t[e], localStorage.setItem(Ln, JSON.stringify(t));
  } catch {
  }
}, Un = (e) => {
  const t = An();
...
function Ts({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const s = e.ai, [i, o] = N(null), [l, m] = N([]), [y, b] = N([]), [h, w] = N([]), [X, j] = N([]), [R, $] = N(Jn), [Y, ce] = N(!1), ee = _e(!0), V = _e(null), J = P(async () => {
    if (!s) {
      $(Jn);
      return;
    }
    try {
      ...
      $({
        ...
        source: "none",
        label: "Tytus AI unavailable",
        reason: p instanceof Error ? p.message : String(p)
      });
    }
  }, [s]), E = P(async (p) => {
    if (s)
      try {
        const k = await s.listArtifacts({ threadId: p });
        if (!ee.current) return;
        w(k.map(Kn));
      } catch (k) {
        r(`AI artifacts unavailable: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [s, r]), W = P(async (p) => {
    if (s)
      try {
        const k = await s.listMessages(p);
        if (!ee.current) return;
        const L = l.find((I) => I.id === p) ?? null;
        L && o(L), b(k.map(yt).filter(Boolean)), j([]), await E(p);
      } catch (k) {
        r(`Load chat failed: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [s, E, r, l]), le = P(async () => {
    if (s)
      try {
        const p = Un(await s.listThreads({ workspaceKey: rt, status: "active" })), k = p[0] ?? await s.createThread({ workspaceKey: rt, title: "Atomek chat" });
        if (!ee.current) return;
        m(p[0] ? p : [k]), o(k);
        const L = await s.listMessages(k.id);
        if (!ee.current) return;
        b(L.map(yt).filter(Boolean)), await E(k.id);
      } catch (p) {
        r(`AI chat unavailable: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [s, E, r]);
  ie(() => (ee.current = !0, J(), le(), () => {
    ee.current = !1;
  }), [le, J]);
  const M = P(async () => {
    if (!s) return null;
    if (i) return i;
    const p = await s.createThread({ workspaceKey: rt, title: "Atomek chat" });
    return ee.current && (o(p), m((k) => [p, ...k.filter((L) => L.id !== p.id)])), p;
  }, [s, i]), Pe = P(async () => {
    if (!s) return;
    const p = await s.createThread({ workspaceKey: rt, title: "Atomek chat" });
    o(p), m((k) => [p, ...k.filter((L) => L.id !== p.id)]), b([]), w([]), j([]), r("New AI chat created");
  }, [s, r]), Ae = P(async (p, k) => {
    if (!s) return;
    const L = k.trim();
    if (L)
      try {
        const I = s.updateThread, oe = l.find((ge) => ge.id === p) ?? i, H = typeof I == "function" ? await I({ threadId: p, title: L }) : { ...oe ?? await M(), id: p, title: L, updatedAt: Date.now() };
        typeof I != "function" && zs(p, L), m((ge) => ge.map((be) => be.id === H.id ? H : be)), o((ge) => ge?.id === H.id ? H : ge), r(typeof I == "function" ? `Renamed chat: ${H.title}` : `Renamed chat locally: ${H.title}`);
      } catch (I) {
        r(`Rename chat failed: ${I instanceof Error ? I.message : String(I)}`);
      }
  }, [s, M, r, i, l]), He = P(async (p) => {
    if (s)
      try {
        await s.deleteThread(p), Xs(p);
        const k = l.filter((L) => L.id !== p);
        if (m(k), i?.id === p) {
          const L = k[0] ?? await s.createThread({ workspaceKey: rt, title: "Atomek chat" });
          o(L), m((oe) => oe.some((H) => H.id === L.id) ? oe : [L, ...oe]);
          const I = await s.listMessages(L.id);
          b(I.map(yt).filter(Boolean)), await E(L.id);
        }
        r("Deleted AI chat");
      } catch (k) {
        r(`Delete chat failed: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [s, E, r, i?.id, l]), Xe = P(async (p) => {
    if (!s) return null;
    try {
      const k = await M();
      if (!k) return null;
      const L = await s.createArtifact({
        threadId: k.id,
        messageId: p.messageId ?? null,
        title: p.title?.trim() || Zn(p.body, "Atomek artifact"),
        kind: p.kind ?? "markdown",
        body: p.body
      }), I = Kn(L);
      return w((oe) => [I, ...oe.filter((H) => H.id !== I.id)]), r(`Saved AI artifact: ${I.title}`), I;
    } catch (k) {
      return r(`Save artifact failed: ${k instanceof Error ? k.message : String(k)}`), null;
    }
  }, [s, M, r]), T = P(async (p) => {
    if (s)
      try {
        await s.deleteArtifact(p), w((k) => k.filter((L) => L.id !== p)), r("Deleted AI artifact");
      } catch (k) {
        r(`Delete artifact failed: ${k instanceof Error ? k.message : String(k)}`);
      }
  }, [s, r]), _ = P(async (p) => {
    if (!s) return null;
    try {
      const k = await s.writeMemory({
        title: p.title?.trim() || Zn(p.body, "Atomek memory"),
        body: p.body,
        metadata: {
          source: "atomek",
          messageId: p.messageId ?? null
      ...
      return j((L) => [k, ...L.filter((I) => I.id !== k.id)].slice(0, 5)), r(`Remembered: ${k.title}`), k;
    } catch (k) {
      return r(`Remember failed: ${k instanceof Error ? k.message : String(k)}`), null;
    }
  }, [s, r]), fe = P(async (p) => {
    if (!s) return [];
    const k = await s.searchMemory({ query: p, limit: 5 });
    return ee.current && j(k), k;
  }, [s]), xe = P(async (p, k = {}) => {
    const L = p.trim();
    if (!L || !s) return null;
    ce(!0);
    ...
    try {
      const H = await M();
      if (!H) return null;
      const ge = await fe(L).catch(() => []), be = Ns(ge), Ee = k.requestContext ?? t, ve = be ? [...Ee, be] : [...Ee];
      let Ve = null;
      for await (const ae of s.sendMessage({
        threadId: H.id,
        body: L,
        ...
        if (ae.type === "message_created") {
          const re = yt(ae.message);
          if (!re) continue;
          re.role === "assistant" && (Ve = re.id), b((G) => [...G.filter((se) => se.id !== re.id), re]);
        }
        if (ae.type === "token" && (Ve = ae.messageId, b((re) => re.some((G) => G.id === ae.messageId) ? re.map(
          (G) => G.id === ae.messageId ? { ...G, body: ae.body, status: "streaming" } : G
        ) : [...re, {
          id: ae.messageId,
          role: "assistant",
          body: ae.body,
          status: "streaming",
          createdAt: Date.now()
        }])), ae.type === "message_updated" || ae.type === "done") {
          const re = yt(ae.message);
          if (!re) continue;
          re.role === "assistant" && (I = re), b((G) => G.map((se) => se.id === re.id ? re : se)), re.gatewayLabel && r(`AI answered via ${re.gatewayLabel}`);
        }
        if (ae.type === "run_failed") {
          const re = oe.signal.aborted;
          r(re ? "AI response stopped" : `AI failed: ${ae.error}`), Ve && b((G) => G.map((se) => se.id !== Ve ? se : re ? { ...se, status: "complete", error: void 0, body: se.body || "Stopped by user." } : { ...se, status: "error", error: ae.error, body: ae.error }));
        }
      }
      const pe = Un(await s.listThreads({ workspaceKey: rt, status: "active" }).catch(() => []));
      return ee.current && pe.length > 0 && m(pe), J(), I;
    } catch (H) {
      return H instanceof DOMException && H.name === "AbortError" ? (r("AI response stopped"), I) : (r(`AI failed: ${H instanceof Error ? H.message : String(H)}`), null);
    } finally {
      V.current === oe && (V.current = null), ce(!1);
    }
  }, [s, n.gatewayPreference, n.model, M, fe, J, t, r]), Q = P(() => {
    V.current?.abort(), r("Stopping AI response…");
  }, [r]);
  return {
    aiStatus: R,
    artifacts: h,
...
const Ls = (e) => `tytus-workbench:///${encodeURI(e.path)}`, As = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Os = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: s }) => {
  const i = new Set(t), o = e.map((l) => {
    ...
    return {
      ...
      language: l.language,
      version: r[l.id] ?? 1,
      contentHash: As(l.content),
      dirty: l.dirty,
      source: l.source,
      selection: m && s ? s : void 0,
      updatedAt: Date.now(),
      open: i.has(l.id),
      active: m
    };
...
}, Cs = "active-file", Hs = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Ss = (e, t = "file") => `${t}:${e.id}`, js = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let s = [];
  return t.scope === "active-selection" ? r?.selection ? s = [Ws(r, r.selection, !0)] : r && (s = [At(r, !0, "Active file")]) : t.scope === "active-file" ? r && (s = [At(r, !0, "Active file")]) : t.scope === "open-editors" ? s = e.openDocumentIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => At(i, !0, "Open editor")) : t.scope === "selected-files" && (s = t.selectedFileIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => At(i, !1, "Selected file"))), Vs(s).filter((i) => !n.has(i.id));
}, Vs = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, Ds = 4e3, Fs = 8e3, Is = 4e3, qs = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Rs = (e, t, n) => {
  const r = Ms(e, n), s = [], i = new Map(t.map((o) => [o.id, o]));
  for (const o of r) {
    if (!o.fileId) continue;
    const l = i.get(o.fileId), m = e.byId.get(o.fileId);
    ...
    const y = [
      ...
      o.range ? `Range: ${Bn(o.range)}` : null
    ].filter(Boolean).join(`
`), b = o.kind === "selection" && o.range ? Es(l.content, o.range) : l.content, h = o.kind === "selection" ? Ds : o.label.startsWith("Open editor") ? Is : Fs;
    s.push({
...
`));
}, Bn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, Gn = 75e4, Ys = 2800, Js = 240, Zs = 80, Ks = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${$e(t.content)}`).sort().join("")
), mn = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? Gn) || e.content.length > (t.maxFileBytes ?? Gn) ? "huge" : !t.includeDirty && e.dirty ? null : ei(e.path) ? "vendor" : ti(e) ? "binary" : null;
}, Qn = (e, t = {}) => {
  const n = Date.now(), r = [], s = [], i = [];
  for (const o of e) {
    const l = o.size ?? o.content.length, m = mn(o, t);
    if (m) {
      s.push({ fileId: o.id, name: o.name, path: o.path, language: o.language, reason: m, size: l });
      continue;
    }
    const y = $e(o.content), b = Bs(o, y, n, t);
    i.push(...b), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
...
}, Bs = (e, t = $e(e.content), n = Date.now(), r = {}) => {
  const s = Math.max(500, r.maxChunkChars ?? Ys), i = Math.min(Math.max(0, r.chunkOverlapChars ?? Js), Math.floor(s / 2)), o = Math.max(1, r.maxChunksPerFile ?? Zs), l = $s(e.content), m = [];
  let y = 0, b = 0;
  for (; y < e.content.length && m.length < o; ) {
    const h = Qs(e.content, y, Math.min(e.content.length, y + s)), w = e.content.slice(y, h).trim();
    if (w.length > 0) {
      const X = _s(l, y, h);
      m.push({
        id: `${e.id}:${t}:${b}:${X.startLineNumber}-${X.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const r = $n(e, t), s = $n(e, Math.max(t, n - 1));
  return {
    startLineNumber: r + 1,
    startColumn: t - e[r] + 1,
    endLineNumber: s + 1,
  ...
  return 0;
}, ei = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => Ks.has(t)), ti = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && Us.has(t) || e.content.includes("\0")) return !0;
...
class ni {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Qn(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Qn(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return ai(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const st = (e = [], t = {}) => new ni(e, t), ai = (e, t, n = {}) => {
  ...
  for (const l of e.files) {
    ...
    if (mn(m, n)) {
      i.push({ fileId: l.fileId, path: m.path, status: "skipped-now", indexedHash: l.hash, currentHash: $e(m.content) });
      continue;
    }
    const b = $e(m.content);
    b !== l.hash ? i.push({ fileId: l.fileId, path: m.path, status: "changed", indexedHash: l.hash, currentHash: b }) : m.dirty !== l.dirty && i.push({ fileId: l.fileId, path: m.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: b });
  }
  for (const l of t)
    !s.has(l.id) && !mn(l, n) && i.push({ fileId: l.id, path: l.path, status: "new", currentHash: $e(l.content) });
  const o = e.signature !== ga(t);
  return { stale: i.length > 0, signatureChanged: o, files: i };
}, ri = 8, si = 12e3, ka = (e, t, n = {}, r) => {
  const s = ci(t);
  if (s.length === 0) return [];
  const i = new Set(r?.files.map((b) => b.fileId) ?? []), o = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => li(b, s)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, h) => h.score - b.score || b.chunk.path.localeCompare(h.chunk.path) || b.chunk.ordinal - h.chunk.ordinal), l = [], m = /* @__PURE__ */ new Set();
  let y = n.maxChars ?? si;
  for (const b of o) {
    if (l.length >= (n.limit ?? ri) || y <= 0) break;
...
[...context clipped...]` : b.chunk.text;
    l.push(ii(b, w, i.has(b.chunk.fileId))), y -= w.length;
  }
  return l;
}, ii = (e, t = e.chunk.text, n = !1) => {
...
`), ci = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const y = an(n, m), b = an(r, m), h = an(s, m), w = i === m ? 1 : 0;
    y + b + h + w !== 0 && (l.push(m), o += Math.min(y, 12), o += b * 4, o += h * 6, o += w * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (o += 3));
  }
  ...
  const n = e.toLowerCase(), r = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, y) => m - y)[0] ?? 0, s = Math.max(0, r - 120), i = Math.min(e.length, r + 260), o = s > 0 ? "…" : "", l = i < e.length ? "…" : "";
  return `${o}${e.slice(s, i).replace(/\s+/g, " ").trim()}${l}`;
}, ui = (e, t = {}) => {
  const { autoRefresh: n = !1, ...r } = t, s = _e(e), i = _e(null);
  i.current || (i.current = st(e, r));
  const [o, l] = N(() => i.current?.getSnapshot() ?? st(e, r).getSnapshot());
  s.current = e;
  const m = P((w = s.current) => {
    const X = (i.current ?? st([], r)).refresh(w, r);
    return l(X), X;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), y = P((w = s.current) => {
    const X = (i.current ?? st([], r)).update(w, r);
    return l(X), X;
  }, [m]);
  ie(() => {
    n && m(e);
  }, [n, e, m]);
  const b = (i.current ?? st([], r)).staleReport(e), h = P((w, X = {}) => {
    const j = i.current ?? st([], r);
    return ka(j.getSnapshot(), w, X, j.staleReport(s.current));
  }, []);
  return { snapshot: o, staleReport: b, isStale: b.stale, refresh: m, update: y, retrieve: h };
}, hi = "tytus.atomek.semanticVector:v1", Pa = "__gateway_default__", xa = (e) => e?.trim() || Pa, fi = (e) => {
  const t = xa(e);
  return t === Pa ? "gateway default" : t;
}, Na = (e, t, n) => [
  hi,
  ...
  if (!za()) return null;
  try {
    const r = Na(e, t, n), s = window.localStorage.getItem(r);
    if (!s) return null;
  ...
  if (!za()) return null;
  try {
    const t = Na(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, wt = 8, bi = 12e3, vi = 180, yi = async (e, t, n, r, s = {}, i, o) => {
  const l = ka(t, n, { ...s, limit: Math.max(s.limit ?? wt, 16) }, i), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: l.slice(0, s.limit ?? wt), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const y = xa(r.embeddingModel), b = await m.embedText({
      input: n,
    ...
    const w = new Map(l.map((V) => [V.chunkId, V])), X = zi(n), j = /* @__PURE__ */ new Map();
    t.chunks.filter((V) => s.includeDirty !== !1 || !V.dirty).slice(0, vi).forEach((V) => j.set(V.id, V)), l.forEach((V) => {
      const J = t.byChunkId.get(V.chunkId);
      J && j.set(J.id, J);
    });
    const R = Array.from(j.values()), $ = [];
    let Y = 0;
    for (const V of R) {
      o?.aborted;
      const J = mi(e.appId, y, V) ?? await gi(m, e.appId, y, V, r, o);
      if (!J) continue;
      J.updatedAt > Date.now() - 1e3 && (Y += 1);
      const E = xi(h, J.vector);
      if (!Number.isFinite(E)) continue;
      const le = w.get(V.id)?.score ?? Xi(V, X), M = Ni(le) * 0.42 + Math.max(0, E) * 0.58;
      $.push({ chunk: V, keywordScore: le, vectorScore: E, score: M, matchedTerms: X.filter((Pe) => V.text.toLowerCase().includes(Pe)) });
    }
    if ($.length === 0) return { hits: l.slice(0, s.limit ?? wt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: Y };
    const ce = new Set(i?.files.map((V) => V.fileId) ?? []), ee = $.sort((V, J) => J.score - V.score || J.keywordScore - V.keywordScore || V.chunk.path.localeCompare(J.chunk.path) || V.chunk.ordinal - J.chunk.ordinal).slice(0, Math.max(s.limit ?? wt, 1)).map((V) => ki(V, ce.has(V.chunk.fileId)));
    return {
      hits: Pi(ee, s.maxChars ?? bi),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${fi(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: Y
    };
  } catch (y) {
    return {
      hits: l.slice(0, s.limit ?? wt),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: Ti(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, Ni = (e) => Math.min(1, Math.max(0, e / 24)), zi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Xi = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, Li = /```([^\n`]*)\n([\s\S]*?)```/g, Ai = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function On(e) {
  return Array.from(e.matchAll(Li)).map((t) => Ta(t[1] ?? "", ji(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Ta(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, s = [];
  for (const i of n) {
    const o = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? r[o[1].toLowerCase()] = Mi(o[2]) : s.push(i.toLowerCase());
  }
...
function La(e) {
  return [...On(e).filter((r) => Le(r, "diff") || Le(r, "patch")).map((r) => r.content), e].filter((r, s, i) => Oi(r) && i.findIndex((o) => o === r) === s);
}
function Oi(e) {
...
`);
    /^@@\s+-\d+/m.test(b) && i.push({ raw: b, ...rn(b) });
  }
  return i.length > 0 ? i : [{ raw: t, ...rn(t) }];
}
function rn(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), r = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), s = Fe(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = Fe(t?.[3] || t?.[4] || r?.[1] || r?.[2] || "") || null, o = i && i !== "/dev/null" ? i : s;
...
function pn(e) {
  return On(e).filter((t) => Hi(t)).map((t) => ({
    content: Wi(t.content),
...
`), s = [];
  ...
  for (let l = 0; l < r.length; l += 1) {
    const m = r[l].match(Ai);
    if (!m) continue;
    o = !0;
    const y = Number(m[1]), b = Math.max(0, y - 1);
    if (b < i) return null;
    for (s.push(...n.slice(i, b)), i = b, l += 1; l < r.length; l += 1) {
      ...
      const w = h[0], X = h.slice(1);
      if (w === " ") {
        if (n[i] !== X) return null;
        s.push(X), i += 1;
        continue;
      }
      if (w === "-") {
...
function Vi(e) {
  ...
  for (const b of La(t))
    for (const h of Ci(b)) {
      if (!h.path) {
        if (s) {
          const j = bn(s.content, h.raw);
          j && j !== s.content && o.set(s.id, Ot({
            file: s,
            proposedContent: j,
            sourceTitle: r,
      ...
      const X = bn(w.file.content, h.raw);
      if (!X || X === w.file.content) {
  ...
  for (const b of pn(t)) {
    const h = b.path ? _n(n, b.path) : s ? { file: s, normalizedPatchPath: Fe(s.path), confidence: "exact" } : null;
    if (!h) {
  ...
  const m = pn(t).some((b) => b.path);
  if (o.size === 0 && s && !m) {
    const b = Di(t, s);
    b && b.content !== s.content && o.set(s.id, Ot({
      file: s,
      proposedContent: b.content,
      sourceTitle: r,
      extractionLabel: b.label,
...
`), s = Math.max(n.length, r.length);
  let i = 0, o = 0, l = 0;
  for (let m = 0; m < s; m += 1)
    n[m] !== r[m] && (n[m] === void 0 ? i += 1 : r[m] === void 0 ? o += 1 : l += 1);
...
function Ot(e) {
  ...
  return {
    ...
    stats: Fi(e.file.content, e.proposedContent),
    base: {
...
const Ft = (e) => typeof e == "object" && e !== null, Ei = (e) => Ft(e.ai) ? e.ai : null, Yi = (e) => {
  if (!Ft(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, Ji = (e) => {
  const t = Yi(e);
  return !t || !Ft(e) ? null : { ...e, id: t };
}, je = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(je) : Ft(e) ? Object.values(e).some(je) : !1, Zi = async (e, t) => {
  const n = Ei(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(Ji).filter((s) => !!s) : [];
}, Ki = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : je(e.capability) || je(e.capabilities) || je(e.modality) || je(e.modalities) || je(e.task) || je(e.tasks) || je(e.type) || je(e.kind), Oa = (e) => typeof e == "object" && e !== null, Ui = (e) => Oa(e.ai) ? e.ai : null, Ca = (e) => typeof Ui(e)?.embedText == "function", Bi = async (e, t) => Ca(e) ? (await Zi(e, t)).filter(Ki) : [], Gi = (e) => Oa(e.ai) ? Ca(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", Qi = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], $i = [
...
function no(e) {
  const t = e.results.length > 0 ? e.results.map((r, s) => [
    ...
    `Status: ${r.status}`,
    "Output:",
    uo(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function ro(e) {
  ...
  for (const r of t) {
    const s = so(r.content);
    if (!s?.scripts) continue;
    const i = Sa(r.path), o = io(e, i, s);
    if (!o) continue;
    const l = Object.keys(s.scripts).filter((y) => typeof s.scripts?.[y] == "string"), m = oo(l);
    for (const y of m) {
      const b = `${o} run ${y}`;
      n.push({
        id: ja(`${i}:${b}`),
...
function io(e, t, n) {
  ...
  for (const [r, s] of $i)
    if (e.some((i) => lo(i.path) === r && Sa(i.path) === t)) return s;
  return null;
}
function oo(e) {
  const t = Qi.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, s) => r.localeCompare(s));
  return [...t, ...n];
}
function co(e) {
  const t = /* @__PURE__ */ new Set();
...
function bo({ className: e }) {
  return /* @__PURE__ */ a("img", { src: po, alt: "ATOMEK", className: e, draggable: !1 });
}
const vo = `# Tytus Resource Fabric
...
], xo = "tytus-atomek-handles", lt = "handles", No = 1, Ma = () => new Promise((e, t) => {
  const n = indexedDB.open(xo, No);
  n.onupgradeneeded = () => {
    const r = n.result;
    r.objectStoreNames.contains(lt) || r.createObjectStore(lt);
  }, n.onsuccess = () => e(n.result), n.onerror = () => t(n.error ?? new Error("Failed to open Atomek handle store"));
});
async function ea(e, t) {
  const n = await Ma();
  await new Promise((r, s) => {
    const i = n.transaction(lt, "readwrite");
    i.objectStore(lt).put(t, e), i.oncomplete = () => r(), i.onerror = () => s(i.error ?? new Error("Failed to persist file handle"));
  }), n.close();
}
async function sn(e) {
  const t = await Ma(), n = await new Promise((r, s) => {
    const o = t.transaction(lt, "readonly").objectStore(lt).get(e);
    o.onsuccess = () => r(o.result ?? null), o.onerror = () => s(o.error ?? new Error("Failed to read file handle"));
  });
  return t.close(), n;
}
const zo = Xr(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Xo = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Va = "tytus.workspace.recent", Da = "tytus.workspace.layout", Fa = "tytus.atomek.session.v2", Ia = "tytus.atomek.chatAiSettings", qa = "tytus.atomek.currentMission", vn = "tytus.atomek.currentMissionChanged", jt = {
...
function on(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - To), r = n < 1180, s = r ? 200 : 240, i = r ? 300 : 340, o = r ? 420 : 560, l = Math.max(s, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), y = Math.floor(n * (r ? 0.34 : 0.36)), b = n - m - o, h = Math.max(i, Math.min(r ? 500 : 640, y, b));
...
function Co(e) {
  const t = e?.resources ?? [], n = (l) => l.status === "ready" || l.status === "available", r = t.filter((l) => l.kind === "pod-agent" && ut(l) === "openclaw"), s = t.filter((l) => l.kind === "pod-agent" && ut(l) === "hermes"), i = t.filter((l) => l.kind === "local-cli" && n(l)), o = t.filter((l) => l.kind === "shared-folder" && n(l));
  return [
...
function gt(e, t, n, r, s) {
  return {
    ...
    resourceLabel: r ? ke(r) : s,
    status: r?.status ?? "needs-setup",
    trustTier: r?.trustTier ?? "not-available",
    sandbox: r?.sandbox ?? "none"
  };
}
function ht(e, t = "repo-repair") {
  const n = xt.find((M) => M.id === t) ?? xt[0], r = e?.resources ?? [], s = Qe(r, (M) => M.kind === "pod-agent" && ut(M) === "openclaw"), i = Qe(r, (M) => M.kind === "pod-agent" && ut(M) === "hermes"), o = Qe(r, (M) => M.kind === "ail-route"), l = Qe(r, (M) => M.kind === "shared-folder"), m = De(r, "local-cli", ["claude"]), y = De(r, "local-cli", ["opencode", "open code"]), b = De(r, "local-cli", ["codex"]), h = De(r, "local-cli", ["pi"]), w = De(r, "local-cli", ["kimi"]), X = Qe(r, (M) => M.kind === "local-cli"), j = De(r, "app-skill", ["juli3ta", "music", "song"]), R = De(r, "app-skill", ["blender", "3d"]), $ = De(r, "app-skill", ["remotion", "video", "render"]), Y = Qe(r, (M) => M.kind === "app-skill"), ce = t === "pod-local" ? s ?? i ?? o ?? m ?? X : t === "creative-production" ? i ?? o ?? m ?? X : t === "research-watch" ? s ?? o ?? i ?? m ?? X : m ?? y ?? o ?? X, ee = t === "creative-production" ? j ?? R ?? $ ?? Y ?? y ?? m ?? X : y ?? m ?? b ?? X, V = t === "pod-local" ? b ?? h ?? w ?? s ?? m ?? X : t === "research-watch" ? h ?? w ?? b ?? s ?? X : b ?? h ?? s ?? w ?? X, J = t === "creative-production" ? j ?? R ?? $ ?? Y : t === "research-watch" ? De(r, "app-skill", ["browser", "channel", "telegram"]) ?? Y : Y, E = [
    gt("planner", "Planner", "Break goal into tasks, risks, and required context.", ce, "No planner agent ready"),
    gt("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", ee, "No local/app implementer ready"),
    gt("reviewer", "Reviewer", "Independent critique before approval or handoff.", V, "No reviewer agent ready"),
    gt("team-desk", "Team Desk", "Shared mission folder for transcripts, outputs, proposals, and handoff.", l, "Mission folder only until shared folder is bound")
  ];
  (t === "creative-production" || J) && E.push(gt("app-tool", "App Tool", "Drive installed local app skill through mission context.", J, "No configured app skill"));
  const W = E.filter((M) => M.status === "ready" || M.status === "available" || M.status === "degraded").length, le = W >= E.length - 1 ? "ready" : W >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: le, assignments: E };
}
function et(e, t, n) {
  if (n && xt.some((s) => s.id === n)) return n;
  const r = e.toLowerCase();
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(r) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(r) ? "research-watch" : (t?.resources ?? []).some((s) => s.kind === "pod-agent" && Ea(s)) ? "pod-local" : "repo-repair";
}
function jo(e) {
  return xt.map((t) => ht(e, t.id));
}
function St(e) {
...
function Pt(e) {
  ...
  try {
    localStorage.setItem(qa, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(vn, { detail: t }));
  } catch {
  }
}
function Mo() {
  try {
    ...
    return !t.missionId || !t.title ? null : {
      ...
      teamPresetId: xt.some((n) => n.id === t.teamPresetId) ? t.teamPresetId : void 0
    };
  } catch {
    return null;
  }
...
function Nt(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", s = ht(t, et(r, t, n)), i = (h) => s.assignments.find((w) => w.role === h), o = i("planner"), l = i("implementer"), m = i("reviewer"), y = i("team-desk"), b = i("app-tool");
  return [
    {
      id: "task-scope",
      ...
      resourceHint: o ? St(o) : "Planner agent",
      role: "planner",
    ...
    },
    {
      ...
      resourceHint: l ? St(l) : "Local/app implementer",
      role: "implementer",
    ...
    } : null,
    {
      ...
      resourceHint: m ? `${St(m)} · ${y?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
...
function Wt(e, t, n, r, s, i) {
  const o = ht(t, et(s || e.goal, t, i ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
...
function Za(e, t, n, r) {
  const s = et(n || e.goal, t, r ?? e.teamPresetId), i = ht(t, s), o = Nt(n || e.goal, t, s);
...
function Do({ host: e }) {
  const t = _e(null), [n, r] = N(0), s = he(() => xc(), []), i = he(() => yc(), []), [o, l] = N(i.activity ?? "computer"), [m, y] = N(s.primaryVisible), [b, h] = N(s.primaryWidth), [w, X] = N(i.secondaryTab ?? "chat"), [j, R] = N(s.secondaryVisible), [$, Y] = N(s.secondaryWidth), [ce, ee] = N(!!i.bottomPanelVisible), [V, J] = N(i.bottomPanelTab ?? "problems"), [E, W] = N(s.markdownPreviewVisible), [le, M] = N(!!i.welcomeClosed), [Pe, Ae] = N(i.folder ? { name: i.folder.name, files: [] } : null), [He, Xe] = N(i.folder?.handleKey ?? null), [T, _] = N(() => wc(i.files)), [fe, xe] = N(() => (i.openEditorIds ?? []).filter((c) => (i.files ?? []).some((f) => f.id === c))), [Q, p] = N(() => i.activeFileId && (i.files ?? []).some((c) => c.id === i.activeFileId) ? i.activeFileId : null), [k, L] = N(i.query ?? ""), [I, oe] = N({ lineNumber: 1, column: 1 }), [H, ge] = N(null), [be, Ee] = N({}), [ve, Ve] = N(Cs), [pe, ae] = N([]), [re, G] = N([]), [se, It] = N(null), [Ye, Oe] = N(i.chatInput ?? ""), [qt, ft] = N(!1), [Rt, Et] = N(""), [Xt, u] = N([]), [g, z] = N(null), [A, O] = N(null), [q, B] = N(!1), [F, Ce] = N(() => Nc()), [Se, Z] = N(null), [S, te] = N(null), [ye, me] = N(""), [Te, Ie] = N(""), [Je, Ze] = N(""), [tt, Ke] = N("failed"), [_a, Sn] = N(null), [er, C] = N("Ready"), [Yt, tr] = N(() => vc()), Ne = fe.map((c) => T.find((f) => f.id === c)).filter(Boolean), D = Q ? T.find((c) => c.id === Q) ?? null : null, jn = he(() => Os({ files: T, openEditorIds: fe, activeFileId: Q, versions: be, activeSelection: H }), [Q, H, be, T, fe]), Wn = he(() => ({
    scope: ve,
    removedAttachmentIds: pe,
    selectedFileIds: []
  }), [ve, pe]), mt = he(() => Rs(jn, T, Wn), [Wn, jn, T]), qe = ui(T, { autoRefresh: !0, includeDirty: !0 }), Mn = he(() => re.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [re]), nr = he(() => [...mt.attachments, ...Mn], [mt.attachments, Mn]), K = Ts({ host: e, requestContext: mt.parts, chatSettings: F, setStatus: C }), Ue = he(
    () => [...K.artifacts, ...Xt].sort((c, f) => f.createdAt - c.createdAt),
    [K.artifacts, Xt]
  ), Tt = q && !D, Jt = !D && !Tt && !le, we = he(() => T.filter((c) => c.dirty), [T]), ar = he(() => {
    const c = k.trim().toLowerCase();
    return c ? T.filter((f) => f.path.toLowerCase().includes(c)) : T;
  }, [T, k]);
  ie(() => {
    Ee((c) => {
      const f = {};
      for (const v of T) f[v.id] = c[v.id] ?? 1;
      return f;
    });
  }, [T]), ie(() => {
    ae([]), G([]);
  ...
  const rr = P((c) => {
    if (c.kind === "index-hit") {
      G((f) => f.filter((v) => v.id !== c.id)), C(`Removed project context: ${c.label}`);
      return;
    }
    ae((f) => f.includes(c.id) ? f : [...f, c.id]), C(`Removed chat context: ${c.label}`);
  }, []), Lt = P(() => {
    B(!0), p(null), M(!0), C("Atomek settings opened");
  }, []), Vn = P(() => {
    B(!1), p((c) => c ?? fe.at(-1) ?? null), fe.length === 0 && M(!1), C("Atomek settings closed");
  }, [fe]), nt = P((c) => {
    Ee((f) => ({ ...f, [c]: (f[c] ?? 1) + 1 }));
  }, []), sr = P((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const f = on(n), v = c.clientX, x = $, U = (vt) => {
      const en = x + (v - vt.clientX);
      Y(Ht(en, f.secondaryMin, f.secondaryMax));
    }, ue = () => {
      window.removeEventListener("pointermove", U), window.removeEventListener("pointerup", ue);
    };
    window.addEventListener("pointermove", U), window.addEventListener("pointerup", ue);
  }, [$, n]), ir = P((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const f = on(n), v = c.clientX, x = b, U = (vt) => {
      const en = x + (vt.clientX - v);
      h(Ht(en, f.primaryMin, f.primaryMax));
    }, ue = () => {
      window.removeEventListener("pointermove", U), window.removeEventListener("pointerup", ue);
    };
    window.addEventListener("pointermove", U), window.addEventListener("pointerup", ue);
    ...
    tr((f) => {
      const v = [c, ...f.filter((x) => x.path !== c.path)].slice(0, 10);
      return localStorage.setItem(Va, JSON.stringify(v)), v;
    });
  }, []), de = P((c, f) => {
    xe((v) => v.includes(c.id) ? v : [...v, c.id]), p(c.id), M(!1), Sn(f ?? null), oe({ lineNumber: f ?? 1, column: 1 });
  }, []), or = P((c) => {
    const f = {
      id: `atomek-doc:${c.id}`,
      name: c.fileName,
    ...
    _((v) => ln(v, [f])), de(f), X("chat"), R(!0), W(!0), C(`Opened Atomek guide: ${c.title}`);
  }, [de]), cr = P((c) => {
    if (!c.fileId) return;
    const f = T.find((v) => v.id === c.fileId);
    f && (de(f, c.range?.startLineNumber ?? 1), C(`Revealed context: ${c.label}`));
  }, [T, de]), pt = P(async () => {
    if (cn(we, "open new files"))
      try {
        const c = await vs();
        if (c.length === 0) return;
        _((f) => ln(f, c)), await Promise.all(c.map(async (f) => {
          const v = f.handle ? `file:${f.path}` : void 0;
          f.handle && v && await ea(v, f.handle), Be({ name: f.name, path: f.path, kind: "file", handleKey: v, at: Date.now() });
        })), de(c[0]), C(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && C(`Open file failed: ${c.message}`);
      }
  }, [we, de, Be]), Zt = P(async () => {
    if (cn(we, "open another folder"))
      try {
        const c = await ys(), f = c.handle ? `folder:${c.name}` : null;
        c.handle && f && await ea(f, c.handle), Xe(f), Ae(c), _(c.files), xe([]), p(null), M(!1), Be({ name: c.name, path: c.name, kind: "folder", handleKey: f ?? void 0, at: Date.now() }), C(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
        c.name !== "AbortError" && C(`Open folder failed: ${c.message}`);
      }
  }, [we, Be]), lr = P((c) => {
    if (!Q) return;
    let f = !1;
    _((v) => v.map((x) => x.id !== Q || x.content === c ? x : (f = !0, { ...x, content: c, dirty: !0 }))), f && nt(Q);
  }, [Q, nt]), Kt = P(async () => {
    if (D)
      try {
        const c = await nn(D);
        _((f) => f.map((v) => v.id === c.id ? c : v)), C(`Saved ${c.name}`);
      } catch (c) {
        C(`Save failed: ${c.message}`);
      }
  }, [D]), dr = P(async (c) => {
    const f = T.find((x) => x.id === c);
    if (!f) return null;
    const v = await nn(f);
    return _((x) => x.map((U) => U.id === v.id ? v : U)), v;
  }, [T]), Dn = P(async () => {
    const c = T.filter((f) => f.dirty);
    if (c.length === 0) {
      C("No dirty files to save");
      return;
    }
    try {
      const f = await Promise.all(c.map((x) => nn(x))), v = new Map(f.map((x) => [x.id, x]));
      _((x) => x.map((U) => v.get(U.id) ?? U)), Z(null), C(`Saved ${f.length} dirty file${f.length === 1 ? "" : "s"}`);
    } catch (f) {
      C(`Save all failed: ${f.message}`);
    }
  }, [T]), Ut = P((c) => {
    const f = T.find((v) => v.id === c);
    if (f?.dirty && !window.confirm(`${f.name} has unsaved changes. Close without saving?`)) {
      C(`Close canceled — ${f.name} has unsaved changes`);
      return;
    }
    xe((v) => {
      const x = v.filter((U) => U !== c);
      return Q === c && (p(x.at(-1) ?? null), ge(null)), x;
    });
  }, [Q, T]), ur = P(() => {
    cn(we, "close all editors") && (xe([]), p(null), Sn(null), M(!1), C("Closed all editors"));
  }, [we]), Bt = P(() => {
    const c = T.filter((v) => v.name.startsWith("Untitled")).length + 1, f = {
...
`,
    ...
    return G(x), x.length === 0 ? (C(qe.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), f) : (v.mode !== "hybrid" && v.reason ? C(v.reason) : C(`${x.length} project context hit${x.length === 1 ? "" : "s"} · ${v.reason ?? "hybrid retrieval"}`), [
      ...f,
      ...wi(x)
    ]);
  }, [mt.parts, ve, F, e, qe]), Gt = P(() => {
    if (!D && Ne.length === 0) {
      C("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const f = [
      `Create a polished Markdown artifact from ${D?.path ?? `${Ne.length} open editors`}.`,
    ...
    R(!0), X("chat"), C("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const v = await Re(f), x = await K.askAgent(f, { requestContext: v });
      !x || x.status === "error" || K.createArtifact({
        messageId: x.id,
        title: `AI synthesis — ${D?.name ?? "open workspace"}`,
        kind: "markdown",
        body: x.body
      }).then(() => {
        R(!0), X("outputs"), ee(!0), J("output");
      });
    })();
  }, [D, K, Re, Ne.length]), Qt = P(async (c) => {
    const f = T.find((v) => v.path === c.path || v.name === c.name);
    if (f) {
      de(f), C(`Opened recent ${f.name}`);
      return;
    }
    if (!c.handleKey) {
    ...
    try {
      if (c.kind === "folder") {
        const U = await sn(c.handleKey);
        if (!U || !await tn(U, "readwrite")) {
          C("Browser permission expired for this folder. Use Open Folder once to refresh it.");
          return;
        }
        const ue = await Yn(U);
        Xe(c.handleKey), Ae(ue), _(ue.files), xe([]), p(null), M(!1), Be({ ...c, at: Date.now() }), C(`Reopened ${ue.name} (${ue.files.length} text files indexed)`);
        return;
      }
      const v = await sn(c.handleKey);
      if (!v || !await tn(v, "readwrite")) {
        C("Browser permission expired for this file. Use Open File once to refresh it.");
        return;
      }
      const x = await ws([v]);
      if (!x[0]) {
        C(`Recent file is not readable text: ${c.name}`);
        return;
      }
      _((U) => ln(U, x)), de(x[0]), Be({ ...c, at: Date.now() }), C(`Reopened ${x[0].name}`);
    } catch (v) {
      C(`Open recent failed: ${v instanceof Error ? v.message : String(v)}`);
    }
  }, [T, de, Be]), ze = P((c) => {
    R(!0), X("chat"), Oe(""), (async () => {
      const f = await Re(c);
      await K.askAgent(c, { requestContext: f });
    })();
  }, [K, Re]), Fn = P(async (c) => {
    if (!e.skills?.get) {
...
[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : f.body, x = [
...
`);
      Oe((U) => [U.trim(), x].filter(Boolean).join(`

`)), R(!0), X("chat"), C(`Attached skill ${f.title} to chat input`);
    } catch (f) {
      C(`Failed to attach skill: ${f instanceof Error ? f.message : String(f)}`);
    }
  }, [e.skills]), hr = P((c) => {
    const f = K.messages.findIndex((x) => x.id === c.id), v = K.messages.slice(0, f < 0 ? void 0 : f).reverse().find((x) => x.role === "user");
    if (!v?.body.trim()) {
      C("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const x = await Re(v.body);
      await K.askAgent(v.body, { requestContext: x });
    })();
  }, [K, Re]), fr = P((c) => {
    K.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const f = un(T, dn(c.title || c.kind)), v = {
    ...
    _((x) => [...x, v]), de(v), C(`Opened ${c.title} as editable file`);
  }, [T, de]), _t = P(() => {
...
`), U = {
    ...
    u((ue) => [U, ...ue]), ee(!0), J("output"), C(`Captured manual check: ${f}`);
  }, []), at = P((c, f) => {
    const v = Vi({
      body: f,
      files: T,
      ...
      edits: v.edits.map(pc),
      skipped: v.skipped
    };
    return v.edits.length > 1 ? (O(x), z(null), C(`Previewing AI workspace patch for ${v.edits.length} files`), !0) : v.edits.length === 1 ? (z(x.edits[0]), O(null), C(`Previewing AI patch for ${x.edits[0].fileName}`), !0) : (C(v.skipped.length > 0 ? `No applicable edit found. ${v.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [D, be, T]), In = P((c, f) => {
    const v = {
    ...
    u((x) => [v, ...x]), R(!0), X("outputs"), ee(!0), J("output"), C(`Saved local job output: ${c}`), (f.includes("```diff") || f.includes("--- a/")) && at(c, f);
  }, [at]), br = P(() => {
    const c = Ye.trim();
    if (!c) return;
    Oe("");
    const f = ac(c);
    (async () => {
      const v = await Re(c), x = await K.askAgent(c, { requestContext: v });
      if (!f || !x || x.status === "error") return;
      at(x.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", x.body) || C("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [K, Re, Ye, at]), vr = P(() => {
    se && (It(null), ze(rc(se)));
  }, [ze, se]), bt = P((c) => {
    ...
    Ze(v), me(""), Ie(""), Ke("failed"), ee(!0), J("terminal"), C(f.commands.length > 0 ? `Manual check ready: copy ${f.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [T]), yr = P((c) => {
    c.trim() && (dt(c), Ze(c), C(`Manual check command copied: ${c}`));
  }, []), wr = P(() => {
    te((c) => {
      if (!c) return c;
      const f = eo(c, ye), v = f.commands.at(-1)?.command ?? "";
      return v && Ze(v), f;
    }), me("");
  }, [ye]), gr = P(() => {
    if (!S || !Je.trim()) {
    ...
    const c = Je.trim();
    te((f) => f && to(f, c, tt, Te)), Ie(""), C(`Captured manual check result: ${c} (${tt})`);
  }, [Te, Je, S, tt]), kr = P(() => {
    S && ze(no(S));
  }, [ze, S]), Pr = P(() => {
    if (!g) return;
    const c = T.find((f) => f.id === g.fileId);
    if (!c) {
      C(`Cannot apply edit — ${g.fileName} is no longer open`), z(null);
      return;
    }
    c.content !== g.originalContent && !window.confirm(`${g.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (_((f) => f.map((v) => v.id === g.fileId ? { ...v, content: g.proposedContent, dirty: !0 } : v)), nt(g.fileId), Z(`AI edit applied to ${c.name}. Save All to persist it to disk.`), z(null), bt(`AI edit applied to ${c.name}`), C(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [nt, T, g, bt]), xr = P(() => {
    if (!g) return;
    const c = un(T, `${dn(g.fileName)}-ai-edit`), f = {
      id: `pending-edit-${g.fileId}-${Date.now()}`,
      name: c,
      path: c,
      language: D?.language ?? "markdown",
      content: g.proposedContent,
      dirty: !0,
      source: "generated"
    };
    _((v) => [...v, f]), de(f), z(null), C(`Opened proposed edit as ${c}`);
  }, [D?.language, T, de, g]), Nr = P(() => {
    if (!A) return;
    const c = new Map(A.edits.map((v) => [v.fileId, v]));
    T.some((v) => {
      const x = c.get(v.id);
      return x && v.content !== x.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (_((v) => v.map((x) => {
      const U = c.get(x.id);
      return U ? { ...x, content: U.proposedContent, dirty: !0 } : x;
    })), c.forEach((v, x) => nt(x)), Z(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), O(null), bt(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), C(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [nt, T, A, bt]), zr = P(() => {
    if (!A) return;
    const c = [...T], f = A.edits.map((v) => {
      const x = un(c, `${dn(v.fileName)}-ai-edit`), U = T.find((vt) => vt.id === v.fileId), ue = {
        id: `workspace-patch-${v.fileId}-${Date.now()}-${x}`,
        name: x,
        path: x,
        language: U?.language ?? "markdown",
    ...
    _((v) => [...v, ...f]), f[0] && de(f[0]), O(null), C(`Opened ${f.length} proposed edit file${f.length === 1 ? "" : "s"}`);
  }, [T, de, A]), qn = P((c) => {
    if (c === "explain") {
      ze("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (c === "improve") {
      ze("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (c === "plan") {
      ze("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (c === "edit") {
      ze("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    ze("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [ze]);
  return ie(() => {
    const c = (f) => {
      (f.metaKey || f.ctrlKey) && (f.key.toLowerCase() === "o" && (f.preventDefault(), pt()), f.key.toLowerCase() === "s" && (f.preventDefault(), Kt()), f.key.toLowerCase() === "w" && (f.preventDefault(), Q && Ut(Q)), f.key.toLowerCase() === "b" && (f.preventDefault(), y((x) => !x)), f.shiftKey && f.key.toLowerCase() === "f" && (f.preventDefault(), l("search"), y(!0)), (f.key.toLowerCase() === "k" || f.key.toLowerCase() === "p") && (f.preventDefault(), ft(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [Q, Ut, pt, Kt]), ie(() => {
    const c = (f) => {
      we.length !== 0 && (f.preventDefault(), f.returnValue = "");
    };
    ...
    if (!n) return;
    const c = on(n);
    h((f) => Ht(f, c.primaryMin, c.primaryMax)), Y((f) => Ht(f, c.secondaryMin, c.secondaryMax));
  }, [n]), ie(() => {
    const c = { primaryVisible: m, primaryWidth: b, secondaryVisible: j, secondaryWidth: $, markdownPreviewVisible: E };
    ...
    return (async () => {
      try {
        const f = await sn(i.folder?.handleKey ?? "");
        if (!f || !await tn(f, "readwrite") || c) return;
        const v = await Yn(f);
        if (c) return;
        Xe(i.folder?.handleKey ?? null), Ae(v), _((x) => Pc(x, v.files)), xe((x) => x.filter((U) => v.files.some((ue) => ue.id === U) || T.some((ue) => ue.id === U))), C(`Restored ${v.name} (${v.files.length} text files indexed)`);
      } catch (f) {
        c || C(`Workspace restore failed: ${f instanceof Error ? f.message : String(f)}`);
      }
    })(), () => {
      c = !0;
    };
    ...
    kc({
      activity: o,
      folder: Pe ? { name: Pe.name, handleKey: He } : null,
      files: gc(T),
      openEditorIds: fe,
      activeFileId: Q,
      query: k,
      ...
      welcomeClosed: le,
      secondaryTab: w,
      bottomPanelVisible: ce,
      bottomPanelTab: V
    });
  }, [o, Q, V, ce, Ye, He, T, Pe, fe, k, w, le]), ie(() => {
    localStorage.setItem(Ia, JSON.stringify(F));
  }, [F]), /* @__PURE__ */ d(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${m ? "" : "no-primary"} ${j ? "" : "no-secondary"} ${ce ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${b}px`, "--workbench-secondary-width": `${$}px` },
      children: [
        /* @__PURE__ */ a(Fo, { active: o, setActive: (c) => {
          if (c === o && m) {
            y(!1);
            return;
          }
          l(c), y(!0);
        }, openSettings: Lt, settingsActive: Tt }),
        m && /* @__PURE__ */ d("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            Io,
            {
              host: e,
              ...
              openFile: pt,
              openFolder: Zt,
              openWorkbenchFile: de,
              newFile: Bt,
              recent: Yt,
              reopenRecent: Qt,
              setStatus: C,
              hasFsAccess: bs(),
              attachSkillToChat: Fn,
              saveLocalJobOutput: In,
              activeFile: D
            }
          ),
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: ir, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => ft(!0), children: "Workspace" }),
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              Yo,
              {
                ...
                closeSettings: Vn,
                secondaryVisible: j,
                toggleSecondary: () => R((c) => !c),
                canPreview: D?.language === "markdown",
                previewVisible: E,
                togglePreview: () => W((c) => !c)
              }
            ),
            /* @__PURE__ */ a(Eo, { file: D, folder: Pe, showWelcome: Jt }),
            Se && we.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: Se }),
              /* @__PURE__ */ a("button", { onClick: () => {
                Dn();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => Z(null), title: "Dismiss", children: /* @__PURE__ */ a(Me, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: D ? /* @__PURE__ */ d("div", { className: D.language === "markdown" && E ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(Tr, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                zo,
                {
                  file: D,
                  revealLine: _a,
              ...
              D.language === "markdown" && E && /* @__PURE__ */ a(Qo, { content: D.content })
            ] }) : Tt ? /* @__PURE__ */ a(
              sc,
              {
                host: e,
            ...
            ) : Jt ? /* @__PURE__ */ a(Ko, { host: e, openFile: pt, openFolder: Zt, newFile: Bt, recent: Yt, reopenRecent: Qt, setStatus: C, openControlTower: () => {
              l("computer"), y(!0);
            }, openChat: () => {
              X("chat"), R(!0);
            }, openEmbeddedDoc: or }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(Dt, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M(!1), children: "Show Agent Team" })
            ] }) }),
            ce && /* @__PURE__ */ a(
              _o,
              {
                ...
                runAiSynthesis: Gt,
                captureManualCheck: _t,
                openOutputAsFile: $t,
                manualCheckSession: S,
                manualCheckCommandInput: ye,
                setManualCheckCommandInput: me,
                manualCheckOutputInput: Te,
                setManualCheckOutputInput: Ie,
                manualCheckSelectedCommand: Je,
                setManualCheckSelectedCommand: Ze,
                manualCheckStatus: tt,
                setManualCheckStatus: Ke,
                copyManualCheckCommand: yr,
                addManualCheckCommand: wr,
                recordManualCheckResult: gr,
                askAgentFromManualChecks: kr,
                onClose: () => ee(!1)
              }
            )
          ] })
        ...
        j && /* @__PURE__ */ a(
          ec,
          {
            ...
            chatInput: Ye,
            setChatInput: Oe,
            chatMessages: K.messages,
            chatThread: K.thread,
            chatThreads: K.threads,
            askAgent: br,
            stopChat: K.stopChat,
            regenerateMessage: hr,
            newChat: () => {
              K.newChat();
            },
            selectThread: (c) => {
            ...
            saveMessageAsArtifact: fr,
            rememberMessage: mr,
            previewEditFromMessage: (c) => at(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: qn,
            pendingPatchPrompt: se,
            generatePatchPrompt: vr,
            workspaceFileCount: T.length,
            aiStatus: K.aiStatus,
            chatSettings: F,
            openSettings: Lt,
            busy: K.busy,
            memoryHitCount: K.memoryHits.length,
            outputs: Ue,
            runAiSynthesis: Gt,
            captureManualCheck: _t,
            openOutputAsFile: $t,
            previewEditFromOutput: (c) => at(c.title, c.body),
            canPreviewEdit: T.length > 0,
            clearOutputs: () => u([]),
            deleteArtifact: (c) => {
              K.deleteArtifact(c);
            },
            ...
            contextScope: ve,
            setContextScope: Ve,
            contextAttachments: nr,
            removeContextAttachment: rr,
            revealContextAttachment: cr,
            projectIndexSummary: `${qe.snapshot.files.length} files · ${qe.snapshot.chunks.length} chunks`,
            projectIndexStale: qe.isStale,
            refreshProjectIndex: () => {
              const c = qe.refresh(T);
              C(`Project index refreshed: ${c.files.length} files · ${c.chunks.length} chunks`);
            },
            onResizeStart: sr,
            onClose: () => R(!1)
          }
        ...
        qt && /* @__PURE__ */ a(
          Go,
          {
            ...
            activeFile: D,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: Bt },
              ...
              ...Yt.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => {
                Qt(c);
              } })),
              { label: "File: Save All", detail: `${we.length} dirty file${we.length === 1 ? "" : "s"}`, run: () => {
                Dn();
              }, disabled: we.length === 0 },
              { label: "File: Close All Editors", detail: `${Ne.length} open editor${Ne.length === 1 ? "" : "s"}`, run: ur, disabled: Ne.length === 0 },
              ...
              { label: "Help: Show Agent Team", detail: "Open the Atomek Resource Fabric landing page", run: () => {
                p(null), M(!1), l("computer"), y(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: m ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => y((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: j ? "Hide right AI side bar" : "Show right AI side bar", run: () => R((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: ce ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => ee((c) => !c) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Lt },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => bt("Manual check requested from command palette") },
              { label: "View: Toggle Markdown Preview", detail: D?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => W((c) => !c), disabled: D?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: D || Ne.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Gt, disabled: !D && Ne.length === 0 },
              { label: "AI: Explain Active File", detail: D ? `Ask Cortex to explain ${D.path}` : "Open a file first", run: () => ze("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !D },
              { label: "AI: Improve Active File", detail: D ? `Ask Cortex for concrete edits to ${D.path}` : "Open a file first", run: () => ze("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !D },
              { label: "AI: Draft Editable Replacement", detail: D ? `Ask Cortex for a full-file replacement for ${D.path}` : "Open a file first", run: () => qn("edit"), disabled: !D },
              { label: "AI: Plan Workspace Work", detail: Ne.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => ze("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: Ne.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: D ? "Persist active file in host.ai artifacts" : "Open a file first", run: pr, disabled: !D },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: _t },
              { label: "AI: Open Latest Artifact as File", detail: Ue[0] ? `Create editable file from ${Ue[0].title}` : "No outputs yet", run: () => Ue[0] && $t(Ue[0]), disabled: Ue.length === 0 }
            ],
            openWorkbenchFile: de,
            onClose: () => ft(!1)
          }
        ...
        /* @__PURE__ */ a(mc, { status: er, file: D ?? Xo, cursor: I, fileCount: T.length, dirtyCount: we.length })
      ]
    }
  );
}
function Fo({ active: e, setActive: t, openSettings: n, settingsActive: r }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-activity-brand", title: "Atomek", children: /* @__PURE__ */ a(Wa, { size: 30, variant: "cream" }) }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(la, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(is, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(ua, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(ca, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(gn, { size: 25 }), label: "Agent Team", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(Fr, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(Ge, { icon: /* @__PURE__ */ a(ds, { size: 23 }), label: "Settings", active: r, onClick: n })
  ] });
}
function Ge({ icon: e, label: t, active: n, onClick: r }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: r, children: e });
}
function Io(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(Uo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(sa, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(sa, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a($a, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(qo, { ...e });
}
function qo(e) {
  const t = !e.folder, n = he(() => Ro(e.files, e.folder?.name), [e.files, e.folder?.name]), [r, s] = N(() => /* @__PURE__ */ new Set());
  ie(() => {
    const o = Ka(n);
    s((l) => {
      const m = new Set(l);
      return o.forEach((y) => m.add(y)), m;
    });
  }, [n]);
  const i = P((o) => {
    s((l) => {
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(We, { children: [
        ...
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : Ua(n, e.activeFileId, e.openWorkbenchFile, r, i)
      ] }),
      ...
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((o) => /* @__PURE__ */ d("button", { className: "workbench-tree-row", onClick: () => {
        ...
        o.kind === "file" ? /* @__PURE__ */ a(Pn, { size: 14 }) : /* @__PURE__ */ a(da, { size: 14 }),
...
function Hn({ file: e, active: t, onOpen: n, basePath: r, depth: s = 0, label: i, detail: o }) {
  const l = r && e.path.startsWith(`${r}/`) ? e.path.slice(r.length + 1) : e.path, m = s || Math.max(0, l.split("/").length - 1);
  return /* @__PURE__ */ d("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": m }, onClick: n, title: e.path, children: [
...
function Ro(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), s = (o, l, m = n) => {
    const y = r.get(o);
    if (y) return y;
  ...
  e.forEach((o) => {
    ...
    let y = n, b = "";
    m.slice(0, -1).forEach((h) => {
      b = b ? `${b}/${h}` : h, y = s(b, h, y).children;
    }), y.push({ name: m.at(-1) ?? o.name, path: l, file: o, children: [] });
  });
  const i = (o) => o.sort((l, m) => +!!l.file - +!!m.file || l.name.localeCompare(m.name)).map((l) => ({ ...l, children: i(l.children) }));
  return i(n);
}
function Ka(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...Ka(t.children)]);
}
function Ua(e, t, n, r, s, i = 0) {
  return e.map((o) => {
    if (o.file)
    ...
    return /* @__PURE__ */ d("div", { children: [
      /* @__PURE__ */ d("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => s(o.path), title: l ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        l ? /* @__PURE__ */ a(Vt, { size: 12 }) : /* @__PURE__ */ a(Vt, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ a(Nn, { size: 14 }) : /* @__PURE__ */ a(da, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: o.children.length })
      ] }),
      l ? Ua(o.children, t, n, r, s, i + 1) : null
    ] }, o.path);
  });
}
function Eo({ file: e, folder: t, showWelcome: n }) {
...
function Yo(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ a(kn, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(es, { size: 16 }) })
  ] });
}
function Jo(e) {
...
function Zo(e) {
  ...
  return [...e?.resources ?? []].sort((n, r) => (t[n.kind] ?? 9) - (t[r.kind] ?? 9) || ke(n).localeCompare(ke(r))).slice(0, 8);
}
function Ko({
  host: e,
  openFile: t,
  ...
  const [b, h] = N("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [w, X] = N(null), [j, R] = N([]), [$, Y] = N(!1), [ce, ee] = N(null), [V, J] = N(null), [E, W] = N("pod-local"), le = P(async () => {
    Y(!0), J(null);
    try {
      ...
      X(p ?? null), R(k), p && o(`Agent team loaded · ${p.resources.length} resources · ${p.warnings.length} warnings`);
    } catch (p) {
      const k = p instanceof Error ? p.message : String(p);
      J(k), o(`Agent team resource load failed: ${k}`);
    } finally {
      Y(!1);
    }
  }, [e.missions, e.resources, o]), M = P((p, k) => {
    try {
      e.windows.openOrFocus(p), o(`Opened ${k}`);
    } catch (L) {
      o(`Open ${k} failed: ${L instanceof Error ? L.message : String(L)}`);
    }
  }, [e.windows, o]);
  ie(() => {
    le();
  ...
  const Pe = P(async () => {
    ...
    const p = b.trim() || "Coordinate a Tytus mission.", k = et(p, w, E);
    Y(!0);
    try {
      const L = await e.missions.create({
        ...
        goal: L.goal,
        rootPath: L.rootPath,
        name: L.rootPath.split("/").pop() || L.missionId,
        source: "tray",
        teamPresetId: k
      }, oe = {
      ...
      await e.missions.write({
        rootPath: L.rootPath,
        files: [
          { path: "MISSION.md", content: Wt(I, w, null, [], p, k) },
          { path: "MISSION.json", content: Za(I, w, p, k) },
          { path: "RESOURCES.md", content: Mt(w) },
          { path: "TASKS.md", content: Ya(Nt(p, w, k)) },
          { path: "HANDOFF.md", content: Ja(I) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(oe)}
` },
...
`) }
      ...
      }), ee(L), Pt(I), o(`Mission created: ${L.rootPath}`), l();
    } catch (L) {
      o(`Mission create failed: ${L instanceof Error ? L.message : String(L)}`);
    } finally {
      Y(!1);
    }
  }, [b, w, e.missions, l, o, E]), Ae = Jo(w), He = Co(w), Xe = Ho(w), T = Zo(w), _ = w?.warnings ?? [], fe = jo(w), xe = ht(w, et(b, w, E)), Q = Nt(b, w, xe.id);
  return /* @__PURE__ */ d("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ d("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ d("div", { className: "workbench-brand-lockup", "aria-label": "Atomek", children: [
        /* @__PURE__ */ a(Wa, { size: 74, variant: "acid" }),
        /* @__PURE__ */ d("div", { children: [
          /* @__PURE__ */ a(bo, { className: "workbench-brand-wordmark" }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-control-goal-row", children: [
        /* @__PURE__ */ a("textarea", { value: b, onChange: (p) => h(p.target.value), rows: 3, "aria-label": "Mission goal" }),
        /* @__PURE__ */ d("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            Pe();
          }, disabled: $, children: "Start mission" }),
      ...
      /* @__PURE__ */ a("div", { className: "workbench-team-preset-strip", "aria-label": "Team presets", children: fe.map((p) => /* @__PURE__ */ d(
        "button",
        {
          className: `workbench-team-preset-card ${p.id === xe.id ? "active" : ""} ${p.readiness}`,
          onClick: () => W(p.id),
          children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: p.label }),
              /* @__PURE__ */ a("span", { children: p.readiness })
            ] }),
            /* @__PURE__ */ a("p", { children: p.summary }),
            /* @__PURE__ */ a("small", { children: p.assignments.map((k) => k.resourceLabel).slice(0, 3).join(" · ") })
          ]
        },
        p.id
      )) }),
      ce ? /* @__PURE__ */ d("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: ce.rootPath })
      ] }) : null,
      V ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: V }) : null
    ] }),
    /* @__PURE__ */ d("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
          ...
          /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: () => {
            ...
            /* @__PURE__ */ a(kt, { size: 13 }),
            " ",
            $ ? "Refreshing…" : "Refresh"
          ] })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: Ae.map((p) => /* @__PURE__ */ d("div", { className: "workbench-control-metric", children: [
          /* @__PURE__ */ a("strong", { children: p.value }),
          /* @__PURE__ */ a("span", { children: p.label }),
          /* @__PURE__ */ a("em", { children: p.detail })
        ] }, p.label)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-agent-brand-grid", children: He.map((p) => /* @__PURE__ */ d("div", { className: `workbench-agent-brand-card ${p.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ d("div", { children: [
            /* @__PURE__ */ a("strong", { children: p.label }),
            /* @__PURE__ */ a("span", { children: p.status })
          ] }),
          /* @__PURE__ */ a("b", { children: p.value }),
          /* @__PURE__ */ a("p", { children: p.detail })
        ] }, p.label)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-doc-grid", children: Po.map((p) => /* @__PURE__ */ d("button", { className: "workbench-doc-card", onClick: () => y(p), title: p.summary, children: [
          /* @__PURE__ */ a("strong", { children: p.title }),
          /* @__PURE__ */ a("p", { children: p.summary }),
          /* @__PURE__ */ a("small", { children: p.tags.join(" · ") })
        ] }, p.id)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: Xe.map((p, k) => /* @__PURE__ */ d("div", { className: `workbench-fabric-node ${p.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ a("span", { children: k + 1 }),
          /* @__PURE__ */ a("strong", { children: p.label }),
          /* @__PURE__ */ a("em", { children: p.status }),
          /* @__PURE__ */ a("p", { children: p.detail })
        ] }, p.label)) }),
        /* @__PURE__ */ d("div", { className: "workbench-fabric-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M(Ct.sharedFiles, "Shared Files"), children: "Open shared files" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M(Ct.podInspector, "Pod Inspector"), children: "Open pods" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M(Ct.channels, "Channels"), children: "Open channels" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M(Ct.settings, "Agent Settings"), children: "Open agent settings" })
        ] })
      ] }),
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ d("header", { children: [
          ...
          /* @__PURE__ */ a("span", { children: j.length ? `${j.length} missions` : "none yet" })
        ] }),
        j.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Create a mission once; it stays in Tytus Home/Missions and can be resumed here." }) : null,
        j.slice(0, 4).map((p) => /* @__PURE__ */ d("button", { className: "workbench-control-preset", onClick: () => {
          Pt(p), l();
        }, title: p.rootPath, children: [
          p.title,
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-control-resource-list", children: [
          T.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          T.map((p) => /* @__PURE__ */ d("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: ke(p) }),
              /* @__PURE__ */ a("span", { children: Cn(p) })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${p.status}`, children: p.status })
          ] }, p.id))
        ] }),
        _.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: _.slice(0, 2).map((p) => /* @__PURE__ */ d("span", { children: [
          p.code,
          ": ",
          p.message
        ] }, `${p.code}-${p.resourceId ?? p.message}`)) }) : null
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ a("header", { children: /* @__PURE__ */ a("strong", { children: "Recommended task graph" }) }),
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: Q.map((p, k) => /* @__PURE__ */ d("button", { className: `workbench-task-card ${p.status}`, onClick: () => h(p.prompt), children: [
          /* @__PURE__ */ a("span", { children: k + 1 }),
          /* @__PURE__ */ a("strong", { children: p.title }),
          /* @__PURE__ */ a("em", { children: p.resourceHint }),
          /* @__PURE__ */ a("small", { children: p.assignedResourceLabel })
        ] }, p.id)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
...
function Uo({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: s }) {
  const i = he(() => Bo(e, t), [e, t]), o = he(() => {
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? o.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : o.map((l) => {
        ...
        return /* @__PURE__ */ d("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(Hn, { file: m, active: m.id === s, onOpen: () => r(m) }),
          l.slice(0, 5).map((y) => /* @__PURE__ */ d("button", { className: "workbench-search-hit", onClick: () => r(m, y.lineNumber), title: y.line, children: [
            /* @__PURE__ */ a("span", { className: "workbench-search-line", children: y.lineNumber }),
            /* @__PURE__ */ a("span", { children: y.line })
          ] }, `${m.id}-${y.lineNumber}-${y.line}`)),
...
function Go(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((s) => ({
    label: s.path,
    detail: `Open ${pa(s.language)} file`,
...
function $o(e) {
  const t = e.session ? ao(e.session) : "pending";
...
function _o(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(Qa, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function ec(e) {
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(as, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(Rr, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(Me, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(tc, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      $a,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ a(Qa, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function Ba({ body: e }) {
  const t = he(() => Lo(e), [e]), [n, r] = N(null), s = P((i, o) => {
    (async () => await dt(o) && (r(i), window.setTimeout(() => r((m) => m === i ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((i) => {
      ...
      return /* @__PURE__ */ d("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ d("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ d("button", { onClick: () => s(i.key, i.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ a(Mr, { size: 12 }) : /* @__PURE__ */ a(fn, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: wa(i.body) }
      },
      i.key
    );
  }) });
...
function tc(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = _e(null), [r, s] = N(!0), [i, o] = N(!1), l = he(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  ie(() => {
    const h = n.current;
  ...
  const m = P(() => {
    const h = n.current;
    if (!h) return;
    const X = h.scrollHeight - h.scrollTop - h.clientHeight < 48;
    s(X), X && o(!1);
  }, []), y = P(() => {
    const h = n.current;
  ...
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ d("option", { value: h.id, children: [
              h.title,
              " · ",
              nc(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ d("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ d("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: Hs(e.contextScope) }),
        /* @__PURE__ */ a("em", { children: na(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ a(En, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((h) => {
            const w = typeof h.score == "number" ? h.score.toFixed(2) : null, X = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, j = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, R = [
              h.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: R, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ a(En, { size: 13 }) }),
              h.label,
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: na(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(fa, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(cs, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function na(e, t, n) {
  const r = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : Ga(e.gatewayPreference)];
...
function sc(e) {
  const [t, n] = N([]), [r, s] = N("Loading gateway models…"), [i, o] = N([]), [l, m] = N("Checking embedding capability…");
  ie(() => {
    ...
    return (async () => {
      ...
      try {
        const j = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        n(j.map((R) => ({ id: R.id, gatewayLabel: R.gatewayLabel }))), s(j.length > 0 ? `${j.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (j) {
        if (w.signal.aborted) return;
        n([]), s(`Model discovery failed: ${j instanceof Error ? j.message : String(j)}`);
      }
    })(), () => w.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), ie(() => {
    const w = new AbortController();
    return (async () => {
      const j = Gi(e.host);
      if (j) {
      ...
      try {
        const R = await Bi(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        o(R.map(($) => ({ id: $.id, gatewayLabel: $.gatewayLabel ?? $.source ?? "AIL" }))), m(R.length > 0 ? `${R.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (R) {
        if (w.signal.aborted) return;
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ d("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          Ga(e.chatSettings.gatewayPreference),
    ...
    /* @__PURE__ */ d("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(jt), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function Qa({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: s, openOutputAsFile: i, previewEditFromOutput: o, canPreviewEdit: l = !1, compact: m = !1 }) {
  return /* @__PURE__ */ d("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((y) => /* @__PURE__ */ d("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ d("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void dt(y.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => i(y), children: "Open as file" }),
        o ? /* @__PURE__ */ a(
          "button",
          {
            className: aa(y.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(y),
            disabled: !l,
            children: aa(y.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        y.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(y.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ a(Ba, { body: y.body })
    ] }, y.id))
  ] });
}
function ic({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
...
function oc({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: bc(i.proposedContent) })
      ] }, i.fileId)),
...
function $a({
  ...
  const [l, m] = N([]), [y, b] = N([]), [h, w] = N(null), [X, j] = N(!1), [R, $] = N(null), [Y, ce] = N("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [ee, V] = N(null), [J, E] = N([]), [W, le] = N(() => Mo()), [M, Pe] = N([]), [Ae, He] = N([]), [Xe, T] = N([]), _ = _e([]), [fe, xe] = N("task-execute"), [Q, p] = N("mission"), k = J.find((u) => u.status === "running" || u.status === "canceling") ?? J[0] ?? null, L = o === "dock", I = he(() => ht(h, et(Y || W?.goal || "", h, W?.teamPresetId)), [Y, W?.goal, W?.teamPresetId, h]), oe = he(() => Nt(Y || W?.goal || "", h, I.id), [Y, W?.goal, I.id, h]), H = oe.find((u) => u.id === fe) ?? oe[1] ?? oe[0] ?? null, ge = i.filter((u) => u.dirty).length, be = s ? `${s.path} · ${s.language} · ${s.content.length.toLocaleString()} chars${s.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${ge ? ` · ${ge} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Ee = he(() => [
    {
  ...
  const ve = P(async (u = W) => {
    if (!u?.rootPath || !e.missions?.listRuns)
      return T([]), [];
    try {
      const g = await e.missions.listRuns(u.rootPath);
      return T(g), _.current = g, g;
    } catch (g) {
      return t(`Mission run history failed: ${g instanceof Error ? g.message : String(g)}`), [];
    }
  }, [e.missions, W, t]), Ve = P(async (u, g) => {
    if (!u) return;
    const z = [...g].sort((O, q) => ta(q).localeCompare(ta(O))).slice(0, 100), A = z.map((O) => JSON.stringify(O)).join(`
`) + (z.length ? `
` : "");
    u.rootPath && e.missions?.write ? await e.missions.write({ rootPath: u.rootPath, files: [{ path: "RUNS.jsonl", content: A }] }) : u.handle && await yn(u.handle, "RUNS.jsonl", A), T(z), _.current = z;
  }, [e.missions]), pe = P(async (u, g = W) => {
    if (!g) return;
    const z = [
      u,
      ..._.current.filter((A) => A.id !== u.id)
    ];
    await Ve(g, z);
  }, [W, Ve]), ae = P(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      m([]), b([]), w(null), $("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    j(!0), $(null);
    try {
      const [u, g, z, A] = await Promise.all([
        e.local?.listTools?.().catch((O) => (t(`Local tool discovery failed: ${O instanceof Error ? O.message : String(O)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((O) => (t(`Skill registry discovery failed: ${O instanceof Error ? O.message : String(O)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((O) => (t(`Resource graph discovery failed: ${O instanceof Error ? O.message : String(O)}`), null)) ?? Promise.resolve(null),
        e.missions?.list?.().catch((O) => (t(`Mission list failed: ${O instanceof Error ? O.message : String(O)}`), [])) ?? Promise.resolve([])
      ]);
      m(u), b(g), w(z), Pe(A), W?.rootPath && ve(W), t(`Agent team loaded · ${u.length} tools · ${g.length} skills · ${A.length} missions${z ? ` · ${z.resources.length} resources` : ""}`);
    } catch (u) {
      $(u instanceof Error ? u.message : String(u));
    } finally {
      j(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, ve, W, t]);
  ie(() => {
    ae();
  }, [ae]), ie(() => {
    const u = (g) => {
      const z = g.detail;
      z?.missionId && (le(z), ve(z)), ae();
    };
  ...
  const re = P((u) => {
    ...
    le(g), Pt(g), ve(g), He([{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "mission.resume",
      message: `Mission resumed from Atomek agent team: ${g.rootPath ?? g.name}`,
      data: { runCount: u.runCount ?? 0, taskCount: u.taskCount ?? 0 }
    }]), ce(u.goal || `Continue mission ${u.title}. Review MISSION.md, TASKS.md, RESOURCES.md, and runs/ before acting.`), t(`Resumed mission: ${u.rootPath}`);
  }, [ve, t]), G = P(async (u, g, z = []) => {
    const A = [
      ...Ae,
      ...z,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], O = et(g || u.goal, h, u.teamPresetId), q = Nt(g || u.goal, h, O), B = [
      { path: "MISSION.md", content: Wt(u, h, s, i, g, O) },
      { path: "MISSION.json", content: Za(u, h, g, O) },
      { path: "RESOURCES.md", content: Mt(h) },
      { path: "TASKS.md", content: Ya(q) },
      { path: "HANDOFF.md", content: Ja(u) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: A.map((F) => JSON.stringify(F)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: _.current.map((F) => JSON.stringify(F)).join(`
`) + (_.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (u.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: u.rootPath, files: B });
    else if (u.handle) {
      await ot(u.handle, "runs"), await ot(u.handle, "outputs"), await ot(u.handle, "proposals"), await ot(u.handle, "approvals");
      for (const F of B) await Oo(u.handle, F.path, F.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    He(A), Pt(u);
  }, [s, e.missions, Ae, i, h]), se = P(async (u, g = {}) => {
    if (W)
      return await G(W, u), W;
    const z = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, A = u.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let O = null;
    if (e.missions?.create) {
      const B = await e.missions.create({ title: z, goal: A });
      O = {
      ...
      O = {
        handle: B,
        name: B.name,
        missionId: `mission-${Date.now()}-${Wo(B.name)}`,
        title: z,
        goal: A,
        source: "browser"
      };
    ...
    if (!O) return null;
    const q = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${O.rootPath ?? O.name}` };
    return le(O), Pt(O), He([q]), await G(O, A, [q]), t(`Mission pack ready in ${O.rootPath ?? O.name}`), O;
  }, [e.missions, W, t, G]), It = P(async (u) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const g = Y.trim() || `Open ${u.label} from Atomek with current context.`, z = u.kind === "ai-cli" ? await se(g) : W;
      await e.local.openTerminal({
        toolId: u.id,
        command: u.command,
        cwd: z?.rootPath ?? ra(s),
        prompt: z ? `Atomek mission pack ready at ${z.rootPath ?? z.name}. Read MISSION.md and RESOURCES.md. ${be}` : `Opened from Atomek. ${be}`
      }), t(u.kind === "ai-cli" ? `Opened ${u.label} in Tytus Terminal with mission context. Press Enter there to start it.` : "Opened Tytus Terminal");
    } catch (g) {
      t(`Terminal launch failed: ${g instanceof Error ? g.message : String(g)}`);
    }
  }, [s, be, se, e.local, Y, W, t]), Ye = P(async () => {
    try {
      await se(Y.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (u) {
      t(`Mission folder setup failed: ${u instanceof Error ? u.message : String(u)}`);
    }
  }, [se, Y, t]), Oe = P(async (u, g, z, A = W, O) => {
    if (!A) return;
    const q = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), B = O ?? `runs/${q}-${u.id}.md`, F = B.split("/").pop() || `${q}-${u.id}.md`;
    if (A.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: A.rootPath, files: [{ path: B, content: g }] });
    else if (A.handle) {
      const Ce = await ot(A.handle, "runs");
      await yn(Ce, F, g);
    }
    await G(A, Y, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${u.label} exited ${z}; transcript saved to ${B}`,
      data: { toolId: u.id, exitCode: z, transcript: B }
    }]);
  }, [e.missions, Y, W, G]), qt = P(async (u) => {
    ...
    try {
      ...
      t(`Mission pack failed before local job start: ${q instanceof Error ? q.message : String(q)}`);
      return;
    }
    V(u.id);
    const A = `local-run-${Date.now()}-${u.id}`, O = it();
    E((q) => [{
      id: A,
      toolId: u.id,
      label: u.label,
    ...
    try {
      const q = await e.local.runJob({
        toolId: u.id,
        prompt: z ? [
          "Tytus mission context pack is active.",
          `Mission: ${z.title}`,
          `Goal: ${z.goal}`,
          z.rootPath ? `Mission folder: ${z.rootPath}` : `Mission folder: ${z.name}`,
...
`)
      }), B = q.transcriptPath ?? (z?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id}.md` : void 0), F = [
        `[Atomek] Started ${u.label} local job ${q.id}`,
        H ? `[Atomek] Task: ${H.title} (${H.id})` : "[Atomek] Task: manual",
        z?.rootPath ? `[Atomek] Mission: ${z.rootPath}` : ""
      ].filter(Boolean), Ce = {
        id: A,
        jobId: q.id,
        toolId: u.id,
      ...
      pe(Ce, z).catch((Z) => {
        t(`Mission run index failed: ${Z instanceof Error ? Z.message : String(Z)}`);
      }), E((Z) => Z.map((S) => S.id === A ? { ...S, jobId: q.id, transcriptPath: B } : S));
      const Se = (Z) => {
        E((S) => S.map((te) => te.id === A ? Z(te) : te));
      };
      e.local.streamJob(q.id, {
        onLog: (Z) => {
          F.push(Z), Se((S) => ({ ...S, lines: F.slice(-500) }));
        },
        onDone: (Z) => {
          Z && (F.push(Z), Se((S) => ({ ...S, lines: F.slice(-500) })));
        },
        onFail: (Z) => {
          F.push(`[FAIL] ${Z}`), Se((S) => ({
            ...S,
            ...
            t(`Mission run index failed: ${S instanceof Error ? S.message : String(S)}`);
          }), r(`${u.label} local job failed`, F.join(`
`)), Oe(u, F.join(`
`), -1, z, B).catch((S) => {
            t(`Mission transcript save failed: ${S instanceof Error ? S.message : String(S)}`);
          }), V(null);
        },
        onExit: (Z) => {
          const S = [
...
`);
          Se((te) => ({
            ...
            finishedAt: it(),
            exitCode: Z,
            summary: `${u.label} exited ${Z}`
          }, z).catch((te) => {
            t(`Mission run index failed: ${te instanceof Error ? te.message : String(te)}`);
          }), r(`${u.label} local job`, S), Oe(u, S, Z, z, B).catch((te) => {
      ...
      V(null), E((B) => B.map((F) => F.id === A ? {
        ...F,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...F.lines, `[Atomek] Failed to start: ${q instanceof Error ? q.message : String(q)}`]
      } : F)), t(`Local job failed to start: ${q instanceof Error ? q.message : String(q)}`);
    }
  }, [s, se, e.local, Y, i, h, r, Oe, H, t, pe, G]), ft = P(async (u) => {
    const g = wn(u);
    if (!g) {
      t(`Cannot dispatch ${ke(u)}: missing pod id`);
      return;
    }
    const z = Y.trim();
    if (!z) {
    ...
    let A = null;
    const O = ke(u), q = `pod-run-${Date.now()}-${g}`, B = it(), F = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      A = await se(z), A && await G(A, z, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${O} mission task started`,
        data: { resourceId: u.id, podId: g, taskId: H?.id ?? "manual", taskTitle: H?.title ?? "Manual pod run" }
      }]);
    } catch (S) {
      t(`Mission pack failed before pod task: ${S instanceof Error ? S.message : String(S)}`);
      return;
    }
    const Ce = {
      id: q,
      ...
      transcriptPath: F,
      summary: `Started ${O} for ${H?.title ?? "manual pod run"}`
    };
    E((S) => [{
      id: q,
      toolId: u.id,
      ...
      taskTitle: H?.title ?? "Manual pod run",
      transcriptPath: F,
      lines: [`[Atomek] Dispatching ${O} via Tytus host bridge…`, `[Atomek] Pod: ${g}`, `[Atomek] Task: ${H?.title ?? "manual pod run"}`]
    }, ...S].slice(0, 6)), await pe(Ce, A).catch((S) => {
      t(`Mission run index failed: ${S instanceof Error ? S.message : String(S)}`);
    });
    const Se = [], Z = (S) => {
      Se.push(S), E((te) => te.map((ye) => ye.id === q ? { ...ye, lines: [...ye.lines, S].slice(-500) } : ye));
    };
    try {
      const S = await e.daemon.callPodEndpoint(g, "/v1/models", { method: "GET" });
      if (!S.ok)
        throw new Error(`/v1/models ${S.status}: ${await S.text()}`);
      const te = await S.json(), ye = hc(te);
      if (!ye)
        throw new Error("pod /v1/models returned no model id");
      Z(`[Atomek] Selected pod model from live metadata: ${ye}`);
      const me = await e.daemon.callPodEndpoint(g, "/v1/chat/completions", {
        method: "POST",
...
`)
            },
            {
              role: "user",
              content: [
                A ? `Mission folder: ${A.rootPath ?? A.name}` : "Mission folder: not available",
                A ? `Mission: ${A.title}` : "",
                A ? `Goal: ${A.goal}` : "",
...
`)
      ...
      if (!me.ok)
        throw new Error(`/v1/chat/completions ${me.status}: ${Te}`);
      let Ie = Te;
      try {
        Ie = JSON.parse(Te);
      } catch {
      }
      const Je = typeof Ie == "string" ? Ie : fc(Ie);
      Z(Je);
      const Ze = [
        `# Pod job — ${O}`,
        "",
...
`);
      E((tt) => tt.map((Ke) => Ke.id === q ? { ...Ke, status: "complete", finishedAt: Date.now(), lines: [...Ke.lines, "[Atomek] Pod task complete."].slice(-500) } : Ke)), await pe({ ...Ce, status: "complete", finishedAt: it(), summary: `${O} completed`, transcriptPath: F }, A), r(`${O} pod job`, Ze), await Oe({ id: u.id, label: O, kind: "pod-agent", status: u.status }, Ze, 0, A, F), t(`${O} completed mission task`);
    } catch (S) {
      const te = S instanceof Error ? S.message : String(S);
      Z(`[FAIL] ${te}`), E((me) => me.map((Te) => Te.id === q ? { ...Te, status: "failed", finishedAt: Date.now(), lines: Te.lines.slice(-500) } : Te)), await pe({ ...Ce, status: "failed", finishedAt: it(), exitCode: -1, summary: te, transcriptPath: F }, A).catch((me) => {
        t(`Mission run index failed: ${me instanceof Error ? me.message : String(me)}`);
      });
      const ye = [`# Pod job failed — ${O}`, "", `- Pod: ${g}`, `- Error: ${te}`, "", "```text", Se.join(`
`), "```", ""].join(`
`);
      r(`${O} pod job failed`, ye), await Oe({ id: u.id, label: O, kind: "pod-agent", status: u.status }, ye, -1, A, F).catch((me) => {
        t(`Pod transcript save failed: ${me instanceof Error ? me.message : String(me)}`);
      }), t(`Pod task failed: ${te}`);
    }
  }, [s, se, e.daemon, Y, i, h, r, Oe, H, t, pe, G]), Rt = P(async (u) => {
    if (!u.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    E((g) => g.map((z) => z.id === u.id ? { ...z, status: "canceling", lines: [...z.lines, "[Atomek] Cancel requested…"] } : z)), pe({
      id: u.id,
      jobId: u.jobId,
      toolId: u.toolId,
      label: u.label,
    ...
    try {
      ...
      E((z) => z.map((A) => A.id === u.id ? { ...A, status: "running", lines: [...A.lines, `[Atomek] Cancel failed: ${g instanceof Error ? g.message : String(g)}`] } : A)), t(`Local job cancel failed: ${g instanceof Error ? g.message : String(g)}`);
    }
  }, [e.local, t, pe]), Et = P(async (u) => {
    const g = [
      `Use Tytus resource "${ke(u)}" (${u.kind}) for the next mission step.`,
...
`);
    ce(g), W && await G(W, g, [{
      ...
      message: `Selected resource ${ke(u)}`,
      data: { resourceId: u.id, kind: u.kind, status: u.status }
    }]), t(`Selected ${ke(u)} for mission`);
  }, [W, t, G]), Xt = P((u) => {
    const g = u.setupAction, z = g?.commandPreview ? `${g.label}: ${g.commandPreview}` : g?.deepLink ? `${g.label}: ${g.deepLink}` : g?.label ?? `${ke(u)} needs setup`;
    t(z), dt(g?.commandPreview ?? g?.deepLink ?? z);
  }, [t]);
  return /* @__PURE__ */ d("aside", { className: L ? "workbench-agent-dock" : "workbench-sidebar", children: [
    L ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "AGENT TEAM" }),
    /* @__PURE__ */ d("div", { className: L ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ d("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ a(gn, { size: 18 }),
        /* @__PURE__ */ d("div", { children: [
          /* @__PURE__ */ a("strong", { children: L ? "Mission Runs" : "OpenClaw + Hermes Team Board" }),
      ...
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ...
        " ",
        X ? "Refreshing…" : "Refresh capabilities"
      ] }),
      R && /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: R }),
      /* @__PURE__ */ d("div", { className: "workbench-team-switcher", role: "tablist", "aria-label": "Atomek agent team views", children: [
        /* @__PURE__ */ a("button", { className: Q === "mission" ? "active" : "", onClick: () => p("mission"), children: "Mission" }),
        /* @__PURE__ */ a("button", { className: Q === "runs" ? "active" : "", onClick: () => p("runs"), children: "Runs" }),
        /* @__PURE__ */ a("button", { className: Q === "setup" ? "active" : "", onClick: () => p("setup"), children: "Setup" })
      ] }),
      Q === "mission" ? /* @__PURE__ */ d(We, { children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-context-card mission", children: [
          /* @__PURE__ */ a("strong", { children: W ? W.title : "No mission folder selected" }),
          /* @__PURE__ */ a("span", { children: W ? `${W.rootPath ?? W.name} · ${W.source} · ${Ae.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ d("div", { className: `workbench-team-assignment-summary ${I.readiness}`, children: [
            /* @__PURE__ */ a("strong", { children: I.label }),
            /* @__PURE__ */ d("span", { children: [
              I.readiness,
              " · ",
              I.bestFor
          ...
          I.assignments.map((u) => /* @__PURE__ */ d("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ d("div", { children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
            Ye();
          }, children: W ? "Refresh mission pack" : "Start mission pack" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => W && void G(W, Y), disabled: !W, children: "Rewrite context files" })
        ] }),
        M.length ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
          /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: M.slice(0, L ? 3 : 5).map((u) => /* @__PURE__ */ d(
            "button",
            {
              className: `workbench-mission-row ${W?.missionId === u.missionId ? "active" : ""}`,
              onClick: () => re(u),
              title: u.rootPath,
              children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: Ee.map((u) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ce(u.prompt), children: u.label }, u.label)) }),
        /* @__PURE__ */ a(
          "textarea",
          {
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: oe.map((u, g) => /* @__PURE__ */ d(
          "button",
          {
      ...
      Q === "runs" ? /* @__PURE__ */ d(We, { children: [
        k ? /* @__PURE__ */ d("div", { className: "workbench-agent-run", children: [
          /* @__PURE__ */ d("header", { children: [
            /* @__PURE__ */ d("div", { children: [
              ...
              /* @__PURE__ */ d("span", { children: [
                ...
                k.taskTitle ? ` · ${k.taskTitle}` : ""
              ] }),
              k.transcriptPath ? /* @__PURE__ */ a("span", { children: k.transcriptPath }) : null
            ] }),
            /* @__PURE__ */ d("div", { className: "workbench-agent-run-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => r(`${k.label} local job`, k.lines.join(`
`)), disabled: k.lines.length === 0, children: "Save output" }),
              k.jobId && (k.status === "running" || k.status === "canceling") ? /* @__PURE__ */ d("button", { className: "workbench-button-subtle danger", onClick: () => {
                Rt(k);
              }, disabled: k.status === "canceling", children: [
                /* @__PURE__ */ a(fa, { size: 12 }),
                " ",
                k.status === "canceling" ? "Canceling…" : "Cancel"
              ] }) : null
            ] })
          ] }),
...
`) || "[waiting for output]" })
        ...
        Xe.length ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: Xe.slice(0, L ? 4 : 8).map((u) => /* @__PURE__ */ d("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ d("div", { children: [
            ...
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void dt(u.transcriptPath ?? u.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, u.id)) })
        ] }) : null
      ] }) : null,
      Q === "setup" ? /* @__PURE__ */ d(We, { children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-list compact", children: [
          !h && !X ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
          h?.resources.slice(0, L ? 8 : 5).map((u) => /* @__PURE__ */ d("div", { className: "workbench-resource-row", children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: ke(u) }),
              /* @__PURE__ */ a("span", { children: Cn(u) })
            ] }),
            /* @__PURE__ */ d("div", { className: "workbench-resource-row-actions", children: [
              ...
              u.kind === "pod-agent" && wn(u) ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                ft(u);
              }, children: "Ask pod" }) : null,
              u.status === "needs-setup" || u.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => Xt(u), children: "Setup" }) : null
            ] })
          ] }, u.id))
        ] }),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
          l.length === 0 && !X ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
          l.map((u) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
            ...
            /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                It(u);
              }, disabled: u.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: dc(u) }),
              u.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                qt(u);
              }, disabled: u.status !== "available" || ee !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: uc(u, ee) }) : null
            ] })
          ] }, u.id))
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
          y.length === 0 && !X ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
          y.map((u) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
            ...
            u.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: u.triggers.slice(0, 4).map((g) => /* @__PURE__ */ a("span", { children: g }, g)) }) : null,
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              n(u);
...
function mc({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: s }) {
  return /* @__PURE__ */ d("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: pa(t.language) })
  ] });
}
function cn(e, t) {
...
function vc() {
  try {
    ...
    if (!e) return [];
    const t = JSON.parse(e);
    return Array.isArray(t) ? t.filter((n) => n && typeof n.name == "string" && typeof n.path == "string").slice(0, 10) : [];
  } catch {
    return [];
  }
}
...
function Nc() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : jt.gatewayPreference,
...
function zc({ host: e }) {
  return /* @__PURE__ */ a(Do, { host: e });
}
function Lc(e) {
  return function() {
    return /* @__PURE__ */ a(zc, { host: e.host });
  };
}
export {
  Lc as default
