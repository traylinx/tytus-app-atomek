...
import { forwardRef as wn, createElement as Pt, useState as C, useRef as Le, useCallback as y, useEffect as U, lazy as Fa, useMemo as Y, Suspense as za } from "react";
/**
...
}, vn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), ja = (e) => {
  for (const t in e)
...
async function Ln(e, t, n, a, o) {
  if (!(n.length >= o || a > 8))
    for await (const i of e.values()) {
      ...
      const s = i, l = await Pn(s, `${t}/${s.name}`, "local-folder");
      l && n.push(l);
    }
}
function Tn(e) {
...
function jr({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  ...
  const re = y(async () => {
    ...
    return W.current && (s(A), m((x) => [A, ...x.filter((P) => P.id !== A.id)])), A;
  }, [o, i]), w = y(async () => {
    if (!o) return;
    const A = await o.createThread({ workspaceKey: Me, title: "Atomek chat" });
    s(A), m((x) => [A, ...x.filter((P) => P.id !== A.id)]), p([]), g([]), I([]), a("New AI chat created");
  }, [o, a]), v = y(async (A, x) => {
    if (!o) return;
    const P = x.trim();
    if (P)
  ...
  }, [o, n.gatewayPreference, n.model, re, ce, E, t, a]), it = y(() => {
...
`), vo = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const f = At(n, m), p = At(a, m), u = At(o, m), g = i === m ? 1 : 0;
...
`), a = [];
  ...
  }), a.length === 0) return [{ raw: t, ...$t(t) }];
  const o = Array.from(new Set(a)).sort((s, l) => s - l), i = [];
  for (let s = 0; s < o.length; s += 1) {
    const l = o[s], f = o.find((u) => u > l && n[u].startsWith("diff --git ")) ?? n.length, p = n.slice(l, f).join(`
`);
    /^@@\s+-\d+/m.test(p) && i.push({ raw: p, ...$t(p) });
  }
  return i.length > 0 ? i : [{ raw: t, ...$t(t) }];
}
function $t(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), o = me(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = me(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, s = i && i !== "/dev/null" ? i : o;
...
function Ii({ host: e }) {
  const t = Le(null), [n, a] = C(0), o = Y(() => ic(), []), [i, s] = C("explorer"), [l, m] = C(o.primaryVisible), [f, p] = C(o.primaryWidth), [u, g] = C("chat"), [S, I] = C(o.secondaryVisible), [z, B] = C(o.secondaryWidth), [ae, q] = C(!1), [W, M] = C("problems"), [E, Q] = C(o.markdownPreviewVisible), [pe, K] = C(!1), [re, w] = C(null), [v, T] = C([]), [J, ie] = C([]), [H, ce] = C(null), [Te, it] = C(""), [A, x] = C({ lineNumber: 1, column: 1 }), [P, F] = C(null), [V, _] = C({}), [Z, we] = C(Wr), [Ee, Ve] = C([]), [ve, fe] = C([]), [D, j] = C(null), [ee, te] = C(""), [aa, ct] = C(!1), [ra, oa] = C(""), [qt, Ue] = C([]), [X, $e] = C(null), [be, Oe] = C(null), [Kt, Jt] = C(!1), [Ce, ia] = C(() => cc()), [Xt, Fe] = C(null), [Ne, st] = C(null), [lt, dt] = C(""), [ht, ut] = C(""), [qe, Ke] = C(""), [Je, Gt] = C("failed"), [ca, Yt] = C(null), [sa, $] = C("Ready"), [ze, la] = C(() => oc()), se = J.map((c) => v.find((d) => d.id === c)).filter(Boolean), N = H ? v.find((c) => c.id === H) ?? null : null, Qt = Y(() => Br({ files: v, openEditorIds: J, activeFileId: H, versions: V, activeSelection: P }), [H, P, V, v, J]), Zt = Y(() => ({
    ...
    return c ? v.filter((d) => d.path.toLowerCase().includes(c)) : v;
  }, [v, Te]);
  U(() => {
    _((c) => {
      const d = {};
      for (const b of v) d[b.id] = c[b.id] ?? 1;
      return d;
    });
  }, [v]), U(() => {
    Ve([]), fe([]);
  ...
  const ua = y((c) => {
    if (c.kind === "index-hit") {
      fe((d) => d.filter((b) => b.id !== c.id)), $(`Removed project context: ${c.label}`);
      return;
    }
    Ve((d) => d.includes(c.id) ? d : [...d, c.id]), $(`Removed chat context: ${c.label}`);
  }, []), Ge = y(() => {
    Jt(!0), ce(null), K(!0), $("Atomek settings opened");
  }, []), tn = y(() => {
    Jt(!1), ce((c) => c ?? J.at(-1) ?? null), J.length === 0 && K(!1), $("Atomek settings closed");
  }, [J]), Se = y((c) => {
    _((d) => ({ ...d, [c]: (d[c] ?? 1) + 1 }));
  }, []), ma = y((c) => {
    ...
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const d = Nt(n), b = c.clientX, k = f, R = (De) => {
      const Ct = k + (De.clientX - b);
    ...
    la(d), localStorage.setItem(Gn, JSON.stringify(d));
  }, [ze]), G = y((c, d) => {
    ie((b) => b.includes(c.id) ? b : [...b, c.id]), ce(c.id), K(!1), Yt(d ?? null), x({ lineNumber: d ?? 1, column: 1 });
  }, []), fa = y((c) => {
    if (!c.fileId) return;
    const d = v.find((b) => b.id === c.fileId);
    d && (G(d, c.range?.startLineNumber ?? 1), $(`Revealed context: ${c.label}`));
  }, [v, G]), _e = y(async () => {
    ...
    if (N)
      try {
        const c = await xt(N);
        T((d) => d.map((b) => b.id === c.id ? c : b)), $(`Saved ${c.name}`);
      } catch (c) {
        $(`Save failed: ${c.message}`);
      }
  }, [N]), ga = y(async (c) => {
    const d = v.find((k) => k.id === c);
    if (!d) return null;
    const b = await xt(d);
    return T((k) => k.map((R) => R.id === b.id ? b : R)), b;
  }, [v]), nn = y(async () => {
    const c = v.filter((d) => d.dirty);
    if (c.length === 0) {
      $("No dirty files to save");
      return;
    }
    try {
      const d = await Promise.all(c.map((k) => xt(k))), b = new Map(d.map((k) => [k.id, k]));
      T((k) => k.map((R) => b.get(R.id) ?? R)), Fe(null), $(`Saved ${d.length} dirty file${d.length === 1 ? "" : "s"}`);
    } catch (d) {
      $(`Save all failed: ${d.message}`);
    }
  }, [v]), bt = y((c) => {
    const d = v.find((b) => b.id === c);
...
`,
  ...
  }, [N, L, ke, se.length]), yt = y((c) => {
    const d = v.find((b) => b.path === c.path || b.name === c.name);
    if (d) {
...
`)), I(!0), g("chat"), $(`Attached skill ${d.title} to chat input`);
    ...
    const d = L.messages.findIndex((k) => k.id === c.id), b = L.messages.slice(0, d < 0 ? void 0 : d).reverse().find((k) => k.role === "user");
    if (!b?.body.trim()) {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const d = Mt(v, It(c.title || c.kind)), b = {
    ...
    T((k) => [...k, b]), G(b), $(`Opened ${c.title} as editable file`);
  }, [v, G]), vt = y(() => {
...
`), R = {
    ...
    Ue((k) => [b, ...k]), I(!0), g("outputs"), q(!0), M("output"), $(`Saved local job output: ${c}`), (d.includes("```diff") || d.includes("--- a/")) && Ie(c, d);
  }, [Ie]), $a = y(() => {
    const c = ee.trim();
    if (!c) return;
    te("");
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", k.body) || $("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    c.trim() && (ot(c), Ke(c), $(`Manual check command copied: ${c}`));
  }, []), Ia = y(() => {
    st((c) => {
      if (!c) return c;
    ...
    c.content !== X.originalContent && !window.confirm(`${X.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (T((d) => d.map((b) => b.id === X.fileId ? { ...b, content: X.proposedContent, dirty: !0 } : b)), Se(X.fileId), Fe(`AI edit applied to ${c.name}. Save All to persist it to disk.`), $e(null), je(`AI edit applied to ${c.name}`), $(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Se, v, X, je]), Ta = y(() => {
    if (!X) return;
    const c = Mt(v, `${It(X.fileName)}-ai-edit`), d = {
    ...
    T((b) => [...b, d]), G(d), $e(null), $(`Opened proposed edit as ${c}`);
  }, [N?.language, v, G, X]), Ea = y(() => {
    if (!be) return;
    const c = new Map(be.edits.map((b) => [b.fileId, b]));
    v.some((b) => {
      const k = c.get(b.id);
    ...
    })), c.forEach((b, k) => Se(k)), Fe(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), Oe(null), je(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), $(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Se, v, be, je]), Oa = y(() => {
    if (!be) return;
    const c = [...v], d = be.edits.map((b) => {
      const k = Mt(c, `${It(b.fileName)}-ai-edit`), R = v.find((De) => De.id === b.fileId), de = {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            ...
            refreshProjectIndex: () => {
              const c = ge.refresh(v);
              $(`Project index refreshed: ${c.files.length} files · ${c.chunks.length} chunks`);
            },
            onResizeStart: ma,
            onClose: () => I(!1)
          }
        ...
        aa && /* @__PURE__ */ r(
          _i,
          {
            ...
            commands: [
              ...
              { label: "File: Close All Editors", detail: `${se.length} open editor${se.length === 1 ? "" : "s"}`, run: ka, disabled: se.length === 0 },
              ...
              { label: "View: Toggle Bottom Panel", detail: ae ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => q((c) => !c) },
              ...
              { label: "Atomek: Create AI Synthesis", detail: N || se.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: kt, disabled: !N && se.length === 0 },
...
function Zn(e, t, n, a = 0) {
  return e.map((o) => o.file ? /* @__PURE__ */ r(Ut, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: a, label: o.name }, o.file.id) : /* @__PURE__ */ h("div", { children: [
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-rich-body", children: t.map((i) => {
    ...
    return /* @__PURE__ */ r(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: On(i.body) }
      },
      i.key
    );
  }) });
...
function na({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: o, openOutputAsFile: i, previewEditFromOutput: s, canPreviewEdit: l = !1, compact: m = !1 }) {
  return /* @__PURE__ */ h("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ h("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ h("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ r("button", { onClick: () => void ot(f.body), children: "Copy" }),
  ...
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      ...
      /* @__PURE__ */ h("div", { className: "workbench-computer-list", children: [
        s.length === 0 && !p ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        s.map((w) => /* @__PURE__ */ h("div", { className: "workbench-computer-card", children: [
