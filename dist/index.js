...
import { forwardRef as on, createElement as xt, useState as x, useRef as De, useCallback as w, useEffect as X, lazy as ga, useMemo as K, Suspense as ka } from "react";
/**
 ...
 */
const ya = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), wa = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), Vt = (e) => {
  const t = wa(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, cn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), va = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Ia = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], At = E("chevron-down", Ia);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
async function hr() {
  const e = Mt()?.showOpenFilePicker;
  if (!e) return yn(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((a) => gn(a, a.name, "local-file")));
  return Pt(n.filter(Boolean));
}
async function ur() {
  const e = Mt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await yn(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await kn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Pt(n) };
}
function Mt() {
  const e = window;
...
async function kn(e, t, n, a, i) {
  if (!(n.length >= i || a > 8))
    for await (const o of e.values()) {
      if (n.length >= i) break;
      if (o.kind === "directory") {
        if (lr.has(o.name)) continue;
        await kn(o, `${t}/${o.name}`, n, a + 1, i);
        continue;
      }
      const s = o, d = await gn(s, `${t}/${s.name}`, "local-folder");
      d && n.push(d);
    }
}
function yn(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      ...
      n.remove(), t(Pt(i.filter(Boolean)));
...
`).map(
    ...
    const o = a.split("|").filter(Boolean).map((d) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${d.trim()}</th>`).join(""), s = i.trim().split(`
`).map((d) => `<tr>${d.split("|").filter(Boolean).map(
      (l, f) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${f % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${l.trim()}</td>`
    ).join("")}</tr>`).join("");
...
const Ne = "atomek:default", pr = 3e3, Lt = "tytus.atomek.threadTitleOverrides", Kt = {
  available: !1,
  source: "none",
...
[...clipped ${e.length - t} chars...]`, Xt = (e, t) => {
  const n = e.split(`
`).map((a) => a.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, gr = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Jt = (e) => ({
  id: e.id,
  title: e.title,
  kind: gr(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = Et();
    n[e] = t, localStorage.setItem(Lt, JSON.stringify(n));
  } catch {
  }
}, wr = (e) => {
  try {
    const t = Et();
    delete t[e], localStorage.setItem(Lt, JSON.stringify(t));
  } catch {
  }
}, Gt = (e) => {
  const t = Et();
...
function vr({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const i = e.ai, [o, s] = x(null), [d, m] = x([]), [l, f] = x([]), [g, b] = x([]), [M, P] = x([]), [T, J] = x(Kt), [Q, ae] = x(!1), U = De(!0), S = De(null), F = w(async () => {
    if (!i) {
      J(Kt);
      return;
    }
    ...
    if (i)
      try {
        const y = await i.listArtifacts({ threadId: v });
        if (!U.current) return;
        b(y.map(Jt));
      } catch (y) {
        a(`AI artifacts unavailable: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [i, a]), He = w(async (v) => {
    if (i)
      try {
        const y = await i.listMessages(v);
        if (!U.current) return;
        const N = d.find((L) => L.id === v) ?? null;
        N && s(N), f(y.map(_e).filter(Boolean)), P([]), await G(v);
      } catch (y) {
        a(`Load chat failed: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [i, G, a, d]), C = w(async () => {
    if (i)
      try {
        const v = Gt(await i.listThreads({ workspaceKey: Ne, status: "active" })), y = v[0] ?? await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
        if (!U.current) return;
        m(v[0] ? v : [y]), s(y);
        const N = await i.listMessages(y.id);
        if (!U.current) return;
        f(N.map(_e).filter(Boolean)), await G(y.id);
      } catch (v) {
  ...
  const _ = w(async () => {
    if (!i) return null;
    if (o) return o;
    const v = await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
    return U.current && (s(v), m((y) => [v, ...y.filter((N) => N.id !== v.id)])), v;
  }, [i, o]), ce = w(async () => {
    if (!i) return;
    const v = await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
    s(v), m((y) => [v, ...y.filter((N) => N.id !== v.id)]), f([]), b([]), P([]), a("New AI chat created");
  }, [i, a]), ye = w(async (v, y) => {
    if (!i) return;
    const N = y.trim();
    if (N)
      try {
        const L = i.updateThread, q = d.find((ne) => ne.id === v) ?? o, O = typeof L == "function" ? await L({ threadId: v, title: N }) : { ...q ?? await _(), id: v, title: N, updatedAt: Date.now() };
        typeof L != "function" && yr(v, N), m((ne) => ne.map((le) => le.id === O.id ? O : le)), s((ne) => ne?.id === O.id ? O : ne), a(typeof L == "function" ? `Renamed chat: ${O.title}` : `Renamed chat locally: ${O.title}`);
      } catch (L) {
        a(`Rename chat failed: ${L instanceof Error ? L.message : String(L)}`);
      }
  }, [i, _, a, o, d]), D = w(async (v) => {
    if (i)
      try {
        await i.deleteThread(v), wr(v);
        const y = d.filter((N) => N.id !== v);
        if (m(y), o?.id === v) {
          const N = y[0] ?? await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
          s(N), m((q) => q.some((O) => O.id === N.id) ? q : [N, ...q]);
          const L = await i.listMessages(N.id);
          f(L.map(_e).filter(Boolean)), await G(N.id);
        }
        a("Deleted AI chat");
      } catch (y) {
        a(`Delete chat failed: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [i, G, a, o?.id, d]), se = w(async (v) => {
    if (!i) return null;
    try {
      const y = await _();
      if (!y) return null;
      const N = await i.createArtifact({
        threadId: y.id,
        messageId: v.messageId ?? null,
        title: v.title?.trim() || Xt(v.body, "Atomek artifact"),
        kind: v.kind ?? "markdown",
        body: v.body
      }), L = Jt(N);
      return b((q) => [L, ...q.filter((O) => O.id !== L.id)]), a(`Saved AI artifact: ${L.title}`), L;
    } catch (y) {
      return a(`Save artifact failed: ${y instanceof Error ? y.message : String(y)}`), null;
    }
  }, [i, _, a]), Ie = w(async (v) => {
    if (i)
      try {
        await i.deleteArtifact(v), b((y) => y.filter((N) => N.id !== v)), a("Deleted AI artifact");
      } catch (y) {
        a(`Delete artifact failed: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [i, a]), et = w(async (v) => {
    if (!i) return null;
    try {
      const y = await i.writeMemory({
        title: v.title?.trim() || Xt(v.body, "Atomek memory"),
        body: v.body,
        metadata: {
          source: "atomek",
          messageId: v.messageId ?? null
      ...
      return P((N) => [y, ...N.filter((L) => L.id !== y.id)].slice(0, 5)), a(`Remembered: ${y.title}`), y;
    } catch (y) {
      return a(`Remember failed: ${y instanceof Error ? y.message : String(y)}`), null;
    }
  }, [i, a]), Me = w(async (v) => {
    if (!i) return [];
    const y = await i.searchMemory({ query: v, limit: 5 });
    return U.current && P(y), y;
  }, [i]), Be = w(async (v, y = {}) => {
    const N = v.trim();
    if (!N || !i) return null;
    ae(!0);
    ...
    try {
      const O = await _();
      if (!O) return null;
      const ne = await Me(N).catch(() => []), le = kr(ne), de = y.requestContext ?? t, we = le ? [...de, le] : [...de];
      let ve = null;
      for await (const j of i.sendMessage({
        ...
        if (j.type === "message_created") {
          ...
          if (!z) continue;
          z.role === "assistant" && (ve = z.id), f((W) => [...W.filter((Z) => Z.id !== z.id), z]);
        }
        if (j.type === "token" && (ve = j.messageId, f((z) => z.some((W) => W.id === j.messageId) ? z.map(
          ...
          const z = _e(j.message);
          if (!z) continue;
          z.role === "assistant" && (L = z), f((W) => W.map((Z) => Z.id === z.id ? z : Z)), z.gatewayLabel && a(`AI answered via ${z.gatewayLabel}`);
        }
        if (j.type === "run_failed") {
          const z = q.signal.aborted;
          a(z ? "AI response stopped" : `AI failed: ${j.error}`), ve && f((W) => W.map((Z) => Z.id !== ve ? Z : z ? { ...Z, status: "complete", error: void 0, body: Z.body || "Stopped by user." } : { ...Z, status: "error", error: j.error, body: j.error }));
        }
      }
      const Ce = Gt(await i.listThreads({ workspaceKey: Ne, status: "active" }).catch(() => []));
  ...
  }, [i, n.gatewayPreference, n.model, _, Me, F, t, a]), We = w(() => {
...
const Cr = (e) => `tytus-workbench:///${encodeURI(e.path)}`, xr = (e) => {
...
}, $r = "active-file", Nr = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Sr = (e, t = "file") => `${t}:${e.id}`, Ir = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let i = [];
  return t.scope === "active-selection" ? a?.selection ? i = [Mr(a, a.selection, !0)] : a && (i = [Je(a, !0, "Active file")]) : t.scope === "active-file" ? a && (i = [Je(a, !0, "Active file")]) : t.scope === "open-editors" ? i = e.openDocumentIds.map((o) => e.byId.get(o)).filter(Boolean).map((o) => Je(o, !0, "Open editor")) : t.scope === "selected-files" && (i = t.selectedFileIds.map((o) => e.byId.get(o)).filter(Boolean).map((o) => Je(o, !1, "Selected file"))), Lr(i).filter((o) => !n.has(o.id));
}, Lr = (e) => {
...
[...clipped ${e.length - t} chars...]`, zr = (e, t, n) => {
  const a = Pr(e, n), i = [], o = new Map(t.map((s) => [s.id, s]));
  for (const s of a) {
    if (!s.fileId) continue;
    const d = o.get(s.fileId), m = e.byId.get(s.fileId);
    ...
    const l = [
      ...
      s.range ? `Range: ${Yt(s.range)}` : null
    ].filter(Boolean).join(`
`), f = s.kind === "selection" && s.range ? _r(d.content, s.range) : d.content, g = s.kind === "selection" ? Er : s.label.startsWith("Open editor") ? Tr : Fr;
    i.push({
...
`));
}, Yt = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, Qt = 75e4, Rr = 2800, jr = 240, Dr = 80, Hr = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? Qt) || e.content.length > (t.maxFileBytes ?? Qt) ? "huge" : !t.includeDirty && e.dirty ? null : Xr(e.path) ? "vendor" : Jr(e) ? "binary" : null;
}, Zt = (e, t = {}) => {
  ...
  for (const s of e) {
    ...
    const l = ke(s.content), f = Wr(s, l, n, t);
    o.push(...f), a.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
  ...
  const i = Math.max(500, a.maxChunkChars ?? Rr), o = Math.min(Math.max(0, a.chunkOverlapChars ?? jr), Math.floor(i / 2)), s = Math.max(1, a.maxChunksPerFile ?? Dr), d = qr(e.content), m = [];
  let l = 0, f = 0;
  for (; l < e.content.length && m.length < s; ) {
    const g = Ur(e.content, l, Math.min(e.content.length, l + i)), b = e.content.slice(l, g).trim();
    if (b.length > 0) {
      const M = Kr(d, l, g);
      m.push({
...
` && t.push(n + 1);
  ...
  const a = en(e, t), i = en(e, Math.max(t, n - 1));
  return {
    startLineNumber: a + 1,
    startColumn: t - e[a] + 1,
    endLineNumber: i + 1,
...
class Gr {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Zt(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Zt(t, this.options), this.snapshot;
  }
...
const Se = (e = [], t = {}) => new Gr(e, t), Yr = (e, t, n = {}) => {
  const a = new Map(t.map((d) => [d.id, d])), i = e.byFileId, o = [];
  for (const d of e.files) {
    const m = a.get(d.fileId);
    if (!m) {
      o.push({ fileId: d.fileId, path: d.path, status: "deleted", indexedHash: d.hash });
      continue;
    }
    if ($t(m, n)) {
      o.push({ fileId: d.fileId, path: m.path, status: "skipped-now", indexedHash: d.hash, currentHash: ke(m.content) });
      continue;
    }
    const f = ke(m.content);
    f !== d.hash ? o.push({ fileId: d.fileId, path: m.path, status: "changed", indexedHash: d.hash, currentHash: f }) : m.dirty !== d.dirty && o.push({ fileId: d.fileId, path: m.path, status: "dirty-state-changed", indexedHash: d.hash, currentHash: f });
  }
  for (const d of t)
    !i.has(d.id) && !$t(d, n) && o.push({ fileId: d.id, path: d.path, status: "new", currentHash: ke(d.content) });
  const s = e.signature !== vn(t);
  return { stale: o.length > 0, signatureChanged: s, files: o };
}, Qr = 8, Zr = 12e3, Cn = (e, t, n = {}, a) => {
  const i = ni(t);
  if (i.length === 0) return [];
  const o = new Set(a?.files.map((f) => f.fileId) ?? []), s = e.chunks.filter((f) => n.includeDirty !== !1 || !f.dirty).map((f) => ai(f, i)).filter((f) => f.score >= (n.minScore ?? 1)).sort((f, g) => g.score - f.score || f.chunk.path.localeCompare(g.chunk.path) || f.chunk.ordinal - g.chunk.ordinal), d = [], m = /* @__PURE__ */ new Set();
  let l = n.maxChars ?? Zr;
  for (const f of s) {
    if (d.length >= (n.limit ?? Qr) || l <= 0) break;
    const g = `${f.chunk.fileId}:${f.chunk.ordinal}`;
    if (m.has(g)) continue;
    m.add(g);
    const b = f.chunk.text.length > l ? `${f.chunk.text.slice(0, Math.max(0, l - 28))}
[...context clipped...]` : f.chunk.text;
    d.push(ei(f, b, o.has(f.chunk.fileId))), l -= b.length;
  }
  return d;
...
`), ni = (e) => Array.from(new Set(
  ...
  const d = [];
  for (const m of t) {
    const l = kt(n, m), f = kt(a, m), g = kt(i, m), b = o === m ? 1 : 0;
    l + f + g + b !== 0 && (d.push(m), s += Math.min(l, 12), s += f * 4, s += g * 6, s += b * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (s += 3));
  }
  ...
  const n = e.toLowerCase(), a = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, l) => m - l)[0] ?? 0, i = Math.max(0, a - 120), o = Math.min(e.length, a + 260), s = i > 0 ? "…" : "", d = o < e.length ? "…" : "";
  return `${s}${e.slice(i, o).replace(/\s+/g, " ").trim()}${d}`;
}, ii = (e, t = {}) => {
  const { autoRefresh: n = !1, ...a } = t, i = De(e), o = De(null);
  o.current || (o.current = Se(e, a));
  const [s, d] = x(() => o.current?.getSnapshot() ?? Se(e, a).getSnapshot());
  i.current = e;
  const m = w((b = i.current) => {
    const M = (o.current ?? Se([], a)).refresh(b, a);
    return d(M), M;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), l = w((b = i.current) => {
    const M = (o.current ?? Se([], a)).update(b, a);
    return d(M), M;
  }, [m]);
  X(() => {
    n && m(e);
  }, [n, e, m]);
  const f = (o.current ?? Se([], a)).staleReport(e), g = w((b, M = {}) => {
    const P = o.current ?? Se([], a);
    return Cn(P.getSnapshot(), b, M, P.staleReport(i.current));
  }, []);
  return { snapshot: s, staleReport: f, isStale: f.stale, refresh: m, update: l, retrieve: g };
}, oi = "tytus.atomek.semanticVector:v1", xn = "__gateway_default__", An = (e) => e?.trim() || xn, ci = (e) => {
  const t = An(e);
...
}, Re = 8, di = 12e3, hi = 180, ui = async (e, t, n, a, i = {}, o, s) => {
  const d = Cn(t, n, { ...i, limit: Math.max(i.limit ?? Re, 16) }, o), m = e.ai;
  ...
  try {
    const l = An(a.embeddingModel), f = await m.embedText({
      input: n,
    ...
    const b = new Map(d.map((S) => [S.chunkId, S])), M = yi(n), P = /* @__PURE__ */ new Map();
    t.chunks.filter((S) => i.includeDirty !== !1 || !S.dirty).slice(0, hi).forEach((S) => P.set(S.id, S)), d.forEach((S) => {
      const F = t.byChunkId.get(S.chunkId);
      F && P.set(F.id, F);
    });
    const T = Array.from(P.values()), J = [];
    let Q = 0;
    for (const S of T) {
      s?.aborted;
      const F = si(e.appId, l, S) ?? await fi(m, e.appId, l, S, a, s);
      if (!F) continue;
      F.updatedAt > Date.now() - 1e3 && (Q += 1);
      const G = gi(g, F.vector);
      if (!Number.isFinite(G)) continue;
      const C = b.get(S.id)?.score ?? wi(S, M), _ = ki(C) * 0.42 + Math.max(0, G) * 0.58;
      J.push({ chunk: S, keywordScore: C, vectorScore: G, score: _, matchedTerms: M.filter((ce) => S.text.toLowerCase().includes(ce)) });
    }
    ...
    const ae = new Set(o?.files.map((S) => S.fileId) ?? []), U = J.sort((S, F) => F.score - S.score || F.keywordScore - S.keywordScore || S.chunk.path.localeCompare(F.chunk.path) || S.chunk.ordinal - F.chunk.ordinal).slice(0, Math.max(i.limit ?? Re, 1)).map((S) => pi(S, ae.has(S.chunk.fileId)));
    return {
    ...
    return {
      ...
      reason: `Semantic retrieval failed (${l instanceof Error ? l.message : String(l)}) — using keyword retrieval.`,
      embeddedChunks: 0
    };
  }
}, mi = (e) => e.map((t) => ({
...
}, ki = (e) => Math.min(1, Math.max(0, e / 24)), yi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), wi = (e, t) => {
  const n = `${e.path}
${e.name}
...
${e.text}`.toLowerCase();
  ...
  const n = e.toLowerCase(), a = t.map((s) => n.indexOf(s)).filter((s) => s >= 0).sort((s, d) => s - d)[0] ?? 0, i = Math.max(0, a - 120), o = Math.min(e.length, a + 260);
  return `${i > 0 ? "…" : ""}${e.slice(i, o).replace(/\s+/g, " ").trim()}${o < e.length ? "…" : ""}`;
}, Ci = /```([^\n`]*)\n([\s\S]*?)```/g, xi = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Ft(e) {
  return Array.from(e.matchAll(Ci)).map((t) => In(t[1] ?? "", Ii(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function In(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), a = {}, i = [];
  for (const o of n) {
    const s = o.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? a[s[1].toLowerCase()] = Pi(s[2]) : i.push(o.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: i, attrs: a, content: t };
}
function te(e, t) {
  return (typeof e == "string" ? In(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Mn(e) {
  return [...Ft(e).filter((a) => te(a, "diff") || te(a, "patch")).map((a) => a.content), e].filter((a, i, o) => Ai(a) && o.findIndex((s) => s === a) === i);
}
function Ai(e) {
...
`), a = [];
  ...
  const i = Array.from(new Set(a)).sort((s, d) => s - d), o = [];
  for (let s = 0; s < i.length; s += 1) {
    const d = i[s], l = i.find((g) => g > d && n[g].startsWith("diff --git ")) ?? n.length, f = n.slice(d, l).join(`
`);
    /^@@\s+-\d+/m.test(f) && o.push({ raw: f, ...yt(f) });
  }
  return o.length > 0 ? o : [{ raw: t, ...yt(t) }];
}
function yt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), i = ie(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, o = ie(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, s = o && o !== "/dev/null" ? o : i;
...
function Nt(e) {
  return Ft(e).filter((t) => Ni(t)).map((t) => ({
...
`), i = [];
  ...
  for (let d = 0; d < a.length; d += 1) {
    ...
    if (!m) continue;
    s = !0;
    const l = Number(m[1]), f = Math.max(0, l - 1);
    if (f < o) return null;
    for (i.push(...n.slice(o, f)), o = f, d += 1; d < a.length; d += 1) {
      const g = a[d];
      if (g.startsWith("@@ ")) {
        d -= 1;
        break;
...
function Li(e) {
  const { body: t, files: n, sourceTitle: a, activeFile: i = null, versions: o = {} } = e, s = /* @__PURE__ */ new Map(), d = [];
  for (const f of Mn(t))
    for (const g of $i(f)) {
      if (!g.path) {
        if (i) {
          const P = St(i.content, g.raw);
          P && P !== i.content && s.set(i.id, Ge({
            file: i,
            proposedContent: P,
            sourceTitle: a,
      ...
      const M = St(b.file.content, g.raw);
  ...
  for (const f of Nt(t)) {
    const g = f.path ? tn(n, f.path) : i ? { file: i, normalizedPatchPath: ie(i.path), confidence: "exact" } : null;
    if (!g) {
      d.push(`${f.path ?? "replacement block"}: no opened file`);
      continue;
    }
  ...
  const m = Nt(t).some((f) => f.path);
  if (s.size === 0 && i && !m) {
    const f = Ei(t, i);
    f && f.content !== i.content && s.set(i.id, Ge({
      file: i,
      proposedContent: f.content,
      sourceTitle: a,
      extractionLabel: f.label,
  ...
  return {
    ...
    stats: Ti(l.map((f) => f.stats)),
    kind: l.length === 0 ? "empty" : l.length === 1 ? "single-file" : "multi-file"
  };
}
function Ei(e, t) {
  for (const m of Mn(e)) {
    const l = St(t.content, m);
    if (l) return { content: l, label: "unified diff patch" };
  }
  const n = Ft(e);
  if (n.length === 0) return null;
  const a = zi(t), i = Nt(e)[0];
  if (i) return { content: i.content, label: i.label };
  const o = n.find((m) => a.some((l) => te(m, l)) && !te(m, "diff") && !te(m, "patch"));
  if (o) return { content: o.content, label: `matched ${o.lang || t.language} block` };
  const d = n.filter((m) => !te(m, "diff") && !te(m, "patch")).sort((m, l) => l.content.length - m.content.length)[0];
...
function tn(e, t) {
  ...
  const o = e.find((d) => ie(d.path).endsWith(`/${n}`));
...
`), i = Math.max(n.length, a.length);
  let o = 0, s = 0, d = 0;
  for (let m = 0; m < i; m += 1)
    n[m] !== a[m] && (n[m] === void 0 ? o += 1 : a[m] === void 0 ? s += 1 : d += 1);
  return { added: o, removed: s, changed: d };
}
function Ti(e) {
  return e.reduce((t, n) => ({
...
}, Hi = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : re(e.capability) || re(e.capabilities) || re(e.modality) || re(e.modalities) || re(e.task) || re(e.tasks) || re(e.type) || re(e.kind), Ln = (e) => typeof e == "object" && e !== null, Bi = (e) => Ln(e.ai) ? e.ai : null, En = (e) => typeof Bi(e)?.embedText == "function", Wi = async (e, t) => En(e) ? (await Di(e, t)).filter(Hi) : [], Vi = (e) => Ln(e.ai) ? En(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", Ui = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], qi = [
...
function Qi(e) {
  ...
  for (const a of t) {
    ...
    const o = Tn(a.path), s = eo(e, o, i);
    if (!s) continue;
    const d = Object.keys(i.scripts).filter((l) => typeof i.scripts?.[l] == "string"), m = to(d);
    for (const l of m) {
      const f = `${s} run ${l}`;
      n.push({
        id: On(`${o}:${f}`),
...
function eo(e, t, n) {
  ...
  for (const [a, i] of qi)
    if (e.some((o) => ao(o.path) === a && Tn(o.path) === t)) return i;
  return null;
}
function to(e) {
...
const io = ga(() => import("./WorkbenchMonacoEditor-DbnEHW95.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), oo = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, zn = "tytus.workspace.recent", _n = "tytus.workspace.layout", Rn = "tytus.atomek.chatAiSettings", Ye = {
  gatewayPreference: "auto",
  model: "",
  embeddingModel: ""
};
function co({ host: e }) {
  const t = K(() => zo(), []), [n, a] = x("explorer"), [i, o] = x(t.primaryVisible), [s, d] = x(t.primaryWidth), [m, l] = x("chat"), [f, g] = x(t.secondaryVisible), [b, M] = x(t.secondaryWidth), [P, T] = x(!1), [J, Q] = x("problems"), [ae, U] = x(t.markdownPreviewVisible), [S, F] = x(!1), [G, He] = x(null), [C, _] = x([]), [ce, ye] = x([]), [D, se] = x(null), [Ie, et] = x(""), [Me, Be] = x({ lineNumber: 1, column: 1 }), [We, v] = x(null), [y, N] = x({}), [L, q] = x($r), [O, ne] = x([]), [le, de] = x([]), [we, ve] = x(null), [Ce, j] = x(""), [z, W] = x(!1), [Z, Bn] = x(""), [Ot, tt] = x([]), [H, xe] = x(null), [he, Pe] = x(null), [Wn, nt] = x(!1), [pe, Vn] = x(() => _o()), [zt, Le] = x(null), [Ae, at] = x(null), [rt, it] = x(""), [ot, ct] = x(""), [Ve, Ue] = x(""), [qe, _t] = x("failed"), [Un, Rt] = x(null), [qn, A] = x("Ready"), [Ee, Kn] = x(() => Oo()), ee = ce.map((c) => C.find((u) => u.id === c)).filter(Boolean), $ = D ? C.find((c) => c.id === D) ?? null : null, jt = K(() => Ar({ files: C, openEditorIds: ce, activeFileId: D, versions: y, activeSelection: We }), [D, We, y, C, ce]), Dt = K(() => ({
    scope: L,
    removedAttachmentIds: O,
    selectedFileIds: []
  }), [L, O]), Fe = K(() => zr(jt, C, Dt), [Dt, jt, C]), ue = ii(C, { autoRefresh: !0, includeDirty: !0 }), Ht = K(() => le.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [le]), Xn = K(() => [...Fe.attachments, ...Ht], [Fe.attachments, Ht]), I = vr({ host: e, requestContext: Fe.parts, chatSettings: pe, setStatus: A }), be = K(
    () => [...I.artifacts, ...Ot].sort((c, u) => u.createdAt - c.createdAt),
    [I.artifacts, Ot]
  ), st = !$ && !S, V = K(() => C.filter((c) => c.dirty), [C]), Jn = K(() => {
    const c = Ie.trim().toLowerCase();
    return c ? C.filter((u) => u.path.toLowerCase().includes(c)) : C;
  }, [C, Ie]);
  X(() => {
    N((c) => {
      const u = {};
      for (const p of C) u[p.id] = c[p.id] ?? 1;
      return u;
    });
  }, [C]), X(() => {
    ne([]), de([]);
  ...
  const Gn = w((c) => {
    if (c.kind === "index-hit") {
      de((u) => u.filter((p) => p.id !== c.id)), A(`Removed project context: ${c.label}`);
      return;
    }
    ne((u) => u.includes(c.id) ? u : [...u, c.id]), A(`Removed chat context: ${c.label}`);
  }, []), $e = w((c) => {
    N((u) => ({ ...u, [c]: (u[c] ?? 1) + 1 }));
  }, []), Yn = w((c) => {
    c.preventDefault();
    const u = c.clientX, p = b, k = (fe) => {
      const ze = p + (u - fe.clientX);
      M(Math.max(380, Math.min(760, ze)));
    }, R = () => {
    ...
    const u = [c, ...Ee.filter((p) => p.path !== c.path)].slice(0, 6);
    Kn(u), localStorage.setItem(zn, JSON.stringify(u));
  }, [Ee]), B = w((c, u) => {
    ye((p) => p.includes(c.id) ? p : [...p, c.id]), se(c.id), F(!1), Rt(u ?? null), Be({ lineNumber: u ?? 1, column: 1 });
  }, []), Zn = w((c) => {
    if (!c.fileId) return;
    const u = C.find((p) => p.id === c.fileId);
    u && (B(u, c.range?.startLineNumber ?? 1), A(`Revealed context: ${c.label}`));
  }, [C, B]), Te = w(async () => {
    if (wt(V, "open new files"))
      try {
        const c = await hr();
        if (c.length === 0) return;
        _((u) => Eo(u, c)), c.forEach((u) => Ke({ name: u.name, path: u.path, at: Date.now() })), B(c[0]), A(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && A(`Open file failed: ${c.message}`);
      }
  }, [V, B, Ke]), lt = w(async () => {
    if (wt(V, "open another folder"))
      try {
        const c = await ur();
        He(c), _(c.files), ye([]), se(null), F(!1), Ke({ name: c.name, path: c.name, at: Date.now() }), A(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!D) return;
    let u = !1;
    _((p) => p.map((k) => k.id !== D || k.content === c ? k : (u = !0, { ...k, content: c, dirty: !0 }))), u && $e(D);
  }, [D, $e]), dt = w(async () => {
    if ($)
      try {
        const c = await gt($);
        _((u) => u.map((p) => p.id === c.id ? c : p)), A(`Saved ${c.name}`);
      } catch (c) {
        A(`Save failed: ${c.message}`);
      }
  }, [$]), ta = w(async (c) => {
    const u = C.find((k) => k.id === c);
    if (!u) return null;
    const p = await gt(u);
    ...
    try {
      const u = await Promise.all(c.map((k) => gt(k))), p = new Map(u.map((k) => [k.id, k]));
      _((k) => k.map((R) => p.get(R.id) ?? R)), Le(null), A(`Saved ${u.length} dirty file${u.length === 1 ? "" : "s"}`);
    } catch (u) {
      A(`Save all failed: ${u.message}`);
    }
  }, [C]), ht = w((c) => {
    const u = C.find((p) => p.id === c);
    if (u?.dirty && !window.confirm(`${u.name} has unsaved changes. Close without saving?`)) {
      A(`Close canceled — ${u.name} has unsaved changes`);
      return;
    }
    ye((p) => {
      const k = p.filter((R) => R !== c);
      return D === c && (se(k.at(-1) ?? null), v(null)), k;
    });
  }, [D, C]), na = w(() => {
    wt(V, "close all editors") && (ye([]), se(null), Rt(null), F(!1), A("Closed all editors"));
  }, [V]), ut = w(() => {
    const c = C.filter((p) => p.name.startsWith("Untitled")).length + 1, u = {
...
`,
  ...
  }, [Fe.parts, L, pe, e, ue]), mt = w(() => {
    if (!$ && ee.length === 0) {
      A("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const u = [
      `Create a polished Markdown artifact from ${$?.path ?? `${ee.length} open editors`}.`,
    ...
    g(!0), l("chat"), A("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const p = await me(u), k = await I.askAgent(u, { requestContext: p });
      !k || k.status === "error" || I.createArtifact({
        messageId: k.id,
        title: `AI synthesis — ${$?.name ?? "open workspace"}`,
        kind: "markdown",
        body: k.body
      }).then(() => {
        g(!0), l("outputs"), T(!0), Q("output");
      });
    })();
  }, [$, I, me, ee.length]), ft = w((c) => {
    const u = C.find((p) => p.path === c.path || p.name === c.name);
    if (u) {
    ...
    g(!0), l("chat"), j(""), (async () => {
      const u = await me(c);
      await I.askAgent(c, { requestContext: u });
    })();
  }, [I, me]), aa = w((c) => {
    const u = I.messages.findIndex((k) => k.id === c.id), p = I.messages.slice(0, u < 0 ? void 0 : u).reverse().find((k) => k.role === "user");
    if (!p?.body.trim()) {
    ...
    (async () => {
      ...
      await I.askAgent(p.body, { requestContext: k });
    })();
  }, [I, me]), ra = w((c) => {
    I.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const u = Ct(C, vt(c.title || c.kind)), p = {
    ...
    _((k) => [...k, p]), B(p), A(`Opened ${c.title} as editable file`);
  }, [C, B]), bt = w(() => {
...
`), R = {
    ...
    const u = $o(c);
    (async () => {
      const p = await me(c), k = await I.askAgent(c, { requestContext: p });
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", k.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    Ue(p), it(""), ct(""), _t("failed"), T(!0), Q("terminal"), A(u.commands.length > 0 ? `Manual check ready: copy ${u.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [C]), la = w((c) => {
    c.trim() && (navigator.clipboard?.writeText(c).catch(() => {
    }), Ue(c), A(`Manual check command copied: ${c}`));
  }, []), da = w(() => {
    at((c) => {
      if (!c) return c;
      const u = Xi(c, rt), p = u.commands.at(-1)?.command ?? "";
      return p && Ue(p), u;
    }), it("");
  }, [rt]), ha = w(() => {
    if (!Ae || !Ve.trim()) {
    ...
    const c = Ve.trim();
    at((u) => u && Ji(u, c, qe, ot)), ct(""), A(`Captured manual check result: ${c} (${qe})`);
  }, [ot, Ve, Ae, qe]), ua = w(() => {
    Ae && Y(Gi(Ae));
  }, [Y, Ae]), ma = w(() => {
    if (!H) return;
    const c = C.find((u) => u.id === H.fileId);
    if (!c) {
      A(`Cannot apply edit — ${H.fileName} is no longer open`), xe(null);
      return;
    }
    c.content !== H.originalContent && !window.confirm(`${H.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (_((u) => u.map((p) => p.id === H.fileId ? { ...p, content: H.proposedContent, dirty: !0 } : p)), $e(H.fileId), Le(`AI edit applied to ${c.name}. Save All to persist it to disk.`), xe(null), Oe(`AI edit applied to ${c.name}`), A(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [$e, C, H, Oe]), fa = w(() => {
    if (!H) return;
    const c = Ct(C, `${vt(H.fileName)}-ai-edit`), u = {
    ...
    _((p) => [...p, u]), B(u), xe(null), A(`Opened proposed edit as ${c}`);
  }, [$?.language, C, B, H]), pa = w(() => {
    if (!he) return;
    ...
    })), c.forEach((p, k) => $e(k)), Le(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), Pe(null), Oe(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), A(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [$e, C, he, Oe]), ba = w(() => {
    if (!he) return;
    const c = [...C], u = he.edits.map((p) => {
      const k = Ct(c, `${vt(p.fileName)}-ai-edit`), R = C.find((ze) => ze.id === p.fileId), fe = {
  ...
  return X(() => {
    const c = (u) => {
      (u.metaKey || u.ctrlKey) && (u.key.toLowerCase() === "o" && (u.preventDefault(), Te()), u.key.toLowerCase() === "s" && (u.preventDefault(), dt()), u.key.toLowerCase() === "w" && (u.preventDefault(), D && ht(D)), u.key.toLowerCase() === "b" && (u.preventDefault(), o((k) => !k)), u.shiftKey && u.key.toLowerCase() === "f" && (u.preventDefault(), a("search"), o(!0)), (u.key.toLowerCase() === "k" || u.key.toLowerCase() === "p") && (u.preventDefault(), W(!0)));
    };
    ...
    const c = { primaryVisible: i, primaryWidth: s, secondaryVisible: f, secondaryWidth: b, markdownPreviewVisible: ae };
    localStorage.setItem(_n, JSON.stringify(c));
  }, [ae, i, s, f, b]), X(() => {
    localStorage.setItem(Rn, JSON.stringify(pe));
  }, [pe]), /* @__PURE__ */ h(
    "div",
    {
      className: `workbench-workbench ${i ? "" : "no-primary"} ${f ? "" : "no-secondary"} ${P ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${s}px`, "--workbench-secondary-width": `${b}px` },
      children: [
        /* @__PURE__ */ r(so, { active: n, setActive: (c) => {
          a(c), o(!0);
        }, openSettings: () => nt(!0) }),
        i && /* @__PURE__ */ h("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ r("div", { className: "workbench-primary-resizer", onPointerDown: Qn, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ h("main", { className: "workbench-editor-area", children: [
          ...
          /* @__PURE__ */ h("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ r(mo, { file: $, folder: G, showWelcome: st }),
            zt && V.length > 0 ? /* @__PURE__ */ h("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ r("span", { children: zt }),
              /* @__PURE__ */ r("button", { onClick: () => {
                Bt();
              }, children: "Save all" }),
              /* @__PURE__ */ r("button", { onClick: () => Le(null), title: "Dismiss", children: /* @__PURE__ */ r(oe, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: $ ? /* @__PURE__ */ h("div", { className: $.language === "markdown" && ae ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ r("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ r(ka, { fallback: /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ r(
                io,
                {
                  file: $,
                  revealLine: Un,
              ...
              $.language === "markdown" && ae && /* @__PURE__ */ r(yo, { content: $.content })
            ...
            P && /* @__PURE__ */ r(
              vo,
              {
                ...
                setManualCheckStatus: _t,
                copyManualCheckCommand: la,
                addManualCheckCommand: da,
                recordManualCheckResult: ha,
                askAgentFromManualChecks: ua,
                onClose: () => T(!1)
              }
            )
          ] })
        ...
        f && /* @__PURE__ */ r(
          Co,
          {
            ...
            saveMessageAsArtifact: ra,
            rememberMessage: ia,
            previewEditFromMessage: (c) => Xe(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: Wt,
            pendingPatchPrompt: we,
            generatePatchPrompt: sa,
            ...
            captureManualCheck: bt,
            openOutputAsFile: pt,
            previewEditFromOutput: (c) => Xe(c.title, c.body),
        ...
        z && /* @__PURE__ */ r(
          ko,
          {
            ...
            commands: [
              ...
              ...Ee.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => ft(c) })),
              { label: "File: Save All", detail: `${V.length} dirty file${V.length === 1 ? "" : "s"}`, run: () => {
                Bt();
              }, disabled: V.length === 0 },
              { label: "File: Close All Editors", detail: `${ee.length} open editor${ee.length === 1 ? "" : "s"}`, run: na, disabled: ee.length === 0 },
              ...
              { label: "View: Toggle Primary Side Bar", detail: i ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => o((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: f ? "Hide right AI side bar" : "Show right AI side bar", run: () => g((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: P ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => T((c) => !c) },
              ...
              { label: "View: Toggle Markdown Preview", detail: $?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => U((c) => !c), disabled: $?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: $ || ee.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: mt, disabled: !$ && ee.length === 0 },
              { label: "AI: Explain Active File", detail: $ ? `Ask Cortex to explain ${$.path}` : "Open a file first", run: () => Y("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !$ },
              { label: "AI: Improve Active File", detail: $ ? `Ask Cortex for concrete edits to ${$.path}` : "Open a file first", run: () => Y("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !$ },
              { label: "AI: Draft Editable Replacement", detail: $ ? `Ask Cortex for a full-file replacement for ${$.path}` : "Open a file first", run: () => Wt("edit"), disabled: !$ },
              ...
              { label: "AI: Save Active File as Artifact", detail: $ ? "Persist active file in host.ai artifacts" : "Open a file first", run: oa, disabled: !$ },
        ...
        /* @__PURE__ */ r(Lo, { status: qn, file: $ ?? oo, cursor: Me, fileCount: C.length, dirtyCount: V.length })
      ]
    }
  );
}
function so({ active: e, setActive: t, openSettings: n }) {
  return /* @__PURE__ */ h("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    ...
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(Ba, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(ln, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(sn, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ r("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(Pa, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
...
function lo(e) {
  return e.activity === "search" ? /* @__PURE__ */ r(bo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ r(rn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ r(rn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ r(Po, {}) : /* @__PURE__ */ r(ho, { ...e });
}
function ho(e) {
  ...
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ h(je, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((a) => /* @__PURE__ */ r(Tt, { file: a, active: a.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(a) }, a.id)),
        /* @__PURE__ */ h("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(At, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No readable text files found." }) : jn(n, e.activeFileId, e.openWorkbenchFile)
      ] }),
      ...
      e.recent.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((a) => /* @__PURE__ */ h("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(a), children: [
        /* @__PURE__ */ r(Da, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: a.name })
      ] }, `${a.path}-${a.at}`))
    ] })
  ] });
}
function Tt({ file: e, active: t, onOpen: n, basePath: a, depth: i = 0, label: o }) {
...
function uo(e, t) {
  const n = [], a = /* @__PURE__ */ new Map(), i = (s, d, m = n) => {
    const l = a.get(s);
    if (l) return l;
    const f = { name: d, path: s, children: [] };
  ...
  e.forEach((s) => {
    ...
    m.slice(0, -1).forEach((g) => {
      f = f ? `${f}/${g}` : g, l = i(f, g, l).children;
    }), l.push({ name: m.at(-1) ?? s.name, path: d, file: s, children: [] });
  });
  const o = (s) => s.sort((d, m) => +!!d.file - +!!m.file || d.name.localeCompare(m.name)).map((d) => ({ ...d, children: o(d.children) }));
  return o(n);
}
function jn(e, t, n, a = 0) {
  return e.map((i) => i.file ? /* @__PURE__ */ r(Tt, { file: i.file, active: i.file.id === t, onOpen: () => n(i.file), depth: a, label: i.name }, i.file.id) : /* @__PURE__ */ h("div", { children: [
...
function po({ openFile: e, openFolder: t, newFile: n, recent: a, reopenRecent: i, setStatus: o }) {
...
function bo({ files: e, query: t, setQuery: n, openWorkbenchFile: a, activeFileId: i }) {
  const o = K(() => go(e, t), [e, t]), s = K(() => {
    const d = /* @__PURE__ */ new Map();
    return o.forEach((m) => d.set(m.file.id, [...d.get(m.file.id) ?? [], m])), Array.from(d.values()).slice(0, 50);
  }, [o]);
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? s.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No matches." }) : s.map((d) => {
        ...
        return /* @__PURE__ */ h("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ r(Tt, { file: m, active: m.id === i, onOpen: () => a(m) }),
          d.slice(0, 5).map((l) => /* @__PURE__ */ h("button", { className: "workbench-search-hit", onClick: () => a(m, l.lineNumber), title: l.line, children: [
            /* @__PURE__ */ r("span", { className: "workbench-search-line", children: l.lineNumber }),
            /* @__PURE__ */ r("span", { children: l.line })
          ] }, `${m.id}-${l.lineNumber}-${l.line}`)),
...
function ko(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${pn(i.language)} file`,
...
function vo(e) {
  return /* @__PURE__ */ h("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ r(Hn, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Co(e) {
  return /* @__PURE__ */ h("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ h("div", { className: "workbench-secondary-actions", children: [
        ...
        /* @__PURE__ */ r("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ r(Ea, { size: 16 }) }),
        /* @__PURE__ */ r("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ r(oe, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ r(xo, { ...e }) : /* @__PURE__ */ r(Hn, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function xo(e) {
  const t = De(null), [n, a] = x(!0), [i, o] = x(!1), s = K(() => e.chatMessages.map((l) => `${l.id}:${l.status ?? ""}:${l.body.length}`).join("|"), [e.chatMessages]);
  X(() => {
    const l = t.current;
    l && (n ? (l.scrollTop = l.scrollHeight, o(!1)) : o(!0));
  }, [n, s]);
  const d = w(() => {
    const l = t.current;
    if (!l) return;
    const g = l.scrollHeight - l.scrollTop - l.clientHeight < 48;
    a(g), g && o(!1);
  }, []), m = w(() => {
    const l = t.current;
    l && (l.scrollTop = l.scrollHeight, a(!0), o(!1));
  }, []);
  return /* @__PURE__ */ h("div", { className: "workbench-chat-wrap", children: [
    ...
    /* @__PURE__ */ h("div", { ref: t, className: "workbench-chat-transcript", onScroll: d, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ r("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ h("div", { children: [
        ...
        l.role === "assistant" && l.status !== "streaming" && l.status !== "error" ? /* @__PURE__ */ h("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ r("button", { onClick: () => e.saveMessageAsArtifact(l), children: "Save artifact" }),
          /* @__PURE__ */ r("button", { onClick: () => e.rememberMessage(l), children: "Remember" }),
          /* @__PURE__ */ r("button", { onClick: () => e.previewEditFromMessage(l), disabled: e.workspaceFileCount === 0, children: "Preview edit" }),
          /* @__PURE__ */ h("button", { onClick: () => e.regenerateMessage(l), disabled: e.busy, children: [
            /* @__PURE__ */ r(qt, { size: 11 }),
            " Regenerate"
          ] })
        ] }) : null,
        l.role === "assistant" && l.status === "error" ? /* @__PURE__ */ r("div", { className: "workbench-chat-message-actions", children: /* @__PURE__ */ r("button", { onClick: () => e.regenerateMessage(l), disabled: e.busy, children: "Retry" }) }) : null
      ] }, l.id)),
      i ? /* @__PURE__ */ r("button", { className: "workbench-chat-jump", onClick: m, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ h("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ h("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ r("strong", { children: Nr(e.contextScope) }),
        /* @__PURE__ */ r("em", { children: nn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ h("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ h("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ h("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ r(Ut, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((l) => {
            const f = typeof l.score == "number" ? l.score.toFixed(2) : null, g = typeof l.vectorScore == "number" ? l.vectorScore.toFixed(2) : null, b = typeof l.keywordScore == "number" ? l.keywordScore.toFixed(1) : null, M = [
              l.path,
            ...
            return /* @__PURE__ */ h("span", { className: "workbench-chat-chip", title: M, children: [
              /* @__PURE__ */ r("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(l), disabled: !l.fileId, title: "Reveal context", children: /* @__PURE__ */ r(Ut, { size: 13 }) }),
              l.label,
              f ? /* @__PURE__ */ r("small", { children: f }) : null,
        ...
        /* @__PURE__ */ h("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ r("span", { className: "workbench-chat-route-summary", children: nn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ r("span", {}),
          e.busy ? /* @__PURE__ */ r("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ r(or, { size: 14 }) }) : /* @__PURE__ */ r("button", { className: "workbench-chat-send", onClick: e.askAgent, title: "Send", children: /* @__PURE__ */ r(er, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function nn(e, t, n) {
  const a = e.model.trim(), o = [e.gatewayPreference === "auto" ? t : Dn(e.gatewayPreference)];
...
function So(e) {
  const [t, n] = x([]), [a, i] = x("Loading gateway models…"), [o, s] = x([]), [d, m] = x("Checking embedding capability…");
  X(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(P.map((T) => ({ id: T.id, gatewayLabel: T.gatewayLabel }))), i(P.length > 0 ? `${P.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (P) {
        if (b.signal.aborted) return;
        n([]), i(`Model discovery failed: ${P instanceof Error ? P.message : String(P)}`);
      }
    })(), () => b.abort();
    ...
    /* @__PURE__ */ h("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ h("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ h("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          Dn(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ h("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ h("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ r("datalist", { id: "atomek-embedding-models", children: o.map((b) => /* @__PURE__ */ r("option", { value: b.id, children: b.gatewayLabel }, `${b.gatewayLabel}:${b.id}`)) })
        ] }),
...
function Hn({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: i, openOutputAsFile: o, previewEditFromOutput: s, canPreviewEdit: d = !1, compact: m = !1 }) {
  return /* @__PURE__ */ h("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    /* @__PURE__ */ h("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      ...
      /* @__PURE__ */ h("button", { className: "workbench-button-subtle", onClick: i, children: [
        /* @__PURE__ */ r(ln, { size: 14 }),
        "Capture check"
      ] }),
      /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((l) => /* @__PURE__ */ h("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ h("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ r("strong", { children: l.title }),
        /* @__PURE__ */ r("span", { children: l.source === "ai" ? `AI · ${l.kind}` : l.kind }),
        /* @__PURE__ */ r("button", { onClick: () => o(l), children: "Open as file" }),
        s ? /* @__PURE__ */ r(
          "button",
          {
            className: an(l.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => s(l),
            disabled: !d,
            children: an(l.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        l.source === "ai" ? /* @__PURE__ */ r("button", { onClick: () => n(l.id), children: "Delete" }) : null
      ] }),
      l.body
    ] }, l.id))
  ] });
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ h("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((o) => /* @__PURE__ */ h("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ r("pre", { children: To(o.proposedContent) })
      ] }, o.fileId)),
...
function Lo({ status: e, file: t, cursor: n, fileCount: a, dirtyCount: i }) {
  return /* @__PURE__ */ h("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ r("span", { children: pn(t.language) })
  ] });
}
function wt(e, t) {
