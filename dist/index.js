 ...
 */
const Da = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Ba = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), cn = (e) => {
  const t = Ba(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, xn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), Ha = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Ka = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Ja = z("check", Ka);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Xa = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], rt = z("chevron-down", Xa);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
async function Pr() {
  ...
  if (!e) return Fn(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((a) => En(a, a.name, "local-file")));
  return Bt(n.filter(Boolean));
}
async function Tr() {
  const e = Dt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await Fn(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await On(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Bt(n) };
}
function Dt() {
  const e = window;
...
async function On(e, t, n, a, i) {
  if (!(n.length >= i || a > 8))
    for await (const c of e.values()) {
      if (n.length >= i) break;
      if (c.kind === "directory") {
        if (Ir.has(c.name)) continue;
        await On(c, `${t}/${c.name}`, n, a + 1, i);
        continue;
      }
      const o = c, l = await En(o, `${t}/${o.name}`, "local-folder");
      l && n.push(l);
    }
}
function Fn(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const a = Array.from(n.files ?? []), i = await Promise.all(a.map(async (c) => {
        const o = c.webkitRelativePath || c.name;
        return Ln(o, c.size) ? {
          id: zn(o),
          name: c.name,
          path: o,
...
`).map(
    ...
    const c = a.split("|").filter(Boolean).map((l) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${l.trim()}</th>`).join(""), o = i.trim().split(`
`).map((l) => `<tr>${l.split("|").filter(Boolean).map(
      (p, f) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${f % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${p.trim()}</td>`
    ).join("")}</tr>`).join("");
    return `<table style="border-collapse:collapse;margin:16px 0;width:100%;border:1px solid var(--border-default)"><thead><tr>${c}</tr></thead><tbody>${o}</tbody></table>`;
  }), t = t.replace(/^(?!<[a-z])(.+)$/gim, '<p style="line-height:1.6;margin:12px 0;color:var(--text-primary)">$1</p>'), t = t.replace(/\n+/g, `
`), t;
}
const Te = "atomek:default", Er = 3e3, Ht = "tytus.atomek.threadTitleOverrides", ln = {
  available: !1,
  source: "none",
...
}, Fr = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", hn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Fr(e.kind),
  body: e.body,
...
function jr({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const i = e.ai, [c, o] = x(null), [l, m] = x([]), [p, f] = x([]), [u, g] = x([]), [S, I] = x([]), [O, K] = x(ln), [le, W] = x(!1), G = Ee(!0), M = Ee(null), T = C(async () => {
    if (!i) {
      K(ln);
      return;
    }
    try {
      ...
      K({
        ...
        source: "none",
        label: "Tytus AI unavailable",
        reason: w instanceof Error ? w.message : String(w)
      });
    }
  }, [i]), V = C(async (w) => {
    if (i)
      try {
        const v = await i.listArtifacts({ threadId: w });
        if (!G.current) return;
        g(v.map(hn));
      } catch (v) {
        a(`AI artifacts unavailable: ${v instanceof Error ? v.message : String(v)}`);
      }
  }, [i, a]), we = C(async (w) => {
    if (i)
      try {
        const v = await i.listMessages(w);
        if (!G.current) return;
        const P = l.find((F) => F.id === w) ?? null;
        P && o(P), f(v.map(He).filter(Boolean)), I([]), await V(w);
      } catch (v) {
        a(`Load chat failed: ${v instanceof Error ? v.message : String(v)}`);
      }
  }, [i, V, a, l]), U = C(async () => {
    if (i)
      try {
        const w = un(await i.listThreads({ workspaceKey: Te, status: "active" })), v = w[0] ?? await i.createThread({ workspaceKey: Te, title: "Atomek chat" });
        if (!G.current) return;
        m(w[0] ? w : [v]), o(v);
        const P = await i.listMessages(v.id);
        if (!G.current) return;
        f(P.map(He).filter(Boolean)), await V(v.id);
      } catch (w) {
        a(`AI chat unavailable: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [i, V, a]);
  q(() => (G.current = !0, T(), U(), () => {
    G.current = !1;
  ...
  const ie = C(async () => {
    if (!i) return null;
    if (c) return c;
    const w = await i.createThread({ workspaceKey: Te, title: "Atomek chat" });
    return G.current && (o(w), m((v) => [w, ...v.filter((P) => P.id !== w.id)])), w;
  }, [i, c]), pe = C(async () => {
    if (!i) return;
    const w = await i.createThread({ workspaceKey: Te, title: "Atomek chat" });
    o(w), m((v) => [w, ...v.filter((P) => P.id !== w.id)]), f([]), g([]), I([]), a("New AI chat created");
  }, [i, a]), A = C(async (w, v) => {
    if (!i) return;
    const P = v.trim();
    if (P)
      try {
        const F = i.updateThread, X = l.find((te) => te.id === w) ?? c, j = typeof F == "function" ? await F({ threadId: w, title: P }) : { ...X ?? await ie(), id: w, title: P, updatedAt: Date.now() };
        typeof F != "function" && Rr(w, P), m((te) => te.map((ve) => ve.id === j.id ? j : ve)), o((te) => te?.id === j.id ? j : te), a(typeof F == "function" ? `Renamed chat: ${j.title}` : `Renamed chat locally: ${j.title}`);
      } catch (F) {
        a(`Rename chat failed: ${F instanceof Error ? F.message : String(F)}`);
      }
  }, [i, ie, a, c, l]), Y = C(async (w) => {
    if (i)
      try {
        await i.deleteThread(w), _r(w);
        const v = l.filter((P) => P.id !== w);
        if (m(v), c?.id === w) {
          const P = v[0] ?? await i.createThread({ workspaceKey: Te, title: "Atomek chat" });
          o(P), m((X) => X.some((j) => j.id === P.id) ? X : [P, ...X]);
          const F = await i.listMessages(P.id);
          f(F.map(He).filter(Boolean)), await V(P.id);
        }
        a("Deleted AI chat");
      } catch (v) {
        a(`Delete chat failed: ${v instanceof Error ? v.message : String(v)}`);
      }
  }, [i, V, a, c?.id, l]), k = C(async (w) => {
    if (!i) return null;
    try {
      const v = await ie();
      if (!v) return null;
      const P = await i.createArtifact({
        ...
        title: w.title?.trim() || dn(w.body, "Atomek artifact"),
        kind: w.kind ?? "markdown",
        body: w.body
      }), F = hn(P);
      return g((X) => [F, ...X.filter((j) => j.id !== F.id)]), a(`Saved AI artifact: ${F.title}`), F;
    } catch (v) {
      return a(`Save artifact failed: ${v instanceof Error ? v.message : String(v)}`), null;
    }
  }, [i, ie, a]), H = C(async (w) => {
    if (i)
      try {
        await i.deleteArtifact(w), g((v) => v.filter((P) => P.id !== w)), a("Deleted AI artifact");
      } catch (v) {
        a(`Delete artifact failed: ${v instanceof Error ? v.message : String(v)}`);
      }
  }, [i, a]), E = C(async (w) => {
    if (!i) return null;
    try {
      const v = await i.writeMemory({
        title: w.title?.trim() || dn(w.body, "Atomek memory"),
        body: w.body,
        metadata: {
          source: "atomek",
          messageId: w.messageId ?? null
      ...
      return I((P) => [v, ...P.filter((F) => F.id !== v.id)].slice(0, 5)), a(`Remembered: ${v.title}`), v;
    } catch (v) {
      return a(`Remember failed: ${v instanceof Error ? v.message : String(v)}`), null;
    }
  }, [i, a]), R = C(async (w) => {
    if (!i) return [];
    const v = await i.searchMemory({ query: w, limit: 5 });
    return G.current && I(v), v;
  }, [i]), J = C(async (w, v = {}) => {
    const P = w.trim();
    if (!P || !i) return null;
    W(!0);
    ...
    try {
      const j = await ie();
      if (!j) return null;
      const te = await R(P).catch(() => []), ve = zr(te), Oe = v.requestContext ?? t, Ue = ve ? [...Oe, ve] : [...Oe];
      let Ce = null;
      for await (const B of i.sendMessage({
        threadId: j.id,
        body: P,
        ...
        if (B.type === "message_created") {
          const D = He(B.message);
          if (!D) continue;
          D.role === "assistant" && (Ce = D.id), f((ne) => [...ne.filter((ae) => ae.id !== D.id), D]);
        }
        if (B.type === "token" && (Ce = B.messageId, f((D) => D.some((ne) => ne.id === B.messageId) ? D.map(
          (ne) => ne.id === B.messageId ? { ...ne, body: B.body, status: "streaming" } : ne
        ) : [...D, {
          id: B.messageId,
          role: "assistant",
          body: B.body,
          status: "streaming",
          createdAt: Date.now()
        }])), B.type === "message_updated" || B.type === "done") {
          const D = He(B.message);
          if (!D) continue;
          D.role === "assistant" && (F = D), f((ne) => ne.map((ae) => ae.id === D.id ? D : ae)), D.gatewayLabel && a(`AI answered via ${D.gatewayLabel}`);
        }
        if (B.type === "run_failed") {
          const D = X.signal.aborted;
          a(D ? "AI response stopped" : `AI failed: ${B.error}`), Ce && f((ne) => ne.map((ae) => ae.id !== Ce ? ae : D ? { ...ae, status: "complete", error: void 0, body: ae.body || "Stopped by user." } : { ...ae, status: "error", error: B.error, body: B.error }));
        }
      }
      const be = un(await i.listThreads({ workspaceKey: Te, status: "active" }).catch(() => []));
      return G.current && be.length > 0 && m(be), T(), F;
    } catch (j) {
      return j instanceof DOMException && j.name === "AbortError" ? (a("AI response stopped"), F) : (a(`AI failed: ${j instanceof Error ? j.message : String(j)}`), null);
    } finally {
      M.current === X && (M.current = null), W(!1);
    }
  }, [i, n.gatewayPreference, n.model, ie, R, T, t, a]), oe = C(() => {
...
const Dr = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Br = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Hr = ({ files: e, openEditorIds: t, activeFileId: n, versions: a, activeSelection: i }) => {
  const c = new Set(t), o = e.map((l) => {
    const m = l.id === n;
    return {
      id: l.id,
      uri: Dr(l),
  ...
  let i = [];
  return t.scope === "active-selection" ? a?.selection ? i = [Kr(a, a.selection, !0)] : a && (i = [Ze(a, !0, "Active file")]) : t.scope === "active-file" ? a && (i = [Ze(a, !0, "Active file")]) : t.scope === "open-editors" ? i = e.openDocumentIds.map((c) => e.byId.get(c)).filter(Boolean).map((c) => Ze(c, !0, "Open editor")) : t.scope === "selected-files" && (i = t.selectedFileIds.map((c) => e.byId.get(c)).filter(Boolean).map((c) => Ze(c, !1, "Selected file"))), Xr(i).filter((c) => !n.has(c.id));
}, Xr = (e) => {
...
[...clipped ${e.length - t} chars...]`, ei = (e, t, n) => {
  const a = Jr(e, n), i = [], c = new Map(t.map((o) => [o.id, o]));
  for (const o of a) {
    if (!o.fileId) continue;
    const l = c.get(o.fileId), m = e.byId.get(o.fileId);
    ...
    const p = [
      ...
      o.range ? `Range: ${mn(o.range)}` : null
    ].filter(Boolean).join(`
`), f = o.kind === "selection" && o.range ? ti(l.content, o.range) : l.content, u = o.kind === "selection" ? Gr : o.label.startsWith("Open editor") ? Qr : Yr;
    i.push({
...
`)
  ...
  const n = e.split(`
`), a = Math.max(1, t.startLineNumber), i = Math.max(a, t.endLineNumber), c = n.slice(a - 1, i);
  return c.length === 0 ? "" : c.length === 1 ? c[0].slice(Math.max(0, t.startColumn - 1), Math.max(0, t.endColumn - 1)) : (c[0] = c[0].slice(Math.max(0, t.startColumn - 1)), c[c.length - 1] = c[c.length - 1].slice(0, Math.max(0, t.endColumn - 1)), c.join(`
`));
}, mn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, pn = 75e4, ni = 2800, ai = 240, ri = 80, ii = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Ne(t.content)}`).sort().join("")
), Et = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? pn) || e.content.length > (t.maxFileBytes ?? pn) ? "huge" : !t.includeDirty && e.dirty ? null : ui(e.path) ? "vendor" : mi(e) ? "binary" : null;
}, fn = (e, t = {}) => {
  const n = Date.now(), a = [], i = [], c = [];
  for (const o of e) {
    const l = o.size ?? o.content.length, m = Et(o, t);
    if (m) {
      i.push({ fileId: o.id, name: o.name, path: o.path, language: o.language, reason: m, size: l });
      continue;
    }
    const p = Ne(o.content), f = ci(o, p, n, t);
    c.push(...f), a.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
  ...
  return si({
    ...
    byChunkId: new Map(c.map((o) => [o.id, o]))
  });
}, ci = (e, t = Ne(e.content), n = Date.now(), a = {}) => {
  const i = Math.max(500, a.maxChunkChars ?? ni), c = Math.min(Math.max(0, a.chunkOverlapChars ?? ai), Math.floor(i / 2)), o = Math.max(1, a.maxChunksPerFile ?? ri), l = di(e.content), m = [];
  let p = 0, f = 0;
  for (; p < e.content.length && m.length < o; ) {
    const u = li(e.content, p, Math.min(e.content.length, p + i)), g = e.content.slice(p, u).trim();
    if (g.length > 0) {
      const S = hi(l, p, u);
      m.push({
        id: `${e.id}:${t}:${f}:${S.startLineNumber}-${S.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
`, n);
  if (i >= a) return i + 1;
  const c = Math.max(e.lastIndexOf(". ", n), e.lastIndexOf("; ", n));
  if (c >= a) return c + 1;
...
` && t.push(n + 1);
  ...
  return 0;
}, ui = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => ii.has(t)), mi = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && oi.has(t) || e.content.includes("\0")) return !0;
...
class pi {
  ...
  staleReport(t) {
    return fi(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const Le = (e = [], t = {}) => new pi(e, t), fi = (e, t, n = {}) => {
  const a = new Map(t.map((l) => [l.id, l])), i = e.byFileId, c = [];
  for (const l of e.files) {
    const m = a.get(l.fileId);
    if (!m) {
      c.push({ fileId: l.fileId, path: l.path, status: "deleted", indexedHash: l.hash });
      continue;
    }
    if (Et(m, n)) {
      c.push({ fileId: l.fileId, path: m.path, status: "skipped-now", indexedHash: l.hash, currentHash: Ne(m.content) });
      continue;
    }
    const f = Ne(m.content);
    f !== l.hash ? c.push({ fileId: l.fileId, path: m.path, status: "changed", indexedHash: l.hash, currentHash: f }) : m.dirty !== l.dirty && c.push({ fileId: l.fileId, path: m.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: f });
  }
  for (const l of t)
    !i.has(l.id) && !Et(l, n) && c.push({ fileId: l.id, path: l.path, status: "new", currentHash: Ne(l.content) });
  const o = e.signature !== _n(t);
  return { stale: c.length > 0, signatureChanged: o, files: c };
}, bi = 8, gi = 12e3, jn = (e, t, n = {}, a) => {
  const i = wi(t);
  if (i.length === 0) return [];
  const c = new Set(a?.files.map((f) => f.fileId) ?? []), o = e.chunks.filter((f) => n.includeDirty !== !1 || !f.dirty).map((f) => vi(f, i)).filter((f) => f.score >= (n.minScore ?? 1)).sort((f, u) => u.score - f.score || f.chunk.path.localeCompare(u.chunk.path) || f.chunk.ordinal - u.chunk.ordinal), l = [], m = /* @__PURE__ */ new Set();
  let p = n.maxChars ?? gi;
  for (const f of o) {
    if (l.length >= (n.limit ?? bi) || p <= 0) break;
    const u = `${f.chunk.fileId}:${f.chunk.ordinal}`;
    if (m.has(u)) continue;
    m.add(u);
    const g = f.chunk.text.length > p ? `${f.chunk.text.slice(0, Math.max(0, p - 28))}
[...context clipped...]` : f.chunk.text;
    l.push(ki(f, g, c.has(f.chunk.fileId))), p -= g.length;
  }
  return l;
}, ki = (e, t = e.chunk.text, n = !1) => {
  const { chunk: a, score: i, matchedTerms: c } = e;
  return {
    id: `index-hit:${a.id}`,
    kind: "index-hit",
...
`), wi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), vi = (e, t) => {
  const n = e.text.toLowerCase(), a = e.path.toLowerCase(), i = e.name.toLowerCase(), c = e.language.toLowerCase();
  let o = 0;
  const l = [];
  for (const m of t) {
    const p = $t(n, m), f = $t(a, m), u = $t(i, m), g = c === m ? 1 : 0;
    p + f + u + g !== 0 && (l.push(m), o += Math.min(p, 12), o += f * 4, o += u * 6, o += g * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (o += 3));
  }
  ...
  const n = e.toLowerCase(), a = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, p) => m - p)[0] ?? 0, i = Math.max(0, a - 120), c = Math.min(e.length, a + 260), o = i > 0 ? "…" : "", l = c < e.length ? "…" : "";
  return `${o}${e.slice(i, c).replace(/\s+/g, " ").trim()}${l}`;
}, xi = (e, t = {}) => {
  const { autoRefresh: n = !1, ...a } = t, i = Ee(e), c = Ee(null);
  c.current || (c.current = Le(e, a));
  const [o, l] = x(() => c.current?.getSnapshot() ?? Le(e, a).getSnapshot());
  i.current = e;
  const m = C((g = i.current) => {
    const S = (c.current ?? Le([], a)).refresh(g, a);
    return l(S), S;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), p = C((g = i.current) => {
    const S = (c.current ?? Le([], a)).update(g, a);
    return l(S), S;
  }, [m]);
  q(() => {
    n && m(e);
  }, [n, e, m]);
  const f = (c.current ?? Le([], a)).staleReport(e), u = C((g, S = {}) => {
    const I = c.current ?? Le([], a);
    return jn(I.getSnapshot(), g, S, I.staleReport(i.current));
  }, []);
  return { snapshot: o, staleReport: f, isStale: f.stale, refresh: m, update: p, retrieve: u };
}, Ai = "tytus.atomek.semanticVector:v1", Dn = "__gateway_default__", Bn = (e) => e?.trim() || Dn, $i = (e) => {
  const t = Bn(e);
  ...
  if (!Wn()) return null;
  try {
    const a = Hn(e, t, n), i = window.localStorage.getItem(a);
    if (!i) return null;
    const c = JSON.parse(i);
    return c.chunkId !== n.id || c.contentHash !== n.hash || c.modelAlias !== t || !Array.isArray(c.vector) || c.vector.some((o) => typeof o != "number" || !Number.isFinite(o)) ? null : {
      key: a,
      appId: e,
      chunkId: n.id,
      contentHash: n.hash,
      ...
      gatewayLabel: typeof c.gatewayLabel == "string" ? c.gatewayLabel : "",
      dim: c.vector.length,
      vector: c.vector,
  ...
  if (!Wn()) return null;
  try {
    const t = Hn(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, We = 8, Ii = 12e3, Mi = 180, Pi = async (e, t, n, a, i = {}, c, o) => {
  const l = jn(t, n, { ...i, limit: Math.max(i.limit ?? We, 16) }, c), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: l.slice(0, i.limit ?? We), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const p = Bn(a.embeddingModel), f = await m.embedText({
      input: n,
    ...
    const g = new Map(l.map((M) => [M.chunkId, M])), S = Ri(n), I = /* @__PURE__ */ new Map();
    t.chunks.filter((M) => i.includeDirty !== !1 || !M.dirty).slice(0, Mi).forEach((M) => I.set(M.id, M)), l.forEach((M) => {
      const T = t.byChunkId.get(M.chunkId);
      T && I.set(T.id, T);
    });
    const O = Array.from(I.values()), K = [];
    let le = 0;
    for (const M of O) {
      o?.aborted;
      const T = Ni(e.appId, p, M) ?? await Li(m, e.appId, p, M, a, o);
      if (!T) continue;
      T.updatedAt > Date.now() - 1e3 && (le += 1);
      const V = Fi(u, T.vector);
      if (!Number.isFinite(V)) continue;
      const U = g.get(M.id)?.score ?? _i(M, S), ie = zi(U) * 0.42 + Math.max(0, V) * 0.58;
      K.push({ chunk: M, keywordScore: U, vectorScore: V, score: ie, matchedTerms: S.filter((pe) => M.text.toLowerCase().includes(pe)) });
    }
    if (K.length === 0) return { hits: l.slice(0, i.limit ?? We), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: le };
    const W = new Set(c?.files.map((M) => M.fileId) ?? []), G = K.sort((M, T) => T.score - M.score || T.keywordScore - M.keywordScore || M.chunk.path.localeCompare(T.chunk.path) || M.chunk.ordinal - T.chunk.ordinal).slice(0, Math.max(i.limit ?? We, 1)).map((M) => Ei(M, W.has(M.chunk.fileId)));
    return {
      hits: Oi(G, i.maxChars ?? Ii),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${$i(a.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: le
    };
  } catch (p) {
    return {
      hits: l.slice(0, i.limit ?? We),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: ji(e.chunk.text, e.matchedTerms),
    score: e.score,
  ...
  for (const i of e) {
    if (n <= 0) break;
    const c = i.text.length > n ? `${i.text.slice(0, Math.max(0, n - 28))}
[...context clipped...]` : i.text;
    a.push({ ...i, text: c, charCount: c.length }), n -= c.length;
  }
  return a;
}, Vn = (e) => {
  ...
  let a = 0, i = 0, c = 0;
  for (let o = 0; o < n; o += 1)
    a += e[o] * t[o], i += e[o] * e[o], c += t[o] * t[o];
  return i === 0 || c === 0 ? 0 : a / (Math.sqrt(i) * Math.sqrt(c));
}, zi = (e) => Math.min(1, Math.max(0, e / 24)), Ri = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), _i = (e, t) => {
  const n = `${e.path}
${e.name}
...
${e.text}`.toLowerCase();
  ...
  const n = e.toLowerCase(), a = t.map((o) => n.indexOf(o)).filter((o) => o >= 0).sort((o, l) => o - l)[0] ?? 0, i = Math.max(0, a - 120), c = Math.min(e.length, a + 260);
  return `${i > 0 ? "…" : ""}${e.slice(i, c).replace(/\s+/g, " ").trim()}${c < e.length ? "…" : ""}`;
}, Di = /```([^\n`]*)\n([\s\S]*?)```/g, Bi = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Vt(e) {
  return Array.from(e.matchAll(Di)).map((t) => Un(t[1] ?? "", qi(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Un(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), a = {}, i = [];
  for (const c of n) {
    const o = c.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? a[o[1].toLowerCase()] = Ji(o[2]) : i.push(c.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: i, attrs: a, content: t };
}
function de(e, t) {
  return (typeof e == "string" ? Un(e, "").flags : e.flags).includes(t.toLowerCase());
}
function qn(e) {
  return [...Vt(e).filter((a) => de(a, "diff") || de(a, "patch")).map((a) => a.content), e].filter((a, i, c) => Hi(a) && c.findIndex((o) => o === a) === i);
}
function Hi(e) {
...
`), a = [];
  if (n.forEach((o, l) => {
    (o.startsWith("diff --git ") || o.startsWith("--- ")) && a.push(l);
  }), a.length === 0) return [{ raw: t, ...Nt(t) }];
  const i = Array.from(new Set(a)).sort((o, l) => o - l), c = [];
  for (let o = 0; o < i.length; o += 1) {
    const l = i[o], p = i.find((u) => u > l && n[u].startsWith("diff --git ")) ?? n.length, f = n.slice(l, p).join(`
`);
    /^@@\s+-\d+/m.test(f) && c.push({ raw: f, ...Nt(f) });
  }
  return c.length > 0 ? c : [{ raw: t, ...Nt(t) }];
}
function Nt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), i = fe(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, c = fe(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, o = c && c !== "/dev/null" ? c : i;
  return { path: o && o !== "/dev/null" ? o : null, oldPath: i, newPath: c };
}
function fe(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function Ot(e) {
  return Vt(e).filter((t) => Vi(t)).map((t) => ({
    content: Ki(t.content),
...
function Vi(e) {
  return de(e, "atomek-replace") || de(e, "atomek-full") || de(e, "full-replacement") || de(e, "full") || de(e, "replace");
}
function Ui(e) {
...
`), i = [];
  ...
  for (let l = 0; l < a.length; l += 1) {
    const m = a[l].match(Bi);
    if (!m) continue;
    o = !0;
    const p = Number(m[1]), f = Math.max(0, p - 1);
    if (f < c) return null;
    for (i.push(...n.slice(c, f)), c = f, l += 1; l < a.length; l += 1) {
      const u = a[l];
      if (u.startsWith("@@ ")) {
        l -= 1;
        break;
  ...
  return o ? (i.push(...n.slice(c)), i.join(`
`)) : null;
}
function qi(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function Xi(e) {
  const { body: t, files: n, sourceTitle: a, activeFile: i = null, versions: c = {} } = e, o = /* @__PURE__ */ new Map(), l = [];
  for (const f of qn(t))
    for (const u of Wi(f)) {
      if (!u.path) {
        if (i) {
          const I = Ft(i.content, u.raw);
          I && I !== i.content && o.set(i.id, et({
            file: i,
            proposedContent: I,
            sourceTitle: a,
      ...
      const S = Ft(g.file.content, u.raw);
  ...
  for (const f of Ot(t)) {
    const u = f.path ? gn(n, f.path) : i ? { file: i, normalizedPatchPath: fe(i.path), confidence: "exact" } : null;
    if (!u) {
      l.push(`${f.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (f.content === u.file.content) {
    ...
    o.set(u.file.id, et({
      ...
      versions: c,
      targetPath: f.path ?? u.file.path,
      match: u
    }));
  }
  const m = Ot(t).some((f) => f.path);
  if (o.size === 0 && i && !m) {
    const f = Gi(t, i);
    f && f.content !== i.content && o.set(i.id, et({
      file: i,
      proposedContent: f.content,
      sourceTitle: a,
      extractionLabel: f.label,
  ...
  return {
    ...
    stats: Qi(p.map((f) => f.stats)),
    kind: p.length === 0 ? "empty" : p.length === 1 ? "single-file" : "multi-file"
  };
}
function Gi(e, t) {
  for (const m of qn(e)) {
    const p = Ft(t.content, m);
    if (p) return { content: p, label: "unified diff patch" };
  }
  const n = Vt(e);
  if (n.length === 0) return null;
  const a = eo(t), i = Ot(e)[0];
  if (i) return { content: i.content, label: i.label };
  const c = n.find((m) => a.some((p) => de(m, p)) && !de(m, "diff") && !de(m, "patch"));
  if (c) return { content: c.content, label: `matched ${c.lang || t.language} block` };
  const l = n.filter((m) => !de(m, "diff") && !de(m, "patch")).sort((m, p) => p.content.length - m.content.length)[0];
  return l ? { content: l.content, label: `largest fenced block (${l.lang || "plain"})` } : null;
}
function gn(e, t) {
  const n = fe(t), a = n.split("/").at(-1) ?? n, i = e.find((l) => fe(l.path) === n);
  if (i) return { file: i, normalizedPatchPath: n, confidence: "exact" };
  const c = e.find((l) => fe(l.path).endsWith(`/${n}`));
  if (c) return { file: c, normalizedPatchPath: n, confidence: "suffix" };
  const o = e.find((l) => l.name === a || fe(l.path).endsWith(`/${a}`));
...
`), i = Math.max(n.length, a.length);
  let c = 0, o = 0, l = 0;
  for (let m = 0; m < i; m += 1)
    n[m] !== a[m] && (n[m] === void 0 ? c += 1 : a[m] === void 0 ? o += 1 : l += 1);
  return { added: c, removed: o, changed: l };
}
function Qi(e) {
  return e.reduce((t, n) => ({
...
function et(e) {
  ...
  return {
    ...
    stats: Yi(e.file.content, e.proposedContent),
    base: {
...
const ot = (e) => typeof e == "object" && e !== null, to = (e) => ot(e.ai) ? e.ai : null, no = (e) => {
  if (!ot(e)) return null;
  const t = e.id;
...
}, ue = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(ue) : ot(e) ? Object.values(e).some(ue) : !1, ro = async (e, t) => {
  const n = to(e);
  if (typeof n?.listModels != "function") return [];
  const a = await n.listModels(t);
  return Array.isArray(a) ? a.map(ao).filter((i) => !!i) : [];
}, io = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : ue(e.capability) || ue(e.capabilities) || ue(e.modality) || ue(e.modalities) || ue(e.task) || ue(e.tasks) || ue(e.type) || ue(e.kind), Jn = (e) => typeof e == "object" && e !== null, oo = (e) => Jn(e.ai) ? e.ai : null, Xn = (e) => typeof oo(e)?.embedText == "function", co = async (e, t) => Xn(e) ? (await ro(e, t)).filter(io) : [], so = (e) => Jn(e.ai) ? Xn(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", lo = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], ho = [
...
function fo(e) {
  const t = e.results.length > 0 ? e.results.map((a, i) => [
    ...
    `Status: ${a.status}`,
    "Output:",
    xo(a.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function go(e) {
  ...
  for (const a of t) {
    const i = ko(a.content);
    if (!i?.scripts) continue;
    const c = Yn(a.path), o = yo(e, c, i);
    if (!o) continue;
    const l = Object.keys(i.scripts).filter((p) => typeof i.scripts?.[p] == "string"), m = wo(l);
    for (const p of m) {
      const f = `${o} run ${p}`;
      n.push({
        id: Qn(`${c}:${f}`),
...
function yo(e, t, n) {
  ...
  for (const [a, i] of ho)
    if (e.some((c) => Co(c.path) === a && Yn(c.path) === t)) return i;
  return null;
}
function wo(e) {
  const t = lo.filter((a) => e.includes(a)), n = e.filter((a) => !t.includes(a)).filter((a) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(a)).sort((a, i) => a.localeCompare(i));
  return [...t, ...n];
}
function vo(e) {
  const t = /* @__PURE__ */ new Set();
...
const Ao = _a(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), $o = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Zn = "tytus.workspace.recent", ea = "tytus.workspace.layout", ta = "tytus.atomek.chatAiSettings", at = {
...
function St(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - No), a = n < 1180, i = a ? 200 : 240, c = a ? 300 : 340, o = a ? 420 : 560, l = Math.max(i, Math.min(a ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), p = Math.floor(n * (a ? 0.34 : 0.36)), f = n - m - o, u = Math.max(c, Math.min(a ? 500 : 640, p, f));
  return { primaryMin: i, primaryMax: l, secondaryMin: c, secondaryMax: u };
}
function So(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let a = 0, i = 0, c;
  for (; (c = n.exec(e)) !== null; ) {
    if (c.index > a) {
      const l = e.slice(a, c.index);
      l.trim() && t.push({ type: "markdown", body: l, key: `md-${i}` });
    }
    t.push({
      type: "code",
      language: c[1]?.trim() || "text",
      body: c[2] ?? "",
      key: `code-${i}`
    }), a = c.index + c[0].length, i += 1;
  }
  const o = e.slice(a);
...
function Io({ host: e }) {
  const t = Ee(null), [n, a] = x(0), i = ee(() => ic(), []), [c, o] = x("explorer"), [l, m] = x(i.primaryVisible), [p, f] = x(i.primaryWidth), [u, g] = x("chat"), [S, I] = x(i.secondaryVisible), [O, K] = x(i.secondaryWidth), [le, W] = x(!1), [G, M] = x("problems"), [T, V] = x(i.markdownPreviewVisible), [we, U] = x(!1), [ie, pe] = x(null), [A, Y] = x([]), [k, H] = x([]), [E, R] = x(null), [J, oe] = x(""), [w, v] = x({ lineNumber: 1, column: 1 }), [P, F] = x(null), [X, j] = x({}), [te, ve] = x(Wr), [Oe, Ue] = x([]), [Ce, be] = x([]), [B, D] = x(null), [ne, ae] = x(""), [sa, st] = x(!1), [la, da] = x(""), [qt, qe] = x([]), [Q, Se] = x(null), [ge, Fe] = x(null), [Kt, Jt] = x(!1), [xe, ha] = x(() => oc()), [Xt, ze] = x(null), [Ie, lt] = x(null), [dt, ht] = x(""), [ut, mt] = x(""), [Ke, Je] = x(""), [Xe, Gt] = x("failed"), [ua, Yt] = x(null), [ma, $] = x("Ready"), [Re, pa] = x(() => rc()), ce = k.map((s) => A.find((h) => h.id === s)).filter(Boolean), N = E ? A.find((s) => s.id === E) ?? null : null, Qt = ee(() => Hr({ files: A, openEditorIds: k, activeFileId: E, versions: X, activeSelection: P }), [E, P, X, A, k]), Zt = ee(() => ({
    scope: te,
    removedAttachmentIds: Oe,
    selectedFileIds: []
  }), [te, Oe]), _e = ee(() => ei(Qt, A, Zt), [Zt, Qt, A]), ke = xi(A, { autoRefresh: !0, includeDirty: !0 }), en = ee(() => Ce.map((s) => ({
    id: s.id,
    kind: "index-hit",
    label: s.label,
    path: s.path,
    ...
    snippet: s.snippet
  })), [Ce]), fa = ee(() => [..._e.attachments, ...en], [_e.attachments, en]), L = jr({ host: e, requestContext: _e.parts, chatSettings: xe, setStatus: $ }), Ae = ee(
    () => [...L.artifacts, ...qt].sort((s, h) => h.createdAt - s.createdAt),
    [L.artifacts, qt]
  ), Ge = Kt && !N, pt = !N && !Ge && !we, re = ee(() => A.filter((s) => s.dirty), [A]), ba = ee(() => {
    const s = J.trim().toLowerCase();
    return s ? A.filter((h) => h.path.toLowerCase().includes(s)) : A;
  }, [A, J]);
  q(() => {
    j((s) => {
      const h = {};
      for (const b of A) h[b.id] = s[b.id] ?? 1;
      return h;
    });
  }, [A]), q(() => {
    Ue([]), be([]);
  ...
  const ga = C((s) => {
    if (s.kind === "index-hit") {
      be((h) => h.filter((b) => b.id !== s.id)), $(`Removed project context: ${s.label}`);
      return;
    }
    Ue((h) => h.includes(s.id) ? h : [...h, s.id]), $(`Removed chat context: ${s.label}`);
  }, []), Ye = C(() => {
    Jt(!0), R(null), U(!0), $("Atomek settings opened");
  }, []), tn = C(() => {
    Jt(!1), R((s) => s ?? k.at(-1) ?? null), k.length === 0 && U(!1), $("Atomek settings closed");
  }, [k]), Me = C((s) => {
    j((h) => ({ ...h, [s]: (h[s] ?? 1) + 1 }));
  }, []), ka = C((s) => {
    s.preventDefault(), s.currentTarget.setPointerCapture?.(s.pointerId);
    const h = St(n), b = s.clientX, y = O, _ = (Be) => {
      const xt = y + (b - Be.clientX);
      K(tt(xt, h.secondaryMin, h.secondaryMax));
    }, he = () => {
      window.removeEventListener("pointermove", _), window.removeEventListener("pointerup", he);
    };
    window.addEventListener("pointermove", _), window.addEventListener("pointerup", he);
  }, [O, n]), ya = C((s) => {
    s.preventDefault(), s.currentTarget.setPointerCapture?.(s.pointerId);
    const h = St(n), b = s.clientX, y = p, _ = (Be) => {
      const xt = y + (Be.clientX - b);
      f(tt(xt, h.primaryMin, h.primaryMax));
    }, he = () => {
      window.removeEventListener("pointermove", _), window.removeEventListener("pointerup", he);
    };
    window.addEventListener("pointermove", _), window.addEventListener("pointerup", he);
  }, [p, n]), Qe = C((s) => {
    const h = [s, ...Re.filter((b) => b.path !== s.path)].slice(0, 6);
    pa(h), localStorage.setItem(Zn, JSON.stringify(h));
  }, [Re]), Z = C((s, h) => {
    H((b) => b.includes(s.id) ? b : [...b, s.id]), R(s.id), U(!1), Yt(h ?? null), v({ lineNumber: h ?? 1, column: 1 });
  }, []), wa = C((s) => {
    if (!s.fileId) return;
    const h = A.find((b) => b.id === s.fileId);
    h && (Z(h, s.range?.startLineNumber ?? 1), $(`Revealed context: ${s.label}`));
  }, [A, Z]), je = C(async () => {
    if (It(re, "open new files"))
      try {
        const s = await Pr();
        if (s.length === 0) return;
        Y((h) => tc(h, s)), s.forEach((h) => Qe({ name: h.name, path: h.path, at: Date.now() })), Z(s[0]), $(`Opened ${s.length} local file${s.length === 1 ? "" : "s"}`);
      } catch (s) {
        s.name !== "AbortError" && $(`Open file failed: ${s.message}`);
      }
  }, [re, Z, Qe]), ft = C(async () => {
    if (It(re, "open another folder"))
      try {
        const s = await Tr();
        pe(s), Y(s.files), H([]), R(null), U(!1), Qe({ name: s.name, path: s.name, at: Date.now() }), $(`${s.handle ? "Opened local folder" : "Opened browser fallback folder"} ${s.name} (${s.files.length} text files indexed)`);
      } catch (s) {
        s.name !== "AbortError" && $(`Open folder failed: ${s.message}`);
      }
  }, [re, Qe]), va = C((s) => {
    if (!E) return;
    let h = !1;
    Y((b) => b.map((y) => y.id !== E || y.content === s ? y : (h = !0, { ...y, content: s, dirty: !0 }))), h && Me(E);
  }, [E, Me]), bt = C(async () => {
    if (N)
      try {
        const s = await At(N);
        Y((h) => h.map((b) => b.id === s.id ? s : b)), $(`Saved ${s.name}`);
      } catch (s) {
        $(`Save failed: ${s.message}`);
      }
  }, [N]), Ca = C(async (s) => {
    const h = A.find((y) => y.id === s);
    if (!h) return null;
    const b = await At(h);
    return Y((y) => y.map((_) => _.id === b.id ? b : _)), b;
  }, [A]), nn = C(async () => {
    const s = A.filter((h) => h.dirty);
    if (s.length === 0) {
      $("No dirty files to save");
      return;
    }
    try {
      const h = await Promise.all(s.map((y) => At(y))), b = new Map(h.map((y) => [y.id, y]));
      Y((y) => y.map((_) => b.get(_.id) ?? _)), ze(null), $(`Saved ${h.length} dirty file${h.length === 1 ? "" : "s"}`);
    } catch (h) {
      $(`Save all failed: ${h.message}`);
    }
  }, [A]), gt = C((s) => {
    const h = A.find((b) => b.id === s);
    if (h?.dirty && !window.confirm(`${h.name} has unsaved changes. Close without saving?`)) {
      $(`Close canceled — ${h.name} has unsaved changes`);
      return;
    }
    H((b) => {
      const y = b.filter((_) => _ !== s);
      return E === s && (R(y.at(-1) ?? null), F(null)), y;
    });
  }, [E, A]), xa = C(() => {
    It(re, "close all editors") && (H([]), R(null), Yt(null), U(!1), $("Closed all editors"));
  }, [re]), kt = C(() => {
    const s = A.filter((b) => b.name.startsWith("Untitled")).length + 1, h = {
...
`,
    ...
    return be(y), y.length === 0 ? ($(ke.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), h) : (b.mode !== "hybrid" && b.reason ? $(b.reason) : $(`${y.length} project context hit${y.length === 1 ? "" : "s"} · ${b.reason ?? "hybrid retrieval"}`), [
      ...h,
      ...Ti(y)
    ]);
  }, [_e.parts, te, xe, e, ke]), yt = C(() => {
    if (!N && ce.length === 0) {
      $("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const h = [
      `Create a polished Markdown artifact from ${N?.path ?? `${ce.length} open editors`}.`,
    ...
    I(!0), g("chat"), $("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const b = await ye(h), y = await L.askAgent(h, { requestContext: b });
      !y || y.status === "error" || L.createArtifact({
        messageId: y.id,
        title: `AI synthesis — ${N?.name ?? "open workspace"}`,
        kind: "markdown",
        body: y.body
      }).then(() => {
        I(!0), g("outputs"), W(!0), M("output");
      });
    })();
  }, [N, L, ye, ce.length]), wt = C((s) => {
    const h = A.find((b) => b.path === s.path || b.name === s.name);
    if (h) {
      Z(h), $(`Opened recent ${h.name}`);
      return;
    }
    $("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [A, Z]), se = C((s) => {
    I(!0), g("chat"), ae(""), (async () => {
      const h = await ye(s);
      await L.askAgent(s, { requestContext: h });
    })();
  }, [L, ye]), an = C(async (s) => {
    if (!e.skills?.get) {
      $("Tytus skill registry is not available in this host build");
      return;
    }
    try {
      const h = await e.skills.get(s.id), b = h.body.length > 4500 ? `${h.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : h.body, y = [
...
`);
      ae((_) => [_.trim(), y].filter(Boolean).join(`

`)), I(!0), g("chat"), $(`Attached skill ${h.title} to chat input`);
    } catch (h) {
      $(`Failed to attach skill: ${h instanceof Error ? h.message : String(h)}`);
    }
  }, [e.skills]), Aa = C((s) => {
    const h = L.messages.findIndex((y) => y.id === s.id), b = L.messages.slice(0, h < 0 ? void 0 : h).reverse().find((y) => y.role === "user");
    if (!b?.body.trim()) {
      $("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const y = await ye(b.body);
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    if (!N) {
      $("No active file to save as AI artifact");
      return;
    }
    L.createArtifact({
      title: N.path,
      kind: N.language === "markdown" ? "markdown" : "report",
      body: N.content
    }).then(() => {
      I(!0), g("outputs"), W(!0), M("output");
    });
  }, [N, L]), vt = C((s) => {
    const h = Pt(A, Mt(s.title || s.kind)), b = {
      id: `artifact-file-${s.id}-${Date.now()}`,
      name: h,
      path: h,
      language: "markdown",
      content: s.body,
    ...
    Y((y) => [...y, b]), Z(b), $(`Opened ${s.title} as editable file`);
  }, [A, Z]), Ct = C(() => {
    const s = window.prompt("Check command/name (manual capture only; Atomek will not execute it)");
    if (s === null) return;
    const h = s.trim();
    if (!h) {
      $("Manual check capture canceled — command/name was empty");
      return;
    }
...
`), _ = {
    ...
    qe((he) => [_, ...he]), W(!0), M("output"), $(`Captured manual check: ${h}`);
  }, []), Pe = C((s, h) => {
    const b = Xi({
      body: h,
      files: A,
      ...
      edits: b.edits.map(nc),
      skipped: b.skipped
    };
    return b.edits.length > 1 ? (Fe(y), Se(null), $(`Previewing AI workspace patch for ${b.edits.length} files`), !0) : b.edits.length === 1 ? (Se(y.edits[0]), Fe(null), $(`Previewing AI patch for ${y.edits[0].fileName}`), !0) : ($(b.skipped.length > 0 ? `No applicable edit found. ${b.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [N, X, A]), rn = C((s, h) => {
    const b = {
    ...
    qe((y) => [b, ...y]), I(!0), g("outputs"), W(!0), M("output"), $(`Saved local job output: ${s}`), (h.includes("```diff") || h.includes("--- a/")) && Pe(s, h);
  }, [Pe]), Ia = C(() => {
    const s = ne.trim();
    if (!s) return;
    ae("");
    const h = Uo(s);
    (async () => {
      const b = await ye(s), y = await L.askAgent(s, { requestContext: b });
      if (!h || !y || y.status === "error") return;
      Pe(y.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", y.body) || $("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [L, ye, ne, Pe]), Ma = C(() => {
    B && (D(null), se(qo(B)));
  }, [se, B]), De = C((s) => {
    const h = uo(A, s);
    lt(h);
    const b = h.commands[0]?.command ?? "";
    Je(b), ht(""), mt(""), Gt("failed"), W(!0), M("terminal"), $(h.commands.length > 0 ? `Manual check ready: copy ${h.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [A]), Pa = C((s) => {
    s.trim() && (ct(s), Je(s), $(`Manual check command copied: ${s}`));
  }, []), Ta = C(() => {
    lt((s) => {
      if (!s) return s;
      const h = mo(s, dt), b = h.commands.at(-1)?.command ?? "";
      return b && Je(b), h;
    }), ht("");
  }, [dt]), La = C(() => {
    if (!Ie || !Ke.trim()) {
      $("Select or enter a manual check command before capturing output");
      return;
    }
    const s = Ke.trim();
    lt((h) => h && po(h, s, Xe, ut)), mt(""), $(`Captured manual check result: ${s} (${Xe})`);
  }, [ut, Ke, Ie, Xe]), Ea = C(() => {
    Ie && se(fo(Ie));
  }, [se, Ie]), Oa = C(() => {
    if (!Q) return;
    const s = A.find((h) => h.id === Q.fileId);
    if (!s) {
      $(`Cannot apply edit — ${Q.fileName} is no longer open`), Se(null);
      return;
    }
    s.content !== Q.originalContent && !window.confirm(`${Q.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (Y((h) => h.map((b) => b.id === Q.fileId ? { ...b, content: Q.proposedContent, dirty: !0 } : b)), Me(Q.fileId), ze(`AI edit applied to ${s.name}. Save All to persist it to disk.`), Se(null), De(`AI edit applied to ${s.name}`), $(`Applied AI edit to ${s.name} — unsaved; manual check ready`));
  }, [Me, A, Q, De]), Fa = C(() => {
    if (!Q) return;
    const s = Pt(A, `${Mt(Q.fileName)}-ai-edit`), h = {
      id: `pending-edit-${Q.fileId}-${Date.now()}`,
      name: s,
      path: s,
      language: N?.language ?? "markdown",
      content: Q.proposedContent,
      dirty: !0,
      source: "generated"
    };
    Y((b) => [...b, h]), Z(h), Se(null), $(`Opened proposed edit as ${s}`);
  }, [N?.language, A, Z, Q]), za = C(() => {
    if (!ge) return;
    const s = new Map(ge.edits.map((b) => [b.fileId, b]));
    A.some((b) => {
      const y = s.get(b.id);
      return y && b.content !== y.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (Y((b) => b.map((y) => {
      const _ = s.get(y.id);
      return _ ? { ...y, content: _.proposedContent, dirty: !0 } : y;
    })), s.forEach((b, y) => Me(y)), ze(`AI workspace patch applied to ${s.size} file${s.size === 1 ? "" : "s"}. Save All to persist changes.`), Fe(null), De(`AI workspace patch applied to ${s.size} file${s.size === 1 ? "" : "s"}`), $(`Applied AI workspace patch to ${s.size} file${s.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Me, A, ge, De]), Ra = C(() => {
    if (!ge) return;
    const s = [...A], h = ge.edits.map((b) => {
      const y = Pt(s, `${Mt(b.fileName)}-ai-edit`), _ = A.find((Be) => Be.id === b.fileId), he = {
        id: `workspace-patch-${b.fileId}-${Date.now()}-${y}`,
        name: y,
        path: y,
        language: _?.language ?? "markdown",
    ...
    Y((b) => [...b, ...h]), h[0] && Z(h[0]), Fe(null), $(`Opened ${h.length} proposed edit file${h.length === 1 ? "" : "s"}`);
  }, [A, Z, ge]), on = C((s) => {
    if (s === "explain") {
  ...
  return q(() => {
    const s = (h) => {
      (h.metaKey || h.ctrlKey) && (h.key.toLowerCase() === "o" && (h.preventDefault(), je()), h.key.toLowerCase() === "s" && (h.preventDefault(), bt()), h.key.toLowerCase() === "w" && (h.preventDefault(), E && gt(E)), h.key.toLowerCase() === "b" && (h.preventDefault(), m((y) => !y)), h.shiftKey && h.key.toLowerCase() === "f" && (h.preventDefault(), o("search"), m(!0)), (h.key.toLowerCase() === "k" || h.key.toLowerCase() === "p") && (h.preventDefault(), st(!0)));
    };
    return window.addEventListener("keydown", s), () => window.removeEventListener("keydown", s);
  }, [E, gt, je, bt]), q(() => {
    const s = (h) => {
      re.length !== 0 && (h.preventDefault(), h.returnValue = "");
    };
    ...
    const s = t.current;
    if (!s) return;
    const h = () => a(Math.round(s.getBoundingClientRect().width));
    h();
    ...
    if (!n) return;
    const s = St(n);
    f((h) => tt(h, s.primaryMin, s.primaryMax)), K((h) => tt(h, s.secondaryMin, s.secondaryMax));
  }, [n]), q(() => {
    const s = { primaryVisible: l, primaryWidth: p, secondaryVisible: S, secondaryWidth: O, markdownPreviewVisible: T };
    localStorage.setItem(ea, JSON.stringify(s));
  }, [T, l, p, S, O]), q(() => {
    localStorage.setItem(ta, JSON.stringify(xe));
  }, [xe]), /* @__PURE__ */ d(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${l ? "" : "no-primary"} ${S ? "" : "no-secondary"} ${le ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${p}px`, "--workbench-secondary-width": `${O}px` },
      children: [
        /* @__PURE__ */ r(Mo, { active: c, setActive: (s) => {
          o(s), m(!0);
        }, openSettings: Ye, settingsActive: Ge }),
        l && /* @__PURE__ */ d("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ r("div", { className: "workbench-primary-resizer", onPointerDown: ya, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ r("button", { className: "workbench-command-center", onClick: () => st(!0), children: "Workspace" }),
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ r(
              Oo,
              {
                ...
                closeSettings: tn,
                secondaryVisible: S,
                toggleSecondary: () => I((s) => !s),
                canPreview: N?.language === "markdown",
                previewVisible: T,
                togglePreview: () => V((s) => !s)
              }
            ),
            /* @__PURE__ */ r(Eo, { file: N, folder: ie, showWelcome: pt }),
            Xt && re.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ r("span", { children: Xt }),
              /* @__PURE__ */ r("button", { onClick: () => {
                nn();
              }, children: "Save all" }),
              /* @__PURE__ */ r("button", { onClick: () => ze(null), title: "Dismiss", children: /* @__PURE__ */ r(me, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: N ? /* @__PURE__ */ d("div", { className: N.language === "markdown" && T ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ r("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ r(ja, { fallback: /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ r(
                Ao,
                {
                  file: N,
                  revealLine: ua,
              ...
              N.language === "markdown" && T && /* @__PURE__ */ r(jo, { content: N.content })
            ] }) : Ge ? /* @__PURE__ */ r(
              Ko,
              {
                host: e,
            ...
            ) : pt ? /* @__PURE__ */ r(Fo, { openFile: je, openFolder: ft, newFile: kt, recent: Re, reopenRecent: wt, setStatus: $ }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ r(it, { size: 34 }),
              /* @__PURE__ */ r("p", { children: "No editor open" }),
              /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => U(!1), children: "Show Welcome" })
            ] }) }),
            le && /* @__PURE__ */ r(
              Bo,
              {
                ...
                setManualCheckSelectedCommand: Je,
                manualCheckStatus: Xe,
                setManualCheckStatus: Gt,
                copyManualCheckCommand: Pa,
                addManualCheckCommand: Ta,
                recordManualCheckResult: La,
                askAgentFromManualChecks: Ea,
                onClose: () => W(!1)
              }
            )
          ] })
        ...
        S && /* @__PURE__ */ r(
          Ho,
          {
            ...
            saveMessageAsArtifact: $a,
            rememberMessage: Na,
            previewEditFromMessage: (s) => Pe(s.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", s.body),
            runQuickPrompt: on,
            pendingPatchPrompt: B,
            generatePatchPrompt: Ma,
            ...
            captureManualCheck: Ct,
            openOutputAsFile: vt,
            previewEditFromOutput: (s) => Pe(s.title, s.body),
            ...
            contextScope: te,
            setContextScope: ve,
            contextAttachments: fa,
            removeContextAttachment: ga,
            revealContextAttachment: wa,
            projectIndexSummary: `${ke.snapshot.files.length} files · ${ke.snapshot.chunks.length} chunks`,
            projectIndexStale: ke.isStale,
            refreshProjectIndex: () => {
              const s = ke.refresh(A);
              $(`Project index refreshed: ${s.files.length} files · ${s.chunks.length} chunks`);
            },
            onResizeStart: ka,
            onClose: () => I(!1)
          }
        ...
        sa && /* @__PURE__ */ r(
          _o,
          {
            ...
            commands: [
              ...
              ...Re.map((s) => ({ label: `File: Open Recent — ${s.name}`, detail: s.path, run: () => wt(s) })),
              { label: "File: Save All", detail: `${re.length} dirty file${re.length === 1 ? "" : "s"}`, run: () => {
                nn();
              }, disabled: re.length === 0 },
              { label: "File: Close All Editors", detail: `${ce.length} open editor${ce.length === 1 ? "" : "s"}`, run: xa, disabled: ce.length === 0 },
              ...
              { label: "View: Toggle Primary Side Bar", detail: l ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => m((s) => !s) },
              { label: "View: Toggle Chat Panel", detail: S ? "Hide right AI side bar" : "Show right AI side bar", run: () => I((s) => !s) },
              { label: "View: Toggle Bottom Panel", detail: le ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => W((s) => !s) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Ye },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => De("Manual check requested from command palette") },
              { label: "View: Toggle Markdown Preview", detail: N?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => V((s) => !s), disabled: N?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: N || ce.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: yt, disabled: !N && ce.length === 0 },
              { label: "AI: Explain Active File", detail: N ? `Ask Cortex to explain ${N.path}` : "Open a file first", run: () => se("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !N },
              { label: "AI: Improve Active File", detail: N ? `Ask Cortex for concrete edits to ${N.path}` : "Open a file first", run: () => se("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !N },
              { label: "AI: Draft Editable Replacement", detail: N ? `Ask Cortex for a full-file replacement for ${N.path}` : "Open a file first", run: () => on("edit"), disabled: !N },
              ...
              { label: "AI: Save Active File as Artifact", detail: N ? "Persist active file in host.ai artifacts" : "Open a file first", run: Sa, disabled: !N },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Ct },
              { label: "AI: Open Latest Artifact as File", detail: Ae[0] ? `Create editable file from ${Ae[0].title}` : "No outputs yet", run: () => Ae[0] && vt(Ae[0]), disabled: Ae.length === 0 }
            ],
            openWorkbenchFile: Z,
            onClose: () => st(!1)
          }
        ...
        /* @__PURE__ */ r(ec, { status: ma, file: N ?? $o, cursor: w, fileCount: A.length, dirtyCount: re.length })
      ]
    }
  );
}
function Mo({ active: e, setActive: t, openSettings: n, settingsActive: a }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(Nn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(kr, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(In, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(An, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(zt, { size: 25 }), label: "Computer / Agents", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ r("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(Ya, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ r($e, { icon: /* @__PURE__ */ r(Cr, { size: 23 }), label: "Settings", active: a, onClick: n })
  ] });
}
function $e({ icon: e, label: t, active: n, onClick: a }) {
  return /* @__PURE__ */ r("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: a, children: e });
}
function Po(e) {
  return e.activity === "search" ? /* @__PURE__ */ r(zo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ r(vn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ r(vn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ r(ca, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ r(To, { ...e });
}
function To(e) {
  const t = !e.folder, n = ee(() => Lo(e.files, e.folder?.name), [e.files, e.folder?.name]), [a, i] = x(() => /* @__PURE__ */ new Set());
  q(() => {
    const o = na(n);
    i((l) => {
      const m = new Set(l);
      return o.forEach((p) => m.add(p)), m;
    });
  }, [n]);
  const c = C((o) => {
    i((l) => {
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(Ve, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((o) => /* @__PURE__ */ r(Ut, { file: o, active: o.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(o), label: o.name, detail: o.path }, o.id)),
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(rt, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No readable text files found." }) : aa(n, e.activeFileId, e.openWorkbenchFile, a, c)
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((o) => /* @__PURE__ */ d("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(o), children: [
        /* @__PURE__ */ r(Sn, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: o.name })
      ] }, `${o.path}-${o.at}`))
    ] })
  ] });
}
function Ut({ file: e, active: t, onOpen: n, basePath: a, depth: i = 0, label: c, detail: o }) {
  ...
  return /* @__PURE__ */ d("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": m }, onClick: n, title: e.path, children: [
    ...
    /* @__PURE__ */ d("span", { className: "workbench-row-text", children: [
      /* @__PURE__ */ r("span", { className: "workbench-row-name", children: c ?? l }),
...
function Lo(e, t) {
  const n = [], a = /* @__PURE__ */ new Map(), i = (o, l, m = n) => {
    const p = a.get(o);
    if (p) return p;
    const f = { name: l, path: o, children: [] };
    return a.set(o, f), m.push(f), f;
  };
  e.forEach((o) => {
    const l = t && o.path.startsWith(`${t}/`) ? o.path.slice(t.length + 1) : o.path, m = l.split("/").filter(Boolean);
    let p = n, f = "";
    m.slice(0, -1).forEach((u) => {
      f = f ? `${f}/${u}` : u, p = i(f, u, p).children;
    }), p.push({ name: m.at(-1) ?? o.name, path: l, file: o, children: [] });
  });
  const c = (o) => o.sort((l, m) => +!!l.file - +!!m.file || l.name.localeCompare(m.name)).map((l) => ({ ...l, children: c(l.children) }));
  return c(n);
}
function na(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...na(t.children)]);
}
function aa(e, t, n, a, i, c = 0) {
  return e.map((o) => {
    if (o.file)
      return /* @__PURE__ */ r(Ut, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: c, label: o.name }, o.file.id);
    const l = a.has(o.path);
    return /* @__PURE__ */ d("div", { children: [
      /* @__PURE__ */ d("button", { className: "workbench-folder-row", style: { "--workbench-depth": c }, onClick: () => i(o.path), title: l ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        l ? /* @__PURE__ */ r(rt, { size: 12 }) : /* @__PURE__ */ r(rt, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ r(jt, { size: 14 }) : /* @__PURE__ */ r(Sn, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: o.name }),
        /* @__PURE__ */ r("span", { className: "workbench-row-meta", children: o.children.length })
      ] }),
      l ? aa(o.children, t, n, a, i, c + 1) : null
    ] }, o.path);
  });
}
function Eo({ file: e, folder: t, showWelcome: n }) {
  const a = n ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], i = t && a[0] === t.name ? a.slice(1) : a;
  return /* @__PURE__ */ r("div", { className: "workbench-breadcrumb", children: i.length === 0 ? /* @__PURE__ */ r("span", { children: "Workspace" }) : i.map((c, o) => /* @__PURE__ */ d("span", { className: "workbench-breadcrumb-part", children: [
...
function Oo(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ d("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ r(me, { size: 13 }) })
    ] }),
...
function Fo({ openFile: e, openFolder: t, newFile: n, recent: a, reopenRecent: i, setStatus: c }) {
  return /* @__PURE__ */ d("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ d("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ d("section", { children: [
        ...
        a.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : a.map((o) => /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: () => i(o), children: [
          o.name,
...
function zo({ files: e, query: t, setQuery: n, openWorkbenchFile: a, activeFileId: i }) {
  const c = ee(() => Ro(e, t), [e, t]), o = ee(() => {
    const l = /* @__PURE__ */ new Map();
    return c.forEach((m) => l.set(m.file.id, [...l.get(m.file.id) ?? [], m])), Array.from(l.values()).slice(0, 50);
  }, [c]);
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? o.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No matches." }) : o.map((l) => {
        const m = l[0].file;
        return /* @__PURE__ */ d("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ r(Ut, { file: m, active: m.id === i, onOpen: () => a(m) }),
          l.slice(0, 5).map((p) => /* @__PURE__ */ d("button", { className: "workbench-search-hit", onClick: () => a(m, p.lineNumber), title: p.line, children: [
            /* @__PURE__ */ r("span", { className: "workbench-search-line", children: p.lineNumber }),
            /* @__PURE__ */ r("span", { children: p.line })
          ] }, `${m.id}-${p.lineNumber}-${p.line}`)),
...
`).some((c, o) => c.toLowerCase().includes(n) ? (a.push({ file: i, lineNumber: o + 1, line: c.trim() || "(blank line)" }), a.length >= 200) : !1);
  }), a.slice(0, 200);
}
function _o(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${Tn(i.language)} file`,
...
function Do(e) {
  const t = e.session ? bo(e.session) : "pending";
...
function Bo(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ d("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ r("span", {}),
      /* @__PURE__ */ r("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ r(me, { size: 14 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ r(oa, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Ho(e) {
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        ...
        /* @__PURE__ */ r("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ r(er, { size: 16 }) }),
        /* @__PURE__ */ r("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ r(me, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ r(Wo, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ r(
      ca,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ r(oa, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function ra({ body: e }) {
  const t = ee(() => So(e), [e]), [n, a] = x(null), i = C((c, o) => {
    (async () => await ct(o) && (a(c), window.setTimeout(() => a((m) => m === c ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ r("div", { className: "workbench-rich-body", children: t.map((c) => {
    if (c.type === "code") {
      const o = n === c.key;
      return /* @__PURE__ */ d("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ d("div", { className: "workbench-rich-code-head", children: [
          /* @__PURE__ */ r("span", { children: c.language }),
          /* @__PURE__ */ d("button", { onClick: () => i(c.key, c.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ r(Ja, { size: 12 }) : /* @__PURE__ */ r(Lt, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
        /* @__PURE__ */ r("pre", { children: /* @__PURE__ */ r("code", { children: c.body }) })
      ] }, c.key);
    }
    return /* @__PURE__ */ r(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Rn(c.body) }
      },
      c.key
    );
  }) });
...
function Wo(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Ee(null), [a, i] = x(!0), [c, o] = x(!1), l = ee(() => e.chatMessages.map((u) => `${u.id}:${u.status ?? ""}:${u.body.length}`).join("|"), [e.chatMessages]);
  q(() => {
    const u = n.current;
    u && (a ? (u.scrollTop = u.scrollHeight, o(!1)) : o(!0));
  }, [a, l]);
  const m = C(() => {
    const u = n.current;
    if (!u) return;
    ...
    const u = n.current;
    u && (u.scrollTop = u.scrollHeight, i(!0), o(!1));
  }, []), f = C((u) => {
    ct(u.body);
  }, []);
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
      ...
      /* @__PURE__ */ d(
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((u) => /* @__PURE__ */ d("option", { value: u.id, children: [
              u.title,
              " · ",
              Vo(u.lastMessageAt ?? u.updatedAt)
            ] }, u.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ d("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ r("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ d("div", { children: [
      ...
      ] }) }) : e.chatMessages.map((u) => /* @__PURE__ */ d("div", { className: `workbench-chat-message ${u.role}`, children: [
        ...
        /* @__PURE__ */ r("br", {}),
        /* @__PURE__ */ r(ra, { body: u.body }),
        u.gatewayLabel ? /* @__PURE__ */ d(Ve, { children: [
          /* @__PURE__ */ r("br", {}),
          /* @__PURE__ */ r("small", { children: u.gatewayLabel })
        ] }) : null,
        u.role === "assistant" && u.status !== "streaming" && u.status !== "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => f(u), title: "Copy this answer", children: [
            /* @__PURE__ */ r(Lt, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => e.saveMessageAsArtifact(u), title: "Save this answer as an output artifact", children: [
            /* @__PURE__ */ r(_t, { size: 12 }),
            " Save"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => e.rememberMessage(u), title: "Store this answer in Atomek memory", children: [
            /* @__PURE__ */ r(In, { size: 12 }),
            " Remember"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => e.previewEditFromMessage(u), disabled: e.workspaceFileCount === 0, title: "Preview an editable patch from this answer", children: [
            /* @__PURE__ */ r(Rt, { size: 12 }),
            " Preview"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(u), disabled: e.busy, title: "Regenerate this answer", children: [
            /* @__PURE__ */ r(nt, { size: 12 }),
            " Regenerate"
          ] })
        ] }) : null,
        u.role === "assistant" && u.status === "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => f(u), title: "Copy this error", children: [
            /* @__PURE__ */ r(Lt, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(u), disabled: e.busy, children: [
            /* @__PURE__ */ r(nt, { size: 12 }),
            " Retry"
          ] })
        ] }) : null
      ] }, u.id)),
      c ? /* @__PURE__ */ r("button", { className: "workbench-chat-jump", onClick: p, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ d("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ r("em", { children: kn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            ...
            const g = typeof u.score == "number" ? u.score.toFixed(2) : null, S = typeof u.vectorScore == "number" ? u.vectorScore.toFixed(2) : null, I = typeof u.keywordScore == "number" ? u.keywordScore.toFixed(1) : null, O = [
              u.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: O, children: [
              /* @__PURE__ */ r("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(u), disabled: !u.fileId, title: "Reveal context", children: /* @__PURE__ */ r(sn, { size: 13 }) }),
              u.label,
              g ? /* @__PURE__ */ r("small", { children: g }) : null,
              u.snippet ? /* @__PURE__ */ d("small", { children: [
                u.snippet.slice(0, 60),
                u.snippet.length > 60 ? "…" : ""
              ] }) : null,
              u.dirty ? /* @__PURE__ */ r("small", { children: "dirty" }) : null,
              u.removable ? /* @__PURE__ */ r("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(u), title: "Remove context", children: /* @__PURE__ */ r(me, { size: 11 }) }) : null
            ] }, u.id);
          }),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ r("span", { className: "workbench-chat-route-summary", children: kn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ r("span", {}),
...
function kn(e, t, n) {
  const a = e.model.trim(), c = [e.gatewayPreference === "auto" ? t : ia(e.gatewayPreference)];
  return a && c.push(a), n > 0 && c.push(`${n} memories`), c.join(" · ");
}
function Vo(e) {
...
function Ko(e) {
  const [t, n] = x([]), [a, i] = x("Loading gateway models…"), [c, o] = x([]), [l, m] = x("Checking embedding capability…");
  q(() => {
    ...
    return (async () => {
      ...
      try {
        const I = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: g.signal
        });
        if (g.signal.aborted) return;
        n(I.map((O) => ({ id: O.id, gatewayLabel: O.gatewayLabel }))), i(I.length > 0 ? `${I.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (I) {
        if (g.signal.aborted) return;
        n([]), i(`Model discovery failed: ${I instanceof Error ? I.message : String(I)}`);
      }
    })(), () => g.abort();
    ...
    return (async () => {
      ...
      try {
        const O = await co(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: g.signal
        });
        if (g.signal.aborted) return;
        o(O.map((K) => ({ id: K.id, gatewayLabel: K.gatewayLabel ?? K.source ?? "AIL" }))), m(O.length > 0 ? `${O.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (O) {
        if (g.signal.aborted) return;
        o([]), m(`Embedding model discovery failed: ${O instanceof Error ? O.message : String(O)}`);
      }
    })(), () => g.abort();
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ d("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ d("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ r("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ r(me, { size: 15 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          ia(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ r("datalist", { id: "atomek-embedding-models", children: c.map((g) => /* @__PURE__ */ r("option", { value: g.id, children: g.gatewayLabel }, `${g.gatewayLabel}:${g.id}`)) })
        ] }),
...
function oa({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: i, openOutputAsFile: c, previewEditFromOutput: o, canPreviewEdit: l = !1, compact: m = !1 }) {
  return /* @__PURE__ */ d("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((p) => /* @__PURE__ */ d("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ d("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ r("button", { onClick: () => void ct(p.body), children: "Copy" }),
        /* @__PURE__ */ r("button", { onClick: () => c(p), children: "Open as file" }),
        o ? /* @__PURE__ */ r(
          "button",
          {
            className: yn(p.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(p),
            disabled: !l,
            children: yn(p.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        p.source === "ai" ? /* @__PURE__ */ r("button", { onClick: () => n(p.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ r(ra, { body: p.body })
    ] }, p.id))
  ] });
}
function Jo({ edit: e, onApply: t, onOpenAsFile: n, onClose: a }) {
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ d("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ r("button", { title: "Close", onClick: a, children: /* @__PURE__ */ r(me, { size: 16 }) })
    ] }),
...
function Xo({ patch: e, onApply: t, onOpenAsFiles: n, onClose: a }) {
  const i = e.edits.reduce((c, o) => ({
    added: c.added + o.stats.added,
    removed: c.removed + o.stats.removed,
    changed: c.changed + o.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((c) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        /* @__PURE__ */ d("header", { children: [
          /* @__PURE__ */ r("strong", { children: c.fileName }),
          /* @__PURE__ */ d("span", { children: [
            c.extractionLabel,
            " · +",
            c.stats.added,
        ...
        /* @__PURE__ */ r("pre", { children: ac(c.proposedContent) })
      ] }, c.fileId)),
...
function Yo(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, Go(e.content), "```");
  else if (t.length > 0) {
...
function ca({
  ...
  const [l, m] = x([]), [p, f] = x([]), [u, g] = x(!1), [S, I] = x(null), [O, K] = x("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [le, W] = x(null), [G, M] = x([]), T = G.find((k) => k.status === "running") ?? G[0] ?? null, V = o === "dock", we = c.filter((k) => k.dirty).length, U = i ? `${i.path} · ${i.language} · ${i.content.length.toLocaleString()} chars${i.dirty ? " · dirty" : ""}` : c.length > 0 ? `${c.length} open editor${c.length === 1 ? "" : "s"}${we ? ` · ${we} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", ie = ee(() => [
    {
    ...
    if (!e.local?.listTools && !e.skills?.list) {
      m([]), f([]), I("This Tytus host build does not expose local tools or skill registry yet.");
      return;
    }
    g(!0), I(null);
    try {
      ...
      m(k), f(H), t(`Computer loaded · ${k.length} tools · ${H.length} skills`);
    } catch (k) {
  ...
  const A = C(async (k) => {
    ...
    try {
      await e.local.openTerminal({
        ...
        command: k.command,
        cwd: wn(i),
        prompt: `Opened from Atomek. ${U}`
      }), t(`Opened ${k.label} interactively in Tytus Terminal`);
    } catch (H) {
      t(`Terminal launch failed: ${H instanceof Error ? H.message : String(H)}`);
    }
  }, [i, U, e.local, t]), Y = C(async (k) => {
    ...
    W(k.id);
    const E = `local-run-${Date.now()}-${k.id}`;
    M((R) => [{
      id: E,
      toolId: k.id,
      label: k.label,
      status: "running",
      startedAt: Date.now(),
      lines: [`[Atomek] Starting ${k.label} local job…`]
    }, ...R].slice(0, 6));
    try {
      const R = await e.local.runJob({
        ...
        cwd: wn(i),
        context: Yo(i, c)
      }), J = [`[Atomek] Started ${k.label} local job ${R.id}`], oe = (w) => {
        M((v) => v.map((P) => P.id === E ? w(P) : P));
      };
      e.local.streamJob(R.id, {
        onLog: (w) => {
          J.push(w), oe((v) => ({ ...v, lines: J.slice(-500) }));
        },
        onDone: (w) => {
          w && (J.push(w), oe((v) => ({ ...v, lines: J.slice(-500) })));
        },
        onFail: (w) => {
          J.push(`[FAIL] ${w}`), oe((v) => ({
            ...v,
            ...
            finishedAt: Date.now(),
            lines: J.slice(-500)
          })), a(`${k.label} local job failed`, J.join(`
`)), W(null);
        },
        onExit: (w) => {
          const v = [
...
`);
          oe((P) => ({
            ...P,
            status: w === 0 ? "complete" : "failed",
            exitCode: w,
            finishedAt: Date.now(),
            lines: J.slice(-500)
          })), a(`${k.label} local job`, v), W(null);
        },
        onError: () => t(`Local job stream issue for ${k.label}`)
      }), t(`Started ${k.label} local job`);
    } catch (R) {
      W(null), M((J) => J.map((oe) => oe.id === E ? {
        ...oe,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...oe.lines, `[Atomek] Failed to start: ${R instanceof Error ? R.message : String(R)}`]
      } : oe)), t(`Local job failed to start: ${R instanceof Error ? R.message : String(R)}`);
    }
  }, [i, e.local, O, c, a, t]);
  return /* @__PURE__ */ d("aside", { className: V ? "workbench-agent-dock" : "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: V ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      ...
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ...
        " ",
        u ? "Refreshing…" : "Refresh capabilities"
      ] }),
      S && /* @__PURE__ */ r("div", { className: "workbench-inline-error", children: S }),
      ...
      /* @__PURE__ */ r("div", { className: "workbench-computer-presets", children: ie.map((k) => /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => K(k.prompt), children: k.label }, k.label)) }),
      /* @__PURE__ */ r(
        "textarea",
        {
      ...
      T ? /* @__PURE__ */ d("div", { className: "workbench-agent-run", children: [
        /* @__PURE__ */ d("header", { children: [
          /* @__PURE__ */ d("div", { children: [
            /* @__PURE__ */ r("strong", { children: T.label }),
            /* @__PURE__ */ d("span", { children: [
              T.status,
              typeof T.exitCode == "number" ? ` · exit ${T.exitCode}` : ""
            ] })
          ] }),
          /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => a(`${T.label} local job`, T.lines.join(`
`)), disabled: T.lines.length === 0, children: "Save output" })
        ] }),
        /* @__PURE__ */ r("pre", { className: "workbench-computer-job-log", children: T.lines.join(`
`) || "[waiting for output]" })
      ] }) : null,
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        l.length === 0 && !u ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        l.map((k) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          ...
          /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
            k.kind === "ai-cli" ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
              Y(k);
            }, disabled: k.status !== "available" || le !== null, title: "Run this local agent inside Atomek and stream output here. It cannot write files directly.", children: Zo(k, le) }) : null,
            /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
              A(k);
            }, disabled: k.status !== "available", title: "Launch this tool in the real Tytus terminal with current Atomek context prefilled.", children: Qo(k) })
          ] })
        ] }, k.id))
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        p.length === 0 && !u ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
        p.map((k) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ d("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ d("div", { children: [
          ...
          k.triggers?.length ? /* @__PURE__ */ r("div", { className: "workbench-computer-triggers", children: k.triggers.slice(0, 4).map((H) => /* @__PURE__ */ r("span", { children: H }, H)) }) : null,
          /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
            n(k);
          }, disabled: k.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
        ] }, k.id))
      ] })
    ] })
  ] });
}
function vn({ title: e, body: t }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ r("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function ec({ status: e, file: t, cursor: n, fileCount: a, dirtyCount: i }) {
  return /* @__PURE__ */ d("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ r("span", { children: Tn(t.language) })
  ] });
}
function It(e, t) {
...
function Pt(e, t) {
  const n = new Set(e.map((c) => c.path));
  let a = `${t}.md`, i = 2;
  for (; n.has(a); )
    a = `${t}-${i}.md`, i += 1;
  return a;
...
function cc({ host: e }) {
  return /* @__PURE__ */ r(Io, { host: e });
}
function dc(e) {
  return function() {
    return /* @__PURE__ */ r(cc, { host: e.host });
  };
}
export {
  dc as default
