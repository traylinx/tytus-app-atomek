...
import { forwardRef as ja, createElement as Hn, useState as z, useRef as it, useCallback as N, useEffect as re, lazy as ls, useMemo as de, Suspense as cs } from "react";
/**
...
function Xi({ host: e, requestContext: t, chatSettings: n, selectedTarget: a = Be, setStatus: s }) {
  const i = e.ai, [o, c] = z(null), [h, p] = z([]), [v, y] = z([]), [w, m] = z([]), [A, R] = z([]), [Z, F] = z(ya), [ge, we] = z(!1), j = it(!0), _ = it(null), q = N(async () => {
    if (!i) {
      F(ya);
      return;
    }
    ...
    if (i)
      try {
        const x = ka(await i.listThreads({ workspaceKey: Ye, status: "active" })), L = gi(), V = x.find((ee) => ee.id === L) ?? x[0] ?? await i.createThread({ workspaceKey: Ye, title: "Atomek chat" });
        if (!j.current) return;
  ...
  const ue = N(async () => {
    ...
    try {
      ...
      if (X.kind === "pod-agent") {
        ...
        if (y((ne) => [...ne, he, Ne]), Ft(X.id, [he, Ne]), !X.available) {
          const ne = ba(X.description), me = { ...Ne, body: ne.message, status: "error", error: ne.message };
          return y((u) => u.map((k) => k.id === te ? me : k)), Ft(X.id, [me]), s(ne.message), me;
        }
        if (!K) {
          ...
          return y((u) => u.map((k) => k.id === te ? me : k)), Ft(X.id, [me]), s("Agent chat bridge unavailable"), me;
        }
        let Ie = "";
        const De = (ne, me) => (y((u) => u.map((k) => k.id === te ? ne : k)), Ft(X.id, [he, ne]), s(me), ne);
        try {
          for await (const ne of K.chatAgent({
            ...
            if (ne.type === "session" && ki(X.id, ne.sessionId), ne.type === "token" && (Ie = Bn(`${Ie}${ne.text}`), y((me) => me.map((u) => u.id === te ? { ...u, body: Ie, status: "streaming" } : u))), ne.type === "error") {
              const me = ba(ne.message), u = { ...Ne, body: me.message, status: "error", error: me.message };
      ...
      if (!xe || !i) return null;
      at(xe.id);
      const Ve = await fe(V).catch(() => []), ze = bi(Ve), Ee = ze ? [...le, ze] : [...le];
      let Ze = null;
      for await (const K of i.sendMessage({
        ...
        if (K.type === "message_created") {
          ...
          if (!G) continue;
          G.role === "assistant" && (Ze = G.id), y((he) => [...he.filter((te) => te.id !== G.id), G]);
        }
        if (K.type === "token" && (Ze = K.messageId, y((G) => G.some((he) => he.id === K.messageId) ? G.map(
        ...
        if (K.type === "run_failed") {
          ...
          s(G ? "AI response stopped" : `AI failed: ${K.error}`), Ze && y((he) => he.map((te) => te.id !== Ze ? te : G ? { ...te, status: "complete", error: void 0, body: te.body || "Stopped by user." } : { ...te, status: "error", error: K.error, body: K.error }));
        }
      }
...
`));
  ...
  for (; p < e.content.length && h.length < o; ) {
    const y = Qi(e.content, p, Math.min(e.content.length, p + s)), w = e.content.slice(p, y).trim();
    if (w.length > 0) {
      const m = _i(c, p, y);
      h.push({
...
[...context clipped...]` : v.chunk.text;
    c.push(io(v, w, i.has(v.chunk.fileId))), p -= w.length;
  }
  return c;
...
`), lo = (e) => Array.from(new Set(
  ...
  const c = [];
  for (const h of t) {
    const p = Tn(n, h), v = Tn(a, h), y = Tn(s, h), w = i === h ? 1 : 0;
  ...
  const v = (i.current ?? ht([], a)).staleReport(e), y = N((w, m = {}) => {
    ...
    return tr(A.getSnapshot(), w, m, A.staleReport(s.current));
  }, []);
  ...
  try {
    ...
    const w = new Map(c.map((j) => [j.chunkId, j])), m = zo(n), A = /* @__PURE__ */ new Map();
    ...
    for (const j of R) {
      ...
      const be = w.get(j.id)?.score ?? Xo(j, m), Y = To(be) * 0.42 + Math.max(0, q) * 0.58;
...
function Io(e) {
  ...
  for (const v of lr(t))
    for (const y of Co(v)) {
      ...
      const m = Vn(w.file.content, y.raw);
...
const Tl = ls(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), zl = {
  ...
  dirty: !1,
  source: "sample"
}, vr = "tytus.workspace.recent", yr = "tytus.workspace.layout", gr = "tytus.atomek.session.v2", wr = "tytus.atomek.chatAiSettings", Ln = "atomek:default", kr = "tytus.atomek.currentMission", In = "tytus.atomek.currentMissionChanged", La = "0.4.27", en = {
...
function xt(e, t = "repo-repair") {
  const n = Ct.find((Y) => Y.id === t) ?? Ct[0], a = e?.resources ?? [], s = rt(a, (Y) => Y.kind === "pod-agent" && Pt(Y) === "openclaw"), i = rt(a, (Y) => Y.kind === "pod-agent" && Pt(Y) === "hermes"), o = rt(a, (Y) => Y.kind === "ail-route"), c = rt(a, (Y) => Y.kind === "shared-folder"), h = Ue(a, "local-cli", ["claude"]), p = Ue(a, "local-cli", ["opencode", "open code"]), v = Ue(a, "local-cli", ["codex"]), y = Ue(a, "local-cli", ["pi"]), w = Ue(a, "local-cli", ["kimi"]), m = rt(a, (Y) => Y.kind === "local-cli"), A = Ue(a, "app-skill", ["juli3ta", "music", "song"]), R = Ue(a, "app-skill", ["blender", "3d"]), Z = Ue(a, "app-skill", ["remotion", "video", "render"]), F = rt(a, (Y) => Y.kind === "app-skill"), ge = t === "pod-local" ? s ?? i ?? o ?? h ?? m : t === "creative-production" ? i ?? o ?? h ?? m : t === "research-watch" ? s ?? o ?? i ?? h ?? m : h ?? p ?? o ?? m, we = t === "creative-production" ? A ?? R ?? Z ?? F ?? p ?? h ?? m : p ?? h ?? v ?? m, j = t === "pod-local" ? v ?? y ?? w ?? s ?? h ?? m : t === "research-watch" ? y ?? w ?? v ?? s ?? m : v ?? y ?? s ?? w ?? m, _ = t === "creative-production" ? A ?? R ?? Z ?? F : t === "research-watch" ? Ue(a, "app-skill", ["browser", "channel", "telegram"]) ?? F : F, q = [
...
function Il({ host: e }) {
  const t = it(null), [n, a] = z(0), s = de(() => Tc(), []), i = de(() => wc(), []), o = de(() => gc(yc(), i.recent), [i.recent]), [c, h] = z(i.activity ?? "computer"), [p, v] = z(s.primaryVisible), [y, w] = z(s.primaryWidth), [m, A] = z(i.secondaryTab ?? "chat"), [R, Z] = z(s.secondaryVisible), [F, ge] = z(s.secondaryWidth), [we, j] = z(!!i.bottomPanelVisible), [_, q] = z(i.bottomPanelTab ?? "problems"), [M, be] = z(s.markdownPreviewVisible), [Y, ve] = z(!!i.welcomeClosed), [ue, He] = z(i.folder ? { name: i.folder.name, files: [] } : null), [We, je] = z(i.folder?.handleKey ?? null), [O, oe] = z(() => kc(i.files)), [Pe, fe] = z(() => (i.openEditorIds ?? []).filter((l) => (i.files ?? []).some((f) => f.id === l))), [g, W] = z(() => i.activeFileId && (i.files ?? []).some((l) => l.id === i.activeFileId) ? i.activeFileId : null), [x, L] = z(i.query ?? ""), [V, X] = z({ lineNumber: 1, column: 1 }), [ee, ae] = z(null), [le, xe] = z({}), [Ve, ze] = z(Si), [Ee, Ze] = z([]), [ye, K] = z([]), [G, he] = z(null), [te, Ne] = z(i.chatInput ?? ""), [Ie, De] = z(!1), [ne, me] = z(""), [u, k] = z([]), [T, H] = z(null), [S, E] = z(null), [$, U] = z(!1), [ke, Ke] = z(() => zc()), [B, I] = z([Be]), [se, Le] = z(() => ni(Ln)), [Te, Ae] = z(null), [Fe, lt] = z(null), [ct, Nt] = z(""), [Qe, un] = z(""), [Mt, Wt] = z(""), [Vt, na] = z("failed"), [jr, aa] = z(null), [Mr, C] = z("Ready"), [Tt, Wr] = z(() => o), Se = Pe.map((l) => O.find((f) => f.id === l)).filter(Boolean), D = g ? O.find((l) => l.id === g) ?? null : null, ra = de(() => Oi({ files: O, openEditorIds: Pe, activeFileId: g, versions: le, activeSelection: ee }), [g, ee, le, O, Pe]), sa = de(() => ({
    scope: Ve,
    removedAttachmentIds: Ee,
    selectedFileIds: []
  }), [Ve, Ee]), zt = de(() => Ei(ra, O, sa), [sa, ra, O]), $e = fo(O, { autoRefresh: !0, includeDirty: !0 }), ia = de(() => ye.map((l) => ({
    id: l.id,
    kind: "index-hit",
    label: l.label,
    path: l.path,
    ...
    snippet: l.snippet
  })), [ye]), Vr = de(() => [...zt.attachments, ...ia], [zt.attachments, ia]), oa = de(() => B.find((l) => l.id === se) ?? Be, [B, se]), Q = Xi({ host: e, requestContext: zt.parts, chatSettings: ke, selectedTarget: oa, setStatus: C }), tt = de(
    () => [...Q.artifacts, ...u].sort((l, f) => f.createdAt - l.createdAt),
    [Q.artifacts, u]
  ), It = $ && !D, fn = !D && !It && !Y, Xe = de(() => O.filter((l) => l.dirty), [O]), Ir = de(() => {
  ...
  const Dr = N((l) => {
    ...
    const f = An(n), b = l.clientX, P = y, J = (nt) => {
      ...
      w(Yt(kn, f.primaryMin, f.primaryMax));
    }, ce = () => {
      window.removeEventListener("pointermove", J), window.removeEventListener("pointerup", ce);
    };
    window.addEventListener("pointermove", J), window.addEventListener("pointerup", ce);
    ...
    fe((b) => b.includes(l.id) ? b : [...b, l.id]), W(l.id), ve(!1), aa(f ?? null), X({ lineNumber: f ?? 1, column: 1 });
  }, []), qr = N((l) => {
    const f = {
      id: `atomek-doc:${l.id}`,
      name: l.fileName,
    ...
    if (On(Xe, "open another folder"))
      try {
        ...
        l.handle && f && await Xa(f, l.handle), je(f), He(l), oe(l.files), fe([]), W(null), ve(!1), _e({ name: l.name, path: l.name, kind: "folder", handleKey: f ?? void 0, at: Date.now() }), C(`${l.handle ? "Opened local folder" : "Opened browser fallback folder"} ${l.name} (${l.files.length} text files indexed)`);
      } catch (l) {
    ...
    On(Xe, "close all editors") && (fe([]), W(null), aa(null), ve(!1), C("Closed all editors"));
  }, [Xe]), bn = N(() => {
...
`,
    ...
    try {
      if (l.kind === "folder") {
        ...
        const ce = await pa(J);
        je(l.handleKey), He(ce), oe(ce.files), fe([]), W(null), ve(!1), _e({ ...l, at: Date.now() }), C(`Reopened ${ce.name} (${ce.files.length} text files indexed)`);
        return;
      }
...
`), J = {
    ...
    k((ce) => [J, ...ce]), j(!0), q("output"), C(`Captured manual check: ${f}`);
  }, []), ut = N((l, f) => {
    const b = Io({
      body: f,
      files: O,
    ...
    k((P) => [b, ...P]), Z(!0), A("outputs"), j(!0), q("output"), C(`Saved local job output: ${l}`), (f.includes("```diff") || f.includes("--- a/")) && ut(l, f);
  }, [ut]), $r = N(() => {
    const l = te.trim();
    if (!l) return;
    Ne("");
  ...
  return re(() => {
    ...
    return (async () => {
      try {
        ...
        const b = await pa(f);
        if (l) return;
        je(i.folder?.handleKey ?? null), He(b), oe((P) => Nc(P, b.files)), fe((P) => P.filter((J) => b.files.some((ce) => ce.id === J) || O.some((ce) => ce.id === J))), C(`Restored ${b.name} (${b.files.length} text files indexed)`);
      } catch (f) {
      ...
      children: [
        ...
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          ...
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ r(ql, { file: D, folder: ue, showWelcome: fn }),
            ...
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: D ? /* @__PURE__ */ d("div", { className: D.language === "markdown" && M ? "workbench-editor-split" : "workbench-editor-single", children: [
              ...
              A("chat"), Z(!0);
            }, openEmbeddedDoc: qr }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
...
function Rl(e) {
  const t = !e.folder, n = de(() => El(e.files, e.folder?.name), [e.files, e.folder?.name]), [a, s] = z(() => /* @__PURE__ */ new Set());
  re(() => {
    const o = Xr(n);
    s((c) => {
      const h = new Set(c);
    ...
    try {
      ...
      await e.missions.write({
        ...
        files: [
          ...
          { path: "MISSION.json", content: zr(L, w, g, W) },
          { path: "RESOURCES.md", content: nn(w) },
          { path: "TASKS.md", content: Nr(Ht(g, w, W)) },
...
`) }
  ...
  }, [v, w, e.missions, c, o, q]), ue = Jl(w), He = Sl(w), We = Cl(w), je = Zl(w), O = w?.warnings ?? [], oe = jl(w), Pe = xt(w, ot(v, w, q)), fe = Ht(v, w, Pe.id);
  return /* @__PURE__ */ d("div", { className: "workbench-welcome workbench-control-home", children: [
    ...
    /* @__PURE__ */ d("section", { className: "workbench-control-grid", children: [
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ r("div", { className: "workbench-doc-grid", children: Pl.map((g) => /* @__PURE__ */ d("button", { className: "workbench-doc-card", onClick: () => p(g), title: g.summary, children: [
...
function Ar({ body: e }) {
  const t = de(() => Ll(e), [e]), [n, a] = z(null), s = N((i, o) => {
...
function Cr({
  ...
  const xe = N(async (u = M) => {
    ...
    if (!u) return;
    const T = [...k].sort((S, E) => Aa(E).localeCompare(Aa(S))).slice(0, 100), H = T.map((S) => JSON.stringify(S)).join(`
`) + (T.length ? `
` : "");
    u.rootPath && e.missions?.write ? await e.missions.write({ rootPath: u.rootPath, files: [{ path: "RUNS.jsonl", content: H }] }) : u.handle && await Dn(u.handle, "RUNS.jsonl", H), je(T), O.current = T;
  ...
  re(() => {
    ...
    return window.addEventListener(In, u), () => window.removeEventListener(In, u);
  }, [Ee, xe]);
  const Ze = N((u) => {
    const k = Vl(u);
    be(k), St(k), xe(k), He([{
    ...
    ], S = ot(k || u.goal, y, u.teamPresetId), E = Ht(k || u.goal, y, S), $ = [
      { path: "MISSION.md", content: tn(u, y, s, i, k, S) },
      { path: "MISSION.json", content: zr(u, y, k, S) },
      { path: "RESOURCES.md", content: nn(y) },
      { path: "TASKS.md", content: Nr(E) },
      { path: "HANDOFF.md", content: Tr(u) },
      { path: "INBOX.md", content: `# Mission inbox
...
` }
    ...
    if (u.rootPath && e.missions?.write)
      ...
      for (const U of $) await Ol(u.handle, U.path, U.content);
    } else
  ...
  }, [K, F, t]), te = N(async (u, k, T, H = M, S) => {
    if (!H) return;
    const E = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), $ = S ?? `runs/${E}-${u.id}.md`, U = $.split("/").pop() || `${E}-${u.id}.md`;
    ...
    j(u.id);
    const H = `local-run-${Date.now()}-${u.id}`, S = mt();
    q((E) => [{
      id: H,
      toolId: u.id,
      label: u.label,
...
`);
    ...
    let H = null;
    const S = Oe(u), E = `pod-run-${Date.now()}-${k}`, $ = mt(), U = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
    ...
    q((I) => [{
      ...
      taskTitle: X?.title ?? "Manual pod run",
      transcriptPath: U,
      lines: [`[Atomek] Dispatching ${S} via Tytus host bridge…`, `[Atomek] Pod: ${k}`, `[Atomek] Task: ${X?.title ?? "manual pod run"}`]
...
`);
      q((Nt) => Nt.map((Qe) => Qe.id === E ? { ...Qe, status: "complete", finishedAt: Date.now(), lines: [...Qe.lines, "[Atomek] Pod task complete."].slice(-500) } : Qe)), await ze({ ...ke, status: "complete", finishedAt: mt(), summary: `${S} completed`, transcriptPath: U }, H), a(`${S} pod job`, ct), await te({ id: u.id, label: S, kind: "pod-agent", status: u.status }, ct, 0, H, U), t(`${S} completed mission task`);
    } catch (I) {
      ...
      const Le = [`# Pod job failed — ${S}`, "", `- Pod: ${k}`, `- Error: ${se}`, "", "```text", Ke.join(`
`), "```", ""].join(`
`);
      a(`${S} pod job failed`, Le), await te({ id: u.id, label: S, kind: "pod-agent", status: u.status }, Le, -1, H, U).catch((Te) => {
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-list compact", children: [
          ...
          y?.resources.slice(0, x ? 8 : 5).map((u) => /* @__PURE__ */ d("div", { className: "workbench-resource-row", children: [
            ...
            /* @__PURE__ */ d("div", { className: "workbench-resource-row-actions", children: [
              ...
              u.kind === "pod-agent" && Fn(u) ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
                Ie(u);
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
          ...
          c.map((u) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
            ...
            /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ r("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                G(u);
              }, disabled: u.status !== "available", title: "Launch this tool in a fresh Tytus Terminal with mission context and execute it immediately.", children: uc(u) }),
              u.kind === "ai-cli" ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
                Ne(u);
              }, disabled: u.status !== "available" || we !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: fc(u, we) }) : null
            ] })
          ] }, u.id))
        ] }),
