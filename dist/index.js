 ...
 */
const qa = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Ka = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), ln = (e) => {
  const t = Ka(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, Sn = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), Ga = (e) => {
  for (const t in e)
...
const Xa = Nn(
  ({
    ...
    "svg",
    {
      ...
      className: Sn("lucide", o),
      ...!i && !Ga(d) && { "aria-hidden": "true" },
      ...d
    },
    [
      ...s.map(([f, b]) => Tt(f, b)),
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Za = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], er = q("check", Za);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const tr = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], rt = q("chevron-down", tr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const Or = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Rr() {
  const e = Bt();
  return !!(e?.showOpenFilePicker && e.showDirectoryPicker);
}
async function zr() {
  const e = Bt()?.showOpenFilePicker;
  if (!e) return Dn(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((r) => Fn(r, r.name, "local-file")));
  return Wt(n.filter(Boolean));
}
async function Fr() {
  const e = Bt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await Dn(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await jn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Wt(n) };
}
function Bt() {
  const e = window;
...
async function jn(e, t, n, r, o) {
  if (!(n.length >= o || r > 8))
    for await (const i of e.values()) {
      if (n.length >= o) break;
      if (i.kind === "directory") {
        if (Or.has(i.name)) continue;
        await jn(i, `${t}/${i.name}`, n, r + 1, o);
        continue;
      }
      const s = i, d = await Fn(s, `${t}/${s.name}`, "local-folder");
      d && n.push(d);
    }
}
function Dn(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const r = Array.from(n.files ?? []), o = await Promise.all(r.map(async (i) => {
...
`).map(
    ...
    const i = r.split("|").filter(Boolean).map((d) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${d.trim()}</th>`).join(""), s = o.trim().split(`
...
const Ee = "atomek:default", Dr = 3e3, Ut = "tytus.atomek.threadTitleOverrides", hn = {
  available: !1,
  source: "none",
...
}, _r = (e, t = Dr) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, un = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Hr = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", mn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Hr(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = Vt();
    n[e] = t, localStorage.setItem(Ut, JSON.stringify(n));
  } catch {
  }
}, Ur = (e) => {
  try {
    const t = Vt();
    delete t[e], localStorage.setItem(Ut, JSON.stringify(t));
  } catch {
  }
}, pn = (e) => {
  const t = Vt();
...
function Vr({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const o = e.ai, [i, s] = v(null), [d, m] = v([]), [f, b] = v([]), [h, k] = v([]), [N, M] = v([]), [H, X] = v(hn), [G, te] = v(!1), Y = Re(!0), T = Re(null), B = w(async () => {
    if (!o) {
      X(hn);
      return;
    }
    ...
    if (o)
      try {
        const x = await o.listArtifacts({ threadId: $ });
        if (!Y.current) return;
        k(x.map(mn));
      } catch (x) {
        r(`AI artifacts unavailable: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [o, r]), V = w(async ($) => {
    if (o)
      try {
        const x = await o.listMessages($);
        if (!Y.current) return;
        const E = d.find((W) => W.id === $) ?? null;
        E && s(E), b(x.map(We).filter(Boolean)), M([]), await Q($);
      } catch (x) {
        r(`Load chat failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [o, Q, r, d]), A = w(async () => {
    if (o)
      try {
        const $ = pn(await o.listThreads({ workspaceKey: Ee, status: "active" })), x = $[0] ?? await o.createThread({ workspaceKey: Ee, title: "Atomek chat" });
        if (!Y.current) return;
        m($[0] ? $ : [x]), s(x);
        const E = await o.listMessages(x.id);
        if (!Y.current) return;
        b(E.map(We).filter(Boolean)), await Q(x.id);
      } catch ($) {
        r(`AI chat unavailable: ${$ instanceof Error ? $.message : String($)}`);
      }
  }, [o, Q, r]);
  ne(() => (Y.current = !0, B(), A(), () => {
    Y.current = !1;
  ...
  const j = w(async () => {
    if (!o) return null;
    if (i) return i;
    const $ = await o.createThread({ workspaceKey: Ee, title: "Atomek chat" });
    return Y.current && (s($), m((x) => [$, ...x.filter((E) => E.id !== $.id)])), $;
  }, [o, i]), fe = w(async () => {
    if (!o) return;
    const $ = await o.createThread({ workspaceKey: Ee, title: "Atomek chat" });
    s($), m((x) => [$, ...x.filter((E) => E.id !== $.id)]), b([]), k([]), M([]), r("New AI chat created");
  }, [o, r]), C = w(async ($, x) => {
    if (!o) return;
    const E = x.trim();
    if (E)
      try {
        const W = o.updateThread, p = d.find((P) => P.id === $) ?? i, I = typeof W == "function" ? await W({ threadId: $, title: E }) : { ...p ?? await j(), id: $, title: E, updatedAt: Date.now() };
        typeof W != "function" && Wr($, E), m((P) => P.map((O) => O.id === I.id ? I : O)), s((P) => P?.id === I.id ? I : P), r(typeof W == "function" ? `Renamed chat: ${I.title}` : `Renamed chat locally: ${I.title}`);
      } catch (W) {
        r(`Rename chat failed: ${W instanceof Error ? W.message : String(W)}`);
      }
  }, [o, j, r, i, d]), Z = w(async ($) => {
    if (o)
      try {
        await o.deleteThread($), Ur($);
        const x = d.filter((E) => E.id !== $);
        if (m(x), i?.id === $) {
          const E = x[0] ?? await o.createThread({ workspaceKey: Ee, title: "Atomek chat" });
          s(E), m((p) => p.some((I) => I.id === E.id) ? p : [E, ...p]);
          const W = await o.listMessages(E.id);
          b(W.map(We).filter(Boolean)), await Q(E.id);
        }
        r("Deleted AI chat");
      } catch (x) {
        r(`Delete chat failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [o, Q, r, i?.id, d]), me = w(async ($) => {
    if (!o) return null;
    try {
      const x = await j();
      if (!x) return null;
      const E = await o.createArtifact({
        threadId: x.id,
        messageId: $.messageId ?? null,
        title: $.title?.trim() || un($.body, "Atomek artifact"),
        kind: $.kind ?? "markdown",
        body: $.body
      }), W = mn(E);
      return k((p) => [W, ...p.filter((I) => I.id !== W.id)]), r(`Saved AI artifact: ${W.title}`), W;
    } catch (x) {
      return r(`Save artifact failed: ${x instanceof Error ? x.message : String(x)}`), null;
    }
  }, [o, j, r]), be = w(async ($) => {
    if (o)
      try {
        await o.deleteArtifact($), k((x) => x.filter((E) => E.id !== $)), r("Deleted AI artifact");
      } catch (x) {
        r(`Delete artifact failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [o, r]), ee = w(async ($) => {
    if (!o) return null;
    try {
      const x = await o.writeMemory({
        title: $.title?.trim() || un($.body, "Atomek memory"),
        body: $.body,
        metadata: {
          source: "atomek",
          messageId: $.messageId ?? null
      ...
      return M((E) => [x, ...E.filter((W) => W.id !== x.id)].slice(0, 5)), r(`Remembered: ${x.title}`), x;
    } catch (x) {
      return r(`Remember failed: ${x instanceof Error ? x.message : String(x)}`), null;
    }
  }, [o, r]), ae = w(async ($) => {
    if (!o) return [];
    const x = await o.searchMemory({ query: $, limit: 5 });
    return Y.current && M(x), x;
  }, [o]), de = w(async ($, x = {}) => {
    const E = $.trim();
    if (!E || !o) return null;
    te(!0);
    ...
    try {
      const I = await j();
      if (!I) return null;
      const P = await ae(E).catch(() => []), O = Br(P), R = x.requestContext ?? t, U = O ? [...R, O] : [...R];
      let F = null;
      for await (const z of o.sendMessage({
        threadId: I.id,
        body: E,
        ...
        if (z.type === "message_created") {
          const D = We(z.message);
          if (!D) continue;
          D.role === "assistant" && (F = D.id), b((se) => [...se.filter((ce) => ce.id !== D.id), D]);
        }
        if (z.type === "token" && (F = z.messageId, b((D) => D.some((se) => se.id === z.messageId) ? D.map(
          (se) => se.id === z.messageId ? { ...se, body: z.body, status: "streaming" } : se
        ) : [...D, {
          id: z.messageId,
          role: "assistant",
          body: z.body,
          status: "streaming",
          createdAt: Date.now()
        }])), z.type === "message_updated" || z.type === "done") {
          const D = We(z.message);
          if (!D) continue;
          D.role === "assistant" && (W = D), b((se) => se.map((ce) => ce.id === D.id ? D : ce)), D.gatewayLabel && r(`AI answered via ${D.gatewayLabel}`);
        }
        if (z.type === "run_failed") {
          const D = p.signal.aborted;
          r(D ? "AI response stopped" : `AI failed: ${z.error}`), F && b((se) => se.map((ce) => ce.id !== F ? ce : D ? { ...ce, status: "complete", error: void 0, body: ce.body || "Stopped by user." } : { ...ce, status: "error", error: z.error, body: z.error }));
        }
      }
      const K = pn(await o.listThreads({ workspaceKey: Ee, status: "active" }).catch(() => []));
      return Y.current && K.length > 0 && m(K), B(), W;
    } catch (I) {
      return I instanceof DOMException && I.name === "AbortError" ? (r("AI response stopped"), W) : (r(`AI failed: ${I instanceof Error ? I.message : String(I)}`), null);
    } finally {
      T.current === p && (T.current = null), te(!1);
    }
  }, [o, n.gatewayPreference, n.model, j, ae, B, t, r]), ge = w(() => {
    T.current?.abort(), r("Stopping AI response…");
  }, [r]);
  return {
    aiStatus: H,
    artifacts: h,
...
const qr = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Kr = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Gr = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: o }) => {
  ...
  return {
    ...
    activeDocumentId: n,
    openDocumentIds: t.filter((d) => s.some((m) => m.id === d))
  };
}, Jr = "active-file", Xr = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Yr = (e, t = "file") => `${t}:${e.id}`, Qr = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let o = [];
  return t.scope === "active-selection" ? r?.selection ? o = [Zr(r, r.selection, !0)] : r && (o = [et(r, !0, "Active file")]) : t.scope === "active-file" ? r && (o = [et(r, !0, "Active file")]) : t.scope === "open-editors" ? o = e.openDocumentIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => et(i, !0, "Open editor")) : t.scope === "selected-files" && (o = t.selectedFileIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => et(i, !1, "Selected file"))), to(o).filter((i) => !n.has(i.id));
}, to = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, no = 4e3, ao = 8e3, ro = 4e3, oo = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, io = (e, t, n) => {
  const r = eo(e, n), o = [], i = new Map(t.map((s) => [s.id, s]));
  for (const s of r) {
    if (!s.fileId) continue;
    const d = i.get(s.fileId), m = e.byId.get(s.fileId);
    if (!d || !m || !s.includeBody) continue;
    const f = [
      `Path: ${d.path}`,
      `Language: ${d.language}`,
      `Version: ${m.version}`,
      ...
      s.range ? `Range: ${fn(s.range)}` : null
    ].filter(Boolean).join(`
`), b = s.kind === "selection" && s.range ? so(d.content, s.range) : d.content, h = s.kind === "selection" ? no : s.label.startsWith("Open editor") ? ro : ao;
    o.push({
...
`)).join(`
`)
  }), { parts: o, attachments: r };
}, so = (e, t) => {
  const n = e.split(`
`), r = Math.max(1, t.startLineNumber), o = Math.max(r, t.endLineNumber), i = n.slice(r - 1, o);
...
`));
}, fn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, bn = 75e4, co = 2800, lo = 240, ho = 80, uo = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? bn) || e.content.length > (t.maxFileBytes ?? bn) ? "huge" : !t.includeDirty && e.dirty ? null : wo(e.path) ? "vendor" : yo(e) ? "binary" : null;
}, gn = (e, t = {}) => {
  const n = Date.now(), r = [], o = [], i = [];
  for (const s of e) {
    const d = s.size ?? s.content.length, m = Et(s, t);
    if (m) {
      o.push({ fileId: s.id, name: s.name, path: s.path, language: s.language, reason: m, size: d });
      continue;
    }
    const f = Ie(s.content), b = po(s, f, n, t);
    i.push(...b), r.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
  ...
  return fo({
    ...
    skipped: o,
    chunks: i,
    byFileId: new Map(r.map((s) => [s.fileId, s])),
    byChunkId: new Map(i.map((s) => [s.id, s]))
  });
}, po = (e, t = Ie(e.content), n = Date.now(), r = {}) => {
  const o = Math.max(500, r.maxChunkChars ?? co), i = Math.min(Math.max(0, r.chunkOverlapChars ?? lo), Math.floor(o / 2)), s = Math.max(1, r.maxChunksPerFile ?? ho), d = go(e.content), m = [];
  let f = 0, b = 0;
  for (; f < e.content.length && m.length < s; ) {
    const h = bo(e.content, f, Math.min(e.content.length, f + o)), k = e.content.slice(f, h).trim();
    if (k.length > 0) {
      const N = ko(d, f, h);
      m.push({
  ...
  const r = Math.max(t + 1, n - 500), o = e.lastIndexOf(`
`, n);
  if (o >= r) return o + 1;
...
` && t.push(n + 1);
  ...
  const r = kn(e, t), o = kn(e, Math.max(t, n - 1));
  return {
    startLineNumber: r + 1,
    startColumn: t - e[r] + 1,
    endLineNumber: o + 1,
  ...
  return 0;
}, wo = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => uo.has(t)), yo = (e) => {
...
class vo {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = gn(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = gn(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return Co(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const Oe = (e = [], t = {}) => new vo(e, t), Co = (e, t, n = {}) => {
  const r = new Map(t.map((d) => [d.id, d])), o = e.byFileId, i = [];
  for (const d of e.files) {
    const m = r.get(d.fileId);
    if (!m) {
      i.push({ fileId: d.fileId, path: d.path, status: "deleted", indexedHash: d.hash });
      continue;
    }
    if (Et(m, n)) {
      i.push({ fileId: d.fileId, path: m.path, status: "skipped-now", indexedHash: d.hash, currentHash: Ie(m.content) });
      continue;
    }
    const b = Ie(m.content);
    b !== d.hash ? i.push({ fileId: d.fileId, path: m.path, status: "changed", indexedHash: d.hash, currentHash: b }) : m.dirty !== d.dirty && i.push({ fileId: d.fileId, path: m.path, status: "dirty-state-changed", indexedHash: d.hash, currentHash: b });
  }
  for (const d of t)
    !o.has(d.id) && !Et(d, n) && i.push({ fileId: d.id, path: d.path, status: "new", currentHash: Ie(d.content) });
  const s = e.signature !== Bn(t);
  return { stale: i.length > 0, signatureChanged: s, files: i };
}, xo = 8, $o = 12e3, Wn = (e, t, n = {}, r) => {
  const o = So(t);
  if (o.length === 0) return [];
  const i = new Set(r?.files.map((b) => b.fileId) ?? []), s = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => Io(b, o)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, h) => h.score - b.score || b.chunk.path.localeCompare(h.chunk.path) || b.chunk.ordinal - h.chunk.ordinal), d = [], m = /* @__PURE__ */ new Set();
  let f = n.maxChars ?? $o;
  for (const b of s) {
    if (d.length >= (n.limit ?? xo) || f <= 0) break;
...
[...context clipped...]` : b.chunk.text;
    d.push(Ao(b, k, i.has(b.chunk.fileId))), f -= k.length;
  }
  return d;
}, Ao = (e, t = e.chunk.text, n = !1) => {
  const { chunk: r, score: o, matchedTerms: i } = e;
  return {
    id: `index-hit:${r.id}`,
    kind: "index-hit",
...
`), So = (e) => Array.from(new Set(
  ...
  const n = e.text.toLowerCase(), r = e.path.toLowerCase(), o = e.name.toLowerCase(), i = e.language.toLowerCase();
  let s = 0;
  const d = [];
  for (const m of t) {
    const f = At(n, m), b = At(r, m), h = At(o, m), k = i === m ? 1 : 0;
    f + b + h + k !== 0 && (d.push(m), s += Math.min(f, 12), s += b * 4, s += h * 6, s += k * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (s += 3));
  }
  ...
  const n = e.toLowerCase(), r = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, f) => m - f)[0] ?? 0, o = Math.max(0, r - 120), i = Math.min(e.length, r + 260), s = o > 0 ? "…" : "", d = i < e.length ? "…" : "";
  return `${s}${e.slice(o, i).replace(/\s+/g, " ").trim()}${d}`;
}, Po = (e, t = {}) => {
  const { autoRefresh: n = !1, ...r } = t, o = Re(e), i = Re(null);
  i.current || (i.current = Oe(e, r));
  const [s, d] = v(() => i.current?.getSnapshot() ?? Oe(e, r).getSnapshot());
  o.current = e;
  const m = w((k = o.current) => {
    const N = (i.current ?? Oe([], r)).refresh(k, r);
    return d(N), N;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), f = w((k = o.current) => {
    const N = (i.current ?? Oe([], r)).update(k, r);
    return d(N), N;
  }, [m]);
  ne(() => {
    n && m(e);
  }, [n, e, m]);
  const b = (i.current ?? Oe([], r)).staleReport(e), h = w((k, N = {}) => {
    const M = i.current ?? Oe([], r);
    return Wn(M.getSnapshot(), k, N, M.staleReport(o.current));
  }, []);
  return { snapshot: s, staleReport: b, isStale: b.stale, refresh: m, update: f, retrieve: h };
}, To = "tytus.atomek.semanticVector:v1", Un = "__gateway_default__", Vn = (e) => e?.trim() || Un, Lo = (e) => {
  const t = Vn(e);
  ...
  if (!Kn()) return null;
  try {
    const r = qn(e, t, n), o = window.localStorage.getItem(r);
    if (!o) return null;
...
}, Ue = 8, Ro = 12e3, zo = 180, Fo = async (e, t, n, r, o = {}, i, s) => {
  const d = Wn(t, n, { ...o, limit: Math.max(o.limit ?? Ue, 16) }, i), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: d.slice(0, o.limit ?? Ue), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const f = Vn(r.embeddingModel), b = await m.embedText({
      input: n,
    ...
    const k = new Map(d.map((T) => [T.chunkId, T])), N = Uo(n), M = /* @__PURE__ */ new Map();
    t.chunks.filter((T) => o.includeDirty !== !1 || !T.dirty).slice(0, zo).forEach((T) => M.set(T.id, T)), d.forEach((T) => {
      const B = t.byChunkId.get(T.chunkId);
      B && M.set(B.id, B);
    });
    const H = Array.from(M.values()), X = [];
    let G = 0;
    for (const T of H) {
      s?.aborted;
      const B = Eo(e.appId, f, T) ?? await Do(m, e.appId, f, T, r, s);
      if (!B) continue;
      B.updatedAt > Date.now() - 1e3 && (G += 1);
      const Q = Bo(h, B.vector);
      if (!Number.isFinite(Q)) continue;
      const A = k.get(T.id)?.score ?? Vo(T, N), j = Wo(A) * 0.42 + Math.max(0, Q) * 0.58;
      X.push({ chunk: T, keywordScore: A, vectorScore: Q, score: j, matchedTerms: N.filter((fe) => T.text.toLowerCase().includes(fe)) });
    }
    if (X.length === 0) return { hits: d.slice(0, o.limit ?? Ue), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: G };
    const te = new Set(i?.files.map((T) => T.fileId) ?? []), Y = X.sort((T, B) => B.score - T.score || B.keywordScore - T.keywordScore || T.chunk.path.localeCompare(B.chunk.path) || T.chunk.ordinal - B.chunk.ordinal).slice(0, Math.max(o.limit ?? Ue, 1)).map((T) => _o(T, te.has(T.chunk.fileId)));
    return {
      hits: Ho(Y, o.maxChars ?? Ro),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${Lo(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: G
    };
  } catch (f) {
    return {
      hits: d.slice(0, o.limit ?? Ue),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: qo(e.chunk.text, e.matchedTerms),
    score: e.score,
  ...
  for (const o of e) {
    if (n <= 0) break;
    const i = o.text.length > n ? `${o.text.slice(0, Math.max(0, n - 28))}
[...context clipped...]` : o.text;
    r.push({ ...o, text: i, charCount: i.length }), n -= i.length;
  }
  return r;
}, Gn = (e) => {
  ...
  let r = 0, o = 0, i = 0;
  for (let s = 0; s < n; s += 1)
    r += e[s] * t[s], o += e[s] * e[s], i += t[s] * t[s];
  return o === 0 || i === 0 ? 0 : r / (Math.sqrt(o) * Math.sqrt(i));
}, Wo = (e) => Math.min(1, Math.max(0, e / 24)), Uo = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Vo = (e, t) => {
  const n = `${e.path}
${e.name}
${e.language}
${e.text}`.toLowerCase();
  return t.reduce((r, o) => r + (n.includes(o) ? 1 : 0), 0);
}, qo = (e, t) => {
  const n = e.toLowerCase(), r = t.map((s) => n.indexOf(s)).filter((s) => s >= 0).sort((s, d) => s - d)[0] ?? 0, o = Math.max(0, r - 120), i = Math.min(e.length, r + 260);
  return `${o > 0 ? "…" : ""}${e.slice(o, i).replace(/\s+/g, " ").trim()}${i < e.length ? "…" : ""}`;
}, Ko = /```([^\n`]*)\n([\s\S]*?)```/g, Go = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function qt(e) {
  return Array.from(e.matchAll(Ko)).map((t) => Jn(t[1] ?? "", Zo(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Jn(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, o = [];
  for (const i of n) {
    const s = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? r[s[1].toLowerCase()] = ti(s[2]) : o.push(i.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: o, attrs: r, content: t };
}
function pe(e, t) {
  return (typeof e == "string" ? Jn(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Xn(e) {
  return [...qt(e).filter((r) => pe(r, "diff") || pe(r, "patch")).map((r) => r.content), e].filter((r, o, i) => Jo(r) && i.findIndex((s) => s === r) === o);
}
function Jo(e) {
...
`), r = [];
  if (n.forEach((s, d) => {
    (s.startsWith("diff --git ") || s.startsWith("--- ")) && r.push(d);
  }), r.length === 0) return [{ raw: t, ...Nt(t) }];
  const o = Array.from(new Set(r)).sort((s, d) => s - d), i = [];
  for (let s = 0; s < o.length; s += 1) {
    const d = o[s], f = o.find((h) => h > d && n[h].startsWith("diff --git ")) ?? n.length, b = n.slice(d, f).join(`
`);
    /^@@\s+-\d+/m.test(b) && i.push({ raw: b, ...Nt(b) });
  }
  return i.length > 0 ? i : [{ raw: t, ...Nt(t) }];
}
function Nt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), r = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), o = ve(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = ve(t?.[3] || t?.[4] || r?.[1] || r?.[2] || "") || null, s = i && i !== "/dev/null" ? i : o;
  return { path: s && s !== "/dev/null" ? s : null, oldPath: o, newPath: i };
}
function ve(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function Ot(e) {
  return qt(e).filter((t) => Yo(t)).map((t) => ({
    content: ei(t.content),
...
`).slice(0, 5);
  for (const r of n) {
    const o = r.match(/^\s*(?:\/\/|#|<!--)?\s*(?:atomek-)?(?:path|file)\s*:\s*([^\s>]+)\s*(?:-->)?\s*$/i);
    if (o) return ve(o[1]);
  }
  return null;
}
...
`), o = [];
  ...
  for (let d = 0; d < r.length; d += 1) {
    const m = r[d].match(Go);
    if (!m) continue;
    s = !0;
    const f = Number(m[1]), b = Math.max(0, f - 1);
    if (b < i) return null;
    for (o.push(...n.slice(i, b)), i = b, d += 1; d < r.length; d += 1) {
      const h = r[d];
      if (h.startsWith("@@ ")) {
        d -= 1;
        break;
...
function ni(e) {
  const { body: t, files: n, sourceTitle: r, activeFile: o = null, versions: i = {} } = e, s = /* @__PURE__ */ new Map(), d = [];
  for (const b of Xn(t))
    for (const h of Xo(b)) {
      if (!h.path) {
        if (o) {
          const M = Rt(o.content, h.raw);
          M && M !== o.content && s.set(o.id, tt({
            file: o,
            proposedContent: M,
            sourceTitle: r,
  ...
  for (const b of Ot(t)) {
    const h = b.path ? wn(n, b.path) : o ? { file: o, normalizedPatchPath: ve(o.path), confidence: "exact" } : null;
    if (!h) {
  ...
  if (s.size === 0 && o && !m) {
    const b = ai(t, o);
    b && b.content !== o.content && s.set(o.id, tt({
      file: o,
      proposedContent: b.content,
      sourceTitle: r,
      extractionLabel: b.label,
...
`), o = Math.max(n.length, r.length);
  let i = 0, s = 0, d = 0;
  for (let m = 0; m < o; m += 1)
    n[m] !== r[m] && (n[m] === void 0 ? i += 1 : r[m] === void 0 ? s += 1 : d += 1);
...
const it = (e) => typeof e == "object" && e !== null, ci = (e) => it(e.ai) ? e.ai : null, li = (e) => {
  if (!it(e)) return null;
  const t = e.id;
...
}, we = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(we) : it(e) ? Object.values(e).some(we) : !1, hi = async (e, t) => {
  const n = ci(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(di).filter((o) => !!o) : [];
}, ui = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : we(e.capability) || we(e.capabilities) || we(e.modality) || we(e.modalities) || we(e.task) || we(e.tasks) || we(e.type) || we(e.kind), Qn = (e) => typeof e == "object" && e !== null, mi = (e) => Qn(e.ai) ? e.ai : null, Zn = (e) => typeof mi(e)?.embedText == "function", pi = async (e, t) => Zn(e) ? (await hi(e, t)).filter(ui) : [], fi = (e) => Qn(e.ai) ? Zn(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", bi = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], gi = [
...
function vi(e) {
  const t = e.results.length > 0 ? e.results.map((r, o) => [
    `Check ${o + 1}: ${r.command}`,
    `Status: ${r.status}`,
    "Output:",
    Mi(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function xi(e) {
  ...
  for (const r of t) {
    ...
    const i = ta(r.path), s = Ai(e, i, o);
    if (!s) continue;
    const d = Object.keys(o.scripts).filter((f) => typeof o.scripts?.[f] == "string"), m = Ni(d);
    for (const f of m) {
      const b = `${s} run ${f}`;
      n.push({
        id: na(`${i}:${b}`),
...
function Ai(e, t, n) {
  ...
  for (const [r, o] of gi)
    if (e.some((i) => Ii(i.path) === r && ta(i.path) === t)) return o;
  return null;
}
function Ni(e) {
  const t = bi.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, o) => r.localeCompare(o));
  return [...t, ...n];
}
function Si(e) {
  const t = /* @__PURE__ */ new Set();
...
const Pi = Ua(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Ti = {
  id: "welcome",
  ...
  dirty: !1,
  source: "sample"
}, aa = "tytus.workspace.recent", ra = "tytus.workspace.layout", oa = "tytus.atomek.chatAiSettings", at = {
...
function St(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - Li), r = n < 1180, o = r ? 200 : 240, i = r ? 300 : 340, s = r ? 420 : 560, d = Math.max(o, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, d), f = Math.floor(n * (r ? 0.34 : 0.36)), b = n - m - s, h = Math.max(i, Math.min(r ? 500 : 640, f, b));
  return { primaryMin: o, primaryMax: d, secondaryMin: i, secondaryMax: h };
}
function Ei(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let r = 0, o = 0, i;
  for (; (i = n.exec(e)) !== null; ) {
    if (i.index > r) {
      const d = e.slice(r, i.index);
      d.trim() && t.push({ type: "markdown", body: d, key: `md-${o}` });
    }
    t.push({
      type: "code",
...
async function yn(e, t, n) {
  ...
  const o = await r.call(e, t, { create: !0 });
...
function zi({ host: e }) {
  const t = Re(null), [n, r] = v(0), o = ie(() => fs(), []), [i, s] = v("computer"), [d, m] = v(o.primaryVisible), [f, b] = v(o.primaryWidth), [h, k] = v("chat"), [N, M] = v(o.secondaryVisible), [H, X] = v(o.secondaryWidth), [G, te] = v(!1), [Y, T] = v("problems"), [B, Q] = v(o.markdownPreviewVisible), [V, A] = v(!1), [j, fe] = v(null), [C, Z] = v([]), [me, be] = v([]), [ee, ae] = v(null), [de, ge] = v(""), [$, x] = v({ lineNumber: 1, column: 1 }), [E, W] = v(null), [p, I] = v({}), [P, O] = v(Jr), [R, U] = v([]), [F, K] = v([]), [z, D] = v(null), [se, ce] = v(""), [pa, ct] = v(!1), [fa, ba] = v(""), [Gt, Ke] = v([]), [re, Me] = v(null), [Ce, ze] = v(null), [Jt, Xt] = v(!1), [Ae, ga] = v(() => bs()), [Yt, Fe] = v(null), [Pe, lt] = v(null), [dt, ht] = v(""), [ut, mt] = v(""), [Ge, Je] = v(""), [Xe, Qt] = v("failed"), [ka, Zt] = v(null), [wa, S] = v("Ready"), [je, ya] = v(() => ps()), he = me.map((c) => C.find((u) => u.id === c)).filter(Boolean), L = ee ? C.find((c) => c.id === ee) ?? null : null, en = ie(() => Gr({ files: C, openEditorIds: me, activeFileId: ee, versions: p, activeSelection: E }), [ee, E, p, C, me]), tn = ie(() => ({
    scope: P,
    removedAttachmentIds: R,
    selectedFileIds: []
  }), [P, R]), De = ie(() => io(en, C, tn), [tn, en, C]), xe = Po(C, { autoRefresh: !0, includeDirty: !0 }), nn = ie(() => F.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [F]), va = ie(() => [...De.attachments, ...nn], [De.attachments, nn]), _ = Vr({ host: e, requestContext: De.parts, chatSettings: Ae, setStatus: S }), Ne = ie(
    () => [..._.artifacts, ...Gt].sort((c, u) => u.createdAt - c.createdAt),
    [_.artifacts, Gt]
  ), Ye = Jt && !L, pt = !L && !Ye && !V, le = ie(() => C.filter((c) => c.dirty), [C]), Ca = ie(() => {
    const c = de.trim().toLowerCase();
    return c ? C.filter((u) => u.path.toLowerCase().includes(c)) : C;
  }, [C, de]);
  ne(() => {
    I((c) => {
      const u = {};
      for (const g of C) u[g.id] = c[g.id] ?? 1;
      return u;
    });
  }, [C]), ne(() => {
    U([]), K([]);
  ...
  const xa = w((c) => {
    if (c.kind === "index-hit") {
      K((u) => u.filter((g) => g.id !== c.id)), S(`Removed project context: ${c.label}`);
      return;
    }
    U((u) => u.includes(c.id) ? u : [...u, c.id]), S(`Removed chat context: ${c.label}`);
  }, []), Qe = w(() => {
    Xt(!0), ae(null), A(!0), S("Atomek settings opened");
  }, []), an = w(() => {
    Xt(!1), ae((c) => c ?? me.at(-1) ?? null), me.length === 0 && A(!1), S("Atomek settings closed");
  }, [me]), Te = w((c) => {
    I((u) => ({ ...u, [c]: (u[c] ?? 1) + 1 }));
  }, []), $a = w((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = St(n), g = c.clientX, y = H, J = (Be) => {
      const xt = y + (g - Be.clientX);
      X(nt(xt, u.secondaryMin, u.secondaryMax));
    }, ke = () => {
      window.removeEventListener("pointermove", J), window.removeEventListener("pointerup", ke);
    };
    window.addEventListener("pointermove", J), window.addEventListener("pointerup", ke);
  }, [H, n]), Aa = w((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = St(n), g = c.clientX, y = f, J = (Be) => {
      const xt = y + (Be.clientX - g);
      b(nt(xt, u.primaryMin, u.primaryMax));
    }, ke = () => {
      window.removeEventListener("pointermove", J), window.removeEventListener("pointerup", ke);
    };
    window.addEventListener("pointermove", J), window.addEventListener("pointerup", ke);
    ...
    ya(u), localStorage.setItem(aa, JSON.stringify(u));
  }, [je]), oe = w((c, u) => {
    be((g) => g.includes(c.id) ? g : [...g, c.id]), ae(c.id), A(!1), Zt(u ?? null), x({ lineNumber: u ?? 1, column: 1 });
  }, []), Na = w((c) => {
    if (!c.fileId) return;
    const u = C.find((g) => g.id === c.fileId);
    u && (oe(u, c.range?.startLineNumber ?? 1), S(`Revealed context: ${c.label}`));
  }, [C, oe]), _e = w(async () => {
    if (It(le, "open new files"))
      try {
        const c = await zr();
        if (c.length === 0) return;
        Z((u) => hs(u, c)), c.forEach((u) => Ze({ name: u.name, path: u.path, at: Date.now() })), oe(c[0]), S(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && S(`Open file failed: ${c.message}`);
      }
  }, [le, oe, Ze]), ft = w(async () => {
    if (It(le, "open another folder"))
      try {
        const c = await Fr();
        fe(c), Z(c.files), be([]), ae(null), A(!1), Ze({ name: c.name, path: c.name, at: Date.now() }), S(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
        c.name !== "AbortError" && S(`Open folder failed: ${c.message}`);
      }
  }, [le, Ze]), Sa = w((c) => {
    if (!ee) return;
    let u = !1;
    Z((g) => g.map((y) => y.id !== ee || y.content === c ? y : (u = !0, { ...y, content: c, dirty: !0 }))), u && Te(ee);
  }, [ee, Te]), bt = w(async () => {
    if (L)
      try {
        const c = await $t(L);
        Z((u) => u.map((g) => g.id === c.id ? c : g)), S(`Saved ${c.name}`);
      } catch (c) {
        S(`Save failed: ${c.message}`);
      }
  }, [L]), Ia = w(async (c) => {
    const u = C.find((y) => y.id === c);
    if (!u) return null;
    const g = await $t(u);
    return Z((y) => y.map((J) => J.id === g.id ? g : J)), g;
  }, [C]), rn = w(async () => {
    const c = C.filter((u) => u.dirty);
    if (c.length === 0) {
      S("No dirty files to save");
      return;
    }
    try {
      const u = await Promise.all(c.map((y) => $t(y))), g = new Map(u.map((y) => [y.id, y]));
      Z((y) => y.map((J) => g.get(J.id) ?? J)), Fe(null), S(`Saved ${u.length} dirty file${u.length === 1 ? "" : "s"}`);
    } catch (u) {
      S(`Save all failed: ${u.message}`);
    }
  }, [C]), gt = w((c) => {
    const u = C.find((g) => g.id === c);
    if (u?.dirty && !window.confirm(`${u.name} has unsaved changes. Close without saving?`)) {
      S(`Close canceled — ${u.name} has unsaved changes`);
      return;
    }
    be((g) => {
      const y = g.filter((J) => J !== c);
      return ee === c && (ae(y.at(-1) ?? null), W(null)), y;
    });
  }, [ee, C]), Ma = w(() => {
    It(le, "close all editors") && (be([]), ae(null), Zt(null), A(!1), S("Closed all editors"));
  }, [le]), kt = w(() => {
    const c = C.filter((g) => g.name.startsWith("Untitled")).length + 1, u = {
...
`,
    ...
    return K(y), y.length === 0 ? (S(xe.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), u) : (g.mode !== "hybrid" && g.reason ? S(g.reason) : S(`${y.length} project context hit${y.length === 1 ? "" : "s"} · ${g.reason ?? "hybrid retrieval"}`), [
      ...u,
      ...jo(y)
    ]);
  }, [De.parts, P, Ae, e, xe]), wt = w(() => {
    if (!L && he.length === 0) {
      S("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const u = [
      `Create a polished Markdown artifact from ${L?.path ?? `${he.length} open editors`}.`,
    ...
    M(!0), k("chat"), S("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const g = await $e(u), y = await _.askAgent(u, { requestContext: g });
      !y || y.status === "error" || _.createArtifact({
        messageId: y.id,
        title: `AI synthesis — ${L?.name ?? "open workspace"}`,
        kind: "markdown",
        body: y.body
      }).then(() => {
        M(!0), k("outputs"), te(!0), T("output");
      });
    })();
  }, [L, _, $e, he.length]), yt = w((c) => {
    const u = C.find((g) => g.path === c.path || g.name === c.name);
    if (u) {
      oe(u), S(`Opened recent ${u.name}`);
      return;
    }
    S("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [C, oe]), ue = w((c) => {
    M(!0), k("chat"), ce(""), (async () => {
      const u = await $e(c);
      await _.askAgent(c, { requestContext: u });
    })();
  }, [_, $e]), on = w(async (c) => {
    if (!e.skills?.get) {
...
[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : u.body, y = [
...
`);
      ce((J) => [J.trim(), y].filter(Boolean).join(`

`)), M(!0), k("chat"), S(`Attached skill ${u.title} to chat input`);
    } catch (u) {
      S(`Failed to attach skill: ${u instanceof Error ? u.message : String(u)}`);
    }
  }, [e.skills]), Pa = w((c) => {
    const u = _.messages.findIndex((y) => y.id === c.id), g = _.messages.slice(0, u < 0 ? void 0 : u).reverse().find((y) => y.role === "user");
    if (!g?.body.trim()) {
      S("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const y = await $e(g.body);
      await _.askAgent(g.body, { requestContext: y });
    })();
  }, [_, $e]), Ta = w((c) => {
    _.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const u = Pt(C, Mt(c.title || c.kind)), g = {
    ...
    Z((y) => [...y, g]), oe(g), S(`Opened ${c.title} as editable file`);
  }, [C, oe]), Ct = w(() => {
...
`), J = {
    ...
    Ke((ke) => [J, ...ke]), te(!0), T("output"), S(`Captured manual check: ${u}`);
  }, []), Le = w((c, u) => {
    const g = ni({
      body: u,
      files: C,
    ...
    return g.edits.length > 1 ? (ze(y), Me(null), S(`Previewing AI workspace patch for ${g.edits.length} files`), !0) : g.edits.length === 1 ? (Me(y.edits[0]), ze(null), S(`Previewing AI patch for ${y.edits[0].fileName}`), !0) : (S(g.skipped.length > 0 ? `No applicable edit found. ${g.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [L, p, C]), sn = w((c, u) => {
    const g = {
    ...
    Ke((y) => [g, ...y]), M(!0), k("outputs"), te(!0), T("output"), S(`Saved local job output: ${c}`), (u.includes("```diff") || u.includes("--- a/")) && Le(c, u);
  }, [Le]), Oa = w(() => {
    const c = se.trim();
    if (!c) return;
    ce("");
    const u = ts(c);
    (async () => {
      const g = await $e(c), y = await _.askAgent(c, { requestContext: g });
      if (!u || !y || y.status === "error") return;
      Le(y.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", y.body) || S("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [_, $e, se, Le]), Ra = w(() => {
    z && (D(null), ue(ns(z)));
  }, [ue, z]), He = w((c) => {
    ...
    Je(g), ht(""), mt(""), Qt("failed"), te(!0), T("terminal"), S(u.commands.length > 0 ? `Manual check ready: copy ${u.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [C]), za = w((c) => {
    c.trim() && (st(c), Je(c), S(`Manual check command copied: ${c}`));
  }, []), Fa = w(() => {
    lt((c) => {
      if (!c) return c;
      const u = wi(c, dt), g = u.commands.at(-1)?.command ?? "";
      return g && Je(g), u;
    }), ht("");
  }, [dt]), ja = w(() => {
    if (!Pe || !Ge.trim()) {
    ...
    const c = Ge.trim();
    lt((u) => u && yi(u, c, Xe, ut)), mt(""), S(`Captured manual check result: ${c} (${Xe})`);
  }, [ut, Ge, Pe, Xe]), Da = w(() => {
    Pe && ue(vi(Pe));
  }, [ue, Pe]), _a = w(() => {
    if (!re) return;
    const c = C.find((u) => u.id === re.fileId);
    if (!c) {
      S(`Cannot apply edit — ${re.fileName} is no longer open`), Me(null);
      return;
    }
    c.content !== re.originalContent && !window.confirm(`${re.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (Z((u) => u.map((g) => g.id === re.fileId ? { ...g, content: re.proposedContent, dirty: !0 } : g)), Te(re.fileId), Fe(`AI edit applied to ${c.name}. Save All to persist it to disk.`), Me(null), He(`AI edit applied to ${c.name}`), S(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Te, C, re, He]), Ha = w(() => {
    if (!re) return;
    const c = Pt(C, `${Mt(re.fileName)}-ai-edit`), u = {
      id: `pending-edit-${re.fileId}-${Date.now()}`,
      name: c,
      path: c,
      language: L?.language ?? "markdown",
      content: re.proposedContent,
      dirty: !0,
      source: "generated"
    };
    Z((g) => [...g, u]), oe(u), Me(null), S(`Opened proposed edit as ${c}`);
  }, [L?.language, C, oe, re]), Ba = w(() => {
    if (!Ce) return;
    const c = new Map(Ce.edits.map((g) => [g.fileId, g]));
    C.some((g) => {
      const y = c.get(g.id);
      return y && g.content !== y.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (Z((g) => g.map((y) => {
      const J = c.get(y.id);
      return J ? { ...y, content: J.proposedContent, dirty: !0 } : y;
    })), c.forEach((g, y) => Te(y)), Fe(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), ze(null), He(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), S(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Te, C, Ce, He]), Wa = w(() => {
    if (!Ce) return;
    const c = [...C], u = Ce.edits.map((g) => {
      const y = Pt(c, `${Mt(g.fileName)}-ai-edit`), J = C.find((Be) => Be.id === g.fileId), ke = {
        id: `workspace-patch-${g.fileId}-${Date.now()}-${y}`,
        name: y,
        path: y,
    ...
    Z((g) => [...g, ...u]), u[0] && oe(u[0]), ze(null), S(`Opened ${u.length} proposed edit file${u.length === 1 ? "" : "s"}`);
  }, [C, oe, Ce]), cn = w((c) => {
    if (c === "explain") {
  ...
  return ne(() => {
    const c = (u) => {
      (u.metaKey || u.ctrlKey) && (u.key.toLowerCase() === "o" && (u.preventDefault(), _e()), u.key.toLowerCase() === "s" && (u.preventDefault(), bt()), u.key.toLowerCase() === "w" && (u.preventDefault(), ee && gt(ee)), u.key.toLowerCase() === "b" && (u.preventDefault(), m((y) => !y)), u.shiftKey && u.key.toLowerCase() === "f" && (u.preventDefault(), s("search"), m(!0)), (u.key.toLowerCase() === "k" || u.key.toLowerCase() === "p") && (u.preventDefault(), ct(!0)));
    };
    ...
    const c = t.current;
    if (!c) return;
    const u = () => r(Math.round(c.getBoundingClientRect().width));
    u();
    ...
    if (!n) return;
    const c = St(n);
    b((u) => nt(u, c.primaryMin, c.primaryMax)), X((u) => nt(u, c.secondaryMin, c.secondaryMax));
  }, [n]), ne(() => {
    const c = { primaryVisible: d, primaryWidth: f, secondaryVisible: N, secondaryWidth: H, markdownPreviewVisible: B };
    localStorage.setItem(ra, JSON.stringify(c));
  }, [B, d, f, N, H]), ne(() => {
    localStorage.setItem(oa, JSON.stringify(Ae));
  }, [Ae]), /* @__PURE__ */ l(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${d ? "" : "no-primary"} ${N ? "" : "no-secondary"} ${G ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${f}px`, "--workbench-secondary-width": `${H}px` },
      children: [
        /* @__PURE__ */ a(Fi, { active: i, setActive: (c) => {
          s(c), m(!0);
        }, openSettings: Qe, settingsActive: Ye }),
        d && /* @__PURE__ */ l("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            ji,
            {
              ...
              openFile: _e,
              openFolder: ft,
              openWorkbenchFile: oe,
              newFile: kt,
              recent: je,
              reopenRecent: yt,
              setStatus: S,
              hasFsAccess: Rr(),
              attachSkillToChat: on,
              saveLocalJobOutput: sn,
              activeFile: L
            }
          ),
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: Aa, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ l("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => ct(!0), children: "Workspace" }),
          /* @__PURE__ */ l("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ a(Hi, { file: L, folder: j, showWelcome: pt }),
            Yt && le.length > 0 ? /* @__PURE__ */ l("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: Yt }),
              /* @__PURE__ */ a("button", { onClick: () => {
                rn();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => Fe(null), title: "Dismiss", children: /* @__PURE__ */ a(ye, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: L ? /* @__PURE__ */ l("div", { className: L.language === "markdown" && B ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(Va, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                Pi,
                {
                  file: L,
                  revealLine: ka,
              ...
              L.language === "markdown" && B && /* @__PURE__ */ a(Ji, { content: L.content })
            ] }) : Ye ? /* @__PURE__ */ a(
              as,
              {
                host: e,
            ...
            ) : pt ? /* @__PURE__ */ a(Vi, { host: e, openFile: _e, openFolder: ft, newFile: kt, recent: je, reopenRecent: yt, setStatus: S, openControlTower: () => {
              s("computer"), m(!0);
            }, openChat: () => {
              k("chat"), M(!0);
            } }) : /* @__PURE__ */ l("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(ot, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => A(!1), children: "Show Mission Control" })
            ] }) }),
            G && /* @__PURE__ */ a(
              Yi,
              {
                ...
                manualCheckSelectedCommand: Ge,
                setManualCheckSelectedCommand: Je,
                manualCheckStatus: Xe,
                setManualCheckStatus: Qt,
                copyManualCheckCommand: za,
                addManualCheckCommand: Fa,
                recordManualCheckResult: ja,
                askAgentFromManualChecks: Da,
                onClose: () => te(!1)
              }
            )
          ] })
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            ...
            contextScope: P,
            setContextScope: O,
            contextAttachments: va,
            removeContextAttachment: xa,
            revealContextAttachment: Na,
            projectIndexSummary: `${xe.snapshot.files.length} files · ${xe.snapshot.chunks.length} chunks`,
            projectIndexStale: xe.isStale,
            refreshProjectIndex: () => {
              const c = xe.refresh(C);
              S(`Project index refreshed: ${c.files.length} files · ${c.chunks.length} chunks`);
            },
            onResizeStart: $a,
            onClose: () => M(!1)
          }
        ...
        pa && /* @__PURE__ */ a(
          Gi,
          {
            ...
            commands: [
              ...
              { label: "File: Close All Editors", detail: `${he.length} open editor${he.length === 1 ? "" : "s"}`, run: Ma, disabled: he.length === 0 },
              ...
              { label: "Help: Show Mission Control", detail: "Open the Atomek control tower landing page", run: () => {
                ae(null), A(!1), s("computer"), m(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: d ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => m((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: N ? "Hide right AI side bar" : "Show right AI side bar", run: () => M((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: G ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => te((c) => !c) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Qe },
              ...
              { label: "Atomek: Create AI Synthesis", detail: L || he.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: wt, disabled: !L && he.length === 0 },
              { label: "AI: Explain Active File", detail: L ? `Ask Cortex to explain ${L.path}` : "Open a file first", run: () => ue("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !L },
              { label: "AI: Improve Active File", detail: L ? `Ask Cortex for concrete edits to ${L.path}` : "Open a file first", run: () => ue("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !L },
              { label: "AI: Draft Editable Replacement", detail: L ? `Ask Cortex for a full-file replacement for ${L.path}` : "Open a file first", run: () => cn("edit"), disabled: !L },
              ...
              { label: "AI: Save Active File as Artifact", detail: L ? "Persist active file in host.ai artifacts" : "Open a file first", run: Ea, disabled: !L },
        ...
        /* @__PURE__ */ a(ds, { status: wa, file: L ?? Ti, cursor: $, fileCount: C.length, dirtyCount: le.length })
      ]
    }
  );
}
function Fi({ active: e, setActive: t, openSettings: n, settingsActive: r }) {
  return /* @__PURE__ */ l("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a(Pn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a($r, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a(Ln, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a(In, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a(jt, { size: 25 }), label: "Control Tower", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a(ar, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(Se, { icon: /* @__PURE__ */ a(Ir, { size: 23 }), label: "Settings", active: r, onClick: n })
  ] });
}
function Se({ icon: e, label: t, active: n, onClick: r }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: r, children: e });
}
function ji(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(qi, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(An, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(An, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(ma, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(Di, { ...e });
}
function Di(e) {
  const t = !e.folder, n = ie(() => _i(e.files, e.folder?.name), [e.files, e.folder?.name]), [r, o] = v(() => /* @__PURE__ */ new Set());
  ne(() => {
    const s = ca(n);
    o((d) => {
      const m = new Set(d);
    ...
    /* @__PURE__ */ l("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ l(qe, { children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(rt, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((s) => /* @__PURE__ */ a(Kt, { file: s, active: s.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(s), label: s.name, detail: s.path }, s.id)),
        /* @__PURE__ */ l("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(rt, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : la(n, e.activeFileId, e.openWorkbenchFile, r, i)
      ] }),
      ...
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((s) => /* @__PURE__ */ l("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(s), children: [
        /* @__PURE__ */ a(Tn, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: s.name })
      ] }, `${s.path}-${s.at}`))
    ] })
  ] });
}
function Kt({ file: e, active: t, onOpen: n, basePath: r, depth: o = 0, label: i, detail: s }) {
  const d = r && e.path.startsWith(`${r}/`) ? e.path.slice(r.length + 1) : e.path, m = o || Math.max(0, d.split("/").length - 1);
  return /* @__PURE__ */ l("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": m }, onClick: n, title: e.path, children: [
...
function _i(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), o = (s, d, m = n) => {
    const f = r.get(s);
    if (f) return f;
    const b = { name: d, path: s, children: [] };
    return r.set(s, b), m.push(b), b;
  };
  e.forEach((s) => {
    const d = t && s.path.startsWith(`${t}/`) ? s.path.slice(t.length + 1) : s.path, m = d.split("/").filter(Boolean);
    let f = n, b = "";
    m.slice(0, -1).forEach((h) => {
      b = b ? `${b}/${h}` : h, f = o(b, h, f).children;
    }), f.push({ name: m.at(-1) ?? s.name, path: d, file: s, children: [] });
  });
  const i = (s) => s.sort((d, m) => +!!d.file - +!!m.file || d.name.localeCompare(m.name)).map((d) => ({ ...d, children: i(d.children) }));
  return i(n);
}
function ca(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...ca(t.children)]);
}
function la(e, t, n, r, o, i = 0) {
  return e.map((s) => {
    if (s.file)
      return /* @__PURE__ */ a(Kt, { file: s.file, active: s.file.id === t, onOpen: () => n(s.file), depth: i, label: s.name }, s.file.id);
    const d = r.has(s.path);
    return /* @__PURE__ */ l("div", { children: [
      /* @__PURE__ */ l("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => o(s.path), title: d ? `Collapse ${s.name}` : `Expand ${s.name}`, children: [
        d ? /* @__PURE__ */ a(rt, { size: 12 }) : /* @__PURE__ */ a(rt, { className: "workbench-chevron-collapsed", size: 12 }),
        d ? /* @__PURE__ */ a(Ht, { size: 14 }) : /* @__PURE__ */ a(Tn, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: s.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: s.children.length })
      ] }),
      d ? la(s.children, t, n, r, o, i + 1) : null
    ] }, s.path);
  });
}
function Hi({ file: e, folder: t, showWelcome: n }) {
  const r = n ? ["Mission Control"] : e?.path.split("/").filter(Boolean) ?? [], o = t && r[0] === t.name ? r.slice(1) : r;
  return /* @__PURE__ */ a("div", { className: "workbench-breadcrumb", children: o.length === 0 ? /* @__PURE__ */ a("span", { children: "Workspace" }) : o.map((i, s) => /* @__PURE__ */ l("span", { className: "workbench-breadcrumb-part", children: [
...
function Bi(e) {
  return /* @__PURE__ */ l("div", { className: "workbench-tabs", children: [
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ a(Dt, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(kr, { size: 16 }) })
  ] });
}
function Wi(e) {
  const t = e?.resources ?? [], n = (o) => t.filter((i) => i.kind === o).length, r = t.filter((o) => o.status === "ready" || o.status === "available").length;
  return [
...
function Ui(e) {
  ...
  return [...e?.resources ?? []].sort((n, r) => (t[n.kind] ?? 9) - (t[r.kind] ?? 9) || n.label.localeCompare(r.label)).slice(0, 8);
}
function Vi({
  host: e,
  openFile: t,
  ...
  const [f, b] = v("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [h, k] = v(null), [N, M] = v(!1), [H, X] = v(null), [G, te] = v(null), Y = w(async () => {
    M(!0), te(null);
    try {
      const A = await e.resources?.list?.();
      k(A ?? null), A && s(`Control Tower loaded · ${A.resources.length} resources · ${A.warnings.length} warnings`);
    } catch (A) {
      const j = A instanceof Error ? A.message : String(A);
      te(j), s(`Control Tower resource load failed: ${j}`);
    } finally {
      M(!1);
    }
  }, [e.resources, s]);
  ...
  const T = w(async () => {
    ...
    const A = f.trim() || "Coordinate a Tytus mission.";
    M(!0);
    try {
      const j = await e.missions.create({
        ...
        goal: j.goal,
        rootPath: j.rootPath,
        name: j.rootPath.split("/").pop() || j.missionId,
        source: "tray"
      }, C = {
      ...
      await e.missions.write({
        rootPath: j.rootPath,
        files: [
          { path: "MISSION.md", content: zt(fe, h, null, [], A) },
          { path: "MISSION.json", content: sa(fe, h, A) },
          { path: "RESOURCES.md", content: Ft(h) },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(C)}
` },
...
`) }
      ...
      }), X(j), s(`Mission created: ${j.rootPath}`), d();
    } catch (j) {
      s(`Mission create failed: ${j instanceof Error ? j.message : String(j)}`);
    } finally {
      M(!1);
    }
  }, [f, h, e.missions, d, s]), B = Wi(h), Q = Ui(h), V = h?.warnings ?? [];
  return /* @__PURE__ */ l("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ l("section", { className: "workbench-control-hero-main", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-control-goal-row", children: [
        /* @__PURE__ */ a("textarea", { value: f, onChange: (A) => b(A.target.value), rows: 3, "aria-label": "Mission goal" }),
        /* @__PURE__ */ l("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            T();
          }, disabled: N, children: "Start mission" }),
      ...
      H ? /* @__PURE__ */ l("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: H.rootPath })
      ] }) : null,
      G ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: G }) : null
    ] }),
    /* @__PURE__ */ l("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ l("header", { children: [
          ...
          /* @__PURE__ */ l("button", { className: "workbench-button-subtle", onClick: () => {
            ...
            /* @__PURE__ */ a(Ve, { size: 13 }),
            " ",
            N ? "Refreshing…" : "Refresh"
          ] })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: B.map((A) => /* @__PURE__ */ l("div", { className: "workbench-control-metric", children: [
          /* @__PURE__ */ a("strong", { children: A.value }),
          /* @__PURE__ */ a("span", { children: A.label }),
          /* @__PURE__ */ a("em", { children: A.detail })
        ] }, A.label)) })
      ] }),
      ...
      /* @__PURE__ */ l("article", { className: "workbench-control-card", children: [
        ...
        /* @__PURE__ */ l("button", { className: "workbench-start-link", onClick: r, children: [
          /* @__PURE__ */ a(_t, { size: 18 }),
          "New mission note"
        ] }),
        o.length ? /* @__PURE__ */ l("div", { className: "workbench-control-recent", children: [
          /* @__PURE__ */ a("span", { children: "Recent" }),
          o.slice(0, 3).map((A) => /* @__PURE__ */ a("button", { onClick: () => i(A), children: A.name }, `${A.path}-${A.at}`))
        ] }) : null
      ] }),
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ l("header", { children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-control-resource-list", children: [
          Q.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          Q.map((A) => /* @__PURE__ */ l("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ l("div", { children: [
              ...
              /* @__PURE__ */ l("span", { children: [
                ...
                A.trustTier,
                " · ",
                A.capabilities.slice(0, 3).join(", ") || "status only"
              ] })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${A.status}`, children: A.status })
          ] }, A.id))
        ] }),
        V.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: V.slice(0, 2).map((A) => /* @__PURE__ */ l("span", { children: [
          A.code,
          ": ",
          A.message
        ] }, `${A.code}-${A.resourceId ?? A.message}`)) }) : null
      ] }),
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
...
function qi({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: o }) {
  const i = ie(() => Ki(e, t), [e, t]), s = ie(() => {
    const d = /* @__PURE__ */ new Map();
    return i.forEach((m) => d.set(m.file.id, [...d.get(m.file.id) ?? [], m])), Array.from(d.values()).slice(0, 50);
  }, [i]);
  return /* @__PURE__ */ l("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? s.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : s.map((d) => {
        ...
        return /* @__PURE__ */ l("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(Kt, { file: m, active: m.id === o, onOpen: () => r(m) }),
          d.slice(0, 5).map((f) => /* @__PURE__ */ l("button", { className: "workbench-search-hit", onClick: () => r(m, f.lineNumber), title: f.line, children: [
...
function Ki(e, t) {
  ...
  const r = [];
  return e.forEach((o) => {
    o.path.toLowerCase().includes(n) && r.push({ file: o, lineNumber: 1, line: o.path }), o.content.split(`
`).some((i, s) => i.toLowerCase().includes(n) ? (r.push({ file: o, lineNumber: s + 1, line: i.trim() || "(blank line)" }), r.length >= 200) : !1);
  }), r.slice(0, 200);
}
function Gi(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${Rn(o.language)} file`,
    run: () => e.openWorkbenchFile(o)
  })), r = [...e.commands, ...n].filter((o) => !t || o.label.toLowerCase().includes(t) || o.detail.toLowerCase().includes(t)).slice(0, 12);
  return ne(() => {
    const o = (i) => {
    ...
    /* @__PURE__ */ a(
      "input",
      {
        ...
        onKeyDown: (o) => {
          o.key === "Enter" && r[0] && !r[0].disabled && (r[0].run(), e.onClose());
        }
      }
    ),
    /* @__PURE__ */ a("div", { className: "workbench-command-list", children: r.map((o) => /* @__PURE__ */ l(
      "button",
      {
        disabled: o.disabled,
...
function Xi(e) {
  const t = e.session ? Ci(e.session) : "pending";
  return /* @__PURE__ */ l("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ l(qe, { children: [
      ...
      e.session.results.length > 0 ? /* @__PURE__ */ l("div", { className: "workbench-check-results", children: [
        ...
        e.session.results.map((n, r) => /* @__PURE__ */ l("article", { children: [
...
function Yi(e) {
  return /* @__PURE__ */ l("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(ua, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Qi(e) {
  return /* @__PURE__ */ l("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(vr, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(ir, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(ye, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(Zi, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      ma,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ a(ua, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function da({ body: e }) {
  const t = ie(() => Ei(e), [e]), [n, r] = v(null), o = w((i, s) => {
    (async () => await st(s) && (r(i), window.setTimeout(() => r((m) => m === i ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((i) => {
    if (i.type === "code") {
      ...
      return /* @__PURE__ */ l("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ l("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ l("button", { onClick: () => o(i.key, i.body), title: "Copy code block", children: [
            s ? /* @__PURE__ */ a(er, { size: 12 }) : /* @__PURE__ */ a(Lt, { size: 12 }),
            s ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Hn(i.body) }
      },
      i.key
    );
  }) });
...
function Zi(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Re(null), [r, o] = v(!0), [i, s] = v(!1), d = ie(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  ne(() => {
    const h = n.current;
    h && (r ? (h.scrollTop = h.scrollHeight, s(!1)) : s(!0));
  }, [r, d]);
  const m = w(() => {
    const h = n.current;
    if (!h) return;
  ...
  return /* @__PURE__ */ l("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ l("option", { value: h.id, children: [
              h.title,
              " · ",
              es(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ l("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ l("div", { children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(da, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ l(qe, { children: [
          /* @__PURE__ */ a("br", {}),
    ...
    /* @__PURE__ */ l("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ l("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: Xr(e.contextScope) }),
        /* @__PURE__ */ a("em", { children: Cn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ l("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ l("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ l(qe, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ l("span", { className: "workbench-chat-chip muted", children: [
            ...
            const k = typeof h.score == "number" ? h.score.toFixed(2) : null, N = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, M = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, H = [
              h.path,
            ...
            return /* @__PURE__ */ l("span", { className: "workbench-chat-chip", title: H, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ a(dn, { size: 13 }) }),
              h.label,
              k ? /* @__PURE__ */ a("small", { children: k }) : null,
              h.snippet ? /* @__PURE__ */ l("small", { children: [
                h.snippet.slice(0, 60),
                h.snippet.length > 60 ? "…" : ""
              ] }) : null,
              h.dirty ? /* @__PURE__ */ a("small", { children: "dirty" }) : null,
              h.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(h), title: "Remove context", children: /* @__PURE__ */ a(ye, { size: 11 }) }) : null
            ] }, h.id);
          }),
        ...
        /* @__PURE__ */ l("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: Cn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(Tr, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(Nr, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Cn(e, t, n) {
  const r = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : ha(e.gatewayPreference)];
...
function as(e) {
  const [t, n] = v([]), [r, o] = v("Loading gateway models…"), [i, s] = v([]), [d, m] = v("Checking embedding capability…");
  ne(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(M.map((H) => ({ id: H.id, gatewayLabel: H.gatewayLabel }))), o(M.length > 0 ? `${M.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (M) {
        if (k.signal.aborted) return;
        n([]), o(`Model discovery failed: ${M instanceof Error ? M.message : String(M)}`);
      }
    })(), () => k.abort();
    ...
    return (async () => {
      ...
      try {
        const H = await pi(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: k.signal
        });
        if (k.signal.aborted) return;
        s(H.map((X) => ({ id: X.id, gatewayLabel: X.gatewayLabel ?? X.source ?? "AIL" }))), m(H.length > 0 ? `${H.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (H) {
        if (k.signal.aborted) return;
        s([]), m(`Embedding model discovery failed: ${H instanceof Error ? H.message : String(H)}`);
      }
    })(), () => k.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ l("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ l("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ l("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-chat-models", children: t.map((k) => /* @__PURE__ */ a("option", { value: k.id, children: k.gatewayLabel }, `${k.gatewayLabel}:${k.id}`)) })
        ] }),
        /* @__PURE__ */ l("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          ha(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ l("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-embedding-models", children: i.map((k) => /* @__PURE__ */ a("option", { value: k.id, children: k.gatewayLabel }, `${k.gatewayLabel}:${k.id}`)) })
        ] }),
    ...
    /* @__PURE__ */ l("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(at), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function ua({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: o, openOutputAsFile: i, previewEditFromOutput: s, canPreviewEdit: d = !1, compact: m = !1 }) {
  return /* @__PURE__ */ l("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ l("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ l("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void st(f.body), children: "Copy" }),
      ...
      /* @__PURE__ */ a(da, { body: f.body })
    ] }, f.id))
  ] });
}
function rs({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
...
function os({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ l("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ l("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: ms(i.proposedContent) })
      ] }, i.fileId)),
...
function ss(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, is(e.content), "```");
  else if (t.length > 0) {
...
function ma({
  ...
  const [d, m] = v([]), [f, b] = v([]), [h, k] = v(null), [N, M] = v(!1), [H, X] = v(null), [G, te] = v("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [Y, T] = v(null), [B, Q] = v([]), [V, A] = v(null), [j, fe] = v([]), C = B.find((p) => p.status === "running") ?? B[0] ?? null, Z = s === "dock", me = i.filter((p) => p.dirty).length, be = o ? `${o.path} · ${o.language} · ${o.content.length.toLocaleString()} chars${o.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${me ? ` · ${me} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", ee = ie(() => [
    {
    ...
    try {
      ...
      m(p), b(I), k(P), t(`Computer loaded · ${p.length} tools · ${I.length} skills${P ? ` · ${P.resources.length} resources` : ""}`);
    } catch (p) {
  ...
  const de = w(async (p, I, P = []) => {
    const O = [
      ...j,
      ...P,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], R = [
      { path: "MISSION.md", content: zt(p, h, o, i, I) },
      { path: "MISSION.json", content: sa(p, h, I) },
      { path: "RESOURCES.md", content: Ft(h) },
      { path: "AUDIT.jsonl", content: O.map((U) => JSON.stringify(U)).join(`
`) + `
` }
    ];
    if (p.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: p.rootPath, files: R });
    else if (p.handle) {
      await vn(p.handle, "runs");
      for (const U of R) await yn(p.handle, U.path, U.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    fe(O);
  }, [o, e.missions, j, i, h]), ge = w(async (p, I = {}) => {
    if (V)
      return await de(V, p), V;
    const P = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, O = p.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let R = null;
    if (e.missions?.create) {
      const F = await e.missions.create({ title: P, goal: O });
      R = {
      ...
      R = {
        handle: F,
        name: F.name,
        missionId: `mission-${Date.now()}-${Ri(F.name)}`,
        title: P,
        goal: O,
        source: "browser"
      };
    ...
    if (!R) return null;
    const U = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${R.rootPath ?? R.name}` };
    return A(R), fe([U]), await de(R, O, [U]), t(`Mission pack ready in ${R.rootPath ?? R.name}`), R;
  }, [e.missions, V, t, de]), $ = w(async (p) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const I = G.trim() || `Open ${p.label} from Atomek with current context.`, P = p.kind === "ai-cli" ? await ge(I) : V;
      await e.local.openTerminal({
        toolId: p.id,
        command: p.command,
        cwd: P?.rootPath ?? $n(o),
        prompt: P ? `Atomek mission pack ready at ${P.rootPath ?? P.name}. Read MISSION.md and RESOURCES.md. ${be}` : `Opened from Atomek. ${be}`
      ...
      t(`Terminal launch failed: ${I instanceof Error ? I.message : String(I)}`);
    }
  }, [o, be, ge, e.local, G, V, t]), x = w(async () => {
    try {
      await ge(G.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (p) {
      t(`Mission folder setup failed: ${p instanceof Error ? p.message : String(p)}`);
    }
  }, [ge, G, t]), E = w(async (p, I, P, O = V) => {
    if (!O) return;
    const U = `${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${p.id}.md`, F = `runs/${U}`;
    if (O.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: O.rootPath, files: [{ path: F, content: I }] });
    else if (O.handle) {
      const K = await vn(O.handle, "runs");
      await yn(K, U, I);
    }
    await de(O, G, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${p.label} exited ${P}; transcript saved to ${F}`,
      data: { toolId: p.id, exitCode: P, transcript: F }
    }]);
  }, [e.missions, G, V, de]), W = w(async (p) => {
    ...
    T(p.id);
    const O = `local-run-${Date.now()}-${p.id}`;
    Q((R) => [{
      id: O,
      toolId: p.id,
      label: p.label,
    ...
    try {
      const R = await e.local.runJob({
        ...
        prompt: P ? [
          ...
          `Mission: ${P.title}`,
          `Goal: ${P.goal}`,
          P.rootPath ? `Mission folder: ${P.rootPath}` : `Mission folder: ${P.name}`,
...
`)
      }), U = [`[Atomek] Started ${p.label} local job ${R.id}`], F = (K) => {
        Q((z) => z.map((D) => D.id === O ? K(D) : D));
      };
      e.local.streamJob(R.id, {
        onLog: (K) => {
          U.push(K), F((z) => ({ ...z, lines: U.slice(-500) }));
        },
        onDone: (K) => {
          K && (U.push(K), F((z) => ({ ...z, lines: U.slice(-500) })));
        },
        onFail: (K) => {
          U.push(`[FAIL] ${K}`), F((z) => ({
            ...z,
            ...
            finishedAt: Date.now(),
            lines: U.slice(-500)
          })), r(`${p.label} local job failed`, U.join(`
`)), E(p, U.join(`
`), -1, P).catch((z) => {
...
`);
          F((D) => ({
            ...
            finishedAt: Date.now(),
            lines: U.slice(-500)
          })), r(`${p.label} local job`, z), E(p, z, K, P).catch((D) => {
      ...
      T(null), Q((U) => U.map((F) => F.id === O ? {
        ...F,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...F.lines, `[Atomek] Failed to start: ${R instanceof Error ? R.message : String(R)}`]
      } : F)), t(`Local job failed to start: ${R instanceof Error ? R.message : String(R)}`);
    }
  }, [o, ge, e.local, G, i, h, r, E, t, de]);
  return /* @__PURE__ */ l("aside", { className: Z ? "workbench-agent-dock" : "workbench-sidebar", children: [
    Z ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "CONTROL TOWER" }),
    /* @__PURE__ */ l("div", { className: Z ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ l("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ a(jt, { size: 18 }),
        /* @__PURE__ */ l("div", { children: [
          /* @__PURE__ */ a("strong", { children: Z ? "Mission Runs" : "Tytus Control Tower" }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-context-card mission", children: [
        ...
        /* @__PURE__ */ a("span", { children: V ? `${V.rootPath ?? V.name} · ${V.source} · ${j.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-actions", children: [
        ...
        /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => V && void de(V, G), disabled: !V, children: "Rewrite context files" })
      ] }),
      h?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: h.warnings.slice(0, 3).map((p) => /* @__PURE__ */ l("span", { children: [
        p.code,
        ": ",
        p.message
      ...
      /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: ee.map((p) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => te(p.prompt), children: p.label }, p.label)) }),
      /* @__PURE__ */ a(
        "textarea",
        {
      ...
      C ? /* @__PURE__ */ l("div", { className: "workbench-agent-run", children: [
        /* @__PURE__ */ l("header", { children: [
          /* @__PURE__ */ l("div", { children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => r(`${C.label} local job`, C.lines.join(`
`)), disabled: C.lines.length === 0, children: "Save output" })
        ] }),
        /* @__PURE__ */ a("pre", { className: "workbench-computer-job-log", children: C.lines.join(`
`) || "[waiting for output]" })
      ] }) : null,
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list compact", children: [
        !h && !N ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
        h?.resources.slice(0, Z ? 8 : 5).map((p) => /* @__PURE__ */ l("div", { className: "workbench-resource-row", children: [
          /* @__PURE__ */ l("div", { children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list", children: [
        d.length === 0 && !N ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        d.map((p) => /* @__PURE__ */ l("div", { className: "workbench-computer-card", children: [
          ...
          p.description ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: p.description }) : null,
          /* @__PURE__ */ l("div", { className: "workbench-computer-actions", children: [
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
              $(p);
            }, disabled: p.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: cs(p) }),
            p.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              W(p);
            }, disabled: p.status !== "available" || Y !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: ls(p, Y) }) : null
          ] })
        ] }, p.id))
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
      /* @__PURE__ */ l("div", { className: "workbench-computer-list", children: [
        f.length === 0 && !N ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
        f.map((p) => /* @__PURE__ */ l("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ l("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ l("div", { children: [
          ...
          /* @__PURE__ */ a("p", { className: "workbench-muted", children: p.description }),
          p.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: p.triggers.slice(0, 4).map((I) => /* @__PURE__ */ a("span", { children: I }, I)) }) : null,
...
function ds({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: o }) {
  return /* @__PURE__ */ l("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: Rn(t.language) })
  ] });
}
function It(e, t) {
