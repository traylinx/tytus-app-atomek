 ...
 */
const ti = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), ni = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, i) => i ? i.toUpperCase() : n.toLowerCase()
), kn = (e) => {
  const t = ni(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, jn = (...e) => e.filter((t, n, i) => !!t && t.trim() !== "" && i.indexOf(t) === n).join(" ").trim(), ai = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const ci = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], li = q("check", ci);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const di = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], mt = q("chevron-down", di);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const Bi = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Wi() {
  const e = tn();
...
async function Ui() {
  ...
  if (!e) return Gn(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((i) => Kn(i, i.name, "local-file")));
  return nn(n.filter(Boolean));
}
async function Vi() {
  const e = tn()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await Gn(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await Jn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: nn(n) };
}
function tn() {
  const e = window;
...
async function Jn(e, t, n, i, o) {
  if (!(n.length >= o || i > 8))
    for await (const r of e.values()) {
      if (n.length >= o) break;
      if (r.kind === "directory") {
        if (Bi.has(r.name)) continue;
        await Jn(r, `${t}/${r.name}`, n, i + 1, o);
        continue;
      }
      const s = r, h = await Kn(s, `${t}/${s.name}`, "local-folder");
      h && n.push(h);
    }
}
function Gn(e) {
...
`).map(
    ...
    const r = i.split("|").filter(Boolean).map((h) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${h.trim()}</th>`).join(""), s = o.trim().split(`
`).map((h) => `<tr>${h.split("|").filter(Boolean).map(
...
const We = "atomek:default", Ki = 3e3, an = "tytus.atomek.threadTitleOverrides", yn = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, Qe = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, Ji = (e, t = Ki) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, vn = (e, t) => {
  const n = e.split(`
`).map((i) => i.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Gi = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Cn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Gi(e.kind),
  body: e.body,
...
function Zi({ host: e, requestContext: t, chatSettings: n, setStatus: i }) {
  const o = e.ai, [r, s] = $(null), [h, p] = $([]), [f, b] = $([]), [u, w] = $([]), [I, L] = $([]), [H, X] = $(yn), [K, ae] = $(!1), ce = ze(!0), P = ze(null), _ = v(async () => {
    if (!o) {
      X(yn);
      return;
    }
    ...
    if (o)
      try {
        const A = await o.listArtifacts({ threadId: N });
        if (!ce.current) return;
        w(A.map(Cn));
      } catch (A) {
        i(`AI artifacts unavailable: ${A instanceof Error ? A.message : String(A)}`);
      }
  }, [o, i]), j = v(async (N) => {
    if (o)
      try {
        const A = await o.listMessages(N);
        if (!ce.current) return;
        const S = h.find((B) => B.id === N) ?? null;
        S && s(S), b(A.map(Qe).filter(Boolean)), L([]), await ee(N);
      } catch (A) {
        i(`Load chat failed: ${A instanceof Error ? A.message : String(A)}`);
      }
  }, [o, ee, i, h]), ie = v(async () => {
    if (o)
      try {
        const N = $n(await o.listThreads({ workspaceKey: We, status: "active" })), A = N[0] ?? await o.createThread({ workspaceKey: We, title: "Atomek chat" });
        if (!ce.current) return;
        p(N[0] ? N : [A]), s(A);
        const S = await o.listMessages(A.id);
        if (!ce.current) return;
        b(S.map(Qe).filter(Boolean)), await ee(A.id);
      } catch (N) {
        i(`AI chat unavailable: ${N instanceof Error ? N.message : String(N)}`);
      }
  }, [o, ee, i]);
  oe(() => (ce.current = !0, _(), ie(), () => {
    ce.current = !1;
  }), [ie, _]);
  const ue = v(async () => {
    if (!o) return null;
    if (r) return r;
    const N = await o.createThread({ workspaceKey: We, title: "Atomek chat" });
    return ce.current && (s(N), p((A) => [N, ...A.filter((S) => S.id !== N.id)])), N;
  }, [o, r]), Me = v(async () => {
    if (!o) return;
    const N = await o.createThread({ workspaceKey: We, title: "Atomek chat" });
    s(N), p((A) => [N, ...A.filter((S) => S.id !== N.id)]), b([]), w([]), L([]), i("New AI chat created");
  }, [o, i]), g = v(async (N, A) => {
    if (!o) return;
    const S = A.trim();
    if (S)
      try {
        const B = o.updateThread, te = h.find((ne) => ne.id === N) ?? r, J = typeof B == "function" ? await B({ threadId: N, title: S }) : { ...te ?? await ue(), id: N, title: S, updatedAt: Date.now() };
        typeof B != "function" && Yi(N, S), p((ne) => ne.map((xe) => xe.id === J.id ? J : xe)), s((ne) => ne?.id === J.id ? J : ne), i(typeof B == "function" ? `Renamed chat: ${J.title}` : `Renamed chat locally: ${J.title}`);
      } catch (B) {
        i(`Rename chat failed: ${B instanceof Error ? B.message : String(B)}`);
      }
  }, [o, ue, i, r, h]), E = v(async (N) => {
    if (o)
      try {
        await o.deleteThread(N), Qi(N);
        const A = h.filter((S) => S.id !== N);
        if (p(A), r?.id === N) {
          const S = A[0] ?? await o.createThread({ workspaceKey: We, title: "Atomek chat" });
          s(S), p((te) => te.some((J) => J.id === S.id) ? te : [S, ...te]);
          const B = await o.listMessages(S.id);
          b(B.map(Qe).filter(Boolean)), await ee(S.id);
        }
        i("Deleted AI chat");
      } catch (A) {
        i(`Delete chat failed: ${A instanceof Error ? A.message : String(A)}`);
      }
  }, [o, ee, i, r?.id, h]), he = v(async (N) => {
    if (!o) return null;
    try {
      const A = await ue();
      if (!A) return null;
      const S = await o.createArtifact({
        threadId: A.id,
        messageId: N.messageId ?? null,
        title: N.title?.trim() || vn(N.body, "Atomek artifact"),
        kind: N.kind ?? "markdown",
        body: N.body
      }), B = Cn(S);
      return w((te) => [B, ...te.filter((J) => J.id !== B.id)]), i(`Saved AI artifact: ${B.title}`), B;
    } catch (A) {
      return i(`Save artifact failed: ${A instanceof Error ? A.message : String(A)}`), null;
    }
  }, [o, ue, i]), ve = v(async (N) => {
    if (o)
      try {
        await o.deleteArtifact(N), w((A) => A.filter((S) => S.id !== N)), i("Deleted AI artifact");
      } catch (A) {
        i(`Delete artifact failed: ${A instanceof Error ? A.message : String(A)}`);
      }
  }, [o, i]), Y = v(async (N) => {
    if (!o) return null;
    try {
      const A = await o.writeMemory({
        title: N.title?.trim() || vn(N.body, "Atomek memory"),
        body: N.body,
        metadata: {
          source: "atomek",
          messageId: N.messageId ?? null
      ...
      return L((S) => [A, ...S.filter((B) => B.id !== A.id)].slice(0, 5)), i(`Remembered: ${A.title}`), A;
    } catch (A) {
      return i(`Remember failed: ${A instanceof Error ? A.message : String(A)}`), null;
    }
  }, [o, i]), ge = v(async (N) => {
    if (!o) return [];
    const A = await o.searchMemory({ query: N, limit: 5 });
    return ce.current && L(A), A;
  }, [o]), Ee = v(async (N, A = {}) => {
    const S = N.trim();
    if (!S || !o) return null;
    ae(!0);
    ...
    try {
      const J = await ue();
      if (!J) return null;
      const ne = await ge(S).catch(() => []), xe = Xi(ne), Ce = A.requestContext ?? t, Ne = xe ? [...Ce, xe] : [...Ce];
      let Pe = null;
      for await (const W of o.sendMessage({
        threadId: J.id,
        body: S,
        ...
        if (W.type === "message_created") {
          const G = Qe(W.message);
          if (!G) continue;
          G.role === "assistant" && (Pe = G.id), b((me) => [...me.filter((de) => de.id !== G.id), G]);
        }
        if (W.type === "token" && (Pe = W.messageId, b((G) => G.some((me) => me.id === W.messageId) ? G.map(
          (me) => me.id === W.messageId ? { ...me, body: W.body, status: "streaming" } : me
        ) : [...G, {
          id: W.messageId,
          role: "assistant",
          body: W.body,
          status: "streaming",
          createdAt: Date.now()
        }])), W.type === "message_updated" || W.type === "done") {
          const G = Qe(W.message);
          if (!G) continue;
          G.role === "assistant" && (B = G), b((me) => me.map((de) => de.id === G.id ? G : de)), G.gatewayLabel && i(`AI answered via ${G.gatewayLabel}`);
        }
        if (W.type === "run_failed") {
          const G = te.signal.aborted;
          i(G ? "AI response stopped" : `AI failed: ${W.error}`), Pe && b((me) => me.map((de) => de.id !== Pe ? de : G ? { ...de, status: "complete", error: void 0, body: de.body || "Stopped by user." } : { ...de, status: "error", error: W.error, body: W.error }));
        }
      }
      const le = $n(await o.listThreads({ workspaceKey: We, status: "active" }).catch(() => []));
      return ce.current && le.length > 0 && p(le), _(), B;
    } catch (J) {
      return J instanceof DOMException && J.name === "AbortError" ? (i("AI response stopped"), B) : (i(`AI failed: ${J instanceof Error ? J.message : String(J)}`), null);
    } finally {
      P.current === te && (P.current = null), ae(!1);
    }
  }, [o, n.gatewayPreference, n.model, ue, ge, _, t, i]), Q = v(() => {
    P.current?.abort(), i("Stopping AI response…");
  }, [i]);
  return {
    aiStatus: H,
    artifacts: u,
...
const eo = (e) => `tytus-workbench:///${encodeURI(e.path)}`, to = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, no = ({ files: e, openEditorIds: t, activeFileId: n, versions: i, activeSelection: o }) => {
  const r = new Set(t), s = e.map((h) => {
    const p = h.id === n;
    return {
      id: h.id,
      uri: eo(h),
  ...
  return {
    documents: s,
    byId: new Map(s.map((h) => [h.id, h])),
    activeDocumentId: n,
    openDocumentIds: t.filter((h) => s.some((p) => p.id === h))
  };
}, ao = "active-file", io = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", oo = (e, t = "file") => `${t}:${e.id}`, ro = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let o = [];
  return t.scope === "active-selection" ? i?.selection ? o = [so(i, i.selection, !0)] : i && (o = [lt(i, !0, "Active file")]) : t.scope === "active-file" ? i && (o = [lt(i, !0, "Active file")]) : t.scope === "open-editors" ? o = e.openDocumentIds.map((r) => e.byId.get(r)).filter(Boolean).map((r) => lt(r, !0, "Open editor")) : t.scope === "selected-files" && (o = t.selectedFileIds.map((r) => e.byId.get(r)).filter(Boolean).map((r) => lt(r, !1, "Selected file"))), lo(o).filter((r) => !n.has(r.id));
}, lo = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, ho = 4e3, uo = 8e3, mo = 4e3, po = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, fo = (e, t, n) => {
  const i = co(e, n), o = [], r = new Map(t.map((s) => [s.id, s]));
  for (const s of i) {
    if (!s.fileId) continue;
    const h = r.get(s.fileId), p = e.byId.get(s.fileId);
    if (!h || !p || !s.includeBody) continue;
    const f = [
      `Path: ${h.path}`,
      `Language: ${h.language}`,
      `Version: ${p.version}`,
      ...
      s.range ? `Range: ${xn(s.range)}` : null
    ].filter(Boolean).join(`
`), b = s.kind === "selection" && s.range ? bo(h.content, s.range) : h.content, u = s.kind === "selection" ? ho : s.label.startsWith("Open editor") ? mo : uo;
    o.push({
      kind: s.kind === "selection" ? "selection" : "file",
      title: s.kind === "selection" ? `Active selection: ${h.path}` : s.label,
      text: `${f}
...
`));
}, xn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, Nn = 75e4, go = 2800, ko = 240, wo = 80, yo = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Fe(t.content)}`).sort().join("")
), Vt = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? Nn) || e.content.length > (t.maxFileBytes ?? Nn) ? "huge" : !t.includeDirty && e.dirty ? null : So(e.path) ? "vendor" : Io(e) ? "binary" : null;
}, An = (e, t = {}) => {
  const n = Date.now(), i = [], o = [], r = [];
  for (const s of e) {
    const h = s.size ?? s.content.length, p = Vt(s, t);
    if (p) {
      o.push({ fileId: s.id, name: s.name, path: s.path, language: s.language, reason: p, size: h });
      continue;
    }
    const f = Fe(s.content), b = Co(s, f, n, t);
    r.push(...b), i.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
...
}, Co = (e, t = Fe(e.content), n = Date.now(), i = {}) => {
  const o = Math.max(500, i.maxChunkChars ?? go), r = Math.min(Math.max(0, i.chunkOverlapChars ?? ko), Math.floor(o / 2)), s = Math.max(1, i.maxChunksPerFile ?? wo), h = No(e.content), p = [];
  let f = 0, b = 0;
  for (; f < e.content.length && p.length < s; ) {
    const u = xo(e.content, f, Math.min(e.content.length, f + o)), w = e.content.slice(f, u).trim();
    if (w.length > 0) {
      const I = Ao(h, f, u);
      p.push({
...
` && t.push(n + 1);
  ...
  const i = Sn(e, t), o = Sn(e, Math.max(t, n - 1));
  return {
    startLineNumber: i + 1,
    startColumn: t - e[i] + 1,
    endLineNumber: o + 1,
  ...
  return 0;
}, So = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => yo.has(t)), Io = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && vo.has(t) || e.content.includes("\0")) return !0;
...
class Mo {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = An(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = An(t, this.options), this.snapshot;
  }
...
const Ue = (e = [], t = {}) => new Mo(e, t), Po = (e, t, n = {}) => {
  const i = new Map(t.map((h) => [h.id, h])), o = e.byFileId, r = [];
  for (const h of e.files) {
    const p = i.get(h.fileId);
    if (!p) {
      r.push({ fileId: h.fileId, path: h.path, status: "deleted", indexedHash: h.hash });
      continue;
    }
    if (Vt(p, n)) {
      r.push({ fileId: h.fileId, path: p.path, status: "skipped-now", indexedHash: h.hash, currentHash: Fe(p.content) });
      continue;
    }
    const b = Fe(p.content);
    b !== h.hash ? r.push({ fileId: h.fileId, path: p.path, status: "changed", indexedHash: h.hash, currentHash: b }) : p.dirty !== h.dirty && r.push({ fileId: h.fileId, path: p.path, status: "dirty-state-changed", indexedHash: h.hash, currentHash: b });
  }
  for (const h of t)
    !o.has(h.id) && !Vt(h, n) && r.push({ fileId: h.id, path: h.path, status: "new", currentHash: Fe(h.content) });
  const s = e.signature !== Qn(t);
  return { stale: r.length > 0, signatureChanged: s, files: r };
}, To = 8, Lo = 12e3, Zn = (e, t, n = {}, i) => {
  const o = Ro(t);
  if (o.length === 0) return [];
  const r = new Set(i?.files.map((b) => b.fileId) ?? []), s = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => jo(b, o)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, u) => u.score - b.score || b.chunk.path.localeCompare(u.chunk.path) || b.chunk.ordinal - u.chunk.ordinal), h = [], p = /* @__PURE__ */ new Set();
  let f = n.maxChars ?? Lo;
  for (const b of s) {
    if (h.length >= (n.limit ?? To) || f <= 0) break;
    const u = `${b.chunk.fileId}:${b.chunk.ordinal}`;
    if (p.has(u)) continue;
    p.add(u);
    const w = b.chunk.text.length > f ? `${b.chunk.text.slice(0, Math.max(0, f - 28))}
[...context clipped...]` : b.chunk.text;
    h.push(Oo(b, w, r.has(b.chunk.fileId))), f -= w.length;
  }
  return h;
}, Oo = (e, t = e.chunk.text, n = !1) => {
...
`), Ro = (e) => Array.from(new Set(
  ...
  const h = [];
  for (const p of t) {
    const f = Rt(n, p), b = Rt(i, p), u = Rt(o, p), w = r === p ? 1 : 0;
    f + b + u + w !== 0 && (h.push(p), s += Math.min(f, 12), s += b * 4, s += u * 6, s += w * 3, (n.includes(`function ${p}`) || n.includes(`const ${p}`) || n.includes(`class ${p}`) || n.includes(`type ${p}`)) && (s += 3));
  }
  return s += Math.max(0, 2 - e.ordinal * 0.05), { chunk: e, score: s, matchedTerms: h };
}, Rt = (e, t) => {
  ...
  const n = e.toLowerCase(), i = t.map((p) => n.indexOf(p)).filter((p) => p >= 0).sort((p, f) => p - f)[0] ?? 0, o = Math.max(0, i - 120), r = Math.min(e.length, i + 260), s = o > 0 ? "…" : "", h = r < e.length ? "…" : "";
  return `${s}${e.slice(o, r).replace(/\s+/g, " ").trim()}${h}`;
}, Do = (e, t = {}) => {
  const { autoRefresh: n = !1, ...i } = t, o = ze(e), r = ze(null);
  r.current || (r.current = Ue(e, i));
  const [s, h] = $(() => r.current?.getSnapshot() ?? Ue(e, i).getSnapshot());
  o.current = e;
  const p = v((w = o.current) => {
    const I = (r.current ?? Ue([], i)).refresh(w, i);
    return h(I), I;
  }, [i.maxFileBytes, i.maxChunkChars, i.chunkOverlapChars, i.maxChunksPerFile, i.includeDirty]), f = v((w = o.current) => {
    const I = (r.current ?? Ue([], i)).update(w, i);
    return h(I), I;
  }, [p]);
  oe(() => {
    n && p(e);
  }, [n, e, p]);
  const b = (r.current ?? Ue([], i)).staleReport(e), u = v((w, I = {}) => {
    const L = r.current ?? Ue([], i);
    return Zn(L.getSnapshot(), w, I, L.staleReport(o.current));
  }, []);
  return { snapshot: s, staleReport: b, isStale: b.stale, refresh: p, update: f, retrieve: u };
}, zo = "tytus.atomek.semanticVector:v1", ea = "__gateway_default__", ta = (e) => e?.trim() || ea, _o = (e) => {
  const t = ta(e);
  return t === ea ? "gateway default" : t;
}, na = (e, t, n) => [
  zo,
  ...
  if (!aa()) return null;
  try {
    const i = na(e, t, n), o = window.localStorage.getItem(i);
    if (!o) return null;
...
}, Ze = 8, Wo = 12e3, Uo = 180, Vo = async (e, t, n, i, o = {}, r, s) => {
  const h = Zn(t, n, { ...o, limit: Math.max(o.limit ?? Ze, 16) }, r), p = e.ai;
  if (typeof p?.embedText != "function")
    return { hits: h.slice(0, o.limit ?? Ze), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const f = ta(i.embeddingModel), b = await p.embedText({
      input: n,
    ...
    const w = new Map(h.map((P) => [P.chunkId, P])), I = Qo(n), L = /* @__PURE__ */ new Map();
    t.chunks.filter((P) => o.includeDirty !== !1 || !P.dirty).slice(0, Uo).forEach((P) => L.set(P.id, P)), h.forEach((P) => {
      const _ = t.byChunkId.get(P.chunkId);
      _ && L.set(_.id, _);
    });
    const H = Array.from(L.values()), X = [];
    let K = 0;
    for (const P of H) {
      s?.aborted;
      const _ = Ho(e.appId, f, P) ?? await Ko(p, e.appId, f, P, i, s);
      if (!_) continue;
      _.updatedAt > Date.now() - 1e3 && (K += 1);
      const ee = Xo(u, _.vector);
      if (!Number.isFinite(ee)) continue;
      const ie = w.get(P.id)?.score ?? Zo(P, I), ue = Yo(ie) * 0.42 + Math.max(0, ee) * 0.58;
      X.push({ chunk: P, keywordScore: ie, vectorScore: ee, score: ue, matchedTerms: I.filter((Me) => P.text.toLowerCase().includes(Me)) });
    }
    if (X.length === 0) return { hits: h.slice(0, o.limit ?? Ze), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: K };
    const ae = new Set(r?.files.map((P) => P.fileId) ?? []), ce = X.sort((P, _) => _.score - P.score || _.keywordScore - P.keywordScore || P.chunk.path.localeCompare(_.chunk.path) || P.chunk.ordinal - _.chunk.ordinal).slice(0, Math.max(o.limit ?? Ze, 1)).map((P) => Jo(P, ae.has(P.chunk.fileId)));
    return {
      hits: Go(ce, o.maxChars ?? Wo),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${_o(i.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: K
    };
  } catch (f) {
    return {
      hits: h.slice(0, o.limit ?? Ze),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: er(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, Yo = (e) => Math.min(1, Math.max(0, e / 24)), Qo = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Zo = (e, t) => {
  const n = `${e.path}
${e.name}
...
${e.text}`.toLowerCase();
  ...
  const n = e.toLowerCase(), i = t.map((s) => n.indexOf(s)).filter((s) => s >= 0).sort((s, h) => s - h)[0] ?? 0, o = Math.max(0, i - 120), r = Math.min(e.length, i + 260);
  return `${o > 0 ? "…" : ""}${e.slice(o, r).replace(/\s+/g, " ").trim()}${r < e.length ? "…" : ""}`;
}, tr = /```([^\n`]*)\n([\s\S]*?)```/g, nr = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function rn(e) {
  return Array.from(e.matchAll(tr)).map((t) => oa(t[1] ?? "", sr(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function oa(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), i = {}, o = [];
  for (const r of n) {
    const s = r.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? i[s[1].toLowerCase()] = lr(s[2]) : o.push(r.toLowerCase());
  }
...
function ra(e) {
  return [...rn(e).filter((i) => $e(i, "diff") || $e(i, "patch")).map((i) => i.content), e].filter((i, o, r) => ar(i) && r.findIndex((s) => s === i) === o);
}
function ar(e) {
...
`), i = [];
  if (n.forEach((s, h) => {
    (s.startsWith("diff --git ") || s.startsWith("--- ")) && i.push(h);
  }), i.length === 0) return [{ raw: t, ...jt(t) }];
  const o = Array.from(new Set(i)).sort((s, h) => s - h), r = [];
  for (let s = 0; s < o.length; s += 1) {
    const h = o[s], f = o.find((u) => u > h && n[u].startsWith("diff --git ")) ?? n.length, b = n.slice(h, f).join(`
`);
    /^@@\s+-\d+/m.test(b) && r.push({ raw: b, ...jt(b) });
  }
  return r.length > 0 ? r : [{ raw: t, ...jt(t) }];
}
function jt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), i = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), o = Te(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, r = Te(t?.[3] || t?.[4] || i?.[1] || i?.[2] || "") || null, s = r && r !== "/dev/null" ? r : o;
...
function qt(e) {
  return rn(e).filter((t) => or(t)).map((t) => ({
...
`), o = [];
  ...
  for (let h = 0; h < i.length; h += 1) {
    const p = i[h].match(nr);
    if (!p) continue;
    s = !0;
    const f = Number(p[1]), b = Math.max(0, f - 1);
    if (b < r) return null;
    for (o.push(...n.slice(r, b)), r = b, h += 1; h < i.length; h += 1) {
      const u = i[h];
      if (u.startsWith("@@ ")) {
        h -= 1;
        break;
...
function dr(e) {
  const { body: t, files: n, sourceTitle: i, activeFile: o = null, versions: r = {} } = e, s = /* @__PURE__ */ new Map(), h = [];
  for (const b of ra(t))
    for (const u of ir(b)) {
      if (!u.path) {
        if (o) {
          const L = Kt(o.content, u.raw);
          L && L !== o.content && s.set(o.id, dt({
            file: o,
            proposedContent: L,
            sourceTitle: i,
      ...
      const w = In(n, u.path);
      if (!w) {
        h.push(`${u.path}: no opened file`);
        continue;
      }
      const I = Kt(w.file.content, u.raw);
      if (!I || I === w.file.content) {
        h.push(`${u.path}: patch did not match or produced no change`);
        continue;
      }
      s.set(w.file.id, dt({
        file: w.file,
  ...
  for (const b of qt(t)) {
    const u = b.path ? In(n, b.path) : o ? { file: o, normalizedPatchPath: Te(o.path), confidence: "exact" } : null;
    if (!u) {
      h.push(`${b.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (b.content === u.file.content) {
      h.push(`${u.file.path}: replacement produced no change`);
      continue;
    }
    s.set(u.file.id, dt({
      file: u.file,
      ...
      versions: r,
      targetPath: b.path ?? u.file.path,
      match: u
    }));
  }
  const p = qt(t).some((b) => b.path);
  if (s.size === 0 && o && !p) {
    const b = hr(t, o);
    b && b.content !== o.content && s.set(o.id, dt({
      file: o,
      proposedContent: b.content,
      sourceTitle: i,
      extractionLabel: b.label,
  ...
  return {
    sourceTitle: i,
    edits: f,
    skipped: Array.from(new Set(h)),
    stats: mr(f.map((b) => b.stats)),
    kind: f.length === 0 ? "empty" : f.length === 1 ? "single-file" : "multi-file"
  };
}
function hr(e, t) {
  for (const p of ra(e)) {
    const f = Kt(t.content, p);
    if (f) return { content: f, label: "unified diff patch" };
  }
  const n = rn(e);
  if (n.length === 0) return null;
  const i = fr(t), o = qt(e)[0];
  if (o) return { content: o.content, label: o.label };
  const r = n.find((p) => i.some((f) => $e(p, f)) && !$e(p, "diff") && !$e(p, "patch"));
  if (r) return { content: r.content, label: `matched ${r.lang || t.language} block` };
  const h = n.filter((p) => !$e(p, "diff") && !$e(p, "patch")).sort((p, f) => f.content.length - p.content.length)[0];
  return h ? { content: h.content, label: `largest fenced block (${h.lang || "plain"})` } : null;
}
function In(e, t) {
  const n = Te(t), i = n.split("/").at(-1) ?? n, o = e.find((h) => Te(h.path) === n);
  if (o) return { file: o, normalizedPatchPath: n, confidence: "exact" };
  const r = e.find((h) => Te(h.path).endsWith(`/${n}`));
  if (r) return { file: r, normalizedPatchPath: n, confidence: "suffix" };
  const s = e.find((h) => h.name === i || Te(h.path).endsWith(`/${i}`));
...
`), o = Math.max(n.length, i.length);
  let r = 0, s = 0, h = 0;
  for (let p = 0; p < o; p += 1)
    n[p] !== i[p] && (n[p] === void 0 ? r += 1 : i[p] === void 0 ? s += 1 : h += 1);
  return { added: r, removed: s, changed: h };
}
function mr(e) {
  return e.reduce((t, n) => ({
...
function dt(e) {
  ...
  return {
    ...
    stats: ur(e.file.content, e.proposedContent),
    base: {
...
const ft = (e) => typeof e == "object" && e !== null, br = (e) => ft(e.ai) ? e.ai : null, gr = (e) => {
  if (!ft(e)) return null;
  const t = e.id;
...
}, Se = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Se) : ft(e) ? Object.values(e).some(Se) : !1, wr = async (e, t) => {
  const n = br(e);
  if (typeof n?.listModels != "function") return [];
  const i = await n.listModels(t);
  return Array.isArray(i) ? i.map(kr).filter((o) => !!o) : [];
}, yr = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Se(e.capability) || Se(e.capabilities) || Se(e.modality) || Se(e.modalities) || Se(e.task) || Se(e.tasks) || Se(e.type) || Se(e.kind), ca = (e) => typeof e == "object" && e !== null, vr = (e) => ca(e.ai) ? e.ai : null, la = (e) => typeof vr(e)?.embedText == "function", Cr = async (e, t) => la(e) ? (await wr(e, t)).filter(yr) : [], $r = (e) => ca(e.ai) ? la(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", xr = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], Nr = [
...
function Mr(e) {
  const t = e.results.length > 0 ? e.results.map((i, o) => [
    ...
    `Status: ${i.status}`,
    "Output:",
    Fr(i.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function Tr(e) {
  ...
  for (const i of t) {
    const o = Lr(i.content);
    if (!o?.scripts) continue;
    const r = ha(i.path), s = Or(e, r, o);
    if (!s) continue;
    const h = Object.keys(o.scripts).filter((f) => typeof o.scripts?.[f] == "string"), p = Er(h);
    for (const f of p) {
      const b = `${s} run ${f}`;
      n.push({
        id: ua(`${r}:${b}`),
...
function Or(e, t, n) {
  ...
  for (const [i, o] of Nr)
    if (e.some((r) => jr(r.path) === i && ha(r.path) === t)) return o;
  return null;
}
function Er(e) {
  const t = xr.filter((i) => e.includes(i)), n = e.filter((i) => !t.includes(i)).filter((i) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(i)).sort((i, o) => i.localeCompare(o));
  return [...t, ...n];
}
function Rr(e) {
  const t = /* @__PURE__ */ new Set();
...
const Dr = Za(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), zr = {
  id: "welcome",
  ...
  dirty: !1,
  source: "sample"
}, ma = "tytus.workspace.recent", pa = "tytus.workspace.layout", fa = "tytus.atomek.chatAiSettings", ba = "tytus.atomek.currentMission", Jt = "tytus.atomek.currentMissionChanged", ut = {
...
function Ft(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - _r), i = n < 1180, o = i ? 200 : 240, r = i ? 300 : 340, s = i ? 420 : 560, h = Math.max(o, Math.min(i ? 340 : 420, Math.floor(n * 0.28))), p = Math.min(300, h), f = Math.floor(n * (i ? 0.34 : 0.36)), b = n - p - s, u = Math.max(r, Math.min(i ? 500 : 640, f, b));
  return { primaryMin: o, primaryMax: h, secondaryMin: r, secondaryMax: u };
}
function Hr(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let i = 0, o = 0, r;
  for (; (r = n.exec(e)) !== null; ) {
    if (r.index > i) {
      const h = e.slice(i, r.index);
      h.trim() && t.push({ type: "markdown", body: h, key: `md-${o}` });
    }
    t.push({
      type: "code",
...
function tt(e) {
  ...
  try {
    localStorage.setItem(ba, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(Jt, { detail: t }));
  } catch {
  }
}
function Ur() {
...
function nt(e, t) {
  const n = t?.resources ?? [], i = n.some((p) => p.kind === "pod-agent" && (p.status === "ready" || p.status === "degraded")), o = n.some((p) => p.kind === "local-cli" && (p.status === "ready" || p.status === "available")), r = n.some((p) => p.kind === "shared-folder" && p.status === "ready"), s = n.some((p) => p.kind === "app-skill" && p.status === "ready"), h = e.trim() || "Coordinate a Tytus mission.";
  return [
    {
      id: "task-plan",
      title: "Plan mission",
      prompt: `Turn this goal into an executable plan: ${h}`,
      resourceHint: i ? "pod-agent or AIL route" : "AIL chat",
      status: "ready",
    ...
    },
    {
      ...
      prompt: `Use the mission context and selected resources to execute the plan. Goal: ${h}`,
      resourceHint: o ? "local agent" : s ? "app skill" : "chat",
      status: "waiting",
    ...
    },
    {
      ...
      prompt: `Review outputs for the mission, summarize decisions, and prepare handoff. Goal: ${h}`,
      resourceHint: r ? "shared folder + reviewer agent" : "reviewer agent",
      status: "waiting",
...
function Gt(e, t, n, i, o) {
  return [
    ...
    "## Resource graph",
    "",
    t ? `- ${ga(t.resources)}` : "- not loaded",
    ...t?.warnings?.length ? t.warnings.map((r) => `- Warning: ${r.code} — ${r.message}`) : [],
    "",
    "## Rules",
    "",
...
function ya(e, t, n) {
  ...
  return JSON.stringify({
    ...
    resources: (t?.resources ?? []).filter((o) => o.status === "ready").map((o) => ({
      resourceId: o.id,
      pinnedLabel: o.label,
      pinnedKind: o.kind,
      pinnedCapabilities: o.capabilities
    })),
    permissions: {
    ...
    tasks: i.map((o, r) => ({
      ...
      title: o.title,
      prompt: o.prompt,
      status: r === 0 ? "ready" : "waiting",
      selectedResourceHint: o.resourceHint,
      dependsOn: r === 0 ? [] : [i[r - 1].id],
...
function qr({ host: e }) {
  const t = ze(null), [n, i] = $(0), o = pe(() => Ns(), []), [r, s] = $("computer"), [h, p] = $(o.primaryVisible), [f, b] = $(o.primaryWidth), [u, w] = $("chat"), [I, L] = $(o.secondaryVisible), [H, X] = $(o.secondaryWidth), [K, ae] = $(!1), [ce, P] = $("problems"), [_, ee] = $(o.markdownPreviewVisible), [j, ie] = $(!1), [ue, Me] = $(null), [g, E] = $([]), [he, ve] = $([]), [Y, ge] = $(null), [Ee, Q] = $(""), [N, A] = $({ lineNumber: 1, column: 1 }), [S, B] = $(null), [te, J] = $({}), [ne, xe] = $(ao), [Ce, Ne] = $([]), [Pe, le] = $([]), [W, G] = $(null), [me, de] = $(""), [bt, qe] = $(!1), [gt, kt] = $(""), [d, y] = $([]), [C, R] = $(null), [T, F] = $(null), [z, re] = $(!1), [ke, _e] = $(() => As()), [V, U] = $(null), [se, wt] = $(null), [yt, vt] = $(""), [Ct, $t] = $(""), [at, it] = $(""), [ot, cn] = $("failed"), [Sa, ln] = $(null), [Ia, M] = $("Ready"), [Ke, Ma] = $(() => xs()), we = he.map((c) => g.find((m) => m.id === c)).filter(Boolean), O = Y ? g.find((c) => c.id === Y) ?? null : null, dn = pe(() => no({ files: g, openEditorIds: he, activeFileId: Y, versions: te, activeSelection: S }), [Y, S, te, g, he]), hn = pe(() => ({
    scope: ne,
    removedAttachmentIds: Ce,
    selectedFileIds: []
  }), [ne, Ce]), Je = pe(() => fo(dn, g, hn), [hn, dn, g]), Le = Do(g, { autoRefresh: !0, includeDirty: !0 }), un = pe(() => Pe.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [Pe]), Pa = pe(() => [...Je.attachments, ...un], [Je.attachments, un]), D = Zi({ host: e, requestContext: Je.parts, chatSettings: ke, setStatus: M }), Re = pe(
    () => [...D.artifacts, ...d].sort((c, m) => m.createdAt - c.createdAt),
    [D.artifacts, d]
  ), rt = z && !O, xt = !O && !rt && !j, be = pe(() => g.filter((c) => c.dirty), [g]), Ta = pe(() => {
    const c = Ee.trim().toLowerCase();
    return c ? g.filter((m) => m.path.toLowerCase().includes(c)) : g;
  }, [g, Ee]);
  oe(() => {
    J((c) => {
      const m = {};
      for (const k of g) m[k.id] = c[k.id] ?? 1;
      return m;
    });
  }, [g]), oe(() => {
    Ne([]), le([]);
  ...
  const La = v((c) => {
    if (c.kind === "index-hit") {
      le((m) => m.filter((k) => k.id !== c.id)), M(`Removed project context: ${c.label}`);
      return;
    }
    Ne((m) => m.includes(c.id) ? m : [...m, c.id]), M(`Removed chat context: ${c.label}`);
  }, []), st = v(() => {
    re(!0), ge(null), ie(!0), M("Atomek settings opened");
  }, []), mn = v(() => {
    re(!1), ge((c) => c ?? he.at(-1) ?? null), he.length === 0 && ie(!1), M("Atomek settings closed");
  }, [he]), He = v((c) => {
    J((m) => ({ ...m, [c]: (m[c] ?? 1) + 1 }));
  }, []), Oa = v((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const m = Ft(n), k = c.clientX, x = H, Z = (Ye) => {
      const Ot = x + (k - Ye.clientX);
      X(ht(Ot, m.secondaryMin, m.secondaryMax));
    }, Ae = () => {
      window.removeEventListener("pointermove", Z), window.removeEventListener("pointerup", Ae);
    };
    window.addEventListener("pointermove", Z), window.addEventListener("pointerup", Ae);
  }, [H, n]), Ea = v((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const m = Ft(n), k = c.clientX, x = f, Z = (Ye) => {
      const Ot = x + (Ye.clientX - k);
      b(ht(Ot, m.primaryMin, m.primaryMax));
    }, Ae = () => {
      window.removeEventListener("pointermove", Z), window.removeEventListener("pointerup", Ae);
    };
    window.addEventListener("pointermove", Z), window.addEventListener("pointerup", Ae);
  }, [f, n]), ct = v((c) => {
    const m = [c, ...Ke.filter((k) => k.path !== c.path)].slice(0, 6);
    Ma(m), localStorage.setItem(ma, JSON.stringify(m));
  }, [Ke]), fe = v((c, m) => {
    ve((k) => k.includes(c.id) ? k : [...k, c.id]), ge(c.id), ie(!1), ln(m ?? null), A({ lineNumber: m ?? 1, column: 1 });
  }, []), Ra = v((c) => {
    if (!c.fileId) return;
    const m = g.find((k) => k.id === c.fileId);
    m && (fe(m, c.range?.startLineNumber ?? 1), M(`Revealed context: ${c.label}`));
  }, [g, fe]), Ge = v(async () => {
    if (_t(be, "open new files"))
      try {
        const c = await Ui();
        if (c.length === 0) return;
        E((m) => vs(m, c)), c.forEach((m) => ct({ name: m.name, path: m.path, at: Date.now() })), fe(c[0]), M(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && M(`Open file failed: ${c.message}`);
      }
  }, [be, fe, ct]), Nt = v(async () => {
    if (_t(be, "open another folder"))
      try {
        const c = await Vi();
        Me(c), E(c.files), ve([]), ge(null), ie(!1), ct({ name: c.name, path: c.name, at: Date.now() }), M(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!Y) return;
    let m = !1;
    E((k) => k.map((x) => x.id !== Y || x.content === c ? x : (m = !0, { ...x, content: c, dirty: !0 }))), m && He(Y);
  }, [Y, He]), At = v(async () => {
    if (O)
      try {
        const c = await Et(O);
        E((m) => m.map((k) => k.id === c.id ? c : k)), M(`Saved ${c.name}`);
      } catch (c) {
        M(`Save failed: ${c.message}`);
      }
  }, [O]), Fa = v(async (c) => {
    const m = g.find((x) => x.id === c);
    if (!m) return null;
    const k = await Et(m);
    return E((x) => x.map((Z) => Z.id === k.id ? k : Z)), k;
  }, [g]), pn = v(async () => {
    const c = g.filter((m) => m.dirty);
    if (c.length === 0) {
      M("No dirty files to save");
      return;
    }
    try {
      const m = await Promise.all(c.map((x) => Et(x))), k = new Map(m.map((x) => [x.id, x]));
      E((x) => x.map((Z) => k.get(Z.id) ?? Z)), U(null), M(`Saved ${m.length} dirty file${m.length === 1 ? "" : "s"}`);
    } catch (m) {
      M(`Save all failed: ${m.message}`);
    }
  }, [g]), St = v((c) => {
    const m = g.find((k) => k.id === c);
    if (m?.dirty && !window.confirm(`${m.name} has unsaved changes. Close without saving?`)) {
      M(`Close canceled — ${m.name} has unsaved changes`);
      return;
    }
    ve((k) => {
      const x = k.filter((Z) => Z !== c);
      return Y === c && (ge(x.at(-1) ?? null), B(null)), x;
    });
  }, [Y, g]), Da = v(() => {
    _t(be, "close all editors") && (ve([]), ge(null), ln(null), ie(!1), M("Closed all editors"));
  }, [be]), It = v(() => {
    const c = g.filter((k) => k.name.startsWith("Untitled")).length + 1, m = {
...
`,
    ...
    return le(x), x.length === 0 ? (M(Le.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), m) : (k.mode !== "hybrid" && k.reason ? M(k.reason) : M(`${x.length} project context hit${x.length === 1 ? "" : "s"} · ${k.reason ?? "hybrid retrieval"}`), [
      ...m,
      ...qo(x)
    ]);
  }, [Je.parts, ne, ke, e, Le]), Mt = v(() => {
    if (!O && we.length === 0) {
      M("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const m = [
      `Create a polished Markdown artifact from ${O?.path ?? `${we.length} open editors`}.`,
    ...
    L(!0), w("chat"), M("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const k = await Oe(m), x = await D.askAgent(m, { requestContext: k });
      !x || x.status === "error" || D.createArtifact({
        messageId: x.id,
        title: `AI synthesis — ${O?.name ?? "open workspace"}`,
        kind: "markdown",
        body: x.body
      }).then(() => {
        L(!0), w("outputs"), ae(!0), P("output");
      });
    })();
  }, [O, D, Oe, we.length]), Pt = v((c) => {
    const m = g.find((k) => k.path === c.path || k.name === c.name);
    if (m) {
      fe(m), M(`Opened recent ${m.name}`);
      return;
    }
    ...
    L(!0), w("chat"), de(""), (async () => {
      const m = await Oe(c);
      await D.askAgent(c, { requestContext: m });
    })();
  }, [D, Oe]), fn = v(async (c) => {
    if (!e.skills?.get) {
    ...
    try {
      const m = await e.skills.get(c.id), k = m.body.length > 4500 ? `${m.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : m.body, x = [
...
`);
      de((Z) => [Z.trim(), x].filter(Boolean).join(`

`)), L(!0), w("chat"), M(`Attached skill ${m.title} to chat input`);
    } catch (m) {
      M(`Failed to attach skill: ${m instanceof Error ? m.message : String(m)}`);
    }
  }, [e.skills]), za = v((c) => {
    const m = D.messages.findIndex((x) => x.id === c.id), k = D.messages.slice(0, m < 0 ? void 0 : m).reverse().find((x) => x.role === "user");
    if (!k?.body.trim()) {
      M("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const x = await Oe(k.body);
      await D.askAgent(k.body, { requestContext: x });
    })();
  }, [D, Oe]), _a = v((c) => {
    D.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const m = Bt(g, Ht(c.title || c.kind)), k = {
    ...
    E((x) => [...x, k]), fe(k), M(`Opened ${c.title} as editable file`);
  }, [g, fe]), Lt = v(() => {
...
`), Z = {
    ...
    y((Ae) => [Z, ...Ae]), ae(!0), P("output"), M(`Captured manual check: ${m}`);
  }, []), Be = v((c, m) => {
    const k = dr({
      body: m,
      files: g,
      ...
      edits: k.edits.map(Cs),
      skipped: k.skipped
    };
    return k.edits.length > 1 ? (F(x), R(null), M(`Previewing AI workspace patch for ${k.edits.length} files`), !0) : k.edits.length === 1 ? (R(x.edits[0]), F(null), M(`Previewing AI patch for ${x.edits[0].fileName}`), !0) : (M(k.skipped.length > 0 ? `No applicable edit found. ${k.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [O, te, g]), bn = v((c, m) => {
    const k = {
    ...
    y((x) => [k, ...x]), L(!0), w("outputs"), ae(!0), P("output"), M(`Saved local job output: ${c}`), (m.includes("```diff") || m.includes("--- a/")) && Be(c, m);
  }, [Be]), Wa = v(() => {
    const c = me.trim();
    if (!c) return;
    de("");
    const m = hs(c);
    (async () => {
      const k = await Oe(c), x = await D.askAgent(c, { requestContext: k });
      if (!m || !x || x.status === "error") return;
      Be(x.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", x.body) || M("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [D, Oe, me, Be]), Ua = v(() => {
    W && (G(null), ye(us(W)));
  }, [ye, W]), Xe = v((c) => {
    const m = Ar(g, c);
    wt(m);
    const k = m.commands[0]?.command ?? "";
    it(k), vt(""), $t(""), cn("failed"), ae(!0), P("terminal"), M(m.commands.length > 0 ? `Manual check ready: copy ${m.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [g]), Va = v((c) => {
    c.trim() && (Ve(c), it(c), M(`Manual check command copied: ${c}`));
  }, []), qa = v(() => {
    wt((c) => {
      if (!c) return c;
      const m = Sr(c, yt), k = m.commands.at(-1)?.command ?? "";
      return k && it(k), m;
    }), vt("");
  }, [yt]), Ka = v(() => {
    if (!se || !at.trim()) {
    ...
    const c = at.trim();
    wt((m) => m && Ir(m, c, ot, Ct)), $t(""), M(`Captured manual check result: ${c} (${ot})`);
  }, [Ct, at, se, ot]), Ja = v(() => {
    se && ye(Mr(se));
  }, [ye, se]), Ga = v(() => {
    if (!C) return;
    const c = g.find((m) => m.id === C.fileId);
    if (!c) {
      M(`Cannot apply edit — ${C.fileName} is no longer open`), R(null);
      return;
    }
    c.content !== C.originalContent && !window.confirm(`${C.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (E((m) => m.map((k) => k.id === C.fileId ? { ...k, content: C.proposedContent, dirty: !0 } : k)), He(C.fileId), U(`AI edit applied to ${c.name}. Save All to persist it to disk.`), R(null), Xe(`AI edit applied to ${c.name}`), M(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [He, g, C, Xe]), Xa = v(() => {
    if (!C) return;
    const c = Bt(g, `${Ht(C.fileName)}-ai-edit`), m = {
    ...
    E((k) => [...k, m]), fe(m), R(null), M(`Opened proposed edit as ${c}`);
  }, [O?.language, g, fe, C]), Ya = v(() => {
    if (!T) return;
    const c = new Map(T.edits.map((k) => [k.fileId, k]));
    g.some((k) => {
      const x = c.get(k.id);
      return x && k.content !== x.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (E((k) => k.map((x) => {
      const Z = c.get(x.id);
      return Z ? { ...x, content: Z.proposedContent, dirty: !0 } : x;
    })), c.forEach((k, x) => He(x)), U(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), F(null), Xe(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), M(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [He, g, T, Xe]), Qa = v(() => {
    if (!T) return;
    const c = [...g], m = T.edits.map((k) => {
      const x = Bt(c, `${Ht(k.fileName)}-ai-edit`), Z = g.find((Ye) => Ye.id === k.fileId), Ae = {
        id: `workspace-patch-${k.fileId}-${Date.now()}-${x}`,
        name: x,
        path: x,
    ...
    E((k) => [...k, ...m]), m[0] && fe(m[0]), F(null), M(`Opened ${m.length} proposed edit file${m.length === 1 ? "" : "s"}`);
  }, [g, fe, T]), gn = v((c) => {
    if (c === "explain") {
      ye("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (c === "improve") {
      ye("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (c === "plan") {
      ye("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (c === "edit") {
      ye("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    ye("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [ye]);
  return oe(() => {
    const c = (m) => {
      (m.metaKey || m.ctrlKey) && (m.key.toLowerCase() === "o" && (m.preventDefault(), Ge()), m.key.toLowerCase() === "s" && (m.preventDefault(), At()), m.key.toLowerCase() === "w" && (m.preventDefault(), Y && St(Y)), m.key.toLowerCase() === "b" && (m.preventDefault(), p((x) => !x)), m.shiftKey && m.key.toLowerCase() === "f" && (m.preventDefault(), s("search"), p(!0)), (m.key.toLowerCase() === "k" || m.key.toLowerCase() === "p") && (m.preventDefault(), qe(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [Y, St, Ge, At]), oe(() => {
    const c = (m) => {
      be.length !== 0 && (m.preventDefault(), m.returnValue = "");
    };
    ...
    const c = t.current;
    if (!c) return;
    const m = () => i(Math.round(c.getBoundingClientRect().width));
    m();
    ...
    if (!n) return;
    const c = Ft(n);
    b((m) => ht(m, c.primaryMin, c.primaryMax)), X((m) => ht(m, c.secondaryMin, c.secondaryMax));
  }, [n]), oe(() => {
    const c = { primaryVisible: h, primaryWidth: f, secondaryVisible: I, secondaryWidth: H, markdownPreviewVisible: _ };
    localStorage.setItem(pa, JSON.stringify(c));
  }, [_, h, f, I, H]), oe(() => {
    localStorage.setItem(fa, JSON.stringify(ke));
  }, [ke]), /* @__PURE__ */ l(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${h ? "" : "no-primary"} ${I ? "" : "no-secondary"} ${K ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${f}px`, "--workbench-secondary-width": `${H}px` },
      children: [
        /* @__PURE__ */ a(Kr, { active: r, setActive: (c) => {
          s(c), p(!0);
        }, openSettings: st, settingsActive: rt }),
        h && /* @__PURE__ */ l("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            Jr,
            {
              host: e,
              ...
              saveLocalJobOutput: bn,
              activeFile: O
            }
          ),
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: Ea, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ l("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => qe(!0), children: "Workspace" }),
          /* @__PURE__ */ l("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              Qr,
              {
                ...
                closeSettings: mn,
                secondaryVisible: I,
                toggleSecondary: () => L((c) => !c),
                canPreview: O?.language === "markdown",
                previewVisible: _,
                togglePreview: () => ee((c) => !c)
              }
            ),
            /* @__PURE__ */ a(Yr, { file: O, folder: ue, showWelcome: xt }),
            V && be.length > 0 ? /* @__PURE__ */ l("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: V }),
              /* @__PURE__ */ a("button", { onClick: () => {
                pn();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => U(null), title: "Dismiss", children: /* @__PURE__ */ a(Ie, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: O ? /* @__PURE__ */ l("div", { className: O.language === "markdown" && _ ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(ei, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                Dr,
                {
                  file: O,
                  revealLine: Sa,
              ...
              O.language === "markdown" && _ && /* @__PURE__ */ a(os, { content: O.content })
            ] }) : rt ? /* @__PURE__ */ a(
              ms,
              {
                host: e,
            ...
            ) : xt ? /* @__PURE__ */ a(ts, { host: e, openFile: Ge, openFolder: Nt, newFile: It, recent: Ke, reopenRecent: Pt, setStatus: M, openControlTower: () => {
              s("computer"), p(!0);
            }, openChat: () => {
              w("chat"), L(!0);
            } }) : /* @__PURE__ */ l("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(pt, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ie(!1), children: "Show Mission Control" })
            ] }) }),
            K && /* @__PURE__ */ a(
              ss,
              {
                ...
                runAiSynthesis: Mt,
                captureManualCheck: Lt,
                openOutputAsFile: Tt,
                manualCheckSession: se,
                manualCheckCommandInput: yt,
                setManualCheckCommandInput: vt,
                manualCheckOutputInput: Ct,
                setManualCheckOutputInput: $t,
                manualCheckSelectedCommand: at,
                setManualCheckSelectedCommand: it,
                manualCheckStatus: ot,
                setManualCheckStatus: cn,
                copyManualCheckCommand: Va,
                addManualCheckCommand: qa,
                recordManualCheckResult: Ka,
                askAgentFromManualChecks: Ja,
                onClose: () => ae(!1)
              }
            )
          ] })
        ...
        I && /* @__PURE__ */ a(
          cs,
          {
            ...
            saveMessageAsArtifact: _a,
            rememberMessage: Ha,
            previewEditFromMessage: (c) => Be(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: gn,
            pendingPatchPrompt: W,
            generatePatchPrompt: Ua,
            workspaceFileCount: g.length,
            aiStatus: D.aiStatus,
            chatSettings: ke,
            openSettings: st,
            busy: D.busy,
            memoryHitCount: D.memoryHits.length,
            outputs: Re,
            runAiSynthesis: Mt,
            captureManualCheck: Lt,
            openOutputAsFile: Tt,
            previewEditFromOutput: (c) => Be(c.title, c.body),
            ...
            contextScope: ne,
            setContextScope: xe,
            contextAttachments: Pa,
            removeContextAttachment: La,
            revealContextAttachment: Ra,
            projectIndexSummary: `${Le.snapshot.files.length} files · ${Le.snapshot.chunks.length} chunks`,
            projectIndexStale: Le.isStale,
            refreshProjectIndex: () => {
              const c = Le.refresh(g);
        ...
        bt && /* @__PURE__ */ a(
          is,
          {
            ...
            activeFile: O,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: It },
              ...
              ...Ke.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => Pt(c) })),
              { label: "File: Save All", detail: `${be.length} dirty file${be.length === 1 ? "" : "s"}`, run: () => {
                pn();
              }, disabled: be.length === 0 },
              { label: "File: Close All Editors", detail: `${we.length} open editor${we.length === 1 ? "" : "s"}`, run: Da, disabled: we.length === 0 },
              ...
              { label: "Help: Show Mission Control", detail: "Open the Atomek control tower landing page", run: () => {
                ge(null), ie(!1), s("computer"), p(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: h ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => p((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: I ? "Hide right AI side bar" : "Show right AI side bar", run: () => L((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: K ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => ae((c) => !c) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: st },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => Xe("Manual check requested from command palette") },
              ...
              { label: "Atomek: Create AI Synthesis", detail: O || we.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Mt, disabled: !O && we.length === 0 },
              { label: "AI: Explain Active File", detail: O ? `Ask Cortex to explain ${O.path}` : "Open a file first", run: () => ye("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !O },
              { label: "AI: Improve Active File", detail: O ? `Ask Cortex for concrete edits to ${O.path}` : "Open a file first", run: () => ye("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !O },
              { label: "AI: Draft Editable Replacement", detail: O ? `Ask Cortex for a full-file replacement for ${O.path}` : "Open a file first", run: () => gn("edit"), disabled: !O },
              { label: "AI: Plan Workspace Work", detail: we.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => ye("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: we.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: O ? "Persist active file in host.ai artifacts" : "Open a file first", run: Ba, disabled: !O },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Lt },
              { label: "AI: Open Latest Artifact as File", detail: Re[0] ? `Create editable file from ${Re[0].title}` : "No outputs yet", run: () => Re[0] && Tt(Re[0]), disabled: Re.length === 0 }
            ],
            openWorkbenchFile: fe,
            onClose: () => qe(!1)
          }
        ...
        /* @__PURE__ */ a(ys, { status: Ia, file: O ?? zr, cursor: N, fileCount: g.length, dirtyCount: be.length })
      ]
    }
  );
}
function Kr({ active: e, setActive: t, openSettings: n, settingsActive: i }) {
  return /* @__PURE__ */ l("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(zn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(Oi, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(Hn, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(Fn, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(Yt, { size: 25 }), label: "Control Tower", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(ui, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(je, { icon: /* @__PURE__ */ a(Fi, { size: 23 }), label: "Settings", active: i, onClick: n })
  ] });
}
function je({ icon: e, label: t, active: n, onClick: i }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: i, children: e });
}
function Jr(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(ns, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(En, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(En, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(Aa, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(Gr, { ...e });
}
function Gr(e) {
  const t = !e.folder, n = pe(() => Xr(e.files, e.folder?.name), [e.files, e.folder?.name]), [i, o] = $(() => /* @__PURE__ */ new Set());
  oe(() => {
    const s = va(n);
    o((h) => {
      const p = new Set(h);
    ...
    /* @__PURE__ */ l("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ l(De, { children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(mt, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((s) => /* @__PURE__ */ a(sn, { file: s, active: s.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(s), label: s.name, detail: s.path }, s.id)),
        /* @__PURE__ */ l("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(mt, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : Ca(n, e.activeFileId, e.openWorkbenchFile, i, r)
      ] }),
...
function sn({ file: e, active: t, onOpen: n, basePath: i, depth: o = 0, label: r, detail: s }) {
  const h = i && e.path.startsWith(`${i}/`) ? e.path.slice(i.length + 1) : e.path, p = o || Math.max(0, h.split("/").length - 1);
  return /* @__PURE__ */ l("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": p }, onClick: n, title: e.path, children: [
    ...
    /* @__PURE__ */ l("span", { className: "workbench-row-text", children: [
      /* @__PURE__ */ a("span", { className: "workbench-row-name", children: r ?? h }),
...
function Xr(e, t) {
  const n = [], i = /* @__PURE__ */ new Map(), o = (s, h, p = n) => {
    const f = i.get(s);
    if (f) return f;
    const b = { name: h, path: s, children: [] };
    return i.set(s, b), p.push(b), b;
  };
  e.forEach((s) => {
    const h = t && s.path.startsWith(`${t}/`) ? s.path.slice(t.length + 1) : s.path, p = h.split("/").filter(Boolean);
    let f = n, b = "";
    p.slice(0, -1).forEach((u) => {
      b = b ? `${b}/${u}` : u, f = o(b, u, f).children;
    }), f.push({ name: p.at(-1) ?? s.name, path: h, file: s, children: [] });
  });
  const r = (s) => s.sort((h, p) => +!!h.file - +!!p.file || h.name.localeCompare(p.name)).map((h) => ({ ...h, children: r(h.children) }));
  return r(n);
}
function va(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...va(t.children)]);
}
function Ca(e, t, n, i, o, r = 0) {
  return e.map((s) => {
    if (s.file)
      return /* @__PURE__ */ a(sn, { file: s.file, active: s.file.id === t, onOpen: () => n(s.file), depth: r, label: s.name }, s.file.id);
    const h = i.has(s.path);
    return /* @__PURE__ */ l("div", { children: [
      /* @__PURE__ */ l("button", { className: "workbench-folder-row", style: { "--workbench-depth": r }, onClick: () => o(s.path), title: h ? `Collapse ${s.name}` : `Expand ${s.name}`, children: [
        h ? /* @__PURE__ */ a(mt, { size: 12 }) : /* @__PURE__ */ a(mt, { className: "workbench-chevron-collapsed", size: 12 }),
        h ? /* @__PURE__ */ a(en, { size: 14 }) : /* @__PURE__ */ a(_n, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: s.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: s.children.length })
      ] }),
      h ? Ca(s.children, t, n, i, o, r + 1) : null
    ] }, s.path);
  });
}
function Yr({ file: e, folder: t, showWelcome: n }) {
  const i = n ? ["Mission Control"] : e?.path.split("/").filter(Boolean) ?? [], o = t && i[0] === t.name ? i.slice(1) : i;
  return /* @__PURE__ */ a("div", { className: "workbench-breadcrumb", children: o.length === 0 ? /* @__PURE__ */ a("span", { children: "Workspace" }) : o.map((r, s) => /* @__PURE__ */ l("span", { className: "workbench-breadcrumb-part", children: [
...
function Qr(e) {
  return /* @__PURE__ */ l("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ l("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ a(Ie, { size: 13 }) })
    ] }),
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ a(Qt, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(Si, { size: 16 }) })
  ] });
}
function Zr(e) {
...
function es(e) {
  ...
  return [...e?.resources ?? []].sort((n, i) => (t[n.kind] ?? 9) - (t[i.kind] ?? 9) || n.label.localeCompare(i.label)).slice(0, 8);
}
function ts({
  host: e,
  openFile: t,
  ...
  const [f, b] = $("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [u, w] = $(null), [I, L] = $([]), [H, X] = $(!1), [K, ae] = $(null), [ce, P] = $(null), _ = v(async () => {
    X(!0), P(null);
    try {
      ...
      w(g ?? null), L(E), g && s(`Control Tower loaded · ${g.resources.length} resources · ${g.warnings.length} warnings`);
    } catch (g) {
      const E = g instanceof Error ? g.message : String(g);
      P(E), s(`Control Tower resource load failed: ${E}`);
    } finally {
      X(!1);
    }
  ...
  const ee = v(async () => {
    if (!e.missions?.create || !e.missions?.write) {
      s("Mission API unavailable in this Tytus host build"), h();
      return;
    }
    ...
    try {
      const E = await e.missions.create({
        ...
        data: { resourceCount: u?.resources.length ?? 0 }
      };
      await e.missions.write({
        rootPath: E.rootPath,
        files: [
          { path: "MISSION.md", content: Gt(he, u, null, [], g) },
          { path: "MISSION.json", content: ya(he, u, g) },
          { path: "RESOURCES.md", content: Xt(u) },
          { path: "TASKS.md", content: ka(nt(g, u)) },
          { path: "HANDOFF.md", content: wa(he) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(ve)}
` },
...
`) }
      ...
      }), ae(E), tt(E), s(`Mission created: ${E.rootPath}`), h();
    } catch (E) {
      s(`Mission create failed: ${E instanceof Error ? E.message : String(E)}`);
    } finally {
      X(!1);
    }
  }, [f, u, e.missions, h, s]), j = Zr(u), ie = es(u), ue = u?.warnings ?? [], Me = nt(f, u);
  return /* @__PURE__ */ l("div", { className: "workbench-welcome workbench-control-home", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-control-goal-row", children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            ee();
          }, disabled: H, children: "Start mission" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: h, children: "Inspect resources" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: p, children: "Open chat" })
        ] })
      ] }),
      K ? /* @__PURE__ */ l("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: K.rootPath })
      ] }) : null,
    ...
    /* @__PURE__ */ l("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ l("header", { children: [
          ...
          /* @__PURE__ */ l("button", { className: "workbench-button-subtle", onClick: () => {
            ...
            /* @__PURE__ */ a(et, { size: 13 }),
            " ",
            H ? "Refreshing…" : "Refresh"
          ] })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: j.map((g) => /* @__PURE__ */ l("div", { className: "workbench-control-metric", children: [
      ...
      /* @__PURE__ */ l("article", { className: "workbench-control-card", children: [
        ...
        /* @__PURE__ */ l("button", { className: "workbench-start-link", onClick: i, children: [
          /* @__PURE__ */ a(Zt, { size: 18 }),
          "New mission note"
        ] }),
        o.length ? /* @__PURE__ */ l("div", { className: "workbench-control-recent", children: [
          /* @__PURE__ */ a("span", { children: "Recent" }),
          o.slice(0, 3).map((g) => /* @__PURE__ */ a("button", { onClick: () => r(g), children: g.name }, `${g.path}-${g.at}`))
        ] }) : null
      ] }),
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ l("header", { children: [
          /* @__PURE__ */ a("strong", { children: "Resource graph" }),
          /* @__PURE__ */ a("span", { children: u ? `${u.resources.length} discovered` : "not loaded" })
        ] }),
        /* @__PURE__ */ l("div", { className: "workbench-control-resource-list", children: [
          ie.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          ie.map((g) => /* @__PURE__ */ l("div", { className: "workbench-control-resource", children: [
        ...
        ue.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: ue.slice(0, 2).map((g) => /* @__PURE__ */ l("span", { children: [
          g.code,
          ": ",
          g.message
      ...
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: Me.map((g, E) => /* @__PURE__ */ l("button", { className: `workbench-task-card ${g.status}`, onClick: () => b(g.prompt), children: [
          /* @__PURE__ */ a("span", { children: E + 1 }),
...
function ns({ files: e, query: t, setQuery: n, openWorkbenchFile: i, activeFileId: o }) {
  const r = pe(() => as(e, t), [e, t]), s = pe(() => {
    const h = /* @__PURE__ */ new Map();
    return r.forEach((p) => h.set(p.file.id, [...h.get(p.file.id) ?? [], p])), Array.from(h.values()).slice(0, 50);
  }, [r]);
  return /* @__PURE__ */ l("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "SEARCH" }),
    /* @__PURE__ */ l("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ a("input", { className: "workbench-input", value: t, onChange: (h) => n(h.target.value), placeholder: "Search files and text", autoFocus: !0 }),
      /* @__PURE__ */ l("div", { className: "workbench-section-title", children: [
        /* @__PURE__ */ a(pt, { size: 12 }),
        " Results"
      ] }),
      t.trim() ? s.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : s.map((h) => {
        const p = h[0].file;
        return /* @__PURE__ */ l("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(sn, { file: p, active: p.id === o, onOpen: () => i(p) }),
          h.slice(0, 5).map((f) => /* @__PURE__ */ l("button", { className: "workbench-search-hit", onClick: () => i(p, f.lineNumber), title: f.line, children: [
          ...
          h.length > 5 && /* @__PURE__ */ l("div", { className: "workbench-search-more", children: [
            "+",
            h.length - 5,
            " more matches"
          ] })
...
function is(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${Vn(o.language)} file`,
  ...
  return oe(() => {
    ...
    /* @__PURE__ */ a("div", { className: "workbench-command-list", children: i.map((o) => /* @__PURE__ */ l(
      "button",
      {
        disabled: o.disabled,
...
function rs(e) {
  const t = e.session ? Pr(e.session) : "pending";
  return /* @__PURE__ */ l("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ l(De, { children: [
      ...
      e.session.results.length > 0 ? /* @__PURE__ */ l("div", { className: "workbench-check-results", children: [
        ...
        e.session.results.map((n, i) => /* @__PURE__ */ l("article", { children: [
...
function ss(e) {
  return /* @__PURE__ */ l("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ l("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ a("span", {}),
      /* @__PURE__ */ a("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ a(Ie, { size: 14 }) })
    ] }),
    /* @__PURE__ */ l("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(Na, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function cs(e) {
  return /* @__PURE__ */ l("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(Pi, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(fi, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(Ie, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(ls, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      Aa,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ a(Na, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function $a({ body: e }) {
  const t = pe(() => Hr(e), [e]), [n, i] = $(null), o = v((r, s) => {
    (async () => await Ve(s) && (i(r), window.setTimeout(() => i((p) => p === r ? null : p), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((r) => {
    if (r.type === "code") {
      ...
      return /* @__PURE__ */ l("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ l("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ l("button", { onClick: () => o(r.key, r.body), title: "Copy code block", children: [
            s ? /* @__PURE__ */ a(li, { size: 12 }) : /* @__PURE__ */ a(Ut, { size: 12 }),
            s ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Yn(r.body) }
      },
      r.key
    );
  }) });
...
function ls(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = ze(null), [i, o] = $(!0), [r, s] = $(!1), h = pe(() => e.chatMessages.map((u) => `${u.id}:${u.status ?? ""}:${u.body.length}`).join("|"), [e.chatMessages]);
  oe(() => {
    const u = n.current;
    u && (i ? (u.scrollTop = u.scrollHeight, s(!1)) : s(!0));
  }, [i, h]);
  const p = v(() => {
    const u = n.current;
    if (!u) return;
    const I = u.scrollHeight - u.scrollTop - u.clientHeight < 48;
    o(I), I && s(!1);
  }, []), f = v(() => {
    const u = n.current;
    u && (u.scrollTop = u.scrollHeight, o(!0), s(!1));
  }, []), b = v((u) => {
    Ve(u.body);
  }, []);
  return /* @__PURE__ */ l("div", { className: "workbench-chat-wrap", children: [
      ...
      /* @__PURE__ */ l(
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((u) => /* @__PURE__ */ l("option", { value: u.id, children: [
              u.title,
              " · ",
              ds(u.lastMessageAt ?? u.updatedAt)
            ] }, u.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ l("div", { ref: n, className: "workbench-chat-transcript", onScroll: p, children: [
      ...
      ] }) }) : e.chatMessages.map((u) => /* @__PURE__ */ l("div", { className: `workbench-chat-message ${u.role}`, children: [
        /* @__PURE__ */ a("strong", { children: u.role === "user" ? "You" : "Atomek" }),
        u.status === "streaming" ? /* @__PURE__ */ a("em", { children: " streaming" }) : null,
        u.status === "error" ? /* @__PURE__ */ a("em", { children: " error" }) : null,
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a($a, { body: u.body }),
        u.gatewayLabel ? /* @__PURE__ */ l(De, { children: [
          /* @__PURE__ */ a("br", {}),
          /* @__PURE__ */ a("small", { children: u.gatewayLabel })
        ] }) : null,
        u.role === "assistant" && u.status !== "streaming" && u.status !== "error" ? /* @__PURE__ */ l("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action", onClick: () => b(u), title: "Copy this answer", children: [
            /* @__PURE__ */ a(Ut, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action", onClick: () => e.saveMessageAsArtifact(u), title: "Save this answer as an output artifact", children: [
            /* @__PURE__ */ a(Zt, { size: 12 }),
            " Save"
          ] }),
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action", onClick: () => e.rememberMessage(u), title: "Store this answer in Atomek memory", children: [
            /* @__PURE__ */ a(Hn, { size: 12 }),
            " Remember"
          ] }),
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action", onClick: () => e.previewEditFromMessage(u), disabled: e.workspaceFileCount === 0, title: "Preview an editable patch from this answer", children: [
            /* @__PURE__ */ a(Qt, { size: 12 }),
            " Preview"
          ] }),
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(u), disabled: e.busy, title: "Regenerate this answer", children: [
            /* @__PURE__ */ a(et, { size: 12 }),
            " Regenerate"
          ] })
        ] }) : null,
        u.role === "assistant" && u.status === "error" ? /* @__PURE__ */ l("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action", onClick: () => b(u), title: "Copy this error", children: [
            /* @__PURE__ */ a(Ut, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ l("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(u), disabled: e.busy, children: [
            /* @__PURE__ */ a(et, { size: 12 }),
            " Retry"
          ] })
        ] }) : null
      ] }, u.id)),
      r ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: f, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ l("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ l("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("em", { children: Tn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ l("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ l("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ l("span", { className: "workbench-chat-chip muted", children: [
            ...
            const w = typeof u.score == "number" ? u.score.toFixed(2) : null, I = typeof u.vectorScore == "number" ? u.vectorScore.toFixed(2) : null, L = typeof u.keywordScore == "number" ? u.keywordScore.toFixed(1) : null, H = [
              u.path,
            ...
            return /* @__PURE__ */ l("span", { className: "workbench-chat-chip", title: H, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(u), disabled: !u.fileId, title: "Reveal context", children: /* @__PURE__ */ a(wn, { size: 13 }) }),
              u.label,
              w ? /* @__PURE__ */ a("small", { children: w }) : null,
              u.snippet ? /* @__PURE__ */ l("small", { children: [
                u.snippet.slice(0, 60),
                u.snippet.length > 60 ? "…" : ""
              ] }) : null,
              u.dirty ? /* @__PURE__ */ a("small", { children: "dirty" }) : null,
              u.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(u), title: "Remove context", children: /* @__PURE__ */ a(Ie, { size: 11 }) }) : null
            ] }, u.id);
          }),
        ...
        /* @__PURE__ */ l("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: Tn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(Wn, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(Ri, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Tn(e, t, n) {
  const i = e.model.trim(), r = [e.gatewayPreference === "auto" ? t : xa(e.gatewayPreference)];
...
function ms(e) {
  const [t, n] = $([]), [i, o] = $("Loading gateway models…"), [r, s] = $([]), [h, p] = $("Checking embedding capability…");
  oe(() => {
    ...
    return (async () => {
      ...
      try {
        const L = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        n(L.map((H) => ({ id: H.id, gatewayLabel: H.gatewayLabel }))), o(L.length > 0 ? `${L.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (L) {
        if (w.signal.aborted) return;
        n([]), o(`Model discovery failed: ${L instanceof Error ? L.message : String(L)}`);
      }
    })(), () => w.abort();
    ...
    return (async () => {
      ...
      try {
        const H = await Cr(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        s(H.map((X) => ({ id: X.id, gatewayLabel: X.gatewayLabel ?? X.source ?? "AIL" }))), p(H.length > 0 ? `${H.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (H) {
        if (w.signal.aborted) return;
        s([]), p(`Embedding model discovery failed: ${H instanceof Error ? H.message : String(H)}`);
      }
    })(), () => w.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ l("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ l("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ a("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ a(Ie, { size: 15 }) })
    ] }),
    /* @__PURE__ */ l("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ l("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          xa(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-settings-note", children: h })
      ] })
    ] }),
    /* @__PURE__ */ l("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(ut), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function Na({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: i, captureManualCheck: o, openOutputAsFile: r, previewEditFromOutput: s, canPreviewEdit: h = !1, compact: p = !1 }) {
  return /* @__PURE__ */ l("div", { className: `workbench-panel-list ${p ? "compact" : ""}`, children: [
    /* @__PURE__ */ l("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ l("button", { className: "workbench-button-subtle", onClick: i, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ l("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ l("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void Ve(f.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => r(f), children: "Open as file" }),
        s ? /* @__PURE__ */ a(
          "button",
          {
            className: Ln(f.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => s(f),
            disabled: !h,
            children: Ln(f.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        f.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(f.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ a($a, { body: f.body })
    ] }, f.id))
  ] });
}
function ps({ edit: e, onApply: t, onOpenAsFile: n, onClose: i }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ l("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ l("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ a("button", { title: "Close", onClick: i, children: /* @__PURE__ */ a(Ie, { size: 16 }) })
    ] }),
...
function fs({ patch: e, onApply: t, onOpenAsFiles: n, onClose: i }) {
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ l("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((r) => /* @__PURE__ */ l("article", { className: "workbench-workspace-patch-card", children: [
        /* @__PURE__ */ l("header", { children: [
        ...
        /* @__PURE__ */ a("pre", { children: $s(r.proposedContent) })
      ] }, r.fileId)),
...
function gs(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, bs(e.content), "```");
  else if (t.length > 0) {
...
function Aa({
  ...
  const [h, p] = $([]), [f, b] = $([]), [u, w] = $(null), [I, L] = $(!1), [H, X] = $(null), [K, ae] = $("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [ce, P] = $(null), [_, ee] = $([]), [j, ie] = $(() => Ur()), [ue, Me] = $([]), [g, E] = $([]), [he, ve] = $([]), Y = ze([]), [ge, Ee] = $("execute"), Q = _.find((d) => d.status === "running" || d.status === "canceling") ?? _[0] ?? null, N = s === "dock", A = pe(() => nt(K || j?.goal || "", u), [K, j?.goal, u]), S = A.find((d) => d.id === ge) ?? A[1] ?? A[0] ?? null, B = r.filter((d) => d.dirty).length, te = o ? `${o.path} · ${o.language} · ${o.content.length.toLocaleString()} chars${o.dirty ? " · dirty" : ""}` : r.length > 0 ? `${r.length} open editor${r.length === 1 ? "" : "s"}${B ? ` · ${B} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", J = pe(() => [
    {
  ...
  const ne = v(async (d = j) => {
    if (!d?.rootPath || !e.missions?.listRuns)
      return ve([]), [];
    try {
      const y = await e.missions.listRuns(d.rootPath);
      return ve(y), Y.current = y, y;
    } catch (y) {
      return t(`Mission run history failed: ${y instanceof Error ? y.message : String(y)}`), [];
    }
  }, [e.missions, j, t]), xe = v(async (d, y) => {
    if (!d) return;
    const C = [...y].sort((T, F) => Pn(F).localeCompare(Pn(T))).slice(0, 100), R = C.map((T) => JSON.stringify(T)).join(`
`) + (C.length ? `
` : "");
    d.rootPath && e.missions?.write ? await e.missions.write({ rootPath: d.rootPath, files: [{ path: "RUNS.jsonl", content: R }] }) : d.handle && await Dt(d.handle, "RUNS.jsonl", R), ve(C), Y.current = C;
  }, [e.missions]), Ce = v(async (d, y = j) => {
    if (!y) return;
    const C = [
      d,
      ...Y.current.filter((R) => R.id !== d.id)
    ];
    await xe(y, C);
  }, [j, xe]), Ne = v(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      p([]), b([]), w(null), X("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    L(!0), X(null);
    try {
      const [d, y, C, R] = await Promise.all([
        e.local?.listTools?.().catch((T) => (t(`Local tool discovery failed: ${T instanceof Error ? T.message : String(T)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((T) => (t(`Skill registry discovery failed: ${T instanceof Error ? T.message : String(T)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((T) => (t(`Resource graph discovery failed: ${T instanceof Error ? T.message : String(T)}`), null)) ?? Promise.resolve(null),
        e.missions?.list?.().catch((T) => (t(`Mission list failed: ${T instanceof Error ? T.message : String(T)}`), [])) ?? Promise.resolve([])
      ]);
      p(d), b(y), w(C), Me(R), j?.rootPath && ne(j), t(`Control Tower loaded · ${d.length} tools · ${y.length} skills · ${R.length} missions${C ? ` · ${C.resources.length} resources` : ""}`);
    } catch (d) {
      X(d instanceof Error ? d.message : String(d));
    } finally {
      L(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, ne, j, t]);
  oe(() => {
    Ne();
  }, [Ne]), oe(() => {
    const d = (y) => {
      const C = y.detail;
      C?.missionId && (ie(C), ne(C)), Ne();
    };
    return window.addEventListener(Jt, d), () => window.removeEventListener(Jt, d);
  }, [Ne, ne]);
  const Pe = v((d) => {
    const y = Vr(d);
    ie(y), tt(y), ne(y), E([{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "mission.resume",
      message: `Mission resumed from Control Tower: ${y.rootPath ?? y.name}`,
  ...
  }, [ne, t]), le = v(async (d, y, C = []) => {
    const R = [
      ...g,
      ...C,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], T = nt(y || d.goal, u), F = [
      { path: "MISSION.md", content: Gt(d, u, o, r, y) },
      { path: "MISSION.json", content: ya(d, u, y) },
      { path: "RESOURCES.md", content: Xt(u) },
      { path: "TASKS.md", content: ka(T) },
      { path: "HANDOFF.md", content: wa(d) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: R.map((z) => JSON.stringify(z)).join(`
`) + `
` }
    ];
    if (d.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: d.rootPath, files: F });
    else if (d.handle) {
      await Mn(d.handle, "runs");
      for (const z of F) await Dt(d.handle, z.path, z.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    E(R), tt(d);
  }, [o, e.missions, g, r, u]), W = v(async (d, y = {}) => {
    if (j)
      return await le(j, d), j;
    const C = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, R = d.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let T = null;
    if (e.missions?.create) {
      const z = await e.missions.create({ title: C, goal: R });
      T = {
      ...
      T = {
        handle: z,
        name: z.name,
        missionId: `mission-${Date.now()}-${Wr(z.name)}`,
        title: C,
        goal: R,
        source: "browser"
      };
    ...
    if (!T) return null;
    const F = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${T.rootPath ?? T.name}` };
    return ie(T), tt(T), E([F]), await le(T, R, [F]), t(`Mission pack ready in ${T.rootPath ?? T.name}`), T;
  }, [e.missions, j, t, le]), G = v(async (d) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const y = K.trim() || `Open ${d.label} from Atomek with current context.`, C = d.kind === "ai-cli" ? await W(y) : j;
      await e.local.openTerminal({
        toolId: d.id,
        command: d.command,
        cwd: C?.rootPath ?? On(o),
        prompt: C ? `Atomek mission pack ready at ${C.rootPath ?? C.name}. Read MISSION.md and RESOURCES.md. ${te}` : `Opened from Atomek. ${te}`
  ...
  }, [o, te, W, e.local, K, j, t]), me = v(async () => {
    try {
      await W(K.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (d) {
      t(`Mission folder setup failed: ${d instanceof Error ? d.message : String(d)}`);
    }
  }, [W, K, t]), de = v(async (d, y, C, R = j, T) => {
    if (!R) return;
    const F = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), z = T ?? `runs/${F}-${d.id}.md`, re = z.split("/").pop() || `${F}-${d.id}.md`;
    if (R.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: R.rootPath, files: [{ path: z, content: y }] });
    else if (R.handle) {
      const ke = await Mn(R.handle, "runs");
      await Dt(ke, re, y);
    }
    await le(R, K, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${d.label} exited ${C}; transcript saved to ${z}`,
      data: { toolId: d.id, exitCode: C, transcript: z }
    }]);
  }, [e.missions, K, j, le]), bt = v(async (d) => {
    ...
    let C = null;
    try {
      C = await W(y), C && await le(C, y, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${d.label} background review started`,
        data: { toolId: d.id, taskId: S?.id ?? "manual", taskTitle: S?.title ?? "Manual run" }
      }]);
    } catch (F) {
      t(`Mission pack failed before local job start: ${F instanceof Error ? F.message : String(F)}`);
      return;
    }
    P(d.id);
    const R = `local-run-${Date.now()}-${d.id}`, T = zt();
    ee((F) => [{
      id: R,
      toolId: d.id,
      label: d.label,
      ...
      startedAt: Date.now(),
      taskId: S?.id ?? "manual",
      taskTitle: S?.title ?? "Manual run",
      lines: [`[Atomek] Starting ${d.label} local job for ${S?.title ?? "manual run"}…`]
    }, ...F].slice(0, 6));
    try {
      const F = await e.local.runJob({
        toolId: d.id,
        prompt: C ? [
...
`)
      }), z = F.transcriptPath ?? (C?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${d.id}.md` : void 0), re = [
        `[Atomek] Started ${d.label} local job ${F.id}`,
        S ? `[Atomek] Task: ${S.title} (${S.id})` : "[Atomek] Task: manual",
        ...
        taskTitle: S?.title ?? "Manual run",
        transcriptPath: z,
        summary: `Started ${d.label} for ${S?.title ?? "manual run"}`
      };
      Ce(ke, C).catch((V) => {
        t(`Mission run index failed: ${V instanceof Error ? V.message : String(V)}`);
      }), ee((V) => V.map((U) => U.id === R ? { ...U, jobId: F.id, transcriptPath: z } : U));
      const _e = (V) => {
        ee((U) => U.map((se) => se.id === R ? V(se) : se));
      };
      e.local.streamJob(F.id, {
        onLog: (V) => {
          re.push(V), _e((U) => ({ ...U, lines: re.slice(-500) }));
        },
        onDone: (V) => {
          V && (re.push(V), _e((U) => ({ ...U, lines: re.slice(-500) })));
        },
        onFail: (V) => {
          re.push(`[FAIL] ${V}`), _e((U) => ({
            ...U,
          ...
          }), i(`${d.label} local job failed`, re.join(`
`)), de(d, re.join(`
`), -1, C, z).catch((U) => {
...
`);
          _e((se) => ({
            ...
            finishedAt: zt(),
            exitCode: V,
            summary: `${d.label} exited ${V}`
          }, C).catch((se) => {
            t(`Mission run index failed: ${se instanceof Error ? se.message : String(se)}`);
          }), i(`${d.label} local job`, U), de(d, U, V, C, z).catch((se) => {
            t(`Mission transcript save failed: ${se instanceof Error ? se.message : String(se)}`);
          }), P(null);
        },
        onError: () => t(`Local job stream issue for ${d.label}`)
      }), t(`Started ${d.label} mission run${S ? ` · ${S.title}` : ""}`);
    } catch (F) {
      P(null), ee((z) => z.map((re) => re.id === R ? {
        ...re,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...re.lines, `[Atomek] Failed to start: ${F instanceof Error ? F.message : String(F)}`]
      } : re)), t(`Local job failed to start: ${F instanceof Error ? F.message : String(F)}`);
    }
  }, [o, W, e.local, K, r, u, i, de, S, t, Ce, le]), qe = v(async (d) => {
    if (!d.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    ee((y) => y.map((C) => C.id === d.id ? { ...C, status: "canceling", lines: [...C.lines, "[Atomek] Cancel requested…"] } : C)), Ce({
      id: d.id,
      jobId: d.jobId,
      toolId: d.toolId,
      label: d.label,
    ...
    try {
      ...
      ee((C) => C.map((R) => R.id === d.id ? { ...R, status: "running", lines: [...R.lines, `[Atomek] Cancel failed: ${y instanceof Error ? y.message : String(y)}`] } : R)), t(`Local job cancel failed: ${y instanceof Error ? y.message : String(y)}`);
    }
  }, [e.local, t, Ce]), gt = v(async (d) => {
    const y = [
      `Use Tytus resource "${d.label}" (${d.kind}) for the next mission step.`,
...
`);
    ae(y), j && await le(j, y, [{
      ...
      message: `Selected resource ${d.label}`,
      data: { resourceId: d.id, kind: d.kind, status: d.status }
    }]), t(`Selected ${d.label} for mission`);
  }, [j, t, le]), kt = v((d) => {
    const y = d.setupAction, C = y?.commandPreview ? `${y.label}: ${y.commandPreview}` : y?.deepLink ? `${y.label}: ${y.deepLink}` : y?.label ?? `${d.label} needs setup`;
    t(C), Ve(y?.commandPreview ?? y?.deepLink ?? C);
  }, [t]);
  return /* @__PURE__ */ l("aside", { className: N ? "workbench-agent-dock" : "workbench-sidebar", children: [
    N ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "CONTROL TOWER" }),
    /* @__PURE__ */ l("div", { className: N ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ l("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ a(Yt, { size: 18 }),
        /* @__PURE__ */ l("div", { children: [
          /* @__PURE__ */ a("strong", { children: N ? "Mission Runs" : "Tytus Control Tower" }),
      ...
      H && /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: H }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-context-card mission", children: [
        ...
        /* @__PURE__ */ a("span", { children: j ? `${j.rootPath ?? j.name} · ${j.source} · ${g.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
        u ? /* @__PURE__ */ l("span", { children: [
          ga(u.resources),
          u.warnings.length ? ` · ${u.warnings.length} warnings` : ""
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-actions", children: [
        ...
        /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => j && void le(j, K), disabled: !j, children: "Rewrite context files" })
      ] }),
      ue.length ? /* @__PURE__ */ l(De, { children: [
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
        /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: ue.slice(0, N ? 3 : 5).map((d) => /* @__PURE__ */ l(
          "button",
          {
            className: `workbench-mission-row ${j?.missionId === d.missionId ? "active" : ""}`,
            onClick: () => Pe(d),
            title: d.rootPath,
            children: [
      ...
      u?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: u.warnings.slice(0, 3).map((d) => /* @__PURE__ */ l("span", { children: [
        d.code,
        ": ",
        d.message
      ...
      /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: J.map((d) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ae(d.prompt), children: d.label }, d.label)) }),
      /* @__PURE__ */ a(
        "textarea",
        {
      ...
      /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: A.map((d, y) => /* @__PURE__ */ l(
        "button",
        {
          className: `workbench-task-card ${d.status} ${S?.id === d.id ? "active" : ""}`,
          onClick: () => {
            Ee(d.id), ae(d.prompt);
          },
      ...
      Q ? /* @__PURE__ */ l("div", { className: "workbench-agent-run", children: [
        /* @__PURE__ */ l("header", { children: [
          /* @__PURE__ */ l("div", { children: [
            /* @__PURE__ */ a("strong", { children: Q.label }),
            /* @__PURE__ */ l("span", { children: [
              Q.status,
              typeof Q.exitCode == "number" ? ` · exit ${Q.exitCode}` : "",
              Q.taskTitle ? ` · ${Q.taskTitle}` : ""
            ] }),
            Q.transcriptPath ? /* @__PURE__ */ a("span", { children: Q.transcriptPath }) : null
          ] }),
          /* @__PURE__ */ l("div", { className: "workbench-agent-run-actions", children: [
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => i(`${Q.label} local job`, Q.lines.join(`
`)), disabled: Q.lines.length === 0, children: "Save output" }),
            Q.jobId && (Q.status === "running" || Q.status === "canceling") ? /* @__PURE__ */ l("button", { className: "workbench-button-subtle danger", onClick: () => {
              qe(Q);
            }, disabled: Q.status === "canceling", children: [
              /* @__PURE__ */ a(Wn, { size: 12 }),
              " ",
              Q.status === "canceling" ? "Canceling…" : "Cancel"
            ] }) : null
          ] })
        ] }),
        /* @__PURE__ */ a("pre", { className: "workbench-computer-job-log", children: Q.lines.join(`
`) || "[waiting for output]" })
      ] }) : null,
      he.length ? /* @__PURE__ */ l(De, { children: [
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
        /* @__PURE__ */ a("div", { className: "workbench-run-history", children: he.slice(0, N ? 4 : 8).map((d) => /* @__PURE__ */ l("div", { className: "workbench-run-history-row", children: [
          /* @__PURE__ */ l("div", { children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void Ve(d.transcriptPath ?? d.id), title: "Copy transcript path or run id", children: "Copy path" })
        ] }, d.id)) })
      ] }) : null,
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list compact", children: [
        !u && !I ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
        u?.resources.slice(0, N ? 8 : 5).map((d) => /* @__PURE__ */ l("div", { className: "workbench-resource-row", children: [
          /* @__PURE__ */ l("div", { children: [
          ...
          /* @__PURE__ */ l("div", { className: "workbench-resource-row-actions", children: [
            ...
            d.status === "needs-setup" || d.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => kt(d), children: "Setup" }) : null
          ] })
        ] }, d.id))
      ] }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list", children: [
        h.length === 0 && !I ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        h.map((d) => /* @__PURE__ */ l("div", { className: "workbench-computer-card", children: [
          ...
          /* @__PURE__ */ l("div", { className: "workbench-computer-actions", children: [
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
              G(d);
            }, disabled: d.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: ks(d) }),
            d.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              bt(d);
            }, disabled: d.status !== "available" || ce !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: ws(d, ce) }) : null
          ] })
        ] }, d.id))
      ] }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list", children: [
        ...
        f.map((d) => /* @__PURE__ */ l("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ l("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ l("div", { children: [
              /* @__PURE__ */ a("strong", { children: d.title }),
              /* @__PURE__ */ l("span", { children: [
          ...
          d.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: d.triggers.slice(0, 4).map((y) => /* @__PURE__ */ a("span", { children: y }, y)) }) : null,
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
            n(d);
...
function ys({ status: e, file: t, cursor: n, fileCount: i, dirtyCount: o }) {
  return /* @__PURE__ */ l("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: Vn(t.language) })
  ] });
}
function _t(e, t) {
...
function As() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : ut.gatewayPreference,
...
function Ss({ host: e }) {
  return /* @__PURE__ */ a(qr, { host: e });
}
function Ps(e) {
  return function() {
    return /* @__PURE__ */ a(Ss, { host: e.host });
  };
}
export {
  Ps as default
