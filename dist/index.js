 ...
 */
const tn = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), nn = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, a, i) => i ? i.toUpperCase() : a.toLowerCase()
), rt = (e) => {
  const t = nn(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, mt = (...e) => e.filter((t, a, i) => !!t && t.trim() !== "" && i.indexOf(t) === a).join(" ").trim(), an = (e) => {
  for (const t in e)
...
const on = ut(
  ({
    ...
    "svg",
    {
      ...
      className: mt("lucide", o),
      ...!l && !an(s) && { "aria-hidden": "true" },
      ...s
    },
    [
      ...h.map(([m, g]) => He(m, g)),
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const un = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], $e = M("chevron-down", un);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
function Nt(e, t) {
  ...
  return !(/* @__PURE__ */ new Set(["png", "jpg", "jpeg", "gif", "webp", "avif", "mp3", "wav", "flac", "mp4", "mov", "zip", "gz", "tar", "pdf", "dmg", "sqlite", "db"])).has(a);
}
const Xn = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Qn() {
  const e = Ke();
  return !!(e?.showOpenFilePicker && e.showDirectoryPicker);
}
async function Jn() {
  const e = Ke()?.showOpenFilePicker;
  if (!e) return xt(!1);
  const t = await e({ multiple: !0 }), a = await Promise.all(t.map((i) => At(i, i.name, "local-file")));
  return Xe(a.filter(Boolean));
}
async function Gn() {
  const e = Ke()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await xt(!0) };
  const t = await e({ mode: "readwrite" }), a = [];
  return await Ct(t, t.name, a, 0, 320), { name: t.name, handle: t, files: Xe(a) };
}
function Ke() {
  const e = window;
...
async function Ct(e, t, a, i, o) {
  if (!(a.length >= o || i > 8))
    for await (const l of e.values()) {
      if (a.length >= o) break;
      if (l.kind === "directory") {
        if (Xn.has(l.name)) continue;
        await Ct(l, `${t}/${l.name}`, a, i + 1, o);
        continue;
      }
      const h = l, s = await At(h, `${t}/${h.name}`, "local-folder");
      s && a.push(s);
    }
}
function xt(e) {
  return new Promise((t) => {
    ...
    a.type = "file", a.multiple = !0, a.style.display = "none", e && a.setAttribute("webkitdirectory", ""), a.onchange = async () => {
      const i = Array.from(a.files ?? []), o = await Promise.all(i.map(async (l) => {
        const h = l.webkitRelativePath || l.name;
        return Nt(h, l.size) ? {
          id: $t(h),
          name: l.name,
          path: h,
      ...
      a.remove(), t(Xe(o.filter(Boolean)));
    }, document.body.append(a), a.click();
  });
}
async function We(e) {
  if (!e.handle?.createWritable)
    return Yn(e), { ...e, dirty: !1 };
  const t = await e.handle.createWritable();
  return await t.write(e.content), await t.close(), { ...e, dirty: !1 };
}
function Yn(e) {
  const t = new Blob([e.content], { type: "text/plain;charset=utf-8" }), a = URL.createObjectURL(t), i = document.createElement("a");
  i.href = a, i.download = e.name, i.click(), URL.revokeObjectURL(a);
}
function $t(e) {
  let t = 0;
...
}, ra = (e, t = aa) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, lt = (e, t) => {
  const a = e.split(`
`).map((i) => i.replace(/^#+\s*/, "").trim()).find(Boolean);
  return a ? a.length > 80 ? `${a.slice(0, 77)}...` : a : t;
}, ia = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", ct = (e) => ({
  id: e.id,
  title: e.title,
  kind: ia(e.kind),
  body: e.body,
...
`)
  ...
  try {
    ...
    return !t || typeof t != "object" ? {} : Object.fromEntries(
      Object.entries(t).filter(([, a]) => typeof a == "string" && a.trim()).map(([a, i]) => [a, String(i)])
    );
  } catch {
    return {};
  }
  ...
  try {
    const a = Je();
    a[e] = t, localStorage.setItem(Qe, JSON.stringify(a));
  } catch {
  }
}, ca = (e) => {
  try {
    const t = Je();
    delete t[e], localStorage.setItem(Qe, JSON.stringify(t));
  } catch {
  }
}, st = (e) => {
  const t = Je();
  return e.map((a) => t[a.id] ? { ...a, title: t[a.id] } : a);
};
function sa({ host: e, activeFile: t, openEditors: a, chatSettings: i, setStatus: o }) {
  const l = e.ai, [h, s] = A(null), [f, m] = A([]), [g, N] = A([]), [B, j] = A([]), [he, J] = A([]), [Se, Z] = A(ot), [ae, ue] = A(!1), V = at(!0), H = at(null), le = Y(() => na(t, a), [t, a]), ce = v(async () => {
    if (!l) {
      Z(ot);
      return;
    }
    ...
    if (l)
      try {
        const p = await l.listArtifacts({ threadId: b });
        if (!V.current) return;
        j(p.map(ct));
      } catch (p) {
        o(`AI artifacts unavailable: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [l, o]), L = v(async (b) => {
    if (l)
      try {
        const p = await l.listMessages(b);
        if (!V.current) return;
        const C = f.find((P) => P.id === b) ?? null;
        C && s(C), N(p.map(ye).filter(Boolean)), J([]), await k(b);
      } catch (p) {
        o(`Load chat failed: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [l, k, o, f]), ve = v(async () => {
    if (l)
      try {
        const b = st(await l.listThreads({ workspaceKey: de, status: "active" })), p = b[0] ?? await l.createThread({ workspaceKey: de, title: "Atomek chat" });
        if (!V.current) return;
        m(b[0] ? b : [p]), s(p);
        const C = await l.listMessages(p.id);
        if (!V.current) return;
        N(C.map(ye).filter(Boolean)), await k(p.id);
      } catch (b) {
        o(`AI chat unavailable: ${b instanceof Error ? b.message : String(b)}`);
      }
  }, [l, k, o]);
  te(() => (V.current = !0, ce(), ve(), () => {
    V.current = !1;
  }), [ve, ce]);
  const q = v(async () => {
    if (!l) return null;
    if (h) return h;
    const b = await l.createThread({ workspaceKey: de, title: "Atomek chat" });
    return V.current && (s(b), m((p) => [b, ...p.filter((C) => C.id !== b.id)])), b;
  }, [l, h]), _ = v(async () => {
    if (!l) return;
    const b = await l.createThread({ workspaceKey: de, title: "Atomek chat" });
    s(b), m((p) => [b, ...p.filter((C) => C.id !== b.id)]), N([]), j([]), J([]), o("New AI chat created");
  }, [l, o]), ee = v(async (b, p) => {
    if (!l) return;
    const C = p.trim();
    if (C)
      try {
        const P = l.updateThread, E = f.find((O) => O.id === b) ?? h, S = typeof P == "function" ? await P({ threadId: b, title: C }) : { ...E ?? await q(), id: b, title: C, updatedAt: Date.now() };
        typeof P != "function" && la(b, C), m((O) => O.map((W) => W.id === S.id ? S : W)), s((O) => O?.id === S.id ? S : O), o(typeof P == "function" ? `Renamed chat: ${S.title}` : `Renamed chat locally: ${S.title}`);
      } catch (P) {
        o(`Rename chat failed: ${P instanceof Error ? P.message : String(P)}`);
      }
  }, [l, q, o, h, f]), me = v(async (b) => {
    if (l)
      try {
        await l.deleteThread(b), ca(b);
        const p = f.filter((C) => C.id !== b);
        if (m(p), h?.id === b) {
          const C = p[0] ?? await l.createThread({ workspaceKey: de, title: "Atomek chat" });
          s(C), m((E) => E.some((S) => S.id === C.id) ? E : [C, ...E]);
          const P = await l.listMessages(C.id);
          N(P.map(ye).filter(Boolean)), await k(C.id);
        }
        o("Deleted AI chat");
      } catch (p) {
        o(`Delete chat failed: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [l, k, o, h?.id, f]), Pe = v(async (b) => {
    if (!l) return null;
    try {
      const p = await q();
      if (!p) return null;
      const C = await l.createArtifact({
        threadId: p.id,
        messageId: b.messageId ?? null,
        title: b.title?.trim() || lt(b.body, "Atomek artifact"),
        kind: b.kind ?? "markdown",
        body: b.body
      }), P = ct(C);
      return j((E) => [P, ...E.filter((S) => S.id !== P.id)]), o(`Saved AI artifact: ${P.title}`), P;
    } catch (p) {
      return o(`Save artifact failed: ${p instanceof Error ? p.message : String(p)}`), null;
    }
  }, [l, q, o]), Me = v(async (b) => {
    if (l)
      try {
        await l.deleteArtifact(b), j((p) => p.filter((C) => C.id !== b)), o("Deleted AI artifact");
      } catch (p) {
        o(`Delete artifact failed: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [l, o]), Ne = v(async (b) => {
    if (!l) return null;
    try {
      const p = await l.writeMemory({
        title: b.title?.trim() || lt(b.body, "Atomek memory"),
        body: b.body,
        metadata: {
          source: "atomek",
          messageId: b.messageId ?? null
      ...
      return J((C) => [p, ...C.filter((P) => P.id !== p.id)].slice(0, 5)), o(`Remembered: ${p.title}`), p;
    } catch (p) {
      return o(`Remember failed: ${p instanceof Error ? p.message : String(p)}`), null;
    }
  }, [l, o]), re = v(async (b) => {
    if (!l) return [];
    const p = await l.searchMemory({ query: b, limit: 5 });
    return V.current && J(p), p;
  }, [l]), fe = v(async (b) => {
    const p = b.trim();
    if (!p || !l) return null;
    ue(!0);
    ...
    try {
      const E = await q();
      if (!E) return null;
      const S = await re(p).catch(() => []), O = oa(S), W = O ? [...le, O] : le;
      let U = null;
      for await (const T of l.sendMessage({
        threadId: E.id,
        body: p,
        gatewayPreference: i.gatewayPreference,
        model: i.model.trim() || void 0,
        context: W,
        signal: P.signal
      })) {
        if (T.type === "message_created") {
          const F = ye(T.message);
          if (!F) continue;
          F.role === "assistant" && (U = F.id), N((K) => [...K.filter((X) => X.id !== F.id), F]);
        }
        if (T.type === "token" && (U = T.messageId, N((F) => F.map(
          (K) => K.id === T.messageId ? { ...K, body: T.body, status: "streaming" } : K
        ))), T.type === "message_updated" || T.type === "done") {
          const F = ye(T.message);
          if (!F) continue;
          F.role === "assistant" && (C = F), N((K) => K.map((X) => X.id === F.id ? F : X)), F.gatewayLabel && o(`AI answered via ${F.gatewayLabel}`);
        }
        if (T.type === "run_failed") {
          const F = P.signal.aborted;
          o(F ? "AI response stopped" : `AI failed: ${T.error}`), U && N((K) => K.map(
            (X) => X.id === U ? { ...X, status: "error", error: T.error, body: F ? "Stopped by user." : T.error } : X
          ));
        }
      }
      const Ae = st(await l.listThreads({ workspaceKey: de, status: "active" }).catch(() => []));
      return V.current && Ae.length > 0 && m(Ae), ce(), C;
    } catch (E) {
      return E instanceof DOMException && E.name === "AbortError" ? (o("AI response stopped"), C) : (o(`AI failed: ${E instanceof Error ? E.message : String(E)}`), null);
    } finally {
      H.current === P && (H.current = null), ue(!1);
    }
  }, [l, i.gatewayPreference, i.model, le, q, re, ce, o]), Fe = v(() => {
    H.current?.abort(), o("Stopping AI response…");
  }, [o]);
  return {
    aiStatus: Se,
    artifacts: B,
...
const da = Zt(() => import("./WorkbenchMonacoEditor-BaNVDl1J.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), ha = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, St = "tytus.workspace.recent", Pt = "tytus.workspace.layout", Mt = "tytus.atomek.chatAiSettings", xe = {
  gatewayPreference: "auto",
  model: ""
};
function ua({ host: e }) {
  const t = Y(() => Va(), []), [a, i] = A("explorer"), [o, l] = A(t.primaryVisible), [h, s] = A(t.primaryWidth), [f, m] = A("chat"), [g, N] = A(t.secondaryVisible), [B, j] = A(t.secondaryWidth), [he, J] = A(!1), [Se, Z] = A("problems"), [ae, ue] = A(t.markdownPreviewVisible), [V, H] = A(!1), [le, ce] = A(null), [k, L] = A([]), [ve, q] = A([]), [_, ee] = A(null), [me, Pe] = A(""), [Me, Ne] = A({ lineNumber: 1, column: 1 }), [re, fe] = A(""), [Fe, b] = A(!1), [p, C] = A(""), [P, E] = A([]), [S, O] = A(null), [W, U] = A(null), [Ae, T] = A(!1), [F, K] = A(() => Ha()), [X, pe] = A(null), [zt, Ze] = A(null), [Lt, $] = A("Ready"), [be, _t] = A(() => ja()), D = ve.map((r) => k.find((d) => d.id === r)).filter(Boolean), w = _ ? k.find((r) => r.id === _) ?? null : null, x = sa({ host: e, activeFile: w, openEditors: D, chatSettings: F, setStatus: $ }), ie = Y(
    () => [...x.artifacts, ...P].sort((r, d) => d.createdAt - r.createdAt),
    [x.artifacts, P]
  ), Ee = !w && !V, z = Y(() => k.filter((r) => r.dirty), [k]), Rt = Y(() => {
    const r = me.trim().toLowerCase();
    return r ? k.filter((d) => d.path.toLowerCase().includes(r)) : k;
  }, [k, me]), Wt = v((r) => {
    r.preventDefault();
    const d = r.clientX, u = B, y = (se) => {
      const ge = u + (d - se.clientX);
      j(Math.max(380, Math.min(760, ge)));
    }, I = () => {
      window.removeEventListener("pointermove", y), window.removeEventListener("pointerup", I);
    };
    window.addEventListener("pointermove", y), window.addEventListener("pointerup", I);
  }, [B]), Dt = v((r) => {
    r.preventDefault();
    const d = r.clientX, u = h, y = (se) => {
      const ge = u + (se.clientX - d);
      s(Math.max(240, Math.min(460, ge)));
    }, I = () => {
      window.removeEventListener("pointermove", y), window.removeEventListener("pointerup", I);
    };
    window.addEventListener("pointermove", y), window.addEventListener("pointerup", I);
  }, [h]), Ce = v((r) => {
    const d = [r, ...be.filter((u) => u.path !== r.path)].slice(0, 6);
    _t(d), localStorage.setItem(St, JSON.stringify(d));
  }, [be]), R = v((r, d) => {
    q((u) => u.includes(r.id) ? u : [...u, r.id]), ee(r.id), H(!1), Ze(d ?? null), Ne({ lineNumber: d ?? 1, column: 1 });
  }, []), we = v(async () => {
    if (De(z, "open new files"))
      try {
        const r = await Jn();
        if (r.length === 0) return;
        L((d) => Ta(d, r)), r.forEach((d) => Ce({ name: d.name, path: d.path, at: Date.now() })), R(r[0]), $(`Opened ${r.length} local file${r.length === 1 ? "" : "s"}`);
      } catch (r) {
        r.name !== "AbortError" && $(`Open file failed: ${r.message}`);
      }
  }, [z, R, Ce]), Ie = v(async () => {
    if (De(z, "open another folder"))
      try {
        const r = await Gn();
        ce(r), L(r.files), q([]), ee(null), H(!1), Ce({ name: r.name, path: r.name, at: Date.now() }), $(`${r.handle ? "Opened local folder" : "Opened browser fallback folder"} ${r.name} (${r.files.length} text files indexed)`);
      } catch (r) {
        r.name !== "AbortError" && $(`Open folder failed: ${r.message}`);
      }
  }, [z, Ce]), Bt = v((r) => {
    _ && L((d) => d.map((u) => u.id === _ ? { ...u, content: r, dirty: r !== u.content || u.dirty } : u));
  }, [_]), Oe = v(async () => {
    if (w)
      try {
        const r = await We(w);
        L((d) => d.map((u) => u.id === r.id ? r : u)), $(`Saved ${r.name}`);
      } catch (r) {
        $(`Save failed: ${r.message}`);
      }
  }, [w]), jt = v(async (r) => {
    const d = k.find((y) => y.id === r);
    if (!d) return null;
    const u = await We(d);
    return L((y) => y.map((I) => I.id === u.id ? u : I)), u;
  }, [k]), et = v(async () => {
    const r = k.filter((d) => d.dirty);
    if (r.length === 0) {
      $("No dirty files to save");
      return;
    }
    try {
      const d = await Promise.all(r.map((y) => We(y))), u = new Map(d.map((y) => [y.id, y]));
      L((y) => y.map((I) => u.get(I.id) ?? I)), pe(null), $(`Saved ${d.length} dirty file${d.length === 1 ? "" : "s"}`);
    } catch (d) {
      $(`Save all failed: ${d.message}`);
    }
  }, [k]), Te = v((r) => {
    const d = k.find((u) => u.id === r);
    if (d?.dirty && !window.confirm(`${d.name} has unsaved changes. Close without saving?`)) {
      $(`Close canceled — ${d.name} has unsaved changes`);
      return;
    }
    q((u) => {
      const y = u.filter((I) => I !== r);
      return _ === r && ee(y.at(-1) ?? null), y;
    });
  }, [_, k]), Vt = v(() => {
    De(z, "close all editors") && (q([]), ee(null), Ze(null), H(!1), $("Closed all editors"));
  }, [z]), ze = v(() => {
    const r = k.filter((u) => u.name.startsWith("Untitled")).length + 1, d = {
...
`,
    ...
    const d = [
      `Create a polished Markdown artifact from ${w?.path ?? `${D.length} open editors`}.`,
    ...
    N(!0), m("chat"), $("Asking Atomek to synthesize an AI artifact…"), x.askAgent(d).then((u) => {
      !u || u.status === "error" || x.createArtifact({
        messageId: u.id,
        title: `AI synthesis — ${w?.name ?? "open workspace"}`,
        kind: "markdown",
        body: u.body
      }).then(() => {
        N(!0), m("outputs"), J(!0), Z("output");
      });
    });
  }, [w, x, D.length]), _e = v((r) => {
    const d = k.find((u) => u.path === r.path || u.name === r.name);
    if (d) {
      R(d), $(`Opened recent ${d.name}`);
      return;
    }
    ...
    N(!0), m("chat"), fe(""), x.askAgent(r);
  }, [x]), qt = v((r) => {
    const d = x.messages.findIndex((y) => y.id === r.id), u = x.messages.slice(0, d < 0 ? void 0 : d).reverse().find((y) => y.role === "user");
    if (!u?.body.trim()) {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    x.createArtifact({
      title: w.path,
      kind: w.language === "markdown" ? "markdown" : "report",
      body: w.content
    }).then(() => {
      N(!0), m("outputs"), J(!0), Z("output");
    });
  }, [w, x]), Re = v((r) => {
    const d = je(k, Be(r.title || r.kind)), u = {
      id: `artifact-file-${r.id}-${Date.now()}`,
      name: d,
      path: d,
      language: "markdown",
      content: r.body,
    ...
    L((y) => [...y, u]), R(u), $(`Opened ${r.title} as editable file`);
  }, [k, R]), tt = v((r, d) => {
    const u = za(d, k, r);
    if (u.edits.length > 1) {
      U(u), O(null), $(`Previewing AI workspace patch for ${u.edits.length} files`);
      return;
    }
    if (u.edits.length === 1) {
      O(u.edits[0]), U(null), $(`Previewing AI patch for ${u.edits[0].fileName}`);
      return;
    }
    if (!w) {
      $("Open a file before previewing an AI edit, or ask Atomek for a git-style diff against opened files.");
      return;
    }
    const y = Da(d, w);
    if (!y) {
    ...
    U(null), O({
      fileId: w.id,
      fileName: w.path,
      originalContent: w.content,
      proposedContent: y.content,
      sourceTitle: r,
      extractionLabel: y.label,
      stats: Tt(w.content, y.content)
    }), $(`Previewing AI edit for ${w.name}`);
  }, [w, k]), Qt = v(() => {
    if (!S) return;
    const r = k.find((d) => d.id === S.fileId);
    if (!r) {
      $(`Cannot apply edit — ${S.fileName} is no longer open`), O(null);
      return;
    }
    r.content !== S.originalContent && !window.confirm(`${S.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (L((d) => d.map((u) => u.id === S.fileId ? { ...u, content: S.proposedContent, dirty: !0 } : u)), pe(`AI edit applied to ${r.name}. Save All to persist it to disk.`), O(null), $(`Applied AI edit to ${r.name} — unsaved`));
  }, [k, S]), Jt = v(() => {
    if (!S) return;
    const r = je(k, `${Be(S.fileName)}-ai-edit`), d = {
      id: `pending-edit-${S.fileId}-${Date.now()}`,
      name: r,
      path: r,
      language: w?.language ?? "markdown",
      content: S.proposedContent,
      dirty: !0,
      source: "generated"
    };
    L((u) => [...u, d]), R(d), O(null), $(`Opened proposed edit as ${r}`);
  }, [w?.language, k, R, S]), Gt = v(() => {
    if (!W) return;
    const r = new Map(W.edits.map((u) => [u.fileId, u]));
    k.some((u) => {
      const y = r.get(u.id);
      return y && u.content !== y.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (L((u) => u.map((y) => {
      const I = r.get(y.id);
      return I ? { ...y, content: I.proposedContent, dirty: !0 } : y;
    })), pe(`AI workspace patch applied to ${r.size} file${r.size === 1 ? "" : "s"}. Save All to persist changes.`), U(null), $(`Applied AI workspace patch to ${r.size} file${r.size === 1 ? "" : "s"} — unsaved`));
  }, [k, W]), Yt = v(() => {
    if (!W) return;
    const r = [...k], d = W.edits.map((u) => {
      const y = je(r, `${Be(u.fileName)}-ai-edit`), I = k.find((ge) => ge.id === u.fileId), se = {
        id: `workspace-patch-${u.fileId}-${Date.now()}-${y}`,
        name: y,
        path: y,
        language: I?.language ?? "markdown",
        content: u.proposedContent,
        dirty: !0,
        source: "generated"
      };
      return r.push(se), se;
    });
    L((u) => [...u, ...d]), d[0] && R(d[0]), U(null), $(`Opened ${d.length} proposed edit file${d.length === 1 ? "" : "s"}`);
  }, [k, R, W]), nt = v((r) => {
    if (r === "explain") {
      G("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (r === "improve") {
      G("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (r === "plan") {
      G("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (r === "edit") {
      G("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    G("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [G]);
  return te(() => {
    const r = (d) => {
      (d.metaKey || d.ctrlKey) && (d.key.toLowerCase() === "o" && (d.preventDefault(), we()), d.key.toLowerCase() === "s" && (d.preventDefault(), Oe()), d.key.toLowerCase() === "w" && (d.preventDefault(), _ && Te(_)), d.key.toLowerCase() === "b" && (d.preventDefault(), l((y) => !y)), d.shiftKey && d.key.toLowerCase() === "f" && (d.preventDefault(), i("search"), l(!0)), (d.key.toLowerCase() === "k" || d.key.toLowerCase() === "p") && (d.preventDefault(), b(!0)));
    };
    return window.addEventListener("keydown", r), () => window.removeEventListener("keydown", r);
  }, [_, Te, we, Oe]), te(() => {
    const r = (d) => {
      z.length !== 0 && (d.preventDefault(), d.returnValue = "");
    };
    return window.addEventListener("beforeunload", r), () => window.removeEventListener("beforeunload", r);
  }, [z.length]), te(() => {
    z.length === 0 && pe(null);
  }, [z.length]), te(() => {
    const r = { primaryVisible: o, primaryWidth: h, secondaryVisible: g, secondaryWidth: B, markdownPreviewVisible: ae };
    localStorage.setItem(Pt, JSON.stringify(r));
  }, [ae, o, h, g, B]), te(() => {
    localStorage.setItem(Mt, JSON.stringify(F));
  }, [F]), /* @__PURE__ */ c(
    "div",
    {
      className: `workbench-workbench ${o ? "" : "no-primary"} ${g ? "" : "no-secondary"} ${he ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${h}px`, "--workbench-secondary-width": `${B}px` },
      children: [
        /* @__PURE__ */ n(ma, { active: a, setActive: (r) => {
          i(r), l(!0);
        }, openSettings: () => T(!0) }),
        o && /* @__PURE__ */ c("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ n(
            fa,
            {
              activity: a,
          ...
          /* @__PURE__ */ n("div", { className: "workbench-primary-resizer", onPointerDown: Dt, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ c("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ n("button", { className: "workbench-command-center", onClick: () => b(!0), children: "Workspace" }),
          /* @__PURE__ */ c("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ n(
              ga,
              {
                openEditors: D,
                ...
                closeWelcome: () => H(!0),
                secondaryVisible: g,
                toggleSecondary: () => N((r) => !r),
                canPreview: w?.language === "markdown",
                previewVisible: ae,
                togglePreview: () => ue((r) => !r)
              }
            ),
            /* @__PURE__ */ n(wa, { file: w, folder: le, showWelcome: Ee }),
            X && z.length > 0 ? /* @__PURE__ */ c("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ n("span", { children: X }),
              /* @__PURE__ */ n("button", { onClick: () => {
                et();
              }, children: "Save all" }),
              /* @__PURE__ */ n("button", { onClick: () => pe(null), title: "Dismiss", children: /* @__PURE__ */ n(ne, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ n("div", { className: "workbench-editor-content", children: w ? /* @__PURE__ */ c("div", { className: w.language === "markdown" && ae ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ n("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ n(en, { fallback: /* @__PURE__ */ n("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ n(
                da,
                {
                  file: w,
                  revealLine: zt,
              ...
              w.language === "markdown" && ae && /* @__PURE__ */ n(Aa, { content: w.content })
            ] }) : Ee ? /* @__PURE__ */ n(ya, { openFile: we, openFolder: Ie, newFile: ze, recent: be, reopenRecent: _e, setStatus: $ }) : /* @__PURE__ */ c("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ n(qe, { size: 34 }),
              /* @__PURE__ */ n("p", { children: "No editor open" }),
              /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: () => H(!1), children: "Show Welcome" })
            ] }) }),
            he && /* @__PURE__ */ n(
              Ca,
              {
        ...
        g && /* @__PURE__ */ n(
          xa,
          {
            ...
            chatInput: re,
            setChatInput: fe,
            chatMessages: x.messages,
            chatThread: x.thread,
            chatThreads: x.threads,
            askAgent: Ht,
            stopChat: x.stopChat,
            regenerateMessage: qt,
            newChat: () => {
              x.newChat();
            },
            selectThread: (r) => {
            ...
            saveMessageAsArtifact: Ut,
            rememberMessage: Kt,
            previewEditFromMessage: (r) => tt(r.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", r.body),
            runQuickPrompt: nt,
            workspaceFileCount: k.length,
            aiStatus: x.aiStatus,
            chatSettings: F,
            openSettings: () => T(!0),
            busy: x.busy,
            memoryHitCount: x.memoryHits.length,
            outputs: ie,
            runAiSynthesis: Le,
            openOutputAsFile: Re,
            previewEditFromOutput: (r) => tt(r.title, r.body),
            canPreviewEdit: k.length > 0,
            clearOutputs: () => E([]),
            deleteArtifact: (r) => {
              x.deleteArtifact(r);
            },
        ...
        Fe && /* @__PURE__ */ n(
          Na,
          {
            ...
            activeFile: w,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: ze },
              ...
              ...be.map((r) => ({ label: `File: Open Recent — ${r.name}`, detail: r.path, run: () => _e(r) })),
              { label: "File: Save All", detail: `${z.length} dirty file${z.length === 1 ? "" : "s"}`, run: () => {
                et();
              }, disabled: z.length === 0 },
              { label: "File: Close All Editors", detail: `${D.length} open editor${D.length === 1 ? "" : "s"}`, run: Vt, disabled: D.length === 0 },
              ...
              { label: "View: Toggle Primary Side Bar", detail: o ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => l((r) => !r) },
              { label: "View: Toggle Chat Panel", detail: g ? "Hide right AI side bar" : "Show right AI side bar", run: () => N((r) => !r) },
              { label: "View: Toggle Bottom Panel", detail: he ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => J((r) => !r) },
              { label: "View: Toggle Markdown Preview", detail: w?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => ue((r) => !r), disabled: w?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: w || D.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Le, disabled: !w && D.length === 0 },
              { label: "AI: Explain Active File", detail: w ? `Ask Cortex to explain ${w.path}` : "Open a file first", run: () => G("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !w },
              { label: "AI: Improve Active File", detail: w ? `Ask Cortex for concrete edits to ${w.path}` : "Open a file first", run: () => G("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !w },
              { label: "AI: Draft Editable Replacement", detail: w ? `Ask Cortex for a full-file replacement for ${w.path}` : "Open a file first", run: () => nt("edit"), disabled: !w },
              { label: "AI: Plan Workspace Work", detail: D.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => G("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: D.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: w ? "Persist active file in host.ai artifacts" : "Open a file first", run: Xt, disabled: !w },
              { label: "AI: Open Latest Artifact as File", detail: ie[0] ? `Create editable file from ${ie[0].title}` : "No outputs yet", run: () => ie[0] && Re(ie[0]), disabled: ie.length === 0 }
            ],
            openWorkbenchFile: R,
            onClose: () => b(!1)
          }
        ...
        /* @__PURE__ */ n(Oa, { status: Lt, file: w ?? ha, cursor: Me, fileCount: k.length, dirtyCount: z.length })
      ]
    }
  );
}
function ma({ active: e, setActive: t, openSettings: a }) {
  return /* @__PURE__ */ c("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(gt, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(_n, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n($n, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(hn, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(ft, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ n("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(fn, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ n(oe, { icon: /* @__PURE__ */ n(Bn, { size: 23 }), label: "Manage", active: !1, onClick: a })
  ] });
}
function oe({ icon: e, label: t, active: a, onClick: i }) {
  return /* @__PURE__ */ n("button", { className: `workbench-activity-button ${a ? "active" : ""}`, title: t, "aria-label": t, onClick: i, children: e });
}
function fa(e) {
  return e.activity === "search" ? /* @__PURE__ */ n(ka, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ n(ht, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ n(ht, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ n(Ia, {}) : /* @__PURE__ */ n(pa, { ...e });
}
function pa(e) {
  const t = !e.folder, a = Y(() => ba(e.files, e.folder?.name), [e.files, e.folder?.name]);
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ n("div", { className: "workbench-sidebar-title", children: "EXPLORER" }),
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ c(Ve, { children: [
        /* @__PURE__ */ n("p", { style: { fontWeight: 600, margin: "10px 0" }, children: "NO FOLDER OPENED" }),
        /* @__PURE__ */ n("p", { className: "workbench-muted", children: "You have not yet opened a folder." }),
        /* @__PURE__ */ n("button", { className: "workbench-button-blue", onClick: e.openFolder, children: "Open Folder" }),
        /* @__PURE__ */ n("button", { className: "workbench-button-blue", onClick: e.openFile, children: "Open File" }),
        /* @__PURE__ */ n("button", { className: "workbench-button-blue", onClick: () => e.recent[0] ? e.reopenRecent(e.recent[0]) : e.setStatus("No recent local workspace yet."), children: "Open Recent" }),
        /* @__PURE__ */ n("p", { className: "workbench-muted", children: e.hasFsAccess ? "Local files use browser-native File System Access API." : "Browser fallback may show a file chooser label." })
      ] }) : /* @__PURE__ */ c(Ve, { children: [
        /* @__PURE__ */ c("div", { className: "workbench-sidebar-actions", children: [
          /* @__PURE__ */ c("button", { className: "workbench-button-subtle", onClick: e.openFile, children: [
            /* @__PURE__ */ n(wt, { size: 14 }),
            "Open File"
          ] }),
          /* @__PURE__ */ c("button", { className: "workbench-button-subtle", onClick: e.openFolder, children: [
            /* @__PURE__ */ n(Ue, { size: 14 }),
            "Open Folder"
          ] })
        ] }),
        /* @__PURE__ */ n("input", { className: "workbench-input", value: e.query, onChange: (i) => e.setQuery(i.target.value), placeholder: "Search files" }),
        /* @__PURE__ */ c("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ n($e, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((i) => /* @__PURE__ */ n(Ge, { file: i, active: i.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(i) }, i.id)),
        /* @__PURE__ */ c("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ n($e, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        a.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No readable text files found." }) : Ft(a, e.activeFileId, e.openWorkbenchFile)
      ] }),
      /* @__PURE__ */ n("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((i) => /* @__PURE__ */ c("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(i), children: [
        /* @__PURE__ */ n(Cn, { size: 14 }),
        /* @__PURE__ */ n("span", { className: "workbench-row-name", children: i.name })
      ] }, `${i.path}-${i.at}`))
    ] })
  ] });
}
function Ge({ file: e, active: t, onOpen: a, basePath: i, depth: o = 0, label: l }) {
  const h = i && e.path.startsWith(`${i}/`) ? e.path.slice(i.length + 1) : e.path, s = o || Math.max(0, h.split("/").length - 1);
  return /* @__PURE__ */ c("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": s }, onClick: a, title: e.path, children: [
    /* @__PURE__ */ n(bt, { size: 14 }),
    /* @__PURE__ */ n("span", { className: "workbench-row-name", children: l ?? h }),
    e.dirty && /* @__PURE__ */ n("span", { className: "workbench-row-meta", children: "●" })
  ] });
}
function ba(e, t) {
  const a = [], i = /* @__PURE__ */ new Map(), o = (h, s, f = a) => {
    const m = i.get(h);
    if (m) return m;
    const g = { name: s, path: h, children: [] };
    return i.set(h, g), f.push(g), g;
  };
  e.forEach((h) => {
    const s = t && h.path.startsWith(`${t}/`) ? h.path.slice(t.length + 1) : h.path, f = s.split("/").filter(Boolean);
    let m = a, g = "";
    f.slice(0, -1).forEach((N) => {
      g = g ? `${g}/${N}` : N, m = o(g, N, m).children;
    }), m.push({ name: f.at(-1) ?? h.name, path: s, file: h, children: [] });
  });
  const l = (h) => h.sort((s, f) => +!!s.file - +!!f.file || s.name.localeCompare(f.name)).map((s) => ({ ...s, children: l(s.children) }));
  return l(a);
}
function Ft(e, t, a, i = 0) {
  return e.map((o) => o.file ? /* @__PURE__ */ n(Ge, { file: o.file, active: o.file.id === t, onOpen: () => a(o.file), depth: i, label: o.name }, o.file.id) : /* @__PURE__ */ c("div", { children: [
    /* @__PURE__ */ c("div", { className: "workbench-folder-row", style: { "--workbench-depth": i }, children: [
      /* @__PURE__ */ n($e, { size: 12 }),
      /* @__PURE__ */ n(Ue, { size: 14 }),
      /* @__PURE__ */ n("span", { className: "workbench-row-name", children: o.name })
    ] }),
    Ft(o.children, t, a, i + 1)
  ] }, o.path));
}
function wa({ file: e, folder: t, showWelcome: a }) {
  const i = a ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], o = t && i[0] === t.name ? i.slice(1) : i;
  return /* @__PURE__ */ n("div", { className: "workbench-breadcrumb", children: o.length === 0 ? /* @__PURE__ */ n("span", { children: "Workspace" }) : o.map((l, h) => /* @__PURE__ */ c("span", { className: "workbench-breadcrumb-part", children: [
    h > 0 && /* @__PURE__ */ n("span", { className: "workbench-breadcrumb-sep", children: "›" }),
    l
  ] }, `${l}-${h}`)) });
}
function ga(e) {
  return /* @__PURE__ */ c("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ c("button", { className: "workbench-tab active", children: [
      /* @__PURE__ */ n(qe, { size: 15 }),
      /* @__PURE__ */ n("span", { className: "workbench-tab-name", children: "Welcome" }),
      /* @__PURE__ */ n("span", { className: "workbench-tab-close", role: "button", tabIndex: 0, onClick: (t) => {
        t.stopPropagation(), e.closeWelcome();
      }, children: /* @__PURE__ */ n(ne, { size: 13 }) })
    ] }),
    e.openEditors.map((t) => /* @__PURE__ */ c("button", { className: `workbench-tab ${t.id === e.activeFileId ? "active" : ""}`, onClick: () => e.setActiveFileId(t.id), title: t.path, children: [
      /* @__PURE__ */ n(bt, { size: 15 }),
      /* @__PURE__ */ c("span", { className: "workbench-tab-name", children: [
        t.dirty && /* @__PURE__ */ n("span", { className: "workbench-dirty-dot", children: "●" }),
        t.name
      ] }),
      t.dirty && /* @__PURE__ */ n("span", { className: "workbench-tab-save", role: "button", tabIndex: 0, title: "Save", onClick: (a) => {
        a.stopPropagation(), e.saveFile(t.id);
      }, children: "Save" }),
      /* @__PURE__ */ n("span", { className: "workbench-tab-close", role: "button", tabIndex: 0, onClick: (a) => {
        a.stopPropagation(), e.closeEditor(t.id);
      }, children: /* @__PURE__ */ n(ne, { size: 13 }) })
    ] }, t.id)),
    /* @__PURE__ */ n("div", { style: { flex: 1 } }),
    e.canPreview && /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ n(pt, { size: 16 }) }),
    /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ n(Fn, { size: 16 }) })
  ] });
}
function ya({ openFile: e, openFolder: t, newFile: a, recent: i, reopenRecent: o, setStatus: l }) {
  return /* @__PURE__ */ c("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ c("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ c("section", { children: [
        /* @__PURE__ */ n("h1", { children: "Atomek" }),
        /* @__PURE__ */ n("div", { className: "workbench-welcome-subtitle", children: "Shape files into working artifacts" }),
        /* @__PURE__ */ n("h2", { children: "Start" }),
        /* @__PURE__ */ c("button", { className: "workbench-start-link", onClick: a, children: [
          /* @__PURE__ */ n(wt, { size: 18 }),
          "New File..."
        ] }),
        /* @__PURE__ */ c("button", { className: "workbench-start-link", onClick: e, children: [
          /* @__PURE__ */ n(gt, { size: 18 }),
          "Open File..."
        ] }),
        /* @__PURE__ */ c("button", { className: "workbench-start-link", onClick: t, children: [
          /* @__PURE__ */ n(Ue, { size: 18 }),
          "Open Folder..."
        ] }),
        /* @__PURE__ */ n("h2", { children: "Recent" }),
        i.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : i.map((h) => /* @__PURE__ */ c("button", { className: "workbench-start-link", onClick: () => o(h), children: [
          h.name,
          /* @__PURE__ */ n("span", { className: "workbench-muted", children: "~" })
        ] }, `${h.path}-${h.at}`))
      ] }),
      /* @__PURE__ */ c("section", { children: [
        /* @__PURE__ */ n("h2", { children: "Walkthroughs" }),
        /* @__PURE__ */ c("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ n("strong", { children: "Get Started with Atomek" }),
          /* @__PURE__ */ n("span", { className: "workbench-muted", children: "Open local files, edit with Monaco, then wire Tytus agents and Cortex next." })
        ] }),
        /* @__PURE__ */ c("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ n("strong", { children: "Browse & Edit Local Workspaces" }),
          /* @__PURE__ */ n("span", { className: "workbench-muted", children: "Uses browser-native File System Access API on supported Chromium builds." })
        ] }),
        /* @__PURE__ */ c("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ n("strong", { children: "Learn the Fundamentals" }),
          /* @__PURE__ */ n("span", { className: "workbench-muted", children: "Explorer, tabs, editor, status bar, chat surface." })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ c("label", { className: "workbench-welcome-checkbox", children: [
      /* @__PURE__ */ n("input", { type: "checkbox", defaultChecked: !0 }),
      " Show welcome page on startup"
    ] })
  ] });
}
function ka({ files: e, query: t, setQuery: a, openWorkbenchFile: i, activeFileId: o }) {
  const l = Y(() => va(e, t), [e, t]), h = Y(() => {
    const s = /* @__PURE__ */ new Map();
    return l.forEach((f) => s.set(f.file.id, [...s.get(f.file.id) ?? [], f])), Array.from(s.values()).slice(0, 50);
  }, [l]);
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ n("div", { className: "workbench-sidebar-title", children: "SEARCH" }),
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ n("input", { className: "workbench-input", value: t, onChange: (s) => a(s.target.value), placeholder: "Search files and text", autoFocus: !0 }),
      /* @__PURE__ */ c("div", { className: "workbench-section-title", children: [
        /* @__PURE__ */ n(qe, { size: 12 }),
        " Results"
      ] }),
      t.trim() ? h.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No matches." }) : h.map((s) => {
        const f = s[0].file;
        return /* @__PURE__ */ c("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ n(Ge, { file: f, active: f.id === o, onOpen: () => i(f) }),
          s.slice(0, 5).map((m) => /* @__PURE__ */ c("button", { className: "workbench-search-hit", onClick: () => i(f, m.lineNumber), title: m.line, children: [
            /* @__PURE__ */ n("span", { className: "workbench-search-line", children: m.lineNumber }),
            /* @__PURE__ */ n("span", { children: m.line })
          ] }, `${f.id}-${m.lineNumber}-${m.line}`)),
          s.length > 5 && /* @__PURE__ */ c("div", { className: "workbench-search-more", children: [
            "+",
            s.length - 5,
            " more matches"
          ] })
        ] }, f.id);
      }) : /* @__PURE__ */ n("p", { className: "workbench-muted", children: "Type to search filenames and text in the opened workspace." })
    ] })
  ] });
}
function va(e, t) {
  ...
  const i = [];
  return e.forEach((o) => {
    o.path.toLowerCase().includes(a) && i.push({ file: o, lineNumber: 1, line: o.path }), o.content.split(`
`).some((l, h) => l.toLowerCase().includes(a) ? (i.push({ file: o, lineNumber: h + 1, line: l.trim() || "(blank line)" }), i.length >= 200) : !1);
  }), i.slice(0, 200);
}
function Na(e) {
  const t = e.query.trim().toLowerCase(), a = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${vt(o.language)} file`,
    run: () => e.openWorkbenchFile(o)
  })), i = [...e.commands, ...a].filter((o) => !t || o.label.toLowerCase().includes(t) || o.detail.toLowerCase().includes(t)).slice(0, 12);
  return te(() => {
    const o = (l) => {
      l.key === "Escape" && e.onClose();
    };
    return window.addEventListener("keydown", o), () => window.removeEventListener("keydown", o);
  }, [e.onClose]), /* @__PURE__ */ n("div", { className: "workbench-command-overlay", role: "dialog", "aria-label": "Command Palette", children: /* @__PURE__ */ c("div", { className: "workbench-command-palette", children: [
    /* @__PURE__ */ n(
      "input",
      {
    ...
    /* @__PURE__ */ n("div", { className: "workbench-command-list", children: i.map((o) => /* @__PURE__ */ c(
      "button",
      {
        disabled: o.disabled,
        ...
        children: [
          ...
          /* @__PURE__ */ n("small", { children: o.detail })
        ]
      },
      `${o.label}-${o.detail}`
    )) })
...
function Aa({ content: e }) {
  const t = Y(() => Zn(e), [e]);
  return /* @__PURE__ */ c("aside", { className: "workbench-markdown-preview", children: [
    /* @__PURE__ */ c("div", { className: "workbench-preview-title", children: [
      /* @__PURE__ */ n(pt, { size: 13 }),
      " Preview"
    ] }),
    /* @__PURE__ */ n("div", { className: "workbench-preview-body", dangerouslySetInnerHTML: { __html: t } })
  ] });
}
function Ca(e) {
  return /* @__PURE__ */ c("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ c("div", { className: "workbench-bottom-tabs", children: [
      /* @__PURE__ */ n("button", { className: e.tab === "problems" ? "active" : "", onClick: () => e.setTab("problems"), children: "PROBLEMS" }),
      /* @__PURE__ */ n("button", { className: e.tab === "output" ? "active" : "", onClick: () => e.setTab("output"), children: "OUTPUT" }),
      /* @__PURE__ */ n("button", { className: e.tab === "terminal" ? "active" : "", onClick: () => e.setTab("terminal"), children: "TERMINAL" }),
      /* @__PURE__ */ n("span", {}),
      /* @__PURE__ */ n("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ n(ne, { size: 14 }) })
    ] }),
    /* @__PURE__ */ c("div", { className: "workbench-bottom-body", children: [
      e.tab === "problems" && /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No problems detected in open files. Diagnostics wire in after the base shell is approved." }),
      e.tab === "terminal" && /* @__PURE__ */ c("div", { children: [
        /* @__PURE__ */ n("p", { className: "workbench-muted", children: "Terminal is parked. Native command execution belongs to the pod/daemon integration sprint." }),
        /* @__PURE__ */ n("pre", { className: "workbench-terminal-placeholder", children: "$ atomek --local-first" })
      ] }),
      e.tab === "output" && /* @__PURE__ */ n(Et, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function xa(e) {
  return /* @__PURE__ */ c("aside", { className: "workbench-secondary", children: [
    /* @__PURE__ */ n("div", { className: "workbench-secondary-resizer", onPointerDown: e.onResizeStart, title: "Resize Chat" }),
    /* @__PURE__ */ c("div", { className: "workbench-secondary-tabs", children: [
      /* @__PURE__ */ c("div", { className: "workbench-secondary-tab-group", children: [
        /* @__PURE__ */ n("button", { className: `workbench-secondary-tab ${e.tab === "chat" ? "active" : ""}`, onClick: () => e.setTab("chat"), children: "CHAT" }),
        /* @__PURE__ */ n("button", { className: `workbench-secondary-tab ${e.tab === "outputs" ? "active" : ""}`, onClick: () => e.setTab("outputs"), children: "OUTPUTS" })
      ] }),
      /* @__PURE__ */ c("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ n("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ n(yt, { size: 15 }) }),
        /* @__PURE__ */ n("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ n(bn, { size: 16 }) }),
        /* @__PURE__ */ n("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ n(ne, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ n($a, { ...e }) : /* @__PURE__ */ n(Et, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function $a(e) {
  return /* @__PURE__ */ c("div", { className: "workbench-chat-wrap", children: [
    /* @__PURE__ */ c("div", { className: "workbench-chat-threadbar", children: [
      /* @__PURE__ */ c(
        "select",
        {
          value: e.chatThread?.id ?? "",
          onChange: (t) => e.selectThread(t.target.value),
          disabled: e.busy || e.chatThreads.length === 0,
          title: "Select chat thread",
          children: [
            e.chatThreads.length === 0 ? /* @__PURE__ */ n("option", { value: "", children: "No chats" }) : null,
            e.chatThreads.map((t) => /* @__PURE__ */ c("option", { value: t.id, children: [
              t.title,
              " · ",
              Pa(t.lastMessageAt ?? t.updatedAt)
            ] }, t.id))
          ]
        }
      ),
      /* @__PURE__ */ n(
        "button",
        {
          onClick: () => {
            ...
            const t = window.prompt("Rename chat", e.chatThread.title);
            t !== null && e.renameThread(e.chatThread.id, t);
          },
    ...
    /* @__PURE__ */ n("div", { className: "workbench-chat-transcript", children: e.chatMessages.length === 0 ? /* @__PURE__ */ n("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ c("div", { children: [
      /* @__PURE__ */ n(Pn, { size: 48 }),
      /* @__PURE__ */ n("h3", { children: "Build with Agent" }),
      /* @__PURE__ */ n("p", { children: "Ask about open files, request a plan, or draft an artifact." }),
      /* @__PURE__ */ n("p", { className: "workbench-chat-empty-link", children: e.aiStatus.available ? e.aiStatus.label : e.aiStatus.reason ?? e.aiStatus.label })
    ] }) }) : e.chatMessages.map((t) => /* @__PURE__ */ c("div", { className: `workbench-chat-message ${t.role}`, children: [
      /* @__PURE__ */ n("strong", { children: t.role === "user" ? "You" : "Atomek" }),
      t.status === "streaming" ? /* @__PURE__ */ n("em", { children: " streaming" }) : null,
      t.status === "error" ? /* @__PURE__ */ n("em", { children: " error" }) : null,
      /* @__PURE__ */ n("br", {}),
      t.body,
      t.gatewayLabel ? /* @__PURE__ */ c(Ve, { children: [
        /* @__PURE__ */ n("br", {}),
        /* @__PURE__ */ n("small", { children: t.gatewayLabel })
      ] }) : null,
      t.role === "assistant" && t.status !== "streaming" && t.status !== "error" ? /* @__PURE__ */ c("div", { className: "workbench-chat-message-actions", children: [
        /* @__PURE__ */ n("button", { onClick: () => e.saveMessageAsArtifact(t), children: "Save artifact" }),
        /* @__PURE__ */ n("button", { onClick: () => e.rememberMessage(t), children: "Remember" }),
        /* @__PURE__ */ n("button", { onClick: () => e.previewEditFromMessage(t), disabled: e.workspaceFileCount === 0, children: "Preview edit" }),
        /* @__PURE__ */ c("button", { onClick: () => e.regenerateMessage(t), disabled: e.busy, children: [
          /* @__PURE__ */ n(zn, { size: 11 }),
          " Regenerate"
        ] })
      ] }) : null,
      t.role === "assistant" && t.status === "error" ? /* @__PURE__ */ n("div", { className: "workbench-chat-message-actions", children: /* @__PURE__ */ n("button", { onClick: () => e.regenerateMessage(t), disabled: e.busy, children: "Retry" }) }) : null
    ] }, t.id)) }),
    /* @__PURE__ */ c("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ c("div", { className: "workbench-chat-tip", children: [
        /* @__PURE__ */ n("span", { children: "Context" }),
        /* @__PURE__ */ n("strong", { children: e.activeFile ? e.activeFile.name : "No active file" }),
        /* @__PURE__ */ n("em", { children: Sa(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ c("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ c("div", { className: "workbench-chat-attachments", children: [
          /* @__PURE__ */ n("button", { title: "Add context", children: /* @__PURE__ */ n(yt, { size: 15 }) }),
          /* @__PURE__ */ c("span", { className: "workbench-chat-chip", children: [
            /* @__PURE__ */ n(In, { size: 13 }),
            " ",
            e.activeFile?.name ?? "Open editors"
          ] }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("explain"), disabled: !e.activeFile || e.busy, children: "Explain" }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("improve"), disabled: !e.activeFile || e.busy, children: "Improve" }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("edit"), disabled: !e.activeFile || e.busy, children: "Edit" }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-chip-button", onClick: () => e.runQuickPrompt("draft"), disabled: e.busy, children: "Draft" })
        ] }),
        /* @__PURE__ */ n(
          "textarea",
          {
            ...
            value: e.chatInput,
            onChange: (t) => e.setChatInput(t.target.value),
            onKeyDown: (t) => {
              t.key === "Enter" && !t.shiftKey && (t.preventDefault(), e.busy || e.askAgent());
            },
            placeholder: "Ask Atomek about the open file or describe what to build...",
            rows: 3
          }
        ),
        /* @__PURE__ */ c("div", { className: "workbench-chat-toolbar", children: [
          /* @__PURE__ */ c("button", { className: "workbench-chat-mode", onClick: e.openSettings, title: "Chat AI settings", children: [
            Ye(e.chatSettings.gatewayPreference),
            " ",
            /* @__PURE__ */ n($e, { size: 12 })
          ] }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-mode", onClick: () => e.runQuickPrompt("plan"), disabled: e.busy, children: "Plan" }),
          /* @__PURE__ */ n("span", {}),
          e.busy ? /* @__PURE__ */ n("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ n(qn, { size: 14 }) }) : /* @__PURE__ */ n("button", { className: "workbench-chat-send", onClick: e.askAgent, title: "Send", children: /* @__PURE__ */ n(Wn, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Sa(e, t, a) {
  const i = e.model.trim(), l = [e.gatewayPreference === "auto" ? t : Ye(e.gatewayPreference)];
  return i && l.push(i), a > 0 && l.push(`${a} memories`), l.join(" · ");
}
function Pa(e) {
...
function Ma(e) {
  const [t, a] = A([]), [i, o] = A("Loading gateway models…");
  te(() => {
    const s = new AbortController();
    return (async () => {
      if (!e.host.ai?.listModels) {
        a([]), o("This Tytus build does not expose model discovery yet.");
        return;
      }
      ...
      try {
        ...
        a(m.map((g) => ({ id: g.id, gatewayLabel: g.gatewayLabel }))), o(m.length > 0 ? `${m.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (m) {
        if (s.signal.aborted) return;
        a([]), o(`Model discovery failed: ${m instanceof Error ? m.message : String(m)}`);
      }
    })(), () => s.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]);
  const l = (s) => {
    e.onChange({ ...e.chatSettings, gatewayPreference: s });
  }, h = (s) => {
    e.onChange({ ...e.chatSettings, model: s });
  };
  return /* @__PURE__ */ n("div", { className: "workbench-settings-backdrop", onClick: e.onClose, children: /* @__PURE__ */ c("section", { className: "workbench-settings-dialog", onClick: (s) => s.stopPropagation(), role: "dialog", "aria-modal": "true", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ c("header", { className: "workbench-settings-header", children: [
      /* @__PURE__ */ n(Vn, { size: 15 }),
      /* @__PURE__ */ n("strong", { children: "Atomek Settings" }),
      /* @__PURE__ */ n("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ n(ne, { size: 15 }) })
    ] }),
    /* @__PURE__ */ n("div", { className: "workbench-settings-body", children: /* @__PURE__ */ c("div", { className: "workbench-settings-section", children: [
      /* @__PURE__ */ n("h3", { children: "Chat AI routing" }),
      /* @__PURE__ */ n("p", { children: "Choose which AIL gateway Atomek uses. Model names are not hardcoded here: enter an AIL alias/model from your global gateway config, or leave it empty for the gateway default." }),
      /* @__PURE__ */ c("label", { className: "workbench-settings-label", children: [
        "Gateway",
        /* @__PURE__ */ c(
          "select",
          {
            ...
            children: [
              /* @__PURE__ */ n("option", { value: "auto", children: "Auto failover" }),
              /* @__PURE__ */ n("option", { value: "remote", children: "Remote Tytus AIL only" }),
              /* @__PURE__ */ n("option", { value: "local", children: "Local AIL only" })
            ]
          }
        )
      ] }),
      /* @__PURE__ */ c("label", { className: "workbench-settings-label", children: [
        ...
        /* @__PURE__ */ n("datalist", { id: "atomek-chat-models", children: t.map((s) => /* @__PURE__ */ n("option", { value: s.id, children: s.gatewayLabel }, `${s.gatewayLabel}:${s.id}`)) })
      ] }),
      /* @__PURE__ */ c("div", { className: "workbench-settings-note", children: [
        "Current request: ",
        Ye(e.chatSettings.gatewayPreference),
        e.chatSettings.model.trim() ? ` · ${e.chatSettings.model.trim()}` : " · gateway default"
      ] }),
      /* @__PURE__ */ n("div", { className: "workbench-settings-note", children: i })
    ] }) }),
    /* @__PURE__ */ c("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ n("button", { onClick: () => e.onChange(xe), children: "Reset" }),
      /* @__PURE__ */ n("button", { onClick: e.onClose, children: "Done" })
    ] })
  ] }) });
}
function Et({ outputs: e, clearOutputs: t, deleteArtifact: a, runAiSynthesis: i, openOutputAsFile: o, previewEditFromOutput: l, canPreviewEdit: h = !1, compact: s = !1 }) {
  return /* @__PURE__ */ c("div", { className: `workbench-panel-list ${s ? "compact" : ""}`, children: [
    /* @__PURE__ */ c("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ c("button", { className: "workbench-button-subtle", onClick: i, children: [
        /* @__PURE__ */ n(sn, { size: 14 }),
        "AI synthesis"
      ] }),
      /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ c("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ c("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ n("strong", { children: f.title }),
        /* @__PURE__ */ n("span", { children: f.source === "ai" ? `AI · ${f.kind}` : f.kind }),
        /* @__PURE__ */ n("button", { onClick: () => o(f), children: "Open as file" }),
        l ? /* @__PURE__ */ n(
          "button",
          {
            className: dt(f.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => l(f),
            disabled: !h,
            children: dt(f.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        f.source === "ai" ? /* @__PURE__ */ n("button", { onClick: () => a(f.id), children: "Delete" }) : null
      ] }),
      f.body
    ] }, f.id))
  ] });
}
function Fa({ edit: e, onApply: t, onOpenAsFile: a, onClose: i }) {
  return /* @__PURE__ */ n("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ c("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ c("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ c("div", { children: [
        /* @__PURE__ */ n("strong", { children: "Review AI edit" }),
        /* @__PURE__ */ n("span", { children: e.fileName })
      ] }),
      /* @__PURE__ */ n("button", { title: "Close", onClick: i, children: /* @__PURE__ */ n(ne, { size: 16 }) })
    ] }),
    /* @__PURE__ */ c("div", { className: "workbench-edit-review-meta", children: [
      /* @__PURE__ */ c("span", { children: [
        "Source: ",
        e.sourceTitle
      ] }),
      /* @__PURE__ */ n("span", { children: e.extractionLabel }),
      /* @__PURE__ */ c("span", { children: [
        "+",
        e.stats.added,
        " / -",
    ...
    /* @__PURE__ */ c("div", { className: "workbench-edit-review-grid", children: [
      /* @__PURE__ */ c("div", { className: "workbench-edit-review-pane", children: [
        /* @__PURE__ */ n("h4", { children: "Current" }),
        /* @__PURE__ */ n("pre", { children: e.originalContent })
      ] }),
      /* @__PURE__ */ c("div", { className: "workbench-edit-review-pane proposed", children: [
        /* @__PURE__ */ n("h4", { children: "Proposed" }),
        /* @__PURE__ */ n("pre", { children: e.proposedContent })
      ] })
    ] }),
    /* @__PURE__ */ c("footer", { className: "workbench-edit-review-actions", children: [
      /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: i, children: "Cancel" }),
      /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: a, children: "Open proposed as file" }),
      /* @__PURE__ */ n("button", { className: "workbench-button-primary", onClick: t, children: "Apply to active file" })
    ] })
  ] }) });
}
function Ea({ patch: e, onApply: t, onOpenAsFiles: a, onClose: i }) {
  const o = e.edits.reduce((l, h) => ({
    added: l.added + h.stats.added,
    removed: l.removed + h.stats.removed,
    changed: l.changed + h.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ n("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ c("section", { className: "workbench-edit-review workspace", children: [
    /* @__PURE__ */ c("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ c("div", { children: [
        /* @__PURE__ */ n("strong", { children: "Review AI workspace patch" }),
        /* @__PURE__ */ n("span", { children: e.sourceTitle })
      ] }),
      /* @__PURE__ */ n("button", { title: "Close", onClick: i, children: /* @__PURE__ */ n(ne, { size: 16 }) })
    ] }),
    /* @__PURE__ */ c("div", { className: "workbench-edit-review-meta", children: [
      ...
      e.skipped.length > 0 ? /* @__PURE__ */ c("span", { children: [
        e.skipped.length,
        " skipped"
      ] }) : null
    ] }),
    /* @__PURE__ */ c("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((l) => /* @__PURE__ */ c("article", { className: "workbench-workspace-patch-card", children: [
        /* @__PURE__ */ c("header", { children: [
          /* @__PURE__ */ n("strong", { children: l.fileName }),
          /* @__PURE__ */ c("span", { children: [
            l.extractionLabel,
            " · +",
            l.stats.added,
        ...
        /* @__PURE__ */ n("pre", { children: Wa(l.proposedContent) })
      ] }, l.fileId)),
      e.skipped.length > 0 ? /* @__PURE__ */ c("article", { className: "workbench-workspace-patch-card skipped", children: [
        /* @__PURE__ */ c("header", { children: [
          /* @__PURE__ */ n("strong", { children: "Skipped" }),
          /* @__PURE__ */ n("span", { children: "Paths not open or hunks did not match" })
        ] }),
        /* @__PURE__ */ n("pre", { children: e.skipped.join(`
`) })
      ] }) : null
    ] }),
    /* @__PURE__ */ c("footer", { className: "workbench-edit-review-actions", children: [
      /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: i, children: "Cancel" }),
      /* @__PURE__ */ n("button", { className: "workbench-button-subtle", onClick: a, children: "Open proposals as files" }),
      /* @__PURE__ */ n("button", { className: "workbench-button-primary", onClick: t, children: "Apply workspace patch" })
    ] })
  ] }) });
}
function Ia() {
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ n("div", { className: "workbench-sidebar-title", children: "EXTENSIONS" }),
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ c("div", { className: "workbench-extension-card", children: [
        /* @__PURE__ */ n("strong", { children: "TYTUS EXTENSIONS" }),
        /* @__PURE__ */ n("p", { className: "workbench-muted", children: "Connector surface kept intentionally. Chat, agents, pods, Hyperframes, Blender, and media tools wire in from here." })
      ] }),
      [
      ...
      ].map((t) => /* @__PURE__ */ c("div", { className: "workbench-extension-row", children: [
        /* @__PURE__ */ n(ft, { size: 16 }),
        /* @__PURE__ */ c("div", { children: [
          /* @__PURE__ */ n("strong", { children: t.name }),
          /* @__PURE__ */ n("span", { children: t.detail })
        ] }),
        /* @__PURE__ */ n("em", { children: "coming soon" })
      ] }, t.name))
    ] })
  ] });
}
function ht({ title: e, body: t }) {
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ n("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ n("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function Oa({ status: e, file: t, cursor: a, fileCount: i, dirtyCount: o }) {
  return /* @__PURE__ */ c("footer", { className: "workbench-statusbar", children: [
    /* @__PURE__ */ n("span", { children: "main" }),
    /* @__PURE__ */ c("span", { children: [
      i,
      " files"
    ] }),
    o > 0 && /* @__PURE__ */ c("span", { children: [
      o,
      " unsaved"
    ] }),
    /* @__PURE__ */ n("span", { className: "workbench-status-spacer" }),
    ...
    /* @__PURE__ */ n("span", { children: "Spaces: 2" }),
    /* @__PURE__ */ n("span", { children: "UTF-8" }),
    /* @__PURE__ */ n("span", { children: "LF" }),
    /* @__PURE__ */ n("span", { children: vt(t.language) })
  ] });
}
function De(e, t) {
...
function Ta(e, t) {
  const a = new Map(e.map((i) => [i.id, i]));
  return t.forEach((i) => a.set(i.id, i)), Array.from(a.values());
}
function Be(e) {
  return e.replace(/\.[a-z0-9]+$/i, "").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 54) || "ai-artifact";
}
function je(e, t) {
  const a = new Set(e.map((l) => l.path));
  let i = `${t}.md`, o = 2;
  for (; a.has(i); )
    i = `${t}-${o}.md`, o += 1;
  return i;
...
function za(e, t, a) {
  const o = [
    ...Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((s) => ({
      lang: s[1].trim().toLowerCase(),
      content: Ot(s[2])
    })).filter((s) => s.content.trim().length > 0).filter((s) => Q(s.lang, "diff") || Q(s.lang, "patch")).map((s) => s.content),
    e
  ], l = /* @__PURE__ */ new Map(), h = [];
  for (const s of o)
    for (const f of La(s)) {
      const m = _a(f);
      if (!m) continue;
      const g = Ra(t, m);
      if (!g) {
        h.push(`${m}: no opened file`);
        continue;
      }
      const N = It(g.content, f);
      if (!N || N === g.content) {
        h.push(`${m}: patch did not match or produced no change`);
        continue;
      }
      l.set(g.id, {
        fileId: g.id,
        fileName: g.path,
        originalContent: g.content,
        proposedContent: N,
        sourceTitle: a,
        extractionLabel: `workspace diff (${m})`,
        stats: Tt(g.content, N)
      });
    }
  return { sourceTitle: a, edits: Array.from(l.values()), skipped: Array.from(new Set(h)) };
}
function La(e) {
  const t = e.replace(/\r\n/g, `
`);
...
`), i = [];
  if (a.forEach((h, s) => {
    (h.startsWith("diff --git ") || h.startsWith("--- ")) && i.push(s);
  }), i.length === 0) return [t];
  const o = Array.from(new Set(i)).sort((h, s) => h - s), l = [];
  for (let h = 0; h < o.length; h += 1) {
    const s = o[h], m = o.find((N) => N > s && a[N].startsWith("diff --git ")) ?? a.length, g = a.slice(s, m).join(`
`);
    /^@@\s+-\d+/m.test(g) && l.push(g);
  }
  return l.length > 0 ? l : [t];
}
function _a(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m);
  if (t) return ke(t[3] || t[4] || t[1] || t[2] || "");
  const a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m);
  if (a) return ke(a[1] || a[2] || "");
  const i = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m);
  return i ? ke(i[1] || i[2] || "") : null;
}
function ke(e) {
  return e.replace(/^["']|["']$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function Ra(e, t) {
  const a = ke(t), i = a.split("/").at(-1) ?? a;
  return e.find((o) => {
    const l = ke(o.path);
    return l === a || l.endsWith(`/${a}`) || o.name === i || l.endsWith(`/${i}`);
  }) ?? null;
}
function Wa(e) {
  const t = e.split(`
...
function Da(e, t) {
  const a = Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((m) => ({
    ...
    content: Ot(m[2])
  })).filter((m) => m.content.trim().length > 0), i = [
    ...a.filter((m) => Q(m.lang, "diff") || Q(m.lang, "patch")).map((m) => m.content),
    e
  ];
  for (const m of i) {
    const g = It(t.content, m);
    if (g) return { content: g, label: "unified diff patch" };
  }
  if (a.length === 0) return null;
  const o = Ba(t), l = a.find((m) => Q(m.lang, "atomek-replace") || Q(m.lang, "replace"));
  if (l) return { content: l.content, label: `replacement block (${l.lang || "plain"})` };
  const h = a.find((m) => o.some((g) => Q(m.lang, g)) && !Q(m.lang, "diff") && !Q(m.lang, "patch"));
  if (h) return { content: h.content, label: `matched ${h.lang || t.language} block` };
  const f = a.filter((m) => !Q(m.lang, "diff") && !Q(m.lang, "patch")).sort((m, g) => g.content.length - m.content.length)[0];
  return f ? { content: f.content, label: `largest fenced block (${f.lang || "plain"})` } : null;
}
function It(e, t) {
...
`), o = [];
  ...
  for (let s = 0; s < i.length; s += 1) {
    const f = i[s].match(/^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/);
    if (!f) continue;
    h = !0;
    const m = Number(f[1]), g = Math.max(0, m - 1);
    if (g < l) return null;
    for (o.push(...a.slice(l, g)), l = g, s += 1; s < i.length; s += 1) {
      const N = i[s];
      if (N.startsWith("@@ ")) {
        s -= 1;
        break;
      }
      if (N.startsWith("diff --git ") || N.startsWith("--- ") || N.startsWith("+++ ") || N.startsWith("\\ No newline at end of file")) continue;
      const B = N[0], j = N.slice(1);
      if (B === " ") {
        if (a[l] !== j) return null;
        o.push(j), l += 1;
        continue;
      }
      if (B === "-") {
  ...
  return h ? (o.push(...a.slice(l)), o.join(`
`)) : null;
}
function Ot(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function Q(e, t) {
  return e.split(/[\s,]+/).filter(Boolean).includes(t);
}
function Ba(e) {
...
`), o = Math.max(a.length, i.length);
  let l = 0, h = 0, s = 0;
  for (let f = 0; f < o; f += 1)
    a[f] !== i[f] && (a[f] === void 0 ? l += 1 : i[f] === void 0 ? h += 1 : s += 1);
  return { added: l, removed: h, changed: s };
}
function ja() {
  try {
...
function Va() {
  ...
  try {
    ...
    return {
      primaryVisible: typeof a.primaryVisible == "boolean" ? a.primaryVisible : e.primaryVisible,
      primaryWidth: typeof a.primaryWidth == "number" ? Math.max(240, Math.min(460, a.primaryWidth)) : e.primaryWidth,
      secondaryVisible: typeof a.secondaryVisible == "boolean" ? a.secondaryVisible : e.secondaryVisible,
      secondaryWidth: typeof a.secondaryWidth == "number" ? Math.max(380, Math.min(760, a.secondaryWidth)) : e.secondaryWidth,
      markdownPreviewVisible: typeof a.markdownPreviewVisible == "boolean" ? a.markdownPreviewVisible : e.markdownPreviewVisible
    };
  } catch {
    return e;
  }
...
function Ha() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : xe.gatewayPreference,
...
function qa({ host: e }) {
  return /* @__PURE__ */ n(ua, { host: e });
}
function Xa(e) {
  return function() {
    return /* @__PURE__ */ n(qa, { host: e.host });
  };
}
export {
  Xa as default
