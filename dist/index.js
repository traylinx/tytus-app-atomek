 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Qa = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Za = B("check", Qa);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const er = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], rt = B("chevron-down", er);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
function Ur({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const i = e.ai, [o, s] = v(null), [l, m] = v([]), [f, b] = v([]), [h, k] = v([]), [I, T] = v([]), [W, X] = v(ln), [K, Z] = v(!1), ee = Re(!0), P = Re(null), H = y(async () => {
    if (!i) {
      X(ln);
      return;
    }
    ...
    if (i)
      try {
        ...
        if (!ee.current) return;
        const L = l.find((_) => _.id === $) ?? null;
        L && s(L), b(C.map(We).filter(Boolean)), T([]), await Q($);
      } catch (C) {
    ...
    if (i)
      try {
        ...
        if (!ee.current) return;
        m($[0] ? $ : [C]), s(C);
        const L = await i.listMessages(C.id);
        if (!ee.current) return;
        b(L.map(We).filter(Boolean)), await Q(C.id);
      } catch ($) {
  ...
  const oe = y(async () => {
    ...
    return ee.current && (s($), m((C) => [$, ...C.filter((L) => L.id !== $.id)])), $;
  }, [i, o]), ve = y(async () => {
    if (!i) return;
    const $ = await i.createThread({ workspaceKey: Ee, title: "Atomek chat" });
    s($), m((C) => [$, ...C.filter((L) => L.id !== $.id)]), b([]), k([]), T([]), a("New AI chat created");
  }, [i, a]), x = y(async ($, C) => {
    if (!i) return;
    const L = C.trim();
    if (L)
      try {
        const _ = i.updateThread, p = l.find((S) => S.id === $) ?? o, N = typeof _ == "function" ? await _({ threadId: $, title: L }) : { ...p ?? await oe(), id: $, title: L, updatedAt: Date.now() };
        typeof _ != "function" && Br($, L), m((S) => S.map((E) => E.id === N.id ? N : E)), s((S) => S?.id === N.id ? N : S), a(typeof _ == "function" ? `Renamed chat: ${N.title}` : `Renamed chat locally: ${N.title}`);
      } catch (_) {
        a(`Rename chat failed: ${_ instanceof Error ? _.message : String(_)}`);
      }
  }, [i, oe, a, o, l]), G = y(async ($) => {
    if (i)
      try {
        await i.deleteThread($), Wr($);
        const C = l.filter((L) => L.id !== $);
        if (m(C), o?.id === $) {
          const L = C[0] ?? await i.createThread({ workspaceKey: Ee, title: "Atomek chat" });
          s(L), m((p) => p.some((N) => N.id === L.id) ? p : [L, ...p]);
          const _ = await i.listMessages(L.id);
          b(_.map(We).filter(Boolean)), await Q(L.id);
        }
        a("Deleted AI chat");
      } catch (C) {
        a(`Delete chat failed: ${C instanceof Error ? C.message : String(C)}`);
      }
  }, [i, Q, a, o?.id, l]), me = y(async ($) => {
    if (!i) return null;
    try {
      const C = await oe();
      if (!C) return null;
      const L = await i.createArtifact({
        threadId: C.id,
      ...
      return k((p) => [_, ...p.filter((N) => N.id !== _.id)]), a(`Saved AI artifact: ${_.title}`), _;
    } catch (C) {
    ...
    if (i)
      try {
        await i.deleteArtifact($), k((C) => C.filter((L) => L.id !== $)), a("Deleted AI artifact");
      } catch (C) {
    ...
    try {
      ...
      return T((L) => [C, ...L.filter((_) => _.id !== C.id)].slice(0, 5)), a(`Remembered: ${C.title}`), C;
    } catch (C) {
    ...
    try {
      const N = await oe();
      if (!N) return null;
      const S = await ne(L).catch(() => []), E = Hr(S), O = C.requestContext ?? t, D = E ? [...O, E] : [...O];
      let F = null;
      for await (const R of i.sendMessage({
        threadId: N.id,
        body: L,
        ...
        if (R.type === "message_created") {
          const z = We(R.message);
          if (!z) continue;
          z.role === "assistant" && (F = z.id), b((se) => [...se.filter((ce) => ce.id !== z.id), z]);
        }
        if (R.type === "token" && (F = R.messageId, b((z) => z.some((se) => se.id === R.messageId) ? z.map(
          (se) => se.id === R.messageId ? { ...se, body: R.body, status: "streaming" } : se
        ) : [...z, {
          id: R.messageId,
          role: "assistant",
          body: R.body,
          status: "streaming",
          createdAt: Date.now()
        }])), R.type === "message_updated" || R.type === "done") {
          const z = We(R.message);
          if (!z) continue;
          z.role === "assistant" && (_ = z), b((se) => se.map((ce) => ce.id === z.id ? z : ce)), z.gatewayLabel && a(`AI answered via ${z.gatewayLabel}`);
        }
        if (R.type === "run_failed") {
          const z = p.signal.aborted;
          a(z ? "AI response stopped" : `AI failed: ${R.error}`), F && b((se) => se.map((ce) => ce.id !== F ? ce : z ? { ...ce, status: "complete", error: void 0, body: ce.body || "Stopped by user." } : { ...ce, status: "error", error: R.error, body: R.error }));
        }
      }
      const U = un(await i.listThreads({ workspaceKey: Ee, status: "active" }).catch(() => []));
      return ee.current && U.length > 0 && m(U), H(), _;
    } catch (N) {
      return N instanceof DOMException && N.name === "AbortError" ? (a("AI response stopped"), _) : (a(`AI failed: ${N instanceof Error ? N.message : String(N)}`), null);
    } finally {
      P.current === p && (P.current = null), Z(!1);
    }
  }, [i, n.gatewayPreference, n.model, oe, ne, H, t, a]), be = y(() => {
    P.current?.abort(), a("Stopping AI response…");
  }, [a]);
  return {
    aiStatus: W,
    artifacts: h,
...
`), Ai = (e) => Array.from(new Set(
  ...
  const m = y((k = i.current) => {
    const I = (o.current ?? Oe([], a)).refresh(k, a);
    return l(I), I;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), f = y((k = i.current) => {
    const I = (o.current ?? Oe([], a)).update(k, a);
    return l(I), I;
  }, [m]);
  Y(() => {
    n && m(e);
  }, [n, e, m]);
  const b = (o.current ?? Oe([], a)).staleReport(e), h = y((k, I = {}) => {
    const T = o.current ?? Oe([], a);
    return Wn(T.getSnapshot(), k, I, T.staleReport(i.current));
  }, []);
  ...
  try {
    ...
    const k = new Map(l.map((P) => [P.chunkId, P])), I = Bi(n), T = /* @__PURE__ */ new Map();
    t.chunks.filter((P) => i.includeDirty !== !1 || !P.dirty).slice(0, Oi).forEach((P) => T.set(P.id, P)), l.forEach((P) => {
      const H = t.byChunkId.get(P.chunkId);
      H && T.set(H.id, H);
    });
    const W = Array.from(T.values()), X = [];
    let K = 0;
    for (const P of W) {
      s?.aborted;
      const H = Ti(e.appId, f, P) ?? await zi(m, e.appId, f, P, a, s);
      if (!H) continue;
      H.updatedAt > Date.now() - 1e3 && (K += 1);
      const Q = Di(h, H.vector);
      if (!Number.isFinite(Q)) continue;
      const te = k.get(P.id)?.score ?? Wi(P, I), oe = Hi(te) * 0.42 + Math.max(0, Q) * 0.58;
      X.push({ chunk: P, keywordScore: te, vectorScore: Q, score: oe, matchedTerms: I.filter((ve) => P.text.toLowerCase().includes(ve)) });
    }
    if (X.length === 0) return { hits: l.slice(0, i.limit ?? Ue), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: K };
    const Z = new Set(o?.files.map((P) => P.fileId) ?? []), ee = X.sort((P, H) => H.score - P.score || H.keywordScore - P.keywordScore || P.chunk.path.localeCompare(H.chunk.path) || P.chunk.ordinal - H.chunk.ordinal).slice(0, Math.max(i.limit ?? Ue, 1)).map((P) => ji(P, Z.has(P.chunk.fileId)));
    return {
...
function Xn(e) {
  return [...Ut(e).filter((a) => pe(a, "diff") || pe(a, "patch")).map((a) => a.content), e].filter((a, i, o) => Ki(a) && o.findIndex((s) => s === a) === i);
}
function Ki(e) {
...
function Nt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), i = ye(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, o = ye(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, s = o && o !== "/dev/null" ? o : i;
...
function eo(e) {
  ...
  for (const b of Xn(t))
    for (const h of Gi(b)) {
      ...
      const I = Rt(k.file.content, h.raw);
      if (!I || I === k.file.content) {
  ...
  for (const b of Ot(t)) {
    const h = b.path ? gn(n, b.path) : i ? { file: i, normalizedPatchPath: ye(i.path), confidence: "exact" } : null;
    if (!h) {
...
function to(e, t) {
  ...
  const o = n.find((m) => a.some((f) => pe(m, f)) && !pe(m, "diff") && !pe(m, "patch"));
  if (o) return { content: o.content, label: `matched ${o.lang || t.language} block` };
  const l = n.filter((m) => !pe(m, "diff") && !pe(m, "patch")).sort((m, f) => f.content.length - m.content.length)[0];
  return l ? { content: l.content, label: `largest fenced block (${l.lang || "plain"})` } : null;
}
function gn(e, t) {
  const n = ye(t), a = n.split("/").at(-1) ?? n, i = e.find((l) => ye(l.path) === n);
  if (i) return { file: i, normalizedPatchPath: n, confidence: "exact" };
  const o = e.find((l) => ye(l.path).endsWith(`/${n}`));
  if (o) return { file: o, normalizedPatchPath: n, confidence: "suffix" };
  const s = e.find((l) => l.name === a || ye(l.path).endsWith(`/${a}`));
...
}, ke = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(ke) : ot(e) ? Object.values(e).some(ke) : !1, lo = async (e, t) => {
  const n = oo(e);
  if (typeof n?.listModels != "function") return [];
  const a = await n.listModels(t);
  return Array.isArray(a) ? a.map(co).filter((i) => !!i) : [];
}, ho = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : ke(e.capability) || ke(e.capabilities) || ke(e.modality) || ke(e.modalities) || ke(e.task) || ke(e.tasks) || ke(e.type) || ke(e.kind), Qn = (e) => typeof e == "object" && e !== null, uo = (e) => Qn(e.ai) ? e.ai : null, Zn = (e) => typeof uo(e)?.embedText == "function", mo = async (e, t) => Zn(e) ? (await lo(e, t)).filter(ho) : [], po = (e) => Qn(e.ai) ? Zn(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", fo = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], bo = [
...
function Fo({ host: e }) {
  const t = Re(null), [n, a] = v(0), i = ie(() => ms(), []), [o, s] = v("explorer"), [l, m] = v(i.primaryVisible), [f, b] = v(i.primaryWidth), [h, k] = v("chat"), [I, T] = v(i.secondaryVisible), [W, X] = v(i.secondaryWidth), [K, Z] = v(!1), [ee, P] = v("problems"), [H, Q] = v(i.markdownPreviewVisible), [V, te] = v(!1), [oe, ve] = v(null), [x, G] = v([]), [me, fe] = v([]), [J, ne] = v(null), [de, be] = v(""), [$, C] = v({ lineNumber: 1, column: 1 }), [L, _] = v(null), [p, N] = v({}), [S, E] = v(Gr), [O, D] = v([]), [F, U] = v([]), [R, z] = v(null), [se, ce] = v(""), [ma, ct] = v(!1), [pa, fa] = v(""), [qt, qe] = v([]), [ae, Me] = v(null), [Ce, Fe] = v(null), [Kt, Gt] = v(!1), [Ae, ba] = v(() => ps()), [Jt, ze] = v(null), [Pe, lt] = v(null), [dt, ht] = v(""), [ut, mt] = v(""), [Ke, Ge] = v(""), [Je, Xt] = v("failed"), [ga, Yt] = v(null), [ka, A] = v("Ready"), [je, wa] = v(() => us()), he = me.map((c) => x.find((u) => u.id === c)).filter(Boolean), M = J ? x.find((c) => c.id === J) ?? null : null, Qt = ie(() => Kr({ files: x, openEditorIds: me, activeFileId: J, versions: p, activeSelection: L }), [J, L, p, x, me]), Zt = ie(() => ({
    scope: S,
    removedAttachmentIds: O,
    selectedFileIds: []
  }), [S, O]), _e = ie(() => ii(Qt, x, Zt), [Zt, Qt, x]), xe = Ii(x, { autoRefresh: !0, includeDirty: !0 }), en = ie(() => F.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [F]), ya = ie(() => [..._e.attachments, ...en], [_e.attachments, en]), j = Ur({ host: e, requestContext: _e.parts, chatSettings: Ae, setStatus: A }), Ne = ie(
    () => [...j.artifacts, ...qt].sort((c, u) => u.createdAt - c.createdAt),
    [j.artifacts, qt]
  ), Xe = Kt && !M, pt = !M && !Xe && !V, le = ie(() => x.filter((c) => c.dirty), [x]), va = ie(() => {
    const c = de.trim().toLowerCase();
  ...
  const Ca = y((c) => {
    if (c.kind === "index-hit") {
      U((u) => u.filter((g) => g.id !== c.id)), A(`Removed project context: ${c.label}`);
      return;
    }
    D((u) => u.includes(c.id) ? u : [...u, c.id]), A(`Removed chat context: ${c.label}`);
  }, []), Ye = y(() => {
    Gt(!0), ne(null), te(!0), A("Atomek settings opened");
  }, []), tn = y(() => {
    Gt(!1), ne((c) => c ?? me.at(-1) ?? null), me.length === 0 && te(!1), A("Atomek settings closed");
  }, [me]), Te = y((c) => {
    N((u) => ({ ...u, [c]: (u[c] ?? 1) + 1 }));
  }, []), xa = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = St(n), g = c.clientX, w = W, q = (Be) => {
      const xt = w + (g - Be.clientX);
      X(tt(xt, u.secondaryMin, u.secondaryMax));
    }, ge = () => {
    ...
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = St(n), g = c.clientX, w = f, q = (Be) => {
    ...
    if (It(le, "open new files"))
      try {
        ...
        G((u) => ls(u, c)), c.forEach((u) => Qe({ name: u.name, path: u.path, at: Date.now() })), re(c[0]), A(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
    ...
    if (It(le, "open another folder"))
      try {
        const c = await Fr();
        ve(c), G(c.files), fe([]), ne(null), te(!1), Qe({ name: c.name, path: c.name, at: Date.now() }), A(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!J) return;
    let u = !1;
    G((g) => g.map((w) => w.id !== J || w.content === c ? w : (u = !0, { ...w, content: c, dirty: !0 }))), u && Te(J);
  }, [J, Te]), bt = y(async () => {
    if (M)
      try {
        const c = await $t(M);
        G((u) => u.map((g) => g.id === c.id ? c : g)), A(`Saved ${c.name}`);
      } catch (c) {
    ...
    if (!u) return null;
    const g = await $t(u);
    return G((w) => w.map((q) => q.id === g.id ? g : q)), g;
  }, [x]), nn = y(async () => {
    ...
    try {
      ...
      G((w) => w.map((q) => g.get(q.id) ?? q)), ze(null), A(`Saved ${u.length} dirty file${u.length === 1 ? "" : "s"}`);
    } catch (u) {
    ...
    fe((g) => {
      const w = g.filter((q) => q !== c);
      return J === c && (ne(w.at(-1) ?? null), _(null)), w;
    });
  }, [J, x]), Ia = y(() => {
...
`,
    ...
    return U(w), w.length === 0 ? (A(xe.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), u) : (g.mode !== "hybrid" && g.reason ? A(g.reason) : A(`${w.length} project context hit${w.length === 1 ? "" : "s"} · ${g.reason ?? "hybrid retrieval"}`), [
      ...u,
      ...Fi(w)
    ]);
  }, [_e.parts, S, Ae, e, xe]), wt = y(() => {
    if (!M && he.length === 0) {
      A("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const u = [
      `Create a polished Markdown artifact from ${M?.path ?? `${he.length} open editors`}.`,
    ...
    T(!0), k("chat"), A("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const g = await $e(u), w = await j.askAgent(u, { requestContext: g });
      !w || w.status === "error" || j.createArtifact({
        messageId: w.id,
        title: `AI synthesis — ${M?.name ?? "open workspace"}`,
        kind: "markdown",
        body: w.body
      }).then(() => {
        T(!0), k("outputs"), Z(!0), P("output");
      });
    })();
  }, [M, j, $e, he.length]), yt = y((c) => {
    ...
    T(!0), k("chat"), ce(""), (async () => {
      const u = await $e(c);
      await j.askAgent(c, { requestContext: u });
    })();
  }, [j, $e]), an = y(async (c) => {
    if (!e.skills?.get) {
...
`);
      ce((q) => [q.trim(), w].filter(Boolean).join(`
...
`)), T(!0), k("chat"), A(`Attached skill ${u.title} to chat input`);
    ...
    const u = j.messages.findIndex((w) => w.id === c.id), g = j.messages.slice(0, u < 0 ? void 0 : u).reverse().find((w) => w.role === "user");
    if (!g?.body.trim()) {
      A("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const w = await $e(g.body);
      await j.askAgent(g.body, { requestContext: w });
    })();
  }, [j, $e]), Pa = y((c) => {
    j.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    j.createArtifact({
      title: M.path,
      kind: M.language === "markdown" ? "markdown" : "report",
      body: M.content
    }).then(() => {
      T(!0), k("outputs"), Z(!0), P("output");
    });
    ...
    G((w) => [...w, g]), re(g), A(`Opened ${c.title} as editable file`);
  }, [x, re]), Ct = y(() => {
...
`), q = {
    ...
    qe((ge) => [q, ...ge]), Z(!0), P("output"), A(`Captured manual check: ${u}`);
  }, []), Le = y((c, u) => {
    const g = eo({
      body: u,
      files: x,
    ...
    qe((w) => [g, ...w]), T(!0), k("outputs"), Z(!0), P("output"), A(`Saved local job output: ${c}`), (u.includes("```diff") || u.includes("--- a/")) && Le(c, u);
  }, [Le]), Ea = y(() => {
    const c = se.trim();
    if (!c) return;
    ce("");
    const u = Zo(c);
    (async () => {
      const g = await $e(c), w = await j.askAgent(c, { requestContext: g });
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", w.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    Ge(g), ht(""), mt(""), Xt("failed"), Z(!0), P("terminal"), A(u.commands.length > 0 ? `Manual check ready: copy ${u.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [x]), Ra = y((c) => {
    ...
    c.content !== ae.originalContent && !window.confirm(`${ae.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (G((u) => u.map((g) => g.id === ae.fileId ? { ...g, content: ae.proposedContent, dirty: !0 } : g)), Te(ae.fileId), ze(`AI edit applied to ${c.name}. Save All to persist it to disk.`), Me(null), He(`AI edit applied to ${c.name}`), A(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Te, x, ae, He]), Da = y(() => {
    if (!ae) return;
    const c = Pt(x, `${Mt(ae.fileName)}-ai-edit`), u = {
      ...
      name: c,
      path: c,
      language: M?.language ?? "markdown",
      content: ae.proposedContent,
      dirty: !0,
      source: "generated"
    };
    G((g) => [...g, u]), re(u), Me(null), A(`Opened proposed edit as ${c}`);
  }, [M?.language, x, re, ae]), Ha = y(() => {
    if (!Ce) return;
    const c = new Map(Ce.edits.map((g) => [g.fileId, g]));
    x.some((g) => {
      const w = c.get(g.id);
      return w && g.content !== w.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (G((g) => g.map((w) => {
      const q = c.get(w.id);
      return q ? { ...w, content: q.proposedContent, dirty: !0 } : w;
    ...
    if (!Ce) return;
    const c = [...x], u = Ce.edits.map((g) => {
      const w = Pt(c, `${Mt(g.fileName)}-ai-edit`), q = x.find((Be) => Be.id === g.fileId), ge = {
    ...
    G((g) => [...g, ...u]), u[0] && re(u[0]), Fe(null), A(`Opened ${u.length} proposed edit file${u.length === 1 ? "" : "s"}`);
  }, [x, re, Ce]), on = y((c) => {
    if (c === "explain") {
      ue("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (c === "improve") {
      ue("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (c === "plan") {
      ue("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (c === "edit") {
      ue("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    ue("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [ue]);
  return Y(() => {
    const c = (u) => {
      (u.metaKey || u.ctrlKey) && (u.key.toLowerCase() === "o" && (u.preventDefault(), De()), u.key.toLowerCase() === "s" && (u.preventDefault(), bt()), u.key.toLowerCase() === "w" && (u.preventDefault(), J && gt(J)), u.key.toLowerCase() === "b" && (u.preventDefault(), m((w) => !w)), u.shiftKey && u.key.toLowerCase() === "f" && (u.preventDefault(), s("search"), m(!0)), (u.key.toLowerCase() === "k" || u.key.toLowerCase() === "p") && (u.preventDefault(), ct(!0)));
    };
    ...
    if (!n) return;
    const c = St(n);
    b((u) => tt(u, c.primaryMin, c.primaryMax)), X((u) => tt(u, c.secondaryMin, c.secondaryMax));
  }, [n]), Y(() => {
    const c = { primaryVisible: l, primaryWidth: f, secondaryVisible: I, secondaryWidth: W, markdownPreviewVisible: H };
    ...
    {
      ref: t,
      className: `workbench-workbench ${l ? "" : "no-primary"} ${I ? "" : "no-secondary"} ${K ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${f}px`, "--workbench-secondary-width": `${W}px` },
      children: [
          ...
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ r(
              Bo,
              {
                ...
                canPreview: M?.language === "markdown",
                previewVisible: H,
                togglePreview: () => Q((c) => !c)
              }
            ),
            /* @__PURE__ */ r(Ho, { file: M, folder: oe, showWelcome: pt }),
            Jt && le.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
              ...
              /* @__PURE__ */ r("button", { onClick: () => ze(null), title: "Dismiss", children: /* @__PURE__ */ r(we, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: M ? /* @__PURE__ */ d("div", { className: M.language === "markdown" && H ? "workbench-editor-split" : "workbench-editor-single", children: [
              ...
              M.language === "markdown" && H && /* @__PURE__ */ r(Ko, { content: M.content })
            ] }) : Xe ? /* @__PURE__ */ r(
              ts,
              {
                host: e,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
        ...
        ma && /* @__PURE__ */ r(
          qo,
          {
            ...
            commands: [
              ...
              { label: "File: Close All Editors", detail: `${he.length} open editor${he.length === 1 ? "" : "s"}`, run: Ia, disabled: he.length === 0 },
              ...
              { label: "View: Toggle Bottom Panel", detail: K ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => Z((c) => !c) },
              ...
              { label: "View: Toggle Markdown Preview", detail: M?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => Q((c) => !c), disabled: M?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: M || he.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: wt, disabled: !M && he.length === 0 },
              { label: "AI: Explain Active File", detail: M ? `Ask Cortex to explain ${M.path}` : "Open a file first", run: () => ue("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !M },
              { label: "AI: Improve Active File", detail: M ? `Ask Cortex for concrete edits to ${M.path}` : "Open a file first", run: () => ue("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !M },
              { label: "AI: Draft Editable Replacement", detail: M ? `Ask Cortex for a full-file replacement for ${M.path}` : "Open a file first", run: () => on("edit"), disabled: !M },
              { label: "AI: Plan Workspace Work", detail: he.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => ue("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: he.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: M ? "Persist active file in host.ai artifacts" : "Open a file first", run: La, disabled: !M },
        ...
        /* @__PURE__ */ r(cs, { status: ka, file: M ?? Po, cursor: $, fileCount: x.length, dirtyCount: le.length })
      ]
    }
  );
}
...
function Bo(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ d("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ r(we, { size: 13 }) })
    ] }),
...
function Jo(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ d("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ r("span", {}),
      /* @__PURE__ */ r("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ r(we, { size: 14 }) })
    ] }),
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        ...
        /* @__PURE__ */ r("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ r(we, { size: 15 }) })
      ] })
    ] }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            ...
            const k = typeof h.score == "number" ? h.score.toFixed(2) : null, I = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, T = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, W = [
              h.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: W, children: [
              ...
              h.removable ? /* @__PURE__ */ r("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(h), title: "Remove context", children: /* @__PURE__ */ r(we, { size: 11 }) }) : null
            ] }, h.id);
          }),
...
function ts(e) {
  ...
  Y(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(T.map((W) => ({ id: W.id, gatewayLabel: W.gatewayLabel }))), i(T.length > 0 ? `${T.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (T) {
        if (k.signal.aborted) return;
    ...
    return (async () => {
      ...
      try {
        const W = await mo(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: k.signal
        });
        if (k.signal.aborted) return;
        s(W.map((X) => ({ id: X.id, gatewayLabel: X.gatewayLabel ?? X.source ?? "AIL" }))), m(W.length > 0 ? `${W.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (W) {
        if (k.signal.aborted) return;
        s([]), m(`Embedding model discovery failed: ${W instanceof Error ? W.message : String(W)}`);
      }
    })(), () => k.abort();
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ d("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ d("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ r("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ r(we, { size: 15 }) })
    ] }),
...
function ns({ edit: e, onApply: t, onOpenAsFile: n, onClose: a }) {
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ d("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ r("button", { title: "Close", onClick: a, children: /* @__PURE__ */ r(we, { size: 16 }) })
    ] }),
...
function ua({
  ...
  const [l, m] = v([]), [f, b] = v([]), [h, k] = v(null), [I, T] = v(!1), [W, X] = v(null), [K, Z] = v("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [ee, P] = v(null), [H, Q] = v([]), [V, te] = v(null), [oe, ve] = v([]), x = H.find((p) => p.status === "running") ?? H[0] ?? null, G = s === "dock", me = o.filter((p) => p.dirty).length, fe = i ? `${i.path} · ${i.language} · ${i.content.length.toLocaleString()} chars${i.dirty ? " · dirty" : ""}` : o.length > 0 ? `${o.length} open editor${o.length === 1 ? "" : "s"}${me ? ` · ${me} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", J = ie(() => [
    {
    ...
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      m([]), b([]), k(null), X("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    T(!0), X(null);
    try {
      const [p, N, S] = await Promise.all([
        e.local?.listTools?.().catch((E) => (t(`Local tool discovery failed: ${E instanceof Error ? E.message : String(E)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((E) => (t(`Skill registry discovery failed: ${E instanceof Error ? E.message : String(E)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((E) => (t(`Resource graph discovery failed: ${E instanceof Error ? E.message : String(E)}`), null)) ?? Promise.resolve(null)
      ]);
      m(p), b(N), k(S), t(`Computer loaded · ${p.length} tools · ${N.length} skills${S ? ` · ${S.resources.length} resources` : ""}`);
    } catch (p) {
      X(p instanceof Error ? p.message : String(p));
    } finally {
      T(!1);
    }
...
` }
    ...
    if (p.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: p.rootPath, files: O });
    else if (p.handle) {
      await wn(p.handle, "runs");
      for (const D of O) await kn(p.handle, D.path, D.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    ve(E);
  }, [i, e.missions, oe, o, h]), be = y(async (p, N = {}) => {
    if (V)
      return await de(V, p), V;
    const S = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, E = p.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let O = null;
    if (e.missions?.create) {
      const F = await e.missions.create({ title: S, goal: E });
      O = {
        missionId: F.missionId,
        title: F.title,
        goal: F.goal,
        rootPath: F.rootPath,
        name: F.rootPath.split("/").pop() || F.missionId,
        source: "tray"
      };
    } else if (N.allowBrowserPicker) {
      const F = await Eo();
      if (!F)
      ...
      O = {
        handle: F,
        name: F.name,
        missionId: `mission-${Date.now()}-${Oo(F.name)}`,
        title: S,
        goal: E,
        source: "browser"
      };
    ...
    if (!O) return null;
    const D = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${O.rootPath ?? O.name}` };
    return te(O), ve([D]), await de(O, E, [D]), t(`Mission pack ready in ${O.rootPath ?? O.name}`), O;
  }, [e.missions, V, t, de]), $ = y(async (p) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const N = K.trim() || `Open ${p.label} from Atomek with current context.`, S = p.kind === "ai-cli" ? await be(N) : V;
      await e.local.openTerminal({
        toolId: p.id,
        command: p.command,
        cwd: S?.rootPath ?? $n(i),
        prompt: S ? `Atomek mission pack ready at ${S.rootPath ?? S.name}. Read MISSION.md and RESOURCES.md. ${fe}` : `Opened from Atomek. ${fe}`
      }), t(p.kind === "ai-cli" ? `Opened ${p.label} in Tytus Terminal with mission context. Press Enter there to start it.` : "Opened Tytus Terminal");
    } catch (N) {
      t(`Terminal launch failed: ${N instanceof Error ? N.message : String(N)}`);
    }
  }, [i, fe, be, e.local, K, V, t]), C = y(async () => {
    try {
      await be(K.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (p) {
      t(`Mission folder setup failed: ${p instanceof Error ? p.message : String(p)}`);
    }
  }, [be, K, t]), L = y(async (p, N, S, E = V) => {
    if (!E) return;
    const D = `${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${p.id}.md`, F = `runs/${D}`;
    if (E.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: E.rootPath, files: [{ path: F, content: N }] });
    else if (E.handle) {
      const U = await wn(E.handle, "runs");
      await kn(U, D, N);
    }
    await de(E, K, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${p.label} exited ${S}; transcript saved to ${F}`,
      data: { toolId: p.id, exitCode: S, transcript: F }
    }]);
  }, [e.missions, K, V, de]), _ = y(async (p) => {
    ...
    let S = null;
    try {
      S = await be(N), S && await de(S, N, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${p.label} background review started`,
        data: { toolId: p.id }
      }]);
    } catch (O) {
      t(`Mission pack failed before local job start: ${O instanceof Error ? O.message : String(O)}`);
      return;
    }
    P(p.id);
    const E = `local-run-${Date.now()}-${p.id}`;
    Q((O) => [{
      id: E,
      toolId: p.id,
      label: p.label,
    ...
    try {
      const O = await e.local.runJob({
        toolId: p.id,
        prompt: S ? [
          "Tytus mission context pack is active.",
          `Mission: ${S.title}`,
          `Goal: ${S.goal}`,
          S.rootPath ? `Mission folder: ${S.rootPath}` : `Mission folder: ${S.name}`,
...
`)
      }), D = [`[Atomek] Started ${p.label} local job ${O.id}`], F = (U) => {
        Q((R) => R.map((z) => z.id === E ? U(z) : z));
      };
      e.local.streamJob(O.id, {
        onLog: (U) => {
          D.push(U), F((R) => ({ ...R, lines: D.slice(-500) }));
        },
        onDone: (U) => {
          U && (D.push(U), F((R) => ({ ...R, lines: D.slice(-500) })));
        },
        onFail: (U) => {
          D.push(`[FAIL] ${U}`), F((R) => ({
            ...R,
            ...
            finishedAt: Date.now(),
            lines: D.slice(-500)
          })), a(`${p.label} local job failed`, D.join(`
`)), L(p, D.join(`
`), -1, S).catch((R) => {
...
`);
          F((z) => ({
            ...
            finishedAt: Date.now(),
            lines: D.slice(-500)
          })), a(`${p.label} local job`, R), L(p, R, U, S).catch((z) => {
            t(`Mission transcript save failed: ${z instanceof Error ? z.message : String(z)}`);
          }), P(null);
        },
        onError: () => t(`Local job stream issue for ${p.label}`)
      }), t(`Started ${p.label} local job`);
    } catch (O) {
      P(null), Q((D) => D.map((F) => F.id === E ? {
        ...F,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...F.lines, `[Atomek] Failed to start: ${O instanceof Error ? O.message : String(O)}`]
      } : F)), t(`Local job failed to start: ${O instanceof Error ? O.message : String(O)}`);
    }
  }, [i, be, e.local, K, o, h, a, L, t, de]);
  return /* @__PURE__ */ d("aside", { className: G ? "workbench-agent-dock" : "workbench-sidebar", children: [
    G ? null : /* @__PURE__ */ r("div", { className: "workbench-sidebar-title", children: "COMPUTER / AGENTS" }),
    /* @__PURE__ */ d("div", { className: G ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ d("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ r(Ft, { size: 18 }),
        /* @__PURE__ */ d("div", { children: [
          /* @__PURE__ */ r("strong", { children: G ? "Agent Manager" : "Computer cockpit" }),
          /* @__PURE__ */ r("p", { className: "workbench-muted", children: "Open real local CLIs in Tytus Terminal, or run a read-only background review in this panel. Every dispatch gets a mission pack so agents share context, resources, and transcripts. Same-origin host bridge only. No raw browser shell. No hardcoded models." })
        ] })
      ] }),
      ...
      W && /* @__PURE__ */ r("div", { className: "workbench-inline-error", children: W }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-context-card mission", children: [
        /* @__PURE__ */ r("strong", { children: V ? V.title : "No mission folder selected" }),
        /* @__PURE__ */ r("span", { children: V ? `${V.rootPath ?? V.name} · ${V.source} · ${oe.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
        ...
        /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => V && void de(V, K), disabled: !V, children: "Rewrite context files" })
      ] }),
      ...
      /* @__PURE__ */ r("div", { className: "workbench-computer-presets", children: J.map((p) => /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => Z(p.prompt), children: p.label }, p.label)) }),
      /* @__PURE__ */ r(
        "textarea",
        {
...
`) || "[waiting for output]" })
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-list compact", children: [
        !h && !I ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
        h?.resources.slice(0, G ? 8 : 5).map((p) => /* @__PURE__ */ d("div", { className: "workbench-resource-row", children: [
          /* @__PURE__ */ d("div", { children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        l.length === 0 && !I ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        l.map((p) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          ...
          /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
            /* @__PURE__ */ r("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
              $(p);
            }, disabled: p.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: os(p) }),
            p.kind === "ai-cli" ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
              _(p);
            }, disabled: p.status !== "available" || ee !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: ss(p, ee) }) : null
          ] })
        ] }, p.id))
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        f.length === 0 && !I ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
        f.map((p) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          ...
          p.triggers?.length ? /* @__PURE__ */ r("div", { className: "workbench-computer-triggers", children: p.triggers.slice(0, 4).map((N) => /* @__PURE__ */ r("span", { children: N }, N)) }) : null,
