...
import { forwardRef as kn, createElement as Pt, useState as C, useRef as Me, useCallback as y, useEffect as W, lazy as Ta, useMemo as K, Suspense as Ea } from "react";
/**
 ...
 */
const Fa = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Oa = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), an = (e) => {
  const t = Oa(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, yn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), za = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Wa = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Va = L("check", Wa);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Ua = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], Lt = L("chevron-down", Ua);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
async function Pn(e, t, n, a, o) {
  if (!(n.length >= o || a > 8))
    for await (const i of e.values()) {
      if (n.length >= o) break;
      if (i.kind === "directory") {
        if (Nr.has(i.name)) continue;
        await Pn(i, `${t}/${i.name}`, n, a + 1, o);
        continue;
      }
      const s = i, l = await In(s, `${t}/${s.name}`, "local-folder");
      l && n.push(l);
    }
}
function Ln(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const a = Array.from(n.files ?? []), o = await Promise.all(a.map(async (i) => {
...
`).map(
    ...
    const i = a.split("|").filter(Boolean).map((l) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${l.trim()}</th>`).join(""), s = o.trim().split(`
...
const $e = "atomek:default", Pr = 3e3, Ht = "tytus.atomek.threadTitleOverrides", on = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, je = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, Lr = (e, t = Pr) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, cn = (e, t) => {
  const n = e.split(`
`).map((a) => a.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Tr = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", sn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Tr(e.kind),
  body: e.body,
...
function zr({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const o = e.ai, [i, s] = C(null), [l, m] = C([]), [p, f] = C([]), [h, g] = C([]), [$, M] = C([]), [F, X] = C(on), [le, te] = C(!1), Q = Me(!0), S = Me(null), E = y(async () => {
    if (!o) {
      X(on);
      return;
    }
    ...
    if (o)
      try {
        const w = await o.listArtifacts({ threadId: v });
        if (!Q.current) return;
        g(w.map(sn));
      } catch (w) {
    ...
    if (o)
      try {
        const w = await o.listMessages(v);
        if (!Q.current) return;
        const I = l.find((T) => T.id === v) ?? null;
        I && s(I), f(w.map(je).filter(Boolean)), M([]), await Z(v);
      } catch (w) {
    ...
    if (o)
      try {
        const v = ln(await o.listThreads({ workspaceKey: $e, status: "active" })), w = v[0] ?? await o.createThread({ workspaceKey: $e, title: "Atomek chat" });
        if (!Q.current) return;
        m(v[0] ? v : [w]), s(w);
        const I = await o.listMessages(w.id);
        if (!Q.current) return;
        f(I.map(je).filter(Boolean)), await Z(w.id);
      } catch (v) {
  ...
  const oe = y(async () => {
    if (!o) return null;
    if (i) return i;
    const v = await o.createThread({ workspaceKey: $e, title: "Atomek chat" });
    return Q.current && (s(v), m((w) => [v, ...w.filter((I) => I.id !== v.id)])), v;
  }, [o, i]), Ie = y(async () => {
    if (!o) return;
    const v = await o.createThread({ workspaceKey: $e, title: "Atomek chat" });
    s(v), m((w) => [v, ...w.filter((I) => I.id !== v.id)]), f([]), g([]), M([]), a("New AI chat created");
  }, [o, a]), x = y(async (v, w) => {
    if (!o) return;
    const I = w.trim();
    if (I)
      try {
        const T = o.updateThread, D = l.find((V) => V.id === v) ?? i, O = typeof T == "function" ? await T({ threadId: v, title: I }) : { ...D ?? await oe(), id: v, title: I, updatedAt: Date.now() };
        typeof T != "function" && Fr(v, I), m((V) => V.map((be) => be.id === O.id ? O : be)), s((V) => V?.id === O.id ? O : V), a(typeof T == "function" ? `Renamed chat: ${O.title}` : `Renamed chat locally: ${O.title}`);
      } catch (T) {
        a(`Rename chat failed: ${T instanceof Error ? T.message : String(T)}`);
      }
  }, [o, oe, a, i, l]), Y = y(async (v) => {
    if (o)
      try {
        await o.deleteThread(v), Or(v);
        const w = l.filter((I) => I.id !== v);
        if (m(w), i?.id === v) {
          const I = w[0] ?? await o.createThread({ workspaceKey: $e, title: "Atomek chat" });
          s(I), m((D) => D.some((O) => O.id === I.id) ? D : [I, ...D]);
          const T = await o.listMessages(I.id);
          f(T.map(je).filter(Boolean)), await Z(I.id);
        }
        a("Deleted AI chat");
      } catch (w) {
        a(`Delete chat failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [o, Z, a, i?.id, l]), de = y(async (v) => {
    if (!o) return null;
    try {
      const w = await oe();
      if (!w) return null;
      const I = await o.createArtifact({
        threadId: w.id,
        messageId: v.messageId ?? null,
        title: v.title?.trim() || cn(v.body, "Atomek artifact"),
        kind: v.kind ?? "markdown",
        body: v.body
      }), T = sn(I);
      return g((D) => [T, ...D.filter((O) => O.id !== T.id)]), a(`Saved AI artifact: ${T.title}`), T;
    } catch (w) {
    ...
    if (o)
      try {
        await o.deleteArtifact(v), g((w) => w.filter((I) => I.id !== v)), a("Deleted AI artifact");
      } catch (w) {
    ...
    if (!o) return null;
    try {
      const w = await o.writeMemory({
        title: v.title?.trim() || cn(v.body, "Atomek memory"),
        body: v.body,
        metadata: {
          source: "atomek",
          messageId: v.messageId ?? null
      ...
      return M((I) => [w, ...I.filter((T) => T.id !== w.id)].slice(0, 5)), a(`Remembered: ${w.title}`), w;
    } catch (w) {
      return a(`Remember failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [o, a]), ne = y(async (v) => {
    if (!o) return [];
    const w = await o.searchMemory({ query: v, limit: 5 });
    return Q.current && M(w), w;
  }, [o]), Pe = y(async (v, w = {}) => {
    const I = v.trim();
    if (!I || !o) return null;
    te(!0);
    ...
    try {
      const O = await oe();
      if (!O) return null;
      const V = await ne(I).catch(() => []), be = Er(V), Le = w.requestContext ?? t, We = be ? [...Le, be] : [...Le];
      let ge = null;
      for await (const _ of o.sendMessage({
        threadId: O.id,
        body: I,
        ...
        if (_.type === "message_created") {
          const z = je(_.message);
          if (!z) continue;
          z.role === "assistant" && (ge = z.id), f((U) => [...U.filter((G) => G.id !== z.id), z]);
        }
        if (_.type === "token" && (ge = _.messageId, f((z) => z.some((U) => U.id === _.messageId) ? z.map(
          (U) => U.id === _.messageId ? { ...U, body: _.body, status: "streaming" } : U
        ) : [...z, {
          id: _.messageId,
          role: "assistant",
          body: _.body,
          ...
          const z = je(_.message);
          if (!z) continue;
          z.role === "assistant" && (T = z), f((U) => U.map((G) => G.id === z.id ? z : G)), z.gatewayLabel && a(`AI answered via ${z.gatewayLabel}`);
        }
        if (_.type === "run_failed") {
          const z = D.signal.aborted;
          a(z ? "AI response stopped" : `AI failed: ${_.error}`), ge && f((U) => U.map((G) => G.id !== ge ? G : z ? { ...G, status: "complete", error: void 0, body: G.body || "Stopped by user." } : { ...G, status: "error", error: _.error, body: _.error }));
        }
      }
      const ue = ln(await o.listThreads({ workspaceKey: $e, status: "active" }).catch(() => []));
      return Q.current && ue.length > 0 && m(ue), E(), T;
    } catch (O) {
      return O instanceof DOMException && O.name === "AbortError" ? (a("AI response stopped"), T) : (a(`AI failed: ${O instanceof Error ? O.message : String(O)}`), null);
    } finally {
      S.current === D && (S.current = null), te(!1);
    }
  }, [o, n.gatewayPreference, n.model, oe, ne, E, t, a]), rt = y(() => {
...
const _r = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Rr = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, jr = ({ files: e, openEditorIds: t, activeFileId: n, versions: a, activeSelection: o }) => {
...
}, Dr = "active-file", Hr = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Br = (e, t = "file") => `${t}:${e.id}`, Wr = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let o = [];
  return t.scope === "active-selection" ? a?.selection ? o = [Vr(a, a.selection, !0)] : a && (o = [Ge(a, !0, "Active file")]) : t.scope === "active-file" ? a && (o = [Ge(a, !0, "Active file")]) : t.scope === "open-editors" ? o = e.openDocumentIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ge(i, !0, "Open editor")) : t.scope === "selected-files" && (o = t.selectedFileIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ge(i, !1, "Selected file"))), qr(o).filter((i) => !n.has(i.id));
}, qr = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, Kr = 4e3, Xr = 8e3, Jr = 4e3, Yr = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Gr = (e, t, n) => {
  const a = Ur(e, n), o = [], i = new Map(t.map((s) => [s.id, s]));
  for (const s of a) {
    ...
    const p = [
      ...
      s.range ? `Range: ${dn(s.range)}` : null
    ].filter(Boolean).join(`
`), f = s.kind === "selection" && s.range ? Qr(l.content, s.range) : l.content, h = s.kind === "selection" ? Kr : s.label.startsWith("Open editor") ? Jr : Xr;
    o.push({
...
`));
}, dn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, hn = 75e4, Zr = 2800, eo = 240, to = 80, no = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${ve(t.content)}`).sort().join("")
), Et = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? hn) || e.content.length > (t.maxFileBytes ?? hn) ? "huge" : !t.includeDirty && e.dirty ? null : lo(e.path) ? "vendor" : ho(e) ? "binary" : null;
}, un = (e, t = {}) => {
  ...
  for (const s of e) {
    ...
    const p = ve(s.content), f = ro(s, p, n, t);
    i.push(...f), a.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
...
}, ro = (e, t = ve(e.content), n = Date.now(), a = {}) => {
  const o = Math.max(500, a.maxChunkChars ?? Zr), i = Math.min(Math.max(0, a.chunkOverlapChars ?? eo), Math.floor(o / 2)), s = Math.max(1, a.maxChunksPerFile ?? to), l = co(e.content), m = [];
  let p = 0, f = 0;
  for (; p < e.content.length && m.length < s; ) {
    const h = io(e.content, p, Math.min(e.content.length, p + o)), g = e.content.slice(p, h).trim();
    if (g.length > 0) {
      const $ = so(l, p, h);
      m.push({
        id: `${e.id}:${t}:${f}:${$.startLineNumber}-${$.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const a = mn(e, t), o = mn(e, Math.max(t, n - 1));
  return {
    startLineNumber: a + 1,
    startColumn: t - e[a] + 1,
    endLineNumber: o + 1,
  ...
  return 0;
}, lo = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => no.has(t)), ho = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && ao.has(t) || e.content.includes("\0")) return !0;
...
class uo {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = un(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = un(t, this.options), this.snapshot;
  }
...
const Se = (e = [], t = {}) => new uo(e, t), mo = (e, t, n = {}) => {
  ...
  for (const l of e.files) {
    ...
    if (Et(m, n)) {
      i.push({ fileId: l.fileId, path: m.path, status: "skipped-now", indexedHash: l.hash, currentHash: ve(m.content) });
      continue;
    }
    const f = ve(m.content);
    f !== l.hash ? i.push({ fileId: l.fileId, path: m.path, status: "changed", indexedHash: l.hash, currentHash: f }) : m.dirty !== l.dirty && i.push({ fileId: l.fileId, path: m.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: f });
  }
  for (const l of t)
    !o.has(l.id) && !Et(l, n) && i.push({ fileId: l.id, path: l.path, status: "new", currentHash: ve(l.content) });
  const s = e.signature !== Fn(t);
  return { stale: i.length > 0, signatureChanged: s, files: i };
}, fo = 8, po = 12e3, On = (e, t, n = {}, a) => {
  const o = ko(t);
  if (o.length === 0) return [];
  const i = new Set(a?.files.map((f) => f.fileId) ?? []), s = e.chunks.filter((f) => n.includeDirty !== !1 || !f.dirty).map((f) => yo(f, o)).filter((f) => f.score >= (n.minScore ?? 1)).sort((f, h) => h.score - f.score || f.chunk.path.localeCompare(h.chunk.path) || f.chunk.ordinal - h.chunk.ordinal), l = [], m = /* @__PURE__ */ new Set();
  let p = n.maxChars ?? po;
  for (const f of s) {
    if (l.length >= (n.limit ?? fo) || p <= 0) break;
...
[...context clipped...]` : f.chunk.text;
    l.push(bo(f, g, i.has(f.chunk.fileId))), p -= g.length;
  }
  return l;
}, bo = (e, t = e.chunk.text, n = !1) => {
...
`), ko = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const p = At(n, m), f = At(a, m), h = At(o, m), g = i === m ? 1 : 0;
    p + f + h + g !== 0 && (l.push(m), s += Math.min(p, 12), s += f * 4, s += h * 6, s += g * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (s += 3));
  }
  ...
  const n = e.toLowerCase(), a = t.map((m) => n.indexOf(m)).filter((m) => m >= 0).sort((m, p) => m - p)[0] ?? 0, o = Math.max(0, a - 120), i = Math.min(e.length, a + 260), s = o > 0 ? "…" : "", l = i < e.length ? "…" : "";
  return `${s}${e.slice(o, i).replace(/\s+/g, " ").trim()}${l}`;
}, vo = (e, t = {}) => {
  const { autoRefresh: n = !1, ...a } = t, o = Me(e), i = Me(null);
  i.current || (i.current = Se(e, a));
  const [s, l] = C(() => i.current?.getSnapshot() ?? Se(e, a).getSnapshot());
  o.current = e;
  const m = y((g = o.current) => {
    const $ = (i.current ?? Se([], a)).refresh(g, a);
    return l($), $;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), p = y((g = o.current) => {
    const $ = (i.current ?? Se([], a)).update(g, a);
    return l($), $;
  }, [m]);
  W(() => {
    n && m(e);
  }, [n, e, m]);
  const f = (i.current ?? Se([], a)).staleReport(e), h = y((g, $ = {}) => {
    const M = i.current ?? Se([], a);
    return On(M.getSnapshot(), g, $, M.staleReport(o.current));
  }, []);
  return { snapshot: s, staleReport: f, isStale: f.stale, refresh: m, update: p, retrieve: h };
}, Co = "tytus.atomek.semanticVector:v1", zn = "__gateway_default__", _n = (e) => e?.trim() || zn, xo = (e) => {
  const t = _n(e);
  ...
  if (!jn()) return null;
  try {
    const a = Rn(e, t, n), o = window.localStorage.getItem(a);
    if (!o) return null;
...
}, De = 8, $o = 12e3, So = 180, Mo = async (e, t, n, a, o = {}, i, s) => {
  const l = On(t, n, { ...o, limit: Math.max(o.limit ?? De, 16) }, i), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: l.slice(0, o.limit ?? De), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const p = _n(a.embeddingModel), f = await m.embedText({
      input: n,
    ...
    const g = new Map(l.map((S) => [S.chunkId, S])), $ = Oo(n), M = /* @__PURE__ */ new Map();
    t.chunks.filter((S) => o.includeDirty !== !1 || !S.dirty).slice(0, So).forEach((S) => M.set(S.id, S)), l.forEach((S) => {
      const E = t.byChunkId.get(S.chunkId);
      E && M.set(E.id, E);
    });
    const F = Array.from(M.values()), X = [];
    let le = 0;
    for (const S of F) {
      s?.aborted;
      const E = Ao(e.appId, p, S) ?? await Po(m, e.appId, p, S, a, s);
      if (!E) continue;
      E.updatedAt > Date.now() - 1e3 && (le += 1);
      const Z = Eo(h, E.vector);
      if (!Number.isFinite(Z)) continue;
      const J = g.get(S.id)?.score ?? zo(S, $), oe = Fo(J) * 0.42 + Math.max(0, Z) * 0.58;
      X.push({ chunk: S, keywordScore: J, vectorScore: Z, score: oe, matchedTerms: $.filter((Ie) => S.text.toLowerCase().includes(Ie)) });
    }
    if (X.length === 0) return { hits: l.slice(0, o.limit ?? De), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: le };
    const te = new Set(i?.files.map((S) => S.fileId) ?? []), Q = X.sort((S, E) => E.score - S.score || E.keywordScore - S.keywordScore || S.chunk.path.localeCompare(E.chunk.path) || S.chunk.ordinal - E.chunk.ordinal).slice(0, Math.max(o.limit ?? De, 1)).map((S) => Lo(S, te.has(S.chunk.fileId)));
    return {
      hits: To(Q, o.maxChars ?? $o),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${xo(a.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: le
    };
  } catch (p) {
    return {
      hits: l.slice(0, o.limit ?? De),
...
}, Fo = (e) => Math.min(1, Math.max(0, e / 24)), Oo = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), zo = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, Ro = /```([^\n`]*)\n([\s\S]*?)```/g, jo = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Wt(e) {
  return Array.from(e.matchAll(Ro)).map((t) => Hn(t[1] ?? "", Vo(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Hn(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), a = {}, o = [];
  for (const i of n) {
    const s = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? a[s[1].toLowerCase()] = qo(s[2]) : o.push(i.toLowerCase());
  }
...
function Bn(e) {
  return [...Wt(e).filter((a) => re(a, "diff") || re(a, "patch")).map((a) => a.content), e].filter((a, o, i) => Do(a) && i.findIndex((s) => s === a) === o);
}
function Do(e) {
...
`), a = [];
  ...
  const o = Array.from(new Set(a)).sort((s, l) => s - l), i = [];
  for (let s = 0; s < o.length; s += 1) {
    const l = o[s], p = o.find((h) => h > l && n[h].startsWith("diff --git ")) ?? n.length, f = n.slice(l, p).join(`
`);
    /^@@\s+-\d+/m.test(f) && i.push({ raw: f, ...Nt(f) });
  }
  return i.length > 0 ? i : [{ raw: t, ...Nt(t) }];
}
function Nt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), o = he(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = he(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, s = i && i !== "/dev/null" ? i : o;
...
function Ft(e) {
  return Wt(e).filter((t) => Bo(t)).map((t) => ({
    content: Uo(t.content),
...
`).slice(0, 5);
  for (const a of n) {
    const o = a.match(/^\s*(?:\/\/|#|<!--)?\s*(?:atomek-)?(?:path|file)\s*:\s*([^\s>]+)\s*(?:-->)?\s*$/i);
    if (o) return he(o[1]);
  }
  return null;
}
...
`), o = [];
  ...
  for (let l = 0; l < a.length; l += 1) {
    const m = a[l].match(jo);
    if (!m) continue;
    s = !0;
    const p = Number(m[1]), f = Math.max(0, p - 1);
    if (f < i) return null;
    for (o.push(...n.slice(i, f)), i = f, l += 1; l < a.length; l += 1) {
      const h = a[l];
      if (h.startsWith("@@ ")) {
        l -= 1;
        break;
...
function Ko(e) {
  ...
  for (const f of Bn(t))
    for (const h of Ho(f)) {
      if (!h.path) {
        if (o) {
          const M = Ot(o.content, h.raw);
          M && M !== o.content && s.set(o.id, Qe({
            file: o,
            proposedContent: M,
            sourceTitle: a,
  ...
  for (const f of Ft(t)) {
    const h = f.path ? fn(n, f.path) : o ? { file: o, normalizedPatchPath: he(o.path), confidence: "exact" } : null;
    if (!h) {
  ...
  const m = Ft(t).some((f) => f.path);
  if (s.size === 0 && o && !m) {
    const f = Xo(t, o);
    f && f.content !== o.content && s.set(o.id, Qe({
      file: o,
      proposedContent: f.content,
      sourceTitle: a,
      extractionLabel: f.label,
...
function Qe(e) {
  ...
  return {
    ...
    stats: Jo(e.file.content, e.proposedContent),
    base: {
...
const nt = (e) => typeof e == "object" && e !== null, Zo = (e) => nt(e.ai) ? e.ai : null, ei = (e) => {
  if (!nt(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, ti = (e) => {
  const t = ei(e);
  return !t || !nt(e) ? null : { ...e, id: t };
}, ce = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(ce) : nt(e) ? Object.values(e).some(ce) : !1, ni = async (e, t) => {
  const n = Zo(e);
  if (typeof n?.listModels != "function") return [];
  const a = await n.listModels(t);
  return Array.isArray(a) ? a.map(ti).filter((o) => !!o) : [];
}, ai = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : ce(e.capability) || ce(e.capabilities) || ce(e.modality) || ce(e.modalities) || ce(e.task) || ce(e.tasks) || ce(e.type) || ce(e.kind), Vn = (e) => typeof e == "object" && e !== null, ri = (e) => Vn(e.ai) ? e.ai : null, Un = (e) => typeof ri(e)?.embedText == "function", oi = async (e, t) => Un(e) ? (await ni(e, t)).filter(ai) : [], ii = (e) => Vn(e.ai) ? Un(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", ci = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], si = [
...
function ui(e) {
  const t = e.results.length > 0 ? e.results.map((a, o) => [
    ...
    `Status: ${a.status}`,
    "Output:",
    wi(a.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function fi(e) {
  ...
  for (const a of t) {
    const o = pi(a.content);
    if (!o?.scripts) continue;
    const i = Kn(a.path), s = bi(e, i, o);
    if (!s) continue;
    const l = Object.keys(o.scripts).filter((p) => typeof o.scripts?.[p] == "string"), m = gi(l);
    for (const p of m) {
      const f = `${s} run ${p}`;
      n.push({
        id: Xn(`${i}:${f}`),
...
function bi(e, t, n) {
  ...
  for (const [a, o] of si)
    if (e.some((i) => yi(i.path) === a && Kn(i.path) === t)) return o;
  return null;
}
function gi(e) {
  const t = ci.filter((a) => e.includes(a)), n = e.filter((a) => !t.includes(a)).filter((a) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(a)).sort((a, o) => a.localeCompare(o));
  return [...t, ...n];
}
function ki(e) {
  const t = /* @__PURE__ */ new Set();
...
const vi = Ta(() => import("./WorkbenchMonacoEditor-DbnEHW95.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Ci = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Jn = "tytus.workspace.recent", Yn = "tytus.workspace.layout", Gn = "tytus.atomek.chatAiSettings", et = {
...
function $t(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - xi), a = n < 1180, o = a ? 200 : 240, i = a ? 300 : 340, s = a ? 420 : 560, l = Math.max(o, Math.min(a ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), p = Math.floor(n * (a ? 0.34 : 0.36)), f = n - m - s, h = Math.max(i, Math.min(a ? 500 : 640, p, f));
  return { primaryMin: o, primaryMax: l, secondaryMin: i, secondaryMax: h };
}
function Ai(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let a = 0, o = 0, i;
...
function Ni({ host: e }) {
  const t = Me(null), [n, a] = C(0), o = K(() => Zi(), []), [i, s] = C("explorer"), [l, m] = C(o.primaryVisible), [p, f] = C(o.primaryWidth), [h, g] = C("chat"), [$, M] = C(o.secondaryVisible), [F, X] = C(o.secondaryWidth), [le, te] = C(!1), [Q, S] = C("problems"), [E, Z] = C(o.markdownPreviewVisible), [Be, J] = C(!1), [oe, Ie] = C(null), [x, Y] = C([]), [de, Ce] = C([]), [j, ne] = C(null), [Pe, rt] = C(""), [v, w] = C({ lineNumber: 1, column: 1 }), [I, T] = C(null), [D, O] = C({}), [V, be] = C(Dr), [Le, We] = C([]), [ge, ue] = C([]), [_, z] = C(null), [U, G] = C(""), [na, ot] = C(!1), [aa, ra] = C(""), [Ut, it] = C([]), [H, xe] = C(null), [me, Te] = C(null), [qt, Kt] = C(!1), [ke, oa] = C(() => ec()), [Xt, Ee] = C(null), [Ae, ct] = C(null), [st, lt] = C(""), [dt, ht] = C(""), [Ve, Ue] = C(""), [qe, Jt] = C("failed"), [ia, Yt] = C(null), [ca, A] = C("Ready"), [Fe, sa] = C(() => Qi()), ae = de.map((c) => x.find((u) => u.id === c)).filter(Boolean), N = j ? x.find((c) => c.id === j) ?? null : null, Gt = K(() => jr({ files: x, openEditorIds: de, activeFileId: j, versions: D, activeSelection: I }), [j, I, D, x, de]), Qt = K(() => ({
    scope: V,
    removedAttachmentIds: Le,
    selectedFileIds: []
  }), [V, Le]), Oe = K(() => Gr(Gt, x, Qt), [Qt, Gt, x]), fe = vo(x, { autoRefresh: !0, includeDirty: !0 }), Zt = K(() => ge.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [ge]), la = K(() => [...Oe.attachments, ...Zt], [Oe.attachments, Zt]), P = zr({ host: e, requestContext: Oe.parts, chatSettings: ke, setStatus: A }), ye = K(
    () => [...P.artifacts, ...Ut].sort((c, u) => u.createdAt - c.createdAt),
    [P.artifacts, Ut]
  ), Ke = qt && !N, ut = !N && !Ke && !Be, q = K(() => x.filter((c) => c.dirty), [x]), da = K(() => {
    const c = Pe.trim().toLowerCase();
    return c ? x.filter((u) => u.path.toLowerCase().includes(c)) : x;
  }, [x, Pe]);
  W(() => {
    O((c) => {
      const u = {};
      for (const b of x) u[b.id] = c[b.id] ?? 1;
      return u;
    });
  }, [x]), W(() => {
    We([]), ue([]);
  ...
  const ha = y((c) => {
    if (c.kind === "index-hit") {
      ue((u) => u.filter((b) => b.id !== c.id)), A(`Removed project context: ${c.label}`);
      return;
    }
    We((u) => u.includes(c.id) ? u : [...u, c.id]), A(`Removed chat context: ${c.label}`);
  }, []), Xe = y(() => {
    Kt(!0), ne(null), J(!0), A("Atomek settings opened");
  }, []), en = y(() => {
    Kt(!1), ne((c) => c ?? de.at(-1) ?? null), de.length === 0 && J(!1), A("Atomek settings closed");
  }, [de]), Ne = y((c) => {
    O((u) => ({ ...u, [c]: (u[c] ?? 1) + 1 }));
  }, []), ua = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = $t(n), b = c.clientX, k = F, R = (Re) => {
      const vt = k + (b - Re.clientX);
      X(Ze(vt, u.secondaryMin, u.secondaryMax));
    }, ie = () => {
      window.removeEventListener("pointermove", R), window.removeEventListener("pointerup", ie);
    };
    window.addEventListener("pointermove", R), window.addEventListener("pointerup", ie);
  }, [F, n]), ma = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = $t(n), b = c.clientX, k = p, R = (Re) => {
      const vt = k + (Re.clientX - b);
      f(Ze(vt, u.primaryMin, u.primaryMax));
    }, ie = () => {
      window.removeEventListener("pointermove", R), window.removeEventListener("pointerup", ie);
    };
    window.addEventListener("pointermove", R), window.addEventListener("pointerup", ie);
  }, [p, n]), Je = y((c) => {
    const u = [c, ...Fe.filter((b) => b.path !== c.path)].slice(0, 6);
    sa(u), localStorage.setItem(Jn, JSON.stringify(u));
  }, [Fe]), B = y((c, u) => {
    Ce((b) => b.includes(c.id) ? b : [...b, c.id]), ne(c.id), J(!1), Yt(u ?? null), w({ lineNumber: u ?? 1, column: 1 });
  }, []), fa = y((c) => {
    if (!c.fileId) return;
    const u = x.find((b) => b.id === c.fileId);
    u && (B(u, c.range?.startLineNumber ?? 1), A(`Revealed context: ${c.label}`));
  }, [x, B]), ze = y(async () => {
    if (St(q, "open new files"))
      try {
        const c = await Sr();
        if (c.length === 0) return;
        Y((u) => Ji(u, c)), c.forEach((u) => Je({ name: u.name, path: u.path, at: Date.now() })), B(c[0]), A(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && A(`Open file failed: ${c.message}`);
      }
  }, [q, B, Je]), mt = y(async () => {
    if (St(q, "open another folder"))
      try {
        const c = await Mr();
        Ie(c), Y(c.files), Ce([]), ne(null), J(!1), Je({ name: c.name, path: c.name, at: Date.now() }), A(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!j) return;
    let u = !1;
    Y((b) => b.map((k) => k.id !== j || k.content === c ? k : (u = !0, { ...k, content: c, dirty: !0 }))), u && Ne(j);
  }, [j, Ne]), ft = y(async () => {
    if (N)
      try {
        const c = await xt(N);
        Y((u) => u.map((b) => b.id === c.id ? c : b)), A(`Saved ${c.name}`);
      } catch (c) {
        A(`Save failed: ${c.message}`);
      }
  }, [N]), ba = y(async (c) => {
    const u = x.find((k) => k.id === c);
    if (!u) return null;
    const b = await xt(u);
    return Y((k) => k.map((R) => R.id === b.id ? b : R)), b;
  }, [x]), tn = y(async () => {
    ...
    try {
      const u = await Promise.all(c.map((k) => xt(k))), b = new Map(u.map((k) => [k.id, k]));
      Y((k) => k.map((R) => b.get(R.id) ?? R)), Ee(null), A(`Saved ${u.length} dirty file${u.length === 1 ? "" : "s"}`);
    } catch (u) {
      A(`Save all failed: ${u.message}`);
    }
  }, [x]), pt = y((c) => {
    const u = x.find((b) => b.id === c);
    ...
    Ce((b) => {
      ...
      return j === c && (ne(k.at(-1) ?? null), T(null)), k;
    });
  }, [j, x]), ga = y(() => {
    St(q, "close all editors") && (Ce([]), ne(null), Yt(null), J(!1), A("Closed all editors"));
  }, [q]), bt = y(() => {
    const c = x.filter((b) => b.name.startsWith("Untitled")).length + 1, u = {
...
`,
    ...
    return ue(k), k.length === 0 ? (A(fe.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), u) : (b.mode !== "hybrid" && b.reason ? A(b.reason) : A(`${k.length} project context hit${k.length === 1 ? "" : "s"} · ${b.reason ?? "hybrid retrieval"}`), [
      ...u,
      ...Io(k)
    ]);
  }, [Oe.parts, V, ke, e, fe]), gt = y(() => {
    if (!N && ae.length === 0) {
      A("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const u = [
      `Create a polished Markdown artifact from ${N?.path ?? `${ae.length} open editors`}.`,
    ...
    M(!0), g("chat"), A("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const b = await pe(u), k = await P.askAgent(u, { requestContext: b });
      !k || k.status === "error" || P.createArtifact({
        ...
        M(!0), g("outputs"), te(!0), S("output");
      });
    })();
  }, [N, P, pe, ae.length]), kt = y((c) => {
    const u = x.find((b) => b.path === c.path || b.name === c.name);
    if (u) {
    ...
    M(!0), g("chat"), G(""), (async () => {
      const u = await pe(c);
      await P.askAgent(c, { requestContext: u });
    })();
  }, [P, pe]), ka = y((c) => {
    const u = P.messages.findIndex((k) => k.id === c.id), b = P.messages.slice(0, u < 0 ? void 0 : u).reverse().find((k) => k.role === "user");
    if (!b?.body.trim()) {
      A("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const k = await pe(b.body);
      await P.askAgent(b.body, { requestContext: k });
    })();
  }, [P, pe]), ya = y((c) => {
    P.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const u = It(x, Mt(c.title || c.kind)), b = {
    ...
    Y((k) => [...k, b]), B(b), A(`Opened ${c.title} as editable file`);
  }, [x, B]), wt = y(() => {
...
`), R = {
    ...
    it((ie) => [R, ...ie]), te(!0), S("output"), A(`Captured manual check: ${u}`);
  }, []), Ye = y((c, u) => {
    const b = Ko({
      body: u,
      files: x,
      ...
      edits: b.edits.map(Yi),
      skipped: b.skipped
    };
    return b.edits.length > 1 ? (Te(k), xe(null), A(`Previewing AI workspace patch for ${b.edits.length} files`), !0) : b.edits.length === 1 ? (xe(k.edits[0]), Te(null), A(`Previewing AI patch for ${k.edits[0].fileName}`), !0) : (A(b.skipped.length > 0 ? `No applicable edit found. ${b.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [N, D, x]), Ca = y(() => {
    const c = U.trim();
    if (!c) return;
    G("");
    const u = Bi(c);
    (async () => {
      const b = await pe(c), k = await P.askAgent(c, { requestContext: b });
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", k.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    const u = li(x, c);
    ct(u);
    const b = u.commands[0]?.command ?? "";
    Ue(b), lt(""), ht(""), Jt("failed"), te(!0), S("terminal"), A(u.commands.length > 0 ? `Manual check ready: copy ${u.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [x]), Aa = y((c) => {
    c.trim() && (at(c), Ue(c), A(`Manual check command copied: ${c}`));
  }, []), Na = y(() => {
    ct((c) => {
      if (!c) return c;
      const u = di(c, st), b = u.commands.at(-1)?.command ?? "";
      return b && Ue(b), u;
    }), lt("");
  }, [st]), $a = y(() => {
    if (!Ae || !Ve.trim()) {
    ...
    const c = Ve.trim();
    ct((u) => u && hi(u, c, qe, dt)), ht(""), A(`Captured manual check result: ${c} (${qe})`);
  }, [dt, Ve, Ae, qe]), Sa = y(() => {
    Ae && ee(ui(Ae));
  }, [ee, Ae]), Ma = y(() => {
    if (!H) return;
    const c = x.find((u) => u.id === H.fileId);
    if (!c) {
      A(`Cannot apply edit — ${H.fileName} is no longer open`), xe(null);
      return;
    }
    c.content !== H.originalContent && !window.confirm(`${H.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (Y((u) => u.map((b) => b.id === H.fileId ? { ...b, content: H.proposedContent, dirty: !0 } : b)), Ne(H.fileId), Ee(`AI edit applied to ${c.name}. Save All to persist it to disk.`), xe(null), _e(`AI edit applied to ${c.name}`), A(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Ne, x, H, _e]), Ia = y(() => {
    if (!H) return;
    const c = It(x, `${Mt(H.fileName)}-ai-edit`), u = {
    ...
    Y((b) => [...b, u]), B(u), xe(null), A(`Opened proposed edit as ${c}`);
  }, [N?.language, x, B, H]), Pa = y(() => {
    if (!me) return;
    const c = new Map(me.edits.map((b) => [b.fileId, b]));
    x.some((b) => {
      const k = c.get(b.id);
      return k && b.content !== k.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (Y((b) => b.map((k) => {
      const R = c.get(k.id);
      return R ? { ...k, content: R.proposedContent, dirty: !0 } : k;
    })), c.forEach((b, k) => Ne(k)), Ee(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), Te(null), _e(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), A(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Ne, x, me, _e]), La = y(() => {
    if (!me) return;
    const c = [...x], u = me.edits.map((b) => {
      const k = It(c, `${Mt(b.fileName)}-ai-edit`), R = x.find((Re) => Re.id === b.fileId), ie = {
        id: `workspace-patch-${b.fileId}-${Date.now()}-${k}`,
        name: k,
        path: k,
    ...
    Y((b) => [...b, ...u]), u[0] && B(u[0]), Te(null), A(`Opened ${u.length} proposed edit file${u.length === 1 ? "" : "s"}`);
  }, [x, B, me]), nn = y((c) => {
    if (c === "explain") {
      ee("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (c === "improve") {
      ee("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (c === "plan") {
      ee("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (c === "edit") {
      ee("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    ee("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [ee]);
  return W(() => {
    const c = (u) => {
      (u.metaKey || u.ctrlKey) && (u.key.toLowerCase() === "o" && (u.preventDefault(), ze()), u.key.toLowerCase() === "s" && (u.preventDefault(), ft()), u.key.toLowerCase() === "w" && (u.preventDefault(), j && pt(j)), u.key.toLowerCase() === "b" && (u.preventDefault(), m((k) => !k)), u.shiftKey && u.key.toLowerCase() === "f" && (u.preventDefault(), s("search"), m(!0)), (u.key.toLowerCase() === "k" || u.key.toLowerCase() === "p") && (u.preventDefault(), ot(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [j, pt, ze, ft]), W(() => {
    const c = (u) => {
      q.length !== 0 && (u.preventDefault(), u.returnValue = "");
    };
    ...
    if (!n) return;
    const c = $t(n);
    f((u) => Ze(u, c.primaryMin, c.primaryMax)), X((u) => Ze(u, c.secondaryMin, c.secondaryMax));
  }, [n]), W(() => {
    const c = { primaryVisible: l, primaryWidth: p, secondaryVisible: $, secondaryWidth: F, markdownPreviewVisible: E };
    localStorage.setItem(Yn, JSON.stringify(c));
  }, [E, l, p, $, F]), W(() => {
    localStorage.setItem(Gn, JSON.stringify(ke));
  }, [ke]), /* @__PURE__ */ d(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${l ? "" : "no-primary"} ${$ ? "" : "no-secondary"} ${le ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${p}px`, "--workbench-secondary-width": `${F}px` },
      children: [
        /* @__PURE__ */ r($i, { active: i, setActive: (c) => {
          s(c), m(!0);
        }, openSettings: Xe, settingsActive: Ke }),
        ...
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ r("button", { className: "workbench-command-center", onClick: () => ot(!0), children: "Workspace" }),
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ r(Pi, { file: N, folder: oe, showWelcome: ut }),
            Xt && q.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ r("span", { children: Xt }),
              /* @__PURE__ */ r("button", { onClick: () => {
                tn();
              }, children: "Save all" }),
              /* @__PURE__ */ r("button", { onClick: () => Ee(null), title: "Dismiss", children: /* @__PURE__ */ r(se, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: N ? /* @__PURE__ */ d("div", { className: N.language === "markdown" && E ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ r("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ r(Ea, { fallback: /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ r(
                vi,
                {
                  file: N,
                  revealLine: ia,
              ...
              N.language === "markdown" && E && /* @__PURE__ */ r(zi, { content: N.content })
            ] }) : Ke ? /* @__PURE__ */ r(
              Vi,
              {
                host: e,
            ...
            ) : ut ? /* @__PURE__ */ r(Ti, { openFile: ze, openFolder: mt, newFile: bt, recent: Fe, reopenRecent: kt, setStatus: A }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ r(tt, { size: 34 }),
            ...
            le && /* @__PURE__ */ r(
              Ri,
              {
                ...
                runAiSynthesis: gt,
                captureManualCheck: wt,
                openOutputAsFile: yt,
                manualCheckSession: Ae,
                manualCheckCommandInput: st,
                setManualCheckCommandInput: lt,
                manualCheckOutputInput: dt,
                setManualCheckOutputInput: ht,
                manualCheckSelectedCommand: Ve,
                setManualCheckSelectedCommand: Ue,
                manualCheckStatus: qe,
                setManualCheckStatus: Jt,
                copyManualCheckCommand: Aa,
                addManualCheckCommand: Na,
                recordManualCheckResult: $a,
                askAgentFromManualChecks: Sa,
                onClose: () => te(!1)
              }
            )
          ] })
        ...
        $ && /* @__PURE__ */ r(
          ji,
          {
            ...
            saveMessageAsArtifact: ya,
            rememberMessage: wa,
            previewEditFromMessage: (c) => Ye(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: nn,
            pendingPatchPrompt: _,
            generatePatchPrompt: xa,
            ...
            captureManualCheck: wt,
            openOutputAsFile: yt,
            previewEditFromOutput: (c) => Ye(c.title, c.body),
        ...
        na && /* @__PURE__ */ r(
          Oi,
          {
            ...
            commands: [
              ...
              ...Fe.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => kt(c) })),
              { label: "File: Save All", detail: `${q.length} dirty file${q.length === 1 ? "" : "s"}`, run: () => {
                tn();
              }, disabled: q.length === 0 },
              { label: "File: Close All Editors", detail: `${ae.length} open editor${ae.length === 1 ? "" : "s"}`, run: ga, disabled: ae.length === 0 },
              ...
              { label: "View: Toggle Chat Panel", detail: $ ? "Hide right AI side bar" : "Show right AI side bar", run: () => M((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: le ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => te((c) => !c) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Xe },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => _e("Manual check requested from command palette") },
              { label: "View: Toggle Markdown Preview", detail: N?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => Z((c) => !c), disabled: N?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: N || ae.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: gt, disabled: !N && ae.length === 0 },
              { label: "AI: Explain Active File", detail: N ? `Ask Cortex to explain ${N.path}` : "Open a file first", run: () => ee("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !N },
              { label: "AI: Improve Active File", detail: N ? `Ask Cortex for concrete edits to ${N.path}` : "Open a file first", run: () => ee("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !N },
              { label: "AI: Draft Editable Replacement", detail: N ? `Ask Cortex for a full-file replacement for ${N.path}` : "Open a file first", run: () => nn("edit"), disabled: !N },
              { label: "AI: Plan Workspace Work", detail: ae.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => ee("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: ae.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: N ? "Persist active file in host.ai artifacts" : "Open a file first", run: va, disabled: !N },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: wt },
              { label: "AI: Open Latest Artifact as File", detail: ye[0] ? `Create editable file from ${ye[0].title}` : "No outputs yet", run: () => ye[0] && yt(ye[0]), disabled: ye.length === 0 }
            ],
            openWorkbenchFile: B,
            onClose: () => ot(!1)
          }
        ...
        /* @__PURE__ */ r(Xi, { status: ca, file: N ?? Ci, cursor: v, fileCount: x.length, dirtyCount: q.length })
      ]
    }
  );
}
function $i({ active: e, setActive: t, openSettings: n, settingsActive: a }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(xn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(pr, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(An, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(vn, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(wn, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ r("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(Ka, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ r(we, { icon: /* @__PURE__ */ r(yr, { size: 23 }), label: "Settings", active: a, onClick: n })
  ] });
}
function we({ icon: e, label: t, active: n, onClick: a }) {
  return /* @__PURE__ */ r("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: a, children: e });
}
function Si(e) {
  return e.activity === "search" ? /* @__PURE__ */ r(Ei, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ r(gn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ r(gn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ r(Ki, {}) : /* @__PURE__ */ r(Mi, { ...e });
}
function Mi(e) {
  const t = !e.folder, n = K(() => Ii(e.files, e.folder?.name), [e.files, e.folder?.name]);
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(He, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((a) => /* @__PURE__ */ r(Vt, { file: a, active: a.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(a) }, a.id)),
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(Lt, { size: 12 }),
          " ",
      ...
      e.recent.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((a) => /* @__PURE__ */ d("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(a), children: [
...
function Vt({ file: e, active: t, onOpen: n, basePath: a, depth: o = 0, label: i }) {
  const s = a && e.path.startsWith(`${a}/`) ? e.path.slice(a.length + 1) : e.path, l = o || Math.max(0, s.split("/").length - 1);
  return /* @__PURE__ */ d("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": l }, onClick: n, title: e.path, children: [
    /* @__PURE__ */ r(Cn, { size: 14 }),
    /* @__PURE__ */ r("span", { className: "workbench-row-name", children: i ?? s }),
...
function Ii(e, t) {
  const n = [], a = /* @__PURE__ */ new Map(), o = (s, l, m = n) => {
    ...
    return a.set(s, f), m.push(f), f;
  };
  e.forEach((s) => {
    const l = t && s.path.startsWith(`${t}/`) ? s.path.slice(t.length + 1) : s.path, m = l.split("/").filter(Boolean);
    let p = n, f = "";
    m.slice(0, -1).forEach((h) => {
      f = f ? `${f}/${h}` : h, p = o(f, h, p).children;
    }), p.push({ name: m.at(-1) ?? s.name, path: l, file: s, children: [] });
  });
  const i = (s) => s.sort((l, m) => +!!l.file - +!!m.file || l.name.localeCompare(m.name)).map((l) => ({ ...l, children: i(l.children) }));
  return i(n);
}
function Qn(e, t, n, a = 0) {
  return e.map((o) => o.file ? /* @__PURE__ */ r(Vt, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: a, label: o.name }, o.file.id) : /* @__PURE__ */ d("div", { children: [
    /* @__PURE__ */ d("div", { className: "workbench-folder-row", style: { "--workbench-depth": a }, children: [
      /* @__PURE__ */ r(Lt, { size: 12 }),
      /* @__PURE__ */ r(Rt, { size: 14 }),
      /* @__PURE__ */ r("span", { className: "workbench-row-name", children: o.name })
    ] }),
    Qn(o.children, t, n, a + 1)
  ] }, o.path));
}
function Pi({ file: e, folder: t, showWelcome: n }) {
  const a = n ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], o = t && a[0] === t.name ? a.slice(1) : a;
  return /* @__PURE__ */ r("div", { className: "workbench-breadcrumb", children: o.length === 0 ? /* @__PURE__ */ r("span", { children: "Workspace" }) : o.map((i, s) => /* @__PURE__ */ d("span", { className: "workbench-breadcrumb-part", children: [
...
function Li(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ d("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ r(se, { size: 13 }) })
    ] }),
    ...
    /* @__PURE__ */ r("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ r(lr, { size: 16 }) })
  ] });
}
function Ti({ openFile: e, openFolder: t, newFile: n, recent: a, reopenRecent: o, setStatus: i }) {
  return /* @__PURE__ */ d("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ d("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ d("section", { children: [
        ...
        a.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : a.map((s) => /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: () => o(s), children: [
          s.name,
...
function Ei({ files: e, query: t, setQuery: n, openWorkbenchFile: a, activeFileId: o }) {
  const i = K(() => Fi(e, t), [e, t]), s = K(() => {
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? s.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No matches." }) : s.map((l) => {
        ...
        return /* @__PURE__ */ d("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ r(Vt, { file: m, active: m.id === o, onOpen: () => a(m) }),
...
function Fi(e, t) {
  ...
  const a = [];
  return e.forEach((o) => {
    o.path.toLowerCase().includes(n) && a.push({ file: o, lineNumber: 1, line: o.path }), o.content.split(`
...
function Oi(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((o) => ({
    label: o.path,
    detail: `Open ${Sn(o.language)} file`,
    run: () => e.openWorkbenchFile(o)
  })), a = [...e.commands, ...n].filter((o) => !t || o.label.toLowerCase().includes(t) || o.detail.toLowerCase().includes(t)).slice(0, 12);
  return W(() => {
    const o = (i) => {
    ...
    /* @__PURE__ */ r(
      "input",
      {
        ...
        onKeyDown: (o) => {
          o.key === "Enter" && a[0] && !a[0].disabled && (a[0].run(), e.onClose());
        }
      }
    ),
    /* @__PURE__ */ r("div", { className: "workbench-command-list", children: a.map((o) => /* @__PURE__ */ d(
      "button",
      {
        disabled: o.disabled,
        ...
        children: [
          ...
          /* @__PURE__ */ r("small", { children: o.detail })
        ]
      },
      `${o.label}-${o.detail}`
    )) })
...
function _i(e) {
  const t = e.session ? mi(e.session) : "pending";
  return /* @__PURE__ */ d("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ d(He, { children: [
...
function Ri(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ d("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ r("span", {}),
      /* @__PURE__ */ r("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ r(se, { size: 14 }) })
    ] }),
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ r("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ r(ur, { size: 15 }) }),
        /* @__PURE__ */ r("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ r(Ya, { size: 16 }) }),
        /* @__PURE__ */ r("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ r(se, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ r(Di, { ...e }) : /* @__PURE__ */ r(ta, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function Zn({ body: e }) {
  const t = K(() => Ai(e), [e]), [n, a] = C(null), o = y((i, s) => {
    (async () => await at(s) && (a(i), window.setTimeout(() => a((m) => m === i ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ r("div", { className: "workbench-rich-body", children: t.map((i) => {
    if (i.type === "code") {
      ...
      return /* @__PURE__ */ d("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ d("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ d("button", { onClick: () => o(i.key, i.body), title: "Copy code block", children: [
            s ? /* @__PURE__ */ r(Va, { size: 12 }) : /* @__PURE__ */ r(Tt, { size: 12 }),
            s ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ r(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: En(i.body) }
      },
      i.key
    );
  }) });
...
function Di(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Me(null), [a, o] = C(!0), [i, s] = C(!1), l = K(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  W(() => {
    const h = n.current;
  ...
  const m = y(() => {
    const h = n.current;
    if (!h) return;
    const $ = h.scrollHeight - h.scrollTop - h.clientHeight < 48;
    o($), $ && s(!1);
  }, []), p = y(() => {
    const h = n.current;
  ...
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ d("option", { value: h.id, children: [
              h.title,
              " · ",
              Hi(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ d("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ r("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ d("div", { children: [
        ...
        /* @__PURE__ */ r("br", {}),
        /* @__PURE__ */ r(Zn, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ d(He, { children: [
          /* @__PURE__ */ r("br", {}),
        ...
        h.role === "assistant" && h.status !== "streaming" && h.status !== "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => f(h), title: "Copy this answer", children: [
            /* @__PURE__ */ r(Tt, { size: 12 }),
            " Copy"
          ] }),
        ...
        h.role === "assistant" && h.status === "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => f(h), title: "Copy this error", children: [
            /* @__PURE__ */ r(Tt, { size: 12 }),
            " Copy"
          ] }),
      ...
      i ? /* @__PURE__ */ r("button", { className: "workbench-chat-jump", onClick: p, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ d("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ r("em", { children: pn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ d(He, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ r(rn, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((h) => {
            const g = typeof h.score == "number" ? h.score.toFixed(2) : null, $ = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, M = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, F = [
              h.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: F, children: [
              /* @__PURE__ */ r("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ r(rn, { size: 13 }) }),
              h.label,
              ...
              h.removable ? /* @__PURE__ */ r("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(h), title: "Remove context", children: /* @__PURE__ */ r(se, { size: 11 }) }) : null
            ] }, h.id);
          }),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ r("span", { className: "workbench-chat-route-summary", children: pn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ r("span", {}),
          e.busy ? /* @__PURE__ */ r("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ r(Cr, { size: 14 }) }) : /* @__PURE__ */ r("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ r(gr, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function pn(e, t, n) {
  const a = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : ea(e.gatewayPreference)];
...
function Vi(e) {
  ...
  W(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(M.map((F) => ({ id: F.id, gatewayLabel: F.gatewayLabel }))), o(M.length > 0 ? `${M.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (M) {
        if (g.signal.aborted) return;
        n([]), o(`Model discovery failed: ${M instanceof Error ? M.message : String(M)}`);
      }
    })(), () => g.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), W(() => {
    const g = new AbortController();
    return (async () => {
      const M = ii(e.host);
      if (M) {
      ...
      try {
        const F = await oi(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: g.signal
        });
        if (g.signal.aborted) return;
        s(F.map((X) => ({ id: X.id, gatewayLabel: X.gatewayLabel ?? X.source ?? "AIL" }))), m(F.length > 0 ? `${F.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (F) {
        if (g.signal.aborted) return;
        s([]), m(`Embedding model discovery failed: ${F instanceof Error ? F.message : String(F)}`);
      }
    })(), () => g.abort();
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ d("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ d("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ r("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ r(se, { size: 15 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ r("datalist", { id: "atomek-embedding-models", children: i.map((g) => /* @__PURE__ */ r("option", { value: g.id, children: g.gatewayLabel }, `${g.gatewayLabel}:${g.id}`)) })
        ] }),
    ...
    /* @__PURE__ */ d("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ r("button", { onClick: () => e.onChange(et), children: "Reset" }),
      /* @__PURE__ */ r("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function ta({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: o, openOutputAsFile: i, previewEditFromOutput: s, canPreviewEdit: l = !1, compact: m = !1 }) {
  return /* @__PURE__ */ d("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    /* @__PURE__ */ d("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      ...
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: o, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((p) => /* @__PURE__ */ d("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ d("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ r("button", { onClick: () => void at(p.body), children: "Copy" }),
        /* @__PURE__ */ r("button", { onClick: () => i(p), children: "Open as file" }),
        s ? /* @__PURE__ */ r(
          "button",
          {
            className: bn(p.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => s(p),
            disabled: !l,
            children: bn(p.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        p.source === "ai" ? /* @__PURE__ */ r("button", { onClick: () => n(p.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ r(Zn, { body: p.body })
    ] }, p.id))
  ] });
}
function Ui({ edit: e, onApply: t, onOpenAsFile: n, onClose: a }) {
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ d("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ r("button", { title: "Close", onClick: a, children: /* @__PURE__ */ r(se, { size: 16 }) })
    ] }),
...
function qi({ patch: e, onApply: t, onOpenAsFiles: n, onClose: a }) {
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ r("pre", { children: Gi(i.proposedContent) })
      ] }, i.fileId)),
...
function Ki() {
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ d("div", { className: "workbench-extension-card", children: [
      ...
      ].map((t) => /* @__PURE__ */ d("div", { className: "workbench-extension-row", children: [
        /* @__PURE__ */ r(wn, { size: 16 }),
        /* @__PURE__ */ d("div", { children: [
...
function ec() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : et.gatewayPreference,
...
function rc(e) {
  return function() {
    return /* @__PURE__ */ r(tc, { host: e.host });
  };
}
export {
  rc as default
