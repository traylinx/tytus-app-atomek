...
import { forwardRef as nt, createElement as Be, useState as N, useRef as Ut, useMemo as q, useCallback as v, useEffect as ae, lazy as qt, Suspense as Kt } from "react";
/**
...
`).map((s) => `<tr>${s.split("|").filter(Boolean).map(
      (m, b) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${b % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${m.trim()}</td>`
    ).join("")}</tr>`).join("");
...
}, Kn = (e, t = qn) => e.length <= t ? e : `${e.slice(0, t)}
...
function Gn({ host: e, activeFile: t, openEditors: a, chatSettings: i, setStatus: o }) {
  const l = e.ai, [h, s] = N(null), [p, m] = N([]), [b, A] = N([]), [T, j] = N([]), [he, V] = N(Je), [Ne, Y] = N(!1), _ = Ut(!0), ie = q(() => Un(t, a), [t, a]), oe = v(async () => {
    if (!l) {
      V(Je);
      return;
    }
    ...
    if (l)
      try {
        const w = await l.listArtifacts({ threadId: y });
        if (!_.current) return;
        A(w.map(et));
      } catch (w) {
        o(`AI artifacts unavailable: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [l, o]), ue = v(async () => {
    if (l)
      try {
        const w = (await l.listThreads({ workspaceKey: ke, status: "active" }))[0] ?? await l.createThread({ workspaceKey: ke, title: "Atomek chat" });
        if (!_.current) return;
        s(w);
        const R = await l.listMessages(w.id);
        if (!_.current) return;
        m(R.map(Le).filter(Boolean)), await H(w.id);
      } catch (y) {
        o(`AI chat unavailable: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [l, H, o]);
  ae(() => (_.current = !0, oe(), ue(), () => {
    _.current = !1;
  }), [ue, oe]);
  const le = v(async () => {
    if (!l) return null;
    ...
    return _.current && s(y), y;
  }, [l, h]), k = v(async () => {
    if (!l) return;
    const y = await l.createThread({ workspaceKey: ke, title: "Atomek chat" });
    s(y), m([]), A([]), j([]), o("New AI chat created");
  }, [l, o]), O = v(async (y) => {
    if (!l) return null;
    try {
      const w = await le();
      if (!w) return null;
      const R = await l.createArtifact({
        threadId: w.id,
      ...
      return A((J) => [F, ...J.filter((X) => X.id !== F.id)]), o(`Saved AI artifact: ${F.title}`), F;
    } catch (w) {
      return o(`Save artifact failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [l, le, o]), Ae = v(async (y) => {
    if (l)
      try {
        await l.deleteArtifact(y), A((w) => w.filter((R) => R.id !== y)), o("Deleted AI artifact");
      } catch (w) {
        o(`Delete artifact failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [l, o]), ce = v(async (y) => {
    if (!l) return null;
    try {
      const w = await l.writeMemory({
      ...
      return j((R) => [w, ...R.filter((F) => F.id !== w.id)].slice(0, 5)), o(`Remembered: ${w.title}`), w;
    } catch (w) {
      return o(`Remember failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [l, o]), M = v(async (y) => {
    if (!l) return [];
    const w = await l.searchMemory({ query: y, limit: 5 });
    return _.current && j(w), w;
  }, [l]), K = v(async (y) => {
    const w = y.trim();
    if (!w || !l) return null;
    Y(!0);
    ...
    try {
      const F = await le();
      if (!F) return null;
      const J = await M(w).catch(() => []), X = Qn(J), Ce = X ? [...ie, X] : ie;
      let Q = null;
      for await (const I of l.sendMessage({
        threadId: F.id,
        body: w,
        ...
        if (I.type === "message_created") {
          const S = Le(I.message);
          if (!S) continue;
          S.role === "assistant" && (Q = S.id), m((z) => [...z.filter((Z) => Z.id !== S.id), S]);
        }
        if (I.type === "token" && (Q = I.messageId, m((S) => S.map(
          (z) => z.id === I.messageId ? { ...z, body: I.body, status: "streaming" } : z
        ))), I.type === "message_updated" || I.type === "done") {
          const S = Le(I.message);
          if (!S) continue;
          S.role === "assistant" && (R = S), m((z) => z.map((Z) => Z.id === S.id ? S : Z)), S.gatewayLabel && o(`AI answered via ${S.gatewayLabel}`);
        }
        I.type === "run_failed" && (o(`AI failed: ${I.error}`), Q && m((S) => S.map(
          (z) => z.id === Q ? { ...z, status: "error", error: I.error, body: I.error } : z
        )));
      }
      return oe(), R;
    } catch (F) {
      return o(`AI failed: ${F instanceof Error ? F.message : String(F)}`), null;
    } finally {
      Y(!1);
    }
  }, [l, i.gatewayPreference, i.model, ie, le, M, oe, o]);
  return {
    aiStatus: he,
    artifacts: b,
    busy: Ne,
...
const Yn = qt(() => import("./WorkbenchMonacoEditor-BaNVDl1J.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Jn = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
...
function Zn({ host: e }) {
  const t = q(() => Ma(), []), [a, i] = N("explorer"), [o, l] = N(t.primaryVisible), [h, s] = N(t.primaryWidth), [p, m] = N("chat"), [b, A] = N(t.secondaryVisible), [T, j] = N(t.secondaryWidth), [he, V] = N(!1), [Ne, Y] = N("problems"), [_, ie] = N(t.markdownPreviewVisible), [oe, H] = N(!1), [ue, le] = N(null), [k, O] = N([]), [Ae, ce] = N([]), [M, K] = N(null), [y, w] = N(""), [R, F] = N({ lineNumber: 1, column: 1 }), [J, X] = N(""), [Ce, Q] = N(!1), [I, S] = N(""), [z, Z] = N([]), [E, ee] = N(null), [G, se] = N(null), [xt, xe] = N(!1), [me, $t] = N(() => Fa()), [St, Ke] = N(null), [Pt, C] = N("Ready"), [pe, Mt] = N(() => Pa()), W = Ae.map((r) => k.find((d) => d.id === r)).filter(Boolean), f = M ? k.find((r) => r.id === M) ?? null : null, $ = Gn({ host: e, activeFile: f, openEditors: W, chatSettings: me, setStatus: C }), te = q(
    () => [...$.artifacts, ...z].sort((r, d) => d.createdAt - r.createdAt),
    [$.artifacts, z]
  ), $e = !f && !oe, B = q(() => k.filter((r) => r.dirty), [k]), Ft = q(() => {
    const r = y.trim().toLowerCase();
    return r ? k.filter((d) => d.path.toLowerCase().includes(r)) : k;
  }, [k, y]), It = v((r) => {
    r.preventDefault();
    const d = r.clientX, u = T, g = (de) => {
      const be = u + (d - de.clientX);
      j(Math.max(380, Math.min(760, be)));
    }, P = () => {
    ...
    r.preventDefault();
    const d = r.clientX, u = h, g = (de) => {
      const be = u + (de.clientX - d);
    ...
    const d = [r, ...pe.filter((u) => u.path !== r.path)].slice(0, 6);
    Mt(d), localStorage.setItem(wt, JSON.stringify(d));
  }, [pe]), L = v((r, d) => {
    ce((u) => u.includes(r.id) ? u : [...u, r.id]), K(r.id), H(!1), Ke(d ?? null), F({ lineNumber: d ?? 1, column: 1 });
  }, []), fe = v(async () => {
    if (_e(B, "open new files"))
      try {
        const r = await Rn();
        if (r.length === 0) return;
        O((d) => ya(d, r)), r.forEach((d) => ge({ name: d.name, path: d.path, at: Date.now() })), L(r[0]), C(`Opened ${r.length} local file${r.length === 1 ? "" : "s"}`);
      } catch (r) {
        r.name !== "AbortError" && C(`Open file failed: ${r.message}`);
      }
  }, [B, L, ge]), Se = v(async () => {
    if (_e(B, "open another folder"))
      try {
        const r = await Bn();
        le(r), O(r.files), ce([]), K(null), H(!1), ge({ name: r.name, path: r.name, at: Date.now() }), C(`${r.handle ? "Opened local folder" : "Opened browser fallback folder"} ${r.name} (${r.files.length} text files indexed)`);
      } catch (r) {
        r.name !== "AbortError" && C(`Open folder failed: ${r.message}`);
      }
  }, [B, ge]), Ot = v((r) => {
    M && O((d) => d.map((u) => u.id === M ? { ...u, content: r, dirty: r !== u.content || u.dirty } : u));
  }, [M]), Pe = v(async () => {
    if (f)
      try {
        const r = await ze(f);
        O((d) => d.map((u) => u.id === r.id ? r : u)), C(`Saved ${r.name}`);
      } catch (r) {
        C(`Save failed: ${r.message}`);
      }
  }, [f]), zt = v(async (r) => {
    const d = k.find((g) => g.id === r);
    if (!d) return null;
    const u = await ze(d);
    return O((g) => g.map((P) => P.id === u.id ? u : P)), u;
  }, [k]), Lt = v(async () => {
    ...
    try {
      const d = await Promise.all(r.map((g) => ze(g))), u = new Map(d.map((g) => [g.id, g]));
      O((g) => g.map((P) => u.get(P.id) ?? P)), C(`Saved ${d.length} dirty file${d.length === 1 ? "" : "s"}`);
    } catch (d) {
      C(`Save all failed: ${d.message}`);
    }
  }, [k]), Me = v((r) => {
    const d = k.find((u) => u.id === r);
    if (d?.dirty && !window.confirm(`${d.name} has unsaved changes. Close without saving?`)) {
      C(`Close canceled — ${d.name} has unsaved changes`);
      return;
    }
    ce((u) => {
      const g = u.filter((P) => P !== r);
      return M === r && K(g.at(-1) ?? null), g;
    });
  }, [M, k]), _t = v(() => {
    _e(B, "close all editors") && (ce([]), K(null), Ke(null), H(!1), C("Closed all editors"));
  }, [B]), Fe = v(() => {
...
`,
    ...
    const d = [
      `Create a polished Markdown artifact from ${f?.path ?? `${W.length} open editors`}.`,
    ...
    A(!0), m("chat"), C("Asking Atomek to synthesize an AI artifact…"), $.askAgent(d).then((u) => {
      !u || u.status === "error" || $.createArtifact({
        messageId: u.id,
        title: `AI synthesis — ${f?.name ?? "open workspace"}`,
        kind: "markdown",
        body: u.body
      }).then(() => {
        A(!0), m("outputs"), V(!0), Y("output");
      });
    });
  }, [f, $, W.length]), Ee = v((r) => {
    const d = k.find((u) => u.path === r.path || u.name === r.name);
    if (d) {
      L(d), C(`Opened recent ${d.name}`);
      return;
    }
    C("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [k, L]), Wt = v(() => {
    const r = J.trim();
    r && (X(""), $.askAgent(r));
  }, [$, J]), U = v((r) => {
    A(!0), m("chat"), X(""), $.askAgent(r);
  }, [$]), Tt = v((r) => {
    $.createArtifact({
      messageId: r.id,
      title: r.body.split(`
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const d = Te(k, We(r.title || r.kind)), u = {
    ...
    O((g) => [...g, u]), L(u), C(`Opened ${r.title} as editable file`);
  }, [k, L]), Xe = v((r, d) => {
    const u = va(d, k, r);
    if (u.edits.length > 1) {
      se(u), ee(null), C(`Previewing AI workspace patch for ${u.edits.length} files`);
      return;
    }
    if (u.edits.length === 1) {
      ee(u.edits[0]), se(null), C(`Previewing AI patch for ${u.edits[0].fileName}`);
      return;
    }
    if (!f) {
      C("Open a file before previewing an AI edit, or ask Atomek for a git-style diff against opened files.");
      return;
    }
    const g = $a(d, f);
    if (!g) {
      C("No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again.");
      return;
    }
    se(null), ee({
      fileId: f.id,
      ...
      proposedContent: g.content,
      sourceTitle: r,
      extractionLabel: g.label,
      stats: Ct(f.content, g.content)
    }), C(`Previewing AI edit for ${f.name}`);
  }, [f, k]), Dt = v(() => {
    if (!E) return;
    const r = k.find((d) => d.id === E.fileId);
    if (!r) {
      C(`Cannot apply edit — ${E.fileName} is no longer open`), ee(null);
      return;
    }
    r.content !== E.originalContent && !window.confirm(`${E.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (O((d) => d.map((u) => u.id === E.fileId ? { ...u, content: E.proposedContent, dirty: !0 } : u)), ee(null), C(`Applied AI edit to ${r.name}`));
  }, [k, E]), jt = v(() => {
    if (!E) return;
    const r = Te(k, `${We(E.fileName)}-ai-edit`), d = {
      id: `pending-edit-${E.fileId}-${Date.now()}`,
      name: r,
      path: r,
    ...
    O((u) => [...u, d]), L(d), ee(null), C(`Opened proposed edit as ${r}`);
  }, [f?.language, k, L, E]), Vt = v(() => {
    if (!G) return;
    const r = new Map(G.edits.map((u) => [u.fileId, u]));
    k.some((u) => {
      const g = r.get(u.id);
      return g && u.content !== g.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (O((u) => u.map((g) => {
      const P = r.get(g.id);
      return P ? { ...g, content: P.proposedContent, dirty: !0 } : g;
    })), se(null), C(`Applied AI workspace patch to ${r.size} file${r.size === 1 ? "" : "s"}`));
  }, [k, G]), Ht = v(() => {
    if (!G) return;
    const r = [...k], d = G.edits.map((u) => {
      const g = Te(r, `${We(u.fileName)}-ai-edit`), P = k.find((be) => be.id === u.fileId), de = {
        id: `workspace-patch-${u.fileId}-${Date.now()}-${g}`,
        name: g,
        path: g,
    ...
    O((u) => [...u, ...d]), d[0] && L(d[0]), se(null), C(`Opened ${d.length} proposed edit file${d.length === 1 ? "" : "s"}`);
  }, [k, L, G]), Qe = v((r) => {
    if (r === "explain") {
      U("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (r === "improve") {
      U("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (r === "plan") {
      U("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (r === "edit") {
      U("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    U("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [U]);
  return ae(() => {
    const r = (d) => {
      (d.metaKey || d.ctrlKey) && (d.key.toLowerCase() === "o" && (d.preventDefault(), fe()), d.key.toLowerCase() === "s" && (d.preventDefault(), Pe()), d.key.toLowerCase() === "w" && (d.preventDefault(), M && Me(M)), d.key.toLowerCase() === "b" && (d.preventDefault(), l((g) => !g)), d.shiftKey && d.key.toLowerCase() === "f" && (d.preventDefault(), i("search"), l(!0)), (d.key.toLowerCase() === "k" || d.key.toLowerCase() === "p") && (d.preventDefault(), Q(!0)));
    };
    return window.addEventListener("keydown", r), () => window.removeEventListener("keydown", r);
  }, [M, Me, fe, Pe]), ae(() => {
    const r = (d) => {
      B.length !== 0 && (d.preventDefault(), d.returnValue = "");
    };
    return window.addEventListener("beforeunload", r), () => window.removeEventListener("beforeunload", r);
  }, [B.length]), ae(() => {
    const r = { primaryVisible: o, primaryWidth: h, secondaryVisible: b, secondaryWidth: T, markdownPreviewVisible: _ };
    ...
    "div",
    {
      className: `workbench-workbench ${o ? "" : "no-primary"} ${b ? "" : "no-secondary"} ${he ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${h}px`, "--workbench-secondary-width": `${T}px` },
      children: [
        ...
        o && /* @__PURE__ */ c("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ n("div", { className: "workbench-primary-resizer", onPointerDown: Et, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ c("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ n("button", { className: "workbench-command-center", onClick: () => Q(!0), children: "Workspace" }),
          /* @__PURE__ */ c("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ n(
              ia,
              {
                ...
                closeWelcome: () => H(!0),
                secondaryVisible: b,
                toggleSecondary: () => A((r) => !r),
                canPreview: f?.language === "markdown",
                previewVisible: _,
                togglePreview: () => ie((r) => !r)
              }
            ),
            /* @__PURE__ */ n(ra, { file: f, folder: ue, showWelcome: $e }),
            /* @__PURE__ */ n("div", { className: "workbench-editor-content", children: f ? /* @__PURE__ */ c("div", { className: f.language === "markdown" && _ ? "workbench-editor-split" : "workbench-editor-single", children: [
              ...
              f.language === "markdown" && _ && /* @__PURE__ */ n(da, { content: f.content })
            ] }) : $e ? /* @__PURE__ */ n(oa, { openFile: fe, openFolder: Se, newFile: Fe, recent: pe, reopenRecent: Ee, setStatus: C }) : /* @__PURE__ */ c("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ n(De, { size: 34 }),
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", r.body),
        ...
        Ce && /* @__PURE__ */ n(
          sa,
          {
            ...
            activeFile: f,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: Fe },
              ...
              ...pe.map((r) => ({ label: `File: Open Recent — ${r.name}`, detail: r.path, run: () => Ee(r) })),
              { label: "File: Save All", detail: `${B.length} dirty file${B.length === 1 ? "" : "s"}`, run: () => {
                Lt();
              }, disabled: B.length === 0 },
              { label: "File: Close All Editors", detail: `${W.length} open editor${W.length === 1 ? "" : "s"}`, run: _t, disabled: W.length === 0 },
              ...
              { label: "View: Toggle Chat Panel", detail: b ? "Hide right AI side bar" : "Show right AI side bar", run: () => A((r) => !r) },
              { label: "View: Toggle Bottom Panel", detail: he ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => V((r) => !r) },
              { label: "View: Toggle Markdown Preview", detail: f?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => ie((r) => !r), disabled: f?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: f || W.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Ie, disabled: !f && W.length === 0 },
              { label: "AI: Explain Active File", detail: f ? `Ask Cortex to explain ${f.path}` : "Open a file first", run: () => U("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !f },
              { label: "AI: Improve Active File", detail: f ? `Ask Cortex for concrete edits to ${f.path}` : "Open a file first", run: () => U("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !f },
              { label: "AI: Draft Editable Replacement", detail: f ? `Ask Cortex for a full-file replacement for ${f.path}` : "Open a file first", run: () => Qe("edit"), disabled: !f },
              { label: "AI: Plan Workspace Work", detail: W.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => U("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: W.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: f ? "Persist active file in host.ai artifacts" : "Open a file first", run: Bt, disabled: !f },
              { label: "AI: Open Latest Artifact as File", detail: te[0] ? `Create editable file from ${te[0].title}` : "No outputs yet", run: () => te[0] && Oe(te[0]), disabled: te.length === 0 }
            ],
            openWorkbenchFile: L,
            onClose: () => Q(!1)
          }
        ...
        /* @__PURE__ */ n(ka, { status: Pt, file: f ?? Jn, cursor: R, fileCount: k.length, dirtyCount: B.length })
      ]
    }
  );
}
function ea({ active: e, setActive: t, openSettings: a }) {
  return /* @__PURE__ */ c("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(ct, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(Pn, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(kn, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(an, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(rt, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ n("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(ln, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ n(ne, { icon: /* @__PURE__ */ n(En, { size: 23 }), label: "Manage", active: !1, onClick: a })
  ] });
}
function ne({ icon: e, label: t, active: a, onClick: i }) {
  ...
  return /* @__PURE__ */ c("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ c(Re, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((i) => /* @__PURE__ */ n(Ue, { file: i, active: i.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(i) }, i.id)),
        /* @__PURE__ */ c("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ n(ve, { size: 12 }),
          " ",
...
function Ue({ file: e, active: t, onOpen: a, basePath: i, depth: o = 0, label: l }) {
...
function aa(e, t) {
  const a = [], i = /* @__PURE__ */ new Map(), o = (h, s, p = a) => {
    const m = i.get(h);
    if (m) return m;
    const b = { name: s, path: h, children: [] };
    return i.set(h, b), p.push(b), b;
  };
  e.forEach((h) => {
    const s = t && h.path.startsWith(`${t}/`) ? h.path.slice(t.length + 1) : h.path, p = s.split("/").filter(Boolean);
    let m = a, b = "";
    p.slice(0, -1).forEach((A) => {
      b = b ? `${b}/${A}` : A, m = o(b, A, m).children;
    }), m.push({ name: p.at(-1) ?? h.name, path: s, file: h, children: [] });
  });
  const l = (h) => h.sort((s, p) => +!!s.file - +!!p.file || s.name.localeCompare(p.name)).map((s) => ({ ...s, children: l(s.children) }));
  return l(a);
}
function yt(e, t, a, i = 0) {
  return e.map((o) => o.file ? /* @__PURE__ */ n(Ue, { file: o.file, active: o.file.id === t, onOpen: () => a(o.file), depth: i, label: o.name }, o.file.id) : /* @__PURE__ */ c("div", { children: [
...
function ia(e) {
  return /* @__PURE__ */ c("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ c("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ n(re, { size: 13 }) })
    ] }),
    ...
    /* @__PURE__ */ n("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ n(An, { size: 16 }) })
  ] });
}
    ...
    /* @__PURE__ */ c("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? h.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No matches." }) : h.map((s) => {
        ...
        return /* @__PURE__ */ c("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ n(Ue, { file: p, active: p.id === o, onOpen: () => i(p) }),
          s.slice(0, 5).map((m) => /* @__PURE__ */ c("button", { className: "workbench-search-hit", onClick: () => i(p, m.lineNumber), title: m.line, children: [
            /* @__PURE__ */ n("span", { className: "workbench-search-line", children: m.lineNumber }),
            /* @__PURE__ */ n("span", { children: m.line })
          ] }, `${p.id}-${m.lineNumber}-${m.line}`)),
...
function ha(e) {
  return /* @__PURE__ */ c("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ c("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ n("span", {}),
      /* @__PURE__ */ n("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ n(re, { size: 14 }) })
    ] }),
    /* @__PURE__ */ c("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ n(vt, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function ua(e) {
  return /* @__PURE__ */ c("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ c("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ c("div", { className: "workbench-secondary-actions", children: [
        ...
        /* @__PURE__ */ n("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ n(re, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ n(ma, { ...e }) : /* @__PURE__ */ n(vt, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function ma(e) {
      ...
      /* @__PURE__ */ c("div", { className: "workbench-chat-box", children: [
        ...
        /* @__PURE__ */ c("div", { className: "workbench-chat-toolbar", children: [
          /* @__PURE__ */ c("button", { className: "workbench-chat-mode", onClick: e.openSettings, title: "Chat AI settings", children: [
            qe(e.chatSettings.gatewayPreference),
            " ",
            /* @__PURE__ */ n(ve, { size: 12 })
          ] }),
          /* @__PURE__ */ n("button", { className: "workbench-chat-mode", onClick: () => e.runQuickPrompt("plan"), disabled: e.busy, children: "Plan" }),
          /* @__PURE__ */ n("span", {}),
          /* @__PURE__ */ n("button", { className: "workbench-chat-send", onClick: e.askAgent, disabled: e.busy, title: "Send", children: /* @__PURE__ */ n(Fn, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function pa(e, t, a) {
  const i = e.model.trim(), l = [e.gatewayPreference === "auto" ? t : qe(e.gatewayPreference)];
...
function fa(e) {
  ...
  ae(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        a(m.map((b) => ({ id: b.id, gatewayLabel: b.gatewayLabel }))), o(m.length > 0 ? `${m.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (m) {
        if (s.signal.aborted) return;
  ...
  return /* @__PURE__ */ n("div", { className: "workbench-settings-backdrop", onClick: e.onClose, children: /* @__PURE__ */ c("section", { className: "workbench-settings-dialog", onClick: (s) => s.stopPropagation(), role: "dialog", "aria-modal": "true", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ c("header", { className: "workbench-settings-header", children: [
      /* @__PURE__ */ n(zn, { size: 15 }),
      /* @__PURE__ */ n("strong", { children: "Atomek Settings" }),
      /* @__PURE__ */ n("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ n(re, { size: 15 }) })
    ] }),
...
function vt({ outputs: e, clearOutputs: t, deleteArtifact: a, runAiSynthesis: i, openOutputAsFile: o, previewEditFromOutput: l, canPreviewEdit: h = !1, compact: s = !1 }) {
  return /* @__PURE__ */ c("div", { className: `workbench-panel-list ${s ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ n("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((p) => /* @__PURE__ */ c("div", { className: "workbench-output-card", children: [
...
function ba({ edit: e, onApply: t, onOpenAsFile: a, onClose: i }) {
  return /* @__PURE__ */ n("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ c("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ c("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ n("button", { title: "Close", onClick: i, children: /* @__PURE__ */ n(re, { size: 16 }) })
    ] }),
...
`), i = [];
  ...
  for (let h = 0; h < o.length; h += 1) {
    const s = o[h], m = o.find((A) => A > s && a[A].startsWith("diff --git ")) ?? a.length, b = a.slice(s, m).join(`
`);
    /^@@\s+-\d+/m.test(b) && l.push(b);
  }
...
function $a(e, t) {
  const a = Array.from(e.matchAll(/```([^\n`]*)\n([\s\S]*?)```/g)).map((m) => ({
    ...
    ...a.filter((m) => D(m.lang, "diff") || D(m.lang, "patch")).map((m) => m.content),
    e
  ];
  for (const m of i) {
    const b = Nt(t.content, m);
    if (b) return { content: b, label: "unified diff patch" };
  }
  if (a.length === 0) return null;
  const o = Sa(t), l = a.find((m) => D(m.lang, "atomek-replace") || D(m.lang, "replace"));
  if (l) return { content: l.content, label: `replacement block (${l.lang || "plain"})` };
  const h = a.find((m) => o.some((b) => D(m.lang, b)) && !D(m.lang, "diff") && !D(m.lang, "patch"));
  if (h) return { content: h.content, label: `matched ${h.lang || t.language} block` };
  const p = a.filter((m) => !D(m.lang, "diff") && !D(m.lang, "patch")).sort((m, b) => b.content.length - m.content.length)[0];
...
`), o = [];
  ...
  for (let s = 0; s < i.length; s += 1) {
    ...
    if (!p) continue;
    h = !0;
    const m = Number(p[1]), b = Math.max(0, m - 1);
    if (b < l) return null;
    for (o.push(...a.slice(l, b)), l = b, s += 1; s < i.length; s += 1) {
      const A = i[s];
      if (A.startsWith("@@ ")) {
        s -= 1;
        break;
      }
      if (A.startsWith("diff --git ") || A.startsWith("--- ") || A.startsWith("+++ ") || A.startsWith("\\ No newline at end of file")) continue;
      const T = A[0], j = A.slice(1);
      if (T === " ") {
...
function za(e) {
  return function() {
    return /* @__PURE__ */ n(Ia, { host: e.host });
  };
}
export {
  za as default
