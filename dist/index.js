 ...
 */
const Va = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), qa = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), sn = (e) => {
  const t = qa(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, Sn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), Ka = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Qa = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], Za = j("check", Qa);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const er = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], rt = j("chevron-down", er);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const Er = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Or() {
  const e = Dt();
...
async function Rr() {
  ...
  if (!e) return _n(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((a) => zn(a, a.name, "local-file")));
  return Ht(n.filter(Boolean));
}
async function Fr() {
  const e = Dt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await _n(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await jn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Ht(n) };
}
function Dt() {
  const e = window;
...
async function jn(e, t, n, a, i) {
  if (!(n.length >= i || a > 8))
    for await (const o of e.values()) {
      if (n.length >= i) break;
      if (o.kind === "directory") {
        if (Er.has(o.name)) continue;
        await jn(o, `${t}/${o.name}`, n, a + 1, i);
        continue;
      }
      const s = o, l = await zn(s, `${t}/${s.name}`, "local-folder");
      l && n.push(l);
    }
}
function _n(e) {
...
const Ee = "atomek:default", jr = 3e3, Bt = "tytus.atomek.threadTitleOverrides", ln = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, We = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, _r = (e, t = jr) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, dn = (e, t) => {
  const n = e.split(`
`).map((a) => a.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Dr = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", hn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Dr(e.kind),
  body: e.body,
...
function Ur({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const i = e.ai, [o, s] = v(null), [l, m] = v([]), [f, b] = v([]), [h, k] = v([]), [N, T] = v([]), [_, J] = v(ln), [G, Z] = v(!1), ee = Re(!0), M = Re(null), z = y(async () => {
    if (!i) {
      J(ln);
      return;
    }
    ...
    if (i)
      try {
        const C = await i.listArtifacts({ threadId: $ });
    ...
    if (i)
      try {
        const C = await i.listMessages($);
        if (!ee.current) return;
        const O = l.find((p) => p.id === $) ?? null;
        O && s(O), b(C.map(We).filter(Boolean)), T([]), await Q($);
      } catch (C) {
    ...
    if (i)
      try {
        const $ = un(await i.listThreads({ workspaceKey: Ee, status: "active" })), C = $[0] ?? await i.createThread({ workspaceKey: Ee, title: "Atomek chat" });
        if (!ee.current) return;
        m($[0] ? $ : [C]), s(C);
        const O = await i.listMessages(C.id);
        if (!ee.current) return;
        b(O.map(We).filter(Boolean)), await Q(C.id);
      } catch ($) {
        a(`AI chat unavailable: ${$ instanceof Error ? $.message : String($)}`);
      }
  }, [i, Q, a]);
  Y(() => (ee.current = !0, z(), te(), () => {
    ee.current = !1;
  ...
  const oe = y(async () => {
    if (!i) return null;
    if (o) return o;
    const $ = await i.createThread({ workspaceKey: Ee, title: "Atomek chat" });
    return ee.current && (s($), m((C) => [$, ...C.filter((O) => O.id !== $.id)])), $;
  }, [i, o]), ye = y(async () => {
    if (!i) return;
    const $ = await i.createThread({ workspaceKey: Ee, title: "Atomek chat" });
    s($), m((C) => [$, ...C.filter((O) => O.id !== $.id)]), b([]), k([]), T([]), a("New AI chat created");
  }, [i, a]), x = y(async ($, C) => {
    if (!i) return;
    const O = C.trim();
    if (O)
      try {
        const p = i.updateThread, L = l.find((S) => S.id === $) ?? o, P = typeof p == "function" ? await p({ threadId: $, title: O }) : { ...L ?? await oe(), id: $, title: O, updatedAt: Date.now() };
        typeof p != "function" && Br($, O), m((S) => S.map((E) => E.id === P.id ? P : E)), s((S) => S?.id === P.id ? P : S), a(typeof p == "function" ? `Renamed chat: ${P.title}` : `Renamed chat locally: ${P.title}`);
      } catch (p) {
        a(`Rename chat failed: ${p instanceof Error ? p.message : String(p)}`);
      }
  }, [i, oe, a, o, l]), V = y(async ($) => {
    if (i)
      try {
        await i.deleteThread($), Wr($);
        const C = l.filter((O) => O.id !== $);
        if (m(C), o?.id === $) {
          const O = C[0] ?? await i.createThread({ workspaceKey: Ee, title: "Atomek chat" });
          s(O), m((L) => L.some((P) => P.id === O.id) ? L : [O, ...L]);
          const p = await i.listMessages(O.id);
          b(p.map(We).filter(Boolean)), await Q(O.id);
        }
        a("Deleted AI chat");
      } catch (C) {
        a(`Delete chat failed: ${C instanceof Error ? C.message : String(C)}`);
      }
  }, [i, Q, a, o?.id, l]), ue = y(async ($) => {
    if (!i) return null;
    try {
      const C = await oe();
      if (!C) return null;
      const O = await i.createArtifact({
        threadId: C.id,
      ...
      return k((L) => [p, ...L.filter((P) => P.id !== p.id)]), a(`Saved AI artifact: ${p.title}`), p;
    } catch (C) {
    ...
    if (i)
      try {
        await i.deleteArtifact($), k((C) => C.filter((O) => O.id !== $)), a("Deleted AI artifact");
      } catch (C) {
    ...
    try {
      ...
      return T((O) => [C, ...O.filter((p) => p.id !== C.id)].slice(0, 5)), a(`Remembered: ${C.title}`), C;
    } catch (C) {
      return a(`Remember failed: ${C instanceof Error ? C.message : String(C)}`), null;
    }
  }, [i, a]), ne = y(async ($) => {
    if (!i) return [];
    const C = await i.searchMemory({ query: $, limit: 5 });
    return ee.current && T(C), C;
  }, [i]), $e = y(async ($, C = {}) => {
    const O = $.trim();
    if (!O || !i) return null;
    Z(!0);
    ...
    try {
      const P = await oe();
      if (!P) return null;
      const S = await ne(O).catch(() => []), E = Hr(S), D = C.requestContext ?? t, K = E ? [...D, E] : [...D];
      let B = null;
      for await (const U of i.sendMessage({
        threadId: P.id,
        body: O,
        ...
        if (U.type === "message_created") {
          const W = We(U.message);
          if (!W) continue;
          W.role === "assistant" && (B = W.id), b((se) => [...se.filter((ce) => ce.id !== W.id), W]);
        }
        if (U.type === "token" && (B = U.messageId, b((W) => W.some((se) => se.id === U.messageId) ? W.map(
          (se) => se.id === U.messageId ? { ...se, body: U.body, status: "streaming" } : se
        ) : [...W, {
          id: U.messageId,
          role: "assistant",
          body: U.body,
          status: "streaming",
          createdAt: Date.now()
        }])), U.type === "message_updated" || U.type === "done") {
          const W = We(U.message);
          if (!W) continue;
          W.role === "assistant" && (p = W), b((se) => se.map((ce) => ce.id === W.id ? W : ce)), W.gatewayLabel && a(`AI answered via ${W.gatewayLabel}`);
        }
        if (U.type === "run_failed") {
          const W = L.signal.aborted;
          a(W ? "AI response stopped" : `AI failed: ${U.error}`), B && b((se) => se.map((ce) => ce.id !== B ? ce : W ? { ...ce, status: "complete", error: void 0, body: ce.body || "Stopped by user." } : { ...ce, status: "error", error: U.error, body: U.error }));
        }
      }
      const X = un(await i.listThreads({ workspaceKey: Ee, status: "active" }).catch(() => []));
      return ee.current && X.length > 0 && m(X), z(), p;
    } catch (P) {
      return P instanceof DOMException && P.name === "AbortError" ? (a("AI response stopped"), p) : (a(`AI failed: ${P instanceof Error ? P.message : String(P)}`), null);
    } finally {
      M.current === L && (M.current = null), Z(!1);
    }
  }, [i, n.gatewayPreference, n.model, oe, ne, z, t, a]), pe = y(() => {
    M.current?.abort(), a("Stopping AI response…");
  }, [a]);
  return {
    aiStatus: _,
    artifacts: h,
...
const Vr = (e) => `tytus-workbench:///${encodeURI(e.path)}`, qr = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Kr = ({ files: e, openEditorIds: t, activeFileId: n, versions: a, activeSelection: i }) => {
...
}, Gr = "active-file", Jr = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Xr = (e, t = "file") => `${t}:${e.id}`, Yr = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let i = [];
  return t.scope === "active-selection" ? a?.selection ? i = [Qr(a, a.selection, !0)] : a && (i = [Ze(a, !0, "Active file")]) : t.scope === "active-file" ? a && (i = [Ze(a, !0, "Active file")]) : t.scope === "open-editors" ? i = e.openDocumentIds.map((o) => e.byId.get(o)).filter(Boolean).map((o) => Ze(o, !0, "Open editor")) : t.scope === "selected-files" && (i = t.selectedFileIds.map((o) => e.byId.get(o)).filter(Boolean).map((o) => Ze(o, !1, "Selected file"))), ei(i).filter((o) => !n.has(o.id));
}, ei = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, ti = 4e3, ni = 8e3, ai = 4e3, ri = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, ii = (e, t, n) => {
  const a = Zr(e, n), i = [], o = new Map(t.map((s) => [s.id, s]));
  for (const s of a) {
    ...
    const f = [
      ...
      s.range ? `Range: ${mn(s.range)}` : null
    ].filter(Boolean).join(`
`), b = s.kind === "selection" && s.range ? oi(l.content, s.range) : l.content, h = s.kind === "selection" ? ti : s.label.startsWith("Open editor") ? ai : ni;
    i.push({
...
`));
}, mn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, pn = 75e4, si = 2800, ci = 240, li = 80, di = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Ie(t.content)}`).sort().join("")
), Et = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? pn) || e.content.length > (t.maxFileBytes ?? pn) ? "huge" : !t.includeDirty && e.dirty ? null : gi(e.path) ? "vendor" : ki(e) ? "binary" : null;
}, fn = (e, t = {}) => {
  ...
  for (const s of e) {
    ...
    const f = Ie(s.content), b = ui(s, f, n, t);
    o.push(...b), a.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
...
}, ui = (e, t = Ie(e.content), n = Date.now(), a = {}) => {
  const i = Math.max(500, a.maxChunkChars ?? si), o = Math.min(Math.max(0, a.chunkOverlapChars ?? ci), Math.floor(i / 2)), s = Math.max(1, a.maxChunksPerFile ?? li), l = fi(e.content), m = [];
  let f = 0, b = 0;
  for (; f < e.content.length && m.length < s; ) {
    const h = pi(e.content, f, Math.min(e.content.length, f + i)), k = e.content.slice(f, h).trim();
    if (k.length > 0) {
      const N = bi(l, f, h);
      m.push({
        id: `${e.id}:${t}:${b}:${N.startLineNumber}-${N.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const a = bn(e, t), i = bn(e, Math.max(t, n - 1));
  return {
    startLineNumber: a + 1,
    startColumn: t - e[a] + 1,
    endLineNumber: i + 1,
  ...
  return 0;
}, gi = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => di.has(t)), ki = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && hi.has(t) || e.content.includes("\0")) return !0;
...
class wi {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = fn(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = fn(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return yi(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const Oe = (e = [], t = {}) => new wi(e, t), yi = (e, t, n = {}) => {
  ...
  for (const l of e.files) {
    ...
    if (Et(m, n)) {
      o.push({ fileId: l.fileId, path: m.path, status: "skipped-now", indexedHash: l.hash, currentHash: Ie(m.content) });
      continue;
    }
    const b = Ie(m.content);
    b !== l.hash ? o.push({ fileId: l.fileId, path: m.path, status: "changed", indexedHash: l.hash, currentHash: b }) : m.dirty !== l.dirty && o.push({ fileId: l.fileId, path: m.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: b });
  }
  for (const l of t)
    !i.has(l.id) && !Et(l, n) && o.push({ fileId: l.id, path: l.path, status: "new", currentHash: Ie(l.content) });
  const s = e.signature !== Bn(t);
  return { stale: o.length > 0, signatureChanged: s, files: o };
}, vi = 8, Ci = 12e3, Wn = (e, t, n = {}, a) => {
  const i = Ai(t);
  if (i.length === 0) return [];
  const o = new Set(a?.files.map((b) => b.fileId) ?? []), s = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => Ni(b, i)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, h) => h.score - b.score || b.chunk.path.localeCompare(h.chunk.path) || b.chunk.ordinal - h.chunk.ordinal), l = [], m = /* @__PURE__ */ new Set();
  let f = n.maxChars ?? Ci;
  for (const b of s) {
    if (l.length >= (n.limit ?? vi) || f <= 0) break;
    const h = `${b.chunk.fileId}:${b.chunk.ordinal}`;
    if (m.has(h)) continue;
    m.add(h);
    const k = b.chunk.text.length > f ? `${b.chunk.text.slice(0, Math.max(0, f - 28))}
[...context clipped...]` : b.chunk.text;
    l.push(xi(b, k, o.has(b.chunk.fileId))), f -= k.length;
  }
  return l;
}, xi = (e, t = e.chunk.text, n = !1) => {
...
`), Ai = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const f = At(n, m), b = At(a, m), h = At(i, m), k = o === m ? 1 : 0;
    f + b + h + k !== 0 && (l.push(m), s += Math.min(f, 12), s += b * 4, s += h * 6, s += k * 3, (n.includes(`function ${m}`) || n.includes(`const ${m}`) || n.includes(`class ${m}`) || n.includes(`type ${m}`)) && (s += 3));
  }
  ...
  const { autoRefresh: n = !1, ...a } = t, i = Re(e), o = Re(null);
  o.current || (o.current = Oe(e, a));
  const [s, l] = v(() => o.current?.getSnapshot() ?? Oe(e, a).getSnapshot());
  i.current = e;
  const m = y((k = i.current) => {
    const N = (o.current ?? Oe([], a)).refresh(k, a);
    return l(N), N;
  }, [a.maxFileBytes, a.maxChunkChars, a.chunkOverlapChars, a.maxChunksPerFile, a.includeDirty]), f = y((k = i.current) => {
    const N = (o.current ?? Oe([], a)).update(k, a);
    return l(N), N;
  }, [m]);
  Y(() => {
    n && m(e);
  }, [n, e, m]);
  const b = (o.current ?? Oe([], a)).staleReport(e), h = y((k, N = {}) => {
    const T = o.current ?? Oe([], a);
    return Wn(T.getSnapshot(), k, N, T.staleReport(i.current));
  }, []);
  return { snapshot: s, staleReport: b, isStale: b.stale, refresh: m, update: f, retrieve: h };
}, Mi = "tytus.atomek.semanticVector:v1", Un = "__gateway_default__", Vn = (e) => e?.trim() || Un, Pi = (e) => {
  const t = Vn(e);
  return t === Un ? "gateway default" : t;
}, qn = (e, t, n) => [
  Mi,
  ...
  if (!Kn()) return null;
  try {
    const a = qn(e, t, n), i = window.localStorage.getItem(a);
    if (!i) return null;
...
}, Ue = 8, Ei = 12e3, Oi = 180, Ri = async (e, t, n, a, i = {}, o, s) => {
  const l = Wn(t, n, { ...i, limit: Math.max(i.limit ?? Ue, 16) }, o), m = e.ai;
  if (typeof m?.embedText != "function")
    return { hits: l.slice(0, i.limit ?? Ue), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const f = Vn(a.embeddingModel), b = await m.embedText({
      input: n,
    ...
    const k = new Map(l.map((M) => [M.chunkId, M])), N = Bi(n), T = /* @__PURE__ */ new Map();
    t.chunks.filter((M) => i.includeDirty !== !1 || !M.dirty).slice(0, Oi).forEach((M) => T.set(M.id, M)), l.forEach((M) => {
      const z = t.byChunkId.get(M.chunkId);
      z && T.set(z.id, z);
    });
    const _ = Array.from(T.values()), J = [];
    let G = 0;
    for (const M of _) {
      s?.aborted;
      const z = Ti(e.appId, f, M) ?? await zi(m, e.appId, f, M, a, s);
      if (!z) continue;
      z.updatedAt > Date.now() - 1e3 && (G += 1);
      const Q = Di(h, z.vector);
      if (!Number.isFinite(Q)) continue;
      const te = k.get(M.id)?.score ?? Wi(M, N), oe = Hi(te) * 0.42 + Math.max(0, Q) * 0.58;
      J.push({ chunk: M, keywordScore: te, vectorScore: Q, score: oe, matchedTerms: N.filter((ye) => M.text.toLowerCase().includes(ye)) });
    }
    if (J.length === 0) return { hits: l.slice(0, i.limit ?? Ue), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: G };
    const Z = new Set(o?.files.map((M) => M.fileId) ?? []), ee = J.sort((M, z) => z.score - M.score || z.keywordScore - M.keywordScore || M.chunk.path.localeCompare(z.chunk.path) || M.chunk.ordinal - z.chunk.ordinal).slice(0, Math.max(i.limit ?? Ue, 1)).map((M) => ji(M, Z.has(M.chunk.fileId)));
    return {
      hits: _i(ee, i.maxChars ?? Ei),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${Pi(a.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: G
    };
  } catch (f) {
    return {
      hits: l.slice(0, i.limit ?? Ue),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: Ui(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, Hi = (e) => Math.min(1, Math.max(0, e / 24)), Bi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Wi = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, Vi = /```([^\n`]*)\n([\s\S]*?)```/g, qi = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Ut(e) {
  return Array.from(e.matchAll(Vi)).map((t) => Jn(t[1] ?? "", Yi(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Jn(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), a = {}, i = [];
  for (const o of n) {
    const s = o.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? a[s[1].toLowerCase()] = Zi(s[2]) : i.push(o.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: i, attrs: a, content: t };
}
function me(e, t) {
  return (typeof e == "string" ? Jn(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Xn(e) {
  return [...Ut(e).filter((a) => me(a, "diff") || me(a, "patch")).map((a) => a.content), e].filter((a, i, o) => Ki(a) && o.findIndex((s) => s === a) === i);
}
function Ki(e) {
...
`), a = [];
  ...
  for (let s = 0; s < i.length; s += 1) {
    const l = i[s], f = i.find((h) => h > l && n[h].startsWith("diff --git ")) ?? n.length, b = n.slice(l, f).join(`
`);
    /^@@\s+-\d+/m.test(b) && o.push({ raw: b, ...Nt(b) });
  }
  return o.length > 0 ? o : [{ raw: t, ...Nt(t) }];
}
function Nt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), a = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), i = we(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, o = we(t?.[3] || t?.[4] || a?.[1] || a?.[2] || "") || null, s = o && o !== "/dev/null" ? o : i;
...
function Ot(e) {
  return Ut(e).filter((t) => Ji(t)).map((t) => ({
...
`), i = [];
  ...
  for (let l = 0; l < a.length; l += 1) {
    const m = a[l].match(qi);
    if (!m) continue;
    s = !0;
    const f = Number(m[1]), b = Math.max(0, f - 1);
    if (b < o) return null;
    for (i.push(...n.slice(o, b)), o = b, l += 1; l < a.length; l += 1) {
      const h = a[l];
      if (h.startsWith("@@ ")) {
        l -= 1;
        break;
...
function eo(e) {
  ...
  for (const b of Xn(t))
    for (const h of Gi(b)) {
      if (!h.path) {
        if (i) {
          const T = Rt(i.content, h.raw);
          T && T !== i.content && s.set(i.id, et({
            file: i,
            proposedContent: T,
            sourceTitle: a,
      ...
      const N = Rt(k.file.content, h.raw);
      if (!N || N === k.file.content) {
  ...
  for (const b of Ot(t)) {
    const h = b.path ? gn(n, b.path) : i ? { file: i, normalizedPatchPath: we(i.path), confidence: "exact" } : null;
    if (!h) {
  ...
  const m = Ot(t).some((b) => b.path);
  if (s.size === 0 && i && !m) {
    const b = to(t, i);
    b && b.content !== i.content && s.set(i.id, et({
      file: i,
      proposedContent: b.content,
      sourceTitle: a,
      extractionLabel: b.label,
...
function et(e) {
  ...
  return {
    ...
    stats: no(e.file.content, e.proposedContent),
    base: {
...
const ot = (e) => typeof e == "object" && e !== null, oo = (e) => ot(e.ai) ? e.ai : null, so = (e) => {
  if (!ot(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, co = (e) => {
  const t = so(e);
  return !t || !ot(e) ? null : { ...e, id: t };
}, ge = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(ge) : ot(e) ? Object.values(e).some(ge) : !1, lo = async (e, t) => {
  const n = oo(e);
  if (typeof n?.listModels != "function") return [];
  const a = await n.listModels(t);
  return Array.isArray(a) ? a.map(co).filter((i) => !!i) : [];
}, ho = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : ge(e.capability) || ge(e.capabilities) || ge(e.modality) || ge(e.modalities) || ge(e.task) || ge(e.tasks) || ge(e.type) || ge(e.kind), Qn = (e) => typeof e == "object" && e !== null, uo = (e) => Qn(e.ai) ? e.ai : null, Zn = (e) => typeof uo(e)?.embedText == "function", mo = async (e, t) => Zn(e) ? (await lo(e, t)).filter(ho) : [], po = (e) => Qn(e.ai) ? Zn(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", fo = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], bo = [
...
function yo(e) {
  const t = e.results.length > 0 ? e.results.map((a, i) => [
    ...
    `Status: ${a.status}`,
    "Output:",
    Io(a.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function Co(e) {
  ...
  for (const a of t) {
    const i = xo(a.content);
    if (!i?.scripts) continue;
    const o = ta(a.path), s = $o(e, o, i);
    if (!s) continue;
    const l = Object.keys(i.scripts).filter((f) => typeof i.scripts?.[f] == "string"), m = Ao(l);
    for (const f of m) {
      const b = `${s} run ${f}`;
      n.push({
        id: na(`${o}:${b}`),
...
function $o(e, t, n) {
  ...
  for (const [a, i] of bo)
    if (e.some((o) => So(o.path) === a && ta(o.path) === t)) return i;
  return null;
}
function Ao(e) {
  const t = fo.filter((a) => e.includes(a)), n = e.filter((a) => !t.includes(a)).filter((a) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(a)).sort((a, i) => a.localeCompare(i));
  return [...t, ...n];
}
function No(e) {
  const t = /* @__PURE__ */ new Set();
...
const Mo = Wa(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Po = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, aa = "tytus.workspace.recent", ra = "tytus.workspace.layout", ia = "tytus.atomek.chatAiSettings", at = {
...
function St(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - To), a = n < 1180, i = a ? 200 : 240, o = a ? 300 : 340, s = a ? 420 : 560, l = Math.max(i, Math.min(a ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), f = Math.floor(n * (a ? 0.34 : 0.36)), b = n - m - s, h = Math.max(o, Math.min(a ? 500 : 640, f, b));
  return { primaryMin: i, primaryMax: l, secondaryMin: o, secondaryMax: h };
}
function Lo(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let a = 0, i = 0, o;
...
function yn(e, t, n, a, i) {
  return [
    ...
    "## Resource graph",
    "",
    t ? `- ${oa(t.resources)}` : "- not loaded",
    ...t?.warnings?.length ? t.warnings.map((o) => `- Warning: ${o.code} — ${o.message}`) : [],
    "",
    "## Rules",
    "",
...
function Ro(e, t, n) {
  return JSON.stringify({
    ...
    resources: (t?.resources ?? []).filter((a) => a.status === "ready").map((a) => ({
      resourceId: a.id,
      pinnedLabel: a.label,
      pinnedKind: a.kind,
      pinnedCapabilities: a.capabilities
    })),
    permissions: {
    ...
    tasks: [{
      id: `task-${Date.now()}`,
      title: n.slice(0, 80) || "Atomek local agent task",
      prompt: n,
      status: "ready",
      dependsOn: [],
...
function Fo({ host: e }) {
  const t = Re(null), [n, a] = v(0), i = ie(() => ms(), []), [o, s] = v("explorer"), [l, m] = v(i.primaryVisible), [f, b] = v(i.primaryWidth), [h, k] = v("chat"), [N, T] = v(i.secondaryVisible), [_, J] = v(i.secondaryWidth), [G, Z] = v(!1), [ee, M] = v("problems"), [z, Q] = v(i.markdownPreviewVisible), [R, te] = v(!1), [oe, ye] = v(null), [x, V] = v([]), [ue, fe] = v([]), [q, ne] = v(null), [$e, pe] = v(""), [$, C] = v({ lineNumber: 1, column: 1 }), [O, p] = v(null), [L, P] = v({}), [S, E] = v(Gr), [D, K] = v([]), [B, X] = v([]), [U, W] = v(null), [se, ce] = v(""), [ma, ct] = v(!1), [pa, fa] = v(""), [qt, qe] = v([]), [ae, Me] = v(null), [ve, Fe] = v(null), [Kt, Gt] = v(!1), [Ae, ba] = v(() => ps()), [Jt, ze] = v(null), [Pe, lt] = v(null), [dt, ht] = v(""), [ut, mt] = v(""), [Ke, Ge] = v(""), [Je, Xt] = v("failed"), [ga, Yt] = v(null), [ka, A] = v("Ready"), [je, wa] = v(() => us()), de = ue.map((c) => x.find((u) => u.id === c)).filter(Boolean), I = q ? x.find((c) => c.id === q) ?? null : null, Qt = ie(() => Kr({ files: x, openEditorIds: ue, activeFileId: q, versions: L, activeSelection: O }), [q, O, L, x, ue]), Zt = ie(() => ({
    scope: S,
    removedAttachmentIds: D,
    selectedFileIds: []
  }), [S, D]), _e = ie(() => ii(Qt, x, Zt), [Zt, Qt, x]), Ce = Ii(x, { autoRefresh: !0, includeDirty: !0 }), en = ie(() => B.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [B]), ya = ie(() => [..._e.attachments, ...en], [_e.attachments, en]), F = Ur({ host: e, requestContext: _e.parts, chatSettings: Ae, setStatus: A }), Ne = ie(
    () => [...F.artifacts, ...qt].sort((c, u) => u.createdAt - c.createdAt),
    [F.artifacts, qt]
  ), Xe = Kt && !I, pt = !I && !Xe && !R, le = ie(() => x.filter((c) => c.dirty), [x]), va = ie(() => {
    const c = $e.trim().toLowerCase();
    return c ? x.filter((u) => u.path.toLowerCase().includes(c)) : x;
  }, [x, $e]);
  Y(() => {
    P((c) => {
      const u = {};
      for (const g of x) u[g.id] = c[g.id] ?? 1;
      return u;
    });
  }, [x]), Y(() => {
    K([]), X([]);
  ...
  const Ca = y((c) => {
    if (c.kind === "index-hit") {
      X((u) => u.filter((g) => g.id !== c.id)), A(`Removed project context: ${c.label}`);
      return;
    }
    K((u) => u.includes(c.id) ? u : [...u, c.id]), A(`Removed chat context: ${c.label}`);
  }, []), Ye = y(() => {
    Gt(!0), ne(null), te(!0), A("Atomek settings opened");
  }, []), tn = y(() => {
    Gt(!1), ne((c) => c ?? ue.at(-1) ?? null), ue.length === 0 && te(!1), A("Atomek settings closed");
  }, [ue]), Te = y((c) => {
    P((u) => ({ ...u, [c]: (u[c] ?? 1) + 1 }));
  }, []), xa = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = St(n), g = c.clientX, w = _, H = (Be) => {
      const xt = w + (g - Be.clientX);
      J(tt(xt, u.secondaryMin, u.secondaryMax));
    }, be = () => {
      window.removeEventListener("pointermove", H), window.removeEventListener("pointerup", be);
    };
    window.addEventListener("pointermove", H), window.addEventListener("pointerup", be);
  }, [_, n]), $a = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const u = St(n), g = c.clientX, w = f, H = (Be) => {
      const xt = w + (Be.clientX - g);
      b(tt(xt, u.primaryMin, u.primaryMax));
    }, be = () => {
      window.removeEventListener("pointermove", H), window.removeEventListener("pointerup", be);
    };
    window.addEventListener("pointermove", H), window.addEventListener("pointerup", be);
  }, [f, n]), Qe = y((c) => {
    const u = [c, ...je.filter((g) => g.path !== c.path)].slice(0, 6);
    wa(u), localStorage.setItem(aa, JSON.stringify(u));
  }, [je]), re = y((c, u) => {
    fe((g) => g.includes(c.id) ? g : [...g, c.id]), ne(c.id), te(!1), Yt(u ?? null), C({ lineNumber: u ?? 1, column: 1 });
  }, []), Aa = y((c) => {
    if (!c.fileId) return;
    const u = x.find((g) => g.id === c.fileId);
    u && (re(u, c.range?.startLineNumber ?? 1), A(`Revealed context: ${c.label}`));
  }, [x, re]), De = y(async () => {
    if (It(le, "open new files"))
      try {
        const c = await Rr();
        if (c.length === 0) return;
        V((u) => ls(u, c)), c.forEach((u) => Qe({ name: u.name, path: u.path, at: Date.now() })), re(c[0]), A(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && A(`Open file failed: ${c.message}`);
      }
  }, [le, re, Qe]), ft = y(async () => {
    if (It(le, "open another folder"))
      try {
        const c = await Fr();
        ye(c), V(c.files), fe([]), ne(null), te(!1), Qe({ name: c.name, path: c.name, at: Date.now() }), A(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!q) return;
    let u = !1;
    V((g) => g.map((w) => w.id !== q || w.content === c ? w : (u = !0, { ...w, content: c, dirty: !0 }))), u && Te(q);
  }, [q, Te]), bt = y(async () => {
    if (I)
      try {
        const c = await $t(I);
        V((u) => u.map((g) => g.id === c.id ? c : g)), A(`Saved ${c.name}`);
      } catch (c) {
        A(`Save failed: ${c.message}`);
      }
  }, [I]), Sa = y(async (c) => {
    const u = x.find((w) => w.id === c);
    if (!u) return null;
    const g = await $t(u);
    return V((w) => w.map((H) => H.id === g.id ? g : H)), g;
  }, [x]), nn = y(async () => {
    const c = x.filter((u) => u.dirty);
    if (c.length === 0) {
      A("No dirty files to save");
      return;
    }
    try {
      const u = await Promise.all(c.map((w) => $t(w))), g = new Map(u.map((w) => [w.id, w]));
      V((w) => w.map((H) => g.get(H.id) ?? H)), ze(null), A(`Saved ${u.length} dirty file${u.length === 1 ? "" : "s"}`);
    } catch (u) {
      A(`Save all failed: ${u.message}`);
    }
  }, [x]), gt = y((c) => {
    const u = x.find((g) => g.id === c);
    if (u?.dirty && !window.confirm(`${u.name} has unsaved changes. Close without saving?`)) {
      A(`Close canceled — ${u.name} has unsaved changes`);
      return;
    }
    fe((g) => {
      const w = g.filter((H) => H !== c);
      return q === c && (ne(w.at(-1) ?? null), p(null)), w;
    });
  }, [q, x]), Ia = y(() => {
    It(le, "close all editors") && (fe([]), ne(null), Yt(null), te(!1), A("Closed all editors"));
  }, [le]), kt = y(() => {
    const c = x.filter((g) => g.name.startsWith("Untitled")).length + 1, u = {
...
`,
    ...
    return X(w), w.length === 0 ? (A(Ce.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), u) : (g.mode !== "hybrid" && g.reason ? A(g.reason) : A(`${w.length} project context hit${w.length === 1 ? "" : "s"} · ${g.reason ?? "hybrid retrieval"}`), [
      ...u,
      ...Fi(w)
    ]);
  }, [_e.parts, S, Ae, e, Ce]), wt = y(() => {
    if (!I && de.length === 0) {
      A("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const u = [
      `Create a polished Markdown artifact from ${I?.path ?? `${de.length} open editors`}.`,
    ...
    T(!0), k("chat"), A("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const g = await xe(u), w = await F.askAgent(u, { requestContext: g });
      !w || w.status === "error" || F.createArtifact({
        messageId: w.id,
        title: `AI synthesis — ${I?.name ?? "open workspace"}`,
        kind: "markdown",
        body: w.body
      }).then(() => {
        T(!0), k("outputs"), Z(!0), M("output");
      });
    })();
  }, [I, F, xe, de.length]), yt = y((c) => {
    const u = x.find((g) => g.path === c.path || g.name === c.name);
    if (u) {
      re(u), A(`Opened recent ${u.name}`);
      return;
    }
    A("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [x, re]), he = y((c) => {
    T(!0), k("chat"), ce(""), (async () => {
      const u = await xe(c);
      await F.askAgent(c, { requestContext: u });
    })();
  }, [F, xe]), an = y(async (c) => {
    if (!e.skills?.get) {
    ...
    try {
      const u = await e.skills.get(c.id), g = u.body.length > 4500 ? `${u.body.slice(0, 4500)}

[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : u.body, w = [
...
`);
      ce((H) => [H.trim(), w].filter(Boolean).join(`

`)), T(!0), k("chat"), A(`Attached skill ${u.title} to chat input`);
    } catch (u) {
      A(`Failed to attach skill: ${u instanceof Error ? u.message : String(u)}`);
    }
  }, [e.skills]), Ma = y((c) => {
    const u = F.messages.findIndex((w) => w.id === c.id), g = F.messages.slice(0, u < 0 ? void 0 : u).reverse().find((w) => w.role === "user");
    if (!g?.body.trim()) {
      A("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const w = await xe(g.body);
      await F.askAgent(g.body, { requestContext: w });
    })();
  }, [F, xe]), Pa = y((c) => {
    F.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    F.createArtifact({
      title: I.path,
      kind: I.language === "markdown" ? "markdown" : "report",
      body: I.content
    }).then(() => {
      T(!0), k("outputs"), Z(!0), M("output");
    });
  }, [I, F]), vt = y((c) => {
    const u = Pt(x, Mt(c.title || c.kind)), g = {
    ...
    V((w) => [...w, g]), re(g), A(`Opened ${c.title} as editable file`);
  }, [x, re]), Ct = y(() => {
...
`), H = {
    ...
    qe((be) => [H, ...be]), Z(!0), M("output"), A(`Captured manual check: ${u}`);
  }, []), Le = y((c, u) => {
    const g = eo({
      body: u,
      files: x,
      ...
      edits: g.edits.map(ds),
      skipped: g.skipped
    };
    return g.edits.length > 1 ? (Fe(w), Me(null), A(`Previewing AI workspace patch for ${g.edits.length} files`), !0) : g.edits.length === 1 ? (Me(w.edits[0]), Fe(null), A(`Previewing AI patch for ${w.edits[0].fileName}`), !0) : (A(g.skipped.length > 0 ? `No applicable edit found. ${g.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [I, L, x]), rn = y((c, u) => {
    const g = {
    ...
    qe((w) => [g, ...w]), T(!0), k("outputs"), Z(!0), M("output"), A(`Saved local job output: ${c}`), (u.includes("```diff") || u.includes("--- a/")) && Le(c, u);
  }, [Le]), Ea = y(() => {
    const c = se.trim();
    if (!c) return;
    ce("");
    const u = Zo(c);
    (async () => {
      const g = await xe(c), w = await F.askAgent(c, { requestContext: g });
      if (!u || !w || w.status === "error") return;
      Le(w.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", w.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [F, xe, se, Le]), Oa = y(() => {
    U && (W(null), he(es(U)));
  }, [he, U]), He = y((c) => {
    const u = go(x, c);
    lt(u);
    const g = u.commands[0]?.command ?? "";
    Ge(g), ht(""), mt(""), Xt("failed"), Z(!0), M("terminal"), A(u.commands.length > 0 ? `Manual check ready: copy ${u.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [x]), Ra = y((c) => {
    c.trim() && (st(c), Ge(c), A(`Manual check command copied: ${c}`));
  }, []), Fa = y(() => {
    lt((c) => {
      if (!c) return c;
      const u = ko(c, dt), g = u.commands.at(-1)?.command ?? "";
      return g && Ge(g), u;
    }), ht("");
  }, [dt]), za = y(() => {
    if (!Pe || !Ke.trim()) {
    ...
    const c = Ke.trim();
    lt((u) => u && wo(u, c, Je, ut)), mt(""), A(`Captured manual check result: ${c} (${Je})`);
  }, [ut, Ke, Pe, Je]), ja = y(() => {
    Pe && he(yo(Pe));
  }, [he, Pe]), _a = y(() => {
    if (!ae) return;
    const c = x.find((u) => u.id === ae.fileId);
    if (!c) {
      A(`Cannot apply edit — ${ae.fileName} is no longer open`), Me(null);
      return;
    }
    c.content !== ae.originalContent && !window.confirm(`${ae.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (V((u) => u.map((g) => g.id === ae.fileId ? { ...g, content: ae.proposedContent, dirty: !0 } : g)), Te(ae.fileId), ze(`AI edit applied to ${c.name}. Save All to persist it to disk.`), Me(null), He(`AI edit applied to ${c.name}`), A(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Te, x, ae, He]), Da = y(() => {
    if (!ae) return;
    const c = Pt(x, `${Mt(ae.fileName)}-ai-edit`), u = {
      id: `pending-edit-${ae.fileId}-${Date.now()}`,
      name: c,
      path: c,
      language: I?.language ?? "markdown",
      content: ae.proposedContent,
      dirty: !0,
      source: "generated"
    };
    V((g) => [...g, u]), re(u), Me(null), A(`Opened proposed edit as ${c}`);
  }, [I?.language, x, re, ae]), Ha = y(() => {
    if (!ve) return;
    const c = new Map(ve.edits.map((g) => [g.fileId, g]));
    x.some((g) => {
      const w = c.get(g.id);
      return w && g.content !== w.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (V((g) => g.map((w) => {
      const H = c.get(w.id);
      return H ? { ...w, content: H.proposedContent, dirty: !0 } : w;
    })), c.forEach((g, w) => Te(w)), ze(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), Fe(null), He(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), A(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Te, x, ve, He]), Ba = y(() => {
    if (!ve) return;
    const c = [...x], u = ve.edits.map((g) => {
      const w = Pt(c, `${Mt(g.fileName)}-ai-edit`), H = x.find((Be) => Be.id === g.fileId), be = {
        id: `workspace-patch-${g.fileId}-${Date.now()}-${w}`,
        name: w,
        path: w,
        language: H?.language ?? "markdown",
    ...
    V((g) => [...g, ...u]), u[0] && re(u[0]), Fe(null), A(`Opened ${u.length} proposed edit file${u.length === 1 ? "" : "s"}`);
  }, [x, re, ve]), on = y((c) => {
    if (c === "explain") {
      he("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (c === "improve") {
      he("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (c === "plan") {
      he("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (c === "edit") {
      he("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    he("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [he]);
  return Y(() => {
    const c = (u) => {
      (u.metaKey || u.ctrlKey) && (u.key.toLowerCase() === "o" && (u.preventDefault(), De()), u.key.toLowerCase() === "s" && (u.preventDefault(), bt()), u.key.toLowerCase() === "w" && (u.preventDefault(), q && gt(q)), u.key.toLowerCase() === "b" && (u.preventDefault(), m((w) => !w)), u.shiftKey && u.key.toLowerCase() === "f" && (u.preventDefault(), s("search"), m(!0)), (u.key.toLowerCase() === "k" || u.key.toLowerCase() === "p") && (u.preventDefault(), ct(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [q, gt, De, bt]), Y(() => {
    const c = (u) => {
      le.length !== 0 && (u.preventDefault(), u.returnValue = "");
    };
    ...
    const c = t.current;
    if (!c) return;
    const u = () => a(Math.round(c.getBoundingClientRect().width));
    u();
    ...
    if (!n) return;
    const c = St(n);
    b((u) => tt(u, c.primaryMin, c.primaryMax)), J((u) => tt(u, c.secondaryMin, c.secondaryMax));
  }, [n]), Y(() => {
    const c = { primaryVisible: l, primaryWidth: f, secondaryVisible: N, secondaryWidth: _, markdownPreviewVisible: z };
    localStorage.setItem(ra, JSON.stringify(c));
  }, [z, l, f, N, _]), Y(() => {
    localStorage.setItem(ia, JSON.stringify(Ae));
  }, [Ae]), /* @__PURE__ */ d(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${l ? "" : "no-primary"} ${N ? "" : "no-secondary"} ${G ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${f}px`, "--workbench-secondary-width": `${_}px` },
      children: [
        /* @__PURE__ */ r(zo, { active: o, setActive: (c) => {
          s(c), m(!0);
        }, openSettings: Ye, settingsActive: Xe }),
        l && /* @__PURE__ */ d("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ r(
            jo,
            {
              host: e,
              ...
              openFile: De,
              openFolder: ft,
              openWorkbenchFile: re,
              newFile: kt,
              recent: je,
              reopenRecent: yt,
              setStatus: A,
              hasFsAccess: Or(),
              attachSkillToChat: an,
              saveLocalJobOutput: rn,
              activeFile: I
            }
          ),
          /* @__PURE__ */ r("div", { className: "workbench-primary-resizer", onPointerDown: $a, title: "Resize Explorer" })
        ] }),
          ...
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ r(
              Bo,
              {
                ...
                canPreview: I?.language === "markdown",
                previewVisible: z,
                togglePreview: () => Q((c) => !c)
              }
            ),
            /* @__PURE__ */ r(Ho, { file: I, folder: oe, showWelcome: pt }),
            Jt && le.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ r("span", { children: Jt }),
              /* @__PURE__ */ r("button", { onClick: () => {
                nn();
              }, children: "Save all" }),
              /* @__PURE__ */ r("button", { onClick: () => ze(null), title: "Dismiss", children: /* @__PURE__ */ r(ke, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: I ? /* @__PURE__ */ d("div", { className: I.language === "markdown" && z ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ r("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ r(Ua, { fallback: /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ r(
                Mo,
                {
                  file: I,
                  revealLine: ga,
              ...
              I.language === "markdown" && z && /* @__PURE__ */ r(Ko, { content: I.content })
            ] }) : Xe ? /* @__PURE__ */ r(
              ts,
              {
                host: e,
            ...
            ) : pt ? /* @__PURE__ */ r(Wo, { openFile: De, openFolder: ft, newFile: kt, recent: je, reopenRecent: yt, setStatus: A }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ r(it, { size: 34 }),
              /* @__PURE__ */ r("p", { children: "No editor open" }),
              /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => te(!1), children: "Show Welcome" })
            ] }) }),
            G && /* @__PURE__ */ r(
              Jo,
              {
                ...
                runAiSynthesis: wt,
                captureManualCheck: Ct,
                openOutputAsFile: vt,
                manualCheckSession: Pe,
                manualCheckCommandInput: dt,
                setManualCheckCommandInput: ht,
                manualCheckOutputInput: ut,
                setManualCheckOutputInput: mt,
                manualCheckSelectedCommand: Ke,
                setManualCheckSelectedCommand: Ge,
                manualCheckStatus: Je,
                setManualCheckStatus: Xt,
                copyManualCheckCommand: Ra,
                addManualCheckCommand: Fa,
                recordManualCheckResult: za,
        ...
        N && /* @__PURE__ */ r(
          Xo,
          {
            ...
            chatInput: se,
            setChatInput: ce,
            chatMessages: F.messages,
            chatThread: F.thread,
            chatThreads: F.threads,
            askAgent: Ea,
            stopChat: F.stopChat,
            regenerateMessage: Ma,
            newChat: () => {
              F.newChat();
            },
            selectThread: (c) => {
            ...
            saveMessageAsArtifact: Pa,
            rememberMessage: Ta,
            previewEditFromMessage: (c) => Le(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: on,
            pendingPatchPrompt: U,
            generatePatchPrompt: Oa,
            workspaceFileCount: x.length,
            aiStatus: F.aiStatus,
            chatSettings: Ae,
            openSettings: Ye,
            busy: F.busy,
            memoryHitCount: F.memoryHits.length,
            outputs: Ne,
            runAiSynthesis: wt,
            captureManualCheck: Ct,
            openOutputAsFile: vt,
            previewEditFromOutput: (c) => Le(c.title, c.body),
            ...
            contextScope: S,
            setContextScope: E,
            contextAttachments: ya,
            removeContextAttachment: Ca,
            revealContextAttachment: Aa,
            projectIndexSummary: `${Ce.snapshot.files.length} files · ${Ce.snapshot.chunks.length} chunks`,
            projectIndexStale: Ce.isStale,
            refreshProjectIndex: () => {
              const c = Ce.refresh(x);
        ...
        ma && /* @__PURE__ */ r(
          qo,
          {
            ...
            commands: [
              ...
              ...je.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => yt(c) })),
              { label: "File: Save All", detail: `${le.length} dirty file${le.length === 1 ? "" : "s"}`, run: () => {
                nn();
              }, disabled: le.length === 0 },
              { label: "File: Close All Editors", detail: `${de.length} open editor${de.length === 1 ? "" : "s"}`, run: Ia, disabled: de.length === 0 },
              ...
              { label: "View: Toggle Chat Panel", detail: N ? "Hide right AI side bar" : "Show right AI side bar", run: () => T((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: G ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => Z((c) => !c) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Ye },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => He("Manual check requested from command palette") },
              { label: "View: Toggle Markdown Preview", detail: I?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => Q((c) => !c), disabled: I?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: I || de.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: wt, disabled: !I && de.length === 0 },
              { label: "AI: Explain Active File", detail: I ? `Ask Cortex to explain ${I.path}` : "Open a file first", run: () => he("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !I },
              { label: "AI: Improve Active File", detail: I ? `Ask Cortex for concrete edits to ${I.path}` : "Open a file first", run: () => he("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !I },
              { label: "AI: Draft Editable Replacement", detail: I ? `Ask Cortex for a full-file replacement for ${I.path}` : "Open a file first", run: () => on("edit"), disabled: !I },
              { label: "AI: Plan Workspace Work", detail: de.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => he("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: de.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: I ? "Persist active file in host.ai artifacts" : "Open a file first", run: La, disabled: !I },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: Ct },
              { label: "AI: Open Latest Artifact as File", detail: Ne[0] ? `Create editable file from ${Ne[0].title}` : "No outputs yet", run: () => Ne[0] && vt(Ne[0]), disabled: Ne.length === 0 }
            ],
            openWorkbenchFile: re,
            onClose: () => ct(!1)
          }
        ...
        /* @__PURE__ */ r(cs, { status: ka, file: I ?? Po, cursor: $, fileCount: x.length, dirtyCount: le.length })
      ]
    }
  );
}
function zo({ active: e, setActive: t, openSettings: n, settingsActive: a }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(Pn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(xr, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(Ln, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(In, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(Ft, { size: 25 }), label: "Computer / Agents", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ r("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(nr, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ r(Se, { icon: /* @__PURE__ */ r(Sr, { size: 23 }), label: "Settings", active: a, onClick: n })
  ] });
}
function Se({ icon: e, label: t, active: n, onClick: a }) {
  return /* @__PURE__ */ r("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: a, children: e });
}
function jo(e) {
  return e.activity === "search" ? /* @__PURE__ */ r(Uo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ r(An, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ r(An, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ r(ua, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ r(_o, { ...e });
}
function _o(e) {
  const t = !e.folder, n = ie(() => Do(e.files, e.folder?.name), [e.files, e.folder?.name]), [a, i] = v(() => /* @__PURE__ */ new Set());
  Y(() => {
    const s = sa(n);
    i((l) => {
      const m = new Set(l);
      return s.forEach((f) => m.add(f)), m;
    });
  }, [n]);
  const o = y((s) => {
    i((l) => {
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(Ve, { children: [
        ...
        /* @__PURE__ */ r("input", { className: "workbench-input", value: e.query, onChange: (s) => e.setQuery(s.target.value), placeholder: "Search files" }),
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(rt, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((s) => /* @__PURE__ */ r(Vt, { file: s, active: s.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(s), label: s.name, detail: s.path }, s.id)),
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ r(rt, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No readable text files found." }) : ca(n, e.activeFileId, e.openWorkbenchFile, a, o)
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((s) => /* @__PURE__ */ d("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(s), children: [
        /* @__PURE__ */ r(Tn, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: s.name })
      ] }, `${s.path}-${s.at}`))
    ] })
  ] });
}
function Vt({ file: e, active: t, onOpen: n, basePath: a, depth: i = 0, label: o, detail: s }) {
  const l = a && e.path.startsWith(`${a}/`) ? e.path.slice(a.length + 1) : e.path, m = i || Math.max(0, l.split("/").length - 1);
  return /* @__PURE__ */ d("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": m }, onClick: n, title: e.path, children: [
    /* @__PURE__ */ r(Mn, { size: 14 }),
    /* @__PURE__ */ d("span", { className: "workbench-row-text", children: [
      /* @__PURE__ */ r("span", { className: "workbench-row-name", children: o ?? l }),
      s ? /* @__PURE__ */ r("span", { className: "workbench-row-detail", children: s }) : null
    ] }),
...
function sa(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...sa(t.children)]);
}
function ca(e, t, n, a, i, o = 0) {
  return e.map((s) => {
    if (s.file)
      return /* @__PURE__ */ r(Vt, { file: s.file, active: s.file.id === t, onOpen: () => n(s.file), depth: o, label: s.name }, s.file.id);
    const l = a.has(s.path);
    return /* @__PURE__ */ d("div", { children: [
      /* @__PURE__ */ d("button", { className: "workbench-folder-row", style: { "--workbench-depth": o }, onClick: () => i(s.path), title: l ? `Collapse ${s.name}` : `Expand ${s.name}`, children: [
        l ? /* @__PURE__ */ r(rt, { size: 12 }) : /* @__PURE__ */ r(rt, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ r(_t, { size: 14 }) : /* @__PURE__ */ r(Tn, { size: 14 }),
        /* @__PURE__ */ r("span", { className: "workbench-row-name", children: s.name }),
        /* @__PURE__ */ r("span", { className: "workbench-row-meta", children: s.children.length })
      ] }),
      l ? ca(s.children, t, n, a, i, o + 1) : null
    ] }, s.path);
  });
}
function Ho({ file: e, folder: t, showWelcome: n }) {
  const a = n ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], i = t && a[0] === t.name ? a.slice(1) : a;
  return /* @__PURE__ */ r("div", { className: "workbench-breadcrumb", children: i.length === 0 ? /* @__PURE__ */ r("span", { children: "Workspace" }) : i.map((o, s) => /* @__PURE__ */ d("span", { className: "workbench-breadcrumb-part", children: [
...
function Bo(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ d("button", { className: "workbench-tab active", children: [
      ...
      }, children: /* @__PURE__ */ r(ke, { size: 13 }) })
    ] }),
    ...
    e.canPreview && /* @__PURE__ */ r("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ r(zt, { size: 16 }) }),
    /* @__PURE__ */ r("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ r(gr, { size: 16 }) })
  ] });
}
function Wo({ openFile: e, openFolder: t, newFile: n, recent: a, reopenRecent: i, setStatus: o }) {
  return /* @__PURE__ */ d("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ d("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ d("section", { children: [
        ...
        /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: e, children: [
          /* @__PURE__ */ r(Pn, { size: 18 }),
          "Open File..."
        ] }),
        /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: t, children: [
          /* @__PURE__ */ r(_t, { size: 18 }),
          "Open Folder..."
        ] }),
        /* @__PURE__ */ r("h2", { children: "Recent" }),
        a.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : a.map((s) => /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: () => i(s), children: [
          s.name,
      ...
      /* @__PURE__ */ d("section", { children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-walkthrough-card", children: [
    ...
    /* @__PURE__ */ d("label", { className: "workbench-welcome-checkbox", children: [
      /* @__PURE__ */ r("input", { type: "checkbox", defaultChecked: !0 }),
      " Show welcome page on startup"
    ] })
  ] });
}
function Uo({ files: e, query: t, setQuery: n, openWorkbenchFile: a, activeFileId: i }) {
  const o = ie(() => Vo(e, t), [e, t]), s = ie(() => {
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? s.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No matches." }) : s.map((l) => {
        ...
        return /* @__PURE__ */ d("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ r(Vt, { file: m, active: m.id === i, onOpen: () => a(m) }),
          l.slice(0, 5).map((f) => /* @__PURE__ */ d("button", { className: "workbench-search-hit", onClick: () => a(m, f.lineNumber), title: f.line, children: [
...
function qo(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${Rn(i.language)} file`,
  ...
  return Y(() => {
    ...
    /* @__PURE__ */ r("div", { className: "workbench-command-list", children: a.map((i) => /* @__PURE__ */ d(
      "button",
      {
        disabled: i.disabled,
...
function Go(e) {
  const t = e.session ? vo(e.session) : "pending";
  return /* @__PURE__ */ d("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ d(Ve, { children: [
...
function Jo(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ d("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ r("span", {}),
      /* @__PURE__ */ r("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ r(ke, { size: 14 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ r(ha, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Xo(e) {
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ r("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ r(yr, { size: 15 }) }),
        /* @__PURE__ */ r("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ r(ir, { size: 16 }) }),
        /* @__PURE__ */ r("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ r(ke, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ r(Yo, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ r(
      ua,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ r(ha, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function la({ body: e }) {
  const t = ie(() => Lo(e), [e]), [n, a] = v(null), i = y((o, s) => {
    (async () => await st(s) && (a(o), window.setTimeout(() => a((m) => m === o ? null : m), 1200)))();
  }, []);
  return /* @__PURE__ */ r("div", { className: "workbench-rich-body", children: t.map((o) => {
    if (o.type === "code") {
      ...
      return /* @__PURE__ */ d("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ d("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ d("button", { onClick: () => i(o.key, o.body), title: "Copy code block", children: [
            s ? /* @__PURE__ */ r(Za, { size: 12 }) : /* @__PURE__ */ r(Lt, { size: 12 }),
            s ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ r(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Hn(o.body) }
      },
      o.key
    );
  }) });
...
function Yo(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Re(null), [a, i] = v(!0), [o, s] = v(!1), l = ie(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  Y(() => {
    const h = n.current;
    h && (a ? (h.scrollTop = h.scrollHeight, s(!1)) : s(!0));
  }, [a, l]);
  const m = y(() => {
    const h = n.current;
    if (!h) return;
    const N = h.scrollHeight - h.scrollTop - h.clientHeight < 48;
    i(N), N && s(!1);
  }, []), f = y(() => {
    const h = n.current;
    h && (h.scrollTop = h.scrollHeight, i(!0), s(!1));
  }, []), b = y((h) => {
    st(h.body);
  }, []);
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ d("option", { value: h.id, children: [
              h.title,
              " · ",
              Qo(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ d("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ r("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ d("div", { children: [
        ...
        /* @__PURE__ */ r("strong", { children: h.role === "user" ? "You" : "Atomek" }),
        h.status === "streaming" ? /* @__PURE__ */ r("em", { children: " streaming" }) : null,
        h.status === "error" ? /* @__PURE__ */ r("em", { children: " error" }) : null,
        /* @__PURE__ */ r("br", {}),
        /* @__PURE__ */ r(la, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ d(Ve, { children: [
          /* @__PURE__ */ r("br", {}),
          /* @__PURE__ */ r("small", { children: h.gatewayLabel })
        ] }) : null,
    ...
    /* @__PURE__ */ d("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ d("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ r("strong", { children: Jr(e.contextScope) }),
        /* @__PURE__ */ r("em", { children: Cn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ d(Ve, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ r(cn, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((h) => {
            const k = typeof h.score == "number" ? h.score.toFixed(2) : null, N = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, T = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, _ = [
              h.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: _, children: [
              /* @__PURE__ */ r("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ r(cn, { size: 13 }) }),
              h.label,
              k ? /* @__PURE__ */ r("small", { children: k }) : null,
              h.snippet ? /* @__PURE__ */ d("small", { children: [
                h.snippet.slice(0, 60),
                h.snippet.length > 60 ? "…" : ""
              ] }) : null,
              h.dirty ? /* @__PURE__ */ r("small", { children: "dirty" }) : null,
              h.removable ? /* @__PURE__ */ r("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(h), title: "Remove context", children: /* @__PURE__ */ r(ke, { size: 11 }) }) : null
            ] }, h.id);
          }),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ r("span", { className: "workbench-chat-route-summary", children: Cn(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ r("span", {}),
          e.busy ? /* @__PURE__ */ r("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ r(Pr, { size: 14 }) }) : /* @__PURE__ */ r("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ r(Ar, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Cn(e, t, n) {
  const a = e.model.trim(), o = [e.gatewayPreference === "auto" ? t : da(e.gatewayPreference)];
...
function ts(e) {
  ...
  Y(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(T.map((_) => ({ id: _.id, gatewayLabel: _.gatewayLabel }))), i(T.length > 0 ? `${T.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (T) {
        if (k.signal.aborted) return;
        n([]), i(`Model discovery failed: ${T instanceof Error ? T.message : String(T)}`);
      }
    })(), () => k.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), Y(() => {
    const k = new AbortController();
    return (async () => {
      const T = po(e.host);
      if (T) {
      ...
      try {
        const _ = await mo(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: k.signal
        });
        if (k.signal.aborted) return;
        s(_.map((J) => ({ id: J.id, gatewayLabel: J.gatewayLabel ?? J.source ?? "AIL" }))), m(_.length > 0 ? `${_.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (_) {
        if (k.signal.aborted) return;
        s([]), m(`Embedding model discovery failed: ${_ instanceof Error ? _.message : String(_)}`);
      }
    })(), () => k.abort();
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ d("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ d("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ r("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ r(ke, { size: 15 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          da(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ r("datalist", { id: "atomek-embedding-models", children: o.map((k) => /* @__PURE__ */ r("option", { value: k.id, children: k.gatewayLabel }, `${k.gatewayLabel}:${k.id}`)) })
        ] }),
    ...
    /* @__PURE__ */ d("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ r("button", { onClick: () => e.onChange(at), children: "Reset" }),
      /* @__PURE__ */ r("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function ha({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: i, openOutputAsFile: o, previewEditFromOutput: s, canPreviewEdit: l = !1, compact: m = !1 }) {
  return /* @__PURE__ */ d("div", { className: `workbench-panel-list ${m ? "compact" : ""}`, children: [
    /* @__PURE__ */ d("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: a, children: [
        /* @__PURE__ */ r(Ft, { size: 14 }),
        "AI synthesis"
      ] }),
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: i, children: [
        /* @__PURE__ */ r(In, { size: 14 }),
        "Capture check"
      ] }),
      /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ d("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ d("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ r("button", { onClick: () => void st(f.body), children: "Copy" }),
        /* @__PURE__ */ r("button", { onClick: () => o(f), children: "Open as file" }),
        s ? /* @__PURE__ */ r(
          "button",
          {
            className: xn(f.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => s(f),
            disabled: !l,
            children: xn(f.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        f.source === "ai" ? /* @__PURE__ */ r("button", { onClick: () => n(f.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ r(la, { body: f.body })
    ] }, f.id))
  ] });
}
function ns({ edit: e, onApply: t, onOpenAsFile: n, onClose: a }) {
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ d("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ r("button", { title: "Close", onClick: a, children: /* @__PURE__ */ r(ke, { size: 16 }) })
    ] }),
...
function as({ patch: e, onApply: t, onOpenAsFiles: n, onClose: a }) {
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((o) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ r("pre", { children: hs(o.proposedContent) })
      ] }, o.fileId)),
...
function ss(e, t) {
  return t === e.id ? `${e.label} running…` : "Run in panel";
}
function ua({
  host: e,
  setStatus: t,
  ...
  const [l, m] = v([]), [f, b] = v([]), [h, k] = v(null), [N, T] = v(!1), [_, J] = v(null), [G, Z] = v("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [ee, M] = v(null), [z, Q] = v([]), [R, te] = v(null), [oe, ye] = v([]), x = z.find((p) => p.status === "running") ?? z[0] ?? null, V = s === "dock", ue = o.filter((p) => p.dirty).length, fe = i ? `${i.path} · ${i.language} · ${i.content.length.toLocaleString()} chars${i.dirty ? " · dirty" : ""}` : o.length > 0 ? `${o.length} open editor${o.length === 1 ? "" : "s"}${ue ? ` · ${ue} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", q = ie(() => [
    {
    ...
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      m([]), b([]), k(null), J("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    T(!0), J(null);
    try {
      const [p, L, P] = await Promise.all([
      ...
      m(p), b(L), k(P), t(`Computer loaded · ${p.length} tools · ${L.length} skills${P ? ` · ${P.resources.length} resources` : ""}`);
    } catch (p) {
      J(p instanceof Error ? p.message : String(p));
    } finally {
      T(!1);
    }
  }, [e.local, e.resources, e.skills, t]);
  Y(() => {
    ne();
  }, [ne]);
  const $e = y(async (p) => {
    ...
    try {
      await e.local.openTerminal({
        ...
        command: p.command,
        cwd: $n(i),
        prompt: `Opened from Atomek. ${fe}`
      }), t(`Opened ${p.label} interactively in Tytus Terminal`);
    } catch (L) {
      t(`Terminal launch failed: ${L instanceof Error ? L.message : String(L)}`);
    }
  }, [i, fe, e.local, t]), pe = y(async (p, L, P = []) => {
    const S = [
      ...oe,
      ...P,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], E = [
      { path: "MISSION.md", content: yn(p, h, i, o, L) },
      { path: "MISSION.json", content: Ro(p, h, L) },
      { path: "RESOURCES.md", content: vn(h) },
      { path: "AUDIT.jsonl", content: S.map((D) => JSON.stringify(D)).join(`
`) + `
` }
    ];
    if (p.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: p.rootPath, files: E });
    else if (p.handle) {
      await wn(p.handle, "runs");
      for (const D of E) await kn(p.handle, D.path, D.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    ye(S);
  }, [i, e.missions, oe, o, h]), $ = y(async () => {
    try {
      const p = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, L = G.trim() || "Coordinate Tytus resources for the current Atomek task.";
      let P;
      if (e.missions?.create) {
        const E = await e.missions.create({ title: p, goal: L });
        P = {
          missionId: E.missionId,
          title: E.title,
          goal: E.goal,
        ...
        const E = await Eo();
        if (!E) {
          t("Mission folder picker unavailable in this browser context");
          return;
        }
        P = {
          handle: E,
          name: E.name,
          missionId: `mission-${Date.now()}-${Oo(E.name)}`,
          title: p,
          goal: L,
          source: "browser"
        };
      ...
      te(P);
      const S = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.selected", message: `Mission folder selected: ${P.rootPath ?? P.name}` };
      ye([S]), await pe(P, G, [S]), t(`Mission pack ready in ${P.rootPath ?? P.name}`);
    } catch (p) {
      t(`Mission folder setup failed: ${p instanceof Error ? p.message : String(p)}`);
    }
  }, [e.missions, G, t, pe]), C = y(async (p, L, P) => {
    if (!R) return;
    const E = `${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${p.id}.md`, D = `runs/${E}`;
    if (R.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: R.rootPath, files: [{ path: D, content: L }] });
    else if (R.handle) {
      const K = await wn(R.handle, "runs");
      await kn(K, E, L);
    }
    await pe(R, G, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${p.label} exited ${P}; transcript saved to ${D}`,
      data: { toolId: p.id, exitCode: P, transcript: D }
    }]);
  }, [e.missions, G, R, pe]), O = y(async (p) => {
    ...
    M(p.id), R && await pe(R, L, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.start",
      message: `${p.label} local job started`,
      data: { toolId: p.id }
    }]);
    const P = `local-run-${Date.now()}-${p.id}`;
    Q((S) => [{
      id: P,
      toolId: p.id,
      label: p.label,
      status: "running",
      startedAt: Date.now(),
      lines: [`[Atomek] Starting ${p.label} local job…`]
    }, ...S].slice(0, 6));
    try {
      const S = await e.local.runJob({
        toolId: p.id,
        prompt: R ? [
          "Tytus mission context pack is active.",
          `Mission: ${R.title}`,
          `Goal: ${R.goal}`,
          R.rootPath ? `Mission folder: ${R.rootPath}` : `Mission folder: ${R.name}`,
...
`)
      }), E = [`[Atomek] Started ${p.label} local job ${S.id}`], D = (K) => {
        Q((B) => B.map((X) => X.id === P ? K(X) : X));
      };
      e.local.streamJob(S.id, {
        onLog: (K) => {
          E.push(K), D((B) => ({ ...B, lines: E.slice(-500) }));
        },
        onDone: (K) => {
          K && (E.push(K), D((B) => ({ ...B, lines: E.slice(-500) })));
        },
        onFail: (K) => {
          E.push(`[FAIL] ${K}`), D((B) => ({
            ...B,
            ...
            finishedAt: Date.now(),
            lines: E.slice(-500)
          })), a(`${p.label} local job failed`, E.join(`
`)), C(p, E.join(`
`), -1).catch((B) => {
            t(`Mission transcript save failed: ${B instanceof Error ? B.message : String(B)}`);
          }), M(null);
        },
        onExit: (K) => {
          const B = [
...
`);
          D((X) => ({
            ...
            finishedAt: Date.now(),
            lines: E.slice(-500)
          })), a(`${p.label} local job`, B), C(p, B, K).catch((X) => {
            t(`Mission transcript save failed: ${X instanceof Error ? X.message : String(X)}`);
          }), M(null);
        },
        onError: () => t(`Local job stream issue for ${p.label}`)
      }), t(`Started ${p.label} local job`);
    } catch (S) {
      M(null), Q((E) => E.map((D) => D.id === P ? {
        ...D,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...D.lines, `[Atomek] Failed to start: ${S instanceof Error ? S.message : String(S)}`]
      } : D)), t(`Local job failed to start: ${S instanceof Error ? S.message : String(S)}`);
    }
  }, [i, e.local, G, R, o, h, a, C, t, pe]);
  return /* @__PURE__ */ d("aside", { className: V ? "workbench-agent-dock" : "workbench-sidebar", children: [
    V ? null : /* @__PURE__ */ r("div", { className: "workbench-sidebar-title", children: "COMPUTER / AGENTS" }),
    /* @__PURE__ */ d("div", { className: V ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ d("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ r(Ft, { size: 18 }),
        /* @__PURE__ */ d("div", { children: [
          /* @__PURE__ */ r("strong", { children: V ? "Agent Manager" : "Computer cockpit" }),
          /* @__PURE__ */ r("p", { className: "workbench-muted", children: "Run Claude, pi, OpenCode, Codex, Gemini, Qwen, Kimi, Aider, or Terminal from Atomek with the active file context. Same-origin host bridge only. No raw browser shell. No hardcoded models." })
        ] })
      ] }),
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ...
        " ",
        N ? "Refreshing…" : "Refresh capabilities"
      ] }),
      _ && /* @__PURE__ */ r("div", { className: "workbench-inline-error", children: _ }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "MISSION PACK" }),
      /* @__PURE__ */ d("div", { className: "workbench-computer-context-card mission", children: [
        /* @__PURE__ */ r("strong", { children: R ? R.title : "No mission folder selected" }),
        /* @__PURE__ */ r("span", { children: R ? `${R.rootPath ?? R.name} · ${R.source} · ${oe.length} audit events · transcripts saved under runs/` : "Create a durable context pack before dispatching agents that need shared handoff state." }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
        /* @__PURE__ */ r("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
          $();
        }, children: R ? "Refresh mission pack" : "Create/select mission folder" }),
        /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => R && void pe(R, G), disabled: !R, children: "Write pack now" })
      ] }),
      h?.warnings.length ? /* @__PURE__ */ r("div", { className: "workbench-resource-warnings", children: h.warnings.slice(0, 3).map((p) => /* @__PURE__ */ d("span", { children: [
        p.code,
        ": ",
        p.message
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-context-card", children: [
        /* @__PURE__ */ r("strong", { children: fe }),
        /* @__PURE__ */ r("span", { children: i ? "Local agents receive clipped active-file content and must return previewable edits." : "Open a file to give local agents useful context." })
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "TASK FOR LOCAL AGENT" }),
      /* @__PURE__ */ r("div", { className: "workbench-computer-presets", children: q.map((p) => /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => Z(p.prompt), children: p.label }, p.label)) }),
      /* @__PURE__ */ r(
        "textarea",
        {
      ...
      x ? /* @__PURE__ */ d("div", { className: "workbench-agent-run", children: [
        /* @__PURE__ */ d("header", { children: [
          /* @__PURE__ */ d("div", { children: [
            /* @__PURE__ */ r("strong", { children: x.label }),
            /* @__PURE__ */ d("span", { children: [
              x.status,
              typeof x.exitCode == "number" ? ` · exit ${x.exitCode}` : ""
            ] })
          ] }),
          /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => a(`${x.label} local job`, x.lines.join(`
`)), disabled: x.lines.length === 0, children: "Save output" })
        ] }),
        /* @__PURE__ */ r("pre", { className: "workbench-computer-job-log", children: x.lines.join(`
`) || "[waiting for output]" })
      ] }) : null,
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-list compact", children: [
        !h && !N ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
        h?.resources.slice(0, V ? 8 : 5).map((p) => /* @__PURE__ */ d("div", { className: "workbench-resource-row", children: [
          /* @__PURE__ */ d("div", { children: [
            /* @__PURE__ */ r("strong", { children: p.label }),
            /* @__PURE__ */ d("span", { children: [
              p.kind,
              " · ",
              p.trustTier,
          ...
          /* @__PURE__ */ r("span", { className: `workbench-computer-pill ${p.status}`, children: p.status })
        ] }, p.id))
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "LOCAL AGENTS & TOOLS" }),
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        l.length === 0 && !N ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        l.map((p) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          ...
          p.description ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: p.description }) : null,
          /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
            p.kind === "ai-cli" ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
              O(p);
            }, disabled: p.status !== "available" || ee !== null, title: "Run this local agent inside Atomek and stream output here. It cannot write files directly.", children: ss(p, ee) }) : null,
            /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
              $e(p);
            }, disabled: p.status !== "available", title: "Launch this tool in the real Tytus terminal with current Atomek context prefilled.", children: os(p) })
          ] })
        ] }, p.id))
      ] }),
      /* @__PURE__ */ r("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        f.length === 0 && !N ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
        f.map((p) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ d("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ d("div", { children: [
          ...
          /* @__PURE__ */ r("p", { className: "workbench-muted", children: p.description }),
          p.triggers?.length ? /* @__PURE__ */ r("div", { className: "workbench-computer-triggers", children: p.triggers.slice(0, 4).map((L) => /* @__PURE__ */ r("span", { children: L }, L)) }) : null,
          /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
            n(p);
          }, disabled: p.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
        ] }, p.id))
      ] })
    ] })
  ] });
}
function An({ title: e, body: t }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ r("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ r("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function cs({ status: e, file: t, cursor: n, fileCount: a, dirtyCount: i }) {
  return /* @__PURE__ */ d("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ r("span", { children: Rn(t.language) })
  ] });
}
function It(e, t) {
...
function ps() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : at.gatewayPreference,
...
function fs({ host: e }) {
  return /* @__PURE__ */ r(Fo, { host: e });
}
function ks(e) {
  return function() {
    return /* @__PURE__ */ r(fs, { host: e.host });
  };
}
export {
  ks as default
