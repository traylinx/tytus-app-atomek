...
import { jsxs as l, jsx as a, Fragment as He } from "react/jsx-runtime";
import { forwardRef as Ln, createElement as Ft, useState as C, useRef as Be, useCallback as y, useEffect as ne, lazy as Ya, useMemo as le, Suspense as Qa } from "react";
/**
 ...
 */
const Za = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), eo = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, o) => o ? o.toUpperCase() : n.toLowerCase()
), fn = (e) => {
  const t = eo(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, On = (...e) => e.filter((t, n, o) => !!t && t.trim() !== "" && o.indexOf(t) === n).join(" ").trim(), to = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const ro = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], so = V("check", ro);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const co = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], ut = V("chevron-down", co);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
async function Wo() {
  ...
  if (!e) return qn(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((o) => Un(o, o.name, "local-file")));
  return Xt(n.filter(Boolean));
}
async function Uo() {
  const e = Gt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await qn(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await Vn(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Xt(n) };
}
function Gt() {
  const e = window;
...
async function Vn(e, t, n, o, i) {
  if (!(n.length >= i || o > 8))
    for await (const r of e.values()) {
      if (n.length >= i) break;
      if (r.kind === "directory") {
        if (Ho.has(r.name)) continue;
        await Vn(r, `${t}/${r.name}`, n, o + 1, i);
        continue;
      }
      const s = r, d = await Un(s, `${t}/${s.name}`, "local-folder");
      d && n.push(d);
    }
}
function qn(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const o = Array.from(n.files ?? []), i = await Promise.all(o.map(async (r) => {
      ...
      n.remove(), t(Xt(i.filter(Boolean)));
...
`).map(
    ...
    const r = o.split("|").filter(Boolean).map((d) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${d.trim()}</th>`).join(""), s = i.trim().split(`
...
const ze = "atomek:default", qo = 3e3, Yt = "tytus.atomek.threadTitleOverrides", gn = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, Xe = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, Jo = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", wn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Jo(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = Qt();
    n[e] = t, localStorage.setItem(Yt, JSON.stringify(n));
  } catch {
  }
}, Yo = (e) => {
  try {
    const t = Qt();
    delete t[e], localStorage.setItem(Yt, JSON.stringify(t));
  } catch {
  }
}, yn = (e) => {
  const t = Qt();
...
function Qo({ host: e, requestContext: t, chatSettings: n, setStatus: o }) {
  const i = e.ai, [r, s] = C(null), [d, p] = C([]), [f, b] = C([]), [u, w] = C([]), [S, T] = C([]), [U, G] = C(gn), [q, ee] = C(!1), ae = Be(!0), P = Be(null), B = y(async () => {
    if (!i) {
      G(gn);
      return;
    }
    ...
    if (i)
      try {
        const x = await i.listArtifacts({ threadId: N });
        if (!ae.current) return;
        w(x.map(wn));
      } catch (x) {
        o(`AI artifacts unavailable: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [i, o]), H = y(async (N) => {
    if (i)
      try {
        const x = await i.listMessages(N);
        if (!ae.current) return;
        const j = d.find((D) => D.id === N) ?? null;
        j && s(j), b(x.map(Xe).filter(Boolean)), T([]), await Q(N);
      } catch (x) {
        o(`Load chat failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [i, Q, o, d]), te = y(async () => {
    if (i)
      try {
        const N = yn(await i.listThreads({ workspaceKey: ze, status: "active" })), x = N[0] ?? await i.createThread({ workspaceKey: ze, title: "Atomek chat" });
        if (!ae.current) return;
        p(N[0] ? N : [x]), s(x);
        const j = await i.listMessages(x.id);
        if (!ae.current) return;
        b(j.map(Xe).filter(Boolean)), await Q(x.id);
      } catch (N) {
        o(`AI chat unavailable: ${N instanceof Error ? N.message : String(N)}`);
      }
  }, [i, Q, o]);
  ne(() => (ae.current = !0, B(), te(), () => {
    ae.current = !1;
  }), [te, B]);
  const ce = y(async () => {
    if (!i) return null;
    if (r) return r;
    const N = await i.createThread({ workspaceKey: ze, title: "Atomek chat" });
    return ae.current && (s(N), p((x) => [N, ...x.filter((j) => j.id !== N.id)])), N;
  }, [i, r]), xe = y(async () => {
    if (!i) return;
    const N = await i.createThread({ workspaceKey: ze, title: "Atomek chat" });
    s(N), p((x) => [N, ...x.filter((j) => j.id !== N.id)]), b([]), w([]), T([]), o("New AI chat created");
  }, [i, o]), g = y(async (N, x) => {
    if (!i) return;
    const j = x.trim();
    if (j)
      try {
        const D = i.updateThread, ie = d.find((Z) => Z.id === N) ?? r, F = typeof D == "function" ? await D({ threadId: N, title: j }) : { ...ie ?? await ce(), id: N, title: j, updatedAt: Date.now() };
        typeof D != "function" && Xo(N, j), p((Z) => Z.map((Ne) => Ne.id === F.id ? F : Ne)), s((Z) => Z?.id === F.id ? F : Z), o(typeof D == "function" ? `Renamed chat: ${F.title}` : `Renamed chat locally: ${F.title}`);
      } catch (D) {
        o(`Rename chat failed: ${D instanceof Error ? D.message : String(D)}`);
      }
  }, [i, ce, o, r, d]), O = y(async (N) => {
    if (i)
      try {
        await i.deleteThread(N), Yo(N);
        const x = d.filter((j) => j.id !== N);
        if (p(x), r?.id === N) {
          const j = x[0] ?? await i.createThread({ workspaceKey: ze, title: "Atomek chat" });
          s(j), p((ie) => ie.some((F) => F.id === j.id) ? ie : [j, ...ie]);
          const D = await i.listMessages(j.id);
          b(D.map(Xe).filter(Boolean)), await Q(j.id);
        }
        o("Deleted AI chat");
      } catch (x) {
        o(`Delete chat failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [i, Q, o, r?.id, d]), he = y(async (N) => {
    if (!i) return null;
    try {
      const x = await ce();
      if (!x) return null;
      const j = await i.createArtifact({
        threadId: x.id,
        messageId: N.messageId ?? null,
        title: N.title?.trim() || kn(N.body, "Atomek artifact"),
        kind: N.kind ?? "markdown",
        body: N.body
      }), D = wn(j);
      return w((ie) => [D, ...ie.filter((F) => F.id !== D.id)]), o(`Saved AI artifact: ${D.title}`), D;
    } catch (x) {
      return o(`Save artifact failed: ${x instanceof Error ? x.message : String(x)}`), null;
    }
  }, [i, ce, o]), ke = y(async (N) => {
    if (i)
      try {
        await i.deleteArtifact(N), w((x) => x.filter((j) => j.id !== N)), o("Deleted AI artifact");
      } catch (x) {
        o(`Delete artifact failed: ${x instanceof Error ? x.message : String(x)}`);
      }
  }, [i, o]), R = y(async (N) => {
    if (!i) return null;
    try {
      const x = await i.writeMemory({
        title: N.title?.trim() || kn(N.body, "Atomek memory"),
        body: N.body,
        metadata: {
          source: "atomek",
          messageId: N.messageId ?? null
      ...
      return T((j) => [x, ...j.filter((D) => D.id !== x.id)].slice(0, 5)), o(`Remembered: ${x.title}`), x;
    } catch (x) {
      return o(`Remember failed: ${x instanceof Error ? x.message : String(x)}`), null;
    }
  }, [i, o]), oe = y(async (N) => {
    if (!i) return [];
    const x = await i.searchMemory({ query: N, limit: 5 });
    return ae.current && T(x), x;
  }, [i]), we = y(async (N, x = {}) => {
    const j = N.trim();
    if (!j || !i) return null;
    ee(!0);
    ...
    try {
      const F = await ce();
      if (!F) return null;
      const Z = await oe(j).catch(() => []), Ne = Go(Z), Te = x.requestContext ?? t, Ie = Ne ? [...Te, Ne] : [...Te];
      let Ae = null;
      for await (const X of i.sendMessage({
        threadId: F.id,
        body: j,
        ...
        if (X.type === "message_created") {
          const J = Xe(X.message);
          if (!J) continue;
          J.role === "assistant" && (Ae = J.id), b((h) => [...h.filter((v) => v.id !== J.id), J]);
        }
        if (X.type === "token" && (Ae = X.messageId, b((J) => J.some((h) => h.id === X.messageId) ? J.map(
          (h) => h.id === X.messageId ? { ...h, body: X.body, status: "streaming" } : h
        ) : [...J, {
          id: X.messageId,
          role: "assistant",
          body: X.body,
          status: "streaming",
          createdAt: Date.now()
        }])), X.type === "message_updated" || X.type === "done") {
          const J = Xe(X.message);
          if (!J) continue;
          J.role === "assistant" && (D = J), b((h) => h.map((v) => v.id === J.id ? J : v)), J.gatewayLabel && o(`AI answered via ${J.gatewayLabel}`);
        }
        if (X.type === "run_failed") {
          const J = ie.signal.aborted;
          o(J ? "AI response stopped" : `AI failed: ${X.error}`), Ae && b((h) => h.map((v) => v.id !== Ae ? v : J ? { ...v, status: "complete", error: void 0, body: v.body || "Stopped by user." } : { ...v, status: "error", error: X.error, body: X.error }));
        }
      }
      const ye = yn(await i.listThreads({ workspaceKey: ze, status: "active" }).catch(() => []));
      return ae.current && ye.length > 0 && p(ye), B(), D;
    } catch (F) {
      return F instanceof DOMException && F.name === "AbortError" ? (o("AI response stopped"), D) : (o(`AI failed: ${F instanceof Error ? F.message : String(F)}`), null);
    } finally {
      P.current === ie && (P.current = null), ee(!1);
    }
  }, [i, n.gatewayPreference, n.model, ce, oe, B, t, o]), re = y(() => {
...
const Zo = (e) => `tytus-workbench:///${encodeURI(e.path)}`, ei = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, ti = ({ files: e, openEditorIds: t, activeFileId: n, versions: o, activeSelection: i }) => {
  const r = new Set(t), s = e.map((d) => {
    const p = d.id === n;
    return {
      id: d.id,
      uri: Zo(d),
      ...
      language: d.language,
      version: o[d.id] ?? 1,
      contentHash: ei(d.content),
      dirty: d.dirty,
      source: d.source,
      selection: p && i ? i : void 0,
      updatedAt: Date.now(),
      open: r.has(d.id),
      active: p
    };
...
}, ni = "active-file", ai = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", oi = (e, t = "file") => `${t}:${e.id}`, ii = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let i = [];
  return t.scope === "active-selection" ? o?.selection ? i = [ri(o, o.selection, !0)] : o && (i = [ct(o, !0, "Active file")]) : t.scope === "active-file" ? o && (i = [ct(o, !0, "Active file")]) : t.scope === "open-editors" ? i = e.openDocumentIds.map((r) => e.byId.get(r)).filter(Boolean).map((r) => ct(r, !0, "Open editor")) : t.scope === "selected-files" && (i = t.selectedFileIds.map((r) => e.byId.get(r)).filter(Boolean).map((r) => ct(r, !1, "Selected file"))), ci(i).filter((r) => !n.has(r.id));
}, ci = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, li = 4e3, di = 8e3, hi = 4e3, ui = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, mi = (e, t, n) => {
  const o = si(e, n), i = [], r = new Map(t.map((s) => [s.id, s]));
  for (const s of o) {
    if (!s.fileId) continue;
    const d = r.get(s.fileId), p = e.byId.get(s.fileId);
...
`), b = s.kind === "selection" && s.range ? pi(d.content, s.range) : d.content, u = s.kind === "selection" ? li : s.label.startsWith("Open editor") ? hi : di;
    i.push({
...
`)).join(`
`)
  }), { parts: i, attachments: o };
}, pi = (e, t) => {
  const n = e.split(`
`), o = Math.max(1, t.startLineNumber), i = Math.max(o, t.endLineNumber), r = n.slice(o - 1, i);
...
`));
}, vn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, Cn = 75e4, fi = 2800, bi = 240, gi = 80, ki = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Re(t.content)}`).sort().join("")
), zt = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? Cn) || e.content.length > (t.maxFileBytes ?? Cn) ? "huge" : !t.includeDirty && e.dirty ? null : Ni(e.path) ? "vendor" : Ai(e) ? "binary" : null;
}, $n = (e, t = {}) => {
  const n = Date.now(), o = [], i = [], r = [];
  for (const s of e) {
    const d = s.size ?? s.content.length, p = zt(s, t);
    if (p) {
      i.push({ fileId: s.id, name: s.name, path: s.path, language: s.language, reason: p, size: d });
      continue;
    }
    const f = Re(s.content), b = yi(s, f, n, t);
    r.push(...b), o.push({
      fileId: s.id,
      name: s.name,
      path: s.path,
...
}, yi = (e, t = Re(e.content), n = Date.now(), o = {}) => {
  const i = Math.max(500, o.maxChunkChars ?? fi), r = Math.min(Math.max(0, o.chunkOverlapChars ?? bi), Math.floor(i / 2)), s = Math.max(1, o.maxChunksPerFile ?? gi), d = $i(e.content), p = [];
  let f = 0, b = 0;
  for (; f < e.content.length && p.length < s; ) {
    const u = Ci(e.content, f, Math.min(e.content.length, f + i)), w = e.content.slice(f, u).trim();
    if (w.length > 0) {
      const S = xi(d, f, u);
      p.push({
        id: `${e.id}:${t}:${b}:${S.startLineNumber}-${S.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
  ...
  const o = Math.max(t + 1, n - 500), i = e.lastIndexOf(`
`, n);
  if (i >= o) return i + 1;
...
` && t.push(n + 1);
  ...
  const o = xn(e, t), i = xn(e, Math.max(t, n - 1));
  return {
    startLineNumber: o + 1,
    startColumn: t - e[o] + 1,
    endLineNumber: i + 1,
  ...
  return 0;
}, Ni = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => ki.has(t)), Ai = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && wi.has(t) || e.content.includes("\0")) return !0;
...
const _e = (e = [], t = {}) => new Si(e, t), Ii = (e, t, n = {}) => {
  const o = new Map(t.map((d) => [d.id, d])), i = e.byFileId, r = [];
  for (const d of e.files) {
    const p = o.get(d.fileId);
    if (!p) {
      r.push({ fileId: d.fileId, path: d.path, status: "deleted", indexedHash: d.hash });
      continue;
    }
    if (zt(p, n)) {
      r.push({ fileId: d.fileId, path: p.path, status: "skipped-now", indexedHash: d.hash, currentHash: Re(p.content) });
      continue;
    }
    const b = Re(p.content);
    b !== d.hash ? r.push({ fileId: d.fileId, path: p.path, status: "changed", indexedHash: d.hash, currentHash: b }) : p.dirty !== d.dirty && r.push({ fileId: d.fileId, path: p.path, status: "dirty-state-changed", indexedHash: d.hash, currentHash: b });
  }
  for (const d of t)
    !i.has(d.id) && !zt(d, n) && r.push({ fileId: d.id, path: d.path, status: "new", currentHash: Re(d.content) });
  const s = e.signature !== Gn(t);
  return { stale: r.length > 0, signatureChanged: s, files: r };
}, Mi = 8, Pi = 12e3, Xn = (e, t, n = {}, o) => {
  const i = Oi(t);
  if (i.length === 0) return [];
  const r = new Set(o?.files.map((b) => b.fileId) ?? []), s = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => Ei(b, i)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, u) => u.score - b.score || b.chunk.path.localeCompare(u.chunk.path) || b.chunk.ordinal - u.chunk.ordinal), d = [], p = /* @__PURE__ */ new Set();
  let f = n.maxChars ?? Pi;
  for (const b of s) {
    if (d.length >= (n.limit ?? Mi) || f <= 0) break;
...
[...context clipped...]` : b.chunk.text;
    d.push(Ti(b, w, r.has(b.chunk.fileId))), f -= w.length;
  }
  return d;
}, Ti = (e, t = e.chunk.text, n = !1) => {
  const { chunk: o, score: i, matchedTerms: r } = e;
  return {
    id: `index-hit:${o.id}`,
    kind: "index-hit",
...
`), Oi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Ei = (e, t) => {
  const n = e.text.toLowerCase(), o = e.path.toLowerCase(), i = e.name.toLowerCase(), r = e.language.toLowerCase();
  let s = 0;
  const d = [];
  for (const p of t) {
    const f = Tt(n, p), b = Tt(o, p), u = Tt(i, p), w = r === p ? 1 : 0;
    f + b + u + w !== 0 && (d.push(p), s += Math.min(f, 12), s += b * 4, s += u * 6, s += w * 3, (n.includes(`function ${p}`) || n.includes(`const ${p}`) || n.includes(`class ${p}`) || n.includes(`type ${p}`)) && (s += 3));
  }
  ...
  const n = e.toLowerCase(), o = t.map((p) => n.indexOf(p)).filter((p) => p >= 0).sort((p, f) => p - f)[0] ?? 0, i = Math.max(0, o - 120), r = Math.min(e.length, o + 260), s = i > 0 ? "…" : "", d = r < e.length ? "…" : "";
  return `${s}${e.slice(i, r).replace(/\s+/g, " ").trim()}${d}`;
}, ji = (e, t = {}) => {
  const { autoRefresh: n = !1, ...o } = t, i = Be(e), r = Be(null);
  r.current || (r.current = _e(e, o));
  const [s, d] = C(() => r.current?.getSnapshot() ?? _e(e, o).getSnapshot());
  i.current = e;
  const p = y((w = i.current) => {
    const S = (r.current ?? _e([], o)).refresh(w, o);
    return d(S), S;
  }, [o.maxFileBytes, o.maxChunkChars, o.chunkOverlapChars, o.maxChunksPerFile, o.includeDirty]), f = y((w = i.current) => {
    const S = (r.current ?? _e([], o)).update(w, o);
    return d(S), S;
  }, [p]);
  ne(() => {
    n && p(e);
  }, [n, e, p]);
  const b = (r.current ?? _e([], o)).staleReport(e), u = y((w, S = {}) => {
    const T = r.current ?? _e([], o);
    return Xn(T.getSnapshot(), w, S, T.staleReport(i.current));
  }, []);
  return { snapshot: s, staleReport: b, isStale: b.stale, refresh: p, update: f, retrieve: u };
}, Fi = "tytus.atomek.semanticVector:v1", Yn = "__gateway_default__", Qn = (e) => e?.trim() || Yn, Di = (e) => {
  const t = Qn(e);
  return t === Yn ? "gateway default" : t;
}, Zn = (e, t, n) => [
  Fi,
  ...
  if (!ea()) return null;
  try {
    const o = Zn(e, t, n), i = window.localStorage.getItem(o);
    if (!i) return null;
  ...
  if (!ea()) return null;
  try {
    const t = Zn(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, Ye = 8, Hi = 12e3, Bi = 180, Wi = async (e, t, n, o, i = {}, r, s) => {
  const d = Xn(t, n, { ...i, limit: Math.max(i.limit ?? Ye, 16) }, r), p = e.ai;
  if (typeof p?.embedText != "function")
    return { hits: d.slice(0, i.limit ?? Ye), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const f = Qn(o.embeddingModel), b = await p.embedText({
      input: n,
    ...
    const w = new Map(d.map((P) => [P.chunkId, P])), S = Xi(n), T = /* @__PURE__ */ new Map();
    t.chunks.filter((P) => i.includeDirty !== !1 || !P.dirty).slice(0, Bi).forEach((P) => T.set(P.id, P)), d.forEach((P) => {
      const B = t.byChunkId.get(P.chunkId);
      B && T.set(B.id, B);
    });
    const U = Array.from(T.values()), G = [];
    let q = 0;
    for (const P of U) {
      s?.aborted;
      const B = zi(e.appId, f, P) ?? await Vi(p, e.appId, f, P, o, s);
      if (!B) continue;
      B.updatedAt > Date.now() - 1e3 && (q += 1);
      const Q = Ji(u, B.vector);
      if (!Number.isFinite(Q)) continue;
      const te = w.get(P.id)?.score ?? Yi(P, S), ce = Gi(te) * 0.42 + Math.max(0, Q) * 0.58;
      G.push({ chunk: P, keywordScore: te, vectorScore: Q, score: ce, matchedTerms: S.filter((xe) => P.text.toLowerCase().includes(xe)) });
    }
    if (G.length === 0) return { hits: d.slice(0, i.limit ?? Ye), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: q };
    const ee = new Set(r?.files.map((P) => P.fileId) ?? []), ae = G.sort((P, B) => B.score - P.score || B.keywordScore - P.keywordScore || P.chunk.path.localeCompare(B.chunk.path) || P.chunk.ordinal - B.chunk.ordinal).slice(0, Math.max(i.limit ?? Ye, 1)).map((P) => qi(P, ee.has(P.chunk.fileId)));
    return {
      hits: Ki(ae, i.maxChars ?? Hi),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${Di(o.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: q
    };
  } catch (f) {
    return {
      hits: d.slice(0, i.limit ?? Ye),
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: Qi(e.chunk.text, e.matchedTerms),
    score: e.score,
  ...
  for (const i of e) {
    if (n <= 0) break;
    const r = i.text.length > n ? `${i.text.slice(0, Math.max(0, n - 28))}
[...context clipped...]` : i.text;
    o.push({ ...i, text: r, charCount: r.length }), n -= r.length;
  }
  return o;
}, ta = (e) => {
  ...
  let o = 0, i = 0, r = 0;
  for (let s = 0; s < n; s += 1)
    o += e[s] * t[s], i += e[s] * e[s], r += t[s] * t[s];
  return i === 0 || r === 0 ? 0 : o / (Math.sqrt(i) * Math.sqrt(r));
}, Gi = (e) => Math.min(1, Math.max(0, e / 24)), Xi = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Yi = (e, t) => {
  const n = `${e.path}
${e.name}
${e.language}
${e.text}`.toLowerCase();
  return t.reduce((o, i) => o + (n.includes(i) ? 1 : 0), 0);
}, Qi = (e, t) => {
  const n = e.toLowerCase(), o = t.map((s) => n.indexOf(s)).filter((s) => s >= 0).sort((s, d) => s - d)[0] ?? 0, i = Math.max(0, o - 120), r = Math.min(e.length, o + 260);
  return `${i > 0 ? "…" : ""}${e.slice(i, r).replace(/\s+/g, " ").trim()}${r < e.length ? "…" : ""}`;
}, Zi = /```([^\n`]*)\n([\s\S]*?)```/g, er = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Zt(e) {
  return Array.from(e.matchAll(Zi)).map((t) => na(t[1] ?? "", ir(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function na(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), o = {}, i = [];
  for (const r of n) {
    const s = r.match(/^([a-z0-9_-]+)=(.+)$/i);
    s ? o[s[1].toLowerCase()] = sr(s[2]) : i.push(r.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: i, attrs: o, content: t };
}
function be(e, t) {
  return (typeof e == "string" ? na(e, "").flags : e.flags).includes(t.toLowerCase());
}
function aa(e) {
  return [...Zt(e).filter((o) => be(o, "diff") || be(o, "patch")).map((o) => o.content), e].filter((o, i, r) => tr(o) && r.findIndex((s) => s === o) === i);
}
function tr(e) {
...
`), o = [];
  ...
  }), o.length === 0) return [{ raw: t, ...Lt(t) }];
  const i = Array.from(new Set(o)).sort((s, d) => s - d), r = [];
  for (let s = 0; s < i.length; s += 1) {
    const d = i[s], f = i.find((u) => u > d && n[u].startsWith("diff --git ")) ?? n.length, b = n.slice(d, f).join(`
`);
    /^@@\s+-\d+/m.test(b) && r.push({ raw: b, ...Lt(b) });
  }
  return r.length > 0 ? r : [{ raw: t, ...Lt(t) }];
}
function Lt(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), o = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), i = Se(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, r = Se(t?.[3] || t?.[4] || o?.[1] || o?.[2] || "") || null, s = r && r !== "/dev/null" ? r : i;
  return { path: s && s !== "/dev/null" ? s : null, oldPath: i, newPath: r };
}
function Se(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function _t(e) {
  return Zt(e).filter((t) => ar(t)).map((t) => ({
    content: rr(t.content),
...
function ar(e) {
  return be(e, "atomek-replace") || be(e, "atomek-full") || be(e, "full-replacement") || be(e, "full") || be(e, "replace");
}
function or(e) {
...
`).slice(0, 5);
  for (const o of n) {
    const i = o.match(/^\s*(?:\/\/|#|<!--)?\s*(?:atomek-)?(?:path|file)\s*:\s*([^\s>]+)\s*(?:-->)?\s*$/i);
    if (i) return Se(i[1]);
  }
  return null;
}
...
`), i = [];
  ...
  for (let d = 0; d < o.length; d += 1) {
    const p = o[d].match(er);
    if (!p) continue;
    s = !0;
    const f = Number(p[1]), b = Math.max(0, f - 1);
    if (b < r) return null;
    for (i.push(...n.slice(r, b)), r = b, d += 1; d < o.length; d += 1) {
      const u = o[d];
      if (u.startsWith("@@ ")) {
        d -= 1;
        break;
  ...
  return s ? (i.push(...n.slice(r)), i.join(`
`)) : null;
}
function ir(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function cr(e) {
  const { body: t, files: n, sourceTitle: o, activeFile: i = null, versions: r = {} } = e, s = /* @__PURE__ */ new Map(), d = [];
  for (const b of aa(t))
    for (const u of nr(b)) {
      if (!u.path) {
        if (i) {
          const T = Ht(i.content, u.raw);
          T && T !== i.content && s.set(i.id, lt({
            file: i,
            proposedContent: T,
            sourceTitle: o,
      ...
      const w = Nn(n, u.path);
      if (!w) {
        d.push(`${u.path}: no opened file`);
        continue;
      }
      const S = Ht(w.file.content, u.raw);
      if (!S || S === w.file.content) {
        d.push(`${u.path}: patch did not match or produced no change`);
        continue;
      }
      s.set(w.file.id, lt({
        file: w.file,
  ...
  for (const b of _t(t)) {
    const u = b.path ? Nn(n, b.path) : i ? { file: i, normalizedPatchPath: Se(i.path), confidence: "exact" } : null;
    if (!u) {
      d.push(`${b.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (b.content === u.file.content) {
      d.push(`${u.file.path}: replacement produced no change`);
      continue;
    }
    s.set(u.file.id, lt({
      file: u.file,
  ...
  const p = _t(t).some((b) => b.path);
  if (s.size === 0 && i && !p) {
    const b = lr(t, i);
    b && b.content !== i.content && s.set(i.id, lt({
      file: i,
      proposedContent: b.content,
      sourceTitle: o,
      extractionLabel: b.label,
...
`), i = Math.max(n.length, o.length);
  let r = 0, s = 0, d = 0;
  for (let p = 0; p < i; p += 1)
    n[p] !== o[p] && (n[p] === void 0 ? r += 1 : o[p] === void 0 ? s += 1 : d += 1);
  return { added: r, removed: s, changed: d };
}
function hr(e) {
  return e.reduce((t, n) => ({
...
function lt(e) {
  ...
  return {
    ...
    stats: dr(e.file.content, e.proposedContent),
    base: {
...
const pt = (e) => typeof e == "object" && e !== null, pr = (e) => pt(e.ai) ? e.ai : null, fr = (e) => {
  if (!pt(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, br = (e) => {
  const t = fr(e);
  return !t || !pt(e) ? null : { ...e, id: t };
}, Ce = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Ce) : pt(e) ? Object.values(e).some(Ce) : !1, gr = async (e, t) => {
  const n = pr(e);
  if (typeof n?.listModels != "function") return [];
  const o = await n.listModels(t);
  return Array.isArray(o) ? o.map(br).filter((i) => !!i) : [];
}, kr = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Ce(e.capability) || Ce(e.capabilities) || Ce(e.modality) || Ce(e.modalities) || Ce(e.task) || Ce(e.tasks) || Ce(e.type) || Ce(e.kind), ia = (e) => typeof e == "object" && e !== null, wr = (e) => ia(e.ai) ? e.ai : null, ra = (e) => typeof wr(e)?.embedText == "function", yr = async (e, t) => ra(e) ? (await gr(e, t)).filter(kr) : [], vr = (e) => ia(e.ai) ? ra(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", Cr = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], $r = [
...
function Sr(e) {
  const t = e.results.length > 0 ? e.results.map((o, i) => [
    `Check ${i + 1}: ${o.command}`,
    `Status: ${o.status}`,
    "Output:",
    Rr(o.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function Mr(e) {
  ...
  for (const o of t) {
    const i = Pr(o.content);
    if (!i?.scripts) continue;
    const r = ca(o.path), s = Tr(e, r, i);
    if (!s) continue;
    const d = Object.keys(i.scripts).filter((f) => typeof i.scripts?.[f] == "string"), p = Lr(d);
    for (const f of p) {
      const b = `${s} run ${f}`;
      n.push({
        id: la(`${r}:${b}`),
...
function Tr(e, t, n) {
  ...
  for (const [o, i] of $r)
    if (e.some((r) => Er(r.path) === o && ca(r.path) === t)) return i;
  return null;
}
function Lr(e) {
  const t = Cr.filter((o) => e.includes(o)), n = e.filter((o) => !t.includes(o)).filter((o) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(o)).sort((o, i) => o.localeCompare(i));
  return [...t, ...n];
}
function Or(e) {
  const t = /* @__PURE__ */ new Set();
...
const jr = Ya(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Fr = {
  id: "welcome",
  ...
  dirty: !1,
  source: "sample"
}, da = "tytus.workspace.recent", ha = "tytus.workspace.layout", ua = "tytus.atomek.chatAiSettings", ma = "tytus.atomek.currentMission", Bt = "tytus.atomek.currentMissionChanged", ht = {
...
function Ot(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - Dr), o = n < 1180, i = o ? 200 : 240, r = o ? 300 : 340, s = o ? 420 : 560, d = Math.max(i, Math.min(o ? 340 : 420, Math.floor(n * 0.28))), p = Math.min(300, d), f = Math.floor(n * (o ? 0.34 : 0.36)), b = n - p - s, u = Math.max(r, Math.min(o ? 500 : 640, f, b));
  return { primaryMin: i, primaryMax: d, secondaryMin: r, secondaryMax: u };
}
function zr(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let o = 0, i = 0, r;
  for (; (r = n.exec(e)) !== null; ) {
    if (r.index > o) {
      const d = e.slice(o, r.index);
      d.trim() && t.push({ type: "markdown", body: d, key: `md-${i}` });
    }
    t.push({
      type: "code",
...
async function An(e, t, n) {
  ...
  const i = await o.call(e, t, { create: !0 });
...
function Ze(e) {
  ...
  try {
    localStorage.setItem(ma, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(Bt, { detail: t }));
  } catch {
  }
}
function Br() {
...
function et(e, t) {
  const n = t?.resources ?? [], o = n.some((p) => p.kind === "pod-agent" && (p.status === "ready" || p.status === "degraded")), i = n.some((p) => p.kind === "local-cli" && (p.status === "ready" || p.status === "available")), r = n.some((p) => p.kind === "shared-folder" && p.status === "ready"), s = n.some((p) => p.kind === "app-skill" && p.status === "ready"), d = e.trim() || "Coordinate a Tytus mission.";
  return [
    {
      id: "task-plan",
      title: "Plan mission",
    ...
    },
    {
      ...
      resourceHint: i ? "local agent" : s ? "app skill" : "chat",
      status: "waiting",
...
function ga(e, t, n) {
  ...
  return JSON.stringify({
    ...
    resources: (t?.resources ?? []).filter((i) => i.status === "ready").map((i) => ({
      resourceId: i.id,
      pinnedLabel: i.label,
      pinnedKind: i.kind,
      pinnedCapabilities: i.capabilities
    })),
    permissions: {
    ...
    tasks: o.map((i, r) => ({
      ...
      selectedResourceHint: i.resourceHint,
      dependsOn: r === 0 ? [] : [o[r - 1].id],
...
function Ur({ host: e }) {
  const t = Be(null), [n, o] = C(0), i = le(() => $s(), []), [r, s] = C("computer"), [d, p] = C(i.primaryVisible), [f, b] = C(i.primaryWidth), [u, w] = C("chat"), [S, T] = C(i.secondaryVisible), [U, G] = C(i.secondaryWidth), [q, ee] = C(!1), [ae, P] = C("problems"), [B, Q] = C(i.markdownPreviewVisible), [H, te] = C(!1), [ce, xe] = C(null), [g, O] = C([]), [he, ke] = C([]), [R, oe] = C(null), [we, re] = C(""), [N, x] = C({ lineNumber: 1, column: 1 }), [j, D] = C(null), [ie, F] = C({}), [Z, Ne] = C(ni), [Te, Ie] = C([]), [Ae, ye] = C([]), [X, J] = C(null), [h, v] = C(""), [A, W] = C(!1), [I, me] = C(""), [E, ge] = C([]), [z, K] = C(null), [se, We] = C(null), [tn, nn] = C(!1), [Le, xa] = C(() => xs()), [an, Ue] = C(null), [je, ft] = C(null), [bt, gt] = C(""), [kt, wt] = C(""), [nt, at] = C(""), [ot, on] = C("failed"), [Na, rn] = C(null), [Aa, M] = C("Ready"), [Ve, Sa] = C(() => Cs()), pe = he.map((c) => g.find((m) => m.id === c)).filter(Boolean), L = R ? g.find((c) => c.id === R) ?? null : null, sn = le(() => ti({ files: g, openEditorIds: he, activeFileId: R, versions: ie, activeSelection: j }), [R, j, ie, g, he]), cn = le(() => ({
    scope: Z,
    removedAttachmentIds: Te,
    selectedFileIds: []
  }), [Z, Te]), qe = le(() => mi(sn, g, cn), [cn, sn, g]), Me = ji(g, { autoRefresh: !0, includeDirty: !0 }), ln = le(() => Ae.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [Ae]), Ia = le(() => [...qe.attachments, ...ln], [qe.attachments, ln]), _ = Qo({ host: e, requestContext: qe.parts, chatSettings: Le, setStatus: M }), Oe = le(
    () => [..._.artifacts, ...E].sort((c, m) => m.createdAt - c.createdAt),
    [_.artifacts, E]
  ), it = tn && !L, yt = !L && !it && !H, ue = le(() => g.filter((c) => c.dirty), [g]), Ma = le(() => {
    const c = we.trim().toLowerCase();
  ...
  const Pa = y((c) => {
    if (c.kind === "index-hit") {
      ye((m) => m.filter((k) => k.id !== c.id)), M(`Removed project context: ${c.label}`);
      return;
    }
    Ie((m) => m.includes(c.id) ? m : [...m, c.id]), M(`Removed chat context: ${c.label}`);
  }, []), rt = y(() => {
    nn(!0), oe(null), te(!0), M("Atomek settings opened");
  }, []), dn = y(() => {
    nn(!1), oe((c) => c ?? he.at(-1) ?? null), he.length === 0 && te(!1), M("Atomek settings closed");
  }, [he]), Fe = y((c) => {
    F((m) => ({ ...m, [c]: (m[c] ?? 1) + 1 }));
  }, []), Ta = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const m = Ot(n), k = c.clientX, $ = U, Y = (Ge) => {
      const Mt = $ + (k - Ge.clientX);
      G(dt(Mt, m.secondaryMin, m.secondaryMax));
    }, ve = () => {
      window.removeEventListener("pointermove", Y), window.removeEventListener("pointerup", ve);
    };
    window.addEventListener("pointermove", Y), window.addEventListener("pointerup", ve);
  }, [U, n]), La = y((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const m = Ot(n), k = c.clientX, $ = f, Y = (Ge) => {
      const Mt = $ + (Ge.clientX - k);
      b(dt(Mt, m.primaryMin, m.primaryMax));
    }, ve = () => {
      window.removeEventListener("pointermove", Y), window.removeEventListener("pointerup", ve);
    };
    window.addEventListener("pointermove", Y), window.addEventListener("pointerup", ve);
  }, [f, n]), st = y((c) => {
    const m = [c, ...Ve.filter((k) => k.path !== c.path)].slice(0, 6);
    Sa(m), localStorage.setItem(da, JSON.stringify(m));
  }, [Ve]), de = y((c, m) => {
    ke((k) => k.includes(c.id) ? k : [...k, c.id]), oe(c.id), te(!1), rn(m ?? null), x({ lineNumber: m ?? 1, column: 1 });
  }, []), Oa = y((c) => {
    if (!c.fileId) return;
    const m = g.find((k) => k.id === c.fileId);
    m && (de(m, c.range?.startLineNumber ?? 1), M(`Revealed context: ${c.label}`));
  }, [g, de]), Ke = y(async () => {
    if (Et(ue, "open new files"))
      try {
        const c = await Wo();
        if (c.length === 0) return;
        O((m) => ws(m, c)), c.forEach((m) => st({ name: m.name, path: m.path, at: Date.now() })), de(c[0]), M(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && M(`Open file failed: ${c.message}`);
      }
  }, [ue, de, st]), vt = y(async () => {
    if (Et(ue, "open another folder"))
      try {
        const c = await Uo();
        xe(c), O(c.files), ke([]), oe(null), te(!1), st({ name: c.name, path: c.name, at: Date.now() }), M(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!R) return;
    let m = !1;
    O((k) => k.map(($) => $.id !== R || $.content === c ? $ : (m = !0, { ...$, content: c, dirty: !0 }))), m && Fe(R);
  }, [R, Fe]), Ct = y(async () => {
    if (L)
      try {
        const c = await Pt(L);
        O((m) => m.map((k) => k.id === c.id ? c : k)), M(`Saved ${c.name}`);
      } catch (c) {
    ...
    if (!m) return null;
    const k = await Pt(m);
    return O(($) => $.map((Y) => Y.id === k.id ? k : Y)), k;
  }, [g]), hn = y(async () => {
    ...
    try {
      const m = await Promise.all(c.map(($) => Pt($))), k = new Map(m.map(($) => [$.id, $]));
      O(($) => $.map((Y) => k.get(Y.id) ?? Y)), Ue(null), M(`Saved ${m.length} dirty file${m.length === 1 ? "" : "s"}`);
    } catch (m) {
    ...
    ke((k) => {
      const $ = k.filter((Y) => Y !== c);
      return R === c && (oe($.at(-1) ?? null), D(null)), $;
    });
  }, [R, g]), ja = y(() => {
    Et(ue, "close all editors") && (ke([]), oe(null), rn(null), te(!1), M("Closed all editors"));
  }, [ue]), xt = y(() => {
...
`,
    ...
    return ye($), $.length === 0 ? (M(Me.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), m) : (k.mode !== "hybrid" && k.reason ? M(k.reason) : M(`${$.length} project context hit${$.length === 1 ? "" : "s"} · ${k.reason ?? "hybrid retrieval"}`), [
      ...m,
      ...Ui($)
    ]);
  }, [qe.parts, Z, Le, e, Me]), Nt = y(() => {
    if (!L && pe.length === 0) {
      M("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const m = [
      `Create a polished Markdown artifact from ${L?.path ?? `${pe.length} open editors`}.`,
    ...
    T(!0), w("chat"), M("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const k = await Pe(m), $ = await _.askAgent(m, { requestContext: k });
      !$ || $.status === "error" || _.createArtifact({
        messageId: $.id,
        title: `AI synthesis — ${L?.name ?? "open workspace"}`,
        kind: "markdown",
        body: $.body
      }).then(() => {
        T(!0), w("outputs"), ee(!0), P("output");
      });
    })();
  }, [L, _, Pe, pe.length]), At = y((c) => {
    const m = g.find((k) => k.path === c.path || k.name === c.name);
    if (m) {
      de(m), M(`Opened recent ${m.name}`);
      return;
    }
    ...
    T(!0), w("chat"), v(""), (async () => {
      const m = await Pe(c);
      await _.askAgent(c, { requestContext: m });
    })();
  }, [_, Pe]), un = y(async (c) => {
    if (!e.skills?.get) {
...
[Skill pack clipped by Atomek. Ask for the full pack if needed.]` : m.body, $ = [
...
`);
      v((Y) => [Y.trim(), $].filter(Boolean).join(`

`)), T(!0), w("chat"), M(`Attached skill ${m.title} to chat input`);
    } catch (m) {
      M(`Failed to attach skill: ${m instanceof Error ? m.message : String(m)}`);
    }
  }, [e.skills]), Fa = y((c) => {
    const m = _.messages.findIndex(($) => $.id === c.id), k = _.messages.slice(0, m < 0 ? void 0 : m).reverse().find(($) => $.role === "user");
    if (!k?.body.trim()) {
      M("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const $ = await Pe(k.body);
      await _.askAgent(k.body, { requestContext: $ });
    })();
  }, [_, Pe]), Da = y((c) => {
    _.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    _.createArtifact({
      title: L.path,
      kind: L.language === "markdown" ? "markdown" : "report",
      body: L.content
    }).then(() => {
      T(!0), w("outputs"), ee(!0), P("output");
    });
  }, [L, _]), St = y((c) => {
    const m = jt(g, Rt(c.title || c.kind)), k = {
    ...
    O(($) => [...$, k]), de(k), M(`Opened ${c.title} as editable file`);
  }, [g, de]), It = y(() => {
...
`), Y = {
    ...
    ge((ve) => [Y, ...ve]), ee(!0), P("output"), M(`Captured manual check: ${m}`);
  }, []), De = y((c, m) => {
    const k = cr({
      body: m,
      files: g,
    ...
    return k.edits.length > 1 ? (We($), K(null), M(`Previewing AI workspace patch for ${k.edits.length} files`), !0) : k.edits.length === 1 ? (K($.edits[0]), We(null), M(`Previewing AI patch for ${$.edits[0].fileName}`), !0) : (M(k.skipped.length > 0 ? `No applicable edit found. ${k.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [L, ie, g]), mn = y((c, m) => {
    const k = {
    ...
    ge(($) => [k, ...$]), T(!0), w("outputs"), ee(!0), P("output"), M(`Saved local job output: ${c}`), (m.includes("```diff") || m.includes("--- a/")) && De(c, m);
  }, [De]), Ha = y(() => {
    const c = h.trim();
    if (!c) return;
    v("");
    const m = ls(c);
    (async () => {
      const k = await Pe(c), $ = await _.askAgent(c, { requestContext: k });
      if (!m || !$ || $.status === "error") return;
      De($.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", $.body) || M("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [_, Pe, h, De]), Ba = y(() => {
    X && (J(null), fe(ds(X)));
  }, [fe, X]), Je = y((c) => {
    ...
    at(k), gt(""), wt(""), on("failed"), ee(!0), P("terminal"), M(m.commands.length > 0 ? `Manual check ready: copy ${m.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [g]), Wa = y((c) => {
    c.trim() && (tt(c), at(c), M(`Manual check command copied: ${c}`));
  }, []), Ua = y(() => {
    ft((c) => {
      if (!c) return c;
      const m = Nr(c, bt), k = m.commands.at(-1)?.command ?? "";
      return k && at(k), m;
    }), gt("");
  }, [bt]), Va = y(() => {
    if (!je || !nt.trim()) {
    ...
    const c = nt.trim();
    ft((m) => m && Ar(m, c, ot, kt)), wt(""), M(`Captured manual check result: ${c} (${ot})`);
  }, [kt, nt, je, ot]), qa = y(() => {
    je && fe(Sr(je));
  }, [fe, je]), Ka = y(() => {
    if (!z) return;
    const c = g.find((m) => m.id === z.fileId);
    if (!c) {
      M(`Cannot apply edit — ${z.fileName} is no longer open`), K(null);
      return;
    }
    c.content !== z.originalContent && !window.confirm(`${z.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (O((m) => m.map((k) => k.id === z.fileId ? { ...k, content: z.proposedContent, dirty: !0 } : k)), Fe(z.fileId), Ue(`AI edit applied to ${c.name}. Save All to persist it to disk.`), K(null), Je(`AI edit applied to ${c.name}`), M(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Fe, g, z, Je]), Ja = y(() => {
    if (!z) return;
    const c = jt(g, `${Rt(z.fileName)}-ai-edit`), m = {
      id: `pending-edit-${z.fileId}-${Date.now()}`,
      name: c,
      path: c,
      language: L?.language ?? "markdown",
      content: z.proposedContent,
      dirty: !0,
      source: "generated"
    };
    O((k) => [...k, m]), de(m), K(null), M(`Opened proposed edit as ${c}`);
  }, [L?.language, g, de, z]), Ga = y(() => {
    if (!se) return;
    const c = new Map(se.edits.map((k) => [k.fileId, k]));
    g.some((k) => {
      const $ = c.get(k.id);
      return $ && k.content !== $.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (O((k) => k.map(($) => {
      const Y = c.get($.id);
      return Y ? { ...$, content: Y.proposedContent, dirty: !0 } : $;
    })), c.forEach((k, $) => Fe($)), Ue(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), We(null), Je(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), M(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Fe, g, se, Je]), Xa = y(() => {
    if (!se) return;
    const c = [...g], m = se.edits.map((k) => {
      const $ = jt(c, `${Rt(k.fileName)}-ai-edit`), Y = g.find((Ge) => Ge.id === k.fileId), ve = {
        id: `workspace-patch-${k.fileId}-${Date.now()}-${$}`,
        name: $,
        path: $,
    ...
    O((k) => [...k, ...m]), m[0] && de(m[0]), We(null), M(`Opened ${m.length} proposed edit file${m.length === 1 ? "" : "s"}`);
  }, [g, de, se]), pn = y((c) => {
    if (c === "explain") {
  ...
  return ne(() => {
    const c = (m) => {
      (m.metaKey || m.ctrlKey) && (m.key.toLowerCase() === "o" && (m.preventDefault(), Ke()), m.key.toLowerCase() === "s" && (m.preventDefault(), Ct()), m.key.toLowerCase() === "w" && (m.preventDefault(), R && $t(R)), m.key.toLowerCase() === "b" && (m.preventDefault(), p(($) => !$)), m.shiftKey && m.key.toLowerCase() === "f" && (m.preventDefault(), s("search"), p(!0)), (m.key.toLowerCase() === "k" || m.key.toLowerCase() === "p") && (m.preventDefault(), W(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [R, $t, Ke, Ct]), ne(() => {
    const c = (m) => {
      ue.length !== 0 && (m.preventDefault(), m.returnValue = "");
    };
    ...
    const c = t.current;
    if (!c) return;
    const m = () => o(Math.round(c.getBoundingClientRect().width));
    m();
    ...
    if (!n) return;
    const c = Ot(n);
    b((m) => dt(m, c.primaryMin, c.primaryMax)), G((m) => dt(m, c.secondaryMin, c.secondaryMax));
  }, [n]), ne(() => {
    const c = { primaryVisible: d, primaryWidth: f, secondaryVisible: S, secondaryWidth: U, markdownPreviewVisible: B };
    localStorage.setItem(ha, JSON.stringify(c));
  }, [B, d, f, S, U]), ne(() => {
    localStorage.setItem(ua, JSON.stringify(Le));
  }, [Le]), /* @__PURE__ */ l(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${d ? "" : "no-primary"} ${S ? "" : "no-secondary"} ${q ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${f}px`, "--workbench-secondary-width": `${U}px` },
      children: [
        /* @__PURE__ */ a(Vr, { active: r, setActive: (c) => {
          s(c), p(!0);
        }, openSettings: rt, settingsActive: it }),
        d && /* @__PURE__ */ l("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: La, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ l("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => W(!0), children: "Workspace" }),
          /* @__PURE__ */ l("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              Xr,
              {
                ...
                canPreview: L?.language === "markdown",
                previewVisible: B,
                togglePreview: () => Q((c) => !c)
              }
            ),
            /* @__PURE__ */ a(Gr, { file: L, folder: ce, showWelcome: yt }),
            an && ue.length > 0 ? /* @__PURE__ */ l("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: an }),
              /* @__PURE__ */ a("button", { onClick: () => {
                hn();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => Ue(null), title: "Dismiss", children: /* @__PURE__ */ a($e, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: L ? /* @__PURE__ */ l("div", { className: L.language === "markdown" && B ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(Qa, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                jr,
                {
                  file: L,
                  revealLine: Na,
              ...
              L.language === "markdown" && B && /* @__PURE__ */ a(as, { content: L.content })
            ] }) : it ? /* @__PURE__ */ a(
              hs,
              {
                host: e,
            ...
            ) : yt ? /* @__PURE__ */ a(Zr, { host: e, openFile: Ke, openFolder: vt, newFile: xt, recent: Ve, reopenRecent: At, setStatus: M, openControlTower: () => {
              s("computer"), p(!0);
            }, openChat: () => {
              w("chat"), T(!0);
            } }) : /* @__PURE__ */ l("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(mt, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => te(!1), children: "Show Mission Control" })
            ] }) }),
            q && /* @__PURE__ */ a(
              is,
              {
                ...
                runAiSynthesis: Nt,
                captureManualCheck: It,
                openOutputAsFile: St,
                manualCheckSession: je,
                manualCheckCommandInput: bt,
                setManualCheckCommandInput: gt,
                manualCheckOutputInput: kt,
                setManualCheckOutputInput: wt,
                manualCheckSelectedCommand: nt,
                setManualCheckSelectedCommand: at,
                manualCheckStatus: ot,
                setManualCheckStatus: on,
                copyManualCheckCommand: Wa,
                addManualCheckCommand: Ua,
                recordManualCheckResult: Va,
                askAgentFromManualChecks: qa,
                onClose: () => ee(!1)
              }
            )
          ] })
        ...
        S && /* @__PURE__ */ a(
          rs,
          {
            ...
            saveMessageAsArtifact: Da,
            rememberMessage: za,
            previewEditFromMessage: (c) => De(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: pn,
            pendingPatchPrompt: X,
            generatePatchPrompt: Ba,
            ...
            captureManualCheck: It,
            openOutputAsFile: St,
            previewEditFromOutput: (c) => De(c.title, c.body),
            ...
            contextScope: Z,
            setContextScope: Ne,
            contextAttachments: Ia,
            removeContextAttachment: Pa,
            revealContextAttachment: Oa,
            projectIndexSummary: `${Me.snapshot.files.length} files · ${Me.snapshot.chunks.length} chunks`,
            projectIndexStale: Me.isStale,
            refreshProjectIndex: () => {
              const c = Me.refresh(g);
        ...
        A && /* @__PURE__ */ a(
          ns,
          {
            ...
            activeFile: L,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: xt },
              ...
              ...Ve.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => At(c) })),
              { label: "File: Save All", detail: `${ue.length} dirty file${ue.length === 1 ? "" : "s"}`, run: () => {
                hn();
              }, disabled: ue.length === 0 },
              { label: "File: Close All Editors", detail: `${pe.length} open editor${pe.length === 1 ? "" : "s"}`, run: ja, disabled: pe.length === 0 },
              ...
              { label: "Help: Show Mission Control", detail: "Open the Atomek control tower landing page", run: () => {
                oe(null), te(!1), s("computer"), p(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: d ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => p((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: S ? "Hide right AI side bar" : "Show right AI side bar", run: () => T((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: q ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => ee((c) => !c) },
              ...
              { label: "View: Toggle Markdown Preview", detail: L?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => Q((c) => !c), disabled: L?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: L || pe.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: Nt, disabled: !L && pe.length === 0 },
              { label: "AI: Explain Active File", detail: L ? `Ask Cortex to explain ${L.path}` : "Open a file first", run: () => fe("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !L },
              { label: "AI: Improve Active File", detail: L ? `Ask Cortex for concrete edits to ${L.path}` : "Open a file first", run: () => fe("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !L },
              { label: "AI: Draft Editable Replacement", detail: L ? `Ask Cortex for a full-file replacement for ${L.path}` : "Open a file first", run: () => pn("edit"), disabled: !L },
              { label: "AI: Plan Workspace Work", detail: pe.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => fe("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: pe.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: L ? "Persist active file in host.ai artifacts" : "Open a file first", run: _a, disabled: !L },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: It },
              { label: "AI: Open Latest Artifact as File", detail: Oe[0] ? `Create editable file from ${Oe[0].title}` : "No outputs yet", run: () => Oe[0] && St(Oe[0]), disabled: Oe.length === 0 }
            ],
            openWorkbenchFile: de,
            onClose: () => W(!1)
          }
        ...
        /* @__PURE__ */ a(ks, { status: Aa, file: L ?? Fr, cursor: N, fileCount: g.length, dirtyCount: ue.length })
      ]
    }
  );
}
function Vr({ active: e, setActive: t, openSettings: n, settingsActive: o }) {
  return /* @__PURE__ */ l("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(jn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(Lo, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(Dn, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(En, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(Vt, { size: 25 }), label: "Control Tower", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(ho, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(Ee, { icon: /* @__PURE__ */ a(jo, { size: 23 }), label: "Settings", active: o, onClick: n })
  ] });
}
function Ee({ icon: e, label: t, active: n, onClick: o }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: o, children: e });
}
function qr(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(es, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(Tn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(Tn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a($a, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(Kr, { ...e });
}
function Kr(e) {
  const t = !e.folder, n = le(() => Jr(e.files, e.folder?.name), [e.files, e.folder?.name]), [o, i] = C(() => /* @__PURE__ */ new Set());
  ne(() => {
    const s = ka(n);
    i((d) => {
      const p = new Set(d);
  ...
  return /* @__PURE__ */ l("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ l(He, { children: [
        ...
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((s) => /* @__PURE__ */ a(en, { file: s, active: s.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(s), label: s.name, detail: s.path }, s.id)),
        /* @__PURE__ */ l("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(ut, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : wa(n, e.activeFileId, e.openWorkbenchFile, o, r)
      ] }),
...
function en({ file: e, active: t, onOpen: n, basePath: o, depth: i = 0, label: r, detail: s }) {
  const d = o && e.path.startsWith(`${o}/`) ? e.path.slice(o.length + 1) : e.path, p = i || Math.max(0, d.split("/").length - 1);
  return /* @__PURE__ */ l("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": p }, onClick: n, title: e.path, children: [
    ...
    /* @__PURE__ */ l("span", { className: "workbench-row-text", children: [
      /* @__PURE__ */ a("span", { className: "workbench-row-name", children: r ?? d }),
...
function Jr(e, t) {
  const n = [], o = /* @__PURE__ */ new Map(), i = (s, d, p = n) => {
    const f = o.get(s);
    if (f) return f;
  ...
  e.forEach((s) => {
    ...
    p.slice(0, -1).forEach((u) => {
      b = b ? `${b}/${u}` : u, f = i(b, u, f).children;
    }), f.push({ name: p.at(-1) ?? s.name, path: d, file: s, children: [] });
  });
  const r = (s) => s.sort((d, p) => +!!d.file - +!!p.file || d.name.localeCompare(p.name)).map((d) => ({ ...d, children: r(d.children) }));
  return r(n);
}
function ka(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...ka(t.children)]);
}
function wa(e, t, n, o, i, r = 0) {
  return e.map((s) => {
    if (s.file)
      return /* @__PURE__ */ a(en, { file: s.file, active: s.file.id === t, onOpen: () => n(s.file), depth: r, label: s.name }, s.file.id);
    const d = o.has(s.path);
    return /* @__PURE__ */ l("div", { children: [
      /* @__PURE__ */ l("button", { className: "workbench-folder-row", style: { "--workbench-depth": r }, onClick: () => i(s.path), title: d ? `Collapse ${s.name}` : `Expand ${s.name}`, children: [
        d ? /* @__PURE__ */ a(ut, { size: 12 }) : /* @__PURE__ */ a(ut, { className: "workbench-chevron-collapsed", size: 12 }),
        d ? /* @__PURE__ */ a(Jt, { size: 14 }) : /* @__PURE__ */ a(Fn, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: s.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: s.children.length })
      ] }),
      d ? wa(s.children, t, n, o, i, r + 1) : null
    ] }, s.path);
  });
}
function Gr({ file: e, folder: t, showWelcome: n }) {
  const o = n ? ["Mission Control"] : e?.path.split("/").filter(Boolean) ?? [], i = t && o[0] === t.name ? o.slice(1) : o;
  return /* @__PURE__ */ a("div", { className: "workbench-breadcrumb", children: i.length === 0 ? /* @__PURE__ */ a("span", { children: "Workspace" }) : i.map((r, s) => /* @__PURE__ */ l("span", { className: "workbench-breadcrumb-part", children: [
...
function Xr(e) {
  return /* @__PURE__ */ l("div", { className: "workbench-tabs", children: [
    ...
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(Ao, { size: 16 }) })
  ] });
}
function Yr(e) {
  const t = e?.resources ?? [], n = (i) => t.filter((r) => r.kind === i).length, o = t.filter((i) => i.status === "ready" || i.status === "available").length;
  return [
...
function Zr({
  ...
  const [f, b] = C("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [u, w] = C(null), [S, T] = C([]), [U, G] = C(!1), [q, ee] = C(null), [ae, P] = C(null), B = y(async () => {
    G(!0), P(null);
    try {
      ...
      w(g ?? null), T(O), g && s(`Control Tower loaded · ${g.resources.length} resources · ${g.warnings.length} warnings`);
    } catch (g) {
      const O = g instanceof Error ? g.message : String(g);
      P(O), s(`Control Tower resource load failed: ${O}`);
    } finally {
      G(!1);
    }
  ...
  const Q = y(async () => {
    ...
    G(!0);
    try {
      const O = await e.missions.create({
        ...
        goal: O.goal,
        rootPath: O.rootPath,
        name: O.rootPath.split("/").pop() || O.missionId,
        source: "tray"
      }, ke = {
      ...
      await e.missions.write({
        rootPath: O.rootPath,
        files: [
          { path: "MISSION.md", content: Wt(he, u, null, [], g) },
          { path: "MISSION.json", content: ga(he, u, g) },
          { path: "RESOURCES.md", content: Ut(u) },
          { path: "TASKS.md", content: fa(et(g, u)) },
          { path: "HANDOFF.md", content: ba(he) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(ke)}
` },
...
`) }
      ...
      }), ee(O), Ze(O), s(`Mission created: ${O.rootPath}`), d();
    } catch (O) {
      s(`Mission create failed: ${O instanceof Error ? O.message : String(O)}`);
    } finally {
      G(!1);
    }
  }, [f, u, e.missions, d, s]), H = Yr(u), te = Qr(u), ce = u?.warnings ?? [], xe = et(f, u);
  return /* @__PURE__ */ l("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ l("section", { className: "workbench-control-hero-main", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-control-goal-row", children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            Q();
          }, disabled: U, children: "Start mission" }),
      ...
      q ? /* @__PURE__ */ l("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: q.rootPath })
      ] }) : null,
      ae ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: ae }) : null
    ] }),
    /* @__PURE__ */ l("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ l("header", { children: [
          ...
          /* @__PURE__ */ l("button", { className: "workbench-button-subtle", onClick: () => {
            ...
            " ",
            U ? "Refreshing…" : "Refresh"
          ] })
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: H.map((g) => /* @__PURE__ */ l("div", { className: "workbench-control-metric", children: [
      ...
      /* @__PURE__ */ l("article", { className: "workbench-control-card", children: [
        ...
        S.slice(0, 4).map((g) => /* @__PURE__ */ l("button", { className: "workbench-control-preset", onClick: () => {
          Ze(g), d();
      ...
      /* @__PURE__ */ l("article", { className: "workbench-control-card", children: [
        ...
        i.length ? /* @__PURE__ */ l("div", { className: "workbench-control-recent", children: [
          /* @__PURE__ */ a("span", { children: "Recent" }),
          i.slice(0, 3).map((g) => /* @__PURE__ */ a("button", { onClick: () => r(g), children: g.name }, `${g.path}-${g.at}`))
        ] }) : null
      ] }),
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ l("header", { children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-control-resource-list", children: [
          te.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          te.map((g) => /* @__PURE__ */ l("div", { className: "workbench-control-resource", children: [
        ...
        ce.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: ce.slice(0, 2).map((g) => /* @__PURE__ */ l("span", { children: [
          g.code,
          ": ",
          g.message
      ...
      /* @__PURE__ */ l("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: xe.map((g, O) => /* @__PURE__ */ l("button", { className: `workbench-task-card ${g.status}`, onClick: () => b(g.prompt), children: [
...
function es({ files: e, query: t, setQuery: n, openWorkbenchFile: o, activeFileId: i }) {
  const r = le(() => ts(e, t), [e, t]), s = le(() => {
    const d = /* @__PURE__ */ new Map();
    return r.forEach((p) => d.set(p.file.id, [...d.get(p.file.id) ?? [], p])), Array.from(d.values()).slice(0, 50);
  }, [r]);
  return /* @__PURE__ */ l("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? s.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : s.map((d) => {
        ...
        return /* @__PURE__ */ l("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(en, { file: p, active: p.id === i, onOpen: () => o(p) }),
...
function ts(e, t) {
  ...
  const o = [];
  return e.forEach((i) => {
    i.path.toLowerCase().includes(n) && o.push({ file: i, lineNumber: 1, line: i.path }), i.content.split(`
`).some((r, s) => r.toLowerCase().includes(n) ? (o.push({ file: i, lineNumber: s + 1, line: r.trim() || "(blank line)" }), o.length >= 200) : !1);
  }), o.slice(0, 200);
}
function ns(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${Bn(i.language)} file`,
    run: () => e.openWorkbenchFile(i)
  })), o = [...e.commands, ...n].filter((i) => !t || i.label.toLowerCase().includes(t) || i.detail.toLowerCase().includes(t)).slice(0, 12);
  return ne(() => {
    const i = (r) => {
    ...
    /* @__PURE__ */ a(
      "input",
      {
        ...
        onKeyDown: (i) => {
          i.key === "Enter" && o[0] && !o[0].disabled && (o[0].run(), e.onClose());
        }
      }
    ),
    /* @__PURE__ */ a("div", { className: "workbench-command-list", children: o.map((i) => /* @__PURE__ */ l(
      "button",
      {
        disabled: i.disabled,
...
function os(e) {
  const t = e.session ? Ir(e.session) : "pending";
  return /* @__PURE__ */ l("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ l(He, { children: [
...
function is(e) {
  return /* @__PURE__ */ l("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ l("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ a("span", {}),
      /* @__PURE__ */ a("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ a($e, { size: 14 }) })
    ] }),
    /* @__PURE__ */ l("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(Ca, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function rs(e) {
  return /* @__PURE__ */ l("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(Mo, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(po, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a($e, { size: 15 }) })
      ] })
    ] }),
    ...
    ) : /* @__PURE__ */ a(Ca, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function ya({ body: e }) {
  const t = le(() => zr(e), [e]), [n, o] = C(null), i = y((r, s) => {
    (async () => await tt(s) && (o(r), window.setTimeout(() => o((p) => p === r ? null : p), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((r) => {
    if (r.type === "code") {
      ...
      return /* @__PURE__ */ l("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ l("div", { className: "workbench-rich-code-head", children: [
          ...
          /* @__PURE__ */ l("button", { onClick: () => i(r.key, r.body), title: "Copy code block", children: [
            s ? /* @__PURE__ */ a(so, { size: 12 }) : /* @__PURE__ */ a(Dt, { size: 12 }),
            s ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Jn(r.body) }
      },
      r.key
    );
  }) });
...
function ss(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Be(null), [o, i] = C(!0), [r, s] = C(!1), d = le(() => e.chatMessages.map((u) => `${u.id}:${u.status ?? ""}:${u.body.length}`).join("|"), [e.chatMessages]);
  ne(() => {
    const u = n.current;
    u && (o ? (u.scrollTop = u.scrollHeight, s(!1)) : s(!0));
  }, [o, d]);
  const p = y(() => {
    const u = n.current;
    if (!u) return;
  ...
  return /* @__PURE__ */ l("div", { className: "workbench-chat-wrap", children: [
        ...
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((u) => /* @__PURE__ */ l("option", { value: u.id, children: [
              u.title,
              " · ",
              cs(u.lastMessageAt ?? u.updatedAt)
            ] }, u.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ l("div", { ref: n, className: "workbench-chat-transcript", onScroll: p, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ l("div", { children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(ya, { body: u.body }),
        u.gatewayLabel ? /* @__PURE__ */ l(He, { children: [
          /* @__PURE__ */ a("br", {}),
    ...
    /* @__PURE__ */ l("div", { className: "workbench-chat-composer", children: [
      ...
      /* @__PURE__ */ l("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ l("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ l(He, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ l("span", { className: "workbench-chat-chip muted", children: [
            ...
            const w = typeof u.score == "number" ? u.score.toFixed(2) : null, S = typeof u.vectorScore == "number" ? u.vectorScore.toFixed(2) : null, T = typeof u.keywordScore == "number" ? u.keywordScore.toFixed(1) : null, U = [
              u.path,
            ...
            return /* @__PURE__ */ l("span", { className: "workbench-chat-chip", title: U, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(u), disabled: !u.fileId, title: "Reveal context", children: /* @__PURE__ */ a(bn, { size: 13 }) }),
              u.label,
              ...
              u.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(u), title: "Remove context", children: /* @__PURE__ */ a($e, { size: 11 }) }) : null
            ] }, u.id);
          }),
        ...
        /* @__PURE__ */ l("div", { className: "workbench-chat-toolbar compact", children: [
          ...
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(_n, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(Eo, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function In(e, t, n) {
  const o = e.model.trim(), r = [e.gatewayPreference === "auto" ? t : va(e.gatewayPreference)];
  return o && r.push(o), n > 0 && r.push(`${n} memories`), r.join(" · ");
}
function cs(e) {
...
function hs(e) {
  const [t, n] = C([]), [o, i] = C("Loading gateway models…"), [r, s] = C([]), [d, p] = C("Checking embedding capability…");
  ne(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(T.map((U) => ({ id: U.id, gatewayLabel: U.gatewayLabel }))), i(T.length > 0 ? `${T.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (T) {
        if (w.signal.aborted) return;
        n([]), i(`Model discovery failed: ${T instanceof Error ? T.message : String(T)}`);
      }
    })(), () => w.abort();
  }, [e.chatSettings.gatewayPreference, e.host.ai]), ne(() => {
    const w = new AbortController();
    return (async () => {
      const T = vr(e.host);
      if (T) {
      ...
      try {
        const U = await yr(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        s(U.map((G) => ({ id: G.id, gatewayLabel: G.gatewayLabel ?? G.source ?? "AIL" }))), p(U.length > 0 ? `${U.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (U) {
        if (w.signal.aborted) return;
        s([]), p(`Embedding model discovery failed: ${U instanceof Error ? U.message : String(U)}`);
      }
    })(), () => w.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ l("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ l("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ a("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ a($e, { size: 15 }) })
    ] }),
    /* @__PURE__ */ l("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ l("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ l("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          va(e.chatSettings.gatewayPreference),
    ...
    /* @__PURE__ */ l("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(ht), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function Ca({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: o, captureManualCheck: i, openOutputAsFile: r, previewEditFromOutput: s, canPreviewEdit: d = !1, compact: p = !1 }) {
  return /* @__PURE__ */ l("div", { className: `workbench-panel-list ${p ? "compact" : ""}`, children: [
    ...
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((f) => /* @__PURE__ */ l("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ l("div", { className: "workbench-output-head", children: [
        ...
        /* @__PURE__ */ a("button", { onClick: () => void tt(f.body), children: "Copy" }),
...
function us({ edit: e, onApply: t, onOpenAsFile: n, onClose: o }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ l("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ l("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ a("button", { title: "Close", onClick: o, children: /* @__PURE__ */ a($e, { size: 16 }) })
    ] }),
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ l("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ l("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((r) => /* @__PURE__ */ l("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: vs(r.proposedContent) })
      ] }, r.fileId)),
...
function $a({
  ...
  const [d, p] = C([]), [f, b] = C([]), [u, w] = C(null), [S, T] = C(!1), [U, G] = C(null), [q, ee] = C("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [ae, P] = C(null), [B, Q] = C([]), [H, te] = C(() => Br()), [ce, xe] = C([]), [g, O] = C([]), [he, ke] = C("execute"), R = B.find((h) => h.status === "running" || h.status === "canceling") ?? B[0] ?? null, oe = s === "dock", we = le(() => et(q || H?.goal || "", u), [q, H?.goal, u]), re = we.find((h) => h.id === he) ?? we[1] ?? we[0] ?? null, N = r.filter((h) => h.dirty).length, x = i ? `${i.path} · ${i.language} · ${i.content.length.toLocaleString()} chars${i.dirty ? " · dirty" : ""}` : r.length > 0 ? `${r.length} open editor${r.length === 1 ? "" : "s"}${N ? ` · ${N} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", j = le(() => [
    {
    ...
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      p([]), b([]), w(null), G("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    T(!0), G(null);
    try {
      const [h, v, A, W] = await Promise.all([
        e.local?.listTools?.().catch((I) => (t(`Local tool discovery failed: ${I instanceof Error ? I.message : String(I)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((I) => (t(`Skill registry discovery failed: ${I instanceof Error ? I.message : String(I)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((I) => (t(`Resource graph discovery failed: ${I instanceof Error ? I.message : String(I)}`), null)) ?? Promise.resolve(null),
        e.missions?.list?.().catch((I) => (t(`Mission list failed: ${I instanceof Error ? I.message : String(I)}`), [])) ?? Promise.resolve([])
      ]);
      p(h), b(v), w(A), xe(W), t(`Control Tower loaded · ${h.length} tools · ${v.length} skills · ${W.length} missions${A ? ` · ${A.resources.length} resources` : ""}`);
    } catch (h) {
  ...
  ne(() => {
    ...
    return window.addEventListener(Bt, h), () => window.removeEventListener(Bt, h);
  }, [D]);
  const ie = y((h) => {
    const v = Wr(h);
    te(v), Ze(v), O([{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "mission.resume",
      message: `Mission resumed from Control Tower: ${v.rootPath ?? v.name}`,
      data: { runCount: h.runCount ?? 0, taskCount: h.taskCount ?? 0 }
    }]), ee(h.goal || `Continue mission ${h.title}. Review MISSION.md, TASKS.md, RESOURCES.md, and runs/ before acting.`), t(`Resumed mission: ${h.rootPath}`);
  }, [t]), F = y(async (h, v, A = []) => {
    const W = [
      ...g,
      ...A,
      ...
      { path: "MISSION.md", content: Wt(h, u, i, r, v) },
      { path: "MISSION.json", content: ga(h, u, v) },
      { path: "RESOURCES.md", content: Ut(u) },
      { path: "TASKS.md", content: fa(I) },
      { path: "HANDOFF.md", content: ba(h) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: W.map((E) => JSON.stringify(E)).join(`
`) + `
` }
    ];
    if (h.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: h.rootPath, files: me });
    else if (h.handle) {
      await Sn(h.handle, "runs");
      for (const E of me) await An(h.handle, E.path, E.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    O(W), Ze(h);
  }, [i, e.missions, g, r, u]), Z = y(async (h, v = {}) => {
    if (H)
      return await F(H, h), H;
    const A = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, W = h.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let I = null;
    if (e.missions?.create) {
      const E = await e.missions.create({ title: A, goal: W });
      I = {
        missionId: E.missionId,
        title: E.title,
        goal: E.goal,
      ...
      I = {
        handle: E,
        name: E.name,
        missionId: `mission-${Date.now()}-${Hr(E.name)}`,
        title: A,
        goal: W,
        source: "browser"
      };
    ...
    if (!I) return null;
    const me = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${I.rootPath ?? I.name}` };
    return te(I), Ze(I), O([me]), await F(I, W, [me]), t(`Mission pack ready in ${I.rootPath ?? I.name}`), I;
  }, [e.missions, H, t, F]), Ne = y(async (h) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const v = q.trim() || `Open ${h.label} from Atomek with current context.`, A = h.kind === "ai-cli" ? await Z(v) : H;
      await e.local.openTerminal({
        toolId: h.id,
        command: h.command,
        cwd: A?.rootPath ?? Pn(i),
        prompt: A ? `Atomek mission pack ready at ${A.rootPath ?? A.name}. Read MISSION.md and RESOURCES.md. ${x}` : `Opened from Atomek. ${x}`
      }), t(h.kind === "ai-cli" ? `Opened ${h.label} in Tytus Terminal with mission context. Press Enter there to start it.` : "Opened Tytus Terminal");
    } catch (v) {
      t(`Terminal launch failed: ${v instanceof Error ? v.message : String(v)}`);
    }
  }, [i, x, Z, e.local, q, H, t]), Te = y(async () => {
    try {
      await Z(q.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (h) {
      t(`Mission folder setup failed: ${h instanceof Error ? h.message : String(h)}`);
    }
  }, [Z, q, t]), Ie = y(async (h, v, A, W = H) => {
    if (!W) return;
    const me = `${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${h.id}.md`, E = `runs/${me}`;
    if (W.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: W.rootPath, files: [{ path: E, content: v }] });
    else if (W.handle) {
      const ge = await Sn(W.handle, "runs");
      await An(ge, me, v);
    }
    await F(W, q, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${h.label} exited ${A}; transcript saved to ${E}`,
      data: { toolId: h.id, exitCode: A, transcript: E }
    }]);
  }, [e.missions, q, H, F]), Ae = y(async (h) => {
    ...
    let A = null;
    try {
      A = await Z(v), A && await F(A, v, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${h.label} background review started`,
        data: { toolId: h.id, taskId: re?.id ?? "manual", taskTitle: re?.title ?? "Manual run" }
      }]);
    } catch (I) {
    ...
    P(h.id);
    const W = `local-run-${Date.now()}-${h.id}`;
    Q((I) => [{
      id: W,
      toolId: h.id,
      label: h.label,
      ...
      startedAt: Date.now(),
      taskId: re?.id ?? "manual",
      taskTitle: re?.title ?? "Manual run",
      lines: [`[Atomek] Starting ${h.label} local job for ${re?.title ?? "manual run"}…`]
    }, ...I].slice(0, 6));
    try {
...
`)
      }), me = A?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${h.id}.md` : void 0, E = [
        `[Atomek] Started ${h.label} local job ${I.id}`,
        re ? `[Atomek] Task: ${re.title} (${re.id})` : "[Atomek] Task: manual",
        A?.rootPath ? `[Atomek] Mission: ${A.rootPath}` : ""
      ].filter(Boolean);
      Q((z) => z.map((K) => K.id === W ? { ...K, jobId: I.id, transcriptPath: I.transcriptPath ?? me } : K));
      const ge = (z) => {
        Q((K) => K.map((se) => se.id === W ? z(se) : se));
      };
      e.local.streamJob(I.id, {
        onLog: (z) => {
          E.push(z), ge((K) => ({ ...K, lines: E.slice(-500) }));
        },
        onDone: (z) => {
          z && (E.push(z), ge((K) => ({ ...K, lines: E.slice(-500) })));
        },
        onFail: (z) => {
          E.push(`[FAIL] ${z}`), ge((K) => ({
            ...K,
            ...
            finishedAt: Date.now(),
            lines: E.slice(-500)
          })), o(`${h.label} local job failed`, E.join(`
`)), Ie(h, E.join(`
`), -1, A).catch((K) => {
            t(`Mission transcript save failed: ${K instanceof Error ? K.message : String(K)}`);
          }), P(null);
        },
        onExit: (z) => {
          const K = [
...
`);
          ge((se) => ({
            ...
            finishedAt: Date.now(),
            lines: E.slice(-500)
          })), o(`${h.label} local job`, K), Ie(h, K, z, A).catch((se) => {
            t(`Mission transcript save failed: ${se instanceof Error ? se.message : String(se)}`);
          }), P(null);
        },
        onError: () => t(`Local job stream issue for ${h.label}`)
      }), t(`Started ${h.label} mission run${re ? ` · ${re.title}` : ""}`);
    } catch (I) {
      P(null), Q((me) => me.map((E) => E.id === W ? {
        ...E,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...E.lines, `[Atomek] Failed to start: ${I instanceof Error ? I.message : String(I)}`]
      } : E)), t(`Local job failed to start: ${I instanceof Error ? I.message : String(I)}`);
    }
  }, [i, Z, e.local, q, r, u, o, Ie, re, t, F]), ye = y(async (h) => {
    if (!h.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    Q((v) => v.map((A) => A.id === h.id ? { ...A, status: "canceling", lines: [...A.lines, "[Atomek] Cancel requested…"] } : A));
    try {
      await e.local.cancelJob(h.jobId), t(`Cancel requested for ${h.label}`);
    } catch (v) {
      Q((A) => A.map((W) => W.id === h.id ? { ...W, status: "running", lines: [...W.lines, `[Atomek] Cancel failed: ${v instanceof Error ? v.message : String(v)}`] } : W)), t(`Local job cancel failed: ${v instanceof Error ? v.message : String(v)}`);
    }
  }, [e.local, t]), X = y(async (h) => {
    const v = [
      `Use Tytus resource "${h.label}" (${h.kind}) for the next mission step.`,
      `Capabilities: ${h.capabilities.join(", ") || "status only"}.`,
      `Sandbox: ${h.sandbox}. Trust: ${h.trustTier}.`,
      h.allowedRoots.length ? `Allowed roots: ${h.allowedRoots.join(", ")}` : "No direct roots exposed.",
...
`);
    ee(v), H && await F(H, v, [{
      ...
      message: `Selected resource ${h.label}`,
      data: { resourceId: h.id, kind: h.kind, status: h.status }
    }]), t(`Selected ${h.label} for mission`);
  }, [H, t, F]), J = y((h) => {
    const v = h.setupAction, A = v?.commandPreview ? `${v.label}: ${v.commandPreview}` : v?.deepLink ? `${v.label}: ${v.deepLink}` : v?.label ?? `${h.label} needs setup`;
    t(A), tt(v?.commandPreview ?? v?.deepLink ?? A);
  }, [t]);
  return /* @__PURE__ */ l("aside", { className: oe ? "workbench-agent-dock" : "workbench-sidebar", children: [
    oe ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "CONTROL TOWER" }),
    /* @__PURE__ */ l("div", { className: oe ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ l("div", { className: "workbench-computer-hero", children: [
        /* @__PURE__ */ a(Vt, { size: 18 }),
        /* @__PURE__ */ l("div", { children: [
          /* @__PURE__ */ a("strong", { children: oe ? "Mission Runs" : "Tytus Control Tower" }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-context-card mission", children: [
        /* @__PURE__ */ a("strong", { children: H ? H.title : "No mission folder selected" }),
        /* @__PURE__ */ a("span", { children: H ? `${H.rootPath ?? H.name} · ${H.source} · ${g.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-actions", children: [
        /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
          Te();
        }, children: H ? "Refresh mission pack" : "Start mission pack" }),
        /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => H && void F(H, q), disabled: !H, children: "Rewrite context files" })
      ] }),
      ce.length ? /* @__PURE__ */ l(He, { children: [
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
        /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: ce.slice(0, oe ? 3 : 5).map((h) => /* @__PURE__ */ l(
          "button",
          {
            className: `workbench-mission-row ${H?.missionId === h.missionId ? "active" : ""}`,
            onClick: () => ie(h),
            title: h.rootPath,
            children: [
      ...
      u?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: u.warnings.slice(0, 3).map((h) => /* @__PURE__ */ l("span", { children: [
        h.code,
        ": ",
        h.message
      ] }, `${h.code}-${h.resourceId ?? h.message}`)) }) : null,
      ...
      /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: j.map((h) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ee(h.prompt), children: h.label }, h.label)) }),
      /* @__PURE__ */ a(
        "textarea",
        {
      ...
      /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: we.map((h, v) => /* @__PURE__ */ l(
        "button",
        {
          className: `workbench-task-card ${h.status} ${re?.id === h.id ? "active" : ""}`,
          onClick: () => {
            ke(h.id), ee(h.prompt);
          },
          title: "Load this task prompt",
          children: [
            /* @__PURE__ */ a("span", { children: v + 1 }),
            /* @__PURE__ */ a("strong", { children: h.title }),
            /* @__PURE__ */ a("em", { children: h.resourceHint })
          ]
        },
        h.id
      )) }),
      R ? /* @__PURE__ */ l("div", { className: "workbench-agent-run", children: [
        /* @__PURE__ */ l("header", { children: [
          /* @__PURE__ */ l("div", { children: [
          ...
          /* @__PURE__ */ l("div", { className: "workbench-agent-run-actions", children: [
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => o(`${R.label} local job`, R.lines.join(`
`)), disabled: R.lines.length === 0, children: "Save output" }),
            R.jobId && (R.status === "running" || R.status === "canceling") ? /* @__PURE__ */ l("button", { className: "workbench-button-subtle danger", onClick: () => {
              ye(R);
            }, disabled: R.status === "canceling", children: [
              /* @__PURE__ */ a(_n, { size: 12 }),
              " ",
...
`) || "[waiting for output]" })
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list compact", children: [
        !u && !S ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
        u?.resources.slice(0, oe ? 8 : 5).map((h) => /* @__PURE__ */ l("div", { className: "workbench-resource-row", children: [
          /* @__PURE__ */ l("div", { children: [
            ...
            /* @__PURE__ */ l("span", { children: [
              ...
              h.trustTier,
              " · ",
              h.capabilities.slice(0, 3).join(", ")
            ] })
          ] }),
          /* @__PURE__ */ l("div", { className: "workbench-resource-row-actions", children: [
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${h.status}`, children: h.status }),
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              X(h);
            }, children: "Use" }),
            h.status === "needs-setup" || h.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => J(h), children: "Setup" }) : null
          ] })
        ] }, h.id))
      ] }),
      ...
      /* @__PURE__ */ l("div", { className: "workbench-computer-list", children: [
        d.length === 0 && !S ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
        d.map((h) => /* @__PURE__ */ l("div", { className: "workbench-computer-card", children: [
          ...
          h.description ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: h.description }) : null,
          /* @__PURE__ */ l("div", { className: "workbench-computer-actions", children: [
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
              Ne(h);
            }, disabled: h.status !== "available", title: "Launch this tool in the real Tytus Terminal with mission context prefilled. Atomek types the command; you press Enter to start it.", children: bs(h) }),
            h.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              Ae(h);
            }, disabled: h.status !== "available" || ae !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: gs(h, ae) }) : null
          ] })
        ] }, h.id))
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
      /* @__PURE__ */ l("div", { className: "workbench-computer-list", children: [
        f.length === 0 && !S ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
        f.map((h) => /* @__PURE__ */ l("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ l("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ l("div", { children: [
              /* @__PURE__ */ a("strong", { children: h.title }),
              /* @__PURE__ */ l("span", { children: [
          ...
          /* @__PURE__ */ a("p", { className: "workbench-muted", children: h.description }),
          h.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: h.triggers.slice(0, 4).map((v) => /* @__PURE__ */ a("span", { children: v }, v)) }) : null,
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
            n(h);
          }, disabled: h.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
        ] }, h.id))
      ] })
    ] })
  ] });
...
function ks({ status: e, file: t, cursor: n, fileCount: o, dirtyCount: i }) {
  return /* @__PURE__ */ l("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: Bn(t.language) })
  ] });
}
function Et(e, t) {
...
function xs() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : ht.gatewayPreference,
