...
import { jsx as a, jsxs as d, Fragment as We } from "react/jsx-runtime";
import { forwardRef as la, createElement as fn, useState as z, useRef as Qe, useCallback as x, useEffect as se, lazy as Cr, useMemo as oe, Suspense as Sr } from "react";
/**
 ...
 */
const Hr = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), jr = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), Yn = (e) => {
  const t = jr(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, da = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), Wr = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const Ir = [["path", { d: "M20 6 9 17l-5-5", key: "1gmf2c" }]], qr = ae("check", Ir);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
 * See the LICENSE file in the root directory of this source tree.
 */
const Rr = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], It = ae("chevron-down", Rr);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const ys = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function ws() {
  const e = Tn();
  return !!(e?.showOpenFilePicker && e.showDirectoryPicker);
}
async function gs() {
  const e = Tn()?.showOpenFilePicker;
  if (!e) return ga(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((r) => Xn(r, r.name, "local-file")));
  return Lt(n.filter(Boolean));
}
async function ks() {
  const e = Tn()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await ga(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await Ln(t, t.name, n, 0, 320), { name: t.name, handle: t, files: Lt(n) };
}
async function Ps(e) {
  const t = await Promise.all(e.map((n) => Xn(n, n.name, "local-file")));
  return Lt(t.filter(Boolean));
}
async function Zn(e) {
  const t = [];
  return await Ln(e, e.name, t, 0, 320), { name: e.name, handle: e, files: Lt(t) };
}
async function an(e, t = "readwrite") {
  try {
...
async function Ln(e, t, n, r, s) {
  if (!(n.length >= s || r > 8))
    for await (const i of e.values()) {
      if (n.length >= s) break;
      if (i.kind === "directory") {
        if (ys.has(i.name)) continue;
        await Ln(i, `${t}/${i.name}`, n, r + 1, s);
        continue;
      }
      const o = i, l = await Xn(o, `${t}/${o.name}`, "local-folder");
      l && n.push(l);
    }
}
function ga(e) {
...
`).map((l) => `<tr>${l.split("|").filter(Boolean).map(
      (y, b) => `<td style="padding:8px 12px;border:1px solid var(--border-default);font-size:13px;background:${b % 2 === 0 ? "transparent" : "var(--bg-hover)"};color:var(--text-primary)">${y.trim()}</td>`
    ).join("")}</tr>`).join("");
...
const st = "atomek:default", Ns = 3e3, An = "tytus.atomek.threadTitleOverrides", xa = "tytus.atomek.selectedThreadId", Kn = {
  available: !1,
  source: "none",
  label: "Tytus AI unavailable",
  reason: "host.ai is not available in this Tytus build."
}, Pt = (e) => e.role !== "user" && e.role !== "assistant" ? null : {
  id: e.id,
  role: e.role,
  body: e.body,
  status: e.status,
...
}, zs = (e, t = Ns) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Un = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Ts = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", Bn = (e) => ({
  id: e.id,
  title: e.title,
  kind: Ts(e.kind),
  body: e.body,
...
`)
  ...
  try {
    const n = On();
    n[e] = t, localStorage.setItem(An, JSON.stringify(n));
  } catch {
  }
}, As = (e) => {
  try {
    const t = On();
    delete t[e], localStorage.setItem(An, JSON.stringify(t));
  } catch {
  }
}, Gn = (e) => {
  const t = On();
  ...
  try {
    return localStorage.getItem(xa)?.trim() || null;
  } catch {
    return null;
  }
}, Ue = (e) => {
...
function Cs({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const s = e.ai, [i, o] = z(null), [l, p] = z([]), [y, b] = z([]), [h, w] = z([]), [C, X] = z([]), [K, E] = z(Kn), [q, ue] = z(!1), re = Qe(!0), j = Qe(null), B = x(async () => {
    if (!s) {
      E(Kn);
      return;
    }
    try {
      ...
      E({
        ...
        source: "none",
        label: "Tytus AI unavailable",
        reason: m instanceof Error ? m.message : String(m)
      });
    }
  }, [s]), R = x(async (m) => {
    if (s)
      try {
        const g = await s.listArtifacts({ threadId: m });
        if (!re.current) return;
        w(g.map(Bn));
      } catch (g) {
        r(`AI artifacts unavailable: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [s, r]), M = x(async (m) => {
    if (s)
      try {
        const g = await s.listMessages(m);
        if (!re.current) return;
        const T = l.find((F) => F.id === m) ?? null;
        T && o(T), Ue(m), b(g.map(Pt).filter(Boolean)), X([]), await R(m);
      } catch (g) {
        r(`Load chat failed: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [s, R, r, l]), ce = x(async () => {
    if (s)
      try {
        const m = Gn(await s.listThreads({ workspaceKey: st, status: "active" })), g = Os(), T = m.find((ie) => ie.id === g) ?? m[0] ?? await s.createThread({ workspaceKey: st, title: "Atomek chat" });
        if (!re.current) return;
        p(m[0] ? m : [T]), o(T), Ue(T.id);
        const F = await s.listMessages(T.id);
        if (!re.current) return;
        b(F.map(Pt).filter(Boolean)), await R(T.id);
      } catch (m) {
        r(`AI chat unavailable: ${m instanceof Error ? m.message : String(m)}`);
      }
  }, [s, R, r]);
  se(() => (re.current = !0, B(), ce(), () => {
    re.current = !1;
  }), [ce, B]);
  const D = x(async () => {
    if (!s) return null;
    if (i) return i;
    const m = await s.createThread({ workspaceKey: st, title: "Atomek chat" });
    return re.current && (o(m), Ue(m.id), p((g) => [m, ...g.filter((T) => T.id !== m.id)])), m;
  }, [s, i]), he = x(async () => {
    if (!s) return;
    const m = await s.createThread({ workspaceKey: st, title: "Atomek chat" });
    o(m), Ue(m.id), p((g) => [m, ...g.filter((T) => T.id !== m.id)]), b([]), w([]), X([]), r("New AI chat created");
  }, [s, r]), ve = x(async (m, g) => {
    if (!s) return;
    const T = g.trim();
    if (T)
      try {
        const F = s.updateThread, ie = l.find((ke) => ke.id === m) ?? i, H = typeof F == "function" ? await F({ threadId: m, title: T }) : { ...ie ?? await D(), id: m, title: T, updatedAt: Date.now() };
        typeof F != "function" && Ls(m, T), p((ke) => ke.map((Ne) => Ne.id === H.id ? H : Ne)), o((ke) => ke?.id === H.id ? H : ke), r(typeof F == "function" ? `Renamed chat: ${H.title}` : `Renamed chat locally: ${H.title}`);
      } catch (F) {
        r(`Rename chat failed: ${F instanceof Error ? F.message : String(F)}`);
      }
  }, [s, D, r, i, l]), Xe = x(async (m) => {
    if (s)
      try {
        await s.deleteThread(m), As(m);
        const g = l.filter((T) => T.id !== m);
        if (p(g), i?.id === m) {
          const T = g[0] ?? await s.createThread({ workspaceKey: st, title: "Atomek chat" });
          o(T), Ue(T.id), p((ie) => ie.some((H) => H.id === T.id) ? ie : [T, ...ie]);
          const F = await s.listMessages(T.id);
          b(F.map(Pt).filter(Boolean)), await R(T.id);
        }
        r("Deleted AI chat");
      } catch (g) {
        r(`Delete chat failed: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [s, R, r, i?.id, l]), Se = x(async (m) => {
    if (!s) return null;
    try {
      const g = await D();
      if (!g) return null;
      Ue(g.id);
      const T = await s.createArtifact({
        threadId: g.id,
        messageId: m.messageId ?? null,
        title: m.title?.trim() || Un(m.body, "Atomek artifact"),
        kind: m.kind ?? "markdown",
        body: m.body
      }), F = Bn(T);
      return w((ie) => [F, ...ie.filter((H) => H.id !== F.id)]), r(`Saved AI artifact: ${F.title}`), F;
    } catch (g) {
      return r(`Save artifact failed: ${g instanceof Error ? g.message : String(g)}`), null;
    }
  }, [s, D, r]), Le = x(async (m) => {
    if (s)
      try {
        await s.deleteArtifact(m), w((g) => g.filter((T) => T.id !== m)), r("Deleted AI artifact");
      } catch (g) {
        r(`Delete artifact failed: ${g instanceof Error ? g.message : String(g)}`);
      }
  }, [s, r]), L = x(async (m) => {
    if (!s) return null;
    try {
      const g = await s.writeMemory({
        title: m.title?.trim() || Un(m.body, "Atomek memory"),
        body: m.body,
        metadata: {
          source: "atomek",
          messageId: m.messageId ?? null
      ...
      return X((T) => [g, ...T.filter((F) => F.id !== g.id)].slice(0, 5)), r(`Remembered: ${g.title}`), g;
    } catch (g) {
      return r(`Remember failed: ${g instanceof Error ? g.message : String(g)}`), null;
    }
  }, [s, r]), _ = x(async (m) => {
    if (!s) return [];
    const g = await s.searchMemory({ query: m, limit: 5 });
    return re.current && X(g), g;
  }, [s]), be = x(async (m, g = {}) => {
    const T = m.trim();
    if (!T || !s) return null;
    ue(!0);
    ...
    try {
      ...
      if (!H) return null;
      Ue(H.id);
      const ke = await _(T).catch(() => []), Ne = Xs(ke), Ve = g.requestContext ?? t, He = Ne ? [...Ve, Ne] : [...Ve];
      let Pe = null;
      for await (const ee of s.sendMessage({
        threadId: H.id,
        body: T,
        ...
        if (ee.type === "message_created") {
          const te = Pt(ee.message);
          if (!te) continue;
          te.role === "assistant" && (Pe = te.id), b(($) => [...$.filter((ne) => ne.id !== te.id), te]);
        }
        if (ee.type === "token" && (Pe = ee.messageId, b((te) => te.some(($) => $.id === ee.messageId) ? te.map(
          ($) => $.id === ee.messageId ? { ...$, body: ee.body, status: "streaming" } : $
        ) : [...te, {
          id: ee.messageId,
          role: "assistant",
          body: ee.body,
          status: "streaming",
          createdAt: Date.now()
        }])), ee.type === "message_updated" || ee.type === "done") {
          const te = Pt(ee.message);
          if (!te) continue;
          te.role === "assistant" && (F = te), b(($) => $.map((ne) => ne.id === te.id ? te : ne)), te.gatewayLabel && r(`AI answered via ${te.gatewayLabel}`);
        }
        if (ee.type === "run_failed") {
          const te = ie.signal.aborted;
          r(te ? "AI response stopped" : `AI failed: ${ee.error}`), Pe && b(($) => $.map((ne) => ne.id !== Pe ? ne : te ? { ...ne, status: "complete", error: void 0, body: ne.body || "Stopped by user." } : { ...ne, status: "error", error: ee.error, body: ee.error }));
        }
      }
      const ye = Gn(await s.listThreads({ workspaceKey: st, status: "active" }).catch(() => []));
      return re.current && ye.length > 0 && p(ye), B(), F;
    } catch (H) {
      return H instanceof DOMException && H.name === "AbortError" ? (r("AI response stopped"), F) : (r(`AI failed: ${H instanceof Error ? H.message : String(H)}`), null);
    } finally {
      j.current === ie && (j.current = null), ue(!1);
    }
  }, [s, n.gatewayPreference, n.model, D, _, B, t, r]), fe = x(() => {
    j.current?.abort(), r("Stopping AI response…");
  }, [r]);
  return {
    aiStatus: K,
    artifacts: h,
...
const Ss = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Hs = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, js = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: s }) => {
  const i = new Set(t), o = e.map((l) => {
    ...
    return {
      ...
      language: l.language,
      version: r[l.id] ?? 1,
      contentHash: Hs(l.content),
      dirty: l.dirty,
      source: l.source,
      selection: p && s ? s : void 0,
      updatedAt: Date.now(),
      open: i.has(l.id),
      active: p
    };
  ...
  return {
    ...
    activeDocumentId: n,
    openDocumentIds: t.filter((l) => o.some((p) => p.id === l))
  };
}, Ws = "active-file", Ms = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", Vs = (e, t = "file") => `${t}:${e.id}`, Ds = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let s = [];
  return t.scope === "active-selection" ? r?.selection ? s = [Fs(r, r.selection, !0)] : r && (s = [Ct(r, !0, "Active file")]) : t.scope === "active-file" ? r && (s = [Ct(r, !0, "Active file")]) : t.scope === "open-editors" ? s = e.openDocumentIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ct(i, !0, "Open editor")) : t.scope === "selected-files" && (s = t.selectedFileIds.map((i) => e.byId.get(i)).filter(Boolean).map((i) => Ct(i, !1, "Selected file"))), qs(s).filter((i) => !n.has(i.id));
}, qs = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, Rs = 4e3, Es = 8e3, Ys = 4e3, Js = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Zs = (e, t, n) => {
  const r = Is(e, n), s = [], i = new Map(t.map((o) => [o.id, o]));
  for (const o of r) {
    if (!o.fileId) continue;
    const l = i.get(o.fileId), p = e.byId.get(o.fileId);
    if (!l || !p || !o.includeBody) continue;
    const y = [
      `Path: ${l.path}`,
      `Language: ${l.language}`,
      `Version: ${p.version}`,
      ...
      o.range ? `Range: ${Qn(o.range)}` : null
    ].filter(Boolean).join(`
`), b = o.kind === "selection" && o.range ? Ks(l.content, o.range) : l.content, h = o.kind === "selection" ? Rs : o.label.startsWith("Open editor") ? Ys : Es;
    s.push({
...
`));
}, Qn = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, $n = 75e4, Us = 2800, Bs = 240, Gs = 80, Qs = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${Ge(t.content)}`).sort().join("")
), pn = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? $n) || e.content.length > (t.maxFileBytes ?? $n) ? "huge" : !t.includeDirty && e.dirty ? null : ri(e.path) ? "vendor" : si(e) ? "binary" : null;
}, _n = (e, t = {}) => {
  const n = Date.now(), r = [], s = [], i = [];
  for (const o of e) {
    const l = o.size ?? o.content.length, p = pn(o, t);
    if (p) {
      s.push({ fileId: o.id, name: o.name, path: o.path, language: o.language, reason: p, size: l });
      continue;
    }
    const y = Ge(o.content), b = _s(o, y, n, t);
    i.push(...b), r.push({
      fileId: o.id,
      name: o.name,
      path: o.path,
...
}, _s = (e, t = Ge(e.content), n = Date.now(), r = {}) => {
  const s = Math.max(500, r.maxChunkChars ?? Us), i = Math.min(Math.max(0, r.chunkOverlapChars ?? Bs), Math.floor(s / 2)), o = Math.max(1, r.maxChunksPerFile ?? Gs), l = ni(e.content), p = [];
  let y = 0, b = 0;
  for (; y < e.content.length && p.length < o; ) {
    const h = ti(e.content, y, Math.min(e.content.length, y + s)), w = e.content.slice(y, h).trim();
    if (w.length > 0) {
      const C = ai(l, y, h);
      p.push({
        id: `${e.id}:${t}:${b}:${C.startLineNumber}-${C.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const r = ea(e, t), s = ea(e, Math.max(t, n - 1));
  return {
    startLineNumber: r + 1,
    startColumn: t - e[r] + 1,
    endLineNumber: s + 1,
  ...
  return 0;
}, ri = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => Qs.has(t)), si = (e) => {
  const t = e.name.includes(".") ? e.name.split(".").pop()?.toLowerCase() : void 0;
  if (t && $s.has(t) || e.content.includes("\0")) return !0;
...
class ii {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = _n(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = _n(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return oi(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const it = (e = [], t = {}) => new ii(e, t), oi = (e, t, n = {}) => {
  const r = new Map(t.map((l) => [l.id, l])), s = e.byFileId, i = [];
  for (const l of e.files) {
    const p = r.get(l.fileId);
    if (!p) {
      i.push({ fileId: l.fileId, path: l.path, status: "deleted", indexedHash: l.hash });
      continue;
    }
    if (pn(p, n)) {
      i.push({ fileId: l.fileId, path: p.path, status: "skipped-now", indexedHash: l.hash, currentHash: Ge(p.content) });
      continue;
    }
    const b = Ge(p.content);
    b !== l.hash ? i.push({ fileId: l.fileId, path: p.path, status: "changed", indexedHash: l.hash, currentHash: b }) : p.dirty !== l.dirty && i.push({ fileId: l.fileId, path: p.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: b });
  }
  for (const l of t)
    !s.has(l.id) && !pn(l, n) && i.push({ fileId: l.id, path: l.path, status: "new", currentHash: Ge(l.content) });
  const o = e.signature !== Na(t);
  return { stale: i.length > 0, signatureChanged: o, files: i };
}, ci = 8, li = 12e3, za = (e, t, n = {}, r) => {
  const s = hi(t);
  if (s.length === 0) return [];
  const i = new Set(r?.files.map((b) => b.fileId) ?? []), o = e.chunks.filter((b) => n.includeDirty !== !1 || !b.dirty).map((b) => fi(b, s)).filter((b) => b.score >= (n.minScore ?? 1)).sort((b, h) => h.score - b.score || b.chunk.path.localeCompare(h.chunk.path) || b.chunk.ordinal - h.chunk.ordinal), l = [], p = /* @__PURE__ */ new Set();
  let y = n.maxChars ?? li;
  for (const b of o) {
    if (l.length >= (n.limit ?? ci) || y <= 0) break;
    const h = `${b.chunk.fileId}:${b.chunk.ordinal}`;
    if (p.has(h)) continue;
    p.add(h);
    const w = b.chunk.text.length > y ? `${b.chunk.text.slice(0, Math.max(0, y - 28))}
[...context clipped...]` : b.chunk.text;
    l.push(di(b, w, i.has(b.chunk.fileId))), y -= w.length;
  }
  return l;
}, di = (e, t = e.chunk.text, n = !1) => {
...
`), hi = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const p of t) {
    const y = sn(n, p), b = sn(r, p), h = sn(s, p), w = i === p ? 1 : 0;
    y + b + h + w !== 0 && (l.push(p), o += Math.min(y, 12), o += b * 4, o += h * 6, o += w * 3, (n.includes(`function ${p}`) || n.includes(`const ${p}`) || n.includes(`class ${p}`) || n.includes(`type ${p}`)) && (o += 3));
  }
  ...
  const n = e.toLowerCase(), r = t.map((p) => n.indexOf(p)).filter((p) => p >= 0).sort((p, y) => p - y)[0] ?? 0, s = Math.max(0, r - 120), i = Math.min(e.length, r + 260), o = s > 0 ? "…" : "", l = i < e.length ? "…" : "";
  return `${o}${e.slice(s, i).replace(/\s+/g, " ").trim()}${l}`;
}, pi = (e, t = {}) => {
  const { autoRefresh: n = !1, ...r } = t, s = Qe(e), i = Qe(null);
  i.current || (i.current = it(e, r));
  const [o, l] = z(() => i.current?.getSnapshot() ?? it(e, r).getSnapshot());
  s.current = e;
  const p = x((w = s.current) => {
    const C = (i.current ?? it([], r)).refresh(w, r);
    return l(C), C;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), y = x((w = s.current) => {
    const C = (i.current ?? it([], r)).update(w, r);
    return l(C), C;
  }, [p]);
  se(() => {
    n && p(e);
  }, [n, e, p]);
  const b = (i.current ?? it([], r)).staleReport(e), h = x((w, C = {}) => {
    const X = i.current ?? it([], r);
    return za(X.getSnapshot(), w, C, X.staleReport(s.current));
  }, []);
  return { snapshot: o, staleReport: b, isStale: b.stale, refresh: p, update: y, retrieve: h };
}, bi = "tytus.atomek.semanticVector:v1", Ta = "__gateway_default__", Xa = (e) => e?.trim() || Ta, vi = (e) => {
  const t = Xa(e);
  return t === Ta ? "gateway default" : t;
}, La = (e, t, n) => [
  bi,
  ...
  if (!Aa()) return null;
  try {
    const r = La(e, t, n), s = window.localStorage.getItem(r);
    if (!s) return null;
  ...
  if (!Aa()) return null;
  try {
    const t = La(e.appId, e.modelAlias, { id: e.chunkId, hash: e.contentHash }), n = { ...e, key: t };
    return window.localStorage.setItem(t, JSON.stringify(n)), n;
  } catch {
    return null;
  }
}, xt = 8, gi = 12e3, ki = 180, Pi = async (e, t, n, r, s = {}, i, o) => {
  const l = za(t, n, { ...s, limit: Math.max(s.limit ?? xt, 16) }, i), p = e.ai;
  if (typeof p?.embedText != "function")
    return { hits: l.slice(0, s.limit ?? xt), mode: "keyword", reason: "Semantic index unavailable — using keyword retrieval.", embeddedChunks: 0 };
  try {
    const y = Xa(r.embeddingModel), b = await p.embedText({
      input: n,
    ...
    const w = new Map(l.map((j) => [j.chunkId, j])), C = Ai(n), X = /* @__PURE__ */ new Map();
    t.chunks.filter((j) => s.includeDirty !== !1 || !j.dirty).slice(0, ki).forEach((j) => X.set(j.id, j)), l.forEach((j) => {
      const B = t.byChunkId.get(j.chunkId);
      B && X.set(B.id, B);
    });
    const K = Array.from(X.values()), E = [];
    let q = 0;
    for (const j of K) {
      o?.aborted;
      const B = yi(e.appId, y, j) ?? await Ni(p, e.appId, y, j, r, o);
      if (!B) continue;
      B.updatedAt > Date.now() - 1e3 && (q += 1);
      const R = Xi(h, B.vector);
      if (!Number.isFinite(R)) continue;
      const ce = w.get(j.id)?.score ?? Oi(j, C), D = Li(ce) * 0.42 + Math.max(0, R) * 0.58;
      E.push({ chunk: j, keywordScore: ce, vectorScore: R, score: D, matchedTerms: C.filter((he) => j.text.toLowerCase().includes(he)) });
    }
    if (E.length === 0) return { hits: l.slice(0, s.limit ?? xt), mode: "vector-fallback", reason: "Semantic retrieval produced no vectors — using keyword retrieval.", embeddedChunks: q };
    const ue = new Set(i?.files.map((j) => j.fileId) ?? []), re = E.sort((j, B) => B.score - j.score || B.keywordScore - j.keywordScore || j.chunk.path.localeCompare(B.chunk.path) || j.chunk.ordinal - B.chunk.ordinal).slice(0, Math.max(s.limit ?? xt, 1)).map((j) => zi(j, ue.has(j.chunk.fileId)));
    return {
      hits: Ti(re, s.maxChars ?? gi),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${vi(r.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: q
    };
  } catch (y) {
    return {
      hits: l.slice(0, s.limit ?? xt),
      mode: "vector-fallback",
      reason: `Semantic retrieval failed (${y instanceof Error ? y.message : String(y)}) — using keyword retrieval.`,
      embeddedChunks: 0
    };
  }
}, xi = (e) => e.map((t) => ({
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: Ci(e.chunk.text, e.matchedTerms),
    score: e.score,
...
}, Li = (e) => Math.min(1, Math.max(0, e / 24)), Ai = (e) => Array.from(new Set(
  e.toLowerCase().split(/[^a-z0-9_.$/-]+/i).map((t) => t.trim()).filter((t) => t.length >= 2)
)), Oi = (e, t) => {
  const n = `${e.path}
${e.name}
...
}, Si = /```([^\n`]*)\n([\s\S]*?)```/g, Hi = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function Cn(e) {
  return Array.from(e.matchAll(Si)).map((t) => Ca(t[1] ?? "", Di(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Ca(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, s = [];
  for (const i of n) {
    const o = i.match(/^([a-z0-9_-]+)=(.+)$/i);
    o ? r[o[1].toLowerCase()] = Ii(o[2]) : s.push(i.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: s, attrs: r, content: t };
}
function Ce(e, t) {
  return (typeof e == "string" ? Ca(e, "").flags : e.flags).includes(t.toLowerCase());
}
function Sa(e) {
  return [...Cn(e).filter((r) => Ce(r, "diff") || Ce(r, "patch")).map((r) => r.content), e].filter((r, s, i) => ji(r) && i.findIndex((o) => o === r) === s);
}
function ji(e) {
...
`), r = [];
  ...
  }), r.length === 0) return [{ raw: t, ...on(t) }];
  const s = Array.from(new Set(r)).sort((o, l) => o - l), i = [];
  for (let o = 0; o < s.length; o += 1) {
    const l = s[o], y = s.find((h) => h > l && n[h].startsWith("diff --git ")) ?? n.length, b = n.slice(l, y).join(`
`);
    /^@@\s+-\d+/m.test(b) && i.push({ raw: b, ...on(b) });
  }
  return i.length > 0 ? i : [{ raw: t, ...on(t) }];
}
function on(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), r = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), s = qe(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, i = qe(t?.[3] || t?.[4] || r?.[1] || r?.[2] || "") || null, o = i && i !== "/dev/null" ? i : s;
...
function bn(e) {
  return Cn(e).filter((t) => Mi(t)).map((t) => ({
    content: Fi(t.content),
...
`), s = [];
  ...
  for (let l = 0; l < r.length; l += 1) {
    const p = r[l].match(Hi);
    if (!p) continue;
    o = !0;
    const y = Number(p[1]), b = Math.max(0, y - 1);
    if (b < i) return null;
...
function qi(e) {
  ...
  for (const b of Sa(t))
    for (const h of Wi(b)) {
      if (!h.path) {
        if (s) {
          const X = vn(s.content, h.raw);
          X && X !== s.content && o.set(s.id, St({
            file: s,
            proposedContent: X,
            sourceTitle: r,
      ...
      const w = ta(n, h.path);
      if (!w) {
        l.push(`${h.path}: no opened file`);
        continue;
      }
      const C = vn(w.file.content, h.raw);
      if (!C || C === w.file.content) {
        l.push(`${h.path}: patch did not match or produced no change`);
        continue;
      }
      o.set(w.file.id, St({
        file: w.file,
  ...
  for (const b of bn(t)) {
    const h = b.path ? ta(n, b.path) : s ? { file: s, normalizedPatchPath: qe(s.path), confidence: "exact" } : null;
    if (!h) {
      l.push(`${b.path ?? "replacement block"}: no opened file`);
      continue;
    }
    if (b.content === h.file.content) {
      l.push(`${h.file.path}: replacement produced no change`);
      continue;
    }
    o.set(h.file.id, St({
      file: h.file,
  ...
  const p = bn(t).some((b) => b.path);
  if (o.size === 0 && s && !p) {
    const b = Ri(t, s);
    b && b.content !== s.content && o.set(s.id, St({
      file: s,
      proposedContent: b.content,
      sourceTitle: r,
      extractionLabel: b.label,
  ...
  return {
    ...
    stats: Yi(y.map((b) => b.stats)),
    kind: y.length === 0 ? "empty" : y.length === 1 ? "single-file" : "multi-file"
  };
}
function Ri(e, t) {
  for (const p of Sa(e)) {
    const y = vn(t.content, p);
    if (y) return { content: y, label: "unified diff patch" };
  }
  const n = Cn(e);
  if (n.length === 0) return null;
  const r = Zi(t), s = bn(e)[0];
  if (s) return { content: s.content, label: s.label };
  const i = n.find((p) => r.some((y) => Ce(p, y)) && !Ce(p, "diff") && !Ce(p, "patch"));
  if (i) return { content: i.content, label: `matched ${i.lang || t.language} block` };
  const l = n.filter((p) => !Ce(p, "diff") && !Ce(p, "patch")).sort((p, y) => y.content.length - p.content.length)[0];
  return l ? { content: l.content, label: `largest fenced block (${l.lang || "plain"})` } : null;
}
function ta(e, t) {
  const n = qe(t), r = n.split("/").at(-1) ?? n, s = e.find((l) => qe(l.path) === n);
  if (s) return { file: s, normalizedPatchPath: n, confidence: "exact" };
  const i = e.find((l) => qe(l.path).endsWith(`/${n}`));
  if (i) return { file: i, normalizedPatchPath: n, confidence: "suffix" };
  const o = e.find((l) => l.name === r || qe(l.path).endsWith(`/${r}`));
...
`), s = Math.max(n.length, r.length);
  let i = 0, o = 0, l = 0;
  for (let p = 0; p < s; p += 1)
    n[p] !== r[p] && (n[p] === void 0 ? i += 1 : r[p] === void 0 ? o += 1 : l += 1);
...
function St(e) {
  ...
  return {
    ...
    stats: Ei(e.file.content, e.proposedContent),
    base: {
...
const Rt = (e) => typeof e == "object" && e !== null, Ki = (e) => Rt(e.ai) ? e.ai : null, Ui = (e) => {
  if (!Rt(e)) return null;
  const t = e.id;
...
}, je = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(je) : Rt(e) ? Object.values(e).some(je) : !1, Gi = async (e, t) => {
  const n = Ki(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(Bi).filter((s) => !!s) : [];
}, Qi = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : je(e.capability) || je(e.capabilities) || je(e.modality) || je(e.modalities) || je(e.task) || je(e.tasks) || je(e.type) || je(e.kind), ja = (e) => typeof e == "object" && e !== null, $i = (e) => ja(e.ai) ? e.ai : null, Wa = (e) => typeof $i(e)?.embedText == "function", _i = async (e, t) => Wa(e) ? (await Gi(e, t)).filter(Qi) : [], eo = (e) => ja(e.ai) ? Wa(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", to = ["typecheck", "test", "lint", "build", "release:check", "verify", "verify:cortex"], no = [
...
function io(e) {
  const t = e.results.length > 0 ? e.results.map((r, s) => [
    ...
    `Status: ${r.status}`,
    "Output:",
    po(r.output || "(no output pasted)", "text")
  ].join(`
`)).join(`
...
function co(e) {
  ...
  for (const r of t) {
    const s = lo(r.content);
    if (!s?.scripts) continue;
    const i = Va(r.path), o = uo(e, i, s);
    if (!o) continue;
    const l = Object.keys(s.scripts).filter((y) => typeof s.scripts?.[y] == "string"), p = ho(l);
    for (const y of p) {
      const b = `${o} run ${y}`;
      n.push({
        id: Da(`${i}:${b}`),
...
function uo(e, t, n) {
  ...
  for (const [r, s] of no)
    if (e.some((i) => mo(i.path) === r && Va(i.path) === t)) return s;
  return null;
}
function ho(e) {
  const t = to.filter((r) => e.includes(r)), n = e.filter((r) => !t.includes(r)).filter((r) => /(^|:)(check|typecheck|test|lint|verify|build)(:|$)/i.test(r)).sort((r, s) => r.localeCompare(s));
  return [...t, ...n];
}
function fo(e) {
  const t = /* @__PURE__ */ new Set();
...
function go({ className: e }) {
  return /* @__PURE__ */ a("img", { src: wo, alt: "ATOMEK", className: e, draggable: !1 });
}
const ko = `# Tytus Resource Fabric
...
], Xo = "tytus-atomek-handles", ht = "handles", Lo = 1, Ia = () => new Promise((e, t) => {
  const n = indexedDB.open(Xo, Lo);
  n.onupgradeneeded = () => {
    const r = n.result;
...
async function na(e, t) {
  ...
  await new Promise((r, s) => {
    const i = n.transaction(ht, "readwrite");
    i.objectStore(ht).put(t, e), i.oncomplete = () => r(), i.onerror = () => s(i.error ?? new Error("Failed to persist file handle"));
  }), n.close();
}
async function cn(e) {
  const t = await Ia(), n = await new Promise((r, s) => {
...
const Ao = Cr(() => import("./WorkbenchMonacoEditor-CJhaNTZX.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), Oo = {
  id: "welcome",
  name: "Agent Team",
  path: "Agent Team",
  language: "text",
...
function ln(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - Co), r = n < 1180, s = r ? 200 : 240, i = r ? 300 : 340, o = r ? 420 : 560, l = Math.max(s, Math.min(r ? 340 : 420, Math.floor(n * 0.28))), p = Math.min(300, l), y = Math.floor(n * (r ? 0.34 : 0.36)), b = n - p - o, h = Math.max(i, Math.min(r ? 500 : 640, y, b));
  return { primaryMin: s, primaryMax: l, secondaryMin: i, secondaryMax: h };
}
function So(e) {
  const t = [], n = /```([^\n`]*)\n?([\s\S]*?)```/g;
  let r = 0, s = 0, i;
...
function Wo(e) {
  const t = e?.resources ?? [], n = (l) => l.status === "ready" || l.status === "available", r = t.filter((l) => l.kind === "pod-agent" && mt(l) === "openclaw"), s = t.filter((l) => l.kind === "pod-agent" && mt(l) === "hermes"), i = t.filter((l) => l.kind === "local-cli" && n(l)), o = t.filter((l) => l.kind === "shared-folder" && n(l));
  return [
...
function Nt(e, t, n, r, s) {
  return {
    ...
    resourceLabel: r ? xe(r) : s,
    status: r?.status ?? "needs-setup",
    trustTier: r?.trustTier ?? "not-available",
    sandbox: r?.sandbox ?? "none"
  };
}
function pt(e, t = "repo-repair") {
  const n = Tt.find((D) => D.id === t) ?? Tt[0], r = e?.resources ?? [], s = Be(r, (D) => D.kind === "pod-agent" && mt(D) === "openclaw"), i = Be(r, (D) => D.kind === "pod-agent" && mt(D) === "hermes"), o = Be(r, (D) => D.kind === "ail-route"), l = Be(r, (D) => D.kind === "shared-folder"), p = Ie(r, "local-cli", ["claude"]), y = Ie(r, "local-cli", ["opencode", "open code"]), b = Ie(r, "local-cli", ["codex"]), h = Ie(r, "local-cli", ["pi"]), w = Ie(r, "local-cli", ["kimi"]), C = Be(r, (D) => D.kind === "local-cli"), X = Ie(r, "app-skill", ["juli3ta", "music", "song"]), K = Ie(r, "app-skill", ["blender", "3d"]), E = Ie(r, "app-skill", ["remotion", "video", "render"]), q = Be(r, (D) => D.kind === "app-skill"), ue = t === "pod-local" ? s ?? i ?? o ?? p ?? C : t === "creative-production" ? i ?? o ?? p ?? C : t === "research-watch" ? s ?? o ?? i ?? p ?? C : p ?? y ?? o ?? C, re = t === "creative-production" ? X ?? K ?? E ?? q ?? y ?? p ?? C : y ?? p ?? b ?? C, j = t === "pod-local" ? b ?? h ?? w ?? s ?? p ?? C : t === "research-watch" ? h ?? w ?? b ?? s ?? C : b ?? h ?? s ?? w ?? C, B = t === "creative-production" ? X ?? K ?? E ?? q : t === "research-watch" ? Ie(r, "app-skill", ["browser", "channel", "telegram"]) ?? q : q, R = [
    Nt("planner", "Planner", "Break goal into tasks, risks, and required context.", ue, "No planner agent ready"),
    Nt("implementer", "Implementer", "Execute local/app work and return transcript or patch proposal.", re, "No local/app implementer ready"),
    Nt("reviewer", "Reviewer", "Independent critique before approval or handoff.", j, "No reviewer agent ready"),
    Nt("team-desk", "Team Desk", "Shared mission folder for transcripts, outputs, proposals, and handoff.", l, "Mission folder only until shared folder is bound")
  ];
  (t === "creative-production" || B) && R.push(Nt("app-tool", "App Tool", "Drive installed local app skill through mission context.", B, "No configured app skill"));
  const M = R.filter((D) => D.status === "ready" || D.status === "available" || D.status === "degraded").length, ce = M >= R.length - 1 ? "ready" : M >= 2 ? "partial" : "needs-setup";
  return { ...n, readiness: ce, assignments: R };
}
function $e(e, t, n) {
  if (n && Tt.some((s) => s.id === n)) return n;
  const r = e.toLowerCase();
  return /(music|song|audio|video|render|blender|remotion|juli3ta|creative)/.test(r) ? "creative-production" : /(research|watch|monitor|news|summar|scan)/.test(r) ? "research-watch" : (t?.resources ?? []).some((s) => s.kind === "pod-agent" && Ka(s)) ? "pod-local" : "repo-repair";
}
function Do(e) {
  return Tt.map((t) => pt(e, t.id));
}
function Wt(e) {
...
function zt(e) {
  ...
  try {
    localStorage.setItem(Ja, JSON.stringify(t)), window.dispatchEvent(new CustomEvent(yn, { detail: t }));
  } catch {
  }
}
function Io() {
  try {
    ...
    return !t.missionId || !t.title ? null : {
      ...
      teamPresetId: Tt.some((n) => n.id === t.teamPresetId) ? t.teamPresetId : void 0
    };
  } catch {
    return null;
  }
...
function Xt(e, t, n) {
  const r = e.trim() || "Coordinate a Tytus mission.", s = pt(t, $e(r, t, n)), i = (h) => s.assignments.find((w) => w.role === h), o = i("planner"), l = i("implementer"), p = i("reviewer"), y = i("team-desk"), b = i("app-tool");
  return [
    {
      id: "task-scope",
      ...
      resourceHint: o ? Wt(o) : "Planner agent",
      role: "planner",
    ...
    },
    {
      ...
      resourceHint: l ? Wt(l) : "Local/app implementer",
      role: "implementer",
    ...
    } : null,
    {
      ...
      resourceHint: p ? `${Wt(p)} · ${y?.resourceLabel ?? "mission folder"}` : "Reviewer + mission folder",
      role: "reviewer",
      assignedResourceLabel: p?.resourceLabel ?? "Reviewer agent",
      status: "waiting",
...
function Dt(e, t, n, r, s, i) {
  const o = pt(t, $e(s || e.goal, t, i ?? e.teamPresetId));
  return [
    `# ${e.title}`,
    "",
    `- Mission ID: \`${e.missionId}\``,
...
function Ga(e, t, n, r) {
  const s = $e(n || e.goal, t, r ?? e.teamPresetId), i = pt(t, s), o = Xt(n || e.goal, t, s);
  return JSON.stringify({
    ...
    tasks: o.map((l, p) => ({
      ...
      status: p === 0 ? "ready" : "waiting",
      selectedResourceHint: l.resourceHint,
      dependsOn: p === 0 ? [] : [o[p - 1].id],
...
function Ro({ host: e }) {
  const t = Qe(null), [n, r] = z(0), s = oe(() => Ac(), []), i = oe(() => Nc(), []), o = oe(() => xc(Pc(), i.recent), [i.recent]), [l, p] = z(i.activity ?? "computer"), [y, b] = z(s.primaryVisible), [h, w] = z(s.primaryWidth), [C, X] = z(i.secondaryTab ?? "chat"), [K, E] = z(s.secondaryVisible), [q, ue] = z(s.secondaryWidth), [re, j] = z(!!i.bottomPanelVisible), [B, R] = z(i.bottomPanelTab ?? "problems"), [M, ce] = z(s.markdownPreviewVisible), [D, he] = z(!!i.welcomeClosed), [ve, Xe] = z(i.folder ? { name: i.folder.name, files: [] } : null), [Se, Le] = z(i.folder?.handleKey ?? null), [L, _] = z(() => zc(i.files)), [be, fe] = z(() => (i.openEditorIds ?? []).filter((c) => (i.files ?? []).some((f) => f.id === c))), [m, g] = z(() => i.activeFileId && (i.files ?? []).some((c) => c.id === i.activeFileId) ? i.activeFileId : null), [T, F] = z(i.query ?? ""), [ie, H] = z({ lineNumber: 1, column: 1 }), [ke, Ne] = z(null), [Ve, He] = z({}), [Pe, ye] = z(Ws), [ee, te] = z([]), [$, ne] = z([]), [_e, Et] = z(null), [Ae, et] = z(i.chatInput ?? ""), [Yt, bt] = z(!1), [Jt, Zt] = z(""), [u, k] = z([]), [N, S] = z(null), [A, I] = z(null), [U, Y] = z(!1), [me, De] = z(() => Oc()), [G, W] = z(null), [Q, we] = z(null), [pe, Oe] = z(""), [Re, tt] = z(""), [Ze, nt] = z(""), [Fe, jn] = z("failed"), [rr, Wn] = z(null), [sr, O] = z("Ready"), [vt, ir] = z(() => o), ze = be.map((c) => L.find((f) => f.id === c)).filter(Boolean), V = m ? L.find((c) => c.id === m) ?? null : null, Mn = oe(() => js({ files: L, openEditorIds: be, activeFileId: m, versions: Ve, activeSelection: ke }), [m, ke, Ve, L, be]), Vn = oe(() => ({
    scope: Pe,
    removedAttachmentIds: ee,
    selectedFileIds: []
  }), [Pe, ee]), yt = oe(() => Zs(Mn, L, Vn), [Vn, Mn, L]), Ee = pi(L, { autoRefresh: !0, includeDirty: !0 }), Dn = oe(() => $.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [$]), or = oe(() => [...yt.attachments, ...Dn], [yt.attachments, Dn]), J = Cs({ host: e, requestContext: yt.parts, chatSettings: me, setStatus: O }), Ke = oe(
    () => [...J.artifacts, ...u].sort((c, f) => f.createdAt - c.createdAt),
    [J.artifacts, u]
  ), At = U && !V, Kt = !V && !At && !D, ge = oe(() => L.filter((c) => c.dirty), [L]), cr = oe(() => {
    const c = T.trim().toLowerCase();
    return c ? L.filter((f) => f.path.toLowerCase().includes(c)) : L;
  }, [L, T]);
  se(() => {
    He((c) => {
      const f = {};
      for (const v of L) f[v.id] = c[v.id] ?? 1;
      return f;
    });
  }, [L]), se(() => {
    te([]), ne([]);
  ...
  const lr = x((c) => {
    if (c.kind === "index-hit") {
      ne((f) => f.filter((v) => v.id !== c.id)), O(`Removed project context: ${c.label}`);
      return;
    }
    te((f) => f.includes(c.id) ? f : [...f, c.id]), O(`Removed chat context: ${c.label}`);
  }, []), Ot = x(() => {
    Y(!0), g(null), he(!0), O("Atomek settings opened");
  }, []), Fn = x(() => {
    Y(!1), g((c) => c ?? be.at(-1) ?? null), be.length === 0 && he(!1), O("Atomek settings closed");
  }, [be]), at = x((c) => {
    He((f) => ({ ...f, [c]: (f[c] ?? 1) + 1 }));
  }, []), dr = x((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const f = ln(n), v = c.clientX, P = q, Z = (kt) => {
      const nn = P + (v - kt.clientX);
      ue(jt(nn, f.secondaryMin, f.secondaryMax));
    }, de = () => {
      window.removeEventListener("pointermove", Z), window.removeEventListener("pointerup", de);
    };
    window.addEventListener("pointermove", Z), window.addEventListener("pointerup", de);
  }, [q, n]), ur = x((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const f = ln(n), v = c.clientX, P = h, Z = (kt) => {
      const nn = P + (kt.clientX - v);
      w(jt(nn, f.primaryMin, f.primaryMax));
    }, de = () => {
      window.removeEventListener("pointermove", Z), window.removeEventListener("pointerup", de);
    };
    window.addEventListener("pointermove", Z), window.addEventListener("pointerup", de);
    ...
    ir((f) => {
      const v = [c, ...f.filter((P) => P.path !== c.path)].slice(0, 10);
      return localStorage.setItem(qa, JSON.stringify(v)), v;
    });
  }, []), le = x((c, f) => {
    fe((v) => v.includes(c.id) ? v : [...v, c.id]), g(c.id), he(!1), Wn(f ?? null), H({ lineNumber: f ?? 1, column: 1 });
  }, []), hr = x((c) => {
    const f = {
      id: `atomek-doc:${c.id}`,
      name: c.fileName,
    ...
    _((v) => Mt(v, [f])), le(f), X("chat"), E(!0), ce(!0), O(`Opened Atomek guide: ${c.title}`);
  }, [le]), fr = x((c) => {
    if (!c.fileId) return;
    const f = L.find((v) => v.id === c.fileId);
    f && (le(f, c.range?.startLineNumber ?? 1), O(`Revealed context: ${c.label}`));
  }, [L, le]), wt = x(async () => {
    if (dn(ge, "open new files"))
      try {
        const c = await gs();
        if (c.length === 0) return;
        _((f) => Mt(f, c)), await Promise.all(c.map(async (f) => {
          const v = f.handle ? `file:${f.path}` : void 0;
          f.handle && v && await na(v, f.handle), Ye({ name: f.name, path: f.path, kind: "file", handleKey: v, at: Date.now() });
        })), le(c[0]), O(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
        c.name !== "AbortError" && O(`Open file failed: ${c.message}`);
      }
  }, [ge, le, Ye]), Ut = x(async () => {
    if (dn(ge, "open another folder"))
      try {
        const c = await ks(), f = c.handle ? `folder:${c.name}:${Date.now()}` : null;
        c.handle && f && await na(f, c.handle), Le(f), Xe(c), _(c.files), fe([]), g(null), he(!1), Ye({ name: c.name, path: c.name, kind: "folder", handleKey: f ?? void 0, at: Date.now() }), O(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
        c.name !== "AbortError" && O(`Open folder failed: ${c.message}`);
      }
  }, [ge, Ye]), mr = x((c) => {
    if (!m) return;
    let f = !1;
    _((v) => v.map((P) => P.id !== m || P.content === c ? P : (f = !0, { ...P, content: c, dirty: !0 }))), f && at(m);
  }, [m, at]), Bt = x(async () => {
    if (V)
      try {
        const c = await rn(V);
        _((f) => f.map((v) => v.id === c.id ? c : v)), O(`Saved ${c.name}`);
      } catch (c) {
        O(`Save failed: ${c.message}`);
      }
  }, [V]), pr = x(async (c) => {
    const f = L.find((P) => P.id === c);
    if (!f) return null;
    const v = await rn(f);
    return _((P) => P.map((Z) => Z.id === v.id ? v : Z)), v;
  }, [L]), In = x(async () => {
    const c = L.filter((f) => f.dirty);
    if (c.length === 0) {
      O("No dirty files to save");
      return;
    }
    try {
      const f = await Promise.all(c.map((P) => rn(P))), v = new Map(f.map((P) => [P.id, P]));
      _((P) => P.map((Z) => v.get(Z.id) ?? Z)), W(null), O(`Saved ${f.length} dirty file${f.length === 1 ? "" : "s"}`);
    } catch (f) {
      O(`Save all failed: ${f.message}`);
    }
  }, [L]), Gt = x((c) => {
    const f = L.find((v) => v.id === c);
    if (f?.dirty && !window.confirm(`${f.name} has unsaved changes. Close without saving?`)) {
      O(`Close canceled — ${f.name} has unsaved changes`);
      return;
    }
    fe((v) => {
      const P = v.filter((Z) => Z !== c);
      return m === c && (g(P.at(-1) ?? null), Ne(null)), P;
    });
  }, [m, L]), br = x(() => {
    dn(ge, "close all editors") && (fe([]), g(null), Wn(null), he(!1), O("Closed all editors"));
  }, [ge]), Qt = x(() => {
    const c = L.filter((v) => v.name.startsWith("Untitled")).length + 1, f = {
...
`,
    ...
    return ne(P), P.length === 0 ? (O(Ee.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), f) : (v.mode !== "hybrid" && v.reason ? O(v.reason) : O(`${P.length} project context hit${P.length === 1 ? "" : "s"} · ${v.reason ?? "hybrid retrieval"}`), [
      ...f,
      ...xi(P)
    ]);
  }, [yt.parts, Pe, me, e, Ee]), $t = x(() => {
    if (!V && ze.length === 0) {
      O("Open a file before asking Atomek to synthesize an AI artifact");
      return;
    }
    const f = [
      `Create a polished Markdown artifact from ${V?.path ?? `${ze.length} open editors`}.`,
    ...
    E(!0), X("chat"), O("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const v = await Je(f), P = await J.askAgent(f, { requestContext: v });
      !P || P.status === "error" || J.createArtifact({
        messageId: P.id,
        title: `AI synthesis — ${V?.name ?? "open workspace"}`,
        kind: "markdown",
        body: P.body
      }).then(() => {
        E(!0), X("outputs"), j(!0), R("output");
      });
    })();
  }, [V, J, Je, ze.length]), _t = x(async (c) => {
    if (c.kind === "folder") {
      const v = L.filter((P) => P.path === c.path || P.path.startsWith(`${c.path}/`));
      if (ve?.name === c.name || v.length > 0) {
        v.length > 0 && (Xe((P) => P?.name === c.name ? P : { name: c.name, files: v }), _((P) => Mt(P, v))), p("explorer"), b(!0), g(null), he(!1), Ye({ ...c, at: Date.now() }), O(`Opened recent folder ${c.name}`);
        return;
      }
    }
    const f = L.find((v) => v.path === c.path || v.name === c.name);
    if (f) {
      le(f), O(`Opened recent ${f.name}`);
      return;
    }
    if (!c.handleKey) {
      O("Recent item has no stored browser permission. Use Open File or Open Folder once, then Atomek can restore it.");
      return;
    }
    try {
      if (c.kind === "folder") {
        const Z = await cn(c.handleKey);
        if (!Z || !await an(Z, "readwrite")) {
          O("Browser permission expired for this folder. Click Open Folder and pick it once to refresh the recent handle.");
          return;
        }
        const de = await Zn(Z);
        Le(c.handleKey), Xe(de), _(de.files), fe([]), g(null), he(!1), Ye({ ...c, at: Date.now() }), O(`Reopened ${de.name} (${de.files.length} text files indexed)`);
        return;
      }
      const v = await cn(c.handleKey);
      if (!v || !await an(v, "readwrite")) {
        O("Browser permission expired for this file. Click Open File and pick it once to refresh the recent handle.");
        return;
      }
      const P = await Ps([v]);
      if (!P[0]) {
        O(`Recent file is not readable text: ${c.name}`);
        return;
      }
      _((Z) => Mt(Z, P)), le(P[0]), Ye({ ...c, at: Date.now() }), O(`Reopened ${P[0].name}`);
    } catch (v) {
      O(`Open recent failed: ${v instanceof Error ? v.message : String(v)}`);
    }
  }, [L, ve?.name, le, Ye]), Te = x((c) => {
    E(!0), X("chat"), et(""), (async () => {
      const f = await Je(c);
      await J.askAgent(c, { requestContext: f });
    })();
  }, [J, Je]), qn = x(async (c) => {
    if (!e.skills?.get) {
      O("Tytus skill registry is not available in this host build");
      return;
    }
    try {
      const f = await e.skills.get(c.id), v = f.body.length > 4500 ? `${f.body.slice(0, 4500)}
...
`);
      et((Z) => [Z.trim(), P].filter(Boolean).join(`

`)), E(!0), X("chat"), O(`Attached skill ${f.title} to chat input`);
    } catch (f) {
      O(`Failed to attach skill: ${f instanceof Error ? f.message : String(f)}`);
    }
  }, [e.skills]), vr = x((c) => {
    const f = J.messages.findIndex((P) => P.id === c.id), v = J.messages.slice(0, f < 0 ? void 0 : f).reverse().find((P) => P.role === "user");
    if (!v?.body.trim()) {
      O("No previous user prompt to regenerate from");
      return;
    }
    (async () => {
      const P = await Je(v.body);
      await J.askAgent(v.body, { requestContext: P });
    })();
  }, [J, Je]), yr = x((c) => {
    J.createArtifact({
      messageId: c.id,
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    if (!V) {
      O("No active file to save as AI artifact");
      return;
    }
    J.createArtifact({
      title: V.path,
      kind: V.language === "markdown" ? "markdown" : "report",
      body: V.content
    }).then(() => {
      E(!0), X("outputs"), j(!0), R("output");
    });
  }, [V, J]), en = x((c) => {
    const f = hn(L, un(c.title || c.kind)), v = {
    ...
    _((P) => [...P, v]), le(v), O(`Opened ${c.title} as editable file`);
  }, [L, le]), tn = x(() => {
    const c = window.prompt("Check command/name (manual capture only; Atomek will not execute it)");
    if (c === null) return;
    const f = c.trim();
    if (!f) {
      O("Manual check capture canceled — command/name was empty");
      return;
    }
...
`), Z = {
    ...
    k((de) => [Z, ...de]), j(!0), R("output"), O(`Captured manual check: ${f}`);
  }, []), rt = x((c, f) => {
    const v = qi({
      body: f,
      files: L,
      ...
      edits: v.edits.map(gc),
      skipped: v.skipped
    };
    return v.edits.length > 1 ? (I(P), S(null), O(`Previewing AI workspace patch for ${v.edits.length} files`), !0) : v.edits.length === 1 ? (S(P.edits[0]), I(null), O(`Previewing AI patch for ${P.edits[0].fileName}`), !0) : (O(v.skipped.length > 0 ? `No applicable edit found. ${v.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [V, Ve, L]), Rn = x((c, f) => {
    const v = {
    ...
    k((P) => [v, ...P]), E(!0), X("outputs"), j(!0), R("output"), O(`Saved local job output: ${c}`), (f.includes("```diff") || f.includes("--- a/")) && rt(c, f);
  }, [rt]), kr = x(() => {
    const c = Ae.trim();
    if (!c) return;
    et("");
    const f = oc(c);
    (async () => {
      const v = await Je(c), P = await J.askAgent(c, { requestContext: v });
      if (!f || !P || P.status === "error") return;
      rt(P.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", P.body) || O("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [J, Je, Ae, rt]), Pr = x(() => {
    _e && (Et(null), Te(cc(_e)));
  }, [Te, _e]), gt = x((c) => {
    const f = ao(L, c);
    we(f);
    const v = f.commands[0]?.command ?? "";
    nt(v), Oe(""), tt(""), jn("failed"), j(!0), R("terminal"), O(f.commands.length > 0 ? `Manual check ready: copy ${f.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [L]), xr = x((c) => {
    c.trim() && (ft(c), nt(c), O(`Manual check command copied: ${c}`));
  }, []), Nr = x(() => {
    we((c) => {
      if (!c) return c;
      const f = ro(c, pe), v = f.commands.at(-1)?.command ?? "";
      return v && nt(v), f;
    }), Oe("");
  }, [pe]), zr = x(() => {
    if (!Q || !Ze.trim()) {
      O("Select or enter a manual check command before capturing output");
      return;
    }
    const c = Ze.trim();
    we((f) => f && so(f, c, Fe, Re)), tt(""), O(`Captured manual check result: ${c} (${Fe})`);
  }, [Re, Ze, Q, Fe]), Tr = x(() => {
    Q && Te(io(Q));
  }, [Te, Q]), Xr = x(() => {
    if (!N) return;
    const c = L.find((f) => f.id === N.fileId);
    if (!c) {
      O(`Cannot apply edit — ${N.fileName} is no longer open`), S(null);
      return;
    }
    c.content !== N.originalContent && !window.confirm(`${N.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (_((f) => f.map((v) => v.id === N.fileId ? { ...v, content: N.proposedContent, dirty: !0 } : v)), at(N.fileId), W(`AI edit applied to ${c.name}. Save All to persist it to disk.`), S(null), gt(`AI edit applied to ${c.name}`), O(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [at, L, N, gt]), Lr = x(() => {
    if (!N) return;
    const c = hn(L, `${un(N.fileName)}-ai-edit`), f = {
      id: `pending-edit-${N.fileId}-${Date.now()}`,
      name: c,
      path: c,
      language: V?.language ?? "markdown",
      content: N.proposedContent,
      dirty: !0,
      source: "generated"
    };
    _((v) => [...v, f]), le(f), S(null), O(`Opened proposed edit as ${c}`);
  }, [V?.language, L, le, N]), Ar = x(() => {
    if (!A) return;
    const c = new Map(A.edits.map((v) => [v.fileId, v]));
    L.some((v) => {
      const P = c.get(v.id);
      return P && v.content !== P.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (_((v) => v.map((P) => {
      const Z = c.get(P.id);
      return Z ? { ...P, content: Z.proposedContent, dirty: !0 } : P;
    })), c.forEach((v, P) => at(P)), W(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), I(null), gt(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), O(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [at, L, A, gt]), Or = x(() => {
    if (!A) return;
    const c = [...L], f = A.edits.map((v) => {
      const P = hn(c, `${un(v.fileName)}-ai-edit`), Z = L.find((kt) => kt.id === v.fileId), de = {
        id: `workspace-patch-${v.fileId}-${Date.now()}-${P}`,
        name: P,
        path: P,
    ...
    _((v) => [...v, ...f]), f[0] && le(f[0]), I(null), O(`Opened ${f.length} proposed edit file${f.length === 1 ? "" : "s"}`);
  }, [L, le, A]), En = x((c) => {
    if (c === "explain") {
      Te("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (c === "improve") {
      Te("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (c === "plan") {
      Te("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (c === "edit") {
      Te("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    Te("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [Te]);
  return se(() => {
    const c = (f) => {
      (f.metaKey || f.ctrlKey) && (f.key.toLowerCase() === "o" && (f.preventDefault(), wt()), f.key.toLowerCase() === "s" && (f.preventDefault(), Bt()), f.key.toLowerCase() === "w" && (f.preventDefault(), m && Gt(m)), f.key.toLowerCase() === "b" && (f.preventDefault(), b((P) => !P)), f.shiftKey && f.key.toLowerCase() === "f" && (f.preventDefault(), p("search"), b(!0)), (f.key.toLowerCase() === "k" || f.key.toLowerCase() === "p") && (f.preventDefault(), bt(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [m, Gt, wt, Bt]), se(() => {
    const c = (f) => {
      ge.length !== 0 && (f.preventDefault(), f.returnValue = "");
    };
    ...
    const c = t.current;
    if (!c) return;
    const f = () => r(Math.round(c.getBoundingClientRect().width));
    f();
    ...
    if (!n) return;
    const c = ln(n);
    w((f) => jt(f, c.primaryMin, c.primaryMax)), ue((f) => jt(f, c.secondaryMin, c.secondaryMax));
  }, [n]), se(() => {
    const c = { primaryVisible: y, primaryWidth: h, secondaryVisible: K, secondaryWidth: q, markdownPreviewVisible: M };
    localStorage.setItem(Ra, JSON.stringify(c));
  }, [M, y, h, K, q]), se(() => {
    if (!i.folder?.handleKey) return;
    let c = !1;
    return (async () => {
      try {
        const f = await cn(i.folder?.handleKey ?? "");
        if (!f || !await an(f, "readwrite") || c) return;
        const v = await Zn(f);
        if (c) return;
        Le(i.folder?.handleKey ?? null), Xe(v), _((P) => Lc(P, v.files)), fe((P) => P.filter((Z) => v.files.some((de) => de.id === Z) || L.some((de) => de.id === Z))), O(`Restored ${v.name} (${v.files.length} text files indexed)`);
      } catch (f) {
        c || O(`Workspace restore failed: ${f instanceof Error ? f.message : String(f)}`);
      }
    })(), () => {
      c = !0;
    };
    ...
    Xc({
      activity: l,
      folder: ve ? { name: ve.name, handleKey: Se } : null,
      files: Tc(L),
      openEditorIds: be,
      activeFileId: m,
      query: T,
      ...
      welcomeClosed: D,
      secondaryTab: C,
      bottomPanelVisible: re,
      bottomPanelTab: B,
      recent: vt
    });
  }, [l, m, B, re, Ae, Se, L, ve, be, T, vt, C, D]), se(() => {
    localStorage.setItem(Ya, JSON.stringify(me));
  }, [me]), /* @__PURE__ */ d(
    "div",
    {
      ref: t,
      className: `workbench-workbench ${y ? "" : "no-primary"} ${K ? "" : "no-secondary"} ${re ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${h}px`, "--workbench-secondary-width": `${q}px` },
      children: [
        /* @__PURE__ */ a(
          Eo,
          {
        ...
        y && /* @__PURE__ */ d("div", { className: "workbench-primary-region", children: [
          /* @__PURE__ */ a(
            Yo,
            {
              host: e,
              ...
              openFile: wt,
              openFolder: Ut,
              openWorkbenchFile: le,
              newFile: Qt,
              recent: vt,
              reopenRecent: _t,
              setStatus: O,
              hasFsAccess: ws(),
              attachSkillToChat: qn,
              saveLocalJobOutput: Rn,
              activeFile: V
            }
          ),
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: ur, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => bt(!0), children: "Workspace" }),
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            /* @__PURE__ */ a(
              Uo,
              {
                openEditors: ze,
                ...
                closeSettings: Fn,
                secondaryVisible: K,
                toggleSecondary: () => E((c) => !c),
                canPreview: V?.language === "markdown",
                previewVisible: M,
                togglePreview: () => ce((c) => !c)
              }
            ),
            /* @__PURE__ */ a(Ko, { file: V, folder: ve, showWelcome: Kt }),
            G && ge.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: G }),
              /* @__PURE__ */ a("button", { onClick: () => {
                In();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => W(null), title: "Dismiss", children: /* @__PURE__ */ a(Me, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: V ? /* @__PURE__ */ d("div", { className: V.language === "markdown" && M ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(Sr, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                Ao,
                {
                  file: V,
                  revealLine: rr,
              ...
              V.language === "markdown" && M && /* @__PURE__ */ a(tc, { content: V.content })
            ] }) : At ? /* @__PURE__ */ a(
              dc,
              {
                host: e,
            ...
            ) : Kt ? /* @__PURE__ */ a(Qo, { host: e, openFile: wt, openFolder: Ut, newFile: Qt, recent: vt, reopenRecent: _t, setStatus: O, openControlTower: () => {
              p("computer"), b(!0);
            }, openChat: () => {
              X("chat"), E(!0);
            }, openEmbeddedDoc: hr }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(qt, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => he(!1), children: "Show Agent Team" })
            ] }) }),
            re && /* @__PURE__ */ a(
              ac,
              {
                ...
                runAiSynthesis: $t,
                captureManualCheck: tn,
                openOutputAsFile: en,
                manualCheckSession: Q,
                manualCheckCommandInput: pe,
                setManualCheckCommandInput: Oe,
                manualCheckOutputInput: Re,
                setManualCheckOutputInput: tt,
                manualCheckSelectedCommand: Ze,
                setManualCheckSelectedCommand: nt,
                manualCheckStatus: Fe,
                setManualCheckStatus: jn,
                copyManualCheckCommand: xr,
                addManualCheckCommand: Nr,
                recordManualCheckResult: zr,
                askAgentFromManualChecks: Tr,
                onClose: () => j(!1)
              }
            )
          ] })
        ...
        K && /* @__PURE__ */ a(
          rc,
          {
            ...
            saveMessageAsArtifact: yr,
            rememberMessage: wr,
            previewEditFromMessage: (c) => rt(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: En,
            pendingPatchPrompt: _e,
            generatePatchPrompt: Pr,
            workspaceFileCount: L.length,
            aiStatus: J.aiStatus,
            chatSettings: me,
            openSettings: Ot,
            busy: J.busy,
            memoryHitCount: J.memoryHits.length,
            outputs: Ke,
            runAiSynthesis: $t,
            captureManualCheck: tn,
            openOutputAsFile: en,
            previewEditFromOutput: (c) => rt(c.title, c.body),
            canPreviewEdit: L.length > 0,
            clearOutputs: () => k([]),
            deleteArtifact: (c) => {
              J.deleteArtifact(c);
            },
            ...
            contextScope: Pe,
            setContextScope: ye,
            contextAttachments: or,
            removeContextAttachment: lr,
            revealContextAttachment: fr,
            projectIndexSummary: `${Ee.snapshot.files.length} files · ${Ee.snapshot.chunks.length} chunks`,
            projectIndexStale: Ee.isStale,
            refreshProjectIndex: () => {
              const c = Ee.refresh(L);
              O(`Project index refreshed: ${c.files.length} files · ${c.chunks.length} chunks`);
            },
            onResizeStart: dr,
            onClose: () => E(!1)
          }
        ...
        Yt && /* @__PURE__ */ a(
          ec,
          {
            ...
            activeFile: V,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: Qt },
              ...
              ...vt.map((c) => ({ label: `File: Open Recent — ${c.name}`, detail: c.path, run: () => {
                _t(c);
              } })),
              { label: "File: Save All", detail: `${ge.length} dirty file${ge.length === 1 ? "" : "s"}`, run: () => {
                In();
              }, disabled: ge.length === 0 },
              { label: "File: Close All Editors", detail: `${ze.length} open editor${ze.length === 1 ? "" : "s"}`, run: br, disabled: ze.length === 0 },
              ...
              { label: "Help: Show Agent Team", detail: "Open the Atomek Resource Fabric landing page", run: () => {
                g(null), he(!1), p("computer"), b(!0);
              } },
              { label: "View: Toggle Primary Side Bar", detail: y ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => b((c) => !c) },
              { label: "View: Toggle Chat Panel", detail: K ? "Hide right AI side bar" : "Show right AI side bar", run: () => E((c) => !c) },
              { label: "View: Toggle Bottom Panel", detail: re ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => j((c) => !c) },
              { label: "Atomek: Open Settings", detail: "Open settings as an editor tab", run: Ot },
              { label: "Checks: Open Manual Check Panel", detail: "Capture copy/paste check commands without host execution", run: () => gt("Manual check requested from command palette") },
              { label: "View: Toggle Markdown Preview", detail: V?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => ce((c) => !c), disabled: V?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: V || ze.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: $t, disabled: !V && ze.length === 0 },
              { label: "AI: Explain Active File", detail: V ? `Ask Cortex to explain ${V.path}` : "Open a file first", run: () => Te("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !V },
              { label: "AI: Improve Active File", detail: V ? `Ask Cortex for concrete edits to ${V.path}` : "Open a file first", run: () => Te("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !V },
              { label: "AI: Draft Editable Replacement", detail: V ? `Ask Cortex for a full-file replacement for ${V.path}` : "Open a file first", run: () => En("edit"), disabled: !V },
              { label: "AI: Plan Workspace Work", detail: ze.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => Te("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: ze.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: V ? "Persist active file in host.ai artifacts" : "Open a file first", run: gr, disabled: !V },
              { label: "AI: Capture Manual Check Output", detail: "Paste check output as an auditable local artifact; no shell execution", run: tn },
              { label: "AI: Open Latest Artifact as File", detail: Ke[0] ? `Create editable file from ${Ke[0].title}` : "No outputs yet", run: () => Ke[0] && en(Ke[0]), disabled: Ke.length === 0 }
            ],
            openWorkbenchFile: le,
            onClose: () => bt(!1)
          }
        ...
        /* @__PURE__ */ a(wc, { status: sr, file: V ?? Oo, cursor: ie, fileCount: L.length, dirtyCount: ge.length })
      ]
    }
  );
}
function Eo({ active: e, setActive: t, togglePrimary: n, openSettings: r, settingsActive: s }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a("button", { className: "workbench-activity-brand", title: "Toggle side bar", "aria-label": "Toggle side bar", onClick: n, children: /* @__PURE__ */ a(Fa, { size: 30, variant: "cream" }) }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(ha, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(ls, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(ma, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(ua, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(kn, { size: 25 }), label: "Agent Team", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(ct, { icon: /* @__PURE__ */ a(fs, { size: 23 }), label: "Settings", active: s, onClick: r })
  ] });
}
function ct({ icon: e, label: t, active: n, onClick: r }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: r, children: e });
}
function Yo(e) {
  return e.activity === "search" ? /* @__PURE__ */ a($o, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(ca, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(ca, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ a(nr, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput, activeFile: e.activeFile, openEditors: e.openEditors }) : /* @__PURE__ */ a(Jo, { ...e });
}
function Jo(e) {
  const t = !e.folder, n = oe(() => Zo(e.files, e.folder?.name), [e.files, e.folder?.name]), [r, s] = z(() => /* @__PURE__ */ new Set());
  se(() => {
    const o = Qa(n);
    s((l) => {
      const p = new Set(l);
      return o.forEach((y) => p.add(y)), p;
    });
  }, [n]);
  const i = x((o) => {
    s((l) => {
      const p = new Set(l);
      return p.has(o) ? p.delete(o) : p.add(o), p;
    });
  }, []);
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "EXPLORER" }),
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(We, { children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-sidebar-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: e.openFile, children: [
            /* @__PURE__ */ a(Nn, { size: 14 }),
            "Open File"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: e.openFolder, children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(It, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((o) => /* @__PURE__ */ a(Hn, { file: o, active: o.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(o), label: o.name, detail: o.path }, o.id)),
        /* @__PURE__ */ d("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(It, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : $a(n, e.activeFileId, e.openWorkbenchFile, r, i)
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((o) => /* @__PURE__ */ d("button", { className: "workbench-tree-row", onClick: () => {
        e.reopenRecent(o);
      }, title: o.path, children: [
        o.kind === "file" ? /* @__PURE__ */ a(xn, { size: 14 }) : /* @__PURE__ */ a(fa, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name })
      ] }, `${o.path}-${o.at}`))
    ] })
  ] });
}
function Hn({ file: e, active: t, onOpen: n, basePath: r, depth: s = 0, label: i, detail: o }) {
  const l = r && e.path.startsWith(`${r}/`) ? e.path.slice(r.length + 1) : e.path, p = s || Math.max(0, l.split("/").length - 1);
  return /* @__PURE__ */ d("button", { className: `workbench-file-row ${t ? "active" : ""}`, style: { "--workbench-depth": p }, onClick: n, title: e.path, children: [
    /* @__PURE__ */ a(xn, { size: 14 }),
    /* @__PURE__ */ d("span", { className: "workbench-row-text", children: [
...
function Zo(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), s = (o, l, p = n) => {
    const y = r.get(o);
    if (y) return y;
    const b = { name: l, path: o, children: [] };
    return r.set(o, b), p.push(b), b;
  };
  e.forEach((o) => {
    const l = t && o.path.startsWith(`${t}/`) ? o.path.slice(t.length + 1) : o.path, p = l.split("/").filter(Boolean);
    let y = n, b = "";
    p.slice(0, -1).forEach((h) => {
      b = b ? `${b}/${h}` : h, y = s(b, h, y).children;
    }), y.push({ name: p.at(-1) ?? o.name, path: l, file: o, children: [] });
  });
  const i = (o) => o.sort((l, p) => +!!l.file - +!!p.file || l.name.localeCompare(p.name)).map((l) => ({ ...l, children: i(l.children) }));
  return i(n);
}
function Qa(e) {
  return e.flatMap((t) => t.file ? [] : [t.path, ...Qa(t.children)]);
}
function $a(e, t, n, r, s, i = 0) {
  return e.map((o) => {
    if (o.file)
      return /* @__PURE__ */ a(Hn, { file: o.file, active: o.file.id === t, onOpen: () => n(o.file), depth: i, label: o.name }, o.file.id);
    const l = r.has(o.path);
    return /* @__PURE__ */ d("div", { children: [
      /* @__PURE__ */ d("button", { className: "workbench-folder-row", style: { "--workbench-depth": i }, onClick: () => s(o.path), title: l ? `Collapse ${o.name}` : `Expand ${o.name}`, children: [
        l ? /* @__PURE__ */ a(It, { size: 12 }) : /* @__PURE__ */ a(It, { className: "workbench-chevron-collapsed", size: 12 }),
        l ? /* @__PURE__ */ a(zn, { size: 14 }) : /* @__PURE__ */ a(fa, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: o.name }),
        /* @__PURE__ */ a("span", { className: "workbench-row-meta", children: o.children.length })
      ] }),
      l ? $a(o.children, t, n, r, s, i + 1) : null
    ] }, o.path);
  });
}
function Ko({ file: e, folder: t, showWelcome: n }) {
  const r = n ? ["Agent Team"] : e?.path.split("/").filter(Boolean) ?? [], s = t && r[0] === t.name ? r.slice(1) : r;
  return /* @__PURE__ */ a("div", { className: "workbench-breadcrumb", children: s.length === 0 ? /* @__PURE__ */ a("span", { children: "Workspace" }) : s.map((i, o) => /* @__PURE__ */ d("span", { className: "workbench-breadcrumb-part", children: [
    o > 0 && /* @__PURE__ */ a("span", { className: "workbench-breadcrumb-sep", children: "›" }),
    i
  ] }, `${i}-${o}`)) });
}
function Uo(e) {
  return /* @__PURE__ */ d("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ d("button", { className: "workbench-tab active", children: [
      /* @__PURE__ */ a(qt, { size: 15 }),
      ...
      }, children: /* @__PURE__ */ a(Me, { size: 13 }) })
    ] }),
    e.settingsOpen && /* @__PURE__ */ d("button", { className: `workbench-tab ${e.settingsActive ? "active" : ""}`, onClick: e.openSettings, title: "Atomek Settings", children: [
      /* @__PURE__ */ a(pa, { size: 15 }),
      /* @__PURE__ */ a("span", { className: "workbench-tab-name", children: "Atomek Settings" }),
      /* @__PURE__ */ a("span", { className: "workbench-tab-close", role: "button", tabIndex: 0, onClick: (t) => {
        t.stopPropagation(), e.closeSettings();
      }, children: /* @__PURE__ */ a(Me, { size: 13 }) })
    ] }),
    e.openEditors.map((t) => /* @__PURE__ */ d("button", { className: `workbench-tab ${t.id === e.activeFileId ? "active" : ""}`, onClick: () => e.setActiveFileId(t.id), title: t.path, children: [
      /* @__PURE__ */ a(xn, { size: 15 }),
      /* @__PURE__ */ d("span", { className: "workbench-tab-name", children: [
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ a(Pn, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(as, { size: 16 }) })
  ] });
}
function Bo(e) {
...
function Go(e) {
  ...
  return [...e?.resources ?? []].sort((n, r) => (t[n.kind] ?? 9) - (t[r.kind] ?? 9) || xe(n).localeCompare(xe(r))).slice(0, 8);
}
function Qo({
  host: e,
  openFile: t,
  ...
  const [b, h] = z("Coordinate a Tytus mission across pods, local agents, shared folders, and app skills."), [w, C] = z(null), [X, K] = z([]), [E, q] = z(!1), [ue, re] = z(null), [j, B] = z(null), [R, M] = z("pod-local"), ce = x(async () => {
    q(!0), B(null);
    try {
      ...
      C(m ?? null), K(g), m && o(`Agent team loaded · ${m.resources.length} resources · ${m.warnings.length} warnings`);
    } catch (m) {
      const g = m instanceof Error ? m.message : String(m);
      B(g), o(`Agent team resource load failed: ${g}`);
    } finally {
      q(!1);
    }
  }, [e.missions, e.resources, o]), D = x((m, g) => {
    try {
      e.windows.openOrFocus(m), o(`Opened ${g}`);
    } catch (T) {
      o(`Open ${g} failed: ${T instanceof Error ? T.message : String(T)}`);
    }
  }, [e.windows, o]);
  se(() => {
    ce();
  ...
  const he = x(async () => {
    ...
    const m = b.trim() || "Coordinate a Tytus mission.", g = $e(m, w, R);
    q(!0);
    try {
      const T = await e.missions.create({
        ...
        data: { resourceCount: w?.resources.length ?? 0 }
      };
      await e.missions.write({
        rootPath: T.rootPath,
        files: [
          { path: "MISSION.md", content: Dt(F, w, null, [], m, g) },
          { path: "MISSION.json", content: Ga(F, w, m, g) },
          { path: "RESOURCES.md", content: Ft(w) },
          { path: "TASKS.md", content: Ua(Xt(m, w, g)) },
          { path: "HANDOFF.md", content: Ba(F) },
...
` },
          { path: "AUDIT.jsonl", content: `${JSON.stringify(ie)}
` },
...
`) }
      ...
      }), re(T), zt(F), o(`Mission created: ${T.rootPath}`), l();
    } catch (T) {
      o(`Mission create failed: ${T instanceof Error ? T.message : String(T)}`);
    } finally {
      q(!1);
    }
  }, [b, w, e.missions, l, o, R]), ve = Bo(w), Xe = Wo(w), Se = Mo(w), Le = Go(w), L = w?.warnings ?? [], _ = Do(w), be = pt(w, $e(b, w, R)), fe = Xt(b, w, be.id);
  return /* @__PURE__ */ d("div", { className: "workbench-welcome workbench-control-home", children: [
    /* @__PURE__ */ d("section", { className: "workbench-control-hero-main", children: [
      /* @__PURE__ */ d("div", { className: "workbench-brand-lockup", "aria-label": "Atomek", children: [
        /* @__PURE__ */ a(Fa, { size: 74, variant: "acid" }),
        /* @__PURE__ */ d("div", { children: [
          /* @__PURE__ */ a(go, { className: "workbench-brand-wordmark" }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-control-goal-row", children: [
        /* @__PURE__ */ a("textarea", { value: b, onChange: (m) => h(m.target.value), rows: 3, "aria-label": "Mission goal" }),
        /* @__PURE__ */ d("div", { className: "workbench-control-hero-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => {
            he();
          }, disabled: E, children: "Start mission" }),
      ...
      /* @__PURE__ */ a("div", { className: "workbench-team-preset-strip", "aria-label": "Team presets", children: _.map((m) => /* @__PURE__ */ d(
        "button",
        {
          className: `workbench-team-preset-card ${m.id === be.id ? "active" : ""} ${m.readiness}`,
          onClick: () => M(m.id),
          children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: m.label }),
              /* @__PURE__ */ a("span", { children: m.readiness })
            ] }),
            /* @__PURE__ */ a("p", { children: m.summary }),
            /* @__PURE__ */ a("small", { children: m.assignments.map((g) => g.resourceLabel).slice(0, 3).join(" · ") })
          ]
        },
        m.id
      )) }),
      ue ? /* @__PURE__ */ d("div", { className: "workbench-control-success", children: [
        "Mission ready: ",
        /* @__PURE__ */ a("code", { children: ue.rootPath })
      ] }) : null,
      j ? /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: j }) : null
    ] }),
    /* @__PURE__ */ d("section", { className: "workbench-control-grid", children: [
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
          /* @__PURE__ */ a("strong", { children: "System inventory" }),
          /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: () => {
            ce();
          }, disabled: E, children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-control-metrics", children: ve.map((m) => /* @__PURE__ */ d("div", { className: "workbench-control-metric", children: [
          /* @__PURE__ */ a("strong", { children: m.value }),
          /* @__PURE__ */ a("span", { children: m.label }),
          /* @__PURE__ */ a("em", { children: m.detail })
        ] }, m.label)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-agent-brand-grid", children: Xe.map((m) => /* @__PURE__ */ d("div", { className: `workbench-agent-brand-card ${m.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ d("div", { children: [
            /* @__PURE__ */ a("strong", { children: m.label }),
            /* @__PURE__ */ a("span", { children: m.status })
          ] }),
          /* @__PURE__ */ a("b", { children: m.value }),
          /* @__PURE__ */ a("p", { children: m.detail })
        ] }, m.label)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-doc-grid", children: To.map((m) => /* @__PURE__ */ d("button", { className: "workbench-doc-card", onClick: () => y(m), title: m.summary, children: [
          /* @__PURE__ */ a("strong", { children: m.title }),
          /* @__PURE__ */ a("p", { children: m.summary }),
          /* @__PURE__ */ a("small", { children: m.tags.join(" · ") })
        ] }, m.id)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-fabric-flow", children: Se.map((m, g) => /* @__PURE__ */ d("div", { className: `workbench-fabric-node ${m.status.replace(/\s+/g, "-")}`, children: [
          /* @__PURE__ */ a("span", { children: g + 1 }),
          /* @__PURE__ */ a("strong", { children: m.label }),
          /* @__PURE__ */ a("em", { children: m.status }),
          /* @__PURE__ */ a("p", { children: m.detail })
        ] }, m.label)) }),
        /* @__PURE__ */ d("div", { className: "workbench-fabric-actions", children: [
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(Ht.sharedFiles, "Shared Files"), children: "Open shared files" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(Ht.podInspector, "Pod Inspector"), children: "Open pods" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(Ht.channels, "Channels"), children: "Open channels" }),
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => D(Ht.settings, "Agent Settings"), children: "Open agent settings" })
        ] })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ a("header", { children: /* @__PURE__ */ a("strong", { children: "First missions" }) }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => h("Review this repo with local OpenCode/Claude, then ask Codex or pi for an independent review. Save all transcripts and proposed patches in the mission folder."), children: "Review + patch repo" }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => h("Ask OpenClaw for independent critique, use Hermes when allocated for deep planning/copy, then run local Claude/OpenCode/Codex for implementation. Keep shared context in the mission folder and approval-gate all edits."), children: "OpenClaw/Hermes + local agent" }),
        /* @__PURE__ */ a("button", { className: "workbench-control-preset", onClick: () => h("Coordinate creative output: script, audio, visuals, render assets, and final handoff through app skills and shared folders."), children: "Creative production" })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card", children: [
        /* @__PURE__ */ d("header", { children: [
          /* @__PURE__ */ a("strong", { children: "Resume" }),
          /* @__PURE__ */ a("span", { children: X.length ? `${X.length} missions` : "none yet" })
        ] }),
        X.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Create a mission once; it stays in Tytus Home/Missions and can be resumed here." }) : null,
        X.slice(0, 4).map((m) => /* @__PURE__ */ d("button", { className: "workbench-control-preset", onClick: () => {
          zt(m), l();
        }, title: m.rootPath, children: [
          m.title,
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card", children: [
        ...
        /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: n, children: [
          /* @__PURE__ */ a(zn, { size: 18 }),
          "Open workspace folder"
        ] }),
        /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: t, children: [
          /* @__PURE__ */ a(ha, { size: 18 }),
          "Open file"
        ] }),
        /* @__PURE__ */ d("button", { className: "workbench-start-link", onClick: r, children: [
          /* @__PURE__ */ a(Nn, { size: 18 }),
          "New mission note"
        ] }),
        s.length ? /* @__PURE__ */ d("div", { className: "workbench-control-recent", children: [
          /* @__PURE__ */ a("span", { children: "Recent" }),
          s.slice(0, 3).map((m) => /* @__PURE__ */ a("button", { onClick: () => {
            i(m);
          }, children: m.name }, `${m.path}-${m.at}`))
        ] }) : null
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ d("header", { children: [
          /* @__PURE__ */ a("strong", { children: "Resource graph" }),
          /* @__PURE__ */ a("span", { children: w ? `${w.resources.length} discovered` : "not loaded" })
        ] }),
        /* @__PURE__ */ d("div", { className: "workbench-control-resource-list", children: [
          Le.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph yet. Start Tytus tray beta30 or newer, then refresh." }) : null,
          Le.map((m) => /* @__PURE__ */ d("div", { className: "workbench-control-resource", children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: xe(m) }),
              /* @__PURE__ */ a("span", { children: Sn(m) })
            ] }),
            /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${m.status}`, children: m.status })
          ] }, m.id))
        ] }),
        L.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: L.slice(0, 2).map((m) => /* @__PURE__ */ d("span", { children: [
          m.code,
          ": ",
          m.message
      ...
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph home", children: fe.map((m, g) => /* @__PURE__ */ d("button", { className: `workbench-task-card ${m.status}`, onClick: () => h(m.prompt), children: [
          /* @__PURE__ */ a("span", { children: g + 1 }),
          /* @__PURE__ */ a("strong", { children: m.title }),
          /* @__PURE__ */ a("em", { children: m.resourceHint }),
          /* @__PURE__ */ a("small", { children: m.assignedResourceLabel })
        ] }, m.id)) })
      ] }),
      /* @__PURE__ */ d("article", { className: "workbench-control-card wide", children: [
        /* @__PURE__ */ a("header", { children: /* @__PURE__ */ a("strong", { children: "Control loop" }) }),
        /* @__PURE__ */ d("ol", { className: "workbench-control-loop", children: [
...
function $o({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: s }) {
  const i = oe(() => _o(e, t), [e, t]), o = oe(() => {
    const l = /* @__PURE__ */ new Map();
    return i.forEach((p) => l.set(p.file.id, [...l.get(p.file.id) ?? [], p])), Array.from(l.values()).slice(0, 50);
  }, [i]);
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? o.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : o.map((l) => {
        const p = l[0].file;
        return /* @__PURE__ */ d("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(Hn, { file: p, active: p.id === s, onOpen: () => r(p) }),
          l.slice(0, 5).map((y) => /* @__PURE__ */ d("button", { className: "workbench-search-hit", onClick: () => r(p, y.lineNumber), title: y.line, children: [
            /* @__PURE__ */ a("span", { className: "workbench-search-line", children: y.lineNumber }),
            /* @__PURE__ */ a("span", { children: y.line })
          ] }, `${p.id}-${y.lineNumber}-${y.line}`)),
          l.length > 5 && /* @__PURE__ */ d("div", { className: "workbench-search-more", children: [
            "+",
            l.length - 5,
            " more matches"
          ] })
...
function ec(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((s) => ({
    label: s.path,
    detail: `Open ${ya(s.language)} file`,
  ...
  return se(() => {
  ...
  }, [e.onClose]), /* @__PURE__ */ a("div", { className: "workbench-command-overlay", role: "dialog", "aria-label": "Command Palette", children: /* @__PURE__ */ d("div", { className: "workbench-command-palette", children: [
    /* @__PURE__ */ a(
      "input",
      {
    ...
    /* @__PURE__ */ a("div", { className: "workbench-command-list", children: r.map((s) => /* @__PURE__ */ d(
      "button",
      {
        disabled: s.disabled,
...
function tc({ content: e }) {
  const t = oe(() => Pa(e), [e]);
  return /* @__PURE__ */ d("aside", { className: "workbench-markdown-preview", children: [
    /* @__PURE__ */ d("div", { className: "workbench-preview-title", children: [
...
function nc(e) {
  const t = e.session ? oo(e.session) : "pending";
  return /* @__PURE__ */ d("div", { className: "workbench-manual-check-panel", children: [
    ...
    e.session ? /* @__PURE__ */ d(We, { children: [
      /* @__PURE__ */ d("div", { className: "workbench-manual-check-head", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-manual-check-grid", children: [
        /* @__PURE__ */ d("section", { children: [
          ...
          /* @__PURE__ */ a("div", { className: "workbench-check-command-list", children: e.session.commands.map((n) => /* @__PURE__ */ d(
            "button",
            {
          ...
          /* @__PURE__ */ d("div", { className: "workbench-check-add-row", children: [
            /* @__PURE__ */ a(
              "input",
              {
        ...
        /* @__PURE__ */ d("section", { children: [
          ...
          /* @__PURE__ */ d("select", { value: e.status, onChange: (n) => e.setStatus(n.target.value), children: [
          ...
          /* @__PURE__ */ d("div", { className: "workbench-check-actions", children: [
      ...
      e.session.results.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-check-results", children: [
        /* @__PURE__ */ a("label", { children: "Captured results" }),
        e.session.results.map((n, r) => /* @__PURE__ */ d("article", { children: [
...
function ac(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ d("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ a("span", {}),
      /* @__PURE__ */ a("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ a(Me, { size: 14 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ a(tr, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function rc(e) {
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    /* @__PURE__ */ a("div", { className: "workbench-secondary-resizer", onPointerDown: e.onResizeStart, title: "Resize Chat" }),
    /* @__PURE__ */ d("div", { className: "workbench-secondary-tabs", children: [
      /* @__PURE__ */ d("div", { className: "workbench-secondary-tab-group", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(is, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(Jr, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(Me, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(sc, { ...e }) : e.tab === "agents" ? /* @__PURE__ */ a(
      nr,
      {
        host: e.host,
    ...
    ) : /* @__PURE__ */ a(tr, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function _a({ body: e }) {
  const t = oe(() => So(e), [e]), [n, r] = z(null), s = x((i, o) => {
    (async () => await ft(o) && (r(i), window.setTimeout(() => r((p) => p === i ? null : p), 1200)))();
  }, []);
  return /* @__PURE__ */ a("div", { className: "workbench-rich-body", children: t.map((i) => {
    if (i.type === "code") {
      const o = n === i.key;
      return /* @__PURE__ */ d("div", { className: "workbench-rich-code", children: [
        /* @__PURE__ */ d("div", { className: "workbench-rich-code-head", children: [
          /* @__PURE__ */ a("span", { children: i.language }),
          /* @__PURE__ */ d("button", { onClick: () => s(i.key, i.body), title: "Copy code block", children: [
            o ? /* @__PURE__ */ a(qr, { size: 12 }) : /* @__PURE__ */ a(mn, { size: 12 }),
            o ? "Copied" : "Copy"
          ] })
        ] }),
    ...
    return /* @__PURE__ */ a(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Pa(i.body) }
      },
      i.key
    );
  }) });
...
function sc(e) {
  const t = e.chatInput.trim().length > 0 && !e.busy, n = Qe(null), [r, s] = z(!0), [i, o] = z(!1), l = oe(() => e.chatMessages.map((h) => `${h.id}:${h.status ?? ""}:${h.body.length}`).join("|"), [e.chatMessages]);
  se(() => {
    const h = n.current;
    h && (r ? (h.scrollTop = h.scrollHeight, o(!1)) : o(!0));
  }, [r, l]);
  const p = x(() => {
    const h = n.current;
    if (!h) return;
    const C = h.scrollHeight - h.scrollTop - h.clientHeight < 48;
    s(C), C && o(!1);
  }, []), y = x(() => {
    const h = n.current;
    h && (h.scrollTop = h.scrollHeight, s(!0), o(!1));
  }, []), b = x((h) => {
    ft(h.body);
  }, []);
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
    /* @__PURE__ */ d("div", { className: "workbench-chat-threadbar", children: [
      /* @__PURE__ */ d(
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((h) => /* @__PURE__ */ d("option", { value: h.id, children: [
              h.title,
              " · ",
              ic(h.lastMessageAt ?? h.updatedAt)
            ] }, h.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ d("div", { ref: n, className: "workbench-chat-transcript", onScroll: p, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ d("div", { children: [
        /* @__PURE__ */ a(ts, { size: 48 }),
      ...
      ] }) }) : e.chatMessages.map((h) => /* @__PURE__ */ d("div", { className: `workbench-chat-message ${h.role}`, children: [
        ...
        /* @__PURE__ */ a("br", {}),
        /* @__PURE__ */ a(_a, { body: h.body }),
        h.gatewayLabel ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ a("br", {}),
          /* @__PURE__ */ a("small", { children: h.gatewayLabel })
        ] }) : null,
        h.role === "assistant" && h.status !== "streaming" && h.status !== "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => b(h), title: "Copy this answer", children: [
            /* @__PURE__ */ a(mn, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => e.saveMessageAsArtifact(h), title: "Save this answer as an output artifact", children: [
            /* @__PURE__ */ a(Nn, { size: 12 }),
            " Save"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => e.rememberMessage(h), title: "Store this answer in Atomek memory", children: [
            /* @__PURE__ */ a(ma, { size: 12 }),
            " Remember"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => e.previewEditFromMessage(h), disabled: e.workspaceFileCount === 0, title: "Preview an editable patch from this answer", children: [
            /* @__PURE__ */ a(Pn, { size: 12 }),
            " Preview"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(h), disabled: e.busy, title: "Regenerate this answer", children: [
            /* @__PURE__ */ a(ut, { size: 12 }),
            " Regenerate"
          ] })
        ] }) : null,
        h.role === "assistant" && h.status === "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => b(h), title: "Copy this error", children: [
            /* @__PURE__ */ a(mn, { size: 12 }),
            " Copy"
          ] }),
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action regen", onClick: () => e.regenerateMessage(h), disabled: e.busy, children: [
            /* @__PURE__ */ a(ut, { size: 12 }),
            " Retry"
          ] })
        ] }) : null
      ] }, h.id)),
      i ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: y, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ d("div", { className: "workbench-chat-tip", children: [
        /* @__PURE__ */ a("span", { children: "Context" }),
        /* @__PURE__ */ a("strong", { children: Ms(e.contextScope) }),
        /* @__PURE__ */ a("em", { children: sa(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          /* @__PURE__ */ d(
            "select",
            {
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ d(We, { children: [
            /* @__PURE__ */ d("button", { className: "workbench-chat-chip-button", onClick: e.refreshProjectIndex, disabled: e.busy, children: [
              /* @__PURE__ */ a(ut, { size: 12 }),
              " Index"
            ] }),
            /* @__PURE__ */ d("span", { className: `workbench-chat-chip ${e.projectIndexStale ? "warn" : "muted"}`, title: "Project index used for query-scoped retrieval", children: [
              /* @__PURE__ */ a(qt, { size: 13 }),
              " ",
              e.projectIndexSummary,
              e.projectIndexStale ? " · stale" : ""
            ] })
          ] }) : null,
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            /* @__PURE__ */ a(Jn, { size: 13 }),
            " No file context"
          ] }) : e.contextAttachments.map((h) => {
            const w = typeof h.score == "number" ? h.score.toFixed(2) : null, C = typeof h.vectorScore == "number" ? h.vectorScore.toFixed(2) : null, X = typeof h.keywordScore == "number" ? h.keywordScore.toFixed(1) : null, K = [
              h.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: K, children: [
              /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(h), disabled: !h.fileId, title: "Reveal context", children: /* @__PURE__ */ a(Jn, { size: 13 }) }),
              h.label,
              w ? /* @__PURE__ */ a("small", { children: w }) : null,
              h.snippet ? /* @__PURE__ */ d("small", { children: [
                h.snippet.slice(0, 60),
                h.snippet.length > 60 ? "…" : ""
              ] }) : null,
              h.dirty ? /* @__PURE__ */ a("small", { children: "dirty" }) : null,
              h.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(h), title: "Remove context", children: /* @__PURE__ */ a(Me, { size: 11 }) }) : null
            ] }, h.id);
          }),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: sa(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(ba, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ a(us, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function sa(e, t, n) {
  const r = e.model.trim(), i = [e.gatewayPreference === "auto" ? t : er(e.gatewayPreference)];
  return r && i.push(r), n > 0 && i.push(`${n} memories`), i.join(" · ");
}
function ic(e) {
...
function lc({ host: e, appName: t, currentVersion: n }) {
  const r = e.apps, [s, i] = z(null), [o, l] = z(!1), [p, y] = z(null), b = x(async () => {
    if (!r?.checkUpdate) {
      y("Update checks need a newer Tytus OS build.");
      return;
    }
    l(!0), y(null);
    try {
      const X = await r.checkUpdate();
      i(X), X.error && y(X.error);
    } catch (X) {
      y(X instanceof Error ? X.message : String(X));
    } finally {
      l(!1);
    }
  }, [r]);
  ...
  const h = x(async () => {
    if (!r?.updateSelf) {
      y("Update needs a newer Tytus OS build.");
      return;
    }
    l(!0), y(null);
    try {
      const X = await r.updateSelf();
      i(X), y(X.error ?? `${t} updated. Close and reopen the app to load the new bundle.`);
    } catch (X) {
      y(X instanceof Error ? X.message : String(X));
    } finally {
      l(!1);
    }
  }, [r, t]), w = s?.latestVersion ?? n, C = !!s?.updateAvailable;
  return /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
    ...
    /* @__PURE__ */ d("div", { style: { display: "flex", gap: 8, flexWrap: "wrap" }, children: [
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: () => void b(), disabled: o, children: [
        /* @__PURE__ */ a(ut, { size: 14 }),
        o ? "Checking…" : "Check for update"
      ] }),
      C ? /* @__PURE__ */ a("button", { className: "workbench-button-primary", onClick: () => void h(), disabled: o, children: o ? "Updating…" : `Update ${t}` }) : null
    ] }),
    /* @__PURE__ */ a("div", { className: "workbench-settings-note", children: p ?? (s ? C ? "Update available." : "You are running the latest available version." : "Checking latest version…") })
  ] });
}
function dc(e) {
  const [t, n] = z([]), [r, s] = z("Loading gateway models…"), [i, o] = z([]), [l, p] = z("Checking embedding capability…");
  se(() => {
    ...
    return (async () => {
      ...
      try {
        const X = await e.host.ai.listModels({
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        n(X.map((K) => ({ id: K.id, gatewayLabel: K.gatewayLabel }))), s(X.length > 0 ? `${X.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (X) {
        if (w.signal.aborted) return;
        n([]), s(`Model discovery failed: ${X instanceof Error ? X.message : String(X)}`);
      }
    })(), () => w.abort();
    ...
    return (async () => {
      ...
      try {
        const K = await _i(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: w.signal
        });
        if (w.signal.aborted) return;
        o(K.map((E) => ({ id: E.id, gatewayLabel: E.gatewayLabel ?? E.source ?? "AIL" }))), p(K.length > 0 ? `${K.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (K) {
        if (w.signal.aborted) return;
        o([]), p(`Embedding model discovery failed: ${K instanceof Error ? K.message : String(K)}`);
      }
    })(), () => w.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-tab", children: /* @__PURE__ */ d("section", { className: "workbench-settings-page", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ d("header", { className: "workbench-settings-header", children: [
      /* @__PURE__ */ a(pa, { size: 15 }),
      /* @__PURE__ */ a("strong", { children: "Atomek Settings" }),
      /* @__PURE__ */ d("span", { style: { fontSize: 11, color: "var(--accent-primary)", border: "1px solid color-mix(in srgb, var(--accent-primary) 40%, transparent)", borderRadius: 999, padding: "2px 7px", fontWeight: 800 }, children: [
        "v",
        aa
      ] }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ a(Me, { size: 15 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ a(lc, { host: e.host, appName: "Atomek", currentVersion: aa }),
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("label", { className: "workbench-settings-label", children: [
          "Gateway",
          /* @__PURE__ */ d(
            "select",
            {
        ...
        /* @__PURE__ */ d("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          er(e.chatSettings.gatewayPreference),
    ...
    /* @__PURE__ */ d("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(Vt), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Close tab" })
    ] })
  ] }) });
}
function tr({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, captureManualCheck: s, openOutputAsFile: i, previewEditFromOutput: o, canPreviewEdit: l = !1, compact: p = !1 }) {
  return /* @__PURE__ */ d("div", { className: `workbench-panel-list ${p ? "compact" : ""}`, children: [
    /* @__PURE__ */ d("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: r, children: [
        /* @__PURE__ */ a(kn, { size: 14 }),
        "AI synthesis"
      ] }),
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle", onClick: s, children: [
        /* @__PURE__ */ a(ua, { size: 14 }),
        "Capture check"
      ] }),
      /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((y) => /* @__PURE__ */ d("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ d("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ a("strong", { children: y.title }),
        /* @__PURE__ */ a("span", { children: y.source === "ai" ? `AI · ${y.kind}` : y.kind }),
        /* @__PURE__ */ a("button", { onClick: () => void ft(y.body), children: "Copy" }),
        /* @__PURE__ */ a("button", { onClick: () => i(y), children: "Open as file" }),
        o ? /* @__PURE__ */ a(
          "button",
          {
            className: ia(y.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => o(y),
            disabled: !l,
            children: ia(y.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        y.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(y.id), children: "Delete" }) : null
      ] }),
      /* @__PURE__ */ a(_a, { body: y.body })
    ] }, y.id))
  ] });
}
function uc({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ d("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ d("div", { children: [
        /* @__PURE__ */ a("strong", { children: "Review AI edit" }),
        /* @__PURE__ */ a("span", { children: e.fileName })
      ] }),
      /* @__PURE__ */ a("button", { title: "Close", onClick: r, children: /* @__PURE__ */ a(Me, { size: 16 }) })
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-edit-review-meta", children: [
      /* @__PURE__ */ d("span", { children: [
        "Source: ",
        e.sourceTitle
      ] }),
    ...
    /* @__PURE__ */ d("div", { className: "workbench-edit-review-grid", children: [
      /* @__PURE__ */ d("div", { className: "workbench-edit-review-pane", children: [
        /* @__PURE__ */ a("h4", { children: "Current" }),
        /* @__PURE__ */ a("pre", { children: e.originalContent })
      ] }),
      /* @__PURE__ */ d("div", { className: "workbench-edit-review-pane proposed", children: [
        /* @__PURE__ */ a("h4", { children: "Proposed" }),
        /* @__PURE__ */ a("pre", { children: e.proposedContent })
      ] })
    ] }),
    /* @__PURE__ */ d("footer", { className: "workbench-edit-review-actions", children: [
...
function hc({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  const s = e.edits.reduce((i, o) => ({
    added: i.added + o.stats.added,
    removed: i.removed + o.stats.removed,
    changed: i.changed + o.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    /* @__PURE__ */ d("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ d("div", { children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-edit-review-meta", children: [
      ...
      e.skipped.length > 0 ? /* @__PURE__ */ d("span", { children: [
        e.skipped.length,
        " skipped"
      ] }) : null
    ] }),
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((i) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        /* @__PURE__ */ d("header", { children: [
        ...
        /* @__PURE__ */ a("pre", { children: kc(i.proposedContent) })
      ] }, i.fileId)),
      e.skipped.length > 0 ? /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card skipped", children: [
        /* @__PURE__ */ d("header", { children: [
...
function mc(e, t) {
  ...
  if (e)
    n.push("", `Active file: ${e.path}`, `Language: ${e.language}`, `Dirty: ${e.dirty ? "yes" : "no"}`, "", "Active file content:", "```" + e.language, fc(e.content), "```");
  else if (t.length > 0) {
...
function nr({
  ...
  const [l, p] = z([]), [y, b] = z([]), [h, w] = z(null), [C, X] = z(!1), [K, E] = z(null), [q, ue] = z("Review the active Atomek context. Return concise findings. If you propose edits, output a unified diff or fenced replacement blocks so Atomek can preview before applying."), [re, j] = z(null), [B, R] = z([]), [M, ce] = z(() => Io()), [D, he] = z([]), [ve, Xe] = z([]), [Se, Le] = z([]), L = Qe([]), [_, be] = z("task-execute"), [fe, m] = z("mission"), g = B.find((u) => u.status === "running" || u.status === "canceling") ?? B[0] ?? null, T = o === "dock", F = oe(() => pt(h, $e(q || M?.goal || "", h, M?.teamPresetId)), [q, M?.goal, M?.teamPresetId, h]), ie = oe(() => Xt(q || M?.goal || "", h, F.id), [q, M?.goal, F.id, h]), H = ie.find((u) => u.id === _) ?? ie[1] ?? ie[0] ?? null, ke = i.filter((u) => u.dirty).length, Ne = s ? `${s.path} · ${s.language} · ${s.content.length.toLocaleString()} chars${s.dirty ? " · dirty" : ""}` : i.length > 0 ? `${i.length} open editor${i.length === 1 ? "" : "s"}${ke ? ` · ${ke} dirty` : ""}` : "No active file — jobs run with only your typed prompt.", Ve = oe(() => [
    {
  ...
  const He = x(async (u = M) => {
    if (!u?.rootPath || !e.missions?.listRuns)
      return Le([]), [];
    try {
      const k = await e.missions.listRuns(u.rootPath);
      return Le(k), L.current = k, k;
    } catch (k) {
      return t(`Mission run history failed: ${k instanceof Error ? k.message : String(k)}`), [];
    }
  }, [e.missions, M, t]), Pe = x(async (u, k) => {
    if (!u) return;
    const N = [...k].sort((A, I) => ra(I).localeCompare(ra(A))).slice(0, 100), S = N.map((A) => JSON.stringify(A)).join(`
`) + (N.length ? `
` : "");
    u.rootPath && e.missions?.write ? await e.missions.write({ rootPath: u.rootPath, files: [{ path: "RUNS.jsonl", content: S }] }) : u.handle && await wn(u.handle, "RUNS.jsonl", S), Le(N), L.current = N;
  }, [e.missions]), ye = x(async (u, k = M) => {
    if (!k) return;
    const N = [
      u,
      ...L.current.filter((S) => S.id !== u.id)
    ];
    await Pe(k, N);
  }, [M, Pe]), ee = x(async () => {
    if (!e.local?.listTools && !e.skills?.list && !e.resources?.list) {
      p([]), b([]), w(null), E("This Tytus host build does not expose local tools, resource graph, or skill registry yet.");
      return;
    }
    X(!0), E(null);
    try {
      const [u, k, N, S] = await Promise.all([
        e.local?.listTools?.().catch((A) => (t(`Local tool discovery failed: ${A instanceof Error ? A.message : String(A)}`), [])) ?? Promise.resolve([]),
        e.skills?.list?.().catch((A) => (t(`Skill registry discovery failed: ${A instanceof Error ? A.message : String(A)}`), [])) ?? Promise.resolve([]),
        e.resources?.list?.().catch((A) => (t(`Resource graph discovery failed: ${A instanceof Error ? A.message : String(A)}`), null)) ?? Promise.resolve(null),
        e.missions?.list?.().catch((A) => (t(`Mission list failed: ${A instanceof Error ? A.message : String(A)}`), [])) ?? Promise.resolve([])
      ]);
      p(u), b(k), w(N), he(S), M?.rootPath && He(M), t(`Agent team loaded · ${u.length} tools · ${k.length} skills · ${S.length} missions${N ? ` · ${N.resources.length} resources` : ""}`);
    } catch (u) {
      E(u instanceof Error ? u.message : String(u));
    } finally {
      X(!1);
    }
  }, [e.local, e.missions, e.resources, e.skills, He, M, t]);
  se(() => {
    ee();
  }, [ee]), se(() => {
    const u = (k) => {
      const N = k.detail;
      N?.missionId && (ce(N), He(N)), ee();
    };
    return window.addEventListener(yn, u), () => window.removeEventListener(yn, u);
  }, [ee, He]);
  const te = x((u) => {
    const k = qo(u);
    ce(k), zt(k), He(k), Xe([{
      ...
      data: { runCount: u.runCount ?? 0, taskCount: u.taskCount ?? 0 }
    }]), ue(u.goal || `Continue mission ${u.title}. Review MISSION.md, TASKS.md, RESOURCES.md, and runs/ before acting.`), t(`Resumed mission: ${u.rootPath}`);
  }, [He, t]), $ = x(async (u, k, N = []) => {
    const S = [
      ...ve,
      ...N,
      { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.pack.write", message: "Mission context pack written from Atomek" }
    ], A = $e(k || u.goal, h, u.teamPresetId), I = Xt(k || u.goal, h, A), U = [
      { path: "MISSION.md", content: Dt(u, h, s, i, k, A) },
      { path: "MISSION.json", content: Ga(u, h, k, A) },
      { path: "RESOURCES.md", content: Ft(h) },
      { path: "TASKS.md", content: Ua(I) },
      { path: "HANDOFF.md", content: Ba(u) },
      { path: "INBOX.md", content: `# Mission inbox
...
Approved handoffs, final artifacts, and user-ready summaries go here.
` },
      { path: "AUDIT.jsonl", content: S.map((Y) => JSON.stringify(Y)).join(`
`) + `
` },
      { path: "RUNS.jsonl", content: L.current.map((Y) => JSON.stringify(Y)).join(`
`) + (L.current.length ? `
` : "") },
      { path: "runs/README.md", content: `# Mission runs
...
` }
    ...
    if (u.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: u.rootPath, files: U });
    else if (u.handle) {
      await lt(u.handle, "runs"), await lt(u.handle, "outputs"), await lt(u.handle, "proposals"), await lt(u.handle, "approvals");
      for (const Y of U) await jo(u.handle, Y.path, Y.content);
    } else
      throw new Error("Mission has neither tray rootPath nor browser folder handle");
    Xe(S), zt(u);
  }, [s, e.missions, ve, i, h]), ne = x(async (u, k = {}) => {
    if (M)
      return await $(M, u), M;
    const N = `Atomek mission ${(/* @__PURE__ */ new Date()).toLocaleString()}`, S = u.trim() || "Coordinate Tytus resources for the current Atomek task.";
    let A = null;
    if (e.missions?.create) {
      const U = await e.missions.create({ title: N, goal: S });
      A = {
        missionId: U.missionId,
        title: U.title,
        goal: U.goal,
        rootPath: U.rootPath,
        name: U.rootPath.split("/").pop() || U.missionId,
        source: "tray"
      };
      ...
      A = {
        handle: U,
        name: U.name,
        missionId: `mission-${Date.now()}-${Fo(U.name)}`,
        title: N,
        goal: S,
        source: "browser"
      };
    ...
    if (!A) return null;
    const I = { ts: (/* @__PURE__ */ new Date()).toISOString(), kind: "mission.folder.ready", message: `Mission folder ready: ${A.rootPath ?? A.name}` };
    return ce(A), zt(A), Xe([I]), await $(A, S, [I]), t(`Mission pack ready in ${A.rootPath ?? A.name}`), A;
  }, [e.missions, M, t, $]), _e = x(async (u) => {
    if (!e.local?.openTerminal) {
      t("Terminal bridge unavailable in this host build");
      return;
    }
    try {
      const k = q.trim() || `Open ${u.label} from Atomek with current context.`, N = u.kind === "ai-cli" ? await ne(k) : M;
      await e.local.openTerminal({
        toolId: u.id,
        command: u.command,
        cwd: N?.rootPath ?? oa(s),
        prompt: N ? `Atomek mission pack ready at ${N.rootPath ?? N.name}. Read MISSION.md and RESOURCES.md. ${Ne}` : `Opened from Atomek. ${Ne}`
      }), t(u.kind === "ai-cli" ? `Started ${u.label} in a fresh Tytus Terminal with mission context.` : "Opened Tytus Terminal");
    } catch (k) {
      t(`Terminal launch failed: ${k instanceof Error ? k.message : String(k)}`);
    }
  }, [s, Ne, ne, e.local, q, M, t]), Et = x(async () => {
    try {
      await ne(q.trim(), { allowBrowserPicker: !0 }) || t("Mission folder setup skipped.");
    } catch (u) {
      t(`Mission folder setup failed: ${u instanceof Error ? u.message : String(u)}`);
    }
  }, [ne, q, t]), Ae = x(async (u, k, N, S = M, A) => {
    if (!S) return;
    const I = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-"), U = A ?? `runs/${I}-${u.id}.md`, Y = U.split("/").pop() || `${I}-${u.id}.md`;
    if (S.rootPath && e.missions?.write)
      await e.missions.write({ rootPath: S.rootPath, files: [{ path: U, content: k }] });
    else if (S.handle) {
      const me = await lt(S.handle, "runs");
      await wn(me, Y, k);
    }
    await $(S, q, [{
      ts: (/* @__PURE__ */ new Date()).toISOString(),
      kind: "local-cli.run.complete",
      message: `${u.label} exited ${N}; transcript saved to ${U}`,
      data: { toolId: u.id, exitCode: N, transcript: U }
    }]);
  }, [e.missions, q, M, $]), et = x(async (u) => {
    ...
    let N = null;
    try {
      N = await ne(k), N && await $(N, k, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "local-cli.run.start",
        message: `${u.label} background review started`,
        data: { toolId: u.id, taskId: H?.id ?? "manual", taskTitle: H?.title ?? "Manual run" }
      }]);
    } catch (I) {
      t(`Mission pack failed before local job start: ${I instanceof Error ? I.message : String(I)}`);
      return;
    }
    j(u.id);
    const S = `local-run-${Date.now()}-${u.id}`, A = ot();
    R((I) => [{
      id: S,
      toolId: u.id,
      label: u.label,
      ...
      startedAt: Date.now(),
      taskId: H?.id ?? "manual",
      taskTitle: H?.title ?? "Manual run",
      lines: [`[Atomek] Starting ${u.label} local job for ${H?.title ?? "manual run"}…`]
    }, ...I].slice(0, 6));
    try {
      const I = await e.local.runJob({
        toolId: u.id,
        prompt: N ? [
          "Tytus mission context pack is active.",
          `Mission: ${N.title}`,
          `Goal: ${N.goal}`,
          N.rootPath ? `Mission folder: ${N.rootPath}` : `Mission folder: ${N.name}`,
...
`)
      }), U = I.transcriptPath ?? (N?.rootPath ? `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id}.md` : void 0), Y = [
        `[Atomek] Started ${u.label} local job ${I.id}`,
        H ? `[Atomek] Task: ${H.title} (${H.id})` : "[Atomek] Task: manual",
        N?.rootPath ? `[Atomek] Mission: ${N.rootPath}` : ""
      ].filter(Boolean), me = {
        id: S,
        jobId: I.id,
        toolId: u.id,
        ...
        taskTitle: H?.title ?? "Manual run",
        transcriptPath: U,
        summary: `Started ${u.label} for ${H?.title ?? "manual run"}`
      };
      ye(me, N).catch((G) => {
        t(`Mission run index failed: ${G instanceof Error ? G.message : String(G)}`);
      }), R((G) => G.map((W) => W.id === S ? { ...W, jobId: I.id, transcriptPath: U } : W));
      const De = (G) => {
        R((W) => W.map((Q) => Q.id === S ? G(Q) : Q));
      };
      e.local.streamJob(I.id, {
        onLog: (G) => {
          Y.push(G), De((W) => ({ ...W, lines: Y.slice(-500) }));
        },
        onDone: (G) => {
          G && (Y.push(G), De((W) => ({ ...W, lines: Y.slice(-500) })));
        },
        onFail: (G) => {
          Y.push(`[FAIL] ${G}`), De((W) => ({
            ...W,
          ...
          }), r(`${u.label} local job failed`, Y.join(`
`)), Ae(u, Y.join(`
`), -1, N, U).catch((W) => {
            t(`Mission transcript save failed: ${W instanceof Error ? W.message : String(W)}`);
          }), j(null);
        },
        onExit: (G) => {
          const W = [
...
`);
          De((Q) => ({
            ...
            finishedAt: ot(),
            exitCode: G,
            summary: `${u.label} exited ${G}`
          }, N).catch((Q) => {
            t(`Mission run index failed: ${Q instanceof Error ? Q.message : String(Q)}`);
          }), r(`${u.label} local job`, W), Ae(u, W, G, N, U).catch((Q) => {
            t(`Mission transcript save failed: ${Q instanceof Error ? Q.message : String(Q)}`);
          }), j(null);
        },
        onError: () => t(`Local job stream issue for ${u.label}`)
      }), t(`Started ${u.label} mission run${H ? ` · ${H.title}` : ""}`);
    } catch (I) {
      j(null), R((U) => U.map((Y) => Y.id === S ? {
        ...Y,
        status: "failed",
        finishedAt: Date.now(),
        lines: [...Y.lines, `[Atomek] Failed to start: ${I instanceof Error ? I.message : String(I)}`]
      } : Y)), t(`Local job failed to start: ${I instanceof Error ? I.message : String(I)}`);
    }
  }, [s, ne, e.local, q, i, h, r, Ae, H, t, ye, $]), Yt = x(async (u) => {
    const k = gn(u);
    if (!k) {
      t(`Cannot dispatch ${xe(u)}: missing pod id`);
      return;
    }
    const N = q.trim();
    if (!N) {
    ...
    let S = null;
    const A = xe(u), I = `pod-run-${Date.now()}-${k}`, U = ot(), Y = `runs/${(/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-")}-${u.id.replace(/[^a-z0-9_.-]/gi, "-")}.md`;
    try {
      S = await ne(N), S && await $(S, N, [{
        ts: (/* @__PURE__ */ new Date()).toISOString(),
        kind: "pod-agent.run.start",
        message: `${A} mission task started`,
        data: { resourceId: u.id, podId: k, taskId: H?.id ?? "manual", taskTitle: H?.title ?? "Manual pod run" }
      }]);
    } catch (W) {
      t(`Mission pack failed before pod task: ${W instanceof Error ? W.message : String(W)}`);
      return;
    }
    const me = {
      id: I,
      ...
      startedAt: U,
      taskId: H?.id ?? "manual",
      taskTitle: H?.title ?? "Manual pod run",
      transcriptPath: Y,
      summary: `Started ${A} for ${H?.title ?? "manual pod run"}`
    };
    R((W) => [{
      id: I,
      toolId: u.id,
      ...
      taskTitle: H?.title ?? "Manual pod run",
      transcriptPath: Y,
      lines: [`[Atomek] Dispatching ${A} via Tytus host bridge…`, `[Atomek] Pod: ${k}`, `[Atomek] Task: ${H?.title ?? "manual pod run"}`]
    }, ...W].slice(0, 6)), await ye(me, S).catch((W) => {
      t(`Mission run index failed: ${W instanceof Error ? W.message : String(W)}`);
    });
    const De = [], G = (W) => {
      De.push(W), R((Q) => Q.map((we) => we.id === I ? { ...we, lines: [...we.lines, W].slice(-500) } : we));
    };
    try {
      const W = await e.daemon.callPodEndpoint(k, "/v1/models", { method: "GET" });
      if (!W.ok)
        throw new Error(`/v1/models ${W.status}: ${await W.text()}`);
      const Q = await W.json(), we = vc(Q);
      if (!we)
        throw new Error("pod /v1/models returned no model id");
      G(`[Atomek] Selected pod model from live metadata: ${we}`);
      const pe = await e.daemon.callPodEndpoint(k, "/v1/chat/completions", {
        method: "POST",
...
`)
            },
            {
              role: "user",
              content: [
                S ? `Mission folder: ${S.rootPath ?? S.name}` : "Mission folder: not available",
                S ? `Mission: ${S.title}` : "",
                S ? `Goal: ${S.goal}` : "",
                H ? `Task: ${H.title} (${H.id})` : "Task: manual",
                "",
                "Mission context:",
                S ? Dt(S, h, s, i, N) : "",
                h ? Ft(h) : "",
...
`)
      ...
      if (!pe.ok)
        throw new Error(`/v1/chat/completions ${pe.status}: ${Oe}`);
      let Re = Oe;
      try {
        Re = JSON.parse(Oe);
      } catch {
      }
      const tt = typeof Re == "string" ? Re : yc(Re);
      G(tt);
      const Ze = [
        `# Pod job — ${A}`,
        "",
        ...
        `- Pod: ${k}`,
        `- Captured: ${(/* @__PURE__ */ new Date()).toISOString()}`,
        `- Task: ${H?.title ?? "Manual pod run"}`,
        "",
        tt,
        ""
      ].join(`
`);
      R((nt) => nt.map((Fe) => Fe.id === I ? { ...Fe, status: "complete", finishedAt: Date.now(), lines: [...Fe.lines, "[Atomek] Pod task complete."].slice(-500) } : Fe)), await ye({ ...me, status: "complete", finishedAt: ot(), summary: `${A} completed`, transcriptPath: Y }, S), r(`${A} pod job`, Ze), await Ae({ id: u.id, label: A, kind: "pod-agent", status: u.status }, Ze, 0, S, Y), t(`${A} completed mission task`);
    } catch (W) {
      const Q = W instanceof Error ? W.message : String(W);
      G(`[FAIL] ${Q}`), R((pe) => pe.map((Oe) => Oe.id === I ? { ...Oe, status: "failed", finishedAt: Date.now(), lines: Oe.lines.slice(-500) } : Oe)), await ye({ ...me, status: "failed", finishedAt: ot(), exitCode: -1, summary: Q, transcriptPath: Y }, S).catch((pe) => {
        t(`Mission run index failed: ${pe instanceof Error ? pe.message : String(pe)}`);
      });
      const we = [`# Pod job failed — ${A}`, "", `- Pod: ${k}`, `- Error: ${Q}`, "", "```text", De.join(`
`), "```", ""].join(`
`);
      r(`${A} pod job failed`, we), await Ae({ id: u.id, label: A, kind: "pod-agent", status: u.status }, we, -1, S, Y).catch((pe) => {
        t(`Pod transcript save failed: ${pe instanceof Error ? pe.message : String(pe)}`);
      }), t(`Pod task failed: ${Q}`);
    }
  }, [s, ne, e.daemon, q, i, h, r, Ae, H, t, ye, $]), bt = x(async (u) => {
    if (!u.jobId || !e.local?.cancelJob) {
      t("Local job cancel bridge unavailable in this host build");
      return;
    }
    R((k) => k.map((N) => N.id === u.id ? { ...N, status: "canceling", lines: [...N.lines, "[Atomek] Cancel requested…"] } : N)), ye({
      id: u.id,
      jobId: u.jobId,
      toolId: u.toolId,
      label: u.label,
    ...
    try {
      await e.local.cancelJob(u.jobId), t(`Cancel requested for ${u.label}`);
    } catch (k) {
      R((N) => N.map((S) => S.id === u.id ? { ...S, status: "running", lines: [...S.lines, `[Atomek] Cancel failed: ${k instanceof Error ? k.message : String(k)}`] } : S)), t(`Local job cancel failed: ${k instanceof Error ? k.message : String(k)}`);
    }
  }, [e.local, t, ye]), Jt = x(async (u) => {
    const k = [
      `Use Tytus resource "${xe(u)}" (${u.kind}) for the next mission step.`,
      `Capabilities: ${u.capabilities.join(", ") || "status only"}.`,
      `Sandbox: ${u.sandbox}. Trust: ${u.trustTier}.`,
      u.allowedRoots.length ? `Allowed roots: ${u.allowedRoots.join(", ")}` : "No direct roots exposed.",
...
`);
    ue(k), M && await $(M, k, [{
      ...
      message: `Selected resource ${xe(u)}`,
      data: { resourceId: u.id, kind: u.kind, status: u.status }
    }]), t(`Selected ${xe(u)} for mission`);
  }, [M, t, $]), Zt = x((u) => {
    const k = u.setupAction, N = k?.commandPreview ? `${k.label}: ${k.commandPreview}` : k?.deepLink ? `${k.label}: ${k.deepLink}` : k?.label ?? `${xe(u)} needs setup`;
    t(N), ft(k?.commandPreview ?? k?.deepLink ?? N);
  }, [t]);
  return /* @__PURE__ */ d("aside", { className: T ? "workbench-agent-dock" : "workbench-sidebar", children: [
    T ? null : /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "AGENT TEAM" }),
    /* @__PURE__ */ d("div", { className: T ? "workbench-agent-dock-scroll" : "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ d("div", { className: "workbench-computer-hero", children: [
      ...
      /* @__PURE__ */ d("button", { className: "workbench-button-subtle workbench-computer-refresh", onClick: () => {
        ee();
      }, disabled: C, children: [
        /* @__PURE__ */ a(ut, { size: 14 }),
        " ",
        C ? "Refreshing…" : "Refresh capabilities"
      ] }),
      K && /* @__PURE__ */ a("div", { className: "workbench-inline-error", children: K }),
      /* @__PURE__ */ d("div", { className: "workbench-team-switcher", role: "tablist", "aria-label": "Atomek agent team views", children: [
        /* @__PURE__ */ a("button", { className: fe === "mission" ? "active" : "", onClick: () => m("mission"), children: "Mission" }),
        /* @__PURE__ */ a("button", { className: fe === "runs" ? "active" : "", onClick: () => m("runs"), children: "Runs" }),
        /* @__PURE__ */ a("button", { className: fe === "setup" ? "active" : "", onClick: () => m("setup"), children: "Setup" })
      ] }),
      fe === "mission" ? /* @__PURE__ */ d(We, { children: [
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "MISSION PACK — SHARED CONTEXT" }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-context-card mission", children: [
          /* @__PURE__ */ a("strong", { children: M ? M.title : "No mission folder selected" }),
          /* @__PURE__ */ a("span", { children: M ? `${M.rootPath ?? M.name} · ${M.source} · ${ve.length} audit events · transcripts saved under runs/` : "Atomek creates this automatically before launching local agents. It is the shared folder agents read/write transcripts from." }),
          h ? /* @__PURE__ */ d("span", { children: [
            Za(h.resources),
        ...
        /* @__PURE__ */ d("div", { className: "workbench-team-assignment-list", children: [
          /* @__PURE__ */ d("div", { className: `workbench-team-assignment-summary ${F.readiness}`, children: [
            /* @__PURE__ */ a("strong", { children: F.label }),
            /* @__PURE__ */ d("span", { children: [
              F.readiness,
              " · ",
              F.bestFor
          ...
          F.assignments.map((u) => /* @__PURE__ */ d("div", { className: "workbench-team-assignment-row", children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: u.label }),
              /* @__PURE__ */ a("span", { children: u.resourceLabel })
            ] }),
            /* @__PURE__ */ a("em", { children: u.status })
          ] }, `${u.role}-${u.resourceId}`))
        ] }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
          ...
          /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => M && void $(M, q), disabled: !M, children: "Rewrite context files" })
        ] }),
        D.length ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESUME MISSION" }),
          /* @__PURE__ */ a("div", { className: "workbench-mission-list", children: D.slice(0, T ? 3 : 5).map((u) => /* @__PURE__ */ d(
            "button",
            {
              className: `workbench-mission-row ${M?.missionId === u.missionId ? "active" : ""}`,
              onClick: () => te(u),
              title: u.rootPath,
              children: [
                /* @__PURE__ */ a("strong", { children: u.title }),
                /* @__PURE__ */ d("span", { children: [
                  u.status ?? "active",
                  " · ",
                  u.taskCount ?? 0,
        ...
        h?.warnings.length ? /* @__PURE__ */ a("div", { className: "workbench-resource-warnings", children: h.warnings.slice(0, 3).map((u) => /* @__PURE__ */ d("span", { children: [
          u.code,
          ": ",
          u.message
        ] }, `${u.code}-${u.resourceId ?? u.message}`)) }) : null,
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "ACTIVE CONTEXT" }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-context-card", children: [
          /* @__PURE__ */ a("strong", { children: Ne }),
        ...
        /* @__PURE__ */ a("div", { className: "workbench-computer-presets", children: Ve.map((u) => /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => ue(u.prompt), children: u.label }, u.label)) }),
        /* @__PURE__ */ a(
          "textarea",
          {
        ...
        /* @__PURE__ */ a("div", { className: "workbench-task-graph", children: ie.map((u, k) => /* @__PURE__ */ d(
          "button",
          {
            className: `workbench-task-card ${u.status} ${H?.id === u.id ? "active" : ""}`,
            onClick: () => {
              be(u.id), ue(u.prompt);
            },
            ...
            children: [
              ...
              /* @__PURE__ */ a("strong", { children: u.title }),
              /* @__PURE__ */ a("em", { children: u.resourceHint }),
              /* @__PURE__ */ a("small", { children: u.assignedResourceLabel })
            ]
          },
          u.id
        )) })
      ] }) : null,
      fe === "runs" ? /* @__PURE__ */ d(We, { children: [
        g ? /* @__PURE__ */ d("div", { className: "workbench-agent-run", children: [
          /* @__PURE__ */ d("header", { children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: g.label }),
              /* @__PURE__ */ d("span", { children: [
                g.status,
                typeof g.exitCode == "number" ? ` · exit ${g.exitCode}` : "",
                g.taskTitle ? ` · ${g.taskTitle}` : ""
              ] }),
              g.transcriptPath ? /* @__PURE__ */ a("span", { children: g.transcriptPath }) : null
            ] }),
            /* @__PURE__ */ d("div", { className: "workbench-agent-run-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => r(`${g.label} local job`, g.lines.join(`
`)), disabled: g.lines.length === 0, children: "Save output" }),
              g.jobId && (g.status === "running" || g.status === "canceling") ? /* @__PURE__ */ d("button", { className: "workbench-button-subtle danger", onClick: () => {
                bt(g);
              }, disabled: g.status === "canceling", children: [
                /* @__PURE__ */ a(ba, { size: 12 }),
                " ",
                g.status === "canceling" ? "Canceling…" : "Cancel"
              ] }) : null
            ] })
          ] }),
          /* @__PURE__ */ a("pre", { className: "workbench-computer-job-log", children: g.lines.join(`
`) || "[waiting for output]" })
        ] }) : null,
        Se.length ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RUN HISTORY" }),
          /* @__PURE__ */ a("div", { className: "workbench-run-history", children: Se.slice(0, T ? 4 : 8).map((u) => /* @__PURE__ */ d("div", { className: "workbench-run-history-row", children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: u.taskTitle || u.label }),
              /* @__PURE__ */ d("span", { children: [
                u.label,
                " · ",
                u.status,
                typeof u.exitCode == "number" ? ` · exit ${u.exitCode}` : ""
              ] }),
              u.transcriptPath ? /* @__PURE__ */ a("small", { children: u.transcriptPath }) : null
            ] }),
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => void ft(u.transcriptPath ?? u.id), title: "Copy transcript path or run id", children: "Copy path" })
          ] }, u.id)) })
        ] }) : null
      ] }) : null,
      fe === "setup" ? /* @__PURE__ */ d(We, { children: [
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "RESOURCE GRAPH" }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-list compact", children: [
          !h && !C ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No resource graph reported yet. Older Tytus host builds need `/api/resources`." }) : null,
          h?.resources.slice(0, T ? 8 : 5).map((u) => /* @__PURE__ */ d("div", { className: "workbench-resource-row", children: [
            /* @__PURE__ */ d("div", { children: [
              /* @__PURE__ */ a("strong", { children: xe(u) }),
              /* @__PURE__ */ a("span", { children: Sn(u) })
            ] }),
            /* @__PURE__ */ d("div", { className: "workbench-resource-row-actions", children: [
              /* @__PURE__ */ a("span", { className: `workbench-computer-pill ${u.status}`, children: u.status }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Jt(u);
              }, children: "Use" }),
              u.kind === "pod-agent" && gn(u) ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                Yt(u);
              }, children: "Ask pod" }) : null,
              u.status === "needs-setup" || u.setupAction ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => Zt(u), children: "Setup" }) : null
            ] })
          ] }, u.id))
        ] }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-explainer", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
          l.length === 0 && !C ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No local tools reported yet." }) : null,
          l.map((u) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
            /* @__PURE__ */ d("div", { className: "workbench-computer-card-head", children: [
              /* @__PURE__ */ d("div", { children: [
                /* @__PURE__ */ a("strong", { children: u.label }),
                /* @__PURE__ */ d("span", { children: [
                  u.kind,
            ...
            u.description ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: u.description }) : null,
            /* @__PURE__ */ d("div", { className: "workbench-computer-actions", children: [
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle workbench-agent-primary-action", onClick: () => {
                _e(u);
              }, disabled: u.status !== "available", title: "Launch this tool in a fresh Tytus Terminal with mission context and execute it immediately.", children: pc(u) }),
              u.kind === "ai-cli" ? /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
                et(u);
              }, disabled: u.status !== "available" || re !== null, title: "Run this local agent as a background read-only review inside Atomek and stream output here. It cannot write files directly.", children: bc(u, re) }) : null
            ] })
          ] }, u.id))
        ] }),
        /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "AGENTIC APP SKILLS" }),
        /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
          y.length === 0 && !C ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No skills reported yet." }) : null,
          y.map((u) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
            /* @__PURE__ */ d("div", { className: "workbench-computer-card-head", children: [
              /* @__PURE__ */ d("div", { children: [
                /* @__PURE__ */ a("strong", { children: u.title }),
                /* @__PURE__ */ d("span", { children: [
            ...
            /* @__PURE__ */ a("p", { className: "workbench-muted", children: u.description }),
            u.triggers?.length ? /* @__PURE__ */ a("div", { className: "workbench-computer-triggers", children: u.triggers.slice(0, 4).map((k) => /* @__PURE__ */ a("span", { children: k }, k)) }) : null,
            /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => {
              n(u);
            }, disabled: u.status === "missing", title: "Insert this skill's instructions into chat so Atomek can use the app/tool correctly.", children: "Use in chat" })
          ] }, u.id))
        ] })
      ] }) : null
    ] })
...
function ca({ title: e, body: t }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function wc({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: s }) {
  return /* @__PURE__ */ d("footer", { className: "workbench-statusbar", children: [
    /* @__PURE__ */ a("span", { children: "main" }),
    /* @__PURE__ */ d("span", { children: [
      r,
      " files"
    ] }),
    s > 0 && /* @__PURE__ */ d("span", { children: [
      s,
      " unsaved"
    ] }),
    ...
    /* @__PURE__ */ a("span", { children: ya(t.language) })
  ] });
}
function dn(e, t) {
...
function Oc() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : Vt.gatewayPreference,
...
function Cc({ host: e }) {
  return /* @__PURE__ */ a(Ro, { host: e });
}
function jc(e) {
  return function() {
    return /* @__PURE__ */ a(Cc, { host: e.host });
  };
}
export {
  jc as default
