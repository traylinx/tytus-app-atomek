...
import { forwardRef as yn, createElement as Pt, useState as v, useRef as Le, useCallback as w, useEffect as K, lazy as Oa, useMemo as te, Suspense as za } from "react";
/**
 ...
 */
const Ra = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), _a = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, a) => a ? a.toUpperCase() : n.toLowerCase()
), rn = (e) => {
  const t = _a(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, wn = (...e) => e.filter((t, n, a) => !!t && t.trim() !== "" && a.indexOf(t) === n).join(" ").trim(), ja = (e) => {
  for (const t in e)
...
async function Ln(e, t, n, a, i) {
  if (!(n.length >= i || a > 8))
    for await (const o of e.values()) {
      if (n.length >= i) break;
      if (o.kind === "directory") {
        if (Sr.has(o.name)) continue;
        await Ln(o, `${t}/${o.name}`, n, a + 1, i);
        continue;
      }
      const s = o, l = await Pn(s, `${t}/${s.name}`, "local-folder");
      l && n.push(l);
    }
}
function Tn(e) {
...
function _r({ host: e, requestContext: t, chatSettings: n, setStatus: a }) {
  const i = e.ai, [o, s] = v(null), [l, m] = v([]), [f, p] = v([]), [u, g] = v([]), [S, I] = v([]), [T, W] = v(cn), [ae, V] = v(!1), G = Le(!0), M = Le(null), k = w(async () => {
    if (!i) {
      W(cn);
      return;
    }
    ...
    if (i)
      try {
        ...
        P && s(P), p(C.map(Be).filter(Boolean)), I([]), await F(x);
      } catch (C) {
    ...
    if (i)
      try {
        const x = dn(await i.listThreads({ workspaceKey: Me, status: "active" })), C = x[0] ?? await i.createThread({ workspaceKey: Me, title: "Atomek chat" });
        if (!G.current) return;
        m(x[0] ? x : [C]), s(C);
  ...
  const U = w(async () => {
    if (!i) return null;
    if (o) return o;
    const x = await i.createThread({ workspaceKey: Me, title: "Atomek chat" });
    return G.current && (s(x), m((C) => [x, ...C.filter((P) => P.id !== x.id)])), x;
  }, [i, o]), ke = w(async () => {
    if (!i) return;
    const x = await i.createThread({ workspaceKey: Me, title: "Atomek chat" });
    s(x), m((C) => [x, ...C.filter((P) => P.id !== x.id)]), p([]), g([]), I([]), a("New AI chat created");
  }, [i, a]), N = w(async (x, C) => {
    if (!i) return;
    const P = C.trim();
    if (P)
      try {
        const O = i.updateThread, q = l.find((Y) => Y.id === x) ?? o, R = typeof O == "function" ? await O({ threadId: x, title: P }) : { ...q ?? await U(), id: x, title: P, updatedAt: Date.now() };
        typeof O != "function" && zr(x, P), m((Y) => Y.map((ye) => ye.id === R.id ? R : ye)), s((Y) => Y?.id === R.id ? R : Y), a(typeof O == "function" ? `Renamed chat: ${R.title}` : `Renamed chat locally: ${R.title}`);
      } catch (O) {
    ...
    if (i)
      try {
        await i.deleteThread(x), Rr(x);
        const C = l.filter((P) => P.id !== x);
        if (m(C), o?.id === x) {
          const P = C[0] ?? await i.createThread({ workspaceKey: Me, title: "Atomek chat" });
          s(P), m((q) => q.some((R) => R.id === P.id) ? q : [P, ...q]);
    ...
    try {
      ...
      return g((q) => [O, ...q.filter((R) => R.id !== O.id)]), a(`Saved AI artifact: ${O.title}`), O;
    } catch (C) {
    ...
    try {
      ...
      return I((P) => [C, ...P.filter((O) => O.id !== C.id)].slice(0, 5)), a(`Remembered: ${C.title}`), C;
    } catch (C) {
    ...
    try {
      ...
      for await (const D of i.sendMessage({
        ...
        if (D.type === "message_created") {
          const _ = Be(D.message);
          if (!_) continue;
          _.role === "assistant" && (we = _.id), p((Q) => [...Q.filter((Z) => Z.id !== _.id), _]);
        }
        if (D.type === "token" && (we = D.messageId, p((_) => _.some((Q) => Q.id === D.messageId) ? _.map(
          ...
          const _ = Be(D.message);
          if (!_) continue;
          _.role === "assistant" && (O = _), p((Q) => Q.map((Z) => Z.id === _.id ? _ : Z)), _.gatewayLabel && a(`AI answered via ${_.gatewayLabel}`);
        }
        if (D.type === "run_failed") {
          const _ = q.signal.aborted;
          a(_ ? "AI response stopped" : `AI failed: ${D.error}`), we && p((Q) => Q.map((Z) => Z.id !== we ? Z : _ ? { ...Z, status: "complete", error: void 0, body: Z.body || "Stopped by user." } : { ...Z, status: "error", error: D.error, body: D.error }));
        }
      }
      const me = dn(await i.listThreads({ workspaceKey: Me, status: "active" }).catch(() => []));
      return G.current && me.length > 0 && m(me), k(), O;
    } catch (R) {
      return R instanceof DOMException && R.name === "AbortError" ? (a("AI response stopped"), O) : (a(`AI failed: ${R instanceof Error ? R.message : String(R)}`), null);
    } finally {
      M.current === q && (M.current = null), V(!1);
    }
...
`), yi = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const m of t) {
    const f = Nt(n, m), p = Nt(a, m), u = Nt(i, m), g = o === m ? 1 : 0;
  ...
  const p = (o.current ?? Pe([], a)).staleReport(e), u = w((g, S = {}) => {
    const I = o.current ?? Pe([], a);
    return zn(I.getSnapshot(), g, S, I.staleReport(i.current));
  }, []);
  return { snapshot: s, staleReport: p, isStale: p.stale, refresh: m, update: f, retrieve: u };
}, xi = "tytus.atomek.semanticVector:v1", Rn = "__gateway_default__", _n = (e) => e?.trim() || Rn, Ni = (e) => {
  const t = _n(e);
  ...
  if (!Dn()) return null;
  try {
    const a = jn(e, t, n), i = window.localStorage.getItem(a);
    if (!i) return null;
...
}, He = 8, Si = 12e3, Ii = 180, Mi = async (e, t, n, a, i = {}, o, s) => {
  const l = zn(t, n, { ...i, limit: Math.max(i.limit ?? He, 16) }, o), m = e.ai;
  ...
  try {
    ...
    const g = new Map(l.map((M) => [M.chunkId, M])), S = zi(n), I = /* @__PURE__ */ new Map();
    t.chunks.filter((M) => i.includeDirty !== !1 || !M.dirty).slice(0, Ii).forEach((M) => I.set(M.id, M)), l.forEach((M) => {
      const k = t.byChunkId.get(M.chunkId);
      k && I.set(k.id, k);
    });
    const T = Array.from(I.values()), W = [];
    let ae = 0;
    for (const M of T) {
      s?.aborted;
      const k = Ai(e.appId, f, M) ?? await Li(m, e.appId, f, M, a, s);
      if (!k) continue;
      k.updatedAt > Date.now() - 1e3 && (ae += 1);
      const F = Fi(u, k.vector);
      if (!Number.isFinite(F)) continue;
      const j = g.get(M.id)?.score ?? Ri(M, S), U = Oi(j) * 0.42 + Math.max(0, F) * 0.58;
      W.push({ chunk: M, keywordScore: j, vectorScore: F, score: U, matchedTerms: S.filter((ke) => M.text.toLowerCase().includes(ke)) });
    }
    ...
    const V = new Set(o?.files.map((M) => M.fileId) ?? []), G = W.sort((M, k) => k.score - M.score || k.keywordScore - M.keywordScore || M.chunk.path.localeCompare(k.chunk.path) || M.chunk.ordinal - k.chunk.ordinal).slice(0, Math.max(i.limit ?? He, 1)).map((M) => Ti(M, V.has(M.chunk.fileId)));
    return {
      hits: Ei(G, i.maxChars ?? Si),
      mode: "hybrid",
      reason: `Hybrid retrieval used ${Ni(a.embeddingModel)} embeddings + keyword ranking.`,
      embeddedChunks: ae
    };
  } catch (f) {
    return {
  ...
  return {
    ...
    text: e.chunk.text,
    snippet: _i(e.chunk.text, e.matchedTerms),
    score: e.score,
...
function Vt(e) {
  return Array.from(e.matchAll(ji)).map((t) => Hn(t[1] ?? "", Ui(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function Hn(e, t) {
...
`);
    /^@@\s+-\d+/m.test(p) && o.push({ raw: p, ...At(p) });
  }
  return o.length > 0 ? o : [{ raw: t, ...At(t) }];
}
function At(e) {
...
function Ji(e) {
  ...
  for (const p of Wn(t))
    for (const u of Hi(p)) {
      if (!u.path) {
        if (i) {
          const I = Ot(i.content, u.raw);
          I && I !== i.content && s.set(i.id, Ze({
            file: i,
            proposedContent: I,
            sourceTitle: a,
...
function ko(e, t, n) {
  ...
  for (const [a, i] of lo)
    if (e.some((o) => vo(o.path) === a && Jn(o.path) === t)) return i;
  return null;
}
function yo(e) {
...
const xo = Oa(() => import("./WorkbenchMonacoEditor-DbnEHW95.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), No = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, Gn = "tytus.workspace.recent", Yn = "tytus.workspace.layout", Qn = "tytus.atomek.chatAiSettings", nt = {
...
function $t(e) {
  const t = Math.max(e || 1400, 760), n = Math.max(0, t - Ao), a = n < 1180, i = a ? 200 : 240, o = a ? 300 : 340, s = a ? 420 : 560, l = Math.max(i, Math.min(a ? 340 : 420, Math.floor(n * 0.28))), m = Math.min(300, l), f = Math.floor(n * (a ? 0.34 : 0.36)), p = n - m - s, u = Math.max(o, Math.min(a ? 500 : 640, f, p));
...
function So({ host: e }) {
  const t = Le(null), [n, a] = v(0), i = te(() => nc(), []), [o, s] = v("explorer"), [l, m] = v(i.primaryVisible), [f, p] = v(i.primaryWidth), [u, g] = v("chat"), [S, I] = v(i.secondaryVisible), [T, W] = v(i.secondaryWidth), [ae, V] = v(!1), [G, M] = v("problems"), [k, F] = v(i.markdownPreviewVisible), [B, j] = v(!1), [U, ke] = v(null), [N, ne] = v([]), [he, Ne] = v([]), [H, ie] = v(null), [Te, ot] = v(""), [x, C] = v({ lineNumber: 1, column: 1 }), [P, O] = v(null), [q, R] = v({}), [Y, ye] = v(Hr), [Ee, Ve] = v([]), [we, me] = v([]), [D, _] = v(null), [Q, Z] = v(""), [aa, ct] = v(!1), [ra, ia] = v(""), [qt, Ue] = v([]), [J, Ae] = v(null), [pe, Fe] = v(null), [Kt, Jt] = v(!1), [ve, oa] = v(() => ac()), [Xt, Oe] = v(null), [$e, st] = v(null), [lt, dt] = v(""), [ht, ut] = v(""), [qe, Ke] = v(""), [Je, Gt] = v("failed"), [ca, Yt] = v(null), [sa, A] = v("Ready"), [ze, la] = v(() => tc()), oe = he.map((c) => N.find((h) => h.id === c)).filter(Boolean), $ = H ? N.find((c) => c.id === H) ?? null : null, Qt = te(() => Br({ files: N, openEditorIds: he, activeFileId: H, versions: q, activeSelection: P }), [H, P, q, N, he]), Zt = te(() => ({
    scope: Y,
    removedAttachmentIds: Ee,
    selectedFileIds: []
  }), [Y, Ee]), Re = te(() => Zr(Qt, N, Zt), [Zt, Qt, N]), fe = Ci(N, { autoRefresh: !0, includeDirty: !0 }), en = te(() => we.map((c) => ({
    id: c.id,
    kind: "index-hit",
    label: c.label,
    path: c.path,
    ...
    snippet: c.snippet
  })), [we]), da = te(() => [...Re.attachments, ...en], [Re.attachments, en]), L = _r({ host: e, requestContext: Re.parts, chatSettings: ve, setStatus: A }), Ce = te(
    () => [...L.artifacts, ...qt].sort((c, h) => h.createdAt - c.createdAt),
    [L.artifacts, qt]
  ), Xe = Kt && !$, mt = !$ && !Xe && !B, ee = te(() => N.filter((c) => c.dirty), [N]), ha = te(() => {
    const c = Te.trim().toLowerCase();
    return c ? N.filter((h) => h.path.toLowerCase().includes(c)) : N;
  }, [N, Te]);
  K(() => {
    R((c) => {
      const h = {};
      for (const b of N) h[b.id] = c[b.id] ?? 1;
      return h;
    });
  }, [N]), K(() => {
    Ve([]), me([]);
  ...
  const ua = w((c) => {
    if (c.kind === "index-hit") {
      me((h) => h.filter((b) => b.id !== c.id)), A(`Removed project context: ${c.label}`);
      return;
    }
    Ve((h) => h.includes(c.id) ? h : [...h, c.id]), A(`Removed chat context: ${c.label}`);
  }, []), Ge = w(() => {
    Jt(!0), ie(null), j(!0), A("Atomek settings opened");
  }, []), tn = w(() => {
    Jt(!1), ie((c) => c ?? he.at(-1) ?? null), he.length === 0 && j(!1), A("Atomek settings closed");
  }, [he]), Se = w((c) => {
    R((h) => ({ ...h, [c]: (h[c] ?? 1) + 1 }));
  }, []), ma = w((c) => {
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const h = $t(n), b = c.clientX, y = T, z = (De) => {
      const Ct = y + (b - De.clientX);
      W(et(Ct, h.secondaryMin, h.secondaryMax));
    }, se = () => {
    ...
    c.preventDefault(), c.currentTarget.setPointerCapture?.(c.pointerId);
    const h = $t(n), b = c.clientX, y = f, z = (De) => {
      const Ct = y + (De.clientX - b);
      p(et(Ct, h.primaryMin, h.primaryMax));
    }, se = () => {
      window.removeEventListener("pointermove", z), window.removeEventListener("pointerup", se);
    };
    window.addEventListener("pointermove", z), window.addEventListener("pointerup", se);
  }, [f, n]), Ye = w((c) => {
    const h = [c, ...ze.filter((b) => b.path !== c.path)].slice(0, 6);
    la(h), localStorage.setItem(Gn, JSON.stringify(h));
  }, [ze]), X = w((c, h) => {
    Ne((b) => b.includes(c.id) ? b : [...b, c.id]), ie(c.id), j(!1), Yt(h ?? null), C({ lineNumber: h ?? 1, column: 1 });
  }, []), fa = w((c) => {
    if (!c.fileId) return;
    const h = N.find((b) => b.id === c.fileId);
    h && (X(h, c.range?.startLineNumber ?? 1), A(`Revealed context: ${c.label}`));
  }, [N, X]), _e = w(async () => {
    if (St(ee, "open new files"))
      try {
        const c = await Mr();
        if (c.length === 0) return;
        ne((h) => Qo(h, c)), c.forEach((h) => Ye({ name: h.name, path: h.path, at: Date.now() })), X(c[0]), A(`Opened ${c.length} local file${c.length === 1 ? "" : "s"}`);
      } catch (c) {
    ...
    if (St(ee, "open another folder"))
      try {
        const c = await Pr();
        ke(c), ne(c.files), Ne([]), ie(null), j(!1), Ye({ name: c.name, path: c.name, at: Date.now() }), A(`${c.handle ? "Opened local folder" : "Opened browser fallback folder"} ${c.name} (${c.files.length} text files indexed)`);
      } catch (c) {
    ...
    if (!H) return;
    let h = !1;
    ne((b) => b.map((y) => y.id !== H || y.content === c ? y : (h = !0, { ...y, content: c, dirty: !0 }))), h && Se(H);
  }, [H, Se]), ft = w(async () => {
    if ($)
      try {
        const c = await xt($);
        ne((h) => h.map((b) => b.id === c.id ? c : b)), A(`Saved ${c.name}`);
      } catch (c) {
        A(`Save failed: ${c.message}`);
      }
  }, [$]), ga = w(async (c) => {
    const h = N.find((y) => y.id === c);
    if (!h) return null;
    const b = await xt(h);
    return ne((y) => y.map((z) => z.id === b.id ? b : z)), b;
  }, [N]), nn = w(async () => {
    const c = N.filter((h) => h.dirty);
    if (c.length === 0) {
      A("No dirty files to save");
      return;
    }
    try {
      const h = await Promise.all(c.map((y) => xt(y))), b = new Map(h.map((y) => [y.id, y]));
      ne((y) => y.map((z) => b.get(z.id) ?? z)), Oe(null), A(`Saved ${h.length} dirty file${h.length === 1 ? "" : "s"}`);
    } catch (h) {
      A(`Save all failed: ${h.message}`);
    }
  }, [N]), bt = w((c) => {
    const h = N.find((b) => b.id === c);
    if (h?.dirty && !window.confirm(`${h.name} has unsaved changes. Close without saving?`)) {
      A(`Close canceled — ${h.name} has unsaved changes`);
      return;
    }
    Ne((b) => {
    ...
    St(ee, "close all editors") && (Ne([]), ie(null), Yt(null), j(!1), A("Closed all editors"));
  }, [ee]), gt = w(() => {
    const c = N.filter((b) => b.name.startsWith("Untitled")).length + 1, h = {
...
`,
    ...
    return me(y), y.length === 0 ? (A(fe.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), h) : (b.mode !== "hybrid" && b.reason ? A(b.reason) : A(`${y.length} project context hit${y.length === 1 ? "" : "s"} · ${b.reason ?? "hybrid retrieval"}`), [
      ...h,
      ...Pi(y)
    ]);
  }, [Re.parts, Y, ve, e, fe]), kt = w(() => {
    if (!$ && oe.length === 0) {
    ...
    I(!0), g("chat"), A("Asking Atomek to synthesize an AI artifact…"), (async () => {
      const b = await be(h), y = await L.askAgent(h, { requestContext: b });
      !y || y.status === "error" || L.createArtifact({
        ...
        I(!0), g("outputs"), V(!0), M("output");
      });
    })();
  }, [$, L, be, oe.length]), yt = w((c) => {
    const h = N.find((b) => b.path === c.path || b.name === c.name);
    if (h) {
      X(h), A(`Opened recent ${h.name}`);
      return;
    }
    ...
    I(!0), g("chat"), Z(""), (async () => {
      const h = await be(c);
      await L.askAgent(c, { requestContext: h });
    })();
  }, [L, be]), ya = w(async (c) => {
    if (!e.skills?.get) {
...
`)), I(!0), g("chat"), A(`Attached skill ${h.title} to chat input`);
    } catch (h) {
      A(`Failed to attach skill: ${h instanceof Error ? h.message : String(h)}`);
    }
  }, [e.skills]), wa = w((c) => {
    const h = L.messages.findIndex((y) => y.id === c.id), b = L.messages.slice(0, h < 0 ? void 0 : h).reverse().find((y) => y.role === "user");
    if (!b?.body.trim()) {
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const h = Mt(N, It(c.title || c.kind)), b = {
    ...
    ne((y) => [...y, b]), X(b), A(`Opened ${c.title} as editable file`);
  }, [N, X]), vt = w(() => {
...
`), z = {
    ...
    Ue((se) => [z, ...se]), V(!0), M("output"), A(`Captured manual check: ${h}`);
  }, []), Ie = w((c, h) => {
    const b = Ji({
      body: h,
      files: N,
      ...
      edits: b.edits.map(Zo),
      skipped: b.skipped
    };
    return b.edits.length > 1 ? (Fe(y), Ae(null), A(`Previewing AI workspace patch for ${b.edits.length} files`), !0) : b.edits.length === 1 ? (Ae(y.edits[0]), Fe(null), A(`Previewing AI patch for ${y.edits[0].fileName}`), !0) : (A(b.skipped.length > 0 ? `No applicable edit found. ${b.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [$, q, N]), Na = w((c, h) => {
    const b = {
    ...
    Ue((y) => [b, ...y]), I(!0), g("outputs"), V(!0), M("output"), A(`Saved local job output: ${c}`), (h.includes("```diff") || h.includes("--- a/")) && Ie(c, h);
  }, [Ie]), Aa = w(() => {
    const c = Q.trim();
    if (!c) return;
    Z("");
    ...
    (async () => {
      ...
      Ie(y.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", y.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    })();
  }, [L, be, Q, Ie]), $a = w(() => {
    D && (_(null), re(Uo(D)));
  }, [re, D]), je = w((c) => {
    ...
    Ke(b), dt(""), ut(""), Gt("failed"), V(!0), M("terminal"), A(h.commands.length > 0 ? `Manual check ready: copy ${h.commands[0].command}` : "Manual check ready: enter a check command to copy");
  }, [N]), Sa = w((c) => {
    c.trim() && (it(c), Ke(c), A(`Manual check command copied: ${c}`));
  }, []), Ia = w(() => {
    st((c) => {
      if (!c) return c;
      const h = uo(c, lt), b = h.commands.at(-1)?.command ?? "";
      return b && Ke(b), h;
    }), dt("");
  }, [lt]), Ma = w(() => {
    if (!$e || !qe.trim()) {
    ...
    const c = qe.trim();
    st((h) => h && mo(h, c, Je, ht)), ut(""), A(`Captured manual check result: ${c} (${Je})`);
  }, [ht, qe, $e, Je]), Pa = w(() => {
    $e && re(po($e));
  }, [re, $e]), La = w(() => {
    if (!J) return;
    const c = N.find((h) => h.id === J.fileId);
    if (!c) {
      A(`Cannot apply edit — ${J.fileName} is no longer open`), Ae(null);
      return;
    }
    c.content !== J.originalContent && !window.confirm(`${J.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (ne((h) => h.map((b) => b.id === J.fileId ? { ...b, content: J.proposedContent, dirty: !0 } : b)), Se(J.fileId), Oe(`AI edit applied to ${c.name}. Save All to persist it to disk.`), Ae(null), je(`AI edit applied to ${c.name}`), A(`Applied AI edit to ${c.name} — unsaved; manual check ready`));
  }, [Se, N, J, je]), Ta = w(() => {
    if (!J) return;
    const c = Mt(N, `${It(J.fileName)}-ai-edit`), h = {
    ...
    ne((b) => [...b, h]), X(h), Ae(null), A(`Opened proposed edit as ${c}`);
  }, [$?.language, N, X, J]), Ea = w(() => {
    if (!pe) return;
    ...
    })), c.forEach((b, y) => Se(y)), Oe(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}. Save All to persist changes.`), Fe(null), je(`AI workspace patch applied to ${c.size} file${c.size === 1 ? "" : "s"}`), A(`Applied AI workspace patch to ${c.size} file${c.size === 1 ? "" : "s"} — unsaved; manual check ready`));
  }, [Se, N, pe, je]), Fa = w(() => {
    if (!pe) return;
    const c = [...N], h = pe.edits.map((b) => {
      const y = Mt(c, `${It(b.fileName)}-ai-edit`), z = N.find((De) => De.id === b.fileId), se = {
    ...
    ne((b) => [...b, ...h]), h[0] && X(h[0]), Fe(null), A(`Opened ${h.length} proposed edit file${h.length === 1 ? "" : "s"}`);
  }, [N, X, pe]), an = w((c) => {
    if (c === "explain") {
  ...
  return K(() => {
    const c = (h) => {
      (h.metaKey || h.ctrlKey) && (h.key.toLowerCase() === "o" && (h.preventDefault(), _e()), h.key.toLowerCase() === "s" && (h.preventDefault(), ft()), h.key.toLowerCase() === "w" && (h.preventDefault(), H && bt(H)), h.key.toLowerCase() === "b" && (h.preventDefault(), m((y) => !y)), h.shiftKey && h.key.toLowerCase() === "f" && (h.preventDefault(), s("search"), m(!0)), (h.key.toLowerCase() === "k" || h.key.toLowerCase() === "p") && (h.preventDefault(), ct(!0)));
    };
    return window.addEventListener("keydown", c), () => window.removeEventListener("keydown", c);
  }, [H, bt, _e, ft]), K(() => {
    const c = (h) => {
      ee.length !== 0 && (h.preventDefault(), h.returnValue = "");
    };
    ...
    const c = t.current;
    if (!c) return;
    const h = () => a(Math.round(c.getBoundingClientRect().width));
    h();
    ...
    if (!n) return;
    const c = $t(n);
    p((h) => et(h, c.primaryMin, c.primaryMax)), W((h) => et(h, c.secondaryMin, c.secondaryMax));
  }, [n]), K(() => {
    ...
    "div",
    {
      ...
      children: [
        /* @__PURE__ */ r(Io, { active: o, setActive: (c) => {
          s(c), m(!0);
        ...
        l && /* @__PURE__ */ d("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ r("div", { className: "workbench-primary-resizer", onPointerDown: pa, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ d("main", { className: "workbench-editor-area", children: [
          ...
          /* @__PURE__ */ d("section", { className: "workbench-editor-stack", children: [
            ...
            Xt && ee.length > 0 ? /* @__PURE__ */ d("div", { className: "workbench-ai-dirty-banner", children: [
            ...
            /* @__PURE__ */ r("div", { className: "workbench-editor-content", children: $ ? /* @__PURE__ */ d("div", { className: $.language === "markdown" && k ? "workbench-editor-split" : "workbench-editor-single", children: [
              ...
              $.language === "markdown" && k && /* @__PURE__ */ r(_o, { content: $.content })
            ] }) : Xe ? /* @__PURE__ */ r(
              qo,
              {
                host: e,
            ...
            ) : mt ? /* @__PURE__ */ r(Fo, { openFile: _e, openFolder: pt, newFile: gt, recent: ze, reopenRecent: yt, setStatus: A }) : /* @__PURE__ */ d("div", { className: "workbench-no-editor", children: [
            ...
            ae && /* @__PURE__ */ r(
              Do,
              {
                ...
                copyManualCheckCommand: Sa,
                addManualCheckCommand: Ia,
                recordManualCheckResult: Ma,
                askAgentFromManualChecks: Pa,
                onClose: () => V(!1)
              }
            )
          ] })
        ...
        S && /* @__PURE__ */ r(
          Bo,
          {
            ...
            saveMessageAsArtifact: va,
            rememberMessage: Ca,
            previewEditFromMessage: (c) => Ie(c.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", c.body),
            runQuickPrompt: an,
            pendingPatchPrompt: D,
            generatePatchPrompt: $a,
            workspaceFileCount: N.length,
            aiStatus: L.aiStatus,
            chatSettings: ve,
            openSettings: Ge,
            busy: L.busy,
            ...
            captureManualCheck: vt,
            openOutputAsFile: wt,
            previewEditFromOutput: (c) => Ie(c.title, c.body),
            canPreviewEdit: N.length > 0,
            clearOutputs: () => Ue([]),
            deleteArtifact: (c) => {
              L.deleteArtifact(c);
            },
        ...
        aa && /* @__PURE__ */ r(
          Ro,
          {
            ...
            commands: [
              ...
              { label: "File: Close All Editors", detail: `${oe.length} open editor${oe.length === 1 ? "" : "s"}`, run: ka, disabled: oe.length === 0 },
              ...
              { label: "View: Toggle Chat Panel", detail: S ? "Hide right AI side bar" : "Show right AI side bar", run: () => I((c) => !c) },
              ...
              { label: "AI: Save Active File as Artifact", detail: $ ? "Persist active file in host.ai artifacts" : "Open a file first", run: xa, disabled: !$ },
        ...
        /* @__PURE__ */ r(Yo, { status: sa, file: $ ?? No, cursor: x, fileCount: N.length, dirtyCount: ee.length })
      ]
    }
  );
}
function Io({ active: e, setActive: t, openSettings: n, settingsActive: a }) {
  return /* @__PURE__ */ d("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(Nn, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    ...
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(Cn, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(zt, { size: 25 }), label: "Computer / Agents", active: e === "computer", onClick: () => t("computer") }),
    /* @__PURE__ */ r(ge, { icon: /* @__PURE__ */ r(vn, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
...
function Mo(e) {
  return e.activity === "search" ? /* @__PURE__ */ r(Oo, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ r(kn, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ r(kn, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "computer" ? /* @__PURE__ */ r(Go, { host: e.host, setStatus: e.setStatus, attachSkillToChat: e.attachSkillToChat, saveLocalJobOutput: e.saveLocalJobOutput }) : e.activity === "extensions" ? /* @__PURE__ */ r(Xo, {}) : /* @__PURE__ */ r(Po, { ...e });
}
function Po(e) {
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      t ? /* @__PURE__ */ d(We, { children: [
        ...
        n.length === 0 ? /* @__PURE__ */ r("p", { className: "workbench-muted", children: "No readable text files found." }) : Zn(n, e.activeFileId, e.openWorkbenchFile)
      ] }),
...
function Zn(e, t, n, a = 0) {
  return e.map((i) => i.file ? /* @__PURE__ */ r(Ut, { file: i.file, active: i.file.id === t, onOpen: () => n(i.file), depth: a, label: i.name }, i.file.id) : /* @__PURE__ */ d("div", { children: [
...
function Ro(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${In(i.language)} file`,
...
function Do(e) {
  return /* @__PURE__ */ d("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "output" && /* @__PURE__ */ r(na, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function Bo(e) {
  return /* @__PURE__ */ d("aside", { className: "workbench-secondary", children: [
    ...
    e.tab === "chat" ? /* @__PURE__ */ r(Ho, { ...e }) : /* @__PURE__ */ r(na, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, captureManualCheck: e.captureManualCheck, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function ea({ body: e }) {
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-rich-body", children: t.map((o) => {
    ...
    return /* @__PURE__ */ r(
      "div",
      {
        ...
        dangerouslySetInnerHTML: { __html: Fn(o.body) }
      },
      o.key
    );
  }) });
  ...
  return /* @__PURE__ */ d("div", { className: "workbench-chat-wrap", children: [
    ...
    /* @__PURE__ */ d("div", { ref: n, className: "workbench-chat-transcript", onScroll: m, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ r("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ d("div", { children: [
        ...
        /* @__PURE__ */ r("br", {}),
        /* @__PURE__ */ r(ea, { body: u.body }),
        u.gatewayLabel ? /* @__PURE__ */ d(We, { children: [
          /* @__PURE__ */ r("br", {}),
        ...
        u.role === "assistant" && u.status !== "streaming" && u.status !== "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => p(u), title: "Copy this answer", children: [
        ...
        u.role === "assistant" && u.status === "error" ? /* @__PURE__ */ d("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ d("button", { className: "workbench-chat-message-action", onClick: () => p(u), title: "Copy this error", children: [
            /* @__PURE__ */ r(Tt, { size: 12 }),
            " Copy"
          ] }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ d("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ d("span", { className: "workbench-chat-chip muted", children: [
            ...
            const g = typeof u.score == "number" ? u.score.toFixed(2) : null, S = typeof u.vectorScore == "number" ? u.vectorScore.toFixed(2) : null, I = typeof u.keywordScore == "number" ? u.keywordScore.toFixed(1) : null, T = [
              u.path,
            ...
            return /* @__PURE__ */ d("span", { className: "workbench-chat-chip", title: T, children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-chat-toolbar compact", children: [
          ...
          /* @__PURE__ */ r("span", {}),
          e.busy ? /* @__PURE__ */ r("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ r(Nr, { size: 14 }) }) : /* @__PURE__ */ r("button", { className: `workbench-chat-send ${t ? "ready" : ""}`, onClick: e.askAgent, title: "Send", disabled: !t, "aria-label": "Send message", children: /* @__PURE__ */ r(yr, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function bn(e, t, n) {
  const a = e.model.trim(), o = [e.gatewayPreference === "auto" ? t : ta(e.gatewayPreference)];
  ...
  K(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(I.map((T) => ({ id: T.id, gatewayLabel: T.gatewayLabel }))), i(I.length > 0 ? `${I.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (I) {
        if (g.signal.aborted) return;
    ...
    /* @__PURE__ */ d("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ d("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ d("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          ta(e.chatSettings.gatewayPreference),
...
function na({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: a, captureManualCheck: i, openOutputAsFile: o, previewEditFromOutput: s, canPreviewEdit: l = !1, compact: m = !1 }) {
  ...
  return /* @__PURE__ */ r("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ d("section", { className: "workbench-edit-review workspace", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((o) => /* @__PURE__ */ d("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ r("pre", { children: ec(o.proposedContent) })
      ] }, o.fileId)),
...
function Go({ host: e, setStatus: t, attachSkillToChat: n, saveLocalJobOutput: a }) {
  const [i, o] = v([]), [s, l] = v([]), [m, f] = v(!1), [p, u] = v(null), [g, S] = v("Inspect this workspace/task and return actionable findings. If you propose edits, output a unified diff that Atomek can preview."), [I, T] = v(null), [W, ae] = v(""), V = w(async () => {
  ...
  return /* @__PURE__ */ d("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ d("div", { className: "workbench-sidebar-scroll", children: [
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        ...
        i.map((k) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          ...
          k.kind === "ai-cli" ? /* @__PURE__ */ r("button", { className: "workbench-button-subtle", onClick: () => {
            M(k);
          }, disabled: k.status !== "available" || I !== null, children: I === k.id ? "Running…" : "Run local job" }) : null
        ] }, k.id))
      ] }),
      ...
      /* @__PURE__ */ d("div", { className: "workbench-computer-list", children: [
        ...
        s.map((k) => /* @__PURE__ */ d("div", { className: "workbench-computer-card", children: [
          /* @__PURE__ */ d("div", { className: "workbench-computer-card-head", children: [
            /* @__PURE__ */ d("div", { children: [
...
function Yo({ status: e, file: t, cursor: n, fileCount: a, dirtyCount: i }) {
  return /* @__PURE__ */ d("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ r("span", { children: In(t.language) })
  ] });
}
function St(e, t) {
...
function cc(e) {
  return function() {
    return /* @__PURE__ */ r(rc, { host: e.host });
  };
}
export {
  cc as default
