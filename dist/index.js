...
import { forwardRef as qt, createElement as dt, useState as C, useRef as ze, useCallback as v, useEffect as U, lazy as Vn, useMemo as V, Suspense as Un } from "react";
/**
 ...
 */
const qn = (e) => e.replace(/([a-z0-9])([A-Z])/g, "$1-$2").toLowerCase(), Kn = (e) => e.replace(
  /^([A-Z])|[\s-_]+(\w)/g,
  (t, n, r) => r ? r.toUpperCase() : n.toLowerCase()
), Lt = (e) => {
  const t = Kn(e);
  return t.charAt(0).toUpperCase() + t.slice(1);
}, Kt = (...e) => e.filter((t, n, r) => !!t && t.trim() !== "" && r.indexOf(t) === n).join(" ").trim(), Xn = (e) => {
  for (const t in e)
 ...
 * See the LICENSE file in the root directory of this source tree.
 */
const na = [["path", { d: "m6 9 6 6 6-6", key: "qrunsl" }]], ht = L("chevron-down", na);
/**
 * @license lucide-react v0.562.0 - ISC
 *
 * This source code is licensed under the ISC license.
...
const _a = /* @__PURE__ */ new Set([".git", "node_modules", "dist", "build", ".next", ".turbo", "coverage", ".cache"]);
function Ra() {
  const e = bt();
...
async function ja() {
  ...
  if (!e) return rn(!1);
  const t = await e({ multiple: !0 }), n = await Promise.all(t.map((r) => nn(r, r.name, "local-file")));
  return gt(n.filter(Boolean));
}
async function Da() {
  const e = bt()?.showDirectoryPicker;
  if (!e)
    return { name: "Browser fallback folder", files: await rn(!0) };
  const t = await e({ mode: "readwrite" }), n = [];
  return await an(t, t.name, n, 0, 320), { name: t.name, handle: t, files: gt(n) };
}
function bt() {
  const e = window;
...
async function an(e, t, n, r, i) {
  if (!(n.length >= i || r > 8))
    for await (const s of e.values()) {
      if (n.length >= i) break;
      if (s.kind === "directory") {
        if (_a.has(s.name)) continue;
        await an(s, `${t}/${s.name}`, n, r + 1, i);
        continue;
      }
      const c = s, l = await nn(c, `${t}/${c.name}`, "local-folder");
      l && n.push(l);
    }
}
function rn(e) {
  return new Promise((t) => {
    ...
    n.type = "file", n.multiple = !0, n.style.display = "none", e && n.setAttribute("webkitdirectory", ""), n.onchange = async () => {
      const r = Array.from(n.files ?? []), i = await Promise.all(r.map(async (s) => {
        const c = s.webkitRelativePath || s.name;
        return tn(c, s.size) ? {
          id: on(c),
          name: s.name,
          path: c,
...
`).map(
    ...
    const s = r.split("|").filter(Boolean).map((l) => `<th style="padding:8px 12px;background:var(--bg-titlebar);font-weight:600;font-size:13px;border:1px solid var(--border-default)">${l.trim()}</th>`).join(""), c = i.trim().split(`
`).map((l) => `<tr>${l.split("|").filter(Boolean).map(
    ...
    return `<table style="border-collapse:collapse;margin:16px 0;width:100%;border:1px solid var(--border-default)"><thead><tr>${s}</tr></thead><tbody>${c}</tbody></table>`;
  }), t = t.replace(/^(?!<[a-z])(.+)$/gim, '<p style="line-height:1.6;margin:12px 0;color:var(--text-primary)">$1</p>'), t = t.replace(/\n+/g, `
`), t;
}
const Ne = "atomek:default", Wa = 3e3, wt = "tytus.atomek.threadTitleOverrides", Tt = {
  available: !1,
  source: "none",
...
}, Va = (e, t = Wa) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, Ot = (e, t) => {
  const n = e.split(`
`).map((r) => r.replace(/^#+\s*/, "").trim()).find(Boolean);
  return n ? n.length > 80 ? `${n.slice(0, 77)}...` : n : t;
}, Ua = (e) => ["briefing", "action-list", "quiz", "plan", "storyboard", "report", "local-draft", "markdown", "memory"].includes(e) ? e : "report", zt = (e) => ({
  id: e.id,
  title: e.title,
  kind: Ua(e.kind),
  body: e.body,
...
function Ga({ host: e, requestContext: t, chatSettings: n, setStatus: r }) {
  const i = e.ai, [s, c] = C(null), [l, u] = C([]), [d, f] = C([]), [g, b] = C([]), [P, M] = C([]), [z, ee] = C(Tt), [fe, re] = C(!1), q = ze(!0), pe = ze(null), J = v(async () => {
    if (!i) {
      ee(Tt);
      return;
    }
    ...
    if (i)
      try {
        const w = await i.listArtifacts({ threadId: y });
        if (!q.current) return;
        b(w.map(zt));
      } catch (w) {
    ...
    if (i)
      try {
        const w = await i.listMessages(y);
        if (!q.current) return;
        const $ = l.find((I) => I.id === y) ?? null;
        $ && c($), f(w.map(Oe).filter(Boolean)), M([]), await te(y);
      } catch (w) {
    ...
    if (i)
      try {
        const y = _t(await i.listThreads({ workspaceKey: Ne, status: "active" })), w = y[0] ?? await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
        if (!q.current) return;
        u(y[0] ? y : [w]), c(w);
        const $ = await i.listMessages(w.id);
        if (!q.current) return;
        f($.map(Oe).filter(Boolean)), await te(w.id);
      } catch (y) {
        r(`AI chat unavailable: ${y instanceof Error ? y.message : String(y)}`);
      }
  }, [i, te, r]);
  U(() => (q.current = !0, J(), x(), () => {
    q.current = !1;
  ...
  const O = v(async () => {
    if (!i) return null;
    if (s) return s;
    const y = await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
    return q.current && (c(y), u((w) => [y, ...w.filter(($) => $.id !== y.id)])), y;
  }, [i, s]), be = v(async () => {
    if (!i) return;
    const y = await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
    c(y), u((w) => [y, ...w.filter(($) => $.id !== y.id)]), f([]), b([]), M([]), r("New AI chat created");
  }, [i, r]), ge = v(async (y, w) => {
    if (!i) return;
    const $ = w.trim();
    if ($)
      try {
        const I = i.updateThread, W = l.find((Y) => Y.id === y) ?? s, E = typeof I == "function" ? await I({ threadId: y, title: $ }) : { ...W ?? await O(), id: y, title: $, updatedAt: Date.now() };
        typeof I != "function" && Ka(y, $), u((Y) => Y.map((oe) => oe.id === E.id ? E : oe)), c((Y) => Y?.id === E.id ? E : Y), r(typeof I == "function" ? `Renamed chat: ${E.title}` : `Renamed chat locally: ${E.title}`);
      } catch (I) {
        r(`Rename chat failed: ${I instanceof Error ? I.message : String(I)}`);
      }
  }, [i, O, r, s, l]), _ = v(async (y) => {
    if (i)
      try {
        await i.deleteThread(y), Xa(y);
        const w = l.filter(($) => $.id !== y);
        if (u(w), s?.id === y) {
          const $ = w[0] ?? await i.createThread({ workspaceKey: Ne, title: "Atomek chat" });
          c($), u((W) => W.some((E) => E.id === $.id) ? W : [$, ...W]);
          const I = await i.listMessages($.id);
          f(I.map(Oe).filter(Boolean)), await te($.id);
        }
        r("Deleted AI chat");
      } catch (w) {
        r(`Delete chat failed: ${w instanceof Error ? w.message : String(w)}`);
      }
  }, [i, te, r, s?.id, l]), ie = v(async (y) => {
    if (!i) return null;
    try {
      const w = await O();
      if (!w) return null;
      const $ = await i.createArtifact({
        ...
        title: y.title?.trim() || Ot(y.body, "Atomek artifact"),
        kind: y.kind ?? "markdown",
        body: y.body
      }), I = zt($);
      return b((W) => [I, ...W.filter((E) => E.id !== I.id)]), r(`Saved AI artifact: ${I.title}`), I;
    } catch (w) {
    ...
    if (i)
      try {
        await i.deleteArtifact(y), b((w) => w.filter(($) => $.id !== y)), r("Deleted AI artifact");
      } catch (w) {
    ...
    try {
      const w = await i.writeMemory({
        title: y.title?.trim() || Ot(y.body, "Atomek memory"),
        body: y.body,
        metadata: {
          source: "atomek",
          messageId: y.messageId ?? null
      ...
      return M(($) => [w, ...$.filter((I) => I.id !== w.id)].slice(0, 5)), r(`Remembered: ${w.title}`), w;
    } catch (w) {
      return r(`Remember failed: ${w instanceof Error ? w.message : String(w)}`), null;
    }
  }, [i, r]), Se = v(async (y) => {
    if (!i) return [];
    const w = await i.searchMemory({ query: y, limit: 5 });
    ...
    try {
      const E = await O();
      if (!E) return null;
      const Y = await Se($).catch(() => []), oe = qa(Y), se = w.requestContext ?? t, we = oe ? [...se, oe] : [...se];
      let ye = null;
      for await (const T of i.sendMessage({
        threadId: E.id,
        body: $,
        ...
        if (T.type === "message_created") {
          const F = Oe(T.message);
          if (!F) continue;
          F.role === "assistant" && (ye = F.id), f((B) => [...B.filter((K) => K.id !== F.id), F]);
        }
        if (T.type === "token" && (ye = T.messageId, f((F) => F.some((B) => B.id === T.messageId) ? F.map(
          (B) => B.id === T.messageId ? { ...B, body: T.body, status: "streaming" } : B
        ) : [...F, {
          id: T.messageId,
          role: "assistant",
          body: T.body,
          ...
          const F = Oe(T.message);
          if (!F) continue;
          F.role === "assistant" && (I = F), f((B) => B.map((K) => K.id === F.id ? F : K)), F.gatewayLabel && r(`AI answered via ${F.gatewayLabel}`);
        }
        if (T.type === "run_failed") {
          const F = W.signal.aborted;
          r(F ? "AI response stopped" : `AI failed: ${T.error}`), ye && f((B) => B.map((K) => K.id !== ye ? K : F ? { ...K, status: "complete", error: void 0, body: K.body || "Stopped by user." } : { ...K, status: "error", error: T.error, body: T.error }));
        }
      }
      const ke = _t(await i.listThreads({ workspaceKey: Ne, status: "active" }).catch(() => []));
      return q.current && ke.length > 0 && u(ke), J(), I;
    } catch (E) {
      return E instanceof DOMException && E.name === "AbortError" ? (r("AI response stopped"), I) : (r(`AI failed: ${E instanceof Error ? E.message : String(E)}`), null);
    } finally {
      pe.current === W && (pe.current = null), re(!1);
    }
  }, [i, n.gatewayPreference, n.model, O, Se, J, t, r]), Re = v(() => {
    pe.current?.abort(), r("Stopping AI response…");
  }, [r]);
  return {
    aiStatus: z,
    artifacts: g,
...
const Ja = (e) => `tytus-workbench:///${encodeURI(e.path)}`, Ya = (e) => {
  let t = 2166136261;
  for (let n = 0; n < e.length; n += 1)
    t ^= e.charCodeAt(n), t = Math.imul(t, 16777619);
  return (t >>> 0).toString(16).padStart(8, "0");
}, Qa = ({ files: e, openEditorIds: t, activeFileId: n, versions: r, activeSelection: i }) => {
  const s = new Set(t), c = e.map((l) => {
    const u = l.id === n;
    return {
      id: l.id,
      uri: Ja(l),
      ...
      language: l.language,
      version: r[l.id] ?? 1,
      contentHash: Ya(l.content),
      dirty: l.dirty,
      source: l.source,
      selection: u && i ? i : void 0,
      updatedAt: Date.now(),
      open: s.has(l.id),
      active: u
    };
  ...
  return {
    documents: c,
    byId: new Map(c.map((l) => [l.id, l])),
    activeDocumentId: n,
    openDocumentIds: t.filter((l) => c.some((u) => u.id === l))
  };
}, Za = "active-file", er = (e) => e === "none" ? "No context" : e === "active-selection" ? "Active selection" : e === "active-file" ? "Active file" : e === "open-editors" ? "Open editors" : e === "selected-files" ? "Selected files" : "Indexed project", tr = (e, t = "file") => `${t}:${e.id}`, nr = (e, t) => [
  "selection",
  e.id,
  t.startLineNumber,
  t.startColumn,
  ...
  let i = [];
  return t.scope === "active-selection" ? r?.selection ? i = [ar(r, r.selection, !0)] : r && (i = [Be(r, !0, "Active file")]) : t.scope === "active-file" ? r && (i = [Be(r, !0, "Active file")]) : t.scope === "open-editors" ? i = e.openDocumentIds.map((s) => e.byId.get(s)).filter(Boolean).map((s) => Be(s, !0, "Open editor")) : t.scope === "selected-files" && (i = t.selectedFileIds.map((s) => e.byId.get(s)).filter(Boolean).map((s) => Be(s, !1, "Selected file"))), ir(i).filter((s) => !n.has(s.id));
}, ir = (e) => {
  const t = /* @__PURE__ */ new Set();
  return e.filter((n) => t.has(n.id) ? !1 : (t.add(n.id), !0));
}, or = 4e3, sr = 8e3, cr = 4e3, lr = (e, t) => e.length <= t ? e : `${e.slice(0, t)}

[...clipped ${e.length - t} chars...]`, dr = (e, t, n) => {
  const r = rr(e, n), i = [], s = new Map(t.map((c) => [c.id, c]));
  for (const c of r) {
    if (!c.fileId) continue;
    const l = s.get(c.fileId), u = e.byId.get(c.fileId);
    if (!l || !u || !c.includeBody) continue;
    const d = [
      `Path: ${l.path}`,
      `Language: ${l.language}`,
      `Version: ${u.version}`,
      `Hash: ${u.contentHash}`,
      `Dirty: ${u.dirty ? "yes" : "no"}`,
      c.range ? `Range: ${Rt(c.range)}` : null
    ].filter(Boolean).join(`
`), f = c.kind === "selection" && c.range ? hr(l.content, c.range) : l.content, g = c.kind === "selection" ? or : c.label.startsWith("Open editor") ? cr : sr;
    i.push({
      kind: c.kind === "selection" ? "selection" : "file",
      title: c.kind === "selection" ? `Active selection: ${l.path}` : c.label,
      text: `${d}
...
`));
}, Rt = (e) => `${e.startLineNumber}:${e.startColumn}-${e.endLineNumber}:${e.endColumn}`, jt = 75e4, ur = 2800, mr = 240, fr = 80, pr = /* @__PURE__ */ new Set([
  ".git",
  ".hg",
  ".svn",
  ".next",
  ...
  e.map((t) => `${t.id}\0${t.path}\0${t.language}\0${t.dirty ? "1" : "0"}\0${me(t.content)}`).sort().join("")
), ut = (e, t = {}) => {
  const n = e.size ?? e.content.length;
  return n === 0 || e.content.length === 0 ? "empty" : n > (t.maxFileBytes ?? jt) || e.content.length > (t.maxFileBytes ?? jt) ? "huge" : !t.includeDirty && e.dirty ? null : xr(e.path) ? "vendor" : Cr(e) ? "binary" : null;
}, Dt = (e, t = {}) => {
  const n = Date.now(), r = [], i = [], s = [];
  for (const c of e) {
    const l = c.size ?? c.content.length, u = ut(c, t);
    if (u) {
      i.push({ fileId: c.id, name: c.name, path: c.path, language: c.language, reason: u, size: l });
      continue;
    }
    const d = me(c.content), f = gr(c, d, n, t);
    s.push(...f), r.push({
      fileId: c.id,
      name: c.name,
      path: c.path,
  ...
  return wr({
    ...
    skipped: i,
    chunks: s,
    byFileId: new Map(r.map((c) => [c.fileId, c])),
    byChunkId: new Map(s.map((c) => [c.id, c]))
  });
}, gr = (e, t = me(e.content), n = Date.now(), r = {}) => {
  const i = Math.max(500, r.maxChunkChars ?? ur), s = Math.min(Math.max(0, r.chunkOverlapChars ?? mr), Math.floor(i / 2)), c = Math.max(1, r.maxChunksPerFile ?? fr), l = kr(e.content), u = [];
  let d = 0, f = 0;
  for (; d < e.content.length && u.length < c; ) {
    const g = yr(e.content, d, Math.min(e.content.length, d + i)), b = e.content.slice(d, g).trim();
    if (b.length > 0) {
      const P = vr(l, d, g);
      u.push({
        id: `${e.id}:${t}:${f}:${P.startLineNumber}-${P.endLineNumber}`,
        fileId: e.id,
        path: e.path,
        name: e.name,
        language: e.language,
...
` && t.push(n + 1);
  ...
  const r = Bt(e, t), i = Bt(e, Math.max(t, n - 1));
  return {
    startLineNumber: r + 1,
    startColumn: t - e[r] + 1,
    endLineNumber: i + 1,
  ...
  return 0;
}, xr = (e) => e.split(/[\\/]+/).filter(Boolean).some((t) => pr.has(t)), Cr = (e) => {
...
class Nr {
  ...
  options;
  constructor(t = [], n = {}) {
    this.options = { ...n }, this.snapshot = Dt(t, this.options);
  }
  getSnapshot() {
  ...
  refresh(t, n = this.options) {
    return this.options = { ...n }, this.snapshot = Dt(t, this.options), this.snapshot;
  }
  update(t, n = this.options) {
    return this.refresh(t, n);
  }
  staleReport(t) {
    return Ar(this.snapshot, t, this.options);
  }
  isStale(t) {
    return this.staleReport(t).stale;
  }
}
const Ae = (e = [], t = {}) => new Nr(e, t), Ar = (e, t, n = {}) => {
  const r = new Map(t.map((l) => [l.id, l])), i = e.byFileId, s = [];
  for (const l of e.files) {
    const u = r.get(l.fileId);
    if (!u) {
      s.push({ fileId: l.fileId, path: l.path, status: "deleted", indexedHash: l.hash });
      continue;
    }
    if (ut(u, n)) {
      s.push({ fileId: l.fileId, path: u.path, status: "skipped-now", indexedHash: l.hash, currentHash: me(u.content) });
      continue;
    }
    const f = me(u.content);
    f !== l.hash ? s.push({ fileId: l.fileId, path: u.path, status: "changed", indexedHash: l.hash, currentHash: f }) : u.dirty !== l.dirty && s.push({ fileId: l.fileId, path: u.path, status: "dirty-state-changed", indexedHash: l.hash, currentHash: f });
  }
  for (const l of t)
    !i.has(l.id) && !ut(l, n) && s.push({ fileId: l.id, path: l.path, status: "new", currentHash: me(l.content) });
  const c = e.signature !== sn(t);
  return { stale: s.length > 0, signatureChanged: c, files: s };
}, $r = 8, Sr = 12e3, Ir = (e, t, n = {}, r) => {
  const i = Lr(t);
  if (i.length === 0) return [];
  const s = new Set(r?.files.map((f) => f.fileId) ?? []), c = e.chunks.filter((f) => n.includeDirty !== !1 || !f.dirty).map((f) => Er(f, i)).filter((f) => f.score >= (n.minScore ?? 1)).sort((f, g) => g.score - f.score || f.chunk.path.localeCompare(g.chunk.path) || f.chunk.ordinal - g.chunk.ordinal), l = [], u = /* @__PURE__ */ new Set();
  let d = n.maxChars ?? Sr;
  for (const f of c) {
    if (l.length >= (n.limit ?? $r) || d <= 0) break;
...
[...context clipped...]` : f.chunk.text;
    l.push(Pr(f, b, s.has(f.chunk.fileId))), d -= b.length;
  }
  return l;
}, Pr = (e, t = e.chunk.text, n = !1) => {
...
`), Lr = (e) => Array.from(new Set(
  ...
  const l = [];
  for (const u of t) {
    const d = it(n, u), f = it(r, u), g = it(i, u), b = s === u ? 1 : 0;
    d + f + g + b !== 0 && (l.push(u), c += Math.min(d, 12), c += f * 4, c += g * 6, c += b * 3, (n.includes(`function ${u}`) || n.includes(`const ${u}`) || n.includes(`class ${u}`) || n.includes(`type ${u}`)) && (c += 3));
  }
  return c += Math.max(0, 2 - e.ordinal * 0.05), { chunk: e, score: c, matchedTerms: l };
}, it = (e, t) => {
  ...
  const n = e.toLowerCase(), r = t.map((u) => n.indexOf(u)).filter((u) => u >= 0).sort((u, d) => u - d)[0] ?? 0, i = Math.max(0, r - 120), s = Math.min(e.length, r + 260), c = i > 0 ? "…" : "", l = s < e.length ? "…" : "";
  return `${c}${e.slice(i, s).replace(/\s+/g, " ").trim()}${l}`;
}, Tr = (e, t = {}) => {
  const { autoRefresh: n = !1, ...r } = t, i = ze(e), s = ze(null);
  s.current || (s.current = Ae(e, r));
  const [c, l] = C(() => s.current?.getSnapshot() ?? Ae(e, r).getSnapshot());
  i.current = e;
  const u = v((b = i.current) => {
    const P = (s.current ?? Ae([], r)).refresh(b, r);
    return l(P), P;
  }, [r.maxFileBytes, r.maxChunkChars, r.chunkOverlapChars, r.maxChunksPerFile, r.includeDirty]), d = v((b = i.current) => {
    const P = (s.current ?? Ae([], r)).update(b, r);
    return l(P), P;
  }, [u]);
  U(() => {
    n && u(e);
  }, [n, e, u]);
  const f = (s.current ?? Ae([], r)).staleReport(e), g = v((b, P = {}) => {
    const M = s.current ?? Ae([], r);
    return Ir(M.getSnapshot(), b, P, M.staleReport(i.current));
  }, []);
  return { snapshot: c, staleReport: f, isStale: f.stale, refresh: u, update: d, retrieve: g };
}, Or = /```([^\n`]*)\n([\s\S]*?)```/g, zr = /^@@\s+-(\d+)(?:,\d+)?\s+\+(\d+)(?:,\d+)?\s+@@/;
function kt(e) {
  return Array.from(e.matchAll(Or)).map((t) => cn(t[1] ?? "", Br(t[2] ?? ""))).filter((t) => t.content.trim().length > 0);
}
function cn(e, t) {
  const n = e.trim().split(/[\s,]+/).filter(Boolean), r = {}, i = [];
  for (const s of n) {
    const c = s.match(/^([a-z0-9_-]+)=(.+)$/i);
    c ? r[c[1].toLowerCase()] = Wr(c[2]) : i.push(s.toLowerCase());
  }
  return { lang: e.trim().toLowerCase(), flags: i, attrs: r, content: t };
}
function G(e, t) {
  return (typeof e == "string" ? cn(e, "").flags : e.flags).includes(t.toLowerCase());
}
function ln(e) {
  return [...kt(e).filter((r) => G(r, "diff") || G(r, "patch")).map((r) => r.content), e].filter((r, i, s) => _r(r) && s.findIndex((c) => c === r) === i);
}
function _r(e) {
...
`), r = [];
  if (n.forEach((c, l) => {
    (c.startsWith("diff --git ") || c.startsWith("--- ")) && r.push(l);
  }), r.length === 0) return [{ raw: t, ...ot(t) }];
  const i = Array.from(new Set(r)).sort((c, l) => c - l), s = [];
  for (let c = 0; c < i.length; c += 1) {
    const l = i[c], d = i.find((g) => g > l && n[g].startsWith("diff --git ")) ?? n.length, f = n.slice(l, d).join(`
`);
    /^@@\s+-\d+/m.test(f) && s.push({ raw: f, ...ot(f) });
  }
  return s.length > 0 ? s : [{ raw: t, ...ot(t) }];
}
function ot(e) {
  const t = e.match(/^diff --git\s+(?:"?a\/(.+?)"?|(\S+))\s+(?:"?b\/(.+?)"?|(\S+))/m), n = e.match(/^---\s+(?:"?a\/(.+?)"?|(\S+))/m), r = e.match(/^\+\+\+\s+(?:"?b\/(.+?)"?|(\S+))/m), i = ne(t?.[1] || t?.[2] || n?.[1] || n?.[2] || "") || null, s = ne(t?.[3] || t?.[4] || r?.[1] || r?.[2] || "") || null, c = s && s !== "/dev/null" ? s : i;
  return { path: c && c !== "/dev/null" ? c : null, oldPath: i, newPath: s };
}
function ne(e) {
  return e.trim().replace(/^['"]|['"]$/g, "").replace(/\\/g, "/").replace(/^[ab]\//, "").replace(/^\.\//, "");
}
function mt(e) {
  return kt(e).filter((t) => jr(t)).map((t) => ({
    content: Hr(t.content),
...
function jr(e) {
  return G(e, "atomek-replace") || G(e, "atomek-full") || G(e, "full-replacement") || G(e, "full") || G(e, "replace");
}
function Dr(e) {
...
`), i = [];
  ...
  for (let l = 0; l < r.length; l += 1) {
    const u = r[l].match(zr);
    if (!u) continue;
    c = !0;
    const d = Number(u[1]), f = Math.max(0, d - 1);
    if (f < s) return null;
    for (i.push(...n.slice(s, f)), s = f, l += 1; l < r.length; l += 1) {
      const g = r[l];
      if (g.startsWith("@@ ")) {
        l -= 1;
        break;
      ...
      const b = g[0], P = g.slice(1);
      if (b === " ") {
        if (n[s] !== P) return null;
        i.push(P), s += 1;
        continue;
      }
      if (b === "-") {
  ...
  return c ? (i.push(...n.slice(s)), i.join(`
`)) : null;
}
function Br(e) {
  return e.replace(/^\n+/, "").replace(/\n+$/, "");
...
function Vr(e) {
  const { body: t, files: n, sourceTitle: r, activeFile: i = null, versions: s = {} } = e, c = /* @__PURE__ */ new Map(), l = [];
  for (const f of ln(t))
    for (const g of Rr(f)) {
      if (!g.path) {
        if (i) {
          const M = ft(i.content, g.raw);
          M && M !== i.content && c.set(i.id, He({
            file: i,
            proposedContent: M,
            sourceTitle: r,
      ...
      const P = ft(b.file.content, g.raw);
      if (!P || P === b.file.content) {
  ...
  for (const f of mt(t)) {
    const g = f.path ? Ht(n, f.path) : i ? { file: i, normalizedPatchPath: ne(i.path), confidence: "exact" } : null;
    if (!g) {
  ...
  const u = mt(t).some((f) => f.path);
  if (c.size === 0 && i && !u) {
    const f = Ur(t, i);
    f && f.content !== i.content && c.set(i.id, He({
      file: i,
      proposedContent: f.content,
      sourceTitle: r,
      extractionLabel: f.label,
...
`), i = Math.max(n.length, r.length);
  let s = 0, c = 0, l = 0;
  for (let u = 0; u < i; u += 1)
    n[u] !== r[u] && (n[u] === void 0 ? s += 1 : r[u] === void 0 ? c += 1 : l += 1);
  return { added: s, removed: c, changed: l };
}
function Kr(e) {
  return e.reduce((t, n) => ({
...
function He(e) {
  ...
  return {
    ...
    stats: qr(e.file.content, e.proposedContent),
    base: {
...
const qe = (e) => typeof e == "object" && e !== null, Jr = (e) => qe(e.ai) ? e.ai : null, Yr = (e) => {
  if (!qe(e)) return null;
  const t = e.id;
  return typeof t == "string" && t.trim() ? t : null;
}, Qr = (e) => {
  const t = Yr(e);
  return !t || !qe(e) ? null : { ...e, id: t };
}, Z = (e) => typeof e == "string" ? /\bembeddings?\b|text-embedding/i.test(e) : Array.isArray(e) ? e.some(Z) : qe(e) ? Object.values(e).some(Z) : !1, Zr = async (e, t) => {
  const n = Jr(e);
  if (typeof n?.listModels != "function") return [];
  const r = await n.listModels(t);
  return Array.isArray(r) ? r.map(Qr).filter((i) => !!i) : [];
}, ei = (e) => (e.embedding ?? e.embeddings ?? e.supportsEmbedding ?? e.supportsEmbeddings) === !0 ? !0 : Z(e.capability) || Z(e.capabilities) || Z(e.modality) || Z(e.modalities) || Z(e.task) || Z(e.tasks) || Z(e.type) || Z(e.kind), hn = (e) => typeof e == "object" && e !== null, ti = (e) => hn(e.ai) ? e.ai : null, un = (e) => typeof ti(e)?.embedText == "function", ni = async (e, t) => un(e) ? (await Zr(e, t)).filter(ei) : [], ai = (e) => hn(e.ai) ? un(e) ? null : "host.ai.embedText is not exposed by this Tytus build." : "host.ai is not available in this Tytus build.", ri = Vn(() => import("./WorkbenchMonacoEditor-DbnEHW95.js").then((e) => e.W).then((e) => ({ default: e.WorkbenchMonacoEditor }))), ii = {
  id: "welcome",
  name: "Welcome",
  path: "Welcome",
  language: "text",
  ...
  dirty: !1,
  source: "sample"
}, mn = "tytus.workspace.recent", fn = "tytus.workspace.layout", pn = "tytus.atomek.chatAiSettings", We = {
  gatewayPreference: "auto",
  model: "",
  embeddingModel: ""
};
function oi({ host: e }) {
  const t = V(() => Fi(), []), [n, r] = C("explorer"), [i, s] = C(t.primaryVisible), [c, l] = C(t.primaryWidth), [u, d] = C("chat"), [f, g] = C(t.secondaryVisible), [b, P] = C(t.secondaryWidth), [M, z] = C(!1), [ee, fe] = C("problems"), [re, q] = C(t.markdownPreviewVisible), [pe, J] = C(!1), [te, Ke] = C(null), [x, O] = C([]), [be, ge] = C([]), [_, ie] = C(null), [$e, Xe] = C(""), [Se, _e] = C({ lineNumber: 1, column: 1 }), [Re, y] = C(null), [w, $] = C({}), [I, W] = C(Za), [E, Y] = C([]), [oe, se] = C([]), [we, ye] = C(null), [ke, T] = C(""), [F, B] = C(!1), [K, yn] = C(""), [xt, Ct] = C([]), [j, ve] = C(null), [ce, Ie] = C(null), [kn, Ge] = C(!1), [Pe, vn] = C(() => Ti()), [Nt, Me] = C(null), [xn, At] = C(null), [Cn, A] = C("Ready"), [Le, Nn] = C(() => Ei()), X = be.map((o) => x.find((m) => m.id === o)).filter(Boolean), N = _ ? x.find((o) => o.id === _) ?? null : null, $t = V(() => Qa({ files: x, openEditorIds: be, activeFileId: _, versions: w, activeSelection: Re }), [_, Re, w, x, be]), St = V(() => ({
    scope: I,
    removedAttachmentIds: E,
    selectedFileIds: []
  }), [I, E]), Ee = V(() => dr($t, x, St), [St, $t, x]), de = Tr(x, { autoRefresh: !0, includeDirty: !0 }), It = V(() => oe.map((o) => ({
    id: o.id,
    kind: "index-hit",
    label: o.label,
    path: o.path,
    ...
    removable: !0,
    implicit: !1
  })), [oe]), An = V(() => [...Ee.attachments, ...It], [Ee.attachments, It]), S = Ga({ host: e, requestContext: Ee.parts, chatSettings: Pe, setStatus: A }), he = V(
    () => [...S.artifacts, ...xt].sort((o, m) => m.createdAt - o.createdAt),
    [S.artifacts, xt]
  ), Je = !N && !pe, H = V(() => x.filter((o) => o.dirty), [x]), $n = V(() => {
    const o = $e.trim().toLowerCase();
    return o ? x.filter((m) => m.path.toLowerCase().includes(o)) : x;
  }, [x, $e]);
  U(() => {
    $((o) => {
      const m = {};
      for (const p of x) m[p.id] = o[p.id] ?? 1;
      return m;
    });
  }, [x]), U(() => {
    Y([]), se([]);
  ...
  const Sn = v((o) => {
    if (o.kind === "index-hit") {
      se((m) => m.filter((p) => p.id !== o.id)), A(`Removed project context: ${o.label}`);
      return;
    }
    Y((m) => m.includes(o.id) ? m : [...m, o.id]), A(`Removed chat context: ${o.label}`);
  }, []), xe = v((o) => {
    $((m) => ({ ...m, [o]: (m[o] ?? 1) + 1 }));
  }, []), In = v((o) => {
    o.preventDefault();
    const m = o.clientX, p = b, k = (Ce) => {
      const Te = p + (m - Ce.clientX);
      P(Math.max(380, Math.min(760, Te)));
    }, R = () => {
    ...
    o.preventDefault();
    const m = o.clientX, p = c, k = (Ce) => {
      const Te = p + (Ce.clientX - m);
    ...
    const m = [o, ...Le.filter((p) => p.path !== o.path)].slice(0, 6);
    Nn(m), localStorage.setItem(mn, JSON.stringify(m));
  }, [Le]), D = v((o, m) => {
    ge((p) => p.includes(o.id) ? p : [...p, o.id]), ie(o.id), J(!1), At(m ?? null), _e({ lineNumber: m ?? 1, column: 1 });
  }, []), Mn = v((o) => {
    if (!o.fileId) return;
    const m = x.find((p) => p.id === o.fileId);
    m && (D(m, o.range?.startLineNumber ?? 1), A(`Revealed context: ${o.label}`));
  }, [x, D]), Fe = v(async () => {
    if (st(H, "open new files"))
      try {
        const o = await ja();
        if (o.length === 0) return;
        O((m) => Pi(m, o)), o.forEach((m) => je({ name: m.name, path: m.path, at: Date.now() })), D(o[0]), A(`Opened ${o.length} local file${o.length === 1 ? "" : "s"}`);
      } catch (o) {
        o.name !== "AbortError" && A(`Open file failed: ${o.message}`);
      }
  }, [H, D, je]), Ye = v(async () => {
    if (st(H, "open another folder"))
      try {
        const o = await Da();
        Ke(o), O(o.files), ge([]), ie(null), J(!1), je({ name: o.name, path: o.name, at: Date.now() }), A(`${o.handle ? "Opened local folder" : "Opened browser fallback folder"} ${o.name} (${o.files.length} text files indexed)`);
      } catch (o) {
    ...
    if (!_) return;
    let m = !1;
    O((p) => p.map((k) => k.id !== _ || k.content === o ? k : (m = !0, { ...k, content: o, dirty: !0 }))), m && xe(_);
  }, [_, xe]), Qe = v(async () => {
    if (N)
      try {
        const o = await rt(N);
        O((m) => m.map((p) => p.id === o.id ? o : p)), A(`Saved ${o.name}`);
      } catch (o) {
        A(`Save failed: ${o.message}`);
      }
  }, [N]), En = v(async (o) => {
    const m = x.find((k) => k.id === o);
    if (!m) return null;
    const p = await rt(m);
    return O((k) => k.map((R) => R.id === p.id ? p : R)), p;
  }, [x]), Pt = v(async () => {
    const o = x.filter((m) => m.dirty);
    if (o.length === 0) {
      A("No dirty files to save");
      return;
    }
    try {
      const m = await Promise.all(o.map((k) => rt(k))), p = new Map(m.map((k) => [k.id, k]));
      O((k) => k.map((R) => p.get(R.id) ?? R)), Me(null), A(`Saved ${m.length} dirty file${m.length === 1 ? "" : "s"}`);
    } catch (m) {
      A(`Save all failed: ${m.message}`);
    }
  }, [x]), Ze = v((o) => {
    const m = x.find((p) => p.id === o);
    if (m?.dirty && !window.confirm(`${m.name} has unsaved changes. Close without saving?`)) {
      A(`Close canceled — ${m.name} has unsaved changes`);
      return;
    }
    ge((p) => {
      const k = p.filter((R) => R !== o);
      return _ === o && (ie(k.at(-1) ?? null), y(null)), k;
    });
  }, [_, x]), Fn = v(() => {
    st(H, "close all editors") && (ge([]), ie(null), At(null), J(!1), A("Closed all editors"));
  }, [H]), et = v(() => {
    const o = x.filter((p) => p.name.startsWith("Untitled")).length + 1, m = {
...
`,
    ...
    const m = [...Ee.parts];
    if (I !== "indexed-project")
      return se([]), m;
    const p = de.retrieve(o, { limit: 8, maxChars: 12e3, includeDirty: !0 });
    return se(p), p.length === 0 ? (A(de.snapshot.chunks.length === 0 ? "Project index is empty — open a folder with readable files first" : "Project index found no matching context for this prompt"), m) : [
      ...m,
      ...p.map((k) => ({
        kind: "workspace",
        title: `Indexed project context — ${k.label}`,
        text: Mr(k)
      }))
    ];
  }, [Ee.parts, I, de]), tt = v(() => {
    ...
    const m = [
      `Create a polished Markdown artifact from ${N?.path ?? `${X.length} open editors`}.`,
    ...
    g(!0), d("chat"), A("Asking Atomek to synthesize an AI artifact…"), S.askAgent(m, { requestContext: le(m) }).then((p) => {
      !p || p.status === "error" || S.createArtifact({
        messageId: p.id,
        title: `AI synthesis — ${N?.name ?? "open workspace"}`,
        kind: "markdown",
        body: p.body
      }).then(() => {
        g(!0), d("outputs"), z(!0), fe("output");
      });
    });
  }, [N, S, le, X.length]), nt = v((o) => {
    const m = x.find((p) => p.path === o.path || p.name === o.name);
    if (m) {
      D(m), A(`Opened recent ${m.name}`);
      return;
    }
    A("Browser security requires permission again — use Open File or Open Folder to reopen local content.");
  }, [x, D]), Q = v((o) => {
    g(!0), d("chat"), T(""), S.askAgent(o, { requestContext: le(o) });
  }, [S, le]), Tn = v((o) => {
    const m = S.messages.findIndex((k) => k.id === o.id), p = S.messages.slice(0, m < 0 ? void 0 : m).reverse().find((k) => k.role === "user");
    if (!p?.body.trim()) {
      A("No previous user prompt to regenerate from");
      return;
    }
    S.askAgent(p.body, { requestContext: le(p.body) });
  }, [S, le]), On = v((o) => {
    S.createArtifact({
      messageId: o.id,
      title: o.body.split(`
...
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek memory",
    ...
    const m = lt(x, ct(o.title || o.kind)), p = {
    ...
    O((k) => [...k, p]), D(p), A(`Opened ${o.title} as editable file`);
  }, [x, D]), De = v((o, m) => {
    const p = Vr({
      body: m,
      files: x,
      ...
      edits: p.edits.map(Mi),
      skipped: p.skipped
    };
    return p.edits.length > 1 ? (Ie(k), ve(null), A(`Previewing AI workspace patch for ${p.edits.length} files`), !0) : p.edits.length === 1 ? (ve(k.edits[0]), Ie(null), A(`Previewing AI patch for ${k.edits[0].fileName}`), !0) : (A(p.skipped.length > 0 ? `No applicable edit found. ${p.skipped.slice(0, 2).join(" · ")}` : "No fenced replacement block or applicable unified diff found. Ask Atomek for an edit again."), !1);
  }, [N, w, x]), Rn = v(() => {
    const o = ke.trim();
    if (!o) return;
    T("");
    const m = xi(o);
    S.askAgent(o, { requestContext: le(o) }).then((p) => {
      if (!m || !p || p.status === "error") return;
      De(p.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek edit", p.body) || A("Edit request answered without a patch. Use Generate patch / Edit to request an applicable diff.");
    ...
    if (!j) return;
    const o = x.find((m) => m.id === j.fileId);
    if (!o) {
      A(`Cannot apply edit — ${j.fileName} is no longer open`), ve(null);
      return;
    }
    o.content !== j.originalContent && !window.confirm(`${j.fileName} changed after the preview was created. Apply the AI edit anyway?`) || (O((m) => m.map((p) => p.id === j.fileId ? { ...p, content: j.proposedContent, dirty: !0 } : p)), xe(j.fileId), Me(`AI edit applied to ${o.name}. Save All to persist it to disk.`), ve(null), A(`Applied AI edit to ${o.name} — unsaved`));
  }, [xe, x, j]), Bn = v(() => {
    if (!j) return;
    const o = lt(x, `${ct(j.fileName)}-ai-edit`), m = {
    ...
    O((p) => [...p, m]), D(m), ve(null), A(`Opened proposed edit as ${o}`);
  }, [N?.language, x, D, j]), Hn = v(() => {
    if (!ce) return;
    const o = new Map(ce.edits.map((p) => [p.fileId, p]));
    x.some((p) => {
      const k = o.get(p.id);
      return k && p.content !== k.originalContent;
    }) && !window.confirm("One or more files changed after the workspace patch preview was created. Apply anyway?") || (O((p) => p.map((k) => {
      const R = o.get(k.id);
      return R ? { ...k, content: R.proposedContent, dirty: !0 } : k;
    })), o.forEach((p, k) => xe(k)), Me(`AI workspace patch applied to ${o.size} file${o.size === 1 ? "" : "s"}. Save All to persist changes.`), Ie(null), A(`Applied AI workspace patch to ${o.size} file${o.size === 1 ? "" : "s"} — unsaved`));
  }, [xe, x, ce]), Wn = v(() => {
    if (!ce) return;
    const o = [...x], m = ce.edits.map((p) => {
      const k = lt(o, `${ct(p.fileName)}-ai-edit`), R = x.find((Te) => Te.id === p.fileId), Ce = {
    ...
    O((p) => [...p, ...m]), m[0] && D(m[0]), Ie(null), A(`Opened ${m.length} proposed edit file${m.length === 1 ? "" : "s"}`);
  }, [x, D, ce]), Mt = v((o) => {
    if (o === "explain") {
      Q("Explain the active file. Focus on purpose, structure, risks, and next useful edits.");
      return;
    }
    if (o === "improve") {
      Q("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful.");
      return;
    }
    if (o === "plan") {
      Q("Create an implementation plan from the open editor context. Be specific, ordered, and call out blockers.");
      return;
    }
    if (o === "edit") {
      Q("Edit the active file or open workspace files. Prefer one git-style unified diff in a fenced diff block, with paths matching opened files. If editing one file, a complete fenced replacement is also OK. Do not use provider-specific tools or model assumptions.");
      return;
    }
    Q("Draft a concrete Markdown artifact from the open editor context. Make it ready to save as an output.");
  }, [Q]);
  return U(() => {
    const o = (m) => {
      (m.metaKey || m.ctrlKey) && (m.key.toLowerCase() === "o" && (m.preventDefault(), Fe()), m.key.toLowerCase() === "s" && (m.preventDefault(), Qe()), m.key.toLowerCase() === "w" && (m.preventDefault(), _ && Ze(_)), m.key.toLowerCase() === "b" && (m.preventDefault(), s((k) => !k)), m.shiftKey && m.key.toLowerCase() === "f" && (m.preventDefault(), r("search"), s(!0)), (m.key.toLowerCase() === "k" || m.key.toLowerCase() === "p") && (m.preventDefault(), B(!0)));
    };
    return window.addEventListener("keydown", o), () => window.removeEventListener("keydown", o);
  }, [_, Ze, Fe, Qe]), U(() => {
    const o = (m) => {
      H.length !== 0 && (m.preventDefault(), m.returnValue = "");
    };
    return window.addEventListener("beforeunload", o), () => window.removeEventListener("beforeunload", o);
  }, [H.length]), U(() => {
    H.length === 0 && Me(null);
  }, [H.length]), U(() => {
    const o = { primaryVisible: i, primaryWidth: c, secondaryVisible: f, secondaryWidth: b, markdownPreviewVisible: re };
    localStorage.setItem(fn, JSON.stringify(o));
  }, [re, i, c, f, b]), U(() => {
    localStorage.setItem(pn, JSON.stringify(Pe));
  }, [Pe]), /* @__PURE__ */ h(
    "div",
    {
      className: `workbench-workbench ${i ? "" : "no-primary"} ${f ? "" : "no-secondary"} ${M ? "has-bottom-panel" : ""}`,
      "data-app": "workbench-vscode-base",
      style: { "--workbench-primary-width": `${c}px`, "--workbench-secondary-width": `${b}px` },
      children: [
        /* @__PURE__ */ a(si, { active: n, setActive: (o) => {
          r(o), s(!0);
        }, openSettings: () => Ge(!0) }),
        i && /* @__PURE__ */ h("div", { className: "workbench-primary-region", children: [
          ...
          /* @__PURE__ */ a("div", { className: "workbench-primary-resizer", onPointerDown: Pn, title: "Resize Explorer" })
        ] }),
        /* @__PURE__ */ h("main", { className: "workbench-editor-area", children: [
          /* @__PURE__ */ a("button", { className: "workbench-command-center", onClick: () => B(!0), children: "Workspace" }),
          /* @__PURE__ */ h("section", { className: "workbench-editor-stack", children: [
            ...
            /* @__PURE__ */ a(hi, { file: N, folder: te, showWelcome: Je }),
            Nt && H.length > 0 ? /* @__PURE__ */ h("div", { className: "workbench-ai-dirty-banner", children: [
              /* @__PURE__ */ a("span", { children: Nt }),
              /* @__PURE__ */ a("button", { onClick: () => {
                Pt();
              }, children: "Save all" }),
              /* @__PURE__ */ a("button", { onClick: () => Me(null), title: "Dismiss", children: /* @__PURE__ */ a(ae, { size: 13 }) })
            ] }) : null,
            /* @__PURE__ */ a("div", { className: "workbench-editor-content", children: N ? /* @__PURE__ */ h("div", { className: N.language === "markdown" && re ? "workbench-editor-split" : "workbench-editor-single", children: [
              /* @__PURE__ */ a("div", { className: "workbench-editor-pane", children: /* @__PURE__ */ a(Un, { fallback: /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: "Loading editor…" }), children: /* @__PURE__ */ a(
                ri,
                {
                  file: N,
                  revealLine: xn,
              ...
              N.language === "markdown" && re && /* @__PURE__ */ a(gi, { content: N.content })
            ] }) : Je ? /* @__PURE__ */ a(mi, { openFile: Fe, openFolder: Ye, newFile: et, recent: Le, reopenRecent: nt, setStatus: A }) : /* @__PURE__ */ h("div", { className: "workbench-no-editor", children: [
              /* @__PURE__ */ a(Ue, { size: 34 }),
              /* @__PURE__ */ a("p", { children: "No editor open" }),
              /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: () => J(!1), children: "Show Welcome" })
            ] }) }),
            M && /* @__PURE__ */ a(
              wi,
              {
        ...
        f && /* @__PURE__ */ a(
          yi,
          {
            ...
            chatThread: S.thread,
            chatThreads: S.threads,
            askAgent: Rn,
            stopChat: S.stopChat,
            regenerateMessage: Tn,
            newChat: () => {
              S.newChat();
            },
            selectThread: (o) => {
            ...
            saveMessageAsArtifact: On,
            rememberMessage: zn,
            previewEditFromMessage: (o) => De(o.body.split(`
`).find(Boolean)?.replace(/^#+\s*/, "").slice(0, 80) || "Atomek answer", o.body),
            runQuickPrompt: Mt,
            pendingPatchPrompt: we,
            generatePatchPrompt: jn,
            workspaceFileCount: x.length,
            aiStatus: S.aiStatus,
            chatSettings: Pe,
            openSettings: () => Ge(!0),
            busy: S.busy,
            memoryHitCount: S.memoryHits.length,
            outputs: he,
            runAiSynthesis: tt,
            openOutputAsFile: at,
            previewEditFromOutput: (o) => De(o.title, o.body),
            ...
            contextScope: I,
            setContextScope: W,
            contextAttachments: An,
            removeContextAttachment: Sn,
            revealContextAttachment: Mn,
            projectIndexSummary: `${de.snapshot.files.length} files · ${de.snapshot.chunks.length} chunks`,
            projectIndexStale: de.isStale,
            refreshProjectIndex: () => {
              const o = de.refresh(x);
        ...
        F && /* @__PURE__ */ a(
          bi,
          {
            ...
            activeFile: N,
            commands: [
              { label: "File: New File", detail: "Create an untitled Markdown file", run: et },
              ...
              ...Le.map((o) => ({ label: `File: Open Recent — ${o.name}`, detail: o.path, run: () => nt(o) })),
              { label: "File: Save All", detail: `${H.length} dirty file${H.length === 1 ? "" : "s"}`, run: () => {
                Pt();
              }, disabled: H.length === 0 },
              { label: "File: Close All Editors", detail: `${X.length} open editor${X.length === 1 ? "" : "s"}`, run: Fn, disabled: X.length === 0 },
              ...
              { label: "View: Toggle Primary Side Bar", detail: i ? "Hide Explorer side bar" : "Show Explorer side bar", run: () => s((o) => !o) },
              { label: "View: Toggle Chat Panel", detail: f ? "Hide right AI side bar" : "Show right AI side bar", run: () => g((o) => !o) },
              { label: "View: Toggle Bottom Panel", detail: M ? "Hide Problems/Output/Terminal panel" : "Show Problems/Output/Terminal panel", run: () => z((o) => !o) },
              { label: "View: Toggle Markdown Preview", detail: N?.language === "markdown" ? "Show or hide Markdown preview split" : "Available for Markdown files", run: () => q((o) => !o), disabled: N?.language !== "markdown" },
              { label: "Atomek: Create AI Synthesis", detail: N || X.length > 0 ? "Ask AIL to produce a saved Markdown artifact" : "Open a file first", run: tt, disabled: !N && X.length === 0 },
              { label: "AI: Explain Active File", detail: N ? `Ask Cortex to explain ${N.path}` : "Open a file first", run: () => Q("Explain the active file. Focus on purpose, structure, risks, and next useful edits."), disabled: !N },
              { label: "AI: Improve Active File", detail: N ? `Ask Cortex for concrete edits to ${N.path}` : "Open a file first", run: () => Q("Review the active file and propose the smallest concrete improvements. Include exact snippets if useful."), disabled: !N },
              { label: "AI: Draft Editable Replacement", detail: N ? `Ask Cortex for a full-file replacement for ${N.path}` : "Open a file first", run: () => Mt("edit"), disabled: !N },
              { label: "AI: Plan Workspace Work", detail: X.length > 0 ? "Use open editors as bounded context" : "Open files first", run: () => Q("Create an implementation plan from the open editor context. Be specific and sequence the work."), disabled: X.length === 0 },
              { label: "AI: Save Active File as Artifact", detail: N ? "Persist active file in host.ai artifacts" : "Open a file first", run: _n, disabled: !N },
              { label: "AI: Open Latest Artifact as File", detail: he[0] ? `Create editable file from ${he[0].title}` : "No outputs yet", run: () => he[0] && at(he[0]), disabled: he.length === 0 }
            ],
            openWorkbenchFile: D,
            onClose: () => B(!1)
          }
        ...
        /* @__PURE__ */ a(Ii, { status: Cn, file: N ?? ii, cursor: Se, fileCount: x.length, dirtyCount: H.length })
      ]
    }
  );
}
function si({ active: e, setActive: t, openSettings: n }) {
  return /* @__PURE__ */ h("aside", { className: "workbench-activity-bar", "aria-label": "Activity Bar", children: [
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a(Qt, { size: 25 }), label: "Explorer", active: e === "explorer", onClick: () => t("explorer") }),
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a($a, { size: 25 }), label: "Search", active: e === "search", onClick: () => t("search") }),
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a(ba, { size: 25 }), label: "Source Control", active: e === "source-control", onClick: () => t("source-control") }),
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a(ta, { size: 25 }), label: "Run and Debug", active: e === "run", onClick: () => t("run") }),
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a(Xt, { size: 25 }), label: "Extensions", active: e === "extensions", onClick: () => t("extensions") }),
    /* @__PURE__ */ a("div", { className: "workbench-activity-spacer" }),
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a(ra, { size: 23 }), label: "Accounts", active: !1, onClick: () => {
    } }),
    /* @__PURE__ */ a(ue, { icon: /* @__PURE__ */ a(Ma, { size: 23 }), label: "Manage", active: !1, onClick: n })
  ] });
}
function ue({ icon: e, label: t, active: n, onClick: r }) {
  return /* @__PURE__ */ a("button", { className: `workbench-activity-button ${n ? "active" : ""}`, title: t, "aria-label": t, onClick: r, children: e });
}
function ci(e) {
  return e.activity === "search" ? /* @__PURE__ */ a(fi, { files: e.files, query: e.query, setQuery: e.setQuery, openWorkbenchFile: e.openWorkbenchFile, activeFileId: e.activeFileId }) : e.activity === "source-control" ? /* @__PURE__ */ a(Ut, { title: "SOURCE CONTROL", body: "No source control provider registered. Git belongs here, not as a fake demo." }) : e.activity === "run" ? /* @__PURE__ */ a(Ut, { title: "RUN AND DEBUG", body: "Run configurations, terminals, and recipe execution will plug into this surface later." }) : e.activity === "extensions" ? /* @__PURE__ */ a(Si, {}) : /* @__PURE__ */ a(li, { ...e });
}
function li(e) {
  const t = !e.folder, n = V(() => di(e.files, e.folder?.name), [e.files, e.folder?.name]);
      ...
      t ? /* @__PURE__ */ h(Ve, { children: [
        ...
        /* @__PURE__ */ h("div", { className: "workbench-sidebar-actions", children: [
          ...
          /* @__PURE__ */ h("button", { className: "workbench-button-subtle", onClick: e.openFolder, children: [
            /* @__PURE__ */ a(pt, { size: 14 }),
            "Open Folder"
          ] })
        ] }),
        /* @__PURE__ */ a("input", { className: "workbench-input", value: e.query, onChange: (r) => e.setQuery(r.target.value), placeholder: "Search files" }),
        /* @__PURE__ */ h("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(ht, { size: 12 }),
          " Open Editors"
        ] }),
        e.openEditors.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No open editors" }) : e.openEditors.map((r) => /* @__PURE__ */ a(vt, { file: r, active: r.id === e.activeFileId, onOpen: () => e.openWorkbenchFile(r) }, r.id)),
        /* @__PURE__ */ h("div", { className: "workbench-section-title", children: [
          /* @__PURE__ */ a(ht, { size: 12 }),
          " ",
          e.folder?.name ?? "Workspace"
        ] }),
        n.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No readable text files found." }) : bn(n, e.activeFileId, e.openWorkbenchFile)
      ] }),
      /* @__PURE__ */ a("div", { className: "workbench-section-title", children: "Recent" }),
      e.recent.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders yet." }) : e.recent.map((r) => /* @__PURE__ */ h("button", { className: "workbench-tree-row", onClick: () => e.reopenRecent(r), children: [
        /* @__PURE__ */ a(fa, { size: 14 }),
        /* @__PURE__ */ a("span", { className: "workbench-row-name", children: r.name })
      ] }, `${r.path}-${r.at}`))
    ] })
  ] });
}
function vt({ file: e, active: t, onOpen: n, basePath: r, depth: i = 0, label: s }) {
  const c = r && e.path.startsWith(`${r}/`) ? e.path.slice(r.length + 1) : e.path, l = i || Math.max(0, c.split("/").length - 1);
...
function di(e, t) {
  const n = [], r = /* @__PURE__ */ new Map(), i = (c, l, u = n) => {
    const d = r.get(c);
    if (d) return d;
    const f = { name: l, path: c, children: [] };
    return r.set(c, f), u.push(f), f;
  };
  e.forEach((c) => {
    const l = t && c.path.startsWith(`${t}/`) ? c.path.slice(t.length + 1) : c.path, u = l.split("/").filter(Boolean);
    let d = n, f = "";
    u.slice(0, -1).forEach((g) => {
      f = f ? `${f}/${g}` : g, d = i(f, g, d).children;
    }), d.push({ name: u.at(-1) ?? c.name, path: l, file: c, children: [] });
  });
  const s = (c) => c.sort((l, u) => +!!l.file - +!!u.file || l.name.localeCompare(u.name)).map((l) => ({ ...l, children: s(l.children) }));
  return s(n);
}
function bn(e, t, n, r = 0) {
  return e.map((i) => i.file ? /* @__PURE__ */ a(vt, { file: i.file, active: i.file.id === t, onOpen: () => n(i.file), depth: r, label: i.name }, i.file.id) : /* @__PURE__ */ h("div", { children: [
    /* @__PURE__ */ h("div", { className: "workbench-folder-row", style: { "--workbench-depth": r }, children: [
      /* @__PURE__ */ a(ht, { size: 12 }),
      /* @__PURE__ */ a(pt, { size: 14 }),
      /* @__PURE__ */ a("span", { className: "workbench-row-name", children: i.name })
    ] }),
    bn(i.children, t, n, r + 1)
  ] }, i.path));
}
function hi({ file: e, folder: t, showWelcome: n }) {
  const r = n ? ["Welcome"] : e?.path.split("/").filter(Boolean) ?? [], i = t && r[0] === t.name ? r.slice(1) : r;
  return /* @__PURE__ */ a("div", { className: "workbench-breadcrumb", children: i.length === 0 ? /* @__PURE__ */ a("span", { children: "Workspace" }) : i.map((s, c) => /* @__PURE__ */ h("span", { className: "workbench-breadcrumb-part", children: [
...
function ui(e) {
  return /* @__PURE__ */ h("div", { className: "workbench-tabs", children: [
    e.showWelcome && /* @__PURE__ */ h("button", { className: "workbench-tab active", children: [
      /* @__PURE__ */ a(Ue, { size: 15 }),
      /* @__PURE__ */ a("span", { className: "workbench-tab-name", children: "Welcome" }),
      /* @__PURE__ */ a("span", { className: "workbench-tab-close", role: "button", tabIndex: 0, onClick: (t) => {
        t.stopPropagation(), e.closeWelcome();
      }, children: /* @__PURE__ */ a(ae, { size: 13 }) })
    ] }),
    ...
    e.canPreview && /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.previewVisible ? "active" : ""}`, title: "Toggle Markdown Preview", onClick: e.togglePreview, children: /* @__PURE__ */ a(Gt, { size: 16 }) }),
    /* @__PURE__ */ a("button", { className: `workbench-editor-action ${e.secondaryVisible ? "active" : ""}`, title: "Toggle Chat", onClick: e.toggleSecondary, children: /* @__PURE__ */ a(ka, { size: 16 }) })
  ] });
}
function mi({ openFile: e, openFolder: t, newFile: n, recent: r, reopenRecent: i, setStatus: s }) {
  return /* @__PURE__ */ h("div", { className: "workbench-welcome", children: [
    /* @__PURE__ */ h("div", { className: "workbench-welcome-grid", children: [
      /* @__PURE__ */ h("section", { children: [
        /* @__PURE__ */ a("h1", { children: "Atomek" }),
        /* @__PURE__ */ a("div", { className: "workbench-welcome-subtitle", children: "Shape files into working artifacts" }),
        /* @__PURE__ */ a("h2", { children: "Start" }),
        ...
        /* @__PURE__ */ h("button", { className: "workbench-start-link", onClick: t, children: [
          /* @__PURE__ */ a(pt, { size: 18 }),
          "Open Folder..."
        ] }),
        /* @__PURE__ */ a("h2", { children: "Recent" }),
        r.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No recent folders, open a folder to start." }) : r.map((c) => /* @__PURE__ */ h("button", { className: "workbench-start-link", onClick: () => i(c), children: [
          c.name,
          /* @__PURE__ */ a("span", { className: "workbench-muted", children: "~" })
        ] }, `${c.path}-${c.at}`))
      ] }),
      /* @__PURE__ */ h("section", { children: [
        /* @__PURE__ */ a("h2", { children: "Walkthroughs" }),
        /* @__PURE__ */ h("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ a("strong", { children: "Get Started with Atomek" }),
          /* @__PURE__ */ a("span", { className: "workbench-muted", children: "Open local files, edit with Monaco, then wire Tytus agents and Cortex next." })
        ] }),
        /* @__PURE__ */ h("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ a("strong", { children: "Browse & Edit Local Workspaces" }),
          /* @__PURE__ */ a("span", { className: "workbench-muted", children: "Uses browser-native File System Access API on supported Chromium builds." })
        ] }),
        /* @__PURE__ */ h("div", { className: "workbench-walkthrough-card", children: [
          /* @__PURE__ */ a("strong", { children: "Learn the Fundamentals" }),
          /* @__PURE__ */ a("span", { className: "workbench-muted", children: "Explorer, tabs, editor, status bar, chat surface." })
        ] })
      ] })
    ] }),
    /* @__PURE__ */ h("label", { className: "workbench-welcome-checkbox", children: [
      /* @__PURE__ */ a("input", { type: "checkbox", defaultChecked: !0 }),
      " Show welcome page on startup"
    ] })
  ] });
}
function fi({ files: e, query: t, setQuery: n, openWorkbenchFile: r, activeFileId: i }) {
  const s = V(() => pi(e, t), [e, t]), c = V(() => {
    const l = /* @__PURE__ */ new Map();
    return s.forEach((u) => l.set(u.file.id, [...l.get(u.file.id) ?? [], u])), Array.from(l.values()).slice(0, 50);
  }, [s]);
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      ...
      t.trim() ? c.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No matches." }) : c.map((l) => {
        const u = l[0].file;
        return /* @__PURE__ */ h("div", { className: "workbench-search-group", children: [
          /* @__PURE__ */ a(vt, { file: u, active: u.id === i, onOpen: () => r(u) }),
          l.slice(0, 5).map((d) => /* @__PURE__ */ h("button", { className: "workbench-search-hit", onClick: () => r(u, d.lineNumber), title: d.line, children: [
            /* @__PURE__ */ a("span", { className: "workbench-search-line", children: d.lineNumber }),
            /* @__PURE__ */ a("span", { children: d.line })
          ] }, `${u.id}-${d.lineNumber}-${d.line}`)),
...
`).some((s, c) => s.toLowerCase().includes(n) ? (r.push({ file: i, lineNumber: c + 1, line: s.trim() || "(blank line)" }), r.length >= 200) : !1);
  }), r.slice(0, 200);
}
function bi(e) {
  const t = e.query.trim().toLowerCase(), n = e.files.slice(0, 80).map((i) => ({
    label: i.path,
    detail: `Open ${en(i.language)} file`,
  ...
  return U(() => {
  ...
  }, [e.onClose]), /* @__PURE__ */ a("div", { className: "workbench-command-overlay", role: "dialog", "aria-label": "Command Palette", children: /* @__PURE__ */ h("div", { className: "workbench-command-palette", children: [
    /* @__PURE__ */ a(
      "input",
      {
    ...
    /* @__PURE__ */ a("div", { className: "workbench-command-list", children: r.map((i) => /* @__PURE__ */ h(
      "button",
      {
        disabled: i.disabled,
...
function wi(e) {
  return /* @__PURE__ */ h("section", { className: "workbench-bottom-panel", "aria-label": "Panel", children: [
    /* @__PURE__ */ h("div", { className: "workbench-bottom-tabs", children: [
      ...
      /* @__PURE__ */ a("span", {}),
      /* @__PURE__ */ a("button", { title: "Close Panel", onClick: e.onClose, children: /* @__PURE__ */ a(ae, { size: 14 }) })
    ] }),
    /* @__PURE__ */ h("div", { className: "workbench-bottom-body", children: [
      ...
      e.tab === "terminal" && /* @__PURE__ */ h("div", { children: [
        /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Terminal is parked. Native command execution belongs to the pod/daemon integration sprint." }),
        /* @__PURE__ */ a("pre", { className: "workbench-terminal-placeholder", children: "$ atomek --local-first" })
      ] }),
      e.tab === "output" && /* @__PURE__ */ a(wn, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, compact: !0 })
    ] })
  ] });
}
function yi(e) {
  return /* @__PURE__ */ h("aside", { className: "workbench-secondary", children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-secondary-tabs", children: [
      ...
      /* @__PURE__ */ h("div", { className: "workbench-secondary-actions", children: [
        /* @__PURE__ */ a("button", { title: "New Chat", onClick: e.newChat, children: /* @__PURE__ */ a(Ca, { size: 15 }) }),
        /* @__PURE__ */ a("button", { title: "Chat Settings", onClick: e.openSettings, children: /* @__PURE__ */ a(oa, { size: 16 }) }),
        /* @__PURE__ */ a("button", { title: "Close Chat", onClick: e.onClose, children: /* @__PURE__ */ a(ae, { size: 15 }) })
      ] })
    ] }),
    e.tab === "chat" ? /* @__PURE__ */ a(ki, { ...e }) : /* @__PURE__ */ a(wn, { outputs: e.outputs, clearOutputs: e.clearOutputs, deleteArtifact: e.deleteArtifact, runAiSynthesis: e.runAiSynthesis, openOutputAsFile: e.openOutputAsFile, previewEditFromOutput: e.previewEditFromOutput, canPreviewEdit: e.canPreviewEdit })
  ] });
}
function ki(e) {
  const t = ze(null), [n, r] = C(!0), [i, s] = C(!1), c = V(() => e.chatMessages.map((d) => `${d.id}:${d.status ?? ""}:${d.body.length}`).join("|"), [e.chatMessages]);
  U(() => {
    const d = t.current;
    d && (n ? (d.scrollTop = d.scrollHeight, s(!1)) : s(!0));
  }, [n, c]);
  const l = v(() => {
    const d = t.current;
    if (!d) return;
    ...
    const d = t.current;
    d && (d.scrollTop = d.scrollHeight, r(!0), s(!1));
  }, []);
  return /* @__PURE__ */ h("div", { className: "workbench-chat-wrap", children: [
      ...
      /* @__PURE__ */ h(
        "select",
        {
          ...
          children: [
            ...
            e.chatThreads.map((d) => /* @__PURE__ */ h("option", { value: d.id, children: [
              d.title,
              " · ",
              vi(d.lastMessageAt ?? d.updatedAt)
            ] }, d.id))
          ]
        }
      ),
    ...
    /* @__PURE__ */ h("div", { ref: t, className: "workbench-chat-transcript", onScroll: l, children: [
      e.chatMessages.length === 0 ? /* @__PURE__ */ a("div", { className: "workbench-chat-empty", children: /* @__PURE__ */ h("div", { children: [
        /* @__PURE__ */ a(wa, { size: 48 }),
      ...
      ] }) }) : e.chatMessages.map((d) => /* @__PURE__ */ h("div", { className: `workbench-chat-message ${d.role}`, children: [
        /* @__PURE__ */ a("strong", { children: d.role === "user" ? "You" : "Atomek" }),
        d.status === "streaming" ? /* @__PURE__ */ a("em", { children: " streaming" }) : null,
        d.status === "error" ? /* @__PURE__ */ a("em", { children: " error" }) : null,
        /* @__PURE__ */ a("br", {}),
        d.body,
        d.gatewayLabel ? /* @__PURE__ */ h(Ve, { children: [
          /* @__PURE__ */ a("br", {}),
          /* @__PURE__ */ a("small", { children: d.gatewayLabel })
        ] }) : null,
        d.role === "assistant" && d.status !== "streaming" && d.status !== "error" ? /* @__PURE__ */ h("div", { className: "workbench-chat-message-actions", children: [
          /* @__PURE__ */ a("button", { onClick: () => e.saveMessageAsArtifact(d), children: "Save artifact" }),
          /* @__PURE__ */ a("button", { onClick: () => e.rememberMessage(d), children: "Remember" }),
          /* @__PURE__ */ a("button", { onClick: () => e.previewEditFromMessage(d), disabled: e.workspaceFileCount === 0, children: "Preview edit" }),
          /* @__PURE__ */ h("button", { onClick: () => e.regenerateMessage(d), disabled: e.busy, children: [
            /* @__PURE__ */ a(Ft, { size: 11 }),
            " Regenerate"
          ] })
        ] }) : null,
        d.role === "assistant" && d.status === "error" ? /* @__PURE__ */ a("div", { className: "workbench-chat-message-actions", children: /* @__PURE__ */ a("button", { onClick: () => e.regenerateMessage(d), disabled: e.busy, children: "Retry" }) }) : null
      ] }, d.id)),
      i ? /* @__PURE__ */ a("button", { className: "workbench-chat-jump", onClick: u, children: "Jump to latest" }) : null
    ] }),
    /* @__PURE__ */ h("div", { className: "workbench-chat-composer", children: [
      /* @__PURE__ */ h("div", { className: "workbench-chat-tip", children: [
        ...
        /* @__PURE__ */ a("strong", { children: er(e.contextScope) }),
        /* @__PURE__ */ a("em", { children: Wt(e.chatSettings, e.aiStatus.label, e.memoryHitCount) })
      ] }),
      /* @__PURE__ */ h("div", { className: "workbench-chat-box", children: [
        /* @__PURE__ */ h("div", { className: "workbench-chat-attachments", children: [
          ...
          e.contextScope === "indexed-project" ? /* @__PURE__ */ h(Ve, { children: [
          ...
          e.contextAttachments.length === 0 ? /* @__PURE__ */ h("span", { className: "workbench-chat-chip muted", children: [
            ...
            " No file context"
          ] }) : e.contextAttachments.map((d) => /* @__PURE__ */ h("span", { className: "workbench-chat-chip", title: [d.path, d.range ? `${d.range.startLineNumber}:${d.range.startColumn}-${d.range.endLineNumber}:${d.range.endColumn}` : null, d.dirty ? "dirty" : null].filter(Boolean).join(" · "), children: [
            /* @__PURE__ */ a("button", { className: "workbench-chat-chip-open", onClick: () => e.revealContextAttachment(d), disabled: !d.fileId, title: "Reveal context", children: /* @__PURE__ */ a(Et, { size: 13 }) }),
            d.label,
            d.dirty ? /* @__PURE__ */ a("small", { children: "dirty" }) : null,
            d.removable ? /* @__PURE__ */ a("button", { className: "workbench-chat-chip-remove", onClick: () => e.removeContextAttachment(d), title: "Remove context", children: /* @__PURE__ */ a(ae, { size: 11 }) }) : null
          ] }, d.id)),
        ...
        /* @__PURE__ */ h("div", { className: "workbench-chat-toolbar compact", children: [
          /* @__PURE__ */ a("span", { className: "workbench-chat-route-summary", children: Wt(e.chatSettings, e.aiStatus.label, e.memoryHitCount) }),
          /* @__PURE__ */ a("span", {}),
          e.busy ? /* @__PURE__ */ a("button", { className: "workbench-chat-send stop", onClick: e.stopChat, title: "Stop", children: /* @__PURE__ */ a(Ta, { size: 14 }) }) : /* @__PURE__ */ a("button", { className: "workbench-chat-send", onClick: e.askAgent, title: "Send", children: /* @__PURE__ */ a(Ia, { size: 16 }) })
        ] })
      ] })
    ] })
  ] });
...
function Wt(e, t, n) {
  const r = e.model.trim(), s = [e.gatewayPreference === "auto" ? t : gn(e.gatewayPreference)];
  return r && s.push(r), n > 0 && s.push(`${n} memories`), s.join(" · ");
}
function vi(e) {
...
function Ni(e) {
  const [t, n] = C([]), [r, i] = C("Loading gateway models…"), [s, c] = C([]), [l, u] = C("Checking embedding capability…");
  U(() => {
    ...
    return (async () => {
      ...
      try {
        ...
        n(M.map((z) => ({ id: z.id, gatewayLabel: z.gatewayLabel }))), i(M.length > 0 ? `${M.length} models discovered from AIL.` : "No models discovered. You can still enter any AIL alias manually.");
      } catch (M) {
        if (b.signal.aborted) return;
    ...
    return (async () => {
      ...
      u("Loading embedding-capable models from AIL…");
      try {
        const z = await ni(e.host, {
          gatewayPreference: e.chatSettings.gatewayPreference,
          signal: b.signal
        });
        if (b.signal.aborted) return;
        c(z.map((ee) => ({ id: ee.id, gatewayLabel: ee.gatewayLabel ?? ee.source ?? "AIL" }))), u(z.length > 0 ? `${z.length} embedding models discovered from AIL metadata.` : "AIL embedding API is present, but no embedding-capable model metadata was returned.");
      } catch (z) {
        if (b.signal.aborted) return;
        c([]), u(`Embedding model discovery failed: ${z instanceof Error ? z.message : String(z)}`);
      }
    })(), () => b.abort();
  ...
  return /* @__PURE__ */ a("div", { className: "workbench-settings-backdrop", onClick: e.onClose, children: /* @__PURE__ */ h("section", { className: "workbench-settings-dialog", onClick: (b) => b.stopPropagation(), role: "dialog", "aria-modal": "true", "aria-label": "Atomek Settings", children: [
    /* @__PURE__ */ h("header", { className: "workbench-settings-header", children: [
      ...
      /* @__PURE__ */ a("button", { onClick: e.onClose, title: "Close", children: /* @__PURE__ */ a(ae, { size: 15 }) })
    ] }),
    /* @__PURE__ */ h("div", { className: "workbench-settings-body", children: [
      /* @__PURE__ */ h("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ h("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-chat-models", children: t.map((b) => /* @__PURE__ */ a("option", { value: b.id, children: b.gatewayLabel }, `${b.gatewayLabel}:${b.id}`)) })
        ] }),
        /* @__PURE__ */ h("div", { className: "workbench-settings-note", children: [
          "Current request: ",
          gn(e.chatSettings.gatewayPreference),
      ...
      /* @__PURE__ */ h("div", { className: "workbench-settings-section", children: [
        ...
        /* @__PURE__ */ h("label", { className: "workbench-settings-label", children: [
          ...
          /* @__PURE__ */ a("datalist", { id: "atomek-embedding-models", children: s.map((b) => /* @__PURE__ */ a("option", { value: b.id, children: b.gatewayLabel }, `${b.gatewayLabel}:${b.id}`)) })
        ] }),
    ...
    /* @__PURE__ */ h("footer", { className: "workbench-settings-footer", children: [
      /* @__PURE__ */ a("button", { onClick: () => e.onChange(We), children: "Reset" }),
      /* @__PURE__ */ a("button", { onClick: e.onClose, children: "Done" })
    ] })
  ] }) });
}
function wn({ outputs: e, clearOutputs: t, deleteArtifact: n, runAiSynthesis: r, openOutputAsFile: i, previewEditFromOutput: s, canPreviewEdit: c = !1, compact: l = !1 }) {
  return /* @__PURE__ */ h("div", { className: `workbench-panel-list ${l ? "compact" : ""}`, children: [
    /* @__PURE__ */ h("div", { style: { display: "flex", gap: 8, marginBottom: 10 }, children: [
      /* @__PURE__ */ h("button", { className: "workbench-button-subtle", onClick: r, children: [
        /* @__PURE__ */ a(Zn, { size: 14 }),
        "AI synthesis"
      ] }),
      /* @__PURE__ */ a("button", { className: "workbench-button-subtle", onClick: t, children: "Clear" })
    ] }),
    e.length === 0 ? /* @__PURE__ */ a("p", { className: "workbench-muted", children: "No outputs yet. Save an AI answer as an artifact or create an AI synthesis." }) : e.map((u) => /* @__PURE__ */ h("div", { className: "workbench-output-card", children: [
      /* @__PURE__ */ h("div", { className: "workbench-output-head", children: [
        /* @__PURE__ */ a("strong", { children: u.title }),
        /* @__PURE__ */ a("span", { children: u.source === "ai" ? `AI · ${u.kind}` : u.kind }),
        /* @__PURE__ */ a("button", { onClick: () => i(u), children: "Open as file" }),
        s ? /* @__PURE__ */ a(
          "button",
          {
            className: Vt(u.body) ? "workbench-output-edit-cta" : void 0,
            onClick: () => s(u),
            disabled: !c,
            children: Vt(u.body) ? "Preview/apply edit" : "Preview edit"
          }
        ) : null,
        u.source === "ai" ? /* @__PURE__ */ a("button", { onClick: () => n(u.id), children: "Delete" }) : null
      ] }),
      u.body
    ] }, u.id))
  ] });
}
function Ai({ edit: e, onApply: t, onOpenAsFile: n, onClose: r }) {
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI edit", children: /* @__PURE__ */ h("section", { className: "workbench-edit-review", children: [
    /* @__PURE__ */ h("header", { className: "workbench-edit-review-head", children: [
      ...
      /* @__PURE__ */ a("button", { title: "Close", onClick: r, children: /* @__PURE__ */ a(ae, { size: 16 }) })
    ] }),
...
function $i({ patch: e, onApply: t, onOpenAsFiles: n, onClose: r }) {
  const i = e.edits.reduce((s, c) => ({
    added: s.added + c.stats.added,
    removed: s.removed + c.stats.removed,
    changed: s.changed + c.stats.changed
  }), { added: 0, removed: 0, changed: 0 });
  return /* @__PURE__ */ a("div", { className: "workbench-edit-review-overlay", role: "dialog", "aria-label": "Review AI workspace patch", children: /* @__PURE__ */ h("section", { className: "workbench-edit-review workspace", children: [
    /* @__PURE__ */ h("header", { className: "workbench-edit-review-head", children: [
      /* @__PURE__ */ h("div", { children: [
    ...
    /* @__PURE__ */ h("div", { className: "workbench-workspace-patch-list", children: [
      e.edits.map((s) => /* @__PURE__ */ h("article", { className: "workbench-workspace-patch-card", children: [
        ...
        /* @__PURE__ */ a("pre", { children: Li(s.proposedContent) })
      ] }, s.fileId)),
...
function Si() {
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: "EXTENSIONS" }),
    /* @__PURE__ */ h("div", { className: "workbench-sidebar-scroll", children: [
      /* @__PURE__ */ h("div", { className: "workbench-extension-card", children: [
        /* @__PURE__ */ a("strong", { children: "TYTUS EXTENSIONS" }),
        /* @__PURE__ */ a("p", { className: "workbench-muted", children: "Connector surface kept intentionally. Chat, agents, pods, Hyperframes, Blender, and media tools wire in from here." })
      ] }),
      [
        ...
        /* @__PURE__ */ a(Xt, { size: 16 }),
        /* @__PURE__ */ h("div", { children: [
          /* @__PURE__ */ a("strong", { children: t.name }),
          /* @__PURE__ */ a("span", { children: t.detail })
        ] }),
        /* @__PURE__ */ a("em", { children: "coming soon" })
      ] }, t.name))
    ] })
  ] });
}
function Ut({ title: e, body: t }) {
  return /* @__PURE__ */ h("aside", { className: "workbench-sidebar", children: [
    /* @__PURE__ */ a("div", { className: "workbench-sidebar-title", children: e }),
    /* @__PURE__ */ a("div", { className: "workbench-empty-pane", children: t })
  ] });
}
function Ii({ status: e, file: t, cursor: n, fileCount: r, dirtyCount: i }) {
  return /* @__PURE__ */ h("footer", { className: "workbench-statusbar", children: [
    ...
    /* @__PURE__ */ a("span", { children: en(t.language) })
  ] });
}
function st(e, t) {
...
function Ti() {
  try {
    ...
    const t = JSON.parse(e);
    return {
      gatewayPreference: t.gatewayPreference === "remote" || t.gatewayPreference === "local" || t.gatewayPreference === "auto" ? t.gatewayPreference : We.gatewayPreference,
...
function Oi({ host: e }) {
  return /* @__PURE__ */ a(oi, { host: e });
}
function Ri(e) {
  return function() {
    return /* @__PURE__ */ a(Oi, { host: e.host });
  };
}
export {
  Ri as default
